...
public class CmsResourceComparisonDialog extends CmsDialog {
  ...
  /**
   * Visitor that collects the xpath expressions of xml contents.
   *
   * <p>
   */
  class CmsXmlContentTextExtractor implements I_CmsXmlContentValueVisitor {
    ...
    /**
     * Creates a new CmsXmlContentTextExtractor.
     *
     * <p>
     *
      ...
      if (value.isSimpleType()) {
        ...
        m_buffer.append(
            widget.getWidgetStringValue(
                getCms(), new CmsResourceInfoDialog(getJsp()), (I_CmsWidgetParameter) value));
      }
    }
  }
   ...
   */
  public CmsResourceComparisonDialog(
   ...
   */
  public static CmsFile readFile(CmsObject cms, CmsUUID structureId, String version)
      throws CmsException {
  ...
  /**
   * Returns either the historical resource or the offline resource, depending on the version
   * number.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  protected static CmsResource readResource(CmsObject cms, CmsUUID id, String version)
      throws CmsException {
  ...
  public void displayDialog() throws Exception {
    ...
    CmsResourceInfoDialog fileInfo =
    ...
    I_CmsResourceType resourceType =
        OpenCms.getResourceManager().getResourceType(propertyDiff.getResourceType());

    if ((resourceType instanceof CmsResourceTypeXmlContent)
        || (resourceType instanceof CmsResourceTypeXmlPage)) {
      ...
      byte[] content1 =
          readFile(getCms(), propertyDiff.getResource1().getStructureId(), getParamVersion1())
              .getContents();
      byte[] content2 =
          readFile(getCms(), propertyDiff.getResource2().getStructureId(), getParamVersion2())
              .getContents();
      ...
      lists.add(propertyDiff);
      CmsMultiListDialog twoLists =
          new CmsMultiListDialog(lists) {
      ...
      // it is possible that here a forward is tried, what should not be sence we already wrote to
      // the output stream.
      // CmsImageComparisonDialog should implement isForwarded, writeDialog and
      // displayDialog(boolean) methods
      ...
      byte[] content1 =
          readFile(getCms(), resource1.getStructureId(), getParamVersion1()).getContents();
      byte[] content2 =
          readFile(getCms(), resource2.getStructureId(), getParamVersion2()).getContents();
      ...
      if (textExtractor != null) {
        try {
          ...
          || (resourceType instanceof CmsResourceTypeJsp)) {
    ...
    try {
      CmsResource resource1 =
          CmsResourceComparisonDialog.readResource(
              getCms(), new CmsUUID(getParamId1()), getParamVersion1());
      CmsResource resource2 =
          CmsResourceComparisonDialog.readResource(
              getCms(), new CmsUUID(getParamId2()), getParamVersion2());
      ...
      if (CmsResourceComparisonDialog.COMPARE_ATTRIBUTES.equals(getParamCompare())) {
        List<?> comparedAttributes =
            CmsResourceComparison.compareAttributes(getCms(), resource1, resource2);
        ...
        List<?> comparedProperties =
            CmsResourceComparison.compareProperties(
                getCms(), resource1, getParamVersion1(), resource2, getParamVersion2());
  ...
  /**
   * Extracts the content from the files according to the file type.
   *
   * <p>
   *
   ...
   */
  private void setContentAsSource(CmsFile file1, CmsFile file2)
    ...
    if (CmsStringUtil.isNotEmpty(getParamElement())) {
      ...
      I_CmsXmlContentValue value1 =
          resource1.getValue(getParamElement(), CmsLocaleManager.getLocale(getParamLocale()));
      I_CmsXmlContentValue value2 =
          resource2.getValue(getParamElement(), CmsLocaleManager.getLocale(getParamLocale()));
      if (value1 == null) {
      ...
      m_differenceDialog.setCopySource(
          new String(file2.getContents(), cms.getRequestContext().getEncoding()));
    }
  }
}
