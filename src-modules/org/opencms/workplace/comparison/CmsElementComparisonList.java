...
public class CmsElementComparisonList extends A_CmsListDialog {
  ...
  /**
   * flag indicating whether xml contents are compared.
   *
   * <p>
   */
   ...
   */
  public CmsElementComparisonList(
  ...
  public static String formatContentValueForDiffTable(
      CmsObject cms, CmsElementComparison comparison, String origValue) {
    ...
    if (comparison instanceof CmsXmlContentElementComparison) {
      if (((CmsXmlContentElementComparison) comparison)
          .getType()
          .equals(CmsXmlDateTimeValue.TYPE_NAME)) {
  ...
  public void executeListSingleActions() throws IOException, ServletException {
    ...
    params.put(
        PARAM_ELEMENT, new String[] {getSelectedItem().get(LIST_COLUMN_ATTRIBUTE).toString()});
    params.put(PARAM_RESOURCE, new String[] {getParamResource()});
    // forward to the element difference screen
    getToolManager().jspForwardTool(this, "/history/comparison/difference", params);
  }
  ...
  protected List<CmsListItem> getListItems() throws CmsException {
    ...
    CmsFile resource1 =
        CmsResourceComparisonDialog.readFile(
            getCms(), new CmsUUID(getParamId1()), getParamVersion1());
    CmsFile resource2 =
        CmsResourceComparisonDialog.readFile(
            getCms(), new CmsUUID(getParamId2()), getParamVersion2());
    Iterator<CmsElementComparison> diffs =
        new CmsXmlDocumentComparison(getCms(), resource1, resource2).getElements().iterator();
    while (diffs.hasNext()) {
      ...
      String value1 =
          CmsStringUtil.escapeHtml(
              CmsStringUtil.substitute(
                  CmsStringUtil.trimToSize(
                      comparison.getVersion1(), CmsPropertyComparisonList.TRIM_AT_LENGTH),
                  "\n",
                  ""));
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    CmsListDirectAction addedAction =
        new CmsListDirectAction(CmsResourceComparison.TYPE_ADDED) {
    ...
    // add state error action
    CmsListDirectAction removedAction =
        new CmsListDirectAction(CmsResourceComparison.TYPE_REMOVED) {
    ...
    // add state error action
    CmsListDirectAction changedAction =
        new CmsListDirectAction(CmsResourceComparison.TYPE_CHANGED) {
    ...
    // add state error action
    CmsListDirectAction unchangedAction =
        new CmsListDirectAction(CmsResourceComparison.TYPE_UNCHANGED) {
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    eventDetails.setShowActionName(
        Messages.get().container(Messages.GUI_COMPARE_SHOW_ALL_ELEMENTS_0));
    eventDetails.setHideActionName(
        Messages.get().container(Messages.GUI_COMPARE_HIDE_IDENTICAL_ELEMENTS_0));
