...
public class CmsLogin extends CmsJspLoginBean {
  ...
  /**
   * The workplace data cookie name, value stores following information:
  ...
  public static String getCopyrightHtml(Locale locale) {
    ...
    html.append(
        Messages.get().getBundle(locale).key(Messages.GUI_LOGIN_OPENCMS_IS_FREE_SOFTWARE_0));
    html.append("</div>\n");
    html.append("<div style=\"text-align: center; font-size: 10px; white-space: nowrap;\">");
  ...
  /**
   * Returns the direct edit path from the user settings, or <code>null</code> if not set.
   *
   * <p>
   *
   ...
   */
  public static List<CmsOrganizationalUnit> getOrgUnitsForLoginDialog(
      CmsObject cms, String predefOu) {
  ...
  /**
   * Initializes the site and project for a CMS context after login, and returns the workplace
   * settings for the corresponding user.
   *
   * <p>
   *
   ...
   */
  public static CmsMessageContainer validateUserAndPasswordNotEmpty(
  ...
  protected static void setCookie(
      Cookie cookie, boolean delete, HttpServletRequest request, HttpServletResponse response) {
    ...
    // set the path
    cookie.setPath(
        CmsStringUtil.joinPaths(OpenCms.getStaticExportManager().getVfsPrefix(), "/system/login"));
    // set the cookie
    response.addCookie(cookie);
  }

  /**
   * Returns the best matching locale for the given request.
   *
   * <p>
   *
  ...
  private static Locale getLocaleForRequest(HttpServletRequest req) {
    ...
    CmsAcceptLanguageHeaderParser parser =
        new CmsAcceptLanguageHeaderParser(req, OpenCms.getWorkplaceManager().getDefaultLocale());
    List<Locale> acceptedLocales = parser.getAcceptedLocales();
    List<Locale> workplaceLocales = OpenCms.getWorkplaceManager().getLocales();
    Locale locale =
        OpenCms.getLocaleManager().getFirstMatchingLocale(acceptedLocales, workplaceLocales);
    if (locale == null) {
  ...
  public String displayDialog() throws IOException, CmsException {
    ...
    if (cms.getRequestContext().getCurrentUser().isGuestUser()) {
      ...
      // set PC type to "public" as default if not already set by cookie, request or if security
      // option is disabled
      if (m_pcType == null) {
    ...
    m_requestedResource =
        CmsRequestUtil.getNotEmptyParameter(
            getRequest(), CmsWorkplaceManager.PARAM_LOGIN_REQUESTED_RESOURCE);
    ...
    if (Boolean.valueOf(m_actionLogin).booleanValue()) {
      ...
      if (emptyValidation != null) {
        ...
        login(
            (m_oufqn == null ? CmsOrganizationalUnit.SEPARATOR : m_oufqn) + m_username, m_password);
        ...
        if (getLoginException() == null) {
          ...
          if (org.opencms.security.Messages.ERR_LOGIN_FAILED_DISABLED_2
              == loginException.getMessageContainer().getKey()) {
            // the user account is disabled
            m_message = Messages.get().container(Messages.GUI_LOGIN_FAILED_DISABLED_0);
          } else if (org.opencms.security.Messages.ERR_LOGIN_FAILED_TEMP_DISABLED_4
              == loginException.getMessageContainer().getKey()) {
          ...
          } else if (org.opencms.security.Messages.ERR_LOGIN_FAILED_WITH_MESSAGE_1
              == loginException.getMessageContainer().getKey()) {
            ...
            if (loginMessage != null) {
              m_message =
                  Messages.get()
                      .container(
                          Messages.GUI_LOGIN_FAILED_WITH_MESSAGE_1, loginMessage.getMessage());
            }
          }
          if (m_message == null) {
    ...
    if (m_action == ACTION_LOGIN) {
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(m_directEditPath)
        ...
        if (CmsWorkplace.JSP_WORKPLACE_URI.equals(resource)) {
          ...
          // probably this is a "Guest" user in a default setup where "Guest" has no access to the
          // Workplace
          m_message =
              Messages.get().container(Messages.GUI_LOGIN_FAILED_NO_WORKPLACE_PERMISSIONS_0);
          m_action = ACTION_DISPLAY;
        } else if (getCmsObject().existsResource(CmsWorkplace.JSP_WORKPLACE_URI)) {
          // resource does either not exist or is not readable, but general workplace permissions
          // are granted
          m_message =
              Messages.get().container(Messages.GUI_LOGIN_UNKNOWN_RESOURCE_1, m_requestedResource);
          ...
          m_message =
              Messages.get()
                  .container(
                      Messages.GUI_LOGIN_FAILED_NO_TARGET_PERMISSIONS_1, m_requestedResource);
          m_action = ACTION_DISPLAY;
        }
      }
  ...
  protected void appendDefaultLoginScript(StringBuffer html, CmsMessageContainer message) {
    ...
    html.append(
        "\t\t\tactStyle = \" style=\\\"margin-left: \" + (currOu.level * 20) + \"px;\\\"\";\n");
    html.append("\t\t}\n");
    ...
    html.append("\thtml += \"");
    html.append(
        Messages.get().getBundle(m_locale).key(Messages.GUI_LOGIN_ORGUNIT_SEARCH_NORESULTS_0));
    html.append("\";\n");
    ...
    // filters the OUs by the provided search term using a timeout, called by the onkeyup event of
    ...
    html.append(
        "\tsearchTimeout = setTimeout(\"showOrgUnits(\\\"\" + trim(searchTerm) + \"\\\");\", 750);\n");
    html.append("}\n");

    // sets the value of the OU search input field, called by the onfocus and onblur event of the
    // field
    ...
    // function to check IE version, in case of a version < IE8 login will be disabled and an error
    // message shown.
  ...
  /**
   * Appends the JavaScript that opens the Direct Edit window after a successful login to the given
   * HTML buffer.
   *
   * <p>
   *
  ...
  /**
   * Appends the JavaScript that opens the Workplace window after a successful login to the given
   * HTML buffer.
   *
   * <p>
   *
  ...
  protected void appendWorkplaceOpenerScript(
      StringBuffer html, String requestedResource, CmsMessageContainer message) {
    ...
    if (agreementInfo.isShowUserAgreement()) {
      openResource =
          agreementInfo.getConfigurationVfsPath()
              + "?"
  ...
  protected String displayLoginForm() {
    ...
    if ((m_action == ACTION_DISPLAY) && OpenCms.getLoginManager().isEnableSecurity()) {
      ...
      html.append(
          CmsStringUtil.escapeHtml(
              Messages.get().getBundle(m_locale).key(Messages.GUI_LOGIN_SECURITY_0)));
      html.append("</b></div>\n");
      html.append("</td>\n");
      html.append("</tr>\n");
      html.append("<tr>\n");
      ...
      html.append(
          CmsStringUtil.escapeHtml(
              Messages.get().getBundle(m_locale).key(Messages.GUI_LOGIN_PCTYPE_PUBLIC_0)));
      html.append("<br/>");
      html.append("<input type=\"radio\" value=\"");
      ...
      html.append(
          CmsStringUtil.escapeHtml(
              Messages.get().getBundle(m_locale).key(Messages.GUI_LOGIN_PCTYPE_PRIVATE_0)));
      html.append("</div></td>\n");
      html.append("</tr>\n");
    }
    ...
    if (m_action == ACTION_DISPLAY) {
      html.append(
          CmsStringUtil.escapeHtml(
              Messages.get().getBundle(m_locale).key(Messages.GUI_LOGIN_MESSAGE_0)));
    } else if (m_action == ACTION_LOGIN) {
      html.append(
          CmsStringUtil.escapeHtml(
    ...
    html.append(
        "<td style=\"width: 60px; text-align: center; vertical-align: top\" rowspan=\"5\">");
    html.append("<img src=\"");
    ...
    if (m_action == ACTION_DISPLAY) {
      ...
      html.append("<tr>\n");
      html.append(
          "<td style=\"white-space: nowrap;\"><div id='ouLabelId' style='display: none;'><b>");
      html.append(Messages.get().getBundle(m_locale).key(Messages.GUI_LOGIN_ORGUNIT_0))
          .append("</b>&nbsp;&nbsp;\n");
      html.append("</div></td>\n");
      html.append(
      ...
      if ((getOus().size() > 1)
          && ((getPreDefOuFqn() == null)
              || getPreDefOuFqn().equals(CmsOrganizationalUnit.SEPARATOR))) {
        // options
   ...
   *
   * <p>This is normally selected by url, and set by the {@link CmsWorkplaceLoginHandler}.
   *
   * <p>
   *
  ...
  protected String getPreDefOuFqn() {
    ...
    if (Boolean.valueOf(m_actionLogout).booleanValue()
        && (getRequest().getAttribute(PARAM_PREDEF_OUFQN) == null)) {
