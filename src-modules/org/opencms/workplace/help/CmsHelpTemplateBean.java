 ...
 *
 * <ul>
 *   <li>Online help will only work with resources of type xmlpage.
 *   <li>Content pages with a property <em>"template-elements"</em> set to a path of a ressource
 *       (jsp, page,...) will get the content produced by <code>
 *       {@link org.opencms.jsp.CmsJspActionElement#getContent(String)}</code> appended after their
 *       own output. This allows to use jsp's in the online help template.
 * </ul>
 *
 * @since 6.0.0
 */
public class CmsHelpTemplateBean extends CmsDialog {
  ...
  public static final String HELPMAPPINGS_FILENAME =
      "mappings_" + "${" + CmsMacroResolver.KEY_REQUEST_LOCALE + "}.properties";
  ...
  public static final String PATH_HELP =
      CmsWorkplace.VFS_PATH_LOCALES + "${" + CmsMacroResolver.KEY_REQUEST_LOCALE + "}/help/";
  ...
  public static final String RFS_HELPMAPPINGS =
      "classes/" + MODULE_NAME.replace('.', '/') + "/" + HELPMAPPINGS_FILENAME;

  /** Absolute path to used JSP templates. */
  public static final String TEMPLATEPATH =
      CmsWorkplace.VFS_PATH_MODULES + MODULE_NAME + "/jsptemplates/";

  /** The log object for this class. */
  private static final Log LOG = CmsLog.getLog(CmsHelpTemplateBean.class);

  /**
   * The online project that is switched to whenever body content is processed that should not be
   * exported.
   */
  private CmsProject m_offlineProject;

  /**
   * The online project that is switched to whenever body content is processed that shall be
   * exported.
   */
  ...
  public String displayBody() {
    ...
    try {
      ...
      result.append(
          "<table class=\"helpcontent\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\n");
      result.append("<tr>\n");
      ...
      try {
        ...
        // as value of property PROPERTY_TEMPLATE_ELEMENTS (currently sitemap.jsp and search.jsp are
        // used)
        CmsProperty elements =
            getCms()
                .readPropertyObject(
                    getParamHelpresource(),
        ...
        if (!elements.isNullProperty()) {
          try {
            ...
            CmsProperty exportProp =
                getCms()
                    .readPropertyObject(elementName, CmsPropertyDefinition.PROPERTY_EXPORT, true);
            ...
            CmsVfsResourceNotFoundException e2 =
                new CmsVfsResourceNotFoundException(
                    Messages.get()
                        .container(
  ...
  public String displayHead() {
    ...
    try {
      ...
      result.append(
          getJsp().link("/system/modules/org.opencms.workplace.help/resources/search.js"));
      ...
      // search index may be attached to resource
      ...
      submitAction
          .append(
              Messages.get()
                  .getBundle(getLocale())
                  .key(Messages.GUI_HELP_ERR_SEARCH_WORD_LENGTH_1, new Integer(3)))
          .append("\');");
      ...
      String searchLink =
          getJsp()
              .link(
                  new StringBuffer(
                          "/system/modules/org.opencms.workplace.help/elements/search.jsp?")
                      .append(CmsLocaleManager.PARAMETER_LOCALE)
                      .append("=")
                      .append(getLocale())
                      .toString());
      ...
      result.append(
          button(
              "javascript:top.body.location.href='"
                  + CmsEncoder.escapeXml(getParamHomelink())
                  + "';",
              null,
              "contents.png",
      ...
      result.append(
          "<input type=\"text\" name=\"query2\" class=\"onlineform\" style=\"width: 120px\" value=\"");
      result.append("");
      result.append(" \">");
      result.append("</td>\n");
      ...
      // display logo
      result.append(
          "<span style=\"display: block; width: 80px; height: 22px; background-image: url(\'");
  ...
  /**
   * Generates the HTML for the online help frameset or redirects to the help body, depending on the
   * build frameset flag.
   *
   * <p>
   *
  ...
  protected String buildHtmlHelpNavigation() {
    ...
    List<CmsJspNavElement> navList =
        getJsp().getNavigation().getNavigationTreeForFolder(currentUri, helpLevel, 99);
  ...
  protected void checkRole() throws CmsRoleViolationException {
    ...
    if (!OpenCms.getDefaultUsers()
        .isUserExport(getCms().getRequestContext().getCurrentUser().getName())) {
  ...
  protected String displayFrameset() {
    ...
    result.append(
        "\t\ttop.location.href = \""
            + getJsp().link(getJsp().getRequestContext().getUri())
            + "\";\n");
    result.append("\t}\n");
    result.append("//-->\n</script>\n");
    result.append("<meta HTTP-EQUIV=\"Content-Type\" CONTENT=\"text/html; charset=utf-8\">\n");
   ...
   *
   * <p>If a mapping information is found, the requested URI is set to the found value.
   *
   * <p>If no workplace resource URI is given, nothing is changed.
   *
   * <p>
   */
          ...
          try {
            ...
            String absolutePath =
                OpenCms.getSystemInfo()
                    .getAbsoluteRfsPathRelativeToWebInf(resolveMacros(RFS_HELPMAPPINGS));
  ...
  /**
   * Attaches the resource name to the request as parameter.
   *
   * <p>
   *
   * @param resourceName a name of a resource
   * @return The given resource name with additional request parameter concatenations of the current
   *     request on this <code>CmsDialog</code>
   */
