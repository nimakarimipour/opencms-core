 ...
 *
 * <p>This is a special dialog that is used for xmlpages in the workplace and the editors.
 *
 * <p>Uses methods from the customized property dialog where possible.
 *
...
public class CmsDialogProperty extends CmsPropertyCustom {
  ...
  public String buildEditForm() {
    ...
    retValue.append("<tr>\n");
    retValue.append(
        "\t<td class=\"textbold\">" + messages.key(Messages.GUI_INPUT_PROPERTY_0) + "</td>\n");
    retValue.append(
        "\t<td class=\"textbold\">" + messages.key(Messages.GUI_LABEL_VALUE_0) + "</td>\n");
    retValue.append(
  ...
  public String buildSelectTemplates(String attributes) {
    ...
    try {
      ...
      currentTemplate =
          getCms()
              .readPropertyObject(getParamResource(), CmsPropertyDefinition.PROPERTY_TEMPLATE, true)
              .getValue();
  ...
  /**
   * Adds the currently selected template value to the option and value list.
   *
   * <p>
   *
  ...
  private void addCurrentTemplate(
      String currentTemplate, List<String> options, List<String> values) {
    ...
    if (CmsStringUtil.isEmpty(currentTemplate)) {
      ...
      try {
        ...
        name =
            getCms()
                .readPropertyObject(currentTemplate, CmsPropertyDefinition.PROPERTY_TITLE, false)
                .getValue();
      } catch (CmsException e) {
