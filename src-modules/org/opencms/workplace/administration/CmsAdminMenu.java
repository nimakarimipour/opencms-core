...
public class CmsAdminMenu extends CmsToolDialog {
  ...
  private CmsIdentifiableObjectContainer<CmsAdminMenuGroup> m_groupContainer =
      new CmsIdentifiableObjectContainer<CmsAdminMenuGroup>(true, true);
   ...
   *
   * <p>If the menu does not exist, it will be created.
   *
   * <p>
   *
  ...
  /**
   * Returns all initialized parameters of the current request that are not in the given exclusion
   * list as hidden field tags that can be inserted in a form.
   *
   * <p>
   *
  ...
  public void installMenu() {
    ...
    CmsAdminMenuGroup helpMenu =
        new CmsAdminMenuGroup(
            "help",
    ...
    Iterator<CmsTool> itElems =
        getToolManager()
            .getToolsForPath(this, getToolManager().getBaseToolPath(this), false)
            .iterator();
    while (itElems.hasNext()) {
      ...
      // cut out the base
      String path =
          tool.getHandler().getPath().substring(getToolManager().getBaseToolPath(this).length());
