...
public abstract class CmsWidgetDialog extends CmsDialog implements I_CmsWidgetDialog {
   ...
   *
   * <p>This must not be <code>null</code>, because it must be available when calling <code>
   * {@link org.opencms.workplace.CmsWorkplace#paramsAsHidden()}</code>.
   *
   * <p>
   */
   ...
   *
   * <p>Depends on the value stored in the <code>{@link CmsDialog#getAction()}</code> method.
   *
   * <p>
   */
   ...
   * @param enabled if true, the button to remove an element is shown, otherwise a spacer is
   *     returned
  ...
  public String buildRemoveElement(String elementName, int index, boolean enabled) {
    ...
    if (enabled) {
      ...
      href.append(");");
      return button(
          href.toString(), null, "deletecontent.png", Messages.GUI_DIALOG_BUTTON_DELETE_0, 0);
    } else {
      return "";
    }
  }
  ...
  /**
   * Clears the "dialog object" for this widget dialog by removing it from the current users
   * session.
   *
   * <p>
   */
    ...
    if (getPages().size() > 1) {
      ...
      if (pageIndex == (getPages().size() - 1)) {
        ...
        return dialogButtons(
            new int[] {BUTTON_BACK, BUTTON_CONTINUE, BUTTON_CANCEL}, new String[3]);
      } else {
   ...
   * @throws IOException if writing to the JSP out fails, or in case of errors forwarding to the
  ...
  /**
   * Performs the dialog actions depending on the initialized action and displays the dialog form if
   * needed.
   *
   * <p>
   *
   * @param writeLater if <code>true</code> no output is written, you have to call manually the
   *     <code>{@link #defaultActionHtml()}</code> method.
   * @throws JspException if dialog actions fail
   * @throws IOException if writing to the JSP out fails, or in case of errors forwarding to the
   *     required result page
   * @throws ServletException in case of errors forwarding to the required result page
  ...
  /**
   * Returns the dialog object for this widget dialog, or <code>null</code> if no dialog object has
   * been set.
   *
   * <p>
   *
  ...
  /**
   * Returns the value of the widget parameter with the given name, or <code>null</code> if no such
   * widget parameter is available.
   *
   * <p>
   *
   ...
   *
   * <p>This HTML includes additional components, for example the &lt;div&gt; tags containing the
   * help texts.
   *
   * <p>
   *
  ...
  /**
   * Returns the values of all widget parameters of this dialog as HTML hidden fields, excluding the
   * widget values that are on the given dialog page.
   *
   * <p>This can be used to create multi-page dialogs where the values are passed from one page to
   * another before everything is submitted. If a widget A is used on page X, there should be no
   * "hidden" HTML field for A since otherwise A would have 2 values when submitting the dialog
   * page: The one from the widget itself and the one from the hidden field. This may lead to
   * undefined results when processing the submitted values.
   *
   * <p>
   *
   * @param excludeDialogPage the dialog page to exclude the values for
   * @return the values of all widget parameters of this dialog as HTML hidden fields, excluding the
   *     widget values that are on the given dialog page
   * @see org.opencms.workplace.CmsWorkplace#paramsAsHidden()
   */
    ...
    while (i.hasNext()) {
      ...
      while (j.hasNext()) {
        ...
        if (CmsStringUtil.isNotEmpty(value)
            && ((excludeDialogPage == null)
                || (!param.getDialogPage().equals(excludeDialogPage)))) {
  ...
  /**
   * Writes the dialog html code, only if the <code>{@link #ACTION_DEFAULT}</code> is set.
   *
   * <p>
   *
   * @throws JspException if dialog actions fail
   ...
   *
   * <p>If the error list has not been initialized yet, this is done automatically.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the dialog should be closed after the values have been committed.
   *
   * <p>The default implementation returns <code>true</code> in case there are no commit errors.
   *
   * <p>
   *
   ...
   * @return a List of all Exceptions that occurred when committing the dialog page.
   *     <p>
   */
  protected List<Throwable> commitWidgetValues(String dialogPage) {
    ...
    while (i.hasNext()) {
      ...
      if ((dialogPage == null)
          || (base.getDialogPage() == null)
          || dialogPage.equals(base.getDialogPage())) {
   ...
   *
   * <p>To get a more complex layout variation, you have to overwrite this method in your dialog
   * class.
   *
   * <p>
   *
      ...
      if (p.hasError()) {
        ...
        while (t != null) {
          if (t instanceof I_CmsThrowable) {
            result.append(
                CmsEncoder.escapeXml(((I_CmsThrowable) t).getLocalizedMessage(getLocale())));
          } else {
  ...
  /**
   * Create the complete widget dialog start block HTML that begins a widget block with optional
   * headline.
   *
   * <p>
   *
   * @param headline the headline String for the block
   * @return the complete widget dialog start block HTML that begins a widget block with optional
   *     headline
   */
  ...
  protected String defaultActionHtmlContent() {
    ...
    StringBuffer result = new StringBuffer(2048);
    result
        .append("<form name=\"EDITOR\" id=\"EDITOR\" method=\"post\" action=\"")
        .append(getDialogRealUri());
    result
        .append("\" class=\"nomargin\" onsubmit=\"return submitAction('")
        .append(DIALOG_OK)
        .append("', null, 'EDITOR');\">\n");
  ...
  protected String defaultActionHtmlStart() throws JspException {
    ...
    result
        .append("<script  src=\"")
        .append(getResourceUri())
        .append("editors/xmlcontent/edit.js\"></script>\n");
    result
        .append("<script  src=\"")
        .append(getResourceUri())
        .append("editors/xmlcontent/help.js\"></script>\n");
    ...
    result
        .append("var actionRemoveElement = \"")
        .append(EDITOR_ACTION_ELEMENT_REMOVE)
        .append("\";\n");
   ...
   *
   * <p>In the default implementation this method returns <code>null</code>. Override this if
   * needed.
   *
   * <p>
   *
   ...
   *
   * <p>The method (@link CmsWidgetDialog#commitWidgetValues(String)) has to set this list.
   *
   * <p>
   *
  ...
  /**
   * Returns the list of all widgets used on this widget dialog, the List must contain Objects of
   * type <code>{@link CmsWidgetDialogParameter}</code>.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the current dialog (page) has commit errors.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the current dialog (page) has validation errors.
   *
   * <p>
   *
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(getParamPage())
        || !getPages().contains(getParamPage())) {
    ...
    try {
      ...
      if (LOG.isInfoEnabled()) {
        LOG.info(
            Messages.get().container(Messages.ERR_WORKPLACE_DIALOG_PARAMS_1, getCurrentToolPath()),
            e);
      }
   ...
   *
   * <p>Use this in the method (@link CmsWidgetDialog#commitWidgetValues(String)) to set the list.
   *
   * <p>
   *
   ...
   *
   * <p>The exception is never seen by the user, so it can be just a <code>new {@link Exception}()
   * </code>.
   *
   * <p>
   *
