...
public class CmsProjectsList extends A_CmsListDialog {
    ...
    if (getParamListAction().equals(LIST_MACTION_DELETE)) {
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_DELETE_SELECTED_PROJECTS_0), e);
      }
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_UNLOCK_SELECTED_PROJECTS_0), e);
      }
    } else {
  ...
  public void executeListSingleActions() throws IOException, ServletException {
    ...
    if (getParamListAction().equals(LIST_DEFACTION_FILES)) {
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_DELETE_PROJECT_1, projectName), e);
      }
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_UNLOCK_PROJECT_1, projectName), e);
      }
    } else {
      throwListUnsupportedActionException();
    }
    ...
    while (itProjects.hasNext()) {
      ...
      item.set(
          ...
          OpenCms.getOrgUnitManager()
              .readOrganizationalUnit(getCms(), project.getOuFqn())
      ...
      try {
        item.set(
            LIST_COLUMN_MANAGER, getCms().readGroup(project.getManagerGroupId()).getSimpleName());
      } catch (Exception e) {
        // ignore
      }
      try {
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    filesAction.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_ACTION_FILES_HELP_0));
    ...
    // lock action
    CmsListDirectAction lockAction =
        new CmsListDirectAction(LIST_ACTION_LOCK) {
    ...
    lockAction.setConfirmationMessage(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_ACTION_LOCK_CONF_0));
    lockAction.setIconPath(PATH_BUTTONS + "project_lock.png");
    lockAction.setEnabled(false);
    lockCol.addDirectAction(lockAction);

    // unlock action
    CmsListDirectAction unlockAction =
        new CmsListDirectAction(LIST_ACTION_UNLOCK) {
    ...
    unlockAction.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_ACTION_UNLOCK_HELP_0));
    unlockAction.setConfirmationMessage(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_ACTION_UNLOCK_CONF_0));
    ...
    publishCol.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_COLS_PUBLISH_HELP_0));
    ...
    CmsListDirectAction publishEnabledAction =
        new CmsListDirectAction(LIST_ACTION_PUBLISH_ENABLED) {
          ...
          protected String resolveOnClic(Locale locale) {
            ...
            String link =
                OpenCms.getLinkManager()
                    .substituteLink(
                        getCms(),
                        "/system/workplace/commons/publish.jsp?publishProjectId="
                            + getItem().getId());
            return "window.location='"
                + link
                + "' + '&closelink='+encodeURI(window.location.href);";
          }
        };
    ...
    CmsListDirectAction publishDisabledAction =
        new CmsListDirectAction(LIST_ACTION_PUBLISH_DISABLED) {
    ...
    deleteAction.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_ACTION_DELETE_HELP_0));
    deleteAction.setConfirmationMessage(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_ACTION_DELETE_CONF_0));
    ...
    defEditAction.setName(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_DEFACTION_EDIT_NAME_0));
    defEditAction.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_DEFACTION_EDIT_HELP_0));
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    CmsListSearchAction searchAction =
        new CmsListSearchAction(metadata.getColumnDefinition(LIST_COLUMN_NAME));
  ...
  protected void setMultiActions(CmsListMetadata metadata) {
    ...
    unlockProject.setName(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_MACTION_UNLOCK_NAME_0));
    unlockProject.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_MACTION_UNLOCK_HELP_0));
    ...
    deleteMultiAction.setName(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_MACTION_DELETE_NAME_0));
    deleteMultiAction.setHelpText(
        Messages.get().container(Messages.GUI_PROJECTS_LIST_MACTION_DELETE_HELP_0));
