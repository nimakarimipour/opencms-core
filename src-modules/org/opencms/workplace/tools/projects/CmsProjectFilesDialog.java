...
public class CmsProjectFilesDialog extends A_CmsListExplorerDialog {
  ...
  public CmsHtmlList getList() {
    ...
    m_filter =
        getJsp()
            .getRequest()
            .getParameter(LIST_IACTION_FILTER + CmsListDropdownAction.SUFFIX_PARAM);
    CmsListDropdownAction listAction = null;
    if (list != null) {
      listAction =
          ((CmsListDropdownAction) list.getMetadata().getIndependentAction(LIST_IACTION_FILTER));
  ...
  /**
   * @see
   *     org.opencms.workplace.list.A_CmsListExplorerDialog#getAdditionalParametersForExplorerForward()
   */
  @Override
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    filterAction.setHelpText(
        Messages.get().container(Messages.GUI_PROJECT_FILES_FILTER_ACTION_HELP_0));
    ...
    while (it.hasNext()) {
      ...
      filterAction.addItem(
          mode.getMode(), Messages.get().container(A_CmsWidget.LABEL_PREFIX + mode.getMode()));
    }
  ...
  protected void validateParamaters() throws Exception {
    ...
    try {
      ...
      setStoredProject(
          getParamProjectid()); // doing this after the readProject call because now we know the id
                                // is valid
    } catch (Exception e) {
  ...
  /**
   * Gets the stored project id from the session.
   *
   * <p>
   *
