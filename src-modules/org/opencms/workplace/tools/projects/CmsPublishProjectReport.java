...
public class CmsPublishProjectReport extends CmsReport {
   ...
   */
  public CmsPublishProjectReport(
  ...
  public void displayReport() throws JspException {
    ...
    switch (getAction()) {
      ...
      default:
        try {
          if (getCms().readProject(new CmsUUID(getParamProjectid())).getType()
        ...
        try {
          ...
          getCms()
              .getRequestContext()
              .setCurrentProject(getCms().readProject(new CmsUUID(getParamProjectid())));
  ...
  /**
   * Starts the publish thread for the project.
   *
   * <p>
   */
  ...
  /**
   * Starts the link validation thread for the project.
   *
   * <p>
   *
  ...
  private void startValidationThread(CmsPublishList publishList) throws JspException {
    ...
    try {
      CmsRelationsValidatorThread thread =
          new CmsRelationsValidatorThread(getCms(), publishList, getSettings());
      thread.start();
