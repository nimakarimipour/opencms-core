...
public class CmsEditProjectDialog extends CmsWidgetDialog {
      ...
      if (!isNewProject()) {
        ...
        ou =
            OpenCms.getRoleManager()
                .getOrgUnitsForRole(getCms(), CmsRole.PROJECT_MANAGER, true)
                .get(0);
      }
  ...
  protected String createDialogHtml(String dialog) {
    ...
    if (dialog.equals(PAGES[0])) {
      ...
      result.append(
          dialogBlockStart(key(Messages.GUI_PROJECT_EDITOR_LABEL_IDENTIFICATION_BLOCK_0)));
      result.append(createWidgetTableStart());
  ...
  protected void defineWidgets() {
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_project, "description", "", PAGES[0], new CmsTextareaWidget(), 0, 1));
    addWidget(
        new CmsWidgetDialogParameter(
            this, "managerGroup", PAGES[0], new CmsGroupWidget(null, null)));
    addWidget(
        new CmsWidgetDialogParameter(this, "userGroup", PAGES[0], new CmsGroupWidget(null, null)));
    if (isNewProject()) {
      int ous = 1;
      try {
        ous =
      ...
      if (ous < 2) {
        addWidget(
            new CmsWidgetDialogParameter(this, "assignedOu", PAGES[0], new CmsDisplayWidget()));
      } else {
        addWidget(
            new CmsWidgetDialogParameter(
                this, "oufqn", PAGES[0], new CmsOrgUnitWidget(CmsRole.PROJECT_MANAGER)));
      }
    } else {
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_project, "deleteAfterPublishing", PAGES[0], new CmsCheckboxWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            this, "resources", PAGES[0], new CmsVfsFileWidget(false, "", false, false)));
  }
  ...
  /**
   * Initializes the project object to work with depending on the dialog state and request
   * parameters.
   *
   * <p>Two initializations of the project object on first dialog call are possible:
   *
   * <ul>
   *   <li>edit an existing project
  ...
  protected void initProjectObject() {
    ...
    try {
      ...
      setUserGroup(
          getCms().getRequestContext().getOuFqn() + OpenCms.getDefaultUsers().getGroupUsers());
    }
  }
  ...
  /**
   * Checks if the given group name is a valid opencms user group.
   *
   * <p>
   *
