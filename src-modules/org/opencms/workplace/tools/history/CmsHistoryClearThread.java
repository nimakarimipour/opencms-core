...
public class CmsHistoryClearThread extends A_CmsReportThread {
  ...
  public void run() {
    ...
    if (m_historyClear
        .getClearDeletedMode()
        .equals(CmsHistoryClearDialog.MODE_CLEANDELETED_DELETE_NONE)) {
      versionsDeleted = -1;
    } else if (m_historyClear
        .getClearDeletedMode()
        .equals(CmsHistoryClearDialog.MODE_CLEANDELETED_DELETE_ALL)) {
      versionsDeleted = 0;
    } else {
      versionsDeleted = 1;
    }
    ...
    getReport()
        .println(
            Messages.get().container(Messages.RPT_DELETE_HISTORY_END_0),
            I_CmsReport.FORMAT_HEADLINE);
  }
}
