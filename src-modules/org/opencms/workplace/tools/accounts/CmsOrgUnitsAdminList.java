...
public class CmsOrgUnitsAdminList extends A_CmsOrgUnitsList {
  ...
  /**
   * Performs a forward to the overview of the single organizational unit the current user is
  ...
  public void forwardToSingleAdminOU() throws ServletException, IOException, CmsException {
    ...
    OpenCms.getWorkplaceManager()
        .getToolManager()
        .jspForwardTool(this, getForwardToolPath(), params);
  }
   ...
   * <p>
   *
   * @return true if the user has more then then one organizational unit to administrate otherwise
   *     false
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    overviewCol.setHelpText(
        Messages.get().container(Messages.GUI_ORGUNITS_LIST_COLS_OVERVIEW_HELP_0));
    overviewCol.setWidth("20");
    overviewCol.setAlign(CmsListColumnAlignEnum.ALIGN_CENTER);
    overviewCol.setSorteable(false);
    // add overview action
    CmsListDirectAction overviewAction =
        new CmsListDirectAction(LIST_ACTION_OVERVIEW) {
    ...
    overviewAction.setName(
        Messages.get().container(Messages.GUI_ORGUNITS_LIST_ACTION_OVERVIEW_NAME_0));
