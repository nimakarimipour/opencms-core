...
public class CmsGroupDependenciesList extends CmsGroupPrincipalDependenciesList {
   ...
   */
  public CmsGroupDependenciesList(
  ...
  public void actionDialog() throws JspException, ServletException, IOException {
    ...
    switch (getAction()) {
      case ACTION_DELETE:
        Iterator<String> it =
            CmsStringUtil.splitAsList(getGroupName(), CmsHtmlList.ITEM_SEPARATOR, true).iterator();
        while (it.hasNext()) {
          String name = it.next();
          try {
      ...
      case ACTION_TRANSFER:
        ...
        getToolManager()
            .jspForwardPage(
                this, getJsp().getRequestContext().getFolderUri() + "group_transfer.jsp", params);
        break;
  ...
  protected String customHtmlEnd() {
    ...
    result.append(
        Messages.get().container(Messages.GUI_DEPENDENCIES_BUTTON_DELETE_0).key(getLocale()));
    ...
    if (getList().getTotalSize() > 0) {
      ...
      result.append("' type='button' value='");
      result.append(
          Messages.get().container(Messages.GUI_DEPENDENCIES_BUTTON_TRANSFER_0).key(getLocale()));
      result.append("' onclick=\"submitAction('");
      result.append(TRANSFER_ACTION);
      result.append("', form);\" class='dialogbutton'>\n");
    }
  ...
  protected String customHtmlStart() {
    ...
    result.append(
        dialogBlockStart(
            Messages.get().container(Messages.GUI_GROUP_DEPENDENCIES_NOTICE_0).key(getLocale())));
