...
public class CmsUsersCsvDownloadDialog extends A_CmsUserDataImexportDialog {
   ...
   */
  public CmsUsersCsvDownloadDialog(
  ...
  /**
   * Generates the CSV file for the given users.
   *
   * <p>
   *
   * @return CSV file
  ...
  public String generateCsv() {
    ...
    try {
      if (((groups == null) || (groups.size() < 1)) && ((roles == null) || (roles.size() < 1))) {
        exportUsers =
            CmsImportExportUserDialog.addExportAllUsers(getCms(), getParamOufqn(), exportUsers);
      } else {
        exportUsers =
            CmsImportExportUserDialog.addExportUsersFromGroups(getCms(), groups, exportUsers);
        exportUsers =
            CmsImportExportUserDialog.addExportUsersFromRoles(
                getCms(), getParamOufqn(), roles, exportUsers);
      }
      ...
      while (itValues.hasNext()) {
        ...
        try {
          ...
          throw new CmsRuntimeException(
              Messages.get().container(Messages.ERR_INVOCATION_TARGET_0), e);
        }
      }
      buffer.append("\n");
    }
              ...
              getJsp()
                  .link(
                      CmsRequestUtil.appendParameter(
                          getDownloadPath(), A_CmsOrgUnitDialog.PARAM_OUFQN, getParamOufqn())))
          .append("\", \"usecvs\");\n");
      result.append("}\n");
  ...
  /**
   * Returns the export options data.
   *
   * <p>
   *
  ...
  protected Map<String, List<String>> getData() {
    ...
    return (Map<String, List<String>>)
        ((Map<String, Object>) getSettings().getDialogObject())
            .get(CmsUserDataExportDialog.class.getName());
  }
