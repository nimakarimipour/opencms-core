...
public abstract class A_CmsEditUserDialog extends CmsWidgetDialog {
  ...
  public void actionCommit() {
      ...
      if (isNewUser()) {
        ...
        if (group != null) {
          if (group.getSimpleName().equals(OpenCms.getDefaultUsers().getGroupAdministrators())) {
            // in case the administrators group is selected, the administrator role must be selected
            // also
      ...
      try {
        webuserOu =
            OpenCms.getOrgUnitManager()
                .readOrganizationalUnit(getCms(), getParamOufqn())
                .hasFlagWebuser();
    ...
    if (errors.isEmpty() && isNewUser()) {
      if ((getParamCloseLink() != null)
          && (getParamCloseLink().indexOf("path=" + getListRootPath()) > -1)) {
        // set closelink
        Map<String, String[]> argMap = new HashMap<String, String[]>();
        argMap.put(PARAM_USERID, new String[] {m_user.getId().toString()});
        argMap.put("oufqn", new String[] {m_paramOufqn});
        setParamCloseLink(
            CmsToolManager.linkForToolPath(getJsp(), getListRootPath() + "/edit", argMap));
      }
    }
   ...
   */
  protected abstract CmsUser createUser(
      String name, String pwd, String desc, Map<String, Object> info) throws CmsException;
    ...
    if (isEditable(m_user)) {
      addWidget(
          new CmsWidgetDialogParameter(
              this, "description", "", PAGES[0], new CmsTextareaWidget(), 0, 1));
      ...
      addWidget(
          new CmsWidgetDialogParameter(
              m_user, "institution", "", PAGES[0], new CmsInputWidget(), 0, 1));
      addWidget(
          new CmsWidgetDialogParameter(
              m_user, "address", "", PAGES[0], new CmsInputWidget(), 0, 1));
      addWidget(
          new CmsWidgetDialogParameter(
              m_user, "zipcode", "", PAGES[0], new CmsInputWidget(), 0, 1));
      addWidget(
          new CmsWidgetDialogParameter(m_user, "city", "", PAGES[0], new CmsInputWidget(), 0, 1));
      addWidget(
          new CmsWidgetDialogParameter(
              m_user, "country", "", PAGES[0], new CmsInputWidget(), 0, 1));
      if (!webuserOu) {
        addWidget(
            new CmsWidgetDialogParameter(
                this, "language", PAGES[0], new CmsSelectWidget(getLanguages())));
        if (!getSites().isEmpty()) {
          addWidget(
              new CmsWidgetDialogParameter(
                  this, "site", PAGES[0], new CmsSelectWidget(getSites())));
        }
        addWidget(
            new CmsWidgetDialogParameter(
                this, "startProject", PAGES[0], new CmsSelectWidget(getProjects())));
        addWidget(
            new CmsWidgetDialogParameter(
                this, "startFolder", PAGES[0], new CmsVfsFileWidget(true, null, false)));
        addWidget(
            new CmsWidgetDialogParameter(
                this, "startView", PAGES[0], new CmsSelectWidget(getViews())));
        if (isNewUser()) {
          addWidget(
              new CmsWidgetDialogParameter(
                  this, "group", PAGES[0], new CmsGroupWidget(null, null, getParamOufqn())));
          addWidget(
              new CmsWidgetDialogParameter(
                  this, "role", PAGES[0], new CmsSelectWidget(getRoles())));
        }
      }
    } else {
      addWidget(
          new CmsWidgetDialogParameter(this, "description", PAGES[0], new CmsDisplayWidget()));
      addWidget(new CmsWidgetDialogParameter(m_user, "lastname", PAGES[0], new CmsDisplayWidget()));
      addWidget(
          new CmsWidgetDialogParameter(m_user, "firstname", PAGES[0], new CmsDisplayWidget()));
      addWidget(new CmsWidgetDialogParameter(m_user, "email", PAGES[0], new CmsDisplayWidget()));
      addWidget(new CmsWidgetDialogParameter(this, "assignedOu", PAGES[0], new CmsDisplayWidget()));
      addWidget(
          new CmsWidgetDialogParameter(m_user, "institution", PAGES[0], new CmsDisplayWidget()));
      ...
      if (!webuserOu) {
        ...
        addWidget(
            new CmsWidgetDialogParameter(this, "startProject", PAGES[0], new CmsDisplayWidget()));
        addWidget(
            new CmsWidgetDialogParameter(this, "startFolder", PAGES[0], new CmsDisplayWidget()));
        addWidget(
            new CmsWidgetDialogParameter(this, "startView", PAGES[0], new CmsDisplayWidget()));
      }
    }
    addWidget(new CmsWidgetDialogParameter(m_user, "enabled", PAGES[0], new CmsCheckboxWidget()));
    addWidget(
        new CmsWidgetDialogParameter(this, "selfManagement", PAGES[0], new CmsCheckboxWidget()));
    if (isPwdChangeAllowed(m_user)) {
      addWidget(
          new CmsWidgetDialogParameter(m_pwdInfo, "newPwd", PAGES[0], new CmsPasswordWidget()));
      addWidget(
          new CmsWidgetDialogParameter(
              m_pwdInfo, "confirmation", PAGES[0], new CmsPasswordWidget()));
    }
  }
   ...
   *
   * <p>Two initializations of the user object on first dialog call are possible:
   *
   * <ul>
   *   <li>edit an existing user
  ...
  protected void initUserObject() {
    ...
    try {
      m_group =
          getCms().readGroup(getParamOufqn() + OpenCms.getDefaultUsers().getGroupUsers()).getName();
    } catch (CmsException e) {
      // ignore
    }
   ...
   *
   * <p>Not editable means that the user can only be activated and deactivated.
   *
   * <p>
   *
  ...
  /**
   * Returns a list of options for the locale selector.
   *
   * <p>
   *
    ...
    while (itLocales.hasNext()) {
      ...
      locales.add(
          new CmsSelectWidgetOption(
              locale.toString(), selected, locale.getDisplayName(getLocale()), null));
    }
    return locales;
  }

  /**
   * Returns a list of options for the project selector.
   *
   * <p>
   *
  ...
  private List<CmsSelectWidgetOption> getProjects() {
    ...
    try {
      ...
      projectsList =
          OpenCms.getOrgUnitManager().getAllAccessibleProjects(getCms(), getParamOufqn(), false);
      ...
      while (itProjects.hasNext()) {
        ...
        if (isNewUser()) {
          projects.add(
              new CmsSelectWidgetOption(
                  project.getSimpleName(), selected, project.getSimpleName(), null));
        } else {
          projects.add(
              new CmsSelectWidgetOption(
  ...
  /**
   * Returns the role select options.
   *
   * <p>
   *
  ...
  private List<CmsSelectWidgetOption> getRoles() {
    ...
    try {
      List<CmsRole> roles =
          new ArrayList<CmsRole>(OpenCms.getRoleManager().getRoles(getCms(), m_paramOufqn, false));
  ...
  /**
   * Returns a list of options for the site selector.
   *
   * <p>
   *
  ...
  private List<CmsSelectWidgetOption> getSites() {
    ...
    List<CmsSite> sitesList =
        OpenCms.getSiteManager().getAvailableSites(getCms(), true, false, getParamOufqn());
    ...
    while (itSites.hasNext()) {
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(siteRoot)) {
        if (sitesList.size() > 1) {
          if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_user.getName())) {
            if (!OpenCms.getRoleManager()
                .hasRole(getCms(), m_user.getName(), CmsRole.VFS_MANAGER)) {
      ...
      sites.add(
          new CmsSelectWidgetOption(
              siteRoot, selected, substituteSiteTitle(site.getTitle()), null));
    }
    return sites;
  }

  /**
   * Returns a list of options for the view selector.
   *
   * <p>
   *
  ...
  private List<CmsSelectWidgetOption> getViews() {
    ...
    List<CmsWorkplaceView> list =
        new ArrayList<CmsWorkplaceView>(OpenCms.getWorkplaceManager().getViews());

    // add the direct edit view to the list
    String displayName =
        getMessages().key(org.opencms.workplace.commons.Messages.GUI_LABEL_DIRECT_EDIT_VIEW_0);
    ...
    for (CmsWorkplaceView view : list) {
      ...
      if (visible) {
        ...
        views.add(
            new CmsSelectWidgetOption(view.getUri(), selected, resolveMacros(view.getKey()), null));
      }
    }
    return views;
  }
