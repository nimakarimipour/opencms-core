...
public class CmsRoleEditList extends A_CmsRolesList {
    ...
    try {
      ...
      if (getParamListAction().equals(LIST_ACTION_ACTIVATE)) {
        ...
        try {
          ...
          throw new CmsRuntimeException(
              Messages.get().container(Messages.ERR_ACTIVATE_ROLE_2, roleName, user.getName()), e);
        }
      } else if (getParamListAction().equals(LIST_ACTION_DEACTIVATE)) {
        // execute the activate action
        if (!OpenCms.getRoleManager()
            .getRolesOfUser(getCms(), user.getName(), "", true, true, true)
            .contains(role)) {
          throw new CmsRuntimeException(
              Messages.get()
                  .container(Messages.ERR_DEACTIVATE_INDIRECT_ROLE_2, roleName, user.getName()));
        }
        try {
          OpenCms.getRoleManager().removeUserFromRole(getCms(), role, user.getName());
          getCms().writeUser(user);
  ...
  public String getIconPath(CmsListItem item) {
    ...
    try {
      ...
      if (OpenCms.getRoleManager()
          .hasRole(getCms(), getCms().readUser(new CmsUUID(getParamUserid())).getName(), role)) {
  ...
  protected void setColumns(CmsListMetadata metadata) {
              ...
              try {
                ...
                if (OpenCms.getRoleManager()
                    .hasRole(
                        getCms(),
                        getCms()
                            .readUser(new CmsUUID(((CmsRoleEditList) getWp()).getParamUserid()))
                            .getName(),
                        role)) {
                  return false;
                }
    ...
    actAction.setHelpText(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_ACTION_ACTIVATE_HELP_0));
    ...
    deactAction.setName(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_ACTION_DEACTIVATE_NAME_0));
    deactAction.setHelpText(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_ACTION_DEACTIVATE_HELP_0));
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    descriptionDetails.setName(
        Messages.get().container(Messages.GUI_ROLEEDIT_DETAIL_DESCRIPTION_NAME_0));
    descriptionDetails.setFormatter(
  ...
  protected void setMultiActions(CmsListMetadata metadata) {
    ...
    activateRole.setName(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_MACTION_ACTIVATE_NAME_0));
    activateRole.setHelpText(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_MACTION_ACTIVATE_HELP_0));
    ...
    deactivateRole.setName(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_MACTION_DEACTIVATE_NAME_0));
    deactivateRole.setHelpText(
        Messages.get().container(Messages.GUI_ROLEEDIT_LIST_MACTION_DEACTIVATE_HELP_0));
