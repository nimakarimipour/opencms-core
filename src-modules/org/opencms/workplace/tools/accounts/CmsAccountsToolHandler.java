...
public class CmsAccountsToolHandler extends CmsDefaultToolHandler {
  ...
  public boolean isEnabled(CmsWorkplace wp) {
    ...
    if (getLink().equals(getPath(GROUPUSERS_FILE))) {
      String groupId =
          CmsRequestUtil.getNotEmptyDecodedParameter(
              wp.getJsp().getRequest(), A_CmsEditGroupDialog.PARAM_GROUPID);
      try {
    ...
    if (getLink().equals(DELETE_FILE)) {
      String ouFqn =
          CmsRequestUtil.getNotEmptyDecodedParameter(
              wp.getJsp().getRequest(), A_CmsOrgUnitDialog.PARAM_OUFQN);
      if (ouFqn == null) {
      ...
      try {
        ...
        if (OpenCms.getOrgUnitManager().getOrganizationalUnits(wp.getCms(), ouFqn, true).size()
            > 0) {
          return false;
        }
    ...
    if (getLink().equals(getPath(ROLEUSERS_EDIT_FILE))) {
      String roleName =
          CmsRequestUtil.getNotEmptyDecodedParameter(
              wp.getJsp().getRequest(), CmsRolesList.PARAM_ROLE);
      if (roleName == null) {
        return false;
      }
      ...
      String paramId =
          CmsRequestUtil.getNotEmptyDecodedParameter(
              wp.getJsp().getRequest(), A_CmsEditUserDialog.PARAM_USERID);
      if (paramId == null) {
        return false;
      }
      ...
      try {
        ...
        return !OpenCms.getRoleManager()
            .hasRole(wp.getCms(), user.getName(), CmsRole.ADMINISTRATOR);
      } catch (CmsException e) {
        // should never happen
  ...
  public boolean isVisible(CmsWorkplace wp) {
    ...
    if (getLink().equals(getPath(OVERVIEW_FILE))) {
      ...
      try {
        List<CmsOrganizationalUnit> orgUnits =
            OpenCms.getRoleManager()
                .getOrgUnitsForRole(cms, CmsRole.ACCOUNT_MANAGER.forOrgUnit(""), true);
        if (orgUnits.size() == 1) {
          return false;
        }
        return !m_webuserOu;
      ...
      try {
        ...
        visible =
            OpenCms.getRoleManager()
                .hasRole(cms, CmsRole.ADMINISTRATOR.forOrgUnit(user.getOuFqn()));
      ...
      return false;
    } else if (getLink().equals(getPath(USERROLE_FILE))
