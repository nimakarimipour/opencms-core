...
public class CmsEditUserAddInfoDialog extends CmsWidgetDialog {
   ...
   */
  public CmsEditUserAddInfoDialog(
        ...
        while (it.hasNext()) {
          ...
          if (infoBean.getValue() == null) {
            ...
            m_user.setAdditionalInfo(
                infoBean.getName(), CmsDataTypeUtil.parse(infoBean.getValue(), infoBean.getType()));
          }
        }
      } else {
        Map<String, Object> readOnly = new HashMap<String, Object>();
        Iterator<Entry<String, Object>> itEntries =
            m_user.getAdditionalInfo().entrySet().iterator();
      ...
      if (getCurrentToolPath().endsWith("/orgunit/users/edit/addinfo/all")) {
        ...
        setParamCloseLink(
            CmsToolManager.linkForToolPath(
                ...
                getCurrentToolPath()
                        .substring(
                            0, getCurrentToolPath().indexOf("/orgunit/users/edit/addinfo/all"))
                    + "/orgunit/users/edit/",
                argMap));
      }
    }
  ...
  /**
   * Returns the edit all flag parameter value.
   *
   * <p>
   *
  ...
  /**
   * Returns the read only add info.
   *
   * <p>
   *
  ...
  /**
   * Sets the modified additional information.
   *
   * <p>
   *
  ...
  /**
   * Sets the edit all flag parameter value.
   *
   * <p>
   *
  ...
  protected String createDialogHtml(String dialog) {
    ...
    if (dialog.equals(PAGES[0])) {
      if (!Boolean.valueOf(getParamEditall()).booleanValue()) {
        int pos = 0;
        Iterator<CmsWorkplaceUserInfoBlock> it =
            OpenCms.getWorkplaceManager().getUserInfoManager().getBlocks().iterator();
        while (it.hasNext()) {
        ...
        result.append(
            createWidgetBlockStart(key(Messages.GUI_USER_EDITOR_LABEL_ADDITIONALINFO_BLOCK_0)));
  ...
  protected void defineWidgets() {
    ...
    if (!Boolean.valueOf(getParamEditall()).booleanValue()) {
      // widgets to display
      Iterator<CmsWorkplaceUserInfoBlock> itBlocks =
          OpenCms.getWorkplaceManager().getUserInfoManager().getBlocks().iterator();
        ...
        while (itEntries.hasNext()) {
          ...
          addWidget(
              new CmsWidgetDialogParameter(
                  m_addInfoList.get(count), "value", entry.getKey(), "", PAGES[0], widget, min, 1));
          count++;
        }
      }
    } else {
  ...
  /**
   * Initializes the additional info bean to work with, depending on the dialog state and request
   * parameters.
   *
   * <p>
   */
    ...
    try {
      if (CmsStringUtil.isEmpty(getParamAction())
        ...
        if (!Boolean.valueOf(getParamEditall()).booleanValue()) {
          ...
          Map<String, SortedMap<String, Object>> dObj =
              (Map<String, SortedMap<String, Object>>) getDialogObject();
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (!Boolean.valueOf(getParamEditall()).booleanValue()) {
      ...
      Map<String, SortedMap<String, Object>> dObj =
          new HashMap<String, SortedMap<String, Object>>();
  ...
  /**
   * Creates a new additional information bean object.
   *
   * <p>
   *
  ...
  private List<CmsUserAddInfoBean> createAddInfoList(CmsUser user) {
    ...
    while (itBlocks.hasNext()) {
      ...
      while (itEntries.hasNext()) {
        ...
        addInfoList.add(
            new CmsUserAddInfoBean(entry.getKey(), value.toString(), entry.getClassType()));
      }
    }
    return addInfoList;
  }
  ...
  /**
   * Builds the additional info maps.
   *
   * <p>
   */
  private void setAddInfoMaps() {
    ...
    while (itEntries.hasNext()) {
      ...
      if ((entry.getValue() == null)
          || CmsStringUtil.isEmptyOrWhitespaceOnly(entry.getValue().toString())) {
        // skip empty entries
        continue;
      }
      if (!entry.getValue().getClass().equals(String.class)) {
