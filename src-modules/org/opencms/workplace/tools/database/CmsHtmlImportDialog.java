 ...
 *
 * <p>WARNING: If the zip file is to great to upload, then only a log entry is created from the
 * following method and this dialog is only refreshed:
 *
 * <p>{@link org.opencms.util.CmsRequestUtil#readMultipartFileItems(HttpServletRequest)}.
 *
 * <p>There are three modes to show the dialog:
 *
 * <p>
 *
 * <ul>
 ...
 *       <ul>
 *         <li>HTTP-Upload is not shown.
 *         <li>default values are saved by action commit.
 *       </ul>
 *   <li>{@link #MODE_STANDARD}
 *       <ul>
 *         <li>HTTP-Upload is shown.
 *         <li>the HTML files would be imported by action commit.
 *       </ul>
 *   <li>{@link #MODE_ADVANCED}
 *       <ul>
 *         <li>This dialog is needed for the advanced button in the new Dialog for the user.
 *         <li>HTTP-Upload is shown.
 *         <li>DestinationDir is not shown.
 *         <li>InputDir is not shown.
 *         <li>the HTML files would be imported by action commit.
 *       </ul>
 * </ul>
 */
public class CmsHtmlImportDialog extends CmsWidgetDialog {
  ...
  protected static final String IMPORT_ACTION_REPORT =
      PATH_WORKPLACE + "admin/database/reports/htmlimport.jsp";
  ...
  /**
   * This must be overwrite, because we need additional the 'enctype' parameter.
   *
   * <p>
   *
  ...
  /**
   * This function fills the <code> {@link CmsHtmlImport} </code> Object based on the values in the
  ...
  protected void fillHtmlImport() {
    ...
    CmsExtendedHtmlImportDefault extimport =
        OpenCms.getImportExportManager().getExtendedHtmlImportDefault();
  ...
  /**
   * @see
   *     org.opencms.workplace.CmsWidgetDialog#fillWidgetValues(javax.servlet.http.HttpServletRequest)
   */
  @Override
    ...
    if (getMultiPartFileItems() != null) {
      parameters =
          CmsRequestUtil.readParameterMapFromMultiPart(
              getCms().getRequestContext().getEncoding(), getMultiPartFileItems());
    } else {
  ...
  /**
   * This function creates a <code> {@link CmsWidgetDialogParameter} </code> Object based on the
   * given properties.
   *
   * <p>
   *
  ...
  /**
   * This function fills the <code> {@link CmsExtendedHtmlImportDefault} </code> Object based on the
   * current values in the dialog.
   *
   * <p>
   */
  ...
  /**
   * Returns a list with all available local's.
   *
   * <p>
   *
  ...
  /**
   * Checks if given mode is to show.
   *
   * <p>
   *
  ...
  /**
   * This function reads the file item and if its exits then the file is saved in the temporary
  ...
  /**
   * Checks if a multipart-request file item exists and returns it.
   *
   * <p>
   *
