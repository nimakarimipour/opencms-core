...
/**
 * This class implements Html-converting routines based on tidy to modify the Html code of the
 * imported Html pages.
 *
 * <p>
 *
...
public class CmsHtmlImportConverter {
   ...
   *
   * <p>This method should be pretty robust and work even if the input HTML does not contains the
   * specified matchers.
   *
   * <p>
   *
  ...
  public void convertHTML(
      Reader input, Writer output, String startPattern, String endPattern, Hashtable properties) {
    ...
    try {
      ...
      if (CmsLog.INIT.isWarnEnabled()) {
        CmsLog.INIT.warn(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_HTMLIMPORT_CONVERSION_ERROR_0, e.getLocalizedMessage()));
      }
      return;
    }
    ...
    if (CmsStringUtil.isNotEmpty(startPattern) && CmsStringUtil.isNotEmpty(endPattern)) {
      ...
      if (extractMain.length() != outString.length()) {
        ...
        // String extractHead = extractHtml(extractMain, "<html>",
        // CmsStringUtil.C_BODY_START_REGEX);
    ...
    if (m_tidy.getParseErrors() != 0) {
      if (CmsLog.INIT.isWarnEnabled()) {
        CmsLog.INIT.warn(
            Messages.get().getBundle().key(Messages.LOG_HTMLIMPORT_CONVERSION_ERROR_0));
      }
    }
    ...
    try {
      ...
      if (CmsLog.INIT.isWarnEnabled()) {
        CmsLog.INIT.warn(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_HTMLIMPORT_CONVERSION_ERROR_1, e.getLocalizedMessage()));
      }
      return;
    }
  }
  ...
  /**
   * Transform element nodes and create end tags in output.
   *
   * <p>
   *
  ...
  /**
   * Transforms element nodes and create start tags in output.
   *
   * <p>
   *
   * @param node actual element node
        ...
        for (int i = attrs.getLength() - 1; i >= 0; i--) {
          ...
          if (name.equals(ATTRIB_HREF)) {
            ...
            if (value.indexOf("://") > 0) {
              ...
              //                            if (value.indexOf("#") > 0) {
              //                                String anchor = value.substring(value.indexOf("#"),
              // value.length());
              //                            }
              // get the new link into the VFS
              String internalUri =
                  m_htmlImport.getAbsoluteUri(
                      value, m_filename.substring(0, m_filename.lastIndexOf("/") + 1));
  ...
  /**
   * Writes meta tags as cms properties by analyzing the meta tags nodes.
   *
   * <p>
   *
  ...
  /**
   * Sets the Property title by analyzing the title node.
   *
   * <p>
   *
  ...
  private void writeTitleProperty(Node node, Hashtable properties) {
    ...
    if ((title != null) && (title.length() > 0)) {
      ...
      properties.put(
          CmsPropertyDefinition.PROPERTY_TITLE, CmsStringUtil.substitute(title, "{subst}", "&#"));
