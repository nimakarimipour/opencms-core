...
public class CmsMergePages extends CmsReport {
   ...
   */
  public CmsMergePages(
      CmsObject cms, PageContext context, HttpServletRequest req, HttpServletResponse res) {
  ...
  public void actionMerge(I_CmsReport report) {
    ...
    m_report = report;
    m_report.println(
        Messages.get().container(Messages.RPT_MERGE_PAGES_BEGIN_0), I_CmsReport.FORMAT_HEADLINE);
   ...
   * <p>
   *
   * @param cms the current cms object If there were no error messages, the buffer is empty.
   *     <p>
   */
  public void validateParameters(CmsObject cms) {
    ...
    if (CmsStringUtil.isEmpty(getParamFolder1())) {
      validationErrors
          .append(messages.key(Messages.GUI_MERGE_PAGES_VALIDATE_FIRST_FOLDER_0))
          .append("<br>");
    } else {
      try {
        ...
        validationErrors
            .append(
                messages.key(Messages.GUI_MERGE_PAGES_VALIDATE_FIRST_FOLDER_1, getParamFolder1()))
            .append("<br>");
      }
    }
    if (CmsStringUtil.isEmpty(getParamFolder2())) {
      validationErrors
          .append(messages.key(Messages.GUI_MERGE_PAGES_VALIDATE_SECOND_FOLDER_0))
          .append("<br>");
    } else {
      try {
        ...
        validationErrors
            .append(
                messages.key(Messages.GUI_MERGE_PAGES_VALIDATE_SECOND_FOLDER_1, getParamFolder2()))
            .append("<br>");
      }
    }
    if (getParamFolder1().equals(getParamFolder2())) {
      validationErrors
          .append(messages.key(Messages.GUI_MERGE_PAGES_VALIDATE_SAME_FOLDER_0))
          .append("<br>");
    }
  ...
  /**
   * Analyses a page in the source morge folder and tests if a resouce with the same name exists in
   * the target merge folder.
   *
   * <p>The method then calcualtes a action for further processing of this page, possible values
   * are:
   *
   * <ul>
   *   <li>C_FOLDER1_EXCLUSIVE: exclusivly found in folder 1
   *   <li>C_FOLDER2_EXCLUSIVE: exclusivly found in folder 2
   *   <li>C_FOLDERS_SIBLING: found in both folders as siblings of each other
   *   <li>C_FOLDERS_EQUALNAMES: found in both folders as individual resources
   *   <li>C_FOLDERS_DIFFERENTTYPES: found in both folders as different types
   * </ul>
   *
   ...
   * @param currentFolder integer value (1 or 2) showing if the source folder is folder 1 or folder
   *     2
   * @return value of the action to do with this page
   */
  private int analyse(
      CmsResource res, String sourceMergeFolder, String targetMergefolder, int currentFolder) {

    int retValue = -1;
    String resourcenameOther =
        getResourceNameInOtherFolder(m_cms.getSitePath(res), sourceMergeFolder, targetMergefolder);
    try {
      CmsResource otherRes =
          m_cms.readResource(resourcenameOther, CmsResourceFilter.IGNORE_EXPIRATION);
      ...
      if (res.getResourceId().equals(otherRes.getResourceId())) {
        ...
        if (res.getTypeId() == otherRes.getTypeId()) {
          // both resources have the same type, so set the action to  "same name". Only those
          // resources can be merged
  ...
  /**
   * Collect all pages in a folders and sort them depending on the required action to do.
   *
   * <p>
   *
  ...
  private void collectFolder(String sourceMergeFolder, String targetMergefolder, int currentFolder)
    ...
    m_report.println(
        Messages.get()
            .container(
                Messages.RPT_SCAN_PAGES_IN_FOLDER_BEGIN_2, sourceMergeFolder, new Integer(size)),
    ...
    while (i.hasNext()) {
      ...
      m_report.println(
          Messages.get().container(Messages.RPT_PROCESS_1, resName), I_CmsReport.FORMAT_NOTE);
      ...
      switch (action) {
        case FOLDER1_EXCLUSIVE:
          ...
          m_report.println(
              Messages.get().container(Messages.RPT_FOLDER1_EXCLUSIVE_0), I_CmsReport.FORMAT_OK);
          break;
        case FOLDER2_EXCLUSIVE:
          m_folder2Exclusive.add(resName);
          m_report.println(
              Messages.get().container(Messages.RPT_FOLDER2_EXCLUSIVE_0), I_CmsReport.FORMAT_OK);
          break;
        case FOLDERS_SIBLING:
          ...
          m_report.println(
              Messages.get().container(Messages.RPT_FOLDERS_SIBLING_0), I_CmsReport.FORMAT_OK);
          break;
        case FOLDERS_EQUALNAMES:
          ...
          m_report.println(
              Messages.get().container(Messages.RPT_FOLDERS_EQUALNAMES_0), I_CmsReport.FORMAT_OK);
          break;
  ...
  /**
   * Collect all pages in the folders to merge and sort them depending on the required action to do.
   *
   * <p>The method will create several lists. Each list contains the resource names of pages and
   * will be used in further steps of the merging process.
   *
   * <ul>
   *   <li>List m_folder1Exclusive: contains all pages which are exclusivly found in folder 1
   *   <li>List m_folder2Exclusive: contains all pages which are exclusivly found in folder 2
   *   <li>List m_foldersSibling: contains all pages which can be found in both folders and are
   *       siblings of each other
   *   <li>List m_foldersEqualnames: contains all pages which can be found in both folders and are
   *       no siblings of each other
   *   <li>List m_foldersDifferenttypes: contains all pages which can be found in both folders but
  ...
  private void collectResources() throws CmsException {
    ...
    String locale1 =
        m_cms.readPropertyObject(getParamFolder1(), "locale", true).getValue(defaultLocale);
    String locale2 =
        m_cms.readPropertyObject(getParamFolder2(), "locale", true).getValue(defaultLocale);
    m_report.println(
    ...
    m_report.println(
        Messages.get().container(Messages.RPT_SCANNING_RESULTS_0), I_CmsReport.FORMAT_HEADLINE);

    m_report.println(
        Messages.get().container(Messages.RPT_FOLDER1_EXCLUSIVE_0), I_CmsReport.FORMAT_HEADLINE);
    reportList(m_folder1Exclusive, false);
    m_report.println(
        Messages.get().container(Messages.RPT_FOLDER2_EXCLUSIVE_0), I_CmsReport.FORMAT_HEADLINE);
    reportList(m_folder2Exclusive, false);
    m_report.println(
        Messages.get().container(Messages.RPT_FOLDERS_SIBLING_0), I_CmsReport.FORMAT_HEADLINE);
    reportList(m_foldersSibling, false);
    m_report.println(
        Messages.get().container(Messages.RPT_FOLDERS_EQUALNAMES_0), I_CmsReport.FORMAT_HEADLINE);
  ...
  /**
   * Gets the name of a resource in the other merge folder.
   *
   * <p>
   *
   ...
   */
  private String getResourceNameInOtherFolder(
  ...
  /**
   * Merges those pages in the two merge folders that have the same name and are no siblings of each
   * other yet.
   *
   * <p>
   *
  ...
  private void mergePages() throws CmsException {
    ...
    if (size > 0) {
      ...
      m_report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, getParamFolder1()));
      m_report.print(
      ...
      m_report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, getParamFolder2()));
      m_report.print(
          org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0));
      ...
      while (i.hasNext()) {
        ...
        try {
          String resFolder2Name =
              getResourceNameInOtherFolder(resFolder1Name, getParamFolder1(), getParamFolder2());
          m_report.print(
              org.opencms.report.Messages.get()
                  .container(
                      org.opencms.report.Messages.RPT_SUCCESSION_2,
          ...
          m_report.print(
              org.opencms.report.Messages.get()
                  .container(org.opencms.report.Messages.RPT_ARGUMENT_1, resFolder1Name));
          m_report.print(
              Messages.get().container(Messages.RPT_DOUBLE_ARROW_0), I_CmsReport.FORMAT_NOTE);
          m_report.print(
              org.opencms.report.Messages.get()
                  .container(org.opencms.report.Messages.RPT_ARGUMENT_1, resFolder2Name));
          m_report.println(
              org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0));

          // get the content of the resource in folder1
          String locale =
              m_cms.readPropertyObject(resFolder1Name, "locale", true).getValue(defaultLocale);
          m_report.print(
              Messages.get().container(Messages.RPT_READ_CONTENT_2, resFolder1Name, locale),
              I_CmsReport.FORMAT_NOTE);
          CmsResource resFolder1 =
              m_cms.readResource(resFolder1Name, CmsResourceFilter.IGNORE_EXPIRATION);
          ...
          m_report.print(
              ...
              I_CmsReport.FORMAT_NOTE);
          CmsResource resFolder2 =
              m_cms.readResource(resFolder2Name, CmsResourceFilter.IGNORE_EXPIRATION);
          ...
          // now get all the text elements from the resource in folder 2 which match the the locale
          // of folder 2
          ...
          // the resource in folder 1 now has all text elements in both locales, so update it in the
          // vfs
      ...
      m_report.println(
          Messages.get().container(Messages.RPT_MERGE_PAGES_END_0), I_CmsReport.FORMAT_HEADLINE);
    }
  }

  /**
   * Creates a report list of all resources in one of the collected lists.
   *
   * <p>
   *
