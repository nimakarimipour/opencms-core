...
public final class CmsTagReplaceSettings {
  ...
  /**
   * The value of the shared {@link #PROPERTY_CONTENTOOLS_TAGREPLACE} to set on resources that have
   * been processed with these settings.
   */
  private String m_propertyValueTagReplaceID;

  /**
   * A map containing lower case tag names of tags to replace as keys and the replacement tag names
  ...
  /**
   * Sets the value of the shared {@link #PROPERTY_CONTENTOOLS_TAGREPLACE} to set on resources that
   * have been processed with these settings.
   *
   * <p>
   *
   * @param propertyValueTagreplaceID the value of the shared {@link
   *     #PROPERTY_CONTENTOOLS_TAGREPLACE} to set on resources that have been processed with these
   *     settings.
   * @throws CmsIllegalArgumentException if the argument is not valid.
   */
  public void setPropertyValueTagReplaceID(String propertyValueTagreplaceID)
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(value)) {
        ...
        try {
          ...
          CmsMessageContainer container =
              Messages.get().container(Messages.GUI_ERR_TAGREPLACE_TAGNAME_INVALID_1, tagName);
  ...
  public void setWorkPath(String workPath) throws CmsIllegalArgumentException {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(workPath)) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.GUI_ERR_WIDGETVALUE_EMPTY_0));
    }
        ...
        if (CmsStringUtil.isEmptyOrWhitespaceOnly(attName)) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_TAGREPLACE_TAG_REMOVE_ATTRIB_OK_0));
          }
        }
      }
    }
  ...
  /**
   * Computes the default property value for resources that have to be marked as "processed by these
   * replacement settings".
   *
   * <p>The default value will be the alphabetically sorted string for replacments or the empty
   * String if the replacements have not been set before.
   *
   * <p>
   *
   * @return the default property value for resources that have to be marked as "processed by these
   *     replacement settings".
   */
