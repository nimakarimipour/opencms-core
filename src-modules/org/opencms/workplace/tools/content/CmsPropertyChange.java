...
public class CmsPropertyChange extends CmsDialog {
  ...
  public static String buildSelectProperty(
      CmsObject cms, String selectValue, String attributes, String selectedValue) {
  ...
  public String buildResultList() {
    ...
    if ((getChangedResources() != null) && (getChangedResources().size() > 0)) {
      ...
      result.append(
          Messages.get()
              .getBundle(getLocale())
              .key(Messages.GUI_INPUT_PROPERTYCHANGE_RESULT_NONE_0));
    }
  ...
  /**
   * Returns the changed resources that were affected by the property change action.
   *
   * <p>
   *
  ...
  /**
   * Performs the main property change value operation on the resource property.
   *
   * <p>
   *
  ...
  private boolean performChangeOperation(boolean recursive) throws CmsException {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(getParamOldValue())) {
      ...
      changedResources =
          setPropertyInFolder(
              getParamResource(), getParamPropertyName(), getParamNewValue(), recursive);
    }
  ...
  /**
   * Sets the given property with the given value to the given resource (potentially recursiv) if it
   * has not been set before.
   *
   * <p>Returns a list with all sub resources that have been modified this way.
   *
   * <p>
   *
  ...
  private List setPropertyInFolder(
      String resourceRootPath, String propertyDefinition, String newValue, boolean recursive)
    ...
    for (int i = 0; i < resources.size(); i++) {
      ...
      if (property.isNullProperty()) {
        ...
        cms.writePropertyObject(
            cms.getRequestContext().removeSiteRoot(res.getRootPath()), newProperty);
  ...
  /**
   * Sets the changed resources that were affected by the property change action.
   *
   * <p>
   *
  ...
  /**
   * Validates the submitted form parameters.
   *
   * <p>If parameters are missing, a localized error message String is created.
   *
   * <p>
   *
  ...
  private boolean validateParameters() {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(getParamResource())
        || !getCms().existsResource(getParamResource())) {
      allOk = false;
      validationErrors
          .append(messages.key(Messages.GUI_PROP_CHANGE_VALIDATE_VFS_RESOURCE_0))
          .append("<br>");
    }
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(getParamOldValue())) {
      allOk = false;
      validationErrors
          .append(messages.key(Messages.GUI_PROP_CHANGE_VALIDATE_OLD_PROP_VALUE_0))
          .append("<br>");
    } else {
      try {
        ...
        allOk = false;
        validationErrors
            .append(messages.key(Messages.GUI_PROP_CHANGE_VALIDATE_OLD_PROP_PATTERN_0))
            .append("<br>");
      }
    }
