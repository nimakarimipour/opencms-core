 ...
 *
 * <p>Caution: The list ID argument has to be dynamic to prevent caching causing exception in case
 * of varying collumns.
 *
 * <p>
 *
...
public class CmsPropertyviewList extends A_CmsListDialog {
   ...
   */
  public CmsPropertyviewList(CmsJspActionElement jsp) throws FileNotFoundException, CmsException {
      ...
      if (m_siblings) {
        try {
          List<CmsResource> siblings =
              cms.readSiblings(cms.getSitePath(resource), CmsResourceFilter.ALL);
          ...
          if (LOG.isErrorEnabled()) {
            LOG.error(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_ERR_PROPERTYVIEWER_READSIBL_1, resource.getRootPath()),
                e);
          }
        }
      }
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    for (String property : m_props) {
      ...
      propCol.setName(
          Messages.get()
              .container(
                  Messages.GUI_LIST_PROPERTYVIEW_COL_PROPERTY_NAME_1, new Object[] {property}));
      propCol.setHelpText(
          Messages.get().container(Messages.GUI_LIST_PROPERTYVIEW_COL_PROPERTY_HELP_0));
   ...
   * @return true if the item contains at least one property with content, false if there is no
   *     property with content
   */
  private boolean fillItem(
      final CmsResource resource, final CmsListItem item, final boolean isSibling, final int id) {
  ...
  /**
   * Internally reads the resources to use.
   *
   * <p>
   *
  ...
  private List<CmsResource> getResources() {
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get().getBundle().key(Messages.LOG_ERR_PROPERTYVIEWER_READRESOURCES_0), e);
      }
  ...
  /**
   * Checks if the resource is in the selected path.
   *
   * <p>
   *
