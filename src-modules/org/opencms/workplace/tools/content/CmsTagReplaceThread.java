...
public class CmsTagReplaceThread extends A_CmsReportThread {
  ...
  public void run() {
    ...
    getReport()
        .print(org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_STAT_0));
    getReport()
        .println(
    ...
    getReport()
        .println(
            Messages.get().container(Messages.RPT_TAGREPLACE_END_0), I_CmsReport.FORMAT_HEADLINE);
  }

  /**
   * Checks the shared property {@link CmsTagReplaceSettings#PROPERTY_CONTENTOOLS_TAGREPLACE} if it
   * has the value of this configuration ({@link
  ...
  private boolean isProcessedBefore(CmsResource resource) throws CmsException {
    ...
    CmsProperty testProp =
        getCms()
            .readPropertyObject(
                resource, CmsTagReplaceSettings.PROPERTY_CONTENTOOLS_TAGREPLACE, false);
  ...
  private void replaceTags() throws CmsException {
    ...
    report.print(
        Messages.get()
            .container(Messages.RPT_TAGREPLACE_READ_RESOURCES_1, m_settings.getWorkPath()));
    report.print(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.RPT_TAGREPLACE_READ_RESOURCES_1, m_settings.getWorkPath()));
    }
    CmsResourceFilter filter =
        CmsResourceFilter.ALL.addRequireType(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_TAGREPLACE_READ_RESOURCES_OK_1, m_settings.getWorkPath()));
    }
    report.println(
   ...
   *
   * <p>No modifications will be done:
   *
   * <ol>
   *   <li>the resource is locked by another user.
   *   <li>the special marker property with the value that stands for the replacement configuration
   *       is set on the resource (shared).
   *   <li>locking of the non-locked resource fails.
   *   <li>Loading of the content fails.
   *   <li>Unmarshalling fails.
   *   <li>Unexpected exception while replacing occur.
   *   <li>Marshalling of XML fails.
   *   <li>Writing of the marker property fails.
   *   <li>Writing of the file fails.
   * </ol>
   *
   * <p>
   *
   ...
   */
  private void replaceTags(CmsResource resource, Integer totalJobCount, Integer actualJobCount)
      throws CmsException {
    ...
    report.print(
        org.opencms.report.Messages.get()
            .container(
                org.opencms.report.Messages.RPT_SUCCESSION_2, actualJobCount, totalJobCount));
    report.print(
        Messages.get()
            .container(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_TAGREPLACE_LOCK_RESOURCE_1, resource.getRootPath()));
    }
    try {
      // checking the lock:
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_TAGREPLACE_LOCK_READ_1, resource.getRootPath()));
      }
      ...
      boolean myLock =
          !lock.isNullLock() && lock.isOwnedBy(getCms().getRequestContext().getCurrentUser());
      if (lock.isNullLock() || myLock) {
        if (!myLock) {
          ...
          // obtaining the lock:
          getCms()
              .lockResource(getCms().getRequestContext().removeSiteRoot(resource.getRootPath()));
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_TAGREPLACE_LOCK_RESOURCE_OK_1, resource.getRootPath()));
          }
        }
      } else {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_TAGREPLACE_RESOURCE_SKIPPED_1, resource.getRootPath()));
        ...
        try {
          ...
          report.println(
              Messages.get()
                  .container(Messages.RPT_TAGREPLACE_SKIP_REASON_LOCKED_1, locker.getName()),
              I_CmsReport.FORMAT_WARNING);
        } catch (Throwable f) {
          report.println(
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    ...
                    .key(Messages.LOG_DEBUG_TAGREPLACE_RESOURCE_SKIPPED_1, resource.getRootPath()));
            LOG.debug(
                Messages.get().getBundle().key(Messages.RPT_TAGREPLACE_SKIP_REASON_ERR_LOCK_0));
          }
        }
        return;
      }
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_WARN_TAGREPLACE_LOCK_RESOURCE_FAILED_1, resource.getRootPath()),
            e);
      }
      report.print(
          org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_SKIPPED_0),
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_TAGREPLACE_LOAD_FILE_1, resource.getRootPath()));
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_TAGREPLACE_LOAD_FILE_OK_1, resource.getRootPath()));
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_TAGREPLACE_UNMARSHAL_1, resource.getRootPath()));
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_TAGREPLACE_UNMARSHAL_OK_1, resource.getRootPath()));
    }
    ...
    while (itLocales.hasNext()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_TAGREPLACE_LOCALE_1, locale.getLanguage()));
      }
      ...
      while (itElements.hasNext()) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_TAGREPLACE_ELEMENT_2, value.getPath(), content));
        }
        try {
    ...
    if (parser.isChangedContent()) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_TAGREPLACE_MARSHAL_1, resource.getRootPath()));
      }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_TAGREPLACE_MARSHAL_OK_1, resource.getRootPath()));
      }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_TAGREPLACE_WRITE_1, resource.getRootPath()));
      }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_TAGREPLACE_WRITE_OK_1, resource.getRootPath()));
      }
      ...
      try {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_DEBUG_TAGREPLACE_PROPERTY_WRITE_OK_0));
        }
        report.println(
            org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_OK_0),
            I_CmsReport.FORMAT_OK);
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .container(Messages.LOG_DEBUG_TAGREPLACE_UNLOCK_FILE_1, resource.getRootPath()));
      }
