...
/**
 * This implementation of the I_CmsContentCheck interface implements a check for resource
 * properties.
 *
 * <p>The following items can be configured and checked:
 *
 * <ul>
 *   <li>Property not set
 *   <li>Property value contains filename
 *   <li>Property value is shorter than a minimum size
 *   <li>Property value contains a given value (with RegEx)
 *   <li>Property value does not contain a given value (with RegEx)
 * </ul>
 *
 * @since 6.1.2
 */
public class CmsContentCheckProperty extends A_CmsContentCheck {
  ...
  private static final String CONFIGURATION =
      CmsContentCheck.VFS_PATH_PLUGIN_FOLDER + "propertycheck/configuration.xml";
  ...
  private static final String ICONPATH =
      "tools/contenttools/icons/big/contentcheck_property_configuration.png";
  ...
  /**
   * @see
   *     org.opencms.workplace.tools.content.check.I_CmsContentCheck#executeContentCheck(org.opencms.file.CmsObject,
   *     org.opencms.workplace.tools.content.check.CmsContentCheckResource)
   */
  @Override
  public CmsContentCheckResource executeContentCheck(
      CmsObject cms, CmsContentCheckResource testResource) throws CmsException {
  ...
  /**
   * @see
   *     org.opencms.workplace.tools.content.check.I_CmsContentCheck#init(org.opencms.file.CmsObject)
   */
  @Override
   ...
   *
   * <p>This method is required to build the widget dialog frontend.
   *
  ...
  /**
   * Gets the configuration of the property check.
   *
   * <p>
   *
    ...
    if ((m_configuredErrorChecks == null) || (m_configuredWarningChecks == null)) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_PROPERTY_CONFIG_FILENAME_1, res.getRootPath()));
      }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_PROPERTY_CONFIG_FILE_1, new String(file.getContents())));
      }
  ...
  /**
   * Reads the configuration for a given xpath and stored all results in a list.
   *
   * <p>
   *
  ...
  private List getConfiguredChecks(CmsXmlContent configuration, String xpath) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_PROPERTY_CONFIG_XPATH_2, xpath, m_locale));
    }
    ...
    for (int i = 1; i <= size; i++) {
      ...
      String propertyname =
          configuration
              .getValue(xpath + "[" + i + "]/" + XPATH_PROPERTYNAME, m_locale)
              .getStringValue(m_cms);
      String type =
          configuration
              .getValue(xpath + "[" + i + "]/" + XPATH_TYPE, m_locale)
              .getStringValue(m_cms);
      String empty =
          configuration
              .getValue(xpath + "[" + i + "]/" + XPATH_EMPTY, m_locale)
              .getStringValue(m_cms);
      String filename =
          configuration
              .getValue(xpath + "[" + i + "]/" + XPATH_FILENAME, m_locale)
              .getStringValue(m_cms);
      String length =
          configuration
              .getValue(xpath + "[" + i + "]/" + XPATH_LENGTH, m_locale)
              .getStringValue(m_cms);
      int values = configuration.getIndexCount(xpath + "[" + i + "]/" + XPATH_VALUE, m_locale);

      // String value = configuration.getValue(xpath + "[" + i + "]/" + XPATH_VALUE,
      ...
      if (values > 0) {
        ...
        for (int j = 1; j <= values; j++) {
          String value =
              configuration
                  .getValue(xpath + "[" + i + "]/" + XPATH_VALUE + "[" + j + "]", m_locale)
                  .getStringValue(m_cms);
  ...
  /**
   * Processes a list of CmsContentCheckProperetyObject and runs all available tests on them.
   *
   * <p>All errors or warnings found are collected in a list returned to the calling method.
   *
  ...
  private List processProperties(List properties, CmsContentCheckResource testResource) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DEBUG_PROPERTY_RESOURCE_1, testResource.getResourceName()));
    }

    // loop through all property tests
    for (int i = 0; i < properties.size(); i++) {
      try {
        CmsContentCheckPropertyObject propObject =
            (CmsContentCheckPropertyObject) properties.get(i);

        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_PROPERTY_PROPERTY_1, propObject.toString()));
        }
        ...
        if ((propObject.getType().equals(CmsContentCheckPropertyObject.TYPE_BOTH))
          ...
          String prop =
              m_cms
                  .readPropertyObject(
                      testResource.getResource(), propObject.getPropertyname(), false)
                  .getValue();
          ...
          if (propObject.isEmpty() && CmsStringUtil.isEmpty(prop)) {
            results.add(
                Messages.get()
                    .getBundle()
                    .key(Messages.ERR_CHECK_NO_PROPERTYNAME_1, propObject.getPropertyname()));
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  Messages.get()
                      .getBundle()
              ...
              for (int j = 0; j < propObject.getValue().size(); j++) {
                ...
                if (LOG.isDebugEnabled()) {
                  LOG.debug(
                      Messages.get()
                          .getBundle()
                          .key(Messages.LOG_DEBUG_PROPERTY_MATCHPATTERN_2, regex, matchValue));
                }
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(
                    Messages.LOG_ERROR_PROCESSING_PROPERTIES_2, testResource.getResourceName(), e));
      }
    }
