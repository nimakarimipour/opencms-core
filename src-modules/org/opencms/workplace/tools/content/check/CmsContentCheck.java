...
/**
 * Main implementation of the content check. It maintains all configured content check plugins and
...
public class CmsContentCheck {
  ...
  public static final String VFS_PATH_PLUGIN_FOLDER =
      CmsWorkplace.VFS_PATH_WORKPLACE + "admin/contenttools/check/plugin/";
   ...
   * <p>
   *
   * @return CmsContentCheckResult object containing all resources that collected an error or
   *     warning
   */
  ...
  public void startContentCheck(CmsObject cms, I_CmsReport report) throws Exception {
    ...
    m_report = report;
    m_report.println(
        Messages.get().container(Messages.RPT_CONTENT_CHECK_BEGIN_0), I_CmsReport.FORMAT_HEADLINE);

    // collect all resources
    m_report.print(
        Messages.get().container(Messages.RPT_CONTENT_COLLECT_BEGIN_0),
    ...
    m_report.println(
        Messages.get()
            .container(Messages.RPT_CONTENT_COLLECT_END_1, new Integer(m_resources.size())),
        I_CmsReport.FORMAT_HEADLINE);

    // now process all resources
    m_report.print(
        Messages.get().container(Messages.RPT_CONTENT_PROCESS_BEGIN_0),
        I_CmsReport.FORMAT_HEADLINE);
    m_report.println(
        org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0),
        I_CmsReport.FORMAT_HEADLINE);
      ...
      while (j.hasNext()) {
        ...
        if (plugin.isActive()) {
          try {
            ...
            errorWarning = true;
            m_report.println(
                Messages.get().container(Messages.RPT_EMPTY_0), I_CmsReport.FORMAT_DEFAULT);
            m_report.print(
    ...
    m_report.print(
        ...
        I_CmsReport.FORMAT_HEADLINE);
    m_report.println(
        Messages.get().container(Messages.RPT_CONTENT_PROCESS_END_0), I_CmsReport.FORMAT_HEADLINE);

    m_report.println(
        Messages.get().container(Messages.RPT_CONTENT_CHECK_END_0), I_CmsReport.FORMAT_HEADLINE);
  }
  ...
  /**
   * Collects all resources required for the content checks and stores them in a Set.
   *
   * <p>The collection of resources is build based on the vfs paths stored in this object. To
   * prevent that resources are collected multiple times (in case of overlapping vfs paths), the
   * results will be stored as a map.
   *
   * @param cms the CmsObject
    ...
    while (i.hasNext()) {
      ...
      try {
        ...
        while (j.hasNext()) {
          ...
          //                    m_report.print(
          //                        Messages.get().container(Messages.RPT_EXTRACT_FROM_PATH_1,
          // cms.getSitePath(res)),
          //                        I_CmsReport.FORMAT_DEFAULT);
          //                    m_report.print(
          //
  ...
  /**
   * Initializes the CmsContent check and reads all available plugins.
   *
   * <p>
   *
  ...
  private void init() throws CmsException {
    ...
    List resources =
        m_cms.readResourcesWithProperty(
            VFS_PATH_PLUGIN_FOLDER, CmsToolManager.HANDLERCLASS_PROPERTY);
    ...
    while (i.hasNext()) {
      ...
      if (res.isFolder()) {
        String classname =
            m_cms
                .readPropertyObject(res.getRootPath(), CmsToolManager.HANDLERCLASS_PROPERTY, false)
                .getValue();
        try {
          ...
          if (objectInstance instanceof I_CmsContentCheck) {
            ...
            LOG.warn(
                Messages.get().getBundle().key(Messages.LOG_CANNOT_CREATE_PLUGIN_1, classname));
          }
        } catch (Throwable t) {
          LOG.error(
              Messages.get().getBundle().key(Messages.LOG_CANNOT_CREATE_PLUGIN_2, classname, t));
        }
      }
    }
  }
