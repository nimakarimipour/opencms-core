...
public class CmsContentCheckDialog extends CmsWidgetDialog {
  ...
  protected static final String CONTENT_CHECK_REPORT =
      PATH_WORKPLACE + "admin/contenttools/check/report.jsp";

  /** The content check JSP result workplace URI. */
  protected static final String CONTENT_CHECK_RESULT =
      PATH_WORKPLACE + "admin/contenttools/check/result.jsp";
        ...
        while (i.hasNext()) {
          ...
          if (!getCms().existsResource(path)) {
            throw new CmsIllegalArgumentException(
                Messages.get().container(Messages.ERR_NO_VFSPATH_0));
          }
        }
      }
  ...
  protected void defineWidgets() {
    ...
    initContentCheck();
    addWidget(
        new CmsWidgetDialogParameter(m_contentCheck, "paths", PAGES[0], new CmsVfsFileWidget()));
  ...
  protected void initWorkplaceRequestValues(
    ...
    // save the current state of the content check object (may be changed because of the widget
    // values)
    setDialogObject(m_contentCheck);
  }
}
