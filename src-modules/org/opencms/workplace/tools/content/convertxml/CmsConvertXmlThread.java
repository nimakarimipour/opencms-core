...
public class CmsConvertXmlThread extends A_CmsReportThread {
  ...
  private boolean lockResource(
      CmsObject cms, CmsProject cmsProject, CmsResource cmsResource, I_CmsReport report)
      throws CmsException {
  ...
  /**
   * Main method to transforms xml contents from files of one format because of new xsd file.
   *
   * <p>
   *
  ...
  private boolean mainTransform(
    ...
    report.println(
        Messages.get().container(Messages.RPT_CONVERTXML_BEGIN_TRANSFORM_0),
        I_CmsReport.FORMAT_NOTE);
    report.println(
        Messages.get().container(Messages.RPT_CONVERTXML_PARAMETERS_0), I_CmsReport.FORMAT_NOTE);
    report.println(
    ...
    if (CmsStringUtil.isEmpty(xsltFile)) {
      report.println(
          Messages.get().container(Messages.RPT_CONVERTXML_NO_XSLT_FILE_0),
    ...
    // get main xsd file string in xml content in format:
    // xsi:noNamespaceSchemaLocation="opencms://system/modules/org.opencms.frontend.templatetwo.demo/schemas/article.xsd"
    int posMainFileBegin = xsltString.indexOf("xsi:noNamespaceSchemaLocation=\"");
    if (posMainFileBegin > 0) {
      String fileName =
          xsltString.substring(posMainFileBegin + "xsi:noNamespaceSchemaLocation=\"".length());
    ...
    if ((fileLength < 5)
        || !newXsdMainFile.substring(fileLength - 4, fileLength).toUpperCase().equals(".XSD")) {
      newXsdMainFile = "";
    }
    if (CmsStringUtil.isEmpty(newXsdMainFile)) {
      report.println(
          Messages.get().container(Messages.RPT_CONVERTXML_NO_XSD_FILE_0),
          I_CmsReport.FORMAT_ERROR);
      transformConditions = false;
    } else {
      report.println(
          Messages.get()
              .container(Messages.RPT_CONVERTXML_PARAMETERS_NEW_XSD_MAINFILE_1, newXsdMainFile),
          I_CmsReport.FORMAT_NOTE);
    }
    // check if vfs folder is set
    if (CmsStringUtil.isEmpty(resourcePath)) {
      report.println(
          Messages.get().container(Messages.RPT_CONVERTXML_NO_VFS_FOLDER_0),
          I_CmsReport.FORMAT_ERROR);
      transformConditions = false;
    }
    ...
    report.println(
        Messages.get().container(Messages.RPT_CONVERTXML_START_SEARCHING_0),
        I_CmsReport.FORMAT_NOTE);
    List<CmsResource> files2Transform = null;
    try {
      files2Transform =
          cmsObject.readResources(
              resourcePath, CmsResourceFilter.requireType(fileFormat), inclSubFolder);
    } catch (CmsException e) {
      m_errorTransform += 1;
      report.println(
          Messages.get().container(Messages.RPT_CONVERTXML_SEARCH_ERROR_0),
          I_CmsReport.FORMAT_ERROR);
      if (LOG.isErrorEnabled()) {
        LOG.error(e.getMessageContainer(), e);
      }
      report.println(
          Messages.get().container(Messages.RPT_CONVERTXML_TRANSFORM_END_0),
          I_CmsReport.FORMAT_NOTE);
      return false;
    }
    ...
    if (files2Transform != null) {
      ...
      report.println(
          Messages.get()
              .container(
                  Messages.RPT_CONVERTXML_FOUND_FILES_1, new Integer(file2Transform).toString()),
          I_CmsReport.FORMAT_OK);
    } else {
      report.println(
          Messages.get().container(Messages.RPT_CONVERTXML_NO_FILES_FOUND_0),
          I_CmsReport.FORMAT_OK);
      return false;
    }
    ...
    CmsObject cmsObject2Publish =
        transformAndWriteFiles(files2Transform, xsltFile, cmsObject, newXsdMainFile, report);
    // publish files in project
    report.println(
        Messages.get().container(Messages.RPT_CONVERTXML_PUBLISHING_FILES_0),
        I_CmsReport.FORMAT_NOTE);
    ...
    report.println(
        Messages.get().container(Messages.RPT_CONVERTXML_RESULT_0), I_CmsReport.FORMAT_NOTE);
    report.println(
        Messages.get()
            .container(
                Messages.RPT_CONVERTXML_FOUND_FILES_1, new Integer(file2Transform).toString()),
    ...
    report.println(
        Messages.get()
            .container(
                Messages.RPT_CONVERTXML_LOCKED_FILES_1, new Integer(m_lockedFiles).toString()),
        I_CmsReport.FORMAT_NOTE);
    ...
    report.println(
        Messages.get().container(Messages.RPT_CONVERTXML_TRANSFORM_END_0), I_CmsReport.FORMAT_NOTE);
    return transformSuccess;
  }

  /**
   * Gets file xml content.
   *
   * <p>
   *
    ...
    try {
      ...
      try {
        ...
        report.println(
            Messages.get()
                .container(
                    Messages.RPT_CONVERTXML_UNLOCK_FILE_1, cmsObject.getSitePath(cmsResource)),
            I_CmsReport.FORMAT_ERROR);
        if (LOG.isErrorEnabled()) {
          LOG.error(e.getMessageContainer(), e);
        }
    ...
    while (iter.hasNext()) {
        ...
        try {
          ...
          if (!lockResource(cms1, project2Publish, cmsResource, report)) {
            report.println(
                Messages.get()
                    .container(
                        Messages.RPT_CONVERTXML_LOCKED_FILE_1, cmsObject.getSitePath(cmsResource)),
                I_CmsReport.FORMAT_ERROR);
            continue;
          }
        } catch (CmsException e) {
      ...
      if (CmsStringUtil.isEmpty(encodingType)) {
        ...
        report.println(
            Messages.get()
                .container(
                    Messages.RPT_CONVERTXML_MISSION_ENCODING_TYPE_1, cmsResource.getRootPath()),
            I_CmsReport.FORMAT_ERROR);
        continue;
      }
      // already transformed?
      if (fileXmlContent.toUpperCase().contains(newXsdMainFile.toUpperCase())) {
        ...
        report.println(
            Messages.get()
                .container(
                    Messages.RPT_CONVERTXML_FILE_ALREADY_TRANSFORMED_1, cmsResource.getRootPath()),
            I_CmsReport.FORMAT_OK);
        continue;
      }
      ...
      try {
        String transformedXmlContent =
            CmsXsltUtil.transformXmlContent(cmsObject, xsltFile, fileXmlContent);
        transformedXmlContent =
            "<?xml version=\"1.0\" encoding=\""
                .concat(encodingType)
                .concat("\"?>")
                .concat(transformedXmlContent);
        ...
        if (resource2Publish) {
          report.println(
              ...
              I_CmsReport.FORMAT_OK);
          cms1.getRequestContext().setCurrentProject(project2Publish);
          setXmlContentFromFile(
              cmsResource, cmsFile, cms1, transformedXmlContent, encodingType, report);
        } else {
          report.println(
              Messages.get()
                  .container(
              ...
              I_CmsReport.FORMAT_OK);
          setXmlContentFromFile(
              cmsResource, cmsFile, cms2, transformedXmlContent, encodingType, report);
        }
