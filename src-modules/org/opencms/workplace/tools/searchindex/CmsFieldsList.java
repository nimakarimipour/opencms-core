...
/**
 * A list that displays the fields of a request parameter given <code>
 * {@link org.opencms.search.fields.CmsLuceneFieldConfiguration}</code> ("fieldconfiguration").
 *
 * <p>This list is no stand-alone page but has to be embedded in another dialog (see <code>
 *  {@link org.opencms.workplace.tools.searchindex.A_CmsEmbeddedListDialog}</code>.
 *
 * <p>
 *
 * @since 6.5.5
...
public class CmsFieldsList extends A_CmsEmbeddedListDialog {
  ...
  protected static final String LIST_ICON_FIELD_EDIT =
      "tools/searchindex/icons/small/fieldconfiguration-editfield.png";
  ...
  public void executeListMultiActions() {
    ...
    if (getParamListAction().equals(LIST_MACTION_DELETEFIELD)) {
      ...
      List<CmsSearchField> fields =
          searchManager.getFieldConfiguration(m_paramFieldconfiguration).getFields();
      ...
      while (itFields.hasNext()) {
        ...
        searchManager.removeSearchFieldConfigurationField(
            searchManager.getFieldConfiguration(m_paramFieldconfiguration), field);
      }
  ...
  public void executeListSingleActions() throws ServletException, IOException {
    ...
    I_CmsSearchFieldConfiguration fieldConfig =
        OpenCms.getSearchManager().getFieldConfiguration(m_paramFieldconfiguration);
    ...
    if (action.equals(LIST_ACTION_EDIT)) {
      ...
      getToolManager()
          .jspForwardTool(
              this, "/searchindex/fieldconfigurations/fieldconfiguration/field/edit", params);
    } else if (action.equals(LIST_ACTION_MAPPING)) {
      // forward to the new mapping screen
      getToolManager()
          .jspForwardTool(
              this, "/searchindex/fieldconfigurations/fieldconfiguration/field/newmapping", params);
    } else if (action.equals(LIST_ACTION_OVERVIEW_FIELD)) {
      // forward to the field configuration overview screen
      getToolManager()
          .jspForwardTool(
              this, "/searchindex/fieldconfigurations/fieldconfiguration/field", params);
  ...
  /**
   * Returns the request parameter "fieldconfiguration".
   *
   * <p>
   *
  ...
  /**
   * Sets the request parameter "fieldconfiguration".
   *
   * <p>Method intended for workplace-properietary automatic filling of request parameter values to
   * dialogs, not for manual invocation.
   *
   * <p>
   *
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    overviewAction.setHelpText(
        Messages.get().container(Messages.GUI_LIST_FIELD_COL_OVERVIEW_NAME_HELP_0));
    ...
    // true action
    CmsListDirectAction storeTrueAction =
        new CmsListDirectAction(LIST_ACTION_STORE_TRUE) {
    ...
    storeTrueAction.setHelpText(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_TRUE_HELP_0));
    storeTrueAction.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_TRUE_CONF_0));
    storeTrueAction.setIconPath(ICON_TRUE);
    // false action
    CmsListDirectAction storeFalseAction =
        new CmsListDirectAction(LIST_ACTION_STORE_FALSE) {
    ...
    storeFalseAction.setHelpText(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_FALSE_HELP_0));
    storeFalseAction.setConfirmationMessage(
    ...
    // true action
    CmsListDirectAction excerptTrueAction =
        new CmsListDirectAction(LIST_ACTION_EXCERPT_TRUE) {
    ...
    excerptTrueAction.setHelpText(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_TRUE_HELP_0));
    excerptTrueAction.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_TRUE_CONF_0));
    excerptTrueAction.setIconPath(ICON_TRUE);
    // false action
    CmsListDirectAction excerptFalseAction =
        new CmsListDirectAction(LIST_ACTION_EXCERPT_FALSE) {
    ...
    excerptFalseAction.setName(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_FALSE_NAME_0));
    excerptFalseAction.setHelpText(
        Messages.get().container(Messages.GUI_LIST_FIELD_ACTION_FALSE_HELP_0));
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    configDetails.setShowActionName(
        Messages.get().container(Messages.GUI_LIST_FIELD_DETAIL_MAPPINGS_SHOW_0));
    ...
    configDetails.setHideActionName(
        Messages.get().container(Messages.GUI_LIST_FIELD_DETAIL_MAPPINGS_HIDE_0));
    ...
    configDetails.setFormatter(
        new CmsListItemDetailsFormatter(
            Messages.get().container(Messages.GUI_LIST_FIELD_DETAIL_MAPPINGS_NAME_0)));
  ...
  protected void setMultiActions(CmsListMetadata metadata) {
    ...
    deleteMultiAction.setName(
        Messages.get().container(Messages.GUI_LIST_FIELD_MACTION_DELETEFIELD_NAME_0));
  ...
  /**
   * Fills details of the field into the given item.
   *
   * <p>
   *
  ...
  private void fillDetailField(CmsListItem item, String detailId) {
    ...
    List<CmsSearchField> fields =
        OpenCms.getSearchManager().getFieldConfiguration(m_paramFieldconfiguration).getFields();
    Iterator<CmsSearchField> itFields = fields.iterator();
  ...
  private List<CmsSearchField> getFields() {
    ...
    I_CmsSearchFieldConfiguration fieldConfig =
        manager.getFieldConfiguration(getParamFieldconfiguration());
