...
/**
 * Abstract dialog class for all dialogs that work on a <code>A_CmsSearchIndex</code>.
 *
 * <p>The <code>{@link #PARAM_INDEXNAME}</code> ("searchindex") is supported by means of widget
...
public abstract class A_CmsEditSearchIndexDialog extends CmsWidgetDialog {
  ...
  /**
   * The request parameter for the search index to work with when contacting this dialog from
   * another.
   *
   * <p>It may be emtpy if we are on the new index dialog (/searchindex/new-index.jsp).
   *
   * <p>
   */
   ...
   */
  public A_CmsEditSearchIndexDialog(
  ...
  public void actionCommit() {
    ...
    try {
      ...
      // check if field configuration has been updated, if thus set field configuration to the now
      // used
  ...
  /**
   * Initializes the user object (a <code>{@link A_CmsSearchIndex}</code> instance.
   *
  ...
  protected void validateParamaters() throws Exception {
    ...
    if (!isNewSearchIndex()) {
      // test the needed parameters: if initial we have "indexname", if from same widget we have
      // name.0
      if ((getParamIndexName() == null) && (getJsp().getRequest().getParameter("name.0") == null)) {
        throw new CmsIllegalStateException(
            Messages.get()
                .container(Messages.ERR_SEARCHINDEX_EDIT_MISSING_PARAM_1, PARAM_INDEXNAME));
      }
    }
  }

  /**
   * Creates a dummy index source.
   *
   * <p>
   *
  ...
  /**
   * Creates a "dummy" search index that is not linked to the search manager and has a <code>null
   * </code> name property that will be used for being filled with the widget bean technology.
   *
   * <p>
   *
   * @return a "dummy" search index that is not linked to the search manager and has a <code>null
   *     </code> name property that will be used for being filled with the widget bean technology
   */
