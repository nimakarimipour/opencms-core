...
/**
 * A <code>{@link org.opencms.workplace.CmsWidgetDialog}</code> that performs a search on the <code>
 * {@link org.opencms.search.CmsSearchIndex}</code> identified by request parameter <code>
 * {@link org.opencms.workplace.tools.searchindex.A_CmsEditSearchIndexDialog#PARAM_INDEXNAME}</code>
 * using an instance of <code>{@link org.opencms.search.CmsSearchParameters}</code> as widget object
 * to fill.
 *
 * <p>
 *
...
public class CmsSearchWidgetDialog extends A_CmsEditSearchIndexDialog {
  ...
  /**
   * Overrides this action that is performed in case an element has been added to a widget parameter
   * (<code>{@link org.opencms.workplace.CmsWidgetDialog#ACTION_ELEMENT_ADD}</code>) or removed (
   * <code>{@link org.opencms.workplace.CmsWidgetDialog#ACTION_ELEMENT_REMOVE}</code>) from a widget
   * parameter to additionally commit these values to the underlying lists.
   *
   * <p>This is necessary because this dialog performs a search for every request (not only if OK is
   * pressed, also if a category or field is added/removed). The search directly uses the underlying
   * Lists of categories, fields,... . More precise: The very same lists that are in the search
   * parameter instance used for search are contained as base collections of the widget parameters.
   * Therefore before every search the changes of categories, fields,... have to be committed here.
   *
   * <p>
   *
   ...
   *
   * <p>Overridden to allow additional validation and encoding of the search query.
   *
   * <p>
   *
  ...
  public void setFields(String fields) {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(searchPage)
        && CmsStringUtil.isEmptyOrWhitespaceOnly(fields)) {
      throw new CmsIllegalStateException(
          org.opencms.workplace.search.Messages.get()
              .container(org.opencms.workplace.search.Messages.ERR_VALIDATE_SEARCH_PARAMS_0));
    }
   ...
   * <p>
   *
   * @param maxDateLastModified the last modification date the resources have to have as maximum to
   *     set
   */
  public void setMaxDateLastModified(String maxDateLastModified) {

    if ((CmsStringUtil.isNotEmptyOrWhitespaceOnly(maxDateLastModified))
        && (!maxDateLastModified.equals("0"))) {
   ...
   * <p>
   *
   * @param minDateLastModified the last modification date the resources have to have as minimum to
   *     set
   */
  public void setMinDateLastModified(String minDateLastModified) {

    if ((CmsStringUtil.isNotEmptyOrWhitespaceOnly(minDateLastModified))
        && (!minDateLastModified.equals("0"))) {
  ...
  /**
   * This dialog does not return on commit but stay for many search requests until it is exited with
   * cancel or up in the workplace.
   *
   * <p>
   *
  ...
  protected String createDialogHtml(String dialog) {
    ...
    if (dialog.equals(PAGES[0])) {
      ...
      result.append(
          dialogBlockStart(key(Messages.GUI_LABEL_SEARCHINDEX_BLOCK_SEARCH_TIME_RANGES_0)));
      result.append(createWidgetTableStart());
   ...
   *
   * <p>Overrides <code> {@link org.opencms.workplace.CmsWidgetDialog#defaultActionHtml()}</code> in
   * order to put additional forms for page links and search results after OK - CANCEL buttons and
  ...
  protected void defineWidgets() {
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_searchParams, "index", "", PAGES[0], new CmsDisplayWidget(), 1, 1));
    addWidget(
        new CmsWidgetDialogParameter(
            m_searchParams, "query", "", PAGES[0], new CmsInputWidget(), 1, 1));
    addWidget(
        new CmsWidgetDialogParameter(
            m_searchParams,
            "sortName",
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_searchParams.getRoots(), "roots", "/", PAGES[0], new CmsVfsFileWidget(), 1, 10));
    addWidget(
        new CmsWidgetDialogParameter(
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_searchParams, "calculateCategories", new CmsCheckboxWidget()));

    // 2nd block with limiting to time ranges
    addWidget(
        new CmsWidgetDialogParameter(this, "minDateCreated", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(this, "maxDateCreated", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            this, "minDateLastModified", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            this, "maxDateLastModified", PAGES[0], new CmsCalendarWidget()));

    // 3rd block "fields to search in"
    addWidget(
        new CmsWidgetDialogParameter(
            this, "fields", PAGES[0], new CmsMultiSelectWidget(getFieldList(), true)));
  }

  /**
   * Overridden to additionally get a hold on the widget object of type <code>
   * {@link CmsSearchParameters}</code>.
   *
   * <p>
   *
  ...
  protected void initUserObject() {
    ...
    if (o == null) {
      ...
      m_searchParams.setCategories(
          new CmsHookListSearchCategory(m_searchParams, m_searchParams.getCategories()));
  ...
  /**
   * Additionally saves <code>{@link #PARAM_SEARCH_PARAMS}</code> to the dialog object map.
   *
   * <p>
   *
   * @see
  ...
  /**
   * Returns the hmtl for the category search results.
   *
   * <p>Note that a valid search (<code>{@link CmsSearch#getSearchResult()}</code> with correct
   * settings and inited) has to be triggered before this call or an empty String will be returned.
   *
   * <p>
   *
    ...
    if (m_searchParams.isCalculateCategories()) {
      ...
      if (categoryMap != null) {
        ...
        for (Map.Entry<String, Integer> entry : categoryMap.entrySet()) {
          result
              .append("  ")
              .append("<a class=\"searchcategory\" href=\"#\" onClick=\"filterCategory('");
  ...
  /**
   * Returns the HTML for the search results.
   *
   * <p>
   *
  ...
  private String createSearchResults() {
    ...
    if (!CmsStringUtil.isEmptyOrWhitespaceOnly(query) && (query.length() > 3)) {
      ...
      result
          ...
          .append(resultView.displaySearchResult(m_search))
          .append("\n</div>\n");
    } else {
  ...
  /**
   * Generates the JavaScript to filter categories.
   *
   * <p>
   *
  ...
  private String filterCategoryJS() {
    ...
    result.append(
        "<input name=\"dummysearchcategory\" id=\"dummysearchcategory\" type=\"hidden\" value=\"\">\n");
    result.append(
        "<input name=\"dummysearchpage\" id=\"dummysearchpage\" type=\"hidden\" value=\"\">\n");
    ...
    result.append(
        "      if(inputFields[i].name != null && inputFields[i].name.indexOf('categories') != -1) {\n");
  ...
  /**
   * Returns the different select options for sort search result criteria.
   *
   * <p>
   *
  ...
  /**
   * Normalizes the JavaScript for the previous search query.
   *
   * <p>
   *
  ...
  /**
   * Returns the JavaScript for submitting the search form.
   *
   * <p>
   *
