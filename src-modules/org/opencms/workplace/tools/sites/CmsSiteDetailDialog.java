...
public class CmsSiteDetailDialog extends CmsWidgetDialog {
  ...
  public void actionCommit() {
    ...
    try {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(getTemplate())) {
        CmsProperty prop =
            new CmsProperty(CmsPropertyDefinition.PROPERTY_TEMPLATE, getTemplate(), getTemplate());
      ...
      if (m_createou) {
        OpenCms.getOrgUnitManager()
            .createOrganizationalUnit(
                ...
                m_ouDescription.replace(
                    "%(site)", m_site.getTitle() + " [" + m_site.getSiteRoot() + "]"),
                0,
      ...
      String contentFolder =
          CmsStringUtil.joinPaths(sitePath, CmsADEManager.CONTENT_FOLDER_NAME + "/");
      ...
      if (!cms.existsResource(sitemapConfig)) {
        ...
        if (config != null) {
          try {
            ...
            I_CmsResourceType containerType =
                OpenCms.getResourceManager()
                    .getResourceType(
                        org.opencms.file.types.CmsResourceTypeXmlContainerPage.RESOURCE_TYPE_NAME);
            CmsResource modelPage =
                cms.createResource(newFolder.getRootPath() + BLANK_HTML, containerType);
            String defTitle =
                Messages.get()
                    .getBundle(getCms().getRequestContext().getLocale())
                    .key(Messages.GUI_DEFAULT_MODEL_TITLE_1, m_site.getTitle());
            String defDes =
                Messages.get()
                    .getBundle(getCms().getRequestContext().getLocale())
                    .key(Messages.GUI_DEFAULT_MODEL_DESCRIPTION_1, m_site.getTitle());
            CmsProperty prop =
                new CmsProperty(CmsPropertyDefinition.PROPERTY_TITLE, defTitle, defTitle);
      ...
      if ((m_site.getOriginalSite() != null)
          && m_site
              .getOriginalSite()
              .getUrl()
              .equals(OpenCms.getSiteManager().getWorkplaceServer())) {
  ...
  /**
   * Sets the the template property.
   *
   * <p>
   *
  ...
  protected String createDialogHtml(String dialog) {
    ...
    String title =
        m_site.getTitle() != null
            ? m_site.getTitle()
            : Messages.get()
                .getBundle(getCms().getRequestContext().getLocale())
                .key(Messages.GUI_SITES_NEW_SITE_TITLE_0);
    ...
    result.append(
        dialogBlockStart(
            Messages.get()
                .getBundle(getCms().getRequestContext().getLocale())
                .key(Messages.GUI_SITES_DETAIL_INFO_1, title)));
    result.append(createWidgetTableStart());
    ...
    if ((getParamEditaction() != null)
        || CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_site.getSecureUrl())) {
      // secure site
      result.append(
          dialogBlockStart(
              Messages.get()
                  .getBundle(getCms().getRequestContext().getLocale())
                  .key(Messages.GUI_SITES_DETAIL_SECURE_1, title)));
      result.append(createWidgetTableStart());
    ...
    if ((DIALOG_EDIT.equals(getParamEditaction()) || DIALOG_NEW.equals(getParamEditaction()))
      ...
      result.append(
          dialogBlockStart(
              Messages.get()
                  .getBundle(getCms().getRequestContext().getLocale())
                  .key(Messages.GUI_SITES_DETAIL_ALIASES_1, title)));
      result.append(createWidgetTableStart());
      if (DIALOG_EDIT.equals(getParamEditaction()) || DIALOG_NEW.equals(getParamEditaction())) {
        result.append(createDialogRowsHtml(++count, count));
    ...
    if (DIALOG_NEW.equals(getParamEditaction()) || DIALOG_EDIT.equals(getParamEditaction())) {
      ...
      addWidget(
          new CmsWidgetDialogParameter(
              m_site, "errorPage", PAGES[0], new CmsVfsFileWidget(true, "", true, false)));
      addWidget(
          new CmsWidgetDialogParameter(
              m_site, "position", PAGES[0], new CmsSelectWidget(createNavOpts(m_site))));
      addWidget(
          new CmsWidgetDialogParameter(m_site, "webserver", PAGES[0], new CmsCheckboxWidget()));
      addWidget(new CmsWidgetDialogParameter(m_site, "parameters", PAGES[0], new CmsInputWidget()));
      if (DIALOG_NEW.equals(getParamEditaction())) {
        addWidget(
            new CmsWidgetDialogParameter(this, "createou", PAGES[0], new CmsCheckboxWidget()));
      ...
      addWidget(
          new CmsWidgetDialogParameter(m_site, "exclusiveUrl", PAGES[0], new CmsCheckboxWidget()));
      addWidget(
          new CmsWidgetDialogParameter(
              m_site, "exclusiveError", PAGES[0], new CmsCheckboxWidget()));
        ...
        errorPage =
            new CmsWidgetDialogParameter(
                ...
                new CmsDisplayWidget(
                    Messages.get()
                        .getBundle(getCms().getRequestContext().getLocale())
                        .key(Messages.GUI_SITES_ERROR_PAGE_NOT_AVAILABLE_0)));
      } else {
        errorPage =
            new CmsWidgetDialogParameter(m_site, "errorPage", PAGES[0], new CmsDisplayWidget());
      }
      addWidget(errorPage);
      addWidget(
          new CmsWidgetDialogParameter(m_site, "webserver", PAGES[0], new CmsDisplayWidget()));
      int count = 5;
      for (String parameter : m_site.getParameters().keySet()) {
        ...
        CmsWidgetDialogParameter widget =
            new CmsWidgetDialogParameter(
                ...
                Messages.get()
                        .getBundle(getCms().getRequestContext().getLocale())
                        .key(Messages.GUI_SITES_DETAIL_PARAMETERS_0)
                    + " ["
                    + (count - 4)
                    + "]",
                new CmsDisplayWidget(),
      ...
      if (m_site.hasSecureServer()) {
        addWidget(
            new CmsWidgetDialogParameter(m_site, "secureUrl", PAGES[0], new CmsDisplayWidget()));
        addWidget(
            new CmsWidgetDialogParameter(m_site, "exclusiveUrl", PAGES[0], new CmsDisplayWidget()));
        addWidget(
            new CmsWidgetDialogParameter(
                m_site, "exclusiveError", PAGES[0], new CmsDisplayWidget()));
      }
      count = 0;
      for (String aliasUrl : m_site.getAliases()) {
        CmsWidgetDialogParameter alias =
            new CmsWidgetDialogParameter(
                aliasUrl,
                aliasUrl,
                Messages.get()
                        .getBundle(getCms().getRequestContext().getLocale())
                        .key(Messages.GUI_SITES_DETAIL_LABEL_ALIAS_0)
                    + " ["
                    + (count + 1)
                    + "]",
                new CmsDisplayWidget(),
  ...
  /**
   * Build select options for the position.
   *
   * <p>
   *
  ...
  private List<CmsSelectWidgetOption> createNavOpts(CmsSiteBean currSite) {
    ...
    for (int i = 0; i < sites.size(); i++) {
      ...
      if ((currSite != null)
          && (currSite.getSiteRoot() != null)
          && currSite.getSiteRoot().equals(siteRoot)) {
        options.add(
  ...
  private CmsResource createSitemapContentFolder(CmsObject cms, CmsResource subSitemapFolder)
    ...
    if (cms.existsResource(sitemapConfigName)) {
      ...
      if (!OpenCms.getResourceManager()
          .getResourceType(configFile)
          .getTypeName()
          .equals(configType.getTypeName())) {
  ...
  /**
   * Creates a template select combo widget.
   *
   * <p>
   *
   * @return the widget
  ...
  private CmsComboWidget createTemplateSelect() {
    ...
    try {
      ...
      List<CmsResource> templates =
          getCms()
              .readResources("/system/", CmsResourceFilter.DEFAULT.addRequireType(templateType));
  ...
  /**
   * Checks if there are at least one character in the folder name, also ensures that it starts and
   * ends with a '/'.
   *
   * <p>
   *
  ...
  /**
   * Initializes the dialog site object.
   *
   * <p>
   */
  private void initSite() {
    ...
    m_createou =
        Boolean.valueOf(module.getParameter(PARAM_CREATE_OU, Boolean.FALSE.toString()))
            .booleanValue();
  ...
  /**
   * Validates the dialog before the commit action is performed.
   *
   * <p>
   *
