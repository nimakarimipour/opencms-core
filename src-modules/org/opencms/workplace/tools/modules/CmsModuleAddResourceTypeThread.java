...
public class CmsModuleAddResourceTypeThread extends A_CmsReportThread {
  ...
  /** Sample file. */
  private static final String SAMPLE_FORMATTER =
      "/system/modules/org.opencms.workplace.tools.modules/samples/sample-formatter.jsp";

  /** Sample file. */
  private static final String SAMPLE_ICON_BIG =
      "/system/modules/org.opencms.workplace.tools.modules/samples/sample-icon_big.png";

  /** Sample file. */
  private static final String SAMPLE_ICON_SMALL =
      "/system/modules/org.opencms.workplace.tools.modules/samples/sample-icon.png";

  /** Sample file. */
  private static final String SAMPLE_SCHEMA =
      "/system/modules/org.opencms.workplace.tools.modules/samples/sample-schema.xsd";
  ...
  public void run() {
    ...
    try {
      ...
      type.addConfigurationParameter(
          CmsResourceTypeXmlContent.CONFIGURATION_SCHEMA, m_resInfo.getSchema());
      ...
      List<CmsExplorerTypeSettings> settings =
          new ArrayList<CmsExplorerTypeSettings>(module.getExplorerTypes());
      ...
      // write the module configuration, init resource types, explorer types and clear OpenCms
      // caches
  ...
  /**
   * Adds the given messages to the workplace properties file.
   *
   * <p>
   *
   * @param messages the messages
   * @param propertiesFile the properties file
   * @param forcePropertyFileEncoding flag, indicating if encoding {@link #PROPERTIES_ENCODING}
   *     should be forced
  ...
  private void addMessagesToPropertiesFile(
      Map<String, String> messages, CmsFile propertiesFile, boolean forcePropertyFileEncoding)
      throws CmsException, UnsupportedEncodingException {

    lockTemporary(propertiesFile);
    String encoding =
        forcePropertyFileEncoding
            ? PROPERTIES_ENCODING
  ...
  /**
   * Adds the given messages to the vfs message bundle.
   *
   * <p>
   *
   * @param messages the messages
   ...
   */
  private void addMessagesToVfsBundle(Map<String, String> messages, CmsFile vfsBundleFile)
      throws CmsException {
  ...
  /**
   * Adds the explorer type messages to the modules workplace bundle.
   *
   * <p>
   *
  ...
  private void addTypeMessages(CmsExplorerTypeSettings setting, String moduleFolder)
    ...
    if (!messages.isEmpty()) {
      ...
      String workplacePropertiesFile =
      ...
      if (cms.existsResource(workplacePropertiesFile)) {
        ...
        String vfsBundleFileName =
            CmsStringUtil.joinPaths(
                moduleFolder, PATH_I18N, m_resInfo.getModuleName() + SUFFIX_BUNDLE_FILE);
        ...
        if (cms.existsResource(vfsBundleFileName)) {
          ...
          String propertyBundleFileName =
              vfsBundleFileName + "_" + CmsLocaleManager.getDefaultLocale();
          if (!cms.existsResource(propertyBundleFileName)) {
            ...
            CmsResource res =
                cms.createResource(
                    ...
                    OpenCms.getResourceManager()
                        .getResourceType(CmsVfsBundleManager.TYPE_PROPERTIES_BUNDLE),
                    null,
                    null);
  ...
  /**
   * Copies sample schema and resource type icons and adds the resources to the module.
   *
   * <p>
   *
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_resInfo.getSchemaTypeName())) {
        ...
        try {
          ...
          String schemaContent =
              new String(schema.getContents(), CmsLocaleManager.getResourceEncoding(cms, schema));
          schemaContent =
              schemaContent.replaceAll(SAMPLE_SCHEMA_TYPE_NAME, m_resInfo.getSchemaTypeName());
  ...
  /**
   * Copies the sample formatter JSP, creates the associated formatter and module configuration.
   *
   * <p>
   *
   * @param moduleFolder the module folder name
   ...
   * @throws CmsException in case something goes wrong copying the resources
   */
  private void createSampleFormatter(String moduleFolder)
      throws CmsIllegalArgumentException, CmsException {

    CmsObject cms = getCms();
    String formatterFolder =
        CmsStringUtil.joinPaths(moduleFolder, CmsModulesEditBase.PATH_FORMATTERS);
    if (!cms.existsResource(formatterFolder)) {
      cms.createResource(formatterFolder, CmsResourceTypeFolder.getStaticTypeId());
    }
    String formatterJSP =
        CmsStringUtil.joinPaths(formatterFolder, m_resInfo.getName() + "-formatter.jsp");
    ...
    String formatterConfig =
        CmsStringUtil.joinPaths(formatterFolder, m_resInfo.getName() + "-formatter-config.xml");
    if (!cms.existsResource(formatterConfig)) {

      cms.createResource(
          formatterConfig,
          OpenCms.getResourceManager()
              .getResourceType(CmsFormatterConfigurationCache.TYPE_FORMATTER_CONFIG)
              .getTypeId());
      ...
      I_CmsXmlContentValue typeValue =
          configContent.getValue(
              CmsFormatterBeanParser.N_TYPE, CmsConfigurationReader.DEFAULT_LOCALE);
      typeValue.setStringValue(cms, m_resInfo.getName());
      I_CmsXmlContentValue formatterValue =
          configContent.getValue(
              CmsFormatterBeanParser.N_JSP, CmsConfigurationReader.DEFAULT_LOCALE);
      formatterValue.setStringValue(cms, formatterJSP);
      I_CmsXmlContentValue formatterNameValue =
          configContent.getValue(
              CmsFormatterBeanParser.N_NICE_NAME, CmsConfigurationReader.DEFAULT_LOCALE);
      ...
      configContent.addValue(
          cms, CmsFormatterBeanParser.N_MATCH, CmsConfigurationReader.DEFAULT_LOCALE, 0);
      configContent.addValue(
          cms,
          CmsFormatterBeanParser.N_MATCH + "/" + CmsFormatterBeanParser.N_WIDTH,
          CmsConfigurationReader.DEFAULT_LOCALE,
          0);
      I_CmsXmlContentValue widthValue =
          configContent.getValue(
      ...
      I_CmsXmlContentValue enabledValue =
          configContent.getValue(
              CmsFormatterBeanParser.N_AUTO_ENABLED, CmsConfigurationReader.DEFAULT_LOCALE);
    ...
    if (!cms.existsResource(moduleConfig)) {
      cms.createResource(
          moduleConfig,
          OpenCms.getResourceManager()
              .getResourceType(CmsADEManager.MODULE_CONFIG_TYPE)
              .getTypeId());
    }
    ...
    I_CmsXmlContentValue resourceTypeValue =
        moduleConfigContent.addValue(
            cms, CmsConfigurationReader.N_RESOURCE_TYPE, CmsConfigurationReader.DEFAULT_LOCALE, 0);
    I_CmsXmlContentValue typeValue =
        moduleConfigContent.getValue(
  ...
  /**
   * Locks the given resource temporarily.
   *
   * <p>
   *
