...
public class CmsCloneModule extends CmsWidgetDialog {
  ...
  private static final String CLONE_MODULE_REPORT =
      "/system/workplace/admin/modules/module-clone-report.jsp";
    ...
    try {
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(m_cloneInfo.getName())) {
        throw new CmsException(
            Messages.get().container(Messages.ERR_CLONEMODULE_EMPTY_PACKAGE_NAME_0));
      }
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(m_cloneInfo.getNiceName())) {
        throw new CmsException(
            Messages.get().container(Messages.ERR_CLONEMODULE_EMPTY_MODULE_NAME_0));
      }
      if (OpenCms.getModuleManager().hasModule(m_cloneInfo.getName())) {
        throw new CmsException(
            Messages.get()
                .container(
                    Messages.ERR_CLONEMODULE_MODULE_ALREADY_EXISTS_1, m_cloneInfo.getName()));
      }
  ...
  protected String createDialogHtml(String page) {
    ...
    if (PAGES[0].equals(page)) {
      ...
      result.append(
          createWidgetBlockStart(getMessages().key(Messages.GUI_CLONEMODULE_MODULE_INFORMATION_0)));
      result.append(createDialogRowsHtml(0, 4));
      result.append(createWidgetBlockEnd());
      result.append(
          createWidgetBlockStart(getMessages().key(Messages.GUI_CLONEMODULE_AUTHOR_INFORMATION_0)));
      result.append(createDialogRowsHtml(5, 6));
      result.append(createWidgetBlockEnd());
      result.append(
          createWidgetBlockStart(
              getMessages().key(Messages.GUI_CLONEMODULE_TRANSLATION_OPTIONS_0)));
  ...
  protected void defineWidgets() {
    ...
    addWidget(
        new CmsWidgetDialogParameter(m_cloneInfo, "formatterSourceModule", new CmsInputWidget()));
    addWidget(
        new CmsWidgetDialogParameter(m_cloneInfo, "formatterTargetModule", new CmsInputWidget()));
    addWidget(
        new CmsWidgetDialogParameter(m_cloneInfo, "changeResourceTypes", new CmsCheckboxWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            m_cloneInfo, "rewriteContainerPages", new CmsCheckboxWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            m_cloneInfo, "applyChangesEverywhere", new CmsCheckboxWidget()));
  }
  ...
  /**
   * Initializes the resource type info bean.
   *
   * <p>
   */
