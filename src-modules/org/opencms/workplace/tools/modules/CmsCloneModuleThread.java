...
public class CmsCloneModuleThread extends A_CmsReportThread {
  ...
  public static final String ICON_PATH =
      CmsWorkplace.VFS_PATH_RESOURCES + CmsWorkplace.RES_PATH_FILETYPES;
  ...
  public List<String> getAllModuleNames() {
    ...
    List<String> sortedModuleNames =
        new ArrayList<String>(OpenCms.getModuleManager().getModuleNames());
  ...
  public void run() {
    ...
    CmsModule sourceModule =
        OpenCms.getModuleManager().getModule(m_cloneInfo.getSourceModuleName());
    ...
    try {
      ...
      Map<I_CmsResourceType, I_CmsResourceType> resTypeMap =
      ...
      if (getCms().existsResource(targetClassesPath)) {
        List<CmsResource> props =
            cms.readResources(targetClassesPath, CmsResourceFilter.DEFAULT_FILES);
        replacesMessages(descKeys, props);
      }

      int type =
          OpenCms.getResourceManager()
              .getResourceType(CmsVfsBundleManager.TYPE_XML_BUNDLE)
              .getTypeId();
      ...
      List<CmsResource> allModuleResources =
          cms.readResources(targetModulePath, CmsResourceFilter.ALL);
      ...
      if (m_cloneInfo.isRewriteContainerPages()) {
        CmsResourceFilter f =
            CmsResourceFilter.requireType(CmsResourceTypeXmlContainerPage.getContainerPageTypeId());
  ...
  /**
   * Returns <code>true</code> if the module has been created successful.
   *
   * <p>
   *
   ...
   * @throws UnsupportedEncodingException if the file content could not be read with the determined
   *     encoding
   */
  private void adjustConfigs(
      CmsModule targetModule, Map<I_CmsResourceType, I_CmsResourceType> resTypeMap)
    ...
    try {
      ...
      Function<String, String> replaceFolderName =
          new ReplaceAll(
  ...
  private void adjustModuleResources(
    ...
    List<String> newTargetResources =
    ...
    List<String> newTargetExcludeResources =
        adjustModuleResourcePaths(
  ...
  private void cloneExplorerTypes(
      CmsModule targetModule, Map<String, String> iconPaths, Map<String, String> descKeys) {
    ...
    for (CmsExplorerTypeSettings expSetting : targetExplorerTypes) {
      ...
      String newExpTypeName =
          alterPrefix(
              oldExpTypeName, m_cloneInfo.getSourceNamePrefix(), m_cloneInfo.getTargetNamePrefix());
  ...
  private Map<I_CmsResourceType, I_CmsResourceType> cloneResourceTypes(
    ...
    Map<I_CmsResourceType, I_CmsResourceType> resourceTypeMapping =
        new HashMap<I_CmsResourceType, I_CmsResourceType>();
        ...
        if (targetResType instanceof A_CmsResourceType) {
            ...
            if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(prop.getValue())) {
              prop.setStructureValue(
                  prop.getStructureValue()
                      .replaceAll(sourceModule.getName(), targetModule.getName())
                      .replaceAll(sourcePathPart, targetPathPart));
          ...
          for (CmsConfigurationCopyResource conres : sourceResType.getConfiguredCopyResources()) {
            concreteTargetResType.addCopyResource(
                conres.getSource(), conres.getTarget(), conres.getTypeString());
          }
        }
  ...
  private void createTargetClassesFolder(
      CmsModule targetModule, String sourceClassesPath, String targetBaseClassesPath)
      throws CmsException {
    ...
    List<CmsResource> propertyFiles =
        getCms().readResources(sourceClassesPath, CmsResourceFilter.ALL);
   ...
   * @throws CmsException if something goes wrong
   */
  private void deleteSourceClassesFolder(
      String targetModulePath, String sourcePathPart, String targetPathPart) throws CmsException {
  ...
  /**
   * Returns <code>true</code> if form input is selected, checked, on or yes.
   *
   * <p>
   *
  ...
  /**
   * Renames the vfs resource bundle files within the target module according to the new module's
   * name.
   *
   * <p>
   *
   ...
   */
  private List<CmsResource> renameXmlVfsBundles(
      List<CmsResource> resources, CmsModule targetModule, String name) throws CmsException {
  ...
  private void replaceFormatterPaths(CmsModule targetModule)
    ...
    CmsResource formatterSourceFolder =
        getCms().readResource("/system/modules/" + m_cloneInfo.getFormatterSourceModule() + "/");
    CmsResource formatterTargetFolder =
        getCms().readResource("/system/modules/" + m_cloneInfo.getFormatterTargetModule() + "/");
    for (I_CmsResourceType type : targetModule.getResourceTypes()) {
      ...
      content =
          content.replaceAll(
              formatterSourceFolder.getRootPath(), formatterTargetFolder.getRootPath());
      file.setContents(content.getBytes(encoding));
  ...
  private void replaceModuleName() throws CmsException, UnsupportedEncodingException {
    ...
    CmsResourceFilter filter =
        CmsResourceFilter.ALL
            .addRequireFile()
            .addExcludeState(CmsResource.STATE_DELETED)
            .addRequireTimerange()
            .addRequireVisible();
    List<CmsResource> resources =
        getCms().readResources(CmsWorkplace.VFS_PATH_MODULES + m_cloneInfo.getName() + "/", filter);
    for (CmsResource resource : resources) {
  ...
  private void replacePath(
      String sourceModulePath, String targetModulePath, List<CmsResource> resources)
      throws CmsException, UnsupportedEncodingException {
  ...
  /**
   * Reads a file into a string, applies a transformation to the string, and writes the string back
   * to the file.
   *
   * <p>
   *
