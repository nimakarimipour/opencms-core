...
/**
 * A <code>{@link org.opencms.workplace.CmsWidgetDialog}</code> that allows modification of the
 * properties of the <code>{@link org.opencms.util.CmsRfsFileViewer}</code> bean.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsRfsFileViewSettingsDialog extends A_CmsRfsFileWidgetDialog {
   ...
   */
  public CmsRfsFileViewSettingsDialog(
  ...
  /**
   * Commits the <code>{@link org.opencms.util.CmsRfsFileViewer}</code> to the <code>
   * {@link org.opencms.workplace.CmsWorkplaceManager}</code>.
   *
   * <p>
   *
  ...
  protected void defineWidgets() {
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_logView, "isLogfile", "page1", new CmsCheckboxWidget(CmsStringUtil.TRUE)));
    addWidget(
        new CmsWidgetDialogParameter(
            m_logView,
            "filePath",
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_logView, "windowSize", "page1", new CmsComboWidget(comboOptions)));
  ...
  /**
   * Returns a list of <code>{@link org.opencms.widgets.CmsSelectWidgetOption}</code> instances for
   * the <code>{@link CmsComboWidget}</code> with the supported encodings of the current system and
   * the default encoding set as default combo option.
   *
   * <p>
   *
   * @return a list of <code>{@link org.opencms.widgets.CmsSelectWidgetOption}</code> instances for
   *     the <code>{@link CmsComboWidget}</code> with the supported encodings of the current system
   *     and the default encoding set as default combo option.
   *     <p>
   */
  private List<CmsSelectWidgetOption> createComboConfigurationEncodingChoice() {
    ...
    // before java 1.5 there is no other way (System property "file.encoding" is implementation
    // detail not in vmspec.
    Charset defaultCs =
    ...
    while (it.hasNext()) {
      ...
      if (cs == defaultCs) {
        ...
        if (!cs.name().startsWith("x")) {
          result.add(
              new CmsSelectWidgetOption(
                  cs.name(), false, null, key(Messages.GUI_WORKPLACE_LOGVIEW_FILE_CHARSET_HELP_0)));
        }
      }
    }
  ...
  /**
   * Returns a list of <code>{@link org.opencms.widgets.CmsSelectWidgetOption}</code> instances for
   * the <code>{@link CmsComboWidget}</code> with default file locations of OpenCms.
   *
   * <p>
   *
   * @return a list of <code>{@link org.opencms.widgets.CmsSelectWidgetOption}</code> instances for
