 ...
 *
 * <p>Defines the list columns and possible actions for logging.
 *
 * <p>
 */
public class CmsLog4JAdminDialog extends A_CmsListDialog {
  ...
  public void executeListSingleActions() throws CmsRuntimeException {
    ...
    if (ACTION_ACTIVATE_LOGFILE.equals(getParamListAction())) {
      ...
      // the button was inactive
        ...
        Collection<Appender> rootAppenders =
            ((Logger) LogManager.getRootLogger()).getAppenders().values();
        ...
        for (Appender appender : rootAppenders) {
          if (appender instanceof FileAppender) {
            ...
            filepath =
                fapp.getFileName().substring(0, fapp.getFileName().lastIndexOf(File.separatorChar));
    ...
    while (iterator.hasNext()) {
      ...
      for (Appender appender : appenders) {
        ...
        if (appender instanceof FileAppender) {
          ...
          String temp = "";
          temp =
              fapp.getFileName().substring(fapp.getFileName().lastIndexOf(File.separatorChar) + 1);
          test = test + temp;
          count++;
          break;
        }
      ...
      if (count == 0) {
        for (Appender appender : parentAppenders) {
          ...
          if (appender instanceof FileAppender) {
            ...
            temp =
                fapp.getFileName()
                    .substring(fapp.getFileName().lastIndexOf(File.separatorChar) + 1);
            test = test + temp;
            count++;
            break;
          }
  ...
  /**
   * Simple check if the logger has the global log file
   *
   * <p>or a single one.
   *
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    fatalColumn.setAlign(CmsListColumnAlignEnum.ALIGN_CENTER);
    fatalColumn.setListItemComparator(new CmsLogLevelListItemComparator());
    CmsListDirectAction fatalActAction =
        new CmsChangeLogLevelAction(ACTION_LOGGING_LEVEL_FATAL, Level.FATAL) {
    ...
    CmsListDirectAction errorActAction =
        new CmsChangeLogLevelAction(ACTION_LOGGING_LEVEL_ERROR, Level.ERROR) {
    ...
    CmsListDirectAction warnAction =
        new CmsChangeLogLevelAction(ACTION_LOGGING_LEVEL_WARN, Level.WARN) {
    ...
    CmsListDirectAction infoAction =
        new CmsChangeLogLevelAction(ACTION_LOGGING_LEVEL_INFO, Level.INFO) {
    ...
    CmsListDirectAction debugAction =
        new CmsChangeLogLevelAction(ACTION_LOGGING_LEVEL_DEBUG, Level.DEBUG) {
    ...
    CmsListDirectAction offAction =
        new CmsChangeLogLevelAction(ACTION_LOGGING_LEVEL_OFF, Level.OFF) {
    ...
    CmsListDirectAction changeFileAction =
        new CmsListDirectAction(ACTION_ACTIVATE_LOGFILE) {
          ...
          public boolean isVisible() {
            ...
            if (logger != null) {
              if (isloggingactivated(logger)) {
                ...
                setName(
                    Messages.get().container(Messages.GUI_LOG_FILE_CHANGE_FILE_HELP_DEACTIVATE_0));
              } else {
                setIconPath(PATH_FILE_INACTIVE);
                setName(
                    Messages.get().container(Messages.GUI_LOG_FILE_CHANGE_FILE_HELP_ACTIVATE_0));
              }
            }
            return super.isVisible();
          }
        };
    changeFileAction.setHelpText(
        Messages.get().container(Messages.GUI_LOG_FILE_CHANGE_FILE_HELP_0));
    ...
    CmsListSearchAction searchAction =
        new CmsListSearchAction(metadata.getColumnDefinition(COLUMN_CHANNELS));
  ...
  protected void setMultiActions(CmsListMetadata metadata) {
    ...
    settoFatal.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LOG_LEVEL_FATAL_MULTI_CONFIRMATION_0));
    ...
    settoError.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LOG_LEVEL_ERROR_MULTI_CONFIRMATION_0));
    ...
    settoWarn.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LOG_LEVEL_WARN_MULTI_CONFIRMATION_0));
    ...
    settoInfo.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LOG_LEVEL_INFO_MULTI_CONFIRMATION_0));
    ...
    settoDebug.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LOG_LEVEL_DEBUG_MULTI_CONFIRMATION_0));
    ...
    settoOff.setConfirmationMessage(
        Messages.get().container(Messages.GUI_LOG_LEVEL_OFF_MULTI_CONFIRMATION_0));
    ...
    while (it_curentlogger.hasNext()) {
      ...
      for (int i = 0; i < prefix.length; i++) {
        ...
        if (temp.lastIndexOf(".") > 1) {
          ...
          Logger temp_logger =
              getLoggerImpl(prefix[i] + "." + temp.substring(0, temp.indexOf(".")));
          // activate the heredity so the logger get the appender from parent logger
  ...
  /**
   * Simple function to set all child loggers to the same value of parent logger if the parent
   * logger leves is changed.
   *
   * <p>
   *
