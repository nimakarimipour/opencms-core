...
public class CmsSearchDialog extends CmsWidgetDialog {
  ...
  public void actionCommit() {
    ...
    try {
      ...
      if (getSettings().getUserSettings().getWorkplaceSearchViewStyle()
          == CmsSearchResultStyle.STYLE_EXPLORER) {
        getSettings().setExplorerPage(1);
        params.put(
            A_CmsListExplorerDialog.PARAM_SHOW_EXPLORER, new String[] {Boolean.TRUE.toString()});
      } else {
        CmsSearchResultsList resultsList = new CmsSearchResultsList(getJsp());
        CmsListMetadata metadata =
            resultsList.getMetadata(CmsSearchResultsList.class.getName(), resultsList.getListId());
        boolean withExcerpts =
            (getSettings().getUserSettings().getWorkplaceSearchViewStyle()
                == CmsSearchResultStyle.STYLE_LIST_WITH_EXCERPTS);
        if (metadata == null) {
          if (!withExcerpts) {
            // prevent the excerpts to be displayed by default
            params.put(
                CmsDialog.PARAM_ACTION, new String[] {A_CmsListDialog.LIST_INDEPENDENT_ACTION});
            params.put(
          ...
          // toggle excerpts
          metadata
              .getItemDetailDefinition(CmsSearchResultsList.LIST_DETAIL_EXCERPT)
              .setVisible(withExcerpts);
        }
        params.put(
            A_CmsListExplorerDialog.PARAM_SHOW_EXPLORER, new String[] {Boolean.FALSE.toString()});
      }
  ...
  protected void defineWidgets() {
    ...
    addWidget(
        new CmsWidgetDialogParameter(
            m_search, "restrictSearch", PAGES[0], new CmsCheckboxWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            m_search, "minDateCreated", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            m_search, "maxDateCreated", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            m_search, "minDateLastModified", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
            m_search, "maxDateLastModified", PAGES[0], new CmsCalendarWidget()));
    addWidget(
        new CmsWidgetDialogParameter(
    ...
    if (isInitialCall()) {
      ...
      index =
          OpenCms.getSearchManager()
              .getIndex(getSettings().getUserSettings().getWorkplaceSearchIndexName());
    } else {
      ...
      index =
          OpenCms.getSearchManager().getIndex(getJsp().getRequest().getParameter("indexName.0"));
    }
    return index;
  }

  /**
   * Creates the select widget configuration for the sort names.
   *
   * <p>
   *
  ...
  private List<CmsSelectWidgetOption> getSortNamesConf() {
    ...
    try {
      ...
      for (int i = 0; i < names.length; i++) {
        retVal.add(
            new CmsSelectWidgetOption(
                names[i], (i == 0), key(A_CmsWidget.LABEL_PREFIX + names[i].toLowerCase())));
      }
    } catch (Exception e) {
      // noop
    }
  ...
  /**
   * Creates the select widget configuration for the index names.
   *
   * <p>
   *
  ...
  /**
   * Gets the information if the search form is in the inital state.
   *
   * <p>
   *
  ...
  private boolean isInitialCall() {
    ...
    // return false in case the form was submitted already or the submit button was pressed or the
    // index was changed
    return !(((getJsp().getRequest().getParameter(CmsDialog.PARAM_ACTION) != null)
            && (getJsp()
                .getRequest()
                .getParameter(CmsDialog.PARAM_ACTION)
                .equals(CmsDialog.PARAM_ACTION_VALUE_FOR_CHANGED_INDEX)))
