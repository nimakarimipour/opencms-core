...
public class CmsSearchResourcesCollector extends A_CmsListResourceCollector {
  ...
  /**
   * @see
   *     org.opencms.workplace.list.A_CmsListResourceCollector#getInternalResources(org.opencms.file.CmsObject,
   *     java.util.Map)
   */
  @Override
  ...
  protected void setAdditionalColumns(CmsListItem item, CmsResourceUtil resUtil) {
    ...
    item.set(
        CmsSearchResultsList.LIST_COLUMN_SCORE,
        new Integer(getSearchResult(item.getId()).getScore()));
  }

  /**
   * Returns the search bean object.
   *
   * <p>
   *
  ...
  private CmsSearch getSearchBean(Map<String, String> params) {
    ...
    if (m_searchBean == null) {
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(m_searchBean.getIndex())) {
        m_searchBean.setIndex(
            getWp().getSettings().getUserSettings().getWorkplaceSearchIndexName());
      }
      m_searchBean.setMatchesPerPage(
          getWp().getSettings().getUserSettings().getExplorerFileEntries());
      m_searchBean.setSearchPage(
          Integer.parseInt(params.get(I_CmsListResourceCollector.PARAM_PAGE)));
      // set search roots
  ...
  /**
   * Returns a new search parameters object from the request parameters.
   *
   * <p>
   *
   * @param params the parameter map
  ...
  private CmsSearchParameters getSearchParameters(Map<String, String> params) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(params.get(PARAM_MINLASTMODIFICATIONDATE))) {
      searchParams.setMinDateLastModified(
          Long.parseLong(params.get(PARAM_MINLASTMODIFICATIONDATE)));
    }
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(params.get(PARAM_MAXLASTMODIFICATIONDATE))) {
      searchParams.setMaxDateLastModified(
          Long.parseLong(params.get(PARAM_MAXLASTMODIFICATIONDATE)));
    }
