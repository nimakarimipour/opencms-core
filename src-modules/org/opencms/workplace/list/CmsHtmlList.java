...
public class CmsHtmlList {
  ...
  /**
   * Generates the list of html option elements for a html select control to select a page of a
   * list.
   *
   * <p>
   *
  ...
  public static String htmlPageSelector(
      int nrPages, int itemsPage, int nrItems, int curPage, Locale locale) {
   ...
   *
   * <p>Equals to <code>{@link #getAllContent()}</code> if no filter is set.
   *
   * <p>
   *
   ...
   *
   * <p>Only current visible item can be retrieved using this method.
   *
   * <p>
   *
   ...
   *
   * <p>Equals to <code>{@link #getTotalNumberOfPages()}</code> if no filter is set.
   *
   * <p>
   *
   ...
   *
   * <p>Equals to <code>{@link #getTotalSize()}</code> if no filter is set.
   *
   * <p>
   *
   * @return the filtered number of items
   ...
   *
   * <p>If this flag is set the list will be surrounded by a box.
   *
   * <p>
   *
  ...
  public synchronized String listHtml() {
    ...
    html.append("<style type='text/css'>");
    html.append(
        "table.list * { word-wrap: break-word !important; white-space: normal !important; }");
    ...
    if (m_visibleItems.isEmpty()) {
      ...
      while (itItems.hasNext()) {
        ...
        if (thread != null) {
          ...
          thread.setProgress(
              ((count * (100 - progressOffset)) / m_visibleItems.size()) + progressOffset);
  ...
  public String listJs() {
    ...
    if (!m_metadata.getMultiActions().isEmpty()) {
      ...
      js.append(" = '");
      js.append(
          CmsStringUtil.escapeJavaScript(messages.key(Messages.GUI_LIST_ACTION_NO_SELECTION_0)));
      js.append("';\n");
   ...
   *
   * <p>Keeping care of all the state like sorted column, sorting order, displayed page and search
   * filter.
   *
   * <p>Try to use it instead of <code>{@link A_CmsListDialog#refreshList()}</code>.
   *
   * <p>
   *
  ...
  public void setSortedColumn(String sortedColumn) throws CmsIllegalArgumentException {
    ...
    if (!m_metadata.isSelfManaged()) {
      ...
      I_CmsListItemComparator c =
          getMetadata().getColumnDefinition(sortedColumn).getListItemComparator();
   ...
   * Sets the list state.
   *
   * <p>This may involve sorting, filtering and paging.
   *
   * <p>
   *
  ...
  protected String htmlBegin() {
    ...
    if (isBoxed()) {
      html.append(
          getWp().dialogBlock(CmsWorkplace.HTML_START, m_name.key(getWp().getLocale()), false));
    }
  ...
  protected String htmlEnd() {
    ...
    if (isBoxed()) {
      html.append(
          getWp().dialogBlock(CmsWorkplace.HTML_END, m_name.key(getWp().getLocale()), false));
    }
    html.append("</div>\n");
    return html.toString();
  }
    ...
    if (isNotSearching) {
      ...
      html.append(
          messages.key(
              ...
              new Object[] {
                m_name.key(getWp().getLocale()), new Integer(getSize()), new Integer(getTotalSize())
              }));
    }
    html.append("\t\t</td>\n");
    html.append("\t</tr>\n");
  ...
  protected String htmlTitle() {
    ...
    Iterator<CmsListItemDetails> itItemDetails =
        getMetadata().getItemDetailDefinitions().iterator();
   ...
   *
   * <p>Should only be used by the <code>{@link A_CmsListDialog}</code> class for temporally
   * removing the metadata object while the list is saved in the <code>
   * {@link org.opencms.workplace.CmsWorkplaceSettings}</code>.
   *
   * <p>
   *
