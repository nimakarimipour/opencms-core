...
public class CmsMultiListDialog {
  ...
  /**
   * Returns <code>true</code> if one of the lists has been forwarded.
   *
   * <p>
   *
  ...
  /**
   * Writes the dialog html code, only if the <code>
   * {@link org.opencms.workplace.CmsDialog#ACTION_DEFAULT}</code> is set.
   *
   * <p>
   *
   * @throws IOException if writing to the JSP out fails, or in case of errros forwarding to the
  ...
  protected String defaultActionHtmlContent() {
    ...
    StringBuffer result = new StringBuffer(2048);
    result.append(
        "<table id='twolists' cellpadding='0' cellspacing='0' align='center' width='100%'>\n");
    Iterator<A_CmsListDialog> i = m_wps.iterator();
    while (i.hasNext()) {
      A_CmsListDialog wp = i.next();
      result.append("\t<tr>\n");
