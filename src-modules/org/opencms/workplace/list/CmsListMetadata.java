...
public class CmsListMetadata {
  ...
  private CmsIdentifiableObjectContainer<CmsListColumnDefinition> m_columns =
      new CmsIdentifiableObjectContainer<CmsListColumnDefinition>(true, false);

  /** The CSV item formatter. * */
  private I_CsvItemFormatter m_csvItemFormatter;

  /** Container for of independent actions. */
  private CmsIdentifiableObjectContainer<I_CmsListAction> m_indepActions =
      new CmsIdentifiableObjectContainer<I_CmsListAction>(true, false);

  /** Container for item detail definitions. */
  private CmsIdentifiableObjectContainer<CmsListItemDetails> m_itemDetails =
      new CmsIdentifiableObjectContainer<CmsListItemDetails>(true, false);

  /** The id of the list. */
  private String m_listId;

  /** Container for multi actions. */
  private CmsIdentifiableObjectContainer<CmsListMultiAction> m_multiActions =
      new CmsIdentifiableObjectContainer<CmsListMultiAction>(true, false);

  /** Search action. */
  private CmsListSearchAction m_searchAction;

  /**
   * if the data is self managed (sorted and filtered by {@link A_CmsListDialog#getListItems()}
   * method).
   */
   ...
   *
   * <p>By default a column is printable if it is the first column in the list, or if it is
   * sorteable.
   *
   * <p>If you want to override this behaviour, use the {@link
   * CmsListColumnDefinition#setPrintable(boolean)} method after calling this one.
   *
   ...
   *
   * <p>It will be executed with a list of <code>{@link CmsListItem}</code>s.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the list definition contains an action.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if at least 'check' multiaction has been set.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the list definition contains a multi action.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if any column definition contains a single action.
   *
   * <p>
   *
  ...
  public String htmlItem(CmsListItem item, boolean odd, boolean isPrintable) {
    ...
    while (itCols.hasNext()) {
      ...
      if ((align != CmsListColumnAlignEnum.ALIGN_LEFT)
          && CmsStringUtil.isNotEmpty(align.toString())) {
  ...
  /**
   * Returns <code>true</code> if the list is searchable.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if any column is sorteable.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this metadata object should not be cached.
   *
   * <p>
   *
   * @return <code>true</code> if this metadata object should not be cached.
   *     <p>
   */
  ...
  /**
   * Throws a runtime exception if there are 2 identical ids.
   *
   * <p>This includes:
   *
   * <p>
   *
   * <ul>
   *   <li><code>{@link CmsListIndependentAction}</code>s
   *   <li><code>{@link CmsListMultiAction}</code>s
   *   <li><code>{@link CmsListItemDetails}</code>
   *   <li><code>{@link CmsListColumnDefinition}</code>s
   *   <li><code>{@link CmsListDefaultAction}</code>s
   *   <li><code>{@link CmsListDirectAction}</code>s
   * </ul>
   */
  /*package*/ void checkIds() {
    ...
    while (itIndepActions.hasNext()) {
      ...
      if (ids.contains(id)) {
        throw new CmsIllegalStateException(
            Messages.get().container(Messages.ERR_DUPLICATED_ID_1, id));
      }
      ids.add(id);
    }
    // multi actions
    ...
    while (itColumns.hasNext()) {
      ...
      if (ids.contains(col.getId())) {
        throw new CmsIllegalStateException(
            Messages.get().container(Messages.ERR_DUPLICATED_ID_1, col.getId()));
      }
      ids.add(col.getId());
      // default actions
  ...
  /**
   * Sets the list id for all column single actions.
   *
   * <p>
   *
