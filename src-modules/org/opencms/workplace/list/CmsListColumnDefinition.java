...
public class CmsListColumnDefinition {
   ...
   *
   * <p>A column could have more than one default action if the visibilities are complementary.
   *
   * <p>
   *
   ...
   *
   * <p>if <code>null</code> a default help text indicating the sort actions is used.
   *
   * <p>
   *
   ...
   *
   * <p>if no comparator was set, the default list item comparator is used.
   *
   * <p>
   *
  ...
  public String htmlHeader(CmsHtmlList list) {
    ...
    if (m_helpText != null) {
      helpText =
          new MessageFormat(m_helpText.key(locale), locale)
              .format(new Object[] {getName().key(locale)});
    } else {
      if (isSorteable()) {
        if (nextOrder.equals(CmsListOrderEnum.ORDER_ASCENDING)) {
          helpText =
              messages.key(
                  Messages.GUI_LIST_COLUMN_ASC_SORT_1, new Object[] {getName().key(locale)});
        } else {
          helpText =
              messages.key(
                  Messages.GUI_LIST_COLUMN_DESC_SORT_1, new Object[] {getName().key(locale)});
        }
      } else {
        helpText =
            messages.key(Messages.GUI_LIST_COLUMN_NO_SORT_1, new Object[] {getName().key(locale)});
      }
    }
   ...
   *
   * <p>if <code>true</code> a default list item comparator is used.
   *
   * <p>if <code>false</code> any previously set list item comparator is removed.
   *
   * <p>
   *
   ...
   * Sets the visible.
   *
   * <p>This will set also the printable flag to <code>false</code>.
   *
   * <p>
   *
