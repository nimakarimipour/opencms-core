...
public class CmsListExplorerFrameset extends CmsExplorerDialog {
   ...
   */
  public CmsListExplorerFrameset(
  ...
  public String defaultActionHtml() {
    ...
    String contentSrc =
        getFrameSource(
            "tool_content",
            getJsp().link(CmsToolManager.ADMINVIEW_ROOT_LOCATION + "/list-explorer.jsp")
                + "?"
                + params);
    ...
    html.append("\t\t<meta HTTP-EQUIV=\"Content-Type\" CONTENT=\"text/html; charset=\"")
        ...
        .append("\">\n");
    String uplink =
        "/opencms_a/opencms/system/workplace/views/admin/admin-main.jsp?path=/projects/files&action=initial&projectid=fa9f561f-be30-11e2-bac3-21ebf444feef&showexplorer=true";
    html.append(
        "<script >var uplink = \"" + CmsStringUtil.escapeJavaScript(uplink) + "\";</script>\n");
    html.append("\t\t<title>\n");
    html.append("\t\t\t")
        .append(
            key(
                org.opencms.workplace.Messages.GUI_LABEL_WPTITLE_1,
                new Object[] {getSettings().getUser().getFullName()}))
        .append("\n");
    html.append("\t\t</title>\n");
    html.append("\t</head>\n");
    ...
    html.append("\t\t<frame ")
        .append(titleSrc)
        .append(" frameborder='0' border='0' noresize scrolling='no'>\n");
    html.append("\t\t<frame ")
        .append(contentSrc)
        .append(
  ...
  public String dialogTitle() {
    ...
    String upLevelLink =
        CmsToolManager.linkForToolPath(
            getJsp(), parentPath, parentTool.getHandler().getParameters(this));
    String listLevelLink =
        CmsToolManager.linkForToolPath(
            getJsp(),
            toolPath,
    ...
    listLevelLink =
        CmsRequestUtil.appendParameter(
            listLevelLink, A_CmsListExplorerDialog.PARAM_SHOW_EXPLORER, Boolean.FALSE.toString());
    String parentName =
        getToolManager().resolveAdminTool(rootKey, parentPath).getHandler().getName();
    ...
    int size =
        (int) Math.ceil((double) items / getSettings().getUserSettings().getExplorerFileEntries());
    // build title
      ...
      html.append(
          A_CmsHtmlIconButton.defaultButtonHtml(
              ...
              org.opencms.workplace.tools.Messages.get()
                  .getBundle(getLocale())
                  .key(org.opencms.workplace.tools.Messages.GUI_ADMIN_VIEW_UPLEVEL_0),
              parentName,
              true,
              "admin/images/up.png",
              null,
  ...
  public void displayFrameSet() throws IOException {
    ...
    if (getJsp().getRequest().getParameter(CmsListExplorerFrameset.PARAM_PAGE) != null) {
      int page =
          Integer.parseInt(getJsp().getRequest().getParameter(CmsListExplorerFrameset.PARAM_PAGE));
      getSettings().setExplorerPage(page);

      if (getSettings().getCollector() instanceof I_CmsListResourceCollector) {
        I_CmsListResourceCollector collector =
            (I_CmsListResourceCollector) getSettings().getCollector();
  ...
  public String paramsAsHidden(Collection<String> excludes) {
    ...
    Map<String, String[]> params =
        CmsCollectionsGenericWrapper.map(getJsp().getRequest().getParameterMap());
  ...
  /**
   * @see
   *     org.opencms.workplace.CmsDialog#initWorkplaceRequestValues(org.opencms.workplace.CmsWorkplaceSettings,
