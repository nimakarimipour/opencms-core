...
public class CmsFrameset extends CmsWorkplace {
  ...
  public static final List<String> FRAMES_LIST =
      Collections.unmodifiableList(Arrays.asList(FRAMES));
  ...
  public String getPreferencesButton() {
    ...
    if (!getCms().getRequestContext().getCurrentUser().isManaged()) {
      ...
      return button(
          null, null, "preferences_in.png", Messages.GUI_BUTTON_PREFERENCES_0, buttonStyle);
    }
  }
  ...
  public String getProjectSelect(String htmlAttributes, String htmlWidth) {
    ...
    try {
      ...
      allProjects =
          OpenCms.getOrgUnitManager()
              .getAllAccessibleProjects(getCms(), ouFqn, settings.getListAllProjects());
    ...
    try {
      ou =
          OpenCms.getOrgUnitManager()
              .readOrganizationalUnit(getCms(), getCms().getRequestContext().getOuFqn());
    } catch (CmsException e) {
  ...
  /**
   * Returns the html for the "publish project" button depending on the current users permissions
   * and the default workplace settings.
   *
   * <p>
   *
  ...
  public String getPublishButton() {
    ...
    String publishButton =
        OpenCms.getWorkplaceManager().getDefaultUserSettings().getPublishButtonAppearance();
  ...
  public String getPublishQueueButton() {
    ...
    js.append(
        getJsp().link("/system/workplace/views/admin/admin-fs.jsp?root=admin&path=/publishqueue"));
    js.append("';");
    js.append("} else {");
    ...
    js.append(
        getJsp()
            .link(
                "/system/workplace/views/admin/admin-fs.jsp?root=explorer&path=/publishqueue&menu=no"));
    js.append("';");
    js.append("};");
    return button(
        js.toString(), null, "publish_queue.png", Messages.GUI_BUTTON_PUBLISHQUEUE_0, buttonStyle);
  }
  ...
  /**
   * Returns the startup URI for display in the main body frame, this can either be the user default
   * view, or (if set) a specific startup resource.
   *
   * <p>
   *
      ...
      while (i.hasNext()) {
        ...
        result =
            CmsRequestUtil.appendParameter(
                result, (String) entry.getKey(), ((String[]) entry.getValue())[0]);
      }
    }
  ...
  /**
   * Returns <code>true</code> if a reload of the main body frame is required.
   *
   * <p>This value is modified with the select options (project, site or view) in the head frame of
   * the Workplace. If a user changes one of these select values, the head frame is posted "against
   * itself". The posted values will be processed by this class, causing the internal Workplace
   * settings to change. After these settings have been changed, a reload of the main body frame is
   * required in order to update it with the new values. A JavaScript in the Workplace head frame
  ...
  /**
   * Indicates if the site selector should be shown in the top frame depending on the count of
   * accessible sites.
   *
   * <p>
   *
  ...
  /**
   * @see org.opencms.workplace.CmsWorkplace#initTimeWarp(org.opencms.db.CmsUserSettings,
  ...
  protected void initWorkplaceRequestValues(
    ...
    // check if a startup page has been set
    String startup =
        CmsRequestUtil.getNotEmptyDecodedParameter(request, CmsFrameset.PARAM_WP_START);
    if (startup != null) {
