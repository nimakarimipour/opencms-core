...
public class CmsExplorer extends CmsWorkplace {
   ...
   *
   * <p>Used by the explorer_body_fs.jsp.
   *
   * <p>
   *
  ...
  public String getFileList() {
    ...
    boolean showUserWhoLastModified =
        (preferences & CmsUserSettings.FILELIST_USER_LASTMODIFIED) > 0;
  ...
  public String getInitializationEntry(
    ...
    if (showTitle) {
      ...
      content.append("\"");
      content.append(
          CmsEncoder.escapeWBlanks(CmsEncoder.escapeXml(title), CmsEncoder.ENCODING_UTF_8));
      content.append("\",");
    } else {
      content.append("\"\",");
    }
    ...
    if (showNavText) {
      ...
      content.append("\"");
      content.append(
          CmsEncoder.escapeWBlanks(CmsEncoder.escapeXml(navText), CmsEncoder.ENCODING_UTF_8));
      content.append("\",");
    } else {
      content.append("\"\",");
    }
    ...
    // position 20: project state, I=resource is inside current project, O=resource is outside
    // current project
   ...
   * @return js code for initializing the explorer view
   * @see #getInitializationHeader()
   * @see #getInitializationEntry(CmsResourceUtil, boolean, boolean, boolean, boolean, boolean,
  ...
  public String getInitializationFooter(int numberOfPages, int selectedPage) {
    ...
    if (getSettings().getErrorMessage() != null) {
      ...
      content.append(
          CmsStringUtil.escapeJavaScript(getSettings().getErrorMessage().key(getLocale())));
      content.append("\");\n");
  ...
  public String getInitializationHeader() {
    ...
    String additionalParams =
        getJsp().getRequest().getParameter(CmsExplorer.PARAMETER_CONTEXTMENUPARAMS);
    ...
    if (OpenCms.getSiteManager().startsWithShared(siteFolderPath)
      ...
      siteFolderPath =
          siteFolderPath.substring(OpenCms.getSiteManager().getSharedFolder().length() - 1);
    }
    ...
    List<String> reloadTreeFolders =
        (List<String>) getJsp().getRequest().getAttribute(REQUEST_ATTRIBUTE_RELOADTREE);
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (CmsStringUtil.isNotEmpty(mode)) {
      ...
      if (!(VIEW_GALLERY.equals(settings.getExplorerMode())
          || VIEW_LIST.equals(settings.getExplorerMode()))) {
  ...
  /**
   * Checks if a folder with a given name exits in the VFS.
   *
   * <p>
   *
  ...
  /**
   * Returns a list resources that should be displayed in the OpenCms Explorer.
   *
   * <p>How the list is build depends on the current Workplace settings of the user.
   *
  ...
  private List<CmsResource> getResources(String resource) {
    ...
    if (VIEW_LIST.equals(getSettings().getExplorerMode())) {
      ...
      A_CmsAjaxGallery gallery =
          A_CmsAjaxGallery.createInstance(getSettings().getGalleryType(), getJsp());
  ...
  /**
   * Checks if a resource with a given name exits in the VFS.
   *
   * <p>
   *
   * @param cms the current cms context
