 ...
 *
 * <p>The following Workplace files use this class:
 *
 * <ul>
...
public class CmsTree extends CmsWorkplace {
  ...
  /**
   * Indicates project awareness, ie. if resources outside of the current project should be
   * displayed as normal.
   */
  ...
  public static String initTree(CmsObject cms, String encoding, String skinUri) {
    ...
    for (int i = 0; i < allResTypes.size(); i++) {
      ...
      CmsExplorerTypeSettings settings =
          OpenCms.getWorkplaceManager().getExplorerTypeSetting(curTypeName);
      if (settings == null) {
        if (LOG.isWarnEnabled()) {
          LOG.warn(
              Messages.get().getBundle().key(Messages.LOG_MISSING_SETTINGS_ENTRY_1, curTypeName));
        }
        settings =
  ...
  /**
   * Determines the root folder of the current tree dependent on users setting of explorer view
   * restriction.
   *
   * <p>
   *
  ...
  public String getSiteSelector(String htmlAttributes) {
    ...
    List<CmsSite> sites =
        OpenCms.getSiteManager()
            .getAvailableSites(
                getCms(), includeRootSite, true, getCms().getRequestContext().getOuFqn());
  ...
  public String getTree() {
    ...
    try {
      ...
      while (targets.hasNext()) {
        ...
        if ((startFolder == null) || (!currentTargetFolder.startsWith(startFolder))) {
              ...
              resources.addAll(
                  getCms()
                      .getResourcesInFolder(
                          currentTargetFolder, CmsResourceFilter.ONLY_VISIBLE_NO_DELETED));
            } else {
              resources.addAll(
                  getCms()
                      .getSubFolders(
          ...
          try {
            ...
            StringTokenizer tok =
                new StringTokenizer(currentTargetFolder.substring(startFolder.length()), "/");
      ...
      if (isProjectAware()) {
        try {
          projectResources =
              getCms().readProjectResources(getCms().getRequestContext().getCurrentProject());
        } catch (CmsException e) {
  ...
  /**
   * Returns the type of this tree (e.g. "copy", "project" etc.), if null this is the default
   * explorer version.
   *
   * <p>
   *
  ...
  /**
   * Indicates if the site selector should be shown depending on the tree type, initial settings and
   * the count of accessible sites.
   *
   * <p>
   *
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (rootFolder.equals(resource)
        && !rootFolder.equals(currentResource)
        && (lastknown == null)
        && !rootloaded) {
  ...
  /**
   * Determines if the site selector frame should be shown depending on the tree type or the value
   * of a request parameter.
   *
   * <p>If only one site is available, the site selector is not displayed.
   *
   * <p>
   *
  ...
  private void computeSiteSelector(HttpServletRequest request) {
    ...
    boolean showFromRequest =
        Boolean.valueOf(request.getParameter(PARAM_SHOWSITESELECTOR)).booleanValue();
  ...
  private String getNode(
      String path, String title, int type, boolean folder, CmsResourceState state, boolean grey) {
  ...
  private String getRootNode() {
    ...
    try {
      ...
      CmsProperty titleProperty =
          getCms().readPropertyObject(folder, CmsPropertyDefinition.PROPERTY_TITLE, false);
      ...
      return getNode(
          resource.getRootPath(), title, resource.getTypeId(), true, resource.getState(), false);
    } catch (CmsException e) {
   ...
   *
   * <p>This is needed for the link dialog in editors as well as the copy, move and link popup
   * dialogs for resources in the VFS.
   *
   * <p>
   *
  ...
  private String getSitePrefix(String prefix, String storedSiteRoot) {
    ...
    if (TYPE_PAGELINK.equals(getTreeType())) {
      ...
      // in vfs copy|move|link or vfs widget mode, don't add the prefix for the current workplace
      // site
      if (storedSiteRoot.equals(prefix)) {
        prefix = "";
      }
  ...
  /**
   * Returns true if a complete new tree must be loaded, false if an existing tree is updated or
   * extended.
   *
   * <p>
   *
  ...
  /**
   * Sets if the site selector should be shown depending on the tree type and the count of
   * accessible sites.
   *
   * <p>
   *
