...
public class CmsNewResourceXmlPage {
   ...
   * @return a sorted map with the body file title as key and absolute path to the body file as
   *     value
   * @throws CmsException if reading a folder or file fails
   */
  public static TreeMap<String, String> getBodies(CmsObject cms, String currWpPath)
      throws CmsException {
   ...
   * @throws CmsException if reading a folder or file fails
   */
  public static TreeMap<String, String> getBodies(
      CmsObject cms, String currWpPath, boolean emptyMap) throws CmsException {
   ...
   * @throws CmsException if reading a folder or file fails
   */
  public static TreeMap<String, String> getTemplates(CmsObject cms, String currWpPath)
      throws CmsException {
   ...
   * @throws CmsException if reading a folder or file fails
   */
  public static TreeMap<String, String> getTemplates(
      CmsObject cms, String currWpPath, boolean emptyMap) throws CmsException {
  ...
  /**
   * Returns a sorted Map of all available elements in the specified subfolder of the OpenCms
   * modules.
   *
   * <p>
   *
  ...
  protected static TreeMap<String, String> getElements(
      CmsObject cms, String elementFolder, String currWpPath, boolean emptyMap)
      throws CmsException {
    ...
    List<CmsResource> modules =
        cms.getSubFolders(CmsWorkplace.VFS_PATH_MODULES, CmsResourceFilter.IGNORE_EXPIRATION);
    for (int i = 0; i < modules.size(); i++) {
      List<CmsResource> moduleTemplateFiles = new ArrayList<CmsResource>();
      String folder = cms.getSitePath(modules.get(i));
      try {
        moduleTemplateFiles =
            cms.getFilesInFolder(
                folder + elementFolder, CmsResourceFilter.DEFAULT.addRequireVisible());
      ...
      for (int j = 0; j < moduleTemplateFiles.size(); j++) {
        ...
        try {
          title =
              cms.readPropertyObject(
                      cms.getSitePath(templateFile), CmsPropertyDefinition.PROPERTY_TITLE, false)
                  .getValue();
          folderProp =
              cms.readPropertyObject(
                      templateFile, CmsPropertyDefinition.PROPERTY_FOLDERS_AVAILABLE, false)
                  .getValue();
        } catch (CmsException e) {
        ...
        if (CmsStringUtil.isNotEmpty(currWpPath) && CmsStringUtil.isNotEmpty(folderProp)) {
          ...
          List<String> folders =
              CmsStringUtil.splitAsList(folderProp, CmsNewResource.DELIM_PROPERTYVALUES);
