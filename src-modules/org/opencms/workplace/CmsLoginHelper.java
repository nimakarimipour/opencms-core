...
public class CmsLoginHelper extends CmsWorkplace {
   ...
   *
   * <p>Check the {@link #getLoginException()} for the error message.
   *
   * <p>
   *
  ...
  public boolean login(
      String userName, String password, String projectName, String siteRoot, String resourceName) {

    if (getCms().getRequestContext().getCurrentUser().isGuestUser()) {
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(userName)
          || CmsStringUtil.isEmptyOrWhitespaceOnly(password)) {
        return false;
      }
      // login the user
      try {
    ...
    try {
      ...
      if (OpenCms.getOrgUnitManager()
          .getAllAccessibleProjects(getCms(), project.getOuFqn(), false)
          .contains(project)) {
    ...
    if (m_loginException == null) {
      ...
      try {
        ...
        Iterator<CmsSite> accessibles =
            OpenCms.getSiteManager().getAvailableSites(getCms(), false).iterator();
