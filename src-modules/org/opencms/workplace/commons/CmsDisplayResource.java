 ...
 *
 * <p>This is required to get correct previews of statically exported pages in the Online project.
 *
 * <p>The following file uses this class:
 *
 * <ul>
...
public class CmsDisplayResource extends CmsDialog {
   ...
   */
  protected static byte[] getHistoricalResourceContent(
      CmsObject cms, String resource, String version) {
    ...
    if (CmsStringUtil.isNotEmpty(getParamVersion())) {
      ...
      if (OpenCms.getSiteManager().isSharedFolder(getCms().getRequestContext().getSiteRoot())) {
        if (!OpenCms.getSiteManager().startsWithShared(resourceStr)) {
          resourceStr =
              CmsStringUtil.joinPaths(OpenCms.getSiteManager().getSharedFolder(), resourceStr);
        }
      }
   ...
   *
   * <p>If the user has a configured timewarp (preferences dialog) a mandatory timewarp will lead to
   * an exception. One cannot auto timewarp with configured timewarp time.
   *
   * <p>
   *
   * @param resource the resource to show
   * @throws CmsVfsResourceNotFoundException if a warp would be needed to show the resource but the
   *     user has a configured timewarp which disallows auto warping
   */
  ...
  /**
   * Returns if the given resource has the state "deleted".
   *
   * <p>
   *
  ...
  /**
   * Prepends the site-root to the given URL.
   *
   * <p>
   *
   * @param url the URL
  ...
  private String prependSiteRoot(String url) {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(site)
        || OpenCms.getSiteManager().isSharedFolder(site)) {
  ...
  /**
   * Displays the requested historic version of the resource.
   *
   * <p>
   */
    ...
    if (result != null) {
      ...
      String contentType =
          OpenCms.getResourceManager()
              .getMimeType(resourceStr, getCms().getRequestContext().getEncoding());
