 ...
 *
 * <p>The progress bar uses Ajax to not reload the whole page. The code which runs inside the thread
 * has to update the progress in the current thread.
 *
 * <p>The progress to be displayed is the progress of building large lists which can take some time
 * until they are finished.
 *
 * <p>There is a progress bar shown with the percentages on the left. Additionaly it is possible to
 * show a description above the progress bar.
 *
 * <p>
 *
 * @see A_CmsListDialog
...
public class CmsProgressWidget {
  ...
  private static Map<String, CmsProgressThread> m_threads =
      new HashMap<String, CmsProgressThread>();
   ...
   *
   * <p>Set to 0 (zero) to disable this.
   *
   * <p>
   */
  ...
  /**
   * The width of the progress bar to use in HTML.
   *
   * <p>
   */
  ...
  public CmsProgressWidget(CmsJspActionElement jsp) {
    ...
    if (getJsp().getRequest().getParameter(PARAMETER_SHOWWAITTIME) != null) {
      m_showWaitTime =
          Integer.valueOf(getJsp().getRequest().getParameter(PARAMETER_SHOWWAITTIME)).intValue();
    }
    ...
    if (getJsp().getRequest().getParameter(PARAMETER_REFRESHRATE) != null) {
      m_refreshRate =
          Integer.valueOf(getJsp().getRequest().getParameter(PARAMETER_REFRESHRATE)).intValue();
    }
   ...
   *
   * <p>The return value depends on the state of the progress/thread. This can be
   *
   * <ul>
   *   <li>the actual progress in percent with an optional description.
   *   <li>the result as the html code for the list.
   *   <li>an error message.
   * </ul>
   *
   * <p>The result will be interpreted by the JavaScript method "updateProgressbar()".
   *
   * <p>
   *
  ...
  public String getActualProgress() {
    ...
    try {
      ...
      if (getProgressThread(getKey()) != null) {
        ...
        if (!thread.isAlive()) {
          ...
          if (thread.getError() != null) {
            return createError(
                Messages.get()
                    .getBundle(getJsp().getRequestContext().getLocale())
                    .key(Messages.GUI_PROGRESS_ERROR_IN_THREAD_0),
                thread.getError());
          }
        ...
        if (LOG.isErrorEnabled()) {
          LOG.error(
              Messages.get().getBundle().key(Messages.LOG_PROGRESS_THREAD_NOT_FOUND_1, getKey()));
        }
        return createError(
            Messages.get()
                .getBundle(getJsp().getRequestContext().getLocale())
                .key(Messages.GUI_PROGRESS_THREAD_NOT_FOUND_1, getKey()));
      }
      ...
      return createError(
          Messages.get()
              .getBundle(getJsp().getRequestContext().getLocale())
              .key(Messages.GUI_PROGRESS_ERROR_CALCULATING_0),
          t);
    }
  }
  ...
  public String getJsIncludes() {
    ...
    result.append(
        org.opencms.workplace.Messages.get()
            .getBundle(getJsp().getRequestContext().getLocale())
            .key(org.opencms.workplace.Messages.GUI_AJAX_REPORT_GIVEUP_0));
    result.append("\");\n");
    ...
    result.append(
        org.opencms.workplace.Messages.get()
            .getBundle(getJsp().getRequestContext().getLocale())
            .key(org.opencms.workplace.Messages.GUI_AJAX_REPORT_ERROR_0));
    result.append(" \" + msg);\n");
  ...
  public String getWidget() {
    ...
    if ((thread != null) && (!thread.isAlive())) {
      ...
      result.append(
          ";border-width:1px;border-style:solid;padding:0px;margin:0px;float:left;display:");
      ...
      result.append(
          org.opencms.workplace.Messages.get()
              .getBundle(getJsp().getRequestContext().getLocale())
              .key(org.opencms.workplace.Messages.GUI_AJAX_REPORT_WAIT_0));
      result.append("</div>\n");
   ...
   * <p>
   *
   * @param jsFinishMethod the name of the JavaScript method to call after progress is finished to
   *     set
   */
   ...
   * <p>
   *
   * @param showWaitTime the time period the show the wait symbol before the progress bar is shown
   *     to set
   */
  ...
  public void startProgress(A_CmsListDialog list, boolean abortExisting) {
    ...
    if (m_threads.get(getKey()) != null) {
      if (abortExisting) {
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_PROGRESS_INTERRUPT_THREAD_1, getKey()));
        }
    ...
    synchronized (m_threads) {
      ...
      for (Iterator<Map.Entry<String, CmsProgressThread>> iter = m_threads.entrySet().iterator();
          iter.hasNext(); ) {
        Map.Entry<String, CmsProgressThread> entry = iter.next();
        CmsProgressThread value = entry.getValue();

        if ((!value.isAlive())
            && ((System.currentTimeMillis() - value.getFinishTime()) > CLEANUP_PERIOD)) {
    ...
    if (LOG.isDebugEnabled()) {
      for (Iterator<String> iter = threadsAbandoned.keySet().iterator(); iter.hasNext(); ) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_PROGRESS_CLEAN_UP_THREAD_1, iter.next()));
      }
    }
  }

  /**
   * Creates the html code for the given error message.
   *
   * <p>
   *
