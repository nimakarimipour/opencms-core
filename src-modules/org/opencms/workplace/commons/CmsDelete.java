...
public class CmsDelete extends CmsMultiDialog implements I_CmsDialogHandler {
  ...
  public String buildDeleteSiblings() {
    ...
    if (isMultiOperation() || (hasSiblings() && hasCorrectLockstate())) {
      ...
      CmsResourceDeleteMode defaultMode =
  ...
  public String buildReport() throws JspException, ServletException, IOException {
    ...
    CmsDeleteBrokenRelationsList list =
        new CmsDeleteBrokenRelationsList(
            getJsp(), getResourceList(), Boolean.valueOf(getParamDeleteSiblings()).booleanValue());
    list.actionDialog();
  ...
  /**
   * Returns <code>true</code> if the current user is allowed to delete the selected resources.
   *
   * <p>
   *
   * @return <code>true</code> if the current user is allowed to delete the selected resources
   */
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(getParamDeleteSiblings())) {
      setParamDeleteSiblings(
          Boolean.toString(
              getSettings().getUserSettings().getDialogDeleteFileMode()
                  == CmsResource.DELETE_REMOVE_SIBLINGS));
    }
