...
public class CmsHistoryList extends A_CmsListDialog {
  ...
  public void executeListSingleActions() throws IOException, ServletException {
    ...
    if (getParamListAction().equals(LIST_ACTION_RESTORE)) {
      try {
        ...
        params.put(PARAM_ACTION, new String[] {DIALOG_INITIAL});
        getToolManager()
            .jspForwardPage(this, "/system/workplace/views/explorer/explorer_files.jsp", params);
  ...
  protected List<CmsListItem> getListItems() throws CmsException {
    ...
    List<I_CmsHistoryResource> historicalVersions =
        getCms().readAllAvailableVersions(getParamResource());
    ...
    while (itVersions.hasNext()) {
          ...
          (CmsResource)
              getCms()
                  .readResource(
                      getCms()
                          .readResource(
                              histRes.getStructureId(), CmsResourceFilter.IGNORE_EXPIRATION)
                          .getStructureId(),
                      histRes.getVersion());
      // not for folder
      if (resource.isFile()) {
      ...
      try {
        user =
            CmsPrincipal.readPrincipalIncludingHistory(getCms(), histRes.getUserLastModified())
                .getName();
      ...
      item.set(
          ...
          getCms().getRequestContext().removeSiteRoot(histRes.getRootPath()));
      // size
      ...
      // invisible version flag: true if history version has content and actions should be enabled,
      // false otherwise
    ...
    if (result.isEmpty()) {
      ...
      try {
        getCms()
            .getRequestContext()
            .setCurrentProject(getCms().readProject(CmsProject.ONLINE_PROJECT_ID));
        onlineResource =
            getCms().readResource(getParamResource(), CmsResourceFilter.IGNORE_EXPIRATION);
        ...
        try {
          user =
              CmsPrincipal.readPrincipalIncludingHistory(
                      getCms(), onlineResource.getUserLastModified())
                  .getName();
        } catch (CmsDbEntryNotFoundException e) {
          // ignore
        }
    ...
    getList()
        ...
        .setVisible(offlineResource.isFile());
    // hide the preview button for folders
    getList()
        .getMetadata()
        .getColumnDefinition(LIST_COLUMN_ICON)
        .setVisible(offlineResource.isFile());
    ...
    if (!offlineResource.getState().isUnchanged()) {
      ...
      item.set(
          LIST_COLUMN_VERSION,
          new CmsVersionWrapper(CmsHistoryResourceHandler.PROJECT_OFFLINE_VERSION));
      // publish date
      item.set(LIST_COLUMN_DATE_PUBLISHED, "-");
      // last modification date
      item.set(
          LIST_COLUMN_DATE_LAST_MODIFIED,
          getMessages().getDateTime(offlineResource.getDateLastModified()));
      // resource type
      ...
      try {
        user =
            CmsPrincipal.readPrincipalIncludingHistory(
                    getCms(), offlineResource.getUserLastModified())
                .getName();
      } catch (CmsDbEntryNotFoundException e) {
        // ignore
      }
  ...
  protected void setColumns(CmsListMetadata metadata) {
    ...
    // add icon action
    CmsListDirectAction restoreAction =
        new CmsListDirectAction(LIST_ACTION_RESTORE) {
    ...
    restoreAction.setConfirmationMessage(
        Messages.get().container(Messages.GUI_HISTORY_CONFIRMATION_0));
    ...
    // add icon action
    CmsListDirectAction fileAction =
        new CmsListResourceIconAction(LIST_ACTION_VIEW, LIST_COLUMN_FILE_TYPE, getCms()) {

          /**
           * @see org.opencms.workplace.list.CmsListResourceIconAction#defButtonHtml(CmsObject,
    ...
    versionCol.setFormatter(
        new I_CmsListFormatter() {

          /**
           * @see org.opencms.workplace.list.I_CmsListFormatter#format(java.lang.Object,
           *     java.util.Locale)
           */
    ...
    CmsListColumnDefinition datePublishedCol =
        new CmsListColumnDefinition(LIST_COLUMN_DATE_PUBLISHED);
    ...
    CmsListColumnDefinition nicenameCol =
        new CmsListColumnDefinition(LIST_COLUMN_DATE_LAST_MODIFIED);
    ...
    CmsListItemSelectionAction sel1Action =
        new CmsListItemSelectionAction(LIST_RACTION_SEL1, LIST_MACTION_COMPARE);
    ...
    // add item selection action
    CmsListItemSelectionAction sel2Action =
        new CmsListItemSelectionAction(LIST_RACTION_SEL2, LIST_MACTION_COMPARE);
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    indexDetails.setShowActionName(
        Messages.get().container(Messages.GUI_LIST_HISTORY_DETAIL_PROJECT_NAME_SHOW_0));
    ...
    indexDetails.setHideActionName(
        Messages.get().container(Messages.GUI_LIST_HISTORY_DETAIL_PROJECT_NAME_HIDE_0));
    ...
    indexDetails.setFormatter(
        new CmsListItemDetailsFormatter(
            Messages.get().container(Messages.GUI_LIST_HISTORY_DETAIL_PROJECT_INFO_0)));
  ...
  protected void setMultiActions(CmsListMetadata metadata) {
    ...
    // add compare action
    CmsListRadioMultiAction compareAction =
        new CmsListRadioMultiAction(
  ...
  /**
   * Fills details of the project into the given item.
   *
   * <p>
   *
  ...
  private void fillDetailProject(CmsListItem item, String detailId) {
    ...
    // search /read for the corresponding history project: it's tag id transmitted from
    // getListItems()
    // in a hidden column
    Object tagIdObj = item.get(LIST_COLUMN_PUBLISH_TAG);
    if (tagIdObj != null) {
      // it is null if the offline version with changes is shown here: now history project available
      // then
