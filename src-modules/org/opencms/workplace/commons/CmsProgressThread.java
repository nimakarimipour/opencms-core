...
/**
 * This class is used to build a list ({@link A_CmsListDialog}) in the background and to show the
 * progress of building to the user.
 *
 * <p>To work correctly the operations while building the list have to update the actual progress.
 *
 * <p>
 *
 * @since 7.0.0
...
public class CmsProgressThread extends A_CmsProgressThread {
  ...
  public void run() {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_PROGRESS_FINISHED_THREAD_1, getKey()));
      }
    }
  }
