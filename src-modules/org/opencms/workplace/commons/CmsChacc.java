...
public class CmsChacc extends CmsDialog {
  ...
  public static String buildSummaryDetailsButtons(CmsDialog wp) {
    ...
    result
        .append("\t<form action=\"")
        .append(wp.getDialogUri())
        .append("\" method=\"post\" name=\"selectshortview\">\n");
    result.append("\t<td>\n");
    ...
    result
        .append("\t<input  type=\"submit\" class=\"dialogbutton\" value=\"")
        .append(wp.key(Messages.GUI_LABEL_SUMMARY_0))
        .append("\"");
    ...
    result
        .append("\t</form>\n\t<form action=\"")
        .append(wp.getDialogUri())
        .append("\" method=\"post\" name=\"selectlongview\">\n");
    result.append("\t<td>\n");
    result.append("\t<input type=\"hidden\" name=\"");
    result.append(PARAM_VIEW);
    ...
    result
        .append("\t<input type=\"submit\" class=\"dialogbutton\" value=\"")
        .append(wp.key(Messages.GUI_LABEL_DETAILS_0))
        .append("\"");
    if ("long".equals(selectedView)) {
      result.append(" disabled=\"disabled\"");
    }
  ...
  public boolean actionAddAce() {
    ...
    if (checkNewEntry(name, arrayPosition)) {
      ...
      if (getInheritOption()
          && getSettings().getUserSettings().getDialogPermissionsInheritOnFolder()) {
    ...
    try {
      result.append(
          buildPermissionEntryForm(
              getSettings().getUser().getId(), buildPermissionsForCurrentUser(), false, false));
    } catch (CmsException e) {
      // should never happen
      if (LOG.isErrorEnabled()) {
        LOG.error(e.getLocalizedMessage(), e);
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        try {
          ...
          Iterator<CmsAccessControlEntry> entries =
              cms.getAccessControlEntries(rootPath, false).iterator();
          while (entries.hasNext()) {
            CmsAccessControlEntry ace = entries.next();
            if (ace.isResponsible()) {
              try {
                responsibles.put(
                    CmsPrincipal.readPrincipalIncludingHistory(cms, ace.getPrincipal()), rootPath);
      ...
      StringBuffer result = new StringBuffer(512);
      result.append(
          dialogToggleStart(key(Messages.GUI_AVAILABILITY_RESPONSIBLES_0), "responsibles", show));
      ...
      while (i.hasNext()) {
        ...
        if ("long".equals(getSettings().getPermissionDetailView())) {
          ...
          if (!resourceRootPath.equals(resourceName)) {
            ...
            result.append(
                key(Messages.GUI_PERMISSION_INHERITED_FROM_1, new Object[] {resourceName}));
            result.append("</div>");
          }
        }
        try {
          if ((ou != null)
              && (OpenCms.getOrgUnitManager().getOrganizationalUnits(getCms(), "", true).size()
                  > 1)) {
            result.append("<br>");
            result
                .append("<img src='")
                .append(getSkinUri())
                .append("explorer/project_none.gif' class='noborder' width='16' height='16' >");
            result
                .append("<img src='")
                .append(getSkinUri())
                .append("explorer/project_none.gif' class='noborder' width='16' height='16' >");
            ...
            try {
              result.append(
                  OpenCms.getOrgUnitManager()
                      .readOrganizationalUnit(getCms(), ou)
  ...
  /**
   * Builds a String with HTML code to display the inherited and own access control entries of a
   * resource.
   *
   * <p>
   *
  ...
  public String buildRightsList() {
    ...
    try {
      Iterator<CmsAccessControlEntry> itAces =
          getCms().getAccessControlEntries(getParamResource(), false).iterator();
    ...
    try {
      Iterator<CmsAccessControlEntry> itAces =
          getCms().getAccessControlEntries(path, true).iterator();
      while (itAces.hasNext()) {
        CmsAccessControlEntry curEntry = itAces.next();
  ...
  /**
   * Returns a list with all error messages which occurred when trying to add a new access control
   * entry.
   *
   * <p>
   *
  ...
  /**
   * Returns a String with all error messages occuring when trying to add a new access control
   * entry.
   *
   * <p>
   *
   ...
   *
   * <p>The name parameter stores the name of the group or user.
   *
   * <p>
   *
   ...
   *
   * <p>The type parameter stores the type of an ace (group or user).
   *
   * <p>
   *
  ...
  /**
   * Initializes some member variables to display the form with the right options for the current
   * user.
   *
   * <p>This method must be called after initWorkplaceRequestValues().
   *
   * <p>
   */
  public void init() {
    ...
    try {
      ...
      if (resource.isFolder()) {
        ...
        if (!resName.endsWith("/")) {
          // append manually a "/" to folder name to avoid issues with check if resource is in
          // project
          resName += "/";
        }
      }
    ...
    if ((!getCms().getRequestContext().getCurrentProject().isOnlineProject()
        ...
        && (OpenCms.getRoleManager().hasRole(getCms(), CmsRole.VFS_MANAGER)
            || (((m_curPermissions.getAllowedPermissions() & CmsPermissionSet.PERMISSION_CONTROL)
                    > 0)
                && !((m_curPermissions.getDeniedPermissions() & CmsPermissionSet.PERMISSION_CONTROL)
                    > 0)))) {
      if (isBlockingLocked()) {
  ...
  /**
   * Returns if the access control entry can be inherited to subfolders and can overwrite inherited
   * permissions.
   *
   * <p>
   *
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (DIALOG_TYPE.equals(getParamAction())) {
      ...
      setParamTitle(
          key(
              Messages.GUI_PERMISSION_CHANGE_1,
  ...
  /**
   * Sets if the access control entry can be inherited to subfolders and can overwrite inherited
   * permissions.
   *
   * <p>This is set in the init() method.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param showInheritedPermissions true if the inherited permissions information should be
  ...
  /**
   * Builds a String with HTML code to display the form to add a new access control entry for the
   * current resource.
   *
   * <p>
   *
  ...
  private String buildAddForm() {
    ...
    if (getEditable()) {
      ...
      result
          .append("<form action=\"")
          .append(getDialogUri())
          .append("\" method=\"post\" name=\"add\" class=\"nomargin\">\n");
      // set parameters to show correct hidden input fields
      ...
      result
          .append("\t<td>")
          .append(buildSelect("name=\"" + PARAM_TYPE + "\"", options, optionValues, -1))
          .append("</td>\n");
  ...
  /**
   * Builds a StringBuffer with HTML code to show a list of all inherited access control entries.
   *
   * <p>
   *
   * @param entries ArrayList with all entries to show for the long view
   * @param parents Map of parent resources needed to get the connected resources for the detailed
   *     view
   * @return StringBuffer with HTML code for all entries
   */
  private StringBuffer buildInheritedList(
      ArrayList<CmsAccessControlEntry> entries, Map<CmsUUID, String> parents) {
    ...
    if ("long".equals(view)) {
      ...
      while (i.hasNext()) {
        ...
        result.append(
            buildPermissionEntryForm(
                curEntry, false, true, getConnectedResource(curEntry, parents)));
      }
    } else {
      // show the short view, use an ACL to build the list
      try {
        // get the inherited ACL of the parent folder
        CmsAccessControlList acList =
            getCms().getAccessControlList(CmsResource.getParentFolder(getParamResource()), false);
  ...
  private String buildInternalForm() {
    ...
    if ((resource != null) && (resource.isFile())) {
      ...
      result
          .append("<form action=\"")
          .append(getDialogUri())
          .append("\" method=\"post\" name=\"internal\" class=\"nomargin\">\n");
      result.append("<table border=\"0\" width=\"100%\">\n");
      result.append("<tr>\n");
      result
          .append("\t<td class=\"dialogpermissioncell\">")
          .append(key(Messages.GUI_PERMISSION_INTERNAL_0));
      ...
      if (getEditable()) {
        result
            .append("<td><input  type=\"submit\" class=\"dialogbutton\" value=\"")
            .append(key(Messages.GUI_LABEL_SET_0))
            .append("\">");
      }
      result.append("</td>\n");
      result.append("</tr>\n");
  ...
  private StringBuffer buildPermissionEntryForm(
      CmsAccessControlEntry entry, boolean editable, boolean extendedView, String inheritRes) {
    ...
    if ((principal != null) && (principal instanceof CmsHistoryPrincipal)) {
      ...
      if (principal.isGroup()) {
        String niceName =
            OpenCms.getWorkplaceManager().translateGroupName(principal.getName(), false);
        name =
            key(
      ...
      flags = CmsAccessControlEntry.ACCESS_FLAGS_USER;
        ...
        && id.equals(CmsAccessControlEntry.PRINCIPAL_ALL_OTHERS_ID.toString())) {
      name = key(Messages.GUI_LABEL_ALLOTHERS_0);
      flags = CmsAccessControlEntry.ACCESS_FLAGS_ALLOTHERS;
    } else if ((id != null)
        && id.equals(CmsAccessControlEntry.PRINCIPAL_OVERWRITE_ALL_ID.toString())) {
    ...
    if ((flags > 0) && ((entry.getFlags() & flags) == 0)) {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle(getLocale())
                .key(Messages.ERR_INVALID_ACE_1, entry.toString()));
      }
      entry =
    ...
    if (!id.equals(CmsAccessControlEntry.PRINCIPAL_OVERWRITE_ALL_ID.toString())) {
      ...
      if (extendedView) {
        ...
        result
            .append("<img src=\"")
            .append(getSkinUri())
            .append("commons/plus.png\" class=\"noborder\" id=\"ic-")
            .append(idValue)
            .append("\"></a>");
      } else {
        result
      ...
      if (editable) {
        result
            .append("<form action=\"")
            .append(getDialogUri())
            .append("\" method=\"post\" class=\"nomargin\" name=\"set")
            .append(idValue)
            .append("\">\n");
        // set parameters to show correct hidden input fields
      ...
      result.append("<tr>\n");
      result.append(
          "\t<td class=\"dialogpermissioncell\"><span class=\"textbold\" unselectable=\"on\">");
      ...
      while (i.hasNext()) {
        ...
        result.append(
            "\t<td class=\"dialogpermissioncell textcenter\"><input type=\"checkbox\" name=\"");
        result
            .append(value)
            .append(PERMISSION_ALLOW)
            .append("\" value=\"")
            .append(value)
            .append("\"")
            .append(disabled);
      ...
      if (editable) {
        ...
        if (!id.equals(CmsAccessControlEntry.PRINCIPAL_ALL_OTHERS_ID.toString())) {
          ...
          result
              .append("\t<td class=\"dialogpermissioncell\">")
              .append(key(Messages.GUI_LABEL_RESPONSIBLE_0))
              .append("</td>\n");
          result.append("\t<td class=\"dialogpermissioncell textcenter\">");
          result
              .append("<input type=\"checkbox\" name=\"")
              .append(PARAM_RESPONSIBLE)
              .append("\" value=\"true\"")
              .append(disabled);
        ...
        result
            .append("\t<td class=\"dialogpermissioncell\">")
            .append(key(Messages.GUI_PERMISSION_OVERWRITE_INHERITED_0))
            .append("</td>\n");
        result.append("\t<td class=\"dialogpermissioncell textcenter\">");
        result
            .append("<input type=\"checkbox\" name=\"")
            .append(PARAM_OVERWRITEINHERITED)
            .append("\" value=\"true\"")
            .append(disabled);
        ...
        if (getInheritOption()) {
          ...
          result
              .append("\t<td class=\"dialogpermissioncell\">")
              .append(key(Messages.GUI_PERMISSION_INHERIT_ON_SUBFOLDERS_0))
              .append("</td>\n");
          result.append("\t<td class=\"dialogpermissioncell textcenter\">");
          result
              .append("<input type=\"checkbox\" name=\"")
              .append(PARAM_INHERIT)
              .append("\" value=\"true\"")
              .append(disabled);
        ...
        result
            .append(
                "\t<td class=\"textcenter\"><input class=\"dialogbutton\" type=\"submit\" value=\"")
            .append(key(Messages.GUI_LABEL_SET_0))
            .append("\"></form></td>\n");
        result.append("\t<td class=\"textcenter\">\n");
        // build the form for the "delete" button
        result
            .append("\t\t<form class=\"nomargin\" action=\"")
            .append(getDialogUri())
            .append("\" method=\"post\" name=\"delete")
            .append(idValue)
            .append("\">\n");
        // set parameters to show correct hidden input fields
        ...
        result.append(paramsAsHidden());
        result
            .append("\t\t<input class=\"dialogbutton\" type=\"submit\" value=\"")
            .append(key(Messages.GUI_LABEL_DELETE_0))
            .append("\">\n");
        result.append("\t\t</form>\n");
        result.append("\t</td>\n");
        result.append("</tr>\n");
  ...
  private StringBuffer buildPermissionEntryForm(
    ...
    try {
      ...
      CmsAccessControlEntry entry =
          new CmsAccessControlEntry(res.getResourceId(), id, curSet, flags);
  ...
  /**
   * Returns the actual real permissions (including role, and any other special check) for the
   * current user.
   *
   * <p>
   *
  ...
  private CmsPermissionSet buildPermissionsForCurrentUser() throws CmsException {
    ...
    CmsResourceUtil resUtil =
        new CmsResourceUtil(
            getCms(), getCms().readResource(getParamResource(), CmsResourceFilter.ALL));
    return resUtil.getPermissionSet();
  }

  /**
   * Builds a StringBuffer with HTML code for the access control entries of a resource.
   *
   * <p>
   *
  ...
  /**
   * Returns if the requested resource if blocking locked.
   *
   * <p>
   *
  ...
  private boolean isBlockingLocked() {
    ...
    blockingFilter =
        blockingFilter.filterNotLockableByUser(getCms().getRequestContext().getCurrentUser());
    try {
  ...
  /**
   * Checks if the current user has the vfs manager role for the current select resource.
   *
   * <p>
   *
   * @return <code>true</code> if the current user has the vfs manager role for the current select
   *     resource
   */
