...
public class CmsNotResourceCategoriesList extends A_CmsResourceCategoriesList {
   ...
   */
  public CmsNotResourceCategoriesList(
  ...
  protected CmsNotResourceCategoriesList(CmsJspActionElement jsp, String listId) {
    ...
    super(
        jsp,
        listId,
        Messages.get().container(Messages.GUI_NOTRESOURCECATEGORIES_LIST_NAME_0),
        true);
  }
  ...
  protected List<CmsCategory> getCategories() throws CmsException {
    ...
    List<CmsCategory> result =
        getCategoryService()
            .readCategories(getJsp().getCmsObject(), null, true, getParamResource());
    ...
    while (itResourceRelations.hasNext()) {
      ...
      if (result.contains(category)
          && resourceRelations.containsAll(
              getCategoryService()
                  .readCategories(
                      getJsp().getCmsObject(), category.getPath(), true, getParamResource()))) {
  ...
  /**
   * @see
   *     org.opencms.workplace.commons.A_CmsResourceCategoriesList#setColumns(org.opencms.workplace.list.CmsListMetadata)
   */
  @Override
        ...
        .setFormatter(
            new I_CmsListFormatter() {
              ...
              public String format(Object data, Locale locale) {
                ...
                CmsListColumnDefinition listColumnDefinition =
                    getList().getMetadata().getColumnDefinition(LIST_COLUMN_STATE);
                CmsListDirectAction stateAction =
                    (CmsListDirectAction) listColumnDefinition.getDirectAction(LIST_ACTION_ADD);
  ...
  /**
   * @see
   *     org.opencms.workplace.commons.A_CmsResourceCategoriesList#setStateActionCol(org.opencms.workplace.list.CmsListMetadata)
   */
  @Override
  protected void setStateActionCol(CmsListMetadata metadata) {
        ...
        new CmsListDirectAction(LIST_ACTION_ADD) {
          ...
          public boolean isEnabled() {
            ...
            try {
              ...
              if (wp.getResourceCategories()
                  .contains(
                      wp.getCategoryService()
                          .readCategory(wp.getCms(), getItem().getId(), wp.getParamResource()))) {
                return false;
              }
            } catch (CmsException e) {
              // ignore
    ...
    stateAction.setName(
        Messages.get().container(Messages.GUI_CATEGORIES_LIST_DEFACTION_ADD_NAME_0));
    stateAction.setHelpText(
        Messages.get().container(Messages.GUI_CATEGORIES_LIST_DEFACTION_ADD_HELP_0));
