...
public class CmsAvailability extends CmsMultiDialog {
  ...
  public static void writeProperty(
      CmsObject cms, String resourcePath, String propertyName, String propertyValue)
      throws CmsException {
    ...
    if (!oldResourceValue.equals(newResourceValue)
        || !oldStructureValue.equals(newStructureValue)) {
  ...
  public void actionCloseDialog() throws JspException {
    ...
    getSettings()
        .getFrameUris()
        .put("body", CmsWorkplace.VFS_PATH_VIEWS + "explorer/explorer_fs.jsp");
    ...
    if (!isMultiOperation()) {
      ...
      try {
        propVal =
            getCms()
                .readPropertyObject(
                    getParamResource(), CmsPropertyDefinition.PROPERTY_ENABLE_NOTIFICATION, false)
                .getValue();
   ...
   *
   * <p>If no siblings exist for this resource, an empty string will be returned.
   *
  ...
  public String buildCheckboxModifySiblings() {
    ...
    try {
      if (isMultiOperation()
          || (getCms().readSiblings(getParamResource(), CmsResourceFilter.ALL).size() > 1)) {
    ...
    if (!isMultiOperation()) {
      try {
        propVal =
            getCms()
                .readPropertyObject(
                    getParamResource(), CmsPropertyDefinition.PROPERTY_NOTIFICATION_INTERVAL, false)
                .getValue();
      } catch (CmsException e) {
        if (LOG.isInfoEnabled()) {
          LOG.info(e.getLocalizedMessage());
  ...
  public String buildResponsibleList() {
    ...
    if (isMultiOperation()) {
      ...
      while (i.hasNext()) {
        ...
        try {
          ...
          Iterator<CmsAccessControlEntry> entries =
              cms.getAccessControlEntries(sitePath, false).iterator();
      ...
      if (responsibles.size() == 0) {
        ...
        result.append(
            dialogToggleStart(
                key(Messages.GUI_AVAILABILITY_RESPONSIBLES_0), "responsibles", false));
  ...
  public String getCurrentDateTime() {
    ...
    return CmsCalendarWidget.getCalendarLocalizedTime(
        getLocale(), getMessages(), System.currentTimeMillis());
  }
    ...
    if (isMultiOperation()) {
      ...
      try {
        ...
        if (res.getDateExpired() == CmsResource.DATE_EXPIRED_DEFAULT) {
          ...
          return CmsCalendarWidget.getCalendarLocalizedTime(
              getLocale(), getMessages(), res.getDateExpired());
        }
      } catch (CmsException e) {
        return CmsCalendarWidget.getCalendarLocalizedTime(
            getLocale(), getMessages(), System.currentTimeMillis());
    ...
    if (isMultiOperation()) {
      ...
      try {
        ...
        if (res.getDateReleased() == CmsResource.DATE_RELEASED_DEFAULT) {
          ...
          return CmsCalendarWidget.getCalendarLocalizedTime(
              getLocale(), getMessages(), res.getDateReleased());
        }
      } catch (CmsException e) {
        return CmsCalendarWidget.getCalendarLocalizedTime(
            getLocale(), getMessages(), System.currentTimeMillis());
   ...
   *
   * <p>The enable_notification parameter if content notification is enabled for this resource.
   *
   * <p>
   *
  ...
  /**
   * Returns the value of the notification interval parameter, or null if this parameter was not
   * provided.
   *
   * <p>
   *
   ...
   *
   * <p>The recursive parameter on folders decides if all subresources of the folder should be
   * touched, too.
   *
   * <p>
   *
  ...
  /**
   * Returns the value of the new release date parameter, or null if this parameter was not
   * provided.
   *
   * <p>The release date parameter stores the new release date as String.
   *
   * <p>
   *
  ...
  /**
   * Returns a localized String for "Group", if the flag of a group ACE, and the localization for
   * "User" otherwise.
   *
   * <p>
   *
  ...
  protected boolean performDialogOperation() throws CmsException {
    ...
    // on folder deletion or multi operation display "please wait" screen, not for simple file
    // deletion
    ...
    if (!resetReleaseDate) {
      try {
        if ((CmsStringUtil.isNotEmptyOrWhitespaceOnly(getParamReleasedate()))
          ...
          releaseDate =
              CmsCalendarWidget.getCalendarDate(getMessages(), getParamReleasedate(), true);
        } else {
          leaveReleaseDate = true;
        }
      } catch (ParseException e) {
        throw new CmsException(
            Messages.get().container(Messages.ERR_PARSE_RELEASEDATE_1, getParamReleasedate()), e);
      }
    }
    ...
    if (!resetExpireDate) {
      try {
        ...
        throw new CmsException(
            Messages.get().container(Messages.ERR_PARSE_EXPIREDATE_1, getParamExpiredate()), e);
      }
    }
    ...
    while (i.hasNext()) {
      ...
      try {
        performSingleResourceAvailability(
            resName, releaseDate, expireDate, leaveReleaseDate, leaveExpireDate, modifyRecursive);
      } catch (CmsException e) {
    ...
    while (i.hasNext()) {
      ...
      try {
        performSingleResourceNotification(
            getCms(), resName, notificationEnabled, notificationInterval, modifySiblings);
      } catch (CmsException e) {
        // collect exceptions to create a detailed output
        addMultiOperationException(e);
      }
  ...
  protected void performSingleResourceAvailability(
    ...
    if (expireDate < releaseDate) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_AVAILABILITY_BAD_TIMEWINDOW_0));
    }
