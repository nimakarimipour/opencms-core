...
public class CmsListResourceLinkRelationCollector extends A_CmsListResourceCollector {
   ...
   * @param isSource indicates if the current request shows the source resources for the relations
   *     are shown
   */
  public CmsListResourceLinkRelationCollector(
      A_CmsListExplorerDialog wp, String resource, boolean isSource) {
  ...
  /**
   * @see
   *     org.opencms.workplace.list.A_CmsListResourceCollector#getResource(org.opencms.file.CmsObject,
  ...
  public CmsResource getResource(CmsObject cms, CmsListItem item) {
    ...
    String itemId;
    if (item.getId()
        .startsWith(item.get(CmsResourceLinkRelationList.LIST_COLUMN_RELATION_TYPE) + "_")) {
  ...
  public List<CmsResource> getResources(CmsObject cms, Map<String, String> params) {
    ...
    Map<CmsResource, List<CmsRelationType>> relationTypes =
        new HashMap<CmsResource, List<CmsRelationType>>();
