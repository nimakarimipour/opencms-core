...
public class CmsChtype extends A_CmsListResourceTypeDialog {
    ...
    try {
      ...
      try {
        ...
        throw new CmsException(
            Messages.get().container(Messages.ERR_GET_RESTYPE_1, getParamSelectedType()));
      }
  ...
  public String dialogButtons() {
    ...
    return dialogButtonsOkAdvancedCancel(
        " onclick=\"submitChtype(form);\"", " onclick=\"submitAdvanced(form);\"", null);
  }
  ...
  public String dialogButtonsOkAdvancedCancel(
      String okAttrs, String advancedAttrs, String cancelAttrs) {
  ...
  protected List<CmsListItem> getListItems() throws CmsException {
      ...
      if (m_availableResTypes.indexOf(CmsNewResource.DELIM_PROPERTYVALUES) > -1) {
        availableResTypes =
            CmsStringUtil.splitAsList(
                m_availableResTypes, CmsNewResource.DELIM_PROPERTYVALUES, true);
      } else {
        availableResTypes =
            CmsStringUtil.splitAsList(m_availableResTypes, CmsProperty.VALUE_LIST_DELIMITER, true);
      }
    }
    ...
    List<CmsExplorerTypeSettings> resTypes =
    ...
    for (int i = 0; i < resTypes.size(); i++) {
      ...
      if (isResourceType) {
        ...
        if (!m_advancedMode
            && m_limitedRestypes
            && (availableResTypes.indexOf(type.getTypeName()) == -1)) {
          // this resource type is not in the list of available types, skip it
          continue;
        }

        int resTypeId =
            OpenCms.getResourceManager().getResourceType(settings.getName()).getTypeId();
        ...
        if (isFolder && OpenCms.getResourceManager().getResourceType(resTypeId).isFolder()) {
          changeable = true;
            ...
            && !OpenCms.getResourceManager().getResourceType(resTypeId).isFolder()) {
  ...
  protected void initWorkplaceRequestValues(
    ...
    if (DIALOG_OK.equals(getParamAction())) {
      ...
      setParamTitle(
          key(Messages.GUI_CHTYPE_1, new Object[] {CmsResource.getName(getParamResource())}));
    }
    ...
    if (CmsNewResource.DIALOG_ADVANCED.equals(getParamAction()) || (sessionAttr != null)) {
      ...
      try {
        newResTypesProperty =
            getCms()
                .readPropertyObject(
                    getParamResource(), CmsPropertyDefinition.PROPERTY_RESTYPES_AVAILABLE, true)
                .getValue();
      } catch (CmsException e) {
