...
public class CmsPublishResourcesList extends A_CmsListExplorerDialog {
   ...
   * @param relativeTo the 'relative to' path, this only affects the generation of the path for the
   *     resource
   * @param publishRelated indicates if the related resources should be included
   */
  public CmsPublishResourcesList(
      CmsJspActionElement jsp, String relativeTo, boolean publishRelated) {
  ...
  public I_CmsListResourceCollector getCollector() {
    ...
    if (m_collector == null) {
      m_collector =
          new A_CmsListResourceCollector(this) {
            ...
            public List<CmsResource> getResources(CmsObject cms, Map<String, String> params) {
              ...
              if (m_publishRelated && getSettings().getPublishList().isDirectPublish()) {
                try {
                  CmsPublishList relatedPL =
                      OpenCms.getPublishManager()
                          .getRelatedResourcesToPublish(cms, getSettings().getPublishList());
                  CmsPublishList mergedPL =
                      OpenCms.getPublishManager()
                          .mergePublishLists(cms, getSettings().getPublishList(), relatedPL);
        ...
        if (detailId.equals(LIST_DETAIL_RELATIONS)) {
          ...
          try {
            ...
            Iterator<CmsRelation> itRelations =
                cms.getRelationsForResource(resource, CmsRelationFilter.TARGETS.filterStrong())
                    .iterator();
            ...
            if (((Boolean) item.get(LIST_COLUMN_IS_RELATED)).booleanValue()) {
              ...
              itRelations =
                  cms.getRelationsForResource(resource, CmsRelationFilter.SOURCES.filterStrong())
                      .iterator();
              while (itRelations.hasNext()) {
                CmsRelation relation = itRelations.next();
  ...
  protected boolean isColumnVisible(int colFlag) {
    ...
    isVisible =
        isVisible
            || ((colFlag == LIST_COLUMN_SITE.hashCode())
                && (OpenCms.getSiteManager().getSites().size() > 1));
    return isVisible;
  }
  ...
  protected void setIndependentActions(CmsListMetadata metadata) {
    ...
    /**
     * Class to render a javascript driven detail action button.
     *
     * <p>
     */
    abstract class DetailsJsAction extends A_CmsListIndependentJsAction {
      ...
      /**
       * @see
       *     org.opencms.workplace.list.CmsListIndependentAction#buttonHtml(org.opencms.workplace.CmsWorkplace)
       */
      @Override
    ...
    I_CmsListAction hideAction =
        new DetailsJsAction(LIST_DETAIL_RELATIONS_HIDE) {
    ...
    I_CmsListAction showAction =
        new DetailsJsAction(LIST_DETAIL_RELATIONS_SHOW) {
    ...
    CmsListItemDetails relationsDetails =
        new CmsListItemDetails(LIST_DETAIL_RELATIONS) {
