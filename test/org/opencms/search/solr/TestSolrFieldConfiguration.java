...
public class TestSolrFieldConfiguration extends OpenCmsTestCase {
  ...
  /**
   * Tests if the search content type "content" works correctly.
   *
   * <p>We check if the content is indexed correctly and if cyclic dependencies are correctly
   * handled.
   *
   * <p>'@see /sites/default/extractLinked/extract-linked.xsd'
   *
  ...
  /**
   * Tests the Solr field configuration that can be done in the XSD of an XML content.
   *
   * <p>'@see /sites/default/xmlcontent/article.xsd'
   *
   * @throws Throwable if something goes wrong
   */
  public void testAppinfoSolrField() throws Throwable {
    ...
    CmsSolrQuery squery =
        new CmsSolrQuery(
            ...
            CmsRequestUtil.createParameterMap(
                "q=path:\"/sites/default/xmlcontent/article_0001.html\""));
    CmsSolrResultList results = index.search(getCmsObject(), squery);
    ...
    squery =
        new CmsSolrQuery(
            null,
            CmsRequestUtil.createParameterMap(
                "q=path:\"/sites/default/xmlcontent/article_0002.html\""));
    results = index.search(getCmsObject(), squery);
    res = results.get(0);
    assertEquals("/sites/default/xmlcontent/article_0002.html", res.getRootPath());
    assertTrue(
        res.getMultivaluedField("ateaser2_en_txt")
            .contains("This is teaser 2 in sample article 2."));
    ...
    assertTrue(
        teaser
            .get(0)
            .equals(
                "This is the sample article number 2. This is just a demo teaser. (>>SearchEgg2<<)"));
  ...
  public void testDependencies() throws Throwable {
    ...
    query.setSearchRoots(
        Collections.singletonList(cms.getRequestContext().addSiteRoot(folderName)));
    ...
    for (CmsSearchResource res : results) {
      ...
      if (depField != null) {
        ...
        CmsDocumentDependency d =
            CmsDocumentDependency.fromDependencyString(depField, res.getRootPath());
  ...
  private void checkIndexedSortOptionFields(
    ...
    assertEquals(
        result.getDateField(instanceDateCopyField), result.getDateField("instancedate_dt"));
    assertEquals(
        result.getDateField(instanceDateCopyField), result.getDateField("instancedate_en_dt"));
    assertEquals(
        result.getDateField(instanceDateCopyField),
        result.getDateField("instancedatecurrenttill_dt"));
    assertEquals(
        result.getDateField(instanceDateCopyField),
        result.getDateField("instancedatecurrenttill_en_dt"));
  }
}
