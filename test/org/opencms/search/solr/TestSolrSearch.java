...
public class TestSolrSearch extends OpenCmsTestCase {
  ...
  public void testAdvancedFacetting() throws Throwable {
    ...
    // creating the query:
    // facet=true&facet.field=Title_exact&facet.mincount=1&facet.query=text:OpenCms&rows=0
  ...
  public void testAdvancedHighlighting() throws Throwable {
    ...
    CmsSolrResultList res =
        OpenCms.getSearchManager().getIndexSolr(AllTests.SOLR_ONLINE).search(getCmsObject(), q);
   ...
   *
   * <p>This test was added in order to verify proper generation of resource "root path" information
   * in the index.
   *
   * @throws Throwable if something goes wrong
   */
  ...
  /**
   * Tests an issue where no results are found in folders that have names like <code>
   * /basisdienstleistungen_-_zka/</code>.
   *
   * <p>
   *
  ...
  public void testIssueWithSpecialFoldernames() throws Exception {
    ...
    query =
        "q=+text:\"Alkacon\" +text:\"OpenCms\" +parent-folders:\"/sites/default/\"&sort=path asc";
    results = index.search(getCmsObject(), query);
  ...
  public void testLimitTimeRangesOptimized() throws Exception {
    ...
    // check max date created (must move back one day because of granularity level in optimized date
    // range search)
  ...
  public void testMultipleSearchRoots() throws Exception {
    ...
    String[][] roots =
        new String[][] {
          ...
          new String[] {
            "/sites/default/folder1/", "/sites/default/folder2/", "/sites/default/types/"
          }
        };
    ...
    for (int i = 0; i < expected.length; i++) {
      ...
      System.out.println(
          "Result for search " + i + " (found " + results.size() + ", expected " + expect + ")");
      AllTests.printResults(getCmsObject(), results, false);
  ...
  public void testPermissionFilterAndLimits() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishProject(adminCms, new CmsShellReport(adminCms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    userCms
        .getRequestContext()
        .setCurrentProject(userCms.readProject(CmsProject.ONLINE_PROJECT_ID));
  ...
  public void testQueryDefaults() throws Throwable {
    ...
    // test default query
    String defaultQuery =
        "q=*:*&fl=*,score&qt=edismax&rows=10&fq=expired:[NOW TO *]&fq=released:[* TO NOW]";
    ...
    // test creating default query by String
    String defaultContextQuery =
        "q=*:*&fl=*,score&qt=edismax&rows=10&fq=con_locales:en&fq=parent-folders:\"/sites/default/\"&fq=expired:[NOW TO *]&fq=released:[* TO NOW]";
    ...
    query =
        new CmsSolrQuery(getCmsObject(), CmsRequestUtil.createParameterMap(defaultContextQuery));
    assertEquals(defaultContextQuery, CmsEncoder.decode(query.toString()));
  }
  ...
  public void testQueryParameterStrength() throws Throwable {
    ...
    //        query = new CmsSolrQuery(getCmsObject(),
    // CmsRequestUtil.createParameterMap("fq=parent-folders:\"/\""));
    ...
    String ex =
        "q={!q.op=OR type=lucene qf=text_de}test&fl=pla,plub&qt=lucene&rows=1000&fq=parent-folders:\"/\"&fq=expired:[NOW TO *]&fq=released:[* TO NOW]&fq=con_locales:de&fq=type:article";
  ...
  public void testXmlContent() throws Throwable {
    ...
    CmsSolrIndex index = OpenCms.getSearchManager().getIndexSolr(AllTests.SOLR_ONLINE);
    // String query = "+text:Alkacon +text:OpenCms +text:Text
    // +parent-folders:/sites/default/types/*";
    query = "q=+text:\">>SearchEgg1<<\"";
    ...
    // now search for another Query "xmlcontent", this must not be found in article 4 since it is
    // excluded
    query = "q=+text:\"xmlcontent\"";
    results = index.search(cms, query);
    assertEquals(1, results.size());
    assertEquals("/sites/default/xmlcontent/article_0003.html", (results.get(0)).getRootPath());
    // assertEquals("/sites/default/xmlcontent/article_0004.html",
    // ((CmsSearchResult)results.get(1)).getPath());
  }

  /**
   * Internal helper for test with same name.
   *
   * <p>
   *
   ...
   * @throws Exception in case the test fails
   */
  private void testUppercaseFolderNameUtil(CmsObject cms, String folderName, int expected)
      throws Exception {
    ...
    if (folderName != null) {
      ...
      // publish the project and update the search index
      OpenCms.getPublishManager()
          .publishProject(cms, new CmsShellReport(cms.getRequestContext().getLocale()));
      OpenCms.getPublishManager().waitWhileRunning();
    }
