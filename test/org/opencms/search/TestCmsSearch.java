...
public class TestCmsSearch extends OpenCmsTestCase {
  ...
  public static void printResults(
      List<CmsSearchResult> searchResult, CmsObject cms, boolean showExcerpt) {
    ...
    while (i.hasNext()) {
      ...
      System.out.print(
          CmsStringUtil.padRight(cms.getRequestContext().removeSiteRoot(res.getPath()), colPath));
      String title = res.getField(CmsSearchField.FIELD_TITLE);
      if (title == null) {
        title = "";
      } else {
            ...
            CmsStringUtil.padRight(
                ""
                    + CmsDateUtil.getDateTime(
                        res.getDateLastModified(), DateFormat.SHORT, Locale.GERMAN),
                17));
      }
  ...
  public void testCmsSearchLargeResult() throws Throwable {
    ...
    OpenCms.getSearchManager()
        .getIndex(INDEX_OFFLINE)
        .addConfigurationParameter(CmsSearchIndex.PERMISSIONS, CmsStringUtil.TRUE);
    OpenCms.getSearchManager()
        .getIndex(INDEX_OFFLINE)
        .addConfigurationParameter(CmsSearchIndex.EXCERPT, CmsStringUtil.TRUE);
    ...
    echo(
        "Search1: "
            + cmsSearchBean.getSearchResultCount()
            + " results found, total duration: "
            + duration
            + " ms");
    ...
    OpenCms.getSearchManager()
        .getIndex(INDEX_OFFLINE)
        .addConfigurationParameter(CmsSearchIndex.EXCERPT, CmsStringUtil.FALSE);
    ...
    OpenCms.getSearchManager()
        .getIndex(INDEX_OFFLINE)
        .addConfigurationParameter(CmsSearchIndex.PERMISSIONS, CmsStringUtil.FALSE);
    OpenCms.getSearchManager()
        .getIndex(INDEX_OFFLINE)
        .addConfigurationParameter(CmsSearchIndex.EXCERPT, CmsStringUtil.TRUE);
  ...
  /**
   * Tests if <code>{@link CmsSearch#setQuery(String)}</code> modifies the query in an undesireable
   * way (changes url encoded Strings).
   *
   * <p>
   */
   ...
   * @throws Exception in case the test fails
   */
  private void testCmsSearchUppercaseFolderNameUtil(CmsObject cms, String folderName, int expected)
      throws Exception {
