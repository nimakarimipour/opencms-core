...
/**
 * Tests the text extraction form a Pdf file.
 *
 * <p>
 */
public class TestMsPowerPointExtraction extends OpenCmsTestCase {
  ...
  /**
   * Tests the PowerPoint text extraction for old OLE2 documents.
   *
   * <p>Also checks special chars like "&auml; &ouml; &uuml; &Auml; &Ouml; &Uuml; &szlig;"
   *
   * <p>
   *
  ...
  public void testPPtExtractionOLE2() throws Exception {
    ...
    InputStream in =
        getClass().getClassLoader().getResourceAsStream("org/opencms/search/extractors/test1.ppt");

    // extract the content
    I_CmsExtractionResult extractionResult =
        CmsExtractorMsOfficeOLE2.getExtractor().extractText(in);
    ...
    // This should be 11, adding 10 for CmsTextExtractor.extractText, and adding one for new
    // CmsExtractionResult.
    ...
    // NOTE: Euro symbol conversion fails - possible reason is that Extraction method class handles
    // only ISO
    // this is "&auml; &ouml; &uuml; &Auml; &Ouml; &Uuml; &szlig;"
    assertTrue(result.indexOf("\u00e4\u00f6\u00fc\u00c4\u00d6\u00dc\u00df") > -1);

    assertEquals(
        "Alkacon Software - The OpenCms experts", items.get(I_CmsExtractionResult.ITEM_TITLE));
  ...
  /**
   * Tests the PowerPoint text extraction for new (MS Office 2007) OOXML documents.
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  public void testPPtExtractionOOXML() throws Exception {
    ...
    @SuppressWarnings("resource")
    InputStream in =
        getClass().getClassLoader().getResourceAsStream("org/opencms/search/extractors/test1.pptx");

    // extract the content
    I_CmsExtractionResult extractionResult =
        CmsExtractorMsOfficeOOXML.getExtractor().extractText(in);
    Map<String, String> items = extractionResult.getContentItems();
    ...
    // This should be 10, adding 9 for CmsTextExtractor.extractText, and adding one for new
    // CmsExtractionResult.
    ...
    // either I am doing something wrong or Tika 0.9 does not support the "company" and "comment"
    // meta information
    // assertEquals("Alkacon Software", items.get(I_CmsExtractionResult.ITEM_COMPANY));
    // assertEquals("This is the comment", items.get(I_CmsExtractionResult.ITEM_COMMENTS));
