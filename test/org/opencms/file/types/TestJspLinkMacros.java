...
public class TestJspLinkMacros extends OpenCmsTestCase {
  ...
  /**
   * Test link parsing when using the link macro.
   *
   * <p>
   *
  ...
  public void testLinkParsing() throws Throwable {
    ...
    CmsResource res2 =
        cms.createResource(
            targetName, CmsResourceTypeJsp.getJSPTypeId(), "Test".getBytes("UTF-8"), null);
    CmsResource res =
        cms.createResource(
            sourceName,
    ...
    List links =
        ((I_CmsLinkParseable) OpenCms.getResourceManager().getResourceType(file))
            .parseLinks(cms, file);
    assertEquals(1, links.size());
    assertEquals(
        new CmsLink(
            "link0", CmsRelationType.JSP_STRONG, res2.getStructureId(), res2.getRootPath(), true),
        links.get(0));
  }
  ...
  public void testLinkGeneration() throws Throwable {
    ...
    String result =
        macroResolver.resolveMacros(CmsEncoder.createString(file.getContents(), "UTF-8"));
