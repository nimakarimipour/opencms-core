...
public class TestPublishing extends OpenCmsTestCase {
  ...
  public void testPublishChangedFiles() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(
            cms, resource3, true, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testPublishContentDate() throws Throwable {
    ...
    assertTrue(
        cms.readResource(offlineResource.getStructureId()).getDateContent()
    ...
    cms.writePropertyObject(
        resName, new CmsProperty(CmsPropertyDefinition.PROPERTY_TITLE, "abc", "def"));
   ...
   *
   * <p>take a modified resource: - create a sibling of it - publish only the sibling - compare
   * online/offline versions of the sibling
   *
   * @throws Throwable if something goes wrong
   */
  ...
  public void testPublishDeletedFiles() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(
            cms, resource2, false, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
   ...
   *
   * <p>take 2 siblings (s1 and s2): - delete one - publish the deleted one - the remaining sibling
   * should be readable in online project - the deleted sibling should be eligible for restoration
   *
   * @throws Throwable if something goes wrong
   */
  public void testPublishDeletedSiblings() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(
            cms, source, true, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    OpenCms.getPublishManager()
        .publishResource(
            cms, sibling, false, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    assertEquals(
        new String("test text".getBytes()), new String(cms.readFile(source).getContents()));
    cms.getRequestContext().setCurrentProject(offline);
   ...
   *
   * <p>take 2 siblings (s1 and s2): - delete all - publish all - non of them should be readable -
  ...
  public void testPublishDeletedSiblings2() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(
            cms, sibling, true, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testPublishFolderWithLockedFiles() throws Throwable {
    ...
    CmsPublishList publishList =
        OpenCms.getPublishManager()
            .getPublishList(cms, Collections.singletonList(cms.readResource(folder)), false, false);
    OpenCms.getPublishManager()
        .publishProject(cms, new CmsShellReport(cms.getRequestContext().getLocale()), publishList);
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testPublishMovedFiles() throws Throwable {
    ...
    // publish it
    CmsPublishList publishList =
        OpenCms.getPublishManager().getPublishList(cms, cms.readResource(target1), false);
    OpenCms.getPublishManager()
        .publishProject(cms, new CmsShellReport(cms.getRequestContext().getLocale()), publishList);
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testPublishNewFiles() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(
            cms, destination3, true, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testPublishNewFilesInNewFolder() throws Throwable {
    ...
    try {
      ...
      if (ex.getMessageContainer().getKey()
          == org.opencms.db.Messages.ERR_DIRECT_PUBLISH_PARENT_NEW_2) {
        error = false;
      }
    }
  ...
  public void testPublishProjectLastmodified() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(cms, path, true, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testPublishProjectLastmodifiedFolder() throws Throwable {
    ...
    CmsPublishList pubList =
        OpenCms.getPublishManager().getPublishList(cms, cms.readResource(path), false);
    assertTrue(pubList.isPublishSubResources());
    assertTrue(pubList.getAllResources().isEmpty());
  }

  /**
   * Test an issue with the publish queue when trying to publish a folder that has already a
   * subresource that is being published.
   *
   * <p>FIRST TRY: - modify and lock a file, publish the file - modify and lock a second file,
   * publish the folder
   *
   * @throws Throwable if something goes wrong
   */
  public void testPublishQueueIssue1() throws Throwable {
    ...
    // stop the publish engine to ensure that the file will not be published before we try to
    // publish the folder
   ...
   * subresource that is being published.
   *
   * <p>SECOND TRY: - modify and lock a file, publish the file - lock the folder, modify a second
   * file, publish the folder
   *
   * @throws Throwable if something goes wrong
   */
   ...
   * subresource that is being published.
   *
   * <p>THIRD TRY: - modify and lock a file, publish the file - modify and lock the folder, publish
   * the folder
   *
   * @throws Throwable if something goes wrong
   */
  ...
  public void testPublishRelatedFiles() throws Throwable {
    ...
    // get the list of related resources, which should be still empty, since the related resource
    // has unchanged state
    CmsPublishList relatedList =
        OpenCms.getPublishManager().getRelatedResourcesToPublish(cms, pubList);
    ...
    CmsPublishList mergedList =
        OpenCms.getPublishManager().mergePublishLists(cms, pubList, relatedList);
    // check the publish list has not been touched
    assertTrue(pubList.getDeletedFolderList().isEmpty());
    assertTrue(pubList.getFolderList().isEmpty());
    assertEquals(1, pubList.getFileList().size());
   ...
   *
   * <p>take 2 siblings (s1 and s2): - change a shared property of s1 - s2 will be marked as changed
   * also - direct publishing s1, should also change s2
   *
   * @throws Throwable if something goes wrong
   */
  public void testPublishResourceProperty() throws Throwable {
    ...
    cms.lockResource(sibling);
    cms.writePropertyObject(
        sibling, new CmsProperty(CmsPropertyDefinition.PROPERTY_TITLE, null, "test4"));
    cms.unlockResource(sibling);
    ...
    assertEquals(
        ((CmsResource) pubList.get(0)).getRootPath(), cms.getRequestContext().addSiteRoot(source));
    assertEquals(
        ((CmsResource) pubList.get(1)).getRootPath(), cms.getRequestContext().addSiteRoot(sibling));
  ...
  public void testPublishResourceWithRelatedGalore() throws Throwable {
    ...
    CmsPublishList pubList =
        OpenCms.getPublishManager().getPublishList(cms, cms.readResource(resourcename), false);
    CmsPublishList relatedList =
        OpenCms.getPublishManager().getRelatedResourcesToPublish(cms, pubList);
    ...
    for (int i = 0; i < 10; i++) {
      ...
      CmsPublishList publishList =
          OpenCms.getPublishManager()
              .getPublishList(cms, cms.readResource(resourcename, CmsResourceFilter.ALL), false);
      // get the related resources
      CmsPublishList relResources =
          OpenCms.getPublishManager().getRelatedResourcesToPublish(cms, publishList);
      // merge the two publish lists
      publishList = OpenCms.getPublishManager().mergePublishLists(cms, publishList, relResources);
      // publish
      OpenCms.getPublishManager()
          .publishProject(
              cms, new CmsShellReport(cms.getRequestContext().getLocale()), publishList);
    }
    OpenCms.getPublishManager().waitWhileRunning();
   ...
   *
   * <p>take 2 siblings (s1 and s2): - change a individual property of s1 - change a shared property
   * of s2 - direct publishing s2, should not change s1
   *
   * @throws Throwable if something goes wrong
   */
  public void testPublishSiblings() throws Throwable {
    ...
    cms.writePropertyObject(
        sibling, new CmsProperty(CmsPropertyDefinition.PROPERTY_TITLE, "test2", null));
    cms.unlockResource(sibling);
    ...
    cms.writePropertyObject(
        source, new CmsProperty(CmsPropertyDefinition.PROPERTY_NAVTEXT, null, "test3"));
    cms.unlockResource(source);
    ...
    assertEquals(
        ((CmsResource) pubList.get(0)).getRootPath(), cms.getRequestContext().addSiteRoot(sibling));
    assertFilter(cms, cms.readResource(sibling), OpenCmsTestResourceFilter.FILTER_EQUAL);
   ...
   *
   * <p>take 2 siblings (s1 and s2): - change a individual property of s1 - s2 remains unchanged -
  ...
  public void testPublishStructureProperty() throws Throwable {
    ...
    cms.lockResource(sibling);
    cms.writePropertyObject(
        sibling, new CmsProperty(CmsPropertyDefinition.PROPERTY_TITLE, "test1", null));
    cms.unlockResource(sibling);
  ...
  /**
   * Returns a project for testing purposes.
   *
   * <p>
   *
