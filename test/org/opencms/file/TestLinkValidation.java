...
public class TestLinkValidation extends OpenCmsTestCase {
   ...
   */
  public static void setContent(CmsObject cms, String filename, String content)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public static void setXmlContent(CmsObject cms, String filename, String html, String link)
      throws CmsException {
  ...
  public void testInternalLinkValidation() throws Throwable {
    ...
    CmsInternalLinksValidator validator =
        new CmsInternalLinksValidator(cms, Collections.singletonList("/"));
   ...
   * @throws Exception if something goes wrong
   */
  private void restore(CmsObject cms, CmsResource resource, CmsShellReport report)
      throws Exception {
    ...
    I_CmsHistoryResource history =
        (I_CmsHistoryResource) historicalVersions.get(historicalVersions.size() - 1);
   ...
   * @throws CmsException if something goes wrong
   */
  private void setXmlContentFileRef(CmsObject cms, String filename, String link1, String link2)
      throws CmsException {
   ...
   * <p>
   *
   * @param mode the mode: <code>0</code> for hmlpage, <code>1</code> for xmlcontent with html and
   *     file ref, <code>2</code> for xmlcontent with html only, <code>3</code> for xmlcontent with
   *     file ref only
   * @throws Exception if something goes wrong
   */
  private void testLinkValidation(int mode) throws Exception {
    ...
    // set the content
    String content1 =
        "<a href='" + filename2 + "'>file2</a><br><a href='" + filename3 + "'>file3</a>";
    ...
    Map validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res1), false),
                report);
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res2), false),
                report);
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res3), false),
                report);
    assertTrue(validation.isEmpty());
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res4), false),
                report);
    assertEquals(validation.size(), 1);
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res5), false),
                report);
    assertEquals(validation.size(), 1);
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res6), false),
                report);
    assertEquals(validation.size(), 1);
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms,
                OpenCms.getPublishManager()
                    .getPublishList(cms, Collections.singletonList(res7), false),
                report);
    assertTrue(validation.isEmpty());
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms, OpenCms.getPublishManager().getPublishList(cms, res56, false), report);
    assertTrue(validation.isEmpty());
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms, OpenCms.getPublishManager().getPublishList(cms, res123, false), report);
    assertTrue(validation.isEmpty());
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms, OpenCms.getPublishManager().getPublishList(cms, resAll, false), report);
    assertTrue(validation.isEmpty());
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms, OpenCms.getPublishManager().getPublishList(cms, resources, false), report);
    assertTrue(validation.isEmpty());
    switch (mode) {
      case 1:
        setXmlContent(cms, filename6, content6, filename5);
    ...
    validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms, OpenCms.getPublishManager().getPublishList(cms, res78, false), report);
    assertTrue(validation.isEmpty());
                ...
                OpenCms.getPublishManager()
                    .getPublishList(
                        ...
                        Collections.singletonList(
                            cms.readResource(filename7, CmsResourceFilter.ALL)),
                        false),
                report);
   ...
   * @throws Exception if something goes wrong
   */
  private Map validateAfterDelete(
      CmsObject cms, List resources, List allResources, CmsShellReport report) throws Exception {
    ...
    Map validation =
        OpenCms.getPublishManager()
            .validateRelations(
                cms, OpenCms.getPublishManager().getPublishList(cms, otherRes, false), report);
