...
public class TestResourceFilter extends OpenCmsTestCase {
  ...
  /**
   * Tests filters that should only allow folders (obtained by {@link
  ...
  public void testAddRequireFolder() throws Exception {
    ...
    echo(
        "Testing if only folders pass the resource filter CmsResourceFilter.ALL.addRequireFolder()");
    assertFilterFolderOnly(filterFolder);

    filterFolder =
        CmsResourceFilter.ALL.addRequireFolder().addExcludeState(CmsResource.STATE_DELETED);
