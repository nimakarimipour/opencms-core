...
public class TestCopy extends OpenCmsTestCase {
  ...
  public void testCopyCategories() throws Exception {
    ...
    assertTrue(
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, category, true, null)
            .contains(cms.readResource(resname)));
    assertTrue(
        CmsCategoryService.getInstance()
            .readResourceCategories(cms, resname)
            .contains(CmsCategoryService.getInstance().readCategory(cms, category, null)));
    ...
    assertTrue(
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, category, true, null)
            .contains(cms.readResource(copyname)));
    assertTrue(
        CmsCategoryService.getInstance()
            .readResourceCategories(cms, copyname)
            .contains(CmsCategoryService.getInstance().readCategory(cms, category, null)));
  }
  ...
  /**
   * Tests the copy operation for a folder, checking if the date of the new folder is the current
   * date.
   *
   * <p>
   *
  ...
  public void testCopyFolderDateIssue() throws Exception {
    ...
    CmsObject cms = getCmsObject();
    echo(
        "Testing copy operation for a folder, checking if the date of the new folder is the current date");
  ...
  /**
   * Tests the to copy a single resource to a destination that already exists but is marked as
   * deleted.
   *
   * <p>
   *
  ...
  public void testCopyOverwriteDeletedFile() throws Exception {
    ...
    assertFilter(
        ...
        source1,
        destination,
        OpenCmsTestResourceFilter.FILTER_COPY_SOURCE_DESTINATION_AS_SIBLING);
  ...
  /**
   * Tests the to copy a single resource to a destination that already exists but is marked as
   * deleted and locked by another user.
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  public void testCopySingleResourceAsNew() throws Exception {
    ...
    cms.createProject(
        "testproject", "a test project", "Users", "Users", CmsProject.PROJECT_TYPE_NORMAL);
