...
public class TestPermissions extends OpenCmsTestCase {
  ...
  public void testFilterForFolder() throws Throwable {
    ...
    resultList =
        cms.getResourcesInFolder(
            folder, CmsResourceFilter.requireType(CmsResourceTypeImage.getStaticTypeId()));
  ...
  public void testLockStatusPermission() throws Throwable {
    ...
    cms.getRequestContext().setCurrentProject(cms.readProject("Offline"));
    assertTrue(
        cms.hasPermissions(res, CmsPermissionSet.ACCESS_WRITE, false, CmsResourceFilter.ALL));
    assertFalse(
        cms.hasPermissions(res, CmsPermissionSet.ACCESS_WRITE, true, CmsResourceFilter.ALL));
  ...
  public void testPublishPermissions() throws Throwable {
    ...
    // Update for 9.5:
    // In the default installation the "Users" group now is set as manager group for the Offline
    // project.
  ...
  public void testSiblingPermissions() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(cms, resName2, true, new CmsLogReport(Locale.ENGLISH, getClass()));
    OpenCms.getPublishManager().waitWhileRunning();
      ...
      if (cms.lookupPrincipal(principal) == null) {
        if (CmsRole.valueOfId(principal) == null) { // it could also be a role
          if (!principal.equals(CmsAccessControlEntry.PRINCIPAL_ALL_OTHERS_ID)
              && !principal.equals(
                  CmsAccessControlEntry.PRINCIPAL_OVERWRITE_ALL_ID)) { // or a special id
  ...
  public void testVisiblePermissionForFolder() throws Throwable {
    ...
    boolean hasViewAccess =
        cms.hasPermissions(
            ...
            CmsResourceFilter.ONLY_VISIBLE);
    assertFalse(
        "the user has view access permission despite the view permission has been removed",
        hasViewAccess);
