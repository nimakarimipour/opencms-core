...
public class TestLock extends OpenCmsTestCase {
   ...
   * Tests lock status of a folder and its siblings.
   *
   * <p>In this test, the folder has some pre-locked siblings with exclusive locks in it
   *
   * @throws Throwable if something goes wrong
   */
   ...
   * Tests lock status of a folder and its siblings.
   *
   * <p>In this test, the folder has some prelocked siblings with shared locks in it
   *
   * @throws Throwable if something goes wrong
   */
  ...
  public void testLockRequired() throws Throwable {
    ...
    try {
      cms.writePropertyObject(
          source, new CmsProperty(CmsPropertyDefinition.PROPERTY_TITLE, "New title", null));
    } catch (CmsLockException e) {
      // must throw a security exception because resource is not locked
      needLock = true;
    }
    ...
    if (!needLock) {
      fail(
          "Write property list operation on resource permitted without a lock on the current user!");
    }
    ...
    try {
      cms.replaceResource(
          source, CmsResourceTypePlain.getStaticTypeId(), "Kaputt".getBytes(), null);
    } catch (CmsLockException e) {
      // must throw a security exception because resource is not locked
      needLock = true;
    }
    ...
    if (!needLock) {
      fail(
          "Change permissions operation on resource permitted without a lock on the current user!");
    }
   ...
   *
   * <p>Issue description: User A creates a new file, but deletes it without ever publishing it. Now
   * user B create a new file with the same name / path. The file was still in the lock manager but
   * for user A, this generated an error for user B.
   *
   * <p>Solution: Remove new files that are deleted from the lock manager.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
