 ...
 */
/**
 * Comment for <code>TestPermissions</code>.
 *
 * <p>
 */
public class TestPublishIssues extends OpenCmsTestCase {
   ...
   *
   * <p>This scenario is described as follows: If a direct publish is made, and the option "publish
   * all siblings" is selected, a file that contains siblings will be added multiple times to the
   * publish list, causing a primary key collision in the publish history table.
   *
   * <p>
   *
  ...
  public void testDirectPublishWithSiblings() throws Throwable {
    ...
    CmsUUID publishId =
        OpenCms.getPublishManager()
            .publishResource(
                cms, "/folder1/", true, new CmsShellReport(cms.getRequestContext().getLocale()));
  ...
  public void testMultipleProjectCreation() throws Exception {
    ...
    // now create the project again - this must NOT throw an exception since projects may have the
    // same name
   ...
   *
   * <p>This displays an issue in the OpenCms v6 project driver, where a project is created with a
   * UNIQUE INDEX based on the given name and the current system time. In rare instances the system
   * time may be identical and an error occurs.
   *
   * <p>The issue was found in the MySQL project driver, but others DBs may be affected as well.
   *
   * <p>
   *
   * @throws Exception in case the test fails
  ...
  public void testPublishFolderWithNewFileFromOtherProject() throws Throwable {
    ...
    CmsObject cms = getCmsObject();
    echo(
        "Testing publishing a deleted folder containing a file that was new created in other project");
   ...
   *
   * <p>This scenario is described as follows: We have users "test1" and "test2". We have two
   * projects, "project1" and "project2". Project "project1" consists of the folder "/". Project
   * "project2" consists of the folder "/folder1/subfolder11/". User "test2" edits the file
   * "/folder1/subfolder11/index.html". After this, user "test1" locks the folder "/folder1" in
   * "project1", and unlocks it again. User "test2" logs in and now publishes "project2".
   *
   * <p>Wanted result: the changed resource "/folder1/subfolder11/index.html" is published with
   * "project2".
   *
   * <p>The test illustrates a change in the logic from OpenCms 5 to OpenCms 6: In OpenCms 5,
   * locking a file caused it to switch to the current users project. In OpenCms 6, this is no
   * longer true, at last not if you just lock a parent folder. So in OpenCms 5, this test would
   * fail, since the resource would be in "project1", not "project2".
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
   ...
   *
   * <p>This scenario is described as follows: We have users "test1" and "test2" and projects
   * "projectA" and "projectB". Both projects contain all resources. We have two folders "/foldera/"
   * and "/folderb/". There is a resource "test.txt" that has a sibling in both folders. User
   * "test1" locks folder "/foldera/" and user "test2" locks folder "/folderb". Now both users try
   * to edit the sibling of "test.txt" in their folder.
   *
   * <p>TODO: How are concurrent file modifications avoided on the sibling?
   *
   * @throws Throwable if something goes wrong
   */
   ...
   *
   * <p>This scenario is described as follows: Direct publishing of folders containing subfolders
   * skips all changed subfolders e.g. direct publish of /folder1/ publishes /folder1/ and
   * /folder1/index.html/, but not /folder1/subfolder11/.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
   ...
   *
   * <p>This scenario is described as follows:
   *
   * <p>Direct publishing of folders containing subfolders skips all (sibling) resources in
   * subfolders.
   *
   * <p>e.g. direct publish of /folder2/folder1/ publishes /folder2/folder1/ and
   * /folder2/folder1/index.html/, but not /folder2/folder1/subfolder11/index.html.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  public void testPublishScenarioD() throws Throwable {
    ...
    Iterator itResources =
        cms.readResources("/folder2/folder1/", CmsResourceFilter.ALL, true).iterator();
   ...
   * <p>This scenario is described as follows:
   *
   * <p>Deletion of folders containing shared locked siblings, after copying a folder creating
   * siblings into a new folder and publishing.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  public void testPublishScenarioE() throws Throwable {
    ...
    // create siblings
    cms.copyResource(
        "/folder1/subfolder12/subsubfolder121", "/test/subtest", CmsResource.COPY_AS_SIBLING);
   ...
   * <p>This scenario is described as follows:
   *
   * <p>We have 2 siblings: sibA.txt and sibB.txt We set a shared property and we publish just one
   * sibling let's say sibA.txt.
   *
   * <p>After publishing both siblings should be unchanged
   *
   * @throws Throwable if something goes wrong
   */
  public void testPublishScenarioF() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(cms, sibA, false, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
   ...
   * <p>This scenario is described as follows:
   *
   * <p>We have 2 siblings: sib1.txt and sib2.txt We do a content modification and we publish just
   * one sibling let's say sib1.txt.
   *
   * <p>After publishing both siblings should be unchanged
   *
   * @throws Throwable if something goes wrong
  ...
  public void testPublishScenarioG() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(cms, sib1, false, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
   ...
   * <p>This scenario is described as follows:
   *
   * <p>We have 2 unchanged siblings: sibX.txt and sibY.txt Now we set a different individual
   * property on each sibling and we publish just one sibling let's say sibX.txt.
   *
   * <p>After publishing only sibX.txt should be unchanged and sibY.txt should still be changed
   *
   * @throws Throwable if something goes wrong
   */
  public void testPublishScenarioH() throws Throwable {
    ...
    OpenCms.getPublishManager()
        .publishResource(cms, sibX, false, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    OpenCms.getPublishManager()
        .publishResource(cms, sibY, false, new CmsShellReport(cms.getRequestContext().getLocale()));
    OpenCms.getPublishManager().waitWhileRunning();
