...
public class TestMoveRename extends OpenCmsTestCase {
  ...
  public void testMultipleMoveResource() throws Throwable {
    ...
    // check lock
    assertFalse(
        cms.getLockedResources("/folder1", CmsLockFilter.FILTER_ALL).contains(destination1));
  ...
  public void testOverwriteMovedResource() throws Exception {
    ...
    // check lock
    assertFalse(
        cms.getLockedResources("/xmlcontent", CmsLockFilter.FILTER_ALL).contains(originalResource));

    cms.moveResource(intermediaryDestination, finalDestination);

    // check lock
    assertFalse(
        cms.getLockedResources("/xmlcontent", CmsLockFilter.FILTER_ALL)
            .contains(intermediaryDestination));
    ...
    // check lock
    assertFalse(
        cms.getLockedResources("/xmlcontent", CmsLockFilter.FILTER_ALL).contains(finalDestination));
  }
  ...
  public void testPublishDeletedFolderWithMovedResource() throws Exception {
    ...
    // check lock
    assertFalse(
        cms.getLockedResources(deletedFolder, CmsLockFilter.FILTER_ALL)
            .contains(deletedFolder + source));
