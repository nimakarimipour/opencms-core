...
public class StringBean extends NodeVisitor implements Serializable {
  ...
  /**
   * If <code>true</code> sequences of whitespace characters are replaced with a single space
   * character.
   */
   ...
   *
   * <p><code>Links</code> is set <code>false</code> so text appears like a browser would display
   * it, albeit without the colour or underline clues normally associated with a link.
   *
   * <p><code>ReplaceNonBreakingSpaces</code> is set <code>true</code>, so that printing the text
   * works, but the extra information regarding these formatting marks is available if you set it
   * false.
   *
   * <p><code>Collapse</code> is set <code>true</code>, so text appears compact like a browser would
   * display it.
   */
  public StringBean() {
  ...
  /**
   * Get the current 'collapse whitespace' state. If set to <code>true</code> this emulates the
   * operation of browsers in interpretting text where <quote>user agents should collapse input
   * white space sequences when producing output inter-word space</quote>. See HTML specification
   * section 9.1 White space <a href="http://www.w3.org/TR/html4/struct/text.html#h-9.1">
   * http://www.w3.org/TR/html4/struct/text.html#h-9.1</a>.
   *
   * @return <code>true</code> if sequences of whitespace (space '&#92;u0020', tab '&#92;u0009',
   *     form feed '&#92;u000C', zero-width space '&#92;u200B', carriage-return '\r' and NEWLINE
   ...
   *
   * @return <code>true</code> if link text is included in the text extracted from the URL, <code>
  ...
  /**
   * Set the current 'collapse whitespace' state. If the setting is changed after the URL has been
   * set, the text from the URL will be reacquired, which is possibly expensive.
   *
   * @param collapse If <code>true</code>, sequences of whitespace will be reduced to a single
   *     space.
   */
  ...
  /**
   * Set the 'include links' state. If the setting is changed after the URL has been set, the text
   * from the URL will be reacquired, which is possibly expensive.
   *
   * @param links Use <code>true</code> if link text is to be included in the text extracted from
   *     the URL, <code>false</code> otherwise.
   */
  ...
  /**
   * Appends a NEWLINE to the output if the tag breaks flow, and possibly sets the state of the PRE
   * and SCRIPT flags.
   *
  ...
  /**
   * Appends a newline to the buffer if there isn't one there already. Except if the buffer is
   * empty.
   */
