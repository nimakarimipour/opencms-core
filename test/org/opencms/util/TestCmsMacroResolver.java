...
public class TestCmsMacroResolver extends OpenCmsTestCase {
  ...
  private static final String MACRO_TEST_I1 =
  ...
  public void testResolveLocalizedMacros() {
    ...
    CmsMessages messages =
        new CmsMessages(org.opencms.xml.content.Messages.get().getBundleName(), Locale.ENGLISH);
  ...
  public void testResolveMacros() {
    ...
    result = resolver.resolveMacros(content);
    assertEquals(
        "REPLACED<<This is a prefix >>REPLACEDREPLACED<<This is a suffix>>REPLACED", result);
    ...
    result = resolver.resolveMacros(content);
    assertEquals(
        "REPLACED<<This is a prefix >>REPLACED${unknown}REPLACED<<This is a suffix>>REPLACED",
        result);
  }
  ...
  public void testResolveMacrosCombinedSyntax() {
    ...
    content =
        "<<This is a prefix >>%(test}-%{test)-%{test}-${test)-$(test}-$(test)<<This is a suffix>>";
    result = resolver.resolveMacros(content);
    // assertEquals("<<This is a prefix >>%(test}-%{test)-%{test}-${test)-$(test}-$(test)<<This is a
    // suffix>>", result);
  ...
  public void testResolverInterface() {
    ...
    result = resolver.resolveMacros(content);
    assertEquals(
        "$REPLACED$}REPLACED is ${ what REPLACED{$} uncle ${REPLACED Scrooge $ owns ${REPLACED}",
        result);
  }
