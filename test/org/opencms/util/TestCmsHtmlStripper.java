...
/**
 * Test case for <code>{@link org.opencms.util.CmsHtmlStripper}</code>.
 *
 * <p>
 *
 * @since 6.9.2
...
public class TestCmsHtmlStripper extends OpenCmsTestCase {
  ...
  /**
   * Tests <code>{@link CmsHtmlStripper#stripHtml(String)}</code> with a configuration that only
   * keeps: "b,p,strong,i".
   *
   * <p>
   *
  ...
  /**
   * Tests <code>{@link CmsHtmlStripper#stripHtml(String)}</code> with all HTML tags of test files
   * as preserve tags (manual bypass-mode).
   *
   * <p>
   *
   * @throws Exception in case the test fails
  ...
  /**
   * Tests <code>{@link CmsHtmlStripper#stripHtml(String)}</code> if it strips newline separators.
   *
   * <p>
   *
   * @throws Exception in case the test fails
  ...
  public void testStripHtmlNewline() throws Exception {
    ...
    String test =
        "<p>Dies ist eine Paragrahph\r hier die 2. Zeile \r\n und die dritte.</p> Nach dem Paragrahp";
    CmsHtmlStripper stripper = new CmsHtmlStripper();
    stripper.addPreserveTag("p");
