...
public class TestCmsUUID extends OpenCmsTestCase {
  ...
  public void testUUIDDeSerialization() throws Exception {
    ...
    // de-serialization of CmsUUIDs before 6.2.0 is impossible because of changes to the UUID
    // implementation

    //        CmsUUID uuid_v605 =
    // (CmsUUID)deSerializeObjectFromFile("org/opencms/util/uuid_v605.bmp");
    //        System.out.println("De-Serialized from version 6.0.5: " + uuid_v605.toString());
    //
    //        CmsUUID uuid_v600 =
    // (CmsUUID)deSerializeObjectFromFile("org/opencms/util/uuid_v600.bmp");
    //        System.out.println("De-Serialized from version 6.0.0: " + uuid_v600.toString());
    //
    //        CmsUUID uuid_v535 =
    // (CmsUUID)deSerializeObjectFromFile("org/opencms/util/uuid_v535.bmp");
  ...
  public void testUUIDEquals() throws Exception {
    ...
    System.out.println(
        "Generating a lot of UUIDs in a loop very fast - warnings may pe printed to console...");
    int testSize = 100000;
    ...
    System.out.println(
        "Time for UUID equals() implementation for " + testSize + " UUIDs : " + time1);
  }

  /**
   * Tests the {@link CmsUUID#isValidUUID(String)} method.
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  protected Object deSerializeObjectFromFile(String name) throws Exception {
    ...
    ObjectInputStream oin =
        new ObjectInputStream(getClass().getClassLoader().getResourceAsStream(name));
    return oin.readObject();
  }
