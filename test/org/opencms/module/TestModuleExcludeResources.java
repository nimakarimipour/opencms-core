...
public class TestModuleExcludeResources extends OpenCmsTestCase {
    ...
    TestSetup wrapper =
        new TestSetup(suite) {
          ...
          protected void setUp() {
            ...
            String path =
                OpenCms.getSystemInfo().getPackagesRfsPath() + CmsSystemInfo.FOLDER_MODULES;
  ...
  /**
   * Tests module import, export and deletion where excluded resources are relevant. All actions are
   * performed relative to the root site.
   */
  ...
  /**
   * Tests module import, export and deletion where excluded resources are relevant. All actions are
   * performed relative to the "/sites/default".
   */
  ...
  /**
   * The test method used by both tests of the class. It imports, exports and deletes modules and
   * checks if the exclude resources set for one module do the job correctly.
   *
   * @param siteRoot the site root, relative to which the modules are imported, exported and
   *     deleted.
   */
  private void testModuleExcludeResources(String siteRoot) {
    ...
    try {
      ...
      CmsModuleImportExportHandler exportHandler =
          CmsModuleImportExportHandler.getExportHandler(cms, module, "Exporting " + mainModule);
      ...
      moduleManager.deleteModule(
          cms, mainModule, false, new CmsShellReport(cms.getRequestContext().getLocale()));

      assertTrue(
          "After main-module deletion, the resource " + includedResource + "is still present.",
          !cms.existsResource(includedResource));
      assertTrue(
          "After main-module deletion, the expected resource "
              + excludedResource
      ...
      OpenCms.getModuleManager()
          .deleteModule(
              cms, subModule, false, new CmsShellReport(cms.getRequestContext().getLocale()));
