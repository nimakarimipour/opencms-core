...
public class TestModuleOperations extends OpenCmsTestCase {
  ...
  public void testModuleActionClass() throws Throwable {
    ...
    OpenCms.getModuleManager()
        .deleteModule(
            cms, module.getName(), false, new CmsShellReport(cms.getRequestContext().getLocale()));
  ...
  public void testModuleDependencies() throws Throwable {
    ...
    String moduleFile =
        OpenCms.getSystemInfo()
            .getAbsoluteRfsPathRelativeToWebInf("packages/" + moduleName + ".zip");
    ...
    CmsModuleDependency dep1 =
        new CmsModuleDependency("org.opencms.test.dependency1", new CmsModuleVersion("1.0"));
    CmsModuleDependency dep2 =
        new CmsModuleDependency("org.opencms.test.dependency2", new CmsModuleVersion("2.0"));
    ...
    CmsModuleDependency dep3 =
        new CmsModuleDependency("org.opencms.test.dependency2", new CmsModuleVersion("1.0"));
    ...
    try {
      OpenCms.getModuleManager()
          .deleteModule(
              cms, dep1.getName(), false, new CmsShellReport(cms.getRequestContext().getLocale()));
    ...
    try {
      OpenCms.getModuleManager()
          .deleteModule(
              cms, dep2.getName(), false, new CmsShellReport(cms.getRequestContext().getLocale()));
    } catch (CmsConfigurationException e) {
      caughtException = true;
    }
    if (!caughtException) {
    ...
    CmsModuleDependency dep4 =
        new CmsModuleDependency("org.opencms.test.modules.test2", new CmsModuleVersion("1.5"));

    module = OpenCms.getModuleManager().getModule(dep1.getName());
    dependencies =
        OpenCms.getModuleManager()
            .checkDependencies(module, CmsModuleManager.DEPENDENCY_MODE_DELETE);
    ...
    OpenCms.getModuleManager()
        .deleteModule(
            cms, moduleName, false, new CmsShellReport(cms.getRequestContext().getLocale()));
  ...
  public void testModuleExport() throws Throwable {
    ...
    OpenCms.getImportExportManager()
        .exportData(
            cms, moduleExportHandler, new CmsShellReport(cms.getRequestContext().getLocale()));
  ...
  public void testModuleImportConflictId() throws Exception {
    ...
    if (OpenCms.getModuleManager().hasModule(moduleName)) {
      fail(
          "Module '"
              + moduleName
              + "' was imported, but should not have been because of id conflicts!");
    }
   ...
   *
   * <p>This test was added because there was an issue where modules with a resource type generated
   * an error "conficting id" during update.
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  public void testModuleUpdateWithResourceId() throws Exception {
    ...
    OpenCms.getModuleManager()
        .deleteModule(
            cms, moduleName, true, new CmsShellReport(cms.getRequestContext().getLocale()));
  ...
  /**
   * Adds a module dependency for the tests.
   *
   * <p>
   *
  ...
  private void addDependency(CmsObject cms, CmsModuleDependency dep)
    ...
    if (OpenCms.getModuleManager().hasModule(dep.getName())) {
      ...
      OpenCms.getModuleManager()
          .deleteModule(
              cms, dep.getName(), true, new CmsShellReport(cms.getRequestContext().getLocale()));
    }
