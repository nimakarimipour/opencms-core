...
/**
 * Unit tests for OpenCms module updates.
 *
 * <p>
 */
public class TestModuleUpdate extends OpenCmsTestCase {
  ...
  public void testAcl() throws Exception {
    ...
    CmsReplaceModuleInfo result =
        OpenCms.getModuleManager()
            .replaceModule(cms, export.getAbsolutePath(), new CmsShellReport(Locale.ENGLISH));
    ...
    // first test that existing resources match their stored version, then check that there are no
    // extra resources
    ...
    assertEquals(
        "Resource count doesn't match", m_currentResourceStrorage.size(), resources.size());
  }
  ...
  public void testExplodedModule() throws Exception {
    ...
    assertFalse(
        ...
        cms.existsResource("/system/module/org.opencms.bar/todelete.txt"));
  }
  ...
  public void testExportPoints() throws Exception {
    ...
    {
      CmsTestModuleBuilder builder = new CmsTestModuleBuilder(cms, MODULE);
      builder.addExportPoint(
          new CmsExportPoint(MODULE_PATH + "/test.txt", target.getAbsolutePath()));
      builder.addModule();
      builder.addFolder("");
  ...
  public void testMoveNewDelete() throws Exception {
    ...
    // use blocks so we don't accidentally use wrong object
    {
      ...
      // foo.txt is the moved file, the module builder helper calculates the uuid from the file name
      // (not path)
  ...
  public void testNestedMove() throws Exception {
    ...
    CmsReplaceModuleInfo replaceInfo =
        OpenCms.getModuleManager()
            .replaceModule(cms, export.getAbsolutePath(), new CmsShellReport(Locale.ENGLISH));
  ...
  public void testNewTypeWithContents() throws Exception {
    ...
    assertEquals(
        typeId, cms.readResource("/system/modules/" + MODULE + "/special.txt").getTypeId());
  }
  ...
  public void testParseLinks() throws Exception {
    ...
    List<CmsRelation> relations =
        cms.readRelations(CmsRelationFilter.relationsFromStructureId(testJsp.getStructureId()));
  ...
  public void testProperties() throws Exception {
    ...
    // use blocks so we don't accidentally use wrong object
    {
      ...
      cms.writePropertyObject(
          MODULE_PATH + "/test.txt", new CmsProperty("Description", "desc", null));
      builder.addTextFile("new.txt", "new");
    ...
    }
    {
      ...
      cms.writePropertyObject(
          MODULE_PATH + "/test.txt", new CmsProperty("template", "template", null));
      builder.publish();
    }
  ...
  public void testRelations() throws Exception {
    ...
    CmsResource relTarget =
        cms.createResource(
            "system/testRelationsTarget", OpenCms.getResourceManager().getResourceType("folder"));
    ...
    // use blocks so we don't accidentally use wrong object
    {
      ...
      CmsResource test2 =
          builder.addFile("jsp", "test2.txt", "%(link.weak:/system/modules/org.test.foo/test.txt");
  ...
  public void testRelationsToImmutable() throws Exception {
    ...
    OpenCms.getModuleManager()
        .replaceModule(cms, export.getAbsolutePath(), new CmsShellReport(Locale.ENGLISH));
    List<CmsRelation> relations =
        cms.readRelations(CmsRelationFilter.relationsFromStructureId(foo.getStructureId()));
  ...
  public void testUnlockedAndUnchanged() throws Exception {
    ...
    assertEquals(
        CmsResource.STATE_UNCHANGED, cms.readResource(MODULE_PATH + "/bar.txt").getState());
    assertTrue(
  ...
  public void testUpdateTypes() throws Exception {
    ...
    assertFalse(
        "shouldn't have type secondttype",
        OpenCms.getResourceManager().hasResourceType("secondtype"));
  }
  ...
  public void testUpdateWithSimpleFileIdConflict2() throws Exception {
    ...
    // First create a module, then try to update it with a module containing the same resource path
    // but with
    ...
    for (int i = 0; i < 2; i++) {
      ...
      CmsReplaceModuleInfo info =
          OpenCms.getModuleManager().replaceModule(cms, importZip.getCanonicalPath(), report);
      if (i == 1) {
  ...
  /**
   * Creates and switches to a new resource storage for the currently executed test case.
   *
   * <p>
   *
  ...
  /**
   * Returns the full path for a resource given the module relative path.
   *
   * <p>
   *
  ...
  /**
   * Removes the test module if it exists.
   *
   * <p>
   *
  ...
  private void removeTestModuleIfExists(CmsObject cms) throws CmsException {
    ...
    if (OpenCms.getModuleManager().hasModule(MODULE)) {
      OpenCms.getModuleManager()
          .deleteModule(cms, MODULE, false, new CmsShellReport(Locale.ENGLISH));
    }
  }
}
