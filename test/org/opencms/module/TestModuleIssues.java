...
/**
 * Unit tests for issues found in the new module mechanism.
 *
 * <p>
 */
public class TestModuleIssues extends OpenCmsTestCase {
  ...
  public static Test suite() {
    ...
    // important: this must be the last called method since the OpenCms installation is removed from
    // there
  ...
  /**
   * Issue: Additional "system" folder created in current site after module import.
   *
   * <p>
   *
  ...
  /**
   * Issue: Module can not be deleted if there are locked resources.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  public void testModuleDeletion() throws Throwable {
    ...
    cms.createResource(
        fileOneName, CmsResourceTypePlain.getStaticTypeId(), "first test file".getBytes(), null);
    cms.createResource(
        fileTwoName, CmsResourceTypePlain.getStaticTypeId(), "second test file".getBytes(), null);
    ...
    try {
      OpenCms.getModuleManager().deleteModule(cms, moduleName, false, report);
      fail(
          "it should not be possible to delete a module when a super folder is locked by other user");
    } catch (CmsLockException e) {
      // ok, ignore
    }
    // the report error, and the module files, and the module itself
    ...
    try {
      OpenCms.getModuleManager().deleteModule(cms, moduleName, false, report);
      fail(
          "it should not be possible to delete a module when a super folder is locked by the same user");
    } catch (CmsLockException e) {
      // ok, ignore
    }
    // the report error, and the module files, and the module itself
  ...
  /**
   * Issue: Shutdown method never called on module.
   *
   * <p>
   *
  ...
  public void testShutdownMethod() throws Exception {
    ...
    I_CmsModuleAction actionInstance =
        OpenCms.getModuleManager().getModule(moduleName).getActionInstance();
