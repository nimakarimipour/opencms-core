 ...
 * format.
 *
 * <p>This is no code functionality test but a configuration validation that was inspired by a 1
 * hour debug session caused by a query.properties file with a trailing tab after the "escape
 * linebreak" backslash.
 *
 * <p>Currently the following checks are made:
 *
 * <ul>
 *   <li><b>Invalid linebreak escape</b><br>
 *       If a linebreak is escaped by a backslash there must not follow any character in that line.
 *   <li><b>Invalid comment line</b><br>
 *       If a comment character is found it has to be the first character of the line or
 *       <ul>
 *         <li>The characters before it have to be fully "trimmable" (Unicode General Category
 *             Separator, Space [Zs]).
 *         <li>The characters before it are a valid key - value pair in properties notation.
 *       </ul>
 * </ul>
 *
 * <p>
...
public class TestQueryProperties extends OpenCmsTestCase {
  ...
  /**
   * Parses the key value pair string.
   *
   * <p>
   *
