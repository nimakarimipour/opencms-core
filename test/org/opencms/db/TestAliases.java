...
public class TestAliases extends OpenCmsTestCase {
  ...
  /**
   * Checks whether a resource's alias paths exactly match a given list of paths, and throws an
   * exception otherwise.
   *
   * <p>
   *
  ...
  public void checkAliases(CmsResource resource, String... aliasPaths) throws Exception {
    ...
    List<CmsAlias> aliases =
        OpenCms.getAliasManager()
            .getAliasesForStructureId(getCmsObject(), resource.getStructureId());
    ...
    MapDifference<String, Boolean> difference =
        Maps.difference(aliasMapFromDb, aliasMapFromParameters);
    assertTrue(
        "Aliases for "
  ...
  /**
   * Tests reading/writing rewrite aliases.
   *
   * <p>
   *
   * @throws Exception
  ...
  public void testRewrites() throws Exception {
    ...
    CmsRewriteAlias alias =
        new CmsRewriteAlias(id, siteRoot, patternString, replacementString, mode);
    CmsAliasManager aliasManager = OpenCms.getAliasManager();
    aliasManager.saveRewriteAliases(getCmsObject(), siteRoot, Collections.singletonList(alias));
    List<CmsRewriteAlias> aliases =
        aliasManager.getRewriteAliases(getCmsObject(), "/sites/default");
    checkRewriteAlias(alias, aliases);
    assertEquals(1, aliases.size());
    aliasManager.saveRewriteAliases(
        getCmsObject(), siteRoot, Collections.<CmsRewriteAlias>emptyList());
    aliases = aliasManager.getRewriteAliases(getCmsObject(), "/sites/default");
    assertEquals(0, aliases.size());

    CmsRewriteAlias alias2 =
        new CmsRewriteAlias(
            new CmsUUID(), siteRoot, patternString, replacementString, CmsAliasMode.redirect);
  ...
  /**
   * Checks if a rewrite alias is contained in a list of rewrite aliases.
   *
   * <p>
   *
