...
public class TestCmsSystemInfo extends OpenCmsTestCase {
  ...
  public void testGetAbsoluteRfsPathRelativeToWebInf() {
    ...
    path =
        sysinfo.getAbsoluteRfsPathRelativeToWebInf(
    ...
    */
    ...
    if (versionId.startsWith("Manual")) {
      ...
      if (!versionId.startsWith("Milestone")) {
        // don't use build number for Milestone builds as the continuation in the numbers is not
        // assured
        assertNotNull("build.number not set", info.get("build.number"));
        assertEquals(
            "Expected keys do not match", "build.number", info.get("build.number").getKeyName());
      }

      assertEquals("Expected keys do not match", "build.date", info.get("build.date").getKeyName());
      assertEquals(
          "Expected keys do not match", "build.system", info.get("build.system").getKeyName());
