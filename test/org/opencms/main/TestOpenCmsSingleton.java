...
/**
 * Unit test the static OpenCms singleton object.
 *
 * <p>
 */
public class TestOpenCmsSingleton extends OpenCmsTestCase {
  ...
  public void testEncoding() throws Exception {
    ...
    CmsResourceTypeJsp jsp =
        (CmsResourceTypeJsp)
            OpenCms.getResourceManager().getResourceType(CmsResourceTypeJsp.getJSPTypeId());
    // note: default test configuration is done in opencms-vfs.xml and may be different from
    // standard installation
    List defaultProperties = jsp.getConfiguredDefaultProperties();
    assertEquals(
        "Test configuration has 2 default properties configured for JSP",
        2,
  ...
  /**
   * Test case for the initCmsObject methods.
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  public void testInitCmsObject() throws Exception {
    ...
    if (!OpenCms.getDefaultUsers()
        .isUserExport(cms.getRequestContext().getCurrentUser().getName())) {
    ...
    if (!OpenCms.getDefaultUsers()
        .isUserAdmin(cms.getRequestContext().getCurrentUser().getName())) {
  ...
  /**
   * Test case for resource initialization.
   *
   * <p>
   *
   * @throws Exception if the test fails
