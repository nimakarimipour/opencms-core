...
public class TestOpenCmsUrlServletFilter extends OpenCmsTestCase {
  ...
  private static String[] DEFAULT_EXCLUDE_PREFIXES =
      new String[] {
        EXPORTPATH, SERVLETPATH, RESOURCES_FOLDER, VAADIN_FOLDER, WEBDAV_FOLDER, WORKPLACE_FOLDER
      };
  /** Test url prefix. */
  ...
  public void testDefaultRegex() {
    ...
    // Test
    String defaultRegex =
        OpenCmsUrlServletFilter.createRegex(SERVLETCONTEXT, DEFAULT_EXCLUDE_PREFIXES, null);
  ...
  /**
   * Test regex build with extra configuration.
   *
   * <p>
   */
  public void testRegexWithConfiguration() {
    ...
    System.out.println(
        "Testing the Regex for URL rewriting via the servlet filter with extra configuration");

    // Test
    String defaultRegex =
        OpenCmsUrlServletFilter.createRegex(
            SERVLETCONTEXT, DEFAULT_EXCLUDE_PREFIXES, ADDITIONAL_CONFIG);
