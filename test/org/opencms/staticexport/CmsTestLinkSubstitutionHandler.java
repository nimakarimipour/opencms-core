...
public class CmsTestLinkSubstitutionHandler extends CmsDefaultLinkSubstitutionHandler {
   ...
   * and in this case replaces it with the localized news path (like <code>/en/news/</code>).
   *
   * <p>
   *
   * @param path the path to check and replace
   * @param locale the locale to build the news path with
   * @return the localized news path if the path points to the system news folder, or the path
   *     parameter if not
   */
  ...
  /**
   * Checks if the given path is pointing to a localized news path(like <code>/en/news/</code>), and
   * in this case replaces it with the system news folder (like <code>/system/news/</code>).
   *
   * <p>
   *
   * @param path the path to check and replace
   * @param addLocale if true, the locale parameter <code>__locale=xx</code> is appended
   * @return the system news folder if the path points to a localized news path, or the path
   *     parameter if not
   */
  ...
  /**
   * @see
   *     org.opencms.staticexport.CmsDefaultLinkSubstitutionHandler#getRootPath(org.opencms.file.CmsObject,
  ...
  public String getRootPath(CmsObject cms, String targetUri, String basePath) {
    ...
    if (result != null) {
      ...
      boolean addLocale =
          (path.indexOf("/de/") >= 0) || (path.indexOf(CmsLocaleManager.PARAMETER_LOCALE) >= 0);
