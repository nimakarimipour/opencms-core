...
/**
 * Tests for static export manager.
 *
 * <p>
 *
 * @since 6.0.0
...
public class TestCmsStaticExportManager extends OpenCmsTestCase {
  ...
  /**
   * Tests the link generation for statically exported files by default suffix.
   *
   * <p>
   *
  ...
  public void testDefaultSuffixLinkGeneration() throws Exception {
    ...
    CmsProperty exportProp =
        cms.readPropertyObject(vfsName1, CmsPropertyDefinition.PROPERTY_EXPORT, true);
    ...
    String rfsPrefix =
        OpenCms.getStaticExportManager()
            .getRfsPrefix(cms.getRequestContext().getSiteRoot() + folder);
    ...
    cms.writePropertyObject(
        folder, new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORTNAME, "testfolder", null));
  ...
  /**
   * Tests the link generation for statically exported JSP files.
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  public void testExportJspLinkGeneration() throws Exception {
    ...
    CmsProperty exportProp =
        new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORT, CmsStringUtil.TRUE, null);
    cms.lockResource(folder);
    ...
    cms.lockResource(folder);
    cms.writePropertyObject(
        folder, new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORTNAME, "myfolder", null));
    cms.unlockResource(folder);
    // publish the changes
    OpenCms.getPublishManager().publishProject(cms);
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    cms.lockResource(vfsName);
    cms.writePropertyObject(
        vfsName, new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORTSUFFIX, ".txt", null));
    cms.unlockResource(vfsName);
    // publish the changes
    OpenCms.getPublishManager().publishProject(cms);
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    echo(
        "Testing export name generating functions for a JSP with only 'exportsuffix' property set");
    ...
    cms.lockResource(vfsName);
    cms.writePropertyObject(
        vfsName, new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORTSUFFIX, ".pdf", null));
    cms.unlockResource(vfsName);
    // publish the changes
    OpenCms.getPublishManager().publishProject(cms);
    OpenCms.getPublishManager().waitWhileRunning();
  ...
  public void testSiteExport() throws Exception {
    ...
    CmsProperty exportProp =
        new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORT, CmsStringUtil.TRUE, null);
    CmsProperty exportNameProp =
        new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORTNAME, "/", null);

    // create site folder one
    CmsResource siteFolder =
        cms.createResource("/sites/site-one/", CmsResourceTypeFolder.getStaticTypeId());
  ...
  /**
   * Checks a link that has no parameters.
   *
   * <p>
   *
  ...
  /**
   * Checks a link that has parameters.
   *
   * <p>
   *
   * @param cms the cms context
