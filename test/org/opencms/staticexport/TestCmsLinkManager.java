...
public class TestCmsLinkManager extends OpenCmsTestCase {
  ...
  private static final String PAGE_01 =
      "<html><body><a href=\"/system/news/test.html?__locale=de\">test</a></body></html>";
  ...
  /**
   * Runs the given test wrapped in an OpenCms instance where servletName and defaultWebAppName are
   * adjusted to test for different OpenCms contexts.
   *
  ...
  /**
   * Test how the OpenCms context is removed from URLs when getting URLs with http://.... Intended
   * behavior: if and only if a link starts with "${server-url}/${context}/" the context is removed.
   *
   * <p>Example: with context "http://localhost:8080/opencms": input link:
   * http://localhost:8080/opencms/path output link: /path input link:
   * http://localhost:8080/opencmswhatever/path output link: /opencmswhatever/path
   *
   * <p>Assumption: OpenCms context never ends with "/".
   *
  ...
  /**
   * Tests symmetric link / root path substitution with a custom link handler.
   *
   * <p>
   *
  ...
  public void testLinkSubstitution() throws Exception {
    ...
    test =
        linkManager.substituteLink(
            cms, CmsLinkManager.getAbsoluteUri("/", "/folder1/index.html"), "/sites/default");
    System.out.println(test);
  ...
  /**
   * Test how the OpenCms context is removed from URLs when getting root URLs. Intended behavior: if
   * and only if a link starts with "${context}/" the context is removed.
   *
   * <p>Example: with context "/opencms": input link: /opencms/path output link: /path input link:
   * /opencmswhatever/path output link: /opencmswhatever/path
   *
   * <p>Assumption: OpenCms context never ends with "/".
   *
   * @throws CmsException from getCmsObject()
  ...
  /**
   * Just a copy of the called method - if the same method is called twice, the JUnit Eclipse plugin
   * (or JUnit itself?) behaves strange (if the test failed or succeeded is only mentioned for the
   * last test occurrence.
   *
   * @throws CmsException from getCmsObject()
   */
  ...
  /**
   * Tests symmetric link / root path substitution.
   *
   * <p>
   *
   * @throws Exception if test fails
  ...
  /**
   * Internal test method for custom link test.
   *
   * <p>
   *
