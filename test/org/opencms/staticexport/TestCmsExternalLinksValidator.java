...
public class TestCmsExternalLinksValidator extends OpenCmsTestCase {
  ...
  /**
   * This test can go wrong if the external sites in the WWW not exists anymore or no connect to the
   * WWW exists.
   *
   * <p>Please remove sites, which not exists anymore.
   *
   * <p>
   *
  ...
  public void testExternalLinksOutside() throws Exception {
    ...
    //        list.add("http://www.si-folkesundhed.dk/Forskning/Sygdomme og
    // tilskadekomst/Ulykker/Nyhedsbrev.aspx");
    //        list.add("http://www.mim.dk/Udgivelser/Milj%F8Danmark/");
    //        list.add("http://www.ug.dk/Videnscenter for vejledning/Forside/Virtuelt
    // tidsskrift.aspx");
    ...
    for (int i = 0; i < list.size(); i++) {
      ...
      System.out.println(
          "  Extenal link encoded: " + new CmsUriSplitter(url, true).toURI().toURL());
