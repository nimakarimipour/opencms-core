...
public class TestCmsJspInstanceDateBean extends OpenCmsTestCase {
  ...
  public void testDateFormatting() {
    ...
    CmsJspDateSeriesBean series =
        new CmsJspDateSeriesBean(seriesDefinition, CmsLocaleManager.getDefaultLocale());
    ...
    assertEquals(
        (new SimpleDateFormat("dd/MM/yy")).format(d1)
            + " - "
            + (new SimpleDateFormat("dd/MM/yy")).format(d2),
        actual);
    actual = bean.getFormat().get("dd/MM/yy|hh:mm");
    assertEquals(
    ...
    assertEquals(
        (new SimpleDateFormat("dd/MM/yy hh:mm")).format(d1)
            + " - "
            + (new SimpleDateFormat("hh:mm")).format(d3),
        actual);
    actual = bean.getFormat().get("dd/MM/yy|hh:mm|dd/MM/yy - hh:mm");
    assertEquals(
        (new SimpleDateFormat("dd/MM/yy - hh:mm")).format(d1)
  ...
  /**
   * Test if explictely setting the end date and/or setting the whole day option behaves as
   * expected.
   */
  public void testExplicitDateChanges() {

    TimeZone currentTimeZone = TimeZone.getDefault();
    // Set a defined time zone such that the times we use for testing really fit for start/end of
    // day.
