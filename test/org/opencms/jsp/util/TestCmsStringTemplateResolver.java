...
/**
 * Tests the string template resolver.
 *
 * <p>
 */
public class TestCmsStringTemplateResolver extends OpenCmsTestCase {
  ...
  public void testContentAccessNestedSchema() throws Exception {
    ...
    assertEquals(
        ...
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Cascade/VfsLink%", bean, null));
    assertEquals(
        "/test.html",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.(\"Cascade[1]/VfsLink[2]\")%", bean, null));
    assertEquals(
        "/index.jsp",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.(\"Cascade[2]/VfsLink\")%", bean, null));
    assertEquals(
        "\n"
    ...
    assertEquals(
        ...
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.valueList.Cascade/VfsLink :{link |<a href='%link%' />} %", bean, null));
    assertEquals(
        "<a href='/index.html' /><a href='/test.html' />",
        CmsStringTemplateRenderer.renderTemplate(
            cms,
  ...
  /**
   * Tests content value access and the {@link CmsJspVfsAccessBean#getExistsXml()} method.
   *
   * <p>
   *
  ...
  public void testRenderContentValue() throws Exception {
    ...
    assertEquals(
        "true",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Title.exists%", article, null));
    assertEquals(
        "Foo",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if(content.value.Title.exists)%Foo%else%Bar%endif%", article, null));
    assertEquals(
        "Bar",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if(content.value.Title.isEmpty)%Foo%else%Bar%endif%", article, null));
    assertEquals(
        "Bar",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if(content.value.Missing.exists)%Foo%else%Bar%endif%", article, null));
    assertEquals(
        "Foo",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if(content.value.Missing.isEmpty)%Foo%else%Bar%endif%", article, null));
    assertEquals(
        "Foo",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if(content.value.Missing.isEmpty)%Foo%else%Bar%endif%", article, null));
  ...
  public void testRenderDate() throws Exception {
    ...
    assertEquals(
        "7/18/17 8:40 AM",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Release.toDate; format=\"M/d/yy h:mm a\"%", article, null));
    assertEquals(
    ...
    assertEquals(
        "8:40 AM",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Release.toDate; format=\"h:mm a\"%", article, null));
    assertEquals(
        "7/18/17",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Release.toDate; format=\"M/d/yy\"%", article, null));
    assertEquals(
        "18/07/2017",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Release.toDate; format=\"dd/MM/yyyy\"%", article, null));
  }

  /**
   * Tests the link rendering.
   *
   * <p>
   *
   * @throws Exception in case something goes wrong
  ...
  public void testRenderLink() throws Exception {
    ...
    assertEquals(
        ...
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%content.value.Homepage.toLink%", article, null));
    assertEquals(
  ...
  /**
   * Tests the object function wrapper.
   *
   * <p>
   *
   * @throws Exception in case something goes wrong
        ...
        CmsStringTemplateRenderer.renderTemplate(
            ...
            Collections.<String, Object>singletonMap(
                ...
                CmsCollectionsGenericWrapper.createLazyMap(
                    new CmsObjectFunctionTransformer(cms)))));
    assertEquals(
        "EQUAL",
        CmsStringTemplateRenderer.renderTemplate(
            cms,
  ...
  /**
   * Tests settings access in the string template.
   *
   * <p>
   *
   * @throws Exception in case something goes wrong
  ...
  public void testRenderSettings() throws Exception {
    ...
    settings.put(
        "dateKey",
        DateFormat.getDateTimeInstance(DateFormat.LONG, DateFormat.LONG).format(settingDate));
    Map<String, CmsJspObjectValueWrapper> wrappedSettings =
        CmsStringTemplateRenderer.wrapSettings(cms, settings);
    Map<String, Object> objects =
        Collections.<String, Object>singletonMap("settings", wrappedSettings);
    ...
    assertEquals(
        "TRUE",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if (settings.settingKey.isSet)%TRUE%else%FALSE%endif%", article, objects));

    assertEquals(
        "FALSE",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if (settings.settingKey.isEmpty)%TRUE%else%FALSE%endif%", article, objects));

    assertEquals(
        (new SimpleDateFormat("dd.MM.yyyy")).format(settingDate),
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%settings.dateKey.toDate; format=\"dd.MM.yyyy\"%", article, objects));

    assertEquals(
        "FALSE",
        CmsStringTemplateRenderer.renderTemplate(
            cms, "%if (settings.noneExistingKey.isSet)%TRUE%else%FALSE%endif%", article, objects));
  }

  /**
   * Test the printStructure utility method.
   *
   * <p>
   *
   * @throws Exception in case something goes wrong
