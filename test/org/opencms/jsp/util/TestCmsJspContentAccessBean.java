...
/**
 * Unit tests for the <code>{@link CmsJspContentAccessBean}</code>.
 *
 * <p>
 *
 * @since 7.0.2
...
public class TestCmsJspContentAccessBean extends OpenCmsTestCase {
  ...
  /**
   * Tests general content access for XML content.
   *
   * <p>
   *
  ...
  public void testContentAccess() throws Exception {
    ...
    assertEquals(
        "This is teaser 2 in sample article 2.", String.valueOf(enValues.get("Teaser").get(1)));

    // now check list access to selected locale
    Map<String, Map<String, List<CmsJspContentAccessValueWrapper>>> localeValues =
        bean.getLocaleValueList();
    ...
    assertEquals(
        ...
        String.valueOf(deValues.get("Teaser").get(2).getResolveMacros()));
    ...
    // create the content access bean with a locale that is not available, so a fall back should be
    // used
  ...
  /**
   * Tests for the {@link CmsJspContentAccessBean#getIsEditable()} method.
   *
   * <p>
   *
   * @throws Exception if the test fails
