...
public class TestPlainQuerySearchConfigurationParser extends OpenCmsTestCase {
  ...
  public void testSpecialParamExtraction() throws CmsException {
    ...
    CmsSearchConfiguration config =
        new CmsSearchConfiguration(new CmsPlainQuerySearchConfigurationParser(configString), cms);
    ...
    config =
        new CmsSearchConfiguration(new CmsPlainQuerySearchConfigurationParser(configString2), cms);
    assertEquals(configString, config.getGeneralConfig().getExtraSolrParams());
    assertEquals("test", config.getGeneralConfig().getSolrCore());

    String configString3 = "a=foo&core=test&b=bar&fl=id,path";
    config =
        new CmsSearchConfiguration(new CmsPlainQuerySearchConfigurationParser(configString3), cms);
    assertEquals(configString, config.getGeneralConfig().getExtraSolrParams());
    assertEquals("test", config.getGeneralConfig().getSolrCore());

    String configString4 = "core=test&a=foo&b=bar&fl=id,path";
    config =
        new CmsSearchConfiguration(new CmsPlainQuerySearchConfigurationParser(configString4), cms);
    assertEquals(configString, config.getGeneralConfig().getExtraSolrParams());
    assertEquals("test", config.getGeneralConfig().getSolrCore());

    String configString5 = "core=test&a=foo&index=Test Index&b=bar&fl=id,path";
    config =
        new CmsSearchConfiguration(new CmsPlainQuerySearchConfigurationParser(configString5), cms);
    assertEquals(configString, config.getGeneralConfig().getExtraSolrParams());
    assertEquals("test", config.getGeneralConfig().getSolrCore());
    ...
    config =
        new CmsSearchConfiguration(new CmsPlainQuerySearchConfigurationParser(configString6), cms);
    assertEquals(configString, config.getGeneralConfig().getExtraSolrParams());
    assertEquals("test", config.getGeneralConfig().getSolrCore());
    assertEquals("Test Index", config.getGeneralConfig().getSolrIndex());
