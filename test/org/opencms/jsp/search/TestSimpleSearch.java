...
public class TestSimpleSearch extends OpenCmsTestCase {
  ...
  private static final String LIST_BASE_FOLDER =
      "/system/modules/org.opencms.test.modules.listtype/resources/lists/";
  ...
  /**
   * Executes several searches with list configurations that differ in the combined category folder
   * restrictions and examines if the results are as expected.
   *
  ...
  private Set<String> searchForConfig(CmsObject cms, String listName) throws CmsException {
    ...
    CmsConfigurationBean listConfigBean =
        CmsConfigParserUtils.parseListConfiguration(cms, listConfig);
    ...
    CmsSolrResultList searchResult =
        index.search(
            cms,
            query.clone(),
            true,
