...
public class TestPublishManager extends OpenCmsTestCase {
  ...
  public void testInitialization2() throws Throwable {
    ...
    // now start publishing again and reinitialize the publish manager and engine while it is
    // running
    OpenCms.getPublishManager().startPublishing();
    OpenCms.getPublishManager().waitWhileRunning();
    ...
    CmsPublishJobFinished jobInHistory =
        (CmsPublishJobFinished) newHistory.get(newHistory.size() - 1);
  ...
  public void testListener() throws Throwable {
    ...
    TestPublishEventListener firstListener =
        new TestPublishEventListener(cms.readResource(destination + "1.gif"));
    OpenCms.getPublishManager().addPublishListener(firstListener);

    // set the listener for the last job
    TestPublishEventListener lastListener =
        new TestPublishEventListener(cms.readResource(destination + max + ".gif"));
    ...
    // at start time of first job: queue should contain all jobs but not the job itself and not the
    // aborted job
    assertEquals(max - 2, firstJobsInQueue[1]);
    // last job is never started
    assertEquals(0, lastJobsInQueue[1]);
    // at finish time of first job: queue should still contain all jobs but not the job itself and
    // not the aborted job
