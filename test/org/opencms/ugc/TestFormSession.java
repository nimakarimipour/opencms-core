...
/**
 * Tests the form session methods.
 *
 * <p>
 */
public class TestFormSession extends OpenCmsTestCase {
  ...
  public void testAddValues() throws Exception {
    ...
    CmsXmlEntityResolver.cacheSystemId(
        SCHEMA_ID_TEXTBLOCK, definition.getSchema().asXML().getBytes(CmsEncoder.ENCODING_UTF_8));
    String fileContent =
        CmsFileUtil.readFile("org/opencms/ugc/tb_00001.xml", CmsEncoder.ENCODING_UTF_8);
    // now create the XML content
    CmsXmlContent xmlContent =
        CmsXmlContentFactory.unmarshal(fileContent, CmsEncoder.ENCODING_UTF_8, resolver);
  ...
  /**
   * Tests that created resources are created in the correct project.
   *
   * <p>
   *
   * @throws Exception -
  ...
  public void testAssignProjectToCreatedResources() throws Exception {
    ...
    CmsResource uploadRes =
        session.createUploadResource(randomFieldName(), "example.bin", new byte[] {1, 2, 3, 4, 5});
  ...
  /**
   * Tests creation of new contents.
   *
   * <p>
   *
   * @throws Exception -
  ...
  public void testDeleteValues() throws Exception {
    ...
    CmsXmlEntityResolver.cacheSystemId(
        SCHEMA_ID_TEXTBLOCK, definition.getSchema().asXML().getBytes(CmsEncoder.ENCODING_UTF_8));
    String fileContent =
        CmsFileUtil.readFile("org/opencms/ugc/tb_00002.xml", CmsEncoder.ENCODING_UTF_8);
    // now create the XML content
    CmsXmlContent xmlContent =
        CmsXmlContentFactory.unmarshal(fileContent, CmsEncoder.ENCODING_UTF_8, resolver);
    CmsUgcSession session = new CmsUgcSession(getCmsObject());
    ...
    String fileContentNoLinks =
        CmsFileUtil.readFile("org/opencms/ugc/tb_00002_no_links.xml", CmsEncoder.ENCODING_UTF_8);
    // all content values should be restored
    assertEquals(fileContentNoLinks, new String(xmlContent.marshal(), CmsEncoder.ENCODING_UTF_8));
  }

  /**
   * Tests that editing multiple files in a single form session causes an error.
   *
   * <p>
   *
   * @throws Exception -
  ...
  /**
   * Tests the comparator used for xpath ordering.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testPathComparator() throws Exception {
    ...
    assertEquals(
        "A should come before B at lower levels", -1, comp.compare("Foo[1]/A[1]", "Foo[1]/B[1]"));
  }
  ...
  /**
   * Tests the wait time for queue is observed.
   *
   * <p>
   *
   * @throws Exception -
  ...
  public void testQueueWaitTime() throws Exception {
    ...
    assertTrue(
        "The elapsed time is below expected wait time", (t2 - t1) >= ((numThreads - 1) * waitTime));
  }

  /**
   * Tests that the session is cleaned up after its parent session is destroyed.
   *
   * <p>
   *
   * @throws Exception -
  ...
  public void testSessionCleanup() throws Exception {
    ...
    // Now check that the session isn't cleaned up if the project contains any non-new,
    // non-unchanged resources
    ...
    session
        .getCmsObject()
        .writeResource(session.getCmsObject().readResource(contentRes.getStructureId()));
  ...
  /**
   * Generate random string to be used as field name.
   *
   * <p>
   *
  ...
  /**
   * Unmarshals the content definition used within the tests.
   *
   * <p>
   *
  ...
  private CmsXmlContentDefinition unmarshalDefinition(CmsXmlEntityResolver resolver)
    ...
    String schemaContent =
        CmsFileUtil.readFile("org/opencms/ugc/textblock.xsd", CmsEncoder.ENCODING_UTF_8);
    CmsXmlContentDefinition definition =
        CmsXmlContentDefinition.unmarshal(schemaContent, SCHEMA_ID_TEXTBLOCK, resolver);
    return definition;
  }
}
