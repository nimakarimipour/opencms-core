...
public class TestCmsXmlEntityResolver extends OpenCmsTestCase {
  ...
  /**
   * Tests the "wrong version of nested subschema still cached after change in VFS" issue.
   *
   * <p>An issue was discovered in the conext of auto correction of XML content: Cached content
   * definition objects where not removed from the cache if a nested subschema was changed.
   *
   * <p>
   *
  ...
  public void testRemoveNestedSubschemaFromCacheIssue() throws Exception {
    ...
    echo(
        "Testing the \"wrong version of nested subschema still cached after change in VFS\" issue");
    ...
    // read the XML content file again - since the subschema has been changed, the XML must be
    // invalid now
