...
/**
 * Tests the OpenCms XML content support for <code>xsd:choice</code>.
 *
 * <p>
 */
public class TestCmsXmlContentChoice extends OpenCmsTestCase {
  ...
  public void testChoiceXmlContentDefinitionCreation() throws Exception {
    ...
    // the point of this test really is that there is no exception thrown here if xsd:choice is in
    // the schema
    CmsXmlContentDefinition definition =
        CmsXmlContentDefinition.unmarshal(
            schema, "http://www.opencms.org/testChoice1.xsd", resolver);
    ...
    assertTrue(
        "Content definition sequence choice maxOccurs is "
    ...
    CmsXmlContentFactory.unmarshal(
        new String(content, CmsEncoder.ENCODING_UTF_8), CmsEncoder.ENCODING_UTF_8, resolver);

    CmsXmlNestedContentDefinition nestA =
        (CmsXmlNestedContentDefinition) definition.getSchemaType("ChoiceTestA");
    CmsXmlNestedContentDefinition nestB =
        (CmsXmlNestedContentDefinition) definition.getSchemaType("ChoiceTestB");
    CmsXmlNestedContentDefinition nestC =
        (CmsXmlNestedContentDefinition) definition.getSchemaType("ChoiceTestC");
  ...
  public void testChoiceXmlContent() throws Exception {
    ...
    CmsXmlContentValueSequence sequence =
        xmlcontent.getValueSequence("ChoiceTestA/StringChoice", Locale.ENGLISH);
    assertNotNull("ChoiceTestA value sequence must not be null", sequence);

    assertTrue(
        "Choice sequence A not recognized",
        xmlcontent.hasChoiceOptions("ChoiceTestA", Locale.ENGLISH));
    assertTrue(
        "Choice sequence B not recognized",
        xmlcontent.hasChoiceOptions("ChoiceTestB", Locale.ENGLISH));
    assertTrue(
        "Choice sequence C not recognized",
        xmlcontent.hasChoiceOptions("ChoiceTestC", Locale.ENGLISH));
  ...
  public void testChoiceAdvancedXmlContentDefinitionCreation() throws Exception {
    ...
    CmsXmlContentDefinition definition =
        CmsXmlContentDefinition.unmarshal(
            schema, "http://www.opencms.org/testChoice2.xsd", resolver);
    ...
    CmsXmlContent content =
        CmsXmlContentFactory.createDocument(
            cms, Locale.ENGLISH, CmsEncoder.ENCODING_UTF_8, definition);
