...
public class TestCmsXmlContentLinks extends OpenCmsTestCase {
   ...
   */
  public static CmsLink getHtmlLink(
      CmsObject cms, CmsXmlContent xmlcontent, String nodeName, String linkName) {

    CmsLinkTable links =
        ((CmsXmlHtmlValue) xmlcontent.getValue(nodeName, Locale.ENGLISH)).getLinkTable();
   ...
   * @return the link object
   */
  public static CmsLink getVfsFileRefLink(
      CmsObject cms, CmsXmlContent xmlcontent, String nodeName) {

    CmsLink link =
        ((CmsXmlVfsFileValue) xmlcontent.getValue(nodeName, Locale.ENGLISH)).getLink(cms);
    assertNotNull(link);
    return link;
  }
  ...
  protected static void initSchemas() throws IOException {
    ...
    String content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-definition-11.xsd", CmsEncoder.ENCODING_UTF_8);
    // store content definition in entitiy resolver
    CmsXmlEntityResolver.cacheSystemId(
        SCHEMA_SYSTEM_ID_11, content.getBytes(CmsEncoder.ENCODING_UTF_8));

    // unmarshal content definition
    content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-definition-13.xsd", CmsEncoder.ENCODING_UTF_8);
    // store content definition in entitiy resolver
    CmsXmlEntityResolver.cacheSystemId(
        SCHEMA_SYSTEM_ID_13, content.getBytes(CmsEncoder.ENCODING_UTF_8));

    // unmarshal content definition
    content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-definition-12.xsd", CmsEncoder.ENCODING_UTF_8);
    // store content definition in entitiy resolver
    CmsXmlEntityResolver.cacheSystemId(
        SCHEMA_SYSTEM_ID_12, content.getBytes(CmsEncoder.ENCODING_UTF_8));

    // unmarshal content definition
    content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-definition-14.xsd", CmsEncoder.ENCODING_UTF_8);
    // store content definition in entitiy resolver
    CmsXmlEntityResolver.cacheSystemId(
        SCHEMA_SYSTEM_ID_14, content.getBytes(CmsEncoder.ENCODING_UTF_8));
  }

  /**
   * Returns a link object prototype for the given type.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  /**
   * Updates a link, and checks the result against the given resource.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  public void testInvalidateFalse() throws Exception {
    ...
    String content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-14.xml", CmsEncoder.ENCODING_UTF_8);
    CmsXmlContent xmlcontent =
        CmsXmlContentFactory.unmarshal(cms, content, CmsEncoder.ENCODING_UTF_8, resolver);
  ...
  /**
   * Tests the link comparator which is used to deterministically order link tables.
   *
   * <p>
   *
  ...
  public void testLinkComparator() throws Exception {
    ...
    List<String> ascendingKeys =
        Arrays.asList(null, "a1", "a2", "a11", "b1", "b2", "b11", "c", "c0", "c00", "c1");
  ...
  public void testRemoveNode() throws Exception {
    ...
    String content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-13.xml", CmsEncoder.ENCODING_UTF_8);
    CmsXmlContent xmlcontent =
        CmsXmlContentFactory.unmarshal(cms, content, CmsEncoder.ENCODING_UTF_8, resolver);
  ...
  public void testRemoveParent() throws Exception {
    ...
    String content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-12.xml", CmsEncoder.ENCODING_UTF_8);
    CmsXmlContent xmlcontent =
        CmsXmlContentFactory.unmarshal(cms, content, CmsEncoder.ENCODING_UTF_8, resolver);
  ...
  /**
   * Tests saving XML contents with links from/to various sites.
   *
   * <p>
   *
  ...
  public void testSiteLinks() throws Exception {
    ...
    // Macros to replace, have to be real URLs of existing files in order to avoid console Exception
    // log output because of malforme URLs
    ...
    for (String targetMacro : new String[] {target1, target2}) {
      ...
      checkSetLink(
          ...
          r);
      checkSetLink(
          "/sites/default", w, targetMacro, getVfsPrefix() + "/system/testfile-system.html", r);
      ...
      checkSetLink(
          ...
          s);
      checkSetLink(
          "/sites/default", w, targetMacro, getVfsPrefix() + "/shared/testfile-shared.html", s);
  ...
  public void testUpdateId() throws Exception {
    ...
    String content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-11.xml", CmsEncoder.ENCODING_UTF_8);
    CmsXmlContent xmlcontent =
        CmsXmlContentFactory.unmarshal(cms, content, CmsEncoder.ENCODING_UTF_8, resolver);
  ...
  /**
   * Replaces the content of an xmlcontent resource with a new content which contains a given link,
   * and then checks if the link has been correctly created.
   *
   * <p>
   *
   ...
   * @throws Exception if something goes wrong
   */
  private void checkSetLink(
      String siteRoot, CmsResource source, String macro, String targetUri, CmsResource expected)
      throws Exception {
  ...
  /**
   * Creates a test file for the testSiteLinks test.
   *
   * <p>
   *
  ...
  private CmsResource createTestFile(String rootPath) throws Exception {
    ...
    CmsResource tsl =
        cms.createResource(rootPath, xmlcontentType, content, Collections.<CmsProperty>emptyList());
    return tsl;
  }
}
