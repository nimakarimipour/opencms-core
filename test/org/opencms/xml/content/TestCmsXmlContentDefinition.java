...
/**
 * Tests for generating an XML content definition.
 *
 * <p>
 */
public class TestCmsXmlContentDefinition extends OpenCmsTestCase {
  ...
  /**
   * Basic test for xml content definitions.
   *
   * <p>
   *
  ...
  public void testCmsXmlContentDefiniton() throws Exception {
    ...
    CmsXmlContentDefinition cd2 =
        CmsXmlContentDefinition.unmarshal(out.toString().getBytes(), null, null);
  ...
  /**
   * Tests creation of an XML content from a XML content definition.
   *
   * <p>
   *
  ...
  public void testCreateXmlContent() throws Exception {
    ...
    CmsXmlEntityResolver.cacheSystemId(
        schemaUri, cd1.getSchema().asXML().getBytes(CmsEncoder.ENCODING_UTF_8));
    CmsXmlEntityResolver resolver = new CmsXmlEntityResolver(null);

    Locale locale = Locale.ENGLISH;

    CmsXmlContent content =
        CmsXmlContentFactory.createDocument(null, locale, CmsEncoder.ENCODING_UTF_8, cd1);
    ...
    CmsXmlEntityResolver.cacheSystemId(
        schemaUri2, cd2.getSchema().asXML().getBytes(CmsEncoder.ENCODING_UTF_8));
    CmsXmlEntityResolver resolver2 = new CmsXmlEntityResolver(null);
    CmsXmlContent content2 =
        CmsXmlContentFactory.createDocument(null, Locale.ENGLISH, CmsEncoder.ENCODING_UTF_8, cd2);
  ...
  /**
   * Tests XML content definition with a different inner / outer sequence name.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testDifferentInnerOuterName() throws Exception {
    ...
    content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-definition-1b.xsd", CmsEncoder.ENCODING_UTF_8);
    CmsXmlContentDefinition cd1 =
        CmsXmlContentDefinition.unmarshal(content, SCHEMA_SYSTEM_ID_1B, resolver);
    ...
    CmsXmlContentDefinition cd2 =
        new CmsXmlContentDefinition("Outer", "Inner", SCHEMA_SYSTEM_ID_1B);
