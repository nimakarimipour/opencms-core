...
public class TestCmsXmlContentSchemaModifications extends OpenCmsTestCase {
  ...
  /**
   * Simple schema data container structure.
   *
   * <p>
   */
  class SchemaDef {
    ...
    /**
     * Creates the schema data container structure.
     *
     * <p>
     *
  ...
  public static Test suite() {
    ...
    suite.addTest(
        new TestCmsXmlContentSchemaModifications("testMaintainOrderInChoiceAfterSchemaChange"));
  ...
  /**
   * Tests if changes in the XML schema structure maintains the order of an existing xsd:choice
   * list.
   *
   * <p>
   *
  ...
  public void testMaintainOrderInChoiceAfterSchemaChange() throws Exception {
    ...
    echo(
        "Testing if changes in the XML schema structure maintains the order of an existing xsd:choice list.");

    List<SchemaDef> originalSchemas = new ArrayList<SchemaDef>();
    List<SchemaDef> changedSchemas = new ArrayList<SchemaDef>();

    originalSchemas.add(
        new SchemaDef("http://www.opencms.org/ChoiceElement.xsd", "choicetest-ori-element.xsd"));
    originalSchemas.add(
        new SchemaDef("http://www.opencms.org/ChoiceTest.xsd", "choicetest-ori.xsd"));

    changedSchemas.add(
        new SchemaDef("http://www.opencms.org/ChoiceElement.xsd", "choicetest-mod-element.xsd"));
    changedSchemas.add(
        new SchemaDef("http://www.opencms.org/ChoiceTest.xsd", "choicetest-mod.xsd"));
  ...
  public void testUsageDemo() throws Exception {
    ...
    // enable "auto correction mode" - this is required or the XML structure will not be fully
    // corrected
    ...
    // in a "real world" use case this should be no problem as the schema will have been changed in
    // another request
    ...
    // if this is set to a Boolean.TRUE object, the XML content is always corrected while saving it
    // to the VFS
    CmsFile file = cms.readFile(filename);

    // this is not normally required, but illustrates the differences in the API behaviour
    // trying to write the file now (with the changed schema but XML still using the old schema) is
    // not possible
    ...
    // write the file again - the correction will now be done automatically while writing (no
    // exceptions)
    file = cms.writeFile(file);
  ...
  public void testXsdTranslation() throws Exception {
    ...
    CmsResourceTranslator xsdTranslator =
        new CmsResourceTranslator(
            new String[] {"s#^http://www\\.opencms\\.org/test1\\.xsd#" + newSchema + "#"}, false);
  ...
  /**
   * Compares a given XML content with the contents of a file in the RFS and asserts the XML is
   * identical.
   *
   * <p>
   *
   ...
   */
  private void assertXmlContent(
      CmsXmlContent xmlcontent, String filename, CmsXmlEntityResolver resolver) throws Exception {
  ...
  /**
   * Updates the OpenCms XML entity resolver cache with a changed XML schema id.
   *
   * <p>
   *
   ...
   * @throws Exception if something goes wrong
   */
  private void cacheSchema(CmsXmlEntityResolver resolver, String id, String filename)
      throws Exception {
  ...
  /**
   * Reads an XML content with the original schema, then changes the schema and corrects the XML
   * content with the new schema.
   *
   * <p>
   *
  ...
  /**
   * Reads an XML content with a list of original schemas, then changes the schemas and corrects the
   * XML content with the new schemas.
   *
   * <p>
   *
   * @param filename the filename to write the XML content to in the OpenCms VFS
