...
/**
 * Tests the OpenCms XML content validation with regex rules
 *
 * <p>
 */
public class TestCmsXmlContentValidation extends OpenCmsTestCase {
  ...
  public static Test suite() {
    ...
    suite.addTest(
        new TestCmsXmlContentValidation("testHandlingOfPatternSyntaxExceptionDuringValidation"));
    suite.addTest(
        new TestCmsXmlContentValidation("testHandlingOfStackOverflowErrorDuringValidation"));
  ...
  public void testHandlingOfStackOverflowErrorDuringValidation() throws Exception {
    final CmsXmlContentErrorHandler validationResult =
        validateXmlFile("org/opencms/xml/content/xmlcontent-definition-evilregex.xsd");
    ...
    final String expectedMessage =
        Messages.get()
            ...
            .key(Messages.GUI_EDITOR_XMLCONTENT_CANNOT_VALIDATE_ERROR_3)
            .split("\\{")[0];
    assertTrue(
  ...
  public void testHandlingOfPatternSyntaxExceptionDuringValidation() throws Exception {
    final CmsXmlContentErrorHandler validationResult =
        validateXmlFile("org/opencms/xml/content/xmlcontent-definition-malformedregex.xsd");
    ...
    final String expectedMessage =
        Messages.get()
            .getBundle(Locale.ENGLISH)
            .key(Messages.GUI_EDITOR_XMLCONTENT_INVALID_RULE_3)
            .split("\\{")[0];
    assertTrue(
        "Expected error during validation not registered in the error handler. Recorded error: '"
            + recordedError
  ...
  private CmsXmlContentErrorHandler validateXmlFile(String SCHEMA_FILENAME) throws Exception {
    ...
    String content =
        CmsFileUtil.readFile(
            "org/opencms/xml/content/xmlcontent-1-mod7.xml", CmsEncoder.ENCODING_UTF_8);
  ...
  private void cacheSchema(CmsXmlEntityResolver resolver, String id, String filename)
    ...
    CmsXmlEntityResolver.cacheSystemId(
        id, definition.getSchema().asXML().getBytes(StandardCharsets.UTF_8));
  }
}
