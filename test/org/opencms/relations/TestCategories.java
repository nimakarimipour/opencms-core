...
public class TestCategories extends OpenCmsTestCase {
  ...
  /**
   * Stores an error message which will cause the test to fail.
   *
   * <p>
   *
  ...
  public void testAdditionalRepository() throws Exception {
    ...
    CmsProperty repoProp =
        new CmsProperty(
            CmsPropertyDefinition.PROPERTY_CATEGORY_REPOSITORY, "/shared/.categories", null);
    ...
    catService.addResourceToCategory(cms, "/testAdditional.txt", "foo");
    {
      ...
      Set<String> actualPaths =
          new HashSet<>(categories.stream().map(cat -> cat.getPath()).collect(Collectors.toList()));
...

    {
      Set<String> available =
          catService.readCategories(cms, "/", true, "/").stream()
              .map(cat -> cat.getPath())
    ...
    repoProp =
        new CmsProperty(
            CmsPropertyDefinition.PROPERTY_CATEGORY_REPOSITORY, "/shared/.categories/", "");
    cms.writePropertyObjects("/", Collections.singletonList(repoProp));
    {
      Set<String> available =
          catService.readCategories(cms, "/", true, "/").stream()
  ...
  /**
   * Tests changing the base folder name of the category repositories.
   *
   * <p>
   *
  ...
  public void testCategoryBaseFolder() throws Exception {
    ...
    List<CmsCategory> cats =
        CmsCategoryService.getInstance().readCategories(cms, null, true, "index.html");
    ...
    CmsCategory catC =
        CmsCategoryService.getInstance()
            .createCategory(cms, null, "c", "C", "C test", "index.html");
  ...
  /**
   * Tests category assignment when changing the base folder name of the category repositories.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testCategoryBaseFolderAssign() throws Exception {
    ...
    List<CmsResource> resources =
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, catA.getPath(), true, "index.html");
    assertEquals(1, resources.size());
    assertEquals(cms.readResource("index.html"), resources.get(0));
    assertTrue(
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, catAA.getPath(), true, "index.html")
            .isEmpty());
    assertTrue(
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, catC.getPath(), true, "index.html")
            .isEmpty());
    ...
    CmsCategory catB =
        CmsCategoryService.getInstance()
            .getCategory(cms, cms.readResource(CmsCategoryService.REPOSITORY_BASE_FOLDER + "b"));
  ...
  /**
   * Tests resource categories reparation after changing the base folder name of the category
   * repositories.
   *
   * <p>
   *
  ...
  public void testCategoryBaseFolderRepair() throws Exception {
    ...
    List<CmsRelation> relations =
        cms.getRelationsForResource(
            "index.html", CmsRelationFilter.TARGETS.filterType(CmsRelationType.CATEGORY));
  ...
  /**
   * Tests the categories when several repositories define the same category.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testCategoryConflict() throws Exception {
    ...
    System.out.println(
        "Testing the categories when several repositories define the same category.");
    CmsObject cms = getCmsObject();
    ...
    CmsCategory catB2 =
        CmsCategoryService.getInstance().createCategory(cms, null, "b", "B2", "B2 test", null);
    // assert the new created category
    ...
    // assert the created resource
    CmsResource resB2 =
        cms.readResource(cms.getRequestContext().removeSiteRoot(catB2.getRootPath()));
    ...
    assertEquals(
        ...
        cms.readPropertyObject(resB2, CmsPropertyDefinition.PROPERTY_DESCRIPTION, false)
            .getValue());
  ...
  /**
   * Tests the categories assignment when several repositories define the same category.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testCategoryConflictAssign() throws Exception {
    ...
    System.out.println(
        "Testing the categories assignment when several repositories define the same category.");
    CmsObject cms = getCmsObject();
    ...
    resources =
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, catB2.getPath(), true, "index.html");
    assertEquals(1, resources.size());
    assertEquals(cms.readResource("index.html"), resources.get(0));
  }

  /**
   * Tests resource categories reparation after deleting/adding a centralized category.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testCategoryConflictRepair() throws Exception {
    ...
    System.out.println(
        "Testing resource categories reparation after deleting/adding a centralized category.");
    CmsObject cms = getCmsObject();
    ...
    assertTrue(
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, catB.getPath(), true, "index.html")
            .isEmpty());
  ...
  /**
   * Tests the category tree access with different repositories.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testCategoryTree() throws Exception {
    ...
    CmsCategory catA =
        CmsCategoryService.getInstance().createCategory(cms, null, "a", "A", "A test", null);
    // assert the new created category
    ...
    CmsCategory catB =
        CmsCategoryService.getInstance()
            .createCategory(cms, null, "b", "B", "B test", "index.html");
    // assert the new created category
    ...
    CmsCategory catAA =
        CmsCategoryService.getInstance().createCategory(cms, catA, "aa", "AA", "AA test", null);
    // assert the new created category
    ...
    // assert the created resource
    CmsResource resAA =
        cms.readResource(cms.getRequestContext().removeSiteRoot(catAA.getRootPath()));
    ...
    assertEquals(
        ...
        cms.readPropertyObject(resAA, CmsPropertyDefinition.PROPERTY_DESCRIPTION, false)
            .getValue());
  ...
  /**
   * Tests the category assignment with different repositories.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testCategoryTreeAssign() throws Exception {
    ...
    assertTrue(
        CmsCategoryService.getInstance().readResourceCategories(cms, "index.html").isEmpty());
    assertTrue(
        CmsCategoryService.getInstance()
            .readCategoryResources(cms, catA.getPath(), true, "index.html")
            .isEmpty());
  ...
  /**
   * Tests copying a file with assigned categories across different category contexts when the
  ...
  public void testCopyValid() throws Exception {
    ...
    CmsCategory d1 =
        CmsCategoryService.getInstance()
            .createCategory(cms, null, "d", "D1", "D1 Test", "/folder1/index.html");
    cms.createResource(
        "/folder2" + CmsCategoryService.getInstance().getRepositoryBaseFolderName(cms),
        CmsResourceTypeFolder.RESOURCE_TYPE_ID);
    CmsCategory d2 =
        CmsCategoryService.getInstance()
            .createCategory(cms, null, "d", "D2", "D2 Test", "/folder2/index.html");

    // assign category
    cms.lockResource("/folder1/index.html");
    CmsCategoryService.getInstance()
        .addResourceToCategory(cms, "/folder1/index.html", d1.getPath());

    // assert assignment
    List<CmsCategory> cats =
        CmsCategoryService.getInstance().readResourceCategories(cms, "/folder1/index.html");
    assertEquals(1, cats.size());
  ...
  /**
   * Tests the case where a moved resource is published, but its category has been deleted and
   * re-created in a different repository.
   *
   * <p>
   *
  ...
  public void testMoveParentFolder() throws Exception {
    ...
    List<CmsCategory> categories =
        service.readResourceCategories(cms, cms.readResource("/moveParentFolderMoved/.before"));
    assertEquals(1, categories.size());
    assertEquals("foo/", categories.get(0).getPath());
    categories =
        service.readResourceCategories(cms, cms.readResource("/moveParentFolderMoved/after"));
    assertEquals(1, categories.size());
    assertEquals("foo/", categories.get(0).getPath());
  }
  ...
  public void testPublishMovedResourceWithCategories1() throws Exception {
    ...
    try {
      OpenCmsTestLogAppender.setBreakOnError(true);
      OpenCmsTestLogAppender.setHandler(
          new I_CmsLogHandler() {

            public void handleLogEvent(LogEvent event) {

              if (event.getLevel().toString().contains("ERROR")) {
                // TODO: This may be an error unrelated to the original error. Need more precise
                // check.
      ...
      cms.deleteResource(
          baseCategories + "/" + "publishRepair_cat1", CmsResource.DELETE_PRESERVE_SIBLINGS);
      if (true) {
  ...
  /**
   * Tests the case where a moved resource is published, but its category has been deleted.
   *
   * <p>
   *
   * @throws Exception in case the test fails
  ...
  public void testPublishMovedResourceWithCategories2() throws Exception {
    ...
    try {
      ...
      cms.deleteResource(
          baseCategories + "/" + "publishRepair_cat2", CmsResource.DELETE_PRESERVE_SIBLINGS);
      OpenCms.getPublishManager().publishProject(cms);
      OpenCms.getPublishManager().waitWhileRunning();
      cms.lockResource(filePath);
      cms.moveResource(filePath, filePath + "_x");
