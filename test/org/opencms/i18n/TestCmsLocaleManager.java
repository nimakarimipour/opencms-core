...
/**
 * Tests for the OpenCms locale manager.
 *
 * <p>
 *
 * @since 6.0.0
...
public class TestCmsLocaleManager extends OpenCmsTestCase {
  ...
  /**
   * Tests selection of the default locale.
   *
   * <p>
   *
  ...
  public void testDefaultLocaleSelection() throws Exception {
    ...
    // direct match
    Locale result =
        localeManager.getBestMatchingLocale(
            Locale.GERMAN, localeManager.getDefaultLocales(), available);
    assertEquals(Locale.GERMAN, result);

    // simplified direct match
    result =
        localeManager.getBestMatchingLocale(
            Locale.GERMANY, localeManager.getDefaultLocales(), available);
    assertEquals(Locale.GERMAN, result);

    // no match, result must be first default
    result =
        localeManager.getBestMatchingLocale(
            Locale.FRENCH, localeManager.getDefaultLocales(), available);
  ...
  public void testGetLocaleVariants() {
    ...
    List<String> expected =
        Arrays.asList(new String[] {base + "_de_DE_WIN", base + "_de_DE", base + "_de"});
    List<String> actual = CmsLocaleManager.getLocaleVariants(base, locale1, false, false);
    assertEquals(expected, actual);

    expected =
        Arrays.asList(new String[] {base + "_de_DE_WIN", base + "_de_DE", base + "_de", base});
    actual = CmsLocaleManager.getLocaleVariants(base, locale1, true, false);
    assertEquals(expected, actual);

    expected =
        Arrays.asList(
            new String[] {base + "_de_DE_WIN", base + "_de_DE", base + "_de", base + "_en_GB"});
    ...
    expected =
        Arrays.asList(
            new String[] {
              base + "_de_DE_WIN", base + "_de_DE", base + "_de", base, base + "_en_GB"
            });
    ...
    expected =
        Arrays.asList(new String[] {base + "_en_GB_WIN", base + "_en_GB", base + "_en", base});
