...
public class TestLocaleGroups extends OpenCmsTestCase {
  ...
  public CmsResource makeResource(String path, Locale locale) throws Exception {
    ...
    CmsProperty prop =
        new CmsProperty(CmsPropertyDefinition.PROPERTY_LOCALE, locale.toString(), null);
    CmsObject cms = getCmsObject();
    return cms.createResource(
        path, CmsResourceTypePlain.getStaticTypeId(), null, Arrays.asList(prop));
  }
  ...
  public void testLocaleVariantRelationsNotCopied() throws Exception {
    ...
    List<CmsRelation> rels =
        cms.readRelations(CmsRelationFilter.ALL.filterStructureId(copy.getStructureId()));
    assertEquals(
        "no relations should exist on copy of locale variant", new ArrayList<CmsRelation>(), rels);
  }
  ...
  public void testNormalizeRelations1() throws Exception {
    ...
    assertEquals(
        0,
        cms.readRelations(
                CmsRelationFilter.relationsFromStructureId(r1.getStructureId())
                    .filterType(CmsRelationType.LOCALE_VARIANT))
            .size());
    assertEquals(
        1,
        cms.readRelations(
                CmsRelationFilter.relationsFromStructureId(r2.getStructureId())
                    .filterType(CmsRelationType.LOCALE_VARIANT))
            .size());
    ...
    assertEquals(
        0,
        cms.readRelations(
                CmsRelationFilter.relationsToStructureId(r2.getStructureId())
                    .filterType(CmsRelationType.LOCALE_VARIANT))
            .size());
    assertEquals(
        1,
        cms.readRelations(
                CmsRelationFilter.relationsToStructureId(r1.getStructureId())
                    .filterType(CmsRelationType.LOCALE_VARIANT))
            .size());

    assertEquals(
        1,
        cms.readRelations(CmsRelationFilter.relationsFromStructureId(r1.getStructureId())).size());
  }
  ...
  public void testNormalizeRelations2() throws Exception {
    ...
    assertEquals(
        0,
        cms.readRelations(CmsRelationFilter.relationsFromStructureId(r3.getStructureId())).size());
    assertEquals(
        1, cms.readRelations(CmsRelationFilter.relationsToStructureId(r1.getStructureId())).size());
    assertEquals(
        1, cms.readRelations(CmsRelationFilter.relationsToStructureId(r3.getStructureId())).size());
  }
