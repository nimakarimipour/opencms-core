...
public abstract class TestCmsMessageBundles extends OpenCmsTestCase {
  ...
  private Map<Locale, List<I_CmsMessageBundle>> m_excludedBundles =
      new HashMap<Locale, List<I_CmsMessageBundle>>();
   ...
   */
  protected String doPreTestBundle(
      String className, String bundleName, Locale locale, boolean client) {
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if ((key.charAt(key.length() - 2) != '_')
          || (!key.startsWith(KEY_PREFIX_ERR)
              ...
              && !key.startsWith(KEY_PREFIX_RPT))) {
        errorMessages.add(
            "Key '" + key + "' must have the form {ERR|LOG|INIT|GUI|RPT}_KEYNAME_{0-9}.");
        continue;
      }
      int argCount = 0;
      try {
  ...
  /**
   * Prepares the test for the given bundle and locale and returns a message bundle that DOES NOT
   * include the default keys.
   *
   * <p>
   *
  ...
  protected CmsMessages getMessageBundle(String bundleName, Locale locale) throws IOException {
    ...
    String fileName =
        CmsStringUtil.substitute(bundleName, ".", "/") + "_" + locale.toString() + ".properties";
   ...
   * <p>
   *
   * @return the <code>I_CmsMessageBundle</code> instances to test: these will be the singleton
   *     instances of the <code>Messages</code> classes residing in every localized package.
   */
  ...
  protected void messagesBundleConstantTest(Locale locale) throws Exception {
    ...
    // the default locale MUST be ENGLISH (this call will also set the default locale to ENGLISH if
    // required)
    ...
    for (I_CmsClientMessageBundle clientBundle : clientBundles) {
      ...
      tmp =
          doPreTestBundle(
              clientBundle.getClass().getName(), clientBundle.getBundleName(), locale, true);
      errors.append(tmp);
      if (CmsStringUtil.isEmpty(tmp)) {
        errors.append(
            doTestBundle(clientBundle.getClientImpl(), clientBundle.getBundleName(), locale));
      }
    }
    if (errors.length() > 0) {
      fail("\n" + errors);
