...
/**
 * Tests login and password related functions.
 *
 * <p>
 *
 * @since 6.0
...
public class TestLoginAndPasswordHandler extends OpenCmsTestCase {
  ...
  /**
   * Tests if the password is digested and stored correctly.
   *
   * <p>
   *
  ...
  /**
   * Tests the login message functions.
   *
   * <p>
   *
  ...
  public void testLoginMessage() throws Exception {
    ...
    loginMessage =
        new CmsLoginMessage(System.currentTimeMillis() + 100000, Long.MAX_VALUE, message, true);
    OpenCms.getLoginManager().setLoginMessage(cms, loginMessage);
    cms.loginUser(test1User, "test1");
  ...
  /**
   * Tests logging in as a user (checking for different kind of exceptions).
   *
   * <p>
   *
   * @throws Exception if the test fails
  ...
  public void testLoginUser() throws Exception {
    ...
    cms.loginUser(adminUser, "admin");
    assertTrue(
        OpenCms.getDefaultUsers().isUserAdmin(cms.getRequestContext().getCurrentUser().getName()));
  ...
  /**
   * Tests if the password is automatically converted from the old to the new hash algorithm.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  public void testPasswordConvesion() throws Throwable {
    ...
    echo(
        "Testing if the password is automatically converted from the old to the new hash algorithm");
    ...
    assertEquals(
        ...
        OpenCms.getPasswordHandler()
            .digest(testData, I_CmsPasswordHandler.DIGEST_TYPE_MD5, CmsEncoder.ENCODING_UTF_8));
  ...
  /**
   * Tests the static "validatePassword" method of the password handler.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  /**
   * Tests basic functionality and availability of the SCrypt algorithm.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  /**
   * Tests the setPassword and resetPassword methods.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  public void testTwoFactorAuthentication() throws Exception {
    ...
    assertTrue(
        "should not be able to log in with dummy verification code and secret", caughtException);

    String code;
    DefaultCodeGenerator generator = new DefaultCodeGenerator();

    CmsSecondFactorSetupInfo info =
        OpenCms.getTwoFactorAuthenticationHandler().generateSetupInfo(user);
  ...
  /**
   * Tests if the user passwords are imported / set correctly.
   *
   * <p>
   *
   * @throws Exception if something goes wrong
  ...
  public void testUserDefaultPasswords() throws Exception {
    ...
    try {
      // check should fail, but if we get an exception then the password does not have the right
      // format
      SCryptUtil.check("{random-value}", user.getPassword());
    } catch (IllegalArgumentException e) {
