...
/**
 * Tests for <code>{@link org.opencms.security.CmsPrincipal}</code> (and it's subclasses).
 *
 * <p>
 */
public class TestCmsPrincipal extends OpenCmsTestCase {
  ...
  /**
   * Tests basic principal read operation.
   *
   * <p>
   *
  ...
  public void testBasicReadOperation() throws Exception {
    ...
    prefixedName =
        CmsPrincipal.getPrefixedGroup(OpenCms.getDefaultUsers().getGroupAdministrators());
    principal = CmsPrincipal.readPrefixedPrincipal(cms, prefixedName);
    ...
    if (caught != null) {
      assertSame(
          org.opencms.db.Messages.ERR_READ_USER_FOR_NAME_1, caught.getMessageContainer().getKey());
    }
    ...
    if (caught != null) {
      assertSame(
          org.opencms.db.Messages.ERR_READ_GROUP_FOR_NAME_1, caught.getMessageContainer().getKey());
    }
  }
  ...
  public void testPrefixMethods() throws Exception {
    ...
    assertTrue(
        "User prefix check with correct user name 2 failed", CmsUser.hasPrefix("  USER.hans"));
    assertTrue(
        "User prefix check with correct user name 3 failed", CmsUser.hasPrefix("USER.hans   "));
    assertTrue(
        "User prefix check with correct user name 4 failed", CmsUser.hasPrefix("User.hans   "));
    assertTrue(
    ...
    // Group checks
    assertTrue(
        "Group prefix check with correct group name 1 failed", CmsGroup.hasPrefix("GROUP.Users"));
    assertTrue(
        "Group prefix check with correct group name 2 failed", CmsGroup.hasPrefix("  GROUP.Users"));
    assertTrue(
        "Group prefix check with correct group name 3 failed",
        CmsGroup.hasPrefix("GROUP.Users   "));
    assertTrue(
        "Group prefix check with correct group name 4 failed",
        CmsGroup.hasPrefix("Group.Users   "));
    assertTrue(
    ...
    assertFalse(
        "Group prefix check with wrong group name 1 failed", CmsGroup.hasPrefix("GROUPUsers"));
    assertFalse(
        "Group prefix check with wrong group name 2 failed", CmsGroup.hasPrefix("GROUPS.Users"));
    assertFalse(
        "Group prefix check with wrong group name 3 failed", CmsGroup.hasPrefix("SGROUPS.Users"));

    // Role checks
    assertTrue(
        "Role prefix check with correct role name 1 failed", CmsRole.hasPrefix("ROLE.EDITOR"));
    assertTrue(
        "Role prefix check with correct role name 2 failed", CmsRole.hasPrefix("  ROLE.EDITOR"));
    assertTrue(
        "Role prefix check with correct role name 3 failed", CmsRole.hasPrefix("ROLE.EDITOR   "));
    assertTrue(
        "Role prefix check with correct role name 3 failed", CmsRole.hasPrefix("Role.EDITOR   "));
    assertTrue(
    ...
    assertFalse(
        "Role prefix check with wrong role name 2 failed", CmsRole.hasPrefix("ROLES.EDITOR"));
    assertFalse(
        "Role prefix check with wrong role name 3 failed", CmsRole.hasPrefix("SROLES.EDITOR"));
  }
