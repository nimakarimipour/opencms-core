...
public class TestXml2Json extends OpenCmsTestCase {
  ...
  public void testAccessExclude() throws Exception {
    ...
    CmsJsonAccessPolicy access =
        CmsJsonAccessPolicy.parse(getClass().getResourceAsStream("access-ex.xml"));
  ...
  public void testAccessGroup() throws Exception {
    ...
    CmsJsonAccessPolicy access =
        CmsJsonAccessPolicy.parse(getClass().getResourceAsStream("access1.xml"));
  ...
  public void testAccessIncludeExclude() throws Exception {
    ...
    CmsJsonAccessPolicy access =
        CmsJsonAccessPolicy.parse(getClass().getResourceAsStream("access-in-ex.xml"));
    CmsObject cms = getCmsObject();
  ...
  public void testCustomRenderer() throws Exception {
    ...
    CmsResource testFileRes =
        cms.createResource(
            testFile, contentType, data.get("input").trim().getBytes("UTF-8"), new ArrayList<>());
  ...
  /**
   * Read XML input and expected output from data file and check if rendered JSON matches expected
   * output.
   *
   ...
   *
   * <p>The file contains entries for the JSON input, the path to be removed, and the expected
   * output.
   *
   * @param name the test data file name
  ...
  protected void runRemoveTest(String name) throws Exception {
    ...
    assertEquals(
        "Failed check in path removal test <" + name + ">", fmtExpectedJson, fmtActualJson);
  }
}
