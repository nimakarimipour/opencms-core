...
public final class OpenCmsTestProperties {
  ...
  /**
   * Returns the absolute path name for the given relative path name if it was found by the context
   * Classloader of the current Thread.
   *
   * <p>The argument has to denote a resource within the Classloaders scope. A <code>
   * {@link java.net.URLClassLoader}</code> implementation for example would try to match a given
   * path name to some resource under it's URL entries.
   *
   * <p>As the result is internally obtained as an URL it is reduced to a file path by the call to
   * <code>{@link java.net.URL#getFile()}</code>. Therefore the returned String will start with a
   * '/' (no problem for java.io).
   *
   * <p>
   *
   * @param fileName the filename to return the path from the Classloader for
   * @return the absolute path name for the given relative path name if it was found by the context
   *     Classloader of the current Thread or an empty String if it was not found
    ...
    if (inputUrl != null) {
      ...
      try {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_MISSING_CLASSLOADER_RESOURCE_2, fileName, Arrays.asList(paths)));
      } catch (Throwable t) {
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_MISSING_CLASSLOADER_RESOURCE_1, fileName));
      }
    }
    return result;
  }
