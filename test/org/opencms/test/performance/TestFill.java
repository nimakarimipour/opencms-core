...
public class TestFill extends OpenCmsTestCase {
  ...
  public void testPermissionsWithOUs() throws Throwable {
    ...
    OpenCms.getWorkplaceManager()
        .getExplorerTypeSetting(CmsResourceTypeJsp.getStaticTypeName())
        .isEditable(cms, res);
    ...
    for (int k = 0; k < x; k++) {
      ...
      for (int i = 0; i < n; i++) {
        int number = (n * k) + i;
        cms.createResource(
            "testPermissionsWithOu" + number, CmsResourceTypeFolder.RESOURCE_TYPE_ID);
   ...
   *   <li>20 files in each folder, 75% binary / 30% text files,
   *   <li>
   *   <li>with 10 properties, 60% individual / 30% shared properties.
   *   <li>
   * </ul>
   *
   * that is a total of app. 10000 files, and 100000 property values.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  /**
   * Performance test for readFile.
   *
   * <p>10,000 files will be read and 20% of them are binary.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
  ...
  /**
   * Tests the <code>{@link CmsObject#readResourcesWithProperty(String)}</code> method.
   *
   * <p>
   *
   * @throws Throwable if something goes wrong
