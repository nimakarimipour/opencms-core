...
/**
 * Appends log events to <code>System.out</code> or <code>System.err</code> using a layout specified
 * by the user. The default target is <code>System.out</code>.
 *
 * <p>TODO Accessing <code>System.out</code> or <code>System.err</code> as a byte stream instead of
 * a writer bypasses the JVM's knowledge of the proper encoding. (RG) Encoding is handled within the
 * Layout. Typically, a Layout will generate a String and then call getBytes which may use a
 * configured encoding or the system default. OTOH, a Writer cannot print byte streams.
 */
@Plugin(
    name = OpenCmsTestLogAppender.PLUGIN_NAME,
    category = Core.CATEGORY_NAME,
    elementType = Appender.ELEMENT_TYPE,
    printObject = true)
public final class OpenCmsTestLogAppender
  ...
  private OpenCmsTestLogAppender(
      final String name,
  ...
  public static OpenCmsTestLogAppender createAppender(
    ...
    return new OpenCmsTestLogAppender(
        ...
        layout,
        filter,
        getManager(target, isFollow, false, layout),
        ignoreExceptions,
        target);
  }

  public static OpenCmsTestLogAppender createDefaultAppenderForLayout(
      final Layout<? extends Serializable> layout) {
    // this method cannot use the builder class without introducing an infinite loop due to
    // DefaultConfiguration
    return new OpenCmsTestLogAppender(
        "DefaultOpenCmsTestLog-" + COUNT.incrementAndGet(),
        layout,
        null,
  ...
  public static class Builder<B extends Builder<B>> extends AbstractOutputStreamAppender.Builder<B>
    ...
    @PluginBuilderAttribute @Required private Target target = DEFAULT_TARGET;

    @PluginBuilderAttribute private boolean follow;

    @PluginBuilderAttribute private boolean direct;
    ...
    public OpenCmsTestLogAppender build() {
      if (follow && direct) {
        throw new IllegalArgumentException(
            "Cannot use both follow and direct on OpenCmsTestLogAppender '" + getName() + "'");
      }
  ...
  private static OutputStreamManager getDefaultManager(
    ...
    // LOG4J2-1176 DefaultConfiguration should not share OutputStreamManager instances to avoid
    // memory leaks.
    final String managerName = target.name() + '.' + follow + '.' + direct + "-" + COUNT.get();
    return OutputStreamManager.getManager(
        managerName, new FactoryData(os, managerName, layout), factory);
  }
  ...
  private static OutputStream getOutputStream(
      final boolean follow, final boolean direct, final Target target) {
    ...
    if (!propsUtil.isOsWindows()
        || propsUtil.getBooleanProperty("log4j.skipJansi", true)
        || direct) {
      return outputStream;
    }
    try {
      ...
      LOGGER.warn(
          ...
          JANSI_CLASS,
          Throwables.getRootCause(ex).toString().trim());
    }
    return outputStream;
  }
  ...
  private static class OpenCmsTestLogManagerFactory
  ...
  private static class OutputStreamManagerEx extends OutputStreamManager {
    protected OutputStreamManagerEx(
