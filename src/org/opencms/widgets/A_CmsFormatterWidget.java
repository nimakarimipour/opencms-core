...
public abstract class A_CmsFormatterWidget extends CmsSelectWidget {
  ...
  /**
   * Comparator used to sort formatter beans in the order in which they should be displayed in the
   * selection.
   *
   * <p>
   */
  public static class FormatterSelectComparator implements Comparator<I_CmsFormatterBean> {
    ...
    public int compare(I_CmsFormatterBean first, I_CmsFormatterBean second) {
      ...
      return ComparisonChain.start()
          .compare(firstSet.toString(), secondSet.toString())
          .compare(first.getRank(), second.getRank())
          .result();
    }
  }
   ...
   */
  public static CmsSelectWidgetOption getWidgetOptionForFormatter(
      CmsObject cms, I_CmsFormatterBean formatter) throws Exception {

    String name =
        formatter.getNiceName(OpenCms.getWorkplaceManager().getWorkplaceLocale(cms))
            + " "
            ...
            + ")";
    String path =
        cms.readResource(new CmsUUID(formatter.getId()), CmsResourceFilter.ONLY_VISIBLE_NO_DELETED)
            .getRootPath();
   ...
   */
  protected abstract Set<String> getSelectedInFile(
  ...
  protected abstract List<CmsSelectWidgetOption> getTypeOptions(
      CmsObject cms, CmsADEConfigData config, boolean allRemoved);
  ...
  protected List<CmsSelectWidgetOption> parseSelectOptions(
    ...
    options.add(
        new CmsSelectWidgetOption(
            "", true, getMessage(cms, Messages.GUI_FORMATTER_EMPTY_SELECTION_0)));
    return options;
  }
  ...
  protected List<CmsSelectWidgetOption> parseSelectOptions(
    ...
    try {
      ...
      options.add(
          new CmsSelectWidgetOption(
              "", true, getMessage(cms, Messages.GUI_FORMATTER_EMPTY_SELECTION_0)));
      List<CmsSelectWidgetOption> formatterOptions =
          getFormatterOptions(cms, adeConfig, path, allRemoved);
        ...
        for (@RUntainted String formatterKey : selected) {
          if (CmsUUID.isValidUUID(formatterKey)) {
            ...
            if (formatter != null) {
              try {
                CmsSelectWidgetOption option =
                    A_CmsFormatterWidget.getWidgetOptionForFormatter(cms, formatter);
