...
public class CmsVfsFileWidget extends A_CmsWidget implements I_CmsADEWidget {
  ...
  /**
   * Flag to determine project awareness, ie. if resources outside of the current project should be
   * displayed as normal.
   */
   ...
   * @param projectAware <code>true</code> if resources outside of the current project should be
   *     displayed as normal
   */
  public CmsVfsFileWidget(
      boolean showSiteSelector, String startSite, boolean includeFiles, boolean projectAware) {
  ...
  public String getDialogIncludes(CmsObject cms, I_CmsWidgetDialog widgetDialog) {
    ...
    result.append("\n");
    result.append(
        getJSIncludeFile(CmsWorkplace.getSkinUri() + "components/widgets/fileselector.js"));
    return result.toString();
  }
  ...
  public String getDialogInitMethod(CmsObject cms, I_CmsWidgetDialog widgetDialog) {
    ...
    // initialize tree javascript, does parts of <code>CmsTree.initTree(CmsObject, encoding,
    // skinuri);</code>
   ...
   *
   * <p>If <code>null</code> is returned, the dialog will display the current site of the current
   * user.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the site selector is shown.
   *
  ...
  public void setConfiguration(String configuration) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(configuration)) {
      ...
      if (startFolderIndex != -1) {
        String startFolder =
            configuration.substring(startFolderIndex + CONFIGURATION_STARTFOLDER.length() + 1);
  ...
  protected JSONObject getJsonConfig(
    ...
    try {
      ...
      if (m_includeFiles) {
        ...
        if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_selectableTypes)
            && !Arrays.asList(m_selectableTypes.split("[, ]+"))
                .contains(CmsResourceTypeXmlContainerPage.getStaticTypeName())) {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_searchTypes)) {
        ...
        config.put(
            I_CmsGalleryProviderConstants.CONFIG_SEARCH_TYPES,
            getDefaultSearchTypes(cms, resource));
      }
      ...
      String treeToken =
          ""
              + Objects.hashCode(
                  m_startSite, cms.getRequestContext().getSiteRoot(), "" + m_selectableTypes);
      ...
      if (m_gallerySelect) {
        ...
        config.put(
            ...
            CmsGalleryTabConfiguration.TC_GALLERIES);
      }
  ...
  /**
   * Computes the tree token, which is used to decide which preloaded tree, if any, to load for the
   * VFS/sitemap tabs.
   *
   * <p>
   *
  ...
  protected String getTreeToken(
      CmsObject cms, A_CmsXmlContentValue value, CmsResource resource, Locale contentLocale) {

    return cms.getRequestContext().getSiteRoot();
  }

  /**
   * Checks whether the given type list contains only folder types.
   *
   * <p>
   *
