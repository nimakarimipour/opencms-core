...
/**
 * This is an abstract class which you can inherit from to relatively easily implement complex
 * widgets for rendering nested contents in Javascript.
 *
 * <p>Subclasses of this class only need to implement the methods getScripts(), getStyleSheets(),
 * getName(), and configure().
 *
 * <p>The name of the initialization function is generated by prepending the string "init_" to the
 * widget name returned by getName().
 */
public abstract class A_CmsNativeComplexWidget implements I_CmsComplexWidget {

  /**
   * The prefix which is prepended to the widget name to get the name of the initialization
   * function.
   */
   ...
   *     <p>The result of this method will be used to determine the initialization function name for
   *     the widget by prepending the prefix "init_". For example, if "foo" is returned by this
   *     method, "init_foo" will be used as the initialization function name.
   */
  ...
  public final CmsComplexWidgetData getWidgetData(CmsObject cms) {
    ...
    CmsExternalWidgetConfiguration config =
        new CmsExternalWidgetConfiguration(getName(), init, scripts, styles);
    return new CmsComplexWidgetData(
        CmsContentDefinition.NATIVE_RENDERER, m_jsonConfig.toString(), config);
  }

  /**
   * Initializes the configuration date from the given configuration string.
   *
   * <p>This should be called by subclasses of this class.
   *
   * <p>
   *
