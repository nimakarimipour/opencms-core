 ...
 *
 * <p>A filter has a unique id, a set of options (consisting of a map whose keys are internal
 * identifiers of the options and whose values are the texts to display to the user), a current
 * value and a 'nice name' to display as caption for the filter.
 *
 * <p>Filter options don't by default have a "null" (=neutral, i.e. filter is not applied) option;
 * if you need this, add a corresponding option in your implementation of I_CmsDataView.
 */
public class CmsDataViewFilter {
   ...
   *
   * <p>Internally, this is stored as a LinkedHashMap, so the order of options is preserved.
   *
   * <p>The keys of the map are internal identifiers for the options, while the values are the texts
   * displayed in the GUI for the options.
   *
