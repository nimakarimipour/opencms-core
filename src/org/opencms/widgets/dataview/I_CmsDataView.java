 ...
 *
 * <p>This provides everything the widget needs to query an external data source, display the query
 * results (items) in tabular form, and provide the data to be stored when the user selects one or
 * multiple items.
 */
public interface I_CmsDataView {
   ...
   *
   * <p>This method should always return the same columns.
   *
   ...
   *
   * <p>This is only used for the initial display of the data table; if the user changes the
   * filters, the method updateFilters() is called instead.
   *
   ...
   *
   * <p>If no data item with that id exists, this method should return null.
   *
   * <p>
   *
  ...
  /**
   * Gets the result for the given query, with the given offset and limited to the given number of
   * results.
   *
   * <p>The returned result includes a list of result items, and a total hit count for the given
   * query (i.e. the number of results that would be returned for offset 0 and unlimited result
   * count). The hit count should be as accurate as possible.
   *
   ...
   *
   * <p>This is called when the user changes any of the current filters in the data select widget.
   * The list passed as a parameter should not be modified, instead a new filter list should be
   * returned. If you do not need a dynamically changing set of filters, just return the argument
   * 'prevFilters' in your implementation.
   *
