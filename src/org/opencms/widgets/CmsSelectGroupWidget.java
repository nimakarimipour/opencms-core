...
public class CmsSelectGroupWidget extends CmsSelectWidget {
   ...
   *
   * <p>The list elements are of type <code>{@link CmsSelectWidgetOption}</code>. The configuration
   * String is parsed only once and then stored internally.
   *
   * <p>
   *
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(filter)) {
        try {
          ...
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERR_WIDGET_SELECTGROUP_PATTERN_1, filter));
        }
      }
