 ...
 *
 * <p>This widget is configurable with the following options:
 *
 * <p>
 *
 * <ul>
 *   <li><code>groupfilter</code>: regular expression to filter available groups
 *   <li><code>groups</code>: comma separated list of group names to show in the select box.
 *       <b>Note</b>: if this configuration option if used, <code>groupfilter</code> and <code>
 *       includesubous</code> are <i>not</i> considered anymore.
 *   <li><code>includesubous</code>: boolean flag to indicate if sub OUs should be scanned for
 *       groups to select
 *   <li><code>oufqn</code>: the fully qualified name of the OU to read the groups from
 *   <li><code>fullnames</code>: boolean flag to indicate whether the full names of groups
 *       (including organizational unit) should be shown
 * </ul>
 *
 ...
 * </code>
 *
 * <p>This means that the +r+v permission is written for the principal <code>GROUP</code> on the
 * resource. Additionally two permissions are written as default: for <code>ALL_OTHERS</code>, no
 * allowed permission is set, for <code>Projectmanagers</code>, "+r+v+w+c" is set.
 *
 * <p>
 *
 * @author Mario Jaeger
...
public class CmsMultiSelectGroupWidget extends CmsSelectGroupWidget {
   ...
   * @param asCheckboxes indicates if used html code is a multi selection list or a list of
   *     checkboxes
  ...
  public String getDialogWidget(
    ...
    if (!m_asCheckBoxes) {
      if (m_requiresActivation) {
        ...
        result.append("</label>&nbsp;");
        // adding hidden input with the current value, because disabled select box value won't be
        // submitted
        result
  ...
  public void setConfiguration(String configuration) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(configuration)) {
      int asCheckBoxesIndex =
          configuration.indexOf(CmsMultiSelectWidget.CONFIGURATION_ASCHECKBOXES);
      ...
      int reqiresActivationIndex =
          configuration.indexOf(CmsMultiSelectWidget.CONFIGURATION_REQUIRES_ACTIVATION);
      if (reqiresActivationIndex != -1) {
        // the height is set
        String requiresActivation =
            configuration.substring(
                reqiresActivationIndex
  ...
  /**
   * @see org.opencms.widgets.CmsSelectGroupWidget#parseSelectOptions(org.opencms.file.CmsObject,
   *     org.opencms.i18n.CmsMessages, org.opencms.widgets.I_CmsWidgetParameter)
   */
  @Override
  protected List<CmsSelectWidgetOption> parseSelectOptions(
      CmsObject cms, CmsMessages messages, I_CmsWidgetParameter param) {
      ...
      if (isUseGroupNames()) {
          ...
          try {
            ...
            result.add(
                new CmsSelectWidgetOption(
                    group.getName(), m_defaultAllAvailable, getGroupLabel(group)));
        ...
        try {
          List<CmsGroup> groups =
              OpenCms.getOrgUnitManager().getGroups(cms, getOuFqn(), isIncludeSubOus());
  ...
  private void parseConfiguration(CmsObject cms, CmsMessages widgetDialog) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(filter)) {
      try {
        ...
        LOG.error(
            Messages.get().getBundle().key(Messages.LOG_ERR_WIDGET_SELECTGROUP_PATTERN_1, filter));
      }
    }
    ...
    m_useFullNames =
        Boolean.parseBoolean(config.get(CmsMultiSelectGroupWidget.CONFIGURATION_FULLNAMES));
