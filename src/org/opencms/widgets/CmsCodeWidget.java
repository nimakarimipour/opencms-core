 ...
 *
 * <p>The configuration string is a JSON string (in the form parseable by
 * org.opencms.json.JSONObject) with the following possible optional attributes:
 *
 * <ul>
 *   <li>mode: the initially selected editor mode (text|javascript|jsp|java|html|xml)
 *   <li>height: the height in pixels (if 'grow' is set to true, this is used as minimum height
 *       instead).
 * </ul>
 */
public class CmsCodeWidget extends A_CmsWidget implements I_CmsADEWidget {
  ...
  private static I_CmsAutoBeanFactory m_configFactory =
  ...
  public String getConfiguration(
    ...
    try {
      ...
      AutoBean<I_CmsCodeMirrorClientConfiguration> clientConfig =
          m_configFactory.createConfiguration();
  ...
  private String getPhrases(Locale userLocale) {
    ...
    try {
      ...
      stream =
          getClass()
              .getResourceAsStream("codemirror_phrases_" + userLocale.getLanguage() + ".json");
      if (stream != null) {
        try {
