...
public class CmsDisplayTypeSelectWidget extends CmsSelectWidget {
  ...
  class FormatterOption {
     ...
     */
    FormatterOption(
        String key, String typeName, String displayType, String label, int rank, Integer typePos) {
  ...
  /**
   * Name of the sitemap attribute to control whether the old or new way of collecting formatter
  ...
  /**
   * Evaluates the display type of the given formatter.
   *
   * <p>
   *
  ...
  /**
   * Returns the available formatter options.
   *
   * <p>
   *
  ...
  private List<FormatterOption> getFormatterOptions(CmsObject cms, CmsResource resource) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_displayContainerTypeConfig)) {
      ...
      if (CmsPropertyDefinition.PROPERTY_TEMPLATE_DISPLAY_TYPES.equals(
          m_displayContainerTypeConfig)) {
        try {
          CmsProperty prop =
              cms.readPropertyObject(
                  resource, CmsPropertyDefinition.PROPERTY_TEMPLATE_DISPLAY_TYPES, true);
              ...
              CmsTemplateLoaderFacade loaderFacade =
                  OpenCms.getResourceManager()
                      .getTemplateLoaderFacade(
                          cms, null, resource, CmsPropertyDefinition.PROPERTY_TEMPLATE);
              CmsResource template = loaderFacade.getLoaderStartResource();
              if (template != null) {
                prop =
                    cms.readPropertyObject(
                        template, CmsPropertyDefinition.PROPERTY_TEMPLATE_DISPLAY_TYPES, false);
    ...
    if (config != null) {
      ...
      boolean useConfiguredFormatters =
          Boolean.parseBoolean(config.getAttribute(ATTR_USE_CONFIG_FORMATTERS, "false"));
    ...
    // Formatters for types in sitemap config come first, in the order defined there, then sorted by
    // formatter rank.
    ...
    Collections.sort(
        ...
        (f1, f2) ->
            ComparisonChain.start()
                ...
                .compare(f1.m_typeName, f2.m_typeName)
                .compare(f1.m_rank, f2.m_rank)
                .compare(f1.m_label, f2.m_label)
                .result());
    return options;
  }
}
