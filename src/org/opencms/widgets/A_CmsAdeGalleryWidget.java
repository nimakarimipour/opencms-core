...
public abstract class A_CmsAdeGalleryWidget extends A_CmsWidget implements I_CmsADEWidget {
  ...
  /**
   * Returns the lower case name of the gallery, for example <code>"html"</code>.
   *
   * <p>
   *
  ...
  protected Map<String, String> getGalleryOpenParams(
    ...
    result.put(
        I_CmsGalleryProviderConstants.CONFIG_GALLERY_STORAGE_PREFIX, getGalleryStoragePrefix());
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(configuration.getGalleryTypes())) {
      result.put(
          I_CmsGalleryProviderConstants.CONFIG_GALLERY_TYPES, configuration.getGalleryTypes());
    }
  ...
  protected JSONObject getJSONConfig(
    ...
    try {
      for (Entry<String, String> paramEntry :
          getGalleryOpenParams(cms, messages, schemaType, cms.getSitePath(resource), 0)
              .entrySet()) {
        result.put(paramEntry.getKey(), paramEntry.getValue());
      }
      JSONObject additional =
          getAdditionalGalleryInfo(cms, cms.getSitePath(resource), messages, null);
      if (additional != null) {
  ...
  protected String getOpenGalleryCall(
      CmsObject cms, I_CmsWidgetDialog widgetDialog, I_CmsWidgetParameter param, long hashId) {

    StringBuffer sb = new StringBuffer(128);
    sb.append("javascript:cmsOpenDialog('");

    // the gallery title
    sb.append(widgetDialog.getMessages().key(Messages.getButtonName(getGalleryName())))
        .append("', '");
  ...
  protected String getOpenPreviewCall(I_CmsWidgetDialog widgetDialog, String id) {
    ...
    sb.append("javascript:cmsOpenPreview('")
        .append(widgetDialog.getMessages().key(Messages.GUI_BUTTON_PREVIEW_0));
  ...
  protected CmsGalleryWidgetConfiguration getWidgetConfiguration(
    ...
    if (m_widgetConfiguration == null) {
      m_widgetConfiguration =
          new CmsGalleryWidgetConfiguration(cms, messages, param, getConfiguration());
    }
