...
public class CmsVfsImageWidget extends CmsAdeImageGalleryWidget {
  ...
  /**
   * @see
   *     org.opencms.widgets.I_CmsWidget#getDialogIncludes(org.opencms.file.CmsObject,org.opencms.widgets.I_CmsWidgetDialog)
   */
  @Override
  ...
  public String getDialogWidget(
    ...
    result.append("<td>");
    result
        .append("<input class=\"xmlInputMedium\" value=\"")
        .append(imageLink)
        .append("\" name=\"");
    ...
    try {
      additional =
          getAdditionalGalleryInfo(
              cms,
              widgetDialog instanceof CmsDialog
                  ? ((CmsDialog) widgetDialog).getParamResource()
                  : null,
    ...
    CmsVfsImageWidgetConfiguration configuration =
        getWidgetConfiguration(cms, widgetDialog.getMessages(), param);
    ...
    if (configuration.isShowFormat()) {
      ...
      // add possible format names and values as JS variables to access them from image gallery
      // window
      result.append("\n<script >");
    ...
    if (configuration.isShowDescription()) {
      ...
      result
          ...
          .append(CmsEncoder.escapeXml(description))
          .append("\" name=\"");
      result.append(PREFIX_DESCRIPTION).append(id).append("\" id=\"");
  ...
  /**
   * @see
   *     org.opencms.widgets.CmsAdeImageGalleryWidget#getAdditionalGalleryInfo(org.opencms.file.CmsObject,
  ...
  /**
   * @see org.opencms.widgets.A_CmsAdeGalleryWidget#getGalleryOpenParams(org.opencms.file.CmsObject,
   *     org.opencms.i18n.CmsMessages, org.opencms.widgets.I_CmsWidgetParameter, java.lang.String,
   *     long)
   */
  @Override
  protected Map<String, String> getGalleryOpenParams(
    ...
    Map<String, String> result =
        super.getGalleryOpenParams(cms, widgetDialog, param, resource, hashId);
    // the current element value will be read by java-script including the image input field and the
    // scale input field
    ...
    if (param != null) {
      if (getWidgetConfiguration(cms, widgetDialog, param).isShowFormat()) {
        ...
        currentElement
            .append("%26format%3D'+escape(document.getElementById('")
            .append(PREFIX_FORMAT)
            .append(param.getId())
   ...
   *
   * <p>If a value is found in the given parameter, this is used. Otherwise the default value of the
   * select options are used. If there is neither a parameter value nor a default value, <code>null
   * </code> is returned.
   *
   * <p>
   *
  ...
  protected String getSelectedValue(
      CmsObject cms, List<CmsSelectWidgetOption> selectOptions, String currentValue) {
