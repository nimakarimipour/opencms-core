...
public class CmsAddFormatterWidget extends A_CmsFormatterWidget {
   ...
   */
  public static List<String> getSelectOptionValues(
      CmsObject cms, String rootPath, boolean allRemoved) {
    ...
    try {
      ...
      List<CmsSelectWidgetOption> options =
          getFormatterOptionsStatic(cms, adeConfig, rootPath, allRemoved);
    ...
    for (I_CmsFormatterBean formatterBean : formatters) {
      ...
      if (formatterBean instanceof CmsMacroFormatterBean) {
        boolean systemOrShared =
            formatterBean.getLocation().startsWith(CmsResource.VFS_FOLDER_SYSTEM + "/")
                || formatterBean
                    .getLocation()
                    .startsWith(OpenCms.getSiteManager().getSharedFolder());
        if (!systemOrShared) {
          String formatterSubSite =
              CmsResource.getParentFolder(
                  CmsResource.getParentFolder(
                      CmsResource.getParentFolder(formatterBean.getLocation())));
          ...
          if (subSite.equals(formatterSubSite) || !subSite.startsWith(formatterSubSite)) {
            // we ignore macro formatters that are defined in the current sub site (they are added
            // automatically), or not within a parent site
            continue;
          }
        }
      }
