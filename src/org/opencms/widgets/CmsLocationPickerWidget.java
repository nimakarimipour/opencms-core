...
public class CmsLocationPickerWidget extends A_CmsWidget implements I_CmsADEWidget {
  ...
  private static final String DEFAULT_CONFIG =
      "{\"edit\":[\"map\", \"address\", \"coords\", \"size\", \"zoom\", \"type\", \"mode\"]}";

  /**
   * Key post fix, so you can display different help text if used a "normal" widget, and a display
   * widget.
   */
        ...
        try {
          apiKey =
              cms.readPropertyObject(
                      sitePath, CmsPropertyDefinition.PROPERTY_GOOGLE_API_KEY_WORKPLACE, true)
                  .getValue();
          keyMessage =
              Messages.get()
                  .getBundle()
          ...
          if (CmsStringUtil.isEmptyOrWhitespaceOnly(apiKey)) {
            apiKey =
                cms.readPropertyObject(
                        sitePath, CmsPropertyDefinition.PROPERTY_GOOGLE_API_KEY, true)
                    .getValue();
            keyMessage =
                Messages.get()
                    .getBundle()
  ...
  public String getDialogWidget(
    ...
    if (CmsStringUtil.TRUE.equalsIgnoreCase(value) || CmsStringUtil.FALSE.equalsIgnoreCase(value)) {
      ...
      if (booleanValue) {
        localizedValue =
            Messages.get().getBundle(widgetDialog.getLocale()).key(Messages.GUI_LABEL_TRUE_0);
      } else {
        localizedValue =
            Messages.get().getBundle(widgetDialog.getLocale()).key(Messages.GUI_LABEL_FALSE_0);
      }
    }
  ...
  /**
   * @see org.opencms.widgets.A_CmsWidget#getHelpBubble(org.opencms.file.CmsObject,
   *     org.opencms.widgets.I_CmsWidgetDialog, org.opencms.widgets.I_CmsWidgetParameter)
   */
  @Override
  ...
  /**
   * @see org.opencms.widgets.A_CmsWidget#getHelpText(org.opencms.widgets.I_CmsWidgetDialog,
