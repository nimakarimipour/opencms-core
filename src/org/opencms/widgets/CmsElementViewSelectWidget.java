 ...
 *
 * <p>If the widget configuration contains the string 'selectparent', the widget will be used for
 * selecting parent views of a view. This makes sense *only* when the widget is used in the element
 * view content itself.
 */
public class CmsElementViewSelectWidget extends CmsSelectWidget {
  ...
  protected List<CmsSelectWidgetOption> parseSelectOptions(
    ...
    for (CmsElementView view : views.values()) {
      ...
      options.add(
          new CmsSelectWidgetOption(value, false, view.getTitle(cms, widgetDialog.getLocale())));
    }
    if (isSelectParent()) {
      //
  ...
  /**
   * Returns true if the 'selectparent' option is enabled.
   *
   * <p>
   *
  ...
  /**
   * Check if the view should be ignored when constructing select options.
   *
   * <p>
   *
  ...
  private boolean shouldIgnore(
      CmsElementView view, Map<CmsUUID, CmsElementView> views, String myPath) {
