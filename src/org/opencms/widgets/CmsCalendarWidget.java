...
public class CmsCalendarWidget extends A_CmsWidget implements I_CmsADEWidget {
   ...
   * the page.
   *
   * <p>The default <code>"opencms"</code> style is used.
   *
   * <p>
   *
   ...
   *
   * <p>This method must be called at the end of a HTML page, e.g. before the closing &lt;body&gt;
   * tag.
   *
   * <p>
   *
  ...
  public static String calendarInit(
    ...
    if (showTime
        && (messages
                .key(org.opencms.workplace.Messages.GUI_CALENDAR_TIMEFORMAT_0)
                .toLowerCase()
                .indexOf("p")
            != -1)) {
      result.append(",\n\t\ttimeFormat     :    \"12\"");
   ...
   */
  public static long getCalendarDate(CmsMessages messages, String dateString, boolean useTime)
  ...
  public static String getCalendarLocalizedTime(
      Locale locale, CmsMessages messages, long timestamp) {
  ...
  /**
   * Returns the language suffix for the calendar-*.js localizations.
   *
   * <p>
   *
  ...
  public String getConfiguration(
    ...
    // ensure configuration is either empty or a valid JSON object - if empty, convert to empty JSON
    // object,
  ...
  public String getDialogWidget(
    ...
    result.append("<td>");
    result.append(
        "<table class=\"editorbuttonbackground\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" id=\"");
    result.append(id);
      ...
      try {
        result =
            getCalendarLocalizedTime(
                widgetDialog.getLocale(), widgetDialog.getMessages(), Long.parseLong(result));
        ...
        if (CmsStringUtil.isNotEmpty(dateTimeValue)) {
          try {
            ...
            if (LOG.isWarnEnabled()) {
              LOG.warn(
                  Messages.get().getBundle().key(Messages.ERR_PARSE_DATETIME_1, dateTimeValue), e);
            }
          }
        } else {
          dateTime = 0;
