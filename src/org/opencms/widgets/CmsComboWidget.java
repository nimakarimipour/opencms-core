...
/**
 * Provides a HTML text input field with optional values to select in a combo box, for use on a
 * widget dialog.
 *
 * <p>Please see the documentation of <code>{@link org.opencms.widgets.CmsSelectWidgetOption}</code>
 * for a description about the configuration String syntax for the select options.
 *
 * <p>The combo widget does use the following select options:
 *
 * <ul>
 *   <li><code>{@link org.opencms.widgets.CmsSelectWidgetOption#getValue()}</code> for the texts to
 *       be displayed in the combo selector
 *   <li><code>{@link org.opencms.widgets.CmsSelectWidgetOption#isDefault()}</code> to fill the
 *       input with a preselected text
 *   <li><code>{@link org.opencms.widgets.CmsSelectWidgetOption#getHelp()}</code> to display an
 *       (optional) help text for the combo option
 * </ul>
 *
 * @since 6.0.0
 */
public class CmsComboWidget extends A_CmsSelectWidget {
  ...
  /**
   * @see org.opencms.widgets.A_CmsWidget#getDialogHtmlEnd(org.opencms.file.CmsObject,
  ...
  public String getDialogHtmlEnd(
    ...
    if (options.size() > 0) {
      ...
      while (i.hasNext()) {
        ...
        String itemId =
            new StringBuffer(64).append("ci").append(id).append('.').append(count).toString();
        ...
        if (option.getHelp() != null) {
          ...
          result.append(
              getJsHelpMouseHandler(
                  widgetDialog, itemId, CmsStringUtil.escapeJavaScript(locValue)));
        }
        result.append(">");
  ...
  public String getDialogWidget(
    ...
    result
        .append("<button name=\"test\" onclick=\"showCombo(\'")
        .append(id)
        .append("\', \'combo")
        .append(id);
