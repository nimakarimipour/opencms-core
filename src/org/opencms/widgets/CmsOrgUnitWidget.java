...
public class CmsOrgUnitWidget extends A_CmsSelectWidget implements I_CmsADEWidget {
  ...
  /**
   * Configuration parameter to set the role the current user must have in the selected ou,
   * optional.
   */
  ...
  public String getDialogIncludes(CmsObject cms, I_CmsWidgetDialog widgetDialog) {
    ...
    result.append(
        getJSIncludeFile(CmsWorkplace.getSkinUri() + "components/widgets/orgunitselector.js"));
    return result.toString();
  }
  ...
  /**
   * Returns the role, or <code>null</code> if none.
   *
   * <p>
   *
  ...
  protected List<CmsSelectWidgetOption> parseSelectOptions(
    ...
    try {
      if (m_role != null) {
        orgUnits.addAll(
            OpenCms.getRoleManager().getOrgUnitsForRole(cms, m_role.forOrgUnit(""), true));
      } else {
    ...
    for (CmsOrganizationalUnit ou : orgUnits) {
      CmsSelectWidgetOption option =
          new CmsSelectWidgetOption(ou.getName(), false, ou.getDisplayName(locale));
      result.add(option);
    }
