...
public abstract class A_CmsSelectWidget extends A_CmsWidget implements I_CmsADEWidget {
   ...
   *
   * <p>Please see <code>{@link CmsSelectWidgetOption}</code> for a description of the syntax of the
   ...
   *
   * <p>Only widgets belonging to the OpenCms core should be marked as internal.
   *
   * <p>
   *
  ...
  protected List<String> getSelectedValues(CmsObject cms, I_CmsWidgetParameter param) {
    ...
    String paramValue = param.getStringValue(cms);
    if (CmsStringUtil.isEmpty(paramValue)) {
      Iterator<CmsSelectWidgetOption> itOptions =
          CmsSelectWidgetOption.getDefaultOptions(m_selectOptions).iterator();
   ...
   *
   * <p>The list elements are of type <code>{@link CmsSelectWidgetOption}</code>.
   *
   * <p>
   *
  ...
  protected List<CmsSelectWidgetOption> parseSelectOptions(
    ...
    if (m_selectOptions == null) {
      ...
      configuration =
          CmsMacroResolver.resolveMacros(configuration, cms, widgetDialog.getMessages());
  ...
  /**
   * Converts a list of select widget options to the configuration string as expected on the client
   * side of the widget.
   *
