...
public class CmsSerialDateValue extends A_CmsSerialDateValue {
   ...
   * @param defaultValue the default value, to be returned if the string can not be read from the
   *     JSON object.
  ...
  private CmsUUID readOptionalUUID(JSONObject json, String key) {
    ...
    if (null != id) {
      try {
        ...
        LOG.debug(
            "Reading optional UUID failed. Could not convert \"" + id + "\" to a valid UUID.");
      }
    }
    return null;
  }
      ...
      if (null != array) {
        ...
        for (int i = 0; i < array.length(); i++) {
          try {
            ...
            LOG.debug(
                "Could not read week of month from JSON. Skipping that particular week of month.",
                e);
          }
        }
        return result;
  ...
  /**
   * Convert a collection of objects to a JSON array with the string representations of that
   * objects.
   *
   ...
   *
   * @return <code>null</code> if the day of month is valid, the key of a suitable error message
   *     otherwise.
   */
   ...
   *
   * @return <code>null</code> if the information is valid, the key of the suitable error message
   *     otherwise.
   */
   ...
   *
   * @return <code>null</code> if a week of month is set, the key for a suitable error message
   *     otherwise.
   */
