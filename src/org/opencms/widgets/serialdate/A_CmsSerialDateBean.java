...
/**
 * Abstract base class for serial date beans. It deals with information common for all serial dates
 * and already provides part of the implementation for calculating all dates of the series.
 */
public abstract class A_CmsSerialDateBean implements I_CmsSerialDateBean {
  ...
  /**
   * Constructor for the abstract class for serial date beans. It takes all the arguments that are
   * common for serial dates and should be called from each sub-class.
   *
   ...
   * @param occurrences the maximal number of occurrences of the event as provided by the serial
   *     date widget. If endType is DATE, this parameter is ignored.
  ...
  /**
   * Returns the date provided as the earliest date the event should take place. The time is set to
   * the starting time of the event.
   *
   * @return date where the event should take place earliest with time set to the date's starting
   *     time.
   */
      ...
      switch (getSerialEndType()) {
        ...
        case TIMES:
          m_hasTooManyOccurrences =
              Boolean.valueOf(m_occurrences > CmsSerialDateUtil.getMaxEvents());
          break;
        case DATE:
          m_hasTooManyOccurrences = Boolean.FALSE;
          calculateDates(); // this will set the value automatically to TRUE in case there are too
                            // many dates.
          break;
        default:
   ...
   * @param previousOccurrences the number of events of the series that took place before the date
   *     to check.
   ...
   *
   * @param date the current event date for a event in the series, which is adjusted to the next
   *     date potentially in the series.
   */
