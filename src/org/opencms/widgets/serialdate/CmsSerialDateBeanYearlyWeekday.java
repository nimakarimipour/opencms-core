...
/**
 * Implementation of @{link org.opencms.widgets.serialdate.I_CmsSerialDateBean} that handles series'
 * specified on a yearly base.
 */
public class CmsSerialDateBeanYearlyWeekday extends A_CmsSerialDateBean {
  ...
  /**
   * Constructs the bean with all the information provided by the {@link
   ...
   * @param weekOfMonth if <code>weekDay</code> is <code>null</code> the day of the month the event
   *     should occur, otherwise the number of the specific week day in the month where event should
   *     occur.
  ...
  protected Calendar getFirstDate() {
    ...
    if ((month > m_month.ordinal())
        || ((month == m_month.ordinal())
            && (date.getTimeInMillis() > firstPossibleMatch.getTimeInMillis()))) {
  ...
  /**
   * Adjusts the day in the provided month, that it fits the specified week day. If there's no match
   * for that provided month, the next possible month is checked.
   *
  ...
  private void setFittingWeekDay(Calendar date) {
    ...
    int firstFittingWeekDay =
        (((m_weekDay.toInt() + I_CmsSerialDateValue.NUM_OF_WEEKDAYS) - weekDayFirst)
                % I_CmsSerialDateValue.NUM_OF_WEEKDAYS)
            + 1;
    int fittingWeekDay =
        firstFittingWeekDay + (I_CmsSerialDateValue.NUM_OF_WEEKDAYS * m_weekOfMonth.ordinal());
