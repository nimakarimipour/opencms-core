...
public class CmsSerialDateBeanMonthlyWeeks extends A_CmsSerialDateBean {
   ...
   * @param weeksOfMonth the weeks in the month the event should take place, e.g., the first and the
   *     third week.
  ...
  /**
   * Sets the day of the month that matches the condition, i.e., the day of month of the 2nd
   * Saturday. If the day does not exist in the current month, the last possible date is set, i.e.,
  ...
  private void toCorrectDateWithDay(Calendar date, WeekOfMonth week) {
    ...
    int daysToFirstWeekDayMatch =
        ((m_weekDay.toInt() + I_CmsSerialDateValue.NUM_OF_WEEKDAYS)
                - (date.get(Calendar.DAY_OF_WEEK)))
            % I_CmsSerialDateValue.NUM_OF_WEEKDAYS;
    date.add(Calendar.DAY_OF_MONTH, daysToFirstWeekDayMatch);
    int wouldBeDayOfMonth =
        date.get(Calendar.DAY_OF_MONTH) + ((week.ordinal()) * I_CmsSerialDateValue.NUM_OF_WEEKDAYS);
  ...
  private void toNextDate(Calendar date, int interval) {
    ...
    toCorrectDateWithDay(date, m_weekOfMonthIterator.next());
    if (previousDate
        == date.getTimeInMillis()) { // this can happen if the fourth and the last week are checked.
      toNextDate(date);
    }
  }
}
