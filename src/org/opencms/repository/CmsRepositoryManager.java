 ...
 *
 * <p>The configuration of the repositories is done in the configuration file <code>
 * opencms-importexport.xml</code>.
 *
 * <p>
 *
 * @since 6.2.4
...
public class CmsRepositoryManager {
  ...
  private Map<String, I_CmsRepository> m_repositoryMap =
      new LinkedHashMap<String, I_CmsRepository>();
   ...
   *
   * <p>Is used if there are no repositories configured.
   *
   * <p>
   *
  ...
  /**
   * Creates a list of resource wrappers from a collection of configuration parameters, for use in
  ...
  public static List<I_CmsResourceWrapper> createResourceWrappersFromConfiguration(
      CmsParameterConfiguration config, String paramName, Log log)
    ...
    if (config.containsKey(paramName)) {
      ...
      for (String wrapperString : wrappers) {
        ...
        try {
          ...
          log.error(
              Messages.get().getBundle().key(Messages.LOG_WRAPPER_CLASS_NOT_FOUND_1, className), e);
        ...
        if (CmsLog.INIT.isInfoEnabled()) {
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_ADD_WRAPPER_1, wrapper.getClass().getName()));
        }
      }
    }
  ...
  /**
   * Gets the additional infos for the user who just logged in which is required for the
  ...
  /**
   * Gets a repository by name, but only if its class is a subclass of the class passed as a
   * parameter.
   *
   * <p>Otherwise, null will be returned.
   *
   * <p>
   *
    ...
    for (I_CmsRepository rep : m_repositoryList) {
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_ADD_REPOSITORY_2, rep.getClass().getName(), rep.getName()));
      }
