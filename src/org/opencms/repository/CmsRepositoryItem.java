...
/**
 * Represents a single entry in the repository. In the context of OpenCms this means a single {@link
 * CmsResource}.
 *
 * <p>
 *
...
public class CmsRepositoryItem implements I_CmsRepositoryItem {
  ...
  /**
   * Construct a new CmsRepositoryItem initialized with the {@link CmsResource} to use and the
   * {@link CmsObjectWrapper} needed for some operations.
   *
   * <p>
   *
    ...
    if (m_mimeType == null) {
      try {
        String encoding =
            m_cms
                .readPropertyObject(
                    m_resource, CmsPropertyDefinition.PROPERTY_CONTENT_ENCODING, true)
                .getValue(OpenCms.getSystemInfo().getDefaultEncoding());

        m_mimeType =
            OpenCms.getResourceManager()
                .getMimeType(m_resource.getRootPath(), encoding, CmsResourceManager.MIMETYPE_TEXT);
