 ...
 *
 * <p>You can get an instance of this class by calling {@link CmsRepository#login(String, String)}.
 *
 * <p>This class provides basic file and folder operations on the resources in the VFS of OpenCms.
 *
 * <p>
 *
...
public class CmsRepositorySession extends A_CmsRepositorySession {
  ...
  /**
   * Constructor with an initialized {@link CmsObjectWrapper} and a {@link CmsRepositoryFilter} to
   * use.
   *
   * <p>
   *
   ...
   */
  public CmsRepositorySession(
      CmsObjectWrapper cms, CmsRepositoryFilter filter, CmsResourceTranslator translation) {
  ...
  public void copy(@RUntainted String src, @RUntainted String dest, boolean overwrite, boolean shallow)
    ...
    if (exists(dest)) {
      ...
      if (overwrite) {
        ...
        throw new CmsVfsResourceAlreadyExistsException(
            Messages.get().container(Messages.ERR_DEST_EXISTS_0));
      }
    }
    ...
    if (shallow) {
      m_cms
          .getRequestContext()
          .setAttribute(A_CmsResourceTypeFolderBase.ATTR_SHALLOW_FOLDER_COPY, Boolean.TRUE);
    }
    try {
      ...
      m_cms
          .getRequestContext()
          .removeAttribute(A_CmsResourceTypeFolderBase.ATTR_SHALLOW_FOLDER_COPY);
    }
  ...
  public List<I_CmsRepositoryItem> list(@RUntainted String path) throws CmsException {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_LIST_ITEMS_SUCESS_1, new Integer(ret.size())));
    }
  ...
  /**
   * @see org.opencms.repository.I_CmsRepositorySession#updateProperties(java.lang.String,
   *     java.util.Map)
   */
  public void updateProperties(String path, Map<CmsPropertyName, String> properties)
      throws CmsException {
    ...
    for (Map.Entry<CmsPropertyName, String> entry : properties.entrySet()) {
      ...
      if (pn.getNamespace().equals(PROPERTY_NAMESPACE)) {
        ...
        try {
          String propName =
              EXTERNAL_PREFIX + encodeNamespace(pn.getNamespace()) + "_" + pn.getName();
  ...
  /**
   * Validates (translates) the given path and checks if it is filtered out.
   *
   * <p>
   *
