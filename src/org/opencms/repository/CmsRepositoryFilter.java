 ...
 *
 * <p>It filters path names, depending on the configuration made.
 *
 * <p>
 *
 * @since 6.2.4
...
public class CmsRepositoryFilter {
  ...
  public void initConfiguration() throws CmsConfigurationException {
    ...
    if ((!TYPE_INCLUDE.equals(m_type)) && (!TYPE_EXCLUDE.equals(m_type))) {
      throw new CmsConfigurationException(
          Messages.get().container(Messages.ERR_INVALID_FILTER_TYPE_1, m_type));
    }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      ...
      while (iter.hasNext()) {
        ...
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_ADD_FILTER_RULE_2, m_type, rule.pattern()));
      }
    }
  ...
  /**
   * Returns if the given path matches or partially matches the pattern.
   *
   * <p>For example the regex "/system/modules/" should match the path "/system/". That's not
   * working with Java 1.4. Starting with Java 1.5 there are possiblities to do that. Until then you
   * have to configure all parent paths as a regex filter.
   *
   * <p>
   *
