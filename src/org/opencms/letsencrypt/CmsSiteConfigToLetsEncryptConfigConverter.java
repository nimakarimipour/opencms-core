...
public class CmsSiteConfigToLetsEncryptConfigConverter {
  ...
  public static class DomainGrouping {
    ...
    /**
     * Deterministically generates a certificate name for a set of domains.
     *
     * <p>
     *
  ...
  /**
   * Timed cache for the public suffix list.
   *
   * <p>
   */
  static class SuffixListCache {
    ...
    /**
     * Gets the public suffix list, loading it if hasn't been loaded before or the time since it was
     * loaded was too long ago.
     *
     * <p>
     *
    ...
    public synchronized PublicSuffixList getPublicSuffixList() {
      ...
      if ((m_suffixList == null) || ((now - m_timestamp) > (1000 * 3600))) {
        ...
        try (InputStream stream =
            CmsSiteConfigToLetsEncryptConfigConverter.class.getResourceAsStream(
  ...
  /**
   * Computes the domain information for a single site.
   *
   * <p>
   *
   * @param site the site
  ...
  /**
   * Computes the SiteDomainInfo bean for a collection of URIs.
   *
   * <p>
   *
  ...
  /**
   * Calculates the domain root for a given uri.
   *
   * <p>
   *
   * @param uri an URI
  ...
  /**
   * Gets the domains for a collection of SiteDomainInfo beans.
   *
   * <p>
   *
  ...
  public boolean run(I_CmsReport report, CmsSiteManagerImpl siteManager) {
    ...
    synchronized (LOCK) {
      // *not* using getAvailable sites here, because the result does not include sites with
      // unpublished site folders if called with a CmsObject in the Online project
  ...
  /**
   * Computes the domain grouping for a set of sites and workplace URLs.
   *
   * <p>
   *
   ...
   */
  private DomainGrouping computeDomainGrouping(
      Collection<CmsSite> sites, Collection<String> workplaceUrls) {
    ...
    if (addSites) {
      ...
      for (String key : infosByRootDomain.keySet()) {
        ...
        if (domains.size() > 100) {
          LOG.info(
              "Too many domains for root domain " + key + ", splitting them up by site instead.");
  ...
  /**
   * Runs the certificate configuration update for a given set of sites and workplace URLS.
   *
   * <p>
   *
   * @param report the report to write to
   ...
   */
  private boolean run(
      I_CmsReport report, Collection<CmsSite> sites, Collection<String> workplaceUrls) {
    ...
    try {
      ...
      if (domainGrouping.isEmpty()) {
        report.println(
            org.opencms.ui.apps.Messages.get()
                .container(org.opencms.ui.apps.Messages.RPT_LETSENCRYPT_NO_DOMAINS_0));
        return false;
      }
      ...
      if (!m_configUpdater.update(certConfig)) {
        report.println(
            org.opencms.ui.apps.Messages.get()
                .container(org.opencms.ui.apps.Messages.RPT_LETSENCRYPT_UPDATE_FAILED_0),
