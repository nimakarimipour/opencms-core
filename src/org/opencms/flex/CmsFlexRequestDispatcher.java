 ...
 * JSPs to be loaded from the OpenCms VFS.
 *
 * <p>This dispatcher will load data from 3 different data sources:
 *
 * <ol>
...
public class CmsFlexRequestDispatcher implements RequestDispatcher {
   ...
   *
   * <p>Forward calls are actually NOT wrapped by OpenCms as of now. So they should not be used in
   ...
   *
   * <p>This method will dispatch to cache, to real file system or to the OpenCms VFS, whatever is
   * needed.
   *
   * <p>This method is much more complex than it should be because of the internal standard
   * buffering of JSP pages. Because of that I can not just intercept and buffer the stream, since I
   * don't have access to it (it is wrapped internally in the JSP pages, which have their own
   * buffer). That leads to a solution where the data is first written to the buffered stream, but
   * without includes. Then it is parsed again later in the <code>{@link CmsFlexResponse}</code>,
   * enriched with the included elements that have been omitted in the first case. I would love to
   * see a simpler solution, but this works for now.
   *
   * <p>
   *
   * @param req the servlet request
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(
                  Messages.LOG_FLEXREQUESTDISPATCHER_INCLUDING_TARGET_2, m_vfsTarget, m_extTarget));
    }
    ...
    if ((m_extTarget == null) && (controller != null)) {
      ...
      try {
        ...
        throw new ServletException(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_FLEXREQUESTDISPATCHER_VFS_ACCESS_EXCEPTION_0),
            e);
      }
    }
  ...
  /**
   * Include an external (non-OpenCms) file using the standard dispatcher.
   *
   * <p>
   *
   * @param req the servlet request
   ...
   * @throws IOException in case something goes wrong
   */
  private void includeExternal(ServletRequest req, ServletResponse res)
      throws ServletException, IOException {
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXREQUESTDISPATCHER_INCLUDING_EXTERNAL_TARGET_1, m_extTarget));
    }
  ...
  private void includeInternalNoCache(
    ...
    try {
      ...
      throw new ServletException(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_FLEXREQUESTDISPATCHER_ERROR_READING_RESOURCE_1, m_vfsTarget),
          e);
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXREQUESTDISPATCHER_INCLUDE_RESOURCE_1, m_vfsTarget));
    }
    try {
    ...
    try {
      ...
      if (f_req.isCacheable()) {
        ...
        if (entry != null) {
          ...
          // cache is on and resource is not yet cached, so we need to read the cache key for the
          // response
          CmsFlexCacheKey res_key =
              cache.getKey(CmsFlexCacheKey.getKeyName(m_vfsTarget, w_req.isOnline()));
          if (res_key != null) {
            ...
            try {
              ...
              cacheProperty =
                  cms.readPropertyObject(resource, CmsPropertyDefinition.PROPERTY_CACHE, true)
                      .getValue();
      ...
      if (entry == null) {
        ...
        try {
          ...
          throw new ServletException(
              Messages.get()
                  .getBundle()
                  .key(Messages.ERR_FLEXREQUESTDISPATCHER_CLASSCAST_EXCEPTION_1, m_vfsTarget),
              e);
        } catch (CmsException e) {
          // file might not exist or no read permissions
          controller.setThrowable(e, m_vfsTarget);
        ...
        try {
          ...
          if (f_res.hasIncludeList()) {
            // to prevent include list and include result list indices going out of sync, add an
            // empty byte array
      ...
      if (f_res.hasIncludeList()) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_FLEXREQUESTDISPATCHER_RESULT_1, new String(result)));
        }
