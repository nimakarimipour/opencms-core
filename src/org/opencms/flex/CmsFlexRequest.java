 ...
 *
 * <p>This class wraps the standard HttpServletRequest so that it's output can be delivered to the
 * CmsFlexCache.
 *
 * <p>
 *
...
public class CmsFlexRequest extends HttpServletRequestWrapper {
  ...
  /**
   * A set of keys of parameters which should be stored in a cached include even if they were not
   * passed as additional parameters to the include call.
   */
  private Set<String> m_dynamicParameters = Sets.newHashSet();

  /**
   * Creates a new CmsFlexRequest wrapper which is most likely the "Top" request wrapper, i.e. the
   * wrapper that is constructed around the first "real" (not wrapped) request.
   *
   * <p>
   *
  ...
  public CmsFlexRequest(HttpServletRequest req, CmsFlexController controller) {
          ...
          OpenCms.fireCmsEvent(
              new CmsEvent(
                  ...
                  Collections.<String, Object>singletonMap(
                      "action", new Integer(CmsFlexCache.CLEAR_ENTRIES))));
          dorecompile = false;
          ...
          if (!(p_on || p_off)) {
            OpenCms.fireCmsEvent(
                new CmsEvent(
                    I_CmsEventListener.EVENT_FLEX_CACHE_CLEAR,
                    Collections.<String, Object>singletonMap(
                        "action", new Integer(CmsFlexCache.CLEAR_ALL))));
          } else {
            if (p_on) {
              OpenCms.fireCmsEvent(
                  new CmsEvent(
                      I_CmsEventListener.EVENT_FLEX_CACHE_CLEAR,
                      Collections.<String, Object>singletonMap(
                          "action", new Integer(CmsFlexCache.CLEAR_ONLINE_ALL))));
            }
            if (p_off) {
              OpenCms.fireCmsEvent(
                  new CmsEvent(
                      I_CmsEventListener.EVENT_FLEX_CACHE_CLEAR,
                      Collections.<String, Object>singletonMap(
                          "action", new Integer(CmsFlexCache.CLEAR_OFFLINE_ALL))));
            }
          }
            ...
            if (p_on) {
              OpenCms.fireCmsEvent(
                  new CmsEvent(
                      I_CmsEventListener.EVENT_FLEX_CACHE_CLEAR,
                      Collections.<String, Object>singletonMap(
                          "action", new Integer(CmsFlexCache.CLEAR_ONLINE_ENTRIES))));
            }
            if (p_off) {
              OpenCms.fireCmsEvent(
                  new CmsEvent(
                      I_CmsEventListener.EVENT_FLEX_CACHE_CLEAR,
                      Collections.<String, Object>singletonMap(
                          "action", new Integer(CmsFlexCache.CLEAR_OFFLINE_ENTRIES))));
            }
          }
        }
      }
    ...
    m_canCache =
        ((((m_isOnline && m_controller.getCmsCache().isEnabled())
                    || (!m_isOnline && m_controller.getCmsCache().cacheOffline()))
                && !nocachepara)
            || dorecompile);
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXREQUEST_CREATED_NEW_REQUEST_1, m_elementUri));
    }
  }

  /**
   * Constructs a new wrapper layer around an (already wrapped) CmsFlexRequest.
   *
   * <p>
   *
  ...
  CmsFlexRequest(HttpServletRequest req, CmsFlexController controller, String resource) {
    ...
    m_controller = controller;
    m_elementUri =
        CmsLinkManager.getAbsoluteUri(resource, m_controller.getCurrentRequest().getElementUri());
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXREQUEST_REUSING_FLEX_REQUEST_1, m_elementUri));
    }
  }

  /**
   * Adds the specified Map to the attributes of the request, added attributes will not overwrite
   * existing attributes in the request.
   *
   * <p>
   *
  ...
  /**
   * Adds the specified Map to the parameters of the request, added parameters will not overwrite
   * existing parameters in the request.
   *
   * <p>Remember that the value for a parameter name in a HttpRequest is a String array. If a
   * parameter name already exists in the HttpRequest, the values will be added to the existing
   * value array. Multiple occurrences of the same value for one parameter are also possible.
   *
   * <p>
   *
   * @param map the map to add
  ...
  /**
   * Return the value of the specified request attribute, if any; otherwise, return <code>null
   * </code>.
   *
   * <p>
   *
  ...
  /**
   * Returns a <code>Map</code> of the attributes of this request.
   *
   * <p>
   *
   * @return a <code>Map</code> containing attribute names as keys and attribute values as map
   *     values
   */
   ...
   *
   * <p>Normally, when caching a JSP which includes another JSP, only the parameters given directly
   * to the include call will be cached in the new flex cache entry's include list. But when the
   * include call happens implicitly (e.g. for elements rendered in a cms:container tag), we can't
   * pass it any parameters. In this case, we have to modify the parameter map of the current flex
   * request in the JSP, and the keys for the modified parameters need to be stored in this set for
   * the Flex cache to work correctly.
   *
  ...
  public String getElementRootPath() {
    ...
    return m_controller
        .getCmsObject()
        .getRequestContext()
        .addSiteRoot(m_elementUriSiteRoot, m_elementUri);
  }

  /**
   * Returns the element URI of the resource currently processed, relative to the current site root.
   *
   * <p>This might be the name of an included resource, not necessarily the name the resource
  ...
  /**
   * Return the value of the specified request parameter, if any; otherwise, return <code>null
   * </code>.
   *
   * <p>If there is more than one value defined, return only the first one.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @return a <code>Map</code> containing parameter names as keys and parameter values as map
   *     values
  ...
  /**
   * Returns the defined values for the specified request parameter, if any; otherwise, return
   * <code>null</code>.
   *
   * <p>
   *
   * @param name Name of the desired request parameter
   * @return the defined values for the specified request parameter, if any; <code>null</code>
   *     otherwise
  ...
  /**
   * Allows requests to be dispatched to internal VFS resources or external JSP pages, overloads the
   * standard servlet API <code>getRequestDispatcher()</code> method.
   *
   * <p>
   *
  ...
  public javax.servlet.RequestDispatcher getRequestDispatcher(String target) {
    ...
    return new CmsFlexRequestDispatcher(
        m_controller.getTopRequest().getRequestDispatcher(absolutUri), absolutUri, null);
  }

  /**
   * Replacement for the standard servlet API getRequestDispatcher() method.
   *
   * <p>This variation is used if an external file (probably JSP) is dispatched to. This external
   * file must have a "mirror" version, i.e. a file in the OpenCms VFS that represents the external
   * file.
   *
   * <p>
   *
  ...
  public CmsFlexRequestDispatcher getRequestDispatcherToExternal(
    ...
    return new CmsFlexRequestDispatcher(
        ...
        CmsLinkManager.getAbsoluteUri(
            vfs_target, m_controller.getCmsObject().getRequestContext().getUri()),
        ext_target);
  }
   ...
   *
   * <p>This ensures that any wrapped request will use the "faked" target parameters. Remember that
   * for the real request, a mixture of PathInfo and other request information is used to identify
   * the target.
   *
   * <p>
   *
  ...
  /**
   * Wraps the request URL, overloading the standard API, the wrapped URL will always point to the
   * currently included VFS resource.
   *
   * <p>
   *
  ...
  /**
   * This is a work around for servlet containers creating a new application dispatcher instead of
   * using our request dispatcher, so missing RFS JSP pages are not requested to OpenCms and the
   * dispatcher is unable to load the included/forwarded JSP file.
   *
   * <p>
   *
   ...
   *
   * <p>This is useful in case directive based includes are used with &lt;%@ include file="..."
   * %&gt; on a JSP. Note that this also forces the request not to be cached.
   *
   * <p>
   *
   ...
   *
   * <p>This is required to distinguish between online and offline resources in the cache. Since the
   * resources have the same name, a suffix [online] or [offline] is added to distinguish the
   * strings when building cache keys. Any resource from a request that isOnline() will be saved
   * with the [online] suffix and vice versa.
   *
   * <p>Resources in the OpenCms workplace are not distinguished between online and offline but have
   * their own suffix [workplace]. The assumption is that if you do change the workplace, this is
   * only on true development machines so you can do the cache clearing manually if required.
   *
   * <p>The suffixes are used so that we have a simple String name for the resources in the cache.
   * This makes it easy to use a standard HashMap for storage of the resources.
   *
   * <p>
   *
   ...
   *
   * <p>The map should be immutable. This will completely replace the attribute map. Use this in
   * combination with {@link #getAttributeMap()} and {@link #addAttributeMap(Map)} in case you want
   * to set the old status of the attribute map after you have modified it for a specific operation.
   *
   * <p>
   *
   ...
   *
   * <p>The map should be immutable. This will completely replace the parameter map. Use this in
   * combination with {@link #getParameterMap()} and {@link #addParameterMap(Map)} in case you want
   * to set the old status of the parameter map after you have modified it for a specific operation.
   *
   * <p>
   *
   * @param map the map to set
   ...
   *
   * <p>The set of include calls is maintained to detect an endless inclusion loop.
   *
   * <p>
   *
  ...
  protected CmsJspLoader getJspLoader() {
    ...
    if (m_jspLoader == null) {
      try {
        m_jspLoader =
            (CmsJspLoader) OpenCms.getResourceManager().getLoader(CmsJspLoader.RESOURCE_LOADER_ID);
  ...
  /**
   * Returns the CmsFlexCacheKey for this request, the key will be calculated if necessary.
   *
   * <p>
   *
   ...
   *
   * <p>Using the request to decide if caching is used or not makes it possible to set caching to
   * false e.g. on a per-user or per-project basis.
   *
   * <p>
   *
