...
public class CmsFlexController {
  ...
  /**
   * Returns the wrapped CmsObject form the provided request, or <code>null</code> if the request is
   * not running inside OpenCms.
   *
   * <p>
   *
   * @param req the current request
  ...
  /**
   * Returns the controller from the given request, or <code>null</code> if the request is not
   * running inside OpenCms.
   *
   * <p>
   *
   * @param req the request to get the controller from
   * @return the controller from the given request, or <code>null</code> if the request is not
   *     running inside OpenCms
   */
  ...
  /**
   * Provides access to a root cause Exception that might have occurred in a complex include
   * scenario.
   *
   * <p>
   *
  ...
  /**
   * Provides access to URI of a VFS resource that caused an exception that might have occurred in a
  ...
  /**
   * Checks if the provided request is running in OpenCms and the current users project is the
   * online project.
   *
   * <p>
   *
  ...
  /**
   * Checks if the request has the "If-Modified-Since" header set, and if so, if the header date
   * value is equal to the provided last modification date.
   *
   * <p>
   *
   ...
   * @return <code>true</code> if the header is set and the header date is equal to the provided
   *     date
   */
  public static boolean isNotModifiedSince(HttpServletRequest req, long dateLastModified) {
    ...
    try {
      ...
      return ((lastModifiedHeader > -1)
          && (((dateLastModified / 1000) * 1000) == lastModifiedHeader));
    } catch (Exception ex) {
      // some clients (e.g. User-Agent: BlackBerry7290/4.1.0 Profile/MIDP-2.0 Configuration/CLDC-1.1
      // VendorID/111)
  ...
  /**
   * Tells the flex controller to never cache the given attribute.
   *
   * <p>
   *
  ...
  /**
   * Sets the <code>Expires</code> date header for a given http request.
   *
   * <p>Also sets the <code>cache-control: max-age</code> header to the time of the expiration. A
   * certain upper limit is imposed on the expiration date parameter to ensure the resources are not
   * cached to long in proxies. This can be controlled by the <code>maxAge</code> parameter. If
   * <code>maxAge</code> is lower then 0, then a default max age of 86400000 msec (1 day) is used.
   *
   * <p>
   *
  ...
  public static void setDateExpiresHeader(HttpServletResponse res, long dateExpires, long maxAge) {
    ...
    if ((dateExpires > now) && (dateExpires != CmsResource.DATE_EXPIRED_DEFAULT)) {
      ...
      // setting the "Expires" header only is not sufficient - even expired documents seems to be
      // cached
      // therefore, the "cache-control: max-age" is also set
      res.setHeader(
          CmsRequestUtil.HEADER_CACHE_CONTROL,
   ...
   *
   * <p>This is public so that pages like the Flex Cache Administration page have a way to access
   * the cache object.
   *
   * <p>
   *
  ...
  /**
   * This method provides access to the top-level CmsResource of the request which is of a type that
   * supports the FlexCache, i.e. usually the CmsFile that is identical to the file uri requested by
   * the user, not he current included element.
   *
   * <p>
   *
  ...
  /**
   * Returns an exception (Throwable) that was caught during inclusion of sub elements, or null if
   * no exceptions where thrown in sub elements.
   *
   * <p>
   *
  ...
  /**
   * Returns the URI of a VFS resource that caused the exception that was caught during inclusion of
   * sub elements, might return null if no URI information was available for the exception.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the controller does not yet contain any requests.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this controller is currently in "forward" mode.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the generated output of the response should be written to the
   * stream directly.
   *
   * <p>
   *
   * @return <code>true</code> if the generated output of the response should be written to the
   *     stream directly
   */
  ...
  /**
   * Returns <code>true</code> if this controller was generated as top level controller.
   *
   * <p>If a resource (e.g. a JSP) is processed and it's content is included in another resource,
   * then this will be <code>false</code>.
   *
   * @return <code>true</code> if this controller was generated as top level controller
   * @see org.opencms.loader.I_CmsResourceLoader#dump(CmsObject, CmsResource, String,
   *     java.util.Locale, HttpServletRequest, HttpServletResponse)
  ...
  public void pop() {
    ...
    if ((m_flexContextInfoList != null) && !m_flexContextInfoList.isEmpty()) {
      CmsFlexRequestContextInfo info =
          m_flexContextInfoList.remove(m_flexContextInfoList.size() - 1);
  ...
  /**
   * Removes request attributes which shouldn't be cached in flex cache entries from a map.
   *
   * <p>
   *
   ...
   *
   * <p>If another exception is already set in this controller, then the additional exception is
   * ignored.
   *
   * <p>
   *
   * @param throwable the exception (Throwable) to set
   * @param resource the URI of the VFS resource the error occurred on (might be <code>null</code>
   *     if unknown)
      ...
      if (LOG.isDebugEnabled()) {
        if (resource != null) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_FLEXCONTROLLER_IGNORED_EXCEPTION_1, resource));
        } else {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_FLEXCONTROLLER_IGNORED_EXCEPTION_0));
        }
      }
    }
    return m_throwable;
  ...
  /**
   * Updates the "last modified" date and the "expires" date for all resources read during this
   * request with the given values.
   *
   * <p>The currently stored value for "last modified" is only updated with the new value if the new
   * value is either larger (i.e. newer) then the stored value, or if the new value is less then
   * zero, which indicates that the "last modified" optimization can not be used because the element
   * is dynamic.
   *
   * <p>The stored "expires" value is only updated if the new value is smaller then the stored
   * value.
   *
   * <p>
   *
  ...
  /**
   * Updates the context info of the request context.
   *
   * <p>
   */
