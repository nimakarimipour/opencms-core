...
public class CmsFlexRequestKey {
  ...
  /**
   * The bean storing the paths which should be used to determine the flex cache buckets for this
   * entry.
   */
   ...
   *
   * <p>The request contains several data items that are neccessary to construct the output. These
   * items are e.g. the Query-String, the requested resource, the current time etc. etc. All
   * required items are saved in the constructed cache - key.
   *
   * <p>
   *
  ...
  public CmsFlexRequestKey(HttpServletRequest req, String target, boolean online) {
    ...
    TemplateBean templateBean =
        (TemplateBean) (req.getAttribute(CmsTemplateContextManager.ATTR_TEMPLATE_BEAN));
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXREQUESTKEY_CREATED_NEW_KEY_1, m_resource));
    }
  }
  ...
  /**
   * Gets the bean containing the paths which should be used to determine the flex cache buckets for
   * this entry.
   *
   * <p>
   *
  ...
  /**
   * Returns the the current users session, or <code>null</code> if the current user has no session.
   *
   * <p>
   *
