 ...
 *
 * <p>The data structure used is a two-level hashtable. This is optimized for the structure of the
 * keys that are used to describe the caching behaviour of the entries. The first hash-level is
 * calculated from the resource name, i.e. the name of the resource as it is referred to in the VFS
 * of OpenCms. The second hash-level is calculated from the cache-key of the resource, which also is
 * a String representing the specifc variation of the cached entry.
 *
 * <p>A suffix [online] or [offline] is appended to te resource name to distinguish between the
 * online and offline projects of OpenCms. Also, for support of JSP based workplace pages, a suffix
 * [workplace] is appended. The same cached workplace pages are used both in the online and all
 * offline projects.
 *
 * <p>Entries in the first level of the cache are of type CmsFlexCacheVariation, which is a
 * sub-class of CmsFlexCache. This class is a simple data type that contains of a Map of
 * CmsFlexCacheEntries, with variations - Strings as keys.
 *
 * <p>Here's a short summary of used terms:
 *
 * <ul>
 *   <li><b>key:</b> A combination of a resource name and a variation. The data structure used is
 *       CmsFlexCacheKey.
 *   <li><b>resource:</b> A String with the resource name and an appended [online] of [offline]
 *       suffix.
 *   <li><b>variation:</b> A String describing a variation of a cached entry in the CmsFlexCache
 *       language.
 *   <li><b>entry:</b> A CmsFlexCacheEntry data structure which is describes a cached OpenCms
 *       resource. For every entry a key is saved which contains the resource name and the
 *       variation.
 * </ul>
 *
 * Cache clearing is handled using events. The cache is fully flushed if an event {@link
 * I_CmsEventListener#EVENT_PUBLISH_PROJECT} or {@link I_CmsEventListener#EVENT_CLEAR_CACHES} is
 * caught.
 *
 * <p>
 *
...
public class CmsFlexCache extends Object implements I_CmsEventListener {
  ...
  /**
   * Extended LRUMap that handles the variations in case a key is removed.
   *
   * <p>
   */
  class CmsFlexKeyMap extends LRUMap {
    ...
    /**
     * Ensures that all variations that referenced by this key are released if the key is released.
     *
     * <p>
     *
    ...
    protected boolean removeLRU(LinkEntry entry) {
      ...
      // make a copy to safely iterate over because the line "m_variationCache.remove(e)" modifies
      // the variation map for the key
   ...
   *
   * <p>The parameter "enabled" is used to control if the cache is actually on or off. Even if you
   * don't need the cache, you still have to create an instance of it with enabled=false. This is
   * because you need some of the FlexCache data structures for JSP inclusion buffering.
   *
   * <p>
   *
  ...
  public CmsFlexCache(CmsFlexCacheConfiguration configuration) {
    ...
    OpenCms.getMemoryMonitor()
        .register(getClass().getName() + ".m_entryLruCache", m_variationCache);
  ...
  /**
   * Copies the key set of a map while synchronizing on the map.
   *
   * <p>
   *
  ...
  /**
   * Copies a map while synchronizing on it.
   *
   * <p>
   *
  ...
  /**
   * Implements the CmsEvent interface, the FlexCache uses the events to clear itself in case a
  ...
  public void cmsEvent(org.opencms.main.CmsEvent event) {
    ...
    switch (event.getType()) {
        ...
        if (!CmsUUID.isValidUUID(publishIdStr)) {
          ...
          try {
            ...
            List<CmsPublishedResource> publishedResources =
                m_cmsObject.readPublishedResources(publishId);
            ...
            if (updateConfiguration) {
              ...
              try {
                m_bucketConfiguration =
                    CmsFlexBucketConfiguration.loadFromVfsFile(m_cmsObject, CONFIG_PATH);
              ...
              boolean bucketClearOk =
                  clearBucketsForPublishList(bucketConfig, publishId, publishedResources);
              if (!bucketClearOk) {
                clear();
              }
            } else {
      ...
      case I_CmsEventListener.EVENT_CLEAR_CACHES:
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_FLEXCACHE_RECEIVED_EVENT_CLEAR_CACHE_0));
        }
        clear();
        break;
      case I_CmsEventListener.EVENT_FLEX_PURGE_JSP_REPOSITORY:
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_FLEXCACHE_RECEIVED_EVENT_PURGE_REPOSITORY_0));
        }
        purgeJspRepository();
        break;
      case I_CmsEventListener.EVENT_FLEX_CACHE_CLEAR:
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_FLEXCACHE_RECEIVED_EVENT_CLEAR_CACHE_PARTIALLY_0));
        }
  ...
  public void dumpKeys(StringBuffer buffer) {
    ...
    synchronized (this) {
      for (Map.Entry<String, CmsFlexCacheVariation> entry :
          synchronizedCopyMap(m_keyCache).entrySet()) {
   ...
   *
   * <p>Useful if you want to show the cache key for a resources, like on the FlexCache
   * administration page.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
   * <p>
   *
   ...
   *
   * <p>Useful if you want to show a list of all cached resources, like on the FlexCache
   * administration page.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
  ...
  /**
   * Returns all variations in the cache for a given resource name. The variations are of type
   * String.
   *
   * <p>Useful if you want to show a list of all cached entry - variations, like on the FlexCache
   * administration page.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
   ...
   *
   * <p>In case a found entry has a timeout set, it will be checked upon lookup. In case the timeout
   * of the entry has been reached, it will be removed from the cache (and null will be returned in
   * this case).
   *
   * <p>
   *
   ...
   *
   * <p>The key describes the conditions under which the value can be cached. Usually the key
   * belongs to the response. The variation describes the conditions under which the entry was
   * created. This is usually calculated from the request. If the variation is != null, the entry is
   * cachable.
   *
   * <p>
   *
  ...
  boolean put(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get().getBundle().key(Messages.LOG_FLEXCACHE_ADD_ENTRY_1, key.getResource()));
    }
    if (variation != null) {
      ...
      if (m_bucketConfiguration != null) {
        try {
          ...
          if (paths.size() > 0) {
            ...
            entry.setBucketSet(
                null); // bucket set of null means entries will be deleted for every publish job
          }
        } catch (Exception e) {
          LOG.error(e.getLocalizedMessage(), e);
        }
  ...
  void putKey(CmsFlexCacheKey key) {
    ...
    if (o == null) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_FLEXCACHE_ADD_KEY_1, key.getResource()));
      }
    }
   ...
   *
   * <p>It clears "one half" of the cache, i.e. either the online or the offline parts. A parameter
   * is used to indicate if only the entries or keys and entries are to be cleared.
   *
   * <p>
   *
  ...
  private synchronized void clearAccordingToSuffix(@RUntainted String suffix, @RUntainted boolean entriesOnly) {
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXCACHE_CLEAR_HALF_2, suffix, Boolean.valueOf(entriesOnly)));
    }
  }

  /**
   * Clears the Flex cache buckets matching the given publish list.
   *
   * <p>
   *
   * @param bucketConfig the bucket configuration to be used for checking which flex cache entry
   *     should be purged
   * @param publishId the publish id
   * @param publishedResources the published resources
   * @return true if the flex buckets could be cleared successfully (if this returns false, the flex
   *     cache should fall back to the old behavior, i.e. clearing everything)
   */
  private boolean clearBucketsForPublishList(
    ...
    try {
      ...
      if (bucketConfig.shouldClearAll(publishedResources)) {
        ...
        synchronized (this) {
          ...
          for (Map.Entry<String, CmsFlexCacheVariation> entry :
            ...
            for (Map.Entry<String, I_CmsLruCacheObject> variationEntry :
                synchronizedCopyMap(variation.m_map).entrySet()) {
      ...
      LOG.error(
          p + "Exception while trying to selectively purge flex cache: " + e.getLocalizedMessage(),
          e);
      return false;
    }
  }
   ...
   *
   * <p>Cached resources from the online project are not touched.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
   * <p>
   ...
   *
   * <p>The keys from the offline projects are not cleared. Cached resources from the online project
   * are not touched.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
   ...
   *
   * <p>Cached resources from the offline projects are not touched.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
   * <p>
  ...
  private void clearOnline() {
    ...
    if (LOG.isInfoEnabled()) {
      LOG.info(
          Messages.get().getBundle().key(Messages.LOG_FLEXCACHE_CLEAR_ONLINE_KEYS_AND_ENTRIES_0));
    }
   ...
   *
   * <p>The keys from the online project are not cleared. Cached resources from the offline projects
   * are not touched.
   *
   * <p>Only users with administrator permissions are allowed to perform this operation.
   *
  ...
  /**
   * This method purges the JSP repository dirs, i.e. it deletes all JSP files that OpenCms has
   * written to the real FS.
   *
   * <p>Obviously this method must be used with caution. Purpose of this method is to allow a
   * complete purge of all JSP pages on a machine after a major update of JSP templates was made.
   *
   * <p>
   */
  private synchronized void purgeJspRepository() {

    CmsJspLoader cmsJspLoader =
        (CmsJspLoader) OpenCms.getResourceManager().getLoader(CmsJspLoader.RESOURCE_LOADER_ID);

    cmsJspLoader.triggerPurge(
        new Runnable() {
  ...
  private void put(CmsFlexCacheKey key, CmsFlexCacheEntry theCacheEntry, @RUntainted String variation) {
    ...
    if (LOG.isDebugEnabled()) {
      ...
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_FLEXCACHE_ADDED_ENTRY_1, theCacheEntry.toString()));
    }
  }
}
