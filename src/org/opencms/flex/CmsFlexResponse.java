 ...
 *
 * <p>This class wraps the standard HttpServletResponse so that it's output can be delivered to the
 * CmsFlexCache.
 *
 * <p>
 *
...
public class CmsFlexResponse extends HttpServletResponseWrapper {
   ...
   *
   * <p>This implementation writes to an internal buffer and optionally to another output stream at
   * the same time.
   *
   * <p>It should be fully transparent to the standard ServletOutputStream.
   *
   * <p>
   */
  private static class CmsServletOutputStream extends ServletOutputStream {
    ...
    /**
     * Constructor that must be used if the stream should write to a buffer and to another stream at
     * the same time.
     *
     * <p>
     *
    ...
    public void flush() throws IOException {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_FLEXRESPONSE_FLUSHED_1, m_servletStream));
      }
      if (m_servletStream != null) {
  ...
  /**
   * State bit indicating whether content type has been set, type may only be set once according to
   * spec.
   */
  ...
  /**
   * Constructor for the CmsFlexResponse, this variation one is usually used to wrap responses for
   * further include calls in OpenCms.
   *
   * <p>
   *
  ...
  public CmsFlexResponse(HttpServletResponse res, CmsFlexController controller) {
    ...
    m_parentWritesOnlyToBuffer =
        controller.getCurrentResponse().hasIncludeList() && !controller.isForwardMode();
   ...
   *
   * <p>Cookies must be set directly as a header, otherwise they might not be set in the super
   * class.
   *
   * <p>
   *
    ...
    if (m_writeOnlyToBuffer) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_FLEXRESPONSE_ADDING_HEADER_TO_HEADERS_2, name, value));
      }
    } else {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
   ...
   *
   * <p>Should be used only in inclusion-scenarios like the JSP cms:include tag processing.
   *
   * <p>
   *
  ...
  public void addToIncludeList(
      String target, Map<String, String[]> parameterMap, Map<String, Object> attributeMap) {
    ...
    CmsJspStandardContextBean bean =
        (CmsJspStandardContextBean) attributeMap.get(CmsJspStandardContextBean.ATTRIBUTE_NAME);
    if (bean != null) {
   ...
   *
   * <p>A suspended response must not write further output to any stream or process a cache entry
   * for itself.
   *
   * <p>Currently, a response is only suspended if it is redirected.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this response has been constructed for the top level element of
   * this request, <code>false</code> if it was constructed for an included sub-element.
   *
   * <p>
   *
   * @return <code>true</code> if this response has been constructed for the top level element of
   *     this request, <code>false</code> if it was constructed for an included sub-element.
   */
    ...
    if (!m_cachingRequired) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_FLEXRESPONSE_TOPRESPONSE_SENDREDIRECT_1, location));
      }
      ...
      try {
        // Checking for possible illegal characters (for example, XSS exploits) before sending the
        // redirect
        // The constructor is key here. That method will throw an URISyntaxException if the URL
        // format is not according to standards (e.g. contains illegal characters, like spaces, < or
        // >, etc).
        @SuppressWarnings("unused")
        URI validURI = new URI(location);
      } catch (URISyntaxException e) {
        // Deliberately NOT passing the original exception, since the URISyntaxException contains
        // the full path,
        // which may include the XSS attempt
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_FLEXRESPONSE_URI_SYNTAX_EXCEPTION_0), e);
  ...
  public void setContentType(String type) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get().getBundle().key(Messages.LOG_FLEXRESPONSE_SETTING_CONTENTTYPE_1, type));
    }
    ...
    if (m_writeOnlyToBuffer) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_FLEXRESPONSE_SETTING_HEADER_IN_HEADERS_2, name, value));
      }
    } else {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
   ...
   *
   * <p>This must be done before the first output is written. Buffering is needed to process
   * elements that can not be written directly to the output stream because their sub - elements
   * have to be processed separately. Which is so far true only for JSP pages.
   *
   * <p>If buffering is on, nothing is written to the output stream even if streaming for this
   * response is enabled.
   *
   * <p>
   *
  ...
  /**
   * Is used to check if the response has an include list, which indicates a) it is probably
   * processing a JSP element and b) it can never be streamed and always must be buffered.
   *
   * <p>
   *
   ...
   *
   * <p>In case the results were written only to the buffer until now, they are now re-written on
   * the output stream, with all included elements.
   *
   * <p>
   *
  ...
  CmsFlexCacheEntry processCacheEntry() throws IOException {
    ...
    if (m_cachingRequired) {
      // cache entry must only be calculated if it's actually needed (always true if we write only
      // to buffer)
   ...
   *
   * <p>Will always be set to <code>"true"</code> if setOnlyBuffering() is set to <code>"true"
   * </code>. Currently this is an optimization for non - JSP elements that are known not to be
   * cachable.
   *
   * <p>
   *
   ...
   *
   * <p>This is important in case a cache entry is constructed, since the cache entry must not
   * consist of output or headers of the included elements.
   *
   * <p>
   *
   * @param value the value to set
  ...
  /**
   * Sets the suspended status of the response, and also sets the suspend status of all responses
  ...
  /**
   * Writes some bytes to the current output stream, this method should be called from
   * CmsFlexCacheEntry.service() only.
   *
   * <p>
   *
  ...
  void writeToOutputStream(byte[] bytes, boolean useArray) throws IOException {
    ...
    if (m_writeOnlyToBuffer) {
      if (useArray) {
        ...
        // In this case the buffer will not write to the servlet stream, but to it's internal buffer
        // only
        m_out.write(bytes);
      }
    } else {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_FLEXRESPONSE_ERROR_WRITING_TO_OUTPUT_STREAM_0));
      }
  ...
  /**
   * Initializes the current responses output stream and the corresponding print writer.
   *
   * <p>
   *
  ...
  private void initStream() throws IOException {
    ...
    if (m_writer == null) {
      ...
      m_writer =
          new PrintWriter(new BufferedWriter(new OutputStreamWriter(m_out, m_encoding)), false);
    }
  }

  /**
   * This method is needed to process pages that can NOT be analyzed directly during delivering
   * (like JSP) because they write to their own buffer.
   *
   * <p>In this case, we don't actually write output of include calls to the stream. Where there are
   * include calls we write a <code>{@link #FLEX_CACHE_DELIMITER}</code> char on the stream to
   * indicate that at this point the output of the include must be placed later. The include targets
   * (resource names) are then saved in the m_includeList.
   *
   * <p>This method must be called after the complete page has been processed. It will contain the
   * output of the page only (no includes), with <code>{@link #FLEX_CACHE_DELIMITER}</code> chars
   * were the include calls should be placed. What we do here is analyze the output and cut it in
   * parts of <code>byte[]</code> arrays which then are saved in the resulting cache entry. For the
   * includes, we just save the name of the resource in the cache entry.
   *
   * <p>If caching is disabled this method is just not called.
   *
   * <p>
   */
    ...
    if (!hasIncludeList()) {
      ...
      while ((i < m_includeList.size()) && (pos < max)) {
        ...
        if ((pos < max) && (result[pos] == FLEX_CACHE_DELIMITER)) {
          ...
          m_cachedEntry.add(
              m_includeList.get(i), m_includeListParameters.get(i), m_includeListAttributes.get(i));
          i++;
        }
      }
      if (pos < max) {
      ...
      if (i >= m_includeList.size()) {
        ...
        m_includeListParameters =
            m_includeListParameters.subList(count, m_includeListParameters.size());
        m_includeListAttributes =
            m_includeListAttributes.subList(count, m_includeListAttributes.size());
      }
    }
  }
