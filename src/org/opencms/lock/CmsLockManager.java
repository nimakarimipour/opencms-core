 ...
 *
 * <p>The lock state depends on the path of the resource, and probably locked parent folders. The
 * result of a query to the lock manager are instances of CmsLock objects.
 *
 * <p>
 *
 * @since 6.0.0
...
public final class CmsLockManager {
  ...
  public void addResource(
      CmsDbContext dbc, CmsResource resource, CmsUser user, CmsProject project, CmsLockType type)
    ...
    if (!type.isSystem() && !type.isExclusive()) {
      ...
      throw new CmsLockException(
          Messages.get().container(Messages.ERR_INVALID_LOCK_TYPE_1, type.toString()));
    }
    ...
    if (currentLock.getType().isShallow() && !type.isShallow() && type.isExclusive()) {
      throw new CmsLockException(
          Messages.get()
              .container(Messages.ERR_LOCK_CANT_UPGRADE_SHALLOW_LOCK_1, currentLock.toString()));
    }
    // check lockability
   ...
   *
   * <p>In case no lock is set, the <code>null lock</code> which can be obtained by {@link
   * CmsLock#getNullLock()} is returned.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> getLockedResources(
    ...
    while (itLocks.hasNext()) {
      ...
      if (!matchesFilter && !filter.isSharedExclusive()) {
        // we don't need to read the resource if the filter didn't match and we don't need to look
        // at the siblings
        continue;
      }
      try {
        lockedResource =
            m_driverManager.readResource(dbc, lock.getResourceName(), CmsResourceFilter.ALL);
  ...
  /**
   * Returns all exclusive locked resources matching the given resource and filter, but uses a cache
   * for resource loookups.
   *
   * <p>
   *
   * @param dbc the database context
    ...
    while (itLocks.hasNext()) {
      ...
      if (lockedResource == null) {
        try {
          ...
          OpenCms.getMemoryMonitor().uncacheLock(lock.getResourceName());
          // we put a dummy resource object in the map so we won't need to read the nonexistent
          // resource again
          CmsResource dummy =
              new CmsResource(
                  null, null, "", 0, false, 0, null, null, 0, null, 0, null, 0, 0, 0, 0, 0, 0);
      ...
      if (filter.isSharedExclusive()
          && (lockedResource != null)
          && (lockedResource.getSiblingCount() > 1)) {
        Iterator<CmsResource> itSiblings = internalReadSiblings(dbc, lockedResource).iterator();
        while (itSiblings.hasNext()) {
          CmsResource sibling = itSiblings.next();
          CmsLock siblingLock = internalSiblingLock(lock, sibling.getRootPath());
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsLock> getLocks(CmsDbContext dbc, String resourceName, CmsLockFilter filter)
      throws CmsException {
  ...
  /**
   * Returns <code>true</code> if the given resource contains a resource that has a system lock.
   *
   * <p>This check is required for certain operations on folders.
   *
   * <p>
   *
   * @param dbc the database context
  ...
  public void moveResource(String source, @RUntainted String destination) {
    ...
    if (lock != null) {
      OpenCms.getMemoryMonitor().uncacheLock(lock.getResourceName());
      CmsLock newLock =
          new CmsLock(destination, lock.getUserId(), lock.getProject(), lock.getType());
  ...
  public void removeDeletedResource(CmsDbContext dbc, @RUntainted String resourceName) throws CmsException {
    ...
    try {
      m_driverManager
          .getVfsDriver(dbc)
          .readResource(dbc, dbc.currentProject().getUuid(), resourceName, false);
      throw new CmsLockException(
          Messages.get()
              .container(
   ...
   *
   * <p>Edition and system locks are removed.
   *
   * <p>
   *
   ...
   *
   * <p>The forceUnlock option should be used with caution.<br>
   ...
   * @param dbc the current database context
   * @param resource the resource
   * @param forceUnlock <code>true</code>, if a resource is forced to get unlocked (only edition
   *     locks), no matter by which user and in which project the resource is currently locked
   * @param removeSystemLock <code>true</code>, if you also want to remove system locks
   * @return the previous {@link CmsLock} object of the resource, or <code>
   *     {@link CmsLock#getNullLock()}</code> if the resource was unlocked
   * @throws CmsException if something goes wrong
   */
  public CmsLock removeResource(
      CmsDbContext dbc, CmsResource resource, boolean forceUnlock, boolean removeSystemLock)
      throws CmsException {
    ...
    if (!lock.isNullLock()) {
      ...
      if (!forceUnlock && (!lock.isOwnedInProjectBy(dbc.currentUser(), dbc.currentProject()))) {
        throw new CmsLockException(
            Messages.get()
                .container(Messages.ERR_RESOURCE_UNLOCK_1, dbc.removeSiteRoot(resourcename)));
      }
      ...
      if (!forceUnlock && lock.isInherited()) {
        throw new CmsLockException(
            Messages.get()
                .container(Messages.ERR_UNLOCK_LOCK_INHERITED_1, dbc.removeSiteRoot(resourcename)));
      }
    }
   ...
   *
   * <p>Only edition lock can be temporary, so no system locks are removed.
   *
   * <p>
   *
   ...
   *
   * @param resourceName the name of the resource
   * @return the lock of a parent folder, or {@link CmsLock#getNullLock()} if no parent folders are
   *     locked by a non system lock
   */
   ...
   *
   * <p>The result is a list of <code>{@link CmsResource}</code> objects. It does NOT contain the
   * resource itself, only the siblings of the resource.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   *     resource, excluding the specified resource itself
   * @throws CmsException if something goes wrong
   */
  private List<CmsResource> internalReadSiblings(CmsDbContext dbc, CmsResource resource)
      throws CmsException {
    ...
    List<CmsResource> siblings =
        m_driverManager
            .getVfsDriver(dbc)
            .readSiblings(dbc, dbc.currentProject().getUuid(), resource, true);
  ...
  private CmsLock internalSiblingLock(CmsLock exclusiveLock, @RUntainted String siblingName) {
    ...
    if ((lock == null) || !exclusiveLock.getEditionLock().isNullLock()) {
      ...
      if (lock == null) {
        lock =
            new CmsLock(siblingName, exclusiveLock.getUserId(), exclusiveLock.getProject(), type);
      } else {
        CmsLock editionLock =
            new CmsLock(siblingName, exclusiveLock.getUserId(), exclusiveLock.getProject(), type);
   ...
   * @param systemLocks <code>true</code> if only system locks should be removed, and <code>false
   *     </code> if only exclusive locks should be removed
