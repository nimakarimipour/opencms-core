...
public final class CmsLockUtil {
   ...
   */
  public static CmsLockActionRecord ensureLock(CmsObject cms, CmsResource resource)
      throws CmsException {
  ...
  /**
   * Utility method for locking and unlocking a set of resources conveniently with the try-with
   * syntax from Java 1.7.
   *
   * <p>This method locks a set of resources and returns a Closeable instance that will unlock the
   * locked resources when its close() method is called.
   *
   * @param cms the CMS context
   ...
   */
  public static AutoCloseable withLockedResources(
      final CmsObject cms, boolean shallow, CmsResource... resources) throws Exception {
          ...
          public void close() {
            ...
            for (Map.Entry<CmsResource, CmsLockActionRecord> entry : lockMap.entrySet()) {
              if (entry.getValue().getChange() == LockChange.locked) {
                ...
                try {
                  resourceToUnlock =
                      cms.readResource(entry.getKey().getStructureId(), CmsResourceFilter.ALL);
                } catch (CmsException e) {
                  LOG.error(e.getLocalizedMessage(), e);
                }
                try {
   ...
   * @throws Exception if something goes wrong
   */
  public static AutoCloseable withLockedResources(final CmsObject cms, CmsResource... resources)
      throws Exception {
