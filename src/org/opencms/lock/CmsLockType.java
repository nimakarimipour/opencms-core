...
public final class CmsLockType extends A_CmsModeIntEnumeration {
  ...
  /**
   * A lock that allows the user to edit the resource's structure record, it's resource record, and
   * its content record.
   *
   * <p>This lock is assigned to files that are locked via the context menu.
   */
  ...
  /**
   * A lock that allows the user to edit the resource's structure record only, but not it's resource
   * record nor content record.
   *
   * <p>This lock is assigned to files if a sibling of the resource record has already an exclusive
   * lock.
   */
   ...
   * record nor content record.
   *
   * <p>This lock is assigned to resources that already have a shared exclusive lock, and then
   * inherit a lock because one of it's parent folders gets locked.
   */
  public static final CmsLockType SHARED_INHERITED = new CmsLockType(1);

  /**
   * A temporary exclusive lock that allows the user to edit the resource's structure record, it's
   * resource record, and its content record.
   *
   * <p>This lock is identical to the {@link #EXCLUSIVE} lock, but it is automatically removed after
  ...
  /**
   * Creates a new lock type with the given name.
   *
   * <p>
   *
  ...
  public boolean isExclusive() {
    ...
    return (this == CmsLockType.EXCLUSIVE)
        || (this == CmsLockType.TEMPORARY)
        || (this == CmsLockType.SHALLOW);
  }
  ...
  /**
   * Returns <code>true</code> if this is an shared exclusive lock.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this is an shared inherited lock.
   *
   * <p>
   *
   ...
   * which can be obtained by {@link CmsLock#getNullLock()}.
   *
   * <p>
   *
