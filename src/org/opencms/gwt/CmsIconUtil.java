...
public final class CmsIconUtil implements I_CmsEventListener {
  ...
  static class CssBuilder {
    ...
    /**
     * Writes the CSS for a single icon rule to a buffer.
     *
     * <p>
     *
    ...
    /**
     * Helper method for appending the CSS for a single resource type to a buffer.
     *
     * <p>
     *
    ...
    /**
     * Converts an icon file name to a full icon URI.
     *
     * <p>
     *
  ...
  /**
   * Helper class for creating the text of the CSS rule for a single icon based on resource type and
   * file suffix.
   *
   * <p>
   */
  static class IconCssRuleBuilder {
    ...
    public void addSelectorForType(String type, boolean small) {
      ...
      String selector =
          String.format(
              template, CmsGwtConstants.TYPE_ICON_CLASS, getResourceTypeIconClass(type, small));
      m_selectors.add(selector);
    }
  ...
  /** Pseudo type icon. */
  public static final String ICON_MODEL_GROUP_BIG =
      CmsGwtConstants.TYPE_ICON_CLASS + " oc-icon-24-modelgroup_copy";

  /** Pseudo type icon. */
  public static final String ICON_MODEL_GROUP_COPY_BIG =
      CmsGwtConstants.TYPE_ICON_CLASS
          + " "
          + CmsExplorerTypeSettings.ICON_STYLE_MODEL_GROUP_COPY_BIG;

  /** Pseudo type icon. */
  public static final String ICON_MODEL_GROUP_COPY_SMALL =
      CmsGwtConstants.TYPE_ICON_CLASS
          + " "
          + CmsExplorerTypeSettings.ICON_STYLE_MODEL_GROUP_COPY_SMALL;

  /** Pseudo type icon. */
  public static final String ICON_NAV_LEVEL_BIG =
      CmsGwtConstants.TYPE_ICON_CLASS + " " + CmsExplorerTypeSettings.ICON_STYLE_NAV_LEVEL_BIG;

  /** Pseudo type icon. */
  public static final String ICON_NAV_LEVEL_SMALL =
      CmsGwtConstants.TYPE_ICON_CLASS + " " + CmsExplorerTypeSettings.ICON_STYLE_NAV_LEVEL_SMALL;

  /** The big resource not found icon name. */
  public static final String NOT_FOUND_ICON_BIG =
      CmsGwtConstants.TYPE_ICON_CLASS + " oc-icon-24-warning";

  /** The small resource not found icon name. */
  public static final String NOT_FOUND_ICON_SMALL =
      CmsGwtConstants.TYPE_ICON_CLASS + " oc-icon-16-warning";
  ...
  /**
   * Returns the resource type name used to display the resource icon. This may differ from the
   * actual resource type in case of navigation level folders and model groups.
   *
   * <p>
   *
  ...
  static String getResourceSubTypeIconClass(String resourceTypeName, String suffix, boolean small) {
    ...
    StringBuffer buffer =
        new StringBuffer(CmsGwtConstants.TYPE_ICON_CLASS)
            .append("_")
            .append(resourceTypeName.hashCode())
            .append("_")
            .append(suffix);
    if (small) {
   ...
   * @return the CSS class
   */
  private static String getFileTypeIconClass(
      String resourceTypeName, String fileName, boolean small) {
   ...
   *
   * <p>Use this the resource type and filename is known.
   *
   * <p>
   *
   * @param resourceTypeName the resource type name
   * @param fileName the filename
   * @param small if true, get the icon classes for the small icon, else for the biggest one
   *     available
   * @return the CSS classes
   */
  private static String getResourceIconClasses(
      String resourceTypeName, String fileName, boolean small) {

    StringBuffer sb = new StringBuffer(CmsGwtConstants.TYPE_ICON_CLASS);
    sb.append(" ")
        .append(getResourceTypeIconClass(resourceTypeName, small))
        .append(" ")
        .append(getFileTypeIconClass(resourceTypeName, fileName, small));
    return sb.toString();
  }

  /**
   * Rebuilds the icon CSS.
   *
   * <p>
   */
  ...
  /**
   * Registers the 'clear caches' event listener.
   *
   * <p>
   */
  private static synchronized void registerListener() {
    ...
    if (!m_listenerRegistered) {
      OpenCms.getEventManager()
          .addCmsEventListener(
              new CmsIconUtil(), new int[] {I_CmsEventListener.EVENT_CLEAR_CACHES});
