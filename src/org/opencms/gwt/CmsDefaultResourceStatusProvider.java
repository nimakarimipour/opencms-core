...
public class CmsDefaultResourceStatusProvider {
   ...
   * @param additionalIds the structure ids of additional resources to include with the relation
   *     targets
   * @param cancelIfChanged if this is true, this method will stop immediately if it finds a changed
   *     resource among the relation targets
  ...
  public static CmsRelationTargetListBean getContainerpageRelationTargets(
      CmsObject cms, CmsUUID source, List<CmsUUID> additionalIds, boolean cancelIfChanged)
      throws CmsException {
    ...
    List<CmsRelation> relations =
        cms.readRelations(CmsRelationFilter.relationsFromStructureId(source));
    for (CmsRelation relation : relations) {
      ...
      try {
        ...
        if (isContainerPage && (type instanceof CmsResourceTypeJsp)) {
          // ignore formatters for container pages, as the normal user probably doesn't want to deal
          // with them
          continue;
        }
        result.add(target);
    ...
    if (relations.size() > MAX_RELATIONS) {
      ...
      for (CmsRelation relation : relations) {
        ...
        try {
          ...
          I_CmsResourceType type = OpenCms.getResourceManager().getResourceType(target);
          if (isContainerPage && (type instanceof CmsResourceTypeJsp)) {
            // ignore formatters for container pages, as the normal user probably doesn't want to
            // deal with them
            continue;
          }
          result.add(target);
    ...
    if (bean != null) {
      String label =
          org.opencms.ade.containerpage.Messages.get()
              .getBundle(locale)
              .key(org.opencms.ade.containerpage.Messages.GUI_ADDINFO_FORMATTER_0);
      result.put(label, bean.getJspRootPath());
      String locationLabel =
          org.opencms.ade.containerpage.Messages.get()
              .getBundle(locale)
              .key(org.opencms.ade.containerpage.Messages.GUI_ADDINFO_FORMATTER_LOCATION_0);
      ...
      String keyLabel =
          org.opencms.ade.containerpage.Messages.get()
              .getBundle(locale)
              .key(org.opencms.ade.containerpage.Messages.GUI_ADDINFO_FORMATTER_KEY_0);
   ...
   * @param additionalStructureIds structure ids of additional resources to include with the
   *     relation targets
  ...
  public CmsResourceStatusBean getResourceStatus(
    ...
    if (!lock.isUnlocked()) {
      ...
      result.setLockState(
          org.opencms.workplace.list.Messages.get()
              .getBundle(locale)
              .key(org.opencms.workplace.list.Messages.GUI_EXPLORER_LIST_ACTION_UNLOCK_NAME_0));
    }
    ...
    List<CmsRelation> relations =
        cms.readRelations(CmsRelationFilter.relationsToStructureId(resource.getStructureId()));
    ...
    if (CmsResourceTypeXmlContainerPage.isContainerPage(resource)) {
      ...
      try {
        ...
        List<CmsRelation> parentRelations =
            cms.readRelations(CmsRelationFilter.relationsToStructureId(parent.getStructureId()));
    ...
    if (relations.size() > MAX_RELATIONS) {
      String errorMessage =
          Messages.get()
              ...
              .key(Messages.GUI_TOO_MANY_RELATIONS_1, "" + MAX_RELATIONS);
        ...
        try {
          CmsPermissionInfo permissionInfo =
              OpenCms.getADEManager()
                  .getPermissionInfo(cms, relationResource, resource.getRootPath());
          if (permissionInfo.hasViewPermission()) {
            CmsResourceStatusRelationBean relationBean =
                createRelationBean(cms, contentLocale, relationResource, permissionInfo);
            CmsSite site =
                OpenCms.getSiteManager().getSiteForRootPath(relationResource.getRootPath());
            if ((site != null)
                && !CmsStringUtil.isPrefixPath(
                    cms.getRequestContext().getSiteRoot(), relationResource.getRootPath())) {
              String siteTitle = site.getTitle();
              if (siteTitle == null) {
                siteTitle = site.getUrl();
              } else {
                siteTitle =
                    CmsWorkplace.substituteSiteTitleStatic(
                        siteTitle, OpenCms.getWorkplaceManager().getWorkplaceLocale(cms));
              }
              relationBean.setSiteRoot(site.getSiteRoot());
              result.getOtherSiteRelationSources().add(relationBean);
              relationBean
                  .getInfoBean()
                  .setTitle("[" + siteTitle + "] " + relationBean.getInfoBean().getTitle());
            } else {
    ...
    if (includeTargets) {
      try {
        result
            .getRelationTargets()
            .addAll(getTargets(cms, contentLocale, resource, additionalStructureIds));
          ...
          try {
            ...
            Optional<CmsResource> detailOnlyPage =
                CmsDetailOnlyContainerUtil.getDetailOnlyResource(
                    cms, realLocale.toString(), detailContent, resource);
    ...
    LinkedHashMap<CmsResourceStatusTabId, String> tabMap =
        new LinkedHashMap<CmsResourceStatusTabId, String>();
    ...
    } else if (OpenCms.getResourceManager().getResourceType(resource)
  ...
  public void sortRelations(CmsObject cms, CmsResourceStatusBean resStatus) {
    ...
    final List<CmsSite> sites =
        OpenCms.getSiteManager()
            .getAvailableSites(cms, false, false, cms.getRequestContext().getOuFqn());
    Collections.sort(
        resStatus.getOtherSiteRelationSources(),
        new Comparator<CmsResourceStatusRelationBean>() {
          ...
          public int compare(CmsResourceStatusRelationBean o1, CmsResourceStatusRelationBean o2) {
            ...
            return ComparisonChain.start()
                .compare(rank(o1), rank(o2))
                .compare(o1.getSitePath(), o2.getSitePath())
                .result();
          }
    ...
    // sort relation sources by path, make sure all resources within .detailContainer folders show
    // last
    Collections.sort(
        resStatus.getRelationSources(),
        new Comparator<CmsResourceStatusRelationBean>() {

          public int compare(
              CmsResourceStatusRelationBean arg0, CmsResourceStatusRelationBean arg1) {
  ...
  protected List<CmsResourceStatusRelationBean> getSiblings(
    ...
    for (CmsResource sibling : cms.readSiblings(resource, CmsResourceFilter.ALL)) {
      ...
      try {
        CmsPermissionInfo permissionInfo =
            OpenCms.getADEManager().getPermissionInfo(cms, sibling, resource.getRootPath());
        if (permissionInfo.hasViewPermission()) {
          CmsResourceStatusRelationBean relationBean =
              createRelationBean(cms, locale, sibling, permissionInfo);
  ...
  protected List<CmsResourceStatusRelationBean> getTargets(
    ...
    CmsRelationTargetListBean listBean =
        getContainerpageRelationTargetsLimited(
            cms, resource.getStructureId(), additionalStructureIds, false);
    List<CmsResourceStatusRelationBean> result = new ArrayList<CmsResourceStatusRelationBean>();
    for (CmsResource target : listBean.getResources()) {
      try {
        CmsPermissionInfo permissionInfo =
            OpenCms.getADEManager().getPermissionInfo(cms, target, resource.getRootPath());
        if (permissionInfo.hasViewPermission()) {
          CmsResourceStatusRelationBean relationBean =
              createRelationBean(cms, locale, target, permissionInfo);
          result.add(relationBean);
        }
      } catch (CmsException e) {
        LOG.error(e.getLocalizedMessage(), e);
  ...
  CmsResourceStatusRelationBean createRelationBean(
    ...
    if (!CmsStringUtil.isEmptyOrWhitespaceOnly(locale)) {
      ...
      CmsGallerySearchResult result =
          CmsGallerySearch.searchById(cms, relationResource.getStructureId(), realLocale);
      if (!CmsStringUtil.isEmptyOrWhitespaceOnly(result.getTitle())) {
    ...
    CmsResourceStatusRelationBean relationBean =
        new CmsResourceStatusRelationBean(
            sourceBean, link, relationResource.getStructureId(), permissionInfo);
  ...
  /**
   * Creates the additional infos from the context parameters.
   *
   * <p>
   *
    ...
    try {
      try {
        ...
        String label =
            org.opencms.ade.containerpage.Messages.get()
                .getBundle(locale)
                .key(org.opencms.ade.containerpage.Messages.GUI_ADDINFO_SCHEMA_0);
      ...
      // because in the former case the jsp id is sometimes incorrect (i.e. inconsistent with the
      // formatter configured in the settings)
