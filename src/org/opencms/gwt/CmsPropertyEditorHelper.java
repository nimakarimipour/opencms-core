...
public class CmsPropertyEditorHelper {
  ...
  /**
   * Structure id which should be used instead of the structure id in a property change set (can be
   * null).
   */
  ...
  public static void updateWysiwygConfig(
      Map<String, CmsXmlContentProperty> propertyConfig, CmsObject cms, CmsResource resource) {
    ...
    for (Map.Entry<String, CmsXmlContentProperty> entry : propertyConfig.entrySet()) {
      ...
      if (prop.getWidget().equals("wysiwyg")) {
        if (wysiwygConfig == null) {
          ...
          try {
            String filePath =
                OpenCms.getSystemInfo().getConfigFilePath(cms, "wysiwyg/property-widget");
            String configFromVfs =
                (String)
                            ...
                            new Transformer() {
                              ...
                              public Object transform(Object rootPath) {
                                ...
                                try {
                                  CmsFile file =
                                      cms.readFile(
                                          (String) rootPath, CmsResourceFilter.IGNORE_EXPIRATION);
  ...
  public CmsPropertiesBean loadPropertyData(CmsUUID id) throws CmsException {
    ...
    CmsADEConfigData config =
        OpenCms.getADEManager().lookupConfiguration(cms, resource.getRootPath());

    Map<String, CmsXmlContentProperty> defaultProperties =
        getDefaultProperties(Collections.singletonList(resource.getStructureId()))
            .get(resource.getStructureId());

    Map<String, CmsXmlContentProperty> mergedConfig =
        config.getPropertyConfiguration(defaultProperties);
  ...
  public void saveProperties(CmsPropertyChangeSet changes) throws CmsException {
    ...
    try {
      Map<String, CmsProperty> ownProps =
          getPropertiesByName(cms.readPropertyObjects(resource, false));
      ...
      for (CmsPropertyModification propMod : changes.getChanges()) {
        if (propMod.isFileNameProperty()) {
          ...
          if ((m_overrideStructureId == null)
              && !resource.getStructureId().equals(propMod.getId())) {
          ...
          resource =
              cms.readResource(resource.getStructureId(), CmsResourceFilter.IGNORE_EXPIRATION);
        } else {
      ...
      for (String propName : defaultProps) {
        CmsXmlContentProperty property =
            new CmsXmlContentProperty(
                propName, "string", "string", "", "", "", "", null, "", "", "false");
  ...
  /**
   * Returns whether the current user has write permissions, the resource is lockable or already
   * locked by the current user and is in the current project.
   *
   * <p>
   *
   * @param cms the cms context
