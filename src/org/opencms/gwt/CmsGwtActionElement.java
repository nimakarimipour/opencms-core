...
public class CmsGwtActionElement extends CmsJspActionElement {
  ...
  public static String createNoCacheScript(String moduleName, String moduleVersion) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(moduleVersion)) {
      result +=
          "?version=" + moduleVersion + "_" + OpenCms.getSystemInfo().getVersionNumber().hashCode();
    }
    result += "\"></script>";
    return result;
  }
  ...
  public static String exportCommon(CmsObject cms, CmsCoreData coreData) throws Exception {
    ...
    // print out the missing permutation message to be used from the nocache.js generated by custom
    // linker
    ...
    sb.append(
        wrapScript(
            ...
            Messages.get()
                ...
                .key(Messages.ERR_NO_PERMUTATION_AVAILABLE_0),
            "';\n"));

    String prefetchedData =
        exportDictionary(
            CmsCoreData.DICT_NAME, I_CmsCoreService.class.getMethod("prefetch"), coreData);
    sb.append(prefetchedData);
    //       sb.append(ClientMessages.get().export(wpLocale));
    sb.append("<style type=\"text/css\">\n @import url(\"")
        .append(iconCssLink(cms))
        .append("\");\n </style>\n");
   ...
   */
  public static String exportDictionary(String name, Method method, Object data)
  ...
  public static String getFontIconCssLink() {
    ...
    return CmsStringUtil.joinPaths(
        OpenCms.getSystemInfo().getContextPath(), "VAADIN/themes/opencms/opencmsFonts.css");
  }
  ...
  /**
   * Generates the link to the icon CSS JSP, and appends a "prefix" request parameter with the given
   * value.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param includeFontCss <code>true</code> to include the OpenCms font CSS, not necessary in case
   *     VAADIN theme is loaded also
   * @return the data
   * @throws Exception if something goes wrong
   */
  public String export(boolean includeFontCss) throws Exception {
    ...
    if (includeFontCss || !OpenCms.getWorkplaceAppManager().getWorkplaceCssUris().isEmpty()) {
      ...
      for (String cssURI : OpenCms.getWorkplaceAppManager().getWorkplaceCssUris()) {
        buffer
            .append("@import url(\"")
            .append(CmsWorkplace.getResourceUri(cssURI))
            .append("\");\n");
      }
      buffer.append("</style>\n");
    }
