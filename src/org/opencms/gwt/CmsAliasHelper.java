...
public class CmsAliasHelper {
  ...
  public String exportAliasesAsCsv(CmsObject cms) throws CmsException {
    ...
    for (CmsAlias alias : aliases) {
      try {
        ...
        csvWriter.writeNext(
            new String[] {
              alias.getAliasPath(), cms.getSitePath(resource), alias.getMode().toString()
            });
    ...
    List<CmsRewriteAlias> rewriteAliases =
        OpenCms.getAliasManager().getRewriteAliases(cms, siteRoot);
   ...
   * @throws CmsException if something goes wrong
   */
  protected Map<String, String> validateAliases(CmsUUID uuid, Map<String, String> aliasPaths)
      throws CmsException {
    ...
    for (String path : duplicatePaths) {
      errorMessagesByPath.put(
          path, Messages.get().getBundle(locale).key(Messages.ERR_ALIAS_DUPLICATE_PATH_0));
    }
    ...
    for (String path : seenPaths) {
      ...
      if (pathError != null) {
        ...
        if (m_cms.existsResource(path, CmsResourceFilter.ALL)) {
          errorMessagesByPath.put(
              path, Messages.get().getBundle(locale).key(Messages.ERR_ALIAS_IS_VFS_0));
        } else {
          List<CmsAlias> aliases =
              OpenCms.getAliasManager()
                  .getAliasesForPath(m_cms, m_cms.getRequestContext().getSiteRoot(), path);
          for (CmsAlias alias : aliases) {
            ...
            if (!otherStructureId.equals(uuid)) {
              try {
                ...
                errorMessagesByPath.put(
                    ...
                    Messages.get()
                        .getBundle(locale)
                        .key(Messages.ERR_ALIAS_ALREADY_USED_1, resource.getRootPath()));
                break;
                ...
                errorMessagesByPath.put(
                    ...
                    Messages.get()
                        .getBundle(locale)
                        .key(Messages.ERR_ALIAS_ALREADY_USED_UNKNOWN_0));
                break;
              }
            }
          }
