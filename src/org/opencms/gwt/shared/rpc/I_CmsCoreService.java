...
public interface I_CmsCoreService extends RemoteService {
   ...
   */
  String changePassword(String oldPassword, String newPassword, String newPasswordConfirm)
   ...
   * <p>
   *
   * @param fromCatPath the category path to start with, can be <code>null</code> or empty to use
   *     the root
   * @param includeSubCats if to include all categories, or first level child categories only
   * @param refVfsPath the reference path (site-relative path according to which the available
   *     category repositories are determined), can be <code>null</code> to only use the system
   *     repository
   * @return the resource categories
   * @throws CmsRpcException if something goes wrong
   */
  List<CmsCategoryTreeEntry> getCategories(
      String fromCatPath, boolean includeSubCats, String refVfsPath) throws CmsRpcException;
   ...
   *     category repositories are determined), can be <code>null</code> to only use the system
   *     repository
   * @param withRepositories flag, indicating if also the category repositories should be returned
   *     as category
   * @return the resource categories
   * @throws CmsRpcException if something goes wrong
   */
  List<CmsCategoryTreeEntry> getCategories(
      String fromCatPath, boolean includeSubCats, String refVfsPath, boolean withRepositories)
      throws CmsRpcException;
   ...
   * @throws CmsRpcException if something goes wrong
   */
  List<CmsContextMenuEntryBean> getContextMenuEntries(CmsUUID structureId, AdeContext context)
      throws CmsRpcException;
  ...
  /**
   * Returns a link for the OpenCms workplace that will reload the whole workplace, switch to the
   ...
   *
   * <p>If the resource does not exist yet, the closest existing ancestor folder will check if it is
   * lockable.
   *
   * <p>
   *
   ...
   * Locks the given resource with a temporary lock.
   *
   * <p>Locking will fail in case the requested resource has been changed since the given load time.
   *
   * <p>
   *
   * @param structureId the resource structure id
   ...
   * @throws CmsRpcException if something goes wrong
   */
  void saveUserSettings(Map<String, String> userSettings, Set<String> edited)
      throws CmsRpcException;
