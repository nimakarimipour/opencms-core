...
public interface I_CmsVfsServiceAsync {
  ...
  /**
   * Forces a resource to be unlocked. In case the given resource is a folder, all sub-resources are
   * also unlocked.
   *
   * <p>
   *
  ...
  /**
   * Gets a {@link CmsListInfoBean} for a given resource.
   *
   * <p>
   *
   ...
   */
  void getResourceHistory(
      CmsUUID structureId, AsyncCallback<CmsHistoryResourceCollection> resultCallback);
  ...
  void loadQuickLaunchItems(
      CmsQuickLaunchParams params, AsyncCallback<List<CmsQuickLaunchData>> resultCallback);
  ...
  void prepareEdit(
      CmsUUID currentPage,
      String fileNameWithMacros,
      AsyncCallback<CmsPrepareEditResponse> callback);
   ...
   */
  void saveProperties(
      CmsPropertyChangeSet changes, boolean updateIndex, AsyncCallback<Void> callback);
   ...
   * @param callback the asynchronous callback
   */
  void substituteLinkForRootPath(
      String currentSiteRoot, String rootPath, AsyncCallback<String> callback);
  ...
  /**
   * Undoes the changes to a given resource, i.e. restores its online content to its offline
   * version.
   *
   * <p>
   *
