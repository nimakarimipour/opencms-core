...
public class CmsClientLock implements IsSerializable {
  ...
  /**
   * The available lock types. Replace with {@link org.opencms.lock.CmsLockType} as soon that
   * fulfills the serializable convention.
   */
  public enum LockType {
    /**
     * A lock that allows the user to edit the resource's structure record, it's resource record,
     * and its content record.
     *
    ...
    /**
     * A lock that allows the user to edit the resource's structure record only, but not it's
     * resource record nor content record.
     *
     * <p>This lock is assigned to files if a sibling of the resource record has already an
     * exclusive lock.
     */
    SHARED_EXCLUSIVE(2),
     ...
     * inherit a lock because one if it's parent folders gets locked.
     */
    SHARED_INHERITED(1),

    /**
     * A temporary exclisive lock that allows the user to edit the resource's structure record, it's
     * resource record, and its content record.
     *
     * <p>This lock is identical to the {@link #EXCLUSIVE} lock, but it is automatically removed
     ...
     * which can be obtained by {@link org.opencms.lock.CmsLock#getNullLock()}.
     *
     * <p>
     *
