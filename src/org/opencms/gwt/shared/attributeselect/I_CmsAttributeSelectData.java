 ...
 *
 * <p>An attribute select widget consists of several attribute filter select boxes and one main
 * select box, such that choosing values from the attribute filters restricts the available options
 * in the main select box to those which have a matching value for every filter attribute.
 */
public interface I_CmsAttributeSelectData {
  ...
  interface AttributeDefinition {
    ...
    /**
     * Gets the neutral option, which is chosen when a pre-existing value is not among the current
     * options for the main select widget.
     *
    ...
    /**
     * Sets the neutral option (will be set when a pre-existing value is not among the current
     * options)
     *
