...
public final class CmsGwtConstants {
  ...
  /**
   * Name of the Javascript callback used to handle property changes triggered from the locale
   * compare view.
   */
  ...
  /**
   * CSS class for containers inflated due to an element dragged into them that is of lower height
   * than the empty container HTML.
   */
  public static final String CLASS_CONTAINER_INFLATED = "oc-container-inflated";

  /**
   * CSS class for elements in detail containers which are used to transfer their settings to the
   * detail eleemnt.
   */
  ...
  /**
   * Name for configuration setting to limit collector results while computing publish lists for
   * collectors.
   */
  ...
  /**
   * Special 'type' name (that isn't actually a type) used to mark default detail pages in the
  ...
  /**
   * A HTML comment that will cause the container page editor to reload the page if it is contained
   * in HTML which is reloaded after the element or its settings have been edited.
   */
  public static final String FORMATTER_RELOAD_MARKER = "<!--FORMATTER_RELOAD_g3jf9o0n-->";

  /**
   * Separator for separating the main part of a formatter key from the sub-key (the part in front
   * of the separator will be used as a key if there is no formatter for the full key).
   */
  ...
  /**
   * Name of Javascript variable used to hold the structure id of the currently selected locale's
   * root folder in the sitemap editor's locale comparison mode.
   */
