...
public class CmsCoreService extends CmsGwtService implements I_CmsCoreService {
  ...
  private static final String EDITOR_BACKLINK_URI =
      "/system/workplace/commons/editor-backlink.html";
   ...
   */
  public static List<CmsCategoryTreeEntry> buildCategoryTree(
      CmsObject cms, List<CmsCategory> categories) {
  ...
  public static List<CmsCategoryTreeEntry> getCategoriesForSitePathStatic(
      CmsObject cms, String sitePath) throws CmsException {
   ...
   */
  public static List<CmsCategoryTreeEntry> getCategoriesForSitePathStatic(
      CmsObject cms, String sitePath, String localCategoryRepositoryPath) throws CmsException {
    ...
    if (null == localCategoryRepositoryPath) {
      ...
      result = buildCategoryTree(cms, categories);
      categories =
          catService.readCategoriesForRepositories(
              cms, "", true, Collections.singletonList(localCategoryRepositoryPath));
      categories = catService.localizeCategories(cms, categories, wpLocale);
  ...
  public static List<CmsContextMenuEntryBean> getContextMenuEntries(
      final CmsObject cms, CmsUUID structureId, final AdeContext context) {

    Map<String, CmsContextMenuEntryBean> entries =
        new LinkedHashMap<String, CmsContextMenuEntryBean>();
    try {
      final List<CmsResource> resources;
      CmsResource resource =
          cms.readResource(structureId, CmsResourceFilter.ALL.addRequireVisible());
      // in case of sitemap editor check visibility with empty list
      if (context.equals(AdeContext.sitemapeditor)) {
        resources = Collections.emptyList();
        cms.getRequestContext()
            .setAttribute(I_CmsDialogContext.ATTR_SITEMAP_CONFIG_RESOURCE, resource);
      } else {
        resources = Collections.singletonList(resource);
      }
      Locale locale = OpenCms.getWorkplaceManager().getWorkplaceLocale(cms);
      // context to check item visibility
      I_CmsDialogContext dcontext =
          new I_CmsDialogContextWithAdeContext() {
      ...
      CmsTreeNode<I_CmsContextMenuItem> root =
          builder.buildAll(OpenCms.getWorkplaceAppManager().getMenuItemProvider().getMenuItems());
      for (CmsTreeNode<I_CmsContextMenuItem> child : root.getChildren()) {
        child.addDataInPreOrder(items);
      }
      Map<String, List<CmsContextMenuEntryBean>> submenus =
          new HashMap<String, List<CmsContextMenuEntryBean>>();
      for (I_CmsContextMenuItem item : items) {
        if (!item.isLeafItem()) {
              ...
              new CmsContextMenuEntryBean(
                  ...
                  CmsStringUtil.isEmptyOrWhitespaceOnly(visibility.getMessageKey())
                      ...
                      : OpenCms.getWorkplaceManager()
                          .getMessages(locale)
                          .getString(visibility.getMessageKey()),
                  false,
          ...
          CmsContextMenuEntryBean itemBean =
              new CmsContextMenuEntryBean(
                  visibility.isActive(),
                  true,
                  jspPath,
          ...
          if (params != null) {
            ...
            for (Entry<String, @RUntainted String> param : params.entrySet()) {
              @RUntainted String value =
                  CmsVfsService.prepareFileNameForEditor(cms, resource, param.getValue());
  ...
  public static I_CmsValidationService getValidationService(@RUntainted String name) throws CmsException {
    ...
    try {
      ...
      throw new CmsException(
          Messages.get().container(Messages.ERR_VALIDATOR_INSTANTIATION_FAILED_1, name), e);
   ...
   *
   * <p>Also checks whether the class with the given name is the subclass of another
   * class/interface.
   *
   * <p>
   *
   ...
   */
  public static <T> T instantiate(@RUntainted Class<T> anInterface, @RUntainted String className) throws CmsException {
    ...
    try {
      ...
      if (!anInterface.isAssignableFrom(cls)) {
        ...
        throw new CmsIllegalArgumentException(
            Messages.get()
                .container(
                    Messages.ERR_INSTANTIATION_INCORRECT_TYPE_2, className, anInterface.getName()));
      }
      ...
      throw new CmsException(
          Messages.get().container(Messages.ERR_INSTANTIATION_FAILED_1, className), e);
    } catch (InstantiationException e) {
      throw new CmsException(
          Messages.get().container(Messages.ERR_INSTANTIATION_FAILED_1, className), e);
    } catch (IllegalAccessException e) {
   ...
   * @throws CmsException if something goes wrong
   */
  public static CmsReturnLinkInfo internalGetLinkForReturnCode(CmsObject cms, @RUntainted String returnCode)
      throws CmsException {

    if (CmsUUID.isValidUUID(returnCode)) {
      try {
        CmsResource pageRes =
            cms.readResource(new CmsUUID(returnCode), CmsResourceFilter.IGNORE_EXPIRATION);
        return new CmsReturnLinkInfo(
            OpenCms.getLinkManager().substituteLink(cms, pageRes), CmsReturnLinkInfo.Status.ok);
      ...
      if (colonIndex >= 0) {
        @RUntainted String before = returnCode.substring(0, colonIndex);
        @RUntainted String after = returnCode.substring(colonIndex + 1);
        ...
        if (CmsUUID.isValidUUID(before) && CmsUUID.isValidUUID(after)) {
          try {
            ...
            CmsResource folder =
                pageRes.isFolder() ? pageRes : cms.readParentFolder(pageRes.getStructureId());
  ...
  /**
   * FInds a category in the given tree.
   *
   * <p>
   *
  ...
  /**
   * @see org.opencms.gwt.shared.rpc.I_CmsCoreService#changePassword(java.lang.String,
  ...
  public List<CmsBroadcastMessage> getBroadcast() {
    ...
    CmsSessionInfo sessionInfo =
        OpenCms.getSessionManager().getSessionInfo(getRequest().getSession());
    if (sessionInfo == null) {
      return null;
    }
    ...
    if (!messageQueue.isEmpty()) {
      ...
      while (!messageQueue.isEmpty()) {
        ...
        if ((broadcastMessage.getLastDisplay() + CmsBroadcast.DISPLAY_AGAIN_TIME)
          ...
          CmsBroadcastMessage message =
              new CmsBroadcastMessage(
  ...
  /**
   * @see org.opencms.gwt.shared.rpc.I_CmsCoreService#getCategories(java.lang.String, boolean,
   *     java.lang.String)
   */
  public List<CmsCategoryTreeEntry> getCategories(
      String fromPath, boolean includeSubCats, String refPath) throws CmsRpcException {
   ...
   */
  public List<CmsCategoryTreeEntry> getCategories(
      String fromPath, boolean includeSubCats, String refPath, boolean showWithRepositories)
      throws CmsRpcException {
    ...
    try {
      ...
      List<CmsCategory> categories =
          catService.readCategoriesForRepositories(
              cms, fromPath, includeSubCats, repositories, showWithRepositories);
      categories =
          catService.localizeCategories(
              cms, categories, OpenCms.getWorkplaceManager().getWorkplaceLocale(cms));
      result = buildCategoryTree(cms, categories);
    } catch (Throwable e) {
      error(e);
    }
  ...
  /**
   * @see
   *     org.opencms.gwt.shared.rpc.I_CmsCoreService#getContextMenuEntries(org.opencms.util.CmsUUID,
   *     org.opencms.gwt.shared.CmsCoreData.AdeContext)
   */
  public List<CmsContextMenuEntryBean> getContextMenuEntries(
      CmsUUID structureId, AdeContext context) throws CmsRpcException {
  ...
  /**
   * @see org.opencms.gwt.shared.rpc.I_CmsCoreService#getUniqueFileName(java.lang.String,
   *     java.lang.String)
   */
  public String getUniqueFileName(String parentFolder, @RUntainted String baseName) {
    ...
    return OpenCms.getResourceManager()
        .getNameGenerator()
        .getUniqueFileName(getCmsObject(), parentFolder, baseName);
  }
    ...
    UserInfo userInfo =
        new UserInfo(
            ...
            OpenCms.getWorkplaceAppManager()
                .getUserIconHelper()
                .getSmallIconPath(cms, cms.getRequestContext().getCurrentUser()),
            isAdmin,
            isDeveloper,
            isCategoryManager,
            isWorkplaceUser,
  ...
  public String getWorkplaceLinkForPath(@RUntainted String path) throws CmsRpcException {
    ...
    try {
      ...
      try {
        folder =
            workCms.readResource(
                currentPath, CmsResourceFilter.IGNORE_EXPIRATION.addRequireVisible());
      } catch (CmsVfsResourceNotFoundException | CmsSecurityException e) {
        throw new CmsException(
            Messages.get().container(Messages.ERR_COULD_NOT_FIND_PARENT_FOLDER_1, path), e);
      }
  ...
  public CmsUserSettingsBean loadUserSettings() throws CmsRpcException {
    ...
    CmsObject cms = getCmsObject();
    CmsClientUserSettingConverter converter =
        new CmsClientUserSettingConverter(cms, getRequest(), getResponse());
    try {
    ...
    try {
      if (cms.existsResource(sitePath, CmsResourceFilter.IGNORE_EXPIRATION)) {
        ...
        try {
          ...
          errorMessage =
              e.getLocalizedMessage(OpenCms.getWorkplaceManager().getWorkplaceLocale(cms));
        }
        ...
        if (parentFolder != null) {
          CmsResource ancestorFolder =
              cms.readResource(parentFolder, CmsResourceFilter.IGNORE_EXPIRATION);
      ...
      if (cms.existsResource(sitePath, CmsResourceFilter.IGNORE_EXPIRATION)) {
        ...
        try {
          ...
          if (resource.getDateLastModified() > loadTime) {
            ...
            CmsMessages messages =
                Messages.get().getBundle(OpenCms.getWorkplaceManager().getWorkplaceLocale(cms));
            return user != null
                ? messages.key(
  ...
  public CmsCoreData prefetch() {
    ...
    boolean toolbarVisible =
        CmsADESessionCache.getCache(getRequest(), getCmsObject()).isToolbarVisible();
    boolean isShowHelp = OpenCms.getADEManager().isShowEditorHelp(cms);

    CmsUUID structureId = null;

    try {
      CmsResource requestedResource =
          cms.readResource(
              cms.getRequestContext().getUri(), CmsResourceFilter.ignoreExpirationOffline(cms));
    ...
    try {
      loginUrl =
          cms.readPropertyObject(
                  cms.getRequestContext().getUri(), CmsPropertyDefinition.PROPERTY_LOGIN_FORM, true)
              .getValue(loginUrl);
    } catch (CmsException e) {
    ...
    String aboutLink =
        OpenCms.getLinkManager()
            .substituteLink(getCmsObject(), "/system/workplace/commons/about.jsp");
    String tinyMCE =
        CmsWorkplace.getStaticResourceUri("/editors/tinymce/jscripts/tinymce/tinymce.min.js");
    boolean uploadDisabled = !OpenCms.getRoleManager().hasRole(cms, CmsRole.EDITOR);
    CmsUploadRestrictionInfo uploadRestrictionInfo =
        OpenCms.getWorkplaceManager().getUploadRestriction().getUploadRestrictionInfo(cms);
    String categoryBaseFolder = CmsCategoryService.getInstance().getRepositoryBaseFolderName(cms);
    CmsGalleryDisabledTypesMode disabledTypesMode =
        sitemapConfig.getDisabledTypeMode(CmsGalleryDisabledTypesMode.mark);
   ...
   */
  public void saveUserSettings(Map<String, String> userSettings, Set<String> edited)
      throws CmsRpcException {
  ...
  /**
   * @see
   *     org.opencms.gwt.shared.rpc.I_CmsCoreService#setResourceCategories(org.opencms.util.CmsUUID,
   *     java.util.List)
   */
  public void setResourceCategories(CmsUUID structureId, List<String> categories)
      throws CmsRpcException {
  ...
  public Map<String, CmsValidationResult> validate(
      Map<String, CmsValidationQuery> validationQueries) throws CmsRpcException {
    ...
    try {
      ...
      for (Map.Entry<String, CmsValidationQuery> queryEntry : validationQueries.entrySet()) {
        ...
        result.put(
            fieldName, validate(query.getValidatorId(), query.getValue(), query.getConfig()));
      }
      return result;
    } catch (Throwable e) {
      error(e);
  ...
  /**
   * @see org.opencms.gwt.shared.rpc.I_CmsCoreService#validate(java.lang.String, java.util.Map,
   ...
   */
  private CmsValidationResult validate(String validator, @RUntainted String value, String config)
      throws Exception {
