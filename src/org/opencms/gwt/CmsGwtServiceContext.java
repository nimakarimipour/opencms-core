 ...
 *
 * <p>We cache instances of this class rather than caching instances of {@link CmsGwtService}
 * directly because its superclass, {@link com.google.gwt.user.server.rpc.RemoteServiceServlet},
 * does some caching which we can't use because it doesn't take the distinction between online and
 * offline requests into account.
 *
 * @since 8.0.0
 */
public class CmsGwtServiceContext implements I_CmsEventListener {
  ...
  protected SerializationPolicy getSerializationPolicy(
      CmsObject cms, String moduleBaseURL, String strongName) {
  ...
  /**
   * Returns the serialization policy, using lazy initialization.
   *
   * <p>
   *
   * @param cms the current cms context
  ...
  private SerializationPolicy getSerializationPolicy(CmsObject cms) {
    ...
    try {
      ...
      if (m_serializationPolicyPath.startsWith(
        ...
        rfsPath =
            CmsStringUtil.joinPaths(OpenCms.getSystemInfo().getWebApplicationRfsPath(), rfsPath);
  ...
  private void uncacheResource(CmsResource resource) {
    ...
    if ((m_serializationPolicyPath != null)
        && resource.getRootPath().equals(m_serializationPolicyPath)) {
      m_serPolicyOffline = null;
    }
  }
