...
public final class CmsQuickLaunchProvider {
   ...
   *
   * <p>The context is a string which identifies where the quick launch menu is located
   *
  ...
  public static List<CmsQuickLaunchData> getQuickLaunchData(
      CmsObject cms, HttpSession session, CmsQuickLaunchParams params) {

    List<CmsQuickLaunchData> result = Lists.newArrayList();
    Locale locale = OpenCms.getWorkplaceManager().getWorkplaceLocale(cms);
    List<I_CmsWorkplaceAppConfiguration> appConfigs =
    ...
    if (params.getPageId() != null) {
      try {
        currentPage =
            cms.readResource(params.getPageId(), CmsResourceFilter.ONLY_VISIBLE_NO_DELETED);
    ...
    for (I_CmsWorkplaceAppConfiguration config : appConfigs) {
      try {
        ...
        if (CmsFileExplorerConfiguration.APP_ID.equals(config.getId())) {
          String page =
              locationCache.getFileExplorerLocation(cms.getRequestContext().getSiteRoot());
          if (page != null) {
            link =
                CmsCoreService.getVaadinWorkplaceLink(
                    cms, cms.getRequestContext().addSiteRoot(page));
          } else {
            if (cms.existsResource("/", CmsResourceFilter.ONLY_VISIBLE_NO_DELETED)) {
              link =
                  CmsCoreService.getVaadinWorkplaceLink(cms, cms.getRequestContext().getSiteRoot());
              ...
              errorMessage =
                  Messages.get()
                      .getBundle(locale)
                      .key(Messages.GUI_QUICKLAUNCH_EXPLORER_NOT_ALLOWED_0);
            }
          }
            ...
            if ((currentPage != null)
              ...
              if (params.getSessionPageId() != null) {
                try {
                  CmsResource page =
                      cms.readResource(
                          params.getSessionPageId(), CmsResourceFilter.IGNORE_EXPIRATION);
                  // only use the page id from session storage if it's in the current site.
                  if (CmsStringUtil.isPrefixPath(
                      cms.getRequestContext().getSiteRoot(), page.getRootPath())) {
              ...
              if (link == null) {
                String page =
                    locationCache.getPageEditorLocation(cms, cms.getRequestContext().getSiteRoot());
                if (page != null) {
                  link = OpenCms.getLinkManager().substituteLink(cms, page);
                } else {
                  reload = true;
          ...
          if (params.isSitemapContext()) {
            ...
            String sitemapLink =
                OpenCms.getLinkManager()
                    .substituteLinkForUnknownTarget(cms, CmsADEManager.PATH_SITEMAP_EDITOR_JSP);
            String page =
                locationCache.getPageEditorLocation(cms, cms.getRequestContext().getSiteRoot());
