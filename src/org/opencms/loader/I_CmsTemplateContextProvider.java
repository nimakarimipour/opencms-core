 ...
 *
 * <p>Implementations of this class are used to dynamically determine a template used for rendering
 * a page at runtime, e.g. there could be one template for desktop browsers and another for mobile
 * browsers.
 *
 * <p>It is possible to override the template using a cookie containing the name of the template
 * context which should be used as a value. The cookie name is determined by the template context
 * provider.
 *
 * <p>
 */
public interface I_CmsTemplateContextProvider {
   ...
   */
  CmsTemplateContext getTemplateContext(
  ...
  /**
   * Gets the value which should be used instead of a property which was read from the template
   * resource.
   *
   * <p>This is needed because before template context providers, it was common to store
   * template-specific configuration in a property on the template JSP. Since template context
   * providers make the result ambiguous, this method is intended as a replacement.
   *
   * <p>
   *
   ...
   */
  String readCommonProperty(CmsObject cms, String propertyName, @RUntainted String fallbackValue)
      throws CmsException;
}
