 ...
 *
 * <p>To scale or process an image, the parameter <code>
 * {@link org.opencms.loader.CmsImageScaler#PARAM_SCALE}</code> has to be appended to the image URI.
 * The value for the parameter needs to be composed from the <code>SCALE_PARAM</code> options
 * provided by the constants in the <code>{@link org.opencms.file.types.CmsResourceTypeImage}</code>
 * class.
 *
 * <p>For example, to scale an image to exact 800x600 pixel with center fitting and a background
 * color of grey, the following parameter String can be used: <code>w:800,h:600,t:0,c:c0c0c0</code>.
 *
 * <p>
 *
 * @since 6.2.0
...
public class CmsImageLoader extends CmsDumpLoader implements I_CmsEventListener {
  ...
  /**
   * The configuration parameter for the OpenCms XML configuration to set the image down scale
   * operation.
   */
  public static final String CONFIGURATION_DOWNSCALE = "image.scaling.downscale";

  /**
   * The configuration parameter for the OpenCms XML configuration to set the image cache
   * repository.
   */
  public static final String CONFIGURATION_IMAGE_FOLDER = "image.folder";

  /**
   * The configuration parameter for the OpenCms XML configuration to set the maximum image blur
   * size.
   */
  public static final String CONFIGURATION_MAX_BLUR_SIZE = "image.scaling.maxblursize";

  /**
   * The configuration parameter for the OpenCms XML configuration to set the maximum image scale
   * size.
   */
   ...
   * to high may cause "out of memory" errors).
   */
  ...
  /**
   * Returns the image down scale parameters, which is set with the {@link #CONFIGURATION_DOWNSCALE}
   * configuration option.
   *
   * <p>If no down scale parameters have been set in the configuration, this will return <code>null
   * </code>.
   *
  ...
  /**
   * Returns the path of the image cache repository folder in the RFS, which is set with the {@link
  ...
  /**
   * The maximum blur size for image re-scale operations, which is set with the {@link
   * #CONFIGURATION_MAX_BLUR_SIZE} configuration option.
   *
   * <p>The default is 2500 * 2500 pixel.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the image scaling and processing capabilities for the OpenCms VFS
   * images have been enabled, <code>false</code> if not.
   *
   * <p>Image scaling is enabled by setting the loader parameter <code>image.scaling.enabled</code>
   * to the value <code>true</code> in the configuration file <code>opencms-vfs.xml</code>.
   *
   * <p>Enabling image processing in OpenCms may require several additional configuration steps on
   * the server running OpenCms, especially in UNIX systems. Here it is often required to have an X
   * window server configured and accessible so that the required Java ImageIO operations work.
   * Therefore the image scaling capabilities in OpenCms are disabled by default.
   *
   * <p>
   *
   * @return <code>true</code> if the image scaling and processing capabilities for the OpenCms VFS
   *     images have been enabled
   */
  ...
  public void addConfigurationParameter(String paramName, String paramValue) {
    ...
    if (CmsStringUtil.isNotEmpty(paramName) && CmsStringUtil.isNotEmpty(paramValue)) {
      ...
      if (CONFIGURATION_MAX_SCALE_SIZE.equals(paramName)) {
        m_maxScaleSize =
            CmsStringUtil.getIntValue(paramValue, CmsImageScaler.SCALE_DEFAULT_MAX_SIZE, paramName);
      }
      if (CONFIGURATION_MAX_BLUR_SIZE.equals(paramName)) {
        m_maxBlurSize =
            CmsStringUtil.getIntValue(
                paramValue, CmsImageScaler.SCALE_DEFAULT_MAX_BLUR_SIZE, paramName);
      }
   ...
   *
   * <p>All results are cached in disk. If the scaled version does not exist in the cache, it is
   * created. Unscaled versions of the images are also stored in the cache.
   *
   * <p>
   *
  ...
  protected CmsFile getScaledImage(CmsObject cms, CmsResource resource, CmsImageScaler scaler)
    ...
    if (content != null) {
      if (resource instanceof CmsFile) {
        ...
        // this is no file, but we don't want to use "upgrade" since we don't need to read the
  ...
  public void initConfiguration() {
    ...
    if (m_vfsDiskCache == null) {
      m_vfsDiskCache =
          new CmsVfsNameBasedDiskCache(
              OpenCms.getSystemInfo().getWebApplicationRfsPath(), m_imageRepositoryFolder);
    }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_IMAGE_REPOSITORY_PATH_1, m_vfsDiskCache.getRepositoryPath()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_IMAGE_SCALING_ENABLED_1, Boolean.valueOf(m_enabled)));
    }
  }
