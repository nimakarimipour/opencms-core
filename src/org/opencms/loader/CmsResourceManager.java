 ...
 * provides methods to access them during OpenCms runtime.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsResourceManager {
  ...
  /**
   * Contains the part of the resource manager configuration that can be changed during runtime by
   * the import / deletion of a module.
   *
   * <p>A module can add resource types and extension mappings to resource types.
   *
   * <p>
   */
  static final class CmsResourceManagerConfiguration {
    ...
    protected @RUntainted Map<String, @RUntainted String> m_extensionMappings;

    /** A list that contains all initialized resource types. */
    protected List<I_CmsResourceType> m_resourceTypeList;

    /**
     * A list that contains all initialized resource types, plus configured types for "unknown"
     * resources.
     */
    ...
    /**
     * Creates a new resource manager data storage.
     *
     * <p>
     */
    ...
    /**
     * Freezes the current configuration by making all data structures unmodifiable that can be
     * accessed form outside this class.
     *
     * <p>
     *
    ...
    protected void freeze(
        I_CmsResourceType restypeUnknownFolder, I_CmsResourceType restypeUnknownFile) {

      // generate the resource type list with unknown resource types
      m_resourceTypeListWithUnknown =
          new ArrayList<I_CmsResourceType>(m_resourceTypeList.size() + 2);
    ...
    /**
     * Returns the configured resource type with the matching type id, or <code>null</code> if a
     * resource type with that id is not configured.
     *
     * <p>
     *
    ...
    /**
     * Returns the configured resource type with the matching type name, or <code>null</code> if a
     * resource type with that name is not configured.
     *
     * <p>
     *
  ...
  public static final String DEFAULT_TEMPLATE =
      CmsWorkplace.VFS_PATH_COMMONS + "template/default.jsp";
  ...
  private @RUntainted Map<String, String> m_mimeTypes;
  ...
  private @RUntainted List<@RUntainted I_CmsResourceType> m_resourceTypesFromXml;
  ...
  /**
   * Creates a new instance for the resource manager, will be called by the VFS configuration
   * manager.
   *
   * <p>
   */
   ...
   */
  public synchronized I_CmsResourceCollector addContentCollector(@RUntainted String className, @RUntainted String order)
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_CONTENT_COLLECTOR_CLASS_NOT_FOUND_1, className),
          e);
      return null;
    }
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_COLLECTOR_BAD_ORDER_NUMBER_1, className), e);
    }
    collector.setOrder(ord);

    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_ADD_COLLECTOR_CLASS_2, className, order));
    }
    ...
    if (m_collectors != null) {
      ...
      m_collectorNameMappings =
          new HashMap<String, I_CmsResourceCollector>(m_collectorNameMappings);
    } else {
    ...
    if (!m_collectors.contains(collector)) {
      ...
      Iterator<@RUntainted String> i = collector.getCollectorNames().iterator();
      while (i.hasNext()) {
        ...
        if (m_collectorNameMappings.containsKey(name)) {
          ...
          if (collector.getOrder() > otherCollector.getOrder()) {
            ...
            if (CmsLog.INIT.isInfoEnabled()) {
              CmsLog.INIT.info(
                  Messages.get().getBundle().key(Messages.INIT_COLLECTOR_REPLACED_1, name));
            }
          } else {
            if (CmsLog.INIT.isInfoEnabled()) {
              CmsLog.INIT.info(
                  Messages.get()
                      .getBundle()
                      .key(Messages.INIT_DUPLICATE_COLLECTOR_SKIPPED_1, name));
            }
          }
        } else {
          m_collectorNameMappings.put(name, collector);
   ...
   */
  public I_CmsHtmlConverter addHtmlConverter(@RUntainted String name, @RUntainted String className)
      throws CmsConfigurationException {

    // check if new conversion option can still be added
    if (m_frozen) {
      throw new CmsConfigurationException(
          Messages.get().container(Messages.ERR_NO_CONFIG_AFTER_STARTUP_0));
    }
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_HTML_CONVERTER_CLASS_NOT_FOUND_1, className),
          e);
      return null;
    }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_ADD_HTML_CONVERTER_CLASS_2, className, name));
    }
   ...
   * @throws CmsConfigurationException in case the resource manager configuration is already
   *     initialized
   */
  public void addLoader(@RUntainted I_CmsResourceLoader loader) throws CmsConfigurationException {
    ...
    @RUntainted int pos = loader.getLoaderId();
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_ADD_LOADER_2, loader.getClass().getName(), new Integer(pos)));
    }
  }
   ...
   *     initialized
   */
  public CmsRelationType addRelationType(@RUntainted String name, String type)
      throws CmsConfigurationException {
    ...
    CmsRelationType relationType =
        new CmsRelationType(m_configuredRelationTypes.size(), name, type);
   ...
   * types.
   *
   * <p>Resource types can also be added from a module.
   *
   * <p>
   *
   ...
   *     initialized
   */
  public void addResourceType(@RUntainted I_CmsResourceType resourceType) throws CmsConfigurationException {
    ...
    if (conflictingType != null) {
      // configuration problem: the resource type (or at least the id or the name) is already
      // configured
      throw new CmsConfigurationException(
          Messages.get()
              .container(
        ...
        try {
          ...
          if (schema != null) {
            CmsXmlContentDefinition contentDefinition =
                CmsXmlContentDefinition.unmarshal(cms, schema);

            CmsDefaultSet<String> allowedContexts =
                contentDefinition.getContentHandler().getAllowedTemplates();
            result.put(resType.getTypeName(), allowedContexts);
          } else {
            LOG.info(
                "No schema for XML type "
                    + resType.getTypeName()
                    + " / "
  ...
  /**
   * Returns the configured content collector with the given name, or <code>null</code> if no
   * collector with this name is configured.
   *
   * <p>
   *
  ...
  /**
   * Returns the default resource type for the given resource name, using the configured resource
   * type file extensions.
   *
   * <p>In case the given name does not map to a configured resource type, {@link
   * CmsResourceTypePlain} is returned.
   *
   * <p>This is only required (and should <i>not</i> be used otherwise) when creating a new resource
   * automatically during file upload or synchronization. Only in this case, the file type for the
   * new resource is determined using this method. Otherwise the resource type is <i>always</i>
   * stored as part of the resource, and is <i>not</i> related to the file name.
   *
   * <p>
   *
   ...
   */
  public I_CmsResourceType getDefaultTypeForName(@RUntainted String resourcename) throws CmsException {

    @RUntainted String typeName = null;
    @RUntainted String suffix = null;
    if (CmsStringUtil.isNotEmpty(resourcename)) {
      @RUntainted int pos = resourcename.lastIndexOf('.');
      if (pos >= 0) {
    ...
    if (CmsLog.INIT.isDebugEnabled()) {
      CmsLog.INIT.debug(
          Messages.get().getBundle().key(Messages.INIT_GET_RESTYPE_2, typeName, suffix));
    }
   ...
   *
   * @param name the name of the option that should trigger the HTML converter class
   * @return the matching HTML converter class name for the specified option name or <code>null
   ...
   *
   * <p>If an encoding parameter that is not <code>null</code> is provided, the returned MIME type
   * is extended with a <code>; charset={encoding}</code> setting.
   *
   * <p>If no MIME type for the given filename can be determined, the default <code>
   * {@link #MIMETYPE_HTML}</code> is used.
   *
   * <p>
   *
   ...
   * is extended with a <code>; charset={encoding}</code> setting.
   *
   * <p>If no MIME type for the given filename can be determined, the provided default is used.
   *
   * <p>
   *
   * @param filename the file name to check the MIME type for
   * @param encoding the default encoding (charset) in case of MIME types is of type "text"
   * @param defaultMimeType the default MIME type to use if no matching type for the filename is
   *     found
   * @return the MIME type for a specified file
   */
  ...
  public I_CmsResourceType getResourceType(@RUntainted String typeName) throws CmsLoaderException {
    ...
    throw new CmsLoaderException(
        Messages.get().container(Messages.ERR_UNKNOWN_RESTYPE_NAME_REQ_1, typeName));
  }
  ...
  public CmsTemplateLoaderFacade getTemplateLoaderFacade(
      CmsObject cms, CmsResource resource, String templateProperty) throws CmsException {
   ...
   */
  public CmsTemplateLoaderFacade getTemplateLoaderFacade(
      CmsObject cms, HttpServletRequest request, CmsResource resource, @RUntainted String templateProperty)
      throws CmsException {
    ...
    if (templateProp == null) {
      ...
      if (namedTemplate == null) {
        ...
        throw new CmsLoaderException(
            Messages.get()
                .container(
                    Messages.ERR_NONDEF_PROP_2, templateProperty, cms.getSitePath(resource)));
      }
      ...
      if (CmsTemplateContextManager.hasPropertyPrefix(templateProp)) {
        templateContext =
            OpenCms.getTemplateContextManager()
                .getTemplateContext(templateProp, cms, request, resource);
    ...
    CmsTemplateLoaderFacade result =
        new CmsTemplateLoaderFacade(getLoader(template), resource, template);
   ...
   * @return <code>true</code> if such a resource type has been configured, <code>false</code>
   *     otherwise
   ...
   *     otherwise
   * @see #getResourceType(int)
   * @deprecated Use {@link #hasResourceType(I_CmsResourceType)} or {@link
   *     #hasResourceType(I_CmsResourceType)} instead. Resource types should always be referenced
   ...
   * @throws CmsRoleViolationException in case the provided OpenCms user context did not have
   *     "module manager" role permissions
   * @throws CmsConfigurationException in case of duplicate resource types in the configuration
   */
  public synchronized void initialize(CmsObject cms)
   ...
   *
   * <p>This will return 'false' if no resource type with the given name is registered.
   *
   * <p>
   *
   ...
   */
  public void setNameGenerator(@RUntainted I_CmsFileNameGenerator nameGenerator)
      throws CmsConfigurationException {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_SET_NAME_GENERATOR_1, nameGenerator.getClass().getName()));
    }
  }
  ...
  private String getTemplateName(CmsObject cms, CmsResource resource) throws CmsException {
    ...
    String templateName =
        (String) (m_templateNameCache.getCachedObject(cms, resource.getRootPath()));
    if (templateName == null) {
      CmsProperty nameProperty =
          cms.readPropertyObject(resource, CmsPropertyDefinition.PROPERTY_TEMPLATE_ELEMENTS, false);
   ...
   * Initialize the HTML converters.
   *
   * <p>HTML converters are configured in the OpenCms <code>opencms-vfs.xml</code> configuration
   * file.
   *
   * <p>For legacy reasons, the default JTidy HTML converter has to be loaded if no explicit HTML
   * converters are configured in the configuration file.
   *
   * <p>
   */
  private void initHtmlConverters() {
    ...
    if (m_configuredHtmlConverters.size() == 0) {
      ...
      m_configuredHtmlConverters.add(
          new CmsHtmlConverterOption(CmsHtmlConverter.PARAM_XHTML, classJTidy, true));
      m_configuredHtmlConverters.add(
          new CmsHtmlConverterOption(CmsHtmlConverter.PARAM_WORD, classJTidy, true));
      m_configuredHtmlConverters.add(
    ...
    for (Iterator<CmsHtmlConverterOption> i = m_configuredHtmlConverters.iterator();
        i.hasNext(); ) {
   ...
   * Initialize the MIME types.
   *
   * <p>MIME types are configured in the OpenCms <code>opencms-vfs.xml</code> configuration file.
   *
   * <p>For legacy reasons, the MIME types are also read from a file <code>"mimetypes.properties"
   * </code> that must be located in the default <code>"classes"</code> folder of the web
   * application.
   *
   * <p>
   */
  private void initMimeTypes() {
    ...
    try {
      ...
      try {
        // second try: read MIME types from loader package (legacy reasons, there are no types by
        // default)
        mimeTypes.load(
            getClass()
                .getClassLoader()
                .getResourceAsStream("org/opencms/loader/mimetypes.properties"));
      } catch (Throwable t2) {
    ...
    List<CmsMimeType> combinedMimeTypes =
        new ArrayList<CmsMimeType>(mimeTypes.size() + m_configuredMimeTypes.size());
    ...
    while (i.hasNext()) {
      ...
      CmsMimeType mimeType =
          new CmsMimeType(entry.getKey().toString(), entry.getValue().toString(), false);
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_NUM_MIMETYPES_1, new Integer(m_mimeTypes.size())));
    }
  }

  /**
   * Adds a new resource type to the internal list of loaded resource types and initializes options
  ...
  private synchronized void initResourceType(
      @RUntainted I_CmsResourceType resourceType, CmsResourceManagerConfiguration configuration) {
    ...
    // add the mappings
    List<@RUntainted String> mappings = resourceType.getConfiguredMappings();
    Iterator<@RUntainted String> i = mappings.iterator();
    while (i.hasNext()) {
      ...
      if (!configuration.m_extensionMappings.containsKey(mapping)) {
        ...
        if (CmsLog.INIT.isInfoEnabled()) {
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_MAP_RESTYPE_2, mapping, resourceType.getTypeName()));
        }
      }
    }
  }
  ...
  /**
   * Initializes member variables required for storing the resource types.
   *
   * <p>
   *
   * @throws CmsConfigurationException in case of duplicate resource types in the configuration
  ...
  private synchronized void initResourceTypes() throws CmsConfigurationException {
    ...
    Iterator<@RUntainted I_CmsResourceType> i;
    i = m_resourceTypesFromXml.iterator();
    while (i.hasNext()) {
      @RUntainted I_CmsResourceType resourceType = i.next();
  ...
  /**
   * Reads a template resource together with its name.
   *
   * <p>
   *
   * @param cms the current CMS context
