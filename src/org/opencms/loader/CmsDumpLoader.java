 ...
 *
 * <p>This loader is also used to deliver static sub-elements of pages processed by other loaders.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsDumpLoader implements I_CmsResourceLoader {
  ...
  public byte[] export(
    ...
    if ((req != null) && (res != null)) {
      ...
      for (Iterator<String> i = OpenCms.getStaticExportManager().getExportHeaders().listIterator();
          i.hasNext(); ) {
  ...
  /**
   * Will always return <code>null</code> since this loader does not need to be configured.
   *
   * <p>
   *
   ...
   * locale) <code>"The OpenCms default resource loader for unprocessed files"</code>.
   *
   * <p>
   *
  ...
  /**
   * Checks if the requested resource must be send to the client by checking the "If-Modified-Since"
   * http header.
   *
   * <p>If the resource has not been modified, the "304 - not modified" header is send to the client
   * and <code>true</code> is returned, otherwise nothing is send and <code>false</code> is
   * returned.
   *
   * <p>
   *
   ...
   */
  protected boolean canSendLastModifiedHeader(
      CmsResource resource, HttpServletRequest req, HttpServletResponse res) {

    // resource state must be unchanged
    if (resource.getState().isUnchanged()
        // the request must not have been send by a workplace user (we can't use "304 - not
        // modified" in workplace
