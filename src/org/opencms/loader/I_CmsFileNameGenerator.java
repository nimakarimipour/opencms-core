 ...
 * a "new" operation in the context of the direct edit interface.
 *
 * <p>
 *
 * @since 8.0.0
...
public interface I_CmsFileNameGenerator {
   ...
   *
   * <p>For example from the given baseName 'test.txt' the copy file name 'test_copy.txt' will be
   * generated.
   *
   * <p>
   *
   ...
   *
   * <p>Used by the collector API as well as the galleries introduced with OpenCms 8 (ADE).
   *
   * @param cms the current OpenCms user context
   ...
   * @param namePattern the pattern to be used when generating the new resource name
   * @param defaultDigits the default number of digits to use for numbering the created file names
   * @param explorerMode if true, tries the name without a number, and also automatically inserts an
   *     underscore before the number
   * @return a new resource name based on the provided OpenCms user context and name pattern
   * @throws CmsException in case something goes wrong
   */
   ...
   *
   * <p>This is used by the <code>urlName</code> mapping of XML contents which enable SEO friendly
   * URLs automatically generated for example from the resource title.
   *
   * <p>Usually the first URL name from this sequence which does not already exist for a different
   * resource will be used for the URL name mapping.
   *
   * <p>
   *
