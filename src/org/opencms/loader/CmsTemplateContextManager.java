...
public class CmsTemplateContextManager {
  ...
  /**
   * A cache in which the template context provider instances are stored, with their class name as
   * the key.
   */
  private Map<String, I_CmsTemplateContextProvider> m_providerInstances =
      new HashMap<String, I_CmsTemplateContextProvider>();
  ...
  /**
   * Creates a bean with information about the current template context, for use in the client-side
   * code.
   *
   * <p>
   *
  ...
  public CmsTemplateContextInfo getContextInfoBean(CmsObject cms, HttpServletRequest request) {
    ...
    if (context != null) {
      ...
      CmsXmlContentProperty settingDefinition =
          createTemplateContextsPropertyDefinition(provider, locale);
      ...
      for (Map.Entry<String, CmsTemplateContext> entry : provider.getAllContexts().entrySet()) {
        ...
        for (CmsClientVariant variant : otherContext.getClientVariants().values()) {
          CmsClientVariantInfo info =
              new CmsClientVariantInfo(
                  variant.getName(),
  ...
  public CmsTemplateContext getTemplateContext(
      String providerName, CmsObject cms, HttpServletRequest request, CmsResource resource) {
   ...
   */
  public I_CmsTemplateContextProvider getTemplateContextProvider(CmsObject cms, String path)
      throws CmsException {
  ...
  /**
   * Retrieves an instance of a template context provider given its name (optionally prefixed by the
   * 'dynamic:' prefix).
   *
   * <p>
   *
  ...
  /**
   * Utility method which either reads a property from the template used for a specific resource, or
   * from the template context provider used for the resource if available.
   *
   * <p>
   *
   * @param cms the CMS context to use
   * @param res the resource from whose template or template context provider the property should be
   *     read
  ...
  public String readPropertyFromTemplate(
    ...
    try {
      CmsProperty templateProp =
          cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_TEMPLATE, true);
  ...
  protected CmsXmlContentProperty createTemplateContextsPropertyDefinition(
    ...
    String niceName =
        Messages.get().getBundle(locale).key(Messages.GUI_SETTING_TEMPLATE_CONTEXTS_NAME_0);
    String description =
        Messages.get().getBundle(locale).key(Messages.GUI_SETTING_TEMPLATE_CONTEXTS_DESCRIPTION_0);
  ...
  /**
   * Helper method for getting the forbidden contexts from the resource manager without a try-catch
   * block.
   *
   * <p>
   *
