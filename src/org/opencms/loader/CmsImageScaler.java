...
public class CmsImageScaler {
  ...
  public static final List<String> FILTERS =
      Arrays.asList(new String[] {FILTER_GRAYSCALE, FILTER_SHADOW});

  /**
   * The (optional) parameter used for sending the scale information of an image in the http
   * request.
   */
  public static final String PARAM_SCALE = "__scale";

  /**
   * The default maximum image size (width * height) to apply image blurring when down scaling
   * (setting this to high may case "out of memory" errors).
   */
  public static final int SCALE_DEFAULT_MAX_BLUR_SIZE = 2500 * 2500;

  /**
   * The default maximum image size (width or height) to allow when up or down scaling an image
  ...
  /**
   * The scaler parameter to indicate to requested image save quality in percent (if applicable, for
   * example used with JPEG images).
   */
  public static final String SCALE_PARAM_QUALITY = "q";

  /**
   * The scaler parameter to indicate to requested <code>{@link java.awt.RenderingHints}</code>
   * settings.
   */
   ...
   * to high may case "out of memory" errors).
   */
  ...
  /**
   * Creates a new image scaler initialized with the height and width of the given image contained
   * in the byte array.
   *
   * <p><b>Please note:</b>The image itself is not stored in the scaler, only the width and height
   * dimensions of the image. To actually scale an image, you need to use <code>
   * {@link #scaleImage(CmsFile)}</code>. This constructor is commonly used only to extract the
   * image dimensions, for example when creating a String value for the <code>
   * {@link CmsPropertyDefinition#PROPERTY_IMAGE_SIZE}</code> property.
   *
   * <p>In case the byte array can not be decoded to an image, or in case of other errors, <code>
   * {@link #isValid()}</code> will return <code>false</code>.
   *
   * <p>
   *
  ...
  public CmsImageScaler(byte[] content, @RUntainted String rootPath) {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.ERR_UNABLE_TO_EXTRACT_IMAGE_SIZE_1, rootPath),
            e);
      }
   ...
   * {@link CmsPropertyDefinition#PROPERTY_IMAGE_SIZE}</code> from the given resource.
   *
   * <p>In case of any errors reading or parsing the property, <code>{@link #isValid()}</code> will
  ...
  public CmsImageScaler(CmsObject cms, CmsResource res) {
    ...
    if ((cms != null) && (res != null)) {
      try {
        CmsProperty sizeProp =
            cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_IMAGE_SIZE, false);
   ...
   *
   * <p>The maximum scale size is checked in order to prevent DOS attacks. Without this, it would be
   * possible to request arbitrary huge images with a simple GET request, which would result in
   * Out-Of-Memory errors if the image is just requested large enough.
   *
   * <p>The maximum blur size is checked since this operation is know to also cause memory issues
   * with large images. If the original image is larger then this, no blur is applied before scaling
   * down, which will result in a less optimal but still usable scale result.
   *
   * <p>
   *
   ...
   */
  public static int[] calculateDimension(
      int sourceWidth, int sourceHeight, int boxWidth, int boxHeight) {
   ...
   *
   * <p>Use {@link #setCropArea(int, int, int, int)} to set this value.
   *
   * <p>
   *
   ...
   * the given scaler size.
   *
   * <p>If no down scale from this to the given scaler is required according to {@link
   * #isDownScaleRequired(CmsImageScaler)}, then <code>null</code> is returned.
   *
   * <p>
   *
   * @param downScaler the image scaler that holds the down scaled target image dimensions
   * @return a new image scaler that is a down scale from the size of <code>this</code> scaler to
  ...
  /**
   * Returns the image type from the given file name based on the file suffix (extension) and the
   * available image writers.
   *
   * <p>For example, for the file name "opencms.gif" the type is GIF, for "opencms.jpg" is is "JPEG"
   * etc.
   *
   * <p>In case the input filename has no suffix, or there is no known image writer for the format
   * defined by the suffix, <code>null</code> is returned.
   *
   * <p>Any non-null result can be used if an image type input value is required.
   *
   * <p>
   *
   * @param filename the file name to get the type for
   * @return the image type from the given file name based on the suffix and the available image
   *     writers, or null if no image writer is available for the format
   */
  ...
  /**
   * Returns the maximum image size (width * height) to apply image blurring when down scaling
   * images.
   *
   * <p>Image blurring is required to achieve the best results for down scale operations when the
   * target image size is 2 times or more smaller then the original image size. This parameter
   * controls the maximum size (width * height) of an image that is blurred before it is down
   * scaled. If the image is larger, no blurring is done. Image blurring is an expensive operation
   * in both CPU usage and memory consumption. Setting the blur size to large may case "out of
   * memory" errors.
   *
   * <p>
   *
   * @return the maximum image size (width * height) to apply image blurring when down scaling
   *     images
   */
   ...
   *
   * <p>If this scaler is not valid (see {@link #isValid()}) the result is undefined.
   *
   * <p>
   *
   ...
   *
   * <p>This is used only if applicable, for example when saving JPEG images.
   *
   * <p>
   *
   ...
   *
   * <dl>
   *   <dt>{@link Simapi#RENDER_QUALITY} (default)
   *   <dd>Use best possible image processing - this may be slow sometimes.
   *   <dt>{@link Simapi#RENDER_SPEED}
   *   <dd>Fastest image processing but worse results - use this for thumbnails or where speed is
   *       more important then quality.
   *   <dt>{@link Simapi#RENDER_MEDIUM}
   *   <dd>Use default rendering hints from JVM - not recommended since it's almost as slow as the
   *       {@link Simapi#RENDER_QUALITY} mode.
   * </dl>
   *
   ...
   * <code>'?'</code> char as a prefix so that this may be directly appended to an image URL.
   *
   * <p>This can be appended to an image request in order to apply image scaling parameters.
   *
   * <p>
   *
  ...
  /**
   * Returns a new image scaler that is a rescaler from <code>this</code> scaler size to the given
   * target scaler size.
   *
   * <p>The height of the target image is calculated in proportion to the original image width. If
   * the width of the the original image is not known, the target image width is calculated in
   * proportion to the original image height.
   *
   * <p>
   *
   * @param target the image scaler that holds the target image dimensions
   * @return a new image scaler that is a rescaler from the <code>this</code> scaler size to the
   *     given target scaler size
   */
   ...
   *
   * <dl>
   *   <dt>0 (default): Scale to exact target size with background padding
   *   <dd>
   *       <ul>
   ...
   *       </ul>
   *   <dt>1: Thumbnail generation mode (like 0 but no image enlargement)
   *   <dd>
   *       <ul>
   ...
   *         <li>fit full image inside target size (only applies if reduced)
   *       </ul>
   *   <dt>2: Scale to exact target size, crop what does not fit
   *   <dd>
   *       <ul>
   *         <li>enlarge image to fit in target size (if required)
   *         <li>reduce image to fit in target size (if required)
   ...
   *       </ul>
   *   <dt>3: Scale and keep image proportions, target size variable
   *   <dd>
   *       <ul>
   *         <li>enlarge image to fit in target size (if required)
   *         <li>reduce image to fit in target size (if required)
   *         <li>keep image aspect ratio / proportions intact
   *         <li>scaled image will not be padded or cropped, so target size is likely not the exact
   *             requested size
   *       </ul>
   *   <dt>4: Don't keep image proportions, use exact target size
   *   <dd>
   *       <ul>
   *         <li>enlarge image to fit in target size (if required)
   *         <li>reduce image to fit in target size (if required)
   *         <li>don't keep image aspect ratio / proportions intact
   *         <li>the image will be scaled exactly to the given target size and likely will be loose
   *             proportions
   *       </ul>
   * </dl>
   *
   * <dt>5: Scale and keep image proportions without enlargement, target size variable with optional
   ...
   *     </ul>
   *
   * <dt>6: Crop around point: Use exact pixels
   * <dd>
   *
   *     <ul>
   *       <li>This type only applies for image crop operations (full crop parameters must be
   *           provided).
   *       <li>In this case the crop coordinates <code>x, y</code> are treated as a point in the
   *           middle of <code>width, height</code>.
   *       <li>With this type, the pixels from the source image are used 1:1 for the target image.
   *     </ul>
   *
   * <dt>7: Crop around point: Use pixels for target size, get maximum out of image
   * <dd>
   *
   *     <ul>
   *       <li>This type only applies for image crop operations (full crop parameters must be
   ...
   *       <li>In this case the crop coordinates <code>x, y</code> are treated as a point in the
   *           middle of <code>width, height</code>.
   *       <li>With this type, as much as possible from the source image is fitted in the target
   *           image size.
   *     </ul>
   *
   * <dt>8: Focal point mode.
   *
   *     <p>If a focal point is set on this scaler, this mode will first crop a region defined by
   *     cx,cy,cw,ch from the original image, then select the largest region of the aspect ratio
   *     defined by w/h in the cropped image containing the focal point, and finally scale that
   *     region to size w x h.
   *
   * @return the type
   */
  public int getType() {
  ...
  /**
   * Returns a new image scaler that is a width based down scale from the size of <code>this</code>
   * scaler to the given scaler size.
   *
   * <p>If no down scale from this to the given scaler is required because the width of <code>this
   * </code> scaler is not larger than the target width, then the image dimensions of <code>this
   * </code> scaler are unchanged in the result scaler. No up scaling is done!
   *
   * <p>
   *
   * @param downScaler the image scaler that holds the down scaled target image dimensions
   * @return a new image scaler that is a down scale from the size of <code>this</code> scaler to
   *     the given target scaler size
   */
  ...
  /**
   * Returns <code>true</code> if all required parameters for image cropping are available.
   *
   * <p>Required parameters are <code>"cx","cy"</code> (x, y start coordinate), and <code>"ch","cw"
   * </code> (crop height and width).
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this image scaler must be down scaled when compared to the given
   * "down scale" image scaler.
   *
   * <p>If either <code>this</code> scaler or the given <code>downScaler</code> is invalid according
   * to {@link #isValid()}, then <code>false</code> is returned.
   *
   * <p>The use case: <code>this</code> scaler represents an image (that is contains width and
   * height of an image). The <code>downScaler</code> represents the maximum wanted image. The
   * scalers are compared and if the image represented by <code>this</code> scaler is too large,
   * <code>true</code> is returned. Image orientation is ignored, so for example an image with
   * 600x800 pixel will NOT be down scaled if the target size is 800x600 but kept unchanged.
   *
   * <p>
   *
   * @param downScaler the down scaler to compare this image scaler with
   * @return <code>true</code> if this image scaler must be down scaled when compared to the given
   *     "down scale" image scaler
   */
  ...
  /**
   * Returns <code>true</code> if the image scaler was only used to read image properties from the
   * VFS.
   *
   * @return <code>true</code> if the image scaler was only used to read image properties from the
   *     VFS
   */
   ...
   *
   * <p>Required parameters are <code>"h"</code> (height), and <code>"w"</code> (width).
   *
   * <p>
   *
   ...
   *
   * <p>The parameter String must have a format like <code>"h:100,w:200,t:1"</code>, that is a comma
   * separated list of attributes followed by a colon ":", followed by a value. As possible
   * attributes, use the constants from this class that start with <code>SCALE_PARAM</Code> for
   * example {@link #SCALE_PARAM_HEIGHT} or {@link #SCALE_PARAM_WIDTH}.
   *
   * <p>
   *
    ...
    while (it.hasNext()) {
      ...
      if (CmsStringUtil.isNotEmpty(k) && CmsStringUtil.isNotEmpty(v)) {
        ...
        if (SCALE_PARAM_HEIGHT.equals(k)) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get().getBundle().key(Messages.ERR_INVALID_IMAGE_SCALE_PARAMS_2, k, v));
          }
        }
      } else {
        if (LOG.isDebugEnabled()) {
  ...
  /**
   * Returns a scaled version of the given image byte content according this image scalers
   * parameters.
   *
   * <p>
   *
   * @param content the image byte content to scale
   * @param image if this is set, this image will be used as base for the scaling rather than a new
   *     image read from the content byte array
  ...
  public byte[] scaleImage(byte[] content, BufferedImage image, @RUntainted String rootPath) {
    ...
    if (imageType == null) {
      ...
      // check if this is another known MIME type, if so DONT use it (images should not be named
      // *.pdf)
      if (mimeType == null) {
    ...
    if (imageType == null) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_UNABLE_TO_SCALE_IMAGE_2, rootPath, toString()));
      }
      return result;
    }
    try {
      ...
      if (isCropping()) {
        if ((getType() == 8) && (m_focalPoint != null)) {
          image =
              scaler.cropToSize(
                  ...
                  m_cropHeight,
                  color);
          // Find the biggest scaling factor which, when applied to a rectangle of dimensions
          // m_width x m_height,
          // would allow the resulting rectangle to still fit inside a rectangle of dimensions
          // m_cropWidth x m_cropHeight
          // (we have to take the minimum because a rectangle that fits on the x axis might still be
          // out of bounds on the y axis, and
          // vice versa).
          ...
          // the focal point's coordinates are in the uncropped image's coordinate system, so we
          // have to subtract cx/cy
          ...
          image =
              scaler.cropPointToSize(
                  image, m_cropX, m_cropY, getType() == 6, m_cropWidth, m_cropHeight);
        } else {
        ...
        switch (getType()) {
          ...
          case 4:
            ...
            break;
          case 5:
            // scale and keep image proportions, target size variable, include maxWidth / maxHeight
            // option
            // image proportions have already been calculated so should not be a problem, use
            // 'false' to make sure image size exactly matches height and width attributes of
            // generated tag
            if (((imageWidth != getWidth()) || (imageHeight != getHeight()))) {
              image = scaler.resize(image, getWidth(), getHeight(), false);
              imageProcessed = true;
   ...
   *
   * <p>Only if all 4 required parameters have been set, the crop area is set accordingly. Moreover,
   * it is not required to specify the target image width and height when using crop, because these
   * parameters can be calculated from the crop area.
   *
   * <p>Scale type 6 and 7 are used for a 'crop around point' operation, see {@link #getType()} for
   * a description.
   *
   * <p>
   */
