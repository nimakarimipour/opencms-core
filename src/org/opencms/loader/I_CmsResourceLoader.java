 ...
 * VFS, process it's contents and deliver the result to the user.
 *
 * <p>The I_CmsResourceLoader operates with Request and Response in much the same way as a standard
 * Java web application.
 *
 * <p>This interface uses a standard servlet {@link javax.servlet.http.HttpServletRequestWrapper} /
 * {@link javax.servlet.http.HttpServletResponseWrapper} that provide access to a special
 * implementation of the {@link javax.servlet.RequestDispatcher}. The handling of the output written
 * to the response is done by this dispatcher. The results are then passed back to OpenCms which
...
public interface I_CmsResourceLoader extends I_CmsConfigurationParameterHandler {
   ...
   *
   * <p>Dumping the content is like calling "load" where the result is not written to the response
   * stream, but to the returned byte array. Dumping is different from an export because the export
   * might actually require that the content is handled or modified in a special way, or set special
   * http headers.
   *
   * <p>Moreover, if the page type is template based, calling "dump" will not trigger the template
   * but directly deliver the contents from the selected element.
   *
   * <p>
   *
   ...
   *
   * <p>During static export, the resource content may be written to 2 streams: The export stream,
   * and the http response output stream. Which stream is actually used depends whether the export
   * is in "on demand" or "after publish" mode. In "on demand" mode, the resource needs to be
   * written both to the response stream and to the file stream. In "after publish" mode, it's
   * usually only written to the file stream, but sometimes it's required to write to the response
   * stream as well.
   *
   * <p>
   *
  ...
  /**
   * Signals if a loader that supports templates must be invoked on the template URI or the resource
   * URI.
   *
   * <p>
   *
  ...
  /**
   * Basic top-page processing method for a I_CmsResourceLoader, this method is called if the page
   * is called as a sub-element on a page not already loaded with a I_CmsResourceLoader.
   *
   * <p>
   *
   * @param cms the initialized CmsObject which provides user permissions
  ...
  /**
   * Does the job of including the requested resource, this method is called directly if the element
   * is called as a sub-element from another I_CmsResourceLoader.
   *
   * <p>
   *
   * @param cms used to access the OpenCms VFS
