 ...
 *
 * <p>This class is called via the front end to publish scheduled a file at a given time.
 *
 * <p>Per default, it publishes all new, edited and deleted resources in the project which are
 * locked in the current project. For all resources in the project which are not locked by the
 * current user is the lock changed. You are able to perform a link validation before publishing the
 * project by adding the parameter <code>linkcheck=true</code>. It is possible to send an email to a
 * user in OpenCms in case somthing went wrong during this process. To do so specifiy a parameter
 * <code>mail-to-user=username_in_opencms</code>. After running this job, the job is deleted.
 * Therefore the job name is to set in the parameter <code>jobname</code>
 *
 * <p>
 *
 * @since 7.5.1
...
public class CmsPublishScheduledJob implements I_CmsScheduledJob {
  ...
  /**
   * Optional parameter containing comma-separated list of structure ids of resources to explicitly
   * unlock at the end of the job (only necessary for resources that don't get published).
   */
  ...
  public synchronized String launch(CmsObject cms, Map<String, String> parameters)
    ...
    CmsWorkplaceReport report =
        new CmsWorkplaceReport(
    ...
    try {
      ...
      // resources that have not been themselves published, but have been locked by the scheduled
      // publish dialog
