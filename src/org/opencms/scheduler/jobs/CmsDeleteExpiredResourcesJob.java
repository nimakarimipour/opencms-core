 ...
 *
 * <p>The user to execute the process should have have access to the required "Workplace manager"
 * role.
 *
 * <p>The "Offline" project has to be configured for this job because the operations cannot be
 * performed in the "Online" project.
 *
 * <p>Job parameters:
 *
 * <p>
 *
 * <dl>
 *   <dt><code>expirationdays={Number/Integer}</code>
 *   <dd>Amount of days a resource has to be expired to be deleted.
 *   <dt><code>resourcetypes={csv list}</code>
 *   <dd>Comma separated list of resource type names to specify the types of expired resources that
 *       may be deleted. If left out, expired resources of all types will be deleted. .
 *   <dt><code>folder={csv list}</code>
 *   <dd>Allows to specify a comma separated list of folders in which all expired resources will be
 *       deleted. If omitted "/" will be taken as single folder for this operation.
 * </dl>
 *
 * <p>The property "delete.expired" (<code>{@link CmsPropertyDefinition#PROPERTY_DELETE_EXPIRED}
 * </code>) may be used to override the global setting of the parameter <code>expirationdays</code>.
 * A value of "never", "false" or "none" will prevent resources from being deleted. Other values are
 * "true" (default) or the amount of days a resource has to be expired for qualification of
 * deletion.
 *
 * <p>Only published / unchanged files will be processed. Anything with unpublished changes will not
 * be touched by the job.
 *
 * <p>Folders with expiration dates are ignored by default. Only if the scheduler parameter
 * "resourcetypes" contains "folder" a folder that has been expired will be deleted (with all
 * contained resources).
 *
 * <p>
 *
...
public class CmsDeleteExpiredResourcesJob implements I_CmsScheduledJob {
  ...
  public @RUntainted String launch(CmsObject cms, Map<String, String> parameters) throws Exception {
    ...
    if (OpenCms.getRunLevel() == OpenCms.RUNLEVEL_4_SERVLET_ACCESS) {
      ...
      I_CmsReport report =
          new CmsLogReport(cms.getRequestContext().getLocale(), CmsDeleteExpiredResourcesJob.class);
      report.println(
          Messages.get().container(Messages.RPT_DELETE_EXPIRED_START_0),
          I_CmsReport.FORMAT_HEADLINE);

      // collect all resources:
      List<CmsResource> resources = Collections.emptyList();
      CmsResourceFilter filter =
          CmsResourceFilter.ALL.addExcludeState(CmsResourceState.STATE_DELETED);
      filter = filter.addRequireExpireBefore(currenttime);

      // if we have configured resource types reading is more complicated because inclusion of
      // several types
      ...
      if (resTypesArr != null) {
        ...
        for (int i = resTypesArr.length - 1; i >= 0; i--) {
          ...
          for (int j = topFoldersArr.length - 1; j >= 0; j--) {
            ...
            changedFiles +=
                deleteExpiredResources(cms, report, resources, expirationdays, currenttime);
          }
        }
      ...
      report.println(
          Messages.get().container(Messages.RPT_DELETE_EXPIRED_END_0), I_CmsReport.FORMAT_HEADLINE);
    }
    return null;
  }

  /**
   * Deletes the expired resources if the have been expired longer than the given amount of days.
   *
   * <p>At this level the resource type is not checked again.
   *
   * <p>
   *
  ...
  private int deleteExpiredResources(
    ...
    while (it.hasNext()) {
      ...
      report.print(
          Messages.get()
              .container(Messages.RPT_DELETE_EXPIRED_PROCESSING_1, new String[] {resourcePath}),
      ...
      if (resource.getState() == CmsResourceState.STATE_UNCHANGED) {
        ...
        try {
          property =
              cms.readPropertyObject(resource, CmsPropertyDefinition.PROPERTY_DELETE_EXPIRED, true);
          ...
          if (!property.isNullProperty()) {
            if (PROPERTY_VALUE_DELETE_EXPIRED_NEVER.equals(propertyValue)
              ...
              // true is allowed, but any other value will be treated as a configuration error and
              // skip the
              // resource:

              if (!Boolean.TRUE.toString().equals(propertyValue)) {
                // NumberFormatException should skip the resource because the property value was
                // mistyped
          ...
          if ((expirationdate != Long.MAX_VALUE)
              && ((currenttime - expirationdate)
                  > (expirationDaysPropertyOverride * MILLIS_PER_DAY))) {
