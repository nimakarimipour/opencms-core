 ...
 *
 * <p>Image size information is stored in the <code>
 * {@link CmsPropertyDefinition#PROPERTY_IMAGE_SIZE}</code> property of an image file must have the
 * format "h:x,w:y" with x and y being positive Integer vaulues.
 *
 * <p>Job parameters:
 *
 * <p>
 *
 * <dl>
 *   <dt><code>downscale=true|false</code>
 *   <dd>Controls if images are automatically downscaled according to the configured image downscale
 *       settings, by default this is <code>false</code>.
 * </dl>
 *
 * @since 6.0.2
 */
public class CmsCreateImageSizeJob implements I_CmsScheduledJob {
  ...
  /**
   * This job parameter controls if images are automatically downscaled according to the configured
   * image downscale settings, by default this is <code>false</code>.
   *
   * <p>Possible values are <code>true</code> or <code>false</code> (default). If this is set to
   * <code>true</code>, then all images are checked against the configured image downscale settings
   * (see {@link CmsImageLoader#CONFIGURATION_DOWNSCALE}). If the image is too large, it is
  ...
  public String launch(CmsObject cms, Map<String, String> parameters) throws Exception {
    ...
    I_CmsReport report =
        new CmsLogReport(cms.getRequestContext().getLocale(), CmsCreateImageSizeJob.class);
    report.println(
        Messages.get().container(Messages.RPT_IMAGE_SIZE_START_0), I_CmsReport.FORMAT_HEADLINE);
        ...
        if (lock.isNullLock() || lock.isOwnedBy(cms.getRequestContext().getCurrentUser())) {
          ...
          if (scaler.isValid()) {
            ...
            if (scaler.isDownScaleRequired(downScaler)) {
              // downscaling is required - just write the file again, in this case everything is
              // updated
              ...
              // update the property if it does not exist or it is different than the newly
              // calculated one
              if (prop.isNullProperty() || !prop.getValue().equals(scaler.toString())) {
                ...
                if (prop.isNullProperty()) {
                  prop =
                      new CmsProperty(
                          CmsPropertyDefinition.PROPERTY_IMAGE_SIZE, null, scaler.toString());
                } else {
    ...
    report.println(
        Messages.get().container(Messages.RPT_IMAGE_SIZE_END_0), I_CmsReport.FORMAT_HEADLINE);

    return Messages.get()
        .getBundle()
        .key(Messages.LOG_IMAGE_SIZE_UPDATE_COUNT_1, new Integer(count));
  }

  /**
   * Locks the given resource (if required).
   *
   * <p>
   *
  ...
  /**
   * Unlocks the given resource (if required).
   *
   * <p>
   *
   * @param cms the OpenCms user context
