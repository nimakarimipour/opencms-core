 ...
 * {@link org.opencms.loader.CmsImageLoader}</code>.
 *
 * <p>Job parameters:
 *
 * <p>
 *
 * <dl>
 *   <dt><code>maxage={time in hours}</code>
 *   <dd>Specifies the maximum age (in hours) images can be unused before they are removed from the
 *       cache. Any image in the image cache folder that has a RFS date of last modification older
 *       than this time is considered expired and is therefore deleted.
 * </dl>
 *
 * @since 6.2.0
 */
public class CmsImageCacheCleanupJob implements I_CmsScheduledJob {
   ...
   *
   * <p>Empty directories are removed as well.
   *
   * <p>
   *
  ...
  /**
   * Removes all expired image cache entries from the given RFS directory, including recursion to
   * subdirectories.
   *
   * <p>
   *
          ...
          if (f.lastModified() < maxAge) {
            try {
              ...
              LOG.error(
                  Messages.get()
                      .getBundle()
                      .key(Messages.LOG_IMAGE_CACHE_UNABLE_TO_DELETE_1, f.getAbsolutePath()));
            }
          }
        }
      }
      if (directory.listFiles().length <= 0) {
        try {
          ...
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_IMAGE_CACHE_UNABLE_TO_DELETE_1, directory.getAbsolutePath()));
        }
      }
    }
    return count;
  ...
  public String launch(CmsObject cms, Map<String, String> parameters) throws Exception {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_IMAGE_CACHE_BAD_MAXAGE_2, maxAgeStr, new Float(maxAge)));
    }
    ...
    return Messages.get()
        .getBundle()
        .key(Messages.LOG_IMAGE_CACHE_CLEANUP_COUNT_1, new Integer(count));
  }
}
