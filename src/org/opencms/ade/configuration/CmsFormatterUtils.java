...
public class CmsFormatterUtils {
   ...
   */
  public static Set<String> getAllFormatterKeys(
      CmsADEConfigData configData, CmsContainerElementBean element) {

    Set<String> result =
        element.getIndividualSettings().entrySet().stream()
            .filter(entry -> entry.getKey().startsWith(CmsFormatterConfig.FORMATTER_SETTINGS_KEY))
