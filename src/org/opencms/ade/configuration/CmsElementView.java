...
public class CmsElementView {
  ...
  public static class ElementViewComparator implements Comparator<CmsElementView> {
    ...
    public int compare(CmsElementView o1, CmsElementView o2) {
      ...
      return ComparisonChain.start()
          ...
          .compare(o1.m_title, o2.m_title, Ordering.natural().nullsLast())
          .result();
    }
  }
  ...
  /**
   * Constructor for the default element view.
   *
   * <p>
   */
  ...
  private void init(CmsObject cms) throws Exception {
    ...
    I_CmsXmlContentValue titleKey =
        content.getValue(N_TITLE_KEY, CmsConfigurationReader.DEFAULT_LOCALE);
    if ((titleKey != null)
    ...
    I_CmsXmlContentValue orderVal =
        content.getValue(N_ORDER, CmsConfigurationReader.DEFAULT_LOCALE);
    if (orderVal != null) {
      try {
    ...
    I_CmsXmlContentValue parentView =
        content.getValue("Parent", CmsConfigurationReader.DEFAULT_LOCALE);
    if (parentView != null) {
      ...
      try {
        ...
        if (CmsStringUtil.isEmpty(stringValue)) {
          ...
          parentViewId =
              new CmsUUID(stringValue.substring(CmsConfigurationReader.VIEW_SCHEME.length()));
        } else {
