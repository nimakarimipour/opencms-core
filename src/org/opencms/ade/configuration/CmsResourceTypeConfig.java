...
public class CmsResourceTypeConfig implements I_CmsConfigurationObject<CmsResourceTypeConfig> {
  ...
  /**
   * Represents the visibility status of a resource type in the 'Add' menu of the container page
   * editor.
   *
   * <p>
   */
  ...
  /**
   * True if this creating/editing for this type should be enabled in lists (e.g. search or
   * contentload tags).
   */
  ...
  public CmsResourceTypeConfig(
      String typeName, boolean disabled, CmsContentFolderDescriptor folder, String pattern) {
   ...
   * @param enableInLists true if the type should be enabled, but only for the direct edit buttons
   *     in lists and not ADE/drag/drop.
   ...
   * @param localization the base name of the bundle to add as workplace bundle for the resource
   *     type
   * @param showInDefaultView if true, the element type should be shown in the default element view
   *     even if it doesn't belong to it
   * @param copyInModels if elements of this type when used in models should be copied instead of
   *     reused
  ...
  public boolean checkCreatable(CmsObject cms, String pageFolderRootPath) throws CmsException {
    ...
    if ((m_folderOrName != null)
        && m_folderOrName.isPageRelative()
        && (pageFolderRootPath == null)) {
      LOG.info(
          "type "
              + m_typeName
    ...
    try {
      ...
      LOG.info(
          "Using " + permissionCheckFolder + " as a permission check folder for " + folderPath);
      CmsExplorerTypeSettings settings =
          OpenCms.getWorkplaceManager().getExplorerTypeSetting(m_typeName);
      if (settings == null) {
        return false;
      }
      boolean editable = settings.isEditable(cms, permissionCheckFolder);
      boolean controlPermission =
          settings
              .getAccess()
              .getPermissions(cms, permissionCheckFolder)
  ...
  /**
   * Checks if a resource type is viewable for the current user. If not, this resource type should
   * not be available at all within the ADE 'add-wizard'.
   *
   * <p>
   *
   * @param cms the current CMS context
  ...
  public boolean checkViewable(CmsObject cms, String referenceUri) {
    ...
    try {
      ...
      CmsResource referenceResource =
          cms.readResource(referenceUri, CmsResourceFilter.ignoreExpirationOffline(cms));
      if (settings == null) {
        // no explorer type
        return false;
      }
  ...
  /**
   * Similar to createNewElement, but just sets parameters on a resource builder instead of actually
  ...
  public CmsResource createNewElement(
    ...
    if (!OpenCms.getLocaleManager()
        .getAvailableLocales(rootCms, folderPath)
        .contains(contentLocale)) {
      contentLocale = OpenCms.getLocaleManager().getDefaultLocale(rootCms, folderPath);
    }
    rootCms
        .getRequestContext()
        .setAttribute(CmsRequestContext.ATTRIBUTE_NEW_RESOURCE_LOCALE, contentLocale);
    if (modelResource != null) {
      // set the model resource
      rootCms
          .getRequestContext()
          .setAttribute(CmsRequestContext.ATTRIBUTE_MODEL, modelResource.getRootPath());
    }
    CmsResource createdResource =
        rootCms.createResource(creationPath, getType(), null, new ArrayList<CmsProperty>(0));
    if (modelResource != null) {
      // set the model resource
  ...
  public AddMenuVisibility getAddMenuVisibility(CmsUUID elementViewId, AddMenuType menuType) {
    ...
    if (isShowInDefaultView()
        && elementViewId.equals(CmsElementView.DEFAULT_ELEMENT_VIEW.getId())) {
  ...
  /**
   * Returns the bundle that is configured as workplace bundle for the resource type, or <code>null
   * </code> if none is configured.
   *
   * @return the bundle that is configured as workplace bundle for the resource type, or <code>null
   *     </code> if none is configured.
   */
  ...
  /**
   * Checks if creating and editing resources of this type should be possible via the edit buttons
   * generated by lists.
   *
  ...
  /**
   * Returns true if this resource type is configured as 'page relative', i.e. elements of this type
   * are to be stored with the container page on which they were created.
   *
   * <p>
   *
   ...
   *
   * <p>This defaults to 'false' if not set.
   *
   * @return true if the type should be shown in the default view event if it doens't belong to that
   *     element view
   */
  ...
  public CmsResourceTypeConfig merge(CmsResourceTypeConfig childConfig) {
    ...
    CmsContentFolderDescriptor folderOrName =
        childConfig.m_folderOrName != null ? childConfig.m_folderOrName : m_folderOrName;
    String namePattern =
        childConfig.m_namePattern != null ? childConfig.m_namePattern : m_namePattern;
    CmsUUID elementView =
        childConfig.m_elementView != null ? childConfig.m_elementView : m_elementView;
    Boolean showInDefaultView =
        childConfig.m_showInDefaultView != null
            ? childConfig.m_showInDefaultView
            : m_showInDefaultView;
    Boolean copyInModels =
        childConfig.m_copyInModels != null ? childConfig.m_copyInModels : m_copyInModels;
    ElementDeleteMode deleteMode =
        childConfig.m_elementDeleteMode != null
            ? childConfig.m_elementDeleteMode
            : m_elementDeleteMode;
    Integer order = childConfig.m_order != null ? childConfig.m_order : m_order;

    boolean mergedDisabled =
        childConfig.m_availabilityNotSet ? isDisabled() : childConfig.isDisabled();
    boolean mergedAddDisabled =
        childConfig.m_availabilityNotSet ? isAddDisabled() : childConfig.isAddDisabled();
    boolean mergedCreateDisabled =
        childConfig.m_availabilityNotSet
            ? isCreateDisabled()
            : (isCreateDisabled() || childConfig.isCreateDisabled());

    boolean mergedEnableInLists =
        childConfig.m_availabilityNotSet ? m_enableInLists : childConfig.m_enableInLists;
    boolean mergedDisableEdit =
        childConfig.m_availabilityNotSet ? m_editDisabled : childConfig.m_editDisabled;

    CmsResourceTypeConfig result =
   ...
   * <p>
   *
   * @param disabled true if the copy should be disabled regardless of whether the original is
   *     disabled
   * @return a copy of the resource type configuration object
   */
   ...
   *
   * <p>Normally, you should use getFolderPath() instead.
   *
   * <p>
   *
