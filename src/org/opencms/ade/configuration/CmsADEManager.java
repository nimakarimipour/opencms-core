 ...
 * related tasks like loading/saving favorite and recent lists.
 *
 * <p>
 */
public class CmsADEManager {
  ...
  public static final String CONFIG_SUFFIX =
      "/" + CmsADEManager.CONTENT_FOLDER_NAME + "/" + CmsADEManager.CONFIG_FILE_NAME;
  ...
  public CmsADEManager(
    ...
    // further initialization is done by the initialize() method. We don't do that in the
    // constructor,
  ...
  public CmsWaitHandle addFormatterCacheWaitHandle(boolean online) {
    ...
    CmsFormatterConfigurationCache cache =
        online ? m_onlineFormatterCache : m_offlineFormatterCache;
   ...
   *
   * <p>Note: Even if this returns true, there may be other reasons preventing the sitemap
   * configuration from being edited by the user.
   *
   ...
   *
   * <p>This is useful for test cases.
   */
   ...
   * <p>
   *
   * @param online true if the Online project formatters should be returned, false for the Offline
   *     formatters
  ...
  public String getDetailPage(
      CmsObject cms, String rootPath, String linkSource, String targetDetailPage) {
  ...
  public Map<String, CmsXmlContentProperty> getElementSettings(CmsObject cms, CmsResource resource)
    ...
    if (CmsResourceTypeXmlContent.isXmlContent(resource)) {
      Map<String, CmsXmlContentProperty> result =
  ...
  public Map<String, CmsXmlContentProperty> getFormatterSettings(
    ...
    if (mainFormatter != null) {
      for (Entry<String, CmsXmlContentProperty> entry :
          mainFormatter.getSettings(config).entrySet()) {
      ...
      if (mainFormatter.hasNestedFormatterSettings()) {
        List<I_CmsFormatterBean> nestedFormatters =
            getNestedFormatters(cms, config, res, locale, req);
        if (nestedFormatters != null) {
          for (I_CmsFormatterBean formatter : nestedFormatters) {
            for (Entry<String, CmsXmlContentProperty> entry :
                formatter.getSettings(config).entrySet()) {
              Visibility visibility = entry.getValue().getVisibility(defaultVisibility);
              switch (visibility) {
                case parentShared:
   ...
   */
  public CmsInheritedContainerState getInheritedContainerState(
      CmsObject cms, CmsResource resource, String name) {
    ...
    CmsContainerConfigurationCache cache =
        online ? m_onlineContainerConfigurationCache : m_offlineContainerConfigurationCache;
   ...
   */
  public CmsInheritedContainerState getInheritedContainerState(
      CmsObject cms, String rootPath, String name) throws CmsException {
  ...
  public List<I_CmsFormatterBean> getNestedFormatters(
      CmsObject cms, CmsADEConfigData config, CmsResource res, Locale locale, ServletRequest req) {
  ...
  public Map<String, String> getParameters(CmsObject cms) {
    ...
    if (cms != null) {
      String userParamsStr =
          (String) (cms.getRequestContext().getCurrentUser().getAdditionalInfo().get("ADE_PARAMS"));
  ...
  public CmsPermissionInfo getPermissionInfo(
      CmsObject cms, CmsResource resource, String contextPath) throws CmsException {
    ...
    if (hasView) {
      try {
        ...
        CmsExplorerTypeSettings settings =
            OpenCms.getWorkplaceManager().getExplorerTypeSetting(type.getTypeName());
        hasView =
            (settings == null)
        ...
        if (hasView
          ...
          CmsResourceTypeConfig localConfigData =
              lookupConfigurationWithCache(cms, contextPath).getResourceType(type.getTypeName());
          if (localConfigData != null) {
            Map<CmsUUID, CmsElementView> elementViews = getElementViews(cms);
            hasView =
                elementViews.containsKey(localConfigData.getElementView())
                    && elementViews
                        .get(localConfigData.getElementView())
                        .hasPermission(cms, resource);
          }
        }
    ...
    String noEdit =
        new CmsResourceUtil(cms, resource)
            .getNoEditReason(OpenCms.getWorkplaceManager().getWorkplaceLocale(cms), true);
   ...
   *
   * <p>This *only* includes plugins defined in site plugins active on the given path, not those
   * referenced in formatters.
   *
   * @param cms the CMS context
  ...
  public Map<String, List<CmsTemplatePluginWrapper>> getPluginsForPath(CmsObject cms, String path) {
    ...
    CmsADEConfigData config =
        lookupConfigurationWithCache(cms, cms.getRequestContext().addSiteRoot(path));

    Multimap<String, CmsTemplatePlugin> plugins =
        CmsTemplatePluginFinder.getActiveTemplatePluginsFromSitePlugins(config);
    Map<String, List<CmsTemplatePluginWrapper>> result = new HashMap<>();
    for (String key : plugins.keySet()) {
      List<CmsTemplatePluginWrapper> wrappers =
          plugins.get(key).stream()
              .map(plugin -> new CmsTemplatePluginWrapper(cms, plugin))
   ...
   *
   * <p>This needs to be in the ADE manager because the user for whom the HTML redirect is being
   * loaded does not necessarily have read permissions for the redirect target, so we read the
   * redirect target with admin privileges.
   *
   * <p>
   *
  ...
  public synchronized void initialize() {
    ...
    // no need to try initialization in case the 'org.opencms.base' is not present and the contained
    // resource types missing
    if ((m_initStatus == Status.notInitialized)
        && OpenCms.getModuleManager().hasModule(MODULE_NAME_ADE_CONFIG)) {
      try {
        ...
        m_onlineCache =
            new CmsConfigurationCache(
                m_onlineCms, m_configType, m_moduleConfigType, m_elementViewType);
        m_offlineCache =
            new CmsConfigurationCache(
                m_offlineCms, m_configType, m_moduleConfigType, m_elementViewType);
        ...
        m_onlineContainerConfigurationCache =
            new CmsContainerConfigurationCache(m_onlineCms, "online inheritance groups");
        m_offlineContainerConfigurationCache =
            new CmsContainerConfigurationCache(m_offlineCms, "offline inheritance groups");
        ...
        m_offlineFormatterCache =
            new CmsFormatterConfigurationCache(m_offlineCms, "offline formatters");
        m_onlineFormatterCache =
            new CmsFormatterConfigurationCache(m_onlineCms, "online formatters");
        ...
        CmsGlobalConfigurationCacheEventHandler handler =
            new CmsGlobalConfigurationCacheEventHandler(m_onlineCms);
        ...
        handler.addCache(
            m_offlineFormatterCache, m_onlineFormatterCache, "formatter configuration cache");
   ...
   *
   * <p>This is not a permission check, but an additional mechanism to prevent users from editing
   * configuration files even if they technically need or have write permissions for these files.
   *
   * @param cms the CMS context
  ...
  /**
   * Checks whether the ADE manager is initialized (this should usually be the case except during
   * the setup).
   *
   * <p>
   *
  ...
  /**
   * Looks up the configuration data for a given sitemap path, but uses a thread-local cache for the
   * current request for efficiency.
   *
   * @param cms the current CMS context
   * @param rootPath the root path for which the configuration data should be looked up
   * @return the configuration data
   ...
   *
   * <p>Normally you shouldn't call this directly since the event handlers take care of updating the
   * configuration.
   */
  public void refresh() {
   ...
   * @throws CmsException if something goes wrong
   */
  public boolean saveDetailPages(
      CmsObject cms, String rootPath, List<CmsDetailPageInfo> detailPages, CmsUUID newId)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void saveFavoriteList(CmsObject cms, List<CmsContainerElementBean> favoriteList)
      throws CmsException {
  ...
  public void waitForCacheUpdate(boolean online) {
    ...
    getCache(online)
        ...
        .enter(2 * CmsConfigurationCache.TASK_DELAY_MILLIS);
  }

  /**
   * Waits until the formatter cache has finished updating itself.
   *
   * <p>This method is only intended for use in test cases.
   *
   ...
   * @param online if true, the resource will be looked up in the online project ,else in the
   *     offline project
  ...
  private void saveElementList(
      CmsObject cms, List<CmsContainerElementBean> elementList, String listKey)
      throws CmsException {
    ...
    // do not store the template contexts, since dragging an element into the page which might be
    // invisible
    // doesn't make sense
