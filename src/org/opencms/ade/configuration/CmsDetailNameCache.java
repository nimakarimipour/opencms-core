 ...
 * <p>
 *
 * <p>Note that this cache may in some cases contain outdated structure ids for URL names, if an URL
 * name has been removed for a content but is not yet mapped to a different content.
 */
public class CmsDetailNameCache implements I_CmsGlobalConfigurationCache {
  ...
  /**
   * Checks if any updates are necessary and if so, performs them.
   *
   * <p>
   */
  ...
  /**
   * Checks if the cache needs to be updated for the resource, and if so, marks the structure id for
   * updating.
   *
   * <p>
   *
    ...
    try {
      ...
      if ((resType instanceof CmsResourceTypeXmlContent)
          && !OpenCms.getResourceManager()
              .matchResourceType(CmsResourceTypeXmlContainerPage.RESOURCE_TYPE_NAME, typeId)) {
  ...
  private Set<String> getUrlNames(CmsUUID id) {
    ...
    try {
      ...
      Set<String> result =
          m_cms.readUrlNameMappings(filter).stream()
  ...
  private void reload() {
    ...
    try {
      List<CmsUrlNameMappingEntry> mappings =
          m_cms.readUrlNameMappings(CmsUrlNameMappingFilter.ALL);
