 ...
 * configurations) at a certain instant in time.
 *
 * <p>
 */
public class CmsADEConfigCacheState {
  ...
  private Map<String, CmsADEConfigDataInternal> m_siteConfigurationsByPath =
      new HashMap<String, CmsADEConfigDataInternal>();

  /** The sitemap attribute editor configurations. */
  private Map<CmsUUID, CmsSitemapAttributeEditorConfiguration>
      m_sitemapAttributeEditorConfigurations;
  ...
  public CmsADEConfigCacheState(
    ...
    CacheBuilder<?, ?> detailPageCacheBuilder =
        CacheBuilder.newBuilder().concurrencyLevel(8).expireAfterWrite(30, TimeUnit.SECONDS);
  ...
  public Map<String, String> computeFolderTypes() throws CmsException {
    ...
    // do this first, since folder types from modules should be overwritten by folder types from
    // sitemaps
    if (m_moduleConfiguration != null) {
      folderTypes.putAll(wrap(m_moduleConfiguration).getFolderTypes());
    }

    List<CmsADEConfigDataInternal> configDataObjects =
        new ArrayList<CmsADEConfigDataInternal>(m_siteConfigurationsByPath.values());
  ...
  /**
   * Creates a new object which represents the changed configuration state given some updates,
   * without changing the current configuration state (this object instance).
   *
   * @param sitemapUpdates a map containing changed sitemap configurations indexed by structure id
   *     (the map values are null if the corresponding sitemap configuration is not valid or could
   *     not be found)
   * @param moduleUpdates the list of *all* module configurations, or null if no module
  ...
  public CmsADEConfigCacheState createUpdatedCopy(
    ...
    Map<CmsUUID, CmsADEConfigDataInternal> newSitemapConfigs =
        Maps.newHashMap(m_siteConfigurations);
    ...
    Map<CmsUUID, CmsSitemapAttributeEditorConfiguration> newAttributeEditorConfigs =
        m_sitemapAttributeEditorConfigurations;
  ...
  /**
   * Gets the sitemap attribute editor configuration with the given id (or null, if there isn't
   * one).
   *
  ...
  /**
   * Helper method to retrieve the parent folder type or <code>null</code> if none available.
   *
   * <p>
   *
  ...
  public List<String> getSubsitesForSiteSelector() {
    ...
    if (m_subsitesForSiteSelector == null) {
      List<String> paths =
          m_siteConfigurations.values().stream()
              .filter(conf -> conf.isIncludeInSiteSelector())
              .map(conf -> conf.getBasePath())
  ...
  /**
   * Helper method for getting the best matching sitemap configuration object for a given root path,
   * ignoring the module configuration.
   *
   * <p>For example, if there are configurations available for the paths /a, /a/b/c, /a/b/x and
   * /a/b/c/d/e, then the method will return the configuration object for /a/b/c when passed the
   * path /a/b/c/d.
   *
   * <p>If no configuration data is found for the path, null will be returned.
   *
   * <p>
   *
    ...
    if (CmsResourceTypeXmlContainerPage.isContainerPage(resource)) {
      ...
      try {
        CmsResource parent =
            getCms()
                .readResource(
                    CmsResource.getParentFolder(resource.getRootPath()), CmsResourceFilter.ALL);
   ...
   */
  protected CmsADEConfigDataInternal mergeConfigurations(
      List<CmsADEConfigDataInternal> configurations) {
  ...
  /**
   * Internal method for collecting structure ids of all configured detail pages and their parent
   * folders.
   *
  ...
  private Set<CmsUUID> collectDetailPageIds() {
    ...
    for (CmsDetailPageInfo detailPageInfo : allDetailPages) {
      try {
        CmsResource detailPageRes =
            getCms().readResource(detailPageInfo.getId(), CmsResourceFilter.ALL);
  ...
  /**
   * For a given master configuration, lists all directly and indirectly referenced master
   * configurations, in sitemap config inheritance order (i.e. referenced master configurations
   * preceding the configurations from which they are referenced).
   *
  ...
  /**
   * Wraps the internal config data into a bean which manages the lookup of inherited
   * configurations.
   *
   * <p>
   *
      ...
      for (String siteConfigPath : siteConfigPaths) {
        ...
        for (CmsUUID id : masterConfigs) {
          ...
          if (masterConfig != null) {
            List<CmsADEConfigDataInternal> masterConfigWithDependencies =
                fillInReferencedMasterConfigurations(masterConfig);
