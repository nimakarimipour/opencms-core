...
public class CmsFormatterBeanParser {
  ...
  /**
   * Additional setting configurations for includes. Entries consist of structure ids of setting
   * definition files as keys and the corresponding setting definition maps as entries.
   */
   ...
   *
   * <p>A new parser instance should be created for every formatter configuration you want to parse.
   *
   * <p>
   *
  ...
  public CmsFormatterBeanParser(
      CmsObject cms, Map<CmsUUID, Map<CmsSharedSettingKey, CmsXmlContentProperty>> settingConfigs) {
  ...
  public I_CmsFormatterBean parse(CmsXmlContent content, @RUntainted String location, String id)
    ...
    boolean isMacroFromatter =
        CmsFormatterConfigurationCache.TYPE_MACRO_FORMATTER.equals(type.getTypeName());
    boolean isFlexFormatter =
        CmsFormatterConfigurationCache.TYPE_FLEX_FORMATTER.equals(type.getTypeName());
    boolean isFunction =
        OpenCms.getResourceManager()
            .matchResourceType(
                CmsResourceTypeFunctionConfig.TYPE_NAME, content.getFile().getTypeId());
    ...
    settingIncludes =
        Lists.reverse(
            settingIncludes); // make defaults from earlier include files 'win' when merging them
                              // into a map
    ...
    CmsSettingConfiguration settingConfig =
        new CmsSettingConfiguration(
            m_settingList, m_additionalSettingConfigs, includeIds, key, displayType);
    ...
    String useMetaMappinsForNormalElementsStr =
        getString(root, N_USE_META_MAPPINGS_FOR_NORMAL_ELEMENTS, "false");
    boolean useMetaMappingsForNormalElements =
        Boolean.parseBoolean(useMetaMappinsForNormalElementsStr);
    ...
    if (isMacroFromatter || isFlexFormatter) {
      ...
      if (link == null) {
        if (isFunction) {
          ...
          // JSP link is not set (for example because the formatter configuration has just been
          // created)
          LOG.info(
              "JSP link is null in formatter configuration: " + content.getFile().getRootPath());
          return null;
        }
      } else {
      ...
      if (isFunction) {
        CmsResource functionFormatter =
            m_cms.readResource(CmsResourceTypeFunctionConfig.FORMATTER_PATH);
        Map<String, String[]> rparams = parseParams(root);
        formatterBean =
            new CmsFunctionFormatterBean(
                m_containerTypes,
                m_formatterResource.getRootPath(),
                m_formatterResource.getStructureId(),
                m_key,
  ...
  private void parseHeadIncludes(I_CmsXmlContentLocation formatterLoc) {
    ...
    if (headIncludeJs != null) {
      for (I_CmsXmlContentValueLocation inlineJsLoc :
          headIncludeJs.getSubValues(N_JAVASCRIPT_INLINE)) {
  ...
  /**
   * Parses the matching criteria (container types or widths) for the formatter.
   *
   * <p>
   *
   ...
   * @throws ParseException if parsing goes wrong
   */
  private void parseMatch(I_CmsXmlContentLocation linkFormatterLoc, boolean strict)
  ...
  /**
   * Parse parameters and put them in a map.
   *
   * <p>
   *
  ...
  /**
   * Reads the referenced formatters.
   *
   * <p>
   *
  ...
  private Map<String, CmsUUID> readReferencedFormatters(CmsXmlContent xmlContent) {
    ...
    List<I_CmsXmlContentValue> formatters =
        xmlContent.getValues(
            CmsMacroFormatterResolver.N_FORMATTERS, CmsLocaleManager.MASTER_LOCALE);
    for (I_CmsXmlContentValue formatterValue : formatters) {
      CmsXmlVfsFileValue file =
          (CmsXmlVfsFileValue)
              xmlContent.getValue(
