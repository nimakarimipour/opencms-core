 ...
 *
 * <p>This class does not immediately update the cached formatter collection when changes in the VFS
 * occur, but instead schedules an update action with a slight delay, so that if many formatters are
 * changed in a short time, only one update operation is needed.
 *
 * <p>Two instances of this cache are needed, one for the Online project and one for Offline
 * projects.
 *
 * <p>
 */
public class CmsFormatterConfigurationCache implements I_CmsGlobalConfigurationCache {
  ...
  /**
   * The current data contained in the formatter cache.
   *
   * <p>This field is reassigned when formatters are changed, but the objects pointed to by this
   * field are immutable.
   *
   * <p>*
   */
  private volatile CmsFormatterConfigurationCacheState m_state =
      new CmsFormatterConfigurationCacheState(Collections.<CmsUUID, I_CmsFormatterBean>emptyMap());
  ...
  public void performUpdate() {
    ...
    // Wrap everything in try-catch because we don't want to leak an exception out of a scheduled
    // task
    try {
  ...
  public void reload() {
    ...
    Map<CmsUUID, Map<CmsSharedSettingKey, CmsXmlContentProperty>> sharedSettingsByStructureId =
        new HashMap<>();
    for (CmsResource resource : settingConfigResources) {
      Map<CmsSharedSettingKey, CmsXmlContentProperty> sharedSettings =
          parseSettingsConfig(resource);
    ...
    try {
      ...
      CmsResourceFilter filter = CmsResourceFilter.ONLY_VISIBLE_NO_DELETED.addRequireType(type);
      formatterResources.addAll(m_cms.readResources("/", filter));
      I_CmsResourceType typeFlex =
          OpenCms.getResourceManager().getResourceType(TYPE_FLEX_FORMATTER);
      CmsResourceFilter filterFlex =
          CmsResourceFilter.ONLY_VISIBLE_NO_DELETED.addRequireType(typeFlex);
      formatterResources.addAll(m_cms.readResources("/", filterFlex));
      I_CmsResourceType typeFunction =
          OpenCms.getResourceManager().getResourceType(CmsResourceTypeFunctionConfig.TYPE_NAME);
      CmsResourceFilter filterFunction =
          CmsResourceFilter.ONLY_VISIBLE_NO_DELETED.addRequireType(typeFunction);
  ...
  /**
   * @see
   *     org.opencms.ade.configuration.I_CmsGlobalConfigurationCache#remove(org.opencms.db.CmsPublishedResource)
   */
  ...
  /**
   * @see
   *     org.opencms.ade.configuration.I_CmsGlobalConfigurationCache#remove(org.opencms.file.CmsResource)
   */
  ...
  /**
   * @see
   *     org.opencms.ade.configuration.I_CmsGlobalConfigurationCache#update(org.opencms.db.CmsPublishedResource)
   */
  ...
  /**
   * @see
   *     org.opencms.ade.configuration.I_CmsGlobalConfigurationCache#update(org.opencms.file.CmsResource)
   */
  ...
  /**
   * Reads a formatter given its structure id and returns it, or null if the formatter couldn't be
   * read.
   *
   * <p>
   *
  ...
  protected I_CmsFormatterBean readFormatter(CmsUUID structureId) {
    ...
    try {
      ...
      formatterBean =
          parser.parse(content, formatterRes.getRootPath(), "" + formatterRes.getStructureId());
    } catch (Exception e) {
  ...
  /**
   * Checks if an update of the formatter is needed and if so, adds its structure id to the update
   * set.
   *
   * <p>
   *
  ...
  private void checkIfUpdateIsNeeded(CmsUUID structureId, @RUntainted String path, int resourceType) {
    ...
    if (manager.matchResourceType(TYPE_SETTINGS_CONFIG, resourceType)) {
      ...
      // the reference to the settings config. So we need to reload everything when a setting
      // configuration
      // changes.
      markForUpdate(RELOAD_MARKER);
      return;
  ...
  /**
   * Adds a formatter structure id to the update set, and schedule an update task unless one is
   * already scheduled.
   *
   * <p>
   *
   ...
   *
   * <p>If a setting definition contains formatter keys, then one entry for each formatter key will
   * be added to the result map, otherwise just one general map entry with formatterKey = null will
   * be generated for that setting.
   *
   * @param resource the resource to parse
   * @return the parsed setting definitions
   */
  private Map<CmsSharedSettingKey, CmsXmlContentProperty> parseSettingsConfig(
    ...
    try {
      ...
      CmsXmlContentRootLocation location =
          new CmsXmlContentRootLocation(settingContent, Locale.ENGLISH);
      for (I_CmsXmlContentValueLocation settingLoc :
          location.getSubValues(CmsFormatterBeanParser.N_SETTING)) {
        CmsXmlContentProperty setting =
            CmsConfigurationReader.parseProperty(m_cms, settingLoc).getPropertyData();
        ...
        for (I_CmsXmlContentValueLocation formatterKeyLoc :
            settingLoc.getSubValues(N_FORMATTER_KEY)) {
