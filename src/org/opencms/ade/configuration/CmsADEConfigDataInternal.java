 ...
 *
 * <p>This is the internal representation stored in the cache. The configuration class which is
 * actually returned by CmsADEManager, and which contains most of the logic related to sitemap
 * configurations, is CmsADEConfigData.
 */
public class CmsADEConfigDataInternal {

  /**
   * Represents the value of an attribute, with additional information about where the value
   * originated from.
   */
  ...
  /**
   * True if detail contents outside the sitemap should not be used with detail pages in the
   * sitemap.
   */
  ...
  /**
   * True if detail pages from this sitemap should be preferred when linking to contents inside this
   * sitemap.
   */
   ...
   * @param removeSharedSettingOverrides true if inherited shared setting overrides should be
   *     removed
  ...
  /**
   * Returns the set of configured dynamic functions, regardless of whether the 'remove all
   * formatters' option is enabled.
   *
    ...
    if (m_ownModelPageConfig == null) {
      ...
      for (CmsModelPageConfigWithoutResource modelPage : m_ownModelPageConfigRaw) {
        try {
          ...
          result.add(
              new CmsModelPageConfig(resource, modelPage.isDefault(), modelPage.isDisabled()));
        } catch (Exception e) {
          LOG.warn(
              "can't read model page for base path " + m_basePath + ": " + e.getLocalizedMessage(),
              e);
        }
      }
   ...
   *
   * <p>If true, container pages will be written in the new format, including using formatter keys
   * when possible.
   *
   ...
   * <p>
   *
   * @return true if inherited property configurations should be discardded.
   *     <p>
   */
  ...
  /**
   * Returns true if detail pages inside this subsite (and descendant subsites) should not be used
   * for contents outside the subsite (and descendant subsites).
   *
   * <p>
   *
  ...
  /**
   * Returns true if detail pages from this sitemap should be preferred for creating links to detail
   * contents located inside this sitemap.
   *
   * <p>
   *
  ...
  /**
   * Returns true if shared setting overrides inherited from other sitemap configurations should be
   * discarded.
   *
  ...
  protected void mergeParent(CmsADEConfigDataInternal parent) {
    ...
    m_ownResourceTypes =
        CmsADEConfigData.combineConfigurationElements(parentTypes, m_ownResourceTypes, false);
    m_ownPropertyConfigurations =
        CmsADEConfigData.combineConfigurationElements(
            parentProperties, m_ownPropertyConfigurations, false);
    m_ownModelPageConfig =
        CmsADEConfigData.combineConfigurationElements(
            parentModelPages, m_ownModelPageConfig, false);
    m_functionReferences =
        CmsADEConfigData.combineConfigurationElements(
            parentFunctionRefs, m_functionReferences, false);
  ...
  protected void processModuleOrdering() {
    ...
    Collections.sort(
        m_ownResourceTypes,
        new Comparator<CmsResourceTypeConfig>() {

          public int compare(CmsResourceTypeConfig a, CmsResourceTypeConfig b) {

            return ComparisonChain.start()
                .compare(a.getOrder(), b.getOrder())
                .compare(a.getTypeName(), b.getTypeName())
                .result();
          }
        });
    ...
    Collections.sort(
        m_ownPropertyConfigurations,
        new Comparator<CmsPropertyConfig>() {
    ...
    Collections.sort(
        m_functionReferences,
        new Comparator<CmsFunctionReference>() {
