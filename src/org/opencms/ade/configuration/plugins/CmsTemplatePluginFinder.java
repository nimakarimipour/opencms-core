...
public class CmsTemplatePluginFinder {
   ...
   */
  public static Multimap<String, CmsTemplatePlugin> getActiveTemplatePluginsFromSitePlugins(
  ...
  /**
   * Collects the referenced plugins from the current page (from container elements, detail
   * elements, etc.).
   *
   ...
   */
  private static List<CmsTemplatePlugin> collectPluginsForCurrentPage(
      CmsJspStandardContextBean standardContext) throws CmsException {
    ...
    if (standardContext.getDetailContentId() != null) {
      try {
        ...
        CmsFormatterConfiguration detailContentFormatters =
            config.getFormatters(cms, detailContent);
   ...
   */
  private static Multimap<String, CmsTemplatePlugin> getActivePlugins(
    ...
    for (String group : pluginsByGroup.keySet()) {
      List<CmsTemplatePlugin> active =
          sortAndDeduplicatePlugins((List<CmsTemplatePlugin>) (pluginsByGroup.get(group)));
   ...
   */
  private static List<CmsTemplatePlugin> sortAndDeduplicatePlugins(
      List<CmsTemplatePlugin> plugins) {
  ...
  /**
   * Gets the multimap of plugins for the current page by group, with each group sorted and
   * de-duplicated.
   *
