...
public final class CmsDetailOnlyContainerUtil {
   ...
   */
  public static String getDetailContainerLocale(
      CmsObject cms, String contentLocale, CmsResource resource) {
      ...
      try {
        CmsProperty prop =
            cms.readPropertyObject(
                resource, CmsPropertyDefinition.PROPERTY_LOCALE_INDEPENDENT_DETAILS, true);
  ...
  public static String getDetailContentPath(@RUntainted String detailContainersPage) {
    ...
    String parentFolder =
        CmsResource.getParentFolder(CmsResource.getParentFolder(detailContainersPage));
  ...
  public static Optional<CmsResource> getDetailOnlyPage(
      CmsObject cms, CmsResource detailContent, String contentLocale) {

    try {
      CmsObject rootCms = OpenCms.initCmsObject(cms);
      rootCms.getRequestContext().setSiteRoot("");
      String path =
          getDetailOnlyPageNameWithoutLocaleCheck(detailContent.getRootPath(), contentLocale);
  ...
  /**
   * Returns the detail only container page bean or <code>null</code> if none available.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  public static CmsContainerPageBean getDetailOnlyPage(
      CmsObject cms, ServletRequest req, String pageRootPath) {
   ...
   * @param req the current request
   * @param pageRootPath the root path of the page
   * @param lookupContextFirst flag, indicating if the bean should be looked up in the standard
   *     context first.
   * @return the container page bean
   */
  public static CmsContainerPageBean getDetailOnlyPage(
      CmsObject cms, ServletRequest req, String pageRootPath, boolean lookupContextFirst) {

    CmsJspStandardContextBean standardContext = CmsJspStandardContextBean.getInstance(req);
    CmsContainerPageBean detailOnlyPage =
        lookupContextFirst ? standardContext.getDetailOnlyPage() : null;
      ...
      try {
        ...
        String resourceName =
            getDetailOnlyPageNameWithoutLocaleCheck(
                standardContext.getDetailContent().getRootPath(), locale);
        ...
        if (rootCms.existsResource(resourceName)) {
          ...
          resourceName =
              getDetailOnlyPageNameWithoutLocaleCheck(
                  standardContext.getDetailContent().getRootPath(), null);
          if (rootCms.existsResource(resourceName)) {
            resource = rootCms.readResource(resourceName);
          }
        }
        ...
        if (xmlContainerPage != null) {
          ...
          detailOnlyPage =
              CmsTemplateMapper.get(req)
                  .transformContainerpageBean(rootCms, detailOnlyPage, pageRootPath);
  ...
  /**
   * Returns the site/root path to the detail only container page, for site/root path of the detail
   * content.
   *
   * <p>
   *
   ...
   * @return the site or root path to the detail only container page (dependent on providing site or
   *     root path for the detailPath).
   */
  public static @RUntainted String getDetailOnlyPageName(
      CmsObject cms, CmsResource pageResource, @RUntainted String detailPath, String locale) {

    return getDetailOnlyPageNameWithoutLocaleCheck(
        detailPath, getDetailContainerLocale(cms, locale, pageResource));
  }
  ...
  public static Optional<CmsResource> getDetailOnlyResource(
      CmsObject cms, String contentLocale, CmsResource detailContentRes, CmsResource pageRes) {

    Optional<CmsResource> detailOnlyRes =
        getDetailOnlyPage(
            cms, detailContentRes, getDetailContainerLocale(cms, contentLocale, pageRes));
  ...
  public static boolean isDetailContainersPage(CmsObject cms, @RUntainted String detailContainersPage) {
    ...
    try {
      ...
      result =
          parentFolder.endsWith("/" + DETAIL_CONTAINERS_FOLDER_NAME + "/")
  ...
  /**
   * Creates an empty detail-only page for a content, or just reads the resource if the detail-only
   * page already exists.
   *
   * <p>
   *
   ...
   */
  public static CmsResource readOrCreateDetailOnlyPage(
      CmsObject cms, CmsUUID detailId, @RUntainted String detailOnlyRootPath) throws CmsException {
    ...
    if (rootCms.existsResource(detailOnlyRootPath)) {
      ...
      containerpage =
          rootCms.createResource(
              detailOnlyRootPath,
              OpenCms.getResourceManager()
                  .getResourceType(CmsResourceTypeXmlContainerPage.getStaticTypeName()));
    }
    ...
    try {
      ...
      @RUntainted String title =
          cms.readPropertyObject(detailResource, CmsPropertyDefinition.PROPERTY_TITLE, true)
              .getValue();
      if (title != null) {
        title =
            Messages.get()
                .getBundle(OpenCms.getWorkplaceManager().getWorkplaceLocale(cms))
                .key(Messages.GUI_DETAIL_CONTENT_PAGE_TITLE_1, title);
      ...
      List<CmsRelation> relations =
          cms.readRelations(
              CmsRelationFilter.relationsFromStructureId(detailId)
                  .filterType(CmsRelationType.DETAIL_ONLY));
      ...
      if (!hasRelation) {
        ...
        try {
          ...
          cms.addRelationToResource(
              detailResource, containerpage, CmsRelationType.DETAIL_ONLY.getName());
        } finally {
   ...
   *
   * <p>If the detail-only page already exists, it is overwritten.
   *
   * @param cms the current CMS context
  ...
  public static CmsXmlContainerPage saveDetailOnlyPage(
      CmsObject cms, CmsResource content, String locale, CmsContainerPageBean page)
      throws CmsException {

    @RUntainted String detailOnlyPath = getDetailOnlyPageNameWithoutLocaleCheck(content.getRootPath(), locale);
    CmsResource resource =
        readOrCreateDetailOnlyPage(cms, content.getStructureId(), detailOnlyPath);
    CmsXmlContainerPage xmlCntPage =
        CmsXmlContainerPageFactory.unmarshal(cms, cms.readFile(resource));
   ...
   * @return <code>true</code> if single locale detail containers should be used for the given site
   *     root
   */
  public static boolean useSingleLocaleDetailContainers(@RUntainted String siteRoot) {
    ...
    if ((siteRoot != null)
      ...
      result =
          (site != null) && CmsSite.LocalizationMode.singleTree.equals(site.getLocalizationMode());
    }
    return result;
  }
   ...
   *
   * <p>This does not perform any further checks regarding the locale and assumes that all these
   ...
   */
  static @RUntainted String getDetailOnlyPageNameWithoutLocaleCheck(
      @RUntainted String detailContentSitePath, String contentLocale) {
    ...
    if (contentLocale != null) {
      ...
      result =
          CmsStringUtil.joinPaths(
              result, DETAIL_CONTAINERS_FOLDER_NAME, CmsResource.getName(detailContentSitePath));
    }
    return result;
  }
}
