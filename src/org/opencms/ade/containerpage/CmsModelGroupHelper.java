...
public class CmsModelGroupHelper {
  ...
  private static final String[] KEEP_SETTING_IDS =
      new String[] {
   ...
   */
  public static CmsResource createModelGroup(CmsObject cms, CmsADEConfigData configData)
      throws CmsException {

    CmsResourceTypeConfig typeConfig =
        configData.getResourceType(CmsResourceTypeXmlContainerPage.MODEL_GROUP_TYPE_NAME);
   ...
   *
   * <p>This step is necessary when updating from version 10.0.x to 10.5.x.
   *
   * <p>
   *
   ...
   */
  public static boolean updateModelGroupResource(
      CmsObject cms, CmsResource group, String baseContainerName) {
  ...
  public static void updateModelGroupResources(
    ...
    if (CmsFlexController.isCmsRequest(request)) {
      ...
      try {
        ...
        I_CmsResourceType groupType =
            OpenCms.getResourceManager()
                .getResourceType(CmsResourceTypeXmlContainerPage.MODEL_GROUP_TYPE_NAME);
        if (base.isFolder()) {
          resources =
              cms.readResources(
                  basePath, CmsResourceFilter.ONLY_VISIBLE_NO_DELETED.addRequireType(groupType));
        ...
        if (resources.isEmpty()) {
          ...
          for (CmsResource group : resources) {
            ...
            response
                .getWriter()
                .println("Group '" + group.getRootPath() + "' was updated " + updated + "<br />");
          }
        }
      } catch (CmsException e) {
        LOG.error(e.getLocalizedMessage(), e);
   ...
   * @param page the page
   * @param alwaysCopy <code>true</code> to create element copies in case of non model groups and
   *     createNew is set
    ...
    for (Entry<String, CmsContainerElementBean> entry : elements.entrySet()) {
      ...
      if (CmsModelGroupHelper.isModelGroupResource(element.getResource())) {
        ...
        List<CmsContainerBean> modelContainers =
            readModelContainers(
                baseInstanceId, originalInstanceId, modelPage, baseElement.isCopyModel());
        ...
        // here we need to make sure to remove the source container page setting and to set a new
        // element instance id
          ...
          try {
            ...
            if (CmsResourceTypeXmlContainerPage.isContainerPage(sourcePage)) {
              CmsXmlContainerPage xmlCnt =
                  CmsXmlContainerPageFactory.unmarshal(m_cms, m_cms.readFile(sourcePage));
                ...
                if (container.getParentInstanceId() != null) {
                  if (!containerByParent.containsKey(container.getParentInstanceId())) {
                    containerByParent.put(
                        container.getParentInstanceId(), new ArrayList<CmsContainerBean>());
                  }
                    ...
                    if (modelInstanceId == null) {
                      if (child.getId().equals(element.getId())) {
                        ...
                        Map<String, String> setting =
                            new HashMap<String, String>(child.getIndividualSettings());
              ...
              if (foundInstance && containerByParent.containsKey(modelInstanceId)) {
                List<CmsContainerBean> modelContainers =
                    collectModelStructure(
                        modelInstanceId, element.getInstanceId(), containerByParent, true);
                if (alwaysCopy) {
                  modelContainers = createNewElementsForModelGroup(m_cms, modelContainers, locale);
                }
        ...
        try {
          ...
          if (isModelGroupResource(element.getResource())) {
            ...
            CmsContainerElementBean baseElement =
                getModelBaseElement(modelGroupPage, element.getResource());
            if (baseElement == null) {
              LOG.error(
            ...
            String baseInstanceId = baseElement.getInstanceId();
            CmsContainerElementBean replaceElement =
                getModelReplacementElement(element, baseElement, false);
            if (m_sessionCache != null) {
      ...
      if ((container.getParentInstanceId() == null)
        ...
        for (CmsContainerElementBean element : container.getElements()) {
          if (modelInstances.contains(element.getInstanceId())) {
            ...
            CmsContainerElementBean replacer =
                new CmsContainerElementBean(
                    modelId, element.getFormatterId(), element.getIndividualSettings(), false);
      ...
      for (CmsContainerElementBean element : container.getElements()) {
        if (element.isModelGroup() && !element.getId().equals(modelElementId)) {
          ...
          Map<String, String> settings =
              new HashMap<String, String>(element.getIndividualSettings());
          settings.remove(CmsContainerElement.MODEL_GROUP_ID);
          settings.remove(CmsContainerElement.MODEL_GROUP_STATE);
          elements.add(
              new CmsContainerElementBean(
                  element.getId(), element.getFormatterId(), settings, false));
          hasChanges = true;
        } else {
          elements.add(element);
        }
  ...
  private CmsContainerElementBean adjustSettings(
    ...
    String formatterId =
        settings.remove(CmsFormatterConfig.getSettingsKeyForContainer(originalContainer));
  ...
  private Set<String> collectDescendingInstances(
      String instanceId, Map<String, List<CmsContainerBean>> containersByParent) {
    ...
    if (containersByParent.containsKey(instanceId)) {
      for (CmsContainerBean container : containersByParent.get(instanceId)) {
        for (CmsContainerElementBean element : container.getElements()) {
          descendingInstances.addAll(
              collectDescendingInstances(element.getInstanceId(), containersByParent));
        }
      }
    }
  ...
  private List<CmsContainerBean> collectModelStructure(
    ...
    if (containerByParent.containsKey(modelInstanceId)) {
      for (CmsContainerBean container : containerByParent.get(modelInstanceId)) {
        String adjustedContainerName =
            replaceModelId + container.getName().substring(modelInstanceId.length());

        List<CmsContainerElementBean> elements = new ArrayList<CmsContainerElementBean>();
        for (CmsContainerElementBean element : container.getElements()) {
          CmsContainerElementBean copyElement =
              adjustSettings(element, container.getName(), adjustedContainerName, isCopyGroup);
          if (m_sessionCache != null) {
  ...
  private List<CmsContainerBean> createNewElementsForModelGroup(
      CmsObject cms, List<CmsContainerBean> modelContainers, Locale locale) throws CmsException {
  ...
  private CmsContainerPageBean getContainerPageBean(CmsResource resource) throws CmsException {
    ...
    CmsXmlContainerPage xmlCnt =
        CmsXmlContainerPageFactory.unmarshal(m_cms, m_cms.readFile(resource));
  ...
  private CmsContainerElementBean getModelBaseElement(
      CmsContainerPageBean modelGroupPage, CmsResource modelGroupResource) {

    CmsContainerElementBean result = null;
    for (CmsContainerElementBean element : modelGroupPage.getElements()) {
      if (CmsContainerElement.ModelGroupState.isModelGroup
          .name()
          .equals(element.getIndividualSettings().get(CmsContainerElement.MODEL_GROUP_STATE))) {
        result = element;
        break;
      }
    }
      ...
      resetSettings =
          (formatter == null)
              || !formatter
                  ...
                  .contains(
                      OpenCms.getResourceManager()
                          .getResourceType(baseElement.getResource())
                          .getTypeName());
    }
    Map<String, String> settings;
    if (resetSettings) {
      ...
      if (!(baseElement.isCopyModel() && allowCopyModel)) {
        ...
        if (allowCopyModel) {
          ...
          for (Entry<String, String> settingEntry :
              baseElement.getIndividualSettings().entrySet()) {
            if (!settings.containsKey(settingEntry.getKey())) {
              settings.put(settingEntry.getKey(), settingEntry.getValue());
            }
          }
  ...
  private boolean hasIncompatibleFormatters(
      CmsContainerElementBean baseElement, CmsContainerElementBean element) {
    ...
    Set<String> baseFormatterKeys =
        CmsFormatterUtils.getAllFormatterKeys(m_configData, baseElement);
  ...
  private List<CmsContainerBean> readModelContainers(
    ...
    if (containerByParent.containsKey(modelInstanceId)) {
      modelContainers =
          collectModelStructure(modelInstanceId, localInstanceId, containerByParent, isCopyGroup);
    } else {
