...
public class CmsContainerConfigurationCache implements I_CmsGlobalConfigurationCache {
  ...
  private volatile CmsContainerConfigurationCacheState m_state =
      new CmsContainerConfigurationCacheState(new ArrayList<CmsContainerConfigurationGroup>());
  ...
  public void update(CmsPublishedResource resource) {
    ...
    update(
        resource.getStructureId(), resource.getRootPath(), resource.getType(), resource.getState());
  }
   ...
   *
   * <p>E.g. the result for the input '/sites/default/.container-config' will be '/sites/default'.
   *
   * <p>
   *
  ...
  /**
   * Checks whether a given combination of path and resource type belongs to an inherited container
   * configuration file.
   *
   * <p>
   *
   ...
   * @return true if the given root path / type combination matches an inherited container
   *     configuration file
   */
  protected boolean isContainerConfiguration(String rootPath, int type) {
    ...
    return OpenCms.getResourceManager()
            .matchResourceType(
                CmsResourceTypeXmlContainerPage.INHERIT_CONTAINER_CONFIG_TYPE_NAME, type)
   ...
   *
   * <p>If the configuration for a given resource can't be read, the corresponding map entry will be
   * null in the result.
   *
   * @param resources the resources
   * @return the inheritance group configurations, with structure ids of the corresponding resources
   *     as keys
   */
   ...
   *
   * <p>If the configuration for a given id can't be read, the corresponding map entry will be null
   * in the result.
   *
   ...
   *     as keys
   */
  protected Map<CmsUUID, CmsContainerConfigurationGroup> loadFromIds(
