...
public class CmsInheritedContainerState {
  ...
  private List<CmsContainerConfiguration> m_parentConfigurations =
      new ArrayList<CmsContainerConfiguration>();
  ...
  /**
   * Reads the configurations for a root path and its parents from a cache instance and adds them to
   * this state.
   *
   * <p>
   *
   ...
   *
   * <p>The container element beans returned will have additional information available via the
   * getInheritanceInfo method.
   *
   * <p>
   *
  ...
  public List<CmsContainerElementBean> getElements(boolean includeHidden) {
    ...
    Map<String, CmsContainerElementBean> elementsByKey =
    ...
    CmsContainerConfiguration lastElement =
        m_parentConfigurations.get(m_parentConfigurations.size() - 1);
    Set<String> newKeys = new HashSet<String>();
    for (Map.Entry<String, CmsContainerElementBean> entry :
        lastElement.getNewElements().entrySet()) {
    ...
    for (String key : ordering) {
      ...
      if ((element != null) && !keysUsed.contains(key)) {
        CmsContainerElementBean elementToAdd =
            CmsContainerElementBean.cloneWithSettings(element, element.getIndividualSettings());
  ...
  /**
   * Checks whether an element with the given key is actually defined in this inherited container
   * state.
   *
   * <p>
   *
  ...
  /**
   * Computes the visibility for an element.
   *
   * <p>
   *
   * @param visibilities the visibilities for the element in the sequence of parent configurations.
   *     <p>
