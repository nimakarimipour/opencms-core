...
public class CmsContainerConfigurationWriter {
    ...
    CmsInheritedContainerState state =
        OpenCms.getADEManager()
            .getInheritedContainerState(
                cms, CmsResource.getParentFolder(CmsResource.getParentFolder(configPath)), name);
  ...
  public Element serializeSingleConfiguration(
      CmsObject cms, String name, CmsContainerConfiguration config, Element parentElement)
      throws CmsException {
    ...
    for (Map.Entry<String, CmsContainerElementBean> entry : config.getNewElements().entrySet()) {
      ...
      Map<String, CmsXmlContentProperty> settingConfiguration =
          getSettingConfiguration(cms, elementBean.getResource());
      ...
      CmsXmlContentPropertyHelper.saveProperties(
          cms, elementElement, settings, settingConfiguration, true);
    }
    return root;
  }

  /**
   * Converts a list of container elements into a bean which should be saved to the inherited
  ...
  protected CmsContainerConfiguration createConfigurationBean(
      boolean newOrdering, List<CmsContainerElementBean> elements, Set<String> parentKeys) {
    ...
    CmsContainerConfiguration configuration =
        new CmsContainerConfiguration(ordering, visibility, newElements);
   ...
   * @throws CmsException if something goes wrong
   */
  protected Map<String, CmsXmlContentProperty> getSettingConfiguration(
   ...
   *
   * <p>This does nothing if no such entry actually exists.
   *
   * <p>
   *
   * @param cms the current CMS context
   ...
   */
  protected void removeExistingEntry(
      CmsObject cms, CmsXmlContent content, Locale locale, String name) {
    ...
    for (I_CmsXmlContentValue value : values) {
      ...
      I_CmsXmlContentValue nameValue =
          content.getValue(CmsXmlUtils.concatXpath(valueXpath, N_NAME), locale);
  ...
  /**
   * Saves a single container configuration in an XML content object, but doesn't write it to the
   * VFS.
   *
   * <p>If the XML content passed as a parameter is null, a new XML content object will be created
   *
   * @param cms the current CMS context
   * @param content the XML content
