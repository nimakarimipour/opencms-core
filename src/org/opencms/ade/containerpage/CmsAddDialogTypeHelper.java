...
public class CmsAddDialogTypeHelper {
  ...
  public List<CmsResourceTypeBean> getResourceTypes(
    ...
    if (elementView.getExplorerType() != null) {
      List<CmsExplorerTypeSettings> explorerTypes =
          OpenCms.getWorkplaceManager()
              .getExplorerTypesForView(elementView.getExplorerType().getName());
    ...
    return internalGetResourceTypesFromConfig(
        cms, folderRootPath, checkViewableReferenceUri, elementView, additionalTypes, checkEnabled);
  }
  ...
  public void precomputeTypeLists(
    ...
    // Sort list to make sure that 'Other types' view is processed last, because we may need to
    // display
    // types filtered / removed from other views, which we only know once we have processed these
    // views
    Collections.sort(
        views,
        new Comparator<CmsElementView>() {
          ...
          public int compare(CmsElementView view0, CmsElementView view1) {
            ...
            return ComparisonChain.start()
                .compareFalseFirst(view0.isOther(), view1.isOther())
                .result();
          }
        });
  ...
  private List<CmsResourceTypeBean> internalGetResourceTypesFromConfig(
    ...
    for (CmsResourceTypeConfig typeConfig : config.getResourceTypes()) {
      ...
      boolean isModelGroup =
          CmsResourceTypeXmlContainerPage.MODEL_GROUP_TYPE_NAME.equals(typeConfig.getTypeName());
      try {
        AddMenuVisibility visibility =
            typeConfig.getAddMenuVisibility(elementView.getId(), m_menuType);
    ...
    srv.setCms(cms);
    // we put the types 'imported' from other views at the end of the list. Since the sort is
    // stable,
    // relative position of other types remains unchanged
    Collections.sort(
        resourceTypes,
        new Comparator<I_CmsResourceType>() {
    ...
    Collections.sort(
        ...
        new Comparator<I_CmsResourceType>() {
          ...
          public int compare(I_CmsResourceType type1, I_CmsResourceType type2) {
            ...
            CmsExplorerTypeSettings settings1 =
                OpenCms.getWorkplaceManager().getExplorerTypeSetting(type1.getTypeName());
            CmsExplorerTypeSettings settings2 =
                OpenCms.getWorkplaceManager().getExplorerTypeSetting(type2.getTypeName());
            return ComparisonChain.start()
                .compare(settings1.getViewOrder(true), settings2.getViewOrder(true))
                .compare(
                    parse(settings1.getNewResourceOrder()), parse(settings2.getNewResourceOrder()))
                .result();
          }
    ...
    for (I_CmsResourceType addType : additionalTypes) {
      ...
      if (typesFromConfig.contains(typeName)
          && !elementView
              .isOther()) { //  type was already processed (although it may not be in the result
                            // list)
        continue;
      }
    ...
    List<CmsResourceTypeBean> results =
        srv.buildTypesList(resourceTypes, creatableTypes, disabledTypes, null);
