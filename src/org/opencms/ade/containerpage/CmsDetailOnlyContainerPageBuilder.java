...
/**
 * This is a simple helper class to more easily produce container page beans to be used as
 * detail-only containers.
 *
 * <p>To use this helper, you will need to set the type or width of each container and then add the
 * resources to be used as container elements. The type or width must be set because this helper
 * tries to automatically determine a default formatter for each container element.
 *
 * <p>Finally, call the build() method to produce the desired container page bean
 *
 * <p>Element settings and nested containers are currently not supported.
 */
public class CmsDetailOnlyContainerPageBuilder {
  ...
  private CmsContainerBean buildContainerBean(ContainerInfo cnt) {
    ...
    CmsContainerBean result =
        new CmsContainerBean(cnt.getName(), cnt.getType(), null, true, elements);
    return result;
  }

  /**
   * Builds the container element bean for a resource.
   *
   * <p>
   *
   ...
   */
  private CmsContainerElementBean buildContainerElementBean(
      ContainerInfo cnt, CmsResource resource) {

    I_CmsFormatterBean formatter =
        m_config
            .getFormatters(m_cms, resource)
            .getDefaultFormatter(cnt.getEffectiveType(), cnt.getEffectiveWidth());
    CmsUUID formatterId = formatter.getJspStructureId();
    CmsContainerElementBean elementBean =
        new CmsContainerElementBean(
            resource.getStructureId(), formatterId, new HashMap<String, String>(), false);
    return elementBean;
  }

  /**
   * Gets the container information for a given container, creating it if it doesn't already exist.
   *
   * <p>
   *
   * @param name the container name
