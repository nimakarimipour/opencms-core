 ...
 *
 * <p>A replacement configuration contains a list of entries, one for each type, with a list of
 * resources for each type and optionally a title string for each of the resources. The list of
 * resources replaces the search results that would be normally shown for the type in the 'add
 * content' dialog.
 */
public class CmsAddContentRestriction {
  ...
  public static TypeEntry readEntry(CmsObject cms, I_CmsXmlContentValueLocation location) {
    ...
    for (I_CmsXmlContentValueLocation entryLoc : location.getSubValues(N_ENTRY)) {
      CmsXmlVfsFileValue resoureValue =
          (CmsXmlVfsFileValue) (entryLoc.getSubValue(N_RESOURCE).getValue());
    ...
    return new TypeEntry(
        type, resourceList, titleMap, location.getValue().getDocument().getFile().getRootPath());
  }
  ...
  /**
   * Merges this configuration and a child configuration into a new configuration object, where an
   * entry for a type in the child overrides an entry for the same type in the parent.
   *
