 ...
 *
 * <p>see jsp file <tt>/system/modules/org.opencms.ade.galleries/testVfs.jsp</tt>.
 *
 * <p>
 *
 * @since 8.0.0
...
public class CmsGalleryActionElement extends CmsGwtActionElement {
   ...
   */
  public CmsGalleryActionElement(
  ...
  /**
   * Uses the request parameters of the current request to create a gallery configuration object.
   *
   * <p>
   *
  ...
  private CmsGalleryConfiguration createGalleryConfigurationFromRequest(GalleryMode galleryMode) {
    ...
    conf.setReferencePath(
        getRequest().getParameter(I_CmsGalleryProviderConstants.CONFIG_REFERENCE_PATH));
    conf.setGalleryPath(
        getRequest().getParameter(I_CmsGalleryProviderConstants.CONFIG_GALLERY_PATH));
    conf.setCurrentElement(
        getRequest().getParameter(I_CmsGalleryProviderConstants.CONFIG_CURRENT_ELEMENT));
    String resourceTypes =
        getRequest().getParameter(I_CmsGalleryProviderConstants.CONFIG_RESOURCE_TYPES);
    String useLinkDefaultTypes =
        getRequest().getParameter(I_CmsGalleryProviderConstants.PARAM_USE_LINK_DEFAULT_TYPES);
    ...
    if (Boolean.parseBoolean(useLinkDefaultTypes)) {
      try {
        ...
        CmsResource referenceResource =
            cms.readResource(conf.getReferencePath(), CmsResourceFilter.IGNORE_EXPIRATION);
    ...
    String galleryTypes =
        getRequest().getParameter(I_CmsGalleryProviderConstants.CONFIG_GALLERY_TYPES);
    ...
    String galleryStoragePrefix =
        getRequest().getParameter(I_CmsGalleryProviderConstants.CONFIG_GALLERY_STORAGE_PREFIX);
  ...
  /**
   * Returns the serialized initial data for gallery dialog depending on the given mode.
   *
   * <p>
   *
   * @param galleryMode the gallery mode
  ...
  private String export(GalleryMode galleryMode) throws Exception {
    ...
    if ((folderFilter != null) && !folderFilter.isEmpty()) {
      data.setVfsPreloadData(
          CmsGalleryService.generateVfsPreloadData(getCmsObject(), null, folderFilter));
    }
    if ((search != null) && (search.getScope() != null) && (search.getScope() != data.getScope())) {
      // default selected scope option should be the one for which the search has been actually
      // performed
    ...
    sb.append(
        exportDictionary(
            ...
            I_CmsGalleryService.class.getMethod(
                "getInitialSettings", CmsGalleryConfiguration.class),
            data));
    sb.append(
        exportDictionary(
  ...
  /**
   * Returns a javascript tag that contains a variable deceleration that has the close link as
   * value.
   *
   * <p>
   *
  ...
  private String exportCloseLink() {
    ...
    sb.append(
        wrapScript(
            "var ",
            I_CmsGalleryProviderConstants.ATTR_CLOSE_LINK,
            " = \'",
            link(closeLink),
            "\';"));
    return sb.toString();
