...
/**
 * Abstract class which is used to generate the data for showing an already opened tree in the
 * gallery dialog.
 *
 * <p>
 *
...
public abstract class A_CmsTreeTabDataPreloader<T extends I_CmsGalleryTreeEntry<T>> {
   ...
   */
  public T preloadData(
      CmsObject cms, Set<CmsResource> openResources, Set<CmsResource> selectedResources)
      throws CmsException {
  ...
  /**
   * Creates the beans for the loaded resources, and returns the root bean.
   *
   * <p>
   *
  ...
  /**
   * Gets the common ancestor path of a collection of resources.
   *
   * <p>
   *
  ...
  /**
   * Loads the ancestors of a resource.
   *
   * <p>
   *
  ...
  private void loadAncestors(CmsResource resource) throws CmsException {
    ...
    while ((currentResource != null)
        && m_cms.existsResource(currentResource.getStructureId(), m_filter)) {
  ...
  /**
   * Loads the children of the already loaded resources.
   *
   * <p>
   *
   * @throws CmsException if something goes wrong
