 ...
 *
 * <p>The search object collects the current parameters which are used for the search and contains
 * the search results for the current search parameters.
 *
 * @since 8.0.0
 */
public class CmsGallerySearchBean implements IsSerializable {
   ...
   *
   * <p>The constructor copies the content of the provided parameter to the current bean.
   *
   ...
   *
   * <p>This is used for filtering of dynamic function search results in the 'Add menu' case of the
   * gallery dialog.
   *
   ...
   *
   * <p>These are the types which are actually used for the search on the server, rather than the
   * types which are checked in the types tab. The lists are different, for example, if the user
   * hasn't selected any types.
   *
   ...
   * <p>
   *
   * @return <code>true</code> if there are more search results available <code>false</code>
   *     otherwise
   */
   ...
   * <p>
   *
   * @return false if any search parameter is selected, true if there are no search parameter
   *     selected
   */
  ...
  public boolean isEmpty() {
    ...
    List<String>[] params =
        new List[] {m_types, m_galleries, m_categories, new ArrayList<String>(m_folders)};
   ...
   * <p>
   *
   * @param noUploadReason the reason why an upload to the current target folder is not allowed to
   *     set
   */
