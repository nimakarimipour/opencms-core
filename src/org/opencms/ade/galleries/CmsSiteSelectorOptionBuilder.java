...
public class CmsSiteSelectorOptionBuilder {
  ...
  public CmsSiteSelectorOptionBuilder(CmsObject cms) {
    ...
    m_resolver.setMessages(
        OpenCms.getWorkplaceManager()
            .getMessages(OpenCms.getWorkplaceManager().getWorkplaceLocale(m_cms)));
  }
    ...
    if (basePath != null) {
      addOption(
          ...
          Messages.get()
              ...
              .key(Messages.GUI_SITESELECTOR_CURRENT_SUBSITE_0));
    }
  }
  ...
  public void addNormalSites(boolean includeRoot, String startFolder) {
    ...
    List<CmsSite> sites =
        siteManager.getAvailableSites(m_cms, true, false, m_cms.getRequestContext().getOuFqn());
    try {
      ...
      for (CmsSite site : sites) {
        // Do not add the shared site - it will be among sites if you are on the shared site at the
        // moment.
  ...
  public void addSharedSite() {
    ...
    if (shared.endsWith("/")) {
      ...
      // Otherwise the version without slash will be added as well when you are in the shared folder
      // currently.
      shared = shared.substring(0, shared.length() - 1);
    }
    if ((shared != null)
        && m_cms.existsResource(shared, CmsResourceFilter.ONLY_VISIBLE_NO_DELETED)) {
      addOption(
          Type.shared,
          shared,
          org.opencms.workplace.Messages.get()
              .getBundle(OpenCms.getWorkplaceManager().getWorkplaceLocale(m_cms))
              .key(org.opencms.workplace.Messages.GUI_SHARED_TITLE_0));
    }
  }
  ...
  /**
   * Internal helper method for adding an option.
   *
   * <p>
   *
  ...
  private void addOption(Type type, String siteRoot, String message) {
    ...
    CmsSiteSelectorOption option =
        new CmsSiteSelectorOption(type, siteRoot, m_siteRoot.equals(siteRoot), message);
