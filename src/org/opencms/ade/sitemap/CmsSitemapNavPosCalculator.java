...
/**
 * Helper class for recalculating navigation positions when a user has changed the order of
 * navigation entries in the sitemap editor.
 *
 * <p>This is harder than it sounds because we need to handle special cases like e.g. the user
 * inserting an entry between two existing entries with the same navigation position, which means we
 * need to update the navigation positions of multiple entries to force the ordering which the user
 * wanted.
 *
 * <p>
 */
public class CmsSitemapNavPosCalculator {
  ...
  private class PositionInfo {
    ...
    /**
     * Returns true if the position is neither out of bounds nor a position with a maximal nav pos
     * value.
     *
     * <p>
     *
     * @return true if the position is neither out of bounds nor a position with a maximal nav pos
     *     value
     */
  ...
  /**
   * Creates a new sitemap navigation position calculator and performs the navigation position
   * calculation for a given insertion operation.
   *
   * <p>
   *
   ...
   */
  public CmsSitemapNavPosCalculator(
      List<CmsJspNavElement> navigation, CmsResource movedElement, int insertPosition) {

    List<CmsJspNavElement> workList = new ArrayList<CmsJspNavElement>(navigation);
    CmsJspNavElement dummyNavElement =
        new CmsJspNavElement(DUMMY_PATH, movedElement, new HashMap<String, String>());
    ...
    List<Float> newNavPositions =
        interpolatePositions(beforeBlock, afterBlock, blockEnd - blockStart);
