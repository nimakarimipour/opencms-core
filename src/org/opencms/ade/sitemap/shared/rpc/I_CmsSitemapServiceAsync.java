...
public interface I_CmsSitemapServiceAsync {
   ...
   */
  void changeCategory(
      String entryPoint, CmsUUID id, String title, String name, AsyncCallback<Void> callback);
   ...
   * @param copyId the structure id of the resource to copy to create a new model page; if null, the
   *     model page is created as an empty container page
   ...
   */
  void disableModelPage(
      String baseUri, CmsUUID modelPageId, boolean disabled, AsyncCallback<Void> callback);
   ...
   */
  void getAliasImportResult(
      String resultKey, AsyncCallback<List<CmsAliasImportResult>> asyncCallback);
  ...
  void getGalleryData(
      String entryPointUri,
      AsyncCallback<Map<CmsGalleryType, List<CmsGalleryFolderEntry>>> callback);
   ...
   * @param resultCallback the callback for the result
   */
  void getNewElementInfo(
      String entryPointUri, AsyncCallback<List<CmsNewResourceInfo>> resultCallback);
   ...
   * @param rootId the structure id of the resource which corresponds to the tree root in locale
   *     compare mode
   * @param resultCallback the callback to call with the result
   */
  void loadPropertyDataForLocaleCompareView(
      CmsUUID id, CmsUUID rootId, AsyncCallback<CmsLocaleComparePropertyData> resultCallback);
   ...
   * @param callback the async callback
   */
  void mergeSubSitemap(
      String entryPoint, CmsUUID subSitemapId, AsyncCallback<CmsSitemapChange> callback);
   ...
   *
   * <p>This method may change the currently set site root.
   *
   ...
   */
  void saveAliases(
      CmsAliasSaveValidationRequest saveRequest,
   ...
   * @param callback the callback for the result
   */
  void saveSitemapAttributes(
      CmsUUID rootId, Map<String, String> attributes, AsyncCallback<Void> callback);
   ...
   *
   * <p>This is used to keep two users from editing the alias table for a site root concurrently.
   *
   * <p>
   *
   * @param editing true to indicate that the table is still being edited, false to indicate that
   *     the table isn't being edited anymore
   * @param callback the asynchronous callback
   */
