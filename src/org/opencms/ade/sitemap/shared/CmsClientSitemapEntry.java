...
public class CmsClientSitemapEntry implements IsSerializable {
  ...
  private Map<String, CmsClientProperty> m_defaultFileProperties =
      new HashMap<String, CmsClientProperty>();
  ...
  public void initialize(I_CmsSitemapController controller) {
    ...
    m_defaultFileProperties =
        controller.replaceProperties(m_defaultFileId, m_defaultFileProperties);
   ...
   * @param controller a sitemap controller instance
   */
  public void insertSubEntry(
      CmsClientSitemapEntry entry, int position, I_CmsSitemapController controller) {
  ...
  public boolean isEditable() {
    ...
    return m_permissionInfo.hasWritePermission()
        ...
        && (((getLock() == null) || (getLock().getLockOwner() == null))
            || getLock().isOwnedByUser());
  }
  ...
  /**
   * Sets if the entry resource has blocking locked children that can not be locked by the current
   * user.
   *
   * <p>
   *
   * @param hasBlockingLockedChildren <code>true</code> if the entry resource has blocking locked
   *     children
   */
  ...
  public void setSubEntries(
      List<CmsClientSitemapEntry> children, I_CmsSitemapController controller) {
    ...
    if (!m_sitePath.equals(sitepath)) {
      ...
      m_vfsPath =
          CmsStringUtil.joinPaths(
              m_vfsPath.substring(0, start), sitepath, m_vfsPath.substring(stop));
  ...
  private void copyMembers(CmsClientSitemapEntry source) {
    ...
    if (source.getDetailpageTypeName() != null) {
      // do not copy the detail page type name unless it is not null, otherwise newly created detail
      // pages
