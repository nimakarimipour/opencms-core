...
public class CmsSitemapChange implements IsSerializable, Comparable<CmsSitemapChange> {
  ...
  private Map<String, CmsClientProperty> m_defaultFileInternalProperties =
  ...
  private Map<String, CmsClientProperty> m_ownInternalProperties =
      new HashMap<String, CmsClientProperty>();
  ...
  private List<CmsPropertyModification> m_propertyModifications =
  ...
  public void addChangeTitle(String title) {
    ...
    CmsPropertyModification propChange =
        new CmsPropertyModification(m_entryId, CmsClientProperty.PROPERTY_NAVTEXT, title, true);
  ...
  public int compareTo(CmsSitemapChange arg0) {
    ...
    int result =
        CmsResource.getParentFolder(m_sitePath)
            .compareTo(CmsResource.getParentFolder(arg0.m_sitePath));
    if (result == 0) {
      result = m_position - arg0.m_position;
      if (result == 0) {
        // two different entries should never have the same parent and the same name, so this
        // compare should be sufficient
