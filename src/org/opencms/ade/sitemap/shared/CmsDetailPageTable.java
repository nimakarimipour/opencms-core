...
public class CmsDetailPageTable implements Cloneable, Serializable {
  ...
  public Status getStatus(CmsUUID id) {
    ...
    List<CmsDetailPageInfo> pagesWithNoQualifier =
        m_map.computeIfAbsent(type, k -> new ArrayList<>()).stream()
            .filter(detailPage -> detailPage.getQualifier() == null)
  ...
  public boolean isDefaultDetailPage(CmsUUID id) {
    ...
    CmsDetailPageInfo firstUnqualifiedEntry =
        m_map.get(info.getType()).stream()
            .filter(page -> page.getQualifier() == null)
            .findFirst()
            .orElse(null);
  ...
  /**
   * Moves the detail page information for a given page to the front of the detail pages for the
   * same type.
   *
   * <p>
   *
  ...
  public int makeDefault(CmsUUID id) {
    ...
    if (info.getQualifier() != null) {
      CmsDetailPageInfo info2 =
          new CmsDetailPageInfo(
              info.getId(), info.getUri(), info.getType(), null, info.getIconClasses());
      m_infoById.put(id, info2);
      if (info.isInherited()) {
        // this case should be prevented by the GUI, we handle it just to make sure the entry is not
        // saved
  ...
  /**
   * Returns a flat list containing all detail pages for all types which preserves the order of
   * detail pages from each type list.
   *
   * <p>
   *
