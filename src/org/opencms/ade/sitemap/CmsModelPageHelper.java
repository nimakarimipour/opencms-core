...
public class CmsModelPageHelper {
  ...
  public void addModelPageToSitemapConfiguration(
    ...
    content
        .getValue(prefix + "/" + CmsConfigurationReader.N_PAGE, Locale.ENGLISH)
        .setStringValue(m_cms, modelPage.getRootPath());
    content
        .getValue(prefix + "/" + CmsConfigurationReader.N_DISABLED, Locale.ENGLISH)
        .setStringValue(m_cms, "" + disabled);
    content
        .getValue(prefix + "/" + CmsConfigurationReader.N_IS_DEFAULT, Locale.ENGLISH)
        .setStringValue(m_cms, "" + isDefault);
   ...
   * @param copyId structure id of the resource to use as a model for the model page, if any (may be
   *     null)
  ...
  public CmsResource createModelGroupPage(@RUntainted String name, @RUntainted String description, CmsUUID copyId)
    ...
    CmsResourceTypeConfig config =
        m_adeConfig.getResourceType(CmsResourceTypeXmlContainerPage.MODEL_GROUP_TYPE_NAME);
  ...
  public void disableModelPage(CmsResource sitemapConfig, CmsUUID structureId, @RUntainted boolean disabled)
    ...
    if (i < modelPageConfigs.size()) {
      ...
      I_CmsXmlContentValue linkValue =
          content.hasValue(linkValuePath, Locale.ENGLISH)
    ...
    I_CmsXmlContentValue disabledValue =
        content.hasValue(disabledPath, Locale.ENGLISH)
  ...
  /**
   * Tries to either read or create the default folder for model pages in the current sitemap, and
   * returns it.
   *
   * <p>
   *
  ...
  public CmsResource ensureModelFolder(CmsResource rootResource) throws CmsException {
    ...
    String modelFolderPath =
        CmsStringUtil.joinPaths(m_adeConfig.getBasePath(), ".content/.templates");

    try {
      CmsResource result = m_cms.readFolder(modelFolderPath);
      return result;
    } catch (CmsVfsResourceNotFoundException e) {
      CmsProperty searchExclude =
          new CmsProperty(CmsPropertyDefinition.PROPERTY_SEARCH_EXCLUDE, "all", null);
      CmsResource result =
          m_cms.createResource(
              modelFolderPath,
  ...
  /**
   * Creates a model page entry bean from a model page resource.
   *
   * <p>
   *
   ...
   */
  CmsModelPageEntry createModelPageEntry(
      CmsResource resource, boolean disabled, boolean isDefault, Locale locale) {
    ...
    try {
      ...
      CmsProperty descProperty =
          m_cms.readPropertyObject(
              resource, CmsPropertyDefinition.PROPERTY_DESCRIPTION, false, locale);
      ...
      infoBean.addAdditionalInfo(
          Messages.get()
              .getBundle(OpenCms.getWorkplaceManager().getWorkplaceLocale(m_cms))
              .key(Messages.GUI_VFS_PATH_0),
  ...
  /**
   * Sets the default model page within the given configuration file.
   *
   * <p>
   *
  ...
  CmsModelInfo setDefaultModelPage(CmsResource configFile, CmsUUID modelId) throws CmsException {
    ...
    for (I_CmsXmlContentValue value :
        content.getValues(CmsConfigurationReader.N_MODEL_PAGE, locale)) {
      I_CmsXmlContentValue linkValue =
          content.getValue(
              CmsStringUtil.joinPaths(value.getPath(), CmsConfigurationReader.N_PAGE), locale);
      I_CmsXmlContentValue isDefaultValue =
          content.getValue(
    ...
    if (defaultValueIndex != -1) {
      ...
      content
          .getValue(
              CmsConfigurationReader.N_MODEL_PAGE + "[1]/" + CmsConfigurationReader.N_PAGE, locale)
          .setStringValue(m_cms, defaultModelTarget);
      content
          .getValue(
              CmsConfigurationReader.N_MODEL_PAGE + "[1]/" + CmsConfigurationReader.N_DISABLED,
              locale)
          .setStringValue(m_cms, Boolean.FALSE.toString());
      content
          .getValue(
              CmsConfigurationReader.N_MODEL_PAGE + "[1]/" + CmsConfigurationReader.N_IS_DEFAULT,
              locale)
          .setStringValue(m_cms, Boolean.TRUE.toString());
    }
    ...
    for (CmsModelPageConfig config : modelPageConfigs) {
      ...
      try {
        CmsResource modelPage =
            m_cms.readResource(structureId, CmsResourceFilter.IGNORE_EXPIRATION);
        CmsModelPageEntry entry =
            createModelPageEntry(modelPage, config.isDisabled(), config.isDefault(), wpLocale);
        if (entry != null) {
          result.add(entry);
        }
      } catch (CmsVfsResourceNotFoundException e) {
  ...
  /**
   * Helper method for getting the locale from which to read the configuration data.
   *
   * <p>
   *
  ...
  /**
   * Writes a sitemap configuration back to the VFS.
   *
   * <p>
   *
