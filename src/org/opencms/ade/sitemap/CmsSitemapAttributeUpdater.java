...
public class CmsSitemapAttributeUpdater {
  ...
  /**
   * Given a map of attribute values (usually from the attribute editor), this method computes the
   * necessary updates to perform.
   *
   * <p>Attribute values that match the inherited attribute from a parent sitemap configuration
   * should cause the attribute to be removed.
   *
  ...
  public static Map<String, String> computeUpdatesRelativeToInheritedValues(
      Map<String, String> newAttributes, CmsADEConfigData config) {
  ...
  public Map<String, String> getAttributesFromContent() {
    ...
    List<I_CmsXmlContentValue> attributeValues =
        m_sitemapConfig.getValues(CmsConfigurationReader.N_ATTRIBUTE, Locale.ENGLISH);
    for (I_CmsXmlContentValue attr : attributeValues) {
      I_CmsXmlContentValue keyValue =
          m_sitemapConfig.getValue(
              CmsXmlUtils.concatXpath(attr.getPath(), CmsConfigurationReader.N_KEY),
              Locale.ENGLISH);
      I_CmsXmlContentValue valueValue =
          m_sitemapConfig.getValue(
  ...
  /**
   * Completely replaces the sitemap attributes in the XML content with the entries from the map
    ...
    for (Map.Entry<String, String> entry : allValues.entrySet()) {
      ...
      I_CmsXmlContentValue newKeyValue =
          m_sitemapConfig.getValue(
              ...
              Locale.ENGLISH);
      I_CmsXmlContentValue newValueValue =
          m_sitemapConfig.getValue(
   ...
   *
   * <p>This method replaces values that match values inherited from a parent configuration with
   * null (not set).
   *
  ...
  public boolean saveAttributesFromEditorDialog(Map<String, String> attributes) {
    ...
    CmsADEConfigData config =
        OpenCms.getADEManager()
            .lookupConfigurationWithCache(m_cms, m_sitemapConfig.getFile().getRootPath());
   ...
   *
   * <p>The map given as an argument may contain null values. In that case, the corresponding
   * attribute is removed from the XML content.
   *
