 ...
 *
 * <p>It consists of a table from site roots to users. The entries of the table will be removed if
 * they haven't been updated in a given interval of time, but can also be removed manually.
 *
 * <p>
 */
public class CmsAliasEditorLockTable {
  ...
  public CmsAliasEditorLockTable() {
    ...
    CacheBuilder<Object, Object> mm =
        CacheBuilder.newBuilder().expireAfterWrite(TIMEOUT_INTERVAL, TimeUnit.MILLISECONDS);
    m_map = mm.build();
  }

  /**
   * Clears the entry for a given site root, but only if the user from the given CMS context matches
   * the user in the entry.
   *
   * <p>
   *
   ...
   *
   * <p>If this method succeeds, it will return null, but if another user has created an entry for
   * the site root, it will return that user.
   *
   * <p>
   *
   * @param cms the current CMS context
