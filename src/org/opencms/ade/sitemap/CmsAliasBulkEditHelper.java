...
public class CmsAliasBulkEditHelper {
   ...
   */
  public void importAliases(HttpServletRequest request, HttpServletResponse response)
      throws Exception {
  ...
  public CmsAliasEditValidationReply saveAliases(CmsAliasSaveValidationRequest saveRequest)
    ...
    if (m_hasErrors) {
      ...
      OpenCms.getAliasManager()
          .saveRewriteAliases(
              m_cms, m_cms.getRequestContext().getSiteRoot(), convertRewriteData(rewriteData));
      ...
      for (CmsAliasTableRow row : rows) {
        CmsAlias editedAlias =
            new CmsAlias(row.getStructureId(), siteRoot, row.getAliasPath(), row.getMode());
  ...
  /**
   * Converts rewrite alias table rows to rewrite alias objects.
   *
   * <p>
   *
