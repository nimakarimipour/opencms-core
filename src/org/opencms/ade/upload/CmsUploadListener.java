...
public class CmsUploadListener implements ProgressListener, Serializable {
  ...
  public CmsUploadProgessInfo getInfo() {
    ...
    if (m_finished) {
      return new CmsUploadProgessInfo(
          getItem(), (int) getPercent(), UPLOAD_STATE.finished, getContentLength(), getBytesRead());
    }
    return new CmsUploadProgessInfo(
        getItem(), (int) getPercent(), UPLOAD_STATE.running, getContentLength(), getBytesRead());
  }
  ...
  /**
   * Returns <code>true</code> if the process has been canceled due to an error or by the user.
   *
   * <p>
   *
   * @return boolean<code>true</code> if the process has been canceled due to an error or by the
   *     user
   */
   ...
   *   <li>throws an RuntimeException if it was set in the meanwhile (by another request e.g. user
   *       has canceled)
    ...
    if (m_watcher == null) {
      try {
        ...
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_UPLOAD_CREATE_WATCH_DOG_2, getId(), e.getMessage()));
      }
    }
  }
