...
public class CmsUploadService extends CmsGwtService implements I_CmsUploadService {
  ...
  public Boolean cancelUpload() {
    ...
    if (getRequest().getSession().getAttribute(CmsUploadBean.SESSION_ATTRIBUTE_LISTENER_ID)
        != null) {
      CmsUUID listenerId =
          (CmsUUID)
              getRequest().getSession().getAttribute(CmsUploadBean.SESSION_ATTRIBUTE_LISTENER_ID);
      CmsUploadListener listener = CmsUploadBean.getCurrentListener(listenerId);
      if ((listener != null) && !listener.isCanceled()) {
        listener.cancelUpload(
            new CmsUploadException(
                Messages.get().getBundle().key(Messages.ERR_UPLOAD_USER_CANCELED_0)));
        return Boolean.TRUE;
      }
    }
  ...
  /**
   * @see org.opencms.gwt.shared.rpc.I_CmsUploadService#checkUploadFiles(java.util.List,
   *     java.lang.String, boolean)
   */
  public CmsUploadFileBean checkUploadFiles(
    ...
    if (getRequest().getSession().getAttribute(CmsUploadBean.SESSION_ATTRIBUTE_LISTENER_ID)
      ...
      for (String fileName : fileNames) {
        ...
        try {
          ...
          String newResName =
              CmsUploadBean.getNewResourceName(getCmsObject(), fileName, targetFolder, false);
    ...
    return new CmsUploadFileBean(
        existingResourceNames, invalidFileNames, existingDeleted, isActive);
  }
  ...
  /**
   * Checks if a resource already exists for the given path.
   *
   * <p>
   *
   * @param path the path
  ...
  /**
   * Checks if a the resource exists but is marked as deleted.
   *
   * <p>
   *
   * @param path the path
