...
/**
 * A class which is executed in a new thread, so its able to detect when an upload process is frozen
 * and sets an exception in order to be canceled. This doesn't work in Google application engine.
 *
 * <p>
 */
public class CmsUploadTimeoutWatcher extends Thread implements Serializable {
  ...
  public void run() {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .container(Messages.ERR_UPLOAD_INTERRUPT_WATCH_DOG_1, m_listener.toString()),
          e);
    }
    if (m_listener != null) {
      if (((m_listener.getBytesRead() > 0) && (m_listener.getPercent() >= 100))
          || m_listener.isCanceled()) {
        LOG.debug(
            Messages.get()
                .container(Messages.LOG_UPLOAD_FINISHED_WATCHER_1, m_listener.toString()));
        m_listener = null;
      } else {
        if (isFrozen()) {
  ...
  /**
   * Returns <code>true</code> if the upload process is frozen.
   *
   * <p>
   *
