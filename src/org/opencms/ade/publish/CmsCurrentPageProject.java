...
public class CmsCurrentPageProject implements I_CmsVirtualProject {
  ...
  public I_CmsPublishRelatedResourceProvider getRelatedResourceProvider(
      final CmsObject cmsObject, final CmsPublishOptions options) {
        ...
        if (res.getStructureId().toString().equals(pageId)) {
          ...
          try {
            ...
            for (CmsRelation relation : relations) {
              ...
              try {
                onlineTarget =
                    onlineCms.readResource(
                        relation.getTargetId(), CmsResourceFilter.IGNORE_EXPIRATION);

                offlineTarget =
                    cmsObject.readResource(relation.getTargetId(), CmsResourceFilter.ALL);
          ...
          for (Map.Entry<String, String> entry : params.entrySet()) {
            if (entry.getKey().startsWith(CmsPublishOptions.PARAM_COLLECTOR_INFO)) {
              try {
                AutoBean<I_CmsContentLoadCollectorInfo> autoBean =
                ...
                if (null == publishListProvider) { // unregistered collector
                  ...
                  publishListProvider =
                      (I_CmsCollectorPublishListProvider) collectorClass.newInstance();
                }
              ...
              try {
                if (CmsUUID.isValidUUID(token)) {
                  CmsResource collectorRes =
                      cms.readResource(new CmsUUID(token), CmsResourceFilter.ALL);
                ...
                LOG.error(
                    "Error processing collector item " + token + ": " + e.getLocalizedMessage(), e);
              }
            }
          }
        }
