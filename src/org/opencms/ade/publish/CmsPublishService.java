...
public class CmsPublishService extends CmsGwtService implements I_CmsPublishService {
  ...
  public static String wrapProjectName(CmsObject cms, String name) {
    ...
    return Messages.get()
        ...
        .key(Messages.GUI_NORMAL_PROJECT_1, name);
  }

  /**
   * @see
   *     org.opencms.ade.publish.shared.rpc.I_CmsPublishService#executeAction(org.opencms.ade.publish.shared.CmsWorkflowAction,
  ...
  public CmsWorkflowResponse executeAction(CmsWorkflowAction action, CmsWorkflowActionParams params)
    ...
    try {
      ...
      if (params.getToken() == null) {
        ...
        response =
            OpenCms.getWorkflowManager().executeAction(cms, action, options, publishResources);
      } else {
  ...
  public CmsPublishData getPublishData(
    ...
    // need to put this into params here so that the virtual project for direct publishing is
    // included in the result of getManageableProjects()
    if (!pathList.isEmpty()) {
    ...
    if (!pathList.isEmpty()) {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(projectParam)
          && CmsUUID.isValidUUID(projectParam)) {
        ...
        for (CmsProjectBean project : projects) {
          if (selectedProject.equals(project.getId())) {
            ...
            if (project.isWorkflowProject()) {
              canOverrideWorkflow = false;
              workflowId =
                  OpenCms.getWorkflowManager().getWorkflowForWorkflowProject(selectedProject);
            }
            break;
          }
        }
  ...
  /**
   * @see
   *     org.opencms.ade.publish.shared.rpc.I_CmsPublishService#getResourceGroups(org.opencms.ade.publish.shared.CmsWorkflow,
  ...
  public CmsPublishGroupList getResourceGroups(
    ...
    try {
      ...
      I_CmsPublishResourceFormatter formatter =
          workflowManager.createFormatter(cms, workflow, options);
      CmsWorkflowResources workflowResources = null;
      workflowResources =
          workflowManager.getWorkflowResources(cms, workflow, options, projectChanged, false);
      if (workflowResources.getOverrideWorkflow() != null) {
        overrideWorkflowId = workflowResources.getOverrideWorkflow().getId();
        formatter =
            workflowManager.createFormatter(cms, workflowResources.getOverrideWorkflow(), options);
      }
      if (workflowResources.isTooMany()) {
        // too many resources, send a publish list token to the client which can be used later to
        // restore the resource list
      ...
      I_CmsVirtualProject virtualProject =
          workflowManager.getRealOrVirtualProject(options.getProjectId());
      I_CmsPublishRelatedResourceProvider relProvider =
          virtualProject.getRelatedResourceProvider(getCmsObject(), options);
      ResourceMap resourcesAndRelated =
          getResourcesAndRelated(
  ...
  protected void addSiblings(Set<CmsResource> publishResources) {
    ...
    for (CmsResource resource : publishResources) {
      if (!resource.getState().isUnchanged()) {
        try {
          List<CmsResource> changedSiblings =
              getCmsObject().readSiblings(resource, CmsResourceFilter.ALL_MODIFIED);
  ...
  /**
   * Returns the cached publish options, creating it if it doesn't already exist.
   *
   * <p>
   *
  ...
  /**
   * Returns the id of the last used workflow for the current user.
   *
   * <p>
   *
  ...
  /**
   * Gets the resource map containing the publish resources together with their related resources.
   *
   * <p>
   *
   ...
   * @param keepOriginalUnchangedResources flag which determines whether unchanged resources in the
   *     original resource list should be kept or removed
  ...
  private ResourceMap getResourcesAndRelated(
    ...
    Set<CmsResource> visibleResources =
        resources.stream()
            .filter(
                resource ->
                    getCmsObject()
                        .existsResource(
                            resource.getStructureId(), CmsResourceFilter.ALL.addRequireVisible()))
            .collect(Collectors.toSet());
    ResourceMap result;
    if (includeRelated) {
      CmsPublishRelationFinder relationFinder =
          new CmsPublishRelationFinder(
              getCmsObject(), visibleResources, keepOriginalUnchangedResources, relProvider);
  ...
  /**
   * Converts a list of IDs to resources.
   *
   * <p>
   *
  ...
  /**
   * Saves the given options to the session.
   *
   * <p>
   *
  ...
  /**
   * Writes the id of the last used workflow to the current user.
   *
   * <p>
   *
   * @param workflowId the workflow id
