 ...
 *
 * <p>This virtual project gets the names of the resources to publish from the publish parameter
 * map. If the 'add contents' mode is enabled (which is also determined from the publish
 * parameters), the contents of folders are added to the list of publish resources. This virtual
 * project is only available if any file names are passed via the
 */
public class CmsDirectPublishProject implements I_CmsVirtualProject {
  ...
  protected void addSubResources(CmsObject cms, Set<CmsResource> resources) throws CmsException {
    ...
    for (CmsResource res : resources) {
      if (res.isFolder()) {
        try {
          List<CmsResource> childrenOfCurrentResource =
              rootCms.readResources(res.getRootPath(), CmsResourceFilter.ALL, true);
  ...
  /**
   * Gets the set of site paths from the publish parameters.
   *
   * <p>
   *
   * @param params the publish parameters
