...
/**
 * Helper class for finding all related resources for a set of resources to be published, for use
 * with the new ADE publish dialog.
 *
 * <p>
 */
public class CmsPublishRelationFinder {
  ...
  /**
   * A map from resources to sets of resources, which automtically instantiates an empty set when
   * accessing a key that doesn't exist via get().
   *
   * <p>
   */
  public static class ResourceMap extends HashMap<CmsResource, Set<CmsResource>> {
     ...
     *
     * <p>The given predicate is used to check whether any single resource should be kept. If it
     * returns false for a top-level resource (map key), the parent will be removed and all its
     * children added as keys. If it returns false for a map value, the value will be removed for
     * its key.
     *
  ...
  /**
   * The resource types whose resources will be also added as related resources, even if the
   * relation pointing to them is a weak relation.
   *
   * <p>
   */
  ...
  /**
   * Flag which controls whether unchanged resources in the original resource list should be kept or
   * removed.
   */
   ...
   * @param keepOriginalUnchangedResources true if unchanged resources from the original resource
   *     list should be kept
   ...
   * <p>
   *
   * @return a ResourceMap which has resources from the original set of resources as keys, and sets
   *     of related resources as values
   */
  public ResourceMap getPublishRelatedResources() {
    ...
    ResourceMap publishRelatedResources =
        getChangedResourcesReachableFromOriginalResources(reachable);
  ...
  /**
   * Removes unchanged resources from the top level, and if they have children which do not occur
   * anywhere else, moves these children to the top level.
   *
   * <p>
   *
   ...
   */
  public void removeUnchangedTopLevelResources(
      ResourceMap publishRelatedResources, ResourceMap reachability) {
    ...
    while (findAndMoveParentWithChildren(
        childrenOfUnchangedParents, reachability, publishRelatedResources)) {
      // do nothing
    }
  ...
  /**
   * Computes the "reachability map", given the map of direct relations between resources.
   *
   * <p>
   *
    ...
    do {
      ...
      for (CmsResource source : result.keySet()) {
        for (CmsResource target : result.get(source)) {
          // need to check if the key is present, otherwise we may get a
  ...
  /**
   * Gets a ResourceMap which contains, for each resource reachable from the original set of
   * resources, the directly related resources.
   *
   * <p>
   *
      ...
      if (!currentResource.getState().isDeleted()) {
        ...
        for (CmsResource target : directlyRelatedResources) {
          if (m_cms.existsResource(
              target.getStructureId(), CmsResourceFilter.ALL.addRequireVisible())) {
  ...
  /**
   * Tries to find a parent with related children in a set, and moves them to a result ResourceMap.
   *
   * <p>
   *
  ...
  private boolean findAndMoveParentWithChildren(
      Set<CmsResource> originalSet, ResourceMap reachability, ResourceMap result) {
  ...
  /**
   * Gets the resources which are reachable from the original set of resources and are not
   * unchanged.
   *
   * <p>
   *
  ...
  private ResourceMap getChangedResourcesReachableFromOriginalResources(ResourceMap reachable) {
    ...
    for (CmsResource res : m_originalResources) {
      ...
      for (CmsResource item : reachableItems) {
        if (!isUnchangedAndShouldBeRemoved(item)
            && !item.getStructureId().equals(res.getStructureId())) {
  ...
  /**
   * Fetches the directly related resources for a given resource.
   *
   * <p>
   *
  ...
  private Set<CmsResource> getDirectlyRelatedResources(CmsResource currentResource) {
    ...
    try {
      ...
      LOG.error(
          "Error processing parent folder for "
  ...
  /**
   * Reads the relations from a given resource, and returns an empty list if an error occurs while
   * reading them.
   *
   * <p>
   *
  ...
  private List<CmsRelation> getRelationsFromResource(CmsResource currentResource) {
    ...
    try {
      return m_cms.readRelations(
          CmsRelationFilter.relationsFromStructureId(currentResource.getStructureId()));
    } catch (CmsException e) {
  ...
  /**
   * Reads a resource with a given id, but will get a resource from a cache if it has already been
   * read before.
   *
   * <p>If an error occurs, null will be returned.
   *
  ...
  /**
   * Checks if the resource is unchanged *and* should be removed.
   *
   * <p>
   *
  ...
  /**
   * Removes those resources as keys from the resource map which also occur as related resources
  ...
  /**
   * Checks if the resource should be added to the related resources even if the relation pointing
   * to it is a weak relation.
   *
   * <p>
   *
  ...
  private boolean shouldAddWeakRelationTarget(CmsResource weakRelationTarget) {
    ...
    for (String typeName : VALID_WEAK_RELATION_TARGET_TYPES) {
      if (OpenCms.getResourceManager()
          .matchResourceType(typeName, weakRelationTarget.getTypeId())) {
        return true;
      }
    }
    return false;
