...
public class CmsPublish {
    ...
    try {
      publishList =
          OpenCms.getPublishManager()
              .getPublishListAll(m_cms, pubResources, m_options.isIncludeSiblings(), true);
    ...
    for (String resourceName : validator.keySet()) {
      ...
      try {
        CmsResource resource =
            m_cms.readResource(
                m_cms.getRequestContext().removeSiteRoot(resourceName), CmsResourceFilter.ALL);
          ...
          for (CmsRelation relation : infoEntry.getRelations()) {
            try {
              ...
              CmsPublishResourceInfo info =
                  new CmsPublishResourceInfo(
                      Messages.get()
                          .getBundle(m_workplaceLocale)
                          .key(Messages.GUI_BROKEN_LINK_ONLINE_0),
          ...
          try {
            ...
            CmsPublishResourceInfo info =
                new CmsPublishResourceInfo(
                    Messages.get()
                        .getBundle(m_workplaceLocale)
                        .key(Messages.GUI_RESOURCE_MISSING_ONLINE_0),
  ...
  public void publishResources(List<CmsResource> resources) throws CmsException {
    ...
    I_CmsReport report =
        new CmsWorkplaceReport(
            cms.getRequestContext().getLocale(), cms.getRequestContext().getSiteRoot(), null);
    CmsPublishManager publishManager = OpenCms.getPublishManager();
  ...
  public CmsPublishResource relationToBean(CmsRelation relation) {
    ...
    bean.setBigIconClasses(
        CmsIconUtil.getIconClasses(CmsResourceTypePlain.getStaticTypeName(), null, false));
    return bean;
  }
