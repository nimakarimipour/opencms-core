...
public abstract class A_CmsPublishGroupHelper<RESOURCE, GROUP> {
  ...
  /**
   * Given a descending list of dates represented as longs, this method computes a map from the
   * dates to their age in (local) days.
   *
   * <p>
   *
   ...
   *
   * <p>The first date must be later than the second date.
   *
  ...
  public List<GROUP> getGroups(List<RESOURCE> resources) {
    ...
    Map<GroupAge, List<RESOURCE>> resourcesByAge =
        partitionPublishResourcesByAge(sortedResources, daysMap);
    List<List<RESOURCE>> youngGroups = partitionYoungResources(resourcesByAge.get(GroupAge.young));
    List<List<RESOURCE>> mediumGroups =
        partitionMediumResources(resourcesByAge.get(GroupAge.medium), daysMap);
    ...
    switch (age) {
      case young:
        groupName =
            Messages.get()
                .getBundle(m_locale)
                .key(Messages.GUI_GROUPNAME_SESSION_1, new Date(groupDate));
      ...
      case medium:
        groupName =
            Messages.get()
                .getBundle(m_locale)
                .key(Messages.GUI_GROUPNAME_DAY_1, new Date(groupDate));
        break;
      case old:
      default:
        groupName =
            Messages.get().getBundle(m_locale).key(Messages.GUI_GROUPNAME_EVERYTHING_ELSE_0);
        break;
    }
    return groupName;
  }
   ...
   */
  public List<List<RESOURCE>> partitionMediumResources(
      List<RESOURCE> resources, Map<Long, Integer> days) {
   ...
   * @param days the map from modification dates to the number of (local) days since the
   *     modification
   * @return a map from age enum values to the list of resources which fall into the corresponding
   *     age group
   */
