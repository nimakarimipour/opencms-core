...
public class CmsContentEditorActionElement extends CmsGwtActionElement {
   ...
   */
  public CmsContentEditorActionElement(
  ...
  /**
   * Adds link and script tags to the buffer if required for external widgets.
   *
   * <p>
   *
  ...
  private void addExternalResourceTags(StringBuffer sb, CmsContentDefinition definition) {
    ...
    for (CmsExternalWidgetConfiguration configuration :
  ...
  /**
   * Returns the prefetch data include.
   *
   * <p>
   *
  ...
  private String getPrefetch() throws Exception {
    ...
    sb.append(
        wrapScript(
            I_CmsContentService.PARAM_BACKLINK,
            "='",
            CmsStringUtil.escapeJavaScript(backlink) + "';\n"));
