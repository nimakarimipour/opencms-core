 ...
 *
 * <p>If "Remove all formatters" is selected/deselected, the set of formatters that can be
 * added/removed changes. That means, formerly explicitly added/removed formatters in the sitemap
 * config can not be added/removed anymore. The respective content nodes must be removed. This is
 * done by that handler.
 */
public class CmsEditorChangeHandlerFormatterSelection extends A_CmsXmlContentEditorChangeHandler {
   ...
   *
   * @see
   *     org.opencms.xml.content.I_CmsXmlContentEditorChangeHandler#handleChange(org.opencms.file.CmsObject,
  ...
  public CmsXmlContent handleChange(
    ...
    if ((changedPaths.size() != 1)
        && content
            .getValue(changedPaths.iterator().next(), locale)
            .getTypeName()
            .equals(CmsXmlBooleanValue.TYPE_NAME)) {
      LOG.error(
          Messages.get()
              .container(
    ...
    if (removeAll && content.hasValue(REMOVE_PATH, locale)) {
      ...
      CmsXmlContentValueSequence addSequence =
          content.getValueSequence(ADD_PATH_SINGLE_NODE, locale);
