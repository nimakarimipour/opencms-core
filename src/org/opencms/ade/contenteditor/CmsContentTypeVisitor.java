...
public class CmsContentTypeVisitor {
  ...
  protected class DisplayTypeEvaluator {
     ...
     */
    protected CmsAttributeConfiguration getEvaluatedConfiguration(
        DisplayType predecessor, DisplayType successor) {
      ...
      if (resultingType.equals(DisplayType.none)) {
        if (m_rule.equals(EvaluationRule.rootLevel)) {
          ...
          boolean strong =
              m_rule.equals(EvaluationRule.none)
                  || (m_rule.equals(EvaluationRule.optional)
                      && m_default.equals(DisplayType.singleline))
                  || (m_rule.equals(EvaluationRule.labelLength)
                      && m_default.equals(DisplayType.wide));
          if (((predecessor == null) || (successor == null)) && strong) {
            resultingType = m_default;
          } else if ((predecessor != null) || (successor != null)) {

            // check if the proposed type matches neither the type of the predecessor nor the type
            // of the successor
            if (!(((predecessor != null) && resultingType.equals(predecessor))
                || ((successor != null) && resultingType.equals(successor)))) {
              DisplayType match =
                  (predecessor != null)
                          && (predecessor.equals(DisplayType.wide)
                              || predecessor.equals(DisplayType.singleline))
                      ? predecessor
                      : ((successor != null)
                              && (successor.equals(DisplayType.wide)
                                  || successor.equals(DisplayType.singleline))
                          ? successor
                          : null);
  ...
  private Map<String, CmsComplexWidgetData> m_complexWidgets =
      new HashMap<String, CmsComplexWidgetData>();
  ...
  /**
   * Indicates the visited content has fields that are configured to be invisible to the current
   * user.
   */
   ...
   * @return <code>true</code> if the content type widgets are compatible with the new content
   *     editor
  ...
  public boolean isEditorCompatible(CmsXmlContentDefinition xmlContentDefinition)
    ...
    for (I_CmsXmlSchemaType subType : xmlContentDefinition.getTypeSequence()) {
      if (subType.isSimpleType()) {
        ...
        CmsXmlContentDefinition subTypeDefinition =
            ((CmsXmlNestedContentDefinition) subType).getNestedContentDefinition();
  ...
  /**
   * Returns the tab informations for the given content definition.
   *
   * <p>
   *
  ...
  /**
   * Returns the help information for this value.
   *
   * <p>
   *
   * @param value the value
  ...
  /**
   * Returns the schema type message key.
   *
   * <p>
   *
  ...
  /**
   * Checks if the content value widget is compatible with the new content editor.
   *
   * <p>
   *
  ...
  /**
   * Returns if an element with the given path will be displayed at root level of a content editor
   * tab.
   *
   * <p>
   *
   * @param path the element path
   * @return <code>true</code> if an element with the given path will be displayed at root level of
   *     a content editor tab
   */
  ...
  /**
   * Reads the attribute configuration for the given schema type. May return <code>null</code> if no
   * special configuration was set.
   *
   * <p>
   *
  ...
  private DisplayTypeEvaluator readConfiguration(A_CmsXmlContentValue schemaType, String path) {
    ...
    try {
      if ((cms.getRequestContext().getAttribute(CmsRequestContext.ATTRIBUTE_ADE_CONTEXT_PATH)
              == null)
          && (m_file != null)) {
        cms.getRequestContext()
            .setAttribute(CmsRequestContext.ATTRIBUTE_ADE_CONTEXT_PATH, m_file.getRootPath());
      }
      ...
      if (widget != null) {
        ...
        if (widget instanceof I_CmsADEWidget) {
          ...
          if (!adeWidget.isInternal() && !m_widgetConfigurations.containsKey(widgetName)) {
            CmsExternalWidgetConfiguration externalConfiguration =
                new CmsExternalWidgetConfiguration(
                    widgetName,
    ...
    boolean visible =
        !m_contentHandler.hasVisibilityHandlers()
            || m_contentHandler.isVisible(cms, schemaType, path.substring(1), m_file, m_locale);
    if (!visible) {
      // set the has invisible flag
      m_hasInvisible = true;
    }
    boolean localeSynchronized =
        (m_contentHandler.hasSynchronizedElements()
    ...
    CmsAttributeConfiguration result =
  ...
  /**
   * Reads the default value for the given type.
   *
   * <p>
   *
   * @param schemaType the schema type
  ...
  private CmsType readTypes(CmsXmlContentDefinition xmlContentDefinition, String path) {
    ...
    String typeName =
        (CmsStringUtil.isEmptyOrWhitespaceOnly(path) ? "" : (path + ":"))
    ...
    if (type.isChoice()) {
      ...
      type.addAttribute(
          CmsType.CHOICE_ATTRIBUTE_NAME, choiceType, 1, xmlContentDefinition.getChoiceMaxOccurs());
    }
    ...
    for (I_CmsXmlSchemaType subType : xmlContentDefinition.getTypeSequence()) {
      ...
      if (choiceType != null) {
        choiceType.addAttribute(
            subAttributeName, subEntityType, subType.getMinOccurs(), subType.getMaxOccurs());
      } else {
    ...
    for (int i = 0; i < evaluators.size(); i++) {
      ...
      DisplayType successor =
          ((i + 1) < evaluators.size()) ? evaluators.get(i + 1).getProposedType() : null;
