 ...
 * value.
 *
 * <p>This is useful because the default value used may contain macros, which are expanded every
 * time the change handler is called, rather than only during the initial editor load.
 *
 * <p>The change handler's configuration consists of two strings separated by a pipe symbol:
 *
 * <p>path|default
 *
 * <p>path is the path of the field relative to the changed field which should be filled, while
 * default is the default value to write to the field (possibly containing macros).
 */
