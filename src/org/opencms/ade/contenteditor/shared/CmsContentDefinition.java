...
public class CmsContentDefinition extends org.opencms.acacia.shared.CmsContentDefinition {
  ...
  public CmsContentDefinition(
      List<CmsModelResourceInfo> modelInfos, String newLink, CmsUUID referenceId, String locale) {
   ...
   * @param performedAutocorrection flag indicating the current content has an invalid XML structure
   *     and was auto corrected
   * @param autoUnlock false if the editor should not unlock resources automatically in standalone
   *     mode
  ...
  public CmsContentDefinition(
    ...
    m_externalWidgetConfigurations =
        new ArrayList<CmsExternalWidgetConfiguration>(externalWidgetConfigurations);
   ...
   * @param considerDefaults if default values should be added according to minimum occurrence
   *     settings
   */
    ...
    for (String attributeName : entityType.getAttributeNames()) {
      ...
      if (transferAttributes.contains(attributeName)) {
        ...
        CmsEntityAttribute attribute =
            original != null ? original.getAttribute(attributeName) : null;
        if (attribute != null) {
          if (attributeType.isSimpleType()) {
            ...
            if (considerDefaults) {
              for (int i = attribute.getValueCount();
                  i < entityType.getAttributeMinOccurrence(attributeName);
                  i++) {
                target.addAttributeValue(
                    attributeName, attributeConfigurations.get(attributeName).getDefaultValue());
              }
            }
          } else {
        ...
        if (!attributeType.isSimpleType()) {
          ...
          if (targetAttribute != null) {
            for (int i = 0; i < targetAttribute.getComplexValues().size(); i++) {
              ...
              CmsEntity subOriginal =
                  (originalAttribute != null) && (originalAttribute.getComplexValues().size() > i)
  ...
  /**
   * Returns <code>true</code> if any editor change handlers have been configured for this content
   * type.
   *
   * <p>
   *
   * @return <code>true</code> if any editor change handlers have been configured for this content
   *     type.
   *     <p>
   */
  ...
  /**
   * Returns true if the direct edit flag is set, which means that the editor was opened from the
   * classic direct edit mode.
   *
   * <p>
   *
  ...
  /**
   * Returns if the model file informations are present, in this case no additional data is
   * contained.
   *
   * <p>
   *
