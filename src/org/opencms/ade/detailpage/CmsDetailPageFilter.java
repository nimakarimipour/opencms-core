 ...
 * fixed detail content.
 *
 * <p>Note: Filtering on detail page types is already handled elsewhere, so this class only handles
 * detail pages for a single type.
 */
public class CmsDetailPageFilter {
  ...
  public Stream<CmsDetailPageInfo> filterDetailPages(List<CmsDetailPageInfo> infos) {
    ...
    // the sort method uses a stable sort, so detail page order will be preserved for those entries
    // with the same sort key
    Collections.sort(
        infos2,
        new Comparator<CmsDetailPageInfo>() {
          ...
          private int getSortKey(CmsDetailPageInfo info) {
            ...
            boolean defaultDetailFlag =
                info.getType().equals(CmsGwtConstants.DEFAULT_DETAILPAGE_TYPE);
    ...
    if (!infos2.stream().anyMatch(detailPage -> detailPage.getQualifier() == null)) {
      if ((infos2.size() != 0) && LOG.isWarnEnabled()) {
        LOG.warn(
            "No unqualified detail page entries found in list - probably a configuration error: "
                + infos2);
      }
      return Collections.<CmsDetailPageInfo>emptyList().stream();
    }
    return infos2.stream()
        .filter(info -> (info.getQualifier() == null) || checkQualifier(info.getQualifier()));
  }
