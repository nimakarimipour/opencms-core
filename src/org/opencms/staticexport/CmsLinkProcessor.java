...
public class CmsLinkProcessor extends CmsHtmlParser {
  ...
  private static final String[] EMBED_TAG_LINKED_ATTRIBS =
      new String[] {ATTRIBUTE_SRC, "pluginurl", "pluginspage"};

  /** List of attributes that may contain links for the object tag ("codebase" has to be first). */
  private static final String[] OBJECT_TAG_LINKED_ATTRIBS =
      new String[] {"codebase", "data", "datasrc"};
  ...
  /**
   * Another OpenCms context based on the current users OpenCms context, but with the site root set
   * to '/'.
   */
  ...
  public CmsLinkProcessor(
      CmsObject cms, CmsLinkTable linkTable, String encoding, String relativePath) {
  ...
  /**
   * Escapes all <code>&</code>, e.g. replaces them with a <code>&amp;</code>.
   *
   * <p>
   *
  ...
  /**
   * Unescapes all <code>&amp;amp;</code>, that is replaces them with a <code>&</code>.
   *
   * <p>
   *
   ...
   *
   * <p>Macros are replaced by links.
   *
   * <p>
   *
   ...
   *
   * <p>Links are replaced by macros.
   *
   * <p>
   *
   * @param content the content to process
    ...
    if (tag instanceof LinkTag) {
      ...
      if (TAG_EMBED.equals(tag.getTagName())) {
        ...
        if ((src != null) && !src.startsWith("//")) {
          // link processing does not work for protocol-relative URLs, which were once used in
          // Youtube embed
          // codes.
  ...
  protected void processObjectTag(ObjectTag tag) {
    ...
    for (int i = 0; i < OBJECT_TAG_LINKED_ATTRIBS.length; i++) {
      ...
      if ((i == 0) && (tag.getAttribute(attr) != null)) {
        // if code base is available, the other attributes are relative to it, so do not process
        // them
        break;
      }
    }
   ...
   *
   * <p>if not set, it will be set from the title of the given resource.
   *
   * <p>
   *
    ...
    if (!hasAltAttrib) {
      ...
      if ((internalUri != null) && (m_rootCms != null)) {
        ...
        try {
          value =
              m_rootCms
                  .readPropertyObject(internalUri, CmsPropertyDefinition.PROPERTY_TITLE, false)
                  .getValue();
        } catch (CmsException e) {
  ...
  /**
   * Use the {@link org.opencms.file.wrapper.CmsObjectWrapper} to restore the link in the VFS.
   *
   * <p>
   *
