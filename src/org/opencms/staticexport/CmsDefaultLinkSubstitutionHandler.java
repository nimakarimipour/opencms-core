 ...
 *
 * @since 7.0.2
 * @see CmsLinkManager#substituteLink(org.opencms.file.CmsObject, String, String, boolean) for the
 *     method where this handler is used.
 */
public class CmsDefaultLinkSubstitutionHandler implements I_CmsLinkSubstitutionHandler {

  /**
   * Request context attribute name to make the link substitution handler treat the link like an
   * image link.
   *
   * <p>
   */
  ...
  /**
   * Key for a request context attribute to control whether the getRootPath method uses the current
   * site root for workplace requests. The getRootPath method clears this attribute when called.
   */
  public static final String DONT_USE_CURRENT_SITE_FOR_WORKPLACE_REQUESTS =
  ...
  /**
   * Prefix used for request context attributes to control whether a different site root should be
   * used in appendServerPrefix.
   */
   ...
   *
   * <p>If the target URI contains no site information, but starts with the opencms context, the
   * context is removed:
   *
   * <pre>
   ...
   * from the provided OpenCms user context:
   *
   * <pre>
   * /folder/page.html -> /sites/mysite/folder/page.html</pre>
   *
   * If the path of the target URI is relative, i.e. does not start with "/", the path will be
   * prefixed with the current site and the given relative path, then normalized. If no relative
   * path is given, <code>null</code> is returned. If the normalized path is outsite a site, null is
   * returned.
   *
   * <pre>
   ...
   * ../../page.html -> null</pre>
   *
   * If the target URI contains a scheme/server name that denotes an opencms site, it is replaced by
   * the appropriate site path:
   *
   * <pre>
   * http://www.mysite.de/folder/page.html -> /sites/mysite/folder/page.html</pre>
   *
   * <p>If the target URI contains a scheme/server name that does not match with any site, or if the
   * URI is opaque or invalid, <code>null</code> is returned:
   *
   * <pre>
   * http://www.elsewhere.com/page.html -> null
   * mailto:someone@elsewhere.com -> null</pre>
   *
   * @see org.opencms.staticexport.I_CmsLinkSubstitutionHandler#getLink(org.opencms.file.CmsObject,
   *     java.lang.String, java.lang.String, boolean)
   */
  public String getLink(CmsObject cms, @RUntainted String link, String siteRoot, boolean forceSecure) {
  ...
  public String getLink(
      CmsObject cms, @RUntainted String link, @RUntainted String siteRoot, String targetDetailPage, boolean forceSecure) {
    ...
    try {
      ...
      if (!rootVfsName.startsWith(CmsWorkplace.VFS_PATH_WORKPLACE)) {
        // never use the ADE manager for workplace links, to be sure the workplace stays usable in
        // case of configuration errors
        I_CmsDetailPageHandler finder = OpenCms.getADEManager().getDetailPageHandler();
        detailPage =
            finder.getDetailPage(
                cms, rootVfsName, cms.getRequestContext().getUri(), targetDetailPage);
      }
      if (detailPage != null) {
        ...
        try {
          ...
          vfsName =
              CmsStringUtil.joinPaths(
                  detailPage, cms.getDetailName(element, locale, defaultLocales), "/");
    ...
    if (cms.getRequestContext().getCurrentProject().isOnlineProject()) {
      ...
      if (exportManager.relativeLinksInExport(cms.getRequestContext().getSiteRoot() + oriUri)) {
        ...
        if (uriBaseName == null) {
          ...
          if (exportManager.isExportLink(cms, oriUri)) {
            ...
            CmsPair<String, String> uriParamPair =
                addVfsPrefix(cms, oriUri, targetSite, parameters);
      ...
      // (We really need the target site root in the cache key, because different resources with the
      // same site paths
      // but in different sites may have different export settings. It seems we don't really need
      // the site root
      // from the request context as part of the key, but we'll leave it in to make sure we don't
      // break anything.)
      ...
      if (resultLink == null) {
        ...
        try {
          ...
          if (exportManager.isExportLink(cms, vfsName)) {
            ...
            CmsPair<String, String> uriParamPair =
                addVfsPrefix(cms, vfsName, targetSite, parameters);
      ...
      if (targetSite.hasSecureServer() || currentSite.hasSecureServer()) {
        ...
        if (!vfsName.startsWith(CmsWorkplace.VFS_PATH_SYSTEM)) {
          ...
          boolean hasIsImageLinkAttr =
              Boolean.parseBoolean("" + cms.getRequestContext().getAttribute(ATTR_IS_IMAGE_LINK));
          if ((linkType != imageId) && !hasIsImageLinkAttr) {
            ...
            if (detailContent == null) {
              ...
              secureLink =
                  isDetailPageLinkSecure(cms, detailPage, detailContent, targetSite, secureRequest);
            }
      ...
      if (OpenCms.getRunLevel() >= OpenCms.RUNLEVEL_3_SHELL_ACCESS) {
        ...
        CmsPair<@RUntainted String, String> uriParamPair = addVfsPrefix(cms, vfsName, targetSite, parameters);
        resultLink = uriParamPair.getFirst();
        parameters = uriParamPair.getSecond();
      }
  ...
  /**
   * @see
   *     org.opencms.staticexport.I_CmsLinkSubstitutionHandler#getRootPath(org.opencms.file.CmsObject,
   ...
   *
   * <p>This method is required as a hook used in {@link CmsLocalePrefixLinkSubstitutionHandler}.
   *
   * <p>
   *
   ...
   */
  protected CmsPair<@RUntainted String, String> addVfsPrefix(
      CmsObject cms, String vfsName, CmsSite targetSite, @RUntainted String parameters) {

    return new CmsPair<String, String>(
        OpenCms.getStaticExportManager().getVfsPrefix().concat(vfsName), parameters);
  }
  ...
  protected String generateCacheKey(
      CmsObject cms, String targetSiteRoot, String detailPagePart, String absoluteLink) {
  ...
  protected String getSimpleRootPath(CmsObject cms, @RUntainted String targetUri, String basePath) {
    ...
    if (uri.isAbsolute()) {
      ...
      if (OpenCms.getSiteManager().isMatching(targetMatcher)
        ...
        boolean isWorkplaceServer =
            OpenCms.getSiteManager().isWorkplaceRequest(targetMatcher)
                || targetMatcher.equals(cms.getRequestContext().getRequestMatcher());
        if (isWorkplaceServer) {
          ...
          if (targetSiteRoot != null) {
            ...
            selectedPath =
                getRootPathForSite(cms, path, cms.getRequestContext().getSiteRoot(), false);
            String pathForMatchedSite =
                getRootPathForSite(
                    cms,
                    path,
            ...
            try {
              cms.getRequestContext().setSiteRoot("");
              // the path for the current site normally is preferred, but if it doesn't exist and
              // the path for the matched site
    ...
    if ((context != null)
        && (path.startsWith(context + "/") || (path.startsWith(vfsPrefix + "/")))) {
    ...
    if (CmsStringUtil.isNotEmpty(path) && (path.charAt(0) != '/')) {
      if (basePath != null) {
        ...
        absolutePath =
            CmsLinkManager.getAbsoluteUri(path, cms.getRequestContext().addSiteRoot(basePath));
        if (OpenCms.getSiteManager().getSiteRoot(absolutePath) != null) {
          return absolutePath + suffix;
        }
        // HACK: some editor components (e.g. HtmlArea) mix up the editor URL with the current
        // request URL
        absolutePath =
            CmsLinkManager.getAbsoluteUri(
                path, cms.getRequestContext().getSiteRoot() + CmsWorkplace.VFS_PATH_EDITORS);
        if (OpenCms.getSiteManager().getSiteRoot(absolutePath) != null) {
          return absolutePath + suffix;
        }
  ...
  protected boolean isDetailPageLinkSecure(
    ...
    try {
      ...
      LOG.error(
          "Error while checking whether detail page link should be secure: "
  ...
  protected boolean isSecureLink(
      CmsObject cms, String vfsName, CmsSite targetSite, boolean secureRequest) {

    return OpenCms.getStaticExportManager()
        .isSecureLink(cms, vfsName, targetSite.getSiteRoot(), secureRequest);
  }
  ...
  /**
   * Gets the suffix (query + fragment) of the URI.
   *
   * <p>
   *
   * @param uri the URI
  ...
  /**
   * Tries to interpret the given URI as a detail page URI and returns the detail content's root
   * path if possible.
   *
   * <p>If the given URI is not a detail URI, null will be returned.
   *
   * <p>
   *
    ...
    try {
      ...
      CmsUUID detailId =
          OpenCms.getADEManager()
              .getDetailIdCache(cms.getRequestContext().getCurrentProject().isOnlineProject())
              .getDetailId(name);
      if (detailId == null) {
        return null;
      }
  ...
  /**
   * Returns the target site for the given path.
   *
   * <p>
   *
   * @param cms the cms context
