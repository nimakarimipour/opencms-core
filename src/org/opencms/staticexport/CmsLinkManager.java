 ...
 *
 * <p>Since this functionality is closely related to the static export, this class resides in the
 * static export package.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsLinkManager {
   ...
   *
   * <p>If "relativeUri" is already absolute, it is returned unchanged. This method also returns
   * "relativeUri" unchanged if it is not well-formed.
   *
   * <p>
   *
   ...
   */
  public static @RUntainted String getAbsoluteUri(@RUntainted String relativeUri, String baseUri) {
  ...
  /**
   * Returns the resource root path for the given target URI in the OpenCms VFS, or <code>null
   * </code> in case the target URI points to an external site.
   *
   * <p>
   *
   ...
   * @return the resource root path for the given target URI in the OpenCms VFS, or <code>null
   *     </code> in case the target URI points to an external site
   * @deprecated use {@link #getRootPath(CmsObject, String, String)} instead, obtain the link
   *     manager with {@link OpenCms#getLinkManager()}
   */
  @Deprecated
  public static String getSitePath(CmsObject cms, String basePath, @RUntainted String targetUri) {
   ...
   *
   * <p>The scheme component is something like <code>http:</code> or <code>ftp:</code>.
   *
   * <p>
   *
  ...
  public static boolean hasScheme(String uri) {
    ...
    int pos = uri.indexOf(':');
    // don't want to be misguided by a potential ':' in the query section of the URI (is this
    // possible / allowed?)
  ...
  /**
   * Returns <code>true</code> in case the given URI is absolute.
   *
   * <p>An URI is considered absolute if one of the following is true:
   *
   * <ul>
  ...
  /**
   * Given a path to a VFS resource, the method removes the OpenCms context, in case the path is
   * prefixed by that context.
   *
   ...
   *
   * <p>Like <code>http://site.enterprise.com:8080/index.html</code>.
   *
   * <p>In case the resource name is a full root path, the site from the root path will be used.
   * Otherwise the resource is assumed to be in the current site set be the OpenCms user context.
   *
   * <p>Please note that this method will always return the link as it will appear in the "Online"
   * project, that is after the resource has been published. In case you need a method that just
   * returns the link with the full server prefix, use {@link #getServerLink(CmsObject, String)}.
   *
   * <p>
   *
  ...
  public String getOnlineLink(
      CmsObject cms, String resourceName, String targetDetailPage, boolean forceSecure) {
   ...
   *
   * <p>Like <code>
   * http://site.enterprise.com:8080/permalink/4b65369f-1266-11db-8360-bf0f6fbae1f8.html</code>.
   *
   * <p>
   *
   ...
   */
  public String getPermalink(CmsObject cms, @RUntainted String resourceName) {
   ...
   * @return the perma link
   */
  public String getPermalink(CmsObject cms, @RUntainted String resourceName, CmsUUID detailContentId) {
  ...
  /**
   * Returns the perma link for the current page based on the URI and detail content id stored in
   * the CmsObject passed as a parameter.<p<
   *
  ...
  public String getPermalinkForCurrentPage(CmsObject cms) {
    ...
    return getPermalink(
        cms, cms.getRequestContext().getUri(), cms.getRequestContext().getDetailContentId());
  }
   ...
   * </code> in case the link points to an external site.
   *
   * <p>This methods does not support relative target URI links, so the given URI must be an
   * absolute link.
   *
   * <p>See {@link #getRootPath(CmsObject, String)} for a full explanation of this method.
   *
   * <p>
   *
   * @param cms the current users OpenCms context
   * @param targetUri the target URI link
   * @return the resource root path in the OpenCms VFS for the given target URI link, or <code>null
   *     </code> in case the link points to an external site
   * @see #getRootPath(CmsObject, String, String)
   * @since 7.0.2
   */
  public @RUntainted String getRootPath(CmsObject cms, @RUntainted String targetUri) {
   ...
   * </code> in case the link points to an external site.
   *
   * <p>The default implementation applies the following transformations to the link:
   *
   * <ul>
   *   <li>In case the link starts with a VFS prefix (for example <code>/opencms/opencms</code>,
   *       this prefix is removed from the result
   *   <li>In case the link is not a root path, the current site root is appended to the result.
   *       <p>
   *   <li>In case the link is relative, it will be made absolute using the given absolute <code>
   *       basePath</code> as starting point.
   *       <p>
   *   <li>In case the link contains a server schema (for example <code>http://www.mysite.de/</code>
   *       ), which points to a configured site in OpenCms, the server schema is replaced with the
   *       root path of the site.
   *       <p>
   *   <li>In case the link points to an external site, or in case it is not a valid URI, then
   *       <code>null</code> is returned.
   *       <p>
   * </ul>
   *
   * Please note the above text describes the default behavior as implemented by {@link
   * CmsDefaultLinkSubstitutionHandler}, which can be fully customized using the {@link
   * I_CmsLinkSubstitutionHandler} interface.
   *
   * <p>
   *
   * @param cms the current users OpenCms context
   * @param targetUri the target URI link
   * @param basePath path to use as base in case the target URI is relative (can be <code>null
   *     </code>)
   * @return the resource root path in the OpenCms VFS for the given target URI link, or <code>null
   *     </code> in case the link points to an external site
   * @see I_CmsLinkSubstitutionHandler for the interface that can be used to fully customize the
   *     link substitution
   ...
   */
  public @RUntainted String getServerLink(CmsObject cms, String resourceName) {
   ...
   * @see #getOnlineLink(CmsObject, String)
   */
  public @RUntainted String getServerLink(CmsObject cms, String resourceName, boolean forceSecure) {
   ...
   *
   * <p>This should only be used for resources under /system or /shared.<p<
   *
   * @param cms the current OpenCms user context
   * @param resourceName the resource to generate the online link for
   * @param forceSecure forces the secure server prefix
   ...
   * <p>
   *
   * @param cms an OpenCms user context that must have the permissions for role {@link
   *     CmsRole#ROOT_ADMIN}.
   *     <p>
   * @param linkSubstitutionHandler the handler to set
   * @throws CmsRoleViolationException in case the provided OpenCms user context does not have the
   *     required permissions
   */
  public void setLinkSubstitutionHandler(
   ...
   */
  public void setPointerLinkValidationResult(
      CmsExternalLinksValidationResult externLinkValidationResult) {
   ...
   *
   * <p>The result will contain the configured context path and servlet name, and in the case of the
   * "online" project it will also be rewritten according to to the configured static export
   * settings.
   *
   * <p>Should the current site of the given OpenCms user context <code>cms</code> be different from
   * the site root of the given resource, the result will contain the full server URL to the target
   * resource.
   *
   * <p>Please note the above text describes the default behavior as implemented by {@link
   * CmsDefaultLinkSubstitutionHandler}, which can be fully customized using the {@link
   * I_CmsLinkSubstitutionHandler} interface.
   *
   * <p>
  ...
  /**
   * Returns a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   * VFS resource indicated by the given <code>link</code> in the current site, for use on web
   * pages.
   *
   * <p>The provided <code>link</code> is assumed to be the contained in the site currently set in
   * the provided OpenCms user context <code>cms</code>.
   *
   * <p>The result will be an absolute link that contains the configured context path and servlet
   * name, and in the case of the "online" project it will also be rewritten according to to the
   * configured static export settings.
   *
   * <p>In case <code>link</code> is a relative URI, the current URI contained in the provided
   * OpenCms user context <code>cms</code> is used to make the relative <code>link</code> absolute.
   *
   * <p>Please note the above text describes the default behavior as implemented by {@link
   * CmsDefaultLinkSubstitutionHandler}, which can be fully customized using the {@link
   * I_CmsLinkSubstitutionHandler} interface.
   ...
   *
   * @param cms the current OpenCms user context
   * @param link the link to process which is assumed to point to a VFS resource, with optional
   *     parameters
   * @return a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   *     VFS resource indicated by the given <code>link</code> in the current site
   */
  public @RUntainted String substituteLink(CmsObject cms, String link) {
  ...
  /**
   * Returns a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   * VFS resource indicated by the given <code>link</code> and <code>siteRoot</code>, for use on web
   * pages.
   *
   * <p>The result will be an absolute link that contains the configured context path and servlet
   * name, and in the case of the "online" project it will also be rewritten according to to the
   ...
   * OpenCms user context <code>cms</code> is used to make the relative <code>link</code> absolute.
   *
   * <p>The provided <code>siteRoot</code> is assumed to be the "home" of the link. In case the
   * current site of the given OpenCms user context <code>cms</code> is different from the provided
   * <code>siteRoot</code>, the full server prefix is appended to the result link.
   *
   * <p>Please note the above text describes the default behavior as implemented by {@link
   * CmsDefaultLinkSubstitutionHandler}, which can be fully customized using the {@link
   * I_CmsLinkSubstitutionHandler} interface.
   ...
   * Returns a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   * VFS resource indicated by the given <code>link</code> and <code>siteRoot</code>, for use on web
   * pages, using the configured link substitution handler.
   *
   * <p>The result will be an absolute link that contains the configured context path and servlet
   * name, and in the case of the "online" project it will also be rewritten according to to the
   * configured static export settings.
   ...
   * <code>siteRoot</code>, the full server prefix is appended to the result link.
   *
   * <p>A server prefix is also added if
   *
   * <ul>
   *   <li>the link is contained in a normal document and the link references a secure document
   *   <li>the link is contained in a secure document and the link references a normal document
   * </ul>
   *
   * Please note the above text describes the default behavior as implemented by {@link
   * CmsDefaultLinkSubstitutionHandler}, which can be fully customized using the {@link
   ...
   *     parameters
   * @param siteRoot the site root of the <code>link</code>
   * @param forceSecure if <code>true</code> generates always an absolute URL (with protocol and
   *     server name) for secure links
   * @return a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   ...
   * @see CmsDefaultLinkSubstitutionHandler for the default link substitution handler
   */
  public @RUntainted String substituteLink(CmsObject cms, @RUntainted String link, String siteRoot, boolean forceSecure) {
  ...
  /**
   * Returns a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   * VFS resource indicated by the given root path, for use on web pages.
   *
   * <p>The result will contain the configured context path and servlet name, and in the case of the
   * "online" project it will also be rewritten according to to the configured static export
   * settings.
   *
   * <p>Should the current site of the given OpenCms user context <code>cms</code> be different from
   * the site root of the given resource root path, the result will contain the full server URL to
   * the target resource.
   *
   * <p>
   *
  ...
  /**
   * Returns a link <i>from</i> the URI stored in the provided OpenCms user context <i>to</i> the
   * given <code>link</code>, for use on web pages.
   *
   * <p>A number of tests are performed with the <code>link</code> in order to find out how to
   * create the link:
   *
   * <ul>
   *   <li>If <code>link</code> is empty, an empty String is returned.
   *   <li>If <code>link</code> starts with an URI scheme component, for example <code>http://
   *       </code>, and does not point to an internal OpenCms site, it is returned unchanged.
   *   <li>If <code>link</code> is an absolute URI that starts with a configured site root, the site
   *       root is cut from the link and the same result as {@link #substituteLink(CmsObject,
   ...
   *     given <code>link</code>
   */
  public @RUntainted String substituteLinkForUnknownTarget(CmsObject cms, String link, boolean forceSecure) {
  ...
  public String substituteLinkForUnknownTarget(
      CmsObject cms, @RUntainted String link, String targetDetailPage, boolean forceSecure) {

    if (CmsStringUtil.isEmpty(link)) {
      return "";
    }
    @RUntainted String sitePath = link;
   ...
   * Returns the link for the given resource in the current project, with full server prefix.
   *
   * <p>The input link must already have been processed according to the link substitution rules.
   ...
   * @param workplaceLink if this is set, use the workplace server prefix even if we are in the
   *     Online project
   * @return the link for the given resource in the current project, with full server prefix
   */
  private @RUntainted String appendServerPrefix(
      CmsObject cms, @RUntainted String link, String pathWithOptionalParameters, boolean workplaceLink) {
