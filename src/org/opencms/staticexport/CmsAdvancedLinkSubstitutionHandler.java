 ...
 *
 * <p>You can define additional paths that are always used as external links, even if they point to
 * the same configured site than the OpenCms itself.
 *
 * @since 7.5.0
...
public class CmsAdvancedLinkSubstitutionHandler extends CmsDefaultLinkSubstitutionHandler {
  ...
  /**
   * @see
   *     org.opencms.staticexport.CmsDefaultLinkSubstitutionHandler#getLink(org.opencms.file.CmsObject,
  ...
  /**
   * Returns the exclude prefix list.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  /**
   * Reads the link exclude definition file and extracts all excluded links stored in it.
   *
   * <p>
   *
  ...
  private List<String> readLinkExcludes(CmsObject cms) {
    ...
    try {
      ...
      String filePath =
          OpenCms.getSystemInfo().getConfigFilePath(cms, LINK_EXCLUDE_DEFINIFITON_FILE);
      ...
      for (int i = 1; i <= count; i++) {
        ...
        String exclude =
            linkExcludeDefinitions.getStringValue(cms, XPATH_LINK + "[" + i + "]", Locale.ENGLISH);
