...
/**
 * This class represents a file for use by the JLAN server component. It currently just wraps an
 * OpenCms resource.
 *
 * <p>
 */
public class CmsJlanNetworkFile extends NetworkFile {
  ...
  public void flushFile() throws IOException {
    ...
    Boolean ignoreErrors =
        (Boolean)
            m_cms.getRequestContext().getAttribute(CmsJlanRepository.JLAN_IGNORE_WRITE_ERRORS);
    ...
    try {
      if (writeCount > m_lastFlush) {
        ...
        if (file != null) {
          ...
          String rootPath =
              m_cms.getRequestContext().addSiteRoot(CmsJlanDiskInterface.getCmsPath(getFullName()));
  ...
  @Override
  public int readFile(byte[] buffer, int length, int bufferOffset, long fileOffset)
      throws IOException {
    ...
    try {
      ...
      if (m_resource.isFolder()) {
        ...
        List<CmsResource> children =
            m_cms.getResourcesInFolder(
                m_cms.getSitePath(m_resource), CmsJlanDiskInterface.STANDARD_FILTER);
        for (CmsResource child : children) {
          CmsJlanNetworkFile childFile =
              new CmsJlanNetworkFile(m_cms, child, getFullChildPath(child));
  ...
  /**
   * Gets the CmsFile instance for this file, or null if the file contents haven'T been loaded
   * already.
   *
   * <p>
   *
  ...
  protected void load(boolean needContent) throws IOException, CmsException {
    ...
    try {
      ...
      if (m_resource.isFile() && needContent && (!(m_resource instanceof CmsFile))) {
        m_resource =
            m_cms.readFile(m_cms.getSitePath(m_resource), CmsJlanDiskInterface.STANDARD_FILTER);
      }
      if (!m_bufferInitialized && (getFile() != null)) {
        // readResource may already have returned a CmsFile, this is why we need to initialize the
        // buffer
  ...
  /**
   * Replace sequences of consecutive slashes/backslashes to a single backslash.
   *
   * <p>
   *
