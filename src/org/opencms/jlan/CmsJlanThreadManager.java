 ...
 *
 * <p>Since the JLAN server requires its own thread and cannot be run in the same thread as the
 * startup, this creates a new thread solely for starting JLAN.
 *
 * <p>
 */
public class CmsJlanThreadManager {
  ...
  /**
   * The maximum amount of time OpenCms should wait during shutdown after trying to stop the JLAN
   * server.
   */
  ...
  public synchronized void start() {
    ...
    String path =
        OpenCms.getSystemInfo().getAbsoluteRfsPathRelativeToWebInf("config/jlanConfig.xml");
  ...
  /**
   * Tries to stop the JLAN server and return after it is stopped, but will also return if the
   * thread hasn't stopped after MAX_SHUTDOWN_WAIT_MILLIS.
   */
  public synchronized void stop() {
    ...
    if (m_thread != null) {
      ...
      while (m_thread.isAlive()
          && ((System.currentTimeMillis() - timeBeforeShutdownWasCalled)
