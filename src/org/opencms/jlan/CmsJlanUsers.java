 ...
 *
 * <p>Since JLAN requires MD4 password hashes for authentication, which are not by default stored in
 * the database, users who want to use the JLAN functionality should log in at least once into
 * OpenCms normally so that the MD4 hash of their password can be computed and stored.
 *
 * <p>
 */
public class CmsJlanUsers implements UsersInterface {
   ...
   *
   * <p>This is needed because either JLAN or the client cuts off the part before the slash during
   * authentication, so OpenCms never gets to see it. So if we want CIFS authentication for users
   * from non-root OUs, we need to use a different separator.
   *
   * <p>
   *
  ...
  public UserAccount getUserAccount(String userName) {
    ...
    try {
      ...
      if (OpenCms.getTwoFactorAuthenticationHandler().needsTwoFactorAuthentication(user)) {
        LOG.warn(
            "JLAN: Users for who two-factor authentication is enabled cannot access the network share.");
      }
      if (jlanHash != null) {
  ...
  /**
   * @see
   *     org.alfresco.jlan.server.auth.UsersInterface#initializeUsers(org.alfresco.jlan.server.config.ServerConfiguration,
