...
/**
 * Contains the search parameters for a call to <code>
 * {@link org.opencms.search.CmsSearchIndex#search(org.opencms.file.CmsObject, CmsSearchParameters)}
 * </code>.
 *
 * <p>Primary purpose is translation of search arguments to response parameters and from request
 * parameters as well as support for creation of restrictions of several search query parameter
 * sets.
 *
 * <p>
 *
...
public class CmsSearchParameters {
  ...
  public static class CmsSearchFieldQuery {
     ...
     */
    public CmsSearchFieldQuery(
        String fieldName, Occur fieldOccur, List<String> termList, Occur termOccur) {
     ...
     *
     * <p>Please note: Since there is only one term, the ocucr parameter for the term combination is
     * not required and set to <code>null</code>.
     *
     * <p>
     *
     * @param fieldName the field name
  ...
  public static final Sort SORT_DATE_CREATED =
      new Sort(new SortField(CmsSearchField.FIELD_DATE_CREATED, SortField.Type.STRING, true));

  /** Sort result documents by date of last modification, then score. */
  public static final Sort SORT_DATE_LASTMODIFIED =
      new Sort(new SortField(CmsSearchField.FIELD_DATE_LASTMODIFIED, SortField.Type.STRING, true));
  ...
  public static final String[] SORT_NAMES = {
    "SORT_DEFAULT", "SORT_DATE_CREATED", "SORT_DATE_LASTMODIFIED", "SORT_TITLE"
  };
  ...
  public static final Sort SORT_TITLE =
      new Sort(
          new SortField[] {
            new SortField(CmsSearchField.FIELD_TITLE, SortField.Type.STRING), SortField.FIELD_SCORE
          });
  ...
  /**
   * Indicates if all fields should be used for generating the excerpt, regardless if they have been
   * searched or not.
   */
  ...
  /**
   * Only resource that are sub-resource of one of the search roots are included in the search
   * result.
   */
  ...
  /**
   * Creates a new search parameter instance with no search query and default values for the
   * remaining parameters.
   *
   * <p>Before using this search parameters for a search method <code>{@link #setQuery(String)}
  ...
  /**
   * Creates a new search parameter instance with the provided search query and default values for
   * the remaining parameters.
   *
   * <p>Only the "meta" field (combination of content and title) will be used for search. No search
   * root restriction is chosen. No category restriction is used. No category counts are calculated
   * for the result. Sorting is turned off. This is a simple but fast setup.
   *
   * <p>
   *
   ...
   * @param roots only resource that are sub-resource of one of the search roots are included in the
   *     search result
   ...
   * @param calculateCategories if <code>true</code>, the category count is calculated for all
   *     search results (use with caution, this option uses much performance)
   ...
   *
   * <p>If this is used, any setting made with {@link #setQuery(String)} and {@link
   * #setFields(List)} will be ignored and only the individual field search settings will be used.
   *
   * <p>
   *
  ...
  /**
   * Returns the maximum last modification date a resource must have to be included in the search
   * result.
   *
   * <p>
   *
   * @return the maximum last modification date a resource must have to be included in the search
   *     result
   */
  ...
  /**
   * Returns the minimum last modification date a resource must have to be included in the search
   * result.
   *
   * <p>
   *
   * @return the minimum last modification date a resource must have to be included in the search
   *     result
   */
   ...
   *
   * <p>The parsed query is automatically set by the OpenCms search index when a query is created
   * with either {@link #setQuery(String)} or {@link #addFieldQuery(CmsSearchFieldQuery)}. The
   * Lucene query build from the parameters is stored here and can be later used for paging through
   * the results.
   *
   * <p>Please note that this returns only to the query part, not the filter part of the search.
   *
   * <p>
   *
   ...
   *
   * <p>Only resource that are sub-resource of one of the search roots are included in the search
   * result.
   *
   * <p>
   *
  ...
  /**
   * Returns the search index to search in or null if not set before (<code>
   * {@link #setSearchIndex(CmsSearchIndex)}</code>).
   *
   * <p>
   *
   * @return the search index to search in or null if not set before (<code>
   *     {@link #setSearchIndex(CmsSearchIndex)}</code>)
   */
   ...
   *
   * <p>This method is a "sibling" to method <code>{@link #getRoots()}</code> but with the support
   * of being usable with widget technology.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the category count is calculated for all search results.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if fields configured for the excerpt should be used for generating
   * the excerpt only if they have been actually searched in.
   *
   * <p>The default setting is <code>false</code>, which means all text fields configured for the
   * excerpt will be used to generate the excerpt, regardless if they have been searched in or not.
   *
   * <p>Please note: A field will only be included in the excerpt if it has been configured as
   * <code>excerpt="true"</code> in <code>opencms-search.xml</code>. This method controls if so
   * configured fields are used depending on the fields searched, see {@link #setFields(List)}.
   *
   * <p>
   *
   * @return <code>true</code> if fields configured for the excerpt should be used for generating
   *     the excerpt only if they have been actually searched in
   */
  ...
  /**
   * Returns <code>true</code> if the query part should be ignored so that only filters are used for
   * searching.
   *
   * <p>
   *
   * @return <code>true</code> if the query part should be ignored so that only filters are used for
   *     searching
   * @since 8.0.0
   */
   ...
   *
   * <p>This is mainly intended for "search in search result" functions.
   *
   * <p>The restricted query is build of the queries of both parameters, appended with AND.
   *
   * <p>The lists in the restriction for <code>{@link #getFields()}</code>, <code>
   * {@link #getRoots()}</code> and <code>{@link #getCategories()}</code> are <b>intersected</b>
   * with the lists of this search parameters. Only elements contained in both lists are included
   * for the created search parameters. If a list in either the restriction or in this search
   * parameters is <code>null</code>, the list from the other search parameters is used directly.
   *
   * <p>The values for <code>{@link #isCalculateCategories()}</code> and <code>{@link #getSort()}
   * </code> of this parameters are used for the restricted parameters.
   *
   * <p>
   *
  ...
  public @RUntainted CmsSearchParameters restrict(CmsSearchParameters restriction) {
    ...
    if (restriction.getQuery() != null) {
      // don't let Lucene max terms be exceeded in case someone reuses a CmsSearch and continuously
      // restricts
    ...
    // restrict fields
    @RUntainted List<String> fields = null;
    ...
    // restrict roots
    @RUntainted List<String> roots = null;
    if ((m_roots != null) && (m_roots.size() > 0)) {
      if ((restriction.getRoots() != null) && (restriction.getRoots().size() > 0)) {
        ...
        // TODO: This only works if there are equal paths in both parameter sets - for two distinct
        // sets
    ...
    // restrict categories
    @RUntainted List<String> categories = null;
    ...
    @RUntainted List<String> resourceTypes = null;
    ...
    @RUntainted CmsSearchParameters result =
        new CmsSearchParameters(
            query.toString(),
            fields,
            roots,
            categories,
   ...
   * <p>
   *
   * @param flag true if category counts shall be calculated for the corresponding search results or
   *     false if not
   */
   ...
   *
   * <p>Enter an odd value to achieve a nice, "symmetric" output.
   *
   * <p>
   *
  ...
  /**
   * Controls if the excerpt from a field is generated only for searched fields, or for all fields
   * (the default).
   *
   * <p>
   *
   * @param excerptOnlySearchedFields if <code>true</code>, the excerpt is generated only from the
   *     fields actually searched in
  ...
  /**
   * Sets the flag to indicate if the query part should be ignored so that only filters are used for
   * searching.
   *
   * <p>
   *
      ...
      try {
        ...
        if (index == null) {
          throw new CmsException(
              Messages.get().container(Messages.ERR_INDEX_NOT_FOUND_1, indexName));
        }
        setSearchIndex(index);
      } catch (Exception exc) {
        if (LOG.isErrorEnabled()) {
          LOG.error(
              Messages.get().getBundle().key(Messages.LOG_INDEX_ACCESS_FAILED_1, indexName), exc);
        }
      }
    }
  }
  ...
  /**
   * Sets the maximum last modification date a resource must have to be included in the search
   * result.
   *
   * <p>
   *
  ...
  /**
   * Sets the minimum last modification date a resource must have to be included in the search
   * result.
   *
   * <p>
   *
   ...
   * the results.
   *
   * <p>Please note that this applies only to the query part, not the filter part of the search.
   *
   * <p>
   *
   ...
   *
   * <p>The decoding here is tailored for query strings that are additionally manually UTF-8 encoded
   * at client side (javascript) to get around an issue with special chars in applications that use
   * non- UTF-8 encoding (e.g. ISO-8859-1) OpenCms applications. It is not recommended to use this
   * with front ends that don't encode manually as characters like sole "%" (without number suffix)
  ...
  public void setQuery(@RUntainted String query) {
    ...
    // for use with widgets the exception is thrown here to enforce the error message next to the
    // widget
  ...
  /**
   * Sets the internal member of type <code>{@link Sort}</code> according to the given sort name.
   *
   * <p>For a list of valid sort names, please see <code>{@link #SORT_NAMES}</code>.
   *
   * <p>
   *
