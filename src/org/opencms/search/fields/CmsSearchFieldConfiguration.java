...
public class CmsSearchFieldConfiguration extends A_CmsSearchFieldConfiguration {
   ...
   * Map)}. The method adds the configured fields.
   *
  ...
  /**
   * Creates the Lucene Document with this field configuration for the provided VFS resource, search
   * index and content.
   *
  ...
  public I_CmsSearchDocument createDocument(
    ...
    document =
        appendContentBlob(document, cms, resource, extraction, properties, propertiesSearched);
    ...
    document =
        appendCategories(document, cms, resource, extraction, properties, propertiesSearched);
    document =
        appendFieldMappings(document, cms, resource, extraction, properties, propertiesSearched);
    document =
  ...
  /**
   * Overriding this method allows to append some 'extra' values/fields to a document without
   * overriding the {@link #createDocument} method itself.
   *
   * <p>The method {@link #createDocument} reads all properties of the current resource which is an
   * expensive operation. In order to avoid reading those properties twice, this method has been
   * introduced.
   *
   * <p>Compared with all the other appender methods the name of this method is generic.
   *
   * <p>In this default implementation the document is returned unchanged.
   *
   * <p>
   *
  ...
  protected I_CmsSearchDocument appendDates(
    ...
    document.addDateField(
        CmsSearchField.FIELD_DATE_LASTMODIFIED, resource.getDateLastModified(), true);
  ...
  protected I_CmsSearchDocument appendFieldMapping(
    ...
    for (I_CmsSearchFieldMapping mapping : field.getMappings()) {
      String mapResult =
          mapping.getStringValue(cms, resource, extractionResult, properties, propertiesSearched);
      if (mapResult != null) {
        if (text.length() > 0) {
          text.append('\n');
        }
