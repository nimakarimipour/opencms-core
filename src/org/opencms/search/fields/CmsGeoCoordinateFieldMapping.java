...
public class CmsGeoCoordinateFieldMapping implements I_CmsSearchFieldMapping {
      ...
      try {
        switch (entry.getType()) {
          ...
          case link:
            if (depth >= MAX_DEPTH) {
              LOG.error(
                  "maximum depth exceeded for linked geo-coordinate mapping in "
                      + res.getRootPath());
              return null;
            }
            ...
            for (String path : paths) {
              try {
                ...
                CmsExtractionResult linkExtractionResult =
                ...
                if ((linkExtractionResult != null) && (contentHolder[0] != null)) {
                  I_CmsXmlContentHandler linkContentHandler =
                      contentHolder[0].getContentDefinition().getContentHandler();
                  CmsGeoMappingConfiguration linkMappingConfig =
                      linkContentHandler.getGeoMappingConfiguration();
   ...
   * result. If not available, search for a coordinates value in other locales. In the case of a
   * multi-valued field, only the first coordinates value is returned. Further values are ignored.
   *
