...
public class CmsSearchFieldMapping implements I_CmsSearchFieldMapping {
      ...
      case 3: // item (retrieve value for the given XPath from the content items)
        if ((extractionResult != null) && CmsStringUtil.isNotEmptyOrWhitespaceOnly(getParam())) {
          ...
          if (paramParts.length > 1) {
            ...
            localizedContentItems =
                extractionResult.getContentItems(CmsLocaleManager.getLocale(paramParts[0].trim()));
            ...
            switch (attribute) {
                ...
                content =
                    m_isLucene
                        ? DateTools.timeToString(
                            res.getDateContent(), DateTools.Resolution.MILLISECOND)
              ...
              case dateCreated:
                content =
                    m_isLucene
                        ? DateTools.timeToString(
                            res.getDateCreated(), DateTools.Resolution.MILLISECOND)
              ...
              case dateExpired:
                if (m_isLucene) {
                  ...
                  if (expirationDate == CmsResource.DATE_EXPIRED_DEFAULT) {
                    ...
                    content =
                        DateTools.timeToString(expirationDate, DateTools.Resolution.MILLISECOND);
                  }
                } else {
              ...
              case dateReleased:
                content =
                    m_isLucene
                        ? DateTools.timeToString(
                            res.getDateReleased(), DateTools.Resolution.MILLISECOND)
    ...
    if (mappingType == null) {
      ...
      throw new CmsRuntimeException(
          new CmsMessageContainer(
              Messages.get(), Messages.ERR_FIELD_TYPE_UNKNOWN_1, new Object[] {type}));
    }
    setType(mappingType);
  }

  /**
   * Returns a "\n" separated String of values for the given XPath if according content items can be
   * found.
   *
   * <p>
   *
   ...
   * @return a "\n" separated String of element values found in the content items for the given
   *     XPath
   */
  private String getContentItemForXPath(Map<String, String> contentItems, String xpath) {
    ...
    if (contentItems.get(xpath) != null) { // content item found for XPath
      ...
      SortedMap<List<Integer>, String> valueMap =
          new TreeMap<>(
              new Comparator<List<Integer>>() {

                // expects lists of the same length that contain only non-null values. This is given
                // for the use case.
      ...
      for (Map.Entry<String, String> entry : contentItems.entrySet()) {
        if (CmsXmlUtils.removeXpath(entry.getKey())
            .equals(xpath)) { // the removed path refers an item
