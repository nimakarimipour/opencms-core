...
public interface I_CmsSearchIndex extends I_CmsConfigurationParameterHandler, Serializable {
  ...
  /**
   * The method should return the extraction result of a content from the index, if sure the content
   * has not changed since last indexing. This will prevent re-extraction of the content. Preventing
   * re-extraction saves time while indexing, e.g., if only meta-data of a content has changed. If
   * no up-to-date content can be provided from the index, return <code>null</code>.
   *
   * @param resource the resource the content should be provided for.
   * @return the up-to-date extraction result as gained from the index - if possible, or <code>null
   *     </code>, if no up-to-date extraction result can be obtained from the index.
   */
  ...
  /**
   * Returns <code>true</code> if full text is extracted by this index.
   *
   * <p>Full text content extraction can be turned off in the index search configuration parameters
   * in <code>opencms-search.xml</code>. Not extraction the full text information will highly
   * improve performance.
   *
   * <p>
   *
   ...
   * #getRebuildMode()} is either set to {@value #REBUILD_MODE_AUTO} or {@value
   * #REBUILD_MODE_OFFLINE}. Moreover, at least one update must have been written to the index
   * already.
   *
   ...
   *
   * <p>In case the check fails, the <code>enabled</code> property is set to <code>false</code>
   *
   * @param cms a OpenCms user context to perform the checks with (should have "Administrator"
   *     permissions)
   ...
   * @return he document type factory used for the given resource in this index, or <code>null
   *     </code> in case the resource is not indexed by this index
   */
  ...
  /**
   * Returns <code>true</code> if this index is currently disabled.
   *
   * <p>
   *
  ...
  /**
   * Method called by the search manager if the index has changed. Typically the index searcher is
   * reset when the method is called.
   *
   * @param force if <code>false</code> the index might decide itself it it has to act on the
   *     change, if <code>true</code> it should act, even if itself cannot detect an index change.
   */
