...
public final class CmsDocumentDependency {
  ...
  /**
   * Creates a new dependency container for the given VFS resource.
   *
   * <p>
   *
   ...
   * Creates a new dependency container for the given VFS resource.
   *
   * <p>Additional constructor with extra root path, to be used only for test cases.
   *
   * <p>
   *
  ...
  private CmsDocumentDependency(CmsPublishedResource resource, String rootPath) {
    ...
    if (matcher.find()) {
      ...
      locale =
          suffix.length() == 5
              ? new Locale(langString, suffix.substring(3, 5))
    ...
    // we must remove file suffixes like "plapla.doc", ".pdf" etc. because attachments can have
    // different types
        ...
        if (input.startsWith("{")) {
          ...
          if (jsonDoc.has(JSON_MAIN)) {
            ...
            CmsDocumentDependency main =
                new CmsDocumentDependency(null, jsonMain.getString(JSON_PATH));
   ...
   * @return a dependency object for the given parameters
   */
  public static CmsDocumentDependency load(
      CmsObject cms, CmsResource res, List<CmsResource> resources) {
   ...
   *
   * <p><b>Please note:</b> This must be used with caution since the information may be required to
   * generate documents for several configured indexes. It must be ensured that this is called only
   * when all indexes have been updated.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
  ...
  /**
   * Reads the dependency object for the given root path in the OpenCms runtime context.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
    ...
    try {
      ...
      if (json.has(JSON_LANGUAGES)) {
        ...
        for (int i = 0; i < jsonLanguages.length(); i++) {
          ...
          CmsDocumentDependency lang =
              new CmsDocumentDependency(null, jsonLang.getString(JSON_PATH));
      ...
      if (json.has(JSON_ATTACHMENTS)) {
        ...
        for (int i = 0; i < jsonAttachments.length(); i++) {
          try {
            ...
            CmsDocumentDependency att =
                new CmsDocumentDependency(null, jsonAttachment.getString(JSON_PATH));
            ...
            if (jsonAttachment.has(JSON_LANGUAGES)) {
              ...
              for (int j = 0; j < jsonAttLanguages.length(); j++) {
                ...
                CmsDocumentDependency attLang =
                    new CmsDocumentDependency(null, jsonAttLanguage.getString(JSON_PATH));
   ...
   *
   * <p>To be used when incremental updating an index.
   *
   * <p>
   *
  ...
  public void readDependencies(CmsObject cms) {
    ...
    try {
      ...
      List<CmsResource> folderContent =
          cms.getResourcesInFolder(
              CmsResource.getParentFolder(
                  cms.getRequestContext().removeSiteRoot(getResource().getRootPath())),
      ...
      if (r.isFile() && !getResource().getRootPath().equals(r.getRootPath())) {
        CmsPublishedResource pubRes =
            new CmsPublishedResource(
                r, getResource().getPublishTag(), CmsResourceState.STATE_CHANGED);
   ...
   *
   * <p>This done to optimize indexing speed. When the index update information is calculated, all
   * dependencies for a resource must be calculated also. The same information is later needed when
   * the Lucene document is created, for example in order to store the list of other available
   * languages.
   *
   * <p>
   *
