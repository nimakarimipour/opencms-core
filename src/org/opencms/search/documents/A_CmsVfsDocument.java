 ...
 * requires a specialized implementation of <code>
 * {@link I_CmsDocumentFactory#extractContent(CmsObject, CmsResource, I_CmsSearchIndex)}</code> for
 * text extraction from the binary document content.
 *
 * <p>
 *
 * @since 6.0.0
...
public abstract class A_CmsVfsDocument implements I_CmsDocumentFactory {
  ...
  /**
   * Generic type name used as default for all types that are globally unconfigured. Note that any
   * special xml content is already configured if xmlcontent is configured.
   */
  ...
  /**
   * Creates a document factory lookup key for the given resource type name / MIME type
   * configuration.
   *
   * <p>If the given <code>mimeType</code> is <code>null</code>, this indicates that the key should
   ...
   * <p>
   *
   * @see org.opencms.search.documents.I_CmsDocumentFactory#createDocument(CmsObject, CmsResource,
   *     I_CmsSearchIndex)
   */
      ...
      if (content == null) {
        ...
        try {
          ...
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.ERR_TEXT_EXTRACTION_1, resource.getRootPath()),
              e);
        }
      }
    }
  ...
  /**
   * @see org.opencms.search.documents.I_CmsDocumentFactory#getDocumentKeys(java.util.List,
   *     java.util.List)
   */
  public List<String> getDocumentKeys(List<String> resourceTypes, List<String> mimeTypes)
      throws CmsException {
  ...
  /**
   * @see
   *     org.opencms.search.documents.I_CmsDocumentFactory#setCache(org.opencms.search.documents.CmsExtractionResultCache)
   */
  ...
  protected void logContentExtraction(CmsResource resource, I_CmsSearchIndex index) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_EXTRACT_CONTENT_2, resource.getRootPath(), index.getName()));
    }
  }
  ...
  protected CmsFile readFile(CmsObject cms, CmsResource resource)
      throws CmsException, CmsIndexNoContentException {
