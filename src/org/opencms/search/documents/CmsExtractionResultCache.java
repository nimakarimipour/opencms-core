 ...
 *
 * <p>This cache operates on resource file names, plus a hash code calculated from {@link
 * org.opencms.file.CmsResource#getDateLastModified()} and {@link
 * org.opencms.file.CmsResource#getLength()}. Optional a locale can be appended to this name.
 *
 * <p>Since text extraction is done only on the content of a resource, all siblings must have the
 * same content. The difference can be only by the locale setting in case of an XML content or XML
 * page. However, the most problematic contents to extract for the search are in fact the MS Office
 * and PDF formats. For these documents, all siblings must produce the exact same text extraction
 * result.
 *
 * <p>This cache is usable for resources from the online AND the offline project at the same time,
 * because any change to a resource will result in a changed hash code. This means a resource
 * changed in the offline project will have a new hash code compared to the online project. If the
 * resource is identical in the online and the offline project, the generated hash codes will be the
 * same.
 *
 * <p>
 *
...
public class CmsExtractionResultCache {
   ...
   * <p>
   *
   * @param maxAge the maximum age of the extraction result cache files in hours (or fractions of
   *     hours)
              ...
              try {
                ...
                if (LOG.isWarnEnabled()) {
                  LOG.warn(
                      Messages.get()
                          .getBundle()
                          .key(Messages.LOG_EXCERPT_CACHE_DELETE_ERROR_1, f.getAbsolutePath()),
                      e);
                }
              }
            }
  ...
  /**
   * Returns the RFS name used for caching an the text extraction result based on the given VFS
   * resource and locale.
   *
   * <p>
   *
  ...
  public String getCacheName(CmsResource resource, Locale locale, String docTypeName) {
    ...
    // please note that we need only worry about last change in content, since properties are
    // ignored here
    buf.append('_');
  ...
  /**
   * Returns the extraction result in the requested file in the disk cache, or <code>null</code> if
   * the file is not found in the cache, or is found but out-dated.
   *
   * <p>
   *
   * @param rfsName the file RFS name to look up in the cache
   * @return the extraction result stored in the requested file in the RFS disk cache, or <code>null
   *     </code>
   */
