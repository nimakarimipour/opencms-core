...
public class CmsSearchIndex extends A_CmsSearchIndex {
  ...
  public static final String[] DOC_META_FIELDS =
      new String[] {CmsSearchField.FIELD_META, CmsSearchField.FIELD_CONTENT};
  ...
  /**
   * Constant for additional parameter for controlling how many hits are loaded at maximum (default:
   * 1000).
   */
   ...
   * <p>
   */
  protected static final StoredFieldVisitor VISITOR =
      new StoredFieldVisitor() {

        /**
         * @see
         *     org.apache.lucene.index.StoredFieldVisitor#needsField(org.apache.lucene.index.FieldInfo)
         */
        @Override
        public Status needsField(FieldInfo fieldInfo) {

          return !CmsSearchFieldConfiguration.LAZY_FIELDS.contains(fieldInfo.name)
              ? Status.YES
              : Status.NO;
        }
      };
   ...
   *
   * <p>It is recommended to use the constructor <code>{@link #CmsSearchIndex(String)}</code> as it
   * enforces the mandatory name argument.
   *
   * <p>
   */
  ...
  /**
   * Generates a list of date terms for the optimized date range search with "daily" granularity
   * level.
   *
   * <p>How this works:
   *
   * <ul>
   *   <li>For each document, terms are added for the year, the month and the day the document was
   *       modified or created) in. So for example if a document is modified at February 02, 2009,
   *       then the following terms are stored for this document: "20090202", "200902" and "2009".
   *   <li>In case a date range search is done, then all possible matches for the provided rage are
   *       created as search terms and matched with the document terms.
   *   <li>Consider the following use case: You want to find out if a resource has been changed in
   *       the time between November 29, 2007 and March 01, 2009. One term to match is simply "2008"
   *       because if a document was modified in 2008, then it is clearly in the date range. Other
   *       terms are "200712", "200901" and "200902", because all documents modified in these months
   *       are also a certain matches. Finally we need to add terms for "20071129", "20071130" and
   *       "20090301" to match the days in the starting and final month.
   * </ul>
   *
   ...
   *
   * <p>The result will contain dates formatted like "yyyyMMDD", for example "20080131".
   *
   * <p>
   *
   ...
   *
   * <p>The result will contain dates formatted like "yyyyMM", for example "200801".
   *
   * <p>
   *
   ...
   *
   * <p>The result will contain dates formatted like "yyyy", for example "2008".
   *
   * <p>
   *
  ...
  public void addConfigurationParameter(@RUntainted String key, @RUntainted String value) {
    ...
    if (MAX_HITS.equals(key)) {
      try {
        ...
        LOG.error(
            Messages.get().getBundle().key(Messages.LOG_INVALID_PARAM_3, value, key, getName()));
      }
  ...
  public I_CmsExtractionResult getContentIfUnchanged(CmsResource resource) {
    ...
    if ((oldDoc != null)
        && (oldDoc.getFieldValueAsDate(CmsSearchField.FIELD_DATE_CONTENT) != null)) {
      long contentDateIndex =
          oldDoc.getFieldValueAsDate(CmsSearchField.FIELD_DATE_CONTENT).getTime();
   ...
   * @deprecated Use {@link #getDocument(String, String)} instead and provide {@link
   *     org.opencms.search.fields.CmsLuceneField#FIELD_PATH} as field to search in
   */
  @Deprecated
   ...
   *
   * <p>Use this method to search for documents which have unique field values, like a unique id.
   *
   * <p>
   *
   ...
   *
   * <p>The number of maximum documents to load from the index must be specified. The default of
   * this setting is {@link CmsSearchIndex#MAX_HITS_DEFAULT} (5000). This means that at maximum 5000
   * results are returned from the index. Please note that this number may be reduced further
   * because of OpenCms read permissions or per-user file visibility settings not controlled in the
   * index.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if backup re-indexing is done by this index.
   *
   * <p>This is an optimization method by which the old extracted content is reused in order to save
   * performance when re-indexing.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if permissions are checked for search results by this index.
   *
   * <p>If permission checks are not required, they can be turned off in the index search
   * configuration parameters in <code>opencms-search.xml</code>. Not checking permissions will
   * improve performance.
   *
   * <p>This is can be of use in scenarios when you know that all search results are always
   * readable, which is usually true for public websites that do not have personalized accounts.
   *
   * <p>Please note that even if a result is returned where the current user has no read
   * permissions, the user can not actually access this document. It will only appear in the search
   * result list, but if the user clicks the link to open the document he will get an error.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the document time range is checked with a granularity level of
   * seconds for search results by this index.
   *
   * <p>Since OpenCms 8.0, time range checks are always done if {@link
   * CmsSearchParameters#setMinDateLastModified(long)} or any of the corresponding methods are used.
   * This is done very efficiently using optimized Lucene filers. However, the granularity of these
   * checks are done only on a daily basis, which means that you can only find "changes made
   * yesterday" but not "changes made last hour". For normal limitation of search results, a daily
   * granularity should be enough.
   *
   * <p>If time range checks with a granularity level of seconds are required, they can be turned on
   * in the index search configuration parameters in <code>opencms-search.xml</code>. Not checking
   * the time range with a granularity level of seconds will improve performance.
   *
   * <p>By default the granularity level of seconds is turned off since OpenCms 8.0
   *
   * <p>
   *
   * @return <code>true</code> if the document time range is checked with a granularity level of
   *     seconds for search results by this index
   */
   ...
   *
   * <p>If no except is required, generation can be turned off in the index search configuration
   * parameters in <code>opencms-search.xml</code>. Not generating an excerpt will improve
   * performance.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if a resource requires read permission to be included in the result
   * list.
   *
   * <p>
   *
   * @return <code>true</code> if a resource requires read permission to be included in the result
   *     list
   */
   ...
   *
   * <p>The result is returned as List with entries of type I_CmsSearchResult.
   *
   * <p>
   *
    ...
    try {
      ...
      builder =
          appendDateLastModifiedFilter(
              builder, params.getMinDateLastModified(), params.getMaxDateLastModified());
      ...
      if (!params.isIgnoreQuery()) {
        ...
        // please note that we still need both variables in case the query is a MatchAllDocsQuery -
        // see below
        query = fieldsQuery;
      }
      ...
      if (hits != null) {
        long hitCount =
            hits.totalHits.value > hits.scoreDocs.length
        ...
        Set<String> returnFields =
            ((CmsLuceneFieldConfiguration) getFieldConfiguration()).getReturnFields();
        Set<String> excerptFields =
            ((CmsLuceneFieldConfiguration) getFieldConfiguration()).getExcerptFields();
        ...
        for (int i = 0, cnt = 0; (i < hitCount) && (cnt < end); i++) {
          try {
            ...
            if ((isInTimeRange(doc, params)) && (hasReadPermission(searchCms, searchDoc))) {
              ...
              if (cnt >= start) {
                // do not use the resource to obtain the raw content, read it from the lucene
                // document!
                String excerpt = null;
      ...
      throw new CmsSearchException(
          Messages.get().container(Messages.ERR_SEARCH_PARAMS_1, params), e);
    } catch (Exception e) {
      throw new CmsSearchException(
          Messages.get().container(Messages.ERR_SEARCH_PARAMS_1, params), e);
    } finally {
   ...
   *
   * <p>This must be set at least to 50, or this setting is ignored.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param requireViewPermission controls if a resource requires view permission to be displayed in
   *     the result list
   */
   ...
   *
   * <p>This will close the local Lucene index searcher instance.
   *
   * <p>
   */
  @Override
  ...
  /**
   * Returns the name (<code>{@link #getName()}</code>) of this search index.
   *
   * <p>
   *
   ...
   *
   * <p>In case the provided List is null or empty, the original filter is left unchanged.
   *
   * <p>The original filter parameter is extended and also provided as return value.
   *
   * <p>
   *
  ...
  protected BooleanQuery.Builder appendCategoryFilter(
      CmsObject cms, BooleanQuery.Builder filter, List<String> categories) {
   ...
   *
   * <p>If the start time is equal to {@link Long#MIN_VALUE} and the end time is equal to {@link
   * Long#MAX_VALUE} than the original filter is left unchanged.
   *
   * <p>The original filter parameter is extended and also provided as return value.
   *
   * <p>
   ...
   * @return the extended filter clause
   */
  protected BooleanQuery.Builder appendDateCreatedFilter(
      BooleanQuery.Builder filter, long startTime, long endTime) {

    // create special optimized sub-filter for the date last modified search
    Query dateFilter =
        createDateRangeFilter(CmsSearchField.FIELD_DATE_CREATED_LOOKUP, startTime, endTime);
    if (dateFilter != null) {
  ...
  /**
   * Appends a date of last modification filter to the given filter clause that matches the given
   * time range.
   *
   * <p>If the start time is equal to {@link Long#MIN_VALUE} and the end time is equal to {@link
   * Long#MAX_VALUE} than the original filter is left unchanged.
  ...
  protected BooleanQuery.Builder appendDateLastModifiedFilter(
    ...
    // create special optimized sub-filter for the date last modified search
    Query dateFilter =
        createDateRangeFilter(CmsSearchField.FIELD_DATE_LASTMODIFIED_LOOKUP, startTime, endTime);
    if (dateFilter != null) {
      // extend main filter with the created date filter
      filter.add(new BooleanClause(dateFilter, BooleanClause.Occur.MUST));
    }
   ...
   *
   * <p>In case the provided List is null or empty, the current request context site root is
   * appended.
   *
   * <p>The original filter parameter is extended and also provided as return value.
   *
   ...
   * @return the extended filter clause
   */
  protected BooleanQuery.Builder appendPathFilter(
      CmsObject cms, BooleanQuery.Builder filter, List<String> roots) {
  ...
  /**
   * Appends the a resource type filter to the given filter clause that matches all given resource
   * types.
   *
   * <p>In case the provided List is null or empty, the original filter is left unchanged.
   *
  ...
  protected BooleanQuery.Builder appendResourceTypeFilter(
      CmsObject cms, BooleanQuery.Builder filter, List<String> resourceTypes) {
   ...
   *
   * <p>If the start date is equal to {@link Long#MIN_VALUE} and the end date is equal to {@link
   * Long#MAX_VALUE} than <code>null</code> is returned.
   *
   * <p>
   *
  ...
  protected @RUntainted String createIndexBackup() {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_IO_INDEX_BACKUP_CREATE_3, getName(), getPath(), backupPath),
          e);
      backupPath = null;
    } finally {
      if (oldDir != null) {
   ...
   * <p>
   *
   * @param create if <code>true</code> a whole new index is created, if <code>false</code> an
   *     existing index is updated
   ...
   */
  @Override
  protected I_CmsIndexWriter createIndexWriter(boolean create, I_CmsReport report)
    ...
    try {
      ...
      throw new CmsIndexException(
          Messages.get().container(Messages.ERR_IO_INDEX_WRITER_OPEN_2, getPath(), getName()), e);
    }
  ...
  protected Query getMultiTermQueryFilter(String field, String termsStr, List<String> termsList) {
    ...
    if (result == null) {
      ...
      try {
        result =
            termsQuery.createWeight(m_indexSearcher, ScoreMode.COMPLETE_NO_SCORES, 1).getQuery();
  ...
  /**
   * Checks if the OpenCms resource referenced by the result document can be read by the user of the
   * given OpenCms context.
   *
   * <p>Returns the referenced <code>CmsResource</code> or <code>null</code> if the user is not
   * permitted to read the resource.
   *
   * <p>
   *
   * @param cms the OpenCms user context to use for permission testing
   * @param doc the search result document to check
   * @return the referenced <code>CmsResource</code> or <code>null</code> if the user is not
   *     permitted
   */
  ...
  protected CmsResource getResource(
      CmsObject cms, I_CmsSearchDocument doc, CmsResourceFilter filter) {
  ...
  /**
   * Checks if the OpenCms resource referenced by the result document can be read be the user of the
   * given OpenCms context.
   *
   * <p>
   *
  ...
  protected boolean hasReadPermission(CmsObject cms, I_CmsSearchDocument doc) {
    ...
    // Else try to read the resource if this is not possible the user does not have enough
    // permissions
  ...
  protected synchronized void indexSearcherClose(IndexSearcher searcher) {
    ...
    if ((searcher != null) && (searcher.getIndexReader() != null)) {
      try {
        ...
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_INDEX_SEARCHER_CLOSE_1, getName()), e);
      }
    }
  }
   ...
   *
   * <p>In case there is an index searcher still open, it is closed first.
   *
   * <p>For performance reasons, one instance of the index searcher should be kept for all searches.
   * However, if the index is updated or changed this searcher instance needs to be re-initialized.
   *
   * <p>
   *
      ...
      try {
        if (oldSearcher.getIndexReader() instanceof DirectoryReader) {
          IndexReader newReader =
              DirectoryReader.openIfChanged((DirectoryReader) oldSearcher.getIndexReader());
          if (newReader != null) {
        ...
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_INDEX_SEARCHER_REOPEN_1, getName()), e);
      }
    } else {
   ...
   *
   * <p>The creation date and/or the last modification date are checked.
   *
   * <p>
   *
  ...
  protected boolean isInTimeRange(Document doc, CmsSearchParameters params) {
    ...
    try {
      ...
      Date dateCreated =
          DateTools.stringToDate(doc.getField(CmsSearchField.FIELD_DATE_CREATED).stringValue());
   ...
   *
   * <p>Since Lucene 3 apparently the score is no longer calculated by default, but only if the
   * searcher is explicitly told so. This methods checks if, based on the given sort, the score must
   * be calculated.
   *
   * <p>
   *
  ...
  protected boolean isSortScoring(IndexSearcher searcher, Sort sort) {
    ...
    if (sort != null) {
      if ((sort == CmsSearchParameters.SORT_DEFAULT) || (sort == CmsSearchParameters.SORT_TITLE)) {
        ...
        // for all non-defaults: check if the score field is present, in that case we must calculate
        // the score
  ...
  protected boolean needsPermissionCheck(I_CmsSearchDocument doc) {
    ...
    if (!I_CmsSearchDocument.VFS_DOCUMENT_KEY_PREFIX.equals(doc.getType())
      ...
      // this is an unknown VFS resource type (also not the generic "VFS" type of OpenCms before
      // 7.0)
      return false;
    }
    return true;
  ...
  protected void removeIndexBackup(@RUntainted String path) {
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_IO_INDEX_BACKUP_REMOVE_2, getName(), path),
          e);
    }
  }
  ...
  private Map<String, Type> createUninvertingMap() {
    ...
    Map<String, UninvertingReader.Type> uninvertingMap =
        new HashMap<String, UninvertingReader.Type>();
