 ...
 *
 * <p>Typically, the following fields are available for searching:
 *
 * <ul>
 *   <li>title - the title of a resource
 *   <li>keywords - the keywords of a resource
 *   <li>description - the description of a resource
 *   <li>content - the aggregated content of a resource
 *   <li>created - the creation date of a resource
 *   <li>lastmodified - the date of the last modification of a resource
 *   <li>path - the path to display the resource
 *   <li>channel - the channel of a resource
 *   <li>contentdefinition - the name of the content definition class of a resource
 * </ul>
 *
 * @since 6.0.0
 */
public class CmsSearch {
   ...
   * #setField(String[])} will be ignored and only the individual field search settings will be
   * used.
   *
   * <p>When combining occurrences of SHOULD, MUST and MUST_NOT, keep the following in mind: All
   * SHOULD clauses will be grouped and wrapped in one query, all MUST and MUST_NOT clauses will be
   * grouped in another query. This means that at least one of the terms which are given as a SHOULD
   * query must occur in the search result.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if a category overview should be shown as part of the result.
   *
   * <p><b>Please note:</b> The calculation of the category count slows down the search time by an
   * order of magnitude. Make sure that you only use this feature if it's really required! Be
   * especially careful if your search result list can become large (> 1000 documents), since in
   * this case overall system performance will certainly be impacted considerably when calculating
   * the categories.
   *
   * <p>
   *
   ...
   *
   * <p>The key values are Integers representing the page number, the entry holds the corresponding
   * link.
   *
   * <p>
   *
  ...
  public Map<Integer, String> getPageLinks() {
    ...
    String link =
        m_cms.getRequestContext().getUri() + m_parameters.toQueryString() + "&searchPage=";
  ...
  /**
   * Returns the search parameters used for searching, build out of the given individual parameter
   * values.
   *
   * <p>
   *
  ...
  /**
   * Returns the search result for the current query, as a list of <code>{@link CmsSearchResult}
   * </code> objects.
   *
   * <p>
   *
      ...
      if (!m_parameters.isIgnoreQuery()
        ...
        if (m_parameters.getFieldQueries() != null) {
          ...
          for (CmsSearchParameters.CmsSearchFieldQuery fq : m_parameters.getFieldQueries()) {
            for (String keyword : fq.getSearchTerms()) {
              if (CmsStringUtil.isEmpty(keyword) || (keyword.trim().length() < getQueryLength())) {
                ...
                m_lastException =
                    new CmsSearchException(
                        Messages.get()
                            .container(
                                Messages.ERR_QUERY_TOO_SHORT_1, new Integer(getQueryLength())));
          ...
          m_lastException =
              new CmsSearchException(
                  Messages.get()
                      .container(Messages.ERR_QUERY_TOO_SHORT_1, new Integer(getQueryLength())));
  ...
  /**
   * Returns a map of categories (Strings) for the last search result, mapped to the hit count
   * (Integer) of the documents in this category, or <code>null</code> if the categories have not
   * been calculated.
   *
   * <p>
   *
   ...
   *
   * <p>Only resources that are sub-resources of one of the search roots are included in the search
   * result.
   *
   * <p>The search roots are used <i>in addition to</i> the current site root of the user performing
   * the search.
   *
   * <p>By default, the search roots contain only one entry with an empty string.
   *
   * <p>
   *
  ...
  /**
   * Sets the flag that controls calculation of result categories for the next search, use this only
   * if it's really required since the search can become very slow using this option.
   *
   * <p><b>Please note:</b> The calculation of the category count slows down the search time by an
   * order of magnitude. Make sure that you only use this feature if it's really required! Be
   * especially careful if your search result list can become large (> 1000 documents), since in
   ...
   * <p>
   *
   * @param calculateCategories if <code>true</code>, the category count will be calculated for the
   *     next search
   */
  ...
  /**
   * Sets the search categories, all search results must be in one of the categories, the category
   * set must match the indexed category exactly.
   *
   * <p>All categories will automatically be trimmed and lower cased, since search categories are
   * also stored this way in the index.
   *
   * <p>
   *
   ...
   * configured fields are used depending on the fields searched, see {@link #setField(String[])}.
   *
   * <p>
   *
   * @param value if <code>true</code>, the excerpt is generated only from the fields actually
   *     searched in
   */
   ...
   *
   * <p>If the fields are set to <code>null</code>, or not set at all, the default fields "content"
   * and "meta" are used.
   *
   * <p>For a list of valid field names, see the Interface constants of <code>
   * {@link org.opencms.search.documents.I_CmsDocumentFactory}</code>.
   *
   ...
   *
   * <p>A former search result will be deleted.
   *
   * <p>
   *
   ...
   *
   * <p>The syntax of the query depends on the search engine used. A former search result will be
   * deleted.
   *
   * <p>
   *
  ...
  /**
   * Restrict the result of the next search to the results of the last search, restricted with the
   * provided parameters.
   *
   * <p>Use this for "seach in search result" functions.
   *
   * <p>
   *
   ...
   *
   * <p>Works with jsp bean mechanism for request parameter "searchPage" that is generated here for
   * page links.
   *
   * <p>
   *
   ...
   * result.
   *
   * <p>The search roots set here are used <i>in addition to</i> the current site root of the user
  ...
  private void resetLastResult() {
    ...
    if (!m_parsedQuerySet) {
      // don't reset parsed query if it was deliberately set, otherwise initializing search bean
      // from JSP might fail
