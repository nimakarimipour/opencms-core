...
/**
 * Reduces the importance of the <code>{@link #computeNorm(FieldInvertState)}</code> factor for the
 * <code>{@link org.opencms.search.fields.CmsLuceneField#FIELD_CONTENT}</code> field, while keeping
 * the Lucene default for all other fields.
 *
 * <p>This implementation was added since apparently the default length norm is heavily biased for
 * small documents. In the default, even if a term is found in 2 documents the same number of times,
 * the smaller document (containing less terms) will have a score easily 3x as high as the longer
 * document. Using this implementation the importance of the term number is reduced.
 *
 * <p>Inspired by Chuck Williams WikipediaSimilarity.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsSearchSimilarity extends Similarity {
   ...
   * <code>{@link org.opencms.search.fields.CmsLuceneField#FIELD_CONTENT}</code> field, while
  ...
  /**
   * @see org.apache.lucene.search.similarities.Similarity#scorer(float,
   *     org.apache.lucene.search.CollectionStatistics, org.apache.lucene.search.TermStatistics[])
   */
  @Override
   ...
   *
   * @param v if true, tokens with position increment 0 are ignored when computing the norm,
