...
public class CmsVfsIndexer implements I_CmsIndexer {
  ...
  /**
   * @see org.opencms.search.I_CmsIndexer#deleteResources(org.opencms.search.I_CmsIndexWriter,
   *     java.util.List)
   */
  public void deleteResources(
      I_CmsIndexWriter indexWriter, List<CmsPublishedResource> resourcesToDelete) {
  ...
  /**
   * @see org.opencms.search.I_CmsIndexer#getUpdateData(org.opencms.search.CmsSearchIndexSource,
   *     java.util.List)
   */
  public CmsSearchIndexUpdateData getUpdateData(
    ...
    while (i.hasNext()) {
      ...
      if (!pubRes.getStructureId().isNullUUID()) {
        // use utility method from CmsProject to check if published resource is "inside" this index
        // source
  ...
  /**
   * @see org.opencms.search.I_CmsIndexer#newInstance(org.opencms.file.CmsObject,
   *     org.opencms.report.I_CmsReport, org.opencms.search.I_CmsSearchIndex)
   */
  ...
  /**
   * @see org.opencms.search.I_CmsIndexer#rebuildIndex(org.opencms.search.I_CmsIndexWriter,
   *     org.opencms.search.CmsIndexingThreadManager, org.opencms.search.CmsSearchIndexSource)
   */
  public void rebuildIndex(
    ...
    while (i.hasNext()) {
      ...
      try {
        ...
        resources =
            m_cms.readResources(resourceName, CmsResourceFilter.IGNORE_EXPIRATION.addRequireFile());
      } catch (CmsException e) {
        if (m_report != null) {
          m_report.println(
              Messages.get()
                  .container(
                      Messages.RPT_UNABLE_TO_READ_SOURCE_2, resourceName, e.getLocalizedMessage()),
        ...
        if (LOG.isWarnEnabled()) {
          LOG.warn(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_UNABLE_TO_READ_SOURCE_2, resourceName, m_index.getName()),
              e);
        }
      }
      if (resources != null) {
  ...
  /**
   * @see org.opencms.search.I_CmsIndexer#updateResources(org.opencms.search.I_CmsIndexWriter,
   *     org.opencms.search.CmsIndexingThreadManager, java.util.List)
   */
   ...
   *
   * <p>This method decides if the resource has to be included in the "update" or "delete" list.
   *
   * <p>
   *
   ...
   */
  protected void addResourceToUpdateData(
      CmsPublishedResource pubRes, CmsSearchIndexUpdateData updateData) {

    if (pubRes.getState().isDeleted()) {
      // deleted resource just needs to be removed
      updateData.addResourceToDelete(pubRes);
    } else if (pubRes.getState().isNew()
        || pubRes.getState().isChanged()
        || pubRes.getState().isUnchanged()) {
    ...
    try {
      if (LOG.isInfoEnabled()) {
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DELETING_FROM_INDEX_1, resource.getRootPath()));
      }
  ...
  /**
   * Checks if the published resource is inside the time window set with release and expiration
   * date.
   *
   * <p>
   *
  ...
  protected void updateResource(
      I_CmsIndexWriter writer, CmsIndexingThreadManager threadManager, CmsResource resource) {
    ...
    try {
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_IO_INDEX_DOCUMENT_UPDATE_2, rootPath, m_index.getName()),
            e);
      }
    }
  }
