 ...
 *
 * <p>The indexing of a single resource is wrapped into a thread in order to prevent the overall
 * indexer from hanging.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsIndexingThread extends Thread {
   ...
   *
   * <p>In case the resource could not be indexed, <code>null</code> is returned.
   *
   * <p>
   *
    ...
    try {
      ...
      if (isInterrupted() && LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_ABANDONED_THREAD_FINISHED_1, m_res.getRootPath()));
      }

    } catch (CmsIndexNoContentException e) {
      // Ignore exception caused by empty documents, so that the report is not messed up with error
      // message
      m_report.println(
      ...
      if (m_report != null) {
        ...
        m_report.println(
            org.opencms.report.Messages.get()
                .container(org.opencms.report.Messages.RPT_ARGUMENT_1, exc.toString()),
            I_CmsReport.FORMAT_ERROR);
      }
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_INDEX_RESOURCE_FAILED_2, m_res.getRootPath(), m_index.getName()),
            exc);
      }
      ...
      if (!docOk) {
        ...
        if (m_report != null) {
          ...
          m_report.println(
              Messages.get()
                  .container(
                      Messages.ERR_INDEX_RESOURCE_FAILED_2, m_res.getRootPath(), m_index.getName()),
              I_CmsReport.FORMAT_ERROR);
        }
        if (LOG.isErrorEnabled()) {
          LOG.error(
  ...
  /**
   * Creates a document for the resource without extracting the content. The aim is to get a content
   * indexed, even if extraction runs into a timeout.
   *
   * @return the document for the resource generated if the content is discarded, i.e., only meta
  ...
  protected I_CmsSearchDocument createIndexDocument(
    ...
    if (report != null) {
      report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_SUCCESSION_1, String.valueOf(count)),
