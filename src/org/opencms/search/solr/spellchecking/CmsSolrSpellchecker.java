 ...
 * result of the spellchecking operation contains suggestions for misspelled words and is compatible
 * with the expected structure of the tinyMCE editor.
 */
public final class CmsSolrSpellchecker {
  ...
  public void getSpellcheckingResult(
      final HttpServletResponse res, final ServletRequest servletRequest, final CmsObject cms)
  ...
  private CmsSpellcheckingRequest parseHttpRequest(final ServletRequest req, final CmsObject cms) {
    ...
    final String dict =
        req.getParameter(HTTP_PARAMETER_LANG) == null
            ? LANG_DEFAULT
   ...
   * @return CmsSpellcheckingRequest object that contains parsed parameters or null, if JSON input
   *     is not well defined.
   */
  private CmsSpellcheckingRequest parseJsonRequest(JSONObject jsonRequest) {
    ...
    return new CmsSpellcheckingRequest(
        wordsToCheck.toArray(new String[wordsToCheck.size()]), lang, id);
  }
   ...
   * @return Results of the Solr spell check of type SpellCheckResponse or null if something goes
   *     wrong.
   */
