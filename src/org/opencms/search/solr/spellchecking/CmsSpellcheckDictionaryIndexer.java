...
public final class CmsSpellcheckDictionaryIndexer {
  ...
  /**
   * A regex pattern that applies to the Solr spellcheck directories. Matching string example:
   * "spellchecker_en"
   */
  public static final String INDEXES_REGEX = "spellchecker_[a-z]{2}";

  /**
   * A regex pattern that applies to custom dictionaries. Matching string example:
   * "custom_dict_en.txt"
   */
  public static final String CUSTOM_DICTIONARY = "custom_dict_[a-z]{2}.txt";

  /**
   * A regex pattern that applies to the naming of the dictionary files. Matching string example:
   * "dict_en.txt"
   */
  public static final String DICTIONARY_NAME_REGEX = "dict_[a-z]{2}.txt";

  /**
   * A regex pattern that applies to the naming of zipped dictionary files. Matching string example:
   * "dict_en.zip"
   */
  public static final String ZIP_NAME_REGEX = "dict_[a-z]{2}.zip";

  /**
   * Maximum amount of entries while parsing the dictionary. This variable is needed in order to
   * prevent OutOfMemoryExceptions while parsing large dictionaries. If you encounter such
   * exceptions you can adjust its value to a smaller number.
   */
  private static final int MAX_LIST_SIZE = 100000;

  /**
   * FileFilter implementation that returns only directories whose name matches the spellchecker
   * indices regex.
   */
  private static final FileFilter SPELLCHECKING_DIRECTORY_NAME_FILTER =
      new FileFilter() {
  ...
  public static void parseAndAddDictionaries(SolrClient client, CmsObject cms) {
    ...
    try {
      ...
      final List<CmsResource> resources =
          cms.getResourcesInFolder(DEFAULT_DICTIONARY_DIRECTORY, CmsResourceFilter.DEFAULT_FILES);
      ...
      for (final CmsResource resource : resources) {
        ...
        if (null != lang) {
          ...
          readAndAddDocumentsFromStream(
              client, lang, new ByteArrayInputStream(file.getContents()), documents, true);
      ...
      for (CmsResource resource : resources) {
        ...
        if (zipFileName.matches(ZIP_NAME_REGEX)) {
          ...
          try (ZipInputStream zipStream =
              new ZipInputStream(new ByteArrayInputStream(cmsFile.getContents()))) {
   ...
   * @param commit boolean flag indicating whether a "commit" call should be made after adding the
   *     documents
  ...
  /**
   * Returns the timestamp of the index whose index-built operation lies the furthest back in the
   * past.
   *
   * <p>
   *
  ...
  private static long getOldestIndexDate(CmsObject cms) {
    ...
    if (Long.MAX_VALUE == oldestIndexDate) {
      LOG.warn(
          "It appears that no spellcheck indices have been found in "
  ...
  /**
   * Returns whether the Solr spellchecking index directories are empty (not initiliazed) or not.
   *
   ...
   * @param closeStream boolean flag that determines whether to close the inputstream or not.
   */
