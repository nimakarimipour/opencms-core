...
public class CmsSolrIndex extends CmsSearchIndex {
  ...
  /**
   * Constant for additional parameter to set the maximally processed results (start + rows) for
   * searches with this index. It overwrites the global configuration from {@link
   * CmsSolrConfiguration#getMaxProcessedResults()} for this index.
   */
  public static final String SOLR_SEARCH_MAX_PROCESSED_RESULTS = "search.solr.maxProcessedResults";

  /**
   * Constant for additional parameter to set the fields the select handler should return at
   * maximum.
   */
  public static final String SOLR_HANDLER_ALLOWED_FIELDS = "handle.solr.allowedFields";

  /**
   * Constant for additional parameter to set the number results the select handler should return at
   * maxium per request.
   */
  public static final String SOLR_HANDLER_MAX_ALLOWED_RESULTS_PER_PAGE =
      "handle.solr.maxAllowedResultsPerPage";

  /**
   * Constant for additional parameter to set the maximal number of a result, the select handler
   * should return.
   */
  public static final String SOLR_HANDLER_MAX_ALLOWED_RESULTS_AT_ALL =
      "handle.solr.maxAllowedResultsAtAll";
  ...
  /**
   * Constant for additional parameter to configure an external solr server specifically for the
   * index.
   */
  ...
  private static final CmsResource PSEUDO_RES =
      new CmsResource(
          null, null, null, 0, false, 0, null, null, 0L, null, 0L, null, 0L, 0L, 0, 0, 0L, 0);
  ...
  transient @RUntainted SolrClient m_solr;
  ...
  private @RUntainted int m_handlerMaxAllowedResultsPerPage = -1;

  /** The number of maximally allowed results at all when using the handler. */
  private @RUntainted int m_handlerMaxAllowedResultsAtAll = -1;

  /** Flag, indicating if the handler only works in debug mode. */
  private boolean m_handlerSelectDisabled;

  /**
   * Path to the secret file. Must be under /system/.../ or /shared/.../ and readable by all users
   * that should be able to debug.
   */
  ...
  @RUntainted int m_maxProcessedResults = -2; // special value for not initialized.
   ...
   */
  public CmsSolrIndex(@RUntainted String name) throws CmsIllegalArgumentException {
  ...
  /**
   * @see org.opencms.search.CmsSearchIndex#addConfigurationParameter(java.lang.String,
   *     java.lang.String)
   */
  @Override
        ...
        if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(value)) {
          try {
            ...
            CmsException ex =
                new CmsException(
                    Messages.get()
                        .container(Messages.LOG_SOLR_ERR_POST_PROCESSOR_NOT_EXIST_1, value),
                    e);
      ...
      case SOLR_HANDLER_ALLOWED_FIELDS:
        if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(value)) {
          m_handlerAllowedFields =
              Stream.of(value.split(",")).map(v -> v.trim()).toArray(String[]::new);
        }
        break;
  ...
  /**
   * @see org.opencms.search.CmsSearchIndex#createIndexWriter(boolean,
  ...
  public boolean excludeFromIndex(CmsObject cms, CmsResource resource) {
    ...
    // If this is the default offline index than it is used for gallery search that needs all
    // resources indexed.
    ...
    try {
      ...
      String propValue =
          cms.readPropertyObject(resource, CmsPropertyDefinition.PROPERTY_SEARCH_EXCLUDE, true)
              .getValue();
      if (propValue != null) {
    ...
    if (!USE_ALL_LOCALE.equalsIgnoreCase(getLocale().getLanguage())) {
      // check if any resource default locale has a match with the index locale, if not skip
      // resource
      List<Locale> locales = OpenCms.getLocaleManager().getDefaultLocales(cms, resource);
      Locale match =
          OpenCms.getLocaleManager()
              .getFirstMatchingLocale(Collections.singletonList(getLocale()), locales);
      return (match == null);
    }
    return false;
  }
   ...
   */
  public CmsGallerySearchResultList gallerySearch(
      CmsObject cms, CmsGallerySearchParameters params) {
    ...
    try {
      ...
      for (CmsSearchResource resource : list) {
        ...
        CmsGallerySearchResult result =
            new CmsGallerySearchResult(document, cms, (int) document.getScore(), locale);
  ...
  /**
   * Returns the name of the core of the index. NOTE: Index and core name differ since OpenCms 10.5
   * due to new naming rules for cores in SOLR.
   *
  ...
  /**
   * Version of {@link org.opencms.search.CmsSearchIndex#getDocument(java.lang.String,
   * java.lang.String)} where the returned fields can be restricted.
   *
  ...
  public synchronized I_CmsSearchDocument getDocument(String fieldname, String term, String[] fls) {
    ...
    try {
      ...
      // We could have more than one document due to serial dates. We only want one arbitrary
      // document per id/path
   ...
   */
  @Override
  public Locale getLocaleForResource(
      CmsObject cms, CmsResource resource, List<Locale> availableLocales) {
    ...
    if ((availableLocales != null) && (availableLocales.size() > 0)) {
      result =
          OpenCms.getLocaleManager()
              .getBestMatchingLocale(defaultLocales.get(0), defaultLocales, availableLocales);
    }
    if (result == null) {
      result =
          ((availableLocales != null) && availableLocales.isEmpty())
  ...
  /**
   * Returns the maximal number of results (start + rows) that are processed for each search query
   * unless another maximum is explicitly specified in {@link #search(CmsObject, CmsSolrQuery,
   * boolean, ServletResponse, boolean, CmsResourceFilter, int)}.
   *
  ...
  /**
   * Returns the Solr server URL to connect to for this specific index, or <code>null</code> if no
   * specific URL is configured.
   *
   * @return the Solr server URL to connect to for this specific index, or <code>null</code> if no
   *     specific URL is configured.
   */
  ...
  public void initialize() throws CmsSearchException {
    ...
    if (m_maxProcessedResults == -2) {
      m_maxProcessedResults =
          OpenCms.getSearchManager().getSolrServerConfiguration().getMaxProcessedResults();
    }
    try {
   ...
   * </code>
   *
   * @deprecated Use {@link #search(CmsObject, SolrQuery)} or {@link #search(CmsObject, String)}
   *     instead
   */
  @Override
  @Deprecated
   ...
   */
  public CmsSolrResultList search(CmsObject cms, @RUntainted CmsSolrQuery query) throws CmsSearchException {
   ...
   * Performs a search.
   *
   * <p>Returns a list of 'OpenCms resource documents' ({@link CmsSearchResource}) encapsulated
   * within the class {@link CmsSolrResultList}. This list can be accessed exactly like an {@link
   * List} which entries are {@link CmsSearchResource} that extend {@link CmsResource} and holds the
   * Solr implementation of {@link I_CmsSearchDocument} as member. <b>This enables you to deal with
   * the resulting list as you do with well known {@link List} and work on it's entries like you do
   * on {@link CmsResource}.</b>
   *
   ...
   *       (imagine an index containing less documents than requested rows) we try to guarantee the
   *       requested amount of search results and to provide a working pagination with security
   *       check.
   *   <li>To be sure we get enough documents left even the permission check reduces the amount of
   *       found documents, the rows are multiplied by <code>'5'</code> and the current page
   *       additionally the offset is added. The count of documents we don't have enough permissions
   *       for grows with increasing page number, that's why we also multiply the rows by the
   *       current page count.
   *   <li>Also make sure we perform the permission check for all found documents, so start with the
   *       first found doc.
   * </ul>
   *
   * <b>NOTE:</b> If latter pages than the current one are containing protected documents the total
   * hit count will be incorrect, because the permission check ends if we have enough results found
   * for the page to display. With other words latter pages than the current can contain documents
   * that will first be checked if those pages are requested to be displayed, what causes a
   * incorrect hit count.
   *
   * <p>
   *
   * @param cms the current OpenCms context
   * @param ignoreMaxRows <code>true</code> to return all all requested rows, <code>false</code> to
   *     use max rows
   ...
   */
  public CmsSolrResultList search(CmsObject cms, final @RUntainted CmsSolrQuery query, boolean ignoreMaxRows)
  ...
  /**
   * Like {@link #search(CmsObject, CmsSolrQuery, boolean)}, but additionally a resource filter can
   * be specified. By default, the filter depends on the index.
   *
   * @param cms the current OpenCms context
   * @param ignoreMaxRows <code>true</code> to return all all requested rows, <code>false</code> to
   *     use max rows
   ...
   * @param ignoreMaxRows <code>true</code> to return all all requested rows, <code>false</code> to
   *     use max rows
   * @param response the servlet response to write the query result to, may also be <code>null
   *     </code>
   * @param ignoreSearchExclude if set to false, only contents with search_exclude unset or "false"
   *     will be found - typical for the the non-gallery case
  ...
  public CmsSolrResultList search(
    ...
    return search(
        cms, query, ignoreMaxRows, response, ignoreSearchExclude, filter, getMaxProcessedResults());
  }

  /**
   * Performs the actual search.
   *
   * <p>To provide for correct permissions two queries are performed and the response is fused from
   * that queries:
   *
   * <ol>
   *   <li>a query for permission checking, where fl, start and rows is adjusted. From this query
   *       result we take for the response:
   *       <ul>
   *         <li>facets
   *         <li>spellcheck
   *         <li>suggester
   ...
   *         <li>clusters
   *       </ul>
   *   <li>a query that collects only the resources determined by the first query and performs
   *       highlighting. From this query we take for the response:
   *   <li>result
   *   <li>highlighting
   * </ol>
   *
   ...
   * <ul>
   *   <li>groups
   *   <li>collapse - representatives of the collapsed group might be filtered by the permission
   *       check
   *   <li>expand is disabled
   * </ul>
   *
   * @param cms the current OpenCms context
   * @param query the OpenCms Solr query
   * @param ignoreMaxRows <code>true</code> to return all requested rows, <code>false</code> to use
   *     max rows
   * @param response the servlet response to write the query result to, may also be <code>null
   *     </code>
   * @param ignoreSearchExclude if set to false, only contents with search_exclude unset or "false"
  ...
  public CmsSolrResultList search(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_SOLR_DEBUG_ORIGINAL_QUERY_2, query, getName()));
    }
    ...
    @RUntainted int rows =
        null == query.getRows() ? CmsSolrQuery.DEFAULT_ROWS.intValue() : query.getRows().intValue();
        ...
        LOG.info(
            Messages.get()
                ...
                .key(
                    ...
                    new Object[] {
                      query, getName(), Integer.valueOf(rows), Integer.valueOf(ROWS_MAX)
                    }));
      }
      rows = ROWS_MAX;
    }
    ...
    if (null != query.getParams(QUERY_PARAM_EXPAND)) {
      LOG.info(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_SOLR_INFO_REMOVING_EXPAND_2, query, getName()));
      query.remove("expand");
    }

    float maxScore = 0;

    @RUntainted LocalSolrQueryRequest solrQueryRequest = null;
    @RUntainted SolrCore core = null;
    String[] sortParamValues = query.getParams(QUERY_SORT_NAME);
    boolean sortByScoreDesc =
        (null == sortParamValues)
    ...
    try {
      ...
      @RUntainted CmsSolrQuery checkQuery = query.clone();
      ...
      checkQuery.setFields(
          CmsSearchField.FIELD_TYPE, CmsSearchField.FIELD_SOLR_ID, CmsSearchField.FIELD_PATH);
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_SOLR_DEBUG_CHECK_QUERY_2, checkQuery, getName()));
      }
      ...
      for (SolrDocument doc : checkQueryResponse.getResults()) {
        try {
          ...
          visibleHitCount--;
          LOG.warn(
              Messages.get().getBundle().key(Messages.LOG_SOLR_ERR_RESULT_ITERATION_FAILED_0), e);
        }
      }
      processedResults += checkQueryResponse.getResults().size();

      if ((resultSolrIds.size() < rows) && (processedResults < maxToProcess)) {
        @RUntainted CmsSolrQuery secondCheckQuery = checkQuery.clone();
        ...
        do {
          ...
          secondCheckQuery.setRows(
              Integer.valueOf(
                  Long.valueOf(Math.min(maxToProcess - processedResults, itemsToCheck))
                      .intValue()));
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_SOLR_DEBUG_SECONDCHECK_QUERY_2, secondCheckQuery, getName()));
          }
          ...
          for (SolrDocument doc : secondCheckQueryResponse.getResults()) {
            try {
              ...
              // should not happen, but if it does we want to go on with the next result
              // nevertheless
              visibleHitCount--;
              LOG.warn(
      ...
      List<CmsSearchResource> resourceDocumentList =
          new ArrayList<CmsSearchResource>(resultSolrIds.size());
      ...
      @RUntainted SolrQuery queryForResults = query.clone();
      // we add an additional filter, such that we can only find the documents we want to retrieve,
      // as we figured out in the check query.
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_SOLR_DEBUG_RESULT_QUERY_2, queryForResults, getName()));
      }
              ...
              if (m_postProcessor != null) {
                doc =
                    m_postProcessor.process(
                        searchCms, resource, (SolrInputDocument) searchDoc.getDocument());
              }
      ...
      long processTime =
          System.currentTimeMillis() - startTime - solrPermissionTime - solrResultTime;
      ...
      // we are manipulating the checkQueryResponse to set up the final response, we want to
      // deliver.

      // adjust start, max score and hit count displayed in the result list.
      solrDocumentList.setStart(start);
      Float finalMaxScore =
          sortByScoreDesc ? new Float(maxScore) : checkQueryResponse.getResults().getMaxScore();
      solrDocumentList.setMaxScore(finalMaxScore);
      solrDocumentList.setNumFound(visibleHitCount);

      // Exchange the search parameters in the response header by the ones from the (adjusted)
      // original query.
      NamedList<Object> params =
          ((NamedList<Object>) (checkQueryResponse.getHeader().get(HEADER_PARAMS_NAME)));
      params.clear();
      ...
      checkQueryResponse
          .getResponse()
          .setVal(
              checkQueryResponse.getResponse().indexOf(QUERY_RESPONSE_NAME, 0), solrDocumentList);
      ...
      if (query.getHighlight()) {
        NamedList<Object> highlighting =
            (NamedList<Object>) resultQueryResponse.getResponse().get(QUERY_HIGHLIGHTING_NAME);
      ...
      if (LOG.isDebugEnabled()) {
        @RUntainted Object[] logParams =
            new Object[] {
              ...
              new Long(
                  result.getHighlightEndTime() != 0 ? result.getHighlightEndTime() - startTime : 0)
            };
        LOG.debug(
            query.toString()
                + "\n"
      ...
      if (response != null) {
        ...
        core =
            m_solr instanceof EmbeddedSolrServer
   ...
   *
   * <p>NOTE: Currently not available for HTTP server.
   *
   * <p>
   *
   ...
   */
  public void select(
      ServletResponse response, CmsObject cms, @RUntainted CmsSolrQuery query, boolean ignoreMaxRows)
      throws Exception {
   ...
   */
  public void setSolrServer(@RUntainted SolrClient client) {
   ...
   * @throws CmsSearchException if something goes wrong
   */
  public void spellCheck(ServletResponse res, CmsObject cms, @RUntainted CmsSolrQuery q)
      throws CmsSearchException {
    ...
    try {
      ...
      if (m_postProcessor != null) {
            ...
            if (needsPermissionCheck(searchDoc)) {
              ...
              if (resource != null) {
                ...
                if (m_postProcessor != null) {
                  doc =
                      m_postProcessor.process(
                          cms, resource, (SolrInputDocument) searchDoc.getDocument());
                }
                resourceDocumentList.add(new CmsSearchResource(resource, searchDoc));
                solrDocumentList.add(doc);
              }
        ...
        queryResponse
            .getResponse()
            .setVal(queryResponse.getResponse().indexOf(QUERY_RESPONSE_NAME, 0), solrDocumentList);
      }
      ...
      throw new CmsSearchException(
          Messages.get().container(Messages.LOG_SOLR_ERR_SEARCH_EXECUTION_FAILD_1, q), e);
    } finally {
      if (solrQueryRequest != null) {
        solrQueryRequest.close();
      }
          ...
          if (h instanceof ReplicationHandler) {
            h.handleRequest(
                new LocalSolrQueryRequest(
                    core, CmsRequestUtil.createParameterMap("?command=backup")),
   ...
   * @return <code>true</code> iff the resource mirrored by the search result can be read by the
   *     current user.
   */
  ...
  /**
   * Checks if the current user is allowed to access non-online indexes.
   *
   * <p>To access non-online indexes the current user must be a workplace user at least.
   *
   * <p>
   *
   ...
   */
  private @RUntainted String generateCoreName(final @RUntainted String name) {
  ...
  /**
   * Checks if the query should be executed using the debug mode where the security restrictions do
   * not apply.
   *
  ...
  private boolean isDebug(CmsObject cms, CmsSolrQuery query) {
    ...
    String debugSecret =
        (debugSecretValues == null) || (debugSecretValues.length < 1) ? null : debugSecretValues[0];
    if ((null != debugSecret)
        && !debugSecret.trim().isEmpty()
        && (null != m_handlerDebugSecretFile)) {
      try {
        CmsFile secretFile = cms.readFile(m_handlerDebugSecretFile);
        String secret =
            new String(secretFile.getContents(), CmsFileUtil.getEncoding(cms, secretFile));
  ...
  /**
   * Throws an exception if the request can for security reasons not be performed. Security
   * restrictions can be set via parameters of the index.
   *
   * @param cms the current context.
   * @param query the query.
  ...
  private void throwExceptionIfSafetyRestrictionsAreViolated(
      CmsObject cms, CmsSolrQuery query, boolean isSpell) throws CmsSearchException {

    if (!isDebug(cms, query)) {
      if (isSpell) {
        if (m_handlerSpellDisabled) {
          throw new CmsSearchException(
              Messages.get().container(Messages.GUI_HANDLER_REQUEST_NOT_ALLOWED_0));
        }
      } else {
        ...
        @RUntainted int start = null != query.getStart() ? query.getStart().intValue() : 0;
        @RUntainted int rows =
            null != query.getRows()
                ? query.getRows().intValue()
                : CmsSolrQuery.DEFAULT_ROWS.intValue();
        if ((m_handlerMaxAllowedResultsAtAll >= 0)
            && ((rows + start) > m_handlerMaxAllowedResultsAtAll)) {
          throw new CmsSearchException(
              Messages.get()
                  .container(
        ...
        if ((m_handlerMaxAllowedResultsPerPage >= 0)
            && (rows > m_handlerMaxAllowedResultsPerPage)) {
          throw new CmsSearchException(
              Messages.get()
                  .container(
        ...
        if ((null != m_handlerAllowedFields)
            && (Stream.of(m_handlerAllowedFields).anyMatch(x -> true))) {
          if (query.getFields().equals(CmsSolrQuery.ALL_RETURN_FIELDS)) {
            ...
            for (@RUntainted String requestedField : query.getFields().split(",")) {
              if (Stream.of(m_handlerAllowedFields)
                  .noneMatch(allowedField -> allowedField.equals(requestedField))) {
                throw new CmsSearchException(
                    Messages.get()
                        .container(
  ...
  private void writeResp(
      ServletResponse response, @RUntainted SolrQueryRequest queryRequest, @RUntainted SolrQueryResponse queryResponse)
      throws IOException, UnsupportedEncodingException {

    if (m_solr instanceof EmbeddedSolrServer) {
      @RUntainted SolrCore core = ((EmbeddedSolrServer) m_solr).getCoreContainer().getCore(getCoreName());
      Writer out = null;
      try {
        @RUntainted QueryResponseWriter responseWriter = core.getQueryResponseWriter(queryRequest);
        ...
        if (responseWriter instanceof BinaryQueryResponseWriter) {
          ...
          out =
              ((charset == null) || charset.equalsIgnoreCase(UTF8.toString()))
