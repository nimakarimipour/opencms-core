...
public class CmsSolrIndexWriter implements I_CmsSolrIndexWriter {
  ...
  private int m_commitMs =
      new Long(OpenCms.getSearchManager().getSolrServerConfiguration().getSolrCommitMs())
          .intValue();
  ...
  /**
   * Creates a new index writer based on the provided standard Lucene IndexWriter for the provided
   * OpenCms search index instance.
   *
   * <p>The OpenCms search instance is currently used only for improved logging of the index
   * operations.
   *
   * <p>
   *
  ...
  public CmsSolrIndexWriter(SolrClient client, CmsSolrIndex index) {
    ...
    if (m_index != null) {
      LOG.info(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_SOLR_WRITER_CREATE_2, m_index.getName(), m_index.getPath()));
    }
  }
    ...
    if ((m_server != null) && (m_index != null)) {
      try {
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_SOLR_WRITER_COMMIT_2, m_index.getName(), m_index.getPath()));
    ...
    if ((m_server != null) && (m_index != null)) {
      try {
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_SOLR_WRITER_DELETE_ALL_2, m_index.getName(), m_index.getPath()));
    ...
    if ((m_server != null) && (m_index != null)) {
      try {
        ...
        m_server.deleteByQuery(
            m_index.getCoreName(), "id:" + resource.getStructureId().toString(), m_commitMs);
      } catch (SolrServerException e) {
        throw new IOException(e.getLocalizedMessage(), e);
  ...
  /**
   * @see org.opencms.search.I_CmsIndexWriter#updateDocument(java.lang.String,
  ...
  /**
   * Adds Solr documents to the index for the {@link I_CmsSearchDocument}. Documents for serial
   * dates are added for each occurrence once with the date of the respective occurrence.
   *
   ...
   */
  private void addDocumentInstances(I_CmsSearchDocument document)
      throws SolrServerException, IOException {

    List<String> serialDates =
        document.getMultivaluedFieldAsStringList(CmsSearchField.FIELD_SERIESDATES);
    ...
    if (null != serialDates) {
      ...
      List<String> serialDatesEnd =
          document.getMultivaluedFieldAsStringList(CmsSearchField.FIELD_SERIESDATES_END);
      List<String> serialDatesCurrentTill =
          document.getMultivaluedFieldAsStringList(CmsSearchField.FIELD_SERIESDATES_CURRENT_TILL);
      for (int i = 0; i < serialDates.size(); i++) {
        ...
        inputDoc.setField(
            CmsSearchField.FIELD_INSTANCEDATE + CmsSearchField.FIELD_POSTFIX_DATE, date);
        inputDoc.setField(
            CmsSearchField.FIELD_INSTANCEDATE_END + CmsSearchField.FIELD_POSTFIX_DATE, endDate);
        inputDoc.setField(
        ...
        for (String locale :
          ...
          inputDoc.setField(
              ...
              date);
          inputDoc.setField(
              CmsSearchField.FIELD_INSTANCEDATE_END
                  + "_"
                  + locale
        ...
        // remove fields that should not be part of the index, but were used to transport
