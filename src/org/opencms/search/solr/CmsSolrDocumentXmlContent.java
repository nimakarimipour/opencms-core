...
public class CmsSolrDocumentXmlContent extends A_CmsVfsDocument {
  ...
  /**
   * The gallery name is determined by resolving the macros in a string which can either come from a
   * field mapped to the gallery name, or the title, or from default values for those fields. This
   * class is used to select the value to use and performs the macro substitution.
   */
  private static class GalleryNameChooser {
    ...
    /**
     * Selects the description displayed in the gallery.
     *
     * <p>This method assumes that all the available values have been set via the setters of this
     * class.
     *
    ...
    public String getDescription(@RUntainted Locale locale) throws CmsException {
      ...
      for (@RUntainted String resultCandidateWithMacros :
        ...
        if (!CmsStringUtil.isEmptyOrWhitespaceOnly(resultCandidateWithMacros)) {
          CmsGalleryNameMacroResolver resolver =
              new CmsGalleryNameMacroResolver(m_cms, m_content, locale);
      ...
      result =
          m_cms
              .readPropertyObject(
                  m_content.getFile(), CmsPropertyDefinition.PROPERTY_DESCRIPTION, false)
              .getValue();
      return result;
    }
    ...
    public String getGalleryName(@RUntainted Locale locale) throws CmsException {
      ...
      result =
          m_cms
              .readPropertyObject(m_content.getFile(), CmsPropertyDefinition.PROPERTY_TITLE, false)
              .getValue();
      return result;
    }
    ...
    /**
     * Sets the mapped description value.
     *
     * <p>
     *
  ...
  public static void collectSchemaXpathsForSimpleValues(
      CmsObject cms, CmsXmlContentDefinition def, String path, Set<String> result) {
   ...
   */
  public static CmsExtractionResult extractXmlContent(
      CmsObject cms, CmsResource resource, I_CmsSearchIndex index, Locale forceLocale)
      throws CmsException {
  ...
  public static CmsExtractionResult extractXmlContent(
    ...
    Map<Locale, LinkedHashMap<String, String>> items =
        new HashMap<Locale, LinkedHashMap<String, String>>();
    Map<String, String> fieldMappings = new HashMap<String, String>();
    List<Locale> contentLocales =
        forceLocale != null ? Collections.singletonList(forceLocale) : xmlContent.getLocales();
    ...
    for (@RUntainted Locale locale : contentLocales) {
      ...
      for (String xpath : paths) {
        ...
        try {
          ...
          if (value instanceof CmsXmlDateTimeValue) {
            extracted =
                CmsSearchUtil.getDateAsIso8601(((CmsXmlDateTimeValue) value).getDateTimeValue());
          } else {
            ...
            if (CmsStringUtil.isEmptyOrWhitespaceOnly(extracted)
              ...
              // prevent this for elements of type "OpenCmsHtml", since this causes problematic
              // values
            ...
            try {
              if ((null != potentialLinkValue)
                ...
                if (CmsResourceTypeXmlContent.isXmlContent(linkedRes)
                  ...
                  I_CmsExtractionResult exRes =
                      CmsSolrDocumentXmlContent.extractXmlContent(
                          cms, linkedRes, index, locale, newAlreadyExtracted);
        ...
        if (value instanceof CmsXmlSerialDateValue) {
          if ((null != extracted) && !extracted.isEmpty()) {
            ...
            I_CmsSerialDateBean serialDateBean =
                CmsSerialDateBeanFactory.createSerialDateBean(serialDateValue);
            if (null != serialDateBean) {
              ...
              for (Long eventDate : serialDateBean.getDatesAsLong()) {
                ...
                long endDate =
                    null != serialDateBean.getEventDuration()
                ...
                // To not show them at the day after they ended, one millisecond is removed from the
                // end time
                ...
                currentTillValues
                    ...
                    .append(
                        serialDateValue.isCurrentTillEnd()
                            ? Long.valueOf(
                                serialDateValue.endsAtMidNight()
                                        && (endDate > eventDate.longValue())
                                    ? endDate - 1L
                                    : endDate)
                            : eventDate);
              }
              ...
              fieldMappings.put(
                  CmsSearchField.FIELD_SERIESDATES_CURRENT_TILL, currentTillValues.substring(1));
              fieldMappings.put(
                  CmsSearchField.FIELD_SERIESDATES_TYPE, serialDateValue.getDateType().toString());
            } else {
              LOG.warn(
                  "Serial date value \""
      ...
      for (@RUntainted String xpath : xpaths) {
        ...
        for (String mapping : mappings) {
          ...
          if (mapping.equals(MAPPING_GALLERY_NAME)
            ...
            String defaultValue =
                xmlContent.getHandler().getDefault(cms, xmlContent.getFile(), null, xpath, locale);
            if (mapping.equals(MAPPING_GALLERY_NAME)) {
      ...
      fieldMappings.put(
          CmsSearchFieldConfiguration.getLocaleExtendedName(
                  CmsSearchField.FIELD_DESCRIPTION, locale)
              + "_s",
    ...
    // if the content is locale independent, it should have only one content locale, but that should
    // be indexed for all available locales.
    // TODO: One could think of different indexing behavior, i.e., index only for
    // getDefaultLocales(cms,resource)
    //       But using getAvailableLocales(cms,resource) does not work, because locale-available is
    // set to "en" for all that content.
    if ((xmlContent instanceof CmsXmlContent)
        && ((CmsXmlContent) xmlContent).isLocaleIndependent()) {
      if (forceLocale != null) {
    ...
    // add the locales that have been indexed for this document as item and return the extraction
    // result
  ...
  /**
   * @see
   *     org.opencms.search.documents.CmsDocumentXmlContent#extractContent(org.opencms.file.CmsObject,
   *     org.opencms.file.CmsResource, org.opencms.search.I_CmsSearchIndex)
   */
  @Override
  public I_CmsExtractionResult extractContent(
    ...
    try {
      ...
      for (CmsResource detailContainers :
        ...
        I_CmsExtractionResult containersExtractionResult =
            containerpageExtractor.extractContent(cms, detailContainers, index, locale);
      ...
      throw new CmsIndexException(
          Messages.get().container(Messages.ERR_TEXT_EXTRACTION_1, resource), t);
    }
  }
