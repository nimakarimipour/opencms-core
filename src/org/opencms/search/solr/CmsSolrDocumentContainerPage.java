...
public class CmsSolrDocumentContainerPage extends CmsSolrDocumentXmlContent {
  ...
  public I_CmsExtractionResult extractContent(
    ...
    try {
                ...
                if (shouldExtractElement) {
                  all.add(
                      CmsSolrDocumentXmlContent.extractXmlContent(
                          cms, elementResource, index, forceLocale));
                }
              }
            } catch (Exception e) {
              LOG.debug(
      ...
      Map<Locale, LinkedHashMap<String, String>> multilingualValues =
          new HashMap<Locale, LinkedHashMap<String, String>>(localesAvailable.size());
