 ...
 * source field, replaces any matches with the configured replacement string, and writes the
 * resulting string to the target field.
 *
 * <p>For example, with the configuration listed below, the sequence in field <code>path</code> will
 * be matched against the regex <code>(.*)_([a-z]{2}(?:_[A-Z]{2})?)((?:\.[^\.]*)?)$</code>, where
 * matched parts will be replaced by <code>$1$3</code>, i.e., the first and third group of the
 * match. The resulting sequence will be written to <code>path_remove_locale</code>.
 *
 ...
 * &lt;/processor&gt;</pre>
 *
 * <p>If, e.g., a document with value "document_de.txt" in field <code>source</code> is processed,
 * the field <code>path_remove_locale</code> with value "document.txt will be added.
 *
 * <p>To add the update processor to your installation, define an update processor chain as in the
 * following example.
 *
 * <pre class="prettyprint">
...
public class CmsSolrCopyModifiedUpateProcessorFactory extends UpdateRequestProcessorFactory {
  ...
  /**
   * @see
   *     org.apache.solr.update.processor.UpdateRequestProcessorFactory#getInstance(org.apache.solr.request.SolrQueryRequest,
   ...
   *
   * @see
   *     org.apache.solr.update.processor.UpdateRequestProcessorFactory#init(org.apache.solr.common.util.NamedList)
   */
  @Override
  public void init(NamedList args) {
    ...
    if (null == regex) {
      throw new SolrException(
          ErrorCode.SERVER_ERROR, "Missing required init parameter: " + PARAM_REGEX);
    }
    try {
    ...
    if (null == replacement) {
      throw new SolrException(
          ErrorCode.SERVER_ERROR, "Missing required init parameter: " + PARAM_REPLACEMENT);
    }
    ...
    if (null == source) {
      throw new SolrException(
          ErrorCode.SERVER_ERROR, "Missing required init parameter: " + PARAM_SOURCE);
    }
    ...
    if (null == target) {
      throw new SolrException(
          ErrorCode.SERVER_ERROR, "Missing required init parameter: " + PARAM_TARGET);
    }
