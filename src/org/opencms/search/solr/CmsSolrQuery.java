...
public class CmsSolrQuery extends SolrQuery {
  ...
  /** A constant to add the score field to the result documents. */
  public static final String MINIMUM_FIELDS =
  ...
  /** A constant to add the score field to the result documents. */
  public static final String STRUCTURE_FIELDS =
      CmsSearchField.FIELD_PATH
          + ","
          + CmsSearchField.FIELD_TYPE
          + ","
   ...
   * @param all <code>true</code> to combine the given values with 'AND', <code>false</code> for
   *     'OR'
   * @param useQuotes <code>true</code> to surround the given values with double quotes, <code>false
   *     </code> otherwise
   */
   ...
   *
   * <p>You can initialize the query with an HTTP request parameter then make some method calls and
   * finally re-ensure that the initial request parameters will overwrite the changes made in the
   * meanwhile.
   *
   * <p>
   */
  ...
  public void setCategories(List<String> categories) {
    ...
    if ((categories != null) && !categories.isEmpty()) {
      addFilterQuery(
          CmsSearchField.FIELD_CATEGORY + CmsSearchField.FIELD_DYNAMIC_EXACT,
          categories,
          true,
          true);
    }
   ...
   * Sets date ranges.
   *
   * <p>This call will overwrite all existing date ranges for the given keys (name of the date facet
   * field).
   *
   * <p>The parameter Map uses as:
   *
   * <p>
   *
   * <ul>
   *   <li><code>keys: </code>Solr field name {@link org.opencms.search.fields.CmsSearchField} and
   *   <li><code>values: </code> pairs with min date as first and max date as second {@link
   ...
   * </ul>
   *
   * whereby date is Solr formatted: {@link org.opencms.search.CmsSearchUtil#getDateAsIso8601(Date)}
   *
   * <p>
   *
   * @param dateRanges the ranges map with field name as key and a CmsPair with min date as first
   *     and max date as second
   */
  ...
  public void setGeoFilterQuery(String fieldName, String coordinates, String radius, String units) {
    ...
    String geoFilterQuery =
        CmsSolrQueryUtil.composeGeoFilterQuery(fieldName, coordinates, radius, units);
  ...
  /**
   * Sets the requested return fields, but ensures that at least the 'path' and the 'type', 'id' and
   * 'solr_id' are part of the fields returned field list.
   *
   * <p>
   *
   ...
   *
   * <p>Creates and adds a filter query.
   *
   * <p>
   *
   * @param fieldName the field name to create a filter query on
  ...
  private String createFilterQuery(
      String fieldName, List<String> vals, boolean all, boolean useQuotes) {
  ...
  /**
   * Ensures that at least the 'path' and the 'type', 'id' and 'solr_id' are part of the fields
   * returned field list.
   *
   * <p>
   *
   ...
   *
   * <p>Searches in the given Strings for a ":", then takes the field name part and removes the
   * already set filter queries queries that are matching the same field name.
   *
   * <p>
   *
   * @param fqs the filter query Strings in the format <code>fq=fieldname:value</code> that should
   *     be removed
   */
