...
public class CmsIndexingThreadManager {
   ...
   * @param maxModificationsBeforeCommit the maximum number of modifications before a commit in the
   *     search index is triggered
   */
   ...
   *
   * <p>After an indexing thread was started, the manager suspends itself and waits for an amount of
   * time specified by the <code>timeout</code> value. If the timeout value is reached, the indexing
   * thread is aborted by an interrupt signal.
   *
   * <p>
   *
  ...
  public void createIndexingThread(
      CmsVfsIndexer indexer, I_CmsIndexWriter writer, CmsResource res) {

    I_CmsReport report = indexer.getReport();
    m_startedCounter++;
    CmsIndexingThread thread =
        new CmsIndexingThread(indexer.getCms(), res, indexer.getIndex(), m_startedCounter, report);
    ...
    if (thread.isAlive()) {
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            Messages.get().getBundle().key(Messages.LOG_INDEXING_TIMEOUT_1, res.getRootPath()));
      }
      if (report != null) {
        report.println();
        report.print(
   ...
   *
   * <p>The method reports the total number of threads started (equals to the number of indexed
   * files), the number of returned threads (equals to the number of successfully indexed files),
