...
/**
 * Implements the general management and configuration of the search and indexing facilities in
 * OpenCms.
 *
 * <p>
 *
...
public class CmsSearchManager implements I_CmsScheduledJob, I_CmsEventListener {
  ...
  public static final class CmsSearchForceUnlockMode extends A_CmsModeStringEnumeration {
    ...
    public static final CmsSearchForceUnlockMode ONLYFULL =
        new CmsSearchForceUnlockMode("onlyfull");
     ...
     */
    protected CmsSearchForceUnlockMode(@RUntainted String mode) {
  ...
  protected class CmsSearchOfflineHandler implements I_CmsEventListener {
    ...
    public void cmsEvent(CmsEvent event) {
      ...
      switch (event.getType()) {
        ...
        case I_CmsEventListener.EVENT_RESOURCE_DELETED:
          List<CmsResource> eventResources =
              (List<CmsResource>) event.getData().get(I_CmsEventListener.KEY_RESOURCES);
        ...
        case I_CmsEventListener.EVENT_RESOURCES_AND_PROPERTIES_MODIFIED:
          if (I_CmsEventListener.VALUE_CREATE_SIBLING.equals(change)) {
            List<CmsResource> resList =
                (List<CmsResource>) event.getData().get(I_CmsEventListener.KEY_RESOURCES);
            if ((resList != null) && (resList.size() >= 3)) {
              System.out.println(
                  "Sibling creation case, resource = " + resList.get(1).getRootPath());
    ...
    protected synchronized void reIndexResources(List<CmsResource> resources) {
      ...
      List<CmsPublishedResource> resourcesToIndex =
          new ArrayList<CmsPublishedResource>(resources.size());
  ...
  /**
   * The offline indexer thread runs periodically and indexes all resources added by the event
   * handler.
   *
   * <p>
   */
  protected class CmsSearchOfflineIndexThread extends Thread {
    ...
    public void run() {
      ...
      I_CmsReport report =
          new CmsLogReport(m_adminCms.getRequestContext().getLocale(), CmsSearchManager.class);
      ...
      try {
        while (m_isAlive) {
          ...
          if (m_isAlive) {
            ...
            @RUntainted List<CmsPublishedResource> resourcesToIndex = getResourcesToIndex();
            if (resourcesToIndex.size() > 0) {
    ...
    /**
     * Obtains the list of resource to update in the offline index, then optimizes the list by
     ...
     */
    protected @RUntainted List<CmsPublishedResource> getResourcesToIndex() {

      List<CmsPublishedResource> resourcesToIndex = m_handler.getResourcesToIndex();
      List<CmsPublishedResource> result =
          new ArrayList<CmsPublishedResource>(resourcesToIndex.size());
     ...
     */
    protected void startOfflineUpdateThread(
        I_CmsReport report, @RUntainted List<CmsPublishedResource> resourcesToIndex) {

      CmsSearchOfflineIndexWorkThread thread =
          new CmsSearchOfflineIndexWorkThread(report, resourcesToIndex);
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_OI_UPDATE_START_1, Integer.valueOf(resourcesToIndex.size())));
      }
    ...
    /**
     * Helper method which changes the states of resources which are to be indexed but have the
     * wrong path to 'deleted'. This is needed to deal with moved resources, since the documents
     * with the old paths must be removed from the index,
     *
     * @param resourcesToIndex the resources to index
     * @return the resources to index, but resource states are set to 'deleted' for resources with
     *     outdated paths
     */
    private @RUntainted List<CmsPublishedResource> changeStateOfMoveOriginsToDeleted(
        List<CmsPublishedResource> resourcesToIndex) {
      ...
      for (CmsPublishedResource resource : resourcesToIndex) {
        ...
        if (resource.getRootPath().equals(lastValidPath)
            || resource.getStructureId().isNullUUID()) {
          result.add(resource);
        } else {
          result.add(
   ...
   *
   * <p>This was decoupled from the main {@link CmsSearchOfflineIndexThread} in order to avoid
  ...
  protected class CmsSearchOfflineIndexWorkThread extends Thread {
     ...
     * @param resourcesToIndex the list of {@link CmsPublishedResource} objects to index
     */
    protected CmsSearchOfflineIndexWorkThread(
        I_CmsReport report, List<CmsPublishedResource> resourcesToIndex) {
  ...
  /**
   * Default for the maximum number of modifications before a commit in the search index is
   * triggered (500).
   */
  ...
  /**
   * The default timeout value used for generating a document for the search index (60000 msec = 1
   * min).
   */
  ...
  public static Analyzer getAnalyzer(String className) throws Exception {
    ...
    // since Lucene 3.0 most analyzers need a "version" parameter and don't support an empty
    // constructor
  ...
  /**
   * Returns the Solr index configured with the parameters name. The parameters must contain a
   * key/value pair with an existing Solr index, otherwise <code>null</code> is returned.
   *
   * <p>
   *
  ...
  public static final CmsSolrIndex getIndexSolr(CmsObject cms, Map<String, String[]> params) {
    ...
    if (params != null) {
      indexName =
          params.get(OpenCmsSolrHandler.PARAM_CORE) != null
    ...
    if (index == null) {
      // if there is exactly one index, a missing core / index parameter doesn't matter, since there
      // is no choice.
  ...
  /**
   * Returns <code>true</code> if the index for the given name is a Lucene index, <code>false</code>
   * otherwise.
   *
   * <p>
   *
  ...
  public void addAnalyzer(CmsSearchAnalyzer analyzer) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_ADD_ANALYZER_2, analyzer.getLocale(), analyzer.getClassName()));
    }
  }
   ...
   */
  public void addSearchIndex(@RUntainted I_CmsSearchIndex searchIndex) {
  ...
  public void cmsEvent(CmsEvent event) {
    ...
    switch (event.getType()) {
        ...
        if ((event.getData() != null)
          ...
          indexNames =
              CmsStringUtil.splitAsList(
                  (String) event.getData().get(I_CmsEventListener.KEY_INDEX_NAMES), ",", true);
        }
        try {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    ...
                    .key(
                        ...
                        indexNames == null
                            ? ""
                            : CmsStringUtil.collectionAsString(indexNames, ",")),
                new Exception());
          }
          if (indexNames == null) {
      ...
      case I_CmsEventListener.EVENT_CLEAR_CACHES:
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_EVENT_CLEAR_CACHES_0), new Exception());
        }
        break;
      ...
      case I_CmsEventListener.EVENT_REINDEX_ONLINE:
        ...
        try {
          ...
          @SuppressWarnings("unchecked")
          List<CmsResource> resources =
              (List<CmsResource>) eventData.get(I_CmsEventListener.KEY_RESOURCES);
          ...
          List<CmsPublishedResource> publishedResourcesToIndex =
              resourcesToIndex.stream()
                  .map(res -> new CmsPublishedResource(res))
          ...
          if (LOG.isDebugEnabled()) {
            LOG.error(
                Messages.get()
                    .getBundle()
                    .key(Messages.ERR_EVENT_REINDEX_FAILED_1, event.getData()),
                e);
   ...
   *
   * <p>The analyzer is selected according to the analyzer configuration.
   *
   * <p>
   *
  ...
  public Analyzer getAnalyzer(@RUntainted Locale locale) throws CmsSearchException {
    ...
    if (analyzerConf == null) {
      throw new CmsSearchException(
          Messages.get().container(Messages.ERR_ANALYZER_NOT_FOUND_1, locale));
    }
    ...
    try {
      ...
      throw new CmsSearchException(
          Messages.get().container(Messages.ERR_LOAD_ANALYZER_1, className), e);
    }
   ...
   *
   * <p>The keys in the map are {@link Locale} objects, and the values are {@link CmsSearchAnalyzer}
   * objects.
   *
  ...
  /**
   * Returns the configured Solr home directory <code>null</code> if not set.
   *
   * <p>
   *
  ...
  public List<String> getDocumentTypeKeys(CmsResource resource) {
    ...
    String mimeType =
        OpenCms.getResourceManager().getMimeType(resource.getRootPath(), null, "unknown");
  ...
  /**
   * Returns the document type keys used to specify the correct document factory. One resource
   * typically has more than one key. The document factories are matched in the provided order and
   * the first matching factory is used.
   *
   * <p>The keys for type name "typename" and mimetype "mimetype" would be a subset of:
   *
   * <ul>
   *   <li><code>typename_mimetype</code>
   *   <li><code>typename</code>
   *   <li>if <code>typename</code> is a sub-type of <code>containerpage</code>
   *       <ul>
   *         <li><code>containerpage_mimetype</code>
   *         <li><code>containerpage</code>
   *       </ul>
   *   <li>if <code>typename</code> is a sub-type of <code>xmlcontent</code>
   *       <ul>
   *         <li><code>xmlcontent_mimetype</code>
   *         <li><code>xmlcontent</code>
   *       </ul>
   *   <li><code>__unconfigured___mimetype</code>
   *   <li><code>__unconfigured__</code>
   *   <li><code>__all___mimetype</code>
   *   <li><code>__all__</code>
   *       <ul>
   *         Note that all keys except the "__all__"-keys are only added as long as globally there
   *         is no matching factory for the key. This in particular means that a factory matching
   *         "typename" will never be used if you have a factory for "typename__mimetype" - even if
   *         this is not configured for the used index source. Eventually, the content will not be
   *         indexed in such cases.
   *
    ...
    if (null != resourceType) {
      ...
      if (!m_extractionKeys.contains(currentKey)) {
        ...
        if (!m_extractionKeys.contains(currentKey)) {
          ...
          if (!hasGlobalMatch) {
            result.add(
                A_CmsVfsDocument.getDocumentKey(
                    A_CmsVfsDocument.DEFAULT_ALL_UNCONFIGURED_TYPES, mimeType));
            result.add(
                A_CmsVfsDocument.getDocumentKey(
                    A_CmsVfsDocument.DEFAULT_ALL_UNCONFIGURED_TYPES, null));
          }
        }
      }
  ...
  /**
   * Returns the map from document type keys to document factories with all entries for the provided
   * document type names.
   *
   ...
   */
  public Map<String, I_CmsDocumentFactory> getDocumentTypeMapForTypeNames(
      List<String> documentTypeNames) {

    Map<String, I_CmsDocumentFactory> result = new LinkedHashMap<>();
    if (null != documentTypeNames) {
      // Iterate the list in reverse order to prefer factories that are added by document types
      // listed earlier.
   ...
   *
   * <p>In case no configuration is available with the given name, <code>null</code> is returned.
   *
   * <p>
   *
  ...
  public List<I_CmsSearchFieldConfiguration> getFieldConfigurations() {
    ...
    List<I_CmsSearchFieldConfiguration> result =
        new ArrayList<I_CmsSearchFieldConfiguration>(m_fieldConfigurations.values());
   ...
   *
   * <p>The index must exist, otherwise <code>null</code> is returned.
   *
  ...
  /**
   * Returns an unmodifiable list of all configured <code>{@link I_CmsSearchIndex}</code> instances.
   *
   * <p>
   *
   ...
   * @throws CmsRoleViolationException in case the given opencms object does not have <code>
   *     {@link CmsRole#WORKPLACE_MANAGER}</code> permissions
   */
  public void initialize(CmsObject cms) throws CmsRoleViolationException {
    ...
    m_extractionResultCache =
        new CmsExtractionResultCache(
   ...
   *
   * <p>This methods needs to be called if after a change in the index configuration has been made.
   */
  ...
  public @RUntainted String launch(CmsObject cms, Map<String, String> parameters) throws Exception {
    ...
    List<@RUntainted String> updateList = null;
      ...
      for (int i = 0; i < indexNames.length; i++) {
        ...
        if (manager.getIndex(indexNames[i]) != null) {
          ...
          if (LOG.isWarnEnabled()) {
            LOG.warn(
                Messages.get().getBundle().key(Messages.LOG_NO_INDEX_WITH_NAME_1, indexNames[i]));
          }
        }
      }
    }
    ...
    String finishMessage =
        Messages.get()
            .getBundle()
            .key(Messages.LOG_REBUILD_INDEXES_FINISHED_1, CmsStringUtil.formatRuntime(runTime));
   ...
   *
   * <p>May take some time, because the indexes are updated first.
   *
   * <p>
   */
      ...
      for (int i = 0, n = m_indexes.size(); i < n; i++) {
        ...
        try {
          ...
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.ERR_INDEX_REBUILD_ALL_1, searchIndex.getName()),
              e);
        }
      }
  ...
  public void rebuildIndexes(List<@RUntainted String> indexNames, I_CmsReport report) throws CmsException {
    ...
    try {
      ...
      while (i.hasNext()) {
        @RUntainted String indexName = i.next();
        // get the search index by name
        I_CmsSearchIndex index = getIndex(indexName);
        if (index != null) {
          // update the index
  ...
  public void registerSolrIndex(CmsSolrIndex index) throws CmsConfigurationException {
    ...
    if ((m_solrConfig == null) || !m_solrConfig.isEnabled()) {
      ...
      throw new CmsConfigurationException(
          Messages.get().container(Messages.ERR_SOLR_NOT_ENABLED_0));
    }
    ...
    } else if (m_solrConfig.getServerUrl()
        != null) { // Use the globally configured external Solr-Server if present.
      ...
      if (!dataDir.exists()) {
        ...
        if (CmsLog.INIT.isInfoEnabled()) {
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_SOLR_INDEX_DIR_CREATED_2, index.getName(), index.getPath()));
        }
      }
      File instanceDir =
          new File(
  ...
  public boolean removeSearchFieldConfigurationField(
      I_CmsSearchFieldConfiguration fieldConfiguration, CmsSearchField field) {
   ...
   * @return true if remove was successful, false if preconditions for removal are ok but the given
   *     mapping was unknown.
   * @throws CmsIllegalStateException if the given mapping is the last mapping inside the given
   *     field.
   */
  ...
  /**
   * Removes all indexes included in the given list (which must contain the name of an index to
   * remove).
   *
   * <p>
   *
   ...
   * @throws CmsIllegalStateException if the given indexsource is still used by at least one <code>
   *     {@link I_CmsSearchIndex}</code>.
   */
  public boolean removeSearchIndexSource(CmsSearchIndexSource indexsource)
    ...
    if (referrers.size() > 0) {
      throw new CmsIllegalStateException(
          Messages.get()
              .container(
                  Messages.ERR_INDEX_SOURCE_DELETE_2, indexsource.getName(), referrers.toString()));
    }
   ...
   *
   * <p>A highlighter is a class implementing org.opencms.search.documents.I_TermHighlighter.
   *
   * <p>
   *
  ...
  /**
   * Sets the maximum number of modifications before a commit in the search index is triggered as a
   * string.
   *
   * <p>
   *
  ...
  public void setTimeout(@RUntainted String value) {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_PARSE_TIMEOUT_FAILED_2, value, new Long(DEFAULT_TIMEOUT)),
          e);
   ...
   *
   * <p>This will cause all search indices to be shut down.
   *
   * <p>
   */
   ...
   *
   * <p>Can be used to force an index update when it's not convenient to wait until the offline
   * update interval has eclipsed.
   *
   * <p>Since the offline indexes still need some time to update the new resources, the method waits
   * for at most the configurable <code>maxIndexWaitTime</code> to ensure that updating is finished.
   *
   ...
   * update interval has eclipsed.
   *
   * <p>Since the offline index will still need some time to update the new resources even if it
   * runs directly, a wait time of 2500 or so should be given in order to make sure the index
   * finished updating.
   *
   ...
   *
   * <p>We take transitive dependencies into account and handle cyclic dependencies correctly as
   * well.
   *
  ...
  protected List<CmsPublishedResource> addAdditionallyAffectedResources(
    ...
    if (updateResources.size() > 0) {
      ...
      do {
        additionalResources =
            findRelatedContainerPages(adminCms, updateResourceSet, resourcesToCheck);
   ...
   * @param adminCms an OpenCms user context with Admin permissions
   * @param updateResources the resources to be re-indexed
   * @param updateResourcesToCheck the resources to check additionally affected resources for,
   *     subset of updateResources
    ...
    for (CmsPublishedResource checkedRes : updateResourcesToCheck) {
      try {
        CmsRelationFilter filter =
            CmsRelationFilter.relationsToStructureId(checkedRes.getStructureId());
        ...
        try {
          if (resMan.getResourceType(pubRes.getType()) instanceof CmsResourceTypeXmlContent) {
            if (!isGroup(pubRes.getType())) {
              CmsRelationFilter filter =
                  CmsRelationFilter.relationsToStructureId(pubRes.getStructureId()).filterStrong();
              List<CmsRelation> relations = adminCms.readRelations(filter);
              for (CmsRelation relation : relations) {
                CmsResource res = relation.getSource(adminCms, CmsResourceFilter.ALL);
                if (CmsResourceTypeXmlContainerPage.isContainerPage(res)) {
                  ...
                  if (CmsDetailOnlyContainerUtil.isDetailContainersPage(
                      adminCms, adminCms.getSitePath(res))) {
   ...
   *
   * <p>A map stores document factories keyed by a string representing a colon separated list of Cms
   * resource types and/or mimetypes.
   *
   * <p>The keys of this map are used to trigger a document factory to convert a Cms resource into a
   * Lucene index document.
   *
   * <p>A document factory is a class implementing the interface {@link
      ...
      try {
        ...
        if (className == null) {
          throw new CmsIndexException(
              Messages.get().container(Messages.ERR_DOCTYPE_NO_CLASS_DEF_0));
        }
        if (resourceTypes.size() == 0) {
          throw new CmsIndexException(
              Messages.get().container(Messages.ERR_DOCTYPE_NO_RESOURCETYPE_DEF_0));
        }
        ...
        try {
          ...
          documentFactory =
              (I_CmsDocumentFactory)
                  c.getConstructor(new Class[] {String.class}).newInstance(new Object[] {name});
        } catch (ClassNotFoundException exc) {
          throw new CmsIndexException(
              Messages.get().container(Messages.ERR_DOCCLASS_NOT_FOUND_1, className), exc);
        } catch (Exception exc) {
          throw new CmsIndexException(
              Messages.get().container(Messages.ERR_DOCCLASS_INIT_1, className), exc);
        }
        ...
        for (Iterator<String> keyIt =
                documentFactory.getDocumentKeys(resourceTypes, mimeTypes).iterator();
            keyIt.hasNext(); ) {
   ...
   *
   * <p>This initializes also the list of Cms resources types to be indexed by an index source.
   *
   * <p>
   */
  protected void initSearchIndexes() {

    @RUntainted I_CmsSearchIndex index = null;
    for (int i = 0, n = m_indexes.size(); i < n; i++) {
      ...
      if (index.checkConfiguration(m_adminCms)) {
        ...
        try {
          ...
          if (CmsLog.INIT.isWarnEnabled()) {
            ...
            CmsLog.INIT.warn(
                Messages.get().getBundle().key(Messages.INIT_SEARCH_INIT_FAILED_1, index), e);
          }
        }
      }
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        if (index.isEnabled()) {
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_INDEX_CONFIGURED_2, index, index.getProject()));
        } else {
          CmsLog.INIT.warn(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_INDEX_NOT_CONFIGURED_2, index, index.getProject()));
        }
      }
    }
  }
  ...
  protected boolean shouldUpdateAtAll(I_CmsSearchIndex index) {
    ...
    if (I_CmsSearchIndex.REBUILD_MODE_NEVER.equals(index.getRebuildMode())) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_SKIP_REBUILD_FOR_MODE_NEVER_1, index.getName()));
      return false;
    } else {
      return true;
    }
  ...
  /**
   * Incrementally updates all indexes that have their rebuild mode set to <code>"auto"</code> after
  ...
  protected void updateAllIndexes(
      CmsObject adminCms, @RUntainted CmsUUID publishHistoryId, I_CmsReport report) {
    ...
    try {
      ...
      try {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_READING_CHANGED_RESOURCES_FAILED_1, publishHistoryId),
            e);
        return;
      }
      Set<CmsUUID> bothNewAndDeleted =
          getIdsOfPublishResourcesWhichAreBothNewAndDeleted(publishedResources);
      // When published resources with both states 'new' and 'deleted' exist in the same publish job
      // history, the resource has been moved
      ...
      for (CmsPublishedResource res : publishedResources) {
        ...
        if (res.getState().isDeleted() || res.getState().isNew() || res.getState().isChanged()) {
          if (updateResources.contains(res)) {
            ...
            // in this case we make sure to use the value from the publish list because of the
            // "deleted" flag
            boolean hasMoved =
                bothNewAndDeleted.contains(res.getStructureId())
                    || (res.getMovedState() == CmsPublishedResource.STATE_MOVED_DESTINATION)
                    || (res.getMovedState() == CmsPublishedResource.STATE_MOVED_SOURCE);
            // check it this is a moved resource with source / target info, in this case we need
            // both entries
            if (!hasMoved) {
            ...
            if (!res.getState().isDeleted() && (res.getSiblingCount() > 1)) {
              ...
              try {
                ...
                List<CmsResource> siblings =
                    adminCms.readSiblings(res.getRootPath(), CmsResourceFilter.ALL);
                ...
                if (LOG.isWarnEnabled()) {
                  LOG.warn(
                      Messages.get()
                          .getBundle()
                          .key(Messages.LOG_UNABLE_TO_READ_SIBLINGS_1, res.getRootPath()),
                      e);
                }
              }
            }
   ...
   */
  protected void updateAllIndexes(
      CmsObject adminCms, List<CmsPublishedResource> updateResources, I_CmsReport report) {
          ...
          if (I_CmsSearchIndex.REBUILD_MODE_AUTO.equals(index.getRebuildMode())) {
            ...
            try {
              ...
              LOG.error(
                  Messages.get()
                      .getBundle()
                      .key(Messages.LOG_UPDATE_INDEX_FAILED_1, index.getName()),
                  e);
            }
          }
        }
   ...
   *
   * <p>If the optional List of <code>{@link CmsPublishedResource}</code> instances is provided, the
   * index will be incrementally updated for these resources only. If this List is <code>null</code>
   * or empty, the index will be fully rebuild.
   *
   * <p>
   *
   * @param index the index to update or rebuild
   * @param report the report to write output messages to
   * @param resourcesToIndex an (optional) list of <code>{@link CmsPublishedResource}</code> objects
   *     to update in the index
   * @throws CmsException if something goes wrong
   */
  protected void updateIndex(
      I_CmsSearchIndex index, I_CmsReport report, List<CmsPublishedResource> resourcesToIndex)
      throws CmsException {
        ...
        try {
          ...
          if (LOG.isWarnEnabled()) {
            LOG.warn(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_IO_INDEX_WRITER_COMMIT_2, index.getName(), index.getPath()),
                e);
          }
        }
        try {
          ...
          if (LOG.isWarnEnabled()) {
            LOG.warn(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_IO_INDEX_WRITER_OPTIMIZE_2, index.getName(), index.getPath()),
                e);
          }
        }
      }
        ...
        try {
          ...
          if (LOG.isWarnEnabled()) {
            LOG.warn(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_IO_INDEX_WRITER_CLOSE_2, index.getPath(), index.getName()),
                e);
          }
        }
      }
   ...
   * @param index the index to update
   * @param report the report to write output messages to
   * @param resourcesToIndex a list of <code>{@link CmsPublishedResource}</code> objects to update
   *     in the index
   * @throws CmsException if something goes wrong
   */
          ...
          if (hasResourcesToDelete) {
            ...
            while (i.hasNext()) {
              ...
              if (updateCollection.hasResourcesToDelete()) {
                updateCollection
                    .getIndexer()
                    .deleteResources(writer, updateCollection.getResourcesToDelete());
              }
            }
          }
            ...
            while (i.hasNext()) {
              ...
              if (updateCollection.hasResourceToUpdate()) {
                updateCollection
                    .getIndexer()
                    .updateResources(
                        writer, threadManager, updateCollection.getResourcesToUpdate());
              }
            }
   ...
   * @param resourcesToIndex the list of {@link CmsPublishedResource} objects to index
   */
  protected void updateIndexOffline(
      I_CmsReport report, List<CmsPublishedResource> resourcesToIndex) {
  ...
  /**
   * Checks if the given containerpage is used as a detail containers and adds the related detail
  ...
  private void addDetailContent(
      CmsObject adminCms, Set<CmsResource> containerPages, @RUntainted String containerPage) {

    if (CmsDetailOnlyContainerUtil.isDetailContainersPage(adminCms, containerPage)) {

      try {
        CmsResource detailRes =
            adminCms.readResource(
    ...
    try {
      ...
      container =
          CoreContainer.createAndLoad(
              Paths.get(m_solrConfig.getHome()), m_solrConfig.getSolrFile().toPath());
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
  ...
  private void ensureIndexIsUnlocked(@RUntainted String dataDir) {
    ...
    lockFiles.add(
        new File(
            CmsFileUtil.addTrailingSeparator(CmsFileUtil.addTrailingSeparator(dataDir) + "index")
                + "write.lock"));
    lockFiles.add(
        new File(
            CmsFileUtil.addTrailingSeparator(
                    CmsFileUtil.addTrailingSeparator(dataDir) + "spellcheck")
                + "write.lock"));
  ...
  /**
   * Returns the report in the given event data, if <code>null</code> a new log report is used.
   *
   * <p>
   *
  ...
  /**
   * Gets all structure ids for which published resources of both states 'new' and 'deleted' exist
   * in the given list.
   *
   * <p>
   *
