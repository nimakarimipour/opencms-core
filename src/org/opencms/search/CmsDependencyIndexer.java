...
public class CmsDependencyIndexer extends CmsVfsIndexer {
  ...
  /**
   * @see org.opencms.search.CmsVfsIndexer#getUpdateData(org.opencms.search.CmsSearchIndexSource,
   *     java.util.List)
   */
  @Override
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        if (!pubRes.getStructureId().isNullUUID() && pubRes.isFile()) {
          // use utility method from CmsProject to check if published resource is "inside" this
          // index source
          if (CmsProject.isInsideProject(source.getResourcesNames(), pubRes.getRootPath())) {
            ...
            do {
              ...
              if (depIt.hasNext()) {
                // add all dependent documents that are not already included in the original publish
                // list
      ...
      if (resources != null) {
        ...
        for (CmsResource resource : resources) {
          List<CmsResource> folderContent =
              folderLookupMap.get(CmsResource.getFolderPath(resource.getRootPath()));
