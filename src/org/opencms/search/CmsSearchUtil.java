...
public final class CmsSearchUtil {
  ...
  private static final DateFormat DATEFORMAT_ISO_8601 =
  ...
  private static final Collection<String> DEFAULT_HTTP_CLIENT_PATTERNS =
      Arrays.asList(PATTERN_ASCTIME, PATTERN_RFC1036, PATTERN_RFC1123);
  // ---------------------------------------------------------------------------------------

  /**
   * A suite of default date formats that can be parsed, and thus transformed to the Solr specific
   * format
   */
  ...
  /**
   * Private constructor of utitlity class.
   *
   * <p>
   */
  ...
  public static List<String> computeScopeFolders(CmsObject cms, CmsGallerySearchParameters params) {
    ...
    if (params.getReferencePath() != null) {
      subsite =
          OpenCms.getADEManager()
              .getSubSiteRoot(cms, cms.getRequestContext().addSiteRoot(params.getReferencePath()));
      if (subsite != null) {
    ...
    List<String> scopeFolders =
        getSearchRootsForScope(params.getScope(), cms.getRequestContext().getSiteRoot(), subsite);
    return scopeFolders;
  }
   ...
   */
  public static String getDateCreatedTimeRangeFilterQuery(
      String searchField, long startTime, long endTime) {
  ...
  public static List<String> getSearchRootsForScope(
      CmsGallerySearchScope scope, String siteParam, String subSiteParam) {
   ...
   *
   * <p>Parses the date value using the given date formats.
   *
   ...
   * @return a SolrInputDocument with the same fields and values as the SolrDocument.
   * @deprecated This method will be removed in Solr 6.0
   */
  @Deprecated
