 ...
 * use cases when adding own index implementations.
 */
public abstract class A_CmsSearchIndex implements I_CmsSearchIndex {
  ...
  /**
   * An internal enabled flag, used to disable the index if for instance the configured project does
   * not exist.
   */
   ...
   *
   * <p>It is recommended to use the constructor <code>{@link #A_CmsSearchIndex(String)}</code> as
   * it enforces the mandatory name argument.
   *
   * <p>
   */
      ...
      try {
        ...
        if (LOG.isErrorEnabled()) {
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_SEARCHINDEX_CREATE_BAD_PROJECT_2, getProject(), getName()));
        }
      }
    } else {
  ...
  public boolean excludeFromIndex(CmsObject cms, CmsResource resource) {
    ...
    if (resource.isInternal()
      ...
      // don't index internal resources, folders or temporary files or resources with expire date in
      // the past
      return true;
    }
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_UNABLE_TO_READ_PROPERTY_1, resource.getRootPath()));
      }
    }
  ...
  /**
   * We always assume we have no unchanged copy of the content, since it depends on the concrete
   * index. Override the method to enhance indexing performance if you know where to grap the
   * content from your index. See the implementation {@link
   * org.opencms.search.CmsSearchIndex#getContentIfUnchanged(CmsResource)} for an example.
   *
  ...
  /**
   * Returns the document type factory used for the given resource in this index, or <code>null
   * </code> in case the resource is not indexed by this index.
   *
   * <p>A resource is indexed if the following is all true:
   *
   * <ol>
   *   <li>The index contains at last one index source matching the root path of the given resource.
   *   <li>For this matching index source, the document type factory needed by the resource is also
   *       configured.
   * </ol>
   *
   * @param res the resource to check
   * @return the document type factory used for the given resource in this index, or <code>null
   *     </code> in case the resource is not indexed by this index
   */
  @Override
   ...
   */
  public I_CmsIndexWriter getIndexWriter(I_CmsReport report, boolean create)
   ...
   * <p>
   *
   * @throws CmsSearchException if the index source association failed or a configuration error
   *     occurred
   */
  public void initialize() throws CmsSearchException {
    ...
    for (int i = 0, n = m_sourceNames.size(); i < n; i++) {
      ...
      try {
        ...
        throw new CmsSearchException(
            Messages.get().container(Messages.ERR_INDEX_SOURCE_ASSOCIATION_1, sourceName), e);
      }
    }
    ...
    m_fieldConfiguration =
        OpenCms.getSearchManager().getFieldConfiguration(m_fieldConfigurationName);
    if (m_fieldConfiguration == null) {
      // we must have a valid field configuration to continue
      throw new CmsSearchException(
          Messages.get()
              .container(
                  Messages.ERR_FIELD_CONFIGURATION_UNKNOWN_2, m_name, m_fieldConfigurationName));
    }
  ...
  /**
   * Returns <code>true</code> in case this index is updated incremental.
   *
   * <p>An index is updated incremental if the index rebuild mode as defined by {@link
   * #getRebuildMode()} is either set to {@value I_CmsSearchIndex#REBUILD_MODE_AUTO} or {@value
   * I_CmsSearchIndex#REBUILD_MODE_OFFLINE}. Moreover, at least one update must have been written to
   * the index already.
   *
  ...
  /**
   * Set the path to the index/core. This can either be the path to the directory where the index is
   * stored or the URL where the index/core is reached.
   *
    ...
    if (m_indexWriter != null) {
      try {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_IO_INDEX_WRITER_CLOSE_2, getPath(), getName()),
            e);
      }
    }
  }
   ...
   */
  protected abstract I_CmsIndexWriter createIndexWriter(boolean create, I_CmsReport report)
      throws CmsIndexException;
