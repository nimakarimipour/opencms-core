...
/**
 * Base utility class that allows extraction of the indexable "plain" text from a given document
 * format.
 *
 * <p>
 *
...
public abstract class A_CmsTextExtractor implements I_CmsTextExtractor {
  ...
  protected void combineContentItem(
      String itemValue, String itemKey, StringBuffer content, Map<String, String> contentItems) {
  ...
  /**
   * Parses the given input stream with the provided parser and returns the result as a map of
   * content items.
   *
   * <p>
   *
  ...
  protected CmsExtractionResult extractText(InputStream in, Parser parser) throws Exception {
    ...
    combineContentItem(
        meta.get(DublinCore.TITLE), I_CmsExtractionResult.ITEM_TITLE, content, contentItems);
    combineContentItem(
        meta.get(MSOffice.KEYWORDS), I_CmsExtractionResult.ITEM_KEYWORDS, content, contentItems);
    ...
    combineContentItem(
        meta.get(MSOffice.AUTHOR), I_CmsExtractionResult.ITEM_AUTHOR, content, contentItems);
    ...
    //
    combineContentItem(
        meta.get(MSOffice.CATEGORY), I_CmsExtractionResult.ITEM_CATEGORY, content, contentItems);
    //
    combineContentItem(
        meta.get(MSOffice.COMMENTS), I_CmsExtractionResult.ITEM_COMMENTS, content, contentItems);
    ...
    //
    combineContentItem(
        meta.get(MSOffice.MANAGER), I_CmsExtractionResult.ITEM_MANAGER, content, contentItems);
