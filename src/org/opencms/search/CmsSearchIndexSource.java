...
public class CmsSearchIndexSource implements Comparable<CmsSearchIndexSource>, Serializable {
  ...
  /**
   * Returns <code>0</code> if the given object is an index source with the same name.
   *
   * <p>Note that the name of an index source has to be unique within OpenCms.
   *
   * <p>
   *
  ...
  /**
   * Two index sources are considered equal if their names as returned by {@link #getName()} is
   * equal.
   *
   * <p>Note that the name of an index source has to be unique within OpenCms.
   *
   ...
   *
   * <p>Note that only the keys resulting from the document types for this source set are taken into
   * account.
   *
   * <p>
   *
  ...
  /**
   * Overriden to be consistents with overridden method <code>{@link #equals(Object)}</code>.
   *
  ...
  public void init() {
    ...
    m_documentFactories =
        OpenCms.getSearchManager().getDocumentTypeMapForTypeNames(m_documentTypes);
  }
  ...
  /**
   * Returns <code>true</code> in case the given resource root path is contained in the list of
   * configured resource names, and the given document type name is contained in the list if
   * configured document type names of this index source.
   *
   * <p>
   *
   * @param rootPath the resource root path to check
   * @param documentTypeKey the document type key for which the presence of a factory name has to be
   *     checked
   * @return <code>true</code> in case the given resource root path is contained in the list of
   *     configured resource names, and the given document type name is contained in the list if
   *     configured document type names of this index source
   ...
   * @return true if the given document type name was contained before thus could be removed
   ...
   *
   * <p>An Exception is thrown to allow GUI-display of wrong input.
   *
   * <p>
   *
   * @param indexerClassName the class name of the indexer
   * @throws CmsIllegalArgumentException if the given String is not a fully qualified classname
   *     (within this Java VM)
   */
    ...
    try {
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_INDEXER_CREATION_FAILED_1, m_indexerClassName),
            exc);
      }
   ...
   * @throws CmsIllegalArgumentException if argument name is null, an empty or whitespace-only
   *     Strings or already used for another indexsource's name.
   */
  public void setName(String name) throws CmsIllegalArgumentException {
    ...
    if (OpenCms.getRunLevel() > OpenCms.RUNLEVEL_2_INITIALIZING) {
      ...
      // don't test this if the indexsource is not new (widget invokes setName even if it was not
      // changed)
