 ...
 *
 * <p>This class provides in java all resource information used by the explorer view, mostly
 * generated in javascript (see explorer.js)
 *
 * <p>
 *
 * @since 6.0.0
...
public final class CmsResourceUtil {
  ...
  public static class CmsResourceProjectState extends A_CmsModeIntEnumeration {
    ...
    protected static final CmsResourceProjectState LOCKED_FOR_PUBLISHING =
        new CmsResourceProjectState(5);

    /** Constant for the project state locked in current project. */
    protected static final CmsResourceProjectState MODIFIED_IN_CURRENT_PROJECT =
        new CmsResourceProjectState(1);

    /** Constant for the project state locked in other project. */
    protected static final CmsResourceProjectState MODIFIED_IN_OTHER_PROJECT =
        new CmsResourceProjectState(2);
    ...
    /**
     * Checks if this is a {@link #LOCKED_FOR_PUBLISHING} state.
     *
     * <p>
     *
    ...
    /**
     * Checks if this is a {@link #MODIFIED_IN_CURRENT_PROJECT} state.
     *
     * <p>
     *
    ...
    /**
     * Checks if this is a {@link #MODIFIED_IN_OTHER_PROJECT} state.
     *
     * <p>
     *
    ...
    /**
     * Checks if this is a {@link #CLEAN} state.
     *
     * <p>
     *
  ...
  /**
   * Enumeration class for defining the site modes.
   *
   * <p>
   */
  ...
  /** Constant for the project state locked for publishing. */
  public static final CmsResourceProjectState STATE_LOCKED_FOR_PUBLISHING =
      CmsResourceProjectState.LOCKED_FOR_PUBLISHING;

  /** Constant for the project state locked in current project. */
  public static final CmsResourceProjectState STATE_MODIFIED_IN_CURRENT_PROJECT =
      CmsResourceProjectState.MODIFIED_IN_CURRENT_PROJECT;

  /** Constant for the project state locked in other project. */
  public static final CmsResourceProjectState STATE_MODIFIED_IN_OTHER_PROJECT =
      CmsResourceProjectState.MODIFIED_IN_OTHER_PROJECT;
  ...
  /**
   * The project to use to check project state, if <code>null</code> the current project will be
   * used.
   */
  ...
  /**
   * Creates a new {@link CmsResourceUtil} object.
   *
   * <p>
   *
    ...
    if (explorerType.getSmallIconStyle() != null) {
      ...
      return new ExternalResource(
          CmsWorkplace.getResourceUri(
              CmsWorkplace.RES_PATH_FILETYPES + explorerType.getOriginalIcon()));
    }
  }
   ...
   *
   * <p>If greater than zero, the path will be formatted to this number of chars.
   *
   * <p>This only affects the generation of the path for the current resource.
   *
   * <p>
   *
   ...
   *
   * <p>Relative to <code>/system/workplace/resources/</code>.
   *
   * <p>If the resource has no sibling it is the same as {@link #getIconPathResourceType()}.
   *
   * <p>
   *
   ...
   * <p>Relative to <code>/system/workplace/resources/</code>.
   *
   * <p>Returns <code>explorer/project_none.gif</code> if request context is <code>null</code>.
   *
   * <p>
   *
  ...
  public String getIconPathResourceType() {
    ...
    if (!isEditable()) {
      return CmsWorkplace.RES_PATH_FILETYPES
          + OpenCms.getWorkplaceManager()
              .getExplorerTypeSetting(CmsResourceTypePlain.getStaticTypeName())
              .getIcon();
    }
    return CmsWorkplace.RES_PATH_FILETYPES
  ...
  public String getLockedByName() {
    ...
    if (!getLock().isNullLock()) {
      ...
      try {
        lockedBy =
            getCurrentOuRelativeName(
                CmsPrincipal.readPrincipalIncludingHistory(getCms(), getLock().getUserId())
                    .getName());
      } catch (Throwable e) {
  ...
  public String getNavText() {
    ...
    try {
      navText =
          getCms()
              .readPropertyObject(
                  getCms().getSitePath(m_resource), CmsPropertyDefinition.PROPERTY_NAVTEXT, false)
              .getValue();
    } catch (Throwable e) {
      ...
      try {
        ...
        navText =
            getCms()
                .readPropertyObject(
                    m_resource.getRootPath(), CmsPropertyDefinition.PROPERTY_NAVTEXT, false)
                .getValue();
      } catch (Exception e1) {
  ...
  public String getNoEditReason(Locale locale, boolean ignoreExpiration) throws CmsException {
    ...
    if (m_resource instanceof I_CmsHistoryResource) {
      reason = Messages.get().getBundle(locale).key(Messages.GUI_NO_EDIT_REASON_HISTORY_0);
            ...
            false,
            ignoreExpiration ? CmsResourceFilter.IGNORE_EXPIRATION : CmsResourceFilter.DEFAULT)
        || !isEditable()) {
      ...
      if (getLock().getSystemLock().isPublish()) {
        ...
        reason =
            Messages.get()
                .getBundle(locale)
                .key(Messages.GUI_NO_EDIT_REASON_LOCK_1, getLockedByName());
      }
    }
    return reason;
  }
   ...
   *
   * <p>Taking into account following settings:<br>
   *
   * <ul>
   *   <li>site mode
  ...
  public CmsPermissionSet getPermissionSet() {
    ...
    try {
      if (getCms()
          .hasPermissions(
              resource, CmsPermissionSet.ACCESS_CONTROL, false, CmsResourceFilter.ALL)) {
    ...
    try {
      if (getCms()
          .hasPermissions(
              resource, CmsPermissionSet.ACCESS_DIRECT_PUBLISH, false, CmsResourceFilter.ALL)) {
    ...
    try {
      if (getCms()
          .hasPermissions(resource, CmsPermissionSet.ACCESS_READ, false, CmsResourceFilter.ALL)) {
    ...
    try {
      if (getCms()
          .hasPermissions(resource, CmsPermissionSet.ACCESS_VIEW, false, CmsResourceFilter.ALL)) {
    ...
    try {
      if (getCms()
          .hasPermissions(resource, CmsPermissionSet.ACCESS_WRITE, false, CmsResourceFilter.ALL)) {
   ...
   * <ul>
   *   <li>null: unchanged.
   *   <li>true: locked in current project.
   *   <li>false: not locked in current project.
   * </ul>
   *
  ...
  public String getSiteTitle() {
    ...
    try {
      ...
      title =
          getCms()
              .readPropertyObject(title, CmsPropertyDefinition.PROPERTY_TITLE, false)
              .getValue(title);
    } catch (CmsException e) {
      // ignore
    } finally {
   ...
   *
   * <p>For directories it returns <code>SIZE_DIR</code>.
   *
   * <p>
   *
   ...
   *
   * <p>Uses default locale if request context is <code>null</code>.
   *
   * <p>
   *
    ...
    if (m_request == null) {
      name =
          org.opencms.workplace.explorer.Messages.get()
              .getBundle()
              .key(org.opencms.workplace.explorer.Messages.getStateKey(state));
    } else {
      name =
          org.opencms.workplace.explorer.Messages.get()
              .getBundle(m_request.getLocale())
              .key(org.opencms.workplace.explorer.Messages.getStateKey(state));
    }
    return name;
  }
   ...
   *
   * <p>That is, depending on {@link CmsResource#getSiblingCount()}
   *
   * <p>Use it with the {@link #getIconPathExplorer} method.
   *
   * <p>
   *
   ...
   *
   * <p>That is, depending on {@link CmsResource#getDateReleased()} and {@link
  ...
  public String getTimeWindowLayoutStyle() {
    ...
    return getTimeWindowLayoutType() == CmsResourceUtil.LAYOUTSTYLE_INRANGE
        ? ""
        : "font-style:italic;";
  }
   ...
   *   <li><code>{@link CmsResourceUtil#LAYOUTSTYLE_BEFORERELEASE}</code>: The resource is not yet
   *       released
   *   <li><code>{@link CmsResourceUtil#LAYOUTSTYLE_AFTEREXPIRE}</code>: The resource has already
   *       expired
   * </ul>
   *
  ...
  public String getTitle() {
    ...
    try {
      title =
          getCms()
              .readPropertyObject(
                  getCms().getSitePath(m_resource), CmsPropertyDefinition.PROPERTY_TITLE, false)
              .getValue();
    } catch (Throwable e) {
      String storedSiteRoot = getCms().getRequestContext().getSiteRoot();
      try {
        ...
        title =
            getCms()
                .readPropertyObject(
                    m_resource.getRootPath(), CmsPropertyDefinition.PROPERTY_TITLE, false)
                .getValue();
      } catch (Exception e1) {
        // should usually never happen
        if (LOG.isInfoEnabled()) {
  ...
  public String getUserCreated() {
    ...
    try {
      user =
          getCurrentOuRelativeName(
              CmsPrincipal.readPrincipalIncludingHistory(getCms(), m_resource.getUserCreated())
                  .getName());
    } catch (Throwable e) {
  ...
  public String getUserLastModified() {
    ...
    try {
      user =
          getCurrentOuRelativeName(
              CmsPrincipal.readPrincipalIncludingHistory(getCms(), m_resource.getUserLastModified())
                  .getName());
    } catch (Throwable e) {
      LOG.info(e.getLocalizedMessage());
    }
  ...
  /**
   * Returns <code>true</code> if the given resource is editable by the current user.
   *
   * <p>Returns <code>false</code> if no request context is set.
   *
   * <p>
   *
  ...
  public boolean isEditable() {
    ...
    CmsExplorerTypeSettings settings =
        OpenCms.getWorkplaceManager().getExplorerTypeSetting(getResourceTypeName());
    if (settings != null) {
  ...
  /**
   * Returns <code>true</code> if the given resource is in the reference project.
   *
   * <p>Returns <code>false</code> if the request context is <code>null</code>.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the stored resource has been released and has not expired.
   *
   * <p>If no request context is available, the current time is used for the validation check.
   *
   * <p>
   *
  ...
  /**
   * Returns the message bundle for formatting dates, depends on the request locale.
   *
   * <p>
   *
  ...
  private CmsMessages getMessages() {
    ...
    if (m_messages == null) {
      if (m_cms != null) {
        m_messages =
            Messages.get().getBundle(OpenCms.getWorkplaceManager().getWorkplaceLocale(m_cms));
      } else {
  ...
  /**
   * Returns the reference project resources.
   *
   * <p>
   *
