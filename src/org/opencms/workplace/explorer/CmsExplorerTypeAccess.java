...
public class CmsExplorerTypeAccess implements Serializable {
   ...
   *
   * <p>This stores the configuration data in a map which is used in the initialize process to
  ...
  public void createAccessControlList(String resourceType) throws CmsException {
    ...
    if (m_permissionsCache == null) {
      ...
      OpenCms.getMemoryMonitor()
          .register(this.getClass().getName() + "." + resourceType, m_permissionsCache);
    } else {
  ...
  /**
   * Calculates the permissions for this explorer type settings for the user in the given OpenCms
   * user context.
   *
   * <p>
   *
   ...
   * @return the permissions for this explorer type settings for the user in the given OpenCms user
   *     context
   */
  public CmsPermissionSet getPermissions(CmsObject cms, CmsResource resource) {
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_READ_GROUPS_OF_USER_FAILED_1, user.getName()),
          e);
    }
    ...
    if ((defaultPermissions != null) && !user.isGuestUser()) {
      ...
      if (!found) {
        ...
        CmsAccessControlEntry entry =
            new CmsAccessControlEntry(null, user.getId(), defaultPermissions);
        acl.add(entry);
      }
    }
  ...
  /**
   * Returns the cache key for the roles and groups of the current user and the given resource.
   *
   * <p>In this way, it does not matter if the resource and/or user permissions changes, so we never
   * need to clean the cache.
   *
   * <p>And since the cache is a LRU map, old trash entries will be automatically removed.
   *
   * <p>
   *
  ...
  private String getPermissionsCacheKey(CmsObject cms, CmsResource resource) {
    ...
    try {
      ...
      Iterator<?> itRoles =
          OpenCms.getRoleManager().getRolesOfUser(cms, userName, "", true, true, false).iterator();
      while (itRoles.hasNext()) {
