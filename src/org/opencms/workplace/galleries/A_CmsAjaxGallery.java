 ...
 * content editors, WYSIWYG editors and context menu. It
 *
 * <p>It is also used for AJAX requests to dynamically switch galleries or categories and get
 * additional information for the currently active item of the dialog.
 *
 * <p>Extend this class for every gallery type (e.g. image gallery or download gallery) to build.
 *
 * <p>
 *
 * @since 7.5.0
...
public abstract class A_CmsAjaxGallery extends CmsDialog {
  ...
  /**
   * Public empty constructor, required for {@link A_CmsAjaxGallery#createInstance(String,
   * CmsJspActionElement)}.
   *
   * <p>
   */
  ...
  public static A_CmsAjaxGallery createInstance(String galleryTypeName, CmsJspActionElement jsp) {
    ...
    if (template == null) {
      ...
      if (jsp == null) {
        message =
            Messages.get().container(Messages.LOG_UNKNOWN_GALLERY_TYPE_REQ_1, galleryTypeName);
      } else {
        message =
            Messages.get()
                .container(
  ...
  public static void initGallery(CmsDialog wp) {
    ...
    if (wp.useNewStyle()) {
      galleryTypeName =
          CmsResource.getName(CmsResource.getFolderPath(wp.getAdminTool().getHandler().getLink()));
    } else {
      galleryTypeName =
          CmsResource.getName(CmsResource.getFolderPath(wp.getJsp().getRequestContext().getUri()));
    }
  ...
  public void displayDialog() {
    ...
    if (DIALOG_CHANGEITEMTITLE.equals(getParamAction())) {
      ...
      // build the JSON object for the current item with changed resource content
      // (CmsResourcePointer)
  ...
  /**
   * Returns a list of gallery items (resources) for the currently selected gallery and resource
   * type id.
   *
   * <p>
   *
      ...
      if (CmsStringUtil.isNotEmpty(getParamGalleryPath())) {
        try {
          ...
          if (resTypeId == -1) {
            ...
            filter =
                CmsResourceFilter.ONLY_VISIBLE_NO_DELETED.addRequireTimerange().addRequireFile();
          } else {
            // filter all resources of the required type
            filter =
                CmsResourceFilter.ONLY_VISIBLE_NO_DELETED
                    .addRequireTimerange()
                    .addRequireType(resTypeId);
          }
   ...
   *
   * <p>In case of downloadgallery use '-1' to list all resources excluding folders.
   *
   * <p>
   *
   ...
   *
   * <p>This is necessary to distinguish between widget mode, view mode and editor mode.
   *
   * <p>
   *
  ...
  /**
   * Returns the list mode for getting the items, either {@link #LISTMODE_CATEGORY} or {@link
   * #LISTMODE_GALLERY}.
   *
   * <p>
   *
   ...
   *
   * <p>It can be overwritten in the inherited class, e.g. {@link
   * org.opencms.workplace.galleries.CmsAjaxImageGallery#init()}.
   *
   * <p>
   */
  public void init() {
   ...
   *
   * <p>This is necessary to distinguish between widget mode and editor mode.
   *
   * <p>
   *
  ...
  /**
   * Sets the list mode for getting the items, either {@link #LISTMODE_CATEGORY} or {@link
   * #LISTMODE_GALLERY}.
   *
   * <p>
   *
  ...
  protected void buildJsonGalleryList() {
    ...
    while (i.hasNext()) {
      ...
      try {
        ...
        title =
            getCms()
                .readPropertyObject(path, CmsPropertyDefinition.PROPERTY_TITLE, false)
                .getValue("");
      } catch (CmsException e) {
        // error reading title property
        if (LOG.isErrorEnabled()) {
   ...
   *
   * <ul>
   *   <li><code>sitepath</code>: site path to the resource.
   *   <li><code>linkpath</code>: substituted url of the resource.
   *   <li><code>title</code>: title property of the resource.
   *   <li><code>size</code>: size of the resource in kb.
   *   <li><code>datecreated</code>: the creation date of the resource.
   *   <li><code>datelastmodified</code>: the modification date.
   *   <li><code>state</code>: the state of the resource, new or changed.
   *   <li><code>lockedby</code>: indicates if the resource is locked by another user.
   *   <li><code>editable</code>: editable flag to determine if item is editable and can be lockes
   *       by the user.
   *   <li><code>writepermission</code>: flag to indicate if the user has write permissions for
   *       given resource.
   *   <li><code>directpublish</code>: flag to indicate if the user has write direct publish
   *       permission for given resource.
   *   <li><code>description</code>: description property of the resource.
   * </ul>
   *
  ...
  protected void buildJsonItemCommonPart(JSONObject jsonObj, CmsResource res, String sitePath) {
    ...
    try {
      ...
      // 9: item editable flag to determine if item is editable (offline project and can be locked
      // by the current user)
      ...
      try {
        ...
        if (!getCms().getRequestContext().getCurrentProject().isOnlineProject()
          ...
          if (getCms()
              .hasPermissions(res, CmsPermissionSet.ACCESS_WRITE, false, CmsResourceFilter.ALL)) {
            writePermission = true;
          }
          // test if the user has direct publish permission
          if (getCms()
              .hasPermissions(
                  res, CmsPermissionSet.ACCESS_DIRECT_PUBLISH, false, CmsResourceFilter.ALL)) {
   ...
   *
   * <p>The content of the JSON object consists of a common and a specific part of the given
   * resource.
   *
   * <p>
   *
  ...
  protected abstract void buildJsonItemSpecificPart(
      JSONObject jsonObj, CmsResource res, String sitePath);
  ...
  protected void buildJsonResourceItems(List<CmsResource> resourceitems, String parentFolder) {
    ...
    if (CmsStringUtil.isNotEmpty(parentFolder)) {
      // check if there are changes in the currently selected gallery and the user has direct edit
      // permissions
      try {
        if ((OpenCms.getPublishManager()
                .getPublishList(getCms(), getCms().readResource(parentFolder), false)
                .size()
            > 0)) {
   ...
   *
   * <p>This function should be overwritten in {@link
  ...
  protected List<CmsResource> getCategoryItems() {
    ...
    if (CmsStringUtil.isNotEmpty(getParamGalleryPath())) {
      try {
        ...
        result =
            service.readCategoryResources(getCms(), getParamGalleryPath(), true, editedResource);
