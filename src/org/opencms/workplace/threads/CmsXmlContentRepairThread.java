...
public class CmsXmlContentRepairThread extends A_CmsReportThread {
  ...
  public void run() {
    ...
    getReport()
        .println(
            Messages.get().container(Messages.RPT_XMLCONTENTREPAIR_END_0),
            I_CmsReport.FORMAT_HEADLINE);
  }

  /**
   * Performs the correction of the XML content resources according to their XML schema definition.
   *
   * <p>
   *
  ...
  private void repairXmlContents() throws CmsException {
    ...
    CmsResourceFilter filter =
        CmsResourceFilter.IGNORE_EXPIRATION.addRequireType(m_settings.getResourceTypeId());
    String path = CmsResource.getFolderPath(m_settings.getVfsFolder());
    // get the list of resources to check
    List<CmsResource> resources =
        getCms().readResources(path, filter, m_settings.isIncludeSubFolders());
    ...
    while (i.hasNext()) {
      ...
      getReport()
          .print(
              Messages.get().container(Messages.RPT_PROCESSING_XMLCONTENT_0),
              I_CmsReport.FORMAT_NOTE);
      getReport()
          .print(
              org.opencms.report.Messages.get()
      ...
      try {
        ...
        if (fixFile) {
          ...
          if (canWrite) {
            // enable "auto correction mode" - this is required or the XML structure will not be
            // fully corrected
