...
public class CmsModuleDeleteThread extends A_CmsReportThread {
  ...
  public CmsModuleDeleteThread(CmsObject cms, List<String> moduleNames, boolean replaceMode) {
    ...
    super(
        cms, Messages.get().getBundle().key(Messages.GUI_DELETE_MODULE_THREAD_NAME_1, moduleNames));
  ...
  public void run() {
    ...
    try {
      ...
      report.println(e);
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_MODULE_DELETE_FAILED_1, m_moduleNames), e);
    } finally {
      if (!indexingAlreadyPaused) {
