...
public class CmsResourceComparison {
  ...
  /**
   * Helper method that collects all meta attributes of the two file versions and finds out, which
   * of the attributes were added, removed, modified or remain unchanged.
   *
   * <p>
   *
  ...
  public static List<CmsAttributeComparison> compareAttributes(
      CmsObject cms, CmsResource resource1, CmsResource resource2) {
    ...
    comparedAttributes.add(
        new CmsAttributeComparison(Messages.GUI_LABEL_DATE_RELEASED_0, release1, release2));
    String expire1;
    ...
    comparedAttributes.add(
        new CmsAttributeComparison(
            Messages.GUI_LABEL_DATE_LAST_MODIFIED_0, dateLastModified1, dateLastModified2));
    try {
      String type1 =
          OpenCms.getResourceManager().getResourceType(resource1.getTypeId()).getTypeName();
      String type2 =
          OpenCms.getResourceManager().getResourceType(resource2.getTypeId()).getTypeName();
      comparedAttributes.add(
          new CmsAttributeComparison(Messages.GUI_HISTORY_COLS_FILE_TYPE_0, type1, type2));
    ...
    comparedAttributes.add(
        new CmsAttributeComparison(
            Messages.GUI_HISTORY_COLS_DATE_PUBLISHED_0, dateCreated1, dateCreated2));
    try {
      String userLastModified1 = resource1.getUserLastModified().toString();
      try {
        userLastModified1 =
            CmsPrincipal.readPrincipalIncludingHistory(cms, resource1.getUserLastModified())
                .getName();
      ...
      try {
        userLastModified2 =
            CmsPrincipal.readPrincipalIncludingHistory(cms, resource2.getUserLastModified())
                .getName();
      } catch (CmsDbEntryNotFoundException e) {
        // ignore
      }
      comparedAttributes.add(
          new CmsAttributeComparison(
              Messages.GUI_LABEL_USER_LAST_MODIFIED_0, userLastModified1, userLastModified2));
    ...
    comparedAttributes.add(
        new CmsAttributeComparison(Messages.GUI_HISTORY_COLS_RESOURCE_PATH_0, path1, path2));
    return comparedAttributes;
  }

  /**
   * Helper method that finds out, which of the properties were added, removed, modified or remain
   * unchanged.
   *
   * <p>
   *
  ...
  public static List<CmsAttributeComparison> compareProperties(
      CmsObject cms, CmsResource resource1, String version1, CmsResource resource2, String version2)
      throws CmsException {
    ...
    while (i.hasNext()) {
      ...
      comparedProperties.add(
          new CmsAttributeComparison(
              prop.getName(), "", prop.getValue(), CmsResourceComparison.TYPE_ADDED));
    }
    ...
    while (i.hasNext()) {
      ...
      comparedProperties.add(
          new CmsAttributeComparison(
              prop.getName(), prop.getValue(), "", CmsResourceComparison.TYPE_REMOVED));
    }
    ...
    while (i.hasNext()) {
      ...
      if (value1.equals(value2)) {
        comparedProperties.add(
            new CmsAttributeComparison(
                prop.getName(), value1, value2, CmsResourceComparison.TYPE_UNCHANGED));
      } else {
        comparedProperties.add(
            new CmsAttributeComparison(
                prop.getName(), value1, value2, CmsResourceComparison.TYPE_CHANGED));
      }
    }
