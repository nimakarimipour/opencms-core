...
public class CmsLoginUserAgreement extends CmsDialog {
  ...
  protected static final String VFS_PATH_CONFIGFOLDER =
      CmsWorkplace.VFS_PATH_SYSTEM + "login/useragreement/";
   ...
   *
   * <p>See also {@link CmsDialog#dialogScriptSubmit()}
   *
   ...
   * <p>
   *
   * @return <code>true</code> if the user agreement page should be shown for the current user,
      ...
      try {
        ...
        enabled =
            Boolean.valueOf(
                    getCms()
                        .readPropertyObject(
                            content.getFile(), CmsPropertyDefinition.PROPERTY_LOGIN_FORM, false)
                        .getValue())
                .booleanValue();
        if (enabled) {
   ...
   *
   * <p>See also {@link CmsDialog#dialogButtonsHtml(StringBuffer, int, String)}
   *
   ...
   *
   * <p>Returns the absolute path in the OpenCms VFS to the user agreement configuration file.
   *
   * <p>
   */
