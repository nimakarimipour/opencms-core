...
public class CmsToolDialog extends CmsWorkplace {
  ...
  /**
   * Request parameter name for the base tool path in the navigation, should be a parent tool of
   * path.
   */
   ...
   *
   * <p>Should only be used by the <code>{@link CmsDialog#dialogScriptSubmit()}</code> method.
   *
   * <p>
   *
   ...
   *
   * <p>It is called by the <code>{@link org.opencms.workplace.CmsDialog#dialog(int, String)}</code>
   * method.
   *
   * <p>
   *
  ...
  public String dialogTitle() {
    ...
    html.append(
        CmsEncoder.decode(
            CmsToolMacroResolver.resolveMacros(getAdminTool().getHandler().getName(), this)));
    html.append("\n\t\t\t</td>");
  ...
  /**
   * Builds the start HTML for a block area with border and optional subheadline in the dialog
   * content area.
   *
   * <p>
   *
   ...
   * @throws CmsRoleViolationException in case the dialog is opened by a user without the necessary
   *     privileges
   */
  public Map<String, String[]> initAdminTool() throws CmsRoleViolationException {
    ...
    try {
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(wp.getParamCloseLink())) {
        if (!getToolManager()
            .getBaseToolPath(this)
            .equals(getToolManager().getCurrentToolPath(this))) {
          Map<String, String[]> args =
              getToolManager()
                  .resolveAdminTool(getParamRoot(), getParentPath())
                  .getHandler()
                  .getParameters(wp);
    ...
    if (!getToolManager().getCurrentTool(this).getHandler().isEnabled(this)) {
      throw new CmsRoleViolationException(
          Messages.get().container(Messages.ERR_ADMIN_INSUFFICIENT_RIGHTS_0));
    }
  ...
  public String pageHtmlStyle(int segment, String title, String stylesheet) {
    ...
    html.append(
        CmsStringUtil.escapeJavaScript(resolveMacros(getAdminTool().getHandler().getHelpText())));
    html.append("\");\n");
   ...
   *
   * <p>The default is the new style, this parameter is not intended for external use.
   *
   * <p>
   *
  ...
  protected String computeUpLevelLink() {
    ...
    if (parentTool != null) {
      ...
      upLevelLink =
          CmsRequestUtil.appendParameter(upLevelLink, PARAM_FORCE, Boolean.TRUE.toString());
    }
    return upLevelLink;
  }
