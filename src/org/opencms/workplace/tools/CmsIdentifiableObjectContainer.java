 ...
 *
 * <p>It can handle relative or absolute orderings and unique names.
 *
 * <p>
 *
...
public class CmsIdentifiableObjectContainer<T> {
  ...
  /**
   * Internal class just for taking care of the positions in the container.
   *
   * <p>
   *
  ...
  private final List<CmsIdObjectElement<T>> m_orderedObjectList =
      new ArrayList<CmsIdObjectElement<T>>();
   ...
   *
   * <p>Shifts the object currently at that position (if any) and any subsequent objects to the
   ...
   *
   * <p>If <code>uniqueIds</code> is set to <code>false</code> an <code>{@link Object}</code>
   * containing a <code>{@link List}</code> with all the objects with the given id is returned.
   *
   * <p>If the container no contains any object with the given id, <code>null</code> is returned.
   *
   * <p>
   *
   ...
   *
   * <p>if <code>m_uniqueIds</code> is set, it will remove at most one object. otherwise it will
   * remove all elements with the given id.
   *
   * <p>
   *
