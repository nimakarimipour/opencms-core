 ...
 *
 * <p>An admin tool can be a link to itself through the <code>{@link #buttonHtml(CmsWorkplace)}
 * </code> method, as also a group of <code>{@link CmsToolGroup}</code>s through the <code>
 * {@link #groupHtml(CmsWorkplace)}</code> method.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsTool {
  ...
  /** Sub-tools container. */
  private final CmsIdentifiableObjectContainer<CmsToolGroup> m_container =
      new CmsIdentifiableObjectContainer<CmsToolGroup>(true, true);
  ...
  public String buttonHtml(CmsWorkplace wp) {
    ...
    String link =
        CmsToolManager.linkForToolPath(
            wp.getJsp(), getHandler().getPath(), getHandler().getParameters(wp));
  ...
  public String groupHtml(CmsWorkplace wp) {
    ...
    List<CmsTool> subTools =
        OpenCms.getWorkplaceManager()
            .getToolManager()
            .getToolsForPath(wp, getHandler().getPath(), false);
