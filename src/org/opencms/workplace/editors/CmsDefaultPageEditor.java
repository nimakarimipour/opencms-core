 ...
 *
 * <p>Extend this class for all editors that work with the CmsDefaultPage.
 *
 * <p>
 *
 * @since 6.0.0
...
public abstract class CmsDefaultPageEditor extends CmsEditor {
  ...
  /**
   * Page object used from the action and init methods, be sure to initialize this e.g. in the
   * initWorkplaceRequestValues method.
   */
  ...
  public void actionPreview() throws IOException, JspException {
    ...
    String param =
        "?" + org.opencms.i18n.CmsLocaleManager.PARAMETER_LOCALE + "=" + getParamElementlanguage();
   ...
   *
   * <p>This method has to be called on the JSP right before the form display html is created.
   *
   * <p>*
   */
    ...
    if (m_uriStyleSheet == null) {
      try {
        if (OpenCms.getWorkplaceManager().getEditorCssHandlers().size() > 0) {
          ...
          Iterator<I_CmsEditorCssHandler> i =
              OpenCms.getWorkplaceManager().getEditorCssHandlers().iterator();
          while (i.hasNext()) {
          ...
          // for compatibility reasons, read the template property value from the template file to
          // get the CSS
          String currentTemplate = getUriTemplate();
          m_uriStyleSheet =
              getCms()
                  .readPropertyObject(
                      currentTemplate, CmsPropertyDefinition.PROPERTY_TEMPLATE, false)
                  .getValue("");
        }
      } catch (CmsException e) {
        LOG.warn(
            Messages.get().getBundle().key(Messages.LOG_READ_TEMPLATE_PROP_STYLESHEET_FAILED_0), e);
      }
    }
  ...
  public String getUriTemplate() {
    ...
    try {
      result =
          getCms()
              .readPropertyObject(getParamTempfile(), CmsPropertyDefinition.PROPERTY_TEMPLATE, true)
              .getValue("");
    } catch (CmsException e) {
    ...
    if (m_elementList == null) {
      m_elementList =
          CmsDialogElements.computeElements(
              getCms(), m_page, getParamTempfile(), getElementLocale());
    }
  ...
  protected void initBodyElementLanguage() {
    ...
    Locale defaultLocale =
        OpenCms.getLocaleManager().getDefaultLocales(getCms(), getCms().getSitePath(m_file)).get(0);
   ...
   *
   * <p>This has to be called after the element language has been set with setParamBodylanguage().
   *
   * <p>
   *
   * @param elementName the name of the element to initialize or null, if default element should be
   *     used
   */
  ...
  protected void performSaveContent(String body, Locale locale) throws CmsException {
    ...
    if (EDITOR_CLEANUP.equals(getParamAction())) {
      if ((contentConversion == null)
          || (contentConversion.equals(CmsHtmlConverter.PARAM_DISABLED))) {
        // if the current conversion mode is "false" only, we have to remove the "false" value and
        // set it to "cleanup", as "false" will be stronger than all other values
   ...
   * <p>
   *
   * @param save if set to true, the result String is not escaped and the content parameter is not
   *     updated
