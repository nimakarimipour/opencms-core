...
public class CmsDialogElements extends CmsDialog {
  ...
  /**
   * Creates a list of possible elements of a template from the template property
   * "template-elements" and the elements available in the provided xmlPage.
   *
   * <p>
   *
   ...
   * @return the list of elements in a String array with element name, nice name (if present) and
   *     mandatory flag
   */
  public static List<CmsDialogElement> computeElements(
      CmsObject cms, CmsXmlPage xmlPage, String xmlPageUri, Locale locale) {
    ...
    try {
      currentTemplate =
          cms.readPropertyObject(xmlPageUri, CmsPropertyDefinition.PROPERTY_TEMPLATE, true)
              .getValue();
  ...
  /**
   * Creates a list of possible elements of a template from the template property
   * "template-elements" and the elements available in the provided resource file.
   *
   * <p>
   *
   * @param cms the CmsObject
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        if (element.isMandantory()
            ...
            || Boolean.valueOf(
                    getJsp().getRequest().getParameter(PREFIX_PARAM_BODY + element.getName()))
                .booleanValue()) {
  ...
  public String buildElementList() {
    ...
    try {
      ...
      while (i.hasNext()) {
        ...
        retValue.append("<tr>\n");
        retValue.append(
            "\t<td style=\"white-space: nowrap;\" unselectable=\"on\">" + element.getNiceName());
        retValue.append("</td>\n");
        retValue.append(
            "\t<td class=\"textcenter\" unselectable=\"on\"><input type=\"checkbox\" name=\"");
