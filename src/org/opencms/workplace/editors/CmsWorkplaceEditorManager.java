 ...
 *
 * <p>This class provides methods and constants to select the right editor according to:
 *
 * <ul>
 *   <li>the user preferences
 *   <li>the users current browser
 *   <li>the resource type
 *   <li>the editor rankings
 * </ul>
 *
 * <p>
 *
...
public class CmsWorkplaceEditorManager {
  ...
  public CmsWorkplaceEditorManager(CmsObject cms) {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_READ_EDITIR_FOLDER_FAILED_1, CmsEditor.PATH_EDITORS));
    ...
    while (i.hasNext()) {
      ...
      try {
        configFile =
            cms.readFile(
                folderName + EDITOR_CONFIGURATION_FILENAME, CmsResourceFilter.IGNORE_EXPIRATION);
      } catch (CmsException e) {
      ...
      CmsWorkplaceEditorConfiguration editorConfig =
          new CmsWorkplaceEditorConfiguration(
              xmlData, folderName + EDITOR_FILENAME, currentFolder.getName());
      if (editorConfig.isValidConfiguration()) {
        m_editorConfigurations.add(editorConfig);
      }
    }
    m_preferredEditors =
        new HashMap<String, CmsWorkplaceEditorConfiguration>(m_editorConfigurations.size());
  }
   ...
   *
   * <p>This map has the resource type name as key, the value is a sorted map with the ranking as
   * key and a CmsWorkplaceEditorConfiguration object as value.
   *
   * <p>
   *
  ...
  public Map<String, SortedMap<Float, CmsWorkplaceEditorConfiguration>> getConfigurableEditors() {
    ...
    Map<String, SortedMap<Float, CmsWorkplaceEditorConfiguration>> configurableEditors =
        new HashMap<String, SortedMap<Float, CmsWorkplaceEditorConfiguration>>();
    ...
    while (i.hasNext()) {
      ...
      while (k.hasNext()) {
        ...
        CmsExplorerTypeSettings settings =
            OpenCms.getWorkplaceManager().getExplorerTypeSetting(key);
        ...
        if ((currentConfig.getMappingForResourceType(key) == null)
          ...
          SortedMap<Float, CmsWorkplaceEditorConfiguration> editorConfigs =
              configurableEditors.get(key);
        ...
        try (ByteArrayInputStream input = new ByteArrayInputStream(file.getContents())) {
          config =
              new CmsParameterConfiguration(
                  input); // Uses ISO-8859-1, should be OK for config parameters
  ...
  public String getWidgetEditor(CmsRequestContext context, String userAgent) {
    ...
    if (preferredEditorSetting == null) {
      // no preferred editor setting found for this resource type, look for mapped resource type
      // preferred editor
      Iterator<CmsWorkplaceEditorConfiguration> i = m_editorConfigurations.iterator();
      while (i.hasNext()) {
        CmsWorkplaceEditorConfiguration currentConfig = i.next();
  ...
  protected String getDefaultEditorUri(
    ...
    if (context == null) {
      // this is just so that all parameters are used, signature should be identical to
      // getEditorUri(...)
      return null;
    }
  ...
  /**
   * Filters the matching editors for the given resource type from the list of all available
   * editors.
   *
   * <p>
   *
   * @param resourceType the resource type to filter
   * @return a map of filtered editor configurations sorted asceding by the ranking for the current
   *     resource type, with the (Float) ranking as key
   */
  private SortedMap<Float, CmsWorkplaceEditorConfiguration> filterEditorsForResourceType(
      String resourceType) {

    SortedMap<Float, CmsWorkplaceEditorConfiguration> filteredEditors =
        new TreeMap<Float, CmsWorkplaceEditorConfiguration>();
    Iterator<CmsWorkplaceEditorConfiguration> i = m_editorConfigurations.iterator();
    while (i.hasNext()) {
      CmsWorkplaceEditorConfiguration currentConfig = i.next();
  ...
  /**
   * Filters the preferred editor from the list of all available editors.
   *
   * <p>
   *
