 ...
 *
 * <p>Holds all necessary information about an OpenCms editor which is stored in the
 * "editor_configuration.xml" file in each editor folder.
 *
 * <p>Provides methods to get the editor information for the editor manager.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsWorkplaceEditorConfiguration {
  ...
  /**
   * Gets the name of the editor configuration (usually the folder name under
   ...
   *
   * <p>A single map item has the resource type name as key, the value is a String array with two
   * entries:
   *
   * <ul>
   *   <li>Entry 0: the ranking for the resource type
   *   <li>Entry 1: the mapping to another resource type or null
   * </ul>
   *
   * <p>
   *
      ...
      if (matches) {
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_BROWSER_MATCHES_CONFIG_1, currentBrowser));
        }
        return true;
      }
    }
  ...
  private void initialize(Document document, String editorUri) {
    ...
    while (i.hasNext()) {
      ...
      if (CmsStringUtil.isEmpty(name)) {
        logConfigurationError(
            Messages.get().getBundle().key(Messages.ERR_INVALID_RESTYPE_NAME_0), null);
        continue;
      }
      try {
        ...
        logConfigurationError(
            Messages.get().getBundle().key(Messages.ERR_INVALID_RESTYPE_RANKING_1, name), t);
        continue;
      }
    ...
    while (i.hasNext()) {
      ...
      try {
        ...
        logConfigurationError(
            Messages.get().getBundle().key(Messages.ERR_INVALID_RESTYPE_CLASS_1, name), t);
        continue;
      }
      float ranking;
      try {
    ...
    while (i.hasNext()) {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(agentName)) {
        ...
        logConfigurationError(
            Messages.get().getBundle().key(Messages.ERR_INVALID_USERAGENT_DEF_0), null);
      }
    }
  ...
  /**
   * Logs configuration errors and invalidates the current configuration.
   *
   * <p>
   *
  ...
  /**
   * Sets the list of compiled browser patterns.
   *
   * <p>
   *
  ...
  /**
   * Sets the editor label key used for the localized nice name.
   *
   * <p>
   *
  ...
  /**
   * Sets the valid resource types of the editor.
   *
   * <p>
   *
  ...
  /**
   * Sets the valid user agents regular expressions of the editor.
   *
   * <p>
   *
  ...
  /**
   * Sets if the current configuration is valid.
   *
   * <p>
   *
  ...
  /**
   * Sets the widget editor class for rich text editing.
   *
   * <p>
   *
