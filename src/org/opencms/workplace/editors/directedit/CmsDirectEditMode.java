...
/**
 * Constants to indicate which mode to use for placement of the HTML that generates the direct edit
 * buttons.
 *
 * <p>There are 3 basic options for the direct edit mode:
 *
 * <ul>
 *   <li>{@link #FALSE}: Direct edit is disabled.
 *   <li>{@link #AUTO}: Direct edit button HTML is inserted automatically.
 *   <li>{@link #MANUAL}: Direct edit button HTML is inserted manually by using &ltcms: editable
 *       mode="manual" /&gt tags.
 * </ul>
 *
 * There is one global option set for the page / template. The default is {@link #AUTO}.
 *
 * <p>There is an additional constant {@link #TRUE} that means "use the default mode of the page /
 * template".
 *
 * <p>It is possible to switch modes for an individual content loop. This is intended to use with
 * XmlContents that require special placement of the direct edit HTML because the default placement
...
public final class CmsDirectEditMode {
  ...
  /**
   * Indicates direct edit HTML is to be generated according to the default setting of the current
   * page.
   */
  ...
  /**
   * Returns {@link #TRUE} in case the given value is <code>true</code>, {@link #FALSE} otherwise.
   *
   * <p>
   *
   ...
   *
   * <p>For a mode instance <code>A</code>, {@link #toString()} returns the String mode description.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> in case this mode indicates direct edit is enabled.
   *
   * <p>Direct edit is enabled if this mode is not {@link #FALSE}, which is identical to <code>
   * {@link #getMode()} > 0</code>.
   *
