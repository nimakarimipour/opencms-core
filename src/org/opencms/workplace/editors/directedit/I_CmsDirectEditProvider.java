 ...
 * generated pages in offline mode.
 *
 * <p>In case you want to implement this, it's a good idea to extend from {@link
 * A_CmsDirectEditProvider} or {@link CmsDirectEditDefaultProvider} as these already contain the
 * required low level logic.
 *
 * <p>The default direct edit provider used can be configured in <code>opencms-workplace.xml</code>
 * in the <code>&lt;directeditprovider class="..." /&gt;</code> node. The standard provider is the
 * {@link CmsDirectEditDefaultProvider}.
 *
 * <p>
 *
 * @since 6.2.3
...
public interface I_CmsDirectEditProvider extends I_CmsConfigurationParameterHandler, Cloneable {
  ...
  String ATTRIBUTE_DIRECT_EDIT_PROVIDER =
      "org.opencms.workplace.editors.directedit.__directEditProvider";

  /** Key to identify the direct edit provider parameteres. */
  String ATTRIBUTE_DIRECT_EDIT_PROVIDER_PARAMS =
      "org.opencms.workplace.editors.directedit.__directEditProviderParams";
   ...
   */
  void insertDirectEditEmptyList(PageContext context, CmsDirectEditParams params)
      throws JspException;
   ...
   * @throws JspException in case something goes wrong
   */
  void insertDirectEditIncludes(PageContext context, CmsDirectEditParams params)
      throws JspException;
   ...
   */
  void insertDirectEditListMetadata(PageContext context, I_CmsContentLoadCollectorInfo info)
      throws JspException;
   ...
   * @param context the JSP page context to insert the HTML to
   * @param params the parameters for the direct edit call
   * @return <code>true</code> in case a direct edit element was opened, <code>false</code>
   *     otherwise
   * @throws JspException in case something goes wrong
   */
  boolean insertDirectEditStart(PageContext context, CmsDirectEditParams params)
      throws JspException;

  /**
   * Returns <code>true</code> if this provider (currently) operates in manual mode.
   *
   * <p>In manual mode the direct edit HTML is inserted with <code>
   * &lt;cms:enditable mode="manual" /&gt</code> tags. Otherwise the direct edit HTML is
   * automatically inserted in the current page.
   *
   * <p>Some providers may not be able to operate in manual mode. These will always return <code>
   * false</code>.
   *
   * <p>
   *
