...
public class CmsAdvancedDirectEditProvider extends A_CmsDirectEditProvider {
  ...
  I_CmsAutoBeanFactory m_editableDataExtensionsFactory =
  ...
  /**
   * @see
   *     org.opencms.workplace.editors.directedit.A_CmsDirectEditProvider#getResourceInfo(java.lang.String)
   *     <p>Similar to the method in the superclass, but removes the write permission check, as this
  ...
  public void insertDirectEditIncludes(PageContext context, CmsDirectEditParams params)
    ...
    // For Advanced Direct Edit all necessary js and css-code is included by the enableADE tag.
    // Further includes in the head are not needed.

  }

  /**
   * @see
   *     org.opencms.workplace.editors.directedit.A_CmsDirectEditProvider#insertDirectEditListMetadata(javax.servlet.jsp.PageContext,
  ...
  public void insertDirectEditListMetadata(PageContext context, I_CmsContentLoadCollectorInfo info)
    ...
    I_CmsAutoBeanFactory collectorInfoFactory =
        AutoBeanFactorySource.create(I_CmsAutoBeanFactory.class);
    AutoBean<I_CmsContentLoadCollectorInfo> collectorInfoAutoBean =
        collectorInfoFactory.wrapCollectorInfo(info);
      ...
      case 2: // enabled
        try {
          CmsJspStandardContextBean contextBean =
              CmsJspStandardContextBean.getInstance(context.getRequest());
          CmsContainerElementBean element = contextBean.getElement();
          if ((element != null)
              && element.getId().equals(resourceInfo.getResource().getStructureId())) {
  ...
  public String startDirectEditEnabled(
      CmsDirectEditParams params, CmsDirectEditResourceInfo resourceInfo) throws JSONException {
    ...
    if ((resourceInfo.getResource() != null) && resourceInfo.getResource().isFile()) {
      I_CmsResourceType type =
          OpenCms.getResourceManager().getResourceType(resourceInfo.getResource());
    ...
    SitemapDirectEditPermissions sitemapConfigPermissions =
        configData.getDirectEditPermissions(typeName);
    ...
    editableData.put(
        "hasDelete",
        params.getButtonSelection().isShowDelete()
            && writable
            && sitemapConfigPermissions.canEdit());
    // hasNew = params.getButtonSelection().isShowNew() && writable &&
    // sitemapConfigPermissions.canEdit();
    ...
    if ((m_lastPermissionMode == 1) || !writable || (!sitemapConfigPermissions.canEdit())) {
      ...
      if (sitemapConfigPermissions.canCreate()) {
        noEditReason =
            messages.key(
                Messages
                    .GUI_DIRECTEDIT_CAN_ONLY_BE_CREATED_0); // "Can only be created but not edited";
      } else {
        noEditReason =
            messages.key(
                Messages
    ...
    @RUntainted AutoBean<I_CmsEditableDataExtensions> extensions =
        m_editableDataExtensionsFactory.createExtensions();
      ...
      if (!CmsStringUtil.isEmptyOrWhitespaceOnly(uploadFolder) && !"none".equals(uploadFolder)) {
        ...
        try {
          uploadFolderResource =
              m_cms.readResource(uploadFolder, CmsResourceFilter.IGNORE_EXPIRATION);
          hasNew &=
              m_cms.hasPermissions(
                  uploadFolderResource,
                  CmsPermissionSet.ACCESS_WRITE,
          ...
          LOG.debug(
              "hasNew = false for upload folder "
                  + uploadFolder
                  + ", reason: "
    ...
    extensions
        .as()
        .setUploadEnabled(isUploadType && hasUploadSupport() && (params.getUploadFolder() != null));
    extensions.as().setUploadFolder(params.getUploadFolder());
    editableData.put(
        CmsGwtConstants.ATTR_EXTENSIONS, AutoBeanCodex.encode(extensions).getPayload());
