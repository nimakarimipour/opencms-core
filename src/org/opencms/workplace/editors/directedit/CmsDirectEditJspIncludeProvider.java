 ...
 * the 6.2.3 release.
 *
 * <p>Even though placing the HTML of the direct edit buttons appears to be more "flexible" at
 * first, there is a large overhead invloved using this provider as compared to an implementation
 * like {@link CmsDirectEditDefaultProvider}. For every direct edit button on a page, a JSP include
 * is processed <i>twice</i> using this provider, one include for the opening and one for the
 * closing HTML. A JSP include is a costly operation, which means the performance of a website is be
 * impacted if many content managers work on the system that makes great use of direct edit with a
 * lot of elements on a page. In order to avoid this performance impact, OpenCms since version 6.2.3
 * uses the {@link CmsDirectEditDefaultProvider} by default.
 *
...
public class CmsDirectEditJspIncludeProvider extends A_CmsDirectEditProvider {
  ...
  public static final String DIRECT_EDIT_INCLUDE_FILE_URI_DEFAULT =
  ...
  /**
   * Key to identify additional direct edit options, used e.g. to control which direct edit buttons
   * are displayed
   */
  ...
  public static String includeDirectEditElement(
    ...
    CmsJspTagInclude.addParameter(
        parameterMap, I_CmsResourceLoader.PARAMETER_ELEMENT, element, true);
    ...
    if (createLink != null) {
      CmsJspTagInclude.addParameter(
          parameterMap, DIRECT_EDIT_PARAM_NEWLINK, CmsEncoder.encode(createLink), true);
    }
    ...
    try {
      ...
      controller
          .getCurrentResponse()
          .addToIncludeList(jspIncludeFile, parameterMap, CmsRequestUtil.getAtrributeMap(req));
  ...
  public boolean insertDirectEditStart(PageContext context, CmsDirectEditParams params)
    ...
    String result = null;
    CmsDirectEditPermissions permissions =
        getResourceInfo(params, params.getResourceName()).getPermissions();
  ...
  /**
   * Returns <code>false</code> because the JSP include provider does not support manual button
   * placement.
   *
   * <p>
   *
