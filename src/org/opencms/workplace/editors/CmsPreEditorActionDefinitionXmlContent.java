...
/**
 * Pre editor action for XML content resource types, checks if model files are available for the XML
 * content to create in direct edit mode and shows the selection before opening the editor.
 *
 * <p>
 *
 * @since 6.5.4
...
public class CmsPreEditorActionDefinitionXmlContent extends A_CmsPreEditorActionDefinition {
   ...
   */
  @Override
  public boolean doPreAction(CmsResource resource, CmsDialog dialog, String originalParams)
      throws Exception {
    ...
    if (CmsStringUtil.isNotEmpty(newlink)) {
      // pre editor action not executed yet and new link is provided, now check model files for
      // resource type
      ...
      if (CmsResourceTypeXmlContent.getModelFiles(dialog.getCms(), folderPath, type.getTypeName())
              .size()
          > 0) {
        ...
        params.put(
            CmsDialog.PARAM_ACTION, new String[] {CmsResourceTypeXmlContent.DIALOG_CHOOSEMODEL});
