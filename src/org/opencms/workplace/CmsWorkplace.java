...
public abstract class CmsWorkplace {
  ...
  public static final String FILE_EXPLORER_FILELIST =
      VFS_PATH_VIEWS + "explorer/explorer_files.jsp";
  ...
  public static final String VFS_PATH_MODELDIALOG =
      CmsWorkplace.VFS_PATH_COMMONS + "newresource_xmlcontent_modelfile.jsp";

  /** Absolute path to thenew resource dialog. */
  public static final String VFS_PATH_NEWRESOURCEDIALOG =
      CmsWorkplace.VFS_PATH_COMMONS + "newresource_xmlcontent.jsp";
  ...
  private @RUntainted Map<String, @RUntainted String[]> m_parameterMap;
   ...
   * <p>
   *
   * @param parameters a string that will be inserted into the initial select tag, if null no
  ...
  /**
   * Checks if permissions for roles should be editable for the current user on the resource with
   * the given path.
   *
   * <p>
   *
   ...
   * @return <code>true</code> if permissions for roles should be editable for the current user on
    ...
    if (CmsStringUtil.isNotEmpty(startSiteRoot)
      ...
      List<CmsSite> sites =
          OpenCms.getSiteManager()
              .getAvailableSites(cms, false, cms.getRequestContext().getCurrentUser().getOuFqn());
      if (sites.size() == 1) {
        ...
        if (siteRoot == null) {
          // list of available sites contains more than one site, but not the configured default,
          // pick any
          startSiteRoot = sites.get(0).getSiteRoot();
        } else {
  ...
  public static String getStaticResourceUri(String resourceName, String versionInfo) {
    ...
    String uri =
        CmsStringUtil.joinPaths(OpenCms.getSystemInfo().getStaticResourceContext(), resourceName);
    if (versionInfo != null) {
      uri += "?v=" + versionInfo;
    }
    return uri;
  ...
  public static String getStyleUri(CmsJspActionElement jsp) {
    ...
    if (m_styleUri == null) {
      ...
      try {
        jsp.getCmsObject()
            .getRequestContext()
            .setCurrentProject(jsp.getCmsObject().readProject(CmsProject.ONLINE_PROJECT_ID));
   ...
   *
   * <p>To create a temporary file name of a resource name, the prefix char <code>'~'</code> (tilde)
   ...
   */
  public static String getTemporaryFileName(@RUntainted String resourceName) {
  ...
  /**
   * Creates a link for the OpenCms workplace that will reload the whole workplace, switch to the
   * explorer view, the site of the given explorerRootPath and show the folder given in the
   * explorerRootPath.
   *
   * <p>
   *
   * @param jsp needed for link functionality.
   * @param explorerRootPath a root relative folder link (has to end with '/').
   * @return a link for the OpenCms workplace that will reload the whole workplace, switch to the
   *     explorer view, the site of the given explorerRootPath and show the folder given in the
   *     explorerRootPath.
   */
  public static String getWorkplaceExplorerLink(
      final CmsJspActionElement jsp, final String explorerRootPath) {
   ...
   */
  public static String getWorkplaceExplorerLink(
      final CmsObject cms, final @RUntainted String explorerRootPath) {
    ...
    link2Source.append(
        OpenCms.getLinkManager()
            .substituteLinkForUnknownTarget(
                cms, "/system/workplace/views/explorer/explorer_fs.jsp"));
    link2Source.append("&");
    ...
    if (session != null) {
      ...
      workplaceSettings =
          (CmsWorkplaceSettings)
              session.getAttribute(CmsWorkplaceManager.SESSION_WORKPLACE_SETTINGS);
  ...
  /**
   * Updates the user settings in the given workplace settings for the current user, reading the
   * user settings from the database if required.
   *
   * <p>
   *
   * @param cms the cms object for the current user
   * @param settings the workplace settings to update (if <code>null</code> a new instance is
   *     created)
  ...
  /**
   * Updates the given workplace settings, also re-initializing the state of the Workplace to the
   * users preferences (for example setting the startup site and project).
   *
   * <p>The user settings will also be updated by calling <code>
   * {@link #initUserSettings(CmsObject, CmsWorkplaceSettings, boolean)}</code> before updating the
   * workplace project, selected site etc.
   *
   * <p>
   *
   * @param cms the cms object for the current user
  ...
  public static synchronized @RUntainted CmsWorkplaceSettings initWorkplaceSettings(
    ...
    settings.setViewUri(
        OpenCms.getLinkManager().substituteLink(cms, settings.getUserSettings().getStartView()));
    return settings;
  }

  /**
   * Returns <code>true</code> if the given resource is a temporary file.
   *
   * <p>A resource is considered a temporary file it is a file where the {@link
   * CmsResource#FLAG_TEMPFILE} flag has been set, or if the file name (without parent folders)
   * starts with the prefix char <code>'~'</code> (tilde).
   *
   * <p>
   *
   ...
   */
  public static @RUntainted String substituteSiteTitleStatic(@RUntainted String title, @RUntainted Locale locale) {
  ...
  public static void updateUserPreferences(CmsObject cms, HttpServletRequest req) {
    ...
    CmsSynchronizeSettings synchronizeSettings =
  ...
  /**
   * Stores the settings in the given session.
   *
   * <p>
   *
  ...
  /**
   * Returns all parameters of the current workplace class as hidden field tags that can be inserted
   * in a form.
   *
   * <p>
   *
   * @return all parameters of the current workplace class as hidden field tags that can be inserted
   *     in a html form
   */
  public String allParamsAsHidden() {
    ...
    while (i.hasNext()) {
      Entry<String, @RUntainted Object> entry = i.next();
      result.append("<input type=\"hidden\" name=\"");
      result.append(entry.getKey());
      result.append("\" value=\"");
      String encoded =
          CmsEncoder.encode(
              entry.getValue().toString(), getCms().getRequestContext().getEncoding());
   ...
   *
   * <p>The String is formatted as a parameter String (<code>param1=val1&amp;param2=val2</code>)
  ...
  public String allParamsAsRequest() {
    ...
    while (paramNames.hasNext()) {
      ...
      @RUntainted String paramValue = request.getParameter(paramName);
      retValue.append(
          paramName
              + "="
              + CmsEncoder.encode(paramValue, getCms().getRequestContext().getEncoding()));
  ...
  public String buildSelect(
      String parameters, List<String> options, List<String> values, int selected) {
   ...
   * @param target the href link target for the button, if none is given the target will be same
   *     window
  ...
  public String button(
      String href, String target, String image, @RUntainted String label, int type, String imagePath) {
  ...
  public String buttonBarSpacer(int width) {
    ...
    StringBuffer result = new StringBuffer(128);
    result.append(
        "<td><span class=\"norm\"><span unselectable=\"on\" class=\"txtbutton\" style=\"width: ");
    result.append(width);
    result.append("px;\"></span></span></td>\n");
    return result.toString();
  }
    ...
    if (OpenCms.getWorkplaceManager().autoLockResources()) {
      ...
      if (lockable) {
        ...
        throw new CmsException(
            Messages.get().container(Messages.ERR_WORKPLACE_LOCK_RESOURCE_1, resource));
      }
    } else {
      if (!lockable) {
        throw new CmsException(
  ...
  /**
   * First sets site and project in the workplace settings, then fills all class parameter values
   * from the data provided in the current request.
   *
   * <p>
   *
   * @param settings the workplace settings
   ...
   *
   * <p>All methods that start with "setParam" are possible candidates to be automatically filled.
   * The remaining part of the method name is converted to lower case. Then a parameter of this name
   * is searched in the request parameters. If the parameter is found, the "setParam" method is
   * automatically invoked by reflection with the value of the parameter.
   *
   * <p>
   *
  ...
  public void fillParamValues(@RUntainted HttpServletRequest request) {
    ...
    while (i.hasNext()) {
      ...
      // and by decoding not encoded data the data will get corrupted, for instance '1+2' will
      // become '1 2'.
   ...
   *
   * <p>Caution: returns the pure message String (not escaped) or null, if no message is pending.
   *
   * <p>
   *
  ...
  public String getExplorerFileListFullUri() {
    ...
    synchronized (this) {
      m_file_explorer_filelist =
          OpenCms.getLinkManager().substituteLink(getCms(), FILE_EXPLORER_FILELIST);
    }
    return m_file_explorer_filelist;
  }

  /**
   * Returns the html for the frame name and source and stores this information in the workplace
   * settings.
   *
   * <p>
   *
  ...
  /**
   * Returns a list of FileItem instances parsed from the request, in the order that they were
   * transmitted.
   *
   * <p>This list is automatically initialized from the
   * createParameterMapFromMultiPart(HttpServletRequest) method.
   *
   * <p>
   *
   * @return list of FileItem instances parsed from the request, in the order that they were
   *     transmitted
   */
   ...
   *
   * <p>Workplaces static resources are images, css files etc. These are exported during the
   * installation of OpenCms, and are usually only read from this exported location to avoid the
   * overhaead of accessing the database later.
   *
   * <p>
   *
  ...
  /**
   * Builds the start html of the page, including setting of DOCTYPE and inserting a header with the
   * content-type.
   *
   * <p>
   *
  ...
  /**
   * Sets site and project in the workplace settings with the request values of parameters <code>
   * {@link CmsWorkplace#PARAM_WP_SITE}</code> and <code>{@link CmsWorkplace#PARAM_WP_PROJECT}
   * </code>.
   *
   * <p>
   *
  ...
  public boolean initSettings(CmsWorkplaceSettings settings, @RUntainted HttpServletRequest request) {
    ...
    @RUntainted String site = request.getParameter(PARAM_WP_SITE);
    if (site != null) {
      reloadRequired = true;
  ...
  public boolean isSubElement() {
    ...
    return !getJsp()
        .getRequestContext()
        .getUri()
        .equals(getJsp().info("opencms.request.element.uri"));
  }

  /**
   * Returns the localized resource string for a given message key, checking the workplace default
   * resources and all module bundles.
   *
   * <p>If the key was not found, the return value is <code>"??? " + keyName + " ???"</code>.
   *
   * <p>If the key starts with <code>"help."</code> and is not found, the value <code>"index.html"
   * </code> is returned.
   *
   * <p>
   *
  ...
  /**
   * Returns the localized resource string for a given message key, with the provided replacement
   * parameters.
   *
   * <p>If the key was found in the bundle, it will be formatted using a <code>
   * {@link java.text.MessageFormat}</code> using the provided parameters.
   *
   * <p>If the key was not found in the bundle, the return value is <code>"??? " + keyName + " ???"
   * </code>. This will also be returned if the bundle was not properly initialized first.
   *
   * @param keyName the key for the desired string
  ...
  /**
   * Returns the localized resource string for the given message key, checking the workplace default
   * resources and all module bundles.
   *
   * <p>If the key was not found, the provided default value is returned.
   *
   * <p>
   *
   * @param keyName the key for the desired string
  ...
  /**
   * Returns the empty String "" if the provided value is null, otherwise just returns the provided
   * value.
   *
   * <p>Use this method in forms if a getParamXXX method is used, but a String (not null) is
   * required.
   *
   * @param value the String to check
   * @return the empty String "" if the provided value is null, otherwise just returns the provided
   *     value
   */
  ...
  /**
   * Returns the default html for a workplace page, including setting of DOCTYPE and inserting a
  ...
  /**
   * Returns the default html for a workplace page, including setting of DOCTYPE and inserting a
   * header with the content-type, allowing the selection of an individual style sheet.
   *
   * <p>
   *
   * @param segment the HTML segment (START / END)
   * @param title the title of the page, if null no title tag is inserted
   * @param stylesheet the used style sheet, if null the default stylesheet 'workplace.css' is
   *     inserted
   * @return the default html for a workplace page
   */
  ...
  /**
   * Returns all initialized parameters of the current workplace class as hidden field tags that can
   * be inserted in a form.
   *
   * <p>
   *
   * @return all initialized parameters of the current workplace class as hidden field tags that can
   *     be inserted in a html form
   */
  ...
  /**
   * Returns all initialized parameters of the current workplace class that are not in the given
   * exclusion list as hidden field tags that can be inserted in a form.
   *
   * <p>
   *
   * @param excludes the parameters to exclude
   * @return all initialized parameters of the current workplace class that are not in the given
   *     exclusion list as hidden field tags that can be inserted in a form
   */
  ...
  /**
   * Returns all initialized parameters of the current workplace class in the form of a parameter
   * map, i.e. the values are arrays.
   *
   * <p>
   *
   * @return all initialized parameters of the current workplace class in the form of a parameter
   *     map
   */
   ...
   * in the form <code>key1=value1&key2=value2</code> etc.
   *
   * @return all initialized parameters of the current workplace class as request parameters
   */
   ...
   *
   * <p>The following macro contexts are available in the Workplace:
   *
   * <ul>
   *   <li>Macros based on the current users OpenCms context (obtained from the current <code>
   *       {@link CmsObject}</code>).
   *   <li>Localized key macros (obtained from the current <code>{@link CmsMessages}</code>).
   *   <li>Macros from the current JSP page context (obtained by <code>{@link #getJsp()}</code>).
   * </ul>
   *
   ...
   */
  public void sendForward(String location, Map<String, String[]> params)
    ...
    CmsRequestUtil.forwardRequest(
        getJsp().link(location), parameters, getJsp().getRequest(), getJsp().getResponse());
  }
   ...
   *
   * <p>In special cases some parameters might require a different-from-default encoding. This is
   * the case if the content of the parameter was encoded using the JavaScript encodeURIComponent()
   * method on the client, which always encodes in UTF-8.
   *
   * <p>
   *
   ...
   *
   * <p>This method will also handle parameters from forms of type <code>multipart/form-data</code>.
   *
   * <p>
   *
   ...
   *
   * <p>By default the {@link CmsWorkplaceMessages} are initialized.
   *
   * <p>You SHOULD override this method for setting the bundles you really need, using the <code>
   * {@link #addMessages(CmsMessages)}</code> or <code>{@link #addMessages(String)}</code> method.
   *
   * <p>
   */
  protected void initMessages() {

    // no bundles are added by default as all core bundles are added as part of the
    // WorkplaceModuleMessages
  }
   ...
   *
   * <p>Timwarping is controlled by the session attribute {@link
   * CmsContextInfo#ATTRIBUTE_REQUEST_TIME} with a value of type <code>Long</code>.
   *
   * <p>
   *
   ...
   *
   * <p>This method can be used in case there a workplace class was generated using {@link
  ...
  protected void initWorkplaceMembers(CmsObject cms, @RUntainted HttpSession session) {
    ...
    m_settings =
        (CmsWorkplaceSettings)
            m_session.getAttribute(CmsWorkplaceManager.SESSION_WORKPLACE_SETTINGS);
   ...
   * @param request the current request
   */
  protected abstract void initWorkplaceRequestValues(
   ...
   */
  protected String substituteSiteTitle(@RUntainted String title) {
   ...
   *
   * <p>The id of the old project is stored in a member variable to switch back.
   *
   * <p>
   *
  ...
  /**
   * Sets the cms request context and other cms related settings to the values stored in the
   * workplace settings.
   *
   * <p>
   *
  ...
  /**
   * Returns a list of all methods of the current class instance that start with "getParam" and have
   * no parameters.
   *
   * <p>
   *
   * @return a list of all methods of the current class instance that start with "getParam" and have
   *     no parameters
   */
  ...
  /**
   * Returns a list of all methods of the current class instance that start with "setParam" and have
   * exactly one String parameter.
   *
   * <p>
   *
   * @return a list of all methods of the current class instance that start with "setParam" and have
   *     exactly one String parameter
   */
  private List<@RUntainted Method> paramSetMethods() {
    ...
    int length = methods.length;
    for (int i = 0; i < length; i++) {
      @RUntainted Method method = methods[i];
