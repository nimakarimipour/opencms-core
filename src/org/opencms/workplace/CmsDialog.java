...
public class CmsDialog extends CmsToolDialog {
  ...
  /**
   * Constant for the "Discard" button in the build button methods (same function as "Cancel" button
   * but different text on button.
   */
  public static final int BUTTON_DISCARD = 8;

  /**
   * Constant for the "Edit" button in the build button methods (same function as "Ok" button but
  ...
  /**
   * The custom mapping for online help.
   *
   * <p>It will be translated to a javascript variable called onlineHelpUriCustom. If it is set, the
   * top.head javascript for the online help will use this value.
   *
   * <p>
   */
   ...
   *
   * <p>This method is used by dialog elements. The dialog elements do not initialize their own
   * workplace class, but use the initialized instance of the "master" class. This is required to
   * ensure that parameters of the "master" class can properly be kept on the dialog elements.
   *
   * <p>To prevent null pointer exceptions, an empty dialog is returned if nothing is found in the
   * request attributes.
   *
   * <p>
   *
   ...
   */
  public static CmsDialog initCmsDialog(
   ...
   *
   * <p>This method tries to include the URI stored in the workplace settings. This URI is
   * determined by the frame name, which has to be set in the frame name parameter.
   *
   * <p>
   *
  ...
  public void actionCloseDialog() throws JspException {
    ...
    // create a map with empty "resource" parameter to avoid changing the folder when returning to
    // explorer file list
    ...
    if (isPopup()) {
      ...
      try {
        if (CmsLinkManager.isWorkplaceLink(getParamCloseLink())) {
          // in case the close link points to the new workplace, make sure to set the new location
          // on the top frame
          ...
          if (!isForwarded()) {
            ...
            CmsRequestUtil.forwardRequest(
                getParamCloseLink(), getJsp().getRequest(), getJsp().getResponse());
          }
        }
      } catch (Exception e) {
        // forward failed
  ...
  public String dialog(int segment, String attributes) {
    ...
    if (segment == HTML_START) {
      ...
      html.append("<tr><td>\n");
      if (useNewStyle()
          && getToolManager().hasToolPathForUrl(getJsp().getRequestContext().getUri())) {
  ...
  /**
   * Builds the start HTML for a block with 3D border and optional subheadline in the dialog content
   * area.
   *
   * <p>
   *
  ...
  public String dialogButtonsCloseDetails(String closeAttribute, String detailsAttribute) {
    ...
    return dialogButtons(
        new int[] {BUTTON_CLOSE, BUTTON_DETAILS}, new String[] {closeAttribute, detailsAttribute});
  }
  ...
  public String dialogButtonsOkCancel(String okAttributes, String cancelAttributes) {
    ...
    return dialogButtons(
        new int[] {BUTTON_OK, BUTTON_CANCEL}, new String[] {okAttributes, cancelAttributes});
  }
  ...
  public String dialogButtonsOkCancelAdvanced(
      String okAttributes, String cancelAttributes, String advancedAttributes) {
   ...
   * @return the button row
   */
  public String dialogButtonsSetOkCancel(
      String setAttributes, String okAttributes, String cancelAttributes) {
   ...
   *
   * <p>The action value is used on JSP pages to select the proper action in a large "switch"
   * statement.
   *
   * <p>
   *
  ...
  /**
   * Returns the http URI of the current dialog, to be used as value for the "action" attribute of a
   * html form.
   *
   * <p>This URI is the real one.
   *
   * <p>
   *
   ...
   * html form.
   *
   * <p>This URI could not be really the real one...
   *
   * <p>
   *
   * @return the http URI of the current dialog
   ...
   *
   * <p>The action parameter is very important, it will select the dialog action to perform. The
   * value of the {@link #getAction()} method will be initialized from the action parameter.
   *
   * <p>
   *
   ...
   *
   * <p>This parameter is very important. It must match to the localization keys, e.g. "copy" for
   * the copy dialog.
   *
   * <p>This parameter must be set manually by the subclass during first initialization.
   *
   * <p>
   *
   ...
   *
   * <p>Use this parameter to indicate that the dialog is shown in a popup window.
   *
   * <p>
   *
   ...
   *
   * <p>The message parameter is used on dialogs to show any text message.
   *
   * <p>
   *
   ...
   *
   * <p>This stores the request parameter values from a previous dialog, if necessary.
   *
   * <p>
   *
   ...
   *
   * <p>The file parameter selects the file on which the dialog action is to be performed.
   *
   * <p>
   *
   ...
   *
   * <p>This parameter is used to build the title of the dialog. It is a parameter so that the title
   ...
   *
   * <p>This is used to determine whether the dialog shows the option to delete all siblings of the
   * resource or not.
   *
   ...
   * content-type.
   *
   * <p>This overloads the default method of the parent class.
   *
   * <p>
   *
  ...
  /**
   * Builds the start html of the page, including setting of DOCTYPE, inserting a header with the
   * content-type and choosing an individual style sheet.
   *
   * <p>
   *
   * @param title the title for the page
   ...
   *
   * <p>This value will be set to a javascript variable called onlineHelpUriCustom. If it is set,
   * the top.head javascript for the online help will use this value.
   *
   * <p>This method should be called from <code>
   * {@link #initWorkplaceRequestValues(CmsWorkplaceSettings, HttpServletRequest)}</code>, <code>
   * {@link CmsWorkplace#initWorkplaceMembers(CmsJspActionElement)}</code> or from the jsp if the
   * dialog class is used for several actions. It should be used whenever the online help mapping
   * does not work (due to jsp - forwards).
   *
   * <p>
   *
  ...
  protected String buildAjaxWaitMessage() {
    ...
    html.append(
        "<td valign='middle'><span id='ajaxreport-txt' style='color: #000099; font-weight: bold;'>\n");
  ...
  /**
   * Checks if the permissions of the current user on the resource to use in the dialog are
   * sufficient.
   *
   * <p>Automatically generates a CmsMessageContainer object with an error message and stores it in
   * the users session.
   *
   * <p>
   *
   ...
   * @param neededForFolder if true, the permissions are required for the parent folder of the
   *     resource (e.g. for editors)
   * @param errorMessage the message container that is stored in the session in case the permissions
   *     are not sufficient
   * @return true if the permissions are sufficient, otherwise false
   */
  protected boolean checkResourcePermissions(
      CmsPermissionSet required, boolean neededForFolder, CmsMessageContainer errorMessage) {
    ...
    try {
      ...
      if (neededForFolder) {
        ...
        res =
            getCms()
                .readResource(
                    CmsResource.getParentFolder(getParamResource()), CmsResourceFilter.ALL);
      } else {
  ...
  protected String computeCurrentFolder() {
    ...
    if (currentFolder == null) {
      ...
      try {
        currentFolder =
            getCms().getSitePath(getCms().readFolder("/", CmsResourceFilter.IGNORE_EXPIRATION));
      } catch (CmsException e) {
  ...
  protected void openWorkplaceLink(String workplaceLink) throws IOException {
    ...
    // in case the close link points to the new workplace, make sure to set the new location on the
    // top frame
    JspWriter out = getJsp().getJspContext().getOut();
    try {
      out.write(
          "<html><head><script >top.location.href=\"" + workplaceLink + "\";</script></head>\n");
      out.write("</html>\n");
    } finally {
      out.close();
    }
