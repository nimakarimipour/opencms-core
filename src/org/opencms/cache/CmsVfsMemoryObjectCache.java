...
/**
 * Implements a memory cache, that stores objects related to VFS files, providing a cache for the
 * "online" and another for the "offline" project.
 *
 * <p>
 *
 * @since 6.1.3
...
public class CmsVfsMemoryObjectCache extends CmsVfsCache {
  ...
  public static CmsVfsMemoryObjectCache getVfsMemoryObjectCache() {
    ...
    // This means that a CmsVfsMemoryObjectCache initialized in one test class will not get flushed
    // in
  ...
  /**
   * Uses a transformer for loading an object from a path if it has not already been cached, and
   * then caches it.
   *
   * <p>
   *
  ...
  /**
   * Returns a cache key for the given root path based on the status of the given CmsObject.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  private String getCacheKeyForCurrentProject(CmsObject cms, String rootPath) {
    ...
    // check the project
    boolean project =
        (cms != null) ? cms.getRequestContext().getCurrentProject().isOnlineProject() : false;
    return getCacheKey(rootPath, project);
  }

  /**
   * Uncaches a system id (filename) from the internal offline temporary and content definitions
   * caches.
   *
    ...
    if ((null != o) && LOG.isDebugEnabled()) {
      LOG.debug(
          org.opencms.xml.Messages.get()
              .getBundle()
              .key(org.opencms.xml.Messages.LOG_ERR_UNCACHED_SYS_ID_1, key));
    }
  }
}
