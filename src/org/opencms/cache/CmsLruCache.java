 ...
 *
 * <p>The idea of this cache is to separate the caching policy from the data structure where the
 * cached objects are stored. The advantage of doing so is, that the CmsFlexLruCache can identify
 * the last-recently-used object in O(1), whereas you would need at least O(n) to traverse the data
 * structure that stores the cached objects. Second, you can easily use the CmsFlexLruCache to get
 * an LRU cache, no matter what data structure is used to store your objects.
 *
 * <p>The cache policy is affected by the "costs" of the objects being cached. Valuable cache costs
 * might be the byte size of the cached objects for example.
 *
 * <p>To add/remove cached objects from the data structure that stores them, the objects have to
 * implement the methods defined in the interface I_CmsLruCacheObject to be notified when they are
 * added/removed from the CmsFlexLruCache.
 *
 * <p>
 *
...
public class CmsLruCache extends java.lang.Object {
   ...
   * @param theMaxObjectCosts the maximum allowed cache costs per object. Set theMaxObjectCosts to
   *     -1 if you don't want to limit the max. allowed cache costs per object
   */
   ...
   *
   * <p>If add the same object more than once, the object is touched instead.
   *
   * <p>
   *
   * @param theCacheObject the object being added to the cache
   * @return true if the object was added to the cache, false if the object was denied because its
   *     cache costs were higher than the allowed max. cache costs per object
   */
  ...
  /**
   * Removes an object from the list of all cached objects in this cache, no matter what position it
   * has inside the list.
   *
   * <p>
   *
  ...
  /**
   * Touch an existing object in this cache, in the sense that it's "last-recently-used" state is
   * updated.
   *
   * <p>
   *
  ...
  public synchronized boolean touch(I_CmsLruCacheObject theCacheObject) {
    ...
    if (prevObj == null) {
      // case 2: the object at the tail pos., remove it from the tail to put it to the front as the
      // new head
      I_CmsLruCacheObject newTail = nextObj;
      newTail.setPreviousLruObject(null);
      m_listTail = newTail;
    } else {
      // case 3: the object is somewhere within the list, remove it to put it the front as the new
      // head
  ...
  /**
   * Decrease this caches statistics and notify the cached object that it was removed from this
   * cache.
   *
   * <p>
   *
  ...
  /**
   * Removes the last recently used objects from the list of all cached objects as long as the costs
   * of all cached objects are higher than the allowed avg. costs of the cache.
   *
   * <p>
   */
  private void gc() {
