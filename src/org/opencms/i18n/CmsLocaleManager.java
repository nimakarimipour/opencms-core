 ...
 *
 * <p>Locale configuration is done in the configuration file <code>opencms-system.xml</code> in the
 * <code>opencms/system/internationalization</code> node and it's sub-nodes.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsLocaleManager implements I_CmsEventListener {
  ...
  /**
   * Required for setting the default locale on the first possible time.
   *
   * <p>
   */
  static {
  ...
  private @RUntainted I_CmsLocaleHandler m_localeHandler;
  ...
  /**
   * Returns the default locale configured in <code>opencms-system.xml</code>, that is the first
   * locale from the list provided in the <code>opencms/system/internationalization/localesdefault
   * </code> node.
   *
   * <p>
   *
   ...
   *
   * <p>The full name must consist of language code, country code(optional), variant(optional)
   * separated by "_".
   *
   * <p>This method will always return a valid Locale! If the provided locale name is not valid
   * (i.e. leads to an Exception when trying to create the Locale, then the configured default
   * Locale is returned.
   *
   * <p>
   *
   ...
   */
  public static @RUntainted Locale getLocale(@RUntainted String localeName) {
   ...
   *
   * <p>For example, if the input List contains <code>{@link Locale#ENGLISH}</code> and <code>
   * {@link Locale#GERMANY}</code>, the result will be <code>"en, de_DE"</code>.
   *
   * <p>An empty String is returned if the input is <code>null</code>, or contains no elements.
   *
   * <p>
   *
   ...
   */
  public static @RUntainted String getLocaleNames(List<Locale> locales) {
   ...
   */
  public static List<Locale> getLocales(List<@RUntainted String> localeNames) {
  ...
  public static List<String> getLocaleVariants(
      String basename, Locale locale, boolean wantBase, boolean defaultAsBase) {
    ...
    if (null == basename) {
      ...
      boolean wantDefaultAsBase =
          defaultAsBase && !(localeString.startsWith("_" + getDefaultLocale().toString()));
  ...
  public static Locale getMainLocale(CmsObject cms, CmsResource res) {
    ...
    try {
      defaultNames =
          cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_LOCALE, true).getValue();
   ...
   *
   * <p>The content encoding is controlled by the property {@link
   * CmsPropertyDefinition#PROPERTY_CONTENT_ENCODING}, which can be set on the resource or on a
   * parent folder for all resources in this folder.
   *
   * <p>In case no encoding has been set, the default encoding from {@link
   * org.opencms.main.CmsSystemInfo#getDefaultEncoding()} is returned.
   *
   * <p>
   *
  ...
  public static final @RUntainted String getResourceEncoding(CmsObject cms, CmsResource res) {
    ...
    try {
      ...
      if (LOG.isInfoEnabled()) {
        LOG.info(
            Messages.get().getBundle().key(Messages.ERR_READ_ENCODING_PROP_1, res.getRootPath()),
            e);
      }
    }
    if (encoding == null) {
  ...
  /**
   * Sets the default locale of the Java VM to <code>{@link Locale#ENGLISH}</code> if the current
   * default has any other language then English set.
   *
   * <p>This is required because otherwise the default (English) resource bundles would not be
   * displayed for the English locale if a translated default locale exists.
   *
   * <p>Here's an example of how this issues shows up: On a German server, the default locale
   * usually is <code>{@link Locale#GERMAN}</code>. All English translations for OpenCms are located
   * in the "default" message files, for example <code>org.opencms.i18n.message.properties</code>.
   * If the German localization is installed, it will be located in <code>
   * org.opencms.i18n.message_de.properties</code>. If user has English selected as his locale, the
   * default Java lookup mechanism first tries to find <code>org.opencms.i18n.message_en.properties
   * </code>. However, this file does not exist, since the English localization is kept in the
   * default file. Next, the Java lookup mechanism tries to find the servers default locale, which
   * in this example is German. Since there is a German message file, the Java lookup mechanism is
   * finished and uses this German localization, not the default file. Therefore the user get the
   * German localization, not the English one. Setting the default locale explicitly to English
   * avoids this issue.
   *
   * <p>
   */
      ...
      try {
        ...
        if (CmsLog.INIT.isInfoEnabled()) {
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_I18N_DEFAULT_LOCALE_2, Locale.ENGLISH, oldLocale));
        }
      } catch (Exception e) {
        ...
        CmsLog.INIT.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_UNABLE_TO_SET_DEFAULT_LOCALE_2, Locale.ENGLISH, oldLocale),
            e);
      }
    } else {
      if (CmsLog.INIT.isInfoEnabled()) {
  ...
  public void addAvailableLocale(@RUntainted String localeName) {
    ...
    @RUntainted Locale locale = getLocale(localeName);
    // add full variation (language / country / variant)
    if (!m_availableLocales.contains(locale)) {
      m_availableLocales.add(locale);
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get().getBundle().key(Messages.INIT_I18N_CONFIG_ADD_LOCALE_1, locale));
      }
    }
  ...
  /**
   * Implements the CmsEvent interface, the locale manager the events to clear the list of cached
   * keys .
   *
   * <p>
   *
  ...
  public List<Locale> getAvailableLocales(CmsObject cms, CmsResource resource) {
    ...
    try {
      availableNames =
          cms.readPropertyObject(resource, CmsPropertyDefinition.PROPERTY_AVAILABLE_LOCALES, true)
              .getValue();
    } catch (CmsException exc) {
      LOG.debug(
          "Could not read available locales property for resource " + resource.getRootPath(), exc);
    }
    ...
    try {
      availableNames =
          cms.readPropertyObject(
                  resourceName, CmsPropertyDefinition.PROPERTY_AVAILABLE_LOCALES, true)
              .getValue();
    } catch (CmsException exc) {
   ...
   *
   * <p>All names are filtered against the allowed available locales configured in <code>
   * opencms-system.xml</code>.
   *
   * <p>
   *
   ...
   * @return the locale
   */
  public @RUntainted Locale getBestAvailableLocaleForXmlContent(
      CmsObject cms, CmsResource resource, I_CmsXmlDocument content) {

    @RUntainted Locale locale = getDefaultLocale(cms, resource);
  ...
  /**
   * Tries to find the given requested locale (eventually simplified) in the collection of available
   * locales, if the requested locale is not found it will return the first match from the given
   * list of default locales.
   *
   * <p>
   *
   * @param requestedLocale the requested locale, if this (or a simplified version of it) is
  ...
  public Locale getBestMatchingLocale(
      Locale requestedLocale, List<Locale> defaults, List<Locale> available) {
   ...
   *
   * <p>It's possible to override the system default (see {@link #getDefaultLocale()}) by setting
   * the property <code>{@link CmsPropertyDefinition#PROPERTY_LOCALE}</code> to a comma separated
   * list of locale names. This property is inherited from the parent folders. This method will
   * return the first locale from that list.
   *
   * <p>The default locale must be contained in the set of configured available locales, see {@link
   * #getAvailableLocales()}. In case an invalid locale has been set with the property, this locale
   * is ignored and the same result as {@link #getDefaultLocale()} is returned.
   *
   * <p>In case the property <code>{@link CmsPropertyDefinition#PROPERTY_LOCALE}</code> has not been
   * set on the resource or a parent folder, this method returns the same result as {@link
   * #getDefaultLocale()}.
   *
   * <p>
   *
   ...
   * @see #getDefaultLocales(CmsObject, String)
   */
  public Locale getDefaultLocale(CmsObject cms, @RUntainted String resourceName) {
  ...
  /**
   * Returns the list of default {@link Locale}s configured in <code>opencms-system.xml</code>, in
   * the <code>opencms/system/internationalization/localesdefault</code> node.
   *
   * <p>Since the default locale is always available, the result list will always contain at least
   * one Locale.
   *
   * <p>It's possible to override the system default by setting the property <code>
   * {@link CmsPropertyDefinition#PROPERTY_LOCALE}</code> to a comma separated list of locale names.
   * This property is inherited from the parent folders.
   *
   * <p>The default locales must be a subset of the configured available locales, see {@link
   * #getAvailableLocales()}. In case an invalid locale has been set with the property, this locale
   * is ignored.
   *
   * <p>The default locale names are used as a fallback mechanism in case a locale is requested that
   * can not be found, for example when delivering content form an XML content.
   *
   * <p>There is a list of default locales (instead of just one default locale) since there are
   * scenarios when one default is not enough. Consider the following example:<i> The main default
   * locale is set to "en". An example XML content file contains just one language, in this case
   * "de" and not "en". Now a request is made to the file for the locale "fr". If there would be
   * only one default locale ("en"), we would have to give up. But since we allow more then one
   * default, we can deliver the "de" content instead of a blank page.</I>
   *
   * <p>
   *
   ...
   * one Locale.
   *
   * <p>It's possible to override the system default (see {@link #getDefaultLocales()}) by setting
   * the property <code>{@link CmsPropertyDefinition#PROPERTY_LOCALE}</code> to a comma separated
   * list of locale names. This property is inherited from the parent folders.
   *
   * <p>The default locales must be a subset of the configured available locales, see {@link
   * #getAvailableLocales()}. In case an invalid locale has been set with the property, this locale
   * is ignored.
   ...
   * #getDefaultLocales()}.
   *
   * <p>Use this method in case you need to get all configured default options for a resource, if
   * you just need the "the" default locale for a resource, use <code>
   * {@link #getDefaultLocale(CmsObject, String)}</code>.
   *
   * <p>
   *
   * @param cms the current cms permission object
  ...
  public List<Locale> getDefaultLocales(CmsObject cms, CmsResource resource) {
    ...
    try {
      defaultNames =
          cms.readPropertyObject(resource, CmsPropertyDefinition.PROPERTY_LOCALE, true).getValue();
    } catch (CmsException e) {
      LOG.warn(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_READ_ENCODING_PROP_1, cms.getSitePath(resource)),
          e);
    }
   ...
   */
  public @RUntainted List<@RUntainted Locale> getDefaultLocales(CmsObject cms, @RUntainted String resourceName) {

    String defaultNames = null;
    try {
      defaultNames =
          cms.readPropertyObject(resourceName, CmsPropertyDefinition.PROPERTY_LOCALE, true)
              .getValue();
    } catch (CmsException e) {
   ...
   *
   * <p>In case no match is found, code <code>null</code> is returned.
   *
   * <p>
   *
   ...
   */
  public @RUntainted Locale getFirstMatchingLocale(List<Locale> locales, List<Locale> available) {
  ...
  /**
   * Returns the the appropriate locale/encoding for a request, using the "right" locale handler for
   * the given resource.
   *
   * <p>Certain system folders (like the Workplace) require a special locale handler different from
   * the configured handler. Use this method if you want to resolve locales exactly like the system
   * does for a request.
   *
   * <p>
   *
  ...
  public CmsI18nInfo getI18nInfo(
      @RUntainted HttpServletRequest req, CmsUser user, CmsProject project, @RUntainted String resource) {
    ...
    if (req != null) {
      @RUntainted String localeParam = req.getParameter(CmsLocaleManager.PARAMETER_LOCALE);
   ...
   *
   * <p>This handler is used to derive the appropriate locale/encoding for a request.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this locale manager is fully initialized.
   *
   * <p>This is required to prevent errors during unit tests, simple unit tests will usually not
   * have a fully initialized locale manager available.
   *
   * <p>
   *
   ...
   */
  public void setLocaleHandler(@RUntainted I_CmsLocaleHandler localeHandler) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_I18N_CONFIG_LOC_HANDLER_1, m_localeHandler.getClass().getName()));
    }
  }
   ...
   *
   * <p>If the name can not be resolved as time zone ID, then "GMT" is used.
   *
   * <p>
   *
  ...
  /**
   * Returns true if the 'copy page' dialog should reuse elements in auto mode when copying to a
   * different locale.
   *
   * <p>
   *
  ...
  public boolean shouldReuseElements() {
    ...
    boolean isFalseInConfig =
        Boolean.FALSE.toString().equalsIgnoreCase(StringUtils.trim(m_reuseElementsStr));
    return !isFalseInConfig;
  }

  /**
   * Returns a list of available locale names derived from the given locale names.
   *
   * <p>Each name in the given list is checked against the internal hash map of allowed locales, and
   * is appended to the resulting list only if the locale exists.
   *
   * <p>
   *
  ...
  /**
   * Clears the caches in the locale manager.
   *
   * <p>
   */
  private void clearCaches() {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_LOCALE_MANAGER_FLUSH_CACHE_1, "EVENT_CLEAR_CACHES"));
    }
  }

  /**
   * Internal helper, returns an array of default locales for the given default names.
   *
   * <p>If required returns the system configured default locales.
   *
   * <p>
   *
   * @param defaultNames the default locales to use, can be <code>null</code> or a comma separated
   *     list of locales, for example <code>"en, de"</code>
   * @return an array of default locales for the given default names
   */
  private List<@RUntainted Locale> getDefaultLocales(String defaultNames) {
  ...
  /**
   * Initializes the language detection.
   *
   * <p>
   */
        ...
        if (!languagesAdded.contains(lang)) {
          ...
          if (is != null) {
            ...
            LOG.warn(
                Messages.get()
                    .getBundle()
                    .key(Messages.INIT_I18N_LAND_DETECT_PROFILE_NOT_AVAILABLE_1, locale));
          }
        }
      } catch (Exception e) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_I18N_LAND_DETECT_LOADING_PROFILE_FAILED_1, locale),
            e);
      }
    }
    return profiles;
