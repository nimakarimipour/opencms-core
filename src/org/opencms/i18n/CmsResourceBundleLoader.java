 ...
 * cache.
 *
 * <p>The main reason for implementing this is that the Java default resource bundle loading
 * mechanism provided by {@link java.util.ResourceBundle#getBundle(java.lang.String,
 * java.util.Locale)} uses a cache that can NOT be flushed by any standard means. This means for
 * every simple change in a resource bundle, the Java VM (and the webapp container that runs
 * OpenCms) must be restarted. This non-standard resource bundle loader avoids this by providing a
 * flushable cache.
 *
 * <p>In case the requested bundle can not be found, a fallback mechanism to {@link
 * java.util.ResourceBundle#getBundle(java.lang.String, java.util.Locale)} is used to look up the
 * resource bundle with the Java default resource bundle loading mechanism.
 *
 * <p>
 *
...
public final class CmsResourceBundleLoader {
   ...
   *
   * <p>Resource bundles are keyed by the combination of bundle name, locale, and class loader.
   */
  private static class BundleKey {
    ...
    public boolean equals(Object o) {
      ...
      return (m_hashcode == key.m_hashcode)
          && m_baseName.equals(key.m_baseName)
          && m_locale.equals(key.m_locale);
    }
     ...
     * @return <code>true</code> if the given base name is identical to the base name of this bundle
     *     key
     */
  ...
  public static void flushBundleCache() {
    ...
    // Reason: It's not 100% clear if the cache would be filled correctly from the XML after a
    // flush.
    // For example if a reference to an XML content object is held, than after a clear cache, this
    // object would not have a working localization since the schema and handler would not be
    // initialized again.
    // For XML contents that are unmarshalled after the clear cache the localization would work, but
    // it
    // seems likely that old references are held.
    // On the other hand, if something is changed in the XML, the cache is updated anyway, so we
    // won't be
   ...
   *
   * <p>A sequence of candidate bundle names are generated, and tested in this order, where the
   * suffix 1 means the string from the specified locale, and the suffix 2 means the string from the
   * default locale:
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>In the sequence, entries with an empty string are ignored. Next, <code>getBundle</code>
   * tries to instantiate the resource bundle:
   *
   * <ul>
   *   <li>This implementation only resolves property based resource bundles. Class based resource
   *       bundles are nor found.
   *   <li>A search is made for a property resource file, by replacing '.' with '/' and appending
   *       ".properties", and using ClassLoader.getResource(). If a file is found, then a
   *       PropertyResourceBundle is created from the file's contents.
   * </ul>
   *
   * <p>If no resource bundle was found, the default resource bundle loader is used to look for the
   * resource bundle. Class based resource bundles will be found now.
   *
   * <p>
   *
  ...
  /**
   * Tries to load a the bundle for a given locale, also loads the backup locales with the same
   * language.
   *
   * @param baseName the raw bundle name, without locale qualifiers
   * @param locale the locale
