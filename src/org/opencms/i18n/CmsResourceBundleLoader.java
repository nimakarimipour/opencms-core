...
public final class CmsResourceBundleLoader {
     ...
     */
    private static @RUntainted I_CmsResourceBundle tryBundle(@RUntainted String localizedName) {

        @RUntainted I_CmsResourceBundle result = null;

        try {

            @RUntainted String resourceName = localizedName.replace('.', '/') + ".properties";
            ...
            if (additionalBundle != null) {
                ...
                InputStream is = null;
                @RUntainted String path = CmsFileUtil.normalizePath(url);
