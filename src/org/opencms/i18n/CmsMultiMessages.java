 ...
 *
 * <p>Messages are cached for faster lookup. If a localized key is contained in more then one
 * resource bundle, it will be used only from the resource bundle where it was first found in. The
 * resource bundle order is undefined. It is therefore recommended to ensure the uniqueness of all
 * module keys by placing a special prefix in front of all keys of a resource bundle.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsMultiMessages extends CmsMessages {
  ...
  /**
   * For successfully found message keys, indicates the position in the bundle list where they were
   * last found.
   */
   ...
   *
   * <p>The added bundle will be localized with the locale of this multi message bundle.
   *
   * <p>
   *
   ...
   *
   * <p>The messages instance should have been initialized with the same locale as this multi
   * bundle, if not, the locale of the messages instance is automatically replaced. However, this
   * will not work if the added messages instance is in face also of type <code>
   * {@link CmsMultiMessages}</code>.
   *
   * <p>
   *
   * @param messages the messages instance to add
   * @throws CmsIllegalArgumentException if the locale of the given <code>{@link CmsMultiMessages}
   *     </code> does not match the locale of this multi messages
   */
  ...
  public void addMessages(List<CmsMessages> messages) {
    ...
    if (messages == null) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_MULTIMSG_EMPTY_LIST_0));
    }
   ...
   *
   * <p>If the key was not found, <code>null</code> is returned.
   *
   * <p>
   *
  ...
  private @RUntainted String resolveKey(@RUntainted String keyName) {
    ...
    // This may not be successful, because VFS-based bundles are mutable and keys could have been
    // removed.
    if (indexHint != null) {
    ...
    if (result == null) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_MESSAGE_KEY_FOUND_CACHED_2, keyName, result));
      }
      return result;
    }
    if (result == null) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_MESSAGE_KEY_FOUND_2, keyName, result));
      }
      if (!noCache) {
        // cache the result
  ...
  /**
   * Resolves a message key, using the key fallback handler if it is set.
   *
   * <p>
   *
