...
public class CmsVfsBundleManager implements I_CmsEventListener {
  ...
  public CmsVfsBundleManager(CmsObject cms) {
    ...
    OpenCms.getEventManager()
        .addCmsEventListener(
            this,
            new int[] {
              I_CmsEventListener.EVENT_PUBLISH_PROJECT, I_CmsEventListener.EVENT_CLEAR_CACHES
            });
  ...
  public static NameAndLocale getNameAndLocale(CmsResource bundleRes) {
    ...
    if (TYPE_PROPERTIES_BUNDLE.equals(
      ...
      if (localeSuffix == null) {
        ...
        String base =
            fileName.substring(
                0, fileName.lastIndexOf(localeSuffix) - (1 /* cut off trailing underscore, too*/));
  ...
  public synchronized void reload(boolean isStartup) {
    ...
    if ((OpenCms.getRunLevel() > OpenCms.RUNLEVEL_1_CORE_OBJECT)
      ...
      try {
        I_CmsResourceType propType =
            OpenCms.getResourceManager().getResourceType(TYPE_PROPERTIES_BUNDLE);
        propertyBundles =
            m_cms.readResources("/", CmsResourceFilter.ALL.addRequireType(propType), true);
      } catch (Exception e) {
        logError(e, isStartup);
      }
      try {
   ...
   *
   * <p>This will cause the internal reloading Thread not reload in case it is still running.
   *
   * <p>
   */
   ...
   * @param logToErrorChannel if true erros should be written to the error channel instead of the
   *     info channel
   */
  ...
  private void addXmlBundle(CmsResource xmlBundle) {
    ...
    LOG.info(
        String.format(
            "Adding property VFS bundle (path=%s, name=%s)", xmlBundle.getRootPath(), name));
    for (Locale locale : getAllLocales()) {
      CmsVfsBundleParameters params =
          new CmsVfsBundleParameters(name, path, locale, false, CmsVfsResourceBundle.TYPE_XML);
      CmsVfsResourceBundle bundle = new CmsVfsResourceBundle(params);
  ...
  private void scheduleReload() {
    ...
    if (!isReloadScheduled() && (OpenCms.getRunLevel() > OpenCms.RUNLEVEL_1_CORE_OBJECT)) {
      ...
      m_threadCount++;
      Thread thread =
          new Thread("Bundle reload Thread " + m_threadCount) {
