 ...
 *
 * <p>The methods in this class are substitutes for <code>java.net.URLEncoder.encode()</code> and
 * <code>java.net.URLDecoder.decode()</code>. Use the methods from this class in all OpenCms core
 * classes to ensure the encoding is always handled the same way.
 *
 * <p>The de- and encoding uses the same coding mechanism as JavaScript, special characters are
 * replaced with <code>%hex</code> where hex is a two digit hex number.
 *
 * <p><b>Note:</b> On the client side (browser) instead of using the deprecated <code>escape</code>
 * and <code>unescape</code> JavaScript functions, always the use <code>encodeURIComponent</code>
 * and <code>decodeURIComponent</code> functions. Only these work properly with unicode characters.
 *
 * <p>
 *
 * @since 6.0.0
...
public final class CmsEncoder {
  ...
  /**
   * Characters used as replacements for non-alphanumeric Base64 characters when using Base64 for
   * request parameters.
   */
   ...
   *
   * <p>Default encoding for JavaScript decodeUriComponent methods is <code>UTF-8</code> by w3c
   * standard.
   */
  ...
  private static Map<String, @RUntainted String> m_encodingCache = new HashMap<String, String>(16);
  ...
  /**
   * Adjusts the given String by making sure all characters that can be displayed in the given
   * charset are contained as chars, whereas all other non-displayable characters are converted to
   * HTML entities.
   *
   * <p>Just calls {@link #decodeHtmlEntities(String)} first and feeds the result to {@link
   ...
   *
   * <p>This is needed when we want to do redirects to hosts with host names containing
   * international characters like umlauts.
   *
   * <p>
   *
  ...
  /**
   * Creates a String out of a byte array with the specified encoding, falling back to the system
   * default in case the encoding name is not valid.
   *
   * <p>Use this method as a replacement for <code>new String(byte[], encoding)</code> to avoid
   ...
   */
  public static @RUntainted String createString(@RUntainted byte[] bytes, @RUntainted String encoding) {
  ...
  /**
   * Decodes a String using UTF-8 encoding, which is the standard for http data transmission with
   * GET ant POST requests.
   *
   * <p>
   *
  ...
  /**
   * This method is a substitute for <code>URLDecoder.decode()</code>. Use this in all OpenCms core
   * classes to ensure the encoding is always handled the same way.
   *
   * <p>In case you don't know what encoding to use, set the value of the <code>encoding</code>
   * parameter to <code>null</code>. This method will then default to UTF-8 encoding, which is
   ...
   */
  public static @RUntainted String decode(@RUntainted String source, @RUntainted String encoding) {
  ...
  /**
   * Decodes HTML entity references like <code>&amp;#8364;</code> that are contained in the String
   * to a regular character, but only if that character is contained in the given encodings charset.
   *
   * <p>
   *
   * @param input the input to decode the HTML entities in
  ...
  /**
   * Decodes a string used as parameter in an uri in a way independent of other encodings/decodings
   * applied before.
   *
   * <p>
   *
   ...
   */
  public static @RUntainted String decodeParameter(@RUntainted String input) {
  ...
  /**
   * Decodes a parameter which has been encoded from a string list using
  ...
  /**
   * Encodes a String using UTF-8 encoding, which is the standard for http data transmission with
   * GET ant POST requests.
   *
   * <p>
   *
   ...
   */
  public static String encode(@RUntainted String source) {

    return encode(source, ENCODING_UTF_8);
  }

  /**
   * This method is a substitute for <code>URLEncoder.encode()</code>. Use this in all OpenCms core
   * classes to ensure the encoding is always handled the same way.
   *
   * <p>In case you don't know what encoding to use, set the value of the <code>encoding</code>
   * parameter to <code>null</code>. This method will then default to UTF-8 encoding, which is
   ...
   */
  public static @RUntainted String encode(@RUntainted String source, @RUntainted String encoding) {
  ...
  /**
   * Encodes all characters that are contained in the String which can not displayed in the given
   * encodings charset with HTML entity references like <code>&amp;#8364;</code>.
   *
   * <p>This is required since a Java String is internally always stored as Unicode, meaning it can
   * contain almost every character, but the HTML charset used might not support all such
   * characters.
   *
   * <p>
   *
   ...
   */
  public static @RUntainted String encodeHtmlEntities(String input, String encoding) {
  ...
  /**
   * Encodes all characters that are contained in the String which can not displayed in the given
   * encodings charset with Java escaping like <code>\u20ac</code>.
   *
   * <p>This can be used to escape values used in Java property files.
   *
   * <p>
   *
  ...
  /**
   * Encodes a string used as parameter in an uri in a way independent of other encodings/decodings
   * applied later.
   *
   * <p>Used to ensure that GET parameters are not wrecked by wrong or incompatible configuration
   * settings. In order to ensure this, the String is first encoded with html entities for any
   * character that cannot encoded in US-ASCII; additionally, the plus sign is also encoded to avoid
   * problems with the white-space replacer. Finally, the entity prefix is replaced with characters
   * not used as delimiters in urls.
   *
   * <p>
   *
  ...
  /**
   * Encodes a String in a way similar to the JavaScript "encodeURIcomponent" function, using
   * "UTF-8" for character encoding encoding.
   *
   * <p>JavaScript "decodeURIcomponent" can decode Strings that have been encoded using this method.
   *
   ...
   * @return The encoded string
   */
  public static String escape(@RUntainted String source, @RUntainted String encoding) {
  ...
  /**
   * Escapes special characters in a HTML-String with their number-based entity representation, for
   * example &amp; becomes &amp;#38;.
   *
   * <p>A character <code>num</code> is replaced if<br>
  ...
  /**
   * Escapes non ASCII characters in a HTML-String with their number-based entity representation,
   * for example &amp; becomes &amp;#38;.
   *
   * <p>A character <code>num</code> is replaced if<br>
   * <code>(ch > 255)</code>
   ...
   *
   * <p>Replaces all single quotes to double single quotes in the value parameter of the SQL
   * statement.
   *
   * <p>
   *
  ...
  /**
   * Escapes the wildcard characters in a string which will be used as the pattern for a SQL LIKE
   * clause.
   *
   * <p>
   *
   ...
   *
   * <p>Multiple blanks are encoded _multiply_ with <code>%20</code>.
   *
   * <p>
   *
   * @param source The text to be encoded
   ...
   * @return The encoded String
   */
  public static String escapeWBlanks(@RUntainted String source, @RUntainted String encoding) {
  ...
  /**
   * Escapes a String so it may be printed as text content or attribute value in a HTML page or an
   * XML file.
   *
   * <p>This method replaces the following characters in a String:
   *
   * <ul>
   *   <li><b>&lt;</b> with &amp;lt;
   *   <li><b>&gt;</b> with &amp;gt;
   ...
   */
  public static @RUntainted String escapeXml(String source) {
   ...
   *
   * @param source the string to escape
   * @param doubleEscape if <code>false</code>, all entities that already are escaped are left
   *     untouched
   * @return the escaped string
   * @see #escapeHtml(String)
   */
  public static @RUntainted String escapeXml(String source, boolean doubleEscape) {
  ...
  /**
   * Checks if a given encoding name is actually supported, and if so resolves it to it's canonical
   * name, if not it returns the given fallback value.
   *
   * <p>Charsets have a set of aliases. For example, valid aliases for "UTF-8" are "UTF8", "utf-8"
   * or "utf8". This method resolves any given valid charset name to it's "canonical" form, so that
   * simple String comparison can be used when checking charset names internally later.
   *
   * <p>Please see <a
   ...
   *
   * <p>This is an equivalent to the JavaScript "decodeURIComponent" function. It converts from the
   * default "UTF-8" to the currently selected system encoding.
   *
   * <p>
   *
  ...
  public static String redecodeUriComponent(String input) {
    ...
    return new String(
        changeEncoding(
            input.getBytes(), ENCODING_UTF_8, OpenCms.getSystemInfo().getDefaultEncoding()));
  }

  /**
   * Decodes a String in a way similar to the JavaScript "decodeURIcomponent" function, using
   * "UTF-8" for character encoding.
   *
   * <p>This method can decode Strings that have been encoded in JavaScript with
   * "encodeURIcomponent".
   *
   * <p><b>Directly exposed for JSP EL<b>, not through {@link
   * org.opencms.jsp.util.CmsJspElFunctions}.
   ...
   *
   * <p>This method can decode Strings that have been encoded in JavaScript with
   * "encodeURIcomponent", provided "UTF-8" is used as encoding.
   *
   * <p><b>Directly exposed for JSP EL<b>, not through {@link
   * org.opencms.jsp.util.CmsJspElFunctions}.
   *
   ...
   * @return The decoded String
   */
  public static String unescape(String source, @RUntainted String encoding) {
