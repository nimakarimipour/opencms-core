...
public class CmsDefaultLocaleHandler implements I_CmsLocaleHandler {
  ...
  public CmsI18nInfo getI18nInfo(
    ...
    if (adminCms != null) {
      ...
      if (res != null) {
        // the resource may not exist at all (e.g. if an unknown resource was requested by the user
        // in the browser)
        try {
          defaultNames =
              adminCms
                  .readPropertyObject(res, CmsPropertyDefinition.PROPERTY_LOCALE, true)
                  .getValue();
        } catch (CmsException e) {
          LOG.warn(
              Messages.get().getBundle().key(Messages.ERR_READ_ENCODING_PROP_1, resourceName), e);
        }
    ...
    if (req != null) {
      try {
        ...
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_UNSUPPORTED_REQUEST_ENCODING_1, encoding),
            e);
      }
    }
