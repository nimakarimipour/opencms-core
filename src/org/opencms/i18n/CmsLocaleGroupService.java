 ...
 *
 * <p>A locale group is a construct used to group pages which are translations of each other. * A
 * locale group consists of a set of resources connected by relations in the following way:
 *
 * <p>
 *
 * <ul>
 ...
 * </ul>
 *
 * The point of the primary resource is to act as a 'master' resource which translators then use to
 * translate to different locales.
 */
public class CmsLocaleGroupService {
  ...
  public static List<Locale> getPossibleLocales(CmsObject cms, CmsResource currentResource) {
    ...
    try {
      CmsProperty secondaryLocaleProp =
          cms.readPropertyObject(
              currentResource, CmsPropertyDefinition.PROPERTY_SECONDARY_LOCALES, true);
   ...
   *
   * <p>Note: This is a low level method that is hard to use correctly. Please use
   * attachLocaleGroupIndirect if at all possible.
   *
   * @param secondaryPage the page to add
   * @param primaryPage the primary resource of the locale group which the resource should be added
   *     to
   * @throws CmsException if something goes wrong
   */
  public void attachLocaleGroup(CmsResource secondaryPage, CmsResource primaryPage)
      throws CmsException {

    if (secondaryPage.getStructureId().equals(primaryPage.getStructureId())) {
      throw new IllegalArgumentException(
          "A page can not be linked with itself as a locale variant: "
              + secondaryPage.getRootPath());
    }
    ...
    try {
      m_cms.deleteRelationsFromResource(
          secondaryPage, CmsRelationFilter.ALL.filterType(CmsRelationType.LOCALE_VARIANT));
   ...
   *
   * <p>Exactly one of the resources given as an argument must represent a locale group, while the
   * other should be the locale that you wish to attach to the locale group.
   *
   * <p>
   *
  ...
  public void attachLocaleGroupIndirect(CmsResource first, CmsResource second) throws CmsException {
    ...
    int numberOfRealGroups =
        (group1.isRealGroupOrPotentialGroupHead() ? 1 : 0)
   ...
   *
   * <p>This is the case if exactly one of the resources represents a locale group, the locale of
   * the other resource is not already present in the locale group, and if some other permission /
  ...
  public Status checkLinkable(CmsResource firstResource, CmsResource secondResource) {
    ...
    try {
      ...
      if ((mainLocale1 == null) || !(mainLocale1.equals(mainLocale2))) {
        LOG.debug(
            debugPrefix
                + " rejected - incompatible main locale "
                + mainLocale1
                + "/"
                + mainLocale2);
        return Status.other;
      ...
      if (group2
          .getPrimaryResource()
          .getStructureId()
          .equals(group1.getPrimaryResource().getStructureId())) {
  ...
  public void detachLocaleGroup(CmsResource firstPage, CmsResource secondPage) throws CmsException {
    ...
    List<CmsRelation> relations =
        m_cms.readRelations(typeFilter.filterStructureId(secondPage.getStructureId()));
   ...
   *
   * <p>This is used when we use locale group dialogs outside the sitemap editor, so we don't have a
   * clearly defined 'root resource' - this method is used to find a replacement for the root
   * resource which we would have in the sitemap editor.
   *
  ...
  public CmsLocaleGroup readDefaultFileLocaleGroup(CmsResource resource) throws CmsException {
    ...
    if (resource.isFolder()) {
      CmsResource defaultFile =
          m_cms.readDefaultFile(resource, CmsResourceFilter.IGNORE_EXPIRATION);
      if (defaultFile != null) {
    ...
    List<CmsRelation> relations =
        m_cms.readRelations(
            CmsRelationFilter.ALL
                ...
                .filterStructureId(resource.getStructureId()));
   ...
   *
   * <p>If the resource given already is a file, it will be returned, otherwise the default file (or
   * null, if none exists) of the folder will be returned.
   *
