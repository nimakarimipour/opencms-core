...
public class CmsLocaleGroup {
   ...
   */
  public CmsLocaleGroup(
      CmsObject cms, CmsResource primaryResource, List<CmsResource> secondaryResources) {
   ...
   *
   * <p>If the locale group contains more than one resource from the same locale,, which one is used
   * a map value is undefined.
   *
  ...
  public boolean isMarkedNoTranslation(Locale locale) {
    ...
    return (m_noTranslation != null)
        && CmsLocaleManager.getLocales(m_noTranslation).contains(locale);
  }
  ...
  /**
   * Returns true if this is a potential group head, i.e. the locale of the primary resource is the
   * main translation locale configured for the site in which it is located.
   *
   * <p>
   *
  ...
  /**
   * Checks if this is either a real group or a potential group head (i.e. a potential primary
   * resource).
   *
   * <p>
   *
    ...
    try {
      CmsProperty noTranslationProp =
          m_cms.readPropertyObject(
              m_primaryResource, CmsPropertyDefinition.PROPERTY_LOCALE_NOTRANSLATION, false);
