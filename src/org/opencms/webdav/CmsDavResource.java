 ...
 * resource, since DavResource are also created for the target locations for move/copy operations,
 * before any of the moving / copying happens.
 */
public class CmsDavResource implements DavResource {
  ...
  /**
   * Lazily initialized repository item - null means not initialized, Optional.empty means tried to
   * load resource, but it was not found.
   */
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#addLockManager(org.apache.jackrabbit.webdav.lock.LockManager)
   */
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#addMember(org.apache.jackrabbit.webdav.DavResource,
    ...
    for (PropEntry entry : changeList) {
      if (entry instanceof DefaultDavProperty<?>) {
        ...
        CmsPropertyName cmsPropName =
            new CmsPropertyName(prop.getName().getNamespace().getURI(), prop.getName().getName());
  ...
  /**
   * @see org.apache.jackrabbit.webdav.DavResource#copy(org.apache.jackrabbit.webdav.DavResource,
   *     boolean)
   */
  ...
  public String getComplianceClass() {
    ...
    return DavCompliance.concatComplianceClasses(
        new String[] {DavCompliance._1_, DavCompliance._2_});
  }
  ...
  public DavResourceIterator getMembers() {
    ...
    try {
      ...
      List<DavResource> childDavRes =
          children.stream()
              .map(
                  child -> {
                    String childPath =
                        CmsStringUtil.joinPaths(m_locator.getWorkspacePath(), child.getName());
                    DavResourceLocator childLocator =
                        m_locator
                            .getFactory()
                            .createResourceLocator(
                                m_locator.getPrefix(), m_locator.getWorkspacePath(), childPath);
              ...
              .filter(
                  child -> {
                    ...
                    if (!exists) {
                      // one case where this happens is when the child resource has a name that
                      // would be
  ...
  public DavPropertySet getProperties() {
    ...
    ResourceType typeProp =
        new ResourceType(isCollection() ? ResourceType.COLLECTION : ResourceType.DEFAULT_RESOURCE);
    result.add(typeProp);
    result.add(
    ...
    result.add(
        new DefaultDavProperty<String>(DavPropertyName.DISPLAYNAME, "" + getItem().getName()));
    if (!isCollection()) {
      result.add(
          new DefaultDavProperty<String>(
              DavPropertyName.GETCONTENTLENGTH, "" + getItem().getContentLength()));
      ...
      for (Map.Entry<CmsPropertyName, String> entry : cmsProps.entrySet()) {
        ...
        DavPropertyName name =
            DavPropertyName.create(
                propName.getName(), Namespace.getNamespace(propName.getNamespace()));
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#getProperty(org.apache.jackrabbit.webdav.property.DavPropertyName)
   */
  ...
  public String getSupportedMethods() {
    ...
    if (item == null) {
      methods.addAll(
          Arrays.asList(DavMethods.METHOD_OPTIONS, DavMethods.METHOD_PUT, DavMethods.METHOD_MKCOL));
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#isLockable(org.apache.jackrabbit.webdav.lock.Type,
   *     org.apache.jackrabbit.webdav.lock.Scope)
   */
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#refreshLock(org.apache.jackrabbit.webdav.lock.LockInfo,
   *     java.lang.String)
   */
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#removeMember(org.apache.jackrabbit.webdav.DavResource)
   */
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#removeProperty(org.apache.jackrabbit.webdav.property.DavPropertyName)
   */
  public void removeProperty(DavPropertyName propertyName) throws DavException {
    ...
    CmsPropertyName key =
        new CmsPropertyName(propertyName.getNamespace().getURI(), propertyName.getName());
    props.put(key, "");
    try {
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#setProperty(org.apache.jackrabbit.webdav.property.DavProperty)
   */
  ...
  /**
   * @see
   *     org.apache.jackrabbit.webdav.DavResource#spool(org.apache.jackrabbit.webdav.io.OutputContext)
   */
