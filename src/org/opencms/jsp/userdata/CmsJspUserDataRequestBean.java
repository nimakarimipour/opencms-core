 ...
 *
 * <p>The dynamic function calls the action() method on this bean, which returns the new state for
 * the JSP to render. Error/status messages are also available to the JSP via getters.
 */
public class CmsJspUserDataRequestBean {
  ...
  enum State {
    ...
    /**
     * State showing the data request form with user / email / password fields, and possibly an
     * error message.
     */
    form,
    ...
    /**
     * State which is shown when the user confirms their credentials after clicking on the email
     * link.
     */
    viewOk
  }
   ...
   *
   * <p>Returns the next state which should be shown by the JSP.
   *
  ...
  public String action(CmsObject cms, Map<String, String[]> reqParameters) throws CmsException {
    ...
    CmsProperty defaultOuProp =
        CmsProperty.get(CmsPropertyDefinition.PROPERTY_UDR_DEFAULTOU, props);
    ...
    String functionDetail =
        CmsJspStandardContextBean.getFunctionDetailLink(
            cms, CmsDetailPageInfo.FUNCTION_PREFIX, CmsUserDataRequestManager.FUNCTION_NAME, true);
        ...
        m_downloadLink =
            OpenCms.getLinkManager()
                .substituteLinkForUnknownTarget(
                    cms,
                    CmsUserDataResourceHandler.PREFIX
                        + info.getId()
                        + "?"
                        + PARAM_AUTH
                        + "="
  ...
  /**
   * Gets the lazy map used to access the configurable texts to display by the user data request
   * dynamic function.
   *
   * @return the lazy map
   */
   ...
   *
   * <p>The 'best' in this case is found by walking up the OUs, from the most specific to the root
   * OU, which contain the current URI and trying to log in the user with the given name and OU in
   * each of them. The first OU+User combination for which the login is successful is returned.
   *
   * @param cms the CMS context
   ...
   * @throws CmsException if something goes wrong
   */
  private Optional<CmsUser> lookupUser(
      CmsObject cms, String configuredOu, String path, String user, String password)
      throws CmsException {
    ...
    if (configuredOu != null) {
      ous =
          Collections.singletonList(
              OpenCms.getOrgUnitManager().readOrganizationalUnit(cms, configuredOu));
    } else {
