 ...
 *
 * <p>Download links have the form .../userdatarequest/[id]?auth=[authcode] . If the stored user
 * data request with the given id exists, and its auth code matches the auth code given as a
 * parameter, the download for the user data will be started, otherwise HTTP 404 will be returned.
 */
public class CmsUserDataResourceHandler implements I_CmsResourceInit {
  ...
  public CmsResource initResource(
    ...
    CmsUserDataRequestInfo requestInfo =
        OpenCms.getUserDataRequestManager().getRequestStore().load(infoStr).orElse(null);
    if (requestInfo == null) {
      return null;
    }
