 ...
 *
 * <p>A user data domain can provide information about users which have requested their user data.
 ...
 * </ul>
 *
 * In the first case, more than one user can match the email address, in the second case at most one
 * user is found. For each of the users, they will be matched against each of the user data domains
 * (a user can match more than one domain). Then the domain produces the user data for the list of
 * matching users by appending it to an HTML document.
 */
public interface I_CmsUserDataDomain extends I_CmsConfigurationParameterHandler {
   ...
   */
  void appendInfoHtml(
      CmsObject cms, CmsUserDataRequestType reqType, List<CmsUser> user, Element element);

  /**
   * Writes additional information related to an email address which is not directly associated with
   * a specific OpenCms user.
   *
   * @param cms the CMS context
   ...
   */
  default void appendlInfoForEmail(
      CmsObject cms, String email, List<String> searchStrings, Element element) {
