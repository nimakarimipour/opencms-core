 ...
 *
 * <p>Users can request their data either via username/password, in which case the user data will be
 * collected for that user, or by email, in which case the data for all users with the given email
 * address is collected.
 *
 * <p>User data is formatted as HTML by one or more configurable 'user data domain' classes, which
 * implement the I_CmsUserDataDomain interface.
 *
 * <p>After the user requests their data, they are sent an email with a link (which is only valid
 * for a certain time). When opening that link, they have to confirm the credentials they requested
 * the data with, and if they successfully do so, are shown a page with their user data.
 */
public class CmsUserDataRequestManager {
  ...
  /**
   * List of all domains, both the ones from the configuration and the ones loaded via service
   * loader.
   */
  ...
  public static void addDigesterRules(Digester digester, String basePath) {
    ...
    // use pre-existing rules for params in system configuration to set the parameters for the user
    // domain
  }
  ...
  /**
   * Gets the list of all user data domains, both those from the configuration and those loaded via
   * service loader.
   *
  ...
  public Optional<CmsUserDataRequestConfig> loadConfig(CmsObject cms, String path) {
    ...
    try {
      ...
      CmsUserDataRequestConfig result =
          new CmsUserDataRequestConfig(cms, content, cms.getRequestContext().getLocale());
    ...
    for (I_CmsUserDataDomain userDomain : getAllDomains()) {
      if (userDomain.matchesUser(cms, CmsUserDataRequestType.singleUser, user)) {
        userDomain.appendInfoHtml(
            cms, CmsUserDataRequestType.singleUser, Collections.singletonList(user), root);
      }
    }
  ...
  public void startUserDataRequest(CmsObject cms, CmsUserDataRequestConfig config, String email)
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(email)) {
      throw new IllegalArgumentException(
          "Can not use empty email address for user data request by email.");
    }
    try {
  ...
  public String toString() {
    ...
    String collectorsStr =
        getAllDomains().stream().map(domain -> domain.toString()).collect(Collectors.joining(", "));
  ...
  private boolean internalGetInfoForEmail(
    ...
    for (I_CmsUserDataDomain userDomain : getAllDomains()) {
      ...
      report.print(
          Messages.get()
              .container(Messages.RPT_USERDATADOMAIN_COUNT_2, "" + i, "" + getAllDomains().size()));
      report.print(
  ...
  private List<I_CmsUserDataDomain> loadUserDataDomainsFromClasses() {
    ...
    ServiceLoader<I_CmsUserDataDomainProvider> loader =
        ServiceLoader.load(I_CmsUserDataDomainProvider.class);
  ...
  private void sendMail(CmsObject cms, CmsUserDataRequestConfig config, String email, String id)
    ...
    String link =
        CmsJspStandardContextBean.getFunctionDetailLink(
                cms, CmsDetailPageInfo.FUNCTION_PREFIX, FUNCTION_NAME, true)
            + "?"
