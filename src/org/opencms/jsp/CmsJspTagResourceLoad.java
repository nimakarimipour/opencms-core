...
/**
 * Implementation of the <code>&lt;cms:resourceload/&gt;</code> tag, used to access and display
 * resource information from the VFS.
 *
 * <p>
 *
 * @since 8.0
 */
public class CmsJspTagResourceLoad extends CmsJspScopedVarBodyTagSuport
  ...
  /**
   * Constructor used when using <code>resourceload</code> from scriptlet code.
   *
   * <p>
   *
  ...
  protected static List<CmsResource> limitCollectorResult(
      CmsContentInfoBean contentInfoBean, List<CmsResource> collectorResult) {
   ...
   *
   * <p>If this is set to <code>true</code>, then the collector result will only be preloaded, but
   * not iterated.
   *
   * <p>
   *
  ...
  protected void init(I_CmsResourceContainer container) throws JspException {
    ...
    if ((pageAttribCount > 0) && (pageAttribCount < 2)) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_TAG_RESOURCELOAD_INDEX_SIZE_0));
    }
