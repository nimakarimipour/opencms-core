 ...
 *
 * <p>This tag supports the following special "property" values:
 *
 * <ul>
 ...
 *   <li><code>opencms.request.uri</code> returns the name of the currently requested URI in the
 *       OpenCms VFS, e.g. <i>/index.jsp</i>.
 *   <li><code>opencms.request.element.uri</code> returns the name of the currently processed
 *       element, which might be a sub-element like a template part, in the OpenCms VFS, e.g.
 *       <i>/system/modules/org.opencms.welcome/jsptemplates/welcome.jsp</i>.
 ...
 *   <li><code>opencms.request.encoding</code> returns the content encoding that has been set for
 *       the currently requested resource, e.g. <i>ISO-8859-1</i>.
 *   <li><code>opencms.title</code> (since 8.0.0) returns the title of the document that should be
 *       used for the HTML title tag. This is useful for container detail pages, in which case it
 *       will return the Title of the detail, not the container page. Otherwise it just returns the
 *       value of the Title property.
 *   <li><code>opencms.description</code> (since 9.0.1)
 *   <li><code>opencms.keywords</code> (since 9.0.1)
 * </ul>
 *
 * All other property values that are passes to the tag as routed to a standard <code>
 * System.getProperty(value)</code> call, so you can also get information about the Java VM
 * environment, using values like <code>java.vm.version</code> or <code>os.name</code>.
 *
 * <p>If the given property value does not match a key from the special OpenCms values and also not
 * the system values, a (String) message is returned with a formatted error message.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsJspTagInfo extends TagSupport {
  ...
  /**
   * Returns the description of a page delivered from OpenCms, usually used for the <code>
   * description</code> metatag of a HTML page.
   *
   * <p>If no description information has been found, the empty String "" is returned.
   *
   * <p>
   *
  ...
  public static String getDescriptionInfo(CmsFlexController controller, HttpServletRequest req) {
    ...
    try {
      ...
      if (contextBean.isDetailRequest()) {
        ...
        // read the description of the detail resource as fall back (may contain mapping from
        // another locale)
        result =
            cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_DESCRIPTION, false)
                .getValue();
      }
      if (result == null) {
  ...
  /**
   * Returns the keywords of a page delivered from OpenCms, usually used for the <code>keywords
   * </code> metatag of a HTML page.
   *
   * <p>If no description information has been found, the empty String "" is returned.
   *
   * <p>
  ...
  public static String getKeywordsInfo(CmsFlexController controller, HttpServletRequest req) {
    ...
    try {
      ...
      if (contextBean.isDetailRequest()) {
        // this is a request to a detail page
        CmsResource res = contextBean.getDetailContent();
        // read the keywords of the detail resource as fall back (may contain mapping from another
        // locale)
        result =
            cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_KEYWORDS, false).getValue();
      }
      if (result == null) {
        // read the title of the requested resource as fall back
        result =
            cms.readPropertyObject(
                    cms.getRequestContext().getUri(), CmsPropertyDefinition.PROPERTY_KEYWORDS, true)
                .getValue();
      }
    } catch (CmsException e) {
      // NOOP, result will be null
  ...
  public static String getTitleInfo(CmsFlexController controller, HttpServletRequest req) {
    ...
    try {
      ...
      if (contextBean.isDetailRequest()) {
        ...
        result =
            content.getHandler().getTitleMapping(cms, content, cms.getRequestContext().getLocale());
        if (result == null) {
      ...
      if (result == null) {
        ...
        result =
            cms.readPropertyObject(
                    cms.getRequestContext().getUri(), CmsPropertyDefinition.PROPERTY_TITLE, true)
                .getValue();
      }
    } catch (CmsException e) {
      // NOOP, result will be null
  ...
  public static String infoTagAction(@RUntainted String property, HttpServletRequest req) {
    ...
    if (property == null) {
      CmsMessageContainer errMsgContainer =
          Messages.get().container(Messages.GUI_ERR_INVALID_INFO_PROP_0);
    ...
    switch (SYSTEM_PROPERTIES_LIST.indexOf(property)) {
      ...
      case 7: // opencms.request.folder
        result =
            CmsResource.getParentFolder(controller.getCmsObject().getRequestContext().getUri());
        break;
      ...
      default:
        ...
        if (result == null) {
          CmsMessageContainer errMsgContainer =
              Messages.get().container(Messages.GUI_ERR_INVALID_INFO_PROP_1, property);
          return Messages.getLocalizedMessage(errMsgContainer, req);
        }
    }
