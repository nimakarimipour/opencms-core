 ...
 * allows to activate direct editing.
 *
 * <p>It is less flexible but simpler to use than the 'display' tag in that it only allows you to
 * specify a single, fixed formatter configuration as an attribute, rather than a set of
 * type-dependent formatters with the displayformatter tag.
 */
public class CmsJspTagSimpleDisplay extends BodyTagSupport implements I_CmsJspTagParamParent {
  ...
  /**
   * Gets the formatter bean to use, using either the formatter key or formatter path given as
   * attributes.
   *
   * <p>If both the formatter and formatterKey attributes are used, a formatter found for the key is
   * prioritized over the one found for the formatter, if it exists.
   *
   ...
   */
  private I_CmsFormatterBean getFormatterBean(CmsObject cms, CmsADEConfigData config)
      throws CmsException {
    ...
    CmsFormatterConfigurationCacheState formatterCache =
        OpenCms.getADEManager()
            .getCachedFormatters(cms.getRequestContext().getCurrentProject().isOnlineProject());
    I_CmsFormatterBean formatter =
        formatterCache.getFormatters().get(formatterResource.getStructureId());
    return formatter;
  }
