...
public class CmsJspTagEnableAde extends BodyTagSupport {
  ...
  public static void enableAdeTagAction(@RUntainted PageContext context) throws JspException {
    ...
    if (isDirectEditDisabled(req)) {
      try {
        ...
        Integer left =
            (Integer)
                ((HttpServletRequest) req)
                    .getSession()
                    .getAttribute(CmsGwtConstants.PARAM_BUTTON_LEFT);
        ...
        String titleMessage =
            Messages.get()
                ...
                .key(Messages.GUI_TOOLBAR_ENABLE_EDIT_MODE_0);
      ...
      try {
        CmsContainerpageActionElement actionEl =
            new CmsContainerpageActionElement(
                context, (HttpServletRequest) req, (HttpServletResponse) context.getResponse());
  ...
  public static boolean isDirectEditDisabled(ServletRequest request) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(
        request.getParameter(CmsGwtConstants.PARAM_TEMPLATE_CONTEXT))) {
      return true;
    }
  ...
  public static void updateDirectEditFlagInSession(@RUntainted ServletRequest request) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(disabledParam)) {
      if (Boolean.parseBoolean(disabledParam)) {
        ...
        if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(buttonLeft)) {
          ...
          try {
            ...
            if (left.intValue() > 0) {
              ((HttpServletRequest) request)
                  .getSession()
                  .setAttribute(CmsGwtConstants.PARAM_BUTTON_LEFT, left);
            }
        ...
        ((HttpServletRequest) request)
            .getSession()
            .removeAttribute(CmsGwtConstants.PARAM_DISABLE_DIRECT_EDIT);
      }
    }
  }

  /**
   * Returns the preview mode include.
   *
   * <p>
   *
  ...
  private static String getPreviewInclude(String buttonLeft, String titleMessage) {
    ...
    buffer
        ...
        .append(CmsGwtActionElement.getFontIconCssLink())
        .append("\"); </style>\n");
    String heartbeatUrl =
    ...
    buffer.append(
        "<script src=\""
            + CmsWorkplace.getStaticResourceUri("/ade/page-preview.js")
            + "\"></script>\n");
