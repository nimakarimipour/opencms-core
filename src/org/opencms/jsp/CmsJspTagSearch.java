...
public class CmsJspTagSearch extends CmsJspScopedVarBodyTagSuport
    implements I_CmsCollectorPublishListProvider {

  /**
   * Type for the file formats that can be parsed. The format is given via the tag's attribute
   * "fileFormat".
   */
  ...
  /**
   * Number of entries for which content info should be added to allow correct relations in "This
   * page" publish dialog.
   */
  ...
  /**
   * The search index that should be used . It will either be the configured index, or "Solr
   * Offline" / "Solr Online" depending on the project.
   */
  ...
  public static Set<CmsResource> getPublishResourcesInternal(
      CmsObject cms, I_CmsContentLoadCollectorInfo info) throws CmsException {

    CmsSolrIndex solrOnline =
        OpenCms.getSearchManager().getIndexSolr(CmsSolrIndex.DEFAULT_INDEX_NAME_ONLINE);
    CmsSolrIndex solrOffline =
    ...
    try {
      ...
      CmsSolrResultList offlineResults =
          solrOffline.search(
      ...
      CmsSolrResultList onlineResults =
          solrOnline.search(
  ...
  /**
   * Get the value of the specified format of the configuration file (given via the tag's
  ...
  protected void init() throws JspException {
    ...
    try {
      ...
      if (m_configFile != null) {
        CmsFile configFile =
            m_cms.readFile(CmsJspElFunctions.convertRawResource(m_cms, m_configFile));
        if (m_fileFormat == FileFormat.JSON) {
          ...
          config =
              new CmsSearchConfiguration(new CmsJSONSearchConfigurationParser(configString), m_cms);
        } else { // assume XML
          CmsXmlContent xmlContent = CmsXmlContentFactory.unmarshal(m_cms, configFile);
          config =
              new CmsSearchConfiguration(
                  new CmsXMLSearchConfigurationParser(
                      xmlContent, m_cms.getRequestContext().getLocale()),
                  m_cms);
        }
      }
      if (m_configString != null) {
        if (m_configString.trim().startsWith("{")) {
          config =
              new CmsSearchConfiguration(
                  new CmsJSONSearchConfigurationParser(m_configString, config), m_cms);
        } else {
          config =
              new CmsSearchConfiguration(
                  new CmsPlainQuerySearchConfigurationParser(m_configString, config), m_cms);
        }
      }
  ...
  /**
   * Here the search query is composed and executed. The result is wrapped in an easily usable form.
  ...
  private I_CmsSearchResultWrapper getSearchResults() {
    ...
    m_searchController.updateFromRequestParameters(
        pageContext.getRequest().getParameterMap(), false);
    ...
    if (common.getState().getQuery().isEmpty()
        && (!common.getConfig().getIgnoreQueryParam()
            && !common.getConfig().getSearchForEmptyQueryParam())) {
