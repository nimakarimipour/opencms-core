...
public class CmsSearchControllerCommon implements I_CmsSearchControllerCommon {
  ...
  public void addQueryParts(CmsSolrQuery query, CmsObject cms) {
    ...
    if (!m_config.getExtraSolrParams().isEmpty()) {
      ...
      String currentLocale =
          (null == cms ? CmsLocaleManager.getDefaultLocale() : cms.getRequestContext().getLocale())
              .toString();
      Map<String, String[]> extraParamsMap =
          CmsRequestUtil.createParameterMap(m_config.getExtraSolrParams(), true, null);
    ...
    for (String additionalParam : m_state.getAdditionalParameters().keySet()) {
      ...
      if (null != solrValue) {
        ...
        Map<String, String[]> extraParamsMap =
            CmsRequestUtil.createParameterMap(additionalParamString, true, null);
        for (String key : extraParamsMap.keySet()) {
          for (String value : Arrays.asList(extraParamsMap.get(key))) {
            if (SET_VARIABLES.contains(key)) {
              query.set(key, value);
  ...
  public void updateFromRequestParameters(
    ...
    Map<String, String> additionalParameters =
        new HashMap<String, String>(m_config.getAdditionalParameters().size());
  ...
  /**
   * Replaces the macro with the value, unless the value is <code>null</code>, then the macro is
   * kept.
   *
  ...
  private String resolveMacro(final String string, final String macroName, final String value) {
    ...
    return null != value
        ? string.replaceAll("\\%\\(" + Pattern.quote(macroName) + "\\)", value)
        : string;
  }
}
