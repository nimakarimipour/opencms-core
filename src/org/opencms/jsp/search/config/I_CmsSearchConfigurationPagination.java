...
public interface I_CmsSearchConfigurationPagination {
   ...
   *
   * <p>That means, if you specify the sizes [5,8], meaning the first page should have 5 entries,
   * all others 8, the size 8 is returned by the method.
   *
   * <p>To get the size of a specific page use {@link
   * I_CmsSearchConfigurationPagination#getSizeOfPage(int)}.
   *
   * @return The page size of pages that have not explicitely a size set.
   * @deprecated use either {@link I_CmsSearchConfigurationPagination#getSizeOfPage(int)} to get the
   *     size for a specific page or {@link I_CmsSearchControllerPagination#getCurrentPageSize()} to
  ...
  /**
   * Returns the page sizes as configured for the first pages of the search. The last provided size
   * is the size of all following pages.
   *
   ...
   *
   * @param pageNum the number of the page, for which the index of the first item to show on is
   *     requested.
