...
/**
 * Search configuration parser reading a configuration containing a plain Solr query. Only fl might
...
public class CmsPlainQuerySearchConfigurationParser implements I_CmsSearchConfigurationParser {
    ...
    if (null != resString) {
      try {
        ...
        if (LOG.isErrorEnabled()) {
          LOG.error(
              "Ignoring param \"maxresults=" + resString + "\" since its not a valid integer.", e);
        }
      }
    }
    if (null == maxResNum) {
   ...
   *
   * <p>For example, extractParam("a=foo&b=bar", "a") will return CmsPair("b=bar", "foo").
   *
   * @param params the parameter string.
   * @param paramKey the key of the parameter to extract the value for.
   * @return a pair of "params without the extracted parameter" and the value of the extracted
   *     parameter.
   */
