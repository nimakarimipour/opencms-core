...
public class CmsConfigurationBean {
   ...
   */
  public void addCategoryFolderFilter(
      CmsCategoryFolderRestrictionBean listCategoryFolderRestrictionBean) {
  ...
  /**
   * Returns the combined category-folder restrictions.
   *
   * <p>
   *
  ...
  /**
   * Returns the number of results to return maximally, or <code>null</code> if not explicitly
   * specified.
   *
   * @return the number of results to return maximally, or <code>null</code> if not explicitly
   *     specified.
   */
  public Integer getMaximallyReturnedResults() {

    String resString = m_parameterFields.get(PARAM_MAX_RESULTS);
    // Fallback, we first added the restriction as additional parameter. To make it more obvious, we
    // integrated it as extra field.
    // Only if the extra field is not set, we use the additional parameter to be backward
    // compatible.
    if (null == resString) {
   ...
   *
   * @return <code>true</code> iff there are preconfiugred restrictions, <code>false</code>
   *     otherwise.
   */
   ...
   * @return <code>true</code> iff there are preconfigured restrictions for the provided type,
   *     <code>false</code> otherwise.
   */
  public boolean hasTypeSpecificRestriction(String type) {

    return (null != m_preconfiguredRestrictions)
        && m_preconfiguredRestrictions.hasRestrictionForType(type);
  }
