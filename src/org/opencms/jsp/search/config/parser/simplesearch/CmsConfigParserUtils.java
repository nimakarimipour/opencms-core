...
public final class CmsConfigParserUtils {
  ...
  public static CmsConfigurationBean parseListConfiguration(CmsObject cms, CmsResource res) {
    ...
    try {
      ...
      if (restrictValue != null) {
        ...
        I_CmsDateRestriction restriction =
            parser.parse(new CmsXmlContentValueLocation(restrictValue));
        if (restriction == null) {
          LOG.warn(
      ...
      for (I_CmsXmlContentValue parameter : content.getValues(N_PARAMETER, locale)) {
        ...
        I_CmsXmlContentValue valueVal =
            content.getValue(parameter.getPath() + "/" + N_VALUE, locale);
        if ((keyVal != null)
      ...
      List<I_CmsXmlContentValue> categoryFolderRestrictions =
          content.getValues(N_CATEGORY_FOLDER_RESTRICTION, locale);
      if (!categoryFolderRestrictions.isEmpty()) {
        for (I_CmsXmlContentValue restriction : categoryFolderRestrictions) {
          ...
          List<I_CmsXmlContentValue> folderVals =
              content.getValues(CmsXmlUtils.concatXpath(restriction.getPath(), N_FOLDER), locale);
          ...
          I_CmsXmlContentValue categoryVal =
              content.getValue(CmsXmlUtils.concatXpath(restriction.getPath(), N_CATEGORY), locale);
            ...
            try {
              String path =
                  CmsCategoryService.getInstance()
                      .getCategory(cms, cms.getRequestContext().addSiteRoot(sitePath))
                      .getPath();
          ...
          String restrictionCategoryMode =
              content
                  .getValue(CmsXmlUtils.concatXpath(restriction.getPath(), N_CATEGORY_MODE), locale)
                  .getStringValue(cms);
          result.addCategoryFolderFilter(
              new CmsCategoryFolderRestrictionBean(
                  restrictionCategories,
                  restrictionFolders,
                  null == restrictionCategoryMode
                      ? null
                      : CombinationMode.valueOf(restrictionCategoryMode)));
        }
      }
      List<I_CmsXmlContentValue> preconfiguredRestrictions =
          content.getValues(N_PRECONFIGURED_FILTER_QUERY, locale);
      if (!preconfiguredRestrictions.isEmpty()) {
        ...
        for (I_CmsXmlContentValue restriction : preconfiguredRestrictions) {
          String restrictionRule =
              content
                  .getValue(CmsXmlUtils.concatXpath(restriction.getPath(), N_RULE), locale)
                  .getStringValue(cms);
          List<I_CmsXmlContentValue> restrictionVals =
              content.getValues(CmsXmlUtils.concatXpath(restriction.getPath(), N_VALUE), locale);
          List<String> restrictionValues =
              restrictionVals.stream().map(v -> v.getStringValue(cms)).collect(Collectors.toList());
  ...
  public static CmsXmlContent updateBlackList(
      CmsObject cms, CmsXmlContent content, CmsConfigurationBean configBean) {
  ...
  /**
   * Creates the parameter map, mapping from noed names to parameters of the list configuration
   * bean.
   *
