...
/**
 * A single restriction rule, telling for which field and type what kind of restriction should be
 * enforced.
 */
public class CmsRestrictionRule {
  ...
  /**
   * The combination mode for multiple values in different input fields, either AND or OR - match
   * all or only any of the values.
   */
  private CombinationMode m_combinationModeBetweenFields;
  /**
   * The combination mode for multiple values in one input field, either AND or OR - match all or
   * only any of the values.
   */
  private CombinationMode m_combinationModeInField;

  /**
   * Constructs a restriction rule for the provided field that has no type restriction and default
   * combination mode and match type.
   *
   ...
   * @param combinationModeInField the way how values in different fields in one field should be
   *     combined.
   */
  public CmsRestrictionRule(
    ...
    m_combinationModeBetweenFields =
        null == combinationModeBetweenFields ? CombinationMode.OR : combinationModeBetweenFields;
    m_combinationModeInField =
        (null == combinationModeInField) ? m_combinationModeBetweenFields : combinationModeInField;
  }
  ...
  /**
   * Returns the name of the solr field, the restrictions should be enforced on for, dependent on
   * the provided locale.
   *
   * @param l the search locale.
   * @return the name of the solr field, the restrictions should be enforced on for, dependent on
   *     the provided locale.
   */
  ...
  /**
   * Returns the name of the solr field, the restrictions should be enforced on, but without
   * resolving the locale placeholder.
   *
   * @return the name of the solr field, the restrictions should be enforced on, but without
   *     resolving the locale placeholder.
   */
   ...
   *
   * @return the resource type, the restriction should be enforced for. <code>null</code> is
   *     returned iff the restriction should hold for all resource types.
   */
