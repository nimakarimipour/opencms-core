...
public class CmsJSONSearchConfigurationParser implements I_CmsSearchConfigurationParser {
  ...
  /**
   * The JSON key for the default sort option, should hold the name paramvalue for the default
   * option.
   */
  ...
  /** A JSON key. */
  public static final String JSON_KEY_HIGHLIGHTER_MAX_LENGTH_ALTERNATE_FIELD =
  ...
  /** A JSON key. */
  public static final String JSON_KEY_HIGHLIGHTER_FASTVECTORHIGHLIGHTING =
  ...
  protected static List<String> parseMandatoryStringValues(JSONObject json, @RUntainted String key)
    ...
    for (int i = 0; i < array.length(); i++) {
      try {
        ...
        LOG.info(
            Messages.get().getBundle().key(Messages.LOG_OPTIONAL_STRING_ENTRY_UNPARSABLE_1, key),
            e);
      }
    }
    return list;
  ...
  /**
   * Helper for reading an optional String value list - returning <code>null</code> if parsing fails
   * for the whole list, otherwise just skipping unparsable entries.
   *
   * @param json The JSON object where the list should be read from.
   * @param key The key of the value to read.
   * @return The value from the JSON, or <code>null</code> if the value does not exist.
  ...
  public Map<String, I_CmsSearchConfigurationFacetField> parseFieldFacets() {
    ...
    Map<String, I_CmsSearchConfigurationFacetField> facetConfigs =
        new LinkedHashMap<String, I_CmsSearchConfigurationFacetField>();
    try {
  ...
  public I_CmsSearchConfigurationGeoFilter parseGeoFilter() {
    ...
    try {
      ...
      String coordinatesParam =
          parseOptionalStringValue(geoFilter, JSON_KEY_GEO_FILTER_COORDINATES_PARAM);
      ...
      return new CmsSearchConfigurationGeoFilter(
          coordinates, coordinatesParam, fieldName, radius, radiusParam, units, unitsParam);
    } catch (JSONException e) {
      if (null == m_baseConfig) {
        if (LOG.isInfoEnabled()) {
  ...
  public I_CmsSearchConfigurationHighlighting parseHighlighter() {
    ...
    try {
      ...
      String alternateField =
          parseOptionalStringValue(highlighter, JSON_KEY_HIGHLIGHTER_ALTERNATE_FIELD);
      Integer maxAlternateFieldLength =
          parseOptionalIntValue(highlighter, JSON_KEY_HIGHLIGHTER_MAX_LENGTH_ALTERNATE_FIELD);
      ...
      Boolean useFastVectorHighlighting =
          parseOptionalBooleanValue(highlighter, JSON_KEY_HIGHLIGHTER_FASTVECTORHIGHLIGHTING);
  ...
  public I_CmsSearchConfigurationPagination parsePagination() {
    ...
    return CmsSearchConfigurationPagination.create(
        getPageParam(), getPageSizes(), getPageNavLength());
  }
    ...
    try {
      ...
      try {
        ...
        List<String> preselection =
            parseOptionalStringValues(queryFacetObject, JSON_KEY_FACET_PRESELECTION);
        Boolean ignoreAllFacetFilters =
            parseOptionalBooleanValue(queryFacetObject, JSON_KEY_FACET_IGNOREALLFACETFILTERS);
        List<String> excludeTags =
            parseOptionalStringValues(queryFacetObject, JSON_KEY_FACET_EXCLUDETAGS);
        return new CmsSearchConfigurationFacetQuery(
            queries, label, isAndFacet, preselection, ignoreAllFacetFilters, excludeTags);
      } catch (JSONException e) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_QUERY_FACET_MANDATORY_KEY_MISSING_1, JSON_KEY_QUERY_FACET_QUERY),
            e);
        return null;
      }
    } catch (JSONException e) {
  ...
  public Map<String, I_CmsSearchConfigurationFacetRange> parseRangeFacets() {
    ...
    Map<String, I_CmsSearchConfigurationFacetRange> facetConfigs =
        new LinkedHashMap<String, I_CmsSearchConfigurationFacetRange>();
    try {
  ...
  public I_CmsSearchConfigurationSorting parseSorting() {
    ...
    String defaultOptionParamValue =
        parseOptionalStringValue(m_configObject, JSON_KEY_DEFAULT_SORT_OPTION);
  ...
  /**
   * Returns a map with additional request parameters, mapping the parameter names to Solr query
   * parts.
   *
   * @return A map with additional request parameters, mapping the parameter names to Solr query
   *     parts.
   */
    ...
    try {
      ...
      for (int i = 0; i < additionalParams.length(); i++) {
        try {
          ...
          String solrQuery =
              parseOptionalStringValue(currentParam, JSON_KEY_ADDITIONAL_PARAMETERS_SOLRQUERY);
          result.put(param, solrQuery);
        } catch (JSONException e) {
          LOG.error(
              Messages.get().getBundle().key(Messages.ERR_ADDITIONAL_PARAMETER_CONFIG_WRONG_0), e);
          continue;
        }
      }
    } catch (JSONException e) {
      LOG.info(
          Messages.get().getBundle().key(Messages.LOG_ADDITIONAL_PARAMETER_CONFIG_NOT_PARSED_0), e);
  ...
  /**
   * Returns the flag, indicating if the characters in the query string that are commands to Solr
   * should be escaped.
   *
   * @return the flag, indicating if the characters in the query string that are commands to Solr
   *     should be escaped.
   */
  ...
  /**
   * Returns the configured extra parameters that should be given to Solr, or the empty string if no
   * parameters are configured.
   *
   * @return The configured extra parameters that should be given to Solr, or the empty string if no
   *     parameters are configured.
   */
  ...
  /**
   * Returns the configured request parameter for the last query, or the default parameter if no
   * core is configured.
   *
   * @return The configured request parameter for the last query, or the default parameter if no
   *     core is configured.
   */
  protected String getFirstCallParam() {
    ...
    if (param == null) {
      return null != m_baseConfig
          ? m_baseConfig.getGeneralConfig().getReloadedParam()
  ...
  protected Boolean getIgnoreExpirationDate() {
    ...
    Boolean isIgnoreExpirationDate =
        parseOptionalBooleanValue(m_configObject, JSON_KEY_IGNORE_EXPIRATION_DATE);
  ...
  protected Boolean getIgnoreReleaseDate() {
    ...
    Boolean isIgnoreReleaseDate =
        parseOptionalBooleanValue(m_configObject, JSON_KEY_IGNORE_RELEASE_DATE);
  ...
  /**
   * Returns the number of maximally returned results, or <code>null</code> if the indexes default
   * should be used.
   *
   * @param indexName the name of the index to search in.
   * @return The number of maximally returned results, or <code>null</code> if the indexes default
   *     should be used.
   */
  protected int getMaxReturnedResults(String indexName) {

    Integer maxReturnedResults =
        parseOptionalIntValue(m_configObject, JSON_KEY_MAX_RETURNED_RESULTS);
  ...
  /**
   * Returns the configured length of the "Google"-like page navigation, or the default parameter if
   * no core is configured.
   *
   * @return The configured length of the "Google"-like page navigation, or the default parameter if
   *     no core is configured.
   */
  ...
  /**
   * Returns the configured request parameter for the current page, or the default parameter if no
   * core is configured.
   *
   * @return The configured request parameter for the current page, or the default parameter if no
   *     core is configured.
   */
    ...
    try {
      ...
      try {
        ...
        LOG.warn(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_PARSING_PAGE_SIZES_FAILED_1, pageSizesString),
            e);
      }
      if (null == m_baseConfig) {
        if (LOG.isInfoEnabled()) {
  ...
  /**
   * Returns the configured request parameter for the query string, or the default parameter if no
   * core is configured.
   *
   * @return The configured request parameter for the query string, or the default parameter if no
   *     core is configured.
   */
  protected String getQueryParam() {
    ...
    if (param == null) {
      return null != m_baseConfig
          ? m_baseConfig.getGeneralConfig().getQueryParam()
          : DEFAULT_QUERY_PARAM;
    } else {
      return param;
    }
  ...
  /**
   * Returns a flag, indicating if search should be performed using a wildcard if the empty query is
   * given.
   *
   * @return A flag, indicating if search should be performed using a wildcard if the empty query is
   *     given.
   */
  protected Boolean getSearchForEmptyQuery() {

    Boolean isSearchForEmptyQuery =
        parseOptionalBooleanValue(m_configObject, JSON_KEY_SEARCH_FOR_EMPTY_QUERY);
  ...
  /**
   * Returns the list of the configured sort options, or the empty list if no sort options are
   * configured.
   *
   * @return The list of the configured sort options, or the empty list if no sort options are
   *     configured.
   */
  protected List<I_CmsSearchConfigurationSortOption> getSortOptions() {

    List<I_CmsSearchConfigurationSortOption> options =
        new LinkedList<I_CmsSearchConfigurationSortOption>();
    try {
  ...
  /**
   * Returns the configured request parameter for the sort option, or the default parameter if no
   * core is configured.
   *
   * @return The configured request parameter for the sort option, or the default parameter if no
   *     core is configured.
   */
   ...
   * @param baseConfig The optional basic search configuration to overwrite (partly) by the JSON
   *     configuration.
   * @throws JSONException thrown if parsing fails.
   */
  protected void init(String configString, I_CmsSearchConfiguration baseConfig)
      throws JSONException {
   ...
   */
  protected List<I_CmsFacetQueryItem> parseFacetQueryItems(JSONObject queryFacetObject)
      throws JSONException {
  ...
  protected I_CmsSearchConfigurationFacetField parseFieldFacet(JSONObject fieldFacetObject) {
    ...
    try {
      ...
      List<String> preselection =
          parseOptionalStringValues(fieldFacetObject, JSON_KEY_FACET_PRESELECTION);
      Boolean ignoreFilterAllFacetFilters =
          parseOptionalBooleanValue(fieldFacetObject, JSON_KEY_FACET_IGNOREALLFACETFILTERS);
      List<String> excludeTags =
          parseOptionalStringValues(fieldFacetObject, JSON_KEY_FACET_EXCLUDETAGS);
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_FIELD_FACET_MANDATORY_KEY_MISSING_1, JSON_KEY_FACET_FIELD),
          e);
      return null;
    }
  }
  ...
  protected I_CmsSearchConfigurationFacetRange parseRangeFacet(JSONObject rangeFacetObject) {
    ...
    try {
      ...
      List<String> preselection =
          parseOptionalStringValues(rangeFacetObject, JSON_KEY_FACET_PRESELECTION);
      Boolean ignoreAllFacetFilters =
          parseOptionalBooleanValue(rangeFacetObject, JSON_KEY_FACET_IGNOREALLFACETFILTERS);
      List<String> excludeTags =
          parseOptionalStringValues(rangeFacetObject, JSON_KEY_FACET_EXCLUDETAGS);
  ...
  /**
   * Returns a single sort option configuration as configured via the methods parameter, or null if
   * the parameter does not specify a sort option.
   *
  ...
  protected I_CmsSearchConfigurationSortOption parseSortOption(JSONObject json) {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_SORT_OPTION_NOT_PARSABLE_1, JSON_KEY_SORTOPTION_SOLRVALUE),
          e);
      return null;
    }
  }
