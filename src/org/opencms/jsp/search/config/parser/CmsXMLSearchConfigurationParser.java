...
public class CmsXMLSearchConfigurationParser implements I_CmsSearchConfigurationParser {
  ...
  /** XML element name. */
  private static final String XML_ELEMENT_HIGHLIGHTER_MAX_LENGTH_ALTERNATE_FIELD =
  ...
  /** XML element name. */
  private static final String XML_ELEMENT_HIGHLIGHTER_FASTVECTORHIGHLIGHTING =
  ...
  /**
   * Constructor taking the XML content that should be read and the locale in which it should be
   * read.
   *
  ...
  public Map<String, I_CmsSearchConfigurationFacetField> parseFieldFacets() {
    ...
    final CmsXmlContentValueSequence fieldFacets =
        m_xml.getValueSequence(XML_ELEMENT_FIELD_FACETS, m_locale);
    if (fieldFacets != null) {
      for (int i = 0; i < fieldFacets.getElementCount(); i++) {

        final I_CmsSearchConfigurationFacetField config =
            parseFieldFacet(fieldFacets.getValue(i).getPath() + "/");
        if (config != null) {
  ...
  public I_CmsSearchConfigurationHighlighting parseHighlighter() {
    ...
    if (highlighter == null) {
      ...
      try {
        ...
        final Integer snippets =
            parseOptionalIntValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_SNIPPETS);
        final Integer fragsize =
            parseOptionalIntValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_FRAGSIZE);
        final String alternateField =
            parseOptionalStringValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_ALTERNATE_FIELD);
        final Integer maxAlternateFieldLength =
            parseOptionalIntValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_MAX_LENGTH_ALTERNATE_FIELD);
        final String pre =
            parseOptionalStringValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_SIMPLE_PRE);
        final String post =
            parseOptionalStringValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_SIMPLE_POST);
        final String formatter =
            parseOptionalStringValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_FORMATTER);
        final String fragmenter =
            parseOptionalStringValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_FRAGMENTER);
        final Boolean useFastVectorHighlighting =
            parseOptionalBooleanValue(pathPrefix + XML_ELEMENT_HIGHLIGHTER_FASTVECTORHIGHLIGHTING);
        ...
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_MANDATORY_HIGHLIGHTING_FIELD_MISSING_0), e);
        return null;
      }
    }
  }
  ...
  public I_CmsSearchConfigurationFacetQuery parseQueryFacet() {
    ...
    try {
      final List<I_CmsFacetQueryItem> queries =
          parseFacetQueryItems(pathPrefix + XML_ELEMENT_QUERY_FACET_QUERY);
      final String label = parseOptionalStringValue(pathPrefix + XML_ELEMENT_FACET_LABEL);
      final Boolean isAndFacet =
          parseOptionalBooleanValue(pathPrefix + XML_ELEMENT_FACET_ISANDFACET);
      final List<String> preselection =
          parseOptionalStringValues(pathPrefix + XML_ELEMENT_FACET_PRESELECTION);
      final List<String> excludeTags =
          parseOptionalStringValues(pathPrefix + XML_ELEMENT_FACET_EXCLUDETAG);
      final Boolean ignoreAllFacetFilters =
          parseOptionalBooleanValue(pathPrefix + XML_ELEMENT_FACET_IGNOREALLFACETFILTERS);
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_QUERY_FACET_MANDATORY_KEY_MISSING_1, XML_ELEMENT_QUERY_FACET_QUERY),
          e);
      return null;
    }
  }
  ...
  public Map<String, I_CmsSearchConfigurationFacetRange> parseRangeFacets() {
    ...
    final Map<String, I_CmsSearchConfigurationFacetRange> facetConfigs =
        new LinkedHashMap<String, I_CmsSearchConfigurationFacetRange>();
    final CmsXmlContentValueSequence rangeFacets =
        m_xml.getValueSequence(XML_ELEMENT_RANGE_FACETS, m_locale);
    if (rangeFacets != null) {
      for (int i = 0; i < rangeFacets.getElementCount(); i++) {

        final I_CmsSearchConfigurationFacetRange config =
            parseRangeFacet(rangeFacets.getValue(i).getPath() + "/");
        if (config != null) {
          facetConfigs.put(config.getName(), config);
        }
      }
   ...
   *
   * @param pathPrefix The XML Path that leads to the field facet configuration, or <code>null
  ...
  protected I_CmsSearchConfigurationFacetField parseFieldFacet(final @RUntainted String pathPrefix) {
    ...
    try {
      ...
      final String filterQueryModifier =
          parseOptionalStringValue(pathPrefix + XML_ELEMENT_FACET_FILTERQUERYMODIFIER);
      final Boolean isAndFacet =
          parseOptionalBooleanValue(pathPrefix + XML_ELEMENT_FACET_ISANDFACET);
      final List<String> preselection =
          parseOptionalStringValues(pathPrefix + XML_ELEMENT_FACET_PRESELECTION);
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_FIELD_FACET_MANDATORY_KEY_MISSING_1, XML_ELEMENT_FACET_FIELD),
          e);
      return null;
    }
  }
   ...
   *
   * @param pathPrefix The XML Path that leads to the range facet configuration, or <code>null
   *     </code> if the XML was not correctly structured.
   * @return The read configuration, or <code>null</code> if the XML was not correctly structured.
   */
  protected I_CmsSearchConfigurationFacetRange parseRangeFacet(@RUntainted String pathPrefix) {
    ...
    try {
      ...
      final Boolean hardEnd =
          parseOptionalBooleanValue(pathPrefix + XML_ELEMENT_RANGE_FACET_HARDEND);
      final Boolean isAndFacet =
          parseOptionalBooleanValue(pathPrefix + XML_ELEMENT_FACET_ISANDFACET);
      final List<String> preselection =
          parseOptionalStringValues(pathPrefix + XML_ELEMENT_FACET_PRESELECTION);
  ...
  private Map<String, String> getAdditionalRequestParameters() {
    ...
    List<I_CmsXmlContentValue> parametersToParse =
        m_xml.getValues(XML_ELEMENT_ADDITIONAL_PARAMETERS, m_locale);
  ...
  /**
   * Returns the extra Solr parameters specified in the configuration, or the empty string if no
   * extra parameters are configured.
   *
   * @return The extra Solr parameters specified in the configuration, or the empty string if no
   *     extra parameters are configured.
   */
  ...
  /**
   * Returns the configured request parameter for the last query, or the default parameter if the
   * core is not specified.
   *
   * @return The configured request parameter for the last query, or the default parameter if the
   *     core is not specified.
   */
  ...
  /**
   * Returns a flag, indicating if the query and lastquery parameters should be ignored. E.g., if
   * only the additional parameters should be used for the search.
   *
  ...
  /**
   * Returns the configured length of the "Google"-like page navigation, or the default length if it
   * is not configured.
   *
   * @return The configured length of the "Google"-like page navigation, or the default length if it
   *     is not configured.
   */
  ...
  /**
   * Returns the configured request parameter for the current page, or the default parameter if the
   * core is not specified.
   *
   * @return The configured request parameter for the current page, or the default parameter if the
   *     core is not specified.
   */
    ...
    if (pageSizes != null) {
      ...
      if (pageSizesArray.length > 0) {
        try {
          ...
          LOG.warn(
              Messages.get().getBundle().key(Messages.LOG_PARSING_PAGE_SIZES_FAILED_1, pageSizes),
              e);
        }
      }
    }
  ...
  /**
   * Returns the configured request parameter for the current query string, or the default parameter
   * if the core is not specified.
   *
   * @return The configured request parameter for the current query string, or the default parameter
   *     if the core is not specified.
   */
  ...
  private List<I_CmsSearchConfigurationSortOption> getSortOptions() {
    ...
    final List<I_CmsSearchConfigurationSortOption> options =
        new ArrayList<I_CmsSearchConfigurationSortOption>();
    final CmsXmlContentValueSequence sortOptions =
        m_xml.getValueSequence(XML_ELEMENT_SORTOPTIONS, m_locale);
    if (sortOptions == null) {
      return null;
    } else {
      for (int i = 0; i < sortOptions.getElementCount(); i++) {
        final I_CmsSearchConfigurationSortOption option =
            parseSortOption(sortOptions.getValue(i).getPath() + "/");
        if (option != null) {
          options.add(option);
        }
      }
  ...
  /**
   * Returns the configured request parameter for the current sort option, or the default parameter
   * if the core is not specified.
   *
   * @return The configured request parameter for the current sort option, or the default parameter
   *     if the core is not specified.
   */
  ...
  private I_CmsFacetQueryItem parseFacetQueryItem(final String prefix) {
    ...
    I_CmsXmlContentValue query =
        m_xml.getValue(prefix + XML_ELEMENT_QUERY_FACET_QUERY_QUERY, m_locale);
    if (null != query) {
      String queryString = query.getStringValue(null);
      I_CmsXmlContentValue label =
          m_xml.getValue(prefix + XML_ELEMENT_QUERY_FACET_QUERY_LABEL, m_locale);
  ...
  /**
   * Returns the configuration of a single sort option, or <code>null</code> if the XML cannot be
   * read.
   *
   * @param pathPrefix The XML path to the root node of the sort option's configuration.
   * @return The configuration of a single sort option, or <code>null</code> if the XML cannot be
   *     read.
   */
  private I_CmsSearchConfigurationSortOption parseSortOption(final String pathPrefix) {
    ...
    try {
      final String solrValue =
          parseMandatoryStringValue(pathPrefix + XML_ELEMENT_SORTOPTION_SOLRVALUE);
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_SORT_OPTION_NOT_PARSABLE_1, XML_ELEMENT_SORTOPTION_SOLRVALUE),
          e);
      return null;
    }
  }
