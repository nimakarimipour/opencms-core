...
public class CmsSimpleSearchConfigurationParser extends CmsJSONSearchConfigurationParser {
      ...
      switch (this) {
        case DATE_ASC:
          return new CmsSearchConfigurationSortOption(
              "date.asc", "date_asc", getSortDateField(l) + " asc");
        case DATE_DESC:
          return new CmsSearchConfigurationSortOption(
              "date.desc", "date_desc", getSortDateField(l) + " desc");
        case TITLE_ASC:
          return new CmsSearchConfigurationSortOption(
              "title.asc", "title_asc", getSortTitleField(l) + " asc");
        case TITLE_DESC:
          return new CmsSearchConfigurationSortOption(
              "title.desc", "title_desc", getSortTitleField(l) + " desc");
        case ORDER_ASC:
          return new CmsSearchConfigurationSortOption(
              "order.asc", "order_asc", getSortOrderField(l) + " asc");
        case ORDER_DESC:
          return new CmsSearchConfigurationSortOption(
              "order.desc", "order_desc", getSortOrderField(l) + " desc");
        default:
  ...
  public CmsSimpleSearchConfigurationParser(
      CmsObject cms, CmsConfigurationBean config, String additionalParamJSON) throws JSONException {
   ...
   *
   * <p>The point of this is that we know that passing an empty configuration makes it impossible
  ...
  public static CmsSimpleSearchConfigurationParser createInstanceWithNoJsonConfig(
      CmsObject cms, CmsConfigurationBean config) {
  ...
  public Map<String, I_CmsSearchConfigurationFacetRange> parseRangeFacets() {
    ...
    if (m_configObject.has(JSON_KEY_RANGE_FACETS)) {
      ...
      Map<String, I_CmsSearchConfigurationFacetRange> rangeFacets =
          new HashMap<String, I_CmsSearchConfigurationFacetRange>();
      String indexField = FIELD_DATE;
      if (Boolean.parseBoolean(
          m_config.getParameterValue(CmsConfigurationBean.PARAM_FILTER_MULTI_DAY))) {
        indexField = FIELD_DATE_RANGE;
      }
      I_CmsSearchConfigurationFacetRange rangeFacet =
   ...
   *
   * <p>If set, the search will ignore the blacklist from the list configuration.
   *
   * <p>
   *
   ...
   * Sets the pagination.
   *
   * <p>If this is set, parsePagination will always return the set value instead of using the
   * default way to compute the pagination
   *
  ...
  /**
   * @see
   *     org.opencms.jsp.search.config.parser.CmsJSONSearchConfigurationParser#getEscapeQueryChars()
   */
  @Override
  ...
  /**
   * @see
   *     org.opencms.jsp.search.config.parser.CmsJSONSearchConfigurationParser#getIgnoreExpirationDate()
   */
  @Override
  ...
  /**
   * @see
   *     org.opencms.jsp.search.config.parser.CmsJSONSearchConfigurationParser#getIgnoreReleaseDate()
   */
  @Override
  ...
  /**
   * @see
   *     org.opencms.jsp.search.config.parser.CmsJSONSearchConfigurationParser#getMaxReturnedResults(java.lang.String)
   */
  @Override
  ...
  /**
   * @see
   *     org.opencms.jsp.search.config.parser.CmsJSONSearchConfigurationParser#getSearchForEmptyQuery()
   */
  @Override
  ...
  protected List<I_CmsSearchConfigurationSortOption> getSortOptions() {
    ...
    if (m_configObject.has(JSON_KEY_SORTOPTIONS)) {
      ...
      CmsSimpleSearchConfigurationParser.SortOption currentOption =
          CmsSimpleSearchConfigurationParser.SortOption.valueOf(m_config.getSortOrder());
      if (m_sortOrder != null) {
        currentOption = m_sortOrder;
      }
      Locale locale = getSearchLocale();
      options.add(currentOption.getOption(locale));
      CmsSimpleSearchConfigurationParser.SortOption[] sortOptions =
  ...
  String generatePreconfiguredRestriction(
      String type, Map<CmsRestrictionRule, Collection<FieldValues>> restrictionsForType) {
    ...
    if ((null != restrictionsForType) && (restrictionsForType.size() > 0)) {
      ...
      for (Map.Entry<CmsRestrictionRule, Collection<FieldValues>> ruleEntry :
          restrictionsForType.entrySet()) {
        ruleRestrictions.add(
            generatePreconfiguredRestrictionForRule(ruleEntry.getKey(), ruleEntry.getValue()));
      }
      result =
          ruleRestrictions.size() > 1
              ? ruleRestrictions.stream()
                  .reduce((r1, r2) -> (r1 + " " + CombinationMode.AND + " " + r2))
                  .get()
  ...
  String generatePreconfiguredRestrictionForRule(
      CmsRestrictionRule rule, Collection<FieldValues> values) {

    Collection<String> resolvedFieldValues =
        values.stream().map(v -> resolveFieldValues(rule, v)).collect(Collectors.toSet());
  ...
  String getBlacklistFilter() {
    ...
    if (!blacklistStrings.isEmpty()) {
      result =
          "&fq="
              + CmsEncoder.encode(
                  "-id:(" + CmsStringUtil.listAsString(blacklistStrings, " OR ") + ")");
    }
    return result;
  }
        ...
        try {
          path =
              CmsCategoryService.getInstance()
                  .getCategory(m_cms, m_cms.getRequestContext().addSiteRoot(path))
                  .getPath();
  ...
  /**
   * The fields returned by default. Typically the output is done via display formatters and hence
   * nearly no field is necessary. Returning all fields might cause performance problems.
   *
  ...
  String getDefaultReturnFields() {
    ...
    fields.append(',');
    fields
        .append(CmsSearchField.FIELD_INSTANCEDATE_CURRENT_TILL)
        .append(CmsSearchField.FIELD_POSTFIX_DATE);
    fields.append(',');
    fields
        .append(CmsSearchField.FIELD_INSTANCEDATE)
        .append('_')
        .append(getSearchLocale().toString())
        .append(CmsSearchField.FIELD_POSTFIX_DATE);
    fields.append(',');
    fields
        .append(CmsSearchField.FIELD_INSTANCEDATE_END)
        .append('_')
        .append(getSearchLocale().toString())
        .append(CmsSearchField.FIELD_POSTFIX_DATE);
    fields.append(',');
    ...
    fields.append(',');
    fields
        .append(CmsSearchField.FIELD_DISPTITLE)
        .append('_')
        .append(getSearchLocale().toString())
        .append("_sort");
    fields.append(',');
  ...
  /**
   * Returns the folder filter string.
   *
   * <p>
   *
  ...
  /**
   * Returns the Geo filter query string.
   *
   * <p>
   *
  ...
  String getGeoFilterQuery() {
    ...
    if (geoFilterBean != null) {
      String fq =
          CmsSolrQueryUtil.composeGeoFilterQuery(
  ...
  String getPreconfiguredFilterQuery() {
    ...
    if (m_config.hasPreconfiguredRestrictions()) {
      ...
      String restriction =
          generatePreconfiguredRestriction(null, restrictions.getRestrictionsForType(null));
      ...
      for (String type : m_config.getTypes()) {
        restriction =
            generatePreconfiguredRestriction(type, restrictions.getRestrictionsForType(type));
        if (!restriction.isEmpty()) {
  ...
  /**
   * Returns the resource type filter string.
   *
   * <p>
   *
   * @return the folder filter
  ...
  String getResourceTypeFilter() {
    ...
    String result = "";
    // When we have pre-configured restrictions, we need to combine the type filter with these
    // restrictions.
    if (!m_config.hasPreconfiguredRestrictions()) {
      ...
      if (!typeVals.isEmpty()) {
        result =
            "&fq="
                + CmsEncoder.encode("type:(" + CmsStringUtil.listAsString(typeVals, " OR ") + ")");
      }
    }
    return result;
  }
    ...
    if (FieldType.PLAIN.equals(fieldValues.getFieldType())) {
      ...
      switch (rule.getMatchType()) {
        ...
        case INFIX:
          finalValues =
              values.stream()
                  .map(v -> ("(" + v + " OR *" + v + " OR *" + v + "* OR " + v + "*)"))
                  .collect(Collectors.toSet());
          break;
        case POSTFIX:
          finalValues =
              values.stream().map(v -> ("(" + v + " OR *" + v + ")")).collect(Collectors.toSet());
          break;
        case PREFIX:
          finalValues =
              values.stream().map(v -> ("(" + v + " OR " + v + "*)")).collect(Collectors.toSet());
   ...
   *
   * @param categoryConjunction flag, indicating if category selections in the facet should be "AND"
   *     combined.
   * @return the default field facets.
   */
  private Map<String, I_CmsSearchConfigurationFacetField> getDefaultFieldFacets(
      boolean categoryConjunction) {

    Map<String, I_CmsSearchConfigurationFacetField> fieldFacets =
        new HashMap<String, I_CmsSearchConfigurationFacetField>();
    fieldFacets.put(
        FIELD_CATEGORIES,
