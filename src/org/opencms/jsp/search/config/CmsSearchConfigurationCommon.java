...
public class CmsSearchConfigurationCommon implements I_CmsSearchConfigurationCommon {
  ...
  /**
   * A flag, indicating if the empty query should be interpreted as "*:*" or if no search should be
   * performed.
   */
  ...
  /**
   * Constructor for the common search configuration, where all configuration parameters are
   * provided.
   *
   ...
   * @param escapeQueryChars A flag, indicating if special query characters in the query string
   *     should be escaped (default <code>true</code>).
   * @param reloadedParam The first-call request param used by the search form.
   * @param seachForEmptyQuery A flag, indicating if the empty query should be interpreted as "*:*"
   *     or if no search should be performed.
   * @param ignoreQuery A flag, indicating if the query param's values should be used for Solr query
   *     generation.
   ...
   * @param maxReturnedResults Number of results maximally to return. <code><= 0</code> means that
   *     there is no limit.
   */
  public CmsSearchConfigurationCommon(
    ...
    m_additionalParameters =
        additionalParameters != null ? additionalParameters : new HashMap<String, String>();
    m_ignoreReleaseDate = null == ignoreReleaseDate ? false : ignoreReleaseDate.booleanValue();
    m_ignoreExpirationDate =
        null == ignoreExpirationDate ? false : ignoreExpirationDate.booleanValue();
  ...
  /**
   * @see
   *     org.opencms.jsp.search.config.I_CmsSearchConfigurationCommon#getModifiedQuery(java.lang.String)
   */
  @Override
