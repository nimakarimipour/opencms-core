...
/**
 * Interface of the JSP EL friendly wrapper for all Solr search results and the search form
 * controller.
 */
public interface I_CmsSearchResultWrapper {
  ...
  /**
   * Returns the suggestion of "Did you mean ...?" for the complete query - if did you mean is
   * enabled.
   *
   * @return The suggestion of "Did you mean ...?" for the complete query - if did you mean is
   *     enabled.
   */
  Suggestion getDidYouMeanSuggestion();

  /**
   * Returns the empty search state parameters. Use the function to generate just part of the state
   * parameters, instead of manipulating the current state.
   *
  ...
  /**
   * Returns the map from the document ids to the corresponding highlighting results (as map from
   * the highlighted field to the highlighted snippets).
   *
   * @return The map from the document ids to the corresponding highlighting results (as map from
   *     the highlighted field to the highlighted snippets).
   */
  ...
  /**
   * Returns for the specified facet (key) the selected facet entries that are not part of the
   * returned facet entries are provided (value).
   *
   * @return For the specified facet (key) the selected facet entries that are not part of the
   *     returned facet entries are provided (value).
   */
  Map<String, List<String>> getMissingSelectedFieldFacetEntries();

  /**
   * Returns the selected facet entries that are not part of the returned facet entries are provided
   * (value).
   *
   * @return The selected facet entries that are not part of the returned facet entries are provided
   *     (value).
   */
  ...
  /**
   * Returns the number of maximally returned results, this is the minimum of the number of found
   * results {@link #getNumFound()} and the number of results maximally processed {@link
   * I_CmsSearchConfigurationCommon#getMaxReturnedResults()}
   *
