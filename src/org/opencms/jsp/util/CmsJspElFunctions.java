...
public final class CmsJspElFunctions {
   ...
   *   <li>If the input is a {@link ServletRequest}, the OpenCms user context is read from the
   *       request context.
   *   <li>If the input is a {@link PageContext}, the OpenCms user context is read from the request
   *       of the page context.
   *   <li>Otherwise the input is converted to a String which should be a user name, and creation of
   *       a OpenCms user context with this name is attempted. Please note that this will only work
   *       if the user name is either the "Guest" user or the "Export" user.
   *   <li>If no valid OpenCms user context could be created with all of the above, then a new user
   *       context for the "Guest" user is created.
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>The Object is first checked if it is a {@link Date} already, if so it is casted and
   *       returned unchanged.
   *   <li>If not, the input is checked if it is a {@link Long}, and if so the Date is created from
   *       the Long value.
   *   <li>If it's not a Date and not a Long, the Object is transformed to a String and then it's
   *       tried to parse a Long out of the String.
   *   <li>If this fails, it is tried to parse as a Date using the default date formatting.
   *   <li>If this also fails, a new Date is returned that has been initialized with 0.
   *       <p>
   * </ul>
   *
   ...
   *   <li>If the input is of type {@link A_CmsJspValueWrapper}, the wrapper is converted to a
   *       Double using {@link A_CmsJspValueWrapper#getToDouble()}.
  ...
  /**
   * Returns a list of attribute values specified by the attribute name of the items of the given
   * list.
   *
   * <p>
   *
   ...
   *
   * <ul>
   *   <li>The Object is first checked if it is a {@link Locale} already, if so it is casted and
   *       returned.
   *   <li>If not, the input is transformed to a String and then a Locale lookup with this String is
   *       done.
   ...
   *
   * <ul>
   *   <li>If the input is already a {@link CmsResource}, it is casted to the resource and returned
   *       unchanged.
   *   <li>If the input is a String, the given OpenCms context is used to read a resource with this
   *       name from the VFS.
   *   <li>If the input is a {@link CmsUUID}, the given OpenCms context is used to read a resource
   *       with this UUID from the VFS.
   *   <li>Otherwise the input is converted to a String, and then the given OpenCms context is used
   *       to read a resource with this name from the VFS.
   * </ul>
   *
   ...
   *
   * <p>This is only possible if the input object is already a request or if it is a page context.
   *
   * <p>If everything else, this method returns <code>null</code>.
   *
   * <p>
   *
   ...
   *
   * <p>The wrapped result of {@link #convertRawResource(CmsObject, Object)} is returned.
   *
   * @param cms the current OpenCms user context
   * @param input the input to create a resource from
   ...
   */
  public static List<CmsJspResourceWrapper> convertResourceList(
      CmsObject cms, List<CmsResource> list) {
   ...
   *
   * <ul>
   *   <li>The Object is first checked if it is a {@link CmsUUID} already, if so it is casted and
   *       returned.
   *   <li>If not, the input is transformed to a byte[] and a new {@link CmsUUID} is created with
   *       this byte[].
   *   <li>Otherwise the input is casted to a String and a new {@link CmsUUID} is created with this
   *       String.
   * </ul>
   *
   ...
   *
   * <p>There is no way to create an empty list using standard JSTL methods, hence this function.
   *
   * <p>
   *
   ...
   * @return the parameter value from the module parameters, or <code>null</code> if the parameter
   *     is not set
   */
   ...
   *
   * <p>For OpenCms 10.5, this is the same as using <code>${cms.requestContext.uri}</code> on a JSP.
   *
   * <p>
   *
  ...
  /**
   * Returns whether the given value is an instance of {@link A_CmsJspValueWrapper}.
   *
   * <p>
   *
  ...
  /**
   * Converts a string (which is assumed to contain a JSON object whose values are strings only) to
   * a map, for use in JSPs.
   *
   * <p>If the input can't be interpreted as JSON, an empty map is returned.
   *
  ...
  /**
   * Looks up the given key from the map that is passed as a String, and returns either the element
   * found or the empty String.
   *
   * <p>The map String must have the form <code>"key1:value1|key2:value2"</code> etc.
   *
   * <p>
   *
   ...
   *
   * <p>Note that the result is an Object of type {@link java.lang.Long}, so in case the parameter
   * can not be converted to a number, <code>null</code> is returned.
   *
   * <p>
   *
    ...
    if (input instanceof CmsJspContentAccessValueWrapper) {
      ...
      try {
        return CmsHtmlExtractor.extractText(
            String.valueOf(input), OpenCms.getSystemInfo().getDefaultEncoding());
  ...
  /**
   * Converts the given Object to a (Double) number, falling back to a default if this is not
   * possible.
   *
   * <p>In case even the default can not be converted to a number, <code>-1.0</code> is returned.
   *
   * <p>
   *
   ...
   *
   * <p>If a char is cut, <code>" ..."</code> is appended to the result.
   *
   * <p>
   *
