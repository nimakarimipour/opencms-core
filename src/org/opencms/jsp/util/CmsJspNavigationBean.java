...
public class CmsJspNavigationBean {
  ...
  /**
   * Provides a Map with Booleans that indicate if the given URI is the currently active element in
   * the navigation.
   *
   * <p>
   */
        ...
        try {
          ...
          if ((defaultFile != null)
              && m_cms
                  ...
                  .equals(m_cms.getRequestContext().getUri())) {
            result = Boolean.TRUE;
          }
        } catch (
  ...
  /**
   * Provides a Map with Booleans that indicate if the given navigation URI is a parent element of
   * the current URI.
   *
   * <p>
   */
  ...
  /**
   * Returns a lazy initialized Map that provides Booleans that indicate if a given navigation uri
   * is currently active.
   *
   * <p>The provided Map key is assumed to be a String that represents an absolute VFS path.
   *
   ...
   * &lt;/c:forEach&gt;</pre>
   *
   * @return a lazy initialized Map that provides Booleans that indicate if a given navigation uri
   *     is currently active
   */
  ...
  /**
   * Returns a lazy initialized Map that provides Booleans that indicate if the given navigation URI
   * is a parent element of the current URI.
   *
   * <p>The provided Map key is assumed to be a String that represents an absolute VFS path.
   *
   * <p>Usage example on a JSP with the JSTL:
   ...
   * &lt;/c:forEach&gt;</pre>
   *
   * @return a lazy initialized Map that provides Booleans that indicate if the given navigation URI
   *     is a parent element of the current URI
   */
      ...
      switch (m_type) {
        ...
        case breadCrumb:
          if (m_resource != null) {
            // resource is set
            m_items =
                m_builder.getNavigationBreadCrumb(
                    m_resource, m_startLevel, m_endLevel, Boolean.valueOf(m_param).booleanValue());
          } else {
            if (m_startLevel == Integer.MIN_VALUE) {
              // default start level is zero
              m_items =
                  m_builder.getNavigationBreadCrumb(0, Boolean.valueOf(m_param).booleanValue());
            } else {
              if (m_endLevel != Integer.MIN_VALUE) {
                ...
                m_items =
                    m_builder.getNavigationBreadCrumb(
                        m_startLevel, Boolean.valueOf(m_param).booleanValue());
              }
            }
          }
          break;
