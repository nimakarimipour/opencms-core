...
public class CmsSettingDefinitionWrapper {
  ...
  /**
   * Macro resolver used to temporarily replace localization message macros with random UUIDs and
   * then replace the UUIDs with the original key after all other macro processing has happened.
   *
   * <p>We need this because we want to preserve the message key, but evaluate macros they may be
   * nested in.
   */
  ...
  public CmsSettingDefinitionWrapper(
      CmsObject cms, CmsXmlContentProperty settingDef, CmsMacroResolver resolver) {

    m_rawDefinition = settingDef;
    m_cms = cms;
    m_resolvedDefinition =
        CmsXmlContentPropertyHelper.resolveMacrosInProperty(settingDef, resolver);
    CmsKeyDummyMacroResolver keyResolver = new CmsKeyDummyMacroResolver(resolver);
    m_definitionWithKeys =
        CmsXmlContentPropertyHelper.resolveMacrosInProperty(settingDef, keyResolver);
  }

  /**
   * Extracts the message from a string of the form %(key.{message}), or returns null if the input
   * string does not have this form.
   *
  ...
  /**
   * Tries to interpret the widget configuration as a select option configuration and returns the
   * list of select options if this succeeds, and null otherwise.
   *
   * @return the list of parsed select options, or null if the widget configuration couldn't be
  ...
  public List<CmsSelectWidgetOption> getParsedSelectOptions() {
    ...
    try {
      List<CmsSelectWidgetOption> options =
          org.opencms.widgets.CmsSelectWidgetOption.parseOptions(widgetConfig);
      List<CmsSelectWidgetOption> result = new ArrayList<>();
      Set<String> values =
          options.stream().map(option -> option.getValue()).collect(Collectors.toSet());
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(defaultValue) || !values.contains(defaultValue)) {
        CmsSelectWidgetOption noValue =
            new CmsSelectWidgetOption(
                "",
                true,
                org.opencms.gwt.Messages.get()
                    .getBundle(locale)
                    .key(org.opencms.gwt.Messages.GUI_SELECTBOX_EMPTY_SELECTION_0));
        result.add(noValue);
      }
