...
public class CmsJspImageBean {
  ...
  /**
   * The CmsImageScaler that describes the basic adjustments (usually cropping) that have been set
   ...
   *
   * <ul>
   *   <li>key: the variant multiplier, e.g. "2x" (the common retina multiplier)
   *   <li>value: a CmsJspImageBean representing the hi-DPI variant
   * </ul>
   */
  ...
  /**
   * Initializes a new image bean based on a string pointing to a VFS resource that may contain
   * appended scaling parameters.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
   * @param imageUri the URI to read the image from in the OpenCms VFS, may also contain appended
   *     scaling parameters
  ...
  public CmsJspImageBean(CmsObject cms, String imageUri) throws CmsException {
    ...
    if (splitSrc.getQuery() != null) {
      ...
      String[] scaleStr =
          CmsRequestUtil.createParameterMap(splitSrc.getQuery()).get(CmsImageScaler.PARAM_SCALE);
      if (scaleStr != null) {
   ...
   *
   * <p>The input string is is used to read the images from the VFS. It can contain scaling
   * parameters. The additional re-scaling is then applied to the image that has been read.
   *
   * <p>
   *
   * @param cms the current uses OpenCms context
   * @param imageUri the URI to read the image from in the OpenCms VFS, may also contain scaling
   *     parameters
   ...
   *
   * <p>All values must be set with setters later.
   *
   * <p>
   */
      ...
      if ((baseScaler.getFocalPoint() != null)
        ...
        if (baseScaler.isCropping()) {
          ...
          if (targetCropWidth != baseScaler.getCropWidth()) {
            targetX =
                targetX + (int) Math.round((baseScaler.getCropWidth() - targetCropWidth) / 2.0);
          }
          if (targetCropHeight != baseScaler.getCropHeight()) {
            targetY =
                targetY + (int) Math.round((baseScaler.getCropHeight() - targetCropHeight) / 2.0);
          }
  ...
  /**
   * Helper method to check whether the focal point in the scaler is contained in the scaler's crop
   * region.
   *
   * <p>If the scaler has no crop region, true is returned.
   *
   ...
   */
  private static boolean checkCropRegionContainsFocalPoint(
      CmsImageScaler scaler, CmsPoint focalPoint) {
   ...
   *
   * <p>In case the screen size is not really known, it may be a good idea to add some variations
   * for large images to make sure there are some common options in case the basic image is very
   * large.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param hiDpiStr the hi-DPI variation to generate, for example "2.5x".
   *     <p>
  ...
  public CmsJspImageBean createHiDpiVariation(String hiDpiStr) {
    ...
    if (hiDpiStr.matches("^[0-9]+(.[0-9]+)?x$")) {
      ...
      double multiplier =
          Double.valueOf(hiDpiStr.substring(0, hiDpiStr.length() - 1)).doubleValue();
      ...
      CmsImageScaler targetScaler =
          createVariation(
              getWidth(), getHeight(), getBaseScaler(), targetWidth, targetHeight, getQuality());
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            String.format(
                "Illegal multiplier format: '%s' not usable for image scaling", hiDpiStr));
      }
    }
    return result;
  }
   ...
   * <p>
   *
   * @param ratioStr the rato variation to generate, for example "4-3" or "1-1".
   *     <p>
  ...
  public CmsJspImageBean createRatioVariation(String ratioStr) {
    ...
    try {
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            String.format("Illegal ratio format: '%s' not usable for image scaling", ratioStr));
      }
    }
   ...
   * <p>
   *
   * @param widthStr the with variation to generate, for example "1078" or "800".
   *     <p>
  ...
  public CmsJspImageBean createWidthVariation(String widthStr) {
    ...
    try {
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            String.format("Illegal width format: '%s' not usable for image scaling", widthStr));
      }
    }
  ...
  /**
   * Returns a lazy initialized Map that provides access to ratio scaled instances of this image
   * bean.
   *
   * <p>
   *
   * @return a lazy initialized Map that provides access to ratio scaled instances of this image
   *     bean
   ...
   *
   * <p>In case the image was cropped or otherwise manipulated, the values are created for the
   * manipulated version.
   *
   * <p>The return form is "src='(srcUrl)' height='(h)' width='(w)'".
   *
   * <p>
   *
   ...
   *
   * <p>The ratio is in the form 'width-height', for example '4-3' or '16-9'. In case no ratio was
   * set, the pixel dimensions of the image are returned.
   *
   * <p>
   *
   ...
   *
   * <p>In case a ratio has been used to scale the image, the height percentage is calculated based
   * on the ratio, not on the actual image pixel size. This is done to avoid rounding differences.
   *
   * <p>
   *
  ...
  public String getRatioHeightPercentage() {
    ...
    if (m_ratioHeightPercentage == null) {
      ...
      m_ratioHeightPercentage =
          calcRatioHeightPercentage(getScaler().getWidth(), getScaler().getHeight());
    }
  ...
  /**
   * Returns a lazy initialized Map that provides access to hi-DPI scaled instances of this image
   * bean.
   *
   * <p>
   *
   ...
   * </ul>
   *
   * @return a lazy initialized Map that provides access to hi-DPI scaled instances of this image
   *     bean
   */
  ...
  /**
   * Returns a lazy initialized Map that provides access to width scaled instances of this image
   * bean.
   *
   * <p>
   *
   * @return a lazy initialized Map that provides access to width scaled instances of this image
   *     bean
   */
   ...
   *
   * <p>In case no source set entries have been added before, the map is not initialized and <code>
   ...
   *
   * <p>In case the source set has not been initialized, it returns the instance itself.
   *
   ...
   *
   * <p>In case no source set entries have been added before, the map is not initialized and <code>0
   * </code> is returned.
   *
  ...
  /**
   * Getter for {@link #setSrcSets(CmsJspImageBean)} which returns this image bean.
   *
   * <p>Exists to make sure {@link #setSrcSets(CmsJspImageBean)} is available as property on a JSP.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this image bean has been correctly initialized with an image VFS
   * resource.
   *
   * <p>
   *
   * @return <code>true</code> if this image bean has been correctly initialized with an image VFS
   *     resource
   */
  ...
  /**
   * Returns <code>true</code> if the image has been scaled or otherwise processed.
   *
   * <p>
   *
   ...
   *
   * <p>The idea is to make sure large pixel images use a higher JPEG compression in order to reduce
   * the size.
   *
   * <p>The following quality settings are used depending on the image size:
   *
   * <ul>
   ...
   *
   * <p>It is always the original image which is used as a base, never a scaled version. So for
   * example if the image has been cropped by the user, the cropping are is ignored.
   *
   * <p>
   *
  ...
  /**
   * Sets the scaler that describes the basic adjustments (usually cropping) that have been set on
   * the original image.
   *
   * <p>
   *
   * @return the scaler that describes the basic adjustments (usually cropping) that have been set
   *     on the original image
   */
  ...
  /**
   * Returns the scaler that describes the basic adjustments (usually cropping) that have been set
   * on the original image.
   *
   * <p>
   *
   * @param baseScaler the scaler that describes the basic adjustments (usually cropping) that have
   *     been set on the original image
   */
