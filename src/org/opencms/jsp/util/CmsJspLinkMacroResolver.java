 ...
 *
 * <p>The only supported macro is the 'link' macro.
 *
 * <p>
 *
 * @since 6.5.4
...
public class CmsJspLinkMacroResolver implements I_CmsMacroResolver {
  ...
  /**
   * If <code>true</code> the macros get really resolved to valid vfs paths, otherwise only the
   * path/id in the macros are updated.
   */
   ...
   * @param jspRootPath the (optional) jsp root path, needed for saving from the editor to resolve
   *     relative links
   ...
   *
   * <p>Calls <code>{@link #resolveMacros(String)}</code> once for each macro in the input. This
   * means "nested" macros are not supported in this implementation, which is fine since it can't
   * happen in JSP link management anyway.
   *
   * <p>
   *
