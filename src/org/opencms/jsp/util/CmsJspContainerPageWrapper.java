...
public class CmsJspContainerPageWrapper {
  ...
  private String render(CmsJspStandardContextBean context, CmsContainerBean container) {
    ...
    try {
      ...
      // context bean copies stored in flex cache entries, but it doesn't matter since edit mode is
      // never
      ...
      for (CmsContainerElementBean element : container.getElements()) {
        ...
        try {
          ...
          element.initSettings(
              m_cms, adeConfig, formatterBean, locale, controller.getCurrentRequest(), null);
          context.setElement(element);
          CmsResource formatterRes =
              m_cms.readResource(
                  formatterBean.getJspStructureId(), CmsResourceFilter.IGNORE_EXPIRATION);
          byte[] formatterOutput =
              OpenCms.getResourceManager()
                  .getLoader(formatterRes)
                  .dump(
                      m_cms,
                      formatterRes,
                      null,
