 ...
 * (not found) errors.
 *
 * <p>The JSPs using this bean are placed in the OpenCms VFS folder /system/handler/.
 *
 * <p>
 *
 * @since 6.0
...
public class CmsJspStatusBean extends CmsJspActionElement {
   ...
   *
   * <p>The page to get the content from is looked up in the property value "template-elements". If
   * no value is found, the page is read from the "contents/" subfolder of the handler folder.
   *
   * <p>For each status code, an individual page can be created by naming it
   * "content${STATUSCODE}.html". If the individual page can not be found, the content is read from
   * "contentunknown.html".
   *
   * <p>
   *
   ...
   * </code>.
   *
   * <p>The key can use the following parameters for formatting:
   *
   * <ul>
   *   <li>0: the HTTP status code
   *   <li>1: the requested URI
   *   <li>2: the generated error message
   *   <li>3: the servlet name
   *   <li>4: the date of the request
   * </ul>
   *
   ...
   * Returns the localized resource string for a given message key.
   *
   * <p>For a detailed parameter description, see {@link CmsJspStatusBean#key(String)}.
   *
   * <p>
   *
   * @param keyName the key for the desired string
   * @param defaultKeyName the default key for the desired string, used if the keyName delivered no
   *     resource string
   * @return the resource string for the given key
   */
  public String key(String keyName, String defaultKeyName) {
    ...
    if (value.startsWith(CmsMessages.UNKNOWN_KEY_EXTENSION)
        && CmsStringUtil.isNotEmpty(defaultKeyName)) {
   ...
   *
   * <p>Automatically adds a status suffix for the key to get, eg. "keyname" gets the suffix "_404"
   * for a 404 status. For a detailed parameter description, see {@link
   * CmsJspStatusBean#key(String)}.
   *
   * <p>
   *
   * @param keyName the key for the desired string
  ...
  /**
   * Writes the exception into the 'opencms.log', if the exception is not <code>null</code>.
   *
   * <p>
   */
  ...
  /**
   * Returns true if the current user has the "DEVELOPER" role and can view the exception
   * stacktrace.
   *
   * <p>
   *
