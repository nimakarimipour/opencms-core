 ...
 * <code>&lt;cms:contentload&gt;</code> tag.
 *
 * <p>The implementation is optimized for performance and uses lazy initializing of the requested
...
public class CmsJspContentAccessBean {
  ...
  /**
   * Provides Booleans that indicate if a specified locale is available in the XML content, the
   * input is assumed to be a String that represents a Locale.
   *
   * <p>
   */
  ...
  /**
   * Provides Booleans that indicate if a specified path exists in the XML content, the input is
   * assumed to be a String that represents an xpath in the XML content.
   *
   * <p>
   */
  ...
  /**
   * Provides a Map with Booleans that indicate if a specified path exists in the XML content in the
   * selected Locale, the input is assumed to be a String that represents an xpath in the XML
   * content.
   *
   * <p>
   */
  ...
  /**
   * Transformer used for the 'imageDnd' EL attribute which is used to annotate images which can be
  ...
  /**
   * Provides a Map which lets the user access the list of element names from the selected locale in
   * an XML content, the input is assumed to be a String that represents a Locale.
   *
   * <p>
   */
  ...
  /**
   * Provides a Map which lets the user access the RDFA tags for all values in the selected locale
   * in an XML content, the input is assumed to be a String that represents a Locale.
   *
   * <p>
   */
  ...
  /**
   * Provides a Map which lets the user access sub value Lists from the selected locale in an XML
   * content, the input is assumed to be a String that represents a Locale.
   *
   * <p>
   */
  ...
  /**
   * Provides a Map which lets the user access value Lists from the selected locale in an XML
   * content, the input is assumed to be a String that represents a Locale.
   *
   * <p>
   */
  ...
  /**
   * Provides a Map which lets the user access the RDFA tag for a value in an XML content, the input
   * is assumed to be a String that represents an xpath in the XML content.
   *
   * <p>
   */
  public class CmsRdfaTransformer implements Transformer {
    ...
    public Object transform(Object input) {
      ...
      if (CmsJspContentAccessValueWrapper.isDirectEditEnabled(getCmsObject())) {
        return CmsContentService.getRdfaAttributes(
            getRawContent(), m_selectedLocale, String.valueOf(input));
      } else {
        return "";
      }
    }
  ...
  /**
   * Provides a Map which lets the user access sub value Lists in an XML content, the input is
   * assumed to be a String that represents an xpath in the XML content.
   *
   * <p>
   */
  public class CmsSubValueListTransformer implements Transformer {
    ...
    public Object transform(Object input) {
      ...
      List<I_CmsXmlContentValue> values =
          getRawContent().getSubValues(String.valueOf(input), m_selectedLocale);
      List<CmsJspContentAccessValueWrapper> result =
          new ArrayList<CmsJspContentAccessValueWrapper>();
      Iterator<I_CmsXmlContentValue> i = values.iterator();
      while (i.hasNext()) {
        // XML content API offers List of values only as Objects, must iterate them and create
        // Strings
        I_CmsXmlContentValue value = i.next();
        result.add(
            CmsJspContentAccessValueWrapper.createWrapper(getCmsObject(), value, null, null));
      }
      return result;
    }
  }
  ...
  /**
   * Provides a Map which lets the user access value Lists in an XML content, the input is assumed
   * to be a String that represents an xpath in the XML content.
   *
   * <p>
   */
  public class CmsValueListTransformer implements Transformer {
    ...
    public Object transform(Object input) {
      ...
      List<I_CmsXmlContentValue> values =
          getRawContent().getValues(String.valueOf(input), m_selectedLocale);
      List<CmsJspContentAccessValueWrapper> result =
          new ArrayList<CmsJspContentAccessValueWrapper>();
      Iterator<I_CmsXmlContentValue> i = values.iterator();
      while (i.hasNext()) {
  ...
  /**
   * Provides a Map which lets the user access a value in an XML content, the input is assumed to be
   * a String that represents an xpath in the XML content.
   *
   * <p>
   */
  public class CmsValueTransformer implements Transformer {
    ...
    public Object transform(Object input) {
      ...
      I_CmsXmlContentValue value =
          getRawContent().getValue(String.valueOf(input), m_selectedLocale);
      return CmsJspContentAccessValueWrapper.createWrapper(
          getCmsObject(), value, getRawContent(), (String) input, m_selectedLocale);
    }
  }

  /**
   * Constant Map that always returns the {@link
   * CmsJspContentAccessValueWrapper#NULL_VALUE_WRAPPER}.
   */
  protected static final Map<String, CmsJspContentAccessValueWrapper>
      CONSTANT_NULL_VALUE_WRAPPER_MAP =
          new CmsConstantMap<String, CmsJspContentAccessValueWrapper>(
              CmsJspContentAccessValueWrapper.NULL_VALUE_WRAPPER);
  ...
  /**
   * The locale used for accessing entries from the XML content, this may be a fallback default
   * locale.
   */
   ...
   *
   * <p>You must call {@link #init(CmsObject, Locale, I_CmsXmlDocument, CmsResource)} and provide
   * the required values when you use this constructor.
   *
   * <p>
   *
    ...
    return CmsCollectionsGenericWrapper.createLazyMap(
        new Transformer() {
          ...
          public Object transform(Object arg0) {
            ...
            return CmsJspContentAttachmentsBean.getAttachmentsForLocale(
                m_cms, m_resource, localeStr);
          }
        });
  }
   ...
   *
   * <p>This can be used to access information from the raw file on a JSP.
   *
  ...
  /**
   * Returns the site path of the current resource, that is the result of {@link
   * CmsObject#getSitePath(CmsResource)} with the resource obtained by {@link #getFile()}.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
  ...
  /**
   * Returns a lazy initialized Map that provides Booleans that indicate if a specified Locale is
   * available in the XML content.
   *
   * <p>The provided Map key is assumed to be a String that represents a Locale.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
   ...
   * &lt;/cms:contentload&gt;</pre>
   *
   * @return a lazy initialized Map that provides Booleans that indicate if a specified Locale is
   *     available in the XML content
   */
  ...
  /**
   * Returns a lazy initialized Map that provides a Map that provides Booleans that indicate if a
   * value (xpath) is available in the XML content in the selected locale.
   *
   * <p>The first provided Map key is assumed to be a String that represents the Locale, the second
   * provided Map key is assumed to be a String that represents the xpath to the value.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
   ...
   *
   * @return a lazy initialized Map that provides a Map that provides Booleans that indicate if a
   *     value (xpath) is available in the XML content in the selected locale
   * @see #getHasValue()
   */
  ...
  /**
   * Returns a lazy initialized Map that provides Booleans that indicate if a value (xpath) is
   * available in the XML content in the current locale.
   *
   * <p>The provided Map key is assumed to be a String that represents the xpath to the value.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
   ...
   *
   * @return a lazy initialized Map that provides Booleans that indicate if a value (xpath) is
   *     available in the XML content in the current locale
  ...
  /**
   * Returns the structure ID of the current resource, that is the ID of the resource obtained by
   * {@link #getFile()}.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
  ...
  /**
   * Returns <code>true</code> in case the current user is allowed to edit the XML content.
   *
   * <p>If the check is performed from the online project, the user context is internally switched
   * to an offline project. So this may return <code>true</code> even if the user is currently in
   * the online project.
   *
   * <p>"Allowed to edit" here requires "read" and "write" permission for the VFS resource the XML
   * content was created from. It also requires that the VFS resource is not locked by another user.
   * Moreover, the user must be able to access at least one "offline" project.
   *
   * <p>Intended for quick checks to for example show / hide edit buttons for user generated
   * content.
   *
   * <p>
   *
      ...
      if (m_cms.getRequestContext().getCurrentProject().isOnlineProject()) {
        ...
        List<CmsProject> projects =
            OpenCms.getOrgUnitManager()
                .getAllAccessibleProjects(cms, cms.getRequestContext().getOuFqn(), false);
  ...
  /**
   * Returns the Locale this bean is using for content access, this may be a default fall back
   * Locale.
   *
   * <p>
   *
   * @return the Locale this bean is using for content access, this may be a default fall back
   *     Locale
   */
  ...
  /**
   * Gets a lazy map from locales (can be provided as strings or java.util.Locale) to the (default)
   * JSON representation of an XML content.
   *
   ...
   *
   * <p>The provided Map key is assumed to be a String that represents the Locale.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
   ...
   * &lt;/cms:contentload&gt;</pre>
   *
   * @return a lazy initialized Map that provides a Map that provides values from the XML content in
   *     the selected locale
   * @see #getNames()
   */
  ...
  /**
   * Returns a lazy initialized Map that provides a Map that provides values from the XML content in
   * the selected locale.
   *
   * <p>The first provided Map key is assumed to be a String that represents the Locale, the second
   * provided Map key is assumed to be a String that represents the xpath to the value.
  ...
  /**
   * Returns a lazy initialized Map that provides a Map that provides Lists of values from the XML
   * content in the selected locale.
   *
   * <p>The first provided Map key is assumed to be a String that represents the Locale, the second
   * provided Map key is assumed to be a String that represents the xpath to the value.
   *
   ...
   * &lt;/cms:contentload&gt;</pre>
   *
   * @return a lazy initialized Map that provides a Map that provides Lists of values from the XML
   *     content in the selected locale
   * @see #getLocaleValue()
   */
  public Map<String, Map<String, List<CmsJspContentAccessValueWrapper>>> getLocaleValueList() {
    ...
    if (m_localeValueList == null) {
      m_localeValueList =
          CmsCollectionsGenericWrapper.createLazyMap(new CmsLocaleValueListTransformer());
    }
    return m_localeValueList;
  }

  /**
   * Returns a list with all available elements paths (Strings) used in this document in the current
   * locale.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   ...
   * &lt;/cms:contentload&gt;</pre>
   *
   * @return a list with all available elements paths (Strings) used in this document in the current
   *     locale
   * @see #getLocaleNames()
   */
  ...
  public String getPrintStructure() {
    ...
    if (getRawContent().hasLocale(m_requestedLocale)) {
      List<I_CmsXmlContentValue> values =
          new ArrayList<I_CmsXmlContentValue>(getRawContent().getValues(m_requestedLocale));
      Collections.sort(
          values,
          new Comparator<I_CmsXmlContentValue>() {
    ...
    if (m_content == null) {
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get()
                .container(Messages.ERR_XML_CONTENT_UNMARSHAL_1, m_resource.getRootPath()),
            e);
      }
    }
      ...
      m_locale =
          OpenCms.getLocaleManager()
              .getBestMatchingLocale(
                  m_requestedLocale,
                  OpenCms.getLocaleManager()
                      .getDefaultLocales(m_cms, m_cms.getRequestContext().getUri()),
  ...
  /**
   * Returns a lazy initialized Map that provides Lists of direct sub values of the given value from
   * the XML content in the current locale.
   *
   * <p>The provided Map key is assumed to be a String that represents the xpath to the value. Use
   * this method in case you want to iterate over a List of sub values from the XML content.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
   ...
   * &lt;/cms:contentload&gt;</pre>
   *
   * @return a lazy initialized Map that provides Lists of values from the XML content in the
   *     current locale
  ...
  /**
   * Returns a lazy initialized Map that provides Lists of values from the XML content in the
   * current locale.
   *
   * <p>The provided Map key is assumed to be a String that represents the xpath to the value. Use
   * this method in case you want to iterate over a List of values form the XML content.
   *
   * <p>Usage example on a JSP with the JSTL:
   *
   * <pre>
  ...
  /**
   * Returns an instance of a VFS access bean, initialized with the OpenCms user context this bean
   * was created with.
   *
   * <p>
   *
   * @return an instance of a VFS access bean, initialized with the OpenCms user context this bean
   *     was created with
   */
