 ...
 * from a JSP with the JSTL or EL.
 *
 * <p>
 *
 * @since 7.0.2
...
public final class CmsJspVfsAccessBean {
  ...
  /**
   * Transformer that loads a resource from the OpenCms VFS, the input is used as String for the
  ...
  public class CmsAvailableLocaleLoaderTransformer implements Transformer {
    ...
    public Object transform(Object input) {
      ...
      result =
          OpenCms.getLocaleManager().getAvailableLocales(getCmsObject(), String.valueOf(input));
      return result;
    }
  }

  /**
   * Provides Booleans that indicate if a specified resource exists in the OpenCms VFS, the input is
   * used as String for the resource name to read.
   *
   * <p>
   */
  ...
  /**
   * Provides Booleans that indicate if a specified resource exists in the OpenCms VFS and is of
   * type XML content or XML page, the input is used as String for the resource name to read.
   *
   * <p>
   */
  ...
  /**
   * Transformer that loads a resource permission from the OpenCms VFS, the input is used as String
   * for the resource name to read the permissions for.
   *
   * <p>
   */
  ...
  /**
   * Transformer that a properties of a resource from the OpenCms VFS, the input is used as String
   * for the property name to read.
   *
   * <p>
   */
  public class CmsPropertyLoaderSingleTransformer implements Transformer {
    ...
    public Object transform(Object input) {
      ...
      try {
        ...
        result =
            getCmsObject()
                .readPropertyObject(m_resource, String.valueOf(input), m_search)
                .getValue();
      } catch (CmsException e) {
  ...
  /**
   * Transformer that loads properties of a resource from the OpenCms VFS with another lazy map, the
   * input is used as String for the resource name to read.
   *
   * <p>
   */
  ...
  /**
   * Transformer that calculates links to resources in the OpenCms VFS, the input is used as String
   * for the resource name to use as link target.
   *
   * <p>This is using the same logic as {@link org.opencms.jsp.CmsJspTagLink#linkTagAction(String,
  ...
  public static final String ATTRIBUTE_VFS_ACCESS_BEAN =
      CmsJspVfsAccessBean.class.getName() + ".VFS_ACCESS_BEAN";
  ...
  /**
   * Returns a lazily generated map from site paths of resources to the available locales for the
   * resource.
   *
   * @return a lazily generated map from site paths of resources to the available locales for the
   *     resource.
   */
  public Map<String, List<Locale>> getAvailableLocales() {
    ...
    if (m_availableLocales == null) {
      ...
      m_availableLocales =
          CmsCollectionsGenericWrapper.createLazyMap(new CmsAvailableLocaleLoaderTransformer());
    }
   ...
   *
   * <p>Usage example on a JSP with the EL:
   *
   * <pre>
   ...
   *
   * <p>Usage example on a JSP with the EL / JSTL:
   *
   * <pre>
   ...
   * </pre>
   *
   * Usage example on a JSP with the <code>&lt;cms:contentaccess&gt;</code> tag:
   *
   * <pre>
   * &lt;cms:contentload ... &gt;
  ...
  public Map<String, Boolean> getExistsResource() {
    ...
    if (m_existsResource == null) {
      // create lazy map only on demand
      m_existsResource =
          CmsCollectionsGenericWrapper.createLazyMap(new CmsExistsResourceTransformer());
    }
    return m_existsResource;
  }

  /**
   * Returns a map that lazily checks if a resources exists in the VFS and is of type XML content or
   * XML page.
   *
   * <p>Usage example on a JSP with the EL / JSTL:
   *
   ...
   * </pre>
   *
   * @return a map that lazily checks if a resources exists in the VFS and is of type XML content or
   *     XML page
   */
   ...
   *
   * <p>The VFS access bean uses lazy initialized Maps for all access, but once a value has been
   * read it is cached in the Map and not read again from the VFS. This means the lazy Maps act as
   * another layer of cache to the VFS.
   *
   * <p>The VFS access bean instance itself is cached in the OpenCms request context attributes of
   * the {@link CmsObject}, see {@link #create(CmsObject)}. Normally there is a new {@link
   * CmsObject} created for all incoming requests, so the live-time of the VFS access bean is short.
   * In that case the caching of the lazy Maps should improve performance and not be an issue.
   * However, in rare cases an instance of a {@link CmsObject} may be kept for a long time in some
   * custom code. In theses cases flushing the caches of the lazy Maps manually may be required,
   * otherwise the Map caches may be out of sync with the VFS.
   *
  ...
  /**
   * Returns a map that lazily calculates links to files in the OpenCms VFS, which have been
   * adjusted according to the web application path and the OpenCms static export rules.
   *
   ...
   *
   * <p>Relative links are converted to absolute links, using the current OpenCms request context
   * URI as base.
   *
   * <p>Usage example on a JSP with the EL:
   *
   ...
   *
   * <p>Usage in JSP: ${myvfsaccessbeaninstance.localeResource['/foo/bar/index.html']['de']}
   *
  ...
  /**
   * Returns a lazy loading map used to detect the main locale of a resource which is part of a
   * locale group.
   *
   * <p>Usage in JSPs: ${myvfsaccessbeaninstance.mainLocale['/foo/index.html']}
   *
   * @return the lazy loading map
   */
  ...
  /**
   * Returns the parent folder of a resource.
   *
   * <p>This assumes the parameter is either a CmsResource, or a String representing a file name. If
   * the parameter is a CmsResource, the result will be relative to the current site.
   *
  ...
  public String getParentFolder(@RUntainted Object obj) {
    ...
    if (obj instanceof CmsResource) {
      result =
          CmsResource.getParentFolder(m_cms.getRequestContext().getSitePath((CmsResource) obj));
    } else {
   ...
   *
   * <p>This assumes the parameter is either a CmsResource, or a String representing a file name. If
   * the parameter is a CmsResource, the result will be relative to the current site. The root
   * folder "/" has level 0, a folder "/foo/" would have level 1, a folder "/foo/bar/" level 2 etc.
   *
   * <p>
   *
   * @param obj a CmsResource or a String representing a resource name
  ...
  public Map<String, CmsPermissionSet> getReadPermissions() {
    ...
    if (m_permissions == null) {
      // create lazy map only on demand
      m_permissions =
          CmsCollectionsGenericWrapper.createLazyMap(new CmsPermissionsLoaderTransformer());
    }
  ...
  public Map<String, Map<String, String>> getReadProperties() {
    ...
    if (m_properties == null) {
      ...
      m_properties =
          CmsCollectionsGenericWrapper.createLazyMap(
              new CmsResourcePropertyLoaderTransformer(false));
    }
    return m_properties;
  }
  ...
  /**
   * Returns a map that lazily reads all resource properties from the OpenCms VFS, with search and
   * provides locale specific access to them.
   *
   * <p>Usage example on a JSP with the EL:
   *
   * <pre>
  ...
  public Map<String, CmsJspContentAccessBean> getReadXml() {
    ...
    if (m_xmlContent == null) {
      // create lazy map only on demand
      m_xmlContent =
          CmsCollectionsGenericWrapper.createLazyMap(new CmsXmlContentAccessTransformer());
    }
    return m_xmlContent;
  }
   ...
   *
   * <p>This assumes the parameter is either a CmsResource, or a String representing a resource
   * name. The extension will always be lower case.
   *
   * <p>
   *
   * @param obj a CmsResource or a String representing a resource name
   ...
   * @throws CmsException in case reading the resources fails
   */
  public List<CmsJspResourceWrapper> readFilesInFolder(String resourcePath, String filterString)
      throws CmsException {
    ...
    return CmsJspElFunctions.convertResourceList(
        m_cms, m_cms.getFilesInFolder(resourcePath, filter));
  }
  ...
  public CmsJspResourceWrapper readSubsiteFor(String path) throws CmsException {
    ...
    CmsADEConfigData config =
        OpenCms.getADEManager()
            .lookupConfiguration(m_cms, m_cms.getRequestContext().addSiteRoot(path));
    return CmsJspResourceWrapper.wrap(
        m_cms, m_cms.readResource(m_cms.getRequestContext().removeSiteRoot(config.getBasePath())));
  }
}
