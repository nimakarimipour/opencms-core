...
public class CmsJspJsonWrapper extends AbstractCollection<Object> {
  ...
  /**
   * Helper class representing a "handle" to a JSON object's entry through which it is possible to
   * either access the entry's value or remove the entry.
   */
  private static class JSONObjectEntry {
     ...
     *
     * <p>If the key just normally occurs in the given object, a wrapper for that entry will be
     * returned. If the key is the wildcard '*', a list of all entries for the object is returned.
     * If the key does not exist in the object, an empty list is returned.
     *
  ...
  /**
   * Helper method for removing parts from a JSON object with a given path, which is already split
   * into path components.
   *
   * <p>See {@link #removePath(String)} for how the path works.
   *
  ...
  public static void removePathInJson(Object obj, List<String> pathComponents) {
    ...
    if (obj instanceof JSONArray) {
      ...
      List<JSONObjectEntry> childrenForKey =
          JSONObjectEntry.getEntriesForKey((JSONObject) obj, key);
   ...
   *
   * <p>This is an alias for {@link #getObject()}.
   *
   ...
   * Returns the wrapped JSON object.
   *
   * <p>Useful in case you want to insert an existing JSON object into another JSON object.
   *
   * @return the wrapped JSON object
   */
   ...
   *
   * <p>The path is a slash-separated sequence of path components, where each path component is
   * either the name of a JSON field, or the wildcard '*'. The removal process locates all JSON
   * objects matching the parent path of the given path, and then removes the entry given by the
   * last path component from it.
   *
   * <ul>
   *   <li>If a JSON array is encountered while descending the path, the rest of the path is
   *       processed for all elements of the array.
   *   <li>If an object is encountered which does not have an entry with the same name as the next
   *       path component, it and its contents are left unchanged.
