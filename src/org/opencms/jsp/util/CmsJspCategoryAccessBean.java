...
public class CmsJspCategoryAccessBean {
  ...
  /**
   * The path of the main category. All categories of {@link #m_categories} are sub-categories of
   * the main category.
   */
  String m_mainCategoryPath;

  /**
   * Map from the path of a main category to wrappers that hold only the sub-categories of that main
   * category.
   */
   ...
   * @param mainCategoryPath path of the main category for which only sub-categories should be
   *     wrapped.
   */
  CmsJspCategoryAccessBean(CmsObject cms, List<CmsCategory> categories, String mainCategoryPath) {
    m_cms = cms;
    m_mainCategoryPath =
        mainCategoryPath.isEmpty() || mainCategoryPath.endsWith("/")
            ? mainCategoryPath
            : mainCategoryPath + "/";
    ...
    m_categories =
        CmsCategoryService.getInstance()
            .localizeCategories(cms, m_categories, cms.getRequestContext().getLocale());
  }
   ...
   *
   * <p>The method assumes that categories are ordered in the list, i.e., parents are directly
   * followed by their children.
   *
   * <p>NOTE: In the complete category tree a leaf of the wrapped tree part may not be a leaf.
   *
  ...
  /**
   * Returns a map from a category path to the wrapper of all the sub-categories of the category
   * with the path given as key.
   *
