...
public class CmsFunctionRenderer {
  ...
  public static CmsResource getDefaultFunctionJsp(CmsObject cms) {
    ...
    return getDefaultResource(
        cms, "/system/modules/org.opencms.base/formatters/function-default.jsp");
  }

  /**
   * Helper method for cached reading of resources under specific, fixed paths.
   *
   * <p>
   *
   * @param cms the current CMS context
  ...
  public void render() throws IOException, JspException {
    ...
    boolean isNewFunctionType =
        OpenCms.getResourceManager()
            .matchResourceType(
                CmsResourceTypeFunctionConfig.TYPE_NAME, m_element.getResource().getTypeId());
  ...
  /**
   * Gets the formatter bean for the current element.
   *
   * <p>
   *
  ...
  private CmsFunctionFormatterBean getFormatterBean(CmsObject cms) {
    ...
    // Using getId(), *not* getFormatterId() here , since the former is the id of the function
  ...
  /**
   * Returns the function format for the current element.
   *
   * <p>
   *
  ...
  private CmsDynamicFunctionBean.Format getFunctionFormat() {
    ...
    try {
      CmsXmlContent content =
          CmsXmlContentFactory.unmarshal(m_cms, m_cms.readFile(m_element.getResource()));
