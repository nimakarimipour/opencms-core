...
public class CmsJspDateSeriesBean {
   ...
   *
   * <p>If no valid start date is provided, the information for the first event in the series is
   * provided.
   */
  ...
  /**
   * Lazy map from start dates (provided as Long, long value as string or date) to informations on
   * the single event.
   */
   ...
   * @param locale the locale in which dates should be rendered. This can differ from the content
   *     locale, if e.g. on a German page a content that is only present in English is rendered.
   */
   ...
   *
   * <p>NOTE: The parent series cannot be determined if this constructor is used.
   *
   * <p>
   *
   ...
   *
   * <p>In case this is just a single event and not a series, this is identical to the date of the
   * event.
   *
   * <p>
   *
  ...
  public CmsJspInstanceDateBean getFirst() {
    ...
    if ((m_firstEvent == null) && (m_dates != null) && (!m_dates.isEmpty())) {
      m_firstEvent =
          new CmsJspInstanceDateBean((Date) m_dates.first().clone(), CmsJspDateSeriesBean.this);
    }
    return m_firstEvent;
  }
  ...
  /**
   * Returns a lazy map from the start time of a single instance of the series to the date
   * information on the single instance.
   *
   * <p>Start time can be provided as Long, as a String representation of the long value or as Date.
   *
   * <p>If no event exists for the start time, the information for the first event of the series is
   * returned.
   *
   * @return a lazy map from the start time of a single instance of the series to the date
   *     information on the single instance.
   */
  public Map<Object, CmsJspInstanceDateBean> getInstanceInfo() {
    ...
    if (m_singleEvents == null) {
      m_singleEvents =
          CmsCollectionsGenericWrapper.createLazyMap(new CmsSeriesSingleEventTransformer());
    }
  ...
  /**
   * Returns a flag, indicating if the series is defined via a pattern, i.e., not just as via single
   * date.
   *
   * <p>
   *
   * @return a flag, indicating if the series is defined via a pattern, i.e., not just as via single
   *     date
   */
  ...
  /**
   * Returns a flag, indicating if the series is defined by only a single date and not extracted
   * from another series.
   *
   * <p>
   *
   * @return a flag, indicating if the series is defined by only a single date and not extracted
   *     from another series
   */
  ...
  public CmsJspInstanceDateBean getLast() {
    ...
    if ((m_lastEvent == null) && (m_dates != null) && (!m_dates.isEmpty())) {
      m_lastEvent =
          new CmsJspInstanceDateBean((Date) m_dates.last().clone(), CmsJspDateSeriesBean.this);
    }
    return m_lastEvent;
  }
    ...
    if ((m_parentSeries == null) && getIsExtractedDate()) {
      ...
      try {
        ...
        CmsJspContentAccessBean content =
            new CmsJspContentAccessBean(cms, m_value.getLocale(), res);
        ...
        LOG.warn(
            "Parent series with id "
                + m_seriesDefinition.getParentSeriesId()
                + " could not be read.",
            e);
      }
    }
   ...
   * <p>
   *
   * @return <code>true</code> if the event ends on another day than it starts, <code>false</code>
