 ...
 * HTML with the {@link org.opencms.jsp.parse.A_CmsConfiguredHtmlParser}} implementation specified
 * by the "parserClass" attribute.
 *
 * <p>
 *
 * @since 6.1.3
...
public class CmsJspTagParse extends BodyTagSupport {
  ...
  /**
   * Indicates the parse action should be disabled to allow inline editing in the container page
   * editor.
   */
  ...
  /**
   * List of upper case tag name strings of tags that should not be auto-corrected if closing divs
   * are missing.
   */
              ...
              LOG.error(
                  Messages.get()
                      ...
                      .key(
                          ...
                          new Object[] {
                            TAG_NAME, ATT_VISITOR_CLASS, A_CmsConfiguredHtmlParser.class.getName()
                          }),
                  e);
            }
   ...
   *
   * <p>Returns a <code>String</code> that consists of the comma-separated upper case tag names for
   * which this tag will not correct missing closing tags.
   *
   * <p>
   *
   * @return a String that consists of the comma-separated upper case tag names for which this tag
   *     will not correct missing closing tags.
   */
  ...
  /**
   * Returns if the parse action should be disabled to allow inline editing in the container page
   * editor.
   *
   * <p>
   *
   * @return <code>true</code> if the parse action should be disabled to allow inline editing in the
   ...
   *
   * <p>Parses (and potentially transforms) a HTMl content block.
   *
   * <p>
   *
   ...
   */
  public String parseTagAction(
      String content, PageContext context, A_CmsConfiguredHtmlParser parser) {

    String result = null;
    CmsRequestContext cmsContext =
        CmsFlexController.getCmsObject(context.getRequest()).getRequestContext();

    if (parser == null) {
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle(cmsContext.getLocale())
                .key(
                    Messages.GUI_ERR_TAG_ATTRIBUTE_MISSING_2,
                    new Object[] {TAG_NAME, ATT_VISITOR_CLASS}));
      }
      ...
      try {
        ...
        if (LOG.isErrorEnabled()) {
          LOG.error(
              Messages.get()
                  .getBundle(cmsContext.getLocale())
                  .key(Messages.ERR_PROCESS_TAG_1, new Object[] {TAG_NAME}),
              pex);
        }
  ...
  /**
   * Sets if the parse action should be disabled to allow inline editing in the container page
   * editor.
   *
   * <p>
   *
   ...
   *
   * <p>Awaits a <code>String</code> that consists of the comma-separated upper case tag names for
   * which this tag should not correct missing closing tags.
   *
   * <p>
   *
   * @param noAutoCloseTagList a <code>String</code> that consists of the comma-separated upper case
   *     tag names for which this tag should not correct missing closing tags
   */
