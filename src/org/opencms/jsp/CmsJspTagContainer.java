...
public class CmsJspTagContainer extends BodyTagSupport
    implements TryCatchFinally, I_CmsJspTagParamParent {
  ...
  public static final String DUMMY_ELEMENT =
      "<div class='"
  ...
  /**
   * If false, formatters are always included in non-cacheable mode, otherwise they are included in
   * cacheable mode in the Online project only.
   */
  ...
  public static I_CmsFormatterBean ensureValidFormatterSettings(
    ...
    if (formatterBean != null) {
      ...
      if (keyOrId == null) {
        keyOrId =
            CmsFormatterConfig.SCHEMA_FORMATTER_ID + formatterBean.getJspStructureId().toString();
      }
  ...
  public static I_CmsFormatterBean getFormatterConfigurationForElement(
    ...
    if ((formatterBean == null)
        ...
        && !element.getFormatterId().isNullUUID()) {
      ...
      for (I_CmsFormatterBean formatter :
        ...
        if (element.getFormatterId().equals(formatter.getJspStructureId())) {
          ...
          if (formatterConfigId == null) {
            formatterConfigId =
                CmsFormatterConfig.SCHEMA_FORMATTER_ID + element.getFormatterId().toString();
          }
    ...
    if (formatterBean == null) {
      ...
      formatterBean =
          adeConfig
              .getFormatters(cms, element.getResource())
              .getDefaultFormatter(containerType, containerWidth);
    }
    return formatterBean;
  }
  ...
  public static List<CmsContainerElementBean> getGroupContainerElements(
    ...
    CmsXmlGroupContainer xmlGroupContainer =
        CmsXmlGroupContainerFactory.unmarshal(cms, element.getResource(), req);
    CmsGroupContainerBean groupContainer = xmlGroupContainer.getGroupContainer(cms);
    groupContainer =
        CmsTemplateMapper.get(req)
            .transformGroupContainer(
                cms, groupContainer, xmlGroupContainer.getFile().getRootPath());
              ...
              Messages.get()
                  .container(
                      ...
                      OpenCms.getResourceManager()
                          .getResourceType(element.getResource())
                          .getTypeName(),
                      containerType)));
  ...
  public static List<CmsContainerElementBean> getInheritedContainerElements(
      CmsObject cms, CmsContainerElementBean element) {
   ...
   *
   * <p>This will be either {parentInstanceId}-{name} or {namePrefix}-{name} or in case namePrefix
   * equals 'none' {name} only.
   *
   * <p>
   *
  ...
  protected static String getTagOpen(
    ...
    if (containerData != null) {
      buffer
          .append(" " + CmsGwtConstants.ATTR_DATA_CONTAINER + "=\"")
          .append(containerData)
          .append("\" ");
    ...
    if (CmsFlexController.isCmsRequest(req)) {
      ...
      try {
        ...
        m_hasModelGroupAncestor =
            m_editableRequest ? hasModelGroupAncestor(standardContext) : false;
        ...
        boolean detailOnly =
            m_detailOnly || ((m_parentContainer != null) && m_parentContainer.isDetailOnly());
        if (detailOnly) {
          if (detailContent == null) {
            ...
            CmsContainerPageBean detailOnlyPage =
                CmsDetailOnlyContainerUtil.getDetailOnlyPage(cms, req, pageRootPath);
          ...
          container =
              new CmsContainerBean(
                  ...
                  (m_parentContainer == null)
                      || (m_detailOnly && !m_parentContainer.isDetailOnly()),
                  maxElements,
                  Collections.<CmsContainerElementBean>emptyList());
        } else if ((m_parentElement != null)
            && !m_detailOnly // ignore parent information for detail only containers to render
                             // content on different detail pages.
            && !m_parentElement.getInstanceId().equals(container.getParentInstanceId())) {
          // the container parent instance id does not match the parent element instance id, skip
          // rendering to avoid recursion
          LOG.error(
              new CmsIllegalStateException(
                  Messages.get()
                      .container(
        ...
        String tagName =
            CmsStringUtil.isEmptyOrWhitespaceOnly(getTag()) ? DEFAULT_TAG_NAME : getTag();
        pageContext
            .getOut()
            .print(
                getTagOpen(
        ...
        for (CmsContainerElementBean elementBean : allElements) {
          ...
          // this element will not be rendered, in case the detail page is not actually used to
          // render detail content
          boolean skipDetailTemplateElement = false;
          try {
            skipDetailTemplateElement =
                first
                    && !m_editableRequest
                    && m_detailView
                    && (detailElement == null)
                    ...
                    && OpenCms.getADEManager()
                        .getDetailPages(cms, elementBean.getTypeName())
                        .contains(
                            CmsResource.getFolderPath(
                                standardContext.getPageResource().getRootPath()));
          } catch (Exception e) {
            LOG.error(e.getLocalizedMessage(), e);
          }
          first = false;
   ...
   *
   * <p>This is useful for a dynamically generated nested container, to pass information to the
   * formatter used inside that container.
   *
   * <p>If no parameters have been set, this will return <code>null</code>
   *
   ...
   *
   * <p>If the container type has not been set, the name is substituted as type.
   *
   * <p>
   *
   ...
   *
   * <p>If this is set to false, formatters will never be included in cacheable mode, otherwise they
   * will only be included in cacheable mode in the Online project.
   *
   ...
   * @param isDetailView <code>true</code> if this container is currently being used for the detail
   *     view
  ...
  protected String getContainerData(
      CmsObject cms, int maxElements, boolean isDetailView, boolean isDetailOnly) {
          ...
          if (role != null) {
            result =
                OpenCms.getRoleManager()
                    .hasRole(
                        cms, role.forOrgUnit(cms.getRequestContext().getCurrentUser().getOuFqn()));
          }
        }
        if (result) {
          break;
  ...
  /**
   * Generates the detail view element.
   *
   * <p>
   *
      ...
      if (formatter != null) {
        ...
        element =
            new CmsContainerElementBean(
                detailContent.getStructureId(), formatter.getJspStructureId(), settings, false);
  ...
  /**
   * Gets the container width as a number.
   *
   * <p>If the container width is not set, or not a number, -1 will be returned.
   *
   * <p>
   *
  ...
  /**
   * Returns the detail function element.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  private CmsContainerElementBean getDetailFunctionElement(
      CmsObject cms, CmsResource detailFunctionPage, ServletRequest req) {

    try {
      CmsXmlContainerPage xmlContainerPage =
          CmsXmlContainerPageFactory.unmarshal(cms, detailFunctionPage, req);
  ...
  private String getElementInfo(
      CmsObject cms, CmsContainerElementBean elementBean, CmsContainerPageBean page)
      throws Exception {
  ...
  /**
   * Parses the maximum element number from the current container and returns the resulting number.
   *
   * <p>
   *
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(containerMaxElements)) {
      ...
      if (LOG.isWarnEnabled()) {
        LOG.warn(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_MAXELEMENTS_NOT_SET_2, new Object[] {getName(), requestUri}));
      }
    }
    return maxElements;
  }
  ...
  /**
   * Returns the ADE session cache for container elements in case of an editable request, otherwise
   * <code>null</code>.
   *
   * <p>
   *
  ...
  /**
   * Evaluates if this container is nested within a model group.
   *
   * <p>
   *
  ...
  /**
   * Prints an element error tag to the response out.
   *
   * <p>
   *
   ...
   * @param alreadyFull if true, only render invisible elements (they don't count towards the "max
   *     elements")
   * @return true if an element was rendered that counts towards the container's maximum number of
   *     elements
   * @throws Exception if something goes wrong
   */
  private boolean renderContainerElement(
    ...
    CmsTemplateContext context =
        (CmsTemplateContext)
            (request.getAttribute(CmsTemplateContextManager.ATTR_TEMPLATE_CONTEXT));
    ...
    if (context != null) {
      ...
      String rpcContextOverride =
          (String) request.getAttribute(CmsTemplateContextManager.ATTR_RPC_CONTEXT_OVERRIDE);
    ...
    if (!isGroupContainer && !isInheritedContainer) {
      // ensure that the formatter configuration id is added to the element settings, so it will be
      // persisted on save
      formatterConfig =
          ensureValidFormatterSettings(
              cms, element, adeConfig, getName(), containerType, containerWidth);
      element.initSettings(cms, adeConfig, formatterConfig, locale, request, m_settingPresets);
    }
    // writing elements to the session cache to improve performance of the container-page editor in
    // offline project
    if (m_editableRequest) {
    ...
    if (isGroupContainer || isInheritedContainer) {
      ...
      // wrapping the elements with DIV containing initial element data. To be removed by the
      ...
      for (CmsContainerElementBean subelement : subElements) {
        ...
        try {
          ...
          subelement.initSettings(
              cms, adeConfig, subElementFormatterConfig, locale, request, m_settingPresets);
          // writing elements to the session cache to improve performance of the container-page
          // editor
          if (m_editableRequest) {
                  ...
                  new CmsIllegalStateException(
                      Messages.get()
                          .container(
                              ...
                              OpenCms.getResourceManager()
                                  .getResourceType(subelement.getResource())
                                  .getTypeName(),
                              containerType)));
            }
          ...
          try {
            ...
            try {
              CmsResource formatterResource =
                  cms.readResource(subElementFormatterConfig.getJspStructureId());
              ...
              formatterSitePath =
                  cms.getRequestContext()
                      .removeSiteRoot(subElementFormatterConfig.getJspRootPath());
            }
            ...
            printElementErrorTag(
                subelement.getSitePath(), subElementFormatterConfig.getJspRootPath(), e);
          }
      ...
      if (alreadyFull) {
        ...
        try {
          ...
          LOG.debug(
              "Formatter resource can not be found, try reading it from the configuration.", e);
          // the formatter resource can not be found, try reading it form the configuration
          CmsFormatterConfiguration elementFormatters =
              adeConfig.getFormatters(cms, element.getResource());
          I_CmsFormatterBean elementFormatterBean =
              elementFormatters.getDefaultFormatter(containerType, containerWidth);
          ...
          try {
            CmsResource formatterResource =
                cms.readResource(elementFormatterBean.getJspStructureId());
            formatter = cms.getSitePath(formatterResource);
          } catch (CmsVfsResourceNotFoundException ex) {
            LOG.debug("Formatter JSP not found by id, try using path.", ex);
            formatter =
                cms.getRequestContext().removeSiteRoot(elementFormatterBean.getJspRootPath());
          }
        }
  ...
  /**
   * Resets the tag instance and standard context state.
   *
   * <p>
   */
  private void resetState() {
    ...
    CmsJspStandardContextBean cmsContext =
        CmsJspStandardContextBean.getInstance(pageContext.getRequest());
  ...
  /**
   * Helper method to determine whether an element should be shown in a context.
   *
   * <p>
   *
      ...
      if ((element.getResource() != null)
          && !OpenCms.getTemplateContextManager()
              .shouldShowType(
                  contextKey,
                  OpenCms.getResourceManager()
                      .getResourceType(element.getResource().getTypeId())
                      .getTypeName())) {
        return false;
      }
