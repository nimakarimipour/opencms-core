 ...
 *
 * <p>This class is also used to generate the direct edit buttons for the <code>
 * &lt;cms:include editable="..." /&gt;</code> and <code>&lt;cms:contentload editable="..." /&gt;
 * </code> tags.
 *
 * <p>Since OpenCms version 6.2.3, the direct edit button HTML generated is controlled by an
 * instance of {@link I_CmsDirectEditProvider}. The default direct edit provider used can be
 * configured in <code>opencms-workplace.xml</code> in the <code>
 * &lt;directeditprovider class="..." /&gt;</code> node. The standard provider is {@link
 * org.opencms.workplace.editors.directedit.CmsDirectEditDefaultProvider}. It's possible to override
 * the default provider onm a page-by-page basis by initializing direct edit with <code>
 * &lt;cms:editable provider="...." /&gt;</code> on top of the page.
 *
 * <p>Since OpenCms version 6.2.3, it is also possible to place the HTML of the direct edit buttons
 * manually. This is intended for pages where the template HTML is not compatible with the direct
 * edit HTML, which usually results in a funny placement of the direct edit buttons in a totally
 * wrong position. To do manual placement of the direct edit buttons, you need to place <code>
 * &lt;cms:editable mode="manual"&gt;</code> and <code>&lt;/cms:editable&gt;</code> around your
 * HTML. Both tags (start and end) will insert HTML according to the used {@link
 * I_CmsDirectEditProvider}. The direct edit provider used must also support manual placing, or the
 * manual tags will be ignored and the HTML will be inserted at the automatic position. A provider
 * which support manual placing is the {@link
...
public class CmsJspTagEditable extends BodyTagSupport {
   ...
   * @param provider the class name of the direct edit privider to use (may be <code>null</code>,
   *     which means use the default)
   * @param mode the direct edit mode to use (may be <code>null</code>, which means current use mode
   *     on page)
   * @param fileName optional filename parameter for the direct edit provider (may be <code>null
   *     </code>, which means use the default)
  ...
  public static void editableTagAction(
    ...
    if (eb == null) {
      if (CmsStringUtil.isNotEmpty(fileName) && CmsStringUtil.isEmpty(provider)) {
        // if only a filename but no provider class is given, use JSP includes for backward
        // compatibility
      ...
      if (CmsStringUtil.isNotEmpty(provider)) {
        try {
          ...
          // log error
          LOG.error(
              Messages.get().getBundle().key(Messages.ERR_DIRECT_EDIT_PROVIDER_1, provider), e);
        }
      }
      if (eb == null) {
  ...
  public static I_CmsDirectEditProvider getDirectEditProvider(PageContext context) {
    ...
    return (I_CmsDirectEditProvider)
        context.getRequest().getAttribute(I_CmsDirectEditProvider.ATTRIBUTE_DIRECT_EDIT_PROVIDER);
  }
  ...
  public static void insertEditEmpty(
      PageContext context, I_CmsXmlContentContainer container, CmsDirectEditMode mode, String id)
    ...
    if (isEditableRequest(req)
      ...
      I_CmsResourceCollector collector =
          OpenCms.getResourceManager().getContentCollector(container.getCollectorName());
      if (collector == null) {
        throw new CmsException(
            Messages.get()
                .container(Messages.ERR_COLLECTOR_NOT_FOUND_1, container.getCollectorName()));
      }
      String createParam =
      ...
      if ((createParam != null)
        ...
        CmsDirectEditParams params =
            new CmsDirectEditParams(
                createFolderName, CmsDirectEditButtonSelection.NEW, mode, createParam);
        params.setId(id);
  ...
  /**
   * Includes the "direct edit" start element that adds HTML for the editable area to the output
   * page.
   *
   * <p>
   *
   ...
   * @throws JspException in case something goes wrong
   */
  public static boolean startDirectEdit(PageContext context, CmsDirectEditParams params)
      throws JspException {
  ...
  /**
   * Returns the current initialized instance of the direct edit provider parameters from the given
   * page context.
   *
   * <p>Also removes the parameters from the given page context.
   *
   * <p>
   *
   * @param context the current JSP page context
  ...
  protected static CmsDirectEditParams getDirectEditProviderParams(PageContext context) {
    ...
    CmsDirectEditParams result =
        (CmsDirectEditParams)
            req.getAttribute(I_CmsDirectEditProvider.ATTRIBUTE_DIRECT_EDIT_PROVIDER_PARAMS);
    if (result != null) {
   ...
   */
  protected static void setDirectEditProvider(
      PageContext context, I_CmsDirectEditProvider provider) {

    // set the direct edit provider as attribute to the request
    context
        .getRequest()
        .setAttribute(I_CmsDirectEditProvider.ATTRIBUTE_DIRECT_EDIT_PROVIDER, provider);
  }

  /**
   * Sets the current initialized instance of the direct edit provider parameters to the page
   * context.
   *
   * <p>
   *
  ...
  protected static void setDirectEditProviderParams(
      PageContext context, CmsDirectEditParams params) {

    // set the direct edit params as attribute to the request
    context
        .getRequest()
        .setAttribute(I_CmsDirectEditProvider.ATTRIBUTE_DIRECT_EDIT_PROVIDER_PARAMS, params);
  }
  ...
  public int doEndTag() throws JspException {
    ...
    if (m_firstOnPage || m_manualPlacement) {
      // only execute action for the first "editable" tag on the page (include file), or in manual
      // mode
  ...
  /**
   * Opens the direct edit tag, if manual mode is set then the next start HTML for the direct edit
   * buttons is printed to the page.
   *
   * <p>
   *
