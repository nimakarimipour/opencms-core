 ...
 * functionality.
 *
 * <p>If you have large chunks of code on your JSP that you want to move to a Class file, consider
 * creating a subclass of this bean.
 *
...
public class CmsJspBean {
   ...
   *
   * <p>This is a convenience method in case you need access to the CmsObject in your JSP scriplets.
   *
   * <p>
   *
   ...
   *
   * <p>It is required to call one of the init() methods before you can use the instance of this
   * bean.
   *
   ...
   * output page, or <code>false</code> if they will be thrown and have to be handled by the calling
   * class.
   *
   * <p>The default is <code>true</code>. If set to <code>false</code> Exceptions that occur
   * internally will be wrapped into a RuntimeException and thrown to the calling class instance.
   *
   * <p>
   *
   * @return <code>true</code> if Exceptions are suppressed, or <code>false</code> if they will be
   *     thrown and have to be handled by the calling class
   */
   ...
   *
   * <p>This method is required since JSP's are handled in a special way for included template
   * elements, so {@link javax.servlet.ServletResponse#setContentType(java.lang.String)} won't work.
   *
   * <p>Please note that the content type set this way is never cached in the Flex cache, so you
   * must make sure to not cache the element when you use this method.
   *
   * <p>
   *
   ...
   *
   * <p>This method is required since JSP's are handled in a special way for included template
   * elements, so {@link javax.servlet.http.HttpServletResponseWrapper#setStatus(int)} won't work.
   *
   * <p>Please note that the status code set this way is never cached in the Flex cache, so you must
   * make sure to not cache the element when you use this method.
   *
   * <p>
   *
  ...
  /**
   * Controls if Exceptions that occur in methods of this class are suppressed (true) or not
   * (false).
   *
   * <p>The default is <code>true</code>. If set to <code>false</code> all Exceptions that occur
   * internally will be wrapped into a RuntimeException and thrown to the calling class instance.
   *
   * <p>
   *
   ...
   *
   * <p>This is protected since the CmsFlexController this is really an internal OpenCms helper
   * function, not part of the public OpenCms API. It must not be used on JSP pages, only from
   * subclasses of this bean.
   *
   * <p>
   *
  ...
  /**
   * Internally localizes the given <code>CmsMessageContainer</code> to a String.
   *
   * <p>If the user request context is at hand, the user's locale will be chosen. If no user request
   * context is available, the default locale is used.
   *
   * <p>
   *
  ...
  /**
   * Handles any exception that might occur in the context of this element to ensure that templates
   * are not disturbed.
   *
   * <p>
   *
  ...
  protected void handleException(Throwable t) {
    ...
    if (!(m_isSupressingExceptions || getRequestContext().getCurrentProject().isOnlineProject())) {
      ...
      throw new CmsRuntimeException(
          Messages.get()
              .container(Messages.ERR_RUNTIME_1, (uri != null) ? uri : getClass().getName()),
          t);
    }
  }
   ...
   *
   * <p>Override this if you are reusing old workplace classes in a context where no flex controller
   * is available.
   */
  ...
  /**
   * Returns true if this bean has not been initialized (i.e. init() has not been called so far),
   * false otherwise.
   *
   * <p>
   *
