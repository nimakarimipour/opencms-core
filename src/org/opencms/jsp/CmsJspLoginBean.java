...
public class CmsJspLoginBean extends CmsJspActionElement {
  ...
  public static void logLoginException(
    ...
    if (currentLoginException instanceof CmsAuthentificationException) {
      ...
      } else if (org.opencms.security.Messages.ERR_LOGIN_FAILED_NO_USER_2
          == currentLoginException.getMessageContainer().getKey()) {
   ...
   * login was successful or not attempted).
   *
   * <p>
   *
  ...
  /**
   * Returns true if the current user is not the guest user, i.e. if he already has logged in with
   ...
   *
   * <p>Note that if a login project name is provided, this project must exist, otherwise the login
   * is regarded as a failure even if the user data was correct.
   *
   * <p>
   *
   * @param userName the users name
  ...
  public void login(String userName, String password, String projectName) {
    ...
    try {
      ...
      getCmsObject()
          .loginUser(userName, password, secondFactorInfo, getRequestContext().getRemoteAddress());
      ...
      if (projectName != null) {
        ...
        getCmsObject()
            .getRequestContext()
            .setCurrentProject(getCmsObject().readProject(projectName));
      }
      if (!getCmsObject().getRequestContext().getCurrentProject().isOnlineProject()) {
        ...
        if ((loginMessage != null) && loginMessage.isActive()) {
          ...
          OpenCms.getSessionManager()
              .sendBroadcast(null, loginMessage.getMessage(), user, ContentMode.html);
        }
      }
  ...
  public void login(String userName, String password, String projectName, String redirectUri)
    ...
    if (m_loginException == null) {
      try {
        ...
        LOG.warn(
            "Invalid redirect URI " + redirectUri + " in login bean: " + e.getLocalizedMessage(),
            e);
        return;
      }
      if (redirectUri != null) {
  ...
  /**
   * Logs a user out, i.e. destroys the current users session, after that the current page will be
   * redirected to itself one time to ensure that the users session is truly destroyed.
   *
   * <p>
   *
