 ...
 * access and display XML content item information in a formatter.
 *
 * <p>
 *
 * @since 8.0.0
...
public class CmsJspTagFormatter extends CmsJspScopedVarBodyTagSuport {
  ...
  /**
   * Constructor used when using <code>formatter</code> from scriptlet code.
   *
   * <p>
   *
  ...
  protected void init() throws JspException {
    ...
    try {
      ...
      if ((m_element.isInMemoryOnly() || m_element.isTemporaryContent())
        ...
        I_CmsXmlDocument xmlContent =
            CmsXmlContentFactory.unmarshal(
                m_cms, m_element.getResource(), pageContext.getRequest());
      ...
      if (m_rdfa != null) {
        // if the optional "rdfa" parameter has been set, store the rdfa map of the content in the
        // page context scope
