 ...
 *
 * <p><em>No output is generated by this tag!</em> Instead the tag generates a {@link
 * org.opencms.jsp.util.CmsJspImageBean}. This can be used to further process the selected image
 * with the provided image scaling parameters.
 *
 * <p>The following image formats are supported: BMP, GIF, JPEG, PNG, PNM, TIFF.
 *
 * <p><em> Note: Picture scaling is by default only enabled for target size with width and height
 * &lt;=1500. The size can be changed in the image scaler configuration in the file <code>
 * opencms-vfs.xml</code> in the body of the tag <code>&lt;loader&gt;</code>. Also other options for
 * the image scaler are set there. </em>
 *
 * <p>This tag is an alternative to the OpenCms standard tag cms:img, providing additional
 * flexibility. This way you can use scaled images for:
 *
 * <ul>
 *   <li>The standard HTML &lt;img&gt;-Tag.
 *   <li>The HTML 5 &lt;picture&gt;-Tag with multiple sources (hi-DPI variants for retina displays)
 *       for responsive design.
 *   <li>Further processing a scaled image anyway you want on your JSP.
 * </ul>
 *
 * </em>
 *
 * <p>Example for a simple JSP that uses this tag to obtain information about an image: <code>
...
public class CmsJspTagScaleImage extends CmsJspImageScalerTagSupport {
   ...
   * @param hiDpiVariantList optional list of hi-DPI variant sizes to produce, e.g. 1.3x, 1.5x, 2x,
   *     3x
   * @return the created ScaledImageBean bean
   * @throws CmsException in case something goes wrong
   */
  public static CmsJspImageBean imageTagAction(
      CmsObject cms, String imageUri, CmsImageScaler targetScaler, List<String> hiDpiVariantList)
      throws CmsException {
   ...
   * version of the image (and hi-DPI variants if necessary), stores all information in a image bean
   * and stores it as a request attribute (the name for this attribute is given with the tag
   * attribute "var").
   *
   * @return EVAL_BODY_INCLUDE or SKIP_BODY in case of an unexpected Exception (please consult the
   *     OpenCms log file if that happens)
   */
  @Override
    ...
    if (CmsFlexController.isCmsRequest(req)) {
      ...
      try {
        ...
        try {
          ...
          String src =
              CmsLinkManager.getAbsoluteUri(m_src, controller.getCurrentRequest().getElementUri());
  ...
  /**
   * Sets the String containing a comma separated list of hi-DPI variants to produce line
   * "1.3x,1.5x,2x,3x".
   *
   * <p>Currently in most cases "2x" should suffice to generate an additiona image for retina
   * screens.
   *
   * <p>Please note that since 11.0 the variants are created by lazy initialization, so there is
   * usually no need to use this.
   *
   * <p>
   *
