...
public class CmsJspTagInclude extends BodyTagSupport implements I_CmsJspTagParamParent {
   ...
   * @param attrMap a map of attributes for the include, will be merged with the request attributes,
  ...
  public static void includeTagAction(
    ...
    try {
      ...
      Map<String, Object> attributeMap =
          (attrMap == null) ? new HashMap<String, Object>() : new HashMap<String, Object>(attrMap);
      ...
      if (!dynamicParams.isEmpty()) {
        // We want to store the parameters from the request with keys in dynamicParams in the flex
        // response's include list, but only if they're set
  ...
  /**
   * Includes the selected target without caching.
   *
   * <p>
   *
      ...
      if (loader instanceof I_CmsResourceStringDumpLoader) {
        ...
        if (!(req instanceof HttpServletRequest) || !(res instanceof HttpServletResponse)) {
          ...
          CmsLoaderException e =
              new CmsLoaderException(Messages.get().container(Messages.ERR_BAD_REQUEST_RESPONSE_0));
          throw new JspException(e);
        }
        // get the bytes from the loader and convert them to a String
        byte[] result =
            loader.dump(
                cms, file, element, locale, (HttpServletRequest) req, (HttpServletResponse) res);
  ...
  /**
   * Includes the selected target using the Flex cache.
   *
   * <p>
   *
   * @param controller the current JSP controller
  ...
  private static void includeActionWithCache(
    ...
    try {
      ...
      // now use the Flex dispatcher to include the target (this will also work for targets in the
      // OpenCms VFS)
      try {
        controller.getCurrentRequest().getRequestDispatcher(target).include(req, res);
      } finally {
        // write out a FLEX_CACHE_DELIMITER char on the page, this is used as a parsing delimiter
        // later
   ...
   *
   * <p>Parameters added here will be treated like parameters from the HttpRequest on included
   * pages.
   *
   * <p>Remember that the value for a parameter in a HttpRequest is a String array, not just a
   * simple String. If a parameter added here does not already exist in the HttpRequest, it will be
   * added. If a parameter exists, another value will be added to the array of values. If the value
   * already exists for the parameter, nothing will be added, since a value can appear only once per
   * parameter.
   *
   * <p>
   *
  ...
  public int doEndTag() throws JspException {
    ...
    if (CmsFlexController.isCmsRequest(req)) {
      ...
      if (m_target != null) {
        ...
        try {
          String prop =
              cms.readPropertyObject(cms.getRequestContext().getUri(), m_property, true).getValue();
          if (prop != null) {
  ...
  /**
   * Returns the value of <code>{@link #getPage()}</code>.
   *
   * <p>
   *
   ...
   *
   * <p>Cachable is <code>true</code> by default.
   *
   * <p>
   *
   ...
   *
   * <p>Editable is <code>false</code> by default.
   *
   * <p>
   *
  ...
  /**
   * Sets the file, same as using <code>setPage()</code>.
   *
   * <p>
   *
