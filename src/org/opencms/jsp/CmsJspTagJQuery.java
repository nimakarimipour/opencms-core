...
/**
 * Implementation of the <code>&lt;cms:jquery/&gt;</code> tag.
 *
 * <p>OpenCms version 7.0.5 introduced a core module providing jQuery plus some additional plugins.
 * This tag will include the JQuery javascript library depending on the current project. If the
 * current Project is offline the unpacked version is used, if online the packed version will be
 * used.
 *
 * @since 7.0.5
 * @deprecated jQuery should be provided by standard includes directly in the template. The jQuery
 *     version shipping with OpenCms used by this tag is outdated. This tag is kept only for
...
public class CmsJspTagJQuery extends BodyTagSupport {
  ...
  public int doStartTag() throws JspException {
    ...
    try {
      ...
      if (isDynamic()) {
        ...
        pageContext
            .getOut()
            .print("<script src='" + CmsWorkplace.getSkinUri() + file + "' ></script>");
      }
    } catch (Exception ex) {
      if (LOG.isErrorEnabled()) {
        LOG.error(Messages.get().getBundle().key(Messages.ERR_PROCESS_TAG_1, "jquery"), ex);
  ...
  /**
   * Checks if the inclusion is dynamic or not.
   *
   * <p>
   *
