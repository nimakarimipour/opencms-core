...
public class CmsJspTagDisplay extends BodyTagSupport implements I_CmsJspTagParamParent {
      ...
      try {
        if (formatter != null) {
          ...
          if (contextBean.getIsEditMode()) {
            CmsADESessionCache.getCache((HttpServletRequest) (context.getRequest()), cms)
  ...
  public static void displayAction(
    ...
    displayAction(
        element, formatter, true, false, false, false, null, null, null, context, request,
        response);
  }
  ...
  public static void displayAction(
    ...
    CmsContainerElementBean element =
        new CmsContainerElementBean(
            elementResource.getStructureId(), formatter.getJspStructureId(), settings, false);
    displayAction(
        element,
        formatter,
        cacheable,
  ...
  /**
   * If the setting key starts with the key or id of the given formatter, returns the remaining
   * suffix, else null.
   *
  ...
  public static String getSettingKeyForMatchingFormatterPrefix(
    ...
    boolean keyMatch =
        (dynamicFmt.getKey() != null) && dynamicFmt.getKey().equals(formatter.getKey());
   ...
   * @param path the path to the formatter configuration file.
   *     <p>
   */
        ...
        CmsADEConfigData config =
            OpenCms.getADEManager()
                .lookupConfigurationWithCache(
                    cmsForFormatterLookup, cms.getRequestContext().getRootUri());
        if (formatter == null) {
  ...
  public int doStartTag() {
    ...
    if (Boolean.valueOf(m_passSettings).booleanValue()) {
      CmsContainerElementBean element =
          CmsJspStandardContextBean.getInstance(pageContext.getRequest()).getElement();
      if (element != null) {
  ...
  /**
   * Returns the config for the requested resource, or <code>null</code> if not available.
   *
   * <p>
   *
   ...
   */
  private I_CmsFormatterBean getFormatterForType(
      CmsObject cms, CmsResource resource, boolean isOnline) {
   ...
   */
  private Map<String, String> prepareSettings(
      CmsADEConfigData config, I_CmsFormatterBean formatter) {
    ...
    for (Entry<String, String> entry : m_parameterMap.entrySet()) {
      ...
      String fmtSetting =
          getSettingKeyForMatchingFormatterPrefix(config, formatter, entry.getKey());
