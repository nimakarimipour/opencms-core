 ...
 *
 * <p>Of particular importance is the setting of the <code>file</code> attribute, which can take the
 * following values.
 *
 * <p>This attribute allows you to specify where to search for the property.<br>
 * The following values are supported:
 *
 * <DL>
 *   <DT><b>uri</b> (default)
 *   <DD>Look up the property on the file with the uri requested by the user.
 *   <DT><b>search.uri</b> or <b>search</b>
 *   <DD>Look up the property by also checking all parent folders for the property, starting with
 *       the file with uri requested by the user and going "upward" if the property was not found
 *       there.
 *   <DT><b>element.uri</b>
 *   <DD>Look up the property on the currently processed sub - element. This is useful in templates
 *       or other pages that consist of many elements.
 *   <DT><b>search.element.uri</b>
 *   <DD>Look up the property by also checking all parent folders for the property, starting with
 *       the file with the currently processed sub - element and going "upward" if the property was
 *       not found there.
 *   <DT>sitemap
 *   <DD>reads from the current sitemap entry
 *   <DT>search.sitemap
 *   <DD>Look up the property by also checking all parent sitemap entries for the property, starting
 *       with the current sitemap entry and going "upward" if the property was not found there.
 *   <DT>container
 *   <DD>reads from the current container element
 *   <DT><B>{some-file-uri}</B>
 *   <DD>Look up the property on that exact file uri in the OpenCms VFS,<EM> fallback if no other
 *       valid option is selected for the file attribute.</EM>
 * </DL>
 *
 * <p>There are also some deprecated options for the "file" value that are still supported but
 ...
 *   <DT>search-this
 *   <DD>same as <STRONG>search.element.uri</STRONG>
 * </DL>
 *
 * @since 6.0.0
 */
public class CmsJspTagProperty extends TagSupport {
  ...
  /**
   * Tells for which resource properties should be looked up (with or without searching), depending
   * on the {@link FileUse}.
   */
  public static class CmsPropertyAction {
    ...
    /**
     * Returns <code>true</code> if it should be searched for the property, otherwise <code>false
     * </code>.
     *
     * @return <code>true</code> if it should be searched for the property, otherwise <code>false
     *     </code>.
     */
   ...
   * @return String the value of the property or <code>null</code> if not found (and no defaultValue
   *     provided)
    ...
    if (null != propertyAction.getVfsUri()) {
      value =
          CmsProperty.toMap(
              controller
                  .getCmsObject()
                  .readPropertyObjects(propertyAction.getVfsUri(), propertyAction.isSearch()));
    }
    return value;
  }
    ...
    if (null != propertyAction.getVfsUri()) {
      value =
          controller
              .getCmsObject()
              .readPropertyObject(
                  propertyAction.getVfsUri(), property, propertyAction.isSearch(), locale)
              .getValue();
    }
    if (value == null) {
      value = defaultValue;
  ...
  public int doStartTag() throws JspException {
    ...
    if (CmsFlexController.isCmsRequest(req)) {
      ...
      try {
        String prop =
            propertyTagAction(getName(), getFile(), m_defaultValue, m_escapeHtml, req, m_locale);
   ...
   *
   * <p>This is used if a selected property is not found.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param value should be <code>"true"</code> or <code>"false"</code> (all values other then
   *     <code>"true"</code> are considered to be false)
   */
  ...
  public void setLocale(String locale) {
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.ERR_TAG_INVALID_LOCALE_1, "cms:property"), e);
      m_locale = null;
    }
  }
