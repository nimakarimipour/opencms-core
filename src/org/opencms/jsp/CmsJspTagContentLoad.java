 ...
 * content item information from the VFS.
 *
 * <p>Since version 7.0.2 it is also possible to store the results of the content load in the JSP
 * context using a {@link CmsJspContentLoadBean}. Using this bean the loaded XML content objects can
 * be accessed directly using the JSP EL and the JSTL. To use this feature, you need to add the
 * <code>var</code> (and optionally the <code>scope</code>) parameter to the content load tag. For
 * example, if a parameter like <code>var="myVarName"</code> is provided, then the result of the
 * content load is stored in the JSP context variable <code>myVarName</code> with an instance of a
 * {@link CmsJspContentLoadBean}.
 *
 * <p>
 *
 * @since 6.0.0
 */
public class CmsJspTagContentLoad extends CmsJspTagResourceLoad
   ...
   *
   * <p>Initially, this is equal to the locale set using <code>{@link #setLocale(String)}</code>.
   * However, the content locale may change in case a loaded XML content does not have the selected
   * locale available. In this case the next default locale that is available in the content will be
   * used as content locale.
   *
   * <p>
   */
    ...
    if (m_isFirstLoop) {
      ...
      if (!m_cms.getRequestContext().getCurrentProject().isOnlineProject()
        ...
        if (CmsJspTagEditable.getDirectEditProvider(pageContext) != null) {
          CmsJspTagEditable.getDirectEditProvider(pageContext)
              .insertDirectEditListMetadata(pageContext, info);
        }
      }
      m_isFirstLoop = false;
      if (!hasMoreContent && m_editEmpty && (m_directEditLinkForNew != null)) {
        try {
          CmsJspTagEditable.insertEditEmpty(
              pageContext, this, m_directEditMode, m_contentInfoBean.getId());
  ...
  protected void doLoadNextFile() throws CmsException {
    ...
    // as no support for getting the historic version that has been cached by a
  ...
  protected void init(I_CmsXmlContentContainer container) throws JspException {
    ...
    if ((pageAttribCount > 0) && (pageAttribCount < 2)) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_TAG_CONTENTLOAD_INDEX_SIZE_0));
    }
    ...
    try {
      ...
      if (collector == null) {
        throw new CmsException(
            Messages.get().container(Messages.ERR_COLLECTOR_NOT_FOUND_1, m_collectorName));
      }
      ...
      if (!isPreloader()) {
        ...
        m_collectorResult =
            CmsJspTagResourceLoad.limitCollectorResult(m_contentInfoBean, m_collectorResult);
        ...
        if ((createParam != null) && CmsDefaultFileNameGenerator.hasNumberMacro(createParam)) {
          // use "create link" only if collector supports it and it contains the number macro for
          // new file names
