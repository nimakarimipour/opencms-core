 ...
 *
 * <p>The semicolon seperated elements of the decoartion file are stored in a map.
 *
 * <p>The map uses the decoration as keys and CmsDecorationObjects as values.
 *
 * <p>Multiple CmsDecorationMaps form a CmsDecorationBundle.
 *
 * @since 6.1.3
 */
public class CmsDecorationMap implements Comparable<CmsDecorationMap> {
  ...
  /**
   * Extracts the locale from the decoration filename.
   *
   * <p>
   *
  ...
  /**
   * Fills the decoration map with values from the decoation file.
   *
   * <p>
   *
   * @param cms the CmsObject
   ...
   * @throws CmsException if something goes wrong
   */
  private Map<String, CmsDecorationObject> fillMap(CmsObject cms, CmsResource res)
      throws CmsException {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DECORATION_MAP_FILL_MAP_2, m_name, m_decoratorDefinition));
    }
            ...
            if (CmsStringUtil.isNotEmpty(key) && CmsStringUtil.isNotEmpty(value)) {
              ...
              if (LOG.isDebugEnabled()) {
                LOG.debug(
                    Messages.get()
                        .getBundle()
                        .key(Messages.LOG_DECORATION_MAP_ADD_DECORATION_OBJECT_2, decObj, key));
              }
            }
          }
        }
