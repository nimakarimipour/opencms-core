 ...
 *
 * <p>It uses the information of a <code>{@link CmsDecoratorConfiguration}</code> to process the
 * text decorations.
 *
 * @since 6.1.3
 */
public class CmsHtmlDecorator extends CmsHtmlParser {
  ...
  /**
   * Splits a String into substrings along the provided delimiter list and returns the result as a
   * List of Substrings.
   *
   * <p>
   *
   ...
   *
   * <p>This is nescessary if decoration objects should be used for processing more than once. *
   */
  ...
  /**
   * Appends a text decoration to the output.
   *
   * <p>A lookup is made to find a text decoration for each word in the given text. If a text
   * decoration is found, the word will be decorated and added to the output. If no text decoration
   * is found, the word alone will be added to the output.
   *
  ...
  private void appendText(@RUntainted String text, String[] delimiters, boolean recursive) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_HTML_DECORATOR_APPEND_TEXT_2, m_config, text));
    }
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(text) && m_decorate) {
      ...
      for (int i = 0; i < wordCount; i++) {
        ...
        if (mustDecode(word, wordList, i)) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get().getBundle().key(Messages.LOG_HTML_DECORATOR_DECODED_WORD_1, word));
          }
        }
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_HTML_DECORATOR_DECORATION_FOUND_2, wordDecObj, word));
        }
          ...
          if ((decObj == null) && (wordDecObj == null)) {
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  Messages.get()
                      .getBundle()
                      .key(Messages.LOG_HTML_DECORATOR_DECORATION_APPEND_WORD_1, word));
            }
                ...
                Messages.get()
                    ...
                    .key(
                        ...
                        wordDecObj.getContentDecoration(
                            m_config, word, m_cms.getRequestContext().getLocale().toString())));
          }
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_HTML_DECORATOR_DECORATION_APPEND_ORIGINALTEXT_1, text));
      }
  ...
  /**
   * Checks if a word contains a given delimiter.
   *
   * <p>
   *
  ...
  /**
   * Checks if a word must be decoded.
   *
   * <p>The given word is compared to a negative list of words which must not be decoded.
   *
   * <p>
   *
   * @param word the word to test
  ...
  /**
   * Checks if a word starts with a given delimiter.
   *
   * <p>
   *
   * @param word the word to test
