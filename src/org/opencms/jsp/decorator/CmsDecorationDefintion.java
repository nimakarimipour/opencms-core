...
public class CmsDecorationDefintion {
  ...
  /**
   * Returns all different decoration configuration names (like "abbr" or "acronym") that are in the
   * config file pointed to by module parameter "configfile".
   *
   * <p>
   *
   * @param cms needed to access the decoration definition XML content
   * @return all different decoration configuration names (like "abbr" or "acronym") that are in the
   *     config file pointed to by module parameter "configfile"
  ...
  public static List<String> getDecorationDefinitionNames(CmsObject cms) throws CmsException {
    ...
    CmsModule module =
        OpenCms.getModuleManager().getModule("com.alkacon.opencms.extendeddecorator");
   ...
   * @param locale the locale to build the decoration bundle for. If no locale is given, a bundle of
   *     all locales is build
  ...
  public CmsDecorationBundle createDecorationBundle(CmsObject cms, Locale locale)
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DECORATION_DEFINITION_MAP_FILES_2, decorationMapFiles, locale));
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_DECORATION_DEFINITION_MAPS_2, decorationMaps, locale));
    }

    // now that we have all decoration maps we can build the decoration bundle
    // the bundele is depending on the locale, if a locale is given, only those decoration maps that
    // contain the
  ...
  /**
   * Creates a CmsDecorationBundle of text decoration to be used by the decorator based on a list of
   * decoration maps.
   *
   * <p>
   *
  ...
  /**
   * Gets the list of all decoartion map files that match to the current basename.
   *
   * <p>
   *
   * @param cms the CmsObject
  ...
  private List<CmsResource> getDecorationMapFiles(CmsObject cms) throws CmsException {
    ...
    List<CmsResource> resources =
        cms.readResources(
            CmsResource.getParentFolder(m_configurationFile), CmsResourceFilter.DEFAULT);
  ...
  /**
   * Creates a list of decoration map objects from a given list of decoration files.
   *
   * <p>
   *
   * @param cms the CmsObject
  ...
  private List<CmsDecorationMap> getDecorationMaps(
    ...
    while (i.hasNext()) {
      CmsResource res = i.next();
      try {
        CmsDecorationMap decMap =
            (CmsDecorationMap)
                CmsVfsMemoryObjectCache.getVfsMemoryObjectCache()
                    .getCachedObject(cms, res.getRootPath());
        if (decMap == null) {
          decMap = new CmsDecorationMap(cms, res, this);
          CmsVfsMemoryObjectCache.getVfsMemoryObjectCache()
              .putCachedObject(cms, res.getRootPath(), decMap);
        }
        ...
        if (LOG.isErrorEnabled()) {
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DECORATION_DEFINITION_CREATE_MAP_2, res.getName(), e));
        }
      }
    }
