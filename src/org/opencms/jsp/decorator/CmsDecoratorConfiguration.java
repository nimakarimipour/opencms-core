 ...
 *
 * <p>It uses uses the information of one or more <code>{@link CmsDecorationDefintion}</code> to
 * create the pre- and postfixs for text decorations.
 *
 * @since 6.1.3
 */
public class CmsDecoratorConfiguration implements I_CmsDecoratorConfiguration {
  ...
  /**
   * The list with all <code>{@link CmsDecorationDefintion}</code> instances parsed from the config
   * file.
   */
  ...
  /**
   * Adds decorations defined in a <code>{@link CmsDecorationDefintion}</code> object to the map of
   * all decorations.
   *
   * <p>
   *
   * @param decorationDefinition the <code>{@link CmsDecorationDefintion}</code> the decorations to
   *     be added
   * @throws CmsException if something goes wrong
   */
  public void addDecorations(CmsDecorationDefintion decorationDefinition) throws CmsException {

    m_decorations.putAll(
        decorationDefinition.createDecorationBundle(m_cms, m_configurationLocale).getAll());
  }
  ...
  public CmsDecorationDefintion getDecorationDefinition(CmsXmlContent configuration, int i) {
    ...
    String name =
        configuration
            .getValue(XPATH_DECORATION + "[" + i + "]/" + XPATH_NAME, m_configurationLocale)
            .getStringValue(m_cms);
    String markfirst =
        configuration
            .getValue(XPATH_DECORATION + "[" + i + "]/" + XPATH_MARKFIRST, m_configurationLocale)
            .getStringValue(m_cms);
    String pretext =
        configuration
            .getValue(XPATH_DECORATION + "[" + i + "]/" + XPATH_PRETEXT, m_configurationLocale)
            .getStringValue(m_cms);
    String posttext =
        configuration
            .getValue(XPATH_DECORATION + "[" + i + "]/" + XPATH_POSTTEXT, m_configurationLocale)
            .getStringValue(m_cms);
    String pretextfirst =
        configuration
            .getValue(XPATH_DECORATION + "[" + i + "]/" + XPATH_PRETEXTFIRST, m_configurationLocale)
            .getStringValue(m_cms);
    String posttextfirst =
        configuration
            .getValue(
                XPATH_DECORATION + "[" + i + "]/" + XPATH_POSTTEXTFIRST, m_configurationLocale)
            .getStringValue(m_cms);
    String filenname =
        configuration
            .getValue(XPATH_DECORATION + "[" + i + "]/" + XPATH_FILENAME, m_configurationLocale)
            .getStringValue(m_cms);
  ...
  /**
   * Returns the list with all <code>{@link CmsDecorationDefintion}</code> instances parsed from the
   * config file.
   *
   * <p>
   *
   * @return The list with all <code>{@link CmsDecorationDefintion}</code> instances parsed from the
   *     config file
   */
  ...
  public void init(CmsObject cms, String configFile, Locale locale) throws CmsException {
    ...
    if (m_configFile != null) {
      ...
      String uselocale =
          configuration.getValue(XPATH_USELOCALE, m_configurationLocale).getStringValue(m_cms);
      ...
      for (int i = 1; i <= excludeValuesCount; i++) {
        String excludeValue =
            configuration.getStringValue(
                m_cms, XPATH_EXCLUDE + "[" + i + "]", m_configurationLocale);
        m_excludes.add(excludeValue.toLowerCase());
      }

      // now read the exclude attributes
      int excludeAttrValuesCount =
          configuration.getIndexCount(XPATH_EXCLUDEATTR, m_configurationLocale);
      // get all the exclude attribute definitions
      for (int i = 1; i <= excludeAttrValuesCount; i++) {
        String excludeAttrValue =
            configuration.getStringValue(
                m_cms, XPATH_EXCLUDEATTR + "[" + i + "]", m_configurationLocale);
