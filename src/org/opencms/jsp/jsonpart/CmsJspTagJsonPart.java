 ...
 *
 * <p>It only makes sense to use this tag in combination with the servlet filter
 * org.opencms.jsp.jsonpart.CmsJsonPartFilter. This tag converts the text generated by its contained
 * JSP code and converts it into a special encoded form, which is then used by the filter to
 * generate JSON. The 'element' attribute on this tag can be used to control the JSON key which will
 * be used for the content.
 */
