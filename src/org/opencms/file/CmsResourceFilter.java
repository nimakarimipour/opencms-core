 ...
 *
 * <p>Using the constant filters provided by this class you can control "special" behaviour of
 * access to the VFS. For example, in the "Offline" project there can be deleted files, by using
 * this filter you can control if deleted files should be included in a result set or not.
 *
 * <p>
 *
 * @since 6.0.0
...
public final class CmsResourceFilter {
   ...
   *
   * <p>This filter uses the following rules:
   *
   * <ul>
   *   <li>Includes: Resources marked as deleted.
   *   <li>Includes: Resources outside the 'time window' set with release and expiration date.
   *   <li>Includes: Resources marked as 'invisible' using permissions.
   * </ul>
   */
  ...
  public static final CmsResourceFilter ALL_MODIFIED =
      ALL.addExcludeState(CmsResource.STATE_UNCHANGED);
   ...
   *
   * <ul>
   *   <li>Excludes: Resources marked as deleted.
   *   <li>Excludes: Resources outside the 'time window' set with release and expiration date.
   *   <li>Includes: Resources marked as 'invisible' using permissions.
   * </ul>
   */
  public static final CmsResourceFilter DEFAULT =
      ALL.addExcludeState(CmsResource.STATE_DELETED).addRequireTimerange();
   ...
   *   <li>Excludes: Resources marked as deleted.
   *   <li>Excludes: Resources outside the 'time window' set with release and expiration date.
   *   <li>Excludes: Resources marked as 'invisible' using permissions.
   * </ul>
   */
  ...
  public static final CmsResourceFilter IGNORE_EXPIRATION =
      ALL.addExcludeState(CmsResource.STATE_DELETED);

  /**
   * Filter to display only visible resources.
   *
   * <p>This filter used the following rules:
   *
   * <ul>
   *   <li>Includes: Resources marked as deleted.
   *   <li>Includes: Resources outside the 'time window' set with release and expiration date.
  ...
  public static final CmsResourceFilter ONLY_VISIBLE_NO_DELETED =
      ONLY_VISIBLE.addExcludeState(CmsResource.STATE_DELETED);
  ...
  /**
   * Indicates if the resource state (unchanged/new/deleted/modified) is filtered (true) or not
   * (false).
   */
   ...
   * @param type the required resource type
   * @return a filter requiring the given type
   * @deprecated Use {@link #requireType(I_CmsResourceType)} instead. Resource types should always
   *     be referenced either by its type class (preferred) or by type name. Use of int based
  ...
  /**
   * Returns an extended filter to guarantee all filtered resources ignoring the time range
   * (released and not expired) window.
   *
   * <p>This is the reverse of {@link #addRequireTimerange()}.
   *
   * <p>
   *
   ...
   * @param type the resource type to exclude
   * @return a filter excluding the given resource type
   * @deprecated Use {@link #addExcludeType(I_CmsResourceType)} instead. Resource types should
  ...
  /**
   * Returns an extended filter to restrict the results to resources that expire in the given
   * timerange.
   *
   * <p>
   *
  ...
  /**
   * Returns an extended filter to restrict the results to resources modified in the given
   * timerange.
   *
   * <p>
   *
  ...
  /**
   * Returns an extended filter to restrict the results to resources that are released in the given
   * timerange.
   *
   * <p>
   *
  ...
  /**
   * Returns an extended filter to guarantee all filtered resources are valid (released and not
   * expired).
   *
   * <p>
   *
   ...
   *
   * <p>If <code>-1</code> is given as type, the filter will not be extended to require a resource
   * type
   *
   * @param type the required resource type
   ...
   * @param type the required resource type
   * @return a filter requiring the given resource type
   * @deprecated Use {@link #addRequireType(I_CmsResourceType)} instead. Resource types should
   *     always be referenced either by its type class (preferred) or by type name. Use of int based
   *     resource type references will be discontinued in a future OpenCms release.
   */
  @Deprecated
   ...
   *
   * <p>If the result is <code>null</code>, then this flag is not set.
   *
   * <p>
   *
   ...
   *
   * <p>Please note that the "visible permission" setting of the filter is NOT checked in this
   * method since the permission information is not part of the resource. The visible permission
   * information in the filter will be used in the permission checks
   *
  ...
  /**
   * Returns the name of the filter, if it is one of the filters used as a constant of this class
   * and a default message otherwise.
   *
   * <p>
   *
  ...
  public String toString() {
    ...
    if (equals(CmsResourceFilter.ALL)) {
      return "ALL";
    } else if (equals(CmsResourceFilter.ALL_MODIFIED)) {
      return "ALL_MODIFIED";
    } else if (equals(CmsResourceFilter.DEFAULT)) {
      return "DEFAULT";
    } else if (equals(CmsResourceFilter.DEFAULT_FILES)) {
      return "DEFAULT_FILES";
    } else if (equals(CmsResourceFilter.DEFAULT_FOLDERS)) {
      return "DEFAULT_FOLDERS";
    } else if (equals(CmsResourceFilter.IGNORE_EXPIRATION)) {
      return "IGNORE_EXPIRATION";
    } else if (equals(CmsResourceFilter.ONLY_VISIBLE)) {
      return "ONLY_VISIBLE";
    } else if (equals(CmsResourceFilter.ONLY_VISIBLE_NO_DELETED)) {
