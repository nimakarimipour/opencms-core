...
public class CmsResourceTypeImage extends A_CmsResourceType {
  ...
  protected static class CmsImageAdjuster {
    ...
    /**
     * Value for EXIF Orientation tag that says that the image is oriented as-is (requires no
     * rotation/mirroring).
     */
    ...
    /**
     * Calculates the image size and adjusts the image dimensions (if required) accoring to the
     * configured image downscale settings.
     *
     * <p>The image dimensions are always calculated from the given image. The internal list of
     * properties is updated with a value for <code>
     * {@link CmsPropertyDefinition#PROPERTY_IMAGE_SIZE}</code> that contains the calculated image
     * dimensions.
     *
     * <p>
     */
    public void adjust() {
      ...
      CmsProperty p =
          new CmsProperty(CmsPropertyDefinition.PROPERTY_IMAGE_SIZE, null, scaler.toString());
     ...
     *
     * <p>Given a JPEG with the image data 'image' and the orientation 'exifOrientation', this
     * results in an image that, when written to a JPEG with an EXIF orientation of 1, will look the
     * same as the original image when viewed in an image viewer that supports EXIF orientation.
     *
    ...
    private BufferedImage createOrientedCopy(BufferedImage image, int exifOrientation) {
      ...
      // for each pixel, copy it to different coordinates (tx, ty) in the target depending on
      // orientation
      for (int x = 0; x < w; x++) {
        for (int y = 0; y < h; y++) {
          int tx, ty;
  ...
  /**
   * Helper class for parsing SVG sizes.
   *
   * <p>Note: This is *not* intended as a general purpose tool, it is only used for parsing SVG
   * sizes as part of the image.size property determination.
   */
  private static class SvgSize {
    ...
    /**
     * Gets the numeric value of the size.
     *
     * <p>
     *
     * @return the size
  ...
  /**
   * The value for the {@link CmsPropertyDefinition#PROPERTY_IMAGE_SIZE} property if resources in a
   * folder should never be downscaled.
   *
   * <p>
   */
  ...
  private static final String GALLERY_PREVIEW_PROVIDER =
      "org.opencms.ade.galleries.preview.CmsImagePreviewProvider";
   ...
   *
   * <p>If <code>null</code> is returned, image downscaling must not be used for the resource with
   * the given path. This may be the case if image downscaling is not configured at all, or if image
   * downscaling has been disabled for the parent folder by setting the folders property {@link
   * CmsPropertyDefinition#PROPERTY_IMAGE_SIZE} to the value {@link #PROPERTY_VALUE_UNLIMITED}.
   *
   * <p>
   *
  ...
  public static CmsImageScaler getDownScaler(CmsObject cms, @RUntainted String rootPath) {
    ...
    try {
      CmsProperty fileSizeProperty =
          cms.readPropertyObject(parentFolder, CmsPropertyDefinition.PROPERTY_IMAGE_SIZE, true);
  ...
  public CmsResource createResource(
    ...
    if (resourcename.toLowerCase().endsWith(".svg")) {
      ...
      // update the image scale adjuster - this will calculate the image dimensions and (optionally)
      // downscale the size
      adjuster.adjust();
  ...
  public String getGalleryPreviewProvider() {
    ...
    if (m_galleryPreviewProvider == null) {
      m_galleryPreviewProvider =
          getConfiguration()
              .getString(CONFIGURATION_GALLERY_PREVIEW_PROVIDER, GALLERY_PREVIEW_PROVIDER);
    }
  ...
  /**
   * @see org.opencms.file.types.A_CmsResourceType#importResource(org.opencms.file.CmsObject,
    ...
    if (resourcename.toLowerCase().endsWith(".svg")) {
      ...
      if (content != null) {
        try {
          ...
          // create a new image scale adjuster
          CmsImageAdjuster adjuster =
              new CmsImageAdjuster(content, resource.getRootPath(), properties, downScaler);
          // update the image scale adjuster - this will calculate the image dimensions and
          ...
          wrappedRes.setTypeId(
              OpenCms.getResourceManager()
                  .getResourceType(CmsResourceTypeBinary.getStaticTypeId())
                  .getTypeId());
          resource = wrappedRes.getResource();
        }
      }
    }
    return super.importResource(
        cms, securityManager, report, resourcename, resource, content, properties);
  }
   ...
   *     org.opencms.db.CmsSecurityManager, org.opencms.file.CmsResource, int, byte[],
   *     java.util.List)
   */
  @Override
   ...
   *     org.opencms.db.CmsSecurityManager, org.opencms.file.CmsFile)
   */
  @Override
  public CmsFile writeFile(CmsObject cms, CmsSecurityManager securityManager, CmsFile resource)
    ...
    if (CmsImageLoader.isEnabled()) {
      ...
      // create a new image scale adjuster
      CmsImageAdjuster adjuster =
          new CmsImageAdjuster(resource.getContents(), resource.getRootPath(), null, downScaler);
      // update the image scale adjuster - this will calculate the image dimensions and (optionally)
      // adjust the size
      adjuster.adjust();
      // continue with the updated content
  ...
  /**
   * Tries to use the viewbox from the SVG data to determine the image size and add it to the list
   * of properties.
   *
   * <p>
   *
  ...
  protected List<CmsProperty> tryAddImageSizeFromSvg(byte[] content, List<CmsProperty> properties) {
    ...
    try {
      ...
      if (node != null) {
        ...
        if ((width != null)
            && (height != null)
            && Objects.equals(width.getUnit(), height.getUnit())) {
          // If width and height are given and have the same units, just interpret them as pixels,
        ...
        if ((w > 0) && (h > 0)) {
          ...
          Map<String, CmsProperty> propsMap =
              properties == null ? new HashMap<>() : CmsProperty.toObjectMap(properties);
          propsMap.put(
