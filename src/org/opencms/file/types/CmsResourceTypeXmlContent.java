...
public class CmsResourceTypeXmlContent extends A_CmsResourceTypeLinkParseable {
  ...
  public static List<CmsResource> getModelFiles(
    ...
    try {
      ...
      String schema =
          resType.getConfiguration().get(CmsResourceTypeXmlContent.CONFIGURATION_SCHEMA);
      ...
      if (CmsStringUtil.isNotEmpty(masterFolder)) {
        ...
        if (CmsStringUtil.isNotEmpty(masterFolder) && cms.existsResource(masterFolder)) {
          ...
          CmsResourceFilter filter =
              CmsResourceFilter.ONLY_VISIBLE_NO_DELETED.addRequireType(resType.getTypeId());
  ...
  /**
   * Returns <code>true</code> in case the given resource is an XML content.
   *
   * <p>
   *
  ...
  /**
   * @see org.opencms.file.types.A_CmsResourceType#addConfigurationParameter(java.lang.String,
   *     java.lang.String)
   */
  @Override
  ...
  public CmsResource createResource(
    ...
    if ((content == null) || (content.length == 0)) {
      ...
      String modelUri =
          (String) cms.getRequestContext().getAttribute(CmsRequestContext.ATTRIBUTE_MODEL);
      ...
      if (modelUri != null) {
        ...
        newContent =
            CmsXmlContentFactory.createDocument(
                newCms, locale, OpenCms.getSystemInfo().getDefaultEncoding(), contentDefinition);
      }
    ...
    CmsResource resource =
        super.createResource(cms, securityManager, resourcename, content, properties);
  ...
  public String getGalleryPreviewProvider() {
    ...
    if (m_galleryPreviewProvider == null) {
      m_galleryPreviewProvider =
          getConfiguration()
              .getString(CONFIGURATION_GALLERY_PREVIEW_PROVIDER, DEFAULT_GALLERY_PREVIEW_PROVIDER);
    }
        ...
        if (cms.existsResource(m_schema)) {
          ...
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(
                      Messages.LOG_WARN_SCHEMA_RESOURCE_DOES_NOT_EXIST_2, m_schema, getTypeName()));
        }
      } catch (Throwable e) {
        // unable to unmarshal the XML schema configured
        LOG.error(
            Messages.get().getBundle().key(Messages.ERR_BAD_XML_SCHEMA_2, m_schema, getTypeName()),
            e);
      }
    }
  }
    ...
    if (isPossiblyDetailContent(resource)) {
      ...
      if (srcParent.equals(destParent) && !srcName.equals(destName)) {
        ...
        for (CmsResource page : detailOnlyPages) {
          ...
          String newPath =
              CmsStringUtil.joinPaths(CmsResource.getParentFolder(page.getRootPath()), destName);
            ...
            if ((lockRecord != null) && (lockRecord.getChange() == LockChange.locked)) {
              try {
                CmsLockUtil.tryUnlock(
                    rootCms, rootCms.readResource(page.getStructureId(), CmsResourceFilter.ALL));
              } catch (Exception e) {
                LOG.error(e.getLocalizedMessage(), e);
              }
            }
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            org.opencms.db.Messages.get()
                .getBundle()
                .key(org.opencms.db.Messages.ERR_READ_RESOURCE_1, cms.getSitePath(file)),
            e);
      }
      return Collections.emptyList();
    } finally {
        ...
        if (SearchContentType.CONTENT.equals(xmlContent.getHandler().getSearchContentType(value))) {
          ...
          try {
            if ((null != stringValue)
                ...
                && cms.existsResource(stringValue)) {
  ...
  public void setDateExpired(
    ...
    try {
      applyReverseAvailabilityMapping(
          cms, resource, CmsMappingResolutionContext.AttributeType.expiration, dateExpired);
    } catch (Exception e) {
  ...
  public void setDateReleased(
    ...
    try {
      applyReverseAvailabilityMapping(
          cms, resource, CmsMappingResolutionContext.AttributeType.release, dateReleased);
    } catch (Exception e) {
      LOG.error("Reverse availability mapping failed: " + e.getLocalizedMessage(), e);
    }
  ...
  protected Locale getLocaleForNewContent(
    ...
    Locale locale =
        (Locale)
            (cms.getRequestContext().getAttribute(CmsRequestContext.ATTRIBUTE_NEW_RESOURCE_LOCALE));
    if (locale != null) {
      return locale;
    }
    List<Locale> locales =
        OpenCms.getLocaleManager()
            .getDefaultLocales(cms, CmsResource.getParentFolder(resourcename));
  ...
  protected CmsLink getXsdLink(CmsObject cms, CmsXmlContent xmlContent) {
    ...
    if (schema.startsWith(CmsXmlEntityResolver.OPENCMS_SCHEME)) {
      ...
      // schema may not exist as a VFS file because it has just been cached (some test cases do
      // this)
      return null;
    }
    try {
      ...
      CmsLink xsdLink =
          new CmsLink(
              null, CmsRelationType.XSD, schemaRes.getStructureId(), schemaRes.getRootPath(), true);
      return xsdLink;
    } catch (CmsException e) {
      LOG.error(e.getLocalizedMessage(), e);
    }
  ...
  /**
   * Checks if the resource is possibly a detail content.
   *
   * <p>
   *
   * @param resource the resource to check
  ...
  private boolean applyReverseAvailabilityMapping(
    ...
    CmsXmlContentDefinition contentDef =
        CmsXmlContentDefinition.getContentDefinitionForResource(cms, resource);
   ...
   * @throws CmsException if something goes wrong
   */
  private List<CmsResource> getDetailContainerResources(CmsObject cms, CmsResource res)
      throws CmsException {
