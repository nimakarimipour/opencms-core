...
public class CmsResourceTypeFunctionConfig extends CmsResourceTypeXmlAdeConfiguration {
  ...
  public static final String FORMATTER_PATH =
      "/system/modules/org.opencms.base/formatters/function.jsp";
   ...
   *     <p>After writing the file, this method waits until the formatter configuration is update
   *     the next time.
   */
  @Override
  public CmsFile writeFile(CmsObject cms, CmsSecurityManager securityManager, CmsFile resource)
    ...
    String savingStr =
        (String) cms.getRequestContext().getAttribute(CmsContentService.ATTR_EDITOR_SAVING);
    CmsFile file = super.writeFile(cms, securityManager, resource);
    // Formatter configuration cache updates are asynchronous, but to be able to reload a container
    // page
    // element in the page editor directly after editing it and having it reflect the changes made
    // by the user
