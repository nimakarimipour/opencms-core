...
public class CmsResourceTypeXmlPage extends A_CmsResourceTypeLinkParseable {
  ...
  /**
   * Returns <code>true</code> in case the given resource is an XML page.
   *
   * <p>Internally this checks if the type id for the given resource is identical type id of the XML
   * page.
   *
   * <p>
   *
  ...
  /**
   * @see org.opencms.file.types.A_CmsResourceType#createResource(org.opencms.file.CmsObject,
      ...
      try {
        String encoding =
            CmsLocaleManager.getResourceEncoding(
                cms, cms.readResource(CmsResource.getParentFolder(resourcename)));
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_PROCESS_HTML_CONTENT_1, cms.getSitePath(file)),
            e);
      }
