 ...
 *
 * <p>Each file in the VFS must belong to an initialized resource type. The available resource type
 * are read during system startup from the configuration file <code>opencms-vfs.xml</code>.
 *
 * <p>Certain resource types may require special handling for certain operations. This is usually
 * required for write operations, or other operations that modify the VFS database. Therefore, the
 * {@link org.opencms.file.CmsObject} defers handling of this operations to implementations of this
 * interface.
 *
 * <p>If you implement a new resource type, it's a good idea to extend the abstract class {@link
 * org.opencms.file.types.A_CmsResourceType}.
 *
 * <p>Important: The {@link org.opencms.file.CmsObject} passes the {@link
 * org.opencms.db.CmsSecurityManager} object to implementations of this class. Using this object
 * correctly is key to the resource type operations. Mistakes made in the implementation of a
 * resource type can screw up the system security and the database structure, and make you unhappy.
 *
 * <p>
 *
 * @since 6.0.0
...
public interface I_CmsResourceType extends I_CmsConfigurationParameterHandler, Serializable {
  ...
  enum Formatter {
    ...
    /** The gallery list item formatter. */
    GALLERY_LIST(
        "formatter_gallery_list", "/system/workplace/editors/ade/default-list-formatter.jsp"),

    /** The gallery preview formatter. */
    GALLERY_PREVIEW(
        "formatter_gallery_preview", "/system/workplace/editors/ade/default-preview-formatter.jsp"),

    /** The gallery list item formatter. */
    PUBLISH_LIST(
        "formatter_publish_list", "/system/workplace/editors/ade/default-list-formatter.jsp"),
   ...
   *
   * <p>When uploading files into OpenCms, they must be mapped to the different OpenCms resource
   * types. The configuration, to map which extension to which resouce type is done in the OpenCms
   * VFS configuration.
   *
   ...
   * @see CmsObject#chtype(String, int)
   * @see CmsSecurityManager#chtype(org.opencms.file.CmsRequestContext, CmsResource, int)
   * @deprecated Use {@link #chtype(CmsObject, CmsSecurityManager, CmsResource, I_CmsResourceType)}
   *     instead. Resource types should always be referenced either by this type class (preferred)
   *     or by type name. Use of int based resource type references will be discontinued in a future
   *     OpenCms release.
   */
  @Deprecated
   ...
   * @see CmsSecurityManager#copyResource(org.opencms.file.CmsRequestContext, CmsResource, String,
   ...
   * @throws CmsException if something goes wrong
   * @throws CmsIllegalArgumentException if the <code>resource</code> argument is null or of length
   *     0
  ...
  void copyResourceToProject(
      CmsObject cms, CmsSecurityManager securityManager, CmsResource resource)
      throws CmsException, CmsIllegalArgumentException;
   ...
   * @see CmsSecurityManager#createResource(org.opencms.file.CmsRequestContext, String, int, byte[],
   *     List)
   */
   ...
   * @see CmsSecurityManager#createSibling(org.opencms.file.CmsRequestContext, CmsResource, String,
   *     List)
   */
   ...
   * @see CmsSecurityManager#deleteResource(org.opencms.file.CmsRequestContext, CmsResource,
  ...
  /**
   * Returns the default for the <code>cache</code> property setting of this resource type.
   *
   * <p>The <code>cache</code> property is used by the Flex cache implementation to build the cache
   * key that controls the caching behaviour of a resource.
   *
   * <p>If <code>null</code> is returnd, this is the same as turning the cache off by default for
   * this resource type.
   *
   * <p>
   *
   ...
   *
   * <p>This may be different from the instance class name in case the configured class could not be
   * instantiated. If the configured class is unavailable, an instance of <code>
   * {@link CmsResourceTypeUnknown}</code> is used. This enables the import of modules that contain
   * their own resource types classes (which are not available before the module is fully imported).
   *
   * <p>
   *
  ...
  /**
   * Returns the module name if this is an additional resource type which is defined in a module, or
   * <code>null</code>.
   *
   * <p>
   *
   ...
   * @deprecated Use this class or {@link #getTypeName()} instead. Resource types should always be
   *     referenced either by this type class (preferred) or by type name. Use of int based resource
   ...
   * @throws CmsException if something goes wrong
   * @see CmsSecurityManager#moveToLostAndFound(org.opencms.file.CmsRequestContext, CmsResource,
   *     boolean)
   * @see CmsObject#importResource(String, CmsResource, byte[], List)
   * @see CmsSecurityManager#importResource(org.opencms.file.CmsRequestContext, String, CmsResource,
   *     byte[], List, boolean)
   */
  ...
  /**
   * Special version of the configuration initialization used with resource types to set resource
   * type, id and class name, required for the XML configuration.
   *
   * <p><i>Please note:</i> Many resource types defined in the core have in fact a fixed resource
   * type and a fixed id. Configurable name and id is used only for certain types.
   *
   * <p>The provided named class must implement this interface (<code>{@link I_CmsResourceType}
   * </code>). Usually the provided class name should be the class name of the resource type
   * instance, but this may be different in special cases or configuration errors.
   *
   * <p>For example, if a module is imported that contains it's own resource type class files, the
   * included class file are usually not be available until the server is restarted. If the named
   * class given in the XML configuration (or module manifest.xml) is not available, or not
   * implementing <code>{@link I_CmsResourceType}</code>, then <code>{@link CmsResourceTypeUnknown}
   * </code> is used for the resource type instance.
   *
   * <p>
   *
   ...
   *
   * <p>This method will be called once during the OpenCms initialization processs. The VFS will
   * already be available at the time the method is called.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this resource type is direct editable.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this resource type is a folder.
   *
   * <p>
   *
   ...
   *
   * <p>Two resource types are considered identical if their names {@link #getTypeName()} <b>and</b>
   * their ids {@link #getTypeId()} are both the same.
   *
   * <p><b>Please note:</b> Two resource type are considered equal in the sense of {@link
   * Object#equals(Object)} if either if their names {@link #getTypeName()} <b>or</b> their ids
   * {@link #getTypeId()} are equal.
   *
   * <p>
   *
   ...
   * @see CmsSecurityManager#lockResource(org.opencms.file.CmsRequestContext, CmsResource,
   *     CmsLockType)
   */
  void lockResource(
      CmsObject cms, CmsSecurityManager securityManager, CmsResource resource, CmsLockType type)
      throws CmsException;
   ...
   * @see CmsSecurityManager#copyResourceToProject(org.opencms.file.CmsRequestContext, CmsResource)
   */
  void removeResourceFromProject(
      CmsObject cms, CmsSecurityManager securityManager, CmsResource resource)
      throws CmsException, CmsIllegalArgumentException;
   ...
   * @see CmsSecurityManager#replaceResource(org.opencms.file.CmsRequestContext, CmsResource, int,
   *     byte[], List)
   */
   ...
   * @see CmsSecurityManager#replaceResource(org.opencms.file.CmsRequestContext, CmsResource, int,
   *     byte[], List)
   * @deprecated Use {@link #replaceResource(CmsObject, CmsSecurityManager, CmsResource,
   *     I_CmsResourceType, byte[], List)} instead. Resource types should always be referenced
   *     either by this type class (preferred) or by type name. Use of int based resource type
   *     references will be discontinued in a future OpenCms release.
   */
  @Deprecated
  void replaceResource(
      CmsObject cms,
  ...
  void restoreResource(
      CmsObject cms, CmsSecurityManager securityManager, CmsResource resource, int version)
      throws CmsException;
  ...
  /**
   * Sets the folder for adjusting links after copying the copy-resources.
   *
   * <p>
   *
   ...
   * @see CmsSecurityManager#setDateLastModified(org.opencms.file.CmsRequestContext, CmsResource,
   *     long)
   */
  ...
  /**
   * Sets the module name if this is an additional resource type which is defined in a module, or
   * <code>null</code>.
   *
   * <p>
   *
   * @param moduleName the module name if this is an additional resource type which is defined in a
   *     module, or <code>null</code>
   */
   ...
   */
  void undelete(
      CmsObject cms, CmsSecurityManager securityManager, CmsResource resource, boolean recursive)
      throws CmsException;
   ...
   *     {@link org.opencms.file.CmsResource.CmsResourceUndoMode}#UNDO_XXX</code> constants
   * @throws CmsException if something goes wrong
   ...
   *
   * <p>Applies only to resources of type <code>{@link CmsFile}</code> that have a binary content
   * attached.
   *
   * <p>
   *
   ...
   * @see CmsSecurityManager#writePropertyObject(org.opencms.file.CmsRequestContext, CmsResource,
   *     CmsProperty)
   */
  void writePropertyObject(
      CmsObject cms, CmsSecurityManager securityManager, CmsResource resource, CmsProperty property)
      throws CmsException;
   ...
   * @see CmsSecurityManager#writePropertyObjects(org.opencms.file.CmsRequestContext, CmsResource,
   *     List)
   */
