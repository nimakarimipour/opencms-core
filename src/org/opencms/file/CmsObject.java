 ...
 *
 * <p>It encapsulates user identification and permissions. Think of it as an initialized "shell" to
 * access the OpenCms VFS. Every call to a method here will be checked for user permissions
 * according to the <code>{@link org.opencms.file.CmsRequestContext}</code> this CmsObject instance
 * was created with.
 *
 * <p>From a JSP page running in OpenCms, use <code>
 * {@link org.opencms.jsp.CmsJspBean#getCmsObject()}</code> to gain access to the current users
 * CmsObject. Usually this is done with a <code>{@link org.opencms.jsp.CmsJspActionElement}</code>.
 *
 * <p>To generate a new instance of this class in your application, use <code>
 * {@link org.opencms.main.OpenCms#initCmsObject(String)}</code>. The argument String should be the
 * name of the guest user, usually "Guest" and more formally obtained by <code>
 * {@link org.opencms.db.CmsDefaultUsers#getUserGuest()}</code>. This will give you an initialized
 * context with guest user permissions. Then use <code>{@link CmsObject#loginUser(String, String)}
 * </code> to log in the user you want. Obviously you need the password for the new user. You should
 * never try to create an instance of this class using the constructor, this is reserved for
...
public final class CmsObject {
   ...
   *
   * <p><b>Please note:</b> This constructor is internal to OpenCms and not for public use. If you
   * want to create a new instance of a <code>{@link CmsObject}</code> in your application, use
   * <code>{@link org.opencms.main.OpenCms#initCmsObject(String)}</code>.
   *
   * <p>
   *
   ...
   */
  public void addRelationToResource(CmsResource resource, CmsResource target, String type)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void addRelationToResource(String resourceName, String targetPath, String type)
      throws CmsException {
  ...
  /**
   * Convenience method to add the site root from the current user's request context to the given
   * resource name.
   *
   * <p>
   *
  ...
  /**
   * This method works just like {@link CmsObject#adjustLinks(String, String)}, but you can specify
   * multiple source files, and the target folder is interpreted as the folder into which the source
  ...
  /**
   * This method works just like {@link CmsObject#adjustLinks(String, String)}, but instead of
   * specifying a single source and target folder, you can specify multiple sources and the
   * corresponding targets in a map of strings.
   *
   * @param sourceTargetMap a map with the source files as keys and the corresponding targets as
   *     values
   * @param targetParentFolder the folder into which the source files have been copied
   * @throws CmsException if something goes wrong
   */
  public void adjustLinks(Map<String, String> sourceTargetMap, String targetParentFolder)
      throws CmsException {
  ...
  /**
   * Adjusts all links in the target folder that point to the source folder so that they are kept
   * "relative" in the target folder where possible.
   *
   * <p>If a link is found from the target folder to the source folder, then the target folder is
   * checked if a target of the same name is found also "relative" inside the target Folder, and if
   * so, the link is changed to that "relative" target. This is mainly used to keep relative links
   * inside a copied folder structure intact.
   *
   * <p>Example: Image we have folder /folderA/ that contains files /folderA/x1 and /folderA/y1. x1
   * has a link to y1 and y1 to x1. Now someone copies /folderA/ to /folderB/. So we end up with
   * /folderB/x2 and /folderB/y2. Because of the link mechanism in OpenCms, x2 will have a link to
   * y1 and y2 to x1. By using this method, the links from x2 to y1 will be replaced by a link x2 to
   * y2, and y2 to x1 with y2 to x2.
   *
   * <p>Link replacement works for links in XML files as well as relation only type links.
   *
   ...
   *     <ul>
   *       <li><code>{@link I_CmsPrincipal#PRINCIPAL_USER}</code>
   *       <li><code>{@link I_CmsPrincipal#PRINCIPAL_GROUP}</code>
   *     </ul>
   *
    ...
    try {
      ...
      acEntry =
          new CmsAccessControlEntry(
              res.getResourceId(), principal.getId(), allowedPermissions, deniedPermissions, flags);
      ...
      if (principalName.equalsIgnoreCase(CmsAccessControlEntry.PRINCIPAL_ALL_OTHERS_NAME)) {
        ...
        acEntry =
            new CmsAccessControlEntry(
                res.getResourceId(), role.getId(), allowedPermissions, deniedPermissions, flags);
   ...
   * user.
   *
   * <p>This is the "steal lock" operation.
   *
   * <p>
   *
  ...
  public List<CmsResource> changeResourcesInFolderWithProperty(
      String resourcename, @RUntainted String property, @RUntainted String oldValue, @RUntainted String newValue, boolean recursive)
      throws CmsException {
    ...
    return m_securityManager.changeResourcesInFolderWithProperty(
        m_context, resource, property, oldValue, newValue, recursive);
  }

  /**
   * Checks if a login for the given user name and password would work without taking two-factor
   * authentication into account.
   *
   * <p>If this succeeds, it does not actually log the CmsObject in. But if it fails, it throws an
   * exception and increments the failed login counter for the given user.
   *
   ...
   * <p>
   *
   * @param resourcename the name of the resource to change the flags for (full current site
   *     relative path)
   ...
   * <p>
   *
   * @param resourcename the name of the resource to change the type for (full current site relative
   *     path)
   * @param type the new resource type for this resource
   * @throws CmsException if something goes wrong
   */
   ...
   * @param type the new resource type for this resource
   * @throws CmsException if something goes wrong
   * @deprecated Use {@link #chtype(String, I_CmsResourceType)} instead. Resource types should
   *     always be referenced either by its type class (preferred) or by type name. Use of int based
   ...
   *
   * <p>Siblings will be treated according to the <code>
   * {@link org.opencms.file.CmsResource#COPY_PRESERVE_SIBLING}</code> mode.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   * @throws CmsIllegalArgumentException if the <code>destination</code> argument is null or of
   *     length 0
   ...
   *
   * <ul>
   *   <li><code>{@link CmsResource#COPY_AS_NEW}</code>
   *   <li><code>{@link CmsResource#COPY_AS_SIBLING}</code>
   *   <li><code>{@link CmsResource#COPY_PRESERVE_SIBLING}</code>
   * </ul>
   *
   * <p>
   *
  ...
  public void copyResource(
      String source, String destination, CmsResource.CmsResourceCopyMode siblingMode)
      throws CmsException, CmsIllegalArgumentException {

    CmsResource resource = readResource(source, CmsResourceFilter.IGNORE_EXPIRATION);
    getResourceType(resource)
        .copyResource(this, m_securityManager, resource, destination, siblingMode);
  }

  /**
   * Copies a resource to the current project of the user.
   *
   * <p>This is used to extend the current users project with the specified resource, in case that
   * the resource is not yet part of the project. The resource is not really copied like in a
   * regular copy operation, it is in fact only "enabled" in the current users project.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param resourcename the name of the resource to copy to the current project (full current site
   *     relative path)
   * @throws CmsException if something goes wrong
   */
   ...
   * @throws CmsException if operation was not successful
   */
  public CmsGroup createGroup(@RUntainted String groupFqn, String description, int flags, String parent)
      throws CmsException {
  ...
  public CmsProject createProject(
      String name, String description, String groupname, String managergroupname)
      throws CmsException {

    return m_securityManager.createProject(
        m_context, name, description, groupname, managergroupname, CmsProject.PROJECT_TYPE_NORMAL);
  }
  ...
  public CmsProject createProject(
    ...
    return m_securityManager.createProject(
        m_context, name, description, groupname, managergroupname, projecttype);
  }
  ...
  /**
   * Creates a resource with the given properties and content. Will throw an exception, if a
   * resource with the given name already exists.
   *
   * <p>
   *
  ...
  public CmsResource createResource(
      String sitePath, CmsResource resource, byte[] content, List<CmsProperty> properties)
      throws CmsException {
    ...
    return m_securityManager.createResource(
        m_context, m_context.addSiteRoot(sitePath), resource, content, properties);
  }
   ...
   * @throws CmsException if something goes wrong
   * @throws CmsIllegalArgumentException if the given <code>resourcename</code> is null or of length
   *     0
   * @see #createResource(String, int, byte[], List)
   */
  public CmsResource createResource(@RUntainted String resourcename, I_CmsResourceType type)
      throws CmsException, CmsIllegalArgumentException {
   ...
   */
  public CmsResource createResource(
      @RUntainted String resourcename, I_CmsResourceType type, byte[] content, List<CmsProperty> properties)
      throws CmsException, CmsIllegalArgumentException {
   ...
   *     0
   * @see #createResource(String, int, byte[], List)
   * @deprecated Use {@link #createResource(String, I_CmsResourceType)} instead. Resource types
   *     should always be referenced either by its type class (preferred) or by type name. Use of
   *     int based resource type references will be discontinued in a future OpenCms release.
   */
  @Deprecated
  public CmsResource createResource(@RUntainted String resourcename, int type)
      throws CmsException, CmsIllegalArgumentException {

    return createResource(
        resourcename, getResourceType(type), new byte[0], new ArrayList<CmsProperty>(0));
  }
   ...
   * @throws CmsIllegalArgumentException if the <code>resourcename</code> argument is null or of
   *     length 0
   * @deprecated Use {@link #createResource(String, I_CmsResourceType, byte[], List)} instead.
   *     Resource types should always be referenced either by its type class (preferred) or by type
   *     name. Use of int based resource type references will be discontinued in a future OpenCms
   *     release.
   */
  @Deprecated
  public CmsResource createResource(
  ...
  public CmsResource createSibling(String source, String destination, List<CmsProperty> properties)
    ...
    return getResourceType(resource)
        .createSibling(this, m_securityManager, resource, destination, properties);
  }
  ...
  public CmsUser createUser(
      String userFqn, String password, String description, Map<String, Object> additionalInfos)
      throws CmsException {
  ...
  public void deleteHistoricalVersions(
      @RUntainted int versionsToKeep, @RUntainted int versionsDeleted, @RUntainted long timeDeleted, I_CmsReport report)
      throws CmsException {

    m_securityManager.deleteHistoricalVersions(
        m_context, versionsToKeep, versionsDeleted, timeDeleted, report);
  }
   ...
   * Deletes a project.
   *
   * <p>All resources inside the project have to be be reset to their online state.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public void deleteRelationsFromResource(CmsResource resource, CmsRelationFilter filter)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void deleteRelationsFromResource(String resourceName, CmsRelationFilter filter)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void deleteResource(CmsResource res, CmsResourceDeleteMode deletePreserveSiblings)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void deleteResource(@RUntainted String resourcename, CmsResource.CmsResourceDeleteMode siblingMode)
      throws CmsException {
  ...
  public void deleteStaticExportPublishedResource(
      String resourceName, int linkType, String linkParameter) throws CmsException {

    m_securityManager.deleteStaticExportPublishedResource(
        m_context, resourceName, linkType, linkParameter);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsAccessControlEntry> getAccessControlEntries(String resourceName)
      throws CmsException {
   ...
   *
   * @param resourceName the name of the resource
   * @param getInherited <code>true</code>, if inherited access control entries should be returned,
   *     too
  ...
  public List<CmsAccessControlEntry> getAccessControlEntries(
      String resourceName, boolean getInherited) throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public CmsAccessControlList getAccessControlList(String resourceName, boolean inheritedOnly)
      throws CmsException {
  ...
  /**
   * Returns a list of child resources to the given resource that can not be locked by the current
   * user.
   *
   * <p>
   *
   * @param resource the resource
   * @return a list of child resources to the given resource that can not be locked by the current
   *     user
   ...
   * @throws CmsException if operation was not successful
   */
  public List<CmsGroup> getChildren(String groupname, boolean includeSubChildren)
      throws CmsException {
   ...
   *
   * <p>The detail view URI of a content element consists of its detail page URI and the detail name
   ...
   * @throws CmsException if something goes wrong
   */
  public @RUntainted String getDetailName(CmsResource res, Locale locale, List<Locale> defaultLocales)
      throws CmsException {
   ...
   *
   * <p>The result is filtered according to the rules of the <code>{@link CmsResourceFilter#DEFAULT}
   * </code> filter.
   *
   * <p>
   *
   * @param resourcename the full current site relative path of the resource to return the child
   *     resources for
   ...
   * Returns all file resources contained in a folder.
   *
   * <p>With the <code>{@link CmsResourceFilter}</code> provided as parameter you can control if you
   * want to include deleted, invisible or time-invalid resources in the result.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> getFilesInFolder(String resourcename, CmsResourceFilter filter)
      throws CmsException {
   ...
   * @throws CmsException if operation was not successful
   */
  public List<CmsGroup> getGroupsOfUser(String username, boolean directGroupsOnly)
      throws CmsException {
  ...
  public List<CmsGroup> getGroupsOfUser(
      String username, boolean directGroupsOnly, boolean includeOtherOus) throws CmsException {

    return getGroupsOfUser(
        username, directGroupsOnly, includeOtherOus, m_context.getRemoteAddress());
  }
   ...
   */
  public List<CmsGroup> getGroupsOfUser(
      @RUntainted String username, boolean directGroupsOnly, boolean includeOtherOus, String remoteAddress)
      throws CmsException {
   ...
   *
   * <p>If the resource is waiting to be publish you might get a lock of type {@link
   * CmsLockType#PUBLISH}.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param resourcename the name if the resource to get the lock state for (full current site
   *     relative path)
   ...
   * @throws CmsException if operation was not successful
   */
  public List<CmsResource> getLockedResources(CmsResource resource, CmsLockFilter filter)
      throws CmsException {
   ...
   * @throws CmsException if operation was not successful
   */
  public List<String> getLockedResources(String resourceName, CmsLockFilter filter)
      throws CmsException {
  ...
  /**
   * Returns all locked resources within a folder or matches the lock of the given resource, but
   * uses a cache for resource lookup.
   *
   * <p>
   *
   * @param resource the resource to check
  ...
  public List<CmsResource> getLockedResourcesWithCache(
      CmsResource resource, CmsLockFilter filter, Map<String, CmsResource> cache)
      throws CmsException {
   ...
   * <p>
   *
   * @param resourcename the name of the resource to get the "lost and found" name for (full current
   *     site relative path)
   ...
   *
   * <p>You should have view/read permissions on the given resource.
   *
   * <p>You may become source and/or target paths to resource you do not have view/read permissions
   * on.
   *
   * <p>
   *
   ...
   * @param filter the filter to match the relation
   * @return a List containing all {@link org.opencms.relations.CmsRelation} objects for the given
   *     resource matching the given filter
   * @throws CmsException if something goes wrong
   ...
   *
   * <p>This request context is used to authenticate the user for all OpenCms operations. It also
   * contains the request runtime settings, e.g. about the current site this request was made on.
   *
   * <p>
   *
  ...
  public Set<CmsResource> getResourcesForPrincipal(
      CmsUUID principalId, CmsPermissionSet permissions, boolean includeAttr) throws CmsException {

    return m_securityManager.getResourcesForPrincipal(
        getRequestContext(), principalId, permissions, includeAttr);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public @RUntainted List<CmsResource> getResourcesInFolder(String resourcename, CmsResourceFilter filter)
      throws CmsException {
   ...
   *
   * <p>The full root path of a resource is always available using <code>
   * {@link CmsResource#getRootPath()}</code>. From this name this method cuts of the current site
   * root using <code>{@link CmsRequestContext#removeSiteRoot(String)}</code>.
   *
   * <p>If the resource root path does not start with the current site root, it is left untouched.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> getSubFolders(String resourcename, CmsResourceFilter filter)
      throws CmsException {
   ...
   *
   * <p>Users that are "indirectly" in the group are not returned in the result.
   *
   * <p>
   *
   ...
   * @throws CmsException if operation was not successful
   */
  public List<CmsUser> getUsersOfGroup(String groupname, boolean includeOtherOus)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public boolean hasPermissions(CmsResource resource, CmsPermissionSet requiredPermissions)
      throws CmsException {

    return m_securityManager
        .hasPermissions(m_context, resource, requiredPermissions, true, CmsResourceFilter.ALL)
        .isAllowed();
  }
   ...
   * @param resource the resource to check the permissions for
   * @param requiredPermissions the set of permissions to check for
   * @param checkLock if <code>true</code> the lock status of the resource is checked for write
   *     operations and the resource needs be locked by the current user so that the test is passed,
  ...
  public boolean hasPermissions(
    ...
    return I_CmsPermissionHandler.PERM_ALLOWED
        == m_securityManager.hasPermissions(
            m_context, resource, requiredPermissions, checkLock, filter);
  }
  ...
  public void importAccessControlEntries(
      CmsResource resource, List<CmsAccessControlEntry> acEntries) throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void importRelation(String resourceName, String targetPath, String relationType)
      throws CmsException {
   ...
   *
   * <p>If a resource already exists in the VFS (i.e. has the same name and same id) it is replaced
   * by the imported resource.
   *
   * <p>If a resource with the same name but a different id exists, the imported resource is
   * (usually) moved to the "lost and found" folder.
   *
   * <p>
   *
  ...
  public CmsResource importResource(
      String resourcename, CmsResource resource, byte[] content, List<CmsProperty> properties)
      throws CmsException {

    return getResourceType(resource)
        .importResource(this, m_securityManager, null, resourcename, resource, content, properties);
  }
  ...
  public CmsResource importResource(
    ...
    return getResourceType(resource)
        .importResource(
            this, m_securityManager, report, resourcename, resource, content, properties);
  }
   ...
   * Locks a resource.
   *
   * <p>This will be an exclusive, persistent lock that is removed only if the user unlocks it.
   *
   * <p>
   *
  ...
  public void lockResource(CmsResource resource) throws CmsException {
    ...
    getResourceType(resource)
        .lockResource(this, m_securityManager, resource, CmsLockType.EXCLUSIVE);
  }
   ...
   *
   * <p>This will be an exclusive, temporary lock valid only for the current users session. Usually
   * this should not be used directly, this method is intended for the OpenCms workplace only.
   *
   * <p>
   *
   * @param resource the resource to lock
  ...
  public void lockResourceTemporary(CmsResource resource) throws CmsException {
    ...
    getResourceType(resource)
        .lockResource(this, m_securityManager, resource, CmsLockType.TEMPORARY);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public String loginUser(String username, String password, CmsSecondFactorInfo code)
      throws CmsException {
  ...
  public String loginUser(
      String username, String password, CmsSecondFactorInfo code, String remoteAddress)
      throws CmsException {

    // login the user
    CmsUser newUser =
        m_securityManager.loginUser(m_context, username, password, code, remoteAddress);
   ...
   * @throws CmsException if the login was not successful
   */
  public String loginUser(String username, String password, String remoteAddress)
      throws CmsException {
   ...
   *
   * <p>A move operation in OpenCms is always a copy (as sibling) followed by a delete, this is a
   * result of the online/offline structure of the OpenCms VFS. This way you can see the deleted
   * files/folders in the offline project, and you will be unable to undelete them.
   *
   * <p>
   *
   ...
   *
   * <p>The "lost and found" folder is a special system folder.
   *
   * <p>This operation is used e.g. during import of resources when a resource with the same name
   * but a different resource ID already exists in the VFS. In this case, the imported resource is
   * moved to the "lost and found" folder.
   *
   * <p>
   *
   * @param resourcename the name of the resource to move to "lost and found" (full current site
   *     relative path)
   ...
   * @throws CmsException if something goes wrong
   */
  public List<I_CmsHistoryResource> readAllAvailableVersions(CmsResource resource)
      throws CmsException {
   ...
   * @throws CmsException if operation was not successful
   */
  public List<I_CmsHistoryResource> readAllAvailableVersions(String resourceName)
      throws CmsException {
   ...
   *
   * <p>If no folder with the requested resource type is found, null is returned.
   *
   * <p>
   *
   * @param resourcename the name of the resource to start (full current site relative path)
   ...
   * @throws CmsException if something goes wrong
   */
  public String readBestUrlName(CmsUUID id, Locale locale, List<Locale> defaultLocales)
      throws CmsException {
   ...
   * Returns the default resource for the given folder.
   *
   * <p>If the given resource name or id identifies a file, then this file is returned.
   *
   ...
   * @throws CmsSecurityException if the user has no permissions to read the resulting file
   */
  public CmsResource readDefaultFile(@RUntainted String resourceNameOrID)
   ...
   * @throws CmsSecurityException if the user has no permissions to read the resulting file
   */
  public CmsResource readDefaultFile(@RUntainted String resourceNameOrID, CmsResourceFilter filter)
      throws CmsException, CmsSecurityException {
   ...
   *
   * <p>The result list may include resources with the same name of resources (with different id's).
   *
   * <p>Use in conjunction with the {@link #restoreDeletedResource(CmsUUID)} method.
   *
   * <p>
   *
   ...
   */
  public List<I_CmsHistoryResource> readDeletedResources(String resourcename, boolean readTree)
      throws CmsException {
  ...
  /**
   * Reads a file resource (including it's binary content) from the VFS, for the given resource
   * (this may also be an historical version of the resource).
   *
   * <p>In case the input {@link CmsResource} object already is a {@link CmsFile} with contents
   * available, it is casted to a file and returned unchanged. Otherwise the file is read from the
   * VFS.
   *
   * <p>In case you do not need the file content, use <code>{@link #readResource(String)}</code> or
   * <code>{@link #readResource(String, CmsResourceFilter)}</code> instead.
   *
   * <p>No resource filter is applied when reading the resource, since we already have a full
   * resource instance and assume we just want the content for that instance. In case you need to
   * apply a filter, use {@link #readFile(String, CmsResourceFilter)} instead.
   *
   * <p>
   *
  ...
  /**
   * Reads a file resource (including it's binary content) from the VFS, using the <code>
   * {@link CmsResourceFilter#DEFAULT}</code> filter.
   *
   * <p>In case you do not need the file content, use <code>{@link #readResource(String)}</code>
   * instead.
   *
   * <p>
   *
   ...
   * {@link #readResource(String, CmsResourceFilter)}</code> instead.
   *
   * <p>The specified filter controls what kind of resources should be "found" during the read
   * operation. This will depend on the application. For example, using <code>
   * {@link CmsResourceFilter#DEFAULT}</code> will only return currently "valid" resources, while
  ...
  /**
   * Reads a folder resource from the VFS, using the <code>{@link CmsResourceFilter#DEFAULT}</code>
   * filter.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong, ie. {@link
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsProperty> readHistoryPropertyObjects(I_CmsHistoryResource resource)
      throws CmsException {
   ...
   *
   * <p>If there is no structure id mapped to the given name, null will be returned.
   *
   * <p>However if the parameter is a string which represents a valid uuid, it will be directly
   * returned as a {@link CmsUUID} instance.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public CmsUUID readIdForUrlName(@RUntainted String name) throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> readPath(CmsResource resource, CmsResourceFilter filter)
      throws CmsException {
   ...
   * @return the list of all resource names (root paths), as <code>{@link String}</code> objects
   ...
   * @return all <code>{@link CmsResource}</code>s of a project that match a given criteria from the
   *     VFS
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> readProjectView(CmsUUID projectId, CmsResourceState state)
      throws CmsException {
   ...
   *
   * <p>This method is more efficient then using <code>
   * {@link CmsObject#readPropertyObject(String, String, boolean)}</code> if you already have an
   * instance of the resource to look up the property from.
   *
   * <p>
   *
   * @param resource the resource where the property is attached to
   * @param property the property name
   * @param search if true, the property is searched on all parent folders of the resource, if it's
   *     not found attached directly to the resource
  ...
  /**
   * Reads the locale specific version of a property object from a resource specified by a property
   * name.
   *
   * <p>Returns <code>{@link CmsProperty#getNullProperty()}</code> if the property is not found.
   *
  ...
  public CmsProperty readPropertyObject(
      CmsResource resource, String property, boolean search, Locale locale) throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public CmsProperty readPropertyObject(String resourcePath, String property, boolean search)
      throws CmsException {
   ...
   */
  public CmsProperty readPropertyObject(
      String resourcePath, String property, boolean search, Locale locale) throws CmsException {
   ...
   *
   * <p>This method is more efficient then using <code>
   * {@link CmsObject#readPropertyObjects(String, boolean)}</code> if you already have an instance
   * of the resource to look up the property from.
   *
   ...
   * @param search if <code>true</code>, the properties of all parent folders of the resource are
   *     merged with the resource properties.
   * @return a list of <code>{@link CmsProperty}</code> objects
   * @throws CmsException if something goes wrong
   */
  public List<CmsProperty> readPropertyObjects(CmsResource resource, boolean search)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsProperty> readPropertyObjects(String resourcePath, boolean search)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsPublishedResource> readPublishedResources(CmsUUID publishHistoryId)
      throws CmsException {
  ...
  /**
   * Reads a resource from the VFS, using the <code>{@link CmsResourceFilter#DEFAULT}</code> filter.
   *
   ...
   * file, use <code>{@link #readFile(CmsResource)}</code>.
   *
   * <p>
   *
   ...
   * @see #readFolder(String, CmsResourceFilter)
   */
  public CmsResource readResource(CmsUUID structureID, CmsResourceFilter filter)
      throws CmsException {
  ...
  /**
   * Reads the historical resource with the given version for the resource given the given structure
   * id.
   *
   * <p>A resource may be of type <code>{@link CmsFile}</code> or <code>{@link CmsFolder}</code>. In
   * case of a file, the resource will not contain the binary file content. Since reading the binary
   ...
   * file, use <code>{@link #readFile(CmsResource)}</code>.
   *
   * <p>Please note that historical versions are just generated during publishing, so the first
   * version with version number 1 is generated during publishing of a new resource (exception is a
   * new sibling, that may also contain some relevant versions of already published siblings) and
   * the last version available is the version of the current online resource.
   *
   * <p>
   *
   * @param structureID the structure ID of the resource to read
   ...
   * @see #readFolder(String, CmsResourceFilter)
   */
  public CmsResource readResource(String resourcename, CmsResourceFilter filter)
      throws CmsException {
  ...
  /**
   * Reads all resources below the given resource matching the filter criteria, including the full
   * tree below the path only in case the <code>readTree</code> parameter is <code>true</code>.
   *
   * <p>
   *
  ...
  public List<CmsResource> readResources(
      CmsResource resource, CmsResourceFilter filter, boolean readTree) throws CmsException {
   ...
   * @see #readResources(String, CmsResourceFilter, boolean)
   */
  public @RUntainted List<CmsResource> readResources(String resourcename, CmsResourceFilter filter)
      throws CmsException {
  ...
  public @RUntainted List<CmsResource> readResources(
      String resourcename, CmsResourceFilter filter, boolean readTree) throws CmsException {
   ...
   *
   * <p>Will use the {@link CmsResourceFilter#ALL} resource filter.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> readResourcesWithProperty(String propertyDefinition)
      throws CmsException {
   ...
   * @param propertyDefinition the name of the property to check for
   * @return all <code>{@link CmsResource}</code> objects that have a value set for the specified
   *     property in the given path.
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> readResourcesWithProperty(String path, String propertyDefinition)
      throws CmsException {
  ...
  public List<CmsResource> readResourcesWithProperty(
      String path, String propertyDefinition, String value) throws CmsException {
    ...
    return m_securityManager.readResourcesWithProperty(
        m_context, resource, propertyDefinition, value, CmsResourceFilter.ALL);
  }
   ...
   * property set are returned.
   *
   * <p>Will use the given resource filter.
   *
   * <p>
   *
   * @param path the folder to get the resources with the property from
   ...
   */
  public List<CmsResource> readResourcesWithProperty(
      String path, String propertyDefinition, String value, CmsResourceFilter filter)
      throws CmsException {

    CmsResource resource = readResource(path, CmsResourceFilter.IGNORE_EXPIRATION);
    return m_securityManager.readResourcesWithProperty(
        m_context, resource, propertyDefinition, value, filter);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> readSiblings(CmsResource resource, CmsResourceFilter filter)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsResource> readSiblings(String resourcename, CmsResourceFilter filter)
      throws CmsException {
  ...
  public List<CmsResource> readSiblingsForResourceId(CmsUUID resourceId, CmsResourceFilter filter)
    ...
    CmsResource pseudoResource =
        new CmsResource(
            null, resourceId, null, 0, false, 0, null, null, 0, null, 0, null, 0, 0, 0, 0, 0, 0);
   ...
   * @throws CmsException if something goes wrong
   */
  public @RUntainted List<@RUntainted String> readStaticExportResources(int parameterResources, long timestamp)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsUrlNameMappingEntry> readUrlNameMappings(CmsUrlNameMappingFilter filter)
      throws CmsException {
   ...
   *
   * <p>This is used to reduce the current users project with the specified resource, in case that
   * the resource is already part of the project. The resource is not really removed like in a
   * regular copy operation, it is in fact only "disabled" in the current users project.
   *
   * <p>
   *
   * @param resourcename the name of the resource to remove to the current project (full current
   *     site relative path)
   * @throws CmsException if something goes wrong
   */
  ...
  /**
   * Renames a resource to the given destination name, this is identical to a <code>move</code>
   * operation.
   *
   * <p>
   *
   ...
   */
  public void renameResource(String source, @RUntainted String destination) throws CmsException {
  ...
  public void replaceResource(
      String resourcename, I_CmsResourceType type, byte[] content, List<CmsProperty> properties)
      throws CmsException {

    CmsResource resource = readResource(resourcename, CmsResourceFilter.IGNORE_EXPIRATION);
    getResourceType(resource)
        .replaceResource(this, m_securityManager, resource, type, content, properties);
  }
   ...
   * @param properties the new properties of the resource
   * @throws CmsException if something goes wrong
   * @deprecated Use {@link #replaceResource(String, I_CmsResourceType, byte[], List)} instead.
   *     Resource types should always be referenced either by its type class (preferred) or by type
   *     name. Use of int based resource type references will be discontinued in a future OpenCms
   *     release.
   */
   ...
   *
   * <p>These ids can be obtained from the {@link #readDeletedResources(String, boolean)} method.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public void rmacc(String resourceName, String principalType, @RUntainted String principalName)
      throws CmsException {
    ...
    if (CmsUUID.isValidUUID(principalName)) {
      ...
      m_securityManager.removeAccessControlEntry(
          m_context, res, CmsAccessControlEntry.PRINCIPAL_ALL_OTHERS_ID);
    } else if (CmsAccessControlEntry.PRINCIPAL_OVERWRITE_ALL_NAME.equals(principalName)) {
      m_securityManager.removeAccessControlEntry(
          m_context, res, CmsAccessControlEntry.PRINCIPAL_OVERWRITE_ALL_ID);
    } else {
      try {
   ...
   * @throws CmsException if something goes wrong
   */
  public void setDateExpired(CmsResource resource, long dateExpired, boolean recursive)
      throws CmsException {

    getResourceType(resource)
        .setDateExpired(this, m_securityManager, resource, dateExpired, recursive);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public void setDateExpired(String resourcename, long dateExpired, boolean recursive)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void setDateLastModified(String resourcename, long dateLastModified, boolean recursive)
      throws CmsException {

    CmsResource resource = readResource(resourcename, CmsResourceFilter.IGNORE_EXPIRATION);
    getResourceType(resource)
        .setDateLastModified(this, m_securityManager, resource, dateLastModified, recursive);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public void setDateReleased(CmsResource resource, long dateReleased, boolean recursive)
      throws CmsException {

    getResourceType(resource)
        .setDateReleased(this, m_securityManager, resource, dateReleased, recursive);
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public void setDateReleased(String resourcename, long dateReleased, boolean recursive)
      throws CmsException {
  ...
  public void setPassword(
      String username, String oldPassword, CmsSecondFactorInfo secondFactor, String newPassword)
      throws CmsException {
   ...
   *
   * <p>Only resources that have already been published once can be undeleted, if a "new" resource
  ...
  /**
   * Undoes all changes to a resource by restoring the version from the online project to the
   * current offline project.
   *
   * <p>
   *
   * @param resourcename the name of the resource to undo the changes for
   * @param mode the undo mode, one of the <code>{@link CmsResource.CmsResourceUndoMode}#UNDO_XXX
   *     </code> constants
   * @throws CmsException if something goes wrong
   ...
   */
  public void undoChanges(String resourcename, CmsResource.CmsResourceUndoMode mode)
      throws CmsException {
   ...
   *
   * <p>The group has to be a valid OpenCms group.<br>
   ...
   * @throws CmsException if something goes wrong
   */
  public void writeProjectLastModified(CmsResource resource, CmsProject project)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void writePropertyObjects(CmsResource res, List<CmsProperty> properties)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void writePropertyObjects(String resourcename, List<CmsProperty> properties)
      throws CmsException {
   ...
   * @throws CmsException if resource type is set to folder, or if the user has not the rights to
  ...
  public void writeStaticExportPublishedResource(
      String resourceName, int linkType, String linkParameter, long timestamp) throws CmsException {

    m_securityManager.writeStaticExportPublishedResource(
        m_context, resourceName, linkType, linkParameter, timestamp);
  }

  /**
   * Writes a new URL name mapping for a given resource.
   *
   * <p>The first name from the given name sequence which is not already mapped to another resource
   * will be used for the URL name mapping.
   *
   * <p>
   *
  ...
  public String writeUrlNameMapping(
      Iterator<String> nameSeq, CmsUUID structureId, String locale, boolean replaceOnPublish)
      throws CmsException {

    return m_securityManager.writeUrlNameMapping(
        m_context, nameSeq, structureId, locale, replaceOnPublish);
  }

  /**
   * Writes a new URL name mapping for a given resource.
   *
   * <p>This method uses {@link CmsNumberSuffixNameSequence} to generate a sequence of name
   * candidates from the given base name.
   *
   * <p>
   *
   ...
   */
  public String writeUrlNameMapping(
      String name, CmsUUID structureId, String locale, boolean replaceOnPublish)
      throws CmsException {

    return writeUrlNameMapping(
        new CmsNumberSuffixNameSequence(name, false), structureId, locale, replaceOnPublish);
  }
  ...
  private void createRelation(
      @RUntainted CmsResource resource, @RUntainted CmsResource target, String relationType, boolean importCase)
      throws CmsException {

    @RUntainted CmsRelationType type = CmsRelationType.valueOf(relationType);
   ...
   */
  private void createRelation(
      String resourceName, String targetPath, String relationType, boolean importCase)
      throws CmsException {
   ...
   *
   * <p>The event will be given to all registered <code>{@link I_CmsEventListener}</code>s.
   *
   * <p>
   *
   ...
   */
  private void fireEvent(@RUntainted int type, Object data) {

    OpenCms.fireCmsEvent(type, Collections.singletonMap("data", data));
  }

  /**
   * Convenience method to get the initialized resource type instance for the given resource, with a
   * fall back to special "unknown" resource types in case the resource type is not configured.
   *
   * <p>
   *
  ...
  /**
   * Initializes this <code>{@link CmsObject}</code> with the provided user context and database
   * connection.
   *
   * <p>
   *
