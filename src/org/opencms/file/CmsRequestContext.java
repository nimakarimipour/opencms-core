 ...
 * the current project, the selected site and more.
 *
 * <p>
 *
 * @since 6.0.0
...
public final class CmsRequestContext {
  ...
  public static final String ATTRIBUTE_EDITOR =
      CmsRequestContext.class.getName() + ".ATTRIBUTE_EDITOR";
  ...
  public static final String ATTRIBUTE_FULLLINKS =
      CmsRequestContext.class.getName() + ".ATTRIBUTE_FULLLINKS";
  ...
  public static final String ATTRIBUTE_NEW_RESOURCE_LOCALE =
      CmsRequestContext.class.getName() + ".NEW_RESOURCE_LOCALE";
  ...
  private @RUntainted boolean m_forceAbsoluteLinks;
   ...
   * @param requestMatcher the matcher for the current request, that is the host part of the URI
   ...
   * @param forceAbsoluteLinks if true, links should be generated with a server prefix even if we're
   ...
   *
   * <p>Usually, this would be the site root for the current site. However, if a resource from the
   * <code>/system/</code> folder is requested, this will be the empty String.
   *
   * <p>
   *
   ...
   */
  public static @RUntainted String getAdjustedSiteRoot(@RUntainted String siteRoot, @RUntainted String resourcename) {

    if (resourcename.startsWith(CmsWorkplace.VFS_PATH_SYSTEM)
        || OpenCms.getSiteManager().startsWithShared(resourcename)
        || (resourcename.startsWith(CmsWorkplace.VFS_PATH_SITES)
  ...
  /**
   * Adds the current site root of this context to the given resource name, and also translates the
   * resource name with the configured the directory translator.
   *
   * <p>
   *
  ...
  /**
   * Adds the given site root of this context to the given resource name, taking into account
   * special folders like "/system" where no site root must be added, and also translates the
   * resource name with the configured the directory translator.
   *
   * <p>
   *
   ...
   * @return the translated resource name including site root
   */
  public @RUntainted String addSiteRoot(@RUntainted String siteRoot, String resourcename) {
   ...
   *
   * <p>The directory translator is used to translate old VFS path information to a new location.
   * Example: <code>/bodys/index.html --> /system/bodies/</code>.
   *
   * <p>
   *
   ...
   *
   * <p>The file name translator is used to translate filenames from uploaded files to valid OpenCms
   * filenames. Example: <code>W&uuml;ste W&ouml;rter.doc --> Wueste_Woerter.doc</code>.
   *
   * <p>
   *
   ...
   *
   * <p>In normal operation, the request context locale is initialized using {@link
   * org.opencms.i18n.I_CmsLocaleHandler#getI18nInfo(javax.servlet.http.HttpServletRequest, CmsUser,
   * CmsProject, String)} depending on the requested resource URI.
   *
   * <p>
   *
  ...
  /**
   * Returns the matcher for the current request, that is the host part of the URI from the original
   * http request.
   *
   * <p>
   *
   * @return the matcher for the current request, that is the host part of the URI from the original
   *     http request
   */
  ...
  /**
   * Returns true if links to the current site should be generated as absolute links, i.e. with a
   * server prefix.
   *
   ...
   *
   * <p>This is used mainly for CmsReports that continue to use the users context, even after the
   * http request is already finished.
   *
   * <p>
   *
  ...
  /**
   * Removes the current site root prefix from the absolute path in the resource name, that is
   * adjusts the resource name for the current site root.
   *
   * <p>If the resource name does not start with the current site root, it is left untouched.
   *
   * <p>
   *
   * @param resourcename the resource name
  ...
  public @RUntainted String removeSiteRoot(@RUntainted String resourcename) {
    ...
    @RUntainted String siteRoot = getAdjustedSiteRoot(m_siteRoot, resourcename);
    if ((siteRoot == m_siteRoot)
        && resourcename.startsWith(siteRoot)
        && ((resourcename.length() == siteRoot.length())
            || (resourcename.charAt(siteRoot.length()) == '/'))) {
   ...
   *
   * @param forceAbsoluteLinks true if links to the current site should be generated with server
   *     prefix
   */
  ...
  public void setOuFqn(@RUntainted String ouFqn) {
    ...
    if (ouFqn != null) {
      if (ouFqn.startsWith(userOu)
          || (ouFqn.startsWith(CmsOrganizationalUnit.SEPARATOR)
              && ouFqn.substring(1).startsWith(userOu))) {
        m_ouFqn = ouFqn;
      } else {
   ...
   */
  public void setRequestTime(@RUntainted long time) {
  ...
  /**
   * Set the requested resource OpenCms VFS URI, that is the value returned by {@link #getUri()}.
   *
   * <p>Use this with caution! Many things (caches etc.) depend on this value. If you change this
   * value, better make sure that you change it only temporarily and reset it in a <code>
   * try { // do something // } finally { // reset URI // }</code> statement.
   *
   * <p>
   *
   * @param value the value to set the Uri to, must be a complete OpenCms path name like
   *     /system/workplace/style.css
   */
