...
/**
 * Base class for all OpenCms VFS resources like <code>{@link CmsFile}</code> or <code>
 * {@link CmsFolder}</code>.
 *
 * <p>The OpenCms VFS resource is an important object for using the OpenCms API. Basically, all
 * entries in the OpenCms VFS are considered to be "resources". Currently, only two types of
 * resources exists:
 *
 * <ul>
 ...
 * </ul>
 *
 * If you have a resource, you can use {@link #isFile()} or {@link #isFolder()} to learn what kind
 * of subclass you have. Please note that this is usually not required, as the only real difference
 * between a {@link CmsFile} and a {@link CmsResource} is that the {@link CmsFile} also has the
 * contents of the file, which you can obtain using {@link CmsFile#getContents()}. As long as you
 * don't need the content, you can use the {@link CmsResource} for everything else. This is even
 * more true for a {@link CmsFolder}, here you will need the subclass only in special cases, since
 * the signature is identical to {@link CmsResource}.
 *
 * <p>A OpenCms VFS resource can have any number of properties attached, which are represented by a
 * {@link CmsProperty}. To read the properties for a resource, use {@link
 * CmsObject#readPropertyObject(CmsResource, String, boolean)} or use {@link
 * CmsObject#readPropertyObjects(CmsResource, boolean)} to read all properties of the resource.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsResource
    implements I_CmsResource, Cloneable, Serializable, Comparable<I_CmsResource> {
  ...
  public static final class CmsResourceCopyMode extends A_CmsModeIntEnumeration {
    ...
    protected static final CmsResourceCopyMode MODE_COPY_PRESERVE_SIBLING =
        new CmsResourceCopyMode(3);
  ...
  public static final class CmsResourceDeleteMode extends A_CmsModeIntEnumeration {
    ...
    protected static final CmsResourceDeleteMode MODE_DELETE_PRESERVE_SIBLINGS =
        new CmsResourceDeleteMode(1);

    /** Signals that siblings of this resource should be deleted. */
    protected static final CmsResourceDeleteMode MODE_DELETE_REMOVE_SIBLINGS =
        new CmsResourceDeleteMode(2);
    ...
    /**
     * Returns the delete mode object from the old delete mode integer.
     *
     * <p>
     *
  ...
  public static final class CmsResourceUndoMode extends A_CmsModeIntEnumeration {
    ...
    public static final CmsResourceUndoMode MODE_UNDO_CONTENT_RECURSIVE =
        new CmsResourceUndoMode(2);
    ...
    public static final CmsResourceUndoMode MODE_UNDO_MOVE_CONTENT_RECURSIVE =
        new CmsResourceUndoMode(4);
    ...
    /**
     * Returns <code>true</code> if this undo operation is recursive.
     *
     * <p>
     *
    ...
    /**
     * Returns <code>true</code> if this undo mode will undo move operations.
     *
     * <p>
     *
  ...
  /**
   * Special folders below which resources should be treated the same as if they were marked as
   * internal.
   */
  private static List<String> internalFolders =
      Arrays.asList(CmsStringUtil.joinPaths(CmsUserIconHelper.USER_IMAGE_FOLDER, "temp"));

  /** Copy mode for copy resources as new resource. */
  public static final CmsResourceCopyMode COPY_AS_NEW = CmsResourceCopyMode.MODE_COPY_AS_NEW;

  /** Copy mode for copy resources as sibling. */
  public static final CmsResourceCopyMode COPY_AS_SIBLING =
      CmsResourceCopyMode.MODE_COPY_AS_SIBLING;

  /** Copy mode to preserve siblings during copy. */
  public static final CmsResourceCopyMode COPY_PRESERVE_SIBLING =
      CmsResourceCopyMode.MODE_COPY_PRESERVE_SIBLING;
  ...
  /** Signals that siblings of this resource should not be deleted. */
  public static final CmsResourceDeleteMode DELETE_PRESERVE_SIBLINGS =
      CmsResourceDeleteMode.MODE_DELETE_PRESERVE_SIBLINGS;

  /** Signals that siblings of this resource should be deleted. */
  public static final CmsResourceDeleteMode DELETE_REMOVE_SIBLINGS =
      CmsResourceDeleteMode.MODE_DELETE_REMOVE_SIBLINGS;
  ...
  /**
   * Indicates if a resource has been changed in the offline version when compared to the online
   * version.
   */
  public static final CmsResourceState STATE_CHANGED = CmsResourceState.STATE_CHANGED;

  /**
   * Indicates if a resource has been deleted in the offline version when compared to the online
   * version.
   */
  public static final CmsResourceState STATE_DELETED = CmsResourceState.STATE_DELETED;

  /**
   * Special state value that indicates the current state must be kept on a resource, this value
  ...
  /**
   * Indicates if a resource is unchanged in the offline version when compared to the online
   * version.
   */
  ...
  /** Indicates that the undo method will only recursive undo content changes. */
  public static final CmsResourceUndoMode UNDO_CONTENT_RECURSIVE =
      CmsResourceUndoMode.MODE_UNDO_CONTENT_RECURSIVE;

  /** Indicates that the undo method will undo move operations and content changes. */
  public static final CmsResourceUndoMode UNDO_MOVE_CONTENT =
      CmsResourceUndoMode.MODE_UNDO_MOVE_CONTENT;

  /** Indicates that the undo method will undo move operations and recursive content changes. */
  public static final CmsResourceUndoMode UNDO_MOVE_CONTENT_RECURSIVE =
      CmsResourceUndoMode.MODE_UNDO_MOVE_CONTENT_RECURSIVE;
  ...
  /**
   * The name of a resource with it's full path from the root folder including the current site
   * root.
   */
  ...
  /**
   * Checks if the provided resource name is a valid resource name, that is contains only valid
   * characters.
   *
   * <p>A resource name can only be composed of digits, standard ASCII letters and the symbols
   * defined in {@link #NAME_CONSTRAINTS}. A resource name must also not contain only dots.
   *
   * <p>
   *
  ...
  public static void checkResourceName(String name) throws CmsIllegalArgumentException {
    ...
    CmsStringUtil.checkName(
        name, NAME_CONSTRAINTS, Messages.ERR_BAD_RESOURCENAME_4, Messages.get());
   ...
   *
   * <p>The extension will be always lower case.
   *
   * <p>
   *
   ...
   *
   * <p>If the resource name denotes a folder (that is ends with a "/"), the complete path of the
   * folder is returned (not the parent folder path).
   *
   * <p>This is achieved by just cutting of everything behind the last occurrence of a "/" character
   * in the String, no check if performed if the resource exists or not in the VFS, only resources
   * that end with a "/" are considered to be folders.
   *
   * <p>Example: Returns <code>/system/def/</code> for the resource <code>/system/def/file.html
   * </code> and <code>/system/def/</code> for the (folder) resource <code>/system/def/</code>.
   *
   ...
   *
   * <p>The resource name of a file is the name of the file. The resource name of a folder is the
   * folder name with trailing "/". The resource name of the root folder is <code>/</code>.
   *
   ...
   *
   * <p>The parent resource of a file is the folder of the file. The parent resource of a folder is
   * the parent folder. The parent resource of the root folder is <code>null</code>.
   *
   * <p>Example: <code>/system/workplace/</code> has the parent <code>/system/</code>.
   *
   ...
   *
   * <p>The root folder "/" has level 0, a folder "/foo/" would have level 1, a folfer "/foo/bar/"
   * level 2 etc.
   *
   * <p>
   *
  ...
  /**
   * Returns the name of a parent folder of the given resource, that is either minus levels up from
   * the current folder, or that is plus levels down from the root folder.
   *
   * <p>
   *
   * @param resource the name of a resource
  ...
  /**
   * Returns <code>true</code> if the given resource path points to a temporary file name.
   *
   * <p>A resource name is considered a temporary file name if the name of the file (without parent
   * folders) starts with the prefix char <code>'~'</code> (tilde). Existing parent folder elements
   * are removed from the path before the file name is checked.
   *
   * <p>
   *
   ...
   *
   * <p>Please note a number of additional comparators for resources exists as members of this
   * class.
   *
   * <p>
   *
   ...
   *
   * <p>This is useful in case you want to create a copy of a resource and really make sure won't
   * get a {@link CmsFile} or {@link CmsFolder}, which may happen if you just call {@link #clone()}.
   *
   * <p>
   *
   ...
   *
   * <p>This applies only to resources of type {@link CmsFile}, since a {@link CmsFolder} has no
   * content. In case of a folder, <code>-1</code> is always returned as content date.
   *
   * <p>Any modification of a resource, including changes to the resource properties, will increase
   * the "date of last modification" which is returned by {@link #getDateLastModified()}. The "date
   * of the content" as returned by this method only changes when the file content as returned by
   * {@link CmsFile#getContents()} is changed.
   *
   * <p>
   *
   ...
   *
   * <p>If the expiration date has not been set, {@link #DATE_EXPIRED_DEFAULT} is returned. This
   * means: The resource does never expire.
   *
   * <p>
   *
   ...
   *
   * <p>If the release date has not been set, {@link #DATE_RELEASED_DEFAULT} is returned. This
   * means: The resource has always been released.
   *
   * <p>
   *
   ...
   *
   * <p>If the resource is a file, then this is the byte size of the file content. If the resource
   * is a folder, then the size is always -1.
   *
   * <p>
   *
  ...
  /**
   * Returns the file name of this resource without parent folders, for example <code>index.html
   * </code>.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @return the id of the {@link CmsProject} where this resource has been last modified, or <code>
   *     null</code>
   */
  ...
  /**
   * Returns the name of this resource with it's full path from the top level root folder, for
   * example <code>/sites/default/myfolder/index.html</code>.
   *
   * <p>In a presentation level application usually the current site root must be cut of from the
   * root path. Use {@link CmsObject#getSitePath(CmsResource)} to get the "absolute" path of a
   * resource in the current site.
   *
   * <p>
   *
   ...
   *
   * <p>If a resource has no sibling, the total sibling count for this resource is <code>1</code>,
   * if a resource has <code>n</code> siblings, the sibling count is <code>n + 1</code>.
   *
   * <p>
   *
   ...
   *
   * <p>This may be {@link CmsResource#STATE_UNCHANGED}, {@link CmsResource#STATE_CHANGED}, {@link
   * CmsResource#STATE_NEW} or {@link CmsResource#STATE_DELETED}.
   *
   * <p>
   *
  ...
  /**
   * Returns the user id of the {@link CmsUser} who created this resource.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the resource is a {@link CmsFile}, that is not a {@link
   * CmsFolder}.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the resource is a {@link CmsFolder}, that is not a {@link
   * CmsFile}.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the resource is marked as internal.
   *
   * <p>An internal resource can be read by the OpenCms API, but it can not be delivered by a direct
   * request from an outside user.
   *
   * <p>For example if the resource <code>/internal.xml</code> has been set as marked as internal,
   * this resource can not be requested by an HTTP request, so when a user enters <code>
   * http:/www.myserver.com/opencms/opencms/internal.xml</code> in the browser this will generate a
   * {@link CmsVfsResourceNotFoundException}.
   *
   * <p>This state is stored as bit 1 in the resource flags.
   *
   * <p>
   *
  ...
  /**
   * Checks if either the resource's 'internal' flag is set, or if it's below a list of forbidden
   * folders.
   *
  ...
  /**
   * Returns <code>true</code> if the resource has to be labeled with a special icon in the explorer
   * view.
   *
   * <p>This state is stored as bit 2 in the resource flags.
   *
   * <p>
   *
   ...
   *
   * <p>A resource is valid if it is released and not yet expired.
   *
   * <p>
   *
  ...
  public boolean isReleasedAndNotExpired(long time) {
    ...
    return ((time < m_dateExpired) && (time > m_dateReleased))
        || (time == DATE_RELEASED_EXPIRED_IGNORE);
  }

  /**
   * Returns <code>true</code> if this resource is a temporary file.
   *
  ...
  public boolean isTemporaryFile() {
    ...
    return isFile()
        && (((getFlags() & CmsResource.FLAG_TEMPFILE) > 0) || isTemporaryFileName(getName()));
  }

  /**
   * Returns <code>true</code> if this resource was touched.
   *
   * <p>
   *
   ...
   *
   * <p>The resource flags integer is used as bit set that contains special information about the
   * resource. The following methods internally use the resource flags:
   *
   * <ul>
   *   <li>{@link #isInternal()}
   *   <li>{@link #isLabeled()}
