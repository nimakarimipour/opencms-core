 ...
 *
 * <p>Every resource of type "xmlpage" becomes a folder with the same name. That folder contains the
 * locales of the xml page as folders too. In the locale folder there are the elements for that
 * locale as files. The files have the names of the elements with the extension "html". Additionaly
 * there is a file in the root folder of that xml page that contains the controlcode of the xml
 * page. This file has the name "controlcode.xml".
 *
 * <p>
 *
 * @since 6.5.6
...
public class CmsResourceWrapperXmlPage extends A_CmsResourceWrapper {
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#copyResource(org.opencms.file.CmsObject,
   *     java.lang.String, java.lang.String, org.opencms.file.CmsResource.CmsResourceCopyMode)
   */
  @Override
  ...
  public CmsResource createResource(
    ...
    if (xmlPage != null) {
      ...
      if (tokens.length == 1) {
        ...
        // set the content
        xml.setStringValue(
            cms, name, CmsLocaleManager.getLocale(tokens[0]), getStringValue(cms, file, content));
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#deleteResource(CmsObject, String,
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#getLock(org.opencms.file.CmsObject,
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#lockResource(org.opencms.file.CmsObject,
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#readFile(CmsObject, String,
   *     CmsResourceFilter)
   */
  @Override
      ...
      if (tokens.length == 1) {
        ...
        if (xml.hasValue(name, CmsLocaleManager.getLocale(tokens[0]))) {
          ...
          String contentString =
              xml.getStringValue(cms, name, CmsLocaleManager.getLocale(tokens[0]));
          String fullPath =
              xmlPage.getRootPath() + "/" + tokens[0] + "/" + name + "." + EXTENSION_ELEMENT;
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#readResource(CmsObject, String,
   *     CmsResourceFilter)
   */
  @Override
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#restoreLink(org.opencms.file.CmsObject,
   *     java.lang.String)
   */
  @Override
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#unlockResource(org.opencms.file.CmsObject,
   *     java.lang.String)
   */
  @Override
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#writeFile(org.opencms.file.CmsObject,
  ...
  protected @RUntainted String getUriStyleSheet(CmsObject cms, CmsResource res) {
    ...
    try {
      ...
      if (!"".equals(currentTemplate)) {
        ...
        result =
            cms.readPropertyObject(currentTemplate, CmsPropertyDefinition.PROPERTY_TEMPLATE, false)
                .getValue("");
      }
    } catch (CmsException e) {
      // noop
   ...
   *
   * <p>Mainly adds the basic html structure and the css style sheet.
   *
   * <p>
   *
  ...
  /**
   * Returns the {@link CmsResource} of the xml page which belongs to the given resource name (full
   * path).
   *
   * <p>It works up the path till a resource for the path exists in the VFS. If the found resource
   * is a xml page, this resource is returned. If the path does not belong to a xml page <code>null
  ...
  private CmsResource findXmlPage(CmsObject cms, @RUntainted String resourcename) {
    ...
    do {
      ...
      if (cms.existsResource(path) || existResource) {
        ...
        if (existResource) {
          // a resource without the trailing slash does exist, so take the path without the trailing
          // slash
          path = reducedPath;
        }
        try {
  ...
  /**
   * Returns a virtual resource for an element inside a locale.
   *
   * <p>A new (virtual) resource is created with the given path and length. The new created resource
   * uses the values of the origin resource of the xml page where it is possible.
   *
   * <p>
   *
  ...
  /**
   * Creates a new virtual resource for the locale in the xml page as a folder.
   *
   * <p>The new created resource uses the values of the origin resource of the xml page where it is
   * possible.
   *
   * <p>
   *
  ...
  /**
   * Returns the content as a string while using the correct encoding.
   *
   * <p>
   *
   * @param cms the initialized CmsObject
   ...
   */
  private String getStringValue(CmsObject cms, CmsResource resource, byte[] content)
      throws CmsException {
  ...
  /**
   * Returns the path inside a xml page.
   *
   * <p>The remaining path inside a xml page can be the locale and the element name or the name of
   * the control code file.
   *
   * <p>
   *
   * @param cms the initialized CmsObject
  ...
  /**
   * Returns a list with virtual file names for the xml page.
   *
   * <p>Actually that is only the name of the control code file.
   *
   * <p>
   *
