 ...
 * wrappers ({@link I_CmsResourceWrapper}) to change the view to the existing resources in the VFS.
 *
 * <p>Almost every method in this class iterates through the configured list of {@link
 * I_CmsResourceWrapper} and calls the same method there. The first resource wrapper in the list
 * which feels responsible for that action handles it and the iteration ends. So the resource
 * wrappers should check in every method if it is responsible or not. Be careful if there are more
 * than one resource wrapper for the same resource in the VFS, because the first in the list wins.
 * If the iteration is finished and no resource wrapper felt responsible the default action is to
 * call the method in the {@link CmsObject}.
 *
 * <p>It is possible to create an unchanged access to the resource in the VFS by creating a new
 * instance of the CmsObjectWrapper with an empty list of resource wrappers.
 *
 * <p>
 *
 * @since 6.2.4
...
public class CmsObjectWrapper {
  ...
  /**
   * The name of the attribute in the {@link CmsRequestContext} where the current CmsObjectWrapper
   * can be found.
   */
   ...
   * Copies a resource.
   *
   * <p>Iterates through all configured resource wrappers till the first returns <code>true</code>.
   *
   * <p>
   *
   * @see I_CmsResourceWrapper#copyResource(CmsObject, String, String,
   ...
   *
   * <p>Iterates through all configured resource wrappers till the first returns not <code>null
   * </code>.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public void deleteResource(String resourcename, CmsResourceDeleteMode siblingMode)
      throws CmsException {
  ...
  /**
   * Checks the availability of a resource in the VFS, using the {@link CmsResourceFilter#DEFAULT}
   * filter.
   *
   * <p>Here it will be first checked if the resource exists in the VFS by calling {@link
   * org.opencms.file.CmsObject#existsResource(String)}. Only if it doesn't exist in the VFS the
   * method {@link I_CmsResourceWrapper#readResource(CmsObject, String, CmsResourceFilter)} in the
   * configured resource wrappers are called till the first does not throw an exception or returns
   * <code>null</code>.
   *
   * <p>
   *
  ...
  /**
   * Delegate method for {@link CmsObject#getRequestContext()}.
   *
   * <p>
   *
   ...
   *
   * <p>First fetch all child resources from VFS by calling {@link
   * CmsObject#getResourcesInFolder(String, CmsResourceFilter)}. After that all resource wrapper are
   * called {@link I_CmsResourceWrapper#addResourcesToFolder(CmsObject, String, CmsResourceFilter)}
   * to have the chance to add additional resources to those already existing. In that list every
   * resource is given to the appropriate resource wrapper ({@link
   * I_CmsResourceWrapper#wrapResource(CmsObject, CmsResource)}) to have the possibility to change
   * the existing resources. The matching resource wrapper for a resource is found by a call to
   * {@link I_CmsResourceWrapper#isWrappedResource(CmsObject, CmsResource)}.
   *
   * <p>
   *
  ...
  /**
   * Delegate method for {@link CmsObject#getSitePath(CmsResource)}.
   *
   * <p>
   *
   ...
   *
   * <p>Entries in list are from type {@link I_CmsResourceWrapper}.
   *
   * <p>
   *
   ...
   * </code>.
   *
   * <p>If the resource contains textual content and the encoding is UTF-8, then the byte order mask
  ...
  public CmsFile readFile(String resourcename, CmsResourceFilter filter) throws CmsException {
    ...
    if (needUtf8Marker(res)) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_ADD_UTF8_MARKER_1, res.getRootPath()));
      }
  ...
  /**
   * Delegate method for {@link CmsObject#readPropertyObject(CmsResource, String, boolean)}.
   *
   * <p>
   *
  ...
  /**
   * Delegate method for {@link CmsObject#readResource(CmsUUID, CmsResourceFilter)}.
   *
   * <p>
   *
  ...
  /**
   * Delegate method for {@link CmsObject#readUser(CmsUUID)}.
   *
   * <p>
   *
   ...
   *
   * <p>Because it is possible through the <code>CmsObjectWrapper</code> to create "virtual"
   * resources, which can not be found in the VFS, it is necessary to change the links in pages as
   * well, so that they point to resources which really exists in the VFS.
   *
   * <p>Iterates through all configured resource wrappers till the first returns not <code>null
   * </code>.
   *
   ...
   *
   * <p>Because it is possible to change the names of resources inside the VFS by this <code>
   * CmsObjectWrapper</code>, it is necessary to change the links used in pages as well, so that
   * they point to the changed name of the resource.
   *
   * <p>For example: <code>/sites/default/index.html</code> becomes to <code>
   * /sites/default/index.html.jsp</code>, because it is a jsp page, the links in pages where
   * corrected so that they point to the new name (with extension "jsp").
   *
   * <p>Used for the link processing in the class {@link org.opencms.relations.CmsLink}.
   *
   * <p>Iterates through all configured resource wrappers till the first returns not <code>null
   * </code>.
   *
  ...
  /**
   * Enables or disables the automatic adding of byte order marks to plaintext files.
   *
   * <p>
   *
   * @param addByteOrderMark true if byte order marks should be added to plaintext files
   *     automatically
   */
  ...
  /**
   * Try to find a resource type wrapper for the resource.
   *
   * <p>Takes all configured resource type wrappers and ask if one of them is responsible for that
   * resource. The first in the list which feels responsible is returned. If no wrapper could be
  ...
  /**
   * Checks if the resource type needs an UTF-8 marker.
   *
   * <p>If the encoding of the resource is "UTF-8" and the resource type is one of the following:
   * <br>
   *
   * <ul>
   *   <li>{@link CmsResourceTypeJsp}
   *   <li>{@link CmsResourceTypePlain}
   *   <li>{@link CmsResourceTypeXmlContent}
   *   <li>{@link CmsResourceTypeXmlPage}
   * </ul>
   *
   * it needs an UTF-8 marker.
   *
   * <p>
   *
  ...
  /**
   * Checks if the file content already contains the UTF8 marker.
   *
   * <p>
   *
