 ...
 * for every resource in the existing folder.
 *
 * <p>Empty folders don't have the property folder visible.
 *
 * <p>The names of the property files are the same as the resource they belong to with the extension
 * "properties". To keep the correct sorting the names of folders gets additionaly the prefix "__"
 * to keep them at the beginning of the list.
 *
 * <p>When creating new folders, the property folder gets visible after a time period of 60 seconds.
 * For new resources the property file appears after that period too. In this time period it is
 * possible to create the property folder and the property files manually. The properties in the
 * created property files will be set at the resource they belong to.
 *
 * <p>
 *
 * @since 6.5.6
...
public class CmsResourceWrapperPropertyFile extends A_CmsResourceWrapper {
    ...
    if (res != null) {
      ...
      if (resourcename.endsWith(PROPERTY_DIR)) {
        ...
        CmsResourceWrapperUtils.writePropertyFile(
            cms, cms.getRequestContext().removeSiteRoot(res.getRootPath()), content);
        return res;
      }
    }
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#deleteResource(org.opencms.file.CmsObject,
   *     java.lang.String, org.opencms.file.CmsResource.CmsResourceDeleteMode)
   */
  @Override
  ...
  /**
   * @see org.opencms.file.wrapper.A_CmsResourceWrapper#readFile(org.opencms.file.CmsObject,
  ...
  public CmsFile writeFile(CmsObject cms, CmsFile resource) throws CmsException {
    ...
    if (res != null) {
      CmsResourceWrapperUtils.writePropertyFile(
          cms, cms.getRequestContext().removeSiteRoot(res.getRootPath()), resource.getContents());
      return resource;
    }
  ...
  /**
   * Tries to the read the resource with the given structure id using the given CmsObject and
   * returns it, or null if the resource can not be read.
   *
   * <p>
   *
  ...
  /**
   * Checks if a resource exists depending on the creation date and the temp files saved.
   *
   * <p>Dreamweaver copies folders through creating folder for folder and file for file. So there is
   * first a call if the property folder already exists and afterwards create it. If the folder
   * already exists, the copy action fails. In the first time after creating a folder, the property
   * dir does not exists until it is created or the time expired.
   *
   * <p>
   *
  ...
  /**
   * Creates the full path to the property file of the given resource.
   *
   * <p>
   *
  ...
  /**
   * Reads the resource for the property file.
   *
   * <p>
   *
  ...
  private CmsResource getResource(CmsObject cms, String resourcename, CmsResourceFilter filter)
    ...
    if ((path.endsWith(PROPERTY_DIR + "/"))
        && (name.endsWith(CmsResourceWrapperUtils.EXTENSION_PROPERTIES))) {
      ...
      try {
        String resPath =
            CmsResourceWrapperUtils.removeFileExtension(
                cms, parent + name, CmsResourceWrapperUtils.EXTENSION_PROPERTIES);
