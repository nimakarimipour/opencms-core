...
/**
 * Helper class with several methods used by different implementations of the interface {@link
 * I_CmsResourceWrapper}.
 *
 * <p>It provides methods to add or remove file extensions to resources, to handle creating and
 * writing property files and to add the byte order mask to UTF-8 byte contents.
 *
 * <p>
 *
 * @since 6.2.4
...
public final class CmsResourceWrapperUtils {
   ...
   *
   * <p>If the file with the new extension already exists, an index count will be added before the
   * final extension.
   *
   * <p>For example: <code>index.html.1.jsp</code>.
   *
   * <p>
   *
   ...
   *
   * <p>For example looks like this:<br>
   ...
   */
  public static CmsFile createPropertyFile(CmsObject cms, CmsResource res, String path)
      throws CmsException {
    ...
    Map<String, CmsProperty> activeProperties =
        CmsProperty.getPropertyMap(cms.readPropertyObjects(res, false));
   ...
   *
   * <ul>
   *   <li>If there is only one extension, nothing will be removed.
   *   <li>If there are two extensions, the last one will be removed.
   *   <li>If there are more than two extensions the last one will be removed and if then the last
   *       extension is a number, the extension with the number will be removed too.
   * </ul>
   *
  ...
  /**
   * Takes the content which should be formatted as a property file and set them as properties to
   * the resource.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public static void writePropertyFile(CmsObject cms, String resourcename, byte[] content)
      throws CmsException {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        if (key.endsWith(SUFFIX_PROP_SHARED)) {
          propList.add(
              new CmsProperty(
                  key.substring(0, key.length() - SUFFIX_PROP_SHARED.length()), null, value));
        } else if (key.endsWith(SUFFIX_PROP_INDIVIDUAL)) {
          propList.add(
              new CmsProperty(
                  key.substring(0, key.length() - SUFFIX_PROP_INDIVIDUAL.length()), value, null));
        }
      }
  ...
  /**
   * Escapes the value of a property in OpenCms to be displayed correctly in a property file.
   *
   * <p>Mainly handles all escaping sequences that start with a backslash.
   *
   * <p>
   *
  ...
  /**
   * Unescapes the value of a property in a property file to be saved correctly in OpenCms.
   *
   * <p>Mainly handles all escaping sequences that start with a backslash.
   *
   * <p>
