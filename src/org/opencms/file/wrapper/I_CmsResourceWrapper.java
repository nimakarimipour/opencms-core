 ...
 *
 * <p>It is possible to create "new" virtual resource in the view to the clients using the <code>
 * CmsObjectWrapper</code> or to change the existing ones. For example adding the correct file
 * extension for resources, because it is not always given that resources of type jsp have the
 * extension ".jsp". A resource wrapper just could add this extension, so that clients can handle
 * that resource correctly.
 *
 * <p>Each method in the implementing classes first have to check in every method if it is
 * responsible for the action to execute, because the <code>CmsObjectWrapper</code> iterates through
 * all configured resource wrappers and the first which feels responsible wins and the others won't
 * even called.
 *
 * <p>
 *
...
public interface I_CmsResourceWrapper {
  ...
  /**
   * Here it is possible to add additional (virtual) child resources to those already existing in
   * the VFS.
   *
   * <p>
   *
   ...
   */
  List<CmsResource> addResourcesToFolder(
      CmsObject cms, String resourcename, CmsResourceFilter filter) throws CmsException;

  /**
   * If there is a configuration string for the wrapper, this method will be called with the
   * configuration string before it is used.
   *
   * <p>Otherwise, it will not be called.
   *
   ...
   * Copies a resource.
   *
   * <p>First should be a check if the source and/or the destination are handled by this resource
   * wrapper.
   *
   * <p>It is possible that the path in the source or in the destination are virtual paths and so
   * has to be translated into valid paths existing in the VFS to copy the resource.
   *
   * <p>
   *
   ...
   *
   * <p>First should be a check if the resourcename is handled by this resource wrapper.
   *
   * <p>It is possible that the path in the resourcename is a virtual path and so has to be
   * translated into a valid path existing in the VFS to create the resource.
   *
   * <p>
   *
   ...
   *
   * <p>It is possible that the path in the resourcename is a virtual path and so has to be
   * translated into a valid path existing in the VFS to delete the resource.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  boolean deleteResource(CmsObject cms, String resourcename, CmsResourceDeleteMode siblingMode)
      throws CmsException;

  /**
   * Returns the lock for the resource.
   *
   * <p>First should be a check if the resource is handled by this resource wrapper.
   *
   * <p>It is possible that the path in the resource is a virtual path and so has to be translated
   * into a valid path existing in the VFS to get the lock for the resource.
   *
   * <p>
   *
   ...
   * @return the lock state of the resource or null if the action couldn't be handled by this
   *     resource wrapper
   * @throws CmsException if something goes wrong
   */
   ...
   *
   * <p>It is possible that the path in the resourcename is a virtual path and so has to be
   * translated into a valid path existing in the VFS to lock the resource.
   *
   * <p>
   *
   ...
   *
   * <p>It is possible that the path in the source or in the destination are virtual paths and so
   * has to be translated into valid paths existing in the VFS to move the resource.
   *
   * <p>
   *
   ...
   *
   * <p>It is possible that the path in the resourcename is a virtual path and so has to be
   * translated into a valid path existing in the VFS to read the resource.
   *
   * <p>
   *
   ...
   * @return the file resource that was read or null if it could not be handled by this resource
   *     wrapper
   * @throws CmsException if the file resource could not be read for any reason
   */
  CmsFile readFile(CmsObject cms, String resourcename, CmsResourceFilter filter)
      throws CmsException;
   ...
   */
  CmsResource readResource(CmsObject cms, String resourcename, CmsResourceFilter filter)
      throws CmsException;

  /**
   * Returns the link to a existing resource in the VFS for the uri.
   *
   * <p>Turns the virtual path into a real path to a resource in the VFS.
   *
   * <p>
   *
   ...
   *
   * <p>For example: in the VFS: "/sites/default/index.html" and this resource wrapper adds the
   * extension "jsp" then link returned should be "/sites/default/index.html.jsp".
   *
   * <p>
   *
   ...
   *
   * <p>It is possible that the path in the resourcename is a virtual path and so has to be
   * translated into a valid path existing in the VFS to unlock the resource.
   *
   * <p>
   *
   ...
   *
   * <p>For example, some resource types like {@link org.opencms.file.types.CmsResourceTypeJsp} just
   * get a <code>.jsp</code> extension, while other types like {@link
   * org.opencms.file.types.CmsResourceTypeXmlPage} are "exploded" into a folder / file structure.
   *
   * <p>
   *
   * @see CmsObjectWrapper#getResourcesInFolder(String, CmsResourceFilter)
   ...
   *
   * <p>It is possible that the path in the resource is a virtual path and so has to be translated
   * into a valid path existing in the VFS to write the resource.
   *
   * <p>
   *
   ...
   * @return the written resource or null if the action could not be handled by this resource
   *     wrapper
   * @throws CmsException if something goes wrong
   */
