...
public class CmsResourceWrapperModules extends A_CmsResourceWrapper {
  ...
  public static final List<String> FOLDERS =
      Collections.unmodifiableList(Arrays.asList(BASE_PATH, IMPORT_PATH, EXPORT_PATH, LOG_PATH));
   ...
   *
   * <p>Why do we need this if we just want to write files in the import folder and not read them?
   * The reason is that when using this wrapper with the JLAN CIFS connector, some clients check on
   * the status of the import file before they write any data to it, and fail mysteriously if it
   * isn't found, so we have to pretend that the file actually exists after creating it.
   */
  ConcurrentHashMap<String, Long> m_importFileUpdateCache = new ConcurrentHashMap<String, Long>();

  /**
   * @see
   *     org.opencms.file.wrapper.A_CmsResourceWrapper#addResourcesToFolder(org.opencms.file.CmsObject,
  ...
  public CmsResource createResource(
    ...
    if (checkAccess(cms) && matchParentPath(IMPORT_PATH, resourcename)) {
      ...
      OpenCms.getModuleManager()
          ...
          .importModule(CmsResource.getName(resourcename), content);
  ...
  public boolean deleteResource(
    ...
    if (checkAccess(cms) && matchParentPath(EXPORT_PATH, resourcename)) {
      ...
      boolean result =
          OpenCms.getModuleManager().getImportExportRepository().deleteModule(fileName);
      return result;
    } else {
      return false;
    }
  ...
  public CmsFile readFile(CmsObject cms, String resourcename, CmsResourceFilter filter)
    ...
    // already returns a CmsFile when needed
  ...
  public CmsResource readResource(CmsObject cms, String resourcepath, CmsResourceFilter filter)
    ...
    if (checkAccess(cms)) {
      ...
      if (matchParentPath(EXPORT_PATH, resourcepath)) {
        ...
        if (cms.getRequestContext().getAttribute(CmsJlanDiskInterface.NO_FILESIZE_REQUIRED)
            == null) {
          // we *do* require the file size, so we need to get the module data
          LOG.info("Getting data for " + resourcepath);
          CmsModuleImportExportRepository.ModuleExportData data =
              OpenCms.getModuleManager()
                  .getImportExportRepository()
        ...
        try {
          ...
          throw new CmsVfsResourceNotFoundException(
              org.opencms.db.Messages.get()
                  .container(org.opencms.db.Messages.ERR_READ_RESOURCE_1, resourcepath),
              e);
        }
      }
    }
  ...
  public CmsFile writeFile(CmsObject cms, CmsFile resource) throws CmsException {
    ...
    if (checkAccess(cms) && matchParentPath(IMPORT_PATH, resource.getRootPath())) {
      OpenCms.getModuleManager()
          .getImportExportRepository()
          .importModule(CmsResource.getName(resource.getRootPath()), resource.getContents());
  ...
  protected CmsResource createFakeBinaryFile(String rootPath, long dateLastModified)
    ...
    int type =
        OpenCms.getResourceManager()
            .getResourceType(CmsResourceTypeBinary.getStaticTypeName())
            .getTypeId();
  ...
  /**
   * Checks whether the the current user should have access to the module functionality.
   *
   * <p>
   *
  ...
  private List<CmsResource> getVirtualResourcesForBasePath(CmsObject cms) throws CmsException {
    ...
    return Arrays.asList(
        createFakeFolder(IMPORT_PATH), createFakeFolder(EXPORT_PATH), createFakeFolder(LOG_PATH));
  }
  ...
  private List<CmsResource> getVirtualResourcesForExport(CmsObject cms) throws CmsException {
    ...
    List<CmsResource> virtualResources = Lists.newArrayList();
    for (String name :
        OpenCms.getModuleManager().getImportExportRepository().getModuleFileNames()) {
   ...
   *
   * <p>This is basically an equality test, but ignores the presence/absence of trailing slashes.
   *
