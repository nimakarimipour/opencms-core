...
/**
 * Defines a property name, so that <code>{@link CmsProperty}</code> instances can be created with
 * that name.
 *
 * <p>
 *
...
public class CmsPropertyDefinition implements Cloneable, Comparable<CmsPropertyDefinition> {
  ...
  public static final class CmsPropertyType extends A_CmsModeIntEnumeration {
    ...
    /**
     * Creates a new property type with the given identifier.
     *
     * <p>
     *
     ...
     *
     * <p>If the given String matches no known type <code>{@link #NORMAL}</code> will be returned as
     * the default.
     *
     * <p>
     *
  ...
  /**
   * Property for the days a resource has to be expired to be deleted by the <code>
   * {@link  org.opencms.scheduler.jobs.CmsDeleteExpiredResourcesJob}</code>.
   */
  ...
  /**
   * The name of the property which controls whether an element will be used as a copy model by the
  ...
  /**
   * Property used to record the resource type for resources whose type is unknown at import time,
   * so they can be exported with that type later.
   */
  public static final String PROPERTY_EXPORT_TYPE = "export.type";

  /**
   * Property for the resource export name, during export this name is used instead of the resource
   * name.
   */
  ...
  /**
   * Property to control whether historic versions should be removed when deleted resources are
   * published.
   */
  ...
  /**
   * Property for specifying a list of container types used to match formatters in the display
   * formatter selection widget.
   */
  ...
  /**
   * Property to configure the value which should be used instead of the template path when
   * selecting the template in the GUI. Please note that this does not have to actually be a
   * template provider configuration string, this is just the most common use case.
   */
  ...
  /**
   * Name of the property used to control whether mapped URL names should replace previous URL
   * names.
   */
  ...
  private static final CmsPropertyDefinition NULL_PROPERTY_DEFINITION =
      new CmsPropertyDefinition(CmsUUID.getNullUUID(), "", TYPE_NORMAL);
  ...
  /**
   * Creates a new property definition object with the type <code>{@link #TYPE_NORMAL}</code>.
   *
   * <p>
   *
  ...
  /**
   * Checks if the provided property name is a valid property name, that is contains only valid
   * characters.
   *
   * <p>A property name can only be composed of digits, standard ASCII letters and the symbols
   * defined in {@link #NAME_CONSTRAINTS}.
   *
   * <p>
   *
  ...
  public static void checkPropertyName(String name) throws CmsIllegalArgumentException {
    ...
    CmsStringUtil.checkName(
        name, NAME_CONSTRAINTS, Messages.ERR_BAD_PROPERTYNAME_4, Messages.get());
  }
