...
public class CmsHistoryResourceHandler implements I_CmsResourceInit {
  ...
  /**
   * Returns the historical version of a resource, if the given request is displaying a history
   * version.
   *
   * <p>
   *
  ...
  /**
   * Appends the <code>version</code> parameter to the URI if needed.
   *
   * <p>
   *
  ...
  public static String getHistoryResourceURI(String uri, ServletRequest req) {
    ...
    if (CmsHistoryResourceHandler.isHistoryRequest(req)) {
      ...
      histUri =
          CmsRequestUtil.appendParameter(uri, CmsHistoryResourceHandler.PARAM_VERSION, version);
    }
    return histUri;
  }

  /**
   * Returns the correct resource for the given URI, taken into account historical versions marked
   * by the <code>version</code> parameter.
   *
   * <p>
   *
   * @param cms the current CMS context
   * @param resourceUri the resource URI
   * @return the resource, which can be an instance of {@link
   *     org.opencms.file.history.I_CmsHistoryResource}
   * @throws CmsException if something goes wrong
   */
  public static CmsResource getResourceWithHistory(CmsObject cms, String resourceUri)
      throws CmsException {
  ...
  /**
   * Returns <code>true</code> if the given request is displaying an historical version.
   *
   * <p>
   *
   * @param req the request to check
          ...
          if (cms.existsResource(HISTORY_HANDLER)) {
            ...
            try {
              ...
              if (id == CmsHistoryResourceHandler.PROJECT_OFFLINE_VERSION) {
                resource =
                    new CmsHistoryFile(cms.readFile(uri, CmsResourceFilter.IGNORE_EXPIRATION));
              } else {
                ...
                CmsHistoryFile hisRes =
                    (CmsHistoryFile)
                        cms.readResource(
                            cms.readResource(uri, CmsResourceFilter.IGNORE_EXPIRATION)
                                .getStructureId(),
                            id);

                // the resource root path is not changed after the resource is moved or renamed
                // change the resource root path to current root path, so e.g. properties can be
                // read if necessary
              ...
              if (LOG.isErrorEnabled()) {
                LOG.error(
                    Messages.get().getBundle().key(Messages.ERR_HISTORYRESOURCE_2, uri, version));
              }
