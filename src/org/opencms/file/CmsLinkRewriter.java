 ...
 * to another given subtree replaced with relations to the first subtree.
 *
 * <p>
 */
public class CmsLinkRewriter {
  ...
  private List<CmsPair<String, String>> m_sourceTargetPairs =
      new ArrayList<CmsPair<String, String>>();
   ...
   */
  public CmsLinkRewriter(
      CmsObject cms, String targetPath, List<CmsPair<String, String>> sourceTargetPairs) {
          ...
          try {
            ...
            if (resType instanceof CmsResourceTypeXmlContent) {
              ...
              try {
                ...
                LOG.info(
                    "XML content was corrected automatically for resource " + file.getRootPath());
    ...
    if (target.startsWith(source)) {
      throw new CmsIllegalArgumentException(
          org.opencms.file.Messages.get()
              .container(
                  org.opencms.file.Messages.ERR_REWRITE_LINKS_ROOTS_DEPENDENT_2, source, target));
    }
  }
   ...
   *
   * <p>This is necessary because the default copy mechanism does not copy locale relations.
   *
   * @throws CmsException if something goes wrong
   */
  protected void copyLocaleRelations() throws CmsException {
    ...
    List<CmsRelation> localeRelations =
        m_cms.readRelations(CmsRelationFilter.ALL.filterType(CmsRelationType.LOCALE_VARIANT));
        ...
        if ((newRelationSource != null) && (newRelationTarget != null)) {
          try {
            m_cms.addRelationToResource(
                newRelationSource, newRelationTarget, CmsRelationType.LOCALE_VARIANT.getName());
          } catch (CmsException e) {
            LOG.error("Could not transfer locale relation: " + e.getLocalizedMessage(), e);
          }
        } else {
          LOG.warn(
              "Could not transfer locale relation because source/target not found in copy: " + rel);
        }
      }
    }
  ...
  /**
   * Decodes a byte array into a string with a given encoding, or the default encoding if that
   * fails.
   *
   * <p>
   *
  ...
  /**
   * Decodes a file's contents and return the content string and the encoding to use for writing the
   * file back to the VFS.
   *
   * <p>
   *
  ...
  /**
   * Finds relations from the target root folder or its children to the source root folder or its
   * children.
   *
   * <p>
   *
    ...
    try {
      encoding =
          cms.readPropertyObject(
                  resource.getRootPath(), CmsPropertyDefinition.PROPERTY_CONTENT_ENCODING, true)
              .getValue();
    } catch (CmsException e) {
    ...
    if (encoding == null) {
      ...
      if (encoding == null) {
        throw new CmsXmlException(
            Messages.get()
                .container(Messages.ERR_XMLCONTENT_INVALID_ENC_1, resource.getRootPath()));
      }
    }
    return encoding;
  }
  ...
  protected List<CmsPair<CmsResource, CmsResource>> getMatchingResources(
    ...
    List<CmsPair<CmsResource, CmsResource>> result =
        new ArrayList<CmsPair<CmsResource, CmsResource>>();
  ...
  /**
   * Collects a list of resources in a map where the key for each resource is the path relative to a
   * given folder.
   *
   * <p>
   *
   ...
   */
  protected Map<String, CmsResource> getResourcesByRelativePath(
      List<CmsResource> resources, String basePath) {
  ...
  protected void init() throws CmsException {
    ...
    // m_cms.getRequestContext().setAttribute(CmsXmlContent.AUTO_CORRECTION_ATTRIBUTE,
    // Boolean.TRUE);
    ...
    for (CmsPair<String, String> pair : m_sourceTargetPairs) {
      List<CmsPair<CmsResource, CmsResource>> matchingResources =
          getMatchingResources(pair.getFirst(), pair.getSecond());
  ...
  /**
   * Replaces structure ids of resources in the source subtree with the structure ids of the
   * corresponding resources in the target subtree inside a content string.
   *
   * <p>
   *
  ...
  protected String rewriteContentString(String originalContent) {
    ...
    I_CmsRegexSubstitution substitution =
    ...
    for (CmsRelation rel : relations) {
      ...
      if (!relType.isDefinedInContent()) {
        if (newTargetResource != null) {
          ...
          m_cms.addRelationToResource(
              rel.getSourcePath(), newTargetResource.getRootPath(), relType.getName());
        }
      }
    }
  }
