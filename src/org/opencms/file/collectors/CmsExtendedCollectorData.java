 ...
 *
 * <p>The input data String must have the following format:<br>
 * <code>"{VFS URI}|{Resource type}|{Count}|excludeTimerange|{AddParam1}|{AddParam2}..."</code>, for
 * example:<br>
 * <code>"/my/folder/|xmlcontent|5|excludeTimerange|p1|p2|p3|p4"</code> or <code>
 * "/my/folder/|xmlcontent|5|p1|p2|p3|p4"</code>.
 *
 * <p>This extends the basic {@link CmsCollectorData} by allowing to append additional parameters to
 * the input String. The parameters can then be obtained by the collector using {@link
 * #getAdditionalParams()}. It will depend on the collector implementation how these additional
 * parameters are used.
 *
 * <p>
 *
  ...
  public CmsExtendedCollectorData(String data) {
    ...
    if (data == null) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_COLLECTOR_PARAM_EMPTY_0));
    }
    ...
    try {
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_COLLECTOR_PARAM_INVALID_1, data), e2);
      } catch (CmsLoaderException e2) {
        // this resource type does not exist
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_UNKNOWN_RESTYPE_1, type), e2);
      }
    }
    setCount(5);
    if (args.size() > 2) {
      ...
      if (CmsStringUtil.isNotEmpty(count)) {
        try {
          ...
          throw new CmsRuntimeException(
              Messages.get().container(Messages.ERR_COLLECTOR_PARAM_INVALID_1, data), e);
        }
      }
    }
    if (args.size() > 3) {
