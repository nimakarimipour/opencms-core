 ...
 *
 * <p>The comparator can be configured to use any date based on resource attributes or properties.
 ...
 * should be checked. This list of dates identifiers must be Strings which tell the comparator which
 * dates to use. The first valid date identifier that is found for a resource is used as date for
 * comparing this resource to other resources.
 *
 * <p>The following date identifiers can be used to access the corresponding value of a {@link
 * CmsResource}:
 *
 * <ul>
 ...
 *       with that name from the resource, and convert that value to a long. Should this fail for
 *       any reason, the next entry from the list of dates will be processed.
 *       <p>
 *   <li>If no match is found at all, {@link CmsResource#getDateCreated()} is used as default.
 * </ul>
 *
 * Serves as {@link java.util.Comparator} for resources and as comparator key for the resource at
 * the same time. Uses lazy initializing of comparator keys for a resource.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsDateResourceComparator implements Comparator<CmsResource> {
  ...
  private static final String[] DATE_ATTRIBUTES = {
    "dateCreated", "dateLastModified", "dateContent", "dateReleased", "dateExpired"
  };
  ...
  /**
   * Creates a new, empty instance of this comparator key, used for the calculated map valued.
   *
   * <p>
   */
