...
public abstract class A_CmsResourceCollector implements I_CmsResourceCollector {
  ...
  /**
   * The template file separator string for creating a new resource in direct edit mode, can be used
   * to append an explicit template file name in {@link #getCreateParam(CmsObject, String, String)}.
   */
  ...
  public static String createResourceForCollector(
    ...
    if (pos != -1) {
      ...
      templateFileName =
          collectorParams.substring(pos + A_CmsResourceCollector.SEPARATOR_TEMPLATEFILE.length());
    } else {
    ...
    CmsResource templateResource =
        cms.readResource(templateFileName, CmsResourceFilter.IGNORE_EXPIRATION);
    ...
    if (!templateResource.isFolder()) {
      ...
      newContent =
          CmsXmlContentFactory.createDocument(
              cms, locale, template.getEncoding(), template.getContentDefinition());
    } else {
   ...
   *
   * <p>If the given name is null or does not refer to a valid post-create handler class, a default
    ...
    try {
      ...
      if (I_CmsCollectorPostCreateHandler.class.isAssignableFrom(handlerClass)) {
        I_CmsCollectorPostCreateHandler handler =
            (I_CmsCollectorPostCreateHandler) handlerClass.newInstance();
        return handler;
      } else {
        LOG.error(
            "Post-create handler class does not implement I_CmsPostCreateHandler: '"
                + className
                + "'");
        return new CmsDefaultPostCreateHandler();
      }
      ...
      LOG.error(
          "Problem using post-create handler: '" + className + "'," + e.getLocalizedMessage(), e);
      return new CmsDefaultPostCreateHandler();
    }
  }
  ...
  /**
   * @see
   *     org.opencms.file.collectors.I_CmsResourceCollector#getCreateTypeId(org.opencms.file.CmsObject,
   ...
   */
  public Set<CmsResource> getPublishResources(
      final CmsObject cms, final I_CmsContentLoadCollectorInfo info) throws CmsException {
  ...
  protected String getCreateInFolder(CmsObject cms, CmsCollectorData data) throws CmsException {
    ...
    return OpenCms.getResourceManager()
        .getNameGenerator()
        .getNewFileName(cms, data.getFileName(), 4);
  }
   ...
   * @param result a List
   * @param maxSize the maximum size of the List
   * @param explicitNumResults the value of the numResults parameter given to the getResults method
   *     (this overrides maxSize if it is positive)
   * @return the reduced list
   */
  protected List<CmsResource> shrinkToFit(
      List<CmsResource> result, int maxSize, int explicitNumResults) {
