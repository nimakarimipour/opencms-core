 ...
 * sorting based on resource dates.
 *
 * <p>
 *
 * @since 8.0
...
public class CmsChangedResourceCollector extends A_CmsResourceCollector {
  ...
  /**
   * Returns a List of all changed resources in the folder pointed to by the parameter sorted by the
   * date attributes specified in the parameter.
   *
   * <p>
   *
   * @param cms the current CmsObject
   * @param param must contain an extended collector parameter set as described by {@link
   *     CmsExtendedCollectorData}
   * @param tree if true, look in folder and all child folders, if false, look only in given folder
   * @param asc if <code>true</code>, the sort is ascending (old dates first), otherwise it is
   *     descending (new dates first)
   * @param numResults number of results
   * @return a List of all resources in the folder pointed to by the parameter sorted by the
   *     selected dates
  ...
  protected List<CmsResource> allChangedInFolderDate(
    ...
    CmsResourceFilter filter =
        CmsResourceFilter.DEFAULT_FILES
            ...
            .addRequireLastModifiedAfter(dateFrom)
            .addRequireLastModifiedBefore(dateTo);
    ...
    if (params.containsKey(PARAM_KEY_EXCLUDETYPE)) {
      ...
      try {
        ...
        try {
          ...
          throw new CmsRuntimeException(
              Messages.get().container(Messages.ERR_UNKNOWN_RESTYPE_1, excludeType), e2);
        }
      }
      if (typeId != -1) {
