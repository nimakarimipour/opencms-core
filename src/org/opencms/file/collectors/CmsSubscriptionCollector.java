 ...
 * parameters.
 *
 * <p>The configuration of the collectors can be done in the parameter String using key value pairs,
 * separated by the <code>|</code> (pipe) symbol. The following configuration options are available:
 *
 * <p>
 *
 * <ul>
 *   <li><i>currentuser</i>: determines if the current user should be used to read visited or
 *       subscribed resources (not considered if the <code>user</code> parameter is used)
 *   <li><i>daysfrom</i>: the number of days subtracted from the current day specifying the start
 *       point in time from which a resource was visited
 *   <li><i>daysto</i>: the number of days subtracted from the current day specifying the end point
 *       in time to which a resource was visited
 *   <li><i>groups</i>: the users group names, separated by comma, to read subscribed resources for
 *   <li><i>includegroups</i>: the include groups flag to read subscribed resources also for the
 *       given or current users groups (not considered if the <code>groups</code> parameter is used)
 *   <li><i>includesubfolders</i>: the include subfolders flag to read subscribed resources also for
 *       the subfolders of the given parent folder (not considered if the <code>resource</code>
 *       parameter is not used)
 *   <li><i>mode</i>: the subscription read mode, can be <code>all</code>, <code>visited</code> or
 *       <code>unvisited</code> (default)
 *   <li><i>resource</i>: the resource, i.e. the parent folder from which the subscribed or visited
 *       resources should be read from
...
public class CmsSubscriptionCollector extends A_CmsResourceCollector {
  ...
  /**
   * The collector parameter key for the current user flag (to set the user in the filters to the
   * current user).
   */
  public static final String PARAM_KEY_CURRENTUSER = "currentuser";

  /**
   * The collector parameter key for the number of days subtracted from the current day specifying
   * the start point in time from which a resource was visited.
   */
  public static final String PARAM_KEY_DAYSFROM = "daysfrom";

  /**
   * The collector parameter key for the number of days subtracted from the current day specifying
   * the end point in time to which a resource was visited.
   *
   * <p>If the parameter {@link #PARAM_KEY_DAYSFROM} is also used, the value of this key should be
   * less than the value set as {@link #PARAM_KEY_DAYSFROM} parameter.
   */
  public static final String PARAM_KEY_DAYSTO = "daysto";

  /**
   * The collector parameter key for the users group names, separated by comma, to read subscribed
   * resources for.
   */
  public static final String PARAM_KEY_GROUPS = "groups";

  /**
   * The collector parameter key for the include groups flag to read subscribed resources also for
   * the given or current users groups.
   */
  public static final String PARAM_KEY_INCLUDEGROUPS = "includegroups";

  /**
   * The collector parameter key for the include subfolders flag to read subscribed resources also
   * for the subfolders of the given parent folder.
   */
  ...
  /**
   * The collector parameter key for the resource, i.e. the parent folder from which the subscribed
   * or visited resources should be read from.
   */
  ...
  private static final String[] COLLECTORS = {
    "allVisited", "allSubscribed", "allSubscribedDeleted"
  };

  /** Array list for fast collector name lookup. */
  private static final List<String> COLLECTORS_LIST =
      Collections.unmodifiableList(Arrays.asList(COLLECTORS));
   ...
   *     java.lang.String, java.lang.String)
   */
  public List<CmsResource> getResults(
      CmsObject cms, String collectorName, String param, int numResults)
      throws CmsDataAccessException, CmsException {
  ...
  protected List<CmsResource> getSubscribedResources(CmsObject cms, String param, int numResults)
    ...
    List<CmsResource> result =
        OpenCms.getSubscriptionManager()
            .readSubscribedResources(cms, getSubscriptionFilter(cms, param));
   ...
   * @throws CmsException if something goes wrong
   */
  protected CmsSubscriptionFilter getSubscriptionFilter(CmsObject cms, String param)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  protected List<CmsResource> getVisitedResources(CmsObject cms, String param, int numResults)
      throws CmsException {

    List<CmsResource> result =
        OpenCms.getSubscriptionManager()
            .readResourcesVisitedBy(cms, getVisitedByFilter(cms, param));
    Collections.sort(result, I_CmsResource.COMPARE_DATE_LAST_MODIFIED);
    if (numResults > 0) {
      result = shrinkToFit(result, numResults);
    }
  ...
  /**
   * Returns the calculated time with the days delta using the base time.
   *
   * <p>
   *
  ...
  private long getCalculatedTime(long baseTime, String deltaDays, String key, long defaultTime) {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_COLLECTOR_PARAM_INVALID_1, key + "=" + deltaDays));
    }
    return defaultTime;
  }
  ...
  /**
   * Initializes the visited by filter from the parameters.
   *
   * <p>
   *
   ...
   * @param forceSetUser flag to determine if a user has to be set in the filter (should be <code>
   *     true</code> for the visited by filter, <code>false</code> for the subscription filter)
   * @throws CmsException if something goes wrong
   */
  private void initVisitedByFilter(
    ...
    if (params.containsKey(PARAM_KEY_USER)) {
      try {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_COLLECTOR_PARAM_USER_1, params.get(PARAM_KEY_USER)));
        throw e;
      }
    }
    ...
    if (params.containsKey(PARAM_KEY_DAYSFROM)) {
      filter.setFromDate(
          getCalculatedTime(currentTime, params.get(PARAM_KEY_DAYSFROM), PARAM_KEY_DAYSFROM, 0L));
    }
    if (params.containsKey(PARAM_KEY_DAYSTO)) {
      filter.setToDate(
          getCalculatedTime(
              currentTime, params.get(PARAM_KEY_DAYSTO), PARAM_KEY_DAYSTO, Long.MAX_VALUE));
    }
      ...
      try {
        ...
        if (params.containsKey(PARAM_KEY_INCLUDESUBFOLDERS)) {
          boolean includeSubFolders =
              Boolean.valueOf(params.get(PARAM_KEY_INCLUDESUBFOLDERS)).booleanValue();
