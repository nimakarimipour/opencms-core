...
public class CmsSolrCollector extends A_CmsResourceCollector {
  ...
  public List<CmsResource> getResults(CmsObject cms, String name, String param, int numResults)
    ...
    boolean excludeTimerange =
        !cms.getRequestContext().getCurrentProject().isOnlineProject()
            && Boolean.valueOf(paramsAsMap.get(CmsCollectorData.PARAM_EXCLUDETIMERANGE))
                .booleanValue();
    if (excludeTimerange) {
      q.removeExpiration();
    }
    ...
    // Do only return the minimal set of fields, since on can never access the solr result at all,
    // but only the resource itself
  ...
  /**
   * Splits the given parameter String into the query part and the cms specific arguments.
   *
   * <p>
   *
