...
public class CmsDefaultResourceCollector extends A_CmsResourceCollector {
  ...
  /**
   * Returns a List of all resources in the folder pointed to by the parameter sorted by the release
   * date, descending.
   *
   * <p>
   *
   ...
   * @return a List of all resources in the folder pointed to by the parameter sorted by the release
   *     date, descending
   * @throws CmsException if something goes wrong
   */
  protected List<CmsResource> allInFolderDateReleasedDesc(
   ...
   * @throws CmsException if something goes wrong
   */
  protected List<CmsResource> allInFolderNavPos(
      CmsObject cms, String param, boolean readSubTree, int numResults) throws CmsException {
    ...
    for (int i = 0, n = foundResources.size(); i < n; i++) {
      ...
      if ((navElement != null) && (navElement.getNavPosition() != Float.MAX_VALUE)) {
        ...
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_RESOURCE_WITHOUT_NAVPROP_1, cms.getSitePath(resource)));
      }
    }
   ...
   */
  protected List<CmsResource> getAllInFolder(
      CmsObject cms, String param, boolean tree, int numResults)
