 ...
 *
 * <p>A user in OpenCms is uniquely defined by its user named returned by <code>{@link #getName()}
 * </code>.
 *
 * <p>Basic users in OpenCms are users that can access the OpenCms Workplace. Moreover, the user
 * must be created by another user with the <code>
 * {@link org.opencms.security.CmsRole#ACCOUNT_MANAGER}</code> role. These users are "content
 * managers" that actually have write permissions in at last some parts of the VFS.
 *
 * <p>Another possibility is to have users in a 'Guests' group. These users do not have access to
 * the OpenCms Workplace. However, an user in a 'Guests' group can be created by every user, for
 * example the "Guest" user. The main use case is that these users are used for users of the website
 * that can generate their own accounts, in a "please register your account..." scenario. These user
 * accounts can then be used to build personalized web sites.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsUser extends CmsPrincipal implements Cloneable {
   ...
   *
   * <p>Mostly intended to be used with the <code>
   * org.opencms.workplace.tools.accounts.A_CmsEditUserDialog</code>.
   *
   * <p>
   */
  public CmsUser() {
   ...
   *
   * <p>That means, the parameter should only be composed by digits and standard english letters,
   * points, underscores and exact one "At" symbol.
   *
   * <p>
   *
  ...
  /**
   * Returns the "full" name of the given user in the format <code>
   * "{firstname} {lastname} ({username})"</code>, or the empty String <code>""</code> if the user
   * is null.
   *
   * <p>
   *
  ...
  /**
   * Checks if the given String starts with {@link I_CmsPrincipal#PRINCIPAL_USER} followed by a dot.
   *
   * <p>
   *
   * <ul>
   ...
   * @return <code>true</code> in case the String starts with {@link I_CmsPrincipal#PRINCIPAL_USER}
   *     followed by a dot
   */
  ...
  /**
   * Removes the prefix if the given String starts with {@link I_CmsPrincipal#PRINCIPAL_USER}
   * followed by a dot.
   *
   * <p>
   *
   * <ul>
   *   <li>Works if the given String is <code>null</code>.
   *   <li>If the given String does not start with {@link I_CmsPrincipal#PRINCIPAL_USER} followed by
   *       a dot it is returned unchanged.
   ...
   * @return the given String with the prefix {@link I_CmsPrincipal#PRINCIPAL_USER} and the
   ...
   *
   * <p>The "additional information" storage map is a simple {@link java.util.Map} that can be used
   * to store any key / value pairs for the user. Some information parts of the users address are
   * stored in this map by default.
   *
   * <p>
   *
  ...
  /**
   * Returns a value from this users "additional information" storage map, or <code>null</code> if
   * no value for the given key is available.
   *
   * <p>
   *
   ...
   *
   * <p>This information is stored in the "additional information" storage map using the key <code>
   * {@link CmsUserSettings#ADDITIONAL_INFO_CITY}</code>.
   *
   * <p>
   *
   ...
   *
   * <p>This information is stored in the "additional information" storage map using the key <code>
   * {@link CmsUserSettings#ADDITIONAL_INFO_COUNTRY}</code>.
   *
   * <p>
   *
  ...
  public String getDescription(Locale locale) {
    ...
    return macroResolver.resolveMacros(
        (String) getAdditionalInfo(CmsUserSettings.ADDITIONAL_INFO_DESCRIPTION));
  }
      ...
      return org.opencms.security.Messages.get()
          ...
          .key(
              ...
              OpenCms.getOrgUnitManager()
                  .readOrganizationalUnit(cms, getOuFqn())
  ...
  /**
   * Returns the "full" name of the this user in the format <code>
   * "{firstname} {lastname} ({username})"</code>.
   *
   * <p>
   *
   ...
   *
   * <p>This information is stored in the "additional information" storage map using the key <code>
   * {@link CmsUserSettings#ADDITIONAL_INFO_INSTITUTION}</code>.
   *
   * <p>
   *
   ...
   *
   * <p>This information is stored in the "additional information" storage map using the key <code>
   * {@link CmsUserSettings#ADDITIONAL_INFO_ZIPCODE}</code>.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this user is not able to manage itself.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this user was touched.
   *
   * <p>
   *
  ...
  /**
   * Sets the "touched" status of this user to <code>true</code>.
   *
   * <p>
   */
  public void touch() {
