...
public class CmsModuleImportExportHandler implements I_CmsImportExportHandler {
  ...
  public CmsModuleImportExportHandler() {
    ...
    m_description =
        org.opencms.importexport.Messages.get()
            .getBundle()
            .key(org.opencms.importexport.Messages.GUI_CMSIMPORTHANDLER_DEFAULT_DESC_0);
  }
   ...
   * <p>
   *
   * @param cms the {@link CmsObject} used by to set up the handler. The object's site root might be
   *     adjusted to the import site of the module.
   * @param module The module to export
   * @param handlerDescription A description of the export handler, shown when the export thread
   *     using the handler runs.
   * @return CmsModuleImportExportHandler with all module resources
   */
  public static CmsModuleImportExportHandler getExportHandler(
      CmsObject cms, final CmsModule module, final String handlerDescription) {
    ...
    try {
      ...
      if (LOG.isInfoEnabled()) {
        LOG.warn(
            Messages.get().getBundle().key(Messages.ERR_READ_MODULE_RESOURCES_1, module.getName()),
            e);
      }
    }
    ...
    try {
      ...
      if (!file.exists()) {
        throw new IOException(
            "readModuleFromImport: Path '" + importResource + "' does not exist.");
      }
      if (file.isFile()) {
        ...
        if (entry != null) {
          ...
          CmsMessageContainer message =
              Messages.get().container(Messages.ERR_NO_MANIFEST_MODULE_IMPORT_1, importResource);
      ...
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_IO_MODULE_IMPORT_1, importResource);
      LOG.error(message.key(), e);
      throw new CmsConfigurationException(message, e);
    } catch (SAXException e) {
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_SAX_MODULE_IMPORT_1, importResource);
      LOG.error(message.key(), e);
      throw new CmsConfigurationException(message, e);
    } finally {
      try {
  ...
  public static CmsModule readModuleFromManifest(byte[] manifest) throws CmsConfigurationException {
    ...
    try {
      ...
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_IO_MODULE_IMPORT_1, "manifest data");
      LOG.error(message.key(), e);
      throw new CmsConfigurationException(message, e);
    } catch (SAXException e) {
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_SAX_MODULE_IMPORT_1, "manifest data");
      LOG.error(message.key(), e);
      throw new CmsConfigurationException(message, e);
    }
    CmsModule importedModule = handler.getModule();
  ...
  public static void reportBeginImport(I_CmsReport report, @RUntainted String modulePackageName) {
    ...
    if (report instanceof CmsHtmlReport) {
      ...
      report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, modulePackageName));
    }
    report.println(
  ...
  public static void reportEndImport(I_CmsReport report) {
    ...
    report.println(
        Messages.get().container(Messages.RPT_IMPORT_MODULE_END_0), I_CmsReport.FORMAT_HEADLINE);
  }

  /**
   * @see org.opencms.importexport.I_CmsImportExportHandler#exportData(org.opencms.file.CmsObject,
  ...
  public void exportData(CmsObject cms, I_CmsReport report)
    ...
    report.print(
        Messages.get().container(Messages.RPT_EXPORT_MODULE_BEGIN_0), I_CmsReport.FORMAT_HEADLINE);
    if (report instanceof CmsHtmlReport) {
      report.print(
          org.opencms.report.Messages.get()
              .container(
      ...
      report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, getModuleName()));
    }
    report.print(
        org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0));
    ...
    try {
      shouldIncrementVersion =
          module.isAutoIncrement()
              && (module.getVersion().isUpdated()
                  || module.shouldIncrementVersionBasedOnResources(cms));
    } catch (CmsException e) {
    ...
    report.println(
        Messages.get().container(Messages.RPT_EXPORT_MODULE_END_0), I_CmsReport.FORMAT_HEADLINE);
  }
    ...
    try {
      ...
      try {
        ...
        throw new CmsImportExportException(
            Messages.get().container(Messages.ERR_IO_MODULE_IMPORT_1, parameters.getPath()), e);
      } finally {
        helper.closeFile();
  ...
  /**
   * @see org.opencms.importexport.I_CmsImportExportHandler#importData(org.opencms.file.CmsObject,
   *     java.lang.String, java.lang.String, org.opencms.report.I_CmsReport)
   ...
   * @throws CmsConfigurationException if the module is already installed or the dependencies are
   *     not fulfilled
  ...
  private synchronized CmsModule importModule(
    ...
    List<CmsModuleDependency> dependencies =
        OpenCms.getModuleManager()
            .checkDependencies(importedModule, CmsModuleManager.DEPENDENCY_MODE_IMPORT);
    ...
    for (I_CmsResourceType type : importedModule.getResourceTypes()) {
      ...
      if (externalConflictIndex >= 0) {
        I_CmsResourceType conflictingType =
            OpenCms.getResourceManager().getResourceTypes().get(externalConflictIndex);
    ...
    if (!CmsStringUtil.isEmptyOrWhitespaceOnly(importScript)) {
      ...
      report.println(
          Messages.get().container(Messages.RPT_IMPORT_SCRIPT_HEADER_0),
          I_CmsReport.FORMAT_HEADLINE);
