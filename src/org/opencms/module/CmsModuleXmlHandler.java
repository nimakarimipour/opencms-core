...
public class CmsModuleXmlHandler {
  ...
  /**
   * Enum used to distinguish whether we want to generate the XML for a module export's manifest or
   * the XML for writing back opencms-modules.xml.
   */
  ...
  public static void addXmlDigesterRules(Digester digester) {
    ...
    // also need to change the corresponding parameter indexes in the method
    // addXmlDigesterRulesForVersion5Modules.
    ...
    digester.addCallMethod(
        "*/" + N_MODULE + "/" + N_DEPENDENCIES + "/" + N_DEPENDENCY, "addDependency", 2);
    digester.addCallParam(
        "*/" + N_MODULE + "/" + N_DEPENDENCIES + "/" + N_DEPENDENCY,
        0,
        I_CmsXmlConfiguration.A_NAME);
    digester.addCallParam(
        "*/" + N_MODULE + "/" + N_DEPENDENCIES + "/" + N_DEPENDENCY, 1, A_VERSION);
    ...
    digester.addCallParam(
        ...
        I_CmsXmlConfiguration.A_NAME);
    digester.addCallParam(
        "*/" + N_MODULE + "/" + N_PARAMETERS + "/" + I_CmsXmlConfiguration.N_PARAM, 1);
  ...
  public static Element generateXml(CmsModule module, XmlWriteMode writeMode) {
    ...
    if (module.getDateCreated() != CmsModule.DEFAULT_DATE) {
      moduleElement
          .addElement(N_DATECREATED)
          .setText(CmsDateUtil.getHeaderDate(module.getDateCreated()));
    } else {
    ...
    if (!module.isReducedExportMode() && (module.getDateInstalled() != CmsModule.DEFAULT_DATE)) {
      moduleElement
          .addElement(N_DATEINSTALLED)
          .setText(CmsDateUtil.getHeaderDate(module.getDateInstalled()));
    } else {
    ...
    if (explorerTypes.size() > 0) {
      Element explorerTypesElement =
          moduleElement.addElement(CmsWorkplaceConfiguration.N_EXPLORERTYPES);
   ...
   *
   * <p>All invalid characters are replaced by an underscore "_". This is for example used to make
   * sure old (5.0) modules can still be imported, by converting the name to a valid class name.
   *
   * <p>
   *
  ...
  /**
   * Adds the digester rules for OpenCms version 5 modules.
   *
   * <p>
   *
   * @param digester the digester to add the rules to
   ...
   *
   * <p>Adds the type setting as well to a map with the resource type name as key.
   *
  ...
  public void createdModule(
    ...
    List<I_CmsResourceType> moduleResourceTypes =
        new ArrayList<I_CmsResourceType>(m_resourceTypes.size());
