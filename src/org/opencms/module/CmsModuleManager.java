...
public class CmsModuleManager {
  ...
  private CmsModuleImportExportRepository m_importExportRepository =
      new CmsModuleImportExportRepository();
  ...
  public CmsModuleManager(List<CmsModule> configuredModules) {
    ...
    for (int i = 0; i < configuredModules.size(); i++) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get().getBundle().key(Messages.INIT_MOD_CONFIGURED_1, module.getName()));
      }
    }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_NUM_MODS_CONFIGURED_1, new Integer(m_modules.size())));
    }
   ...
   *
   * <p>The module dependencies are get from the installed modules or from the module manifest.xml
   * files found in the given FRS path.
   *
   * <p>Two types of dependency lists can be generated:<br>
   *
   * <ul>
   *   <li>Forward dependency lists: a list of modules that depends on a module
   *   <li>Backward dependency lists: a list of modules that a module depends on
   * </ul>
   *
   * @param rfsAbsPath a RFS absolute path to search for modules, or <code>null</code> to use the
   *     installed modules
   * @param mode if <code>true</code> a list of forward dependency is build, is not a list of
   *     backward dependency
  ...
  public static Map<String, List<String>> buildDepsForModulelist(
      List<String> moduleNames, String rfsAbsPath, boolean mode) throws CmsConfigurationException {
   ...
   * @throws CmsConfigurationException if something goes wrong
   */
  public static Map<CmsModule, String> getAllModulesFromPath(String rfsAbsPath)
      throws CmsConfigurationException {
    ...
    @RUntainted File folder = new File(rfsAbsPath);
    if (folder.exists()) {
      // list all child resources in the given folder
      @RUntainted File[] folderFiles = folder.listFiles();
      if (folderFiles != null) {
        for (int i = 0; i < folderFiles.length; i++) {
          @RUntainted File moduleFile = folderFiles[i];
          if (moduleFile.isFile()
              && !(moduleFile.getAbsolutePath().toLowerCase().endsWith(".zip"))) {
            // skip non-ZIP files
            continue;
          }
  ...
  /**
   * Sorts a given list of module names by dependencies, so that the resulting list can be imported
   * in that given order, that means modules without dependencies first.
   *
   * <p>The module dependencies are get from the installed modules or from the module manifest.xml
   * files found in the given FRS path.
   *
   ...
   * @throws CmsConfigurationException if something goes wrong
   */
  public static @RUntainted List<@RUntainted String> topologicalSort(List<String> moduleNames, String rfsAbsPath)
      throws CmsConfigurationException {

    @RUntainted List<String> modules = new ArrayList<String>(moduleNames);
    List<@RUntainted String> retList = new ArrayList<@RUntainted String>();
    Map<String, List<String>> moduleDependencies =
        buildDepsForModulelist(moduleNames, rfsAbsPath, true);
   ...
   * @throws CmsSecurityException if the required permissions are not available (i.e. no "Admin"
   *     CmsObject has been provided)
  ...
  public synchronized void addModule(CmsObject cms, CmsModule module)
    ...
    try {
      @RUntainted I_CmsModuleAction moduleAction = module.getActionInstance();
      String className = module.getActionClass();
      if ((moduleAction == null) && (className != null)) {
        @RUntainted Class<?> actionClass = Class.forName(className, false, getClass().getClassLoader());
   ...
   *
   * <p>The possible values for the <code>mode</code> parameter are:
   *
   * <dl>
   *   <dt>{@link #DEPENDENCY_MODE_DELETE}
   *   <dd>Check for module deleting, i.e. are other modules dependent on the given module?
   *   <dt>{@link #DEPENDENCY_MODE_IMPORT}
   *   <dd>Check for module importing, i.e. are all dependencies required by the given module
   *       available?
   * </dl>
   *
  ...
  public List<CmsModuleDependency> checkDependencies(CmsModule module, @RUntainted int mode) {
    ...
    if (mode == DEPENDENCY_MODE_DELETE) {
      ...
      throw new CmsRuntimeException(
          Messages.get()
              .container(Messages.ERR_CHECK_DEPENDENCY_INVALID_MODE_1, new Integer(mode)));
    }
  ...
  /**
   * Checks the module selection list for consistency, that means that if a module is selected, all
   * its dependencies are also selected.
   *
   * <p>The module dependencies are get from the installed modules or from the module manifest.xml
   * files found in the given FRS path.
   *
  ...
  public void checkModuleSelectionList(
      List<String> moduleNames, String rfsAbsPath, boolean forDeletion)
    ...
    while (itMods.hasNext()) {
      @RUntainted String moduleName = itMods.next();
      List<String> dependencies = moduleDependencies.get(moduleName);
      if (dependencies != null) {
        @RUntainted List<String> depModules = new ArrayList<String>(dependencies);
   ...
   * @param preserveLibs <code>true</code> to keep any exported file exported into the WEB-INF lib
   *     folder
   * @param report the report to print progress messages to
   * @throws CmsRoleViolationException if the required module manager role permissions are not
   *     available
  ...
  public synchronized void deleteModule(
      CmsObject cms, @RUntainted String moduleName, boolean replace, boolean preserveLibs, I_CmsReport report)
    ...
    if (!replace) {
      ...
      if (!dependencies.isEmpty()) {
        ...
        throw new CmsConfigurationException(
            Messages.get()
                .container(Messages.ERR_MOD_DEPENDENCIES_2, moduleName, message.toString()));
      }
      try {
      ...
      deleteProject =
          cms.readProject(
              Messages.get()
                  ...
                  .key(Messages.GUI_DELETE_MODULE_PROJECT_NAME_1, new Object[] {moduleName}));
    } catch (CmsException e) {
      try {
        ...
        deleteProject =
            cms.createProject(
                ...
                Messages.get()
                    .getBundle(cms.getRequestContext().getLocale())
                    .key(Messages.GUI_DELETE_MODULE_PROJECT_DESC_1, new Object[] {moduleName}),
    ...
    try {
      ...
      CmsLockFilter filter1 =
          CmsLockFilter.FILTER_ALL.filterNotLockableByUser(
    ...
    if (preserveLibs) {
      // to preserve the module libs, remove the responsible export points, before deleting module
      // resources
    ...
    try {
        ...
        try {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_MOVE_RESOURCE_FAILED_1, cms.getSitePath(resource)));
          }
          report.println(e.getMessageContainer(), I_CmsReport.FORMAT_WARNING);
        }
      }
      ...
      report.println(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_HTML_ITAG_1, moduleName),
          I_CmsReport.FORMAT_HEADLINE);

      // move through all module resources and delete them
      for (CmsResource resource : moduleResources) {
        @RUntainted String sitePath = cms.getSitePath(resource);
        try {
          if (LOG.isDebugEnabled()) {
          ...
          report.println(
              org.opencms.report.Messages.get()
                  .container(org.opencms.report.Messages.RPT_ARGUMENT_1, sitePath));
   ...
   */
  public synchronized void deleteModule(
      CmsObject cms, @RUntainted String moduleName, boolean replace, I_CmsReport report)
      throws CmsRoleViolationException, CmsConfigurationException, CmsLockException {
  ...
  /**
   * Returns the module with the given module name, or <code>null</code> if no module with the given
   * name is configured.
   *
   * <p>
   *
   ...
   */
  public @RUntainted Set<String> getModuleNames() {
   ...
   * @throws CmsRoleViolationException if the provided OpenCms context does not have "manage
  ...
  public synchronized void initialize(CmsObject cms, CmsConfigurationManager configurationManager)
        ...
        if (module.getActionClass() != null) {
          try {
            ...
            CmsLog.INIT.info(
                Messages.get()
                    .getBundle()
                    .key(Messages.INIT_CREATE_INSTANCE_FAILED_1, module.getName()),
                e);
          }
        }
        if (moduleAction != null) {
          ...
          if (CmsLog.INIT.isInfoEnabled()) {
            CmsLog.INIT.info(
                Messages.get()
                    .getBundle()
                    .key(Messages.INIT_INITIALIZE_MOD_CLASS_1, moduleAction.getClass().getName()));
          }
          try {
            ...
            LOG.error(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_INSTANCE_INIT_ERR_1, moduleAction.getClass().getName()),
                t);
          }
        }
      }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_NUM_CLASSES_INITIALIZED_1, new Integer(count)));
    }
  }
   ...
   *
   * <p>If there is not already a module with the same name installed, then the module will just be
   * imported normally.
   *
  ...
  public synchronized void shutDown() {
    ...
    while (it.hasNext()) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_SHUTDOWN_MOD_CLASS_1, moduleAction.getClass().getName()));
      }
      try {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_INSTANCE_SHUTDOWN_ERR_1, moduleAction.getClass().getName()),
            t);
      }
    }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_SHUTDOWN_NUM_MOD_CLASSES_1, new Integer(count)));
    }
  ...
  public synchronized void updateModule(CmsObject cms, CmsModule module)
    ...
    if (oldModule == null) {
      // module is not currently configured, no update possible
      throw new CmsConfigurationException(
          Messages.get().container(Messages.ERR_OLD_MOD_ERR_1, module.getName()));
    }
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_INSTANCE_UPDATE_ERR_1, module.getName()), t);
    }
  ...
  private synchronized void initModuleExportPoints() {
    ...
    while (i.hasNext()) {
      ...
      for (int j = 0; j < moduleExportPoints.size(); j++) {
        @RUntainted CmsExportPoint point = moduleExportPoints.get(j);
        if (exportPoints.contains(point)) {
          if (LOG.isWarnEnabled()) {
            LOG.warn(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DUPLICATE_EXPORT_POINT_2, point, module.getName()));
          }
        } else {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_ADD_EXPORT_POINT_2, point, module.getName()));
          }
        }
      }
    }
