...
public class CmsModuleImportData {
  ...
  /**
   * The map of conflicting ids (keys are structure ids from the manifest, values are structure ids
   * from VFS).
   */
  ...
  public boolean checkUpdatable(CmsObject cms) {
    ...
    if ((exportVersion == null)
        || !exportVersion.equals("" + CmsImportVersion10.IMPORT_VERSION10)) {
    ...
    try {
      ...
      for (CmsResourceImportData resData : getResourceData()) {
        ...
        if (parentFolderStr != null) {
          ...
          if ((parent != null) && parent.hasStructureId()) {
            ...
            String key =
                parent.getResource().getStructureId() + "/" + resData.getResource().getName();
            if (resData.hasStructureId()) {
      ...
      // Reject imports which have an entry with (parentId=A,name=B,structureId=C1) if there is a
      // VFS entry with (parentId=A,name=B,structureId=C2), unless
      ...
      for (CmsUUID parentId : parentFolderMaps.keySet()) {
        for (CmsObject cmsToRead : Arrays.asList(cms, onlineCms)) {
          try {
            CmsResource parent =
                cmsToRead.readResource(parentId, CmsResourceFilter.IGNORE_EXPIRATION);
            List<CmsResource> children =
                cmsToRead.readResources(parent, CmsResourceFilter.IGNORE_EXPIRATION, false);
            for (CmsResource child : children) {
              ...
              if ((importStructureId != null)
                  && !importStructureId.equals(child.getStructureId())) {
      ...
      for (CmsResourceImportData resData : getResourceData()) {
        ...
        if (resData.hasStructureId()) {
          ...
          for (CmsObject cmsToRead : Arrays.asList(cms, onlineCms)) {
            try {
              CmsResource resourceFromVfs =
                  cmsToRead.readResource(importPath, CmsResourceFilter.ALL);
              if (!resourceFromVfs.getStructureId().equals(importId)) {
                ...
                if (resData.getResource().isFile()
                  ...
                  // If we have different structure ids, but they don't conflict with anything else
                  // in the manifest or VFS,
                  // we don't compare resource ids. First, because having different resource ids is
                  // normal in this scenario, second
              ...
              if (!resData.isSkipResourceIdCheck()
                  ...
                  && !(resData
                      .getResource()
                      .getResourceId()
                      .equals(resourceFromVfs.getResourceId()))) {
                LOG.info(
   ...
   *
   * <p>The keys are structure ids from the manifest, the values are structure ids from the VFS.
   *
  ...
  /**
   * Check if the module data contains a given structure id.
   *
   * <p>
   *
  ...
  /**
   * Checks if a resource with a given structure id exists in the VFS, either online or offline.
   *
   * <p>
   *
