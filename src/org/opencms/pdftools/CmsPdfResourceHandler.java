 ...
 * format the content identified by detailname using the JSP identified by formatter-id to generate
 * XHTML which is then converted to PDF and returned directly by this handler.
 *
 * <p>In Online mode, the generated PDFs are cached on the real file system, while in Offline mode,
 * the PDF data is always generated on-the-fly.
 *
 * <p>
 */
public class CmsPdfResourceHandler implements I_CmsResourceInit {
  ...
  public CmsResource initResource(
      CmsResource resource, CmsObject cms, HttpServletRequest request, HttpServletResponse response)
    ...
    boolean abort =
        (resource != null)
            || cms.getRequestContext().getUri().startsWith(CmsWorkplace.VFS_PATH_SYSTEM);
    if (abort) {
    ...
    if (response != null) {
      ...
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get()
                .container(
                    Messages.ERR_RESOURCE_INIT_ABORTED_1, CmsPdfResourceHandler.class.getName()),
            e);
      }
    } else {
      return null;
    ...
    if (result == null) {
      ...
      byte[] xhtmlData =
          CmsPdfFormatterUtils.executeJsp(
              cmsForJspExecution, request, response, formatter, content);

      LOG.info(
          "Rendered XHTML from " + content.getRootPath() + " using " + formatter.getRootPath());
      if (LOG.isDebugEnabled()) {
        logXhtmlOutput(formatter, content, xhtmlData);
      }
      // Use the same CmsObject we used for executing the JSP, because the same site root is needed
      // to resolve external resources like images
  ...
  /**
   * Handles a request for a PDF thumbnail.
   *
   * <p>
   *
   * @param cms the current CMS context
  ...
  private void handleThumbnailLink(
    ...
    // use a wrapped resource because we want the cache to store files with the correct (image file)
    // extensions
    ...
    String cacheName =
        m_thumbnailCache.getCacheName(
            wrapperWithImageExtension.getResource(), options + ";" + linkObj.getFormat());
