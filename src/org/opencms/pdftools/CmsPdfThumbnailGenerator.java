...
public class CmsPdfThumbnailGenerator {
   ...
   *
   * <p>The given width and height determine the box in which the thumbnail should fit. The
   * resulting image will always have these dimensions, even if the aspect ratio of the actual PDF
   * page is different from the ratio of the given width and height. In this case, the size of the
   * rendered page will be reduced, and the rest of the image will be filled with blank space.
   *
   * <p>If one of width or height is negative, then that dimension is chosen so the resulting aspect
   * ratio is the aspect ratio of the PDF page.
   *
  ...
  public byte[] generateThumbnail(
      InputStream pdfInputStream, int boxWidth, int boxHeight, String imageFormat, int pageIndex)
      throws Exception {

    org.jpedal.io.ObjectStore.temp_dir =
    ...
    try {
      ...
      double aspectRatio =
          (pageData.getCropBoxWidth(1 + pageIndex) * 1.0)
      ...
      // calculateDimensions only takes integers, but only their ratio matters, we multiply the box
      // width with a big number
      int fakePixelWidth = (int) (FAKE_PIXEL_MULTIPLIER * aspectRatio);
      int fakePixelHeight = (FAKE_PIXEL_MULTIPLIER);
      int[] unpaddedThumbnailDimensions =
          CmsImageScaler.calculateDimension(fakePixelWidth, fakePixelHeight, boxWidth, boxHeight);
      decoder.decodePage(1 + pageIndex);
      BufferedImage pageImage = decoder.getPageAsImage(1 + pageIndex);
      BufferedImage paddedImage =
          new BufferedImage(boxWidth, boxHeight, BufferedImage.TYPE_3BYTE_BGR);
      ...
      AffineTransformOp op =
          new AffineTransformOp(
              AffineTransform.getScaleInstance(
                  (uw * 1.0) / pageImage.getWidth(), (uh * 1.0) / pageImage.getHeight()),
      ...
      // g.drawImage(pageImage, (boxWidth - pageImage.getWidth()) / 2, (boxHeight -
      // pageImage.getHeight()) / 2, null);
