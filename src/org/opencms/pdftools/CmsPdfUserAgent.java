 ...
 *
 * <p>Resources will be loaded from the VFS. Additionally, if there are image scaler parameters in
 * an image URI, the scaled image data will be returned. Please note that this class just reads the
 * data from the linked resources; it will not go through OpenCms's resource loaders, so you can't
 * e.g. use a JSP as a dynamic stylesheet.
 */
public class CmsPdfUserAgent extends NaiveUserAgent {
  ...
  public ImageResource getImageResource(String uri) {
    ...
    if (resource != null) {
      resource =
          new ImageResource(
              resource.getImageUri(), (FSImage) ((ITextFSImage) resource.getImage()).clone());
    } else {
  ...
  /**
   * Converts a byte array to an input stream, but returns null if the byte array is null.
   *
   * <p>
   *
   * @param data the data
  ...
  /**
   * Reads an image from the VFS, scaling it if necessary.
   *
   * <p>
   *
  ...
  /**
   * Scales the image to output resolution.
   *
   * <p>
   *
