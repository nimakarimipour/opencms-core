...
public class A_CmsSerialDateValue implements I_CmsSerialDateValue {
  ...
  /**
   * Set the pattern type specific interval between two events, e.g., number of days, weeks, month,
   * years.
   *
   ...
   *
   * <p>All pattern specific values are reset.
   *
  ...
  /**
   * Set the start time of the events. Unless you specify a single event, the day information is
   * discarded.
   *
  ...
  protected final boolean isDayOfMonthValid() {
    ...
    return (getDayOfMonth() > 0)
        && (getDayOfMonth() <= (getMonth() == null ? 31 : getMonth().getMaximalDay()));
  }

  /**
   * Checks if the duration option is valid.
   *
   * <p>NOTE: This does NOT check, if too many events are specified.
   *
  ...
  /**
   * Check, if the end date of the single event is valid, i.e., either not set or not before the
   * start date.
   *
  ...
  protected final void setDerivedEndType() {
    ...
    m_endType =
        getPatternType().equals(PatternType.NONE) || getPatternType().equals(PatternType.INDIVIDUAL)
            ? EndType.SINGLE
