...
public abstract class A_CmsNotification extends CmsHtmlMail {
    ...
    try {
      ...
      if (CmsStringUtil.isEmpty(m_receiver.getEmail())) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_NOTIFICATION_NO_ADDRESS_1, m_receiver.getName()));
        return null;
      }
      ...
      if (LOG.isInfoEnabled()) {
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_NOTIFICATION_SEND_1, m_receiver.getEmail()));
      }
      ...
      try {
        ...
        if (configParts.length == 2) {
          ...
          LOG.error(
              "Invalid notification header/footer configuration: " + HEADER_FOOTER_CONFIG_PATH);
        }
      ...
      setSubject(
          CmsMacroResolver.resolveMacros(
              m_mailContent.getStringValue(m_cms, "Subject", m_locale), m_macroResolver));
  ...
  protected void appendHtmlHeader(StringBuffer buffer) {
    ...
    if (m_configuredHeader != null) {
      ...
      buffer.append(
          "      a:hover { color:#b31b43; text-decoration:underline; font-weight: bold; }\r\n");
  ...
  protected void appendXMLContent(StringBuffer msg) {
    ...
    msg.append(
        CmsMacroResolver.resolveMacros(
            m_mailContent.getStringValue(m_cms, "Header", m_locale), m_macroResolver));
    ...
    msg.append(
        CmsMacroResolver.resolveMacros(
            m_mailContent.getStringValue(m_cms, "Footer", m_locale), m_macroResolver));
  }
  ...
  /**
   * Overwrite the method to generate the message body of the notification. This text is placed
   * between the header and the footer of the defined xmlcontent and the required html code is
   * added.
   *
   * <p>
   *
  ...
  /**
   * Overwrite the method to return the path to the xmlcontent, where the subject, the header and
   * the footer are defined.
   *
   * <p>
   *
