...
public class CmsContentNotification extends A_CmsNotification {
  ...
  private String m_serverAndContext =
      OpenCms.getSiteManager().getWorkplaceServer() + OpenCms.getSystemInfo().getOpenCmsContext();
  ...
  protected String generateHtmlMsg() {
    ...
    List<CmsExtendedNotificationCause> outdatedResources =
        new ArrayList<CmsExtendedNotificationCause>();
    List<CmsExtendedNotificationCause> resourcesNextDay =
        new ArrayList<CmsExtendedNotificationCause>();
    List<CmsExtendedNotificationCause> resourcesNextWeek =
        new ArrayList<CmsExtendedNotificationCause>();

    // split all resources into three lists: the resources that expire, will be released or get
    // outdated
  ...
  /**
   * Returns a list of CmsNotificationResourceInfos of the resources that will occur in the
   * notification.
   *
   * <p>
   *
   * @return a list of CmsNotificationResourceInfos of the resources that will occur in the
   *     notification
   */
  ...
  private void appendConfirmLink(StringBuffer buf, CmsExtendedNotificationCause notificationCause) {
    ...
    try {
      ...
      CmsUUID projectId =
          getCmsObject().readProject(OpenCms.getSystemInfo().getNotificationProject()).getUuid();
  ...
  private void appendEditLink(StringBuffer buf, CmsExtendedNotificationCause notificationCause) {
    ...
    if (existsEditor(notificationCause.getResource())) {
      try {
        ...
        buf.append("[<a href=\"");
        buf.append(
            CmsRequestUtil.appendParameters(m_uriWorkplace + "editors/editor.jsp", params, false));
        buf.append("\">");
  ...
  private void appendResourceList(
    ...
    if (!notificationCauseList.isEmpty()) {
      htmlMsg.append(
          "<tr><td colspan=\"5\"><br/><p style=\"margin-top:20px;margin-bottom:10px;\"><b>");
      ...
      htmlMsg.append(
          "</div></td><td><div style=\"padding-top:2px;padding-bottom:2px;padding-left:10px;\">");
   ...
   */
  private String buildNotificationListItem(
      CmsExtendedNotificationCause notificationCause, int row) {
    ...
    if ((notificationCause.getResource().getDateReleased() < System.currentTimeMillis())
      ...
      result.append(
          CmsRequestUtil.appendParameters(
              m_uriWorkplace + "commons/displayresource.jsp", params, false));
      result.append("\">");
    ...
    result.append(
        "</td><td><div style=\"white-space:nowrap;padding-left:10px;padding-right:10px;\">");
    result.append(siteRoot);
    result.append(
        "</td><td><div style=\"white-space:nowrap;padding-left:10px;padding-right:10px;\">");
    if (notificationCause.getCause() == CmsExtendedNotificationCause.RESOURCE_EXPIRES) {
      result.append(
          m_messages.key(Messages.GUI_EXPIRES_AT_1, new Object[] {notificationCause.getDate()}));
      ...
      result.append(
          m_messages.key(Messages.GUI_RELEASE_AT_1, new Object[] {notificationCause.getDate()}));
      result.append("</div></td>");
      appendConfirmLink(result, notificationCause);
      appendModifyLink(result, notificationCause);
    } else if (notificationCause.getCause()
        == CmsExtendedNotificationCause.RESOURCE_UPDATE_REQUIRED) {
      result.append(
          m_messages.key(
              Messages.GUI_UPDATE_REQUIRED_1, new Object[] {notificationCause.getDate()}));
      result.append("</div></td>");
      appendConfirmLink(result, notificationCause);
      ...
      result.append(
          m_messages.key(
              ...
              new Object[] {
                new Integer(CmsDateUtil.getDaysPassedSince(notificationCause.getDate()))
              }));
      result.append("</div></td>");
      appendConfirmLink(result, notificationCause);
      appendEditLink(result, notificationCause);
