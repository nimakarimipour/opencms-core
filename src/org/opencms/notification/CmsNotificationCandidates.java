 ...
 * require a notification, creates and sends notifications to their responsible users.
 *
 * <p>
 */
public class CmsNotificationCandidates {
  ...
  /**
   * Collects all resources that will expire in short time, or will become valid, or are not
  ...
  public CmsNotificationCandidates(CmsObject cms) throws CmsException {
    ...
    GregorianCalendar now =
        new GregorianCalendar(TimeZone.getDefault(), CmsLocaleManager.getDefaultLocale());
      ...
      resources =
          m_cms
              .readResourcesWithProperty(
                  folder, CmsPropertyDefinition.PROPERTY_NOTIFICATION_INTERVAL)
              .iterator();
      while (resources.hasNext()) {
        ...
        int notification_interval =
            Integer.parseInt(
                m_cms
                    .readPropertyObject(
                        resource, CmsPropertyDefinition.PROPERTY_NOTIFICATION_INTERVAL, true)
                    .getValue());
        GregorianCalendar intervalBefore =
            new GregorianCalendar(TimeZone.getDefault(), CmsLocaleManager.getDefaultLocale());
    ...
    CmsResourceFilter filter =
        CmsResourceFilter.IGNORE_EXPIRATION.addRequireLastModifiedBefore(
    ...
    CmsResourceFilter resourceFilter =
        CmsResourceFilter.IGNORE_EXPIRATION.addRequireExpireBefore(inOneWeek.getTimeInMillis());
    ...
    resourceFilter =
        CmsResourceFilter.IGNORE_EXPIRATION.addRequireReleaseBefore(inOneWeek.getTimeInMillis());
  ...
  public String notifyResponsibles() throws CmsException {
    ...
    if (notifications.hasNext()) {
      StringBuffer result =
          new StringBuffer(Messages.get().getBundle().key(Messages.LOG_NOTIFICATIONS_SENT_TO_0));
      result.append(' ');
  ...
  /**
   * Returns a collection of CmsContentNotifications, one for each responsible that receives a
   * notification.
   *
   * <p>
   *
   * @return the list of CmsContentNotifications, one for each responsible that receives a
   *     notification
   * @throws CmsException if something goes wrong
   */
  protected Collection<CmsContentNotification> getContentNotifications() throws CmsException {
    ...
    while (notificationCandidates.hasNext()) {
      ...
      String enableNotification =
          m_cms
              .readPropertyObject(
                  resource, CmsPropertyDefinition.PROPERTY_ENABLE_NOTIFICATION, true)
              .getValue();
          ...
          while (responsibles.hasNext()) {
            ...
            if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(responsible.getEmail())) {
              ...
              List<CmsExtendedNotificationCause> resourcesForResponsible =
                  contentNotification.getNotificationCauses();
  ...
  /**
   * Updates the resources that were confirmed by the user. That means deletes the resources that
   * need not a notification any more. removes all resources which do not occur in the candidate
   * list.
   *
   * <p>
   *
   * @param contentNotifications the list of {@link CmsContentNotification} objects to remove from
   *     the set of confirmed resources
  ...
  private Collection<CmsContentNotification> filterConfirmedResources(
    ...
    while (notifications.hasNext()) {
      ...
      List<?> confirmedResourcesList =
          (List<?>)
              responsible.getAdditionalInfo(CmsUserSettings.ADDITIONAL_INFO_CONFIRMED_RESOURCES);
      if (confirmedResourcesList == null) {
        confirmedResourcesList = new ArrayList<Object>();
        responsible.setAdditionalInfo(
            CmsUserSettings.ADDITIONAL_INFO_CONFIRMED_RESOURCES, new ArrayList<Object>());
      }

      List<CmsExtendedNotificationCause> notificationCandidates =
          contentNotification.getNotificationCauses();

      List<CmsExtendedNotificationCause> notificationResources =
          new ArrayList<CmsExtendedNotificationCause>(notificationCandidates);
