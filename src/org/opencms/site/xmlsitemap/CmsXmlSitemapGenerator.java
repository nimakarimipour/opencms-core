...
/**
 * Class for generating XML sitemaps for SEO purposes, as described in <a
...
public class CmsXmlSitemapGenerator {
  ...
  /**
   * A bean that consists of a sitemap URL bean and a priority score, to determine which of multiple
   * entries with the same URL are to be preferred.
   *
   * <p>
   */
  protected class ResultEntry {
    ...
    /**
     * Internal priority to determine which of multiple entries with the same URL is used. Note that
     * this has nothing to do with the priority in the URL bean itself!
     */
    ...
    /**
     * Gets the internal priority used to determine which of multiple entries with the same URL to
     * use.
     *
     * <p>This has nothing to do with the priority defined in the URL beans themselves!
     *
  ...
  protected Map<String, List<CmsResource>> m_detailResources =
      new HashMap<String, List<CmsResource>>();
  ...
  /**
   * A map from structure ids to page aliases below the base folder which point to the given
   * structure id.
   */
  protected Multimap<CmsUUID, CmsAlias> m_pageAliasesBelowBaseFolderByStructureId =
  ...
  public CmsXmlSitemapGenerator(String folderRootPath) throws CmsException {
    ...
    CmsSite site =
        OpenCms.getSiteManager().getSiteForRootPath(CmsStringUtil.joinPaths(folderRootPath, "/"));
  ...
  /**
   * Replaces the protocol/host/port of a link with the ones from the given server URI, if it's not
   * empty.
   *
   * <p>
   *
   ...
   *
   * <p>If the change frequency is not defined in the properties, this method will return null.
   *
   * <p>
   *
  ...
  protected static String getChangeFrequency(List<CmsProperty> properties) {
    ...
    CmsProperty prop =
        CmsProperty.get(CmsPropertyDefinition.PROPERTY_XMLSITEMAP_CHANGEFREQ, properties);
   ...
   *
   * <p>If the page priority can't be found among the properties, -1 will be returned.
   *
   * <p>
   *
  ...
  protected static double getPriority(List<CmsProperty> properties) {
    ...
    CmsProperty prop =
        CmsProperty.get(CmsPropertyDefinition.PROPERTY_XMLSITEMAP_PRIORITY, properties);
    if (prop.isNullProperty()) {
      return -1.0;
    }
    try {
  ...
  /**
   * Generates a list of XML sitemap entry beans for the root folder which has been set in the
   * constructor.
   *
   * <p>
   *
   ...
   *
   * <p>The replacement server URL will replace the scheme/host/port from the URLs returned by
   * getOnlineLink.
   *
  ...
  /**
   * Adds an URL bean to the internal map of results, but only if there is no existing entry with
   * higher internal priority than the priority given as an argument.
   *
   * <p>
   *
  ...
  protected long computeContainerPageModificationDate(CmsResource containerPage)
    ...
    CmsRelationFilter filter =
        CmsRelationFilter.relationsFromStructureId(containerPage.getStructureId())
            .filterType(CmsRelationType.XML_STRONG);
    List<CmsRelation> relations = m_guestCms.readRelations(filter);
    long result = containerPage.getDateLastModified();
    for (CmsRelation relation : relations) {
      try {
        CmsResource target =
            relation.getTarget(m_guestCms, CmsResourceFilter.DEFAULT_FILES.addRequireVisible());
   ...
   *
   * <p>Note: The actual container page used for the result link is not necessarily the container
   * page passed in as parameter - the default detail page in the sitemap containing the page is
   * used.
   *
  ...
  protected List<I_CmsResourceType> getDetailTypesForPage(CmsResource resource) {
    ...
    String parentPath =
        CmsFileUtil.removeTrailingSeparator(CmsResource.getParentFolder(resource.getRootPath()));
  ...
  protected List<CmsResource> getDirectPages() throws CmsException {
    ...
    for (String includeRoot : includeRoots) {
      try {
        ...
        if (resource.isFile()) {
          ...
          List<CmsResource> subtreeFiles =
              m_guestCms.readResources(includeRoot, CmsResourceFilter.DEFAULT_FILES, true);
    ...
    while (filterIter.hasNext()) {
      ...
      if (currentResource.isInternal()
          || m_includeExcludeSet.isExcluded(currentResource.getRootPath())) {
        filterIter.remove();
      }
    }
    return result;
  ...
  protected List<CmsResource> getNavigationPages() {
    ...
    try {
      CmsResource rootDefaultFile =
          m_siteGuestCms.readDefaultFile(
              m_siteGuestCms.getRequestContext().removeSiteRoot(m_baseFolderRootPath),
    ...
    List<CmsJspNavElement> navElements =
        navBuilder.getSiteNavigation(
            m_baseFolderSitePath, CmsJspNavBuilder.Visibility.includeHidden, -1);
      ...
      if (navResource.isFolder()) {
        try {
          CmsResource defaultFile =
              m_guestCms.readDefaultFile(navResource, CmsResourceFilter.DEFAULT_FILES);
          if (defaultFile != null) {
  ...
  protected boolean isAliasBelowBaseFolder(CmsAlias alias) {
    ...
    boolean isBelowBaseFolder =
        CmsStringUtil.isPrefixPath(m_baseFolderSitePath, alias.getAliasPath());
   ...
   */
  protected boolean isValidDetailPageCombination(
      CmsResource page, Locale locale, CmsResource detailRes) {

    return OpenCms.getADEManager()
        .getDetailPageHandler()
        .isValidDetailPage(m_guestCms, page, detailRes);
  }

  /**
   * Replaces the protocol/host/port of a link with the ones from the configured server URI, if it's
   * not empty.
   *
   * <p>
   *
  ...
  private void addAliasLinks(CmsUUID aliasStructureId) {
    ...
    try {
      ...
      Collection<CmsAlias> aliases =
          m_pageAliasesBelowBaseFolderByStructureId.get(aliasStructureId);
      for (CmsAlias alias : aliases) {
        String aliasLink =
            (m_siteRootLink + "/" + alias.getAliasPath()).replaceAll("(?<!:)//+", "/");
        CmsXmlSitemapUrlBean aliasUrlBean =
            new CmsXmlSitemapUrlBean(replaceServerUri(aliasLink), -1, changeFrequency, priority);
  ...
  /**
   * Gets all resources from the folder tree beneath the base folder or the shared folder which have
   * a given type.
   *
   * <p>
   *
  ...
  private List<CmsResource> getDetailResources(I_CmsResourceType type) throws CmsException {
    ...
    if (!m_detailResources.containsKey(typeName)) {
      ...
      String shared =
          CmsFileUtil.removeTrailingSeparator(OpenCms.getSiteManager().getSharedFolder());
      if (shared != null) {
  ...
  private Locale getLocale(CmsResource resource, List<CmsProperty> propertyList) {
    ...
    return OpenCms.getLocaleManager()
        .getDefaultLocale(m_guestCms, m_guestCms.getSitePath(resource));
  }

  /**
   * Reads the data necessary for building the sitemap from the VFS and initializes the internal
   * data structures.
   *
   * <p>
   *
  ...
  private void initializeFileData(String baseSitePath) throws CmsException {
    ...
    List<CmsAlias> siteAliases =
        OpenCms.getAliasManager()
            .getAliasesForSite(m_siteGuestCms, m_siteGuestCms.getRequestContext().getSiteRoot());
