 ...
 * locale.
 *
 * <p>In principle, any detail page for a type somewhere in the system could be used to display
 * contents anywhere else in the system. This sitemap generator, instead of generating all detail
 * page URLs that could possibly be generated, instead tries to find only the best candidate URL for
 * each content / locale combination.
 */
public class CmsDetailPageDuplicateEliminatingSitemapGenerator extends CmsXmlSitemapGenerator {

  /** The logger instance for this class. */
  private static final Log LOG =
      CmsLog.getLog(CmsDetailPageDuplicateEliminatingSitemapGenerator.class);
  ...
  public List<CmsXmlSitemapUrlBean> generateSitemapBeans() throws CmsException {
    ...
    // so first we group the XML sitemap beans belonging to detail pages by their locale/content
    // combination,
    // and then we sort each group by the sitemap configuration where the detail page is coming
    // from,
    ...
    for (CmsXmlSitemapUrlBean urlBean : parentResult) {
      if (urlBean.getDetailPageResource() == null) {
        ...
        String localeKey =
            urlBean.getOriginalResource().getStructureId() + "_" + urlBean.getLocale();
        detailPageBeans.put(localeKey, urlBean);
      }
    }
    Comparator<CmsXmlSitemapUrlBean> pathComparator =
        new Comparator<CmsXmlSitemapUrlBean>() {
  ...
  /**
   * @see
   *     org.opencms.site.xmlsitemap.CmsXmlSitemapGenerator#addDetailLinks(org.opencms.file.CmsResource,
   *     java.util.Locale)
   */
  @Override
  ...
  private CmsPathMap<CmsResource> getPathMapForType(String typeName) throws CmsException {
    ...
    if (!m_pathMapsByType.containsKey(typeName)) {
      CmsPathMap<CmsResource> pathMap =
          readPathMapForType(OpenCms.getResourceManager().getResourceType(typeName));
