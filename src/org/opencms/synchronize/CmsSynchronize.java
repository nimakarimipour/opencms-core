...
public class CmsSynchronize {
  ...
  private static List<I_CmsSynchronizeModification> m_synchronizeModifications =
      new ArrayList<I_CmsSynchronizeModification>();
    ...
    if ((settings != null) && (settings.isSyncEnabled())) {
      ...
      if (!destinationFolder.exists() || !destinationFolder.isDirectory()) {
        ...
        throw new CmsSynchronizeException(
            Messages.get()
                .container(Messages.ERR_RFS_DESTINATION_NOT_THERE_1, m_destinationPathInRfs));
      }
      if (!destinationFolder.canWrite()) {
        // destination folder can't be written to
        throw new CmsSynchronizeException(
            Messages.get()
                .container(Messages.ERR_RFS_DESTINATION_NO_WRITE_1, m_destinationPathInRfs));
      }
  ...
  private void copyFromRfs(@RUntainted String folder) throws CmsException {
    ...
    try {
      ...
      m_report.print(
          Messages.get().container(Messages.RPT_IMPORT_FOLDER_0), I_CmsReport.FORMAT_NOTE);
      m_report.print(
          org.opencms.report.Messages.get()
              .container(
      ...
      m_report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, foldername));
      m_report.print(
          org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0));

      CmsResource newFolder =
          m_cms.createResource(foldername, CmsResourceTypeFolder.RESOURCE_TYPE_ID);
      ...
      CmsSynchronizeList sync =
          new CmsSynchronizeList(
              folder, resourcename, newFolder.getDateLastModified(), fsFile.lastModified());
  ...
  private void createNewLocalFile(@RUntainted File newFile) throws CmsException {
    ...
    try {
      File parentFolder =
          new File(
              newFile
                  ...
                  .substring(0, newFile.getPath().lastIndexOf(File.separator)));
      parentFolder.mkdirs();
      ...
      throw new CmsSynchronizeException(
          Messages.get()
              .container(Messages.ERR_CREATE_FILE_1, this.getClass().getName(), newFile.getPath()),
          e);
    } finally {
      if (fOut != null) {
        try {
    ...
    try {
      ...
      if (!res.getState().isDeleted()) {
        ...
        if (res.isFile()) {
          ...
          m_report.print(
              Messages.get().container(Messages.RPT_EXPORT_FILE_0), I_CmsReport.FORMAT_NOTE);
          m_report.print(
              org.opencms.report.Messages.get()
                  .container(org.opencms.report.Messages.RPT_ARGUMENT_1, m_cms.getSitePath(res)));
          m_report.print(
              Messages.get().container(Messages.RPT_TO_FS_AS_0), I_CmsReport.FORMAT_NOTE);
          m_report.print(
              org.opencms.report.Messages.get()
                  .container(
                      org.opencms.report.Messages.RPT_ARGUMENT_1,
            ...
            try {
              ...
              if (LOG.isWarnEnabled()) {
                LOG.warn(
                    Messages.get()
                        .getBundle()
                        .key(Messages.LOG_SYNCHRONIZE_EXPORT_FAILED_1, res.getRootPath()),
                    e);
              }
              break;
            }
          ...
          m_report.print(
              Messages.get().container(Messages.RPT_EXPORT_FOLDER_0), I_CmsReport.FORMAT_NOTE);
          m_report.print(
              org.opencms.report.Messages.get()
                  .container(org.opencms.report.Messages.RPT_ARGUMENT_1, m_cms.getSitePath(res)));
          m_report.print(
  ...
  private void importToVfs(@RUntainted File fsFile, @RUntainted String resName, @RUntainted String folder) throws CmsException {
    ...
    try {
      ...
      if (fsFile.isFile()) {
        m_report.print(
            Messages.get().container(Messages.RPT_IMPORT_FILE_0), I_CmsReport.FORMAT_NOTE);
      } else {
        m_report.print(
            Messages.get().container(Messages.RPT_IMPORT_FOLDER_0), I_CmsReport.FORMAT_NOTE);
      }
      ...
      CmsResource newFile =
          m_cms.createResource(
              translate(folder) + filename, resType, content, new ArrayList<CmsProperty>());

      m_report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, m_cms.getSitePath(newFile)));
      m_report.print(
          org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0));
      ...
      CmsSynchronizeList syncList =
          new CmsSynchronizeList(
              resName, translate(resName), newRes.getDateLastModified(), fsFile.lastModified());
  ...
  /**
   * Reads the synchronization list from the last sync process form the file system and stores the
   * information in a HashMap.
   *
   * <p>Filenames are stored as keys, CmsSynchronizeList objects as values.
   *
      ...
      try {
        ...
        while (line != null) {
          ...
          if (line != null) {
            ...
            CmsSynchronizeList sync =
                new CmsSynchronizeList(resName, tranResName, modifiedVfs, modifiedFs);
            syncList.put(translate(resName), sync);
          }
        }
      } catch (IOException e) {
        throw new CmsSynchronizeException(
            Messages.get().container(Messages.ERR_READ_SYNC_LIST_0), e);
      } finally {
  ...
  private void removeFromRfs(String folder) throws CmsException {
    ...
    for (int i = 0; i < res.length; i++) {
      ...
      if (sync != null) {
        ...
        if (res[i].isFile()) {
          m_report.print(
              Messages.get().container(Messages.RPT_DEL_FS_FILE_0), I_CmsReport.FORMAT_NOTE);
        } else {
          m_report.print(
              Messages.get().container(Messages.RPT_DEL_FS_FOLDER_0), I_CmsReport.FORMAT_NOTE);
        }
        m_report.print(
            org.opencms.report.Messages.get()
                .container(
   ...
   *
   * <p>During the synchronization process, the following actions will be done:
   *
   * <p>
   *
   * <ul>
   *   <li>Export modified resources from the VFS to the FS
   *   <li>Update resources in the VFS if the corresponding resource in the FS has changed
   *   <li>Delete resources in the VFS if the corresponding resource in the FS has been deleted
   * </ul>
   *
   * @param folder The folder in the VFS to be synchronized with the FS
   * @throws CmsException if something goes wrong
   ...
   *
   * <p>This is necessary since the server RFS does allow different naming conventions than the VFS.
   *
  ...
  /**
   * Imports a resource from the FS to the VFS and updates the synchronization lists.
   *
   * <p>
   *
   * @param res the resource to be exported
  ...
  private void updateFromRfs(CmsResource res) throws CmsSynchronizeException, CmsException {
    ...
    try {
      ...
      throw new CmsSynchronizeException(
          Messages.get().container(Messages.ERR_IMPORT_1, fsFile.getName()));
    }
      ...
      try {
        ...
        if (LOG.isInfoEnabled()) {
          LOG.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_SYNCHRONIZE_UPDATE_FAILED_1, res.getRootPath()),
              e);
        }
        break;
      }
  ...
  /**
   * Writes the synchronization list of the current sync process to the server file system.
   *
   * <p>The file can be found in the synchronization folder
   *
   * @throws CmsException if something goes wrong
   */
