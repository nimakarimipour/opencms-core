...
public class CmsSynchronizeSettings implements Serializable {
  ...
  public void checkValues(CmsObject cms) throws CmsException {
    ...
    if (isEnabled() && (m_destinationPathInRfs == null)) {
      ...
      throw new CmsSynchronizeException(
          Messages.get().container(Messages.ERR_NO_RFS_DESTINATION_0));
    }
   ...
   *
   * <p>The objects in the list are of type <code>{@link String}</code>.
   *
  ...
  /**
   * Returns <code>true</code> if the synchonization is enabled.
   *
   * <p>The synchonization is enabled if both source and destination path is set, and also the
   ...
   * <p>
   *
   * @param destinationPathInRfs the destination path of the synchronization in the "real" file
   *     system to set
   */
   ...
   *
   * <p>The objects in the list must be of type <code>{@link String}</code>.
   *
  ...
  /**
   * Optimizes the list of VFS source files by removing all resources that have a parent resource
