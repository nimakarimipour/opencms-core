...
public class CmsCmisResourceHelper implements I_CmsCmisObjectHelper {
    ...
    try {
      ...
      boolean canWrite =
          !cms.getRequestContext().getCurrentProject().isOnlineProject()
              ...
              && cms.hasPermissions(
                  file, CmsPermissionSet.ACCESS_WRITE, false, CmsResourceFilter.DEFAULT);
  ...
  protected ObjectData collectObjectData(
    ...
    if ((includeRelationships != null) && (includeRelationships != IncludeRelationships.NONE)) {
      ...
      List<ObjectData> relationData =
          m_repository.getRelationshipObjectData(
              context, cms, resource, direction, CmsCmisUtil.splitFilter("*"), false);
  ...
  protected Properties collectProperties(
      CmsObject cms, CmsResource resource, Set<String> orgfilter, ObjectInfoImpl objectInfo) {
    ...
    try {
      ...
      addPropertyDateTime(
          tm, result, typeId, filter, PropertyIds.LAST_MODIFICATION_DATE, lastModified);
      ...
      if (resource.isFolder()) {
        ...
        addPropertyId(
            tm, result, typeId, filter, PropertyIds.BASE_TYPE_ID, BaseTypeId.CMIS_FOLDER.value());
        addPropertyId(
            tm,
            result,
            typeId,
        ...
        addPropertyString(
            tm, result, typeId, filter, PropertyIds.PATH, (path.length() == 0 ? "/" : path));
        ...
        addPropertyIdList(
            tm, result, typeId, filter, PropertyIds.ALLOWED_CHILD_OBJECT_TYPE_IDS, null);
      } else {
        // base type and type name
        addPropertyId(
            tm, result, typeId, filter, PropertyIds.BASE_TYPE_ID, BaseTypeId.CMIS_DOCUMENT.value());
        addPropertyId(
            tm,
            result,
            typeId,
        ...
        addPropertyString(
            tm, result, typeId, filter, PropertyIds.VERSION_LABEL, resource.getName());
        addPropertyId(
            tm,
            result,
            typeId,
        ...
        addPropertyBoolean(
            tm, result, typeId, filter, PropertyIds.IS_VERSION_SERIES_CHECKED_OUT, false);
        addPropertyString(
            tm, result, typeId, filter, PropertyIds.VERSION_SERIES_CHECKED_OUT_BY, null);
        addPropertyString(
            tm, result, typeId, filter, PropertyIds.VERSION_SERIES_CHECKED_OUT_ID, null);
        addPropertyString(tm, result, typeId, filter, PropertyIds.CHECKIN_COMMENT, "");
        addPropertyInteger(
            tm, result, typeId, filter, PropertyIds.CONTENT_STREAM_LENGTH, resource.getLength());
        addPropertyString(
            tm,
            result,
            typeId,
            ...
            OpenCms.getResourceManager()
                .getMimeType(resource.getRootPath(), null, CmsResourceManager.MIMETYPE_TEXT));
        addPropertyString(
            tm, result, typeId, filter, PropertyIds.CONTENT_STREAM_FILE_NAME, resource.getName());
        objectInfo.setHasContent(true);
      ...
      Set<String> propertiesToAdd =
          new LinkedHashSet<String>(m_repository.getTypeManager().getCmsPropertyNames());
      ...
      for (String propName : propertiesToAdd) {
        addPropertyString(
            tm, result, typeId, filter, CmsCmisTypeManager.PROPERTY_PREFIX + propName, null);
      }

      // inherited OpenCms properties
      List<CmsProperty> inheritedProps = cms.readPropertyObjects(resource, true);
      Set<String> inheritedPropertiesToAdd =
          new LinkedHashSet<String>(m_repository.getTypeManager().getCmsPropertyNames());
      ...
      for (String propName : inheritedPropertiesToAdd) {
        addPropertyString(
            tm, result, typeId, filter, CmsCmisTypeManager.INHERITED_PREFIX + propName, null);
      }
      ...
      CmsCmisUtil.addDynamicProperties(
          cms, m_repository.getTypeManager(), result, typeId, resource, filter);
      return result;
  ...
  protected List<RenditionData> collectRenditions(
    ...
    List<I_CmsCmisRenditionProvider> providers =
        m_repository.getRenditionProviders(new CmsCmisRenditionFilter(renditionFilterString));
