 ...
 *
 * <p>Since CMIS requires any object to have an ID by which it is accessed, but OpenCms relations
 * are not addressable by ids, we invent an artificial relation id string of the form
 * REL_(SOURCE_ID)_(TARGET_ID)_(TYPE).
 *
 * <p>
 */
public class CmsCmisRelationHelper implements I_CmsCmisObjectHelper {
        ...
        List<CmsRelation> relations =
            cms.getRelationsForResource(
                ...
                CmsRelationFilter.TARGETS
                    .filterStructureId(m_targetId)
                    .filterType(getRelationType(m_relType)));
  ...
  public static final @RUntainted Pattern RELATION_PATTERN =
      Pattern.compile("^REL_(" + CmsUUID.UUID_REGEX + ")_(" + CmsUUID.UUID_REGEX + ")_(.*)$");
  ...
  /**
   * @see org.opencms.cmis.I_CmsCmisObjectHelper#deleteObject(org.opencms.cmis.CmsCmisCallContext,
  ...
  public void deleteObject(CmsCmisCallContext context, @RUntainted String objectId, boolean allVersions) {
    ...
    try {
      ...
      try {
        CmsRelationFilter relFilter =
            CmsRelationFilter.ALL
                .filterType(getRelationType(rk.getRelType()))
                .filterStructureId(rk.getTargetId());
  ...
  /**
   * @see org.opencms.cmis.I_CmsCmisObjectHelper#getAcl(org.opencms.cmis.CmsCmisCallContext,
   *     java.lang.String, boolean)
   */
  ...
  /**
   * @see
   *     org.opencms.cmis.I_CmsCmisObjectHelper#getAllowableActions(org.opencms.cmis.CmsCmisCallContext,
   *     java.lang.String)
   */
  ...
  /**
   * @see org.opencms.cmis.I_CmsCmisObjectHelper#getObject(org.opencms.cmis.CmsCmisCallContext,
   ...
   */
  protected AllowableActions collectAllowableActions(
      CmsObject cms, CmsResource file, CmsRelation relation) {
    ...
    try {
      ...
      addAction(
          aas, Action.CAN_DELETE_OBJECT, canWrite && !relation.getType().isDefinedInContent());
  ...
  protected Properties collectProperties(
    ...
    try {
      ...
      addPropertyId(
          tm, result, typeId, filter, PropertyIds.SOURCE_ID, relation.getSourceId().toString());
      addPropertyId(
          tm, result, typeId, filter, PropertyIds.TARGET_ID, relation.getTargetId().toString());
  ...
  /**
   * Creates a relation id from the given OpenCms relation object.
   *
   * <p>
   *
