 ...
 *
 * <p>When a token is created for a user, a special additional info item is stored on the user, such
 * that the token uniquely identifies that info item. The value of the info item is the expiration
 * date of the token. A token is validated by looking up the additional info item for the user and
 * checking whether the token is still valid according to the stored expiration date.
 *
 * <p>
 */
public class CmsPersistentLoginTokenHandler {
  ...
  public static class Token {
    ...
    /**
     * Decodes the user name from a hexadecimal string, and returns null if this is not possible.
     *
     * <p>
     *
    ...
    /**
     * Encodes a user name as a hex string for storing in the cookie.
     *
     * <p>
     *
  ...
  /**
   * Generates a new login token for a given user and registers the token in the user's additional
   * info.
   *
   * <p>
   *
   ...
   *
   * <p>This method does not write the user back to the database.
   *
   ...
   *
   * <p>Returns null if no user matching the token is found, or if the token for the user is expired
   *
   * @param tokenString the token for which to find the matching user
   * @return the matching user for the token, or null if no matching user was found or the token is
   *     expired
   */
