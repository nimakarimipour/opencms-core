...
public class CmsDefaultAuthorizationHandler extends A_CmsAuthorizationHandler {
  ...
  /**
   * Configuration parameter to control for which paths startup settings should be applied after
   * HTTP Basic authentication.
   */
  public static final String PARAM_HTTP_BASICAUTH_USESTARTSETTINGS_PATHS =
      "http.basicauth.usestartsettings.paths";

  /**
   * Configuration parameter to control for which users startup settings should be applied after
   * HTTP Basic authentication.
   */
  public static final String PARAM_HTTP_BASICAUTH_USESTARTSETTINGS_USERS =
      "http.basicauth.usestartsettings.users";
   ...
   * @param pathSpec a comma separated list of path prefixes, which may contain %(contextPath)
   *     macros
   ...
   *
   * <p>The user specification string is a comma-separed list of entries of the form TYPE.Name,
   * where TYPE is either ROLE, GROUP, or USER. The method returns true if the user matches any of
   * the groups, roles, or user names from this list.
   *
      ...
      if ("*".equals(userSpecEntry)) {
        ...
        if (groupsOfUser == null) {
          try {
            groupsOfUser =
                cms
                    .getGroupsOfUser(cms.getRequestContext().getCurrentUser().getName(), false)
                    .stream()
  ...
  /**
   * @see org.opencms.security.I_CmsAuthorizationHandler#getLoginFormURL(java.lang.String,
  ...
  /**
   * @see
   *     org.opencms.security.I_CmsAuthorizationHandler#initCmsObject(javax.servlet.http.HttpServletRequest,
   *     org.opencms.security.I_CmsAuthorizationHandler.I_PrivilegedLoginAction)
   */
  public CmsObject initCmsObject(
      HttpServletRequest request, I_CmsAuthorizationHandler.I_PrivilegedLoginAction loginAction) {

    return initCmsObject(request);
  }

  /** @see I_CmsAuthorizationHandler#initCmsObject(HttpServletRequest, String, String) */
  public CmsObject initCmsObject(HttpServletRequest request, String userName, String pwd)
      throws CmsException {
  ...
  public void requestAuthorization(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_AUTHENTICATE_PROPERTY_2, loginFormURL, req.getRequestURI()));
    }
  ...
  /**
   * Checks if the current request contains HTTP basic authentication information in the headers, if
   * so the user is tried to log in with this data, and on success a session is generated.
   *
   * <p>
   *
  ...
  protected CmsObject checkBasicAuthorization(HttpServletRequest req) {
    ...
    try {
      ...
      boolean isWorkplace =
          requestUri.startsWith(OpenCms.getSystemInfo().getWorkplaceContext())
              || requestUri.startsWith(
                  CmsStringUtil.joinPaths(
                      OpenCms.getSystemInfo().getOpenCmsContext(), "/system/workplace"));
   ...
   *
   * <p>This method will not be called for workplace requests; for these the start settings will
   * always be used.
   *
