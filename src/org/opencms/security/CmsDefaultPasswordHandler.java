...
public class CmsDefaultPasswordHandler
    ...
    if (DIGEST_TYPE_PLAIN.equals(m_digestType)) {
      ...
      try {
        ...
        if (useFallback) {
          try {
            success =
                digestedPassword.equals(digest(plainPassword, m_scryptFallback, m_inputEncoding));
  ...
  public String digest(String password, @RUntainted String digestType, @RUntainted String inputEncoding)
    ...
    try {
      ...
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_UNSUPPORTED_ALGORITHM_1, digestType);
      ...
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_UNSUPPORTED_PASSWORD_ENCODING_1, inputEncoding);
      if (LOG.isErrorEnabled()) {
        LOG.error(message.key(), e);
      }
      throw new CmsPasswordEncryptionException(message, e);
  ...
  /**
   * @see
   *     org.opencms.security.I_CmsPasswordSecurityEvaluator#evaluatePasswordSecurity(java.lang.String)
   */
  ...
  public String getPasswordSecurityHint(@RUntainted Locale locale) {
    ...
    return Messages.get()
        .getBundle(locale)
        .key(Messages.GUI_PASSWORD_SECURITY_HINT_1, Integer.valueOf(PASSWORD_SECURE_LENGTH));
  }
  ...
  public void initConfiguration() {
    ...
    if (LOG.isDebugEnabled()) {
      CmsMessageContainer message =
          Messages.get().container(Messages.LOG_INIT_CONFIG_CALLED_1, this);
    ...
    if (scryptSettings != null) {
      ...
      if (settings.length == 3) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_SCRYPT_PARAMETERS_1, scryptSettings));
        }
      }
    }
    ...
    if (scryptFallback != null) {
      ...
      try {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_SCRYPT_PARAMETERS_1, scryptFallback));
        }
      }
    }
  }
  ...
  public void validatePassword(String password) throws CmsSecurityException {
    ...
    if ((password == null) || (password.length() < PASSWORD_MIN_LENGTH)) {
      throw new CmsSecurityException(
          Messages.get()
              .container(Messages.ERR_PASSWORD_TOO_SHORT_1, new Integer(PASSWORD_MIN_LENGTH)));
    }
  }
}
