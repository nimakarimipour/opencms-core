 ...
 * <p>
 *
 * <p>To each single resource, access control entries of type <code>CmsAccessControlEntry</code> can
 * be assigned. An access control entry defines the permissions (both allowed and explicitly denied)
 * of a user or group for this resource.
 *
 * <p>By calling the method <code>getAccessControlList</code> the list is generated on the resource.
 * It contains the result of merging both access control entries defined immediately on the resource
 * and inherited along the folder hierarchie in the OpenCms virtual file system (controlled by flags
 * in the entry).
 *
 * <p>To check the permissions of a user on a distinct resource, the method <code>hasPermissions
 * </code> in the driver manager is called in each operation. This method acts as access guard and
 * matches the required permissions for the operation against the allowed and denied permissions
...
public class CmsAccessControlList implements Serializable {
  ...
  public CmsPermissionSetCustom getPermissions(
      CmsUser user, List<CmsGroup> groups, List<CmsRole> roles) {
   ...
   * Calculates the permissions of the given user and his groups from the access control list.
   *
   * <p>The permissions are returned as permission string in the format {{+|-}{r|w|v|c|i}}*.
   *
   * @param user the user
   * @param groups the groups of this user
   * @param roles the roles of this user
  ...
  /**
   * Sets the allowed permissions of a given access control entry as allowed permissions in the
   * access control list.
   *
   * <p>The denied permissions are left unchanged.
   *
  ...
  /**
   * Sets the denied permissions of a given access control entry as denied permissions in the access
   * control list.
   *
   * <p>The allowed permissions are left unchanged.
   *
   * @param entry the access control entry
   */
