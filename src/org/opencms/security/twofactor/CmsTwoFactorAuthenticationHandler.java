 ...
 *
 * <p>This class can both set up a TOTP second factor for a user, as well as be used to authenticate
 * a user using the verification code generated using their second factor.
 */
public class CmsTwoFactorAuthenticationHandler {
   ...
   */
  public CmsTwoFactorAuthenticationHandler(
      CmsObject adminCms, CmsTwoFactorAuthenticationConfig config) {
  ...
  /**
   * Generates the information needed to share a secret with the user for the purpose of setting up
   * 2FA.
   *
   ...
   *
   * <p>For users excluded from two-factor authentication, this will usually return false, while for
   * users who should use two-factor authentication, the result depends on whether the second factor
  ...
  /**
   * Deletes the two-factor authentication in the user object, but does not write the user to the
   * database.
   *
  ...
  /**
   * Sets up the second factor for the given user, and immediately verifies it with the
   * authentication code given.
   *
   ...
   * @throws CmsSecondFactorSetupException in unexpected circumstances, e.g. if the user already has
   *     a second factor set up or there is no authentication code
   */
  public boolean setUpAndVerifySecondFactor(CmsUser newUser, CmsSecondFactorInfo code)
    ...
    try {
      ...
      // we store the user name (and check it later) so you can't just transfer the stored second
      // factor data to a different user via the GUI
  ...
  /**
   * Gets called when a user is changed so we can check if the second factor information was changed
   * and generate appropriate log messages.
   *
   ...
   *
   * <p>Note that this method assumes that two-factor authentication should be applied to the given
   * user, and always checks the second factor.
   *
   * @param user the user
