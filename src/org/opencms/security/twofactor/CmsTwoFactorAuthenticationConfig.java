...
public class CmsTwoFactorAuthenticationConfig {
  ...
  /**
   * Gets the secret key which is used to encrypt/decrypt two-factor authentication information
   * stored in the user's additional infos.
   *
  ...
  private CmsTwoFactorAuthenticationUserPolicy parsePolicy(Element parent) {
    ...
    List<CmsTwoFactorAuthenticationUserPolicy.Rule> includes =
        parsePolicyRules(parent, "include-users/*");
    List<CmsTwoFactorAuthenticationUserPolicy.Rule> excludes =
        parsePolicyRules(parent, "exclude-users/*");
   ...
   */
  private List<CmsTwoFactorAuthenticationUserPolicy.Rule> parsePolicyRules(
      Element parent, String xpath) {

    return parent.selectNodes(xpath).stream()
        .map(
            node -> {
