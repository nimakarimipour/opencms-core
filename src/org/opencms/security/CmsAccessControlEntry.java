 ...
 *
 * <p>Besides the <code>CmsPermissionSet</code> to define the permissions, the access control entry
 * contains the UUID of the resource and of the principal (user or group) who has the defined
 * permissions. Since the principal is identified by its UUID, any other entity may act as principal
 * also.
 *
...
public class CmsAccessControlEntry {
  ...
  public static final @RUntainted int ACCESS_FLAGS_GROUP = 32;
   ...
   * ACE comparator.
   *
   * <p>Sorts the given list of {@link CmsAccessControlEntry} objects.
   *
   * <p>The 'overwrite all' ace in first place, the 'all others' ace in second place.
   *
   * <p>
   */
  public static final Comparator<CmsAccessControlEntry> COMPARATOR_ACE =
      new Comparator<CmsAccessControlEntry>() {
   ...
   * <p>
   */
  public static final Comparator<CmsUUID> COMPARATOR_PRINCIPALS =
      new Comparator<CmsUUID>() {
  ...
  /**
   * Constructor to create a new access control entry for a given resource based on an existing
   ...
   *
   * <p>Permissions are specified as permission set, flags as bitset.
   *
   * @param resource the resource
   ...
   */
  public CmsAccessControlEntry(
      @RUntainted CmsUUID resource, @RUntainted CmsUUID principal, CmsPermissionSet permissions, @RUntainted int flags) {
   ...
   * Constructor to create a new access control entry on a given resource and a given principal.
   *
   * <p>Permissions and flags are specified as bitsets.
   *
   ...
   */
  public CmsAccessControlEntry(
      @RUntainted CmsUUID resource, @RUntainted CmsUUID principal, int allowed, int denied, @RUntainted int flags) {
   ...
   * Constructor to create a new access control entry on a given resource and a given principal.
   *
   * <p>Permission and flags are specified as string of the format {{+|-}{r|w|v|c|i}}*
   *
   * @param resource the resource
   * @param principal the id of a principal (user or group)
  ...
  static {
    ...
    PRINCIPAL_OVERWRITE_ALL_ID =
        CmsUUID.getConstantUUID(PRINCIPAL_OVERWRITE_ALL_NAME.toLowerCase());
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof CmsAccessControlEntry) {
      ...
      if (other.getPermissions().getAllowedPermissions()
          != getPermissions().getAllowedPermissions()) {
        return false;
      }
      if (other.getPermissions().getDeniedPermissions()
   ...
   *
   * <p>Note: to check if an access control entry is inherited, also the resource id and the id of
   * the current resource must be different.
   *
