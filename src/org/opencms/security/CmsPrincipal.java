...
public abstract class CmsPrincipal implements I_CmsPrincipal, Comparable<I_CmsPrincipal> {
  ...
  /**
   * Filters out all principals that do not have the given flag set, but leaving principals with
   * flags less than <code>{@link I_CmsPrincipal#FLAG_CORE_LIMIT}</code> untouched.
   *
   * <p>The given parameter list is directly modified, so the returned list is the same object as
   * the input list.
   *
   * <p>
   *
   ...
   */
  public static List<? extends CmsPrincipal> filterCoreFlag(
      List<? extends CmsPrincipal> principals, int flag) {
  ...
  /**
   * Filters out all groups with flags greater than <code>{@link I_CmsPrincipal#FLAG_CORE_LIMIT}
   * </code>.
   *
   * <p>The given parameter list is directly modified, so the returned list is the same object as
   * the input list.
  ...
  /**
   * Filters out all users with flags greater than <code>{@link I_CmsPrincipal#FLAG_CORE_LIMIT}
   * </code>.
   *
   * <p>The given parameter list is directly modified, so the returned list is the same object as
   * the input list.
   ...
   * @return the filtered principal list
   */
  public static List<? extends CmsPrincipal> filterFlag(
      List<? extends CmsPrincipal> principals, int flag) {
  ...
  /**
   * Returns the provided group name prefixed with <code>{@link I_CmsPrincipal#PRINCIPAL_GROUP}.
   * </code>.
   *
   * <p>
   *
   * @param name the name to add the prefix to
   * @return the provided group name prefixed with <code>{@link I_CmsPrincipal#PRINCIPAL_GROUP}.
   *     </code>
   */
  ...
  /**
   * Returns the provided user name prefixed with <code>{@link I_CmsPrincipal#PRINCIPAL_USER}.
   * </code>.
   *
   * <p>
   *
   * @param name the name to add the prefix to
   * @return the provided user name prefixed with <code>{@link I_CmsPrincipal#PRINCIPAL_USER}.
   *     </code>
   */
  ...
  /**
   * Utility function to read a prefixed principal from the OpenCms database using the provided
   * OpenCms user context.
   *
   * <p>The principal must be either prefixed with <code>{@link I_CmsPrincipal#PRINCIPAL_GROUP}.
   * </code> or <code>{@link I_CmsPrincipal#PRINCIPAL_USER}.</code>.
   *
   * <p>
   *
  ...
  public static I_CmsPrincipal readPrefixedPrincipal(CmsObject cms, String name)
    ...
    throw new CmsDbEntryNotFoundException(
        Messages.get().container(Messages.ERR_INVALID_PRINCIPAL_1, name));
  }

  /**
   * Utility function to read a principal by its id from the OpenCms database using the provided
   * OpenCms user context.
   *
   * <p>
   *
  ...
  public static I_CmsPrincipal readPrincipal(CmsObject cms, CmsUUID id) throws CmsException {
    ...
    // invalid principal name was given
    throw new CmsDbEntryNotFoundException(
        Messages.get().container(Messages.ERR_INVALID_PRINCIPAL_1, id));
  }
   ...
   *
   * <p>The type must either be <code>{@link I_CmsPrincipal#PRINCIPAL_GROUP}</code> or <code>
   * {@link I_CmsPrincipal#PRINCIPAL_USER}</code>.
   *
   * <p>
   *
   * @param cms the OpenCms user context to use when reading the principal
  ...
  /**
   * Returns the translated display name of this principal if it is a group and the display name
   * otherwise.
   *
   * <p>
   *
