...
public class CmsRoleManager {
  ...
  /**
   * Checks if the user of this OpenCms context is a member of the given role for the given
   * organizational unit.
   *
   * <p>The user must have the given role in at least one parent organizational unit.
   *
   * <p>
   *
   * @param cms the opencms context
   ...
   * resource.
   *
   * <p>The user must have the given role in at least one organizational unit to which this resource
   * belongs.
   *
   * <p>
   *
  ...
  /**
   * Returns all groups of organizational units for which the current user has the {@link
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsGroup> getManageableGroups(CmsObject cms, String ouFqn, boolean includeSubOus)
      throws CmsException {

    List<CmsGroup> groups = new ArrayList<CmsGroup>();
    Iterator<CmsOrganizationalUnit> it =
        getOrgUnitsForRole(cms, CmsRole.ACCOUNT_MANAGER.forOrgUnit(ouFqn), includeSubOus)
            .iterator();
    while (it.hasNext()) {
  ...
  public List<CmsOrganizationalUnit> getManageableOrgUnits(
      CmsObject cms, String ouFqn, boolean includeSubOus, boolean includeWebusers)
      throws CmsException {
  ...
  /**
   * Returns all users of organizational units for which the current user has the {@link
   * CmsRole#ACCOUNT_MANAGER} role.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsUser> getManageableUsers(CmsObject cms, String ouFqn, boolean includeSubOus)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsUser> getManageableUsers(
      CmsObject cms, String ouFqn, boolean includeSubOus, boolean includeWebusers)
      throws CmsException {

    List<CmsOrganizationalUnit> ous =
        getManageableOrgUnits(cms, ouFqn, includeSubOus, includeWebusers);
  ...
  public List<CmsOrganizationalUnit> getOrgUnitsForRole(
      CmsObject cms, CmsRole role, boolean includeSubOus) throws CmsException {

    return m_securityManager.getOrgUnitsForRole(cms.getRequestContext(), role, includeSubOus);
  }

  /**
   * Returns the groups which constitute a given role, i.e. the set of groups such that a member of
   * any of them has the given role.
   *
   * <p>
   *
   ...
   * @throws CmsException if something goes wrong
   */
  public Set<CmsGroup> getRoleGroups(CmsObject cms, CmsRole role, boolean directUsersOnly)
      throws CmsException {
   ...
   */
  public List<CmsRole> getRoles(CmsObject cms, String ouFqn, boolean includeSubOus)
      throws CmsException {

    CmsOrganizationalUnit ou = OpenCms.getOrgUnitManager().readOrganizationalUnit(cms, ouFqn);
    List<CmsGroup> groups =
        m_securityManager.getGroups(cms.getRequestContext(), ou, includeSubOus, true);
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsRole> getRolesForResource(CmsObject cms, CmsUser user, CmsResource resource)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsRole> getRolesForResource(CmsObject cms, String userFqn, String resourceName)
      throws CmsException {
   ...
   * @param directRolesOnly if set only the direct assigned roles will be returned, if not also
   *     indirect roles
  ...
  public List<CmsRole> getRolesOfUser(
    ...
    if (!recursive) {
      groups =
          m_securityManager.getGroupsOfUser(
   ...
   *
   * <p>Users that are "indirectly" in the role are not returned in the result.
   *
   * <p>
   *
   * @param cms the opencms context
  ...
  public List<CmsUser> getUsersOfRole(
      CmsObject cms, CmsRole role, boolean includeOtherOuUsers, boolean directUsersOnly)
      throws CmsException {

    return m_securityManager.getUsersOfGroup(
        cms.getRequestContext(), role.getGroupName(), includeOtherOuUsers, directUsersOnly, true);
  }
   ...
   * @param cms the opencms context
   * @param role the role to check
   * @return <code>true</code> if the given context user has the given role in the given
   *     organizational unit
   */
  public boolean hasRole(CmsObject cms, CmsRole role) {

    return m_securityManager.hasRole(
        cms.getRequestContext(), cms.getRequestContext().getCurrentUser(), role);
  }
  ...
  public boolean hasRoleForResource(CmsObject cms, CmsRole role, CmsResource resource) {
    ...
    return m_securityManager.hasRoleForResource(
        cms.getRequestContext(), cms.getRequestContext().getCurrentUser(), role, resource);
  }
  ...
  public boolean hasRoleForResource(
      CmsObject cms, String userName, CmsRole role, String resourceName) {
  ...
  public void removeUserFromRole(CmsObject cms, CmsRole role, String username) throws CmsException {
    ...
    m_securityManager.removeUserFromGroup(
        cms.getRequestContext(), username, role.getGroupName(), true);
  }
}
