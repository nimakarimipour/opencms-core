...
public class CmsDefaultPermissionHandler implements I_CmsPermissionHandler {
  ...
  /**
   * @see org.opencms.security.I_CmsPermissionHandler#hasPermissions(org.opencms.db.CmsDbContext,
   *     org.opencms.file.CmsResource, org.opencms.security.CmsPermissionSet,
   *     org.opencms.security.I_CmsPermissionHandler.LockCheck, org.opencms.file.CmsResourceFilter)
   */
  public CmsPermissionCheckResult hasPermissions(
    ...
    String cacheKey =
        m_keyGenerator.getCacheKeyForUserPermissions(keyPrefix, dbc, resource, requiredPermissions);
    ...
    boolean writeRequired =
        requiredPermissions.requiresWritePermission()
    ...
    if (requiredPermissions.requiresDirectPublishPermission()) {
      ...
      if ((permissions.getPermissions() & CmsPermissionSet.PERMISSION_DIRECT_PUBLISH) == 0) {
        // but the user has no direct publish permission, so check if the user has the project
        // manager role
        boolean canIgnorePublishPermission =
            m_securityManager.hasRoleForResource(
                dbc, dbc.currentUser(), CmsRole.PROJECT_MANAGER, resource);
          ...
          Iterator<CmsProject> itProjects =
              m_driverManager
                  .getAllManageableProjects(
                      dbc, m_driverManager.readOrganizationalUnit(dbc, user.getOuFqn()), true)
                  .iterator();
          while (itProjects.hasNext()) {
            CmsProject project = itProjects.next();
            if (CmsProject.isInsideProject(
                m_driverManager.readProjectResources(dbc, project), resource)) {
    ...
    try {
      ...
      throw new CmsInitException(
          org.opencms.main.Messages.get()
              .container(org.opencms.main.Messages.ERR_CRITICAL_CLASS_CREATION_1, className),
          e);
    }
  }
}
