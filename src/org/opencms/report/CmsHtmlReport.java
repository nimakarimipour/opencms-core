...
public class CmsHtmlReport extends A_CmsReport {
  ...
  /**
   * Constant for a HTML linebreak with added "real" line break- traditional style for report
   * threads that still use XML templates for their output.
   */
  ...
  /**
   * Counter to remember what is already shown, indicates the next index of the m_content list that
   * has to be reported.
   */
   ...
   * @param locale the locale to use for the output language
   * @param siteRoot the site root of the user who started this report (may be <code>null</code>)
   * @param writeHtml if <code>true</code>, this report should generate HTML instead of JavaScript
   *     output
   ...
   * <p>
   *
   * @return <code>true</code> if the report writes html, and <code>false</code> if the report
  ...
  /**
   * Output helper method to format a reported {@link Throwable} element.
   *
   * <p>This method ensures that exception stack traces are properly escaped when they are added to
   * the report.
   *
   * <p>There is a member variable {@link #m_showExceptionStackTrace} in this class that controls if
   * the stack track is shown or not. In a later version this might be configurable on a per-user
   * basis.
   *
   * <p>
   *
