...
public abstract class A_CmsReportThread extends Thread implements I_CmsReportThread {
  ...
  /**
   * Returns the error exception in case there was an error during the execution of this Thread,
   * null otherwise.
   *
   * <p>
   *
   ...
   *
   * <p>If this returns null, report output is not sent to a logger.
   *
   ...
   *
   * <p>A OpenCms deamon Thread (the "Grim Reaper") will collect all doomed Threads, i.e. threads
   * that are not longer active for some time.
   *
   * <p>
   *
  ...
  protected void initHtmlReport(Locale locale) {
    ...
    m_report =
        new CmsWorkplaceReport(locale, m_cms.getRequestContext().getSiteRoot(), getLogChannel());
  }

  /**
   * Initialize a HTML report for this Thread.
   *
   * <p>This method is reserved for older report threads that still use XML templates to generate
   * their output.
   *
   * <p><em>This report type will not work correctly with the new workplace, so don't use it
   * anymore.</em>
   *
   * @param locale the locale for the report output messages
   */
