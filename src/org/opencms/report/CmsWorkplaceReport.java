 ...
 *
 * <p>The difference to the older CmsHtmlReport class is that this can be used from both the old and
 * the new Vaadin-based workplace, without having to decide when the report is created. Instead, the
 * 'client' requesting report updates has to pass in a report update formatter instance, which is
 * then used to format the report update for each case.
 */
public class CmsWorkplaceReport extends A_CmsReport {
  ...
  /**
   * @see
   *     org.opencms.report.A_CmsReport#getReportUpdate(org.opencms.report.I_CmsReportUpdateFormatter)
   */
  @Override
