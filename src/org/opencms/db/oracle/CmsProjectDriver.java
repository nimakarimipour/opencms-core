...
public class CmsProjectDriver extends org.opencms.db.generic.CmsProjectDriver {
  ...
  public void writePublishReport(CmsDbContext dbc, CmsUUID publishId, byte[] content)
    ...
    internalWritePublishJobData(
        dbc, publishId, "C_ORACLE_PUBLISHJOB_UPDATE_PUBLISHREPORT", "PUBLISH_REPORT", content);
  }

  /**
   * Writes data for a publish job.
   *
   * <p>
   *
  ...
  private void internalWritePublishJobData(
    ...
    try {
      ...
      org.opencms.db.oracle.CmsSqlManager.closeAllInTransaction(
          m_sqlManager, dbc, conn, stmt, res, commit, wasInTransaction);
    }
  }
}
