 ...
 *
 * <p>A history driver is a driver to write projects, resources and properties of resources
 * optionally to a second set of history database tables while resources get published. A unique
 * publish tag ID is used to identify a set of resource that were saved during one publish process.
 *
 * <p>
 *
 * @since 6.9.1
...
public interface I_CmsHistoryDriver {
  ...
  /**
   * Deletes all historical versions of a resource keeping maximal <code>versionsToKeep</code>
   * versions.
   *
   * <p>
   *
   ...
   */
  int deleteEntries(
      CmsDbContext dbc, I_CmsHistoryResource histResource, int versionsToKeep, long time)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<I_CmsHistoryResource> getAllNotDeletedEntries(CmsDbContext dbc)
      throws CmsDataAccessException;
   ...
   *
   * <p>This method returns a list with the history of the resource, i.e. the historical resources,
   * independent of the project they were attached to.<br>
  ...
  /**
   * Reads all deleted (historical) resources below the given path, that the given user deleted by
   * itself.
   *
   * <p>
   *
   ...
   * @param userId the id of the user that deleted the resources, or <code>null</code> to retrieve
   *     them all
   * @return a list of <code>{@link I_CmsHistoryResource}</code> objects
   * @throws CmsDataAccessException if something goes wrong
   */
  List<I_CmsHistoryResource> readDeletedResources(
   ...
   * @return the property definition that corresponds to the given arguments - or <code>null</code>
   *     if not found
   * @throws CmsDataAccessException if something goes wrong
   */
   ...
   *
   * <p>This method is used during the deletion process of older historical data.
   *
   * <p>
   *
   * @param dbc the current database context
