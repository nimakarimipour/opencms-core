...
public class CmsDefaultUsers {
   ...
   *
   * <p>See the constants of this class for the defaule values that are uses.
   *
   * <p>
   */
   ...
   * @deprecated As of OpenCms 9.5, there is no default group for project managers anymore. Use the
   *     role {@link org.opencms.security.CmsRole#PROJECT_MANAGER} instead in case you want a user
   *     to be able to manage projects. For backward compatibility in old installations, this
   *     returns the old default group name "Projectmanagers".
   */
  @Deprecated
   ...
   * @return <code>true</code> if group name is one of OpenCms default groups, <code>false</code> if
   *     it is not or if <code>groupName</code> is <code>null</code> or an empty string (no trim)
  ...
  public boolean isDefaultGroup(String groupName) {
    ...
    isDefault =
        isDefault || groupName.equals(CmsOrganizationalUnit.SEPARATOR + m_groupAdministrators);
   ...
   * @return <code>true</code> if user name is one of OpenCms default users, <code>false</code> if
   *     it is not or if <code>userName</code> is <code>null</code> or an empty string (no trim)
   * @see #getUserAdmin()
   * @see #getUserExport()
   * @see #getUserGuest()
  ...
  public boolean isDefaultUser(String userName) {
    ...
    isDefault =
        isDefault || userName.equals(CmsOrganizationalUnit.SEPARATOR + m_userDeletedResource);
  ...
  protected void init(
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(userAdmin)
      ...
      throw new CmsRuntimeException(
          Messages.get().container(Messages.ERR_USER_GROUP_NAMES_EMPTY_0));
    }
    // set members
