...
/**
 * An alternative export point driver which replaces the RFS targets of some export points with
 * locations in a temporary export folder.
 *
 * <p>This is designed specifically for export points which map to WEB-INF/lib and WEB-INF/classes.
 * The problem with these is that replacing jar files or classes does not work at runtime. If these
 * files are written to a temp folder (which is not picked up by the servlet coontainer's class
 * loader) instead, an external script can be used to update the actual export point locations based
 * on the temp folder while the servlet container is stopped.
 *
 * <p>Since the script needs to know which files to delete in the 'real' (not temporary) export
 * point locations, deletions of files which are mapped to the temp export point folder are not
 * handled by deleting the files in that folder, but rather by replacing them with "dummy" files
 * containing a specific marker string (which is contained in the DELETE_MARKER variable), so the
 * script that updates the 'real' folder needs to delete files if it comes across a file starting
 * with that marker string.
 */
public class CmsTempFolderExportPointDriver extends CmsExportPointDriver {
  ...
  /**
   * The content to be used for dummy files in the temporary export point folders which represent
   * deleted files.
   */
  ...
  /**
   * The list of RFS destinations which should be replaced with the corresponding destinations in
   * the tmporary export point folder.
   */
   ...
   * @param tempFolderDestinations the export point destinations (relative to the web application
   *     folder) which should be replaced with
   */
  public CmsTempFolderExportPointDriver(
      Set<CmsExportPoint> exportpoints, List<String> tempFolderDestinations) {

    super(); // Uses empty superclass constructor, i.e. we have to initialize members from parent
             // class by hand
            ...
            OpenCms.getSystemInfo()
                .getAbsoluteRfsPathRelativeToWebApplication(
                    CmsStringUtil.joinPaths(
                        TEMP_EXPORTPOINT_FOLDER, point.getConfiguredDestination()));
  ...
  /**
   * Removes a leading slash from a path string.
   *
   * <p>
   *
  ...
  public void writeFile(String resourceName, String exportpoint, byte[] content) {
    ...
    // we can use object identity comparison because DELETE_MARKER_BYTES is not used from the
    // outside
    if ((content != DELETE_MARKER_BYTES)
        && shouldUseTempFolderForDestination(point.getConfiguredDestination())) {
  ...
  /**
   * Returns true if the given RFS destination should be replaced with the corresponding destination
   * in the temporary export point folder.
   *
   * <p>
   *
   * @param destination an export point RFS destination
   * @return true if the corresponding export point destination in the temporary export point folder
   *     should be used
   */
