...
public class CmsHistoryDriver implements I_CmsDriver, I_CmsHistoryDriver {
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#createPropertyDefinition(org.opencms.db.CmsDbContext,
   *     java.lang.String, org.opencms.file.CmsPropertyDefinition.CmsPropertyType)
   */
  public CmsPropertyDefinition createPropertyDefinition(
      CmsDbContext dbc, String name, CmsPropertyDefinition.CmsPropertyType type)
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#deleteEntries(CmsDbContext, I_CmsHistoryResource, int,
   *     long)
   */
  public int deleteEntries(
      CmsDbContext dbc, I_CmsHistoryResource resource, int versionsToKeep, long time)
      throws CmsDataAccessException {
    ...
    try {
      ...
      if (res.next()) {
        ...
        if (res.wasNull()) {
          // the database will return a row with a single NULL column if there are no rows at all
          // for the given
          // resource id. This means that we want to clean up all resource history and content
          // history entries
          // for this resource id, and we achieve this by comparing their publish tag with the
          // maximum integer.
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#deletePropertyDefinition(org.opencms.db.CmsDbContext,
  ...
  public List<I_CmsHistoryResource> getAllDeletedEntries(CmsDbContext dbc)
      throws CmsDataAccessException {
  ...
  public List<I_CmsHistoryResource> getAllNotDeletedEntries(CmsDbContext dbc)
      throws CmsDataAccessException {
    ...
    try {
      ...
      for (int i = 0; i < historyResources.size(); i++) {
        ...
        if (i < (historyResources.size() - 1)) {
          // this is one older direct version than histRes (histRes.getPublishTag() >
          // histRes2.getPublishTag())
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#readContent(org.opencms.db.CmsDbContext,
  ...
  public List<I_CmsHistoryResource> readDeletedResources(
      CmsDbContext dbc, CmsUUID structureId, CmsUUID userId) throws CmsDataAccessException {
    ...
    try {
      ...
      if (userId == null) {
        ...
        stmt =
            m_sqlManager.getPreparedStatement(conn, "C_RESOURCES_HISTORY_READ_DELETED_RESTRICTED");
      }
      stmt.setString(1, structureId.toString());
      if (userId != null) {
      ...
      if (userId == null) {
        ...
        stmt =
            m_sqlManager.getPreparedStatement(
                conn, "C_RESOURCES_HISTORY_READ_DELETED_NAME_RESTRICTED");
      }
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#readLastVersion(org.opencms.db.CmsDbContext,
      ...
      if (res.next()) {
        ...
        historyPrincipal =
            new CmsHistoryPrincipal(
                ...
                new CmsUUID(
                    res.getString(m_sqlManager.readQuery("C_PRINCIPALS_HISTORY_USERDELETED"))),
   ...
   */
  public List<String> readProjectResources(CmsDbContext dbc, int publishTag)
      throws CmsDataAccessException {
  ...
  public List<CmsHistoryProject> readProjects(CmsDbContext dbc) throws CmsDataAccessException {
    ...
    try {
      ...
      while (res.next() && (i < max)) {
        tmpProjects.put(
            Integer.valueOf(res.getInt("PUBLISH_TAG")), internalCreateProject(res, null));
        i++;
      }
    } catch (SQLException e) {
      throw new CmsDbSqlException(
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#readProperties(org.opencms.db.CmsDbContext,
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#writePrincipal(CmsDbContext,
   *     org.opencms.security.I_CmsPrincipal)
   */
  public void writePrincipal(CmsDbContext dbc, I_CmsPrincipal principal)
      throws CmsDataAccessException {
  ...
  public void writeProject(CmsDbContext dbc, int publishTag, long publishDate)
      throws CmsDataAccessException {
    ...
    List<String> projectresources =
        m_driverManager.getProjectDriver(dbc).readProjectResources(dbc, currentProject);
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#writeProperties(org.opencms.db.CmsDbContext,
   *     org.opencms.file.CmsResource, java.util.List, int)
   */
  public void writeProperties(
    ...
    Map<CmsProperty, CmsPropertyDefinition> propDefs =
        new HashMap<CmsProperty, CmsPropertyDefinition>();
  ...
  /**
   * @see org.opencms.db.I_CmsHistoryDriver#writeResource(org.opencms.db.CmsDbContext,
   *     org.opencms.file.CmsResource, java.util.List, int)
   */
  public void writeResource(
    ...
    try {
        ...
        if (resource instanceof CmsFile) {
          if (!valResource) {
            if (sibCount < 2) {
              ...
              Set<CmsUUID> changedAndDeleted =
                  (Set<CmsUUID>) dbc.getAttribute(CmsDriverManager.KEY_CHANGED_AND_DELETED);
      ...
      CmsFolder parent =
          m_driverManager
              .getVfsDriver(dbc)
              .readParentFolder(dbc, CmsProject.ONLINE_PROJECT_ID, resource.getStructureId());
      if (parent != null) {
  ...
  protected void internalCleanup(CmsDbContext dbc, I_CmsHistoryResource resource)
    ...
    for (Map.Entry<CmsUUID, Integer> entry : tmpSubResources.entrySet()) {
      I_CmsHistoryResource histResource =
          readResource(dbc, entry.getKey(), entry.getValue().intValue());
  ...
  protected int internalCountProperties(
      CmsDbContext dbc, CmsPropertyDefinition metadef, CmsUUID projectId)
      throws CmsDataAccessException {
    ...
    try {
      ...
      if (res.next()) {
        ...
        throw new CmsDbConsistencyException(
            Messages.get()
                .container(Messages.ERR_NO_PROPERTIES_FOR_PROPERTYDEF_1, metadef.getName()));
      }
    } catch (SQLException e) {
      throw new CmsDbSqlException(
          Messages.get()
  ...
  /**
   * Creates a valid {@link I_CmsHistoryResource} instance from a JDBC ResultSet.
   *
   * <p>
   *
  ...
  protected I_CmsHistoryResource internalCreateResource(@RUntainted ResultSet res) throws SQLException {
    ...
    CmsUUID parentId =
        new CmsUUID(res.getString(m_sqlManager.readQuery("C_RESOURCES_HISTORY_PARENTID")));
  ...
  /**
   * Merges an historical entry for a sibling, based on the structure data from the given historical
   * resource and result set for the resource entry.
   *
   * <p>
   *
