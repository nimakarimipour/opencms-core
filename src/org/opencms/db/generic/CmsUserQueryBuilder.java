...
public class CmsUserQueryBuilder {
  ...
  protected void addAllowedOuCondition(
      CmsSelectQuery select, TableAlias users, List<CmsOrganizationalUnit> allowedOus) {
   ...
   *
   * <p>If the email address is null, no test is added.
   *
  ...
  protected void addEmailCondition(CmsSelectQuery select, TableAlias users, String email) {
    ...
    if (email != null) {
      CmsSimpleQueryFragment condition =
          new CmsSimpleQueryFragment(users.column(colEmail()) + " = ? ", email);
   ...
   */
  protected void addFlagCondition(
      CmsSelectQuery select, TableAlias users, int flags, boolean allowCore) {
   ...
   */
  protected void addGroupCondition(
      CmsSelectQuery select, TableAlias users, CmsUserSearchParameters searchParams) {
    ...
    if (group != null) {
      ...
      select.addCondition(
          new CmsSimpleQueryFragment(
              groupUsers.column(colGroupUserGroupId()) + " = ? ", groupId.toString()));
      select.addCondition(
          new CmsSimpleQueryFragment(
              groupUsers.column(colGroupUserUserId()) + " = " + users.column(colId())));
      if (searchParams.isFilterByGroupOu()) {
        select.addCondition(
            new CmsSimpleQueryFragment(users.column(colOu()) + " = ? ", group.getOuFqn()));
      }
    }
    CmsGroup notGroup = searchParams.getNotGroup();
    if (notGroup != null) {
      CmsSimpleQueryFragment notGroupCondition =
          new CmsSimpleQueryFragment(
              "NOT EXISTS (SELECT "
    ...
    if ((anyGroups != null) && !anyGroups.isEmpty()) {
      ...
      for (CmsGroup grp : anyGroups) {
        groupClause.add(
            new CmsSimpleQueryFragment(
                "GU." + colGroupUserGroupId() + " = ?", grp.getId().toString()));
      }
  ...
  protected void addOrgUnitCondition(
      CmsSelectQuery select, TableAlias users, CmsOrganizationalUnit orgUnit, boolean recursive) {
  ...
  protected void addSearchFilterCondition(
    ...
    if (!CmsStringUtil.isEmptyOrWhitespaceOnly(searchFilter)) {
      ...
      String patternExprTemplate =
          generateConcat(
              "COALESCE(%1$s, '')", "' '", "COALESCE(%2$s, '')", "' '", "COALESCE(%3$s, '')");
      ...
      for (SearchKey key : searchParams.getSearchKeys()) {
        switch (key) {
          case email:
            searchCondition.add(
                new CmsSimpleQueryFragment(
                    wrapLower(users.column(colEmail()), caseInsensitive) + like, searchFilter));
            break;
          case orgUnit:
            searchCondition.add(
                new CmsSimpleQueryFragment(
                    wrapLower(users.column(colOu()), caseInsensitive) + like, searchFilter));
            break;
          default:
            break;
        }
  ...
  protected I_CmsQueryFragment createCoreCondition(TableAlias users) {
    ...
    return new CmsSimpleQueryFragment(
        users.column(colFlags()) + " <= " + I_CmsPrincipal.FLAG_CORE_LIMIT);
  }
  ...
  protected I_CmsQueryFragment createFlagCondition(TableAlias users, int flags) {
    ...
    return new CmsSimpleQueryFragment(
        users.column(colFlags()) + " & ? = ? ", new Integer(flags), new Integer(flags));
  }
  ...
  /**
   * Returns the SQL expression for generating the user's full name in the format 'firstname
   * lastname (loginname)'.
   *
   * <p>
   *
