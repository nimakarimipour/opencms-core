...
public interface I_CmsProjectDriver {
   ...
   */
  int cleanupPublishHistory(CmsDbContext dbc, CmsPublishHistoryCleanupFilter filter)
   ...
   *
   * <p>Therefore it deletes all files, resources and properties.
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void deleteProjectResource(CmsDbContext dbc, CmsUUID projectId, String resourceName)
      throws CmsDataAccessException;
  ...
  void deletePublishHistoryEntry(
      CmsDbContext dbc, CmsUUID publishHistoryId, CmsPublishedResource publishResource)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void deleteUserPublishListEntries(
   ...
   *
   * <p>The content record is only published unless it's UUID is not contained in
   * publishedContentIds. The calling method has to take care about whether an existing content
   * record has to be deleted before or not.
   *
   * <p>The intention of this method is to get overloaded in a project driver for a specific DB
   * server to shift the binary content from the offline into the online table in a more
   * sophisticated way than in the generic ANSI-SQL implementation of this interface.
   *
   * @param dbc the current database context
  ...
  /**
   * Reads the <code>{@link List}&lt{@link org.opencms.lock.CmsLock};&gt; </code> that were saved to
   * the database in the previous run of OpenCms.
   *
   * <p>
   *
   * @param dbc the current database context
   * @return the <code>{@link List}&lt{@link org.opencms.lock.CmsLock};&gt; </code> that were saved
   *     to the database in the previous run of OpenCms.
   * @throws CmsDataAccessException if something goes wrong
   */
   ...
   * resource path for a given project already exists.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<CmsProject> readProjectsForGroup(CmsDbContext dbc, CmsGroup group)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<CmsProject> readProjectsForManagerGroup(CmsDbContext dbc, CmsGroup group)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<CmsProject> readProjectsForUser(CmsDbContext dbc, CmsUser user)
      throws CmsDataAccessException;

  /**
   * Reads the resources that were published during a publish process for a given publish history
   * ID.
   *
   * <p>
   *
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<org.opencms.db.CmsPublishedResource> readPublishedResources(
   ...
   *
   * <p>If <code>(0L, 0L)</code> is passed as time range, all pending jobs are returned.
   *
   * @param dbc the current database context
   ...
   *
   * <p>This must be done when a project will deleted
   *
   * @param dbc the current database context
   * @param project the project to delete
   * @throws CmsDataAccessException if something goes wrong
  ...
  /**
   * Writes the <code>{@link List}&lt{@link org.opencms.lock.CmsLock};&gt; </code> to the database
   * for reuse in the next run of OpenCms.
   *
   * <p>This method must only be called at startup or the in-memory locking will overwritten.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void writeUserPublishListEntries(
