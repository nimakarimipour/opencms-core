...
public class CmsImportFolder {
   ...
   * @param noSubFolder if <code>true</code> no sub folders will be created, if <code>false</code>
   *     the content of the zip file is created 1:1 inclusive sub folders
   * @throws CmsException if something goes wrong
   */
  public CmsImportFolder(@RUntainted byte[] content, String importPath, CmsObject cms, boolean noSubFolder)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public void importFolder(@RUntainted String importFolderName, @RUntainted String importPath, CmsObject cms)
      throws CmsException {
    ...
    try {
      ...
      throw new CmsVfsException(
          Messages.get().container(Messages.ERR_IMPORT_FOLDER_2, importFolderName, importPath), e);
    } finally {
      if (m_zipStreamIn != null) {
        try {
          m_zipStreamIn.close();
   ...
   * @throws CmsException if something goes wrong
   */
  public void importZip(@RUntainted byte[] content, @RUntainted String importPath, CmsObject cms, boolean noSubFolder)
      throws CmsException {
    ...
    try {
      ...
      throw new CmsVfsException(
          Messages.get().container(Messages.ERR_IMPORT_FOLDER_1, importPath), e);
    } finally {
      if (m_zipStreamIn != null) {
        try {
          m_zipStreamIn.close();
  ...
  /**
   * Stores the import resource in an Object member variable.
   *
   * <p>
   *
  ...
  private void getImportResource() throws CmsVfsException {
    ...
    if (m_importResource.isFile()) {
      try {
        ...
        throw new CmsVfsException(
            Messages.get().container(Messages.ERR_NO_ZIPFILE_1, m_importResource.getName()), e);
      }
    }
  }

  /**
   * Imports the resources from the folder in the real file system to the OpenCms VFS.
   *
   * <p>
   *
      ...
      if (currentFile.isDirectory()) {
        ...
        m_importedResources.add(
            m_cms.createResource(
                importPath + currentFile.getName(), CmsResourceTypeFolder.RESOURCE_TYPE_ID));
        importResources(currentFile, importPath + currentFile.getName() + "/");
      } else {
        // import file into cms
        int type =
            OpenCms.getResourceManager().getDefaultTypeForName(currentFile.getName()).getTypeId();
  ...
  /**
   * Imports the resources from a ZIP file in the real file system to the OpenCms VFS.
   *
   * <p>
   *
   ...
   * @throws Exception if something goes wrong during file IO
   */
  private void importZipResource(@RUntainted ZipInputStream zipStreamIn, String importPath, boolean noSubFolder)
      throws Exception {
    ...
    while (true) {
      ...
      stop = 0;
      // open the entry ...
      @RUntainted ZipEntry entry = zipStreamIn.getNextEntry();
      if (entry == null) {
        break;
      }
      entries++; // count number of entries in zip
      @RUntainted String actImportPath = importPath;
      String title = CmsResource.getName(entry.getName());
      String filename =
          m_cms.getRequestContext().getFileTranslator().translateResource(entry.getName());
      ...
      if (!isFolder) {
        // import file into cms
        int type =
            OpenCms.getResourceManager().getDefaultTypeForName(path[path.length - 1]).getTypeId();
        ...
        if (resourceExists) {
          ...
          @RUntainted String newResName = actImportPath + path[path.length - 1];
