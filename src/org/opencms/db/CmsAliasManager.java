...
public class CmsAliasManager {
   ...
   *
   * <p>This should only return either an empty list or a list with a single element.
   *
   ...
   */
  public List<CmsAlias> getAliasesForPath(CmsObject cms, String siteRoot, String aliasPath)
      throws CmsException {

    CmsAlias alias =
        m_securityManager.readAliasByPath(cms.getRequestContext(), siteRoot, aliasPath);
    if (alias == null) {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsAlias> getAliasesForStructureId(CmsObject cms, CmsUUID structureId)
      throws CmsException {

    List<CmsAlias> aliases =
        m_securityManager.readAliasesById(cms.getRequestContext(), structureId);
    Collections.sort(
        aliases,
        new Comparator<CmsAlias>() {
   ...
   * @throws CmsException if something goes wrong
   */
  public List<CmsRewriteAlias> getRewriteAliases(CmsObject cms, String siteRoot)
      throws CmsException {

    CmsRewriteAliasFilter filter = new CmsRewriteAliasFilter().setSiteRoot(siteRoot);
    List<CmsRewriteAlias> result =
        m_securityManager.getRewriteAliases(cms.getRequestContext(), filter);
    return result;
  }
   ...
   * @throws CmsException if something goes wrong
   */
  public CmsRewriteAliasMatcher getRewriteAliasMatcher(CmsObject cms, String siteRoot)
      throws CmsException {
  ...
  public synchronized List<CmsAliasImportResult> importAliases(
      CmsObject cms, byte[] aliasData, String siteRoot, String separator) throws Exception {
  ...
  /**
   * Saves the aliases for a given structure id, <b>completely replacing</b> any existing aliases
   * for the same structure id.
   *
   * <p>
   *
   * @param cms the current CMS context
   ...
   * @throws CmsException if something goes wrong
   */
  public synchronized void updateAliases(
      CmsObject cms, Collection<CmsAlias> toDelete, Collection<CmsAlias> toAdd)
      throws CmsException {
    ...
    for (CmsUUID structureId : allKeys) {
      Set<CmsAlias> aliasesToSave =
          new HashSet<CmsAlias>(getAliasesForStructureId(cms, structureId));
  ...
  protected synchronized CmsAliasImportResult importAlias(
      CmsObject cms, @RUntainted String siteRoot, String aliasPath, String vfsPath, CmsAliasMode mode)
      throws CmsException {
    ...
    if (maybeAlias.isEmpty()) {
      ...
      touch(cms, resource);
      return new CmsAliasImportResult(
          CmsAliasImportStatus.aliasNew, messageImportOk(locale), aliasPath, vfsPath, mode);
    } else {
      CmsAlias existingAlias = maybeAlias.get(0);
      CmsAliasFilter deleteFilter =
          new CmsAliasFilter(
              siteRoot, existingAlias.getAliasPath(), existingAlias.getStructureId());
      m_securityManager.deleteAliases(cms.getRequestContext(), deleteFilter);
      CmsAlias newAlias = new CmsAlias(resource.getStructureId(), siteRoot, aliasPath, mode);
      m_securityManager.addAlias(cms.getRequestContext(), newAlias);
      touch(cms, resource);
      return new CmsAliasImportResult(
          CmsAliasImportStatus.aliasChanged, messageImportUpdate(locale), aliasPath, vfsPath, mode);
    }
  }
  ...
  protected CmsAliasImportResult processAliasImport(
    ...
    try {
      ...
      return new CmsAliasImportResult(
          CmsAliasImportStatus.aliasImportError, e.getLocalizedMessage(), aliasPath, vfsPath, mode);
    }
  }
   ...
   */
  protected CmsAliasImportResult processAliasLine(
      CmsObject cms, @RUntainted String siteRoot, String line, String separator) {
    ...
    try {
      ...
      return new CmsAliasImportResult(
          line, CmsAliasImportStatus.aliasParseError, messageImportInvalidFormat(locale));
    }
  ...
  /**
   * Checks that the user has permissions for a mass edit operation in a given site.
   *
   * <p>
   *
   * @param cms the current CMS context
  ...
  private String messageImportCantReadResource(@RUntainted Locale locale, String path) {
    ...
    return Messages.get()
        .getBundle(locale)
        .key(Messages.ERR_ALIAS_IMPORT_COULD_NOT_READ_RESOURCE_0);
  }
  ...
  /**
   * Handles the import of a rewrite alias.
   *
   * <p>
   *
   * @param cms the current CMS context
  ...
  private CmsAliasImportResult processRewriteImport(
      CmsObject cms, String siteRoot, String source, String target, CmsAliasMode mode) {

    try {
      return m_securityManager.importRewriteAlias(
          cms.getRequestContext(), siteRoot, source, target, mode);
    } catch (CmsException e) {
      return new CmsAliasImportResult(
          CmsAliasImportStatus.aliasImportError, e.getLocalizedMessage(), source, target, mode);
    }
  }

  /**
   * Tries to to touch a resource by setting its last modification date, but only if its state is
   * 'unchanged'.
   *
   * <p>
   *
