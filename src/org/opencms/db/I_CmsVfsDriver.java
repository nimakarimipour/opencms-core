...
public interface I_CmsVfsDriver {
  ...
  /**
   * The internal request attribute to indicate that resource organizational units have to be
   * retrieved.
   */
  ...
  /**
   * Creates a {@link CmsFile} instance from a JDBC ResultSet.
   *
   * <p>
   *
   ...
   * @throws SQLException in case the result set does not include a requested table attribute
   */
  CmsFile createFile(@RUntainted ResultSet res, CmsUUID projectId, boolean hasFileContentInResultSet)
      throws SQLException;

  /**
   * Creates a {@link CmsFolder} instance from a JDBC ResultSet.
   *
   * <p>
   *
   * @param res the JDBC ResultSet
   * @param projectId the ID of the current project
   * @param hasProjectIdInResultSet true if the SQL select query includes the PROJECT_ID table
   *     attribute
   * @return the created folder
   * @throws SQLException in case the result set does not include a requested table attribute
   */
  CmsFolder createFolder(@RUntainted ResultSet res, CmsUUID projectId, boolean hasProjectIdInResultSet)
      throws SQLException;
  ...
  /**
   * Creates a new {@link CmsRelation} object in the database.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void createRelation(CmsDbContext dbc, @RUntainted CmsUUID projectId, @RUntainted CmsRelation relation)
      throws CmsDataAccessException;

  /**
   * Creates a new resource from a given {@link CmsResource} object.
   *
   * <p>This method works for both files and folders. Existing resources get overwritten.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   */
  @RUntainted CmsResource createResource(
   ...
   *
   * @param res the JDBC ResultSet
   * @param projectId the ID of the current project to adjust the modification date in case the
   *     resource is a VFS link
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void deleteAliases(CmsDbContext dbc, CmsProject project, CmsAliasFilter filter)
      throws CmsDataAccessException;
   ...
   *
   * <p>You may specify which whether just structure or resource property values should be deleted,
   * or both of them.
   *
   * <p>
   *
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void deleteRewriteAliases(CmsDbContext dbc, CmsRewriteAliasFilter filter)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<CmsOrganizationalUnit> getResourceOus(
      CmsDbContext dbc, CmsUUID projectId, CmsResource resource) throws CmsDataAccessException;
   ...
   *
   * <p>To obtain JDBC connections from different pools, further {online|offline|history} pool Urls
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void insertAlias(CmsDbContext dbc, CmsProject project, CmsAlias alias)
      throws CmsDataAccessException;

  /**
   * Adds a list of rewrite aliases.
   *
   * <p>When adding a rewrite alias, make sure that no alias with the same id is present in the
   * database.
   *
   * <p>
   *
  ...
  /**
   * Publishes the structure and resource records of an offline resource into it's online
   * counterpart.
   *
   * <p>
   *
  ...
  /**
   * Copies the version number from the offline resource to the online resource, this has to be done
   * during publishing, direct after copying the resource itself.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  CmsFolder readFolder(CmsDbContext dbc, CmsUUID projectId, @RUntainted String foldername)
      throws CmsDataAccessException;

  /**
   * Reads the parent folder of a resource specified by it's structure ID.
   *
   * <p>The parent folder for the root '/' is defined as <code>null</code>.
   *
   * <p>
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException a CmsDbEntryNotFoundException is thrown if the property
   *     definition does not exist
   */
  @RUntainted CmsPropertyDefinition readPropertyDefinition(CmsDbContext dbc, String name, CmsUUID projectId)
      throws CmsDataAccessException;
   ...
   *
   * <p>The implementation must return {@link CmsProperty#getNullProperty()} if the property is not
   * found.
   *
   * <p>TODO: change project parameter to project id
   *
   * @param dbc the current database context
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  CmsProperty readPropertyObject(
   ...
   *
   * <p>The implementation must return an empty list if no properties are found at all.
   *
   * <p>TODO: change project parameter to project id
   *
   * @param dbc the current database context
   * @param project the current project
   * @param resource the resource where the property is attached to
   * @return a list with CmsProperty objects containing both the structure and resource value of the
   *     property
   * @throws CmsDataAccessException if something goes wrong
   */
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  @RUntainted List<@RUntainted CmsRelation> readRelations(
      CmsDbContext dbc, CmsUUID projectId, CmsResource resource, CmsRelationFilter filter)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  @RUntainted CmsResource readResource(
      CmsDbContext dbc, CmsUUID projectId, @RUntainted String filename, boolean includeDeleted)
      throws CmsDataAccessException;
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<CmsResource> readResourcesForPrincipalACE(
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  List<CmsResource> readResourcesForPrincipalAttr(
      CmsDbContext dbc, CmsProject project, CmsUUID principalId) throws CmsDataAccessException;
   ...
   * @param parent the path to the resource used as root of the searched subtree or {@link
   *     CmsDriverManager#READ_IGNORE_PARENT}, {@link CmsDriverManager#READMODE_EXCLUDE_TREE} means
   *     to read immediate children only
   * @param type the resource type of matching resources or {@link
   *     CmsDriverManager#READ_IGNORE_TYPE} (meaning inverted by {@link
   *     CmsDriverManager#READMODE_EXCLUDE_TYPE}
   ...
   * @param startTime the start of the time range for the last modification date of matching
   *     resources or READ_IGNORE_TIME
   * @param endTime the end of the time range for the last modification date of matching resources
   *     or READ_IGNORE_TIME
   ...
   * @param includeDeleted <code>true</code> if deleted siblings should be included in the result
   *     list
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  @RUntainted List<CmsResource> readSiblings(
   ...
   * @return a map with two entries with keys "structure" and "resource" for the structure and
   *     resource version number respectively, the values are {@link Integer} objects and may be
   *     <code>-1</code> if an entry could be found
   * @throws CmsDataAccessException if something goes wrong
   */
  Map<String, Integer> readVersions(
  ...
  void replaceResource(
      CmsDbContext dbc, CmsResource newResource, byte[] newResourceContent, int newResourceType)
      throws CmsDataAccessException;
   ...
   *
   * <p>TODO: add offlineProject parameter
   *
   * @param dbc the current database context
   * @param onlineProject the online project
  ...
  /**
   * Validates if the specified resource ID in the tables of the specified project {offline|online}
   * exists.
   *
   * <p>
   *
  ...
  /**
   * Validates if the specified structure ID in the tables of the specified project {offline|online}
   * exists.
   *
   * <p>
   *
   ...
   * @return true, if the specified structure ID in the tables of the specified project
   ...
   * @throws CmsDataAccessException if something goes wrong
   */
  void writeLastModifiedProjectId(
   ...
   * @param property a CmsProperty object containing both the structure and resource value of the
   *     property
   * @throws CmsDataAccessException if something goes wrong
   */
   ...
   *
   * <p>Common usages of this method are saving the resource information after creating, importing
   * or restoring complete files where all file header attributes are changed. Both the structure
   * and resource records get written. Thus, using this method affects all siblings of a resource!
   * Use {@link #writeResourceState(CmsDbContext, CmsProject, CmsResource, int, boolean)} instead if
   * you just want to update the file state, e.g. of a single sibling.
   *
   * <p>The file state is set to "changed", unless the current state is "new" or "deleted". The
   * "changed" argument allows to choose whether the structure or resource state, or none of them,
   * is set to "changed".
   *
   * <p>The rating of the file state values is as follows:<br>
   * unchanged &lt; changed &lt; new &lt; deleted
   *
   * <p>Second, the "state" of the resource is the structure state, if the structure state has a
   * higher file state value than the resource state. Otherwise the file state is the resource
   * state.
   *
   * <p>
   *
   ...
   * @param changed determines whether the structure or resource state, or none of them, is set to
   *     "changed"
   * @throws CmsDataAccessException if something goes wrong
   ...
   *
   * <p>This method allows to change the resource state to any state by setting the desired state
   * value in the specified CmsResource instance.
   *
   * <p>This method is frequently used while resources are published to set the file state back to
   * "unchanged".
   *
   * <p>Only file state attributes. get updated here. Use {@link #writeResource(CmsDbContext,
   * CmsUUID, CmsResource, int)} instead to write the complete file header.
   *
   * <p>Please refer to the javadoc of {@link #writeResource(CmsDbContext, CmsUUID, CmsResource,
   * int)} to read how setting resource state values affects the file state.
   *
   * <p>
   *
   * @param dbc the current database context
