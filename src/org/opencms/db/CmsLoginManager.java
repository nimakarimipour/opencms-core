 ...
 *
 * <p>Stores invalid login attempts and disables a user account temporarily in case the configured
 * threshold of invalid logins is reached.
 *
 * <p>The invalid login attempt storage operates on a combination of user name, login remote IP
 * address and user type. This means that a user can be disabled for one remote IP, but still be
 * enabled for another remote IP.
 *
 * <p>Also allows to temporarily disallow logins (for example in case of maintenance work on the
 * system).
 *
 * <p>
 *
...
public class CmsLoginManager {
  ...
  /**
   * Contains the data stored for each user in the storage for invalid login attempts.
   *
   * <p>
   */
  private class CmsUserData {
    ...
    /**
     * Returns the bad attempt count for this user.
     *
     * <p>
     *
    ...
    /**
     * Returns the date this disabled user is released again.
     *
     * <p>
     *
    ...
    /**
     * Increases the bad attempt count, disables the data in case the configured threshold is
     * reached.
     *
     * <p>
     */
    ...
    /**
     * Returns <code>true</code> in case this user has been temporarily disabled.
     *
     * <p>
     *
   ...
   * @param maxBadAttempts the number of bad login attempts allowed before an account is temporarily
   *     disabled
   * @param enableSecurity flag to determine if the security option should be enabled on the login
   *     dialog
   * @param tokenLifetime the lifetime of authorization tokens, i.e. the time for which they are
   *     valid
   ...
   *
   * <p>If no max inactivity time is configured, always returns false.
   *
   * @param user the user to check
   ...
   *
   * <p>In case the configured threshold is reached, an Exception is thrown.
   *
   * <p>
   *
   * @param userName the name of the user
   ...
   *
   * <p>In case no logins are allowed, an Exception is thrown.
   *
   * <p>
   *
  ...
  public void checkLoginAllowed() throws CmsAuthentificationException {
    ...
    if ((m_loginMessage != null) && (m_loginMessage.isLoginCurrentlyForbidden())) {
      ...
      throw new CmsAuthentificationException(
          Messages.get()
              .container(Messages.ERR_LOGIN_FAILED_WITH_MESSAGE_1, m_loginMessage.getMessage()));
    }
  }
   ...
   *
   * <p>if <code>null</code> is returned, no login message has been currently set.
   *
   * <p>
   *
   ...
   * Gets the logout URI.
   *
   * <p>If this is not null, users will be redirected to this JSP when logging out from the
   * workplace or page editor. The JSP is responsible for invalidating the user's session.
   *
   ...
   * <p>
   *
   * @return <code>true</code> if the security option ahould be enabled on the login dialog,
   ...
   *
   * <p>This operation requires that the current user has role permissions of <code>
   * {@link CmsRole#ROOT_ADMIN}</code>.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
   * @throws CmsRoleViolationException in case the current user does not have the required role
   *     permissions
   */
  ...
  public boolean requiresPasswordChange(CmsObject cms, CmsUser user) {
    ...
    String lastPasswordChangeStr =
        (String) user.getAdditionalInfo().get(CmsUserSettings.ADDITIONAL_INFO_LAST_PASSWORD_CHANGE);
  ...
  public boolean requiresUserDataCheck(CmsObject cms, CmsUser user) {
    ...
    String lastCheckStr =
        (String) user.getAdditionalInfo().get(CmsUserSettings.ADDITIONAL_INFO_LAST_USER_DATA_CHECK);
   ...
   *     permissions
   */
  public void setBeforeLoginMessage(CmsObject cms, CmsLoginMessage message)
   ...
   *     permissions
   */
  public void setLoginMessage(CmsObject cms, CmsLoginMessage message)
      throws CmsRoleViolationException {
  ...
  /**
   * Unlocks a user who has exceeded his number of failed login attempts so that he can try to log
   * in again.
   *
   * <p>This requires the "account manager" role.
   *
   * @param cms the current CMS context
  ...
  public void unlockUser(CmsObject cms, CmsUser user) throws CmsRoleViolationException {
    ...
    OpenCms.getRoleManager()
        .checkRole(cms, CmsRole.ACCOUNT_MANAGER.forOrgUnit(cms.getRequestContext().getOuFqn()));
   ...
   *
   * <p>In case the configured threshold is reached, the user is disabled for the configured time.
   *
   * <p>
   *
   * @param userName the name of the user
  ...
  /**
   * Helper method to get all the storage keys that match a user's name.
   *
   * <p>
   *
