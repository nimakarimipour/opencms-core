...
/**
 * Abstract base class used for converting from CmsLogEntry instances to the necessary changes in
 * the user's publish list.
 *
 * <p>Multiple log entries can be added via the add() method, and then all changes to the database
 * are performed when the writeToDatabase() method is called.
 */
  ...
  public void writeChangesToDatabase(CmsDbContext dbc, I_CmsProjectDriver projectDriver)
    ...
    for (CmsUUID structureId : m_entries.keySet()) {
      ...
      for (CmsUUID userId : state.getUpdateUsers()) {
        entriesToWrite.add(
            new CmsUserPublishListEntry(userId, structureId, state.getTimestamp(userId)));
      }
    }
    if (entriesToWrite.size() > 0) {
