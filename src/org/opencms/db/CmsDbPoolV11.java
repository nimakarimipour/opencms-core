...
public final class CmsDbPoolV11 {
  ...
  public CmsDbPoolV11(CmsParameterConfiguration config, @RUntainted String key) throws Exception {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_JDBC_POOL_2, getPoolUrl(), m_dataSource.getJdbcUrl()));
    }
  }

  /**
   * Creates the HikariCP configuration based on the configuration of a pool defined in
   * opencms.properties.
   *
  ...
  public static @RUntainted HikariConfig createHikariConfig(CmsParameterConfiguration config, String key) {
    ...
    // these are for backwards compatibility , all other properties not of the form
    // db.pool.poolname.v11..... are ignored
    ...
    if (user != null) {
      user =
          OpenCms.getCredentialsResolver()
              .resolveCredential(I_CmsCredentialsResolver.DB_USER, user);
      hikariProps.put("username", user);
    }
    if (password != null) {
      password =
          OpenCms.getCredentialsResolver()
              .resolveCredential(I_CmsCredentialsResolver.DB_PASSWORD, password);
    ...
    if ((testQueryForDriver != null)
        && CmsStringUtil.isEmptyOrWhitespaceOnly(configuredTestQuery)) {
