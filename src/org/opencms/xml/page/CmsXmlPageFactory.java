...
public final class CmsXmlPageFactory {
  ...
  /**
   * Creates a valid XML page String representation, containing one empty element in the given
   * locale.
   *
   * <p>
   *
  ...
  /**
   * Factory method to unmarshal (read) a XML page instance from a byte array that contains XML
   * data.
   *
   * <p>When unmarshalling, the encoding is read directly from the XML header. The given encoding is
   * used only when marshalling the XML again later.
   *
   * <p>
   *
  ...
  /**
   * Factory method to unmarshal (read) a XML page instance from a OpenCms VFS file that contains
   * XML data.
   *
   * <p>
   *
   ...
   * @throws CmsXmlException if something goes wrong
   */
  public static CmsXmlPage unmarshal(CmsObject cms, CmsFile file, boolean keepEncoding)
      throws CmsXmlException {
    ...
    try {
      allowRelative =
          Boolean.valueOf(
                  cms.readPropertyObject(fileName, CmsXmlPage.PROPERTY_ALLOW_RELATIVE, false)
                      .getValue())
              .booleanValue();
  ...
  /**
   * Factory method to unmarshal (read) a XML page instance from a resource, using the request
   * attributes as cache.
   *
   * <p>
   *
   ...
   * @return the unmarshaled XML page, or null if the given resource was not of type {@link
  ...
  public static CmsXmlPage unmarshal(CmsObject cms, CmsResource resource, ServletRequest req)
    ...
    if (!CmsResourceTypeXmlPage.isXmlPage(resource)) {
      ...
      throw new CmsXmlException(
          Messages.get()
              .container(Messages.ERR_XML_PAGE_FACT_NO_XMLPAGE_TYPE_1, cms.getSitePath(resource)));
    }
  ...
  /**
   * Factory method to unmarshal (read) a XML document instance from a filename in the VFS, using
   * the request attributes as cache.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
   ...
   * @param req the current request
   * @return the unmarshaled XML document, or <code>null</code> if the given resource was not of
   *     type {@link I_CmsXmlDocument}
   * @throws CmsException in something goes wrong
   */
  public static I_CmsXmlDocument unmarshal(CmsObject cms, String filename, ServletRequest req)
    ...
    if (CmsResourceTypeXmlPage.isXmlPage(file)) {
      ...
      throw new CmsXmlException(
          Messages.get().container(Messages.ERR_XML_PAGE_FACT_NO_XML_DOCUMENT_1, file));
    }
