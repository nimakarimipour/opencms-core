...
public class CmsContainerElementBean implements Cloneable {
  ...
  /**
   * Flag indicating if a new element should be created replacing the given one on first edit of a
   * container-page.
   */
   ...
   * @param createNew <code>true</code> if a new element should be created replacing the given one
   *     on first edit of a container-page
   */
   ...
   */
  public static CmsContainerElementBean cloneWithFormatter(
      CmsContainerElementBean source, CmsUUID formatterId) {
  ...
  public static CmsContainerElementBean cloneWithSettings(
      CmsContainerElementBean source, Map<String, String> settings) {
    ...
    CmsContainerElementBean result =
        new CmsContainerElementBean(source.m_elementId, source.m_formatterId, settings, createNew);
   ...
   *
   * <p><b>The represented resource will be in memory only and not in the VFS!!!.</b>
   *
   * <p>
   *
   ...
   * @throws IllegalArgumentException if the resource type not instance of {@link
  ...
  /**
   * Gets the hash code for the element settings.
   *
   * <p>
   *
   ...
   *
   * <p>It is required to call {@link #initResource(CmsObject)} before this method can be used.
   *
   * <p>
   *
   ...
   *
   * <p>Will return <code>null</code> if the element bean has not been initialized with {@link
   * #initResource(org.opencms.file.CmsObject)}.
   *
   * <p>
   *
  ...
  public void initResource(CmsObject cms) throws CmsException {
    ...
    if (m_resource == null) {
      ...
      m_releasedAndNotExpired =
          m_resource.isReleasedAndNotExpired(cms.getRequestContext().getRequestTime());
    } else if (!isInMemoryOnly()) {
      CmsUUID id = m_resource.getStructureId();
      if (id == null) {
        id = getId();
      }
      // the resource object may have a wrong root path, e.g. if it was created before the resource
      // was moved
    ...
    if (formatterBean == null) {
      mergedSettings =
          CmsXmlContentPropertyHelper.mergeDefaults(
              cms, config, m_resource, getIndividualSettings(), locale, request);
    } else {
      mergedSettings =
          CmsXmlContentPropertyHelper.mergeDefaults(
              cms,
              OpenCms.getADEManager()
                  .getFormatterSettings(cms, config, formatterBean, getResource(), locale, request),
  ...
  public boolean isCopyModel() {
    ...
    return Boolean.valueOf(getIndividualSettings().get(CmsContainerElement.USE_AS_COPY_MODEL))
        .booleanValue();
  }

  /**
   * Returns if a new element should be created replacing the given one on first edit of a
   * container-page.
   *
   * <p>
   *
   * @return <code>true</code> if a new element should be created replacing the given one on first
   *     edit of a container-page
   */
  ...
  /**
   * Checks if the element is used for displaying a historical content (usually from the history
   * dialog).
   *
  ...
  public boolean isInheritedContainer(CmsObject cms) throws CmsException {
    ...
    return OpenCms.getResourceManager()
            .getResourceType(CmsResourceTypeXmlContainerPage.INHERIT_CONTAINER_TYPE_NAME)
            .getTypeId()
        == m_resource.getTypeId();
  }
   ...
   * <p>
   *
   * @return <code>true</code> if the represented resource is in memory only and not persisted in
   *     the VFS
   */
   ...
   * <p>
   *
   * @return <code>true</code> if all instances of this element should be replaced within a copy
   *     model
   */
  public boolean isModelGroupAlwaysReplace() {

    return Boolean.parseBoolean(
        getIndividualSettings().get(CmsContainerElement.IS_MODEL_GROUP_ALWAYS_REPLACE));
  }
  ...
  /**
   * Enables / disables 'do not cache' status, which prevents the element from being cached in the
   * session cache.
   *
   ...
   *
   * <p>This causes all merged settings (from defaults etc.) to be lost.
   *
