...
public class CmsMutableContainerPage {
  ...
  /**
   * The list of containers (we just store them as a list instead of a map because the container
  ...
  public static CmsMutableContainerPage fromImmutable(CmsContainerPageBean page) {
    ...
    List<CmsMutableContainer> containers =
        page.getContainers().values().stream()
            .map(container -> CmsMutableContainer.fromImmutable(container))
  ...
  public List<CmsMutableContainer> containers(String name) {
    ...
    return m_containers.stream()
        .filter(container -> container.matches(name))
        .collect(Collectors.toList());
  }
  ...
  public CmsContainerPageBean toImmutable() {
    ...
    List<CmsContainerBean> containers =
        m_containers.stream().map(cnt -> cnt.toImmutable()).collect(Collectors.toList());
