...
public final class CmsADECache extends CmsVfsCache {
  ...
  /**
   * The log to use (static for performance reasons).
   *
   * <p>
   */
        ...
        if (LOG.isDebugEnabled()) {
          if (retValue == null) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_CACHE_MISSED_ONLINE_1, new Object[] {key}));

          } else {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_CACHE_MATCHED_ONLINE_2, new Object[] {key, retValue}));
          }
        }
      } else {
        ...
        if (LOG.isDebugEnabled()) {
          if (retValue == null) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_CACHE_MISSED_OFFLINE_1, new Object[] {key}));

          } else {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_CACHE_MATCHED_OFFLINE_2, new Object[] {key, retValue}));
          }
        }
      }
      if (retValue != null) {
      ...
      if (online) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_CACHE_SET_ONLINE_2, new Object[] {key, containerPage}));
        }
      } else {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_CACHE_SET_OFFLINE_2, new Object[] {key, containerPage}));
        }
      }
    } finally {
      m_lock.writeLock().unlock();
      ...
      if (online) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_CACHE_SET_ONLINE_2, new Object[] {key, groupContainer}));
        }
      } else {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_CACHE_SET_OFFLINE_2, new Object[] {key, groupContainer}));
        }
      }
    } finally {
      m_lock.writeLock().unlock();
  ...
  private void initialize(CmsMemoryMonitor memMonitor, CmsADECacheSettings cacheSettings) {
    ...
    m_containerPagesOffline =
        CmsMemoryMonitor.createLRUCacheMap(cacheSettings.getContainerPageOfflineSize());
    memMonitor.register(
        CmsADECache.class.getName() + ".containerPagesOffline", m_containerPagesOffline);

    m_containerPagesOnline =
        CmsMemoryMonitor.createLRUCacheMap(cacheSettings.getContainerPageOnlineSize());
    memMonitor.register(
        CmsADECache.class.getName() + ".containerPagesOnline", m_containerPagesOnline);

    // container page caches
    m_groupContainersOffline =
        CmsMemoryMonitor.createLRUCacheMap(cacheSettings.getGroupContainerOfflineSize());
    memMonitor.register(
        CmsADECache.class.getName() + ".groupContainersOffline", m_groupContainersOffline);

    m_groupContainersOnline =
        CmsMemoryMonitor.createLRUCacheMap(cacheSettings.getGroupContainerOnlineSize());
    memMonitor.register(
        CmsADECache.class.getName() + ".groupContainersOnline", m_groupContainersOnline);
  }

  /**
   * Removes a cached XML content from the cache if it matches a given resource.
   *
   * <p>
   *
  ...
  private <CONTENT extends CmsXmlContent> void removeCachedContent(
      CmsResource resource, Map<String, CONTENT> cache) {
