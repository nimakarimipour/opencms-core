...
public class CmsDynamicFunctionBean {
  ...
  /**
   * A bean which contains a single format for a dynamic function, which contains of the function
   * JSP, the container settings and the parameters.
   */
  ...
  public Format getFormatForContainer(CmsObject cms, String type, int width) {
    ...
    IdentityHashMap<CmsFormatterBean, Format> formatsByFormatter =
        new IdentityHashMap<CmsFormatterBean, Format>();
    // relate formatters to formats so we can pick the corresponding format after a formatter has
    // been selected
    ...
    CmsFormatterConfiguration formatterConfiguration =
        CmsFormatterConfiguration.create(cms, formatters);
