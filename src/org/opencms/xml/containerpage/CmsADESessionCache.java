...
public final class CmsADESessionCache {
  ...
  /**
   * Stores information about the container page which was last edited, so we can jump back to it
   * later.
   *
   * <p>
   */
  ...
  private Map<String, List<String>> m_recentFormatters =
  ...
  protected CmsADESessionCache(CmsObject cms, HttpServletRequest request) {
    ...
    if (request != null) {
      elementView =
          CmsWorkplace.getWorkplaceSettings(cms, request)
              .getUserSettings()
              .getAdditionalPreference(CmsElementViewPreference.PREFERENCE_NAME, false);
    }
    if (elementView == null) {
   ...
   *
   * <p>In case the request is not editable, <code>null</code> will be returned.
   *
   * <p>
   *
   * @param request the current request
  ...
  public static CmsADESessionCache getCache(HttpServletRequest request, CmsObject cms) {
    ...
    CmsADESessionCache cache =
        (CmsADESessionCache)
            request.getSession().getAttribute(CmsADESessionCache.SESSION_ATTR_ADE_CACHE);
    if (cache == null) {
  ...
  public I_CmsFormatterBean getRecentFormatter(
      String resType, CmsContainer container, CmsADEConfigData config) {
  ...
  /**
   * Returns true if, in this session, a newly opened container page editor window should display
   * edit points for small elements initially.
   *
   * <p>
   *
  ...
  /**
   * Set cached value for the attribute. Used for dynamically loaded values in the Acacia content
   * editor.
   *
