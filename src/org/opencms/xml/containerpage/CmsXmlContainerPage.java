 ...
 *
 * <p>In addition to the XML content interface. It also provides access to more comfortable beans.
 *
 * @since 7.5.2
...
public class CmsXmlContainerPage extends CmsXmlContent {
  ...
  public static final Set<String> LEGACY_SYSTEM_SETTING_NAMES =
   ...
   *
   * <p>The given encoding is used when marshalling the XML again later.
   *
   * <p>
   *
  ...
  /**
   * Create a new container page based on the given default content, that will have all language
   * nodes of the default content and ensures the presence of the given locale.
   *
   * <p>The given encoding is used when marshalling the XML again later.
   *
   * <p>
  ...
  /**
   * Create a new container page based on the given content definition, that will have one language
   * node for the given locale all initialized with default values.
   *
   * <p>The given encoding is used when marshalling the XML again later.
   *
   * <p>
   ...
   */
  public byte[] createContainerPageXml(CmsObject cms, CmsContainerPageBean cntPage)
      throws CmsException {
  ...
  public CmsContainerPageBean getContainerPage(CmsObject cms) {
    ...
    // this is important for 'legacy' container pages which were created before container pages
    // became locale independent
   ...
   * @throws CmsException if something goes wrong
   */
  public void save(CmsObject cms, CmsContainerPageBean cntPage, boolean ifChangedOnly)
      throws CmsException {
  ...
  /**
   * Removes all empty containers and merges the containers of the current document that are not
   * used in the given container page with it.
   *
   * <p>
   *
   * @param cms the current CMS context
   ...
   */
  protected CmsContainerPageBean cleanupContainersContainers(
      CmsObject cms, CmsContainerPageBean cntPage) {

    CmsADEConfigData config =
        OpenCms.getADEManager().lookupConfiguration(cms, getFile().getRootPath());
    ...
    Map<String, CmsContainerElementBean> pageElements =
    ...
    while (cntIt.hasNext()) {
      ...
      if (!currentContainers.containsKey(container.getName())
        ...
        if (!remove) {
          ...
          String settingsKey =
              CmsFormatterConfig.getSettingsKeyForContainer(
          ...
          if (bean != null) {
            remove =
                (bean instanceof CmsFormatterBean)
                    && ((CmsFormatterBean) bean).isStrictContainers();
          }
        }
        if (remove) {
  ...
  /**
   * Fills a {@link CmsXmlVfsFileValue} with the resource identified by the given id.
   *
   * <p>
   *
   * @param cms the current CMS context
  ...
  /**
   * @see org.opencms.xml.content.CmsXmlContent#initDocument(org.opencms.file.CmsObject,
   *     org.dom4j.Document, java.lang.String, org.opencms.xml.CmsXmlContentDefinition)
   */
  @Override
    ...
    for (Iterator<@RUntainted Element> itCntPages =
      ...
      try {
        ...
        for (Iterator<Element> itCnts =
          ...
          CmsXmlContentDefinition cntDef =
              ((CmsXmlNestedContentDefinition) cntSchemaType).getNestedContentDefinition();
          ...
          for (Iterator<Element> itElems =
            ...
            String elemPath =
                CmsXmlUtils.concatXpath(
                    cntPath, CmsXmlUtils.createXpathElement(element.getName(), elemIndex));
            ...
            CmsXmlContentDefinition elemDef =
                ((CmsXmlNestedContentDefinition) elemSchemaType).getNestedContentDefinition();
            ...
            // uri may be null for dynamic functions, try find the element id from the settings
            // later
            ...
            if (formatterKey != null) {
              propertiesMap.put(
                  CmsFormatterConfig.FORMATTER_SETTINGS_KEY + containerName, formatterKey);
            }
            ...
            boolean createNewFromSetting =
                Boolean.parseBoolean(propertiesMap.remove(CmsContainerElement.SETTING_CREATE_NEW));
          ...
          CmsContainerBean newContainerBean =
              new CmsContainerBean(
                  name.getText(),
                  type.getText(),
                  parentInstance != null ? parentInstance.getText() : null,
                  (isRootContainer != null)
                      && Boolean.valueOf(isRootContainer.getText()).booleanValue(),
                  elements);
        ...
        LOG.error(
            org.opencms.xml.content.Messages.get()
                .getBundle()
                .key(org.opencms.xml.content.Messages.LOG_XMLCONTENT_INIT_BOOKMARKS_0),
            e);
      }
    }
  ...
  /**
   * @see org.opencms.xml.A_CmsXmlDocument#initDocument(org.dom4j.Document, java.lang.String,
  ...
  /**
   * Replaces formatter id prefixes for nested settings with corresponding formatter keys, if
   * possible.
   *
   * <p>Also handles replacement of alias keys with main keys in nested settings.
   *
   * @param cms the CMS Context
  ...
  private Map<String, String> fixNestedFormatterSettings(
      CmsObject cms, CmsADEConfigData config, Map<String, String> propertiesMap) {
   ...
   */
  private Map<String, String> processSettingsForSaveV1(
    ...
    for (Map.Entry<String, @RUntainted String> entry : settings.entrySet()) {
      ...
      if (key.startsWith(CmsFormatterConfig.FORMATTER_SETTINGS_KEY)) {
        ...
        if (underscorePos != -1) {
          ...
          if ((dynamicFmt != null)
              && dynamicFmt.getSettings(config).containsKey(partAfterUnderscore)) {
   ...
   * @return the modified element settings
   */
  private Map<String, String> processSettingsForSaveV2(
      CmsADEConfigData config, Map<String, String> settings) {
  ...
  private void saveContainerPageV1(
      CmsObject cms, Element parent, CmsContainerPageBean cntPage, CmsADEConfigData adeConfig)
      throws CmsException {
    ...
    for (String containerName : containerNames) {
      ...
      if (container.isNestedContainer()) {
        cntElement
            .addElement(XmlNode.ParentInstanceId.name())
            .addCDATA(container.getParentInstanceId());
      }
      ...
      for (CmsContainerElementBean element : container.getElements()) {
        ...
        Map<String, CmsXmlContentProperty> propertiesConf =
            OpenCms.getADEManager().getElementSettings(cms, uriRes);

        CmsXmlContentPropertyHelper.saveProperties(
            cms, elemElement, processedSettings, propertiesConf, true);
      }
    }
  }
  ...
  private void saveContainerPageV2(
    ...
    for (String containerName : containerNames) {
      ...
      for (CmsContainerElementBean element : container.getElements()) {
        ...
        Map<String, CmsXmlContentProperty> propertiesConf =
            OpenCms.getADEManager().getElementSettings(cms, elementRes);
        CmsXmlContentPropertyHelper.saveProperties(
            cms, elemElement, processedSettings, propertiesConf, false);
      }
    }
  }

  /**
   * Computes a container sort ordering for saving the containers of a container page bean.
   *
   * <p>
   *
  ...
  private List<String> sortContainerNames(CmsContainerPageBean page) {
    ...
    // Visit all containers via depth-first traversal, using the previously constructed tables and a
    // stack.
    ...
    while (stack.size() > 0) {
      ...
      // avoid already visited containers, in case there are cycles (possible in principle, if you
      // change the container page manually)
      ...
      for (CmsContainerElementBean element : container.getElements()) {
        ...
        if (instanceId != null) {
          List<CmsContainerBean> childContainers =
              new ArrayList<>(containersByParentId.get(instanceId));
    ...
    result.sort(
        (a, b) ->
            ComparisonChain.start()
                .compare(order.get(a), order.get(b), Ordering.natural().nullsLast())
                .compare(a, b)
                .result());
    return result;
  }
  ...
  /**
   * Sort element settings such that system settings come first and normal element settings after
   * that, with each group alphabetically sorted.
   *
  ...
  private LinkedHashMap<String, String> sortSettingsForSave(Map<String, String> settings) {
    ...
    keys.sort(
        (a, b) ->
            ComparisonChain.start()
                .compareTrueFirst(
                    a.startsWith(SYSTEM_SETTING_PREFIX), b.startsWith(SYSTEM_SETTING_PREFIX))
                .compare(a, b)
                .result());
    for (String key : keys) {
   ...
   */
  private Map<String, String> translateMapKeys(
      Map<String, String> settings, Function<String, String> translation) {
  ...
  /**
   * Translates new SYSTEM:: prefixed names for legacy system element settings to their non-prefixed
   * form.
   *
