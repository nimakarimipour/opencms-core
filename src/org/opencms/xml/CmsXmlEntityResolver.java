 ...
 *
 * <p>Also provides a cache for XML content schema definitions.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsXmlEntityResolver implements EntityResolver, I_CmsEventListener {
   ...
   * of each pair is the prefix which should be replaced by the second component of that pair.
   */
  private static final String[][] LEGACY_TRANSLATIONS = {
    {
      "opencms://system/modules/org.opencms.ade.config/schemas/",
      "internal://org/opencms/xml/adeconfig/"
    },
    {
    ...
    {
      "opencms://system/modules/org.opencms.ade.sitemap/schemas/",
      "internal://org/opencms/xml/adeconfig/sitemap/"
    },
    {"opencms://system/modules/org.opencms.ugc/schemas/", "internal://org/opencms/ugc/"},
    {
      "opencms://system/modules/org.opencms.jsp.search/schemas/",
      "internal://org/opencms/jsp/search/"
    }
  };
  ...
  private static final String XMLPAGE_OLD_DTD_SYSTEM_ID_1 =
      "http://www.opencms.org/dtd/6.0/xmlpage.dtd";
   ...
   *
   * <p>If the provided CmsObject is null, then the OpenCms VFS is not searched for XML entities,
   * however the internal cache and other OpenCms internal entities not in the VFS are still
   * resolved.
   *
   * <p>
   *
   ...
   *
   * <p>This cache will NOT be cleared automatically.
   *
   * <p>
   *
   ...
   *
   * <p>This check is required to see if a XML content is based on a file that actually exists in
   * the OpenCms VFS, or if the schema has been just cached without a VFS file.
   *
   * <p>
   *
   * @param systemId the system id ID check
   * @return <code>true</code> if the system ID is in the internal permanent cache, <code>false
   *     </code> otherwise
   */
  ...
  /**
   * Checks whether the given schema id is an internal schema id or is translated to an internal
   * schema id.
   *
   * <p>
   *
  ...
  /**
   * Initializes the internal caches for permanent and temporary system IDs.
   *
   * <p>
   */
  private static void initCaches() {
    ...
    if (OpenCms.getRunLevel() > OpenCms.RUNLEVEL_1_CORE_OBJECT) {
      if ((OpenCms.getMemoryMonitor() != null)
        ...
        OpenCms.getMemoryMonitor()
            .register(CmsXmlEntityResolver.class.getName() + ".cacheTemporary", cacheTemporary);
        ...
        OpenCms.getMemoryMonitor()
            .register(CmsXmlEntityResolver.class.getName() + ".cachePermanent", cachePermanent);

        Map<String, CmsXmlContentDefinition> cacheContentDefinitions =
            CmsMemoryMonitor.createLRUCacheMap(CONTENT_DEFINITION_CACHE_SIZE);
    ...
    switch (event.getType()) {
      ...
      case I_CmsEventListener.EVENT_RESOURCE_MOVED:
        List<CmsResource> resources =
            CmsCollectionsGenericWrapper.list(
  ...
  public InputSource resolveEntity(String publicId, String systemId) throws IOException {
    ...
    if (content != null) {
      ...
      try (InputStream stream =
          getClass().getClassLoader().getResourceAsStream(XMLPAGE_XSD_LOCATION)) {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_XMLPAGE_XSD_NOT_FOUND_1, XMLPAGE_XSD_LOCATION),
            t);
      }

    } else if (systemId.equals(XMLPAGE_OLD_DTD_SYSTEM_ID_1)
        || systemId.endsWith(XMLPAGE_OLD_DTD_SYSTEM_ID_2)) {

      // XML page DTD reference
      try (InputStream stream =
          getClass().getClassLoader().getResourceAsStream(XMLPAGE_OLD_DTD_LOCATION)) {
        // cache the XML page DTD
        content = CmsFileUtil.readFully(stream);
        m_cachePermanent.put(systemId, content);
        return createInputSource(content, systemId);
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_XMLPAGE_DTD_NOT_FOUND_1, XMLPAGE_OLD_DTD_LOCATION),
            t);
      }
      ...
      try (InputStream stream = getClass().getClassLoader().getResourceAsStream(location)) {
        ...
        .substring(0, systemId.lastIndexOf("/") + 1)
  ...
  /**
   * Removes a cached entry for a system id (filename) from the internal offline temporary and
   * content definition caches.
   *
   * <p>The online resources cached for the online project are only flushed when a project is
   * published.
   *
   * <p>
   *
  ...
  public void uncacheSystemId(String systemId) {
    ...
    if (null != o) {
      // if an object was removed from the temporary cache, all XML content definitions must be
      // cleared
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_ERR_UNCACHED_SYS_ID_1, getCacheKey(systemId, false)));
      }
    } else {
      ...
      if ((null != o) && LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_ERR_UNCACHED_CONTENT_DEF_1, getCacheKey(systemId, false)));
      }
    }
  }
  ...
  /**
   * Returns a cache key for the given system id (filename) based on the status of the given project
   * flag.
   *
   * <p>
   *
  ...
  /**
   * Returns a cache key for the given system id (filename) based on the status of the internal
   * CmsObject.
   *
   * <p>
   *
  ...
  private String getCacheKeyForCurrentProject(String systemId) {
    ...
    // check the project
    boolean project =
        (m_cms != null) ? m_cms.getRequestContext().getCurrentProject().isOnlineProject() : false;
   ...
   * @return true, if there is at least one xsd or dtd file in the list of resources to publish,
   *     otherwise false
   */
  private boolean isSchemaDefinitionInPublishList(CmsUUID publishHistoryId) {
    ...
    try {
      List<CmsPublishedResource> publishedResources =
          m_cms.readPublishedResources(publishHistoryId);
