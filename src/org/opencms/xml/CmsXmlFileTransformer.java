...
public class CmsXmlFileTransformer {
  ...
  public CmsXmlFileTransformer(
    ...
    m_onlineCms
        .getRequestContext()
        .setCurrentProject(cms.readProject(CmsProject.ONLINE_PROJECT_NAME));
  ...
  private CmsMessageContainer message(String content) {
    ...
    return org.opencms.report.Messages.get()
        .container(org.opencms.report.Messages.RPT_GENERIC_1, content);
  }
    ...
    for (CmsResource resource : resources) {
      ...
      try (AutoCloseable c = CmsLockUtil.withLockedResources(m_offlineCms, resource)) {
        if (state.isNew()) {
          ...
          m_report.println(
              message("Skipping " + resource.getRootPath() + " because it is deleted."));
          ...
          if (!resource.getRootPath().equals(getOnlinePath(resource))) {
            ...
            m_report.println(
                message(
                    "Warning: Not publishing " + resource.getRootPath() + " because it is moved."));
          } else {
            ...
            if (needToUpdate(offlineContent, newOfflineContent)
              ...
              try {
                ...
                // Put this in a finally block so we write back the offline content even if the
  ...
  private byte[] readOnlineContent(CmsResource res) throws CmsException {
    ...
    return m_onlineCms
        .readFile(m_onlineCms.readResource(res.getStructureId(), CmsResourceFilter.ALL))
        .getContents();
  }

  /**
   * Lenient XML comparison that ignores distinctions like CDATA vs normal text nodes and
  ...
  private byte[] transformContent(byte[] content) throws TransformerException {
    ...
    Transformer transformer =
        m_transformerFactory.newTransformer(new StreamSource(new ByteArrayInputStream(m_xslt)));
    ByteArrayOutputStream baos = new ByteArrayOutputStream();
    transformer.transform(
        new StreamSource(new ByteArrayInputStream(content)), new StreamResult(baos));
