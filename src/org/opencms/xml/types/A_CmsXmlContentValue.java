...
public abstract class A_CmsXmlContentValue implements I_CmsXmlContentValue, I_CmsWidgetParameter {
  ...
  public void appendXmlSchema(Element root) {
    ...
    if ((getMinOccurs() > 1) || (getMinOccurs() == 0)) {
      element.addAttribute(
          CmsXmlContentDefinition.XSD_ATTRIBUTE_MIN_OCCURS, String.valueOf(getMinOccurs()));
    }
    if (getMaxOccurs() > 1) {
      if (getMaxOccurs() == Integer.MAX_VALUE) {
        ...
        element.addAttribute(
            CmsXmlContentDefinition.XSD_ATTRIBUTE_MAX_OCCURS, String.valueOf(getMaxOccurs()));
      }
    }
  }
  ...
  /**
   * @see
   *     org.opencms.xml.types.I_CmsXmlSchemaType#setContentDefinition(org.opencms.xml.CmsXmlContentDefinition)
   */
  ...
  protected CmsRelationType getRelationType(String path) {
    ...
    if (document != null) {
      // the relations set in the main content definition override relations set in the nested
      // definition
      result =
          document.getContentDefinition().getContentHandler().getRelationType(getPath(), result);
    } else {
   ...
   *
   * <p>Please note: No check is performed if the move violates the XML document schema!
   *
   * <p>
   *
  ...
  /**
   * Convenience method to loads the XML schema definition for this value type from an external
   * file.
   *
   * <p>
   *
  ...
  protected String readSchemaDefinition(@RUntainted String schemaUri) throws CmsRuntimeException {
    ...
    try {
      ...
      throw new CmsRuntimeException(
          Messages.get().container(Messages.ERR_XMLCONTENT_LOAD_SCHEMA_1, schemaUri), e);
    }
