...
public interface I_CmsXmlContentValue extends I_CmsXmlSchemaType {
  ...
  public static enum SearchContentType {
    ...
    /**
     * Converts the String into a SearchContentType. Returns <code>null</code> if conversion is not
     * possible.
     *
     * @param type the search content type as String.
     * @return the search content type specified by the provided String, or <code>null</code> if the
     *     String did not specify any search content type.
     */
   ...
   * with special handling of elements in choice groups.
   *
   * <p>This is useful in case there are more than one elements with the same XML node name in the
   * source XML document.
   *
   * <p>Elements in XML choice groups will share the same number space, so a choice sequence will be
   * numbered like this: <code>Title[1], Text[2], Title[3], Image[4]</code>
   *
   * <p>
   *
   * @return the index of this XML content node in the source document with special handling of
   *     elements in choice groups
   * @see #getXmlIndex()
   */
  int getIndex();
   ...
   * <p>
   *
   * @return the total number of XML elements of this type that currently exist in the source
   *     document
   */
  int getMaxIndex();
   ...
   *
   * <p>Plain text in this context means a pure textual representation of the content (i.e. without
   * html tags). The plain text may be <code>null</code>, too, if there is no sound or useful
  ...
  /**
   * Returns the search content type for the value. Default implementation uses the historic
   * isSearchable() method.
   *
  ...
  /**
   * Returns the node index of this XML content value in the source XML document, starting with 0,
   * based on the XML ordering.
   *
   * <p>Elements in choice groups will be numbered like this: <code>
   * Title[1], Text[1], Title[2], Image[1]</code>
   *
   * <p>
   *
   * @return the index of this XML content node in the source document with special handling of
  ...
  /**
   * Returns <code>true</code> in case this value is searchable by default with the integrated full
   * text search.
   *
   * <p>
   *
   ...
   *
   * <p>If the XML content value is already the first in it's sequence, it is not moved.
   *
   * <p>
   */
  void moveDown();
   ...
   *
   * <p>If the XML content value is already the last in it's sequence, it is not moved.
   *
   * <p>
   */
  void moveUp();
   ...
   *
   * <p>This method does provide processing of the content based on the users current OpenCms
   * context. This can be used e.g. for link extraction and replacement in the content.
   *
   * <p>
   *
   * @param cms an initialized instance of a CmsObject
