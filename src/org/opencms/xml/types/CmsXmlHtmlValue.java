...
public class CmsXmlHtmlValue extends A_CmsXmlContentValue implements I_CmsJsonFormattableValue {
    ...
    if (defaultValue != null) {
      try {
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_XMLCONTENT_INVALID_ELEM_DEFAULT_1, defaultValue),
            e);
  ...
  public String getPlainText(CmsObject cms) {
    ...
    if (m_plainTextValue == null) {
      try {
        m_plainTextValue =
            CmsHtmlExtractor.extractText(getStringValue(cms), m_document.getEncoding());
      } catch (Exception exc) {
    ...
    if (finalValue != null) {
      ...
      finalValue =
          finalValue.replaceAll(
              "(?s)// <!\\[CDATA\\[(.*?)// \\]\\]>", "$1"); // special case for embedded Javascript
    ...
    if (linkProcessor != null) {
      try {
        ...
        throw new CmsRuntimeException(
            Messages.get().container(Messages.ERR_HTML_DATA_PROCESSING_0), exc);
      }
    }
      ...
      if (linkProcessor != null) {
        ...
        for (Iterator<CmsLink> i = linkTable.iterator(); i.hasNext(); ) {
          ...
          CmsLinkUpdateUtil.updateXmlForHtmlValue(
              link, link.getName(), links.addElement(CmsXmlPage.NODE_LINK));
        }
      }
    }
  ...
  /**
   * JTidy sometimes erroneouslsy produces HTML containing 'null' characters (Unicode code point 0),
   * which are invalid in an XML document. Until we find a way to prevent JTidy doing that, we
   * remove the null characters from the HTML, and log a warning.
   *
   * <p>
   *
  ...
  /**
   * Creates the String value for this HTML value element.
   *
   * <p>
   *
    ...
    if ((enabled == null) || Boolean.valueOf(enabled.getText()).booleanValue()) {
      ...
      if (!linkTable.isEmpty()) {
        ...
        try {
          ...
          LOG.error(
              Messages.get().getBundle().key(Messages.ERR_XMLCONTENT_LINK_PROCESS_FAILED_0), e);
        }
      }
    }
    return content;
