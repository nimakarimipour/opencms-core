 ...
 *
 * <p>A XML content definition in OpenCms basically consists of a sequence of nodes in the following
 * format:
 *
 * <p><code>
 ...
 * </code>.
 *
 * <p>Internally, each configured element in a XML schema is represented by an instance of this
 * interface. This allows for constructing or changing the XML schema through the provided API.
 *
 * <p>Note that this class only <i>describes the definition</i> of a value in the XML schema. It is
 * not the representation of an actual value from a XML file, for this you need an instance of a
 * {@link org.opencms.xml.types.I_CmsXmlContentValue}.
 *
 * <p>
 *
 * @since 6.0.0
...
public interface I_CmsXmlSchemaType extends Comparable<I_CmsXmlSchemaType> {
  ...
  /**
   * Constant for the XML schema attribute "noNamespaceSchemaLocation" in the XML schema instance
   * namespace.
   */
  QName XSI_NAMESPACE_ATTRIBUTE_NO_SCHEMA_LOCATION =
   ...
   *
   * <p>This is used to dynamically build a XML schema from an instance of a {@link
   * org.opencms.xml.CmsXmlContentDefinition} class.
   *
   * <p>
   *
   ...
   *
   * <p>This is used to dynamically build a vaild XML content object from an initialized {@link
   * org.opencms.xml.CmsXmlContentDefinition} class.
   *
   * <p>Important: This method can only be used during initialization of a XML content object, not
   * to add values to an already initialized XML content. To add values after initialization, use
   * {@link org.opencms.xml.content.CmsXmlContent#addValue(CmsObject, String, Locale, int)}.
   *
   * <p>
   *
   ...
   *
   * <p>This will be greater than 0 only if {@link #isChoiceType()} is <code>true</code>.
   *
   * <p>
   *
   ...
   *
   * <p>If the value belongs to a nested content, then the nested content definition is returned.
   * This means that for documents with nested schemas, the content definition of a value is not
   * necessarily equal to the content definition of the document itself, which can be obtained using
   * {@link I_CmsXmlContentValue#getDocument()}.
   *
   * <p>
   *
   ...
   *
   * <p>The XML element node name can be configured in the schema. For example, the node name could
   * be <code>"Title"</code>, <code>"Teaser"</code> or <code>"Text"</code>. The XML schema controls
   ...
   *
   * <p>By convention, a XML schema type name has the form <code>"OpenCms + ${name}"</code>.
   * Examples are <code>"OpenCmsString"</code> or <code>"OpenCmsBoolean"</code>.
   *
   * <p>The schema type name is fixed by the implementation.
   *
   * <p>
   *
   ...
   *
   * <p>This method is intended to check if this type is a node inside a choice sequence. In order
   * to check if this type defines a choice sequence, use {@link #isChoiceType()}.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this type defines a choice sequence.
   *
   * <p>This method is intended to check if this type is a node defining a choice sequence. In order
   * to check if this type is an option inside of a choice sequence, use {@link #isChoiceOption()}.
   *
   * <p>
   *
   * @return <code>true</code> if this node defines a choice sequence
  ...
  /**
   * Returns <code>true</code> if this is a simple type, or <code>false</code> if this type is a
   * nested schema.
   *
   * <p>If a value is a nested schema, it must be an instance of {@link
   ...
   *
   * <p>This is done automatically when the scheme type is added to a content definition. Usually
   * there is no need to call this method from the application.
   *
   * <p>
   *
  ...
  /**
   * Checks if a given value is valid according to the validation rule (regular expression) used for
   * validation of this schema type in the XML schema.
   *
   * <p>To have a more refined validation according to the special requirements of the content type,
   * use custom validation rules in the appinfo which are processed with {@link
   * org.opencms.xml.content.I_CmsXmlContentHandler#resolveValidation(CmsObject,
   * I_CmsXmlContentValue, org.opencms.xml.content.CmsXmlContentErrorHandler)}.
   *
   * <p>
   *
   * @param value the value to validate
   * @return the validation rule (regular expression) used for this schema type in the XML schema
   * @see org.opencms.xml.content.I_CmsXmlContentHandler#resolveValidation(CmsObject,
   *     I_CmsXmlContentValue, org.opencms.xml.content.CmsXmlContentErrorHandler)
   */
