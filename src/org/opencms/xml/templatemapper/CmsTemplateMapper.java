 ...
 * and settings according to the configuration file /system/config/template-mapping.xml.
 *
 * <p>
 */
public final class CmsTemplateMapper {
  ...
  /**
   * Hidden default constructor, because this is a singleton.
   *
   * <p>
   */
  ...
  public static String getTemplateMapperConfig(ServletRequest request) {
    ...
    CmsTemplateContext templateContext =
        (CmsTemplateContext) request.getAttribute(CmsTemplateContextManager.ATTR_TEMPLATE_CONTEXT);
    if (templateContext != null) {
      ...
      if (provider instanceof I_CmsTemplateMappingContextProvider) {
        result =
            ((I_CmsTemplateMappingContextProvider) provider)
                .getMappingConfigurationPath(templateContext.getKey());
      }
    }
    return result;
  }
   ...
   */
  public CmsContainerPageBean transformContainerpageBean(
      CmsObject cms, CmsContainerPageBean input, String rootPath) {
  ...
  public CmsContainerElementBean transformDetailElement(
      CmsObject cms, CmsContainerElementBean input, String rootPath) {
   ...
   * @return the transformed bean
   */
  public CmsGroupContainerBean transformGroupContainer(
      CmsObject cms, CmsGroupContainerBean input, String rootPath) {
    ...
    CmsGroupContainerBean result =
        new CmsGroupContainerBean(
            input.getTitle(), input.getDescription(), newElements, transformedTypes);
    return result;
  }
  ...
  protected CmsContainerElementBean transformContainerElement(
      CmsObject cms, CmsTemplateMapperConfiguration config, CmsContainerElementBean element) {
      ...
      try {
        if (element.isGroupContainer(cms)) {
          ...
          if (OpenCms.getResourceManager()
              .matchResourceType("function", element.getResource().getTypeId())) {
            ...
            .loadVfsObject(
                ...
                new Transformer() {
                  ...
                  public Object transform(Object input) {
                    ...
                    try {
                      CmsFile file =
                          cms.readFile(m_configPath, CmsResourceFilter.IGNORE_EXPIRATION);
                      ...
                      try (ByteArrayInputStream stream =
                        ...
                        CmsTemplateMapperConfiguration config =
                            new CmsTemplateMapperConfiguration(cms, document);
                        return config;
                      }
                    } catch (Exception e) {
                      LOG.warn(e.getLocalizedMessage(), e);
