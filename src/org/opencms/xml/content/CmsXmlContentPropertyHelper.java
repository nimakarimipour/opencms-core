...
public final class CmsXmlContentPropertyHelper implements Cloneable {
  ...
  /**
   * If a property has this value, the page-property macro for this property will expand to the
  ...
  public static Map<String, String> convertPropertiesToClientFormat(
      CmsObject cms, Map<String, String> props, Map<String, CmsXmlContentProperty> propConfig) {
  ...
  /**
   * Looks up an URI in the sitemap and returns either a sitemap entry id (if the URI is a sitemap
   * URI) or the structure id of a resource (if the URI is a VFS path).
   *
   * <p>
   *
  ...
  /**
   * Creates and configures a new macro resolver for resolving macros which occur in property
   * definitions.
   *
   * <p>
   *
   * @param cms the CMS context
   * @param contentHandler the content handler which contains the message bundle that should be
   *     available in the macro resolver
   * @param content the XML content object
   * @param stringtemplateSource provides stringtemplate templates for use in %(stringtemplate:...)
   *     macros
  ...
  public static CmsMacroResolver getMacroResolverForProperties(
    ...
    final CmsGalleryNameMacroResolver resolver =
        new CmsGalleryNameMacroResolver(cms, content, locale) {
  ...
  public static Map<String, CmsXmlContentProperty> getPropertyInfo(
      CmsObject cms, CmsResource page, CmsResource resource) throws CmsException {

    if (CmsResourceTypeXmlContent.isXmlContent(resource)) {
      I_CmsXmlContentHandler contentHandler =
          CmsXmlContentDefinition.getContentHandlerForResource(cms, resource);
      ...
      CmsMacroResolver resolver =
          getMacroResolverForProperties(cms, contentHandler, content, null, page);
   ...
   *
   * <p>If the type is {@link CmsXmlContentProperty.PropType#vfslist}, the value is parsed as a list
   * of paths and converted to a list of IDs.
   *
   * <p>
   *
   * @param cms the current CMS context
   ...
   *
   * <p>If the type is {@link CmsXmlContentProperty.PropType#vfslist}, the value is parsed as a list
   * of IDs and converted to a list of paths.
   *
   * <p>
   *
   * @param cms the current CMS context
   ...
   *
   * <p>This method first tries to read a sitemap entry with the given id. If this succeeds, the
   * sitemap entry's sitemap path will be returned. If it fails, the method interprets the id as a
   * structure id and tries to read the corresponding resource, and then returns its VFS path.
   *
   * <p>
   *
   * @param cms the CMS context
   ...
   *
   * <p>The configuration string should be a map of key value pairs separated by ':' and '|':
  ...
  public static JSONObject getWidgetConfigurationAsJSON(@RUntainted String widgetConfiguration) {
    ...
    for (Map.Entry<String, @RUntainted String> entry : confEntries.entrySet()) {
      try {
        ...
        LOG.error(
            Messages.get()
                .container(Messages.ERR_XMLCONTENT_UNKNOWN_ELEM_PATH_SCHEMA_1, widgetConfiguration),
            e);
      }
    }
    return result;
  ...
  /**
   * Extends the given properties with the default values from the resource's property
   * configuration.
   *
   * <p>
   *
        ...
        if (formatter != null) {
          propertyConfig =
              OpenCms.getADEManager()
                  .getFormatterSettings(cms, config, formatter, resource, locale, request);
        } else {
          // fall back to schema configuration
          propertyConfig =
              CmsXmlContentDefinition.getContentHandlerForResource(cms, resource)
                  .getSettings(cms, resource);
        }
      } catch (CmsException e) {
        // should never happen
        LOG.error(e.getLocalizedMessage(), e);
  ...
  public static Map<String, String> readProperties(
      CmsObject cms, I_CmsXmlContentLocation baseLocation) {
    ...
    for (I_CmsXmlContentValueLocation propertyLocation : propertyLocations) {
      ...
      I_CmsXmlContentValueLocation valueLocation =
          propertyLocation.getSubValue(CmsXmlContentProperty.XmlNode.Value.name());
      I_CmsXmlContentValueLocation stringLocation =
          valueLocation.getSubValue(CmsXmlContentProperty.XmlNode.String.name());
      I_CmsXmlContentValueLocation fileListLocation =
          valueLocation.getSubValue(CmsXmlContentProperty.XmlNode.FileList.name());
      if (stringLocation != null) {
        value = stringLocation.asString(cms).trim();
      } else if (fileListLocation != null) {
        List<CmsUUID> idList = new ArrayList<CmsUUID>();
        List<I_CmsXmlContentValueLocation> fileLocations =
            fileListLocation.getSubValues(CmsXmlContentProperty.XmlNode.Uri.name());
  ...
  public static Map<String, String> readProperties(
    ...
    // Properties
    for (Iterator<Element> itProps =
            CmsXmlGenericWrapper.elementIterator(
                element, CmsXmlContentProperty.XmlNode.Properties.name());
        itProps.hasNext(); ) {
      ...
      String propPath =
          CmsXmlUtils.concatXpath(
              elemPath, CmsXmlUtils.createXpathElement(property.getName(), propIndex));
      ...
      CmsXmlContentDefinition propDef =
          ((CmsXmlNestedContentDefinition) propSchemaType).getNestedContentDefinition();
      ...
      String valuePath =
          CmsXmlUtils.concatXpath(
              propPath, CmsXmlUtils.createXpathElement(value.getName(), valueIndex));
      ...
      CmsXmlContentDefinition valueDef =
          ((CmsXmlNestedContentDefinition) valueSchemaType).getNestedContentDefinition();
      ...
      if (string != null) {
        ...
        String valueFileListPath =
            CmsXmlUtils.concatXpath(
                valuePath,
                CmsXmlUtils.createXpathElement(valueFileList.getName(), valueFileListIndex));
        I_CmsXmlSchemaType valueFileListSchemaType =
            valueDef.getSchemaType(valueFileList.getName());
        I_CmsXmlContentValue valueFileListValue =
            valueFileListSchemaType.createValue(xmlContent, valueFileList, locale);
        xmlContent.addBookmarkForValue(valueFileListValue, valueFileListPath, locale, true);
        CmsXmlContentDefinition valueFileListDef =
            ((CmsXmlNestedContentDefinition) valueFileListSchemaType).getNestedContentDefinition();
  ...
  /**
   * Resolves macros in the given property information for the given resource (type) AND the current
   * user.
   *
   * <p>
   *
    ...
    if (CmsResourceTypeXmlContent.isXmlContent(resource)) {
      ...
      CmsMacroResolver resolver =
          getMacroResolverForProperties(
              cms, contentHandler, contentGetter.get(), stringtemplateSource, page);
      return resolveMacrosInProperties(propertiesConf, resolver);
    }
  ...
  public static Map<String, CmsXmlContentProperty> resolveMacrosInProperties(
      Map<String, CmsXmlContentProperty> properties, I_CmsMacroResolver resolver) {
   ...
   * @param properties the properties to save, if there is a list of resources, every entry can be a
   *     site path or a UUID
  ...
  public static void saveProperties(
    ...
    // remove old entries
    for (Object propElement :
        parentElement.elements(CmsXmlContentProperty.XmlNode.Properties.name())) {
    ...
    for (Map.Entry<String, String> property : props.entrySet()) {
      ...
      Element propElement =
          parentElement.addElement(CmsXmlContentProperty.XmlNode.Properties.name());
   ...
   */
  protected static void addFileListPropertyValue(
      CmsObject cms, Element valueElement, String propValue) {
    ...
    for (@RUntainted String strId :
      ...
      try {
        ...
        // HACK: here we assume weak relations, but it would be more robust to check it, with smth
        // like:
        // type =
        // xmlContent.getContentDefinition().getContentHandler().getRelationType(fileValueElem.getPath());
  ...
  /**
   * Converts a string containing zero or more structure ids into a string containing the
   * corresponding VFS paths.
   *
   * <p>
   *
  ...
  /**
   * Converts a string containing zero or more VFS paths into a string containing the corresponding
   * structure ids.
   *
   * <p>
   *
  ...
  /**
   * Helper method for converting a map of properties from client format to server format or vice
   * versa.
   *
   * <p>
   *
  ...
  /**
   * Given a string which might be a id or a (sitemap or VFS) URI, this method will return a bean
   * containing the right (sitemap or vfs) root path and (sitemap entry or structure) id.
   *
   * <p>
   *
   * @param cms the current CMS context
   ...
   *
   * <p>This method uses the base name of the property to access the property configuration, i.e. if
   * propName starts with a '#', the part after the '#' will be used as the key for the property
   * configuration.
   *
   * <p>
   *
  ...
  protected static CmsXmlContentProperty getPropertyConfig(
      Map<String, CmsXmlContentProperty> propertyConfig, String propName) {
