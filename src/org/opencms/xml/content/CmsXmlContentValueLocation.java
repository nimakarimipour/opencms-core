...
public class CmsXmlContentValueLocation implements I_CmsXmlContentValueLocation {
  ...
  public CmsXmlContentValueLocation(I_CmsXmlContentValue value) {
    ...
    if (value == null) {
      throw new UnsupportedOperationException(
          "Can't create content value location with a null value.");
    }
    m_value = value;
  }
  ...
  public CmsXmlContentValueLocation getSubValue(@RUntainted String subPath) {
    ...
    I_CmsXmlContentValue subValue =
        m_value.getDocument().getValue(CmsXmlUtils.concatXpath(m_value.getPath(), subPath), locale);
    if (subValue != null) {
  ...
  public List<I_CmsXmlContentValueLocation> getSubValues(@RUntainted String subPath) {
    ...
    Locale locale = m_value.getLocale();
    List<I_CmsXmlContentValue> subValues =
        Lists.newArrayList(
            m_value
                .getDocument()
                .getValues(CmsXmlUtils.concatXpath(m_value.getPath(), subPath), locale));
