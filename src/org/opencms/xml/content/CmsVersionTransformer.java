...
public class CmsVersionTransformer {
  ...
  /**
   * Converts an XML content document to the current version using the version transformation XSLT
   * file which is configured in the schema.
   *
  ...
  public static Document transformDocumentToCurrentVersion(
    ...
    try {
      CmsResource xsltResource =
          cms.readResource(transformation, CmsResourceFilter.IGNORE_EXPIRATION);
      CmsFile xsltFile = cms.readFile(xsltResource);
      // we explicitly want an Xalan transformer factory here, even if we add some other XSLT
      ...
      SAXSource transformationSource =
          new SAXSource(new InputSource(new ByteArrayInputStream(xsltFile.getContents())));
      ...
      throw new CmsRuntimeException(
          Messages.get()
              .container(Messages.ERR_XMLCONTENT_VERSION_TRANSFORMATION_ERROR_1, transformation),
          e);
    }
  }
}
