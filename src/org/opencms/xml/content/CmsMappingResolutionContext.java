 ...
 *
 * <p>Since the content handler instance is shared between all contents of the same XML content
 * type, we can't use it to store data which is only relevant for resolving the mappings of a single
 * XML content, so this class was created.
 */
public class CmsMappingResolutionContext {
  ...
  /**
   * Internal bean used to keep track of URL name mappings.
   *
   * <p>
   */
  ...
  public void commitUrlNameMappings() throws CmsException {
      ...
      try {
        ...
        CmsProperty prop =
            m_cms.readPropertyObject(
                resource, CmsPropertyDefinition.PROPERTY_URLNAME_REPLACE, true);
    ...
    for (InternalUrlNameMappingEntry entry : m_urlNameMappingEntries) {
      ...
      m_cms.writeUrlNameMapping(
          nameSeq, entry.getStructureId(), entry.getLocale().toString(), urlnameReplace);
    }
  }
   ...
   *
   * <p>Needs to also set the attributes on the resource of m_content because it's written later by
   * the content handler.
   *
  ...
  public void setAttribute(CmsResource res, AttributeType type, Long value) throws CmsException {
    ...
    if (type == AttributeType.release) {
      ...
      if ((m_content.getFile() != null)
          && res.getStructureId().equals(m_content.getFile().getStructureId())) {
  ...
  protected void writeAttributes() {
    ...
    try {
      for (CmsResource sibling : m_cms.readSiblings(file, CmsResourceFilter.IGNORE_EXPIRATION)) {
        ...
        try {
          CmsProperty localeProp =
              m_cms.readPropertyObject(sibling, CmsPropertyDefinition.PROPERTY_LOCALE, true);
          ...
          for (Locale locale : locales) {
            // Use first locale from the property which is actually in the content, whether it has
            // the values or not
            // (if it doesn't have them, i.e. null is passed to setAttribute, the default values
            // wlll be set)
  ...
  /**
   * Adds an URL name mapping which should be written to the database later.
   *
   * <p>
   *
