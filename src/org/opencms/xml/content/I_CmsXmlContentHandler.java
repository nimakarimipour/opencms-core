...
public interface I_CmsXmlContentHandler {
  ...
  /**
   * The available mapping types. Currently only available for searchsettings (using the "addto"
   * attribute in the node &lt;solrfield&gt;).
   */
  ...
  /**
   * Checks if an availability attribute should be written back to the content if the availability
   * is changed through the availability dialog.
   *
  ...
  /**
   * Returns the configuration String value for the widget used to edit the given XML content schema
   * type.
   *
   * <p>If no configuration value is available, this method must return <code>null</code>.
   *
   * @param type the value to get the widget configuration for
   * @return the configuration String value for the widget used to edit the given XML content schema
   *     type
   */
   ...
   * content.
   *
   * <p>If a schema type does not have a default value, this method must return <code>null</code>.
   *
   ...
   * <p>
   *
   * @param selfOnly if true, only return editor change handlers configured directly for this
   *     content handler
  ...
  /**
   * Gets the action to perform if the given name refers to a link field which refers to a VFS file
   ...
   *
   * <p>This is used to automatically provide fallbacks for missing message keys in the editor.
   *
   * <p>
   *
  ...
  /**
   * Returns the {@link CmsMessages} that are used to resolve localized keys for the given locale in
   * this content handler.
   *
   * <p>If no localized messages are configured for this content handler, this method returns <code>
   * null</code>.
   *
   * <p>
   *
   ...
   */
  List<String> getNestedFormatters(
   ...
   *
   * <p>If <code>null</code> is returned, no preview is possible for contents using this handler.
   *
   * <p>
   *
   ...
   * @deprecated use {@link #getRelationType(String)} with {@link I_CmsXmlContentValue#getPath()}
   *     instead
   */
  @Deprecated
   ...
   *
   * <p>Moreover, if the value contains a link to another resource, the content of that other
   * resource can be added.
   *
   * @param value the XML content value to check
   * @return the search content type, indicating how the element should be added to the content for
  ...
  /**
   * Returns all configured Search fields for this XML content that should be attached to container
   * pages the content is placed on.
   *
   * <p>
   *
   * @return the Search fields for this XMl content
   ...
   *
   * <p>A search setting defined within the xsd:annotaion node of an XML schema definition can look
   * like:
   *
   * <p><code>&lt;searchsetting element="Image/Align" searchContent="false"/&gt;</code>
   *
   * <p>The returned map contains the 'element' attribute value as keys and the 'searchContent'
   * attribute value as values.
   *
   * <p>
   *
   ...
   *
   * @param cms the current OpenCms user context
   * @param document the XML content to get the title mapping for (this must be of a type that uses
   *     this handler)
   ...
   *
   * <p>If schema versions are used, the the XSLT transformation read from this VFS path is used to
   * transform contents of older versions into the current version.
   *
   ...
   *
   * <p>The handler implementations should use the "appinfo" node of the XML content definition
   * schema to define the mappings of elements to widgets.
   *
   * <p>
   *
  ...
  /**
   * Returns true if the contents for this content handler have schema-based formatters which can be
   * disabled or enabled.
   *
   * <p>
   *
   * @return true if the contents for this content handler have schema-based formatters which can be
   *     disabled or enabled
   */
  ...
  /**
   * Initializes this content handler for the given XML content definition by analyzing the
   * "appinfo" node.
   *
   * <p>
   *
   ...
   *
   * <p>The main difference to the {@link #resolveValidation(CmsObject, I_CmsXmlContentValue,
   * CmsXmlContentErrorHandler)} method is that this method may silently remove some values, for
  ...
  /**
   * Returns <code>true</code> if the XML content should be indexed when it is dropped in a
   * container page, and returns <code>false</code> if this XML content should be indexed as
   * 'stand-alone' document.
   *
   * <p>This flag is intended by excluding XML contents from the search index that are not used as
   * detail pages, but to index those extraction result when they are part of a container page.
   *
   * <p>In order to set this falg add an attribute <code>containerpageOnly="true"</code> to the
   * <code>'&lt;searchsettings&gt;-node'</code> of the XSD of the resource type you want to be
   * indexed only when it is part of a container page.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> in case the given value should be searchable with the integrated full
   * text search.
   *
   * <p>For the full text search, the value of all elements in one locale of the XML content are
   * combined to one big text, which is referred to as the "content" in the context of the full text
   ...
   * @deprecated use {@link #getSearchContentType(I_CmsXmlContentValue)} instead. Will be removed if
   *     plain lucene search is removed.
   */
  @Deprecated
   ...
   *
   * <p>This method is always called after any content is unmarshalled. It can be used to perform
   * customized actions on the given XML content.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
   ...
   *
   * <p>This method is always called before any content gets written. It can be used to perform XML
   * validation, pretty - printing or customized actions on the given XML content.
   *
   * <p>
   *
   * @param cms the current OpenCms user context
   ...
   * @return the file to write to the OpenCms VFS, this will be an updated version of the parameter
   *     file
  ...
  /**
   * Resolves the value mappings of the given XML content value, according to the rules of this XML
   * content handler.
   *
   * <p>
   *
   ...
   *
   * <p>The errorHandler parameter is optional, if <code>null</code> is given a new error handler
