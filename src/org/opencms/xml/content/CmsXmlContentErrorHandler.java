...
public class CmsXmlContentErrorHandler {
  ...
  public void addError(I_CmsXmlContentValue value, String message) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_XMLCONTENT_VALIDATION_ERR_2, value.getPath(), message));
    }
  }
  ...
  public void addWarning(I_CmsXmlContentValue value, String message) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_XMLCONTENT_VALIDATION_WARN_2, value.getPath(), message));
    }
  }
   ...
   *
   * <p>The map contains further maps. The key of the "first" map is the {@link java.util.Locale} of
   * the language where issues where found. The key of the "second" map is a mapping from the
   * element node name obtained with {@link I_CmsXmlContentValue#getPath()} to the error message
   * which is a String.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if there is at last one error in the selected locale.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if there is at last one warning in the selected locale.
   *
   * <p>
   *
   * @param locale the locale to check
  ...
  /**
   * Returns the localized issue map from the given base map.
   *
   * <p>If the base map does not contains an issue map for the given locale, a new map is added for
   * the locale.
   *
   * <p>
   *
   ...
   */
  private Map<String, String> getLocalIssueMap(
      Map<Locale, Map<String, String>> base, Locale locale) {
