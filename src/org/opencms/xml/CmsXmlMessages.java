 ...
 *
 * <p>As fallback if no file was specified or no value was found for the desired key, a common
 * CmsMessages object is used to get the localized value.
 *
 * <p>
 *
 * @since 6.5.4
...
public class CmsXmlMessages extends CmsMessages {
   ...
   *
   * <p>Creates the necessary member objects using the passed arguments.
   *
   * <p>
   *
  ...
  public CmsXmlMessages(
      CmsMessages messages, String configurationFileName, String pathPrefix, Locale locale) {
  ...
  /**
   * Returns the localized resource String from the configuration file, if not found or set from the
   * resource bundle.
   *
   * <p>
   *
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.ERR_INVALID_INIT_USER_1, OpenCms.getDefaultUsers().getUserGuest()));
      }
    }
    if ((cms != null) && CmsStringUtil.isNotEmptyOrWhitespaceOnly(configurationFileName)) {
      ...
      Object o =
          CmsVfsMemoryObjectCache.getVfsMemoryObjectCache()
              .getCachedObject(cms, configurationFileName);
      if (o != null) {
      ...
      try {
        ...
        CmsVfsMemoryObjectCache.getVfsMemoryObjectCache()
            .putCachedObject(cms, configurationFileName, m_localizationContent);
      } catch (CmsException e) {
