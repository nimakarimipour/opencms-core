...
public class CmsJsonRendererContainerPage {
   ...
   */
  public CmsJsonRendererContainerPage(
      CmsObject cms, CmsResource page, Predicate<String> propertyFilter) {
   ...
   *
   * <p>The returned tree consists of container nodes (which have children corresponding the
   * container elements) and element nodes (which have children corresponding to the nested
   * containers of the element). The root of the tree is a dummy element node which does not
   * correspond to any element in the page, but just acts as a container for the top-level
  ...
  public ElementNode buildTree(CmsContainerPageBean page, String rootPath) {
    ...
    for (CmsContainerBean container : page.getContainers().values()) {
      ...
      for (CmsContainerElementBean cachedElementBean : container.getElements()) {
        ...
        elementBean.initSettings(
            m_cms, adeConfig, formatter, Locale.ENGLISH, null, new HashMap<>());
        containerNode.add(elemNode);
      }
    }

    ElementNode rootElement =
        new ElementNode(null, null); // Dummy element containing all the top-level containers
    for (ContainerNode containerNode : containerNodes) {
      ...
      ElementNode parentElement =
          CmsStringUtil.isEmpty(parentId) ? null : elementsByInstanceId.get(parentId);
    ...
    if (elementNode.getElement() != null) {
      ...
      for (Map.Entry<String, String> entry : elementNode.getElement().getSettings().entrySet()) {
        ...
        if (entry.getKey().startsWith("formatterSettings")
            || entry.getKey().equals("element_instance_id")) {
          continue;
        }
   ...
   *
   * <p>This only relies on the container element data itself since container width/type are not
   * stored with the container, so it may return null if no formatter is set as an element setting.
   *
   * @param cms the CMS context
  ...
  private I_CmsFormatterBean getFormatter(
    ...
    Collection<I_CmsFormatterBean> formatterList =
        adeConfig
            .getCachedFormatters()
            .getFormattersForType(
                OpenCms.getResourceManager()
                    .getResourceType(elementBean.getResource())
                    .getTypeName(),
                false);
