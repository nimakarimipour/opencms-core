 ...
 * its nodes.
 *
 * <p>This specific renderer class does not need to be initialized with a CmsJsonHandlerContext, you
 * can just initialize it with a CmsObject.
 */
public class CmsJsonRendererXmlContent implements I_CmsJsonRendererXmlContent {
   ...
   *
   * <p>If this constructor is used, you still have to call one of the initialize() methods before
   * rendering XML content to JSON.
   */
  ...
  /**
   * Builds a simple JSON object with link and path fields whose values are taken from the
  ...
  /**
   * Helper method to apply renderer to all locales of an XML content, and put the resulting objects
   * into a JSON object with the locales as keys.
   *
   ...
   * @throws JSONException if something goes wrong
   */
  public static JSONObject renderAllLocales(
      CmsXmlContent content, I_CmsJsonRendererXmlContent renderer) throws JSONException {
  ...
  /**
   * @see
   *     org.opencms.xml.xml2json.renderer.I_CmsJsonRendererXmlContent#initialize(org.opencms.xml.xml2json.handler.CmsJsonHandlerContext)
   */
  ...
  /**
   * @see
   *     org.opencms.xml.xml2json.renderer.I_CmsJsonRendererXmlContent#render(org.opencms.xml.content.CmsXmlContent,
   *     java.util.Locale)
   */
  @Override
  ...
  protected SimpleEntry<String, Object> renderField(Field field) throws JSONException {
    ...
    if (field.isMultivalue()) {
      ...
      if (field.getFieldDefinition().isChoiceType()) {
        ...
        // Multiple choice values can be represented by either a multivalued field of single-valued
        // choices,
        // or a single-valued field of multivalue choices. Using both in combination doesn't seem to
        // make sense.
        // So we collapse consecutive choice values in a multivalue field to give a uniform JSON
        // syntax for the two
        // cases to make the JSON easier to work with, but we lose some information about the
