...
public class CmsJsonResourceHandler implements I_CmsResourceInit, I_CmsNeedsAdminCmsObject {
  ...
  private ServiceLoader<I_CmsJsonHandlerProvider> m_serviceLoader =
      ServiceLoader.load(I_CmsJsonHandlerProvider.class);
  ...
  public static I_CmsCustomLinkRenderer getLinkRenderer(CmsObject cms) {
    ...
    if (context instanceof CmsJsonHandlerContext) {
      String linkRewriterKey =
          ((CmsJsonHandlerContext) context)
              .getHandlerConfig()
              .get(CmsJsonResourceHandler.PARAM_LINKREWRITE_REFID);
  ...
  /**
   * Produces a link to the given resource, using the link renderer from the current CMS context if
   * it is set.
   *
  ...
  public List<I_CmsJsonHandler> getSubHandlers() {
    ...
    result =
        result.stream()
            .map(h -> new CmsExceptionSafeHandlerWrapper(h))
  ...
  /**
   * @see
   *     org.opencms.main.I_CmsResourceInit#initParameters(org.opencms.configuration.CmsParameterConfiguration)
   */
  ...
  public CmsResource initResource(
    ...
    // we don't care about multiple parameter values, single parameter values are easier to work
    // with
    ...
    try {
      if (cms == null) {
        ...
        CmsJsonHandlerContext context =
            new CmsJsonHandlerContext(
                cms, rootCms, path, resource, singleParams, m_config, accessPolicy);
