...
public class CmsJsonDocumentXmlContent extends CmsJsonDocumentResource {
  ...
  public CmsJsonDocumentXmlContent(
      CmsJsonRequest jsonRequest, CmsXmlContent xmlContent, boolean embedLinkedModelgroup)
      throws Exception {
  ...
  protected void insertJsonLinkedContent(CmsResource resource) throws Exception {
    ...
    try {
      ...
      if (CmsResourceTypeXmlContainerPage.isContainerPage(resource) && m_embedLinkedModelgroup) {
        ...
        CmsJsonDocumentXmlContent document =
            new CmsJsonDocumentXmlContent(m_jsonRequest, xmlContent, false);
        value = document.getJson();
      }
  ...
  /**
   * For each linked content, inserts a JSON representation of the linked content into this JSON
   * document.
   *
   * <p>
   *
  ...
  protected void insertJsonLinkedContents() throws Exception {
    ...
    List<CmsRelation> relationList =
        m_context
            .getCms()
            .getRelationsForResource(m_xmlContent.getFile(), CmsRelationFilter.TARGETS);
  ...
  /**
   * Whether the default locale content shall be shown in the case the requested locale is not
   * available.
   *
   * <p>
   *
  ...
  /**
   * Whether to show the wrapper with resource information. For backward compatibility the wrapper
   * is shown for the all locale request but not for the locale and locale path request as a
   * default. This default behavior can be changed by means of the "wrapper" request parameter. For
   * locale path requests, wrapper information is not available.
   *
  ...
  /**
   * Inserts a JSON representation of this XML content into this JSON document.
   *
   * <p>
   *
   ...
   */
  private void insertJsonContent()
      throws CmsJsonHandlerException, JSONException, PathNotFoundException {
  ...
  /**
   * Inserts all locale contents into this JSON document.
   *
   * <p>
   *
   * @throws JSONException if JSON rendering fails
  ...
  /**
   * Inserts one locale content into this JSON document.
   *
   * <p>
   *
   * @throws JSONException if JSON rendering fails
   * @throws PathNotFoundException if the selected locale does not exist and no fallbackLocale
   *     parameter is given
   */
  private void insertJsonContentLocale() throws JSONException, PathNotFoundException {
    ...
    if (localeExists) {
      ...
      throw new PathNotFoundException(
          "Locale <" + this.m_jsonRequest.getParamLocale() + "> not found.");
    }
    if (isShowWrapperRequest()) {
  ...
  /**
   * Inserts a fragment of one locale content into this JSON document.
   *
   * <p>
   *
  ...
  /**
   * Insert a list of all locale names available for this content.
   *
   * <p>
   *
   * @throws JSONException if JSON rendering fails
