...
public class CmsJsonAccessPolicy {
  ...
  public static final Pattern DEFAULT_PROP_FILTER =
      Pattern.compile("(?i)^(?!.*(?:secret|api|password|key)).*$");
  ...
  public static CmsJsonAccessPolicy parse(InputStream stream) throws DocumentException {
    ...
    List<String> includes =
        root.elements("include").stream()
            .map(elem -> elem.getTextTrim())
            .collect(Collectors.toList());
    List<String> excludes =
        root.elements("exclude").stream()
            .map(elem -> elem.getTextTrim())
            .collect(Collectors.toList());
    ...
    if (m_accessGroup != null) {
      try {
        List<CmsGroup> groups =
            cms.getGroupsOfUser(cms.getRequestContext().getCurrentUser().getName(), true, true);
        boolean foundGroup =
            groups.stream().anyMatch(group -> group.getName().equals(m_accessGroup));
        if (!foundGroup) {
          return false;
        }
  ...
  public boolean checkPropertyAccess(String property) {
    ...
    if (!result) {
      LOG.info(
          "Filtered property " + property + " because it does not match the JSON property filter.");
    }
    return result;
  }
