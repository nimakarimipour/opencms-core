...
/**
 * A queue used to enforce a certain wait time between requests for sessions for the same form
 * configuration.
 *
 * <p>
 */
public class CmsUgcSessionQueue {
  ...
  public static CmsUgcSessionQueue createQueue(CmsUgcConfiguration config) {
    ...
    CmsUgcSessionQueue queue =
        new CmsUgcSessionQueue(
            config.needsQueue(),
            config.getQueueInterval().isPresent() ? config.getQueueInterval().get().longValue() : 0,
            config.getMaxQueueLength().isPresent()
  ...
  /**
   * If there are currently any threads waiting on this queue, wait for the interval given on
   * construction after the currenly last thread stops waiting.
   *
   * <p>
   *
    ...
    if (!m_enabled) {
      ...
      if (timeToWait <= 0) {
        ...
        try {
          ...
          // note that timeToWait can't be 0 here, because this case is handled in the first if()
          // branch,
