...
/**
 * A helper class which processes and stores uploaded form data belonging to a single form edit
 * session.
 *
 * <p>
 */
public class CmsUgcUploadHelper {
  ...
  private ConcurrentHashMap<String, List<FileItem>> m_storedFormData =
      new ConcurrentHashMap<String, List<FileItem>>();

  /**
   * Passes the form data with the given ID to the handler object, then removes it and deletes its
   * stored data.
   *
   * <p>The form data is removed even if an exception is thrown while calling the form data handler.
   *
  ...
  /**
   * Processes a request caused by a form submit.
   *
   * <p>
   *
  ...
  /**
   * Gets the id to be used for storing form data from the request.
   *
   * <p>
   *
