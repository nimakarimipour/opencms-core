...
public class CmsUgcEditService extends CmsGwtService implements I_CmsUgcEditService {
  ...
  public void checkPermissions(CmsObject cms) {
    ...
    // don't do permission checks, since this service should be available to "Guest" users from the
    // frontend
  }
  ...
  public void error(Throwable t) throws CmsUgcException {
    ...
    logError(t);
    CmsUgcException exception =
        t instanceof CmsUgcException ? (CmsUgcException) t : new CmsUgcException(t);
    throw exception;
  }
  ...
  public CmsUgcContent getContent(CmsUUID sessionId) throws CmsUgcException {
    ...
    try {
      CmsUgcSession session =
          CmsUgcSessionFactory.getInstance().getSession(getRequest(), sessionId);
  ...
  /**
   * @see org.opencms.ugc.shared.rpc.I_CmsUgcEditService#saveContent(org.opencms.util.CmsUUID,
   *     java.util.Map)
   */
  ...
  /**
   * @see org.opencms.ugc.shared.rpc.I_CmsUgcEditService#uploadFiles(org.opencms.util.CmsUUID,
   *     java.util.Set, java.lang.String)
   */
  public Map<String, String> uploadFiles(
      final CmsUUID sessionId, final Set<String> fieldNames, final String formDataId)
                  ...
                  for (String fieldName : fieldNames) {
                    ...
                    if (item != null) {
                      CmsResource createdResource =
                          session.createUploadResource(
                              item.getFieldName(), item.getFileName(), item.getData());
                      String sitePath = session.getCmsObject().getSitePath(createdResource);
                      result.put(fieldName, sitePath);
                    } else {
                      LOG.debug(
                          formDataId
                              + ": requested upload for field "
                              + fieldName
                              + " which was empty.");
                    }
                  }
  ...
  /**
   * @see org.opencms.ugc.shared.rpc.I_CmsUgcEditService#validateContent(org.opencms.util.CmsUUID,
   *     java.util.Map)
   */
  ...
  /**
   * Gets the form session with the given id.
   *
   * <p>
   *
  ...
  /**
   * Reads the form content information.
   *
   * <p>
   *
   ...
   */
  private CmsUgcContent readContent(CmsUgcSession session, CmsResource resource)
      throws CmsException {
