...
public class CmsUgcSession implements I_CmsSessionDestroyHandler {
            ...
            new Ordering<String>() {
              ...
              public int compare(String first, String second) {
                ...
                return ComparisonChain.start()
                    .compare(
                        CmsXmlUtils.removeXpathIndex(first), CmsXmlUtils.removeXpathIndex(second))
                    // use reverse order on indexed elements to avoid delete issues
                    .compare(
                        CmsXmlUtils.getXpathIndexInt(second), CmsXmlUtils.getXpathIndexInt(first))
                    .result();
              }
            };
      } else {
            ...
            new Ordering<String>() {
              ...
              public int compare(String first, String second) {
                ...
                return ComparisonChain.start()
                    ...
                    .compare(
                        CmsXmlUtils.getXpathIndexInt(first), CmsXmlUtils.getXpathIndexInt(second))
                    .result();
              }
            };
      }
    ...
    public int compare(String o1, String o2) {
      ...
      if (o1 == null) {
        ...
        result =
            m_elementOrdering
                .lexicographical()
                .compare(Arrays.asList(o1Elements), Arrays.asList(o2Elements));
      }
      return result;
    }
  }
  ...
  /**
   * Flag which indicates whether the project and its resources should be deleted when the session
   * is destroyed.
   */
  ...
  public CmsUgcSession(CmsObject adminCms, CmsObject cms, CmsUgcConfiguration configuration)
    ...
    if ((cms.getRequestContext().getCurrentUser().isGuestUser()
            || configuration.getForceUserSubstitution())
        && m_configuration.getUserForGuests().isPresent()) {
      m_cms =
          OpenCms.initCmsObject(
              adminCms, new CmsContextInfo(m_configuration.getUserForGuests().get().getName()));
  ...
  public CmsResource createUploadResource(String fieldName, String rawFileName, byte[] content)
    ...
    // add a macro before the file extension (if there is a file extension, otherwise just append
    // it)
    ...
    String sitePath =
        CmsStringUtil.joinPaths(m_cms.getRequestContext().removeSiteRoot(uploadRootPath), baseName);
    ...
    do {
      ...
      resolver.addMacro(
          "random", RandomStringUtils.random(8, "0123456789abcdefghijklmnopqrstuvwxyz"));
  ...
  public CmsResource createXmlContent() throws CmsUgcException {
    ...
    try {
      I_CmsResourceType type =
          OpenCms.getResourceManager().getResourceType(m_configuration.getResourceType());
  ...
  public void finish() throws CmsException {
    ...
    if (m_configuration.isAutoPublish()) {
      // we don't necessarily publish with the user who has the locks on the resources, so we need
      // to steal the locks
      List<CmsResource> projectResources =
          projectCms.readProjectView(project.getUuid(), CmsResource.STATE_KEEP);
      for (CmsResource projectResource : projectResources) {
        CmsLock lock = projectCms.getLock(projectResource);
        if (!lock.isUnlocked()
            && !lock.isLockableBy(projectCms.getRequestContext().getCurrentUser())) {
          projectCms.changeLock(projectResource);
        }
      }
      OpenCms.getPublishManager()
          .publishProject(projectCms, new CmsLogReport(Locale.ENGLISH, CmsUgcSession.class));
    } else {
      // try to unlock everything - we don't need this in case of auto-publish, since publishing
      // already unlocks the resources
  ...
  /**
   * Returns the locale to use for messages generated by the form session which are intended to be
  ...
  public CmsResource loadXmlContent(String fileName) throws CmsUgcException {
    ...
    if (fileName.contains("/")) {
      String message =
          Messages.get()
              .container(Messages.ERR_INVALID_FILE_NAME_TO_LOAD_1, fileName)
              .key(getCmsObject().getRequestContext().getLocale());
      throw new CmsUgcException(CmsUgcConstants.ErrorCode.errMisc, message);
    }
    try {
      String contentSitePath =
          m_cms
              .getRequestContext()
              .removeSiteRoot(m_configuration.getContentParentFolder().getRootPath());
    ...
    if (!hasValue) {
      ...
      for (int i = 0; i < pathElements.length; i++) {
        ...
        while (!content.hasValue(currentPath, locale)) {
          content.addValue(
              m_cms, currentPath, locale, CmsXmlUtils.getXpathIndexInt(currentPath) - 1);
        }
      }
    }
   ...
   */
  protected CmsXmlContent addContentValues(CmsFile file, Map<String, String> contentValues)
      throws CmsException {
   ...
   */
  protected void addContentValues(
      CmsXmlContent content, Locale locale, Map<String, String> contentValues)
  ...
  /**
   * Throws an error if the edit resource is already set.
   *
   * <p>
   *
  ...
  private void checkEditResourceNotSet() throws CmsUgcException {
    ...
    if (m_editResource != null) {
      String message =
          Messages.get()
              .container(Messages.ERR_CANT_EDIT_MULTIPLE_CONTENTS_IN_SESSION_0)
              .key(getCmsObject().getRequestContext().getLocale());
  ...
  private void checkNotFinished() throws CmsUgcException {
    ...
    if (m_finished) {
      String message =
          Messages.get()
              .container(Messages.ERR_FORM_SESSION_ALREADY_FINISHED_0)
              .key(getCmsObject().getRequestContext().getLocale());
      throw new CmsUgcException(CmsUgcConstants.ErrorCode.errInvalidAction, message);
    }
  }
  ...
  /**
   * Cleans up the project, but only if it's empty.
   *
   * <p>
   */
  ...
  /**
   * Deletes the given resource which is part of a form session project.
   *
   * <p>
   *
  ...
  /**
   * Returns the new resource site path.
   *
   * <p>
   *
  ...
  /**
   * Checks if all the resource states from a list of resources are 'new'.
   *
   * <p>
   *
  ...
  /**
   * Stores the upload resource and deletes previously uploaded resources for the same form field.
   *
   * <p>
   *
  ...
  private void updateUploadResource(String fieldName, CmsResource upload) {
    ...
    if (prevUploadResource != null) {
      try {
        m_cms.deleteResource(
            m_cms.getSitePath(prevUploadResource), CmsResource.DELETE_PRESERVE_SIBLINGS);
      } catch (Exception e) {
