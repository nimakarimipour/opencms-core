...
public class CmsRemoteShell extends UnicastRemoteObject implements I_CmsRemoteShell {
  ...
  /**
   * Stores remote shell instances which haven't been unregistered yet.
   *
   * <p>
   */
  static class InstanceStore {
    ...
    /**
     * Removes and unexports an instance.
     *
     * <p>
     *
  ...
  /**
   * Byte array stream used to capture output of shell commands; will be cleared for each individual
   * command.
   */
  ...
  public CmsRemoteShell(String additionalCommandsName, int port)
    ...
    if (additionalCommandsName != null) {
      try {
        ...
        throw new IllegalArgumentException(
            "Could not create command class instance for " + additionalCommandsName, e);
      }
    }
    ...
    m_shell =
        new CmsShell(
            cms, "${user}@${project}:${siteroot}|${uri}>", additionalCommands, m_out, m_out);
