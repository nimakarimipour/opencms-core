...
public class CmsRemoteShellClient {
  ...
  public Map<String, String> parseArgs(String[] args) {
    ...
    Set<String> allowedKeys =
        new HashSet<String>(
            Arrays.asList(
                PARAM_ADDITIONAL, PARAM_SCRIPT, PARAM_REGISTRY_PORT, PARAM_REGISTRY_HOST));
    for (String arg : args) {
      if (arg.startsWith("-")) {
   ...
   *
   * <p>Reads commands from either stdin or a file, executes them remotely and displays the results.
   *
   * @throws Exception if something goes wrong
   */
  public void run() throws Exception {
    ...
    I_CmsRemoteShellProvider provider =
        (I_CmsRemoteShellProvider) (registry.lookup(CmsRemoteShellConstants.PROVIDER));
  ...
  /**
   * Executes a command remotely, displays the command output and updates the internal state.
   *
   * <p>
   *
  ...
  /**
   * Exits the shell with an error code, and if possible, notifies the remote shell that it is
   * exiting.
   *
   * <p>
   *
  ...
  /**
   * Returns true if the exit command has been called.
   *
   * <p>
   *
  ...
  /**
   * Returns true if echo mode is enabled.
   *
   * <p>
   *
  ...
  /**
   * Updates the internal client state based on the state received from the server.
   *
   * <p>
   *
