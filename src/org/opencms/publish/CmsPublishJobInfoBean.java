...
public final class CmsPublishJobInfoBean {
  ...
  /**
   * The report to use during the publish process, will be set to <code>null</code> after
   * publishing.
   */
   ...
   *
   * <p>It will be <code>null</code> before starting and after finishing.
   *
   * <p>
   *
   ...
   *
   * <p>Actually sets the enqueue time only if it is not set already (re-enqueue during startup).
   *
   * <p>
   */
   ...
   *
   * <p>Actually only sets the finish time and closes the publish report stream.
   *
   * <p>
   */
  protected void finish() {
    ...
    if (m_finishTime != 0) {
      throw new CmsRuntimeException(
          Messages.get().container(Messages.ERR_PUBLISH_JOB_ALREADY_FINISHED_0));
    }
    m_cms = null;
    m_report = null;
    m_size = m_publishList.size();
  ...
  /**
   * Returns the cms object, will be set to <code>null</code> after publishing.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this is a "direct publish" operation.
   *
   * <p>
   *
   ...
   *
   * <p>Actually sets the starting time, writes the report header and sets the running thread uuid.
   *
   * <p>
   *
  ...
  protected void start(CmsUUID threadUUID) {
    ...
    if (m_startTime != 0) {
      throw new CmsRuntimeException(
          Messages.get().container(Messages.ERR_PUBLISH_JOB_ALREADY_STARTED_0));
    }
