...
/* default */ final class CmsPublishThread extends A_CmsReportThread {
  ...
  protected CmsPublishThread(CmsPublishEngine publishEngine, CmsPublishJobInfoBean publishJob) {
    ...
    super(
        publishJob.getCmsObject(),
        Messages.get().getBundle().key(Messages.GUI_PUBLISH_TRHEAD_NAME_0));
    m_publishJob = publishJob;
    m_publishEngine = publishEngine;

    // if the project to publish is a temporary project
    if (getCms().getRequestContext().getCurrentProject().getType()
  ...
  /**
   * Checks if the current publish job has been aborted.
   *
   * <p>
   *
  ...
  public void run() {
    ...
    try {
      ...
      try {
        ...
        m_publishEngine
            .getDriverManager()
            .publishJob(getCms(), dbc, m_publishJob.getPublishList(), m_report);
      } catch (Throwable e) {
   ...
   *
   * <p>This can only be done until the publish job started event is fired.
   *
   * <p>
   *
  ...
  /**
   * Returns the publish job for this thread.
   *
   * <p>
   *
  ...
  /**
   * Returns the set of all strong link references from the files to publish.
   *
   * <p>
   *
  ...
  protected Set<String> getStrongLinkReferences() {
    ...
    try {
      ...
      jspCms
          .getRequestContext()
          .setCurrentProject(jspCms.readProject(CmsProject.ONLINE_PROJECT_ID));
