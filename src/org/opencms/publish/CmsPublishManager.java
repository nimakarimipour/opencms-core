...
public class CmsPublishManager {
  ...
  private static CopyOnWriteArrayList<Supplier<List<CmsUUID>>> m_specialHistoryIdProviders =
  ...
  public void abortPublishJob(CmsObject cms, CmsPublishJobEnqueued publishJob, boolean removeJob)
    ...
    if (!OpenCms.getRoleManager().hasRole(cms, CmsRole.PROJECT_MANAGER)
      ...
      // Can only be executed by somebody with the role CmsRole#PROJECT_MANAGER or the owner of the
      // job
    ...
    m_publishEngine.abortPublishJob(
        cms.getRequestContext().getCurrentUser().getId(), publishJob, removeJob);
  }
  ...
  /**
   * Check if the thread for the current publish job is still active or was interrupted and so the
   * next job in the queue can be started.
   *
   * <p>
   */
  ...
  /**
   * Cleans up all unreferenced publish history entries except for the ones with a history id
   * provided by the special history id providers.
   *
   * @param cms the CMS context
  ...
  public int cleanupPublishHistory(CmsObject cms) throws CmsException {
    ...
    return m_securityManager.cleanupPublishHistory(
        cms.getRequestContext(), CmsPublishHistoryCleanupFilter.allUnreferencedExcept(exceptions));
  }
  ...
  public int cleanupPublishHistory(CmsObject cms, CmsUUID historyId) throws CmsException {
    ...
    return m_securityManager.cleanupPublishHistory(
        cms.getRequestContext(), CmsPublishHistoryCleanupFilter.forHistoryId(historyId));
  }

  /**
   * Cleans up all unreferenced publish history entries except for the ones with a history id from
   * the given list of exceptions.
   *
   * @param cms the CMS context
   ...
   *
   * <p>The returned publish job may be an enqueued, running or finished publish job.
   *
   * <p>
   *
  ...
  /**
   * Returns a publish list with all new/changed/deleted resources of the current (offline) project
  ...
  public CmsPublishList getPublishList(CmsObject cms) throws CmsException {
    ...
    return m_securityManager.fillPublishList(
        cms.getRequestContext(), new CmsPublishList(cms.getRequestContext().getCurrentProject()));
  }

  /**
   * Returns a publish list with all new/changed/deleted resources of the current (offline) project
   * that actually get published for a direct publish of a single resource.
   *
   * <p>
   *
   * @param cms the cms request context
  ...
  public CmsPublishList getPublishList(
      CmsObject cms, CmsResource directPublishResource, boolean directPublishSiblings)
      throws CmsException {

    return m_securityManager.fillPublishList(
        cms.getRequestContext(), new CmsPublishList(directPublishResource, directPublishSiblings));
  }

  /**
   * Returns a publish list with all new/changed/deleted resources of the current (offline) project
   * that actually get published for a direct publish of a List of resources.
   *
   * <p>
   *
   * @param cms the cms request context
   ...
   */
  public CmsPublishList getPublishList(
      CmsObject cms, List<CmsResource> directPublishResources, boolean directPublishSiblings)
      throws CmsException {
   ...
   * @param directPublishSiblings <code>true</code>, if all eventual siblings of the direct
   *     published resources should also get published.
   * @param publishSubResources indicates if sub-resources in folders should be published (for
   *     direct publish only)
   * @return a publish list
   * @throws CmsException if something goes wrong
   */
   ...
   * @param directPublishSiblings <code>true</code>, if all eventual siblings of the direct
   *     published resources should also get published
   * @param isUserPublishList if true, the publish list consists of resources directly selected by
   *     the user to publish
   * @return a publish list
   * @throws CmsException if something goes wrong
   */
  public CmsPublishList getPublishListAll(
    ...
    CmsPublishList pubList =
        new CmsPublishList(true, directPublishResources, directPublishSiblings);
  ...
  /**
   * Returns the amount of time in seconds the system will wait during shutdown for a running
   * publish job.
   *
   * <p>
   *
  ...
  public CmsPublishList getRelatedResourcesToPublish(CmsObject cms, CmsPublishList publishList)
    ...
    return m_securityManager.getRelatedResourcesToPublish(
        cms.getRequestContext(), publishList, CmsRelationFilter.TARGETS.filterStrong());
  }
  ...
  /**
   * Returns true if publish history entries should be automatically removed when the corresponding
   * publish job is removed.
   *
  ...
  /**
   * Returns the working state, that is if no publish job is waiting to be processed and there is no
   * current running publish job.
   *
   * <p>
   *
  ...
  public CmsPublishList mergePublishLists(
      CmsObject cms, CmsPublishList pubList1, CmsPublishList pubList2) throws CmsException {
   ...
   */
  public CmsUUID publishProject(CmsObject cms, I_CmsReport report, CmsPublishList publishList)
      throws CmsException {
   ...
   * @param directPublishSiblings if a <code>{@link CmsResource}</code> that should get published
   *     directly is provided as an argument, all eventual siblings of this resource get publish
   *     too, if this flag is <code>true</code>.
   * @return the publish history id of the published project
   * @throws CmsException if something goes wrong
   */
  public CmsUUID publishProject(
    ...
    return publishProject(
        cms, report, getPublishList(cms, directPublishResource, directPublishSiblings));
  }

  /**
   * Publishes a single resource, printing messages to a shell report.
   *
   * <p>The siblings of the resource will not be published.
   *
   * <p>
   *
   * @param cms the cms request context
  ...
  public CmsUUID publishResource(
      CmsObject cms, String resourcename, boolean publishSiblings, I_CmsReport report)
      throws Exception {
   ...
   * @throws CmsException if something goes wrong
   */
  public void removeResourceFromUsersPubList(CmsObject cms, Collection<CmsUUID> structureIds)
      throws CmsException {
   ...
   */
  public void setPublishListRemoveMode(
      CmsPublishManager.PublishListRemoveMode publishListRemoveMode) {
   ...
   *     <p>TODO: change return value to List of CmsRelation
   * @throws Exception if something goes wrong
   */
  public Map<String, List<CmsRelation>> validateRelations(
      CmsObject cms, CmsPublishList publishList, I_CmsReport report) throws Exception {
  ...
  public void waitWhileRunning(long ms) {
    ...
    synchronized (this) {
      try {
        Thread.sleep(
            100); // wait a bit to give the publish engine the chance to actualize the state
