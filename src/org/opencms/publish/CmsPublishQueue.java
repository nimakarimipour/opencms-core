...
public class CmsPublishQueue {
  ...
  public static Buffer getQueue() {
    ...
    return BufferUtils.synchronizedBuffer(
        TypedBuffer.decorate(
            new UnboundedFifoBuffer() {
              ...
              /**
               * Called when the queue is full to remove the oldest element.
               *
               * <p>
               *
  ...
  protected boolean abortPublishJob(CmsPublishJobInfoBean publishJob) {
    ...
    if (OpenCms.getMemoryMonitor().getCachedPublishJob(publishJob.getPublishHistoryId().toString())
        != null) {
  ...
  protected List<CmsPublishJobEnqueued> asList() {
    ...
    List<CmsPublishJobInfoBean> cachedPublishJobs =
        OpenCms.getMemoryMonitor().getAllCachedPublishJobs();
    List<CmsPublishJobEnqueued> result =
        new ArrayList<CmsPublishJobEnqueued>(cachedPublishJobs.size());
              ...
              try {
                ...
                if (LOG.isErrorEnabled()) {
                  LOG.error(
                      Messages.get()
                          .getBundle()
                          .key(Messages.ERR_PUBLISH_JOB_INVALID_1, job.getPublishHistoryId()),
                      exc);
                }
  ...
  /**
   * Returns the next publish job to be published, removing it from the queue, or <code>null</code>
