...
public final class CmsPublishEngine {
   ...
   *
   * <p>All resources should already be locked.
   *
   * <p>If possible, the publish job starts immediately.
   *
   * <p>
   *
   ...
   */
  public void enqueuePublishJob(CmsObject cms, CmsPublishList publishList, I_CmsReport report)
      throws CmsException {
    ...
    if ((m_driverManager == null) || (m_dbContextFactory == null)) {
      ...
      throw new CmsPublishException(
          Messages.get().container(Messages.ERR_PUBLISH_ENGINE_NOT_INITIALIZED_0));
    }
    // prevent new jobs if the engine is disabled
    if (m_shuttingDown
        || (!isEnabled() && !OpenCms.getRoleManager().hasRole(cms, CmsRole.ROOT_ADMIN))) {
      // the resources are unlocked in the driver manager
      throw new CmsPublishException(
          Messages.get().container(Messages.ERR_PUBLISH_ENGINE_DISABLED_0));
    }
    ...
    try {
      ...
      throw new CmsException(
          Messages.get()
              .container(Messages.ERR_PUBLISH_ENGINE_QUEUE_1, publishJob.getPublishHistoryId()),
          t);
    }
  ...
  public void setDriverManager(CmsDriverManager driverManager) {
    ...
    try {
      m_adminUserId =
          m_driverManager.readUser(dbc, OpenCms.getDefaultUsers().getUserAdmin()).getId();
   ...
   *
   * <p>NOTE: this method may or may NOT be called (i.e. kill -9 in the stop script), if a system is
   * stopped.
   *
  ...
  public synchronized void shutDown() {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          org.opencms.main.Messages.get()
              .getBundle()
              .key(org.opencms.main.Messages.INIT_SHUTDOWN_START_1, this.getClass().getName()));
    }
   ...
   */
  protected void abortPublishJob(
      CmsUUID userId, CmsPublishJobEnqueued publishJob, boolean removeJob)
  ...
  protected void initialize(
      CmsObject adminCms, boolean publishQueuePersistance, int publishQueueShutdowntime)
      throws CmsException {
  ...
  protected void publishJobFinished(CmsPublishJobInfoBean publishJob) {
    ...
    try {
      ...
      CmsEvent afterPublishEvent =
          new CmsEvent(I_CmsEventListener.EVENT_PUBLISH_PROJECT, eventData);
  ...
  /**
   * Sends a message to the given user, if publish notification is enabled or an error is shown in
   * the message.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the login manager allows login.
   *
   * <p>
   *
   * @return if enabled
