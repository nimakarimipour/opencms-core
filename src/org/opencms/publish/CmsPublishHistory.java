...
public class CmsPublishHistory {
  ...
  public static Buffer getQueue(int size) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_PUBLISH_HISTORY_SIZE_SET_1, new Integer(size)));
    }

    return BufferUtils.synchronizedBuffer(
        TypedBuffer.decorate(
            new CircularFifoBuffer(size) {
  ...
  protected List<CmsPublishJobFinished> asList() {
    ...
    List<CmsPublishJobInfoBean> cachedPublishJobs =
        OpenCms.getMemoryMonitor().getAllCachedPublishJobsInHistory();
    List<CmsPublishJobFinished> result =
        new ArrayList<CmsPublishJobFinished>(cachedPublishJobs.size());
  ...
  protected void initialize() {
    ...
    try {
      ...
      List<CmsPublishJobInfoBean> publishJobs =
          driverManager.readPublishJobs(dbc, 1L, Long.MAX_VALUE);
    ...
    if (OpenCms.getMemoryMonitor().requiresPersistency()) {
      ...
      try {
        OpenCms.getPublishManager()
            .getEngine()
            .getDriverManager()
            .deletePublishJob(dbc, publishJob.getPublishHistoryId());
