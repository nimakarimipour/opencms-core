...
public class CmsMailHost implements Comparable<CmsMailHost> {
  ...
  /**
   * Returns <code>true</code> only if authentication is enabled, the default is <code>false</code>.
   *
   * <p>Authentication is enabled only if both "username" and "password" are not <code>null</code>.
   *
   * <p>
   *
