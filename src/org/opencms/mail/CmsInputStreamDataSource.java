 ...
 *
 * <p>It's intended use is for creation of mail attachments from strings without having to create
 * RFS or VFS resources. Note that this data source will only support read operations and operations
...
public class CmsInputStreamDataSource implements DataSource {
   ...
   *
   * <p>Note that the given input stream has to be resettable. During a mail creation and
   ...
   * So a reset will be made here whenever the internal input stream is retrieved to avoid that the
   * attachments remain empty.
   *
   * <p>The contentType argument should always be a valid MIME type. It is suggested that it is
   ...
   * about the ecoding. Note that some textual documents like xml have their own encoding directive
   * contained and the charset given here (for the mail part header) should not be different from
   * the contained one.
   *
   ...
   * @param name the name that describes the data in the underyling input stream. E.g. the name of a
   *     file that the input stream reads from.
   */
   ...
   * @throws IOException if the constructor-given input stream is not "resettable" ({@link
   *     InputStream#reset()}).
