 ...
 *
 * <p>Create an instance, add the {@link org.htmlparser.Tag} instances to remove and assign this
 * factory to the {@link org.htmlparser.Parser} before starting a visit. A demo usage is shown in
 * <code>CmsTagReplaceParser</code>.
 *
 * <p>The tags are not actually removed: They are linked in the document object model tree of the
 * HTML that the parser generates. They just will not accept any {@link NodeVisitor} instances and
 * therefore be invisible in any output a visitor will generate from the visited tree.
 *
 * <p>The decision whether a tag is removed can be controlled in two ways:
 *
 * <ol>
 *   <li><code>{@link #addTagRemoval(Tag)}</code><br>
 *       <p>The given tag will be removed ("invisible in the DOM").
 *   <li><code>{@link #addTagPreserve(Tag)}</code><br>
 *       <p>The given tag will be kept as-is. The following behaviour happens if this method is
 *       used:
 *       <ol>
 *         <li>Once <code>{@link #addTagPreserve(Tag)}</code> has been called all Tags that are not
 *             added to this method will be removed. <strong>We are in include mode then</strong>.
 *         <li>The Tags provided to <code>{@link #addTagRemoval(Tag)}</code> will only have the
 *             power to hide exactly the same tags that are given to <code>
 *             {@link #addTagPreserve(Tag)}</code>: <strong>Deny is stronger than allow.</strong>
 *       </ol>
 * </ol>
 *
 * @since 6.1.8
...
public final class CmsHtmlTagRemoveFactory extends PrototypicalNodeFactory {
   ...
   *
   * <p>When visiting the corresponding tree of tags, this tag will be there but the visitor will
   * not see it as it is not accepted. This allows "elimination" of this tag in the output the
   * visitor generates from the document object model (e.g. HTML code again).
   *
   * <p>Potential child tags will be visible to visitors (unless they are instances of this class).
   *
   * <p>
   *
   * @since 6.1.8
  ...
  private static final class CmsInvisibleTag implements Tag {
     ...
     *
     * <p>Every property is accessed transparently from the delegate, except that visitors are not
     * welcome.
     *
     * <p>
     *
    ...
    /**
     * @see org.htmlparser.Node#collectInto(org.htmlparser.util.NodeList, org.htmlparser.NodeFilter)
     */
   ...
   *
   * <p>Not only "this" tag will be visible but all parsed Tags that have the same name (case
   * insensitive).
   *
   * <p>The given tag will be kept as-is. The following behaviour happens if this method is used:
   *
   * <ol>
   *   <li>Once <code>{@link #addTagPreserve(Tag)}</code> has been called all Tags that are not
   *       added to this method will be removed. <strong>We are in include mode then</strong>.
   *   <li>The Tags provided to <code>{@link #addTagRemoval(Tag)}</code> will only have the power to
   *       hide exactly the same tags that are given to <code>{@link #addTagPreserve(Tag)}</code>:
   ...
   *
   * <p>Not only "this" tag will be invisible but all parsed Tags that have the same name (case
   * insensitive).
   *
   * <p>
   *
  ...
  /**
   * @see org.htmlparser.PrototypicalNodeFactory#createTagNode(org.htmlparser.lexer.Page, int, int,
   *     java.util.Vector)
   */
  @Override
  ...
  /**
   * Encapsulation of the "preserve / remove" logic.
   *
   * <p>
   *
