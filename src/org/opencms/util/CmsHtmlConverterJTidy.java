 ...
 *
 * <p>Used to clean up HTML code (e.g. remove word tags) and optionally create XHTML from HTML.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsHtmlConverterJTidy extends A_CmsHtmlConverter {
  ...
  private static final List<String> MODES_DEFAULT =
      Collections.unmodifiableList(Arrays.asList(new String[] {CmsHtmlConverter.PARAM_ENABLED}));
  ...
  /**
   * Regular expressions for paragraph replacements -- additionally remove leading and trailing
   * breaks.
   */
   ...
   * Constructor, creates a new CmsHtmlConverterJTidy.
   *
   * <p>Possible values for the conversion mode are:
   *
   * <ul>
   *   <li>{@link CmsHtmlConverter#PARAM_DISABLED}: The conversion is disabled.
   *   <li>{@link CmsHtmlConverter#PARAM_ENABLED}: Conversion is enabled without transformation, so
   *       HTML is pretty printed only.
   ...
   *   <li>{@link CmsHtmlConverter#PARAM_REPLACE_PARAGRAPHS}: Cleanup of paragraphs and
   *       leading/trailing line breaks is enabled.
   * </ul>
   *
    ...
    if (m_modeEnabled) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_PARSING_RUNS_2, this.getClass().getName(), new Integer(count)));
      }
      htmlInput = workHtml;
    }
  ...
  /**
   * Adjusts the HTML input code in WORD mode if necessary.
   *
   * <p>When in WORD mode, the HTML tag must contain the
   * xmlns:o="urn:schemas-microsoft-com:office:office" attribute, otherwise tide will not remove the
   * WORD tags from the document.
   *
  ...
  private void initModes() {
    ...
    if (m_modeEnabled) {
      ...
      // disable URI fixing, because it breaks domain names with special characters (IDNs) in links
      // when used in HTML fields
      ...
      if (m_modeReplaceParagraphs) {
        ...
        String[] newPatterns =
            new String[m_replacePatterns.length + m_replaceParagraphPatterns.length];
        String[] newValues =
            new String[m_replacePatterns.length + m_replaceParagraphPatterns.length];
  ...
  /**
   * Parses a byte array containing HTML code with different parsing modes.
   *
   * <p>
   *
  ...
  /**
   * Parses the htmlInput with regular expressions for cleanup purposes.
   *
   * <p>
   *
   * @param htmlInput the HTML input
