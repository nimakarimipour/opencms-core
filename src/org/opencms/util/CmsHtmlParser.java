 ...
 *
 * <p>This base implementation is only a "pass through" class, that is the content is parsed, but
 * the generated result is exactly identical to the input.
 *
 * <p>
 *
 * @since 6.2.0
...
public class CmsHtmlParser extends NodeVisitor implements I_CmsHtmlNodeVisitor {
  ...
  protected static final String[] TAG_ARRAY =
      new String[] {
        "H1", "H2", "H3", "H4", "H5", "H6", "P", "DIV", "SPAN", "BR", "OL", "UL", "LI", "TABLE",
   ...
   * This allows to avoid auto correction of unclosed HTML tags.
   *
   * <p>
   *
   * @return A node factory that will not autocorrect open tags specified via <code>
   *     {@link #setNoAutoCloseTags(List)}</code>
   */
  protected PrototypicalNodeFactory configureNoAutoCorrectionTags() {
    ...
    while (it.hasNext()) {
      ...
      // TODO: This might break in case registering / unregistering  will change from name based to
  ...
  public String process(String html, String encoding) throws ParserException {
    ...
    if ((m_noAutoCloseTags != null) && (m_noAutoCloseTags.size() > 0)) {
      ...
      // to simple single tags: This allows to finish this tag with opened HTML tags without the
      // effect
  ...
  /**
   * Returns a list of upper case tag names for which parsing / visiting will not correct missing
   * closing tags.
   *
   * <p>
   *
   * @return a List of upper case tag names for which parsing / visiting will not correct missing
   *     closing tags
   */
  ...
  /**
   * Sets a list of upper case tag names for which parsing / visiting should not correct missing
   * closing tags.
   *
   * <p>
   *
   * @param noAutoCloseTagList a list of upper case tag names for which parsing / visiting should
   *     not correct missing closing tags to set.
   */
