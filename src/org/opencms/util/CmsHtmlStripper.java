 ...
 *
 * <p>All tags that are not explicitly allowed via invocation of one of the <code>addPreserve...
 * </code> methods will be missing in the result of the method <code>{@link #stripHtml(String)}
 * </code>.
 *
 * <p>Instances are reusable but not shareable (multithreading). If configuration should be changed
 * between subsequent invocations of <code>{@link #stripHtml(String)}</code> method <code>
 * {@link #reset()}</code> has to be called.
 *
 * <p>
 *
 * @since 6.9.2
...
public final class CmsHtmlStripper {
  ...
  /**
   * Adds a tag that will be preserved by <code>{@link #stripHtml(String)}</code>.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param preserveTags a <code>List&lt;String&gt;</code> with the case-insensitive tag names of
   ...
   *
   * <p>The String will be <code>{@link CmsStringUtil#splitAsList(String, char, boolean)}</code>
   * with <code>tagList</code> as the first argument, <code>separator</code> as the second argument
   * and the third argument set to true (trimming - support).
   *
   * <p>
   *
   ...
   *
   * <p>This is called from the constructor and only has to be called if this instance is reused
   * with a differen configuration (of tags to keep).
   *
   * <p>
   */
  public void reset() {
   ...
   *
   * <p>Additionally tags are replaced / removed according to the configuration of this instance.
   *
   * <p>
   *
   * <h3>Please note:</h3>
  ...
  private String tidy(final String content) {
    ...
    CmsHtmlConverter converter =
        new CmsHtmlConverter(
            CmsEncoder.ENCODING_UTF_8,
            new StringBuffer(CmsHtmlConverter.PARAM_WORD)
                .append(";")
                .append(CmsHtmlConverter.PARAM_XHTML)
                .toString());
