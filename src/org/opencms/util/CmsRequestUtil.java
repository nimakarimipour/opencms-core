...
public final class CmsRequestUtil {
   ...
   *
   * <p>This method takes care about the adding the parameter as an additional parameter (appending
   * <code>&param=value</code>) or as the first parameter (appending <code>?param=value</code>).
   *
   * <p>
   *
   ...
   *
   * <p>The map can contains values of <code>String[]</code> or simple <code>String</code> values.
   *
   * <p>This method takes care about the adding the parameter as an additional parameter (appending
   * <code>&param=value</code>) or as the first parameter (appending <code>?param=value</code>).
   *
  ...
  public static String appendParameters(String url, Map<String, String[]> params, boolean encode) {
    ...
    while (i.hasNext()) {
      ...
      String[] values =
          value instanceof String[] ? (String[]) value : new String[] {value.toString()};
   ...
   *
   * <p>This means that the link is either just a path, or starts with a scheme/domain/port that is
   * either registered in the site configuration, or is the same as that of the current request.
   *
   * @param backlink the link to check
   * @param optionalRequest the current request - may be null if no check against the current
   *     request is desired
  ...
  /**
   * Creates a valid request parameter map from the given map, most notably changing the values form
   * <code>String</code> to <code>String[]</code> if required.
   *
   * <p>If the given parameter map is <code>null</code>, then <code>null</code> is returned.
   *
   * <p>
   *
   ...
   *
   * <p>Please note: This does not parse a full request URI/URL, only the query part that starts
   * after the "?". For example, in the URI <code>/system/index.html?a=b&amp;c=d</code>, the query
   * part is <code>a=b&amp;c=d</code>.
   *
   * <p>If the given String is empty, an empty map is returned.
   *
   * <p>
   *
  ...
  /**
   * Parses the parameters of the given request query part, optionally decodes them, and creates a
   * parameter map out of them.
   *
   * <p>Please note: This does not parse a full request URI/URL, only the query part that starts
   * after the "?". For example, in the URI <code>/system/index.html?a=b&amp;c=d</code>, the query
   * part is <code>a=b&amp;c=d</code>.
   ...
   * @param encoding the character encoding used while decoding. If <code>null</code>, the default
  ...
  /**
   * Returns all parameters of the given request as a request parameter URL String, that is in the
   * form <code>key1=value1&key2=value2</code> etc.
   *
   * <p>The result will be encoded using the <code>{@link CmsEncoder#encode(String)}</code>
   * function.
   *
   * <p>
   *
  ...
  /**
   * Forwards the response to the given target, which may contain parameters appended like for
   * example <code>?a=b&amp;c=d</code>.
   *
   * <p>Please note: If possible, use <code>
   * {@link #forwardRequest(String, Map, HttpServletRequest, HttpServletResponse)}</code> where the
   * parameters are passed as a map, since the parsing of the parameters may introduce issues with
   * encoding and is in general much less effective.
   *
   * <p>The parsing of parameters will likely fail for "large values" (e.g. full blown web forms
   * with &lt;textarea&gt; elements etc. Use this method only if you know that the target will just
   * contain up to 3 parameters which are relatively short and have no encoding or line break
   * issues.
   *
   * <p>
   *
   ...
   *
   * <p>The target URI must NOT have parameters appended like for example <code>?a=b&amp;c=d</code>.
   * The values in the provided map must be of type <code>String[]</code>. If required, use <code>
   * {@link #createParameterMap(Map)}</code> before calling this method to make sure all values are
   * actually of the required array type.
   *
   * <p>
   *
   * @param target the target to forward to (may NOT contain parameters like <code>?a=b&amp;c=d
   *     </code>)
  ...
  public static void forwardRequest(
    ...
    if (target.startsWith(vfsPrefix)) {
      // remove VFS prefix (will also work for empty vfs prefix in ROOT webapp case with proxy
      // rules)
   ...
   *
   * <p>Kept for backward compatibility.
   *
  ...
  public static Map<String, Object> getAttributeMap(ServletRequest req) {
    ...
    Enumeration<String> atrrEnum =
        CmsCollectionsGenericWrapper.enumeration(req.getAttributeNames());
  ...
  /**
   * Reads value from the request parameters, will return <code>null</code> if the value is not
   * available or only white space.
   *
   * <p>The value of the request will also be decoded using <code>{@link CmsEncoder#decode(String)}
   * </code> and also trimmed using <code>{@link String#trim()}</code>.
   *
   * <p>
   *
   ...
   *
   * <p>A session will be initialized if the request does not currently have a session. As a result,
   * the request will always have a session after this method has been called.
   *
   * <p>Will return <code>null</code> if no corresponding object is found in the session.
   *
   * <p>
   *
   ...
   *
   * <p>The result list will contain items of type <code>{@link FileItem}</code>. If the request is
   * not of type <code>multipart/form-data</code>, then <code>null</code> is returned.
   *
   * <p>
   *
   * @param request the HTTP servlet request to parse
   * @return the list of <code>{@link FileItem}</code> extracted from the multipart request, or
   *     <code>null</code> if the request was not of type <code>multipart/form-data</code>
   */
   ...
   *     <code>null</code> if the request was not of type <code>multipart/form-data</code>
   */
  public static List<FileItem> readMultipartFileItems(
      HttpServletRequest request, String tempFolderPath) {
  ...
  /**
   * Creates a "standard" request parameter map from the values of a <code>multipart/form-data
   * </code> request.
   *
   * <p>
   *
   ...
   *
   * <p>This implementation will work only on JSP pages in OpenCms that use the default JSP loader
   * implementation.
   *
   * <p>
   *
   ...
   *
   * <p>Use this method instead of {@link
   * javax.servlet.http.HttpServletResponse#sendRedirect(java.lang.String)} to avoid relative links
   * with secure sites (and issues with apache).
   *
   * <p>
   *
   * @param jsp the OpenCms JSP context
   ...
   */
  public static void redirectRequestSecure(CmsJspActionElement jsp, String target)
      throws IOException {

    jsp.getResponse()
        .sendRedirect(
            OpenCms.getLinkManager().substituteLink(jsp.getCmsObject(), target, null, true));
  }
   ...
   *
   * <p>If no cookie exists with the value, a new cookie will be created.
   *
   * @param jsp the CmsJspActionElement to use
   * @param name the name of the cookie
