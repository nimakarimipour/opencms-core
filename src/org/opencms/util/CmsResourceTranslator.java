 ...
 *
 * <p>This facility is used for translating new file names that contain illegal chars to legal
 * names. This feature is most useful (and currently only used) for uploaded files. It is also
 * applied to uploded ZIP directories that are extracted after upload. The rules that are used for
 * resource name translation are available from {@link
 * org.opencms.file.CmsRequestContext#getFileTranslator()}.
 *
 * <p>Optionally, resource name translation is also applied to all files read from the VFS, so it
 * can be used for accessing files out of teir usual context. This feature is called directoy
 * translation, and the configured directory translations are available from {@link
 * org.opencms.file.CmsRequestContext#getDirectoryTranslator()}.
 *
 * <p>Directory translation was originally required for backward compatibility to the directory
 * layout before OpenCms 5.0 beta 2. In a modern installation, directory translation is usually
 * disabled.
 *
 * <p>The translations can be configured in <code>opencms-vfs.xml</code> in the <code>
 * opencms\vfs\resources\translations</code> node.
 *
 * <p>The default file name translation setting is:<br>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Directory translation is now usually not required and since disabled by default. The directory
 * translation setting to convert an OpenCms 5.0 to 6.0 VFS is:<br>
 *
 * <pre>
...
public class CmsResourceTranslator {
  ...
  /**
   * Keep an array of Perl5Util to randomly select for use, because using just a single one causes
   * contention problems under load (since they're synchronized).
   */
   ...
   *
   * @param translations The array of translations read from the opencms,properties
   * @param continueMatching if <code>true</code>, matching will continue after the first match was
   *     found
   */
  ...
  /**
   * Translate a resource name according to the expressions set in <code>opencms-vfs.xml</code>. If
   * no match is found, the resource name is returned unchanged.
   *
   * <p>
   *
  ...
  public String translateResource(String resourceName) {
    ...
    for (int i = 0; i < m_translations.length; i++) {
      ...
      try {
          ...
          if (m_continueMatching) {
            ...
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  Messages.get()
                      .getBundle()
                      .key(Messages.LOG_TRANSLATION_MATCH_3, new Integer(i), resourceName, result));
            }
        ...
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_MALFORMED_TRANSLATION_RULE_1, m_translations[i]),
            e);
      }
    }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get().getBundle().key(Messages.LOG_TRANSLATION_MATCH_2, resourceName, current));
    }
