...
public class CmsHtmlConverter {
   ...
   *
   * <p>The encoding used by default is {@link CmsEncoder#ENCODING_UTF_8}.
   *
   * <p>
   */
   ...
   * Constructor, creates a new CmsHtmlConverter.
   *
   * <p>Possible values for the default conversion mode are:
   *
   * <ul>
   *   <li>{@link #PARAM_DISABLED}: The conversion is disabled.
   *   <li>{@link #PARAM_ENABLED}: Conversion is enabled without transformation, so HTML is pretty
   *       printed only.
   *   <li>{@link #PARAM_XHTML}: Conversion from HTML to XHTML is enabled.
   *   <li>{@link #PARAM_WORD}: Cleanup of word like HTML tags is enabled.
   *   <li>Other values can be used by the implementing converter class.
   * </ul>
   *
   * Values can be combined with the <code>;</code> separator, so it is e.g. possible to convert to
   * XHTML and clean from word at the same time.
   *
   * <p>
   *
   ...
   *
   * <p>A default value (disabled) is returned if the property could not be read.
   *
   * <p>
   *
    ...
    try {
      ...
      CmsProperty contentConversionProperty =
          cms.readPropertyObject(
              resourceName, CmsPropertyDefinition.PROPERTY_CONTENT_CONVERSION, true);
   ...
   * Converts the given HTML code according to the settings of this converter.
   *
   * <p>If an any error occurs during the conversion process, the original input is returned
   * unmodified.
   *
   * <p>
   *
      ...
      try {
        ...
        LOG.error(
            org.opencms.loader.Messages.get()
                .getBundle()
                .key(org.opencms.loader.Messages.LOG_HTML_CONVERTER_CLASS_NOT_FOUND_1, className),
            e);
   ...
   *
   * <p>Values can be combined with the <code>;</code> separator, so that it is e.g. possible to
   * convert to XHTML and clean from word at the same time.
   *
   * <p>
   *
   * @param encoding the encoding used for the HTML code conversion
