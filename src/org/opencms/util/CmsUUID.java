 ...
 *
 * <p>Spatial uniqueness is derived from ethernet address (MAC, 802.1); temporal from system clock.
 *
 * <p>For more information about the algorithm used, please see <a
 * href="http://www.opengroup.org/dce/info/draft-leach-uuids-guids-01.txt">
 * draft-leach-uuids-guids-01.txt</a>.
 *
 * <p>Because Java is unable to read the MAC address of the machine (without using JNI), the MAC
 * address has to be provided first by using the static {@link #init(String)} method.
 *
 * <p>This class is just a facade wrapper for the "real" UUID implementation.
 *
 * <p>
 *
 * @since 6.0.0
 */
public final class CmsUUID extends Object
    implements Cloneable, Comparable<CmsUUID>, Externalizable {
   ...
   * Creates a new UUID.
   *
   * <p>Please note that the static init() method has to be called first to initialize the internet
  ...
  /**
   * Create a new UUID based on another one (used internal for cloning).
   *
   * <p>
   *
  ...
  public static void checkId(CmsUUID id, boolean canBeNull) {
    ...
    if ((!canBeNull && (id == null)) || id.isNullUUID()) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_INVALID_UUID_1, id));
    }
  }
  ...
  /**
   * Returns a constant (name based) UUID for OpenCms, based on "www.opencms.org" in the dns name
   * space.
   *
   ...
   *
   * <p>The ethernetAddress parameter must represent a 'standard' ethernet MAC address string (e.g.
   * '00:C0:F0:3D:5B:7C').
   *
  ...
  /**
   * Returns <code>true</code> if the given UUID is valid.
   *
   * <p>
   *
  ...
  /**
   * Returns the String representation of this UUID, same as {@link #toString()}.
   *
   * <p>This method is useful if bean like access to the UUID String is required.
   *
   * <p>
   *
