 ...
 *
 * <p>This job will validate all link parseable resources of the context project.
 *
 * <p>Parameters used by this job (all optional): <code>email</code> parameter, should be a comma
 * separated list of email addresses, if empty the email address of the context user will be user
 * instead.
 *
 * <p><code>from</code> parameter, should be the "from" field of the email to send, a valid email
 * address, if empty the default will be used.
 *
 * <p><code>subject</code> parameter, should be the subject of the sent email, if empty a default
 * subject text will be used, a good practice is to use the same job name as subject.
 *
 * <p>
 *
 * @since 6.5.0
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(subject)) {
      subject =
          Messages.get()
              ...
              .key(Messages.GUI_RELATIONS_VALIDATION_DEFAULT_SUBJECT_0);
    }
    if (report != null) {
      msg = report.toString();
    }
