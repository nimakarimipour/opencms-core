...
public class CmsLink {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_BROKEN_LINK_BY_ID_2, m_target, m_structureId),
            e);
      }
      ...
      try {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_BROKEN_LINK_BY_NAME_1, m_target), e1);
        }
        m_structureId = null;
      } finally {
  ...
  public String getLink(CmsObject cms) {
    ...
    if (m_internal) {
      ...
      I_CmsCustomLinkRenderer handler =
          (I_CmsCustomLinkRenderer)
              cms.getRequestContext().getAttribute(CmsLink.CUSTOM_LINK_HANDLER);
      if (handler != null) {
      ...
      if ((cms.getRequestContext().getSiteRoot().length() == 0)
        ...
        // always generated _with_ server name / port so that the source code looks identical to
        // code
   ...
   * @return the processed link
   * @deprecated use {@link #getLink(CmsObject)} instead, the process editor option is set using the
   *     OpenCms request context attributes
   */
  @Deprecated
  ...
  /**
   * Returns the resource this link points to, if it is an internal link and has already been
   * initialized via checkConsistency.
   *
   ...
   *
   * <p>Also updates the structure of the underlying XML page document this link belongs to.
   *
   * <p>Note that you can <b>not</b> update the "internal" or "type" values of the link, so the new
   * link must be of same type (A, IMG) and also remain either an internal or external link.
   *
   * <p>
   *
   ...
   *
   * <p>If anchor and/or query are <code>null</code>, this features are not used.
   *
   * <p>Note that you can <b>not</b> update the "internal" or "type" values of the link, so the new
   * link must be of same type (A, IMG) and also remain either an internal or external link.
   *
  ...
  /**
   * Helper method for creating a uri from its components.
   *
   * <p>
   *
  ...
  /**
   * Sets the component member variables (target, anchor, query) by splitting the uri <code>
   * scheme://authority/path#anchor?query</code>.
   *
   * <p>
   */
  ...
  /**
   * Joins the internal target, anchor and query components to one uri string, setting the internal
   * uri and parameters fields.
   *
   * <p>
   */
  private void setUri() {
