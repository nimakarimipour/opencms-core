 ...
 *
 * <p>Relations are, for instance, href attribs in anchor tags and src attribs in image tags, as
 * well as OpenCmsVfsFile values in Xml Content.
 *
 * <p>External links to targets outside the OpenCms VFS don't get validated.
 *
 * <p>Objects using this class are responsible to handle detected broken links.
 *
 * <p>
 *
 * @since 6.3.0
...
public class CmsRelationSystemValidator {
   ...
   *
   * <p>The result is printed to the given report.
   *
   * <p>Validating references means to answer the question, whether we would have broken links in
   * the online project if the given publish list would get published.
   *
   * <p>
   *
  ...
  public Map<String, List<CmsRelation>> validateResources(
          ...
          try {
            resources.addAll(
                m_driverManager.readResources(
                    dbc, m_driverManager.readResource(dbc, "/", filter), filter, true));
          } catch (CmsException e) {
            LOG.error(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_RETRIEVAL_RESOURCES_1, type.getTypeName()),
                e);
          }
        }
      }
    ...
    for (int index = 0, size = resources.size(); index < size; index++) {
      ...
      if (report != null) {
        ...
        report.print(
            Messages.get().container(Messages.RPT_HTMLLINK_VALIDATING_0), I_CmsReport.FORMAT_NOTE);
        report.print(
            org.opencms.report.Messages.get()
                .container(
                    org.opencms.report.Messages.RPT_ARGUMENT_1, dbc.removeSiteRoot(resourceName)));
        report.print(
            org.opencms.report.Messages.get().container(org.opencms.report.Messages.RPT_DOTS_0));
      }
      List<CmsRelation> brokenLinks =
          validateLinks(dbc, resource, offlineFilesLookup, project, report);
   ...
   * @return true if the resource which has a link to the deleted resource is also going to be
   *     deleted
   */
  protected boolean checkLinkForDeletedLinkTarget(
    ...
    if (fileLookup.containsKey(link)) {
      ...
      boolean hasNoRelations =
          !relationTargetsForLink.contains(relation.getTargetPath())
    ...
    try {
      ...
      try {
        link =
            m_driverManager
                .getVfsDriver(dbc)
                .readResource(dbc, project.getUuid(), relation.getTargetId(), true)
                .getRootPath();
      } catch (CmsVfsResourceNotFoundException e) {
        // reading by id failed, this means that the link variable still equals
        ...
        m_driverManager
            .getVfsDriver(dbc)
            .readResource(dbc, project.getUuid(), relation.getTargetPath(), true);
      }
    } catch (CmsException e) {
        ...
        if (offlineResource.getState().isDeleted()) {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_LINK_VALIDATION_RESOURCEDELETED_1, link));
          }
          isValidLink = false;
        }
      }
  ...
  protected List<CmsRelation> validateLinks(
    ...
    try {
      if (!resource.getState().isDeleted()) {
        ...
        outgoingRelationsOffline =
            m_driverManager.getRelationsForResource(dbc, resource, CmsRelationFilter.TARGETS);
      } else {
        ...
        try {
          incomingRelationsOnline =
              m_driverManager.getRelationsForResource(dbc, resource, CmsRelationFilter.SOURCES);
        } finally {
      ...
      if (report != null) {
        report.println(
            Messages.get()
                .container(Messages.LOG_LINK_SEARCH_1, dbc.removeSiteRoot(resource.getRootPath())),
            I_CmsReport.FORMAT_ERROR);
      }
    ...
    while (itRelations.hasNext()) {
      ...
      if (resource.getState().isDeleted()) {
        ...
        result =
            checkLinkForNewOrChangedLinkSource(dbc, resource, relation, link, project, fileLookup);
      }
