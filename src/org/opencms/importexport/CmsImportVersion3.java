 ...
 *
 * <p>This import format was used in OpenCms 5.1.2 - 5.1.6.
 *
 * <p>
 *
 * @since 6.0.0
  ...
  private void importAllResources() throws CmsImportExportException {
    ...
    try {
      ...
      for (int i = 0; i < fileNodes.size(); i++) {
        ...
        translatedName =
            m_cms.getRequestContext().getDirectoryTranslator().translateResource(translatedName);
    ...
    try {
      ...
      if (m_convertToXmlPage && (type.equals(RESOURCE_TYPE_NEWPAGE_NAME))) {
        ...
        if (content != null) {
          ...
          encoding =
              CmsProperty.get(CmsPropertyDefinition.PROPERTY_CONTENT_ENCODING, properties)
                  .getValue();
          if (encoding == null) {
          ...
          CmsXmlPage xmlPage =
              CmsXmlPageConverter.convertToXmlPage(
                  m_cms, content, getLocale(destination, properties), encoding);
        ...
        resType =
            OpenCms.getResourceManager().getResourceType(CmsResourceTypePointer.getStaticTypeId());
      } else if (type.equals(RESOURCE_TYPE_LEGACY_PAGE_NAME)) {
        resType =
            OpenCms.getResourceManager().getResourceType(CmsResourceTypePlain.getStaticTypeId());
      } else {
