...
public final class CmsXmlPageConverter {
   ...
   * @throws CmsXmlException if there is an error reading xml contents from the byte array into a
   *     document
   */
  @SuppressWarnings("unchecked")
  public static CmsXmlPage convertToXmlPage(
      CmsObject cms, byte[] content, Locale locale, String encoding)
    ...
    if ((xmltemplate == null) || !"XMLTEMPLATE".equals(xmltemplate.getName())) {
      throw new CmsImportExportException(
          Messages.get().container(Messages.ERR_NOT_FOUND_ELEM_XMLTEMPLATE_0));
    }
    ...
    while (i.hasNext()) {
      ...
      if (bodyContent == null) {
        throw new CmsImportExportException(
            Messages.get().container(Messages.ERR_BODY_CONTENT_NOT_FOUND_0));
      }
