...
public class CmsVfsImportExportHandler implements I_CmsImportExportHandler {
  ...
  public void exportData(CmsObject cms, I_CmsReport report)
    ...
    report.println(
        Messages.get().container(Messages.RPT_EXPORT_DB_BEGIN_0), I_CmsReport.FORMAT_HEADLINE);
    new CmsExport(cms, report).exportData(parameters);
    report.println(
        Messages.get().container(Messages.RPT_EXPORT_DB_END_0), I_CmsReport.FORMAT_HEADLINE);
  }

  /**
   * Returns the timestamp to limit the resources to be exported by date.
   *
   * <p>Only resources that have been modified after this date will be exported.
   *
   * <p>
   *
  ...
  public synchronized void importData(CmsObject cms, I_CmsReport report)
    ...
    report.println(
        Messages.get().container(Messages.RPT_IMPORT_DB_BEGIN_0), I_CmsReport.FORMAT_HEADLINE);

    CmsImport vfsImport = new CmsImport(cms, report);
    vfsImport.importData(getImportParameters());

    report.println(
        Messages.get().container(Messages.RPT_IMPORT_DB_END_0), I_CmsReport.FORMAT_HEADLINE);
  }
  ...
  /**
   * Returns the boolean flag to decide whether VFS resources under /system/ should be exported or
   * not.
   *
   * <p>
   *
  ...
  public boolean matches(Document manifest) {
    ...
    boolean hasModuleNode =
        (rootElement
                .selectNodes("./" + CmsModuleXmlHandler.N_MODULE + "/" + CmsModuleXmlHandler.N_NAME)
                .size()
            > 0);
    boolean hasInfoNode =
        (rootElement.selectNodes("./" + CmsImportExportManager.N_INFO).size() == 1);
  ...
  public void setFileName(@RUntainted String fileName) {
    ...
    if (CmsStringUtil.isEmpty(fileName) || !fileName.trim().equals(fileName)) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_BAD_FILE_NAME_1, fileName));
    }
    m_fileName = fileName;
  }
