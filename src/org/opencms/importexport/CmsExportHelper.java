...
public class CmsExportHelper {
   ...
   * @param exportAsFiles indicates if the resources should be exported as individual files or in
   *     one big ZIP file
  ...
  public CmsExportHelper(String exportPath, boolean exportAsFiles, boolean validateXml)
    ...
    CmsXmlSaxWriter saxHandler =
        new CmsXmlSaxWriter(writer, OpenCms.getSystemInfo().getDefaultEncoding());
   ...
   *
   * <p>In case of the RFS export the file is directly written to a file output stream, so calling
   * this method just closes the XML and finishes the stream.
   *
   * <p>
   *
   * @param xmlSaxWriter the SAX writer to use
   ...
   *
   * <p>In case of the ZIP export the manifest is written to an internal StringBuffer first, which
   * is then stored in the ZIP file when this method is called.
   *
   * <p>
   *
   * @param xmlSaxWriter the SAX writer to use
  ...
  protected void writeManifest2Zip(CmsXmlSaxWriter xmlSaxWriter) throws IOException, SAXException {
    ...
    // complex substring operation is required to ensure handling for very large export manifest
    // files
