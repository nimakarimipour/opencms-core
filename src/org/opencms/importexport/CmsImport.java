...
/**
 * Holds the functionality to import resources from the file system or a zip file into the OpenCms
 * VFS.
 *
 * <p>
 *
...
public class CmsImport {
  ...
  /**
   * Constructs a new import object which imports the resources from an OpenCms export zip file or a
   * folder in the "real" file system.
   *
   * <p>
   *
   * @param cms the cms context
   * @param report the output report
   * @throws CmsRoleViolationException if the current user dies not have role permissions to import
   *     the database
   */
  ...
  /**
   * Imports the resources and writes them to the cms VFS, even if there already exist files with
   * the same name.
   *
   * <p>
   *
      ...
      while (i.hasNext()) {
        ...
        if (importVersion.matches(parameters)) {
          m_report.println(
              Messages.get()
                  .container(
                      Messages.RPT_IMPORT_VERSION_1, String.valueOf(importVersion.getVersion())),
          ...
          OpenCms.fireCmsEvent(
              new CmsEvent(I_CmsEventListener.EVENT_FLEX_PURGE_JSP_REPOSITORY, null));
          run = true;
          break;
        }
      }
