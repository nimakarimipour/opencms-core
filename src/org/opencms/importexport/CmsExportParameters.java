...
public class CmsExportParameters {
  ...
  /**
   * Indicates if the resources are exported in one export .ZIP file (the default) or as individual
   * files.
   */
  ...
  /**
   * Resources for with meta data should be exported, even if not in the resources to export. That
   * are super-folders of exported resources, where meta data should be kept in the export.
   */
   ...
   * @param inProject if only resources in the current project are exported
   * @param exportMode the export mode to use
   * @param additionalResourcesToExportWithMetaData the list of export-site relative paths of
   *     folders/files for which meta data should be exported, even if they do not belong the the
   *     resourcesToExport.
   */
  public CmsExportParameters(
      String exportFile,
  ...
  /**
   * Returns the resources to export with (some additional) meta-data, even for reduced meta-data
   * export.
   *
   * @return the resources to export with (some additional) meta-data, even for reduced meta-data
   *     export.
   */
   ...
   * <p>
   *
   * @return <code>false</code> if the resources will be exported in a .ZIP file, <code>true</code>
   ...
   */
  public void setAdditionalResourcesToExportWithMetaData(
      List<String> resourcesToExportWithMetaData) {
  ...
  /**
   * Controls if the resources are exported in one export .ZIP file (the default) or as individual
   * files.
   *
   * <p>
   *
  ...
  public void setPath(String path) {
    ...
    if (CmsStringUtil.isEmpty(path) || !path.trim().equals(path)) {
      throw new CmsIllegalArgumentException(
          Messages.get().container(Messages.ERR_BAD_FILE_NAME_1, path));
    }
    m_path = path;
  }
