 ...
 *
 * <p>This import format is used in OpenCms since 6.3.0.
 *
 * <p>
 *
 * @since 6.3.0
...
public class CmsImportVersion5 extends A_CmsImport {
  ...
  /**
   * Stores all relations defined in the import file to be created after all resources has been
   * imported.
   */
  ...
  protected void importRelations() {
    ...
    m_report.println(
        Messages.get().container(Messages.RPT_END_IMPORT_RELATIONS_0), I_CmsReport.FORMAT_HEADLINE);
  }

  /**
   * Reads all the relations of the resource from the <code>manifest.xml</code> file and adds them
  ...
  protected void importRelations(CmsResource resource, Element parentElement) {
    ...
    List<Node> relationElements =
        parentElement.selectNodes("./" + A_CmsImport.N_RELATIONS + "/" + A_CmsImport.N_RELATION);
    ...
    while (itRelations.hasNext()) {
      ...
      String structureID =
          getChildElementTextValue(relationElement, A_CmsImport.N_RELATION_ATTRIBUTE_ID);
      String targetPath =
          getChildElementTextValue(relationElement, A_CmsImport.N_RELATION_ATTRIBUTE_PATH);
      String relationType =
          getChildElementTextValue(relationElement, A_CmsImport.N_RELATION_ATTRIBUTE_TYPE);
      CmsUUID targetId = new CmsUUID(structureID);
      CmsRelationType type = CmsRelationType.valueOf(relationType);

      CmsRelation relation =
          new CmsRelation(
              resource.getStructureId(), resource.getRootPath(), targetId, targetPath, type);
  ...
  /**
   * Reads all file nodes plus their meta-information (properties, ACL) from the <code>manifest.xml
   * </code> and imports them as Cms resources to the VFS.
   *
   * <p>
   *
    ...
    try {
      ...
      for (int i = 0; i < fileNodes.size(); i++) {
        ...
        if (resourceNotImmutable) {
          ...
          if (res != null) {
            // only set permissions if the resource did not exists or if the keep permissions flag
            // is not set
                ...
                try {
                  if (id.startsWith(I_CmsPrincipal.PRINCIPAL_GROUP)) {
                    ...
                    if (LOG.isWarnEnabled()) {
                      LOG.warn(
                          Messages.get()
                              .getBundle()
                              .key(Messages.LOG_IMPORTEXPORT_ERROR_IMPORTING_ACE_1, id));
                    }
                  }
                  ...
                  if (LOG.isWarnEnabled()) {
                    LOG.warn(
                        Messages.get()
                            .getBundle()
                            .key(Messages.LOG_IMPORTEXPORT_ERROR_IMPORTING_ACE_1, translatedName),
                        e);
                  }
                  m_report.println(e);
                  m_report.addError(e);
            ...
            if (OpenCms.getResourceManager().getResourceType(res.getTypeId())
      ...
      try {
        ...
        if (LOG.isWarnEnabled()) {
          LOG.warn(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_IMPORTEXPORT_REWRITING_1, res.getRootPath()));
        }
        if (LOG.isDebugEnabled()) {
          LOG.debug(e.getLocalizedMessage(), e);
        }
