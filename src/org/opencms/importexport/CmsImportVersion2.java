 ...
 *
 * <p>This import format was used in OpenCms 5.0.0 - 5.1.2.
 *
 * <p>
 *
 * @since 6.0.0
 * @see org.opencms.importexport.A_CmsImport
 * @deprecated this import class is no longer in use and should only be used to import old export
 *     files
 */
@Deprecated
public class CmsImportVersion2 extends A_CmsImport {
  ...
  public void importData(CmsObject cms, I_CmsReport report, CmsImportParameters params)
    ...
    if (OpenCms.getRunLevel() >= OpenCms.RUNLEVEL_3_SHELL_ACCESS) {
      OpenCms.getMemoryMonitor()
          .register(this.getClass().getName() + ".m_folderStorage", m_folderStorage);
      OpenCms.getMemoryMonitor()
          .register(this.getClass().getName() + ".m_pageStorage", m_pageStorage);
      OpenCms.getMemoryMonitor()
          .register(this.getClass().getName() + ".m_linkStorage", m_linkStorage);
      OpenCms.getMemoryMonitor()
          .register(this.getClass().getName() + ".m_linkPropertyStorage", m_linkPropertyStorage);
    }
    ...
    try {
      ...
      m_docXml =
          CmsXmlUtils.unmarshalHelper(
              helper.getFileBytes(CmsImportExportManager.EXPORT_MANIFEST), null);
   ...
   *
   * <p>The content is *NOT* changed in the implementation of this class.
   *
   * <p>
   *
  ...
  /**
   * @see org.opencms.importexport.A_CmsImport#importUser(String, String, String, String, String,
   *     String, long, Map, List)
   */
  @Override
  protected void importUser(
    ...
    super.importUser(
        name, flags, password, firstname, lastname, email, dateCreated, userInfo, userGroups);
  }
  ...
  /**
   * Returns the compatibility web app names.
   *
   * <p>
   *
  ...
  private void importAllResources() throws CmsImportExportException {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_IMPORTEXPORT_ERROR_GETTING_WEBAPP_COMPATIBILITY_NAMES_0),
            e);
      }
      m_report.println(e);
    }
    ...
    old_overwriteCollidingResources =
    ...
    try {
      ...
      for (int i = 0; i < importSize; i++) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_IMPORTEXPORT_TRANSLATED_RESOURCE_NAME_1, translatedName));
        }
        ...
        if (resourceNotImmutable) {
          ...
          m_report.print(
              Messages.get().container(Messages.RPT_IMPORTING_0), I_CmsReport.FORMAT_NOTE);
          m_report.print(
              org.opencms.report.Messages.get()
          ...
          if (res != null) {
            ...
            for (int j = 0; j < acentryNodes.size(); j++) {
              ...
              String allowed =
                  getChildElementTextValue(
                      currentEntry, A_CmsImport.N_ACCESSCONTROL_ALLOWEDPERMISSIONS);
              String denied =
                  getChildElementTextValue(
                      currentEntry, A_CmsImport.N_ACCESSCONTROL_DENIEDPERMISSIONS);
            ...
            m_report.print(
                Messages.get().container(Messages.RPT_SKIPPING_0), I_CmsReport.FORMAT_OK);
            m_report.println(
                org.opencms.report.Messages.get()
                    .container(org.opencms.report.Messages.RPT_ARGUMENT_1, translatedName));
            if (LOG.isInfoEnabled()) {
          ...
          m_report.print(
              Messages.get().container(Messages.RPT_SKIPPING_0), I_CmsReport.FORMAT_NOTE);
          m_report.println(
              org.opencms.report.Messages.get()
                  .container(org.opencms.report.Messages.RPT_ARGUMENT_1, translatedName));
      ...
      OpenCms.getImportExportManager()
          .setOverwriteCollidingResources(old_overwriteCollidingResources);
    }
  }
  ...
  private CmsResource importResource(
    ...
    try {
      ...
      try {
        ...
        isFolder =
            ((size == 0)
                && CmsResourceTypeFolder.RESOURCE_TYPE_NAME.equalsIgnoreCase(resourceTypeName));
      }
      ...
      if (RESOURCE_TYPE_LINK_ID == resourceTypeId) {
        ...
        m_report.print(
            Messages.get().container(Messages.RPT_STORING_LINK_0), I_CmsReport.FORMAT_NOTE);
        ...
        try {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_IMPORTEXPORT_UNABLE_TO_UNLOCK_RESOURCE_1, resName),
                e);
          }
        }
      }
      ...
      CmsMessageContainer message =
          Messages.get()
              .container(Messages.ERR_IMPORTEXPORT_ERROR_IMPORTING_RESOURCE_1, targetName);
      if (LOG.isDebugEnabled()) {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_IMPORTEXPORT_START_MERGING_1, resourcename));
      }

      // in OpenCms versions <5 node names have not been case sensitive. thus, nodes are read both
      // in upper
      ...
      // get the <masterTemplate> node to check the content. this node contains the name of the
      // template file.
      ...
      if (bodyNode != null) {
        ...
        // get the encoding
        String encoding =
            CmsProperty.get(CmsPropertyDefinition.PROPERTY_CONTENT_ENCODING, properties).getValue();
        if (encoding == null) {
        ...
        CmsProperty newProperty =
            new CmsProperty(CmsPropertyDefinition.PROPERTY_TEMPLATE, mastertemplate, null);
        ...
        if (bodyparams != null) {
          for (Iterator<Entry<String, String>> p = bodyparams.entrySet().iterator();
              p.hasNext(); ) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_IMPORTEXPORT_START_IMPORTING_XML_PAGE_0));
        }
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get().getBundle().key(Messages.LOG_IMPORTEXPORT_END_IMPORTING_XML_PAGE_0));
        }
        ...
        m_report.println(
            Messages.get().container(Messages.RPT_NOT_CONVERTED_0), I_CmsReport.FORMAT_OK);
      }

      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get().getBundle().key(Messages.LOG_IMPORTEXPORT_END_MERGING_1, resourcename));
      }
    } catch (CmsXmlException e) {
  ...
  /**
   * Merges the page control files and their corresponding bodies into a single files.
   *
   * <p>
   *
   * @throws CmsImportExportException if something goes wrong
  ...
  private void mergePageFiles() throws CmsXmlException, CmsImportExportException {
    ...
    try {
      ...
      CmsMessageContainer message =
          Messages.get().container(Messages.ERR_IMPORTEXPORT_ERROR_COPYING_PROPERTY_DEFINITIONS_0);
      if (LOG.isDebugEnabled()) {
        LOG.debug(message.key(), e);
      }
    ...
    while (i.hasNext()) {
      ...
      m_report.print(
          org.opencms.report.Messages.get()
              .container(org.opencms.report.Messages.RPT_ARGUMENT_1, resname));
  ...
  /**
   * Deletes the folder structure which has been creating while importing the body files..
   *
   * <p>
   *
   * @throws CmsImportExportException if something goes wrong
  ...
  private void removeFolders() throws CmsImportExportException {
    ...
    try {
      ...
      m_report.println(
          Messages.get().container(Messages.RPT_DELFOLDER_START_0), I_CmsReport.FORMAT_HEADLINE);
      ...
      for (int j = (size - 1); j >= 0; j--) {
        ...
        resname =
            (resname.startsWith("/") ? "" : "/") + resname + (resname.endsWith("/") ? "" : "/");
        // now check if the folder is really empty. Only delete empty folders
        List<CmsResource> files =
            m_cms.getFilesInFolder(resname, CmsResourceFilter.IGNORE_EXPIRATION);

        if (files.size() == 0) {
          List<CmsResource> folders =
              m_cms.getSubFolders(resname, CmsResourceFilter.IGNORE_EXPIRATION);
          if (folders.size() == 0) {
            m_report.print(
                org.opencms.report.Messages.get()
                    .container(
                ...
                I_CmsReport.FORMAT_NOTE);
            m_report.print(
                Messages.get().container(Messages.RPT_DELFOLDER_0), I_CmsReport.FORMAT_NOTE);
            m_report.print(
                org.opencms.report.Messages.get()
                    .container(org.opencms.report.Messages.RPT_ARGUMENT_1, resname),
      ...
      CmsMessageContainer message =
          Messages.get()
              .container(Messages.ERR_IMPORTEXPORT_ERROR_REMOVING_FOLDERS_OF_IMPORTED_BODY_FILES_0);
      if (LOG.isDebugEnabled()) {
        LOG.debug(message.key(), e);
      }
