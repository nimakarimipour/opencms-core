...
public class CmsUserIconHelper {
  ...
  public static boolean hasUserImage(CmsUser user) {
    ...
    return CmsStringUtil.isNotEmptyOrWhitespaceOnly(
        (String) user.getAdditionalInfo(USER_IMAGE_INFO));
  }
  ...
  /**
   * Handles a user image upload. The uploaded file will be scaled and save as a new file beneath
   * /system/userimages/, the original file will be deleted.
   *
   * <p>
   *
   * @param cms the cms context
  ...
  public boolean handleImageUpload(CmsObject cms, List<String> uploadedFiles) {
    ...
    if (uploadedFiles.size() == 1) {
      String tempFile =
          CmsStringUtil.joinPaths(USER_IMAGE_FOLDER, TEMP_FOLDER, uploadedFiles.get(0));
  ...
  private String toPath(String name, IconSize size) {
    ...
    return CmsStringUtil.joinPaths(CmsWorkplace.getSkinUri(), ICON_FOLDER, "" + name.hashCode())
        + size.getSuffix();
  }
  ...
  private String toRfsName(String name, IconSize size) {
    ...
    return CmsStringUtil.joinPaths(m_cache.getRepositoryPath(), "" + name.hashCode())
        + size.getSuffix();
  }
}
