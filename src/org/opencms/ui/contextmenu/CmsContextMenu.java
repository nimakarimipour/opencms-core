 ...
 * context menu. Most useful the menu is when attached for example to Tree or Table which support
 * item and property based context menu detection.
 *
...
public class CmsContextMenu extends AbstractExtension {
  ...
  /**
   * ContextMenuClosedListener is used to listen for the event that the context menu is closed,
   * either when a item is clicked or when the popup is canceled.
   *
   * <p>
   */
  public interface ContextMenuClosedListener extends EventListener {
    ...
    public static final Method MENU_CLOSED =
        ReflectTools.findMethod(
            ContextMenuClosedListener.class, "onContextMenuClosed", ContextMenuClosedEvent.class);
  ...
  public class ContextMenuItem implements Serializable {
    ...
    /**
     * Adds context menu item click listener only to this item. This listener will be invoked only
    ...
    /**
     * Add a new style to the menu item. This method is following the same semantics as {@link
    ...
    /**
     * Removes given click listener from this item. Removing listener affects only this context menu
     * item.
     *
     * <p>
     *
    ...
    /**
     * Remove a style name from this menu item. This method is following the same semantics as
     * {@link Component#removeStyleName(String)}.
     *
     * <p>
     *
    ...
    /**
     * Associates given object with this menu item. Given object can be whatever application
   ...
   * Event contains method for retrieving the clicked item and menu from which the click event
   * originated.
   */
  ...
  /**
   * ContextMenuItemClickListener is listener for context menu items wanting to notify listeners
   * about item click
   */
  public interface ContextMenuItemClickListener extends EventListener {
     ...
     *
     * @param event containing the information of which item was clicked
     */
    public void contextMenuItemClicked(ContextMenuItemClickEvent event);
  }

  /**
   * ContextMenuOpenedListener is used to modify the content of context menu based on what was
   * clicked. For example TableListener can be used to modify context menu based on certain table
   * component clicks.
   *
   * <p>
   */
  public interface ContextMenuOpenedListener extends EventListener {
    ...
    /**
     * ComponentListener is used when context menu is extending a component and works in mode where
     * auto opening is disabled. For example if ContextMenu is assigned to a Layout and layout is
     * right clicked when auto open feature is disabled, the open listener would be called instead
     * of menu opening automatically. Example usage is for example as follows:
     *
     * <p>event.getContextMenu().open(event.getRequestSourceComponent());
     *
     * <p>
     */
    public interface ComponentListener extends ContextMenuOpenedListener {
      ...
      public static final Method MENU_OPENED_FROM_COMPONENT =
          ReflectTools.findMethod(
    ...
    public interface TableListener extends ContextMenuOpenedListener {
      ...
      /** Opened from method. */
      public static final Method MENU_OPENED_FROM_TABLE_FOOTER_METHOD =
          ReflectTools.findMethod(
      ...
      /** Opened from method. */
      public static final Method MENU_OPENED_FROM_TABLE_HEADER_METHOD =
          ReflectTools.findMethod(
              ContextMenuOpenedListener.TableListener.class,
      ...
      /** Opened from method. */
      public static final Method MENU_OPENED_FROM_TABLE_ROW_METHOD =
          ReflectTools.findMethod(
              ContextMenuOpenedListener.TableListener.class,
    ...
    public interface TreeListener extends ContextMenuOpenedListener {
      ...
      /** Opened from method. */
      public static final Method MENU_OPENED_FROM_TREE_ITEM_METHOD =
          ReflectTools.findMethod(
  ...
  /**
   * ContextMenuOpenedOnComponentEvent is an event fired by the context menu when it's opened from a
   * component.
   *
   * <p>
   */
  public static class ContextMenuOpenedOnComponentEvent extends EventObject {
    ...
    public ContextMenuOpenedOnComponentEvent(
        CmsContextMenu contextMenu, int x, int y, Component component) {
      super(component);
  ...
  /**
   * ContextMenuOpenedOnTableFooterEvent is an event that is fired by the context menu when it's
   * opened by clicking on table footer
   */
  public static class ContextMenuOpenedOnTableFooterEvent extends EventObject {
    ...
    public ContextMenuOpenedOnTableFooterEvent(
        CmsContextMenu contextMenu, Table sourceTable, Object propertyId) {
      super(sourceTable);
  ...
  /**
   * ContextMenuOpenedOnTableHeaderEvent is an event fired by the context menu when it's opened by
   * clicking on table header row.
   *
   * <p>
   */
  ...
  /**
   * ContextMenuOpenedOnTableRowEvent is an event that is fired when context menu is opened by
  ...
  public static class ContextMenuOpenedOnTableRowEvent extends EventObject {
    ...
    public ContextMenuOpenedOnTableRowEvent(
        CmsContextMenu contextMenu, Table table, Object itemId, Object propertyId) {
      super(table);
  ...
  /**
   * ContextMenuOpenedOnTreeItemEvent is an event fired by the context menu when it's opened by
  ...
  /** The server RPC. */
  private final I_CmsContextMenuServerRpc m_serverRPC =
      new I_CmsContextMenuServerRpc() {
  ...
  /**
   * Adds listener that will be invoked when context menu is opened from the component to which it's
   * assigned to.
   *
   * <p>
   *
  ...
  /**
   * Adds listener that will be invoked when context menu is opened from com.vaadin.ui.Table
   * component.
   *
   * <p>
   *
  ...
  /**
   * Adds listener that will be invoked when context menu is opened from com.vaadin.ui.Tree
   * component.
   *
   * <p>
   *
  ...
  /**
   * Adds click listener to context menu. This listener will be invoked when any of the menu items
   * in this menu are clicked.
   *
   * <p>
   *
   * @param clickListener the click listener
   ...
   * <p>
   *
   * @return <code>true</code> if open automatically is on. If open automatically is on, it means
   *     that context menu will always be opened when it's host component is right clicked. If
   *     automatic opening is turned off, context menu will only open when server side open(x, y) is
   *     called. Automatic opening avoid having to make server roundtrip whereas "manual" opening
   *     allows to have logic in menu before opening it.
   */
  ...
  public void open(Component component) {
    ...
    getRpcProxy(I_CmsContextMenuClientRpc.class)
        .showContextMenuRelativeTo(component.getConnectorId());
  }

  /**
   * Opens the context menu to given coordinates. ContextMenu must extend component before calling
   * this method. This method is only intended for opening the context menu from server side when
   * using {@link #ContextMenuOpenedListener.ComponentListener}.
   *
   * <p>
   *
   * @param x the client x position
  ...
  public void openForTable(ItemClickEvent event, Table table) {
    ...
    fireEvent(
        new ContextMenuOpenedOnTableRowEvent(
            this, table, event.getItemId(), event.getPropertyId()));
    open(event.getClientX(), event.getClientY());
  }
  ...
  /**
   * Removes given context menu item from the context menu. The given item can be a root item or
   * leaf item or anything in between. If given given is not found from the context menu structure,
  ...
  /**
   * Sets menu to hide automatically after mouse cliks on menu items or area off the menu. If
   * automatic hiding is disabled menu will stay open as long as hide is called from the server
   * side.
   *
   * <p>
   *
  ...
  /**
   * Enables or disables open automatically feature. If open automatically is on, it means that
   * context menu will always be opened when it's host component is right clicked. This will happen
   * on client side without server round trip. If automatic opening is turned off, context menu will
   * only open when server side open(x, y) is called. If automatic opening is disabled you will need
   * a listener implementation for context menu that is called upon client side click event. Another
   * option is to extend context menu and handle the right clicking internally with case specific
   * listener implementation and inside it call open(x, y) method.
   *
   ...
   * <p>
   *
   * @see com.vaadin.server.AbstractClientConnector#setResource(java.lang.String,
  ...
  /**
   * Returns whether the menu has a specific item.
   *
   * <p>
   *
