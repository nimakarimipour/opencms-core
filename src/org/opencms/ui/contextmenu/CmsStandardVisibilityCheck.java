 ...
 *
 * <p>Instances of this class are configured with a set of flags, each of which corresponds to a
 * check to perform which may cause the context menu item to be hidden or deactivated.
 *
 * <p>
 */
public final class CmsStandardVisibilityCheck extends A_CmsSimpleVisibilityCheck {
  ...
  public static final CmsStandardVisibilityCheck COPY_PAGE =
      new CmsStandardVisibilityCheck(roleeditor, notonline, notdeleted, pagefolder);

  /** Default visibility check for 'edit-like' operations on resources. */
  public static final CmsStandardVisibilityCheck DEFAULT =
      new CmsStandardVisibilityCheck(
          roleeditor, notonline, notdeleted, writepermisssion, inproject);
  ...
  public static final I_CmsHasMenuItemVisibility DEFAULT_DEFAULTFILE =
      new CmsStandardVisibilityCheck(
          roleeditor, notonline, notdeleted, writepermisssion, inproject, defaultfile);

  /** Default visibility check for 'edit-like' operations on folders. */
  public static final CmsStandardVisibilityCheck DEFAULT_FOLDERS =
      new CmsStandardVisibilityCheck(
          folder, roleeditor, notonline, notdeleted, writepermisssion, inproject);
   ...
   *
   * <p>Note that the order of the flags does not matter; the checks corresponding to the flags are
  ...
  /**
   * @see
   *     org.opencms.ui.contextmenu.A_CmsSimpleVisibilityCheck#getSingleVisibility(org.opencms.file.CmsObject,
  ...
  public CmsMenuItemVisibilityMode getSingleVisibility(CmsObject cms, CmsResource resource) {
    ...
    if (resource != null) {
      if (flag(roleeditor)
          && !OpenCms.getRoleManager().hasRoleForResource(cms, CmsRole.EDITOR, resource)) {
    ...
    if ((resource != null)) {
      ...
      if (flag(defaultfile)) {
        ...
        try {
          ...
          CmsResource defaultFile =
              cms.readDefaultFile(parentFolder, CmsResourceFilter.IGNORE_EXPIRATION);
          if ((defaultFile == null)
              || !(defaultFile.getStructureId().equals(resource.getStructureId()))) {
            return VISIBILITY_INVISIBLE;
          }
      ...
      if (flag(pagefolder)) {
        ...
        try {
          ...
          if ((defaultFile == null)
              || !CmsResourceTypeXmlContainerPage.isContainerPage(defaultFile)) {
            return VISIBILITY_INVISIBLE;
          }
        } catch (CmsException e) {
      ...
      if (flag(pointer)
          && !OpenCms.getResourceManager()
              .matchResourceType(
                  CmsResourceTypePointer.getStaticTypeName(), resource.getTypeId())) {
        return VISIBILITY_INVISIBLE;
      }
      ...
      if (flag(hassourcecodeeditor)) {
        ...
        boolean hasSourcecodeEditor =
            (type instanceof CmsResourceTypeXmlContent)
                || (type instanceof CmsResourceTypeXmlPage)
                || (type instanceof CmsResourceTypePointer)
                || OpenCms.getResourceManager()
                    .matchResourceType(BundleType.PROPERTY.toString(), resource.getTypeId());
      ...
      if (flag(notunchangedfile)
          && resource.isFile()
          && resUtil.getResource().getState().isUnchanged()) {
      ...
      if (flag(haseditor)
          && (OpenCms.getWorkplaceAppManager().getEditorForResource(cms, resource, false)
              == null)) {
        return VISIBILITY_INVISIBLE;
      }
      ...
      if (flag(notdeleted)
          && (inActiveKey == null)
          && resUtil.getResource().getState().isDeleted()) {
