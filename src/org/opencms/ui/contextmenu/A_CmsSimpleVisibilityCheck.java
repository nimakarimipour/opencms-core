 ...
 *
 * <p>This class automatically handles the case where multiple resources are passed to the
 * getVisibilityMethod. You just need to implement the getSingleVisibility method in subclasses.
 */
public abstract class A_CmsSimpleVisibilityCheck implements I_CmsHasMenuItemVisibility {
   ...
   */
  public abstract CmsMenuItemVisibilityMode getSingleVisibility(
  ...
  public CmsMenuItemVisibilityMode getVisibility(CmsObject cms, List<CmsResource> resources) {
    ...
    if (resources.size() <= 1) {
      ...
      if (currentVisibility.isInActive()) {
        // In the multi-selection case, different resources may cause the menu item to be inactive
        // for different reasons,
        // which would make it hard to give a user-readable message. So we set the status to
        // invisible.
  ...
  /**
   * Gets the priority of a menu item visibility mode.
   *
   * <p>Higher priority modes override the ones with lower priorities.
   *
   * <p>
   *
   * @param mode the mode
