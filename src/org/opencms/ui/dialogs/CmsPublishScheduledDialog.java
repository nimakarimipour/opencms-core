...
public class CmsPublishScheduledDialog extends CmsBasicDialog {
  ...
  void submit() {
    ...
    if (current > publishTime) {
      m_context.error(
          new CmsException(
              Messages.get().container(Messages.ERR_PUBLISH_SCHEDULED_DATE_IN_PAST_1, dateValue)));
    } else {
      try {
        ...
        for (CmsResource resource : m_context.getResources()) {
          ...
          if (resource.isFolder() && m_includeSubResources.getValue().booleanValue()) {
            List<CmsResource> subResources =
                cms.readResources(
                    resource,
                    CmsResourceFilter.ONLY_VISIBLE.addExcludeState(
            ...
            for (CmsResource sub : subResources) {
              ...
              if (cms.hasPermissions(
                  sub, CmsPermissionSet.ACCESS_DIRECT_PUBLISH, false, CmsResourceFilter.ALL)) {
        ...
        String unlockIdList =
            needToUnlockLater.stream()
                .map(res -> res.getStructureId().toString())
   ...
   * @param needToUnlockLater a set to which this method adds resources that should be explicitly
   *     unlocked later
  ...
  private String computeProjectName(@RUntainted String rootPath, Date date) {
    ...
    String projectName =
        CmsVaadinUtils.getMessageText(
            Messages.GUI_PUBLISH_SCHEDULED_PROJECT_NAME_2, rootPath, dateTime);
  ...
  private FormLayout initForm() {
    ...
    m_dateField.setCaption(
        CmsVaadinUtils.getMessageText(
            org.opencms.workplace.commons.Messages.GUI_LABEL_DATE_PUBLISH_SCHEDULED_0));
    form.addComponent(m_dateField);
    m_includeSubResources =
        new CheckBox(
            CmsVaadinUtils.getMessageText(
                org.opencms.workplace.commons.Messages.GUI_PUBLISH_MULTI_SUBRESOURCES_0));
    if (hasFolders()) {
