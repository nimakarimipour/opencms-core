 ...
 * content values, allowing the user to switch between the two views.
 *
 * <p>
 */
public class CmsValueDiff implements I_CmsDiffProvider {
  ...
  public Optional<Component> diff(
      final CmsObject cms, CmsHistoryResourceBean v1, CmsHistoryResourceBean v2)
      throws CmsException {
    ...
    if ((type instanceof CmsResourceTypeXmlContent) || (type instanceof CmsResourceTypeXmlPage)) {
      ...
      final Panel panel =
          new Panel(
              CmsVaadinUtils.getMessageText(
                  Messages.GUI_HISTORY_DIALOG_CONTENT_VALUE_TABLE_CAPTION_0));
      ...
      fileTextCompareButton.addClickListener(
          new ClickListener() {
  ...
  /**
   * Builds the table for the content value comparisons.
   *
   * <p>
   *
  ...
  private Table buildValueComparisonTable(
    ...
    CmsBeanTableBuilder<CmsValueCompareBean> builder =
        CmsBeanTableBuilder.newInstance(
            CmsValueCompareBean.class, A_CmsUI.get().getDisplayType().toString());
              ...
              new ClickListener() {
                ...
                public void buttonClick(ClickEvent event) {
                  ...
                  CmsHistoryDialog.openChildDialog(
                      ...
                      diffPanel,
                      CmsVaadinUtils.getMessageText(
                          Messages.GUI_HISTORY_DIALOG_COMPARE_VALUE_1, row.getXPath()));
                }
              });
      rows.add(row);
    }
  ...
  /**
   * Builds the diff view for the XML text.
   *
   * <p>
   *
   * @param cms the CMS context
  ...
  /**
   * Decodes the given data with the given encoding, falling back to the system encoding if
   * necessary.
   *
   * <p>
   *
