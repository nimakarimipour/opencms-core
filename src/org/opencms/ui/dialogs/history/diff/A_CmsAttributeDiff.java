...
public abstract class A_CmsAttributeDiff implements I_CmsDiffProvider {
  ...
  /**
   * Builds a version string which can be used by CmsResourceComparison from a history resource
   * bean.
   *
   * <p>This is not really a user-readable string, it is meant for internal usage.
   *
   ...
   */
  public static CmsResource readResource(CmsObject cms, CmsHistoryResourceBean bean)
      throws CmsException {

    CmsHistoryVersion versionBean = bean.getVersion();
    if (versionBean.getVersionNumber() != null) {
      return (CmsResource)
          (cms.readResource(bean.getStructureId(), versionBean.getVersionNumber().intValue()));
    } else {
  ...
  public Optional<Component> diff(
    ...
    CmsBeanTableBuilder<CmsPropertyCompareBean> builder =
        CmsBeanTableBuilder.newInstance(
            CmsPropertyCompareBean.class, A_CmsUI.get().getDisplayType().toString());
