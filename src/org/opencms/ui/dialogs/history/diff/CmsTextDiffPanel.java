...
public class CmsTextDiffPanel extends VerticalLayout {
   ...
   * @param selectTextOrHtml true if the option to select between comparison as text and comparison
   *     as HTML should be shown
   * @param selectDiffMode true if the option to select between showing only the changed lines or
  ...
  public CmsTextDiffPanel(
    ...
    m_textOrHtmlSelect.addValueChangeListener(
    ...
    m_diffModeSelect.addValueChangeListener(
        new ValueChangeListener() {
  ...
  public String getDiffHtml() throws Exception {
    ...
    CmsDiffViewMode mode =
        "all".equals(m_diffMode) ? CmsDiffViewMode.ALL : CmsDiffViewMode.DIFF_ONLY;
    ...
    CmsHtmlDifferenceConfiguration conf =
        new CmsHtmlDifferenceConfiguration(
            mode == CmsDiffViewMode.ALL ? -1 : 2, A_CmsUI.get().getLocale());
  ...
  protected String wrapLinesWithUnchangedStyle(String diff) throws IOException {
    ...
    while ((line = br.readLine()) != null) {
      ...
      result
          .append("<div class=\"df-unc\"><span class=\"df-unc\">")
          .append(line)
          .append("</span></div>\n");
    }
