...
public class CmsHistoryDialog extends CmsBasicDialog {
  ...
  private List<I_CmsDiffProvider> m_diffs =
      Arrays.<I_CmsDiffProvider>asList(
  ...
  public CmsHistoryDialog(I_CmsDialogContext context) {
    ...
    try {
      CmsHistoryResourceCollection historyList =
          vfsService.getResourceHistoryInternal(resource.getStructureId());
      ...
      m_group1.setChangeListener(
          new CmsLogicalCheckboxGroup.I_ChangeListener() {
  ...
  /**
   * Replaces the contents of the window containing a given component with a basic dialog consisting
   * of a back button to restore the previous window state and another user provided widget.
   *
   * <p>
   *
   ...
   */
  public static void openChildDialog(
      Component currentComponent, Component newView, String newCaption) {
    ...
    Button backButton =
        new Button(CmsVaadinUtils.getMessageText(Messages.GUI_CHILD_DIALOG_GO_BACK_0));
  ...
  /**
   * Restores a resource's state to the given version, but asks the user for confirmation
   * beforehand.
   *
   * <p>
   *
  ...
  public void actionRestore(final CmsObject cms, final CmsUUID structureId, final Integer version) {
    ...
    String title =
        CmsVaadinUtils.getMessageText(Messages.GUI_HISTORY_DIALOG_CONFIRM_RESTORE_TITLE_0);
  ...
  /**
   * Creates a close button for child dialogs.
   *
   * <p>
   *
    ...
    if (!canCompare(check1, check2)) {
      Notification.show(
          CmsVaadinUtils.getMessageText(
              Messages.GUI_HISTORY_DIALOG_SELECT_TWO_DIFFERENT_VERSIONS_0));
    } else {
  ...
  /**
   * Displays the preview for a given resource version.
   *
   * <p>
   *
  ...
  /**
   * Builds a table containing the different versions of a resource.
   *
   * <p>
   *
  ...
  private Table buildHistoryTable(CmsHistoryResourceCollection historyList) {
    ...
    try {
      CmsBeanTableBuilder<CmsHistoryRow> builder =
          CmsBeanTableBuilder.newInstance(
              CmsHistoryRow.class, A_CmsUI.get().getDisplayType().toString());
  ...
  /**
   * Checks if two different versions are selected, and so can be compared.
   *
   * <p>
   *
