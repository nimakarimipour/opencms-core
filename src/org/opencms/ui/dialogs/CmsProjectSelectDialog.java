...
public class CmsProjectSelectDialog extends CmsBasicDialog {
  ...
  public static void changeSiteOrProject(
      I_CmsDialogContext context, CmsUUID projectId, SiteSelectorOption siteOption) {
    ...
    try {
      ...
      if ((siteRoot != null) && CmsFileExplorerConfiguration.APP_ID.equals(context.getAppId())) {
        if (siteOption.getPath() != null) {
                ...
                .setLocation(
                    OpenCms.getLinkManager()
                        .substituteLinkForUnknownTarget(
                            A_CmsUI.getCmsObject(), siteOption.getPath()));
            return;
          } else {
            Page.getCurrent()
                .open(
          ...
          I_CmsWorkplaceAppConfiguration editorConf =
              OpenCms.getWorkplaceAppManager()
                  .getAppConfiguration(CmsPageEditorConfiguration.APP_ID);
  ...
  private FormLayout initForm() {
    ...
    IndexedContainer projects =
        CmsVaadinUtils.getProjectsContainer(m_context.getCms(), CAPTION_PROPERTY);
    m_projectComboBox =
        prepareComboBox(projects, org.opencms.workplace.Messages.GUI_LABEL_PROJECT_0);
        ...
        CmsUUID ouProject =
            OpenCms.getOrgUnitManager()
                .readOrganizationalUnit(
                    m_context.getCms(), m_context.getCms().getRequestContext().getOuFqn())
                .getProjectId();
  ...
  private ComboBox prepareComboBox(IndexedContainer container, String captionKey) {
    ...
    ComboBox result =
        new ComboBox(CmsVaadinUtils.getWpMessagesForCurrentLocale().key(captionKey), container);
