...
public class CmsAvailabilityDialog extends CmsBasicDialog {
  ...
  public static final String PREF_AVAILABILITY_INCLUDE_CHILDREN_DEFAULT =
      "availabilityIncludeChildrenDefault";
  ...
  public CmsAvailabilityDialog(I_CmsDialogContext dialogContext) {
    ...
    m_notificationIntervalField.addValidator(
        new Validator() {
            ...
            if (!strValue.matches("[0-9]*")) {
              throw new InvalidValueException(
                  CmsVaadinUtils.getMessageText(
                      org.opencms.ui.Messages.GUI_VALIDATOR_EMPTY_OR_NUMBER_0));
            }
          }
        });
    ...
    if (resources.size() == 1) {
      ...
      if (!responsibles.isEmpty()) {
        ...
        for (Map.Entry<CmsPrincipalBean, String> entry : responsibles.entrySet()) {
          ...
          CmsResourceInfo infoWidget =
              new CmsResourceInfo(entry.getKey().getName(), subtitle, icon);
    ...
    boolean includeChildren =
        Boolean.parseBoolean(
            wpSettings
                .getUserSettings()
                .getAdditionalPreference(PREF_AVAILABILITY_INCLUDE_CHILDREN_DEFAULT, true));
  ...
  protected List<CmsUUID> changeAvailability() throws CmsException {
    ...
    for (CmsResource resource : m_dialogContext.getResources()) {
      changeAvailability(
          resource, released, resetReleased, expired, resetExpired, modifySubresources);
    ...
    boolean notificationSettingsUnchanged =
        notificationInterval.equals(m_initialNotificationInterval)
  ...
  CmsAvailabilityInfoBean getAvailabilityInfo(CmsObject cms, CmsResource res) throws CmsException {
    ...
    String notificationInterval =
        cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_NOTIFICATION_INTERVAL, false)
            .getValue();
    ...
    String notificationEnabled =
        cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_ENABLE_NOTIFICATION, false)
            .getValue();
    ...
    for (CmsResource resource : parentResources) {
      ...
      try {
        ...
        List<CmsAccessControlEntry> entries =
            cms.getAccessControlEntries(resource.getRootPath(), false);
        for (CmsAccessControlEntry ace : entries) {
          if (ace.isResponsible()) {
            I_CmsPrincipal principal = cms.lookupPrincipal(ace.getPrincipal());
            if (principal != null) {
              CmsPrincipalBean prinBean =
                  new CmsPrincipalBean(
                      principal.getName(), principal.getDescription(), principal.isGroup());
  ...
  private void changeAvailability(
    ...
    try {
      cms.getRequestContext()
          .setAttribute(CmsResourceTypeXmlContent.ATTR_REVERSE_AVAILABILITY_MAPPING, Boolean.TRUE);
      ...
      cms.getRequestContext()
          .removeAttribute(CmsResourceTypeXmlContent.ATTR_REVERSE_AVAILABILITY_MAPPING);
  ...
  /**
   * Creates a reset checkbox which can enable / disable a date field.
   *
   * <p>
   *
