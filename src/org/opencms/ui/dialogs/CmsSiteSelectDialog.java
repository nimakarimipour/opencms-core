...
public class CmsSiteSelectDialog extends CmsBasicDialog {
  ...
  public static void changeSite(
      CmsEmbeddedDialogContext context, CmsExtendedSiteSelector.SiteSelectorOption option) {

    if ((option.getPath() != null)
        || !CmsStringUtil.comparePaths(
            context.getCms().getRequestContext().getSiteRoot(), option.getSite())) {
      String siteRoot = option.getSite();
      A_CmsUI.get().changeSite(option.getSite());
      if (CmsStringUtil.comparePaths(siteRoot, "/")
          || OpenCms.getSiteManager().isSharedFolder(siteRoot)) {
        // switch to explorer view when selecting shared or root site

        Page.getCurrent()
            .open(CmsCoreService.getFileExplorerLink(A_CmsUI.getCmsObject(), siteRoot), "_top");
        return;
      }
  ...
  private FormLayout initForm() {
    ...
    m_siteComboBox.setValue(
        new CmsExtendedSiteSelector.SiteSelectorOption(
            A_CmsUI.getCmsObject().getRequestContext().getSiteRoot(), null, null));
