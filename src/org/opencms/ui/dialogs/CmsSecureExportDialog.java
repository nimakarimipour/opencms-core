...
public class CmsSecureExportDialog extends CmsBasicDialog {
  ...
  public CmsSecureExportDialog(I_CmsDialogContext context) {
    ...
    m_internalField.addValueChangeListener(
  ...
  protected void loadData() {
    ...
    try {
      ...
      List<CmsProperty> inheritedPropList =
          m_context.getCms().readPropertyObjects(m_resource, true);
      ...
      String exportValue =
          convertPropertyToFieldValue(propMap.get(CmsPropertyDefinition.PROPERTY_EXPORT));
    ...
    for (CmsResource resource : m_context.getResources()) {
      ...
      try {
        ...
        CmsProperty exportProp =
            new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORT, exportValue, null);
        CmsProperty exportNameProp =
            new CmsProperty(CmsPropertyDefinition.PROPERTY_EXPORTNAME, exportname, null);
  ...
  /**
   * Converts a property object to a field value for one of the boolean selection widgets.
   *
   * <p>
   *
  ...
  private void initOptionGroup(OptionGroup optGroup) {
    ...
    optGroup.setItemCaption(
        "true", wpMessages.key(org.opencms.workplace.commons.Messages.GUI_LABEL_TRUE_0));
    optGroup.setItemCaption(
        "false", wpMessages.key(org.opencms.workplace.commons.Messages.GUI_LABEL_FALSE_0));
    optGroup.setItemCaption(
        "", wpMessages.key(org.opencms.workplace.commons.Messages.GUI_SECURE_NOT_SET_0));
  }
}
