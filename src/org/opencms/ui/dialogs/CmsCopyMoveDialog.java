...
public class CmsCopyMoveDialog extends CmsBasicDialog {
        ...
        try {
          if (m_dialogMode.equals(DialogMode.copy)
              | (m_dialogMode.equals(DialogMode.copy_and_move)
                  && CmsResource.getParentFolder(m_context.getResources().get(0).getRootPath())
                      .equals(resource.getRootPath()))) {
            ...
            m_targetPath.setValue(
                getCms().getSitePath(resource)
                    + getTargetName(m_context.getResources().get(0), resource));
          }
  ...
  protected void performSingleOperation(
    ...
    performSingleOperation(
        source, target, getTargetName(source, target), action, overwrite, makroMap);
  }
  ...
  protected void performSingleOperation(
    ...
    if (finalTarget.equals(source.getRootPath()) || finalTarget.startsWith(source.getRootPath())) {
      throw new CmsVfsException(
          org.opencms.workplace.commons.Messages.get()
              .container(
                  org.opencms.workplace.commons.Messages.ERR_COPY_ONTO_ITSELF_1, finalTarget));
    }
    ...
    if (overwrite && getRootCms().existsResource(finalTarget, CmsResourceFilter.ONLY_VISIBLE)) {
      CmsLockUtil.ensureLock(
          getRootCms(), getRootCms().readResource(finalTarget, CmsResourceFilter.ONLY_VISIBLE));
      if (getRootCms().existsResource(finalTarget, CmsResourceFilter.ONLY_VISIBLE_NO_DELETED)) {
        if ((action != Action.container_page_automatic)
          ...
          // container page copy modes don't just call copyResource(), they need special handling
    ...
    if (action == Action.move) {
      ...
      try {
        ...
        CmsContainerPageCopier.CopyMode mode =
      ...
      // Copies resources. Adjust links if action==Action.sub_sitemap, resolves macro if marcoMap if
      // not null or empty
      ...
      CmsResource copyResource =
          getRootCms().readResource(finalTarget, CmsResourceFilter.IGNORE_EXPIRATION);
  ...
  void submit(boolean overwrite, Map<String, String> makroMap) {
    ...
    try {
      ...
      CmsObject cms =
          OpenCms.getSiteManager().getSiteForRootPath(target) != null ? getRootCms() : getCms();
      ...
      if (cms.existsResource(target, CmsResourceFilter.ALL.addRequireFolder())) {
        ...
        if (isSingleResource) {
          ...
          if (target.equals(m_context.getResources().get(0).getRootPath())) {
            throw new CmsVfsException(
                org.opencms.workplace.commons.Messages.get()
                    .container(
                        org.opencms.workplace.commons.Messages.ERR_COPY_ONTO_ITSELF_1, target));
          }
        ...
        if (isSingleResource) {
          // If we have a single resource, we could possible create the target as copy of that
          // resource
          if (cms.existsResource(
              CmsResource.getParentFolder(target), CmsResourceFilter.ALL.addRequireFolder())) {
            targetName = CmsResource.getName(target);
            targetFolder = cms.readResource(CmsResource.getParentFolder(target));
          } else {
            // If the parent folder of the resource does not exist, we will not create it
            // automatically.
      ...
      // Check if dialog for macro resolver has to be shown: action correct?, makroMap==null
      // (default, not set by dialog yet)
      if ((action == Action.sub_sitemap) & (makroMap == null)) {
        if (CmsSiteManager.isFolderWithMacros(
            getRootCms(), m_context.getResources().get(0).getRootPath())) {
  ...
  /**
   * Returns the resources that collide with already existing resources.
   *
   * <p>
   *
  ...
  /**
   * Gets a name for the target resource.
   *
   * <p>
   *
   * @param source Source
    ...
    if (folderRootPath.equals(CmsResource.getParentFolder(source.getRootPath()))) {
      name =
          OpenCms.getResourceManager()
              .getNameGenerator()
              .getCopyFileName(getRootCms(), folderRootPath, source.getName());
    } else {
      name = source.getName();
    }
    return name;
  ...
  /**
   * Checks whether the folder has a default file of the type container page.
   *
   * <p>
   *
  ...
  private boolean hasContainerPageDefaultFile(CmsResource folder) {
    ...
    try {
      CmsResource defaultFile =
          A_CmsUI.getCmsObject().readDefaultFile(folder, CmsResourceFilter.ONLY_VISIBLE_NO_DELETED);
  ...
  private FormLayout initForm() {
    ...
    m_targetPath.setCaption(
        CmsVaadinUtils.getMessageText(
            org.opencms.workplace.commons.Messages.GUI_COPY_MOVE_TARGET_0));
    ...
    if (m_dialogMode != DialogMode.move) {
      ...
      m_actionCombo.setCaption(
          CmsVaadinUtils.getMessageText(org.opencms.ui.Messages.GUI_COPYPAGE_COPY_MODE_0));
      ...
      if (m_context.getResources().size() == 1) {
        if (m_context.getResources().get(0).isFile()) {
          ...
          m_actionCombo.setItemCaption(
              Action.copy_all,
              CmsVaadinUtils.getMessageText(
                  org.opencms.workplace.commons.Messages.GUI_COPY_AS_NEW_0));
          m_actionCombo.addItem(Action.copy_sibling_all);
          m_actionCombo.setItemCaption(
              Action.copy_sibling_all,
              CmsVaadinUtils.getMessageText(
                  org.opencms.workplace.commons.Messages.GUI_CREATE_SIBLING_0));
      ...
      m_actionCombo.setItemStyleGenerator(
    ...
    if (m_context.getResources().size() > 1) {
      m_overwriteExisting =
          new CheckBox(
              CmsVaadinUtils.getMessageText(
                  org.opencms.workplace.commons.Messages.GUI_COPY_MULTI_OVERWRITE_0));
      m_overwriteExisting.setValue(Boolean.FALSE);
  ...
  /**
   * Displays the confirm overwrite dialog.
   *
   * <p>
   *
  ...
  private void showConfirmOverwrite(List<CmsResource> collidingResources) {
    ...
    window.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_COPY_MOVE_CONFIRM_OVERWRITE_TITLE_0));
    final CmsConfirmationDialog dialog =
  ...
  /**
   * Displays the resolve macro dialog.
   *
   * <p>
   *
  ...
  private void showMacroResolverDialog(CmsResource resource) {
    ...
    window.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_COPY_MOVE_SET_MACRO_VALUES_TITLE_0));
    m_macroDialog =
        new CmsMacroResolverDialog(
            new Runnable() {
              ...
              public void run() {
                ...
                submit(
                    true,
                    map); // Overwrite true because this was checked first. If no overwrite, window
                          // is closed and this code isn't called
              }
            },
            new Runnable() {
  ...
  /**
   * Updates the default dialog actions.
   *
   * <p>
   *
