...
public class CmsEmbeddedDialogContext implements I_CmsDialogContext {
   ...
   * <p>
   *
   * @param keepFrame <code>true</code> to keep the embedded iFrame.
   *     <p>
   */
    ...
    if ((project != null) || (siteRoot != null)) {
      ...
      if (siteRoot != null) {
        CmsQuickLaunchLocationCache locationCache =
            CmsQuickLaunchLocationCache.getLocationCache(A_CmsUI.get().getHttpSession());
  ...
  public void finish(SiteSelectorOption option) {
    ...
    if (path == null) {
      ...
      if (foundPage) {
        ...
        Page.getCurrent()
            .open(CmsCoreService.getFileExplorerLink(cms, option.getSite()) + sitePath, "_top");
      }
    }
  }
