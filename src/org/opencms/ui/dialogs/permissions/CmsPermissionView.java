...
public class CmsPermissionView extends CssLayout {
  ...
  final ValueChangeListener m_valueChangeListener =
  ...
  private final TableFieldFactory m_fieldFactory =
  ...
  public CmsPermissionView(
    ...
    if (!isOverwriteAll) {
      ...
      m_permissions.setColumnHeader(
          PROPERTY_LABEL, CmsVaadinUtils.getMessageText(Messages.GUI_PERMISSION_0));
      m_permissions.setColumnHeader(
          PROPERTY_ALLOWED, CmsVaadinUtils.getMessageText(Messages.GUI_PERMISSION_ALLOWED_0));
      m_permissions.setColumnHeader(
          PROPERTY_DISPLAY_ALLOWED,
          CmsVaadinUtils.getMessageText(Messages.GUI_PERMISSION_ALLOWED_0));
      m_permissions.setColumnHeader(
          PROPERTY_DENIED, CmsVaadinUtils.getMessageText(Messages.GUI_PERMISSION_DENIED_0));
      m_permissions.setColumnHeader(
          PROPERTY_DISPLAY_DENIED, CmsVaadinUtils.getMessageText(Messages.GUI_PERMISSION_DENIED_0));
      ...
      if (m_editable) {
        ...
        m_permissions.setVisibleColumns(
            PROPERTY_LABEL, PROPERTY_DISPLAY_ALLOWED, PROPERTY_DISPLAY_DENIED);
      }
    }
  }
  ...
  /**
   * Deletes the current permission set.
   *
   * <p>
   */
  ...
  /**
   * Generates the permissions data container.
   *
   * <p>
   *
  ...
  IndexedContainer getPermissionContainer(CmsPermissionSet permissions) {
    ...
    for (@RUntainted String key : CmsPermissionSet.getPermissionKeys()) {
      ...
      entry
          .getItemProperty(PROPERTY_DISPLAY_ALLOWED)
          .setValue(getCheckBoxLabel(isAllowed(permissions, flag)));
      entry.getItemProperty(PROPERTY_DENIED).setValue(isDenied(permissions, flag));
      entry
          .getItemProperty(PROPERTY_DISPLAY_DENIED)
          .setValue(getCheckBoxLabel(isDenied(permissions, flag)));
  ...
  void setPermissions() {
    ...
    m_changeHandler.setPermissions(
        new CmsPermissionBean(m_principalType, m_principalName, allowed, denied, flags));
  }

  /**
   * Generates a check box label.
   *
   * <p>
   *
