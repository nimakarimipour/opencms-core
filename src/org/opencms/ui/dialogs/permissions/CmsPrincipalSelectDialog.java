...
public class CmsPrincipalSelectDialog extends CmsBasicDialog {
   ...
   * @param roleSelectionAllowed if true, selecting roles should be allowed (if the widget type
   *     allows for roles)
   */
  public CmsPrincipalSelectDialog(
      I_CmsPrincipalSelect cmsPrincipalSelect,
    ...
    try {
      ...
      m_ouCombo =
          CmsVaadinUtils.getOUComboBox(
              m_cms, m_cms.getRequestContext().getOuFqn(), null, includeWebOus);
      ...
      m_table.setColumnHeader(
          ID_CAPTION, CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_GROUP_NAME_0));
      m_table.setColumnHeader(
          ID_DESC, CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_GROUP_DESCRIPTION_0));
      m_table.setColumnHeader(
          ID_OU, CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_GROUP_OU_0));
      ...
      m_tableFilter.setInputPrompt(
          Messages.get()
              ...
              .key(Messages.GUI_EXPLORER_FILTER_0));
    ...
    m_ouCombo.addValueChangeListener(
    ...
    m_typeCombo.addValueChangeListener(
        new ValueChangeListener() {
          ...
          public void valueChange(ValueChangeEvent event) {
            ...
            CmsPrincipalSelect.PrincipalType principalType =
                (CmsPrincipalSelect.PrincipalType) m_typeCombo.getValue();
  ...
  public static void openEmbeddedDialog(
      final CmsEmbeddedDialogContext dialogContext, Map<String, String[]> params) {
  ...
  public static void openEmbeddedDialog(
    ...
    CmsPrincipalSelectDialog dialog =
        new CmsPrincipalSelectDialog(
            null, ou, window, type, realOnly, startType, includeWebOus, false);
    dialog.setSelectHandler(
        new I_PrincipalSelectHandler() {
  ...
  /**
   * Returns the container for the currently selected Principal group.
   *
   * <p>
   *
   ...
   */
  private IndexedContainer getContainerForType(
      CmsPrincipalSelect.PrincipalType type, boolean realOnly, String ou) throws CmsException {
    ...
    if (type == PrincipalType.role) {
      ...
      List<CmsRoleAsPrincipal> roleWrappers =
          roles.stream().map(role -> new CmsRoleAsPrincipal(role)).collect(Collectors.toList());
      res =
          CmsVaadinUtils.getPrincipalContainer(
              A_CmsUI.getCmsObject(),
              roleWrappers,
  ...
  /**
   * Init ComboBox for choosing type of principal.
   *
   * <p>
   *
  ...
  private void initTypeCombo(
      CmsPrincipalSelect.PrincipalType defaultType, boolean roleSelectionAllowed) {
    ...
    item.getItemProperty("caption")
        .setValue(CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_USER_0));
    if (roleSelectionAllowed) {
      if ((m_type == null)
          || m_type.getPrincipalTypes().contains(CmsPrincipalSelect.PrincipalType.role)) {
