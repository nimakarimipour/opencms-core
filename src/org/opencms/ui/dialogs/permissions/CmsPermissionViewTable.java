...
public class CmsPermissionViewTable extends Table {
  ...
  class ViewColumn implements Table.ColumnGenerator {
    ...
    public Object generateCell(Table source, Object itemId, Object columnId) {
      ...
      return m_dialog.buildPermissionEntryForm(
          (CmsAccessControlEntry) itemId, m_editable, false, null);
    }
  }
    ...
    if (hasEntries) {
      ...
      while (i.hasNext()) {
        ...
        CmsPermissionView view =
            m_dialog.buildPermissionEntryForm(
                curEntry, m_editable, false, showRes ? curEntry.getResource() : null);
        Item item = m_container.addItem(view);
        item.getItemProperty(PROP_VIEW)
            .setValue(
                getLayoutFromEntry(
                    cms, curEntry, view, showRes ? parents.get(curEntry.getResource()) : null));
  ...
  /**
   * Filter the table according to string.
   *
   * <p>
   *
   * @param search string
  ...
  private VerticalLayout getLayoutFromEntry(
      CmsObject cms, CmsAccessControlEntry entry, final CmsPermissionView view, String resPath) {
