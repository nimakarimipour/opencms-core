...
public class CmsRestoreDeletedDialog extends CmsBasicDialog {
  ...
  public CmsRestoreDeletedDialog(I_CmsDialogContext context) throws CmsException {
    ...
    List<I_CmsHistoryResource> deletedResources =
        readDeletedResources(m_includeSubfoldersField.getValue().booleanValue());
    ...
    m_includeSubfoldersField.addValueChangeListener(
    ...
    m_selectAllField.addValueChangeListener(
        new ValueChangeListener() {
  ...
  public List<CmsUUID> getSelectedIds() {
    ...
    for (Object itemId : itemIds) {
      ...
      Boolean value =
          (Boolean)
              (m_selectionContainer.getItem(itemId).getItemProperty(PROP_SELECTED).getValue());
  ...
  /**
   * Called on include sub folders change.
   *
   * <p>
   *
   * @param value the new value
  ...
  /**
   * Fills the list of resources to select from.
   *
   * <p>
   *
  ...
  private void initDeletedResources(CmsObject cms, List<I_CmsHistoryResource> deletedResources)
    ...
    Collections.sort(
        deletedResources,
        new Comparator<I_CmsHistoryResource>() {
    ...
    if (deletedResources.isEmpty()) {
      m_deletedResourceContainer.addComponent(
          new Label(
              CmsVaadinUtils.getMessageText(org.opencms.workplace.list.Messages.GUI_LIST_EMPTY_0)));
    }
    for (I_CmsHistoryResource deleted : deletedResources) {
      ...
      CmsResourceInfo info =
          new CmsResourceInfo(
              title, subtitle, CmsResourceUtil.getBigIconResource(explorerType, deleted.getName()));
      info.setWidth("100%");
   ...
   * @throws CmsException if something goes wrong
   */
  private List<I_CmsHistoryResource> readDeletedResources(boolean includeSubFolders)
      throws CmsException {
