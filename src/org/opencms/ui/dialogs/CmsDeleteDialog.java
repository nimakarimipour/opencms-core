...
public class CmsDeleteDialog extends CmsBasicDialog {
  ...
  public CmsDeleteDialog(I_CmsDialogContext context) {
    ...
    m_deleteSiblings.addValueChangeListener(
  ...
  public static Multimap<CmsResource, CmsResource> getBrokenLinks(
      CmsObject cms, List<CmsResource> selectedResources, boolean includeSiblings)
      throws CmsException {
   ...
   */
  public static Multimap<CmsResource, CmsResource> getBrokenLinks(
      CmsObject cms, List<CmsResource> selectedResources, boolean includeSiblings, boolean reverse)
      throws CmsException {
    ...
    for (CmsResource root : selectedResources) {
      ...
      if (root.isFolder()) {
        descendants.addAll(
            cms.readResources(cms.getSitePath(root), CmsResourceFilter.IGNORE_EXPIRATION));
      }
    }
  ...
  void displayBrokenLinks() {
    ...
    try {
      Multimap<CmsResource, CmsResource> brokenLinks =
          getBrokenLinks(
              cms,
      ...
      if (brokenLinks.isEmpty()) {
        ...
        for (CmsResource source : brokenLinks.keySet()) {
          ...
          CmsGwtContextMenuButton contextMenu =
              new CmsGwtContextMenuButton(source.getStructureId(), rpc);
  ...
  /**
   * Checks whether the selected resources have siblings.
   *
   * <p>
   *
