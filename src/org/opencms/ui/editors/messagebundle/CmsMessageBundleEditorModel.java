 ...
 * writes the involved files and provides the contents as {@link IndexedContainer}.
 */
public class CmsMessageBundleEditorModel {
  ...
  public static final class CmsCaseInsensitiveStringComparator implements Comparator<Object> {
    ...
    private static CmsCaseInsensitiveStringComparator m_instance =
        new CmsCaseInsensitiveStringComparator();
  ...
  public static final class ConfigurableMessages {
     ...
     * @param configuredBundle the base name of the configured message bundle (can be <code>null
     *     </code>).
     */
    ...
    /**
     * Returns the message for the key, either from the configured bundle, or - if not found - from
     * the default bundle.
     *
  ...
  /**
   * Extension of {@link Properties} to allow saving with keys alphabetically ordered and without
   * time stamp as first comment.
   *
   * <p>NOTE: Can't handle comments. They are just discarded. NOTE: Most of the class is just a
   * plain copy of the private methods of {@link Properties}, so be aware that adjustments may be
   * necessary if the {@link Properties} implementation changes. NOTE: The solution was taken to
   * guarantee correct escaping when storing properties.
   */
  public static final class SortedProperties extends Properties {
    ...
    /**
     * NOTE: This is a one-to-one copy from {@link java.util.Properties} with HEX_DIGIT renamed to
     * HEX_DIGIT. A table of hex digits.
     */
    ...
    /**
     * NOTE: This is a one-to-one copy from {@link java.util.Properties} Convert a nibble to a hex
     * character.
     *
     ...
     * Converts unicodes to encoded &#92;uxxxx and escapes special characters with a preceding
     * slash.
     *
    ...
    /**
     * Adaption of {@link java.util.Properties#store0(BufferedWriter,String,boolean)}. The behavior
     * differs as follows:
     *
     * <ul>
     *   <li>Comments are not handled.
     *   <li>The time stamp comment is omited.
     *   <li>Messages are stored sorted alphabetically by key
     * </ul>
     *
  ...
  /**
   * The property for configuring the message bundle used for localizing the bundle descriptors
   * entries.
   */
   ...
   * @throws CmsException thrown if reading some of the involved {@link CmsResource}s is not
   *     possible.
  ...
  public CmsMessageBundleEditorModel(CmsObject cms, CmsResource resource)
    ...
    if (cms == null) {
      throw new CmsException(
          Messages.get().container(Messages.ERR_LOADING_BUNDLE_CMS_OBJECT_NULL_0));
    }
        ...
        if (!localesWithEntries.isEmpty()) {
          ...
          for (final Locale l : localesWithEntries.keySet()) {
            ...
            ContextMenuItem menuItem =
                mainItem.addItem(l.getDisplayName(UI.getCurrent().getLocale()));
            menuItem.addItemClickListener(
                new ContextMenuItemClickListener() {

                  public void contextMenuItemClicked(ContextMenuItemClickEvent event) {

                    item.getItemProperty(TableProperty.TRANSLATION)
                        .setValue(localesWithEntries.get(l));
                  }
                });
          }
        }
   ...
   * @param allLanguages <code>true</code> for changing the key for all languages, <code>false
   *     </code> if the key should be changed only for the current language.
  ...
  public KeyChangeResult handleKeyChange(EntryChangeEvent event, boolean allLanguages) {
    ...
    if (m_keyset.getKeySet().contains(event.getNewValue())) {
      m_container
          ...
          .getItemProperty(TableProperty.KEY)
  ...
  public boolean hasDescriptor() {
    ...
    return !m_bundleType.equals(CmsMessageBundleEditorTypes.BundleType.DESCRIPTOR)
        && (m_descContent != null);
  }
  ...
  /**
   * Returns a flag, indicating if the options column (with add and delete option for rows) should
   * be shown in the given edit mode.
   *
  ...
  public void publish() {
    ...
    I_CmsDialogContext context =
        new A_CmsDialogContext("", ContextType.appToolbar, resources) {
   ...
   *
   * @throws UnsupportedEncodingException thrown if localizations from the XML bundle could not be
   *     loaded correctly.
  ...
  public void saveAsPropertyBundle()
      throws UnsupportedEncodingException, CmsException, IOException {
  ...
  /**
   * Adjusts the locale for an already existing container by first saving the translation for the
   * current locale and the loading the values of the new locale.
   *
  ...
  /**
   * Initializes the IndexedContainer shown in the table for the current locale. Therefore, the
   * involved {@link CmsResource}s will be read, if not already done.
   *
  ...
  private IndexedContainer createContainer() throws IOException, CmsException {
    ...
    container.setItemSorter(
        new DefaultItemSorter(CmsCaseInsensitiveStringComparator.getInstance()));
    return container;
  }
   ...
   */
  private IndexedContainer createContainerForBundleWithDescriptor()
    ...
    CmsXmlContentValueSequence messages =
        m_descContent.getValueSequence(Descriptor.N_MESSAGE, Descriptor.LOCALE);
    String descValue;
   ...
   * @throws CmsException thrown if reading the bundle fails.
   */
  private IndexedContainer createContainerForBundleWithoutDescriptor()
      throws IOException, CmsException {
    ...
    for (int i = 0; i < messages.getElementCount(); i++) {
      ...
      String key =
          m_descContent
              ...
              .getStringValue(m_cms);
      item.getItemProperty(TableProperty.KEY).setValue(key);
      item.getItemProperty(TableProperty.DESCRIPTION)
          .setValue(
              m_descContent
                  .getValue(prefix + Descriptor.N_DESCRIPTION, Descriptor.LOCALE)
                  .getStringValue(m_cms));
      item.getItemProperty(TableProperty.DEFAULT)
          .setValue(
              m_descContent
                  .getValue(prefix + Descriptor.N_DEFAULT, Descriptor.LOCALE)
                  .getStringValue(m_cms));
  ...
  /**
   * Creates all propertyvfsbundle files for the currently loaded translations. The method is used
   * to convert xmlvfsbundle files into propertyvfsbundle files.
   *
   * @throws CmsIllegalArgumentException thrown if resource creation fails.
   * @throws CmsLoaderException thrown if the propertyvfsbundle type can't be read from the resource
   *     manager.
  ...
  /**
   * Returns a map with key property as key and item as value.
   *
   * <p>
   *
   * @return HashMap
  ...
  /**
   * Reads the current properties for a language. If not already done, the properties are read from
   * the respective file.
   *
   ...
   * @throws CmsException thrown when reading the resource fails or several bundle descriptors for
   *     the bundle exist.
   */
  private void initDescriptor() throws CmsXmlException, CmsException {
    ...
    if (m_bundleType.equals(CmsMessageBundleEditorTypes.BundleType.DESCRIPTOR)) {
      ...
      // First try to read from same folder like resource, if it fails use
      // CmsMessageBundleEditorTypes.getDescriptor()
      try {
        m_desc =
            m_cms.readResource(
                m_sitepath + m_basename + CmsMessageBundleEditorTypes.Descriptor.POSTFIX);
  ...
  /**
   * Initializes the editor states for the different modes, depending on the type of the opened
   * file.
   */
  private void initEditorStates() {
    ...
    switch (m_bundleType) {
      case PROPERTY:
      case XML:
        if (hasDescriptor()) { // bundle descriptor is present, keys are not editable in default
                               // mode, maybe master mode is available
          ...
          cols.add(TableProperty.TRANSLATION);
          m_editorState.put(
              CmsMessageBundleEditorTypes.EditMode.DEFAULT, new EditorState(cols, true));
        }
        break;
      case DESCRIPTOR:
  ...
  private void initHasMasterMode() throws CmsException {
    ...
    if (hasDescriptor()
        && m_cms.hasPermissions(
            m_desc, CmsPermissionSet.ACCESS_WRITE, false, CmsResourceFilter.ALL)) {
      m_hasMasterMode = true;
    } else {
      m_hasMasterMode = false;
    }
   ...
   */
  private void loadAllRemainingLocalizations()
      throws CmsException, UnsupportedEncodingException, IOException {
  ...
  /**
   * Loads the propertyvfsbundle for the provided locale. If the bundle file is not present, it will
   * be created.
   *
  ...
  /**
   * Loads the localization for the current locale from a bundle of type xmlvfsbundle. It assumes,
   * the content has already been unmarshalled before.
   *
   * @param locale the locale for which the localization should be loaded
   */
  ...
  /**
   * Locks all files of the currently edited bundle (that contain the provided key if it is not
   * null).
   *
   * @param key the key that must be contained in the localization to lock. If null, the files for
   *     all localizations are locked.
  ...
  /**
   * Remove a key for all language versions. If a descriptor is present, the key is only removed in
   * the descriptor.
   *
  ...
  /**
   * Replaces the translations in an existing container with the translations for the provided
   * locale.
   *
  ...
  private boolean replaceValues(@RUntainted Locale locale) {
    ...
    try {
      ...
      // The problem should typically be a problem with locking or reading the file containing the
      // translation.
    ...
    for (Locale l : m_changedTranslations) {
      ...
      if ((null != props) && (null != f)) {
        try {
          ...
          String contentEncodingProperty =
              m_cms
                  .readPropertyObject(file, CmsPropertyDefinition.PROPERTY_CONTENT_ENCODING, false)
                  .getValue();
          if ((null == contentEncodingProperty)
              || !contentEncodingProperty.equals(f.getEncoding())) {
  ...
  private void saveToXmlVfsBundle() throws CmsException {
    ...
    if (m_lockedBundleFiles.get(null)
          ...
          for (@RUntainted Object key : keys) {
            if ((null != key) && !key.toString().isEmpty()) {
              ...
              if (!value.isEmpty()) {
                ...
                i++;
                m_xmlBundle
                    .getValue("Message[" + i + "]/Key", l)
                    .setStringValue(m_cms, key.toString());
      ...
      case XML:
        m_locale =
            OpenCms.getLocaleManager()
                .getBestAvailableLocaleForXmlContent(m_cms, m_resource, m_xmlBundle);
        break;
      case DESCRIPTOR:
        m_basename =
            baseName.substring(
                0, baseName.length() - CmsMessageBundleEditorTypes.Descriptor.POSTFIX.length());
  ...
  private void unmarshalDescriptor() throws CmsXmlException, CmsException {
    ...
    if (null != m_desc) {
      ...
      CmsProperty bundleProp =
          m_cms.readPropertyObject(m_desc, PROPERTY_BUNDLE_DESCRIPTOR_LOCALIZATION, true);
   ...
   *
   * @throws CmsXmlException thrown if update fails due to a wrong XML structure (should never
   *     happen)
   */
  private void updateBundleDescriptorContent() throws CmsXmlException {
    ...
    for (@RUntainted Object key : keys) {
      if ((null != key) && !key.toString().isEmpty()) {
        ...
        m_descContent
            .getValue(messagePrefix + Descriptor.N_KEY, Descriptor.LOCALE)
            .setStringValue(m_cms, (String) key);
        ...
        if ((null != descProp) && (null != descProp.getValue())) {
          ...
          m_descContent
              .getValue(messagePrefix + Descriptor.N_DESCRIPTION, Descriptor.LOCALE)
        ...
        if ((null != defaultValueProp) && (null != defaultValueProp.getValue())) {
          ...
          m_descContent
              .getValue(messagePrefix + Descriptor.N_DEFAULT, Descriptor.LOCALE)
