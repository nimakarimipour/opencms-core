...
public class CmsMessageBundleEditorOptions {
  ...
  /**
   * The upper left component in the options grid (containing the language/mode switches and the
  ...
  /**
   * The lower right component in the options grid (containing the "Add key" input field and the
   * "Add key" button).
   */
  ...
  /**
   * Handles adding a key. Calls the registered listener and wraps it's method in some GUI
   * adjustments.
   */
  void handleAddKey() {
    ...
    if (m_listener.handleAddKey(key)) {
      Notification.show(
          key.isEmpty()
              ? m_messages.key(
                  Messages.GUI_NOTIFICATION_MESSAGEBUNDLEEDITOR_EMPTY_KEY_SUCCESSFULLY_ADDED_0)
              : m_messages.key(
                  Messages.GUI_NOTIFICATION_MESSAGEBUNDLEEDITOR_KEY_SUCCESSFULLY_ADDED_1, key));
    } else {
      CmsMessageBundleEditorTypes.showWarning(
          m_messages.key(
              Messages.GUI_NOTIFICATION_MESSAGEBUNDLEEDITOR_KEY_ALREADEY_EXISTS_CAPTION_0),
          m_messages.key(
              Messages.GUI_NOTIFICATION_MESSAGEBUNDLEEDITOR_KEY_ALREADEY_EXISTS_DESCRIPTION_1,
              key));
    }
    m_addKeyInput.focus();
  ...
  /**
   * Creates the upper right component of the options grid. Creation includes the initialization of
   * {@link #m_filePathField}.
   *
  ...
  private void initAddKeyInput() {
    ...
    final ShortcutListener shortCutListener =
        new ShortcutListener("Add key via ENTER", KeyCode.ENTER, null) {
    ...
    m_addKeyInput.addFocusListener(
        new FocusListener() {
  ...
  /**
   * Initializes the language switcher UI Component {@link #m_languageSwitch}, including {@link
   * #m_languageSelect}.
   *
        ...
        caption +=
            " ("
                + Messages.get()
                    ...
                    .key(Messages.GUI_DEFAULT_LOCALE_0)
                + ")";
      }
  ...
  /**
   * Initializes the lower left component {@link #m_lowerLeftComponent} with the correctly placed
   * "Add key"-label.
   */
  ...
  /**
   * Initializes the lower right component {@link #m_lowerRightComponent}, with all its components,
   * i.e., the "Add key" input field {@link #m_addKeyInput} and the "Add key" button.
   */
  ...
  private void initModeSwitch(final EditMode current) {
    ...
    m_modeSelect.addValueChangeListener(
        new ValueChangeListener() {
  ...
  /**
   * Creates the complete options component. It's a grid with two rows and two columns, styled like:
   *
   * <p>|| [language switch] [mode switch] File-Label || [file path display]
   * -------------------------------------------||---------------------------------- New-Key-Label
   * || [new key input] [add key button] ||
   *
   * <p>NOTE: The second row is not filled with components on initialization, what means keys can
   * not be added Filling is done via {@link #updateShownOptions(boolean, boolean)}
   */
