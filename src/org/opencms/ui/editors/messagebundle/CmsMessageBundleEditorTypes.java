...
public final class CmsMessageBundleEditorTypes {
  ...
  static class ComponentData implements Serializable {
     ...
     * @param lastValue the value in the field when it gets the focus, i.e., before a current edit
     *     operation.
     */
  ...
  /**
   * The editor state holds the information on what columns of the editors table should be editable
   * and if the options column should be shown. The state depends on the loaded bundle and the edit
   * mode.
   */
  ...
  static class EntryChangeEvent {
    ...
    public EntryChangeEvent(
        AbstractTextField source, Object itemId, Object propertyId, String oldKey, String newKey) {
  ...
  /**
   * A column generator that additionally adjusts the appearance of the options buttons to selection
  ...
  static class OptionColumnGenerator implements com.vaadin.v7.ui.Table.ColumnGenerator {
    ...
    public OptionColumnGenerator(final FilterTable table) {
      ...
      m_table = table;
      m_table.addValueChangeListener(
    ...
    /**
     * Registers an item deletion listener. Only one listener can be registered. Registering a new
     * listener will automatically unregister the previous one.
     *
    ...
    /**
     * Call this method, when a new item is selected. It will adjust the style of the option
     * buttons, thus that they stay visible.
     *
  ...
  static class TableKeyboardHandler implements Handler {
    ...
    /**
     * The field factory keeps track of the editable rows and the row/col positions of the
     * TextFields.
     */
    ...
    private Action m_tabPrev =
        new ShortcutAction(
            "Shift+Tab", ShortcutAction.KeyCode.TAB, new int[] {ShortcutAction.ModifierKey.SHIFT});
    /** Down was pressed. */
    ...
    public void handleAction(Action action, Object sender, Object target) {
      ...
      TranslateTableFieldFactory fieldFactory =
          (TranslateTableFieldFactory) m_table.getTableFieldFactory();
      ...
      if (target instanceof AbstractTextField) {
        ...
        if ((action == m_curDown) || (action == m_enter)) {
          ...
          if (colId <= previousColId) {
            ...
            rowId =
                visibleItemIds
                    .get(((currentRow + visibleItemIds.size()) - 1) % visibleItemIds.size())
                    .intValue();
          }
          colId = previousColId;
        }

        AbstractTextField newTF =
            fieldFactory.getValueFields().get(Integer.valueOf(colId)).get(Integer.valueOf(rowId));
        if (newTF != null) {
          newTF.focus();
        }
      }
    ...
    private int getNextColId(List<TableProperty> editableColumns, int colId) {
      ...
      for (int i = colId % editableColumns.size();
          i != (colId - 1);
    ...
    private int getPreviousColId(List<TableProperty> editableColumns, int colId) {
      ...
      for (int i = ((colId + editableColumns.size()) - 2) % editableColumns.size();
          i != (colId - 1);
  ...
  static class TranslateTableCellStyleGenerator implements CellStyleGenerator {
    ...
    /**
     * @see com.vaadin.ui.CustomTable.CellStyleGenerator#getStyle(com.vaadin.ui.CustomTable,
  ...
  static class TranslateTableFieldFactory extends DefaultFieldFactory {
    ...
    private final Set<I_EntryChangeListener> m_keyChangeListeners =
        new HashSet<I_EntryChangeListener>();
    ...
    /**
     * @see com.vaadin.ui.TableFieldFactory#createField(com.vaadin.data.Container, java.lang.Object,
        ...
        if (pid.equals(m_editableColumns.get(i - 1))) {
          ...
          tf.setInputPrompt(
              Messages.get()
                  ...
                  .key(Messages.GUI_PLEASE_ADD_VALUE_0));
              ...
              new BlurListener() {
                ...
                public void blur(BlurEvent event) {
                  ...
                  if (!data.getLastValue().equals(field.getValue())) {
                    EntryChangeEvent ev =
                        new EntryChangeEvent(
                            field, data.getItemId(), pid, data.getLastValue(), field.getValue());
  ...
  public static CmsResource getDescriptor(CmsObject cms, String basename) {
    ...
    query.setFilterQueries(
        "filename:\"" + basename + CmsMessageBundleEditorTypes.Descriptor.POSTFIX + "\"");
    query.add("fl", "path");
    ...
    try {
      ...
      results =
          OpenCms.getSearchManager()
              .getIndexSolr(indexName)
              .search(cms, query, true, null, true, null);
    ...
    switch (results.size()) {
      ...
      default:
        ...
        LOG.warn(
            Messages.get().getBundle().key(Messages.ERR_BUNDLE_DESCRIPTOR_NOT_UNIQUE_1, files));
        return results.get(0);
    }
  }
