...
public class CmsSourceEditor
  ...
  public static class EditorSettings implements I_CmsAppSettings {
    ...
    public void restoreSettings(String storedSettings) {
      ...
      try {
        ...
        //       LOG.error("Failed to restore file explorer settings from '" + storedSettings + "'",
        // e);
      }
    }
  }
  ...
  private static final String[] FONT_SIZES =
      new String[] {"8px", "10px", "12px", "14px", "16px", "18px", "20px"};
  ...
  public void initUI(
    ...
    m_saveAndExit =
        CmsToolBar.createButton(
            FontOpenCms.SAVE_EXIT, messages.key(Messages.GUI_BUTTON_SAVE_AND_EXIT_0), true);
    m_saveAndExit.addClickListener(
        new ClickListener() {
    ...
    m_save =
        CmsToolBar.createButton(FontOpenCms.SAVE, messages.key(Messages.GUI_BUTTON_SAVE_0), true);
    m_save.addClickListener(
        new ClickListener() {
    ...
    Button undo =
        CmsToolBar.createButton(FontOpenCms.UNDO, messages.key(Messages.GUI_BUTTON_UNDO_0), true);
    context.addToolbarButton(undo);

    Button redo =
        CmsToolBar.createButton(FontOpenCms.REDO, messages.key(Messages.GUI_BUTTON_REDO_0), true);
    ...
    Button search =
        CmsToolBar.createButton(
            FontOpenCms.SEARCH, messages.key(Messages.GUI_BUTTON_SEARCH_0), true);
    context.addToolbarButton(search);

    Button replace =
        CmsToolBar.createButton(
            FontOpenCms.SEARCH_REPLACE, messages.key(Messages.GUI_BUTTON_REPLACE_0), true);
    ...
    try {
      settings =
          OpenCms.getWorkplaceAppManager()
              .getAppSettings(A_CmsUI.getCmsObject(), EditorSettings.class);

    } catch (Exception e) {
      settings = new EditorSettings();
    }

    final Button toggleHighlight =
        CmsToolBar.createButton(
            FontOpenCms.HIGHLIGHT, messages.key(Messages.GUI_BUTTON_TOGGLE_HIGHLIGHTING_0));
    toggleHighlight.addClickListener(
        new ClickListener() {
    ...
    final Button toggleLineWrap =
        CmsToolBar.createButton(
            FontOpenCms.WRAP_LINES, messages.key(Messages.GUI_BUTTON_TOGGLE_LINE_WRAPPING_0));
    toggleLineWrap.addClickListener(
        new ClickListener() {
    ...
    final Button toggleBrackets =
        CmsToolBar.createButton(
            FontOpenCms.BRACKETS, messages.key(Messages.GUI_BUTTON_TOBBLE_BRACKET_AUTOCLOSE_0));
    toggleBrackets.addClickListener(
        new ClickListener() {
    ...
    final Button toggleTabs =
        CmsToolBar.createButton(
            FontOpenCms.INVISIBLE_CHARS, messages.key(Messages.GUI_BUTTON_TOGGLE_TAB_VISIBILITY_0));
    toggleTabs.addClickListener(
        new ClickListener() {
    ...
    fontSizeSelect.addValueChangeListener(
        new ValueChangeListener() {
    ...
    m_exit =
        CmsToolBar.createButton(FontOpenCms.EXIT, messages.key(Messages.GUI_BUTTON_CANCEL_0), true);
    m_exit.addClickListener(
        new ClickListener() {
    ...
    m_codeMirror.addValueChangeListener(
        new ValueChangeListener() {
  ...
  /**
   * Unlocks the edited file before leaving the editor.
   *
   * <p>
   */
  void clear() {
    ...
    m_file.tryUnlock();
    OpenCms.getWorkplaceAppManager()
        .storeAppSettings(A_CmsUI.getCmsObject(), EditorSettings.class, getCurrentSettings());
  }

  /**
   * Exits the editor without saving.
   *
   * <p>Will ask to confirm exit on changed contents.
   *
   * <p>
   */
  void exit() {
  ...
  /**
   * Saves the current editor content and leaves the editor.
   *
   * <p>
   */
  void saveAndExit() {
