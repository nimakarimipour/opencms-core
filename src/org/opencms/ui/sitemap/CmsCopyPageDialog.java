...
public class CmsCopyPageDialog extends CmsBasicDialog {
  ...
  public class TargetInfo {
    ...
    /**
     * Gets the target name, or null if only the target folder was given in the target select
     * widget.
     *
     * <p>
     *
    ...
    /**
     * Helper method for reading a resource 'safely', i.e. without logging errors when it fails.
     *
     * <p>
     *
  ...
  void onClickOk() {
    ...
    try {
      CmsContainerPageCopier.CopyMode mode =
          (CmsContainerPageCopier.CopyMode) (m_copyMode.getValue());
      ...
      String errorMessage =
          CmsVaadinUtils.getMessageText(
              Messages.GUI_COPYPAGE_NO_REPLACEMENT_FOUND_1, e.getResource().getRootPath());
      CmsErrorDialog.showErrorDialog(errorMessage, e);
    }
  }

  /**
   * Gets the initial target path to display, based on the selected resource.
   *
   * <p>
   *
   * @param cms the cms context
