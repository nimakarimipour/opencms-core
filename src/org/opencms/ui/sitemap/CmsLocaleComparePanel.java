...
public class CmsLocaleComparePanel extends VerticalLayout implements I_CmsLocaleCompareContext {
  ...
  /**
   * Gets the locales selectable as comparison locales.
   *
   * <p>
   *
  ...
  public void initialize(CmsUUID id, Locale initialComparisonLocale) throws CmsException {
    ...
    List<Locale> possibleLocaleSelections =
        getMainLocaleSelectOptions(cms, res, mainLocale, secondaryLocales);
    ...
    for (Locale selectableLocale : possibleLocaleSelections) {
      ...
      m_rootLocaleSelector.setItemCaption(
          selectableLocale, selectableLocale.getDisplayName(A_CmsUI.get().getLocale()));
    }
    ...
    for (Locale comparisonLocale : comparisonLocales) {
      ...
      m_comparisonLocaleSelector.setItemCaption(
          comparisonLocale, comparisonLocale.getDisplayName(A_CmsUI.get().getLocale()));
      ...
      if ((initialComparisonLocale != null)
        ...
        // if an initial comparison locale is given, it should have priority over the first
        // comparison locale
        selectedComparisonLocale = comparisonLocale;
      }
    }
    ...
    m_rootLocaleSelector.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_LOCALECOMPARE_MAIN_LOCALE_0));
  ...
  /**
   * Switches the root locale to the given value.
   *
   * <p>
   *
  ...
  protected void initTree(CmsResource rootRes) throws CmsException {
    ...
    showHeader();
    CmsSitemapTreeController controller =
        new CmsSitemapTreeController(A_CmsUI.getCmsObject(), rootRes, this, m_treeContainer);
  ...
  /**
   * Gets the possible locale values selectable as main locale.
   *
   * <p>
   *
  ...
  private List<Locale> getMainLocaleSelectOptions(
      CmsObject cms, CmsResource res, Locale mainLocale, List<Locale> secondaryLocales) {
  ...
  /**
   * Shows the header for the currently selected sitemap root.
   *
   * <p>
   *
   * @throws CmsException if something goes wrong
  ...
  private void showHeader() throws CmsException {
    ...
    path =
        OpenCms.getLinkManager()
            .getServerLink(cms, cms.getRequestContext().removeSiteRoot(targetRes.getRootPath()));
    String iconClasses =
        CmsIconUtil.getIconClasses(
            CmsIconUtil.getDisplayType(cms, getRoot()), getRoot().getName(), false);
