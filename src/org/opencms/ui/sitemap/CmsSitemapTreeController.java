...
public class CmsSitemapTreeController {
  ...
  /**
   * Menu entry for opening the explorer.
   *
   * <p>
   */
    ...
    public void executeAction(MenuContext context) {
      ...
      String link =
          CmsCoreService.getVaadinWorkplaceLink(
              A_CmsUI.getCmsObject(), context.getData().getResource().getStructureId());
  ...
  /**
   * Menu entry for opening the info dialog.
   *
   * <p>
   */
  class EntryInfo implements I_CmsSimpleContextMenuEntry<MenuContext> {
    ...
    public void executeAction(MenuContext context) {
      ...
      infoAction.executeAction(
          new DialogContext(context.getData().getResource(), context.getNode()));
    }
  ...
  /**
   * Menu entry for opening the 'LInk locale' dialog.
   *
   * <p>
   */
      ...
      try {
        DialogContext dialogContext =
            new DialogContext(
                ...
                data.getNode());
        CmsLocaleLinkTargetSelectionDialog dialog =
            new CmsLocaleLinkTargetSelectionDialog(dialogContext, m_localeContext);
        dialogContext.start(
  ...
  /**
   * Context menu entry for the 'Do not translate' mark.
   *
   * <p>
   */
  class EntryMark implements I_CmsSimpleContextMenuEntry<MenuContext> {
    ...
    public void executeAction(MenuContext context) {
      ...
      try {
        ...
        CmsResource primary =
            A_CmsUI.getCmsObject()
                .readResource(entry.getClientEntry().getId(), CmsResourceFilter.IGNORE_EXPIRATION);
        if (primary.isFolder()) {
          CmsResource defaultFile =
              A_CmsUI.getCmsObject().readDefaultFile(primary, CmsResourceFilter.IGNORE_EXPIRATION);
          if (defaultFile != null) {
            primary = defaultFile;
          }
        }
        ...
        if (fileToModify2.isFolder()) {
          try {
            fileToModify2 =
                A_CmsUI.getCmsObject()
                    .readDefaultFile(fileToModify2, CmsResourceFilter.IGNORE_EXPIRATION);
          } catch (CmsException e) {
            LOG.error(e.getLocalizedMessage(), e);
          }
        }
        ...
        CmsProperty prop =
            cms.readPropertyObject(
                fileToModify2, CmsPropertyDefinition.PROPERTY_LOCALE_NOTRANSLATION, false);
        ...
        if (!currentLocales.contains(m_localeContext.getComparisonLocale())) {
          ...
          CmsProperty newProp =
              new CmsProperty(
                  CmsPropertyDefinition.PROPERTY_LOCALE_NOTRANSLATION, newPropValue, null);
          cms.writePropertyObjects(fileToModify2, Arrays.asList(newProp));
          DialogContext dialogContext =
              new DialogContext(
                  A_CmsUI.getCmsObject()
                      .readResource(
                          entry.getClientEntry().getId(), CmsResourceFilter.IGNORE_EXPIRATION),
                  node);
  ...
  class EntryProperties implements I_CmsSimpleContextMenuEntry<MenuContext> {
    ...
    public void executeAction(MenuContext context) {
      ...
      ((CmsSitemapUI) A_CmsUI.get())
          .getSitemapExtension()
          .openPropertyDialog(
              context.getData().getResource().getStructureId(), m_root.getStructureId());
    }
  ...
  class EntryUnlink implements I_CmsSimpleContextMenuEntry<MenuContext> {
    ...
    public CmsMenuItemVisibilityMode getVisibility(final MenuContext context) {
      ...
      try {
        ...
        for (Locale testLocale :
            Arrays.asList(m_localeContext.getRootLocale(), m_localeContext.getComparisonLocale())) {
  ...
  /**
   * Context object for the context menu.
   *
   * <p>
   */
  class MenuContext {
    ...
    /**
     * Checks if the currently selected locale is the main locale.
     *
     * <p>
     *
    ...
    public boolean isMainLocale() {
      ...
      return m_localeContext
          .getRootLocale()
          .equals(
              A_CmsUI.getCmsObject()
                  .getLocaleGroupService()
                  .getMainLocale(m_localeContext.getRoot().getRootPath()));
    }
  }
  ...
  public CmsSitemapTreeController(
      CmsObject cms, CmsResource root, I_CmsLocaleCompareContext context, Component parent) {
  ...
  public CmsSitemapTreeNode createNode(final CmsSitemapTreeNodeData entry) {
    ...
    Resource icon =
        CmsResourceIcon.getSitemapResourceIcon(
            A_CmsUI.getCmsObject(), entry.getResource(), IconMode.localeCompare);
    CmsResourceInfo info =
        new CmsResourceInfo(
            entry.getClientEntry().getTitle(), entry.getClientEntry().getSitePath(), icon);
    info =
    ...
    main.setCommand(
        new Command() {
          ...
          public void menuSelected(MenuItem selectedItem) {
            ...
            List<I_CmsSimpleContextMenuEntry<MenuContext>> entries =
                Arrays.asList(
                    new EntryOpen(),
                    new EntryExplorer(),
                    new EntryProperties(),
    ...
    if (entry.isLinked()) {
      CmsSite site =
          OpenCms.getSiteManager().getSiteForRootPath(m_localeContext.getRoot().getRootPath());
      CmsResourceInfo linkedInfo =
          CmsResourceInfo.createSitemapResourceInfo(
              readSitemapEntryFolderIfPossible(entry.getLinkedResource()), site);
      ...
      linkedInfo.setData(
          "linked"); // Data used by click handler to distinguish clicked resource icons
      linkedInfo
          .getResourceIcon()
          .setDescription(CmsVaadinUtils.getMessageText(Messages.GUI_LOCALECOMPARE_OPEN_PAGE_0));
      ...
      if (noTranslation) {
        ...
        String topMessage =
            CmsVaadinUtils.getMessageText(Messages.GUI_LOCALECOMPARE_NO_TRANSLATION_TOP_0);
        String bottomMessage =
            CmsVaadinUtils.getMessageText(Messages.GUI_LOCALECOMPARE_NO_TRANSLATION_BOTTOM_0);
  ...
  public void onClickOpen(CmsSitemapTreeNode node) {
    ...
    if (node.isOpen()) {
      ...
      if (!m_alreadyLoaded.containsKey(node)) {
        ...
        List<CmsSitemapTreeNodeData> children =
            m_treeDataProvider.getChildren((CmsSitemapTreeNodeData) nodeData);
  ...
  public void openPageCopyDialog(CmsSitemapTreeNode node, CmsSitemapTreeNodeData entry) {
    ...
    try {
      CmsResource resource =
          cms.readResource(entry.getClientEntry().getId(), CmsResourceFilter.IGNORE_EXPIRATION);
  ...
  public void updateNode(CmsSitemapTreeNode node) {
    ...
    try {
      CmsSitemapTreeNodeData changedData =
          m_treeDataProvider.getData(
              A_CmsUI.getCmsObject()
                  .readResource(
                      data.getClientEntry().getId(), CmsResourceFilter.IGNORE_EXPIRATION));
  ...
  /**
   * If the given resource is the default file of a sitmeap entry folder, then returns that folder,
  ...
  /**
   * Gets the logger for the tree controller.
   *
   * <p>
   *
   * @return the logger
  ...
  /**
   * Opens the page corresponding to a sitemap entry.
   *
   * <p>
   *
  ...
  private void openTargetPage(CmsSitemapTreeNodeData nodeData, boolean second) {
    ...
    try {
      CmsResource resource =
          A_CmsUI.getCmsObject().readResource(targetId, CmsResourceFilter.IGNORE_EXPIRATION);
      ...
      final boolean sameSite =
          mySiteRoot.equals(OpenCms.getSiteManager().getSiteRoot(resource.getRootPath()));
      ...
      if (sameSite) {
        ...
        String message =
            CmsVaadinUtils.getMessageText(
                Messages.GUI_LOCALECOMPARE_SHOW_WRONGSITE_1, resource.getRootPath());
