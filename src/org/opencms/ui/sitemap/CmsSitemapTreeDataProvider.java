...
public class CmsSitemapTreeDataProvider {
   ...
   */
  public CmsSitemapTreeDataProvider(
      CmsObject cms, CmsResource root, I_CmsLocaleCompareContext context) {
    m_root = root;
    m_localeContext = context;
    try {
      m_cms = OpenCms.initCmsObject(cms);
      List<CmsResource> folders =
          m_cms.readResources(root, CmsResourceFilter.IGNORE_EXPIRATION.addRequireFolder(), true);
      Collections.sort(
          folders,
      ...
      folders.add(
          null); // add null as a dummy value so that in the loop below, lastFolder takes all real
                 // folders as values
      ...
      for (CmsClientSitemapEntry subEnt : ent.getSubEntries()) {
        if (subEnt.isInNavigation()
          ...
          try {
            ...
            Locale l1 =
                OpenCms.getLocaleManager()
                    .getDefaultLocale(
                        svc.getCmsObject(), svc.getCmsObject().readResource(idToRead));
            Locale l2 =
                OpenCms.getLocaleManager()
                    .getDefaultLocale(
                        svc.getCmsObject(),
                        svc.getCmsObject()
                            .readResource(ent.getId(), CmsResourceFilter.IGNORE_EXPIRATION));
            if (!l1.equals(l2)) {
              continue;
            }
          } catch (Exception e) {
          ...
          CmsSitemapTreeNodeData data =
              new CmsSitemapTreeNodeData(
                  m_localeContext.getRootLocale(), m_localeContext.getComparisonLocale());
  ...
  public CmsSitemapTreeNodeData getData(CmsResource resource) {
    ...
    try {
      ...
      CmsClientSitemapEntry ent =
          svc.getChildren(resource.getRootPath(), resource.getStructureId(), 0);
      CmsSitemapTreeNodeData data =
          new CmsSitemapTreeNodeData(
              m_localeContext.getRootLocale(), m_localeContext.getComparisonLocale());
