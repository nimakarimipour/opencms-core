...
public class CmsLocaleLinkTargetSelectionDialog extends CmsResourceSelectDialog {
   ...
   */
  public CmsLocaleLinkTargetSelectionDialog(
      I_CmsDialogContext context, I_CmsLocaleCompareContext localeContext) throws CmsException {
    ...
    if (realFile.isFolder()) {
      CmsResource defaultFile =
          context.getCms().readDefaultFile(realFile, CmsResourceFilter.IGNORE_EXPIRATION);
      if (defaultFile != null) {
    ...
    IndexedContainer siteData =
        (IndexedContainer) getContents().getSiteSelector().getContainerDataSource();
    ...
    for (Map.Entry<@RUntainted Locale, CmsResource> entry : resourcesByLocale.entrySet()) {
      ...
      index++;
      item.getItemProperty(getContents().getSiteSelector().getItemCaptionPropertyId())
          .setValue(
              CmsVaadinUtils.getMessageText(
                  Messages.GUI_LOCALECOMPARE_LOCALE_LABEL_1, localeKey.getDisplayLanguage()));
    }

    addSelectionHandler(
        new I_CmsSelectionHandler<CmsResource>() {
            ...
            new Predicate<Item>() {
              ...
              public boolean apply(Item item) {
                ...
                CmsResource resource =
                    (CmsResource)
                        (item.getItemProperty(CmsResourceTreeContainer.PROPERTY_RESOURCE)
                            .getValue());
                CmsResource srcResource = m_context.getResources().get(0);
                switch (A_CmsUI.getCmsObject()
                    .getLocaleGroupService()
                    .checkLinkable(srcResource, resource)) {
                  case linkable:
                    return true;
                  default:
                    return false;
