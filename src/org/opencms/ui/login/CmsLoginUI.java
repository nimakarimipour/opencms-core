...
public class CmsLoginUI extends A_CmsUI {
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(encryptedTarget)) {
        target =
            CmsLoginController.getLoginTarget(
                cms, getWorkplaceSettings(cms, request.getSession()), null);
      } else {
        try {
    ...
    try {
      ...
      String vaadinBootstrap =
          CmsStringUtil.joinPaths(
              context, "VAADIN/vaadinBootstrap.js?v=" + OpenCms.getSystemInfo().getVersionNumber());
      ...
      resolver.addMacro(
          "workplaceCss", workplaceCssBuffer != null ? workplaceCssBuffer.toString() : "");
   ...
   */
  public static String generateLoginHtmlFragment(CmsObject cms, VaadinRequest request)
      throws IOException {
    ...
    try {
      ...
      String validationResult =
          validator.validateToken(
              A_CmsUI.getCmsObject(), authToken, OpenCms.getLoginManager().getTokenLifetime());
      if (validationResult == null) {
        ...
        A_CmsUI.get()
            .setError(
                Messages.get()
                    .getBundle(A_CmsUI.get().getLocale())
                    .key(Messages.GUI_PWCHANGE_INVALID_TOKEN_0));
  ...
  protected void init(VaadinRequest request) {
    ...
    LoginParameters params =
        (LoginParameters) (request.getWrappedSession().getAttribute(INIT_DATA_SESSION_ATTR));
    if (params == null) {
      params =
          CmsLoginHelper.getLoginParameters(getCmsObject(), (HttpServletRequest) request, true);
        ...
        .setTitle(
            CmsAppWorkplaceUi.WINDOW_TITLE_PREFIX
                + CmsVaadinUtils.getMessageText(
                    org.opencms.workplace.Messages.GUI_LOGIN_HEADLINE_0));
