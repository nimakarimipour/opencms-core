...
public class CmsLoginHelper extends CmsJspLoginBean {
   ...
   */
  public static String getDirectEditPath(
      CmsObject cms, CmsUserSettings userSettings, boolean forceDirectEdit) {
    ...
    if (forceDirectEdit
      ...
      try {
        ...
        CmsResource targetRes =
            cloneCms.readDefaultFile(folder, CmsResourceFilter.ONLY_VISIBLE_NO_DELETED);
        if (targetRes != null) {
  ...
  public static LoginParameters getLoginParameters(
    ...
    String requestedResource =
        CmsRequestUtil.getNotEmptyParameter(
            request, CmsWorkplaceManager.PARAM_LOGIN_REQUESTED_RESOURCE);
    boolean validRequestedResource = false;
    if (!CmsStringUtil.isEmptyOrWhitespaceOnly(requestedResource)) {
      String encryptedRequestedResource =
          request.getParameter(CmsDefaultAuthorizationHandler.PARAM_ENCRYPTED_REQUESTED_RESOURCE);
      try {
        String decryptedResource =
            OpenCms.getDefaultTextEncryption().decrypt(encryptedRequestedResource);
    ...
    return new LoginParameters(
        username, pcType, oufqn, requestedResource, locale, authToken, logout, reset);
  }
  ...
  /**
   * Returns the pc type of the current request.
   *
   * <p>
   *
   * @param request the request
  ...
  /**
   * Returns the pre defined ou fqn.
   *
   * <p>
   *
   * @param cms the cms context
