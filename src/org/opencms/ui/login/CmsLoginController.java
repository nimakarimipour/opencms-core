...
public class CmsLoginController {
   ...
   *
   * <p>This is only used for detecting whether we need to display the user agreement dialog, not
   * for displaying the dialog itself.
   *
   * <p>
   */
  protected static class UserAgreementHelper extends CmsLoginUserAgreement {
    ...
    /**
     * @see
     *     org.opencms.workplace.CmsWorkplace#initWorkplaceMembers(org.opencms.jsp.CmsJspActionElement)
     */
    @Override
  ...
  /**
   * Helper interface for splitting the actual login off into its own object which can be called
  ...
  public static String getLoginTarget(
    ...
    String directEditPath =
        CmsLoginHelper.getDirectEditPath(currentCms, settings.getUserSettings(), false);
    String target = "";
    ...
    if ((requestedResource == null) && (directEditPath != null)) {
      target = directEditPath;
        ...
        && !CmsWorkplace.JSP_WORKPLACE_URI.equals(requestedResource)) {
    ...
    if (showUserAgreement) {
      target =
          userAgreementHelper.getConfigurationVfsPath()
              + "?"
      ...
      if (CmsSitemapEditorConfiguration.APP_ID.equals(fragment)) {
        target =
            OpenCms.getLinkManager()
                    .substituteLink(currentCms, CmsADEManager.PATH_SITEMAP_EDITOR_JSP)
                + "?path="
   ...
   *
   * <p>Important: This works only within vaadin apps.
   *
   * <p>
   */
    ...
    String result =
        OpenCms.getLinkManager()
                .substituteLinkForUnknownTarget(
                    CmsLoginUI.m_adminCms, CmsWorkplaceLoginHandler.LOGIN_HANDLER, false)
            + "?"
    ...
    try {
      try {
        ...
        CmsMessageContainer message =
            org.opencms.workplace.Messages.get()
                .container(org.opencms.workplace.Messages.GUI_LOGIN_FAILED_0);
      ...
      if (OpenCms.getLoginManager().canLockBecauseOfInactivity(currentCms, userObj)) {
        ...
        if (locked) {
          displayError(
              CmsInactiveUserMessages.getLockoutText(A_CmsUI.get().getLocale()), false, false);
          CmsUserLog.logLoginFailure(currentCms, realUser);
          return;
        }
      }
      ...
      if (!CmsStringUtil.isEmptyOrWhitespaceOnly(messageToChange)) {
        CmsChangePasswordDialog passwordDialog =
            new CmsChangePasswordDialog(currentCms, userObj, A_CmsUI.get().getLocale());
        passwordDialog.setAdditionalMessage(messageToChange);
        A_CmsUI.get()
            .setContentToDialog(
                Messages.get()
                        .getBundle(A_CmsUI.get().getLocale())
                        .key(Messages.GUI_PWCHANGE_HEADER_0)
      ...
      LoginContinuation loginContinuation =
          (LoginContext details) -> {
            ...
            // we use another separate CmsObject so we can manually control when it is written to
            // the session info
            ...
            final String loginTarget =
                getLoginTarget(loginCms, settings, m_params.getRequestedResource());
            ...
            if (OpenCms.getLoginManager().requiresUserDataCheck(loginCms, userNonNull)) {
              I_CmsDialogContext context =
                  new A_CmsDialogContext("", ContextType.appToolbar, null) {
  ...
  protected String getPasswordChangeMessage() {
    ...
    try {
      bundle =
          CmsResourceBundleLoader.getBundle(
              "org.opencms.passwordchange.custom", A_CmsUI.get().getLocale());
  ...
  protected void handleError(CmsObject currentCms, String user, Exception e) {
    ...
    if (e instanceof CmsException) {
      ...
      if (org.opencms.security.Messages.ERR_LOGIN_FAILED_DISABLED_2 == exceptionMessage.getKey()) {
        ...
        message =
            org.opencms.workplace.Messages.get()
                .container(org.opencms.workplace.Messages.GUI_LOGIN_FAILED_DISABLED_0);
      } else if (org.opencms.security.Messages.ERR_LOGIN_FAILED_TEMP_DISABLED_4
          == exceptionMessage.getKey()) {
        // the user account is temporarily disabled because of too many login failures
        message =
            org.opencms.workplace.Messages.get()
                .container(org.opencms.workplace.Messages.GUI_LOGIN_FAILED_TEMP_DISABLED_0);
   ...
   *
   * <p>This needs to be called in the <em>last</em> request to the Vaadin servlet in the login
   * process, because switching the session breaks the Vaadin session state.
   *
  ...
  protected void initSessionAndSendMessages(CmsObject currentCms, CmsObject loginCms) {
    ...
    HttpSession session =
        ((HttpServletRequest) VaadinService.getCurrentRequest()).getSession(false);
    if (session != null) {
      session.invalidate();
    }
    session = ((HttpServletRequest) VaadinService.getCurrentRequest()).getSession(true);

    // we don't want currentCms to be used to automatically update the session at the end of the
    // request...
    ...
    OpenCms.getSessionManager()
        .updateSessionInfo(loginCms, (HttpServletRequest) VaadinService.getCurrentRequest());
    ...
    if (storedMessage != null) {
      OpenCms.getSessionManager()
          .sendBroadcast(
              null, storedMessage, loginCms.getRequestContext().getCurrentUser(), ContentMode.html);
    }
  }
   ...
   * @param loginContinuation the handler to which we pass the verification code to continue with
   *     the login
   */
  private void showSecondFactorDialog(LoginContext context, LoginContinuation loginContinuation) {
    ...
    CmsSecondFactorDialog dialog =
        new CmsSecondFactorDialog(
            context.getUser(),
            verificationCode -> {
   ...
   *
   * @param context the login context
   * @param loginContinuation the handler we call with the secret and verification code to set up
   *     2FA and proceed with the loin.
   */
  private void showSecondFactorSetup(LoginContext context, LoginContinuation loginContinuation) {
    ...
    window.setClosable(false);
    window.setResizable(false);
    CmsSecondFactorSetupDialog dialog =
        new CmsSecondFactorSetupDialog(
            context,
            context2 -> {
              try {
                loginContinuation.continueLogin(context);
