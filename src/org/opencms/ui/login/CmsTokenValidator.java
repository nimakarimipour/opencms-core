 ...
 *
 * <p>When a user requests a link to change his password, an authorization token is generated and
 * also stored in the user's additional info (in a slightly different form). When the user opens the
 * 'change password' link sent to him, the authentication token is validated by comparing it to the
 * token in the user's additional info. Additionally, the system checks whether the age of the token
 * stored in the additional infos is older than the maximum age, which can also be configured.
 */
public class CmsTokenValidator {
   ...
   * @throws CmsException if something goes wrong
   */
  public static String createToken(CmsObject cms, CmsUser user, long currentTime)
      throws CmsException {
