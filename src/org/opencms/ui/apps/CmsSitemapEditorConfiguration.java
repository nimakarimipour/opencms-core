...
public class CmsSitemapEditorConfiguration extends A_CmsWorkplaceAppConfiguration
  ...
  public I_CmsWorkplaceApp getAppInstance() {
    ...
    throw new IllegalStateException(
        "The sitemap app should be launched through the app launch command only.");
  }
          ...
          if (!active) {
            message =
                Messages.get()
                    ...
                    .key(Messages.GUI_SITEMAP_COULD_NOT_BE_DETERMINED_0);
          }
        }
      } else {
        message =
            Messages.get()
                .getBundle(OpenCms.getWorkplaceManager().getWorkplaceLocale(cms))
                .key(Messages.GUI_SITEMAP_NOT_AVAILABLE_0);
      }
  ...
  /**
   * Opens the sitemap editor for the current site.
   *
   * <p>
   */
  void openSitemapEditor() {
    ...
    Notification.show(
        CmsVaadinUtils.getMessageText(Messages.GUI_SITEMAP_NOT_AVAILABLE_0), Type.WARNING_MESSAGE);
  }
  ...
  private String getPath(CmsObject cms, HttpSession session) {
    ...
    if (page != null) {
      CmsADEConfigData conf =
          OpenCms.getADEManager()
              .lookupConfiguration(cms, cms.getRequestContext().addSiteRoot(page));
