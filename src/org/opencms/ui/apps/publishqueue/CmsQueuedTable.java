...
public class CmsQueuedTable extends Table {
  ...
  public class Row {
    ...
    public List<?> getResourceList() {
      ...
      if (m_resourceList == null) {
        if (m_job instanceof CmsPublishJobFinished) {
          try {
            m_resourceList =
                A_CmsUI.getCmsObject().readPublishedResources(m_job.getPublishHistoryId());
          } catch (Exception e) {
            LOG.error(e.getLocalizedMessage(), e);
          }
        } else if (m_job instanceof CmsPublishJobEnqueued) {
          m_resourceList =
              ((CmsPublishJobEnqueued) m_job).getOriginalPublishList().getAllResources();
    ...
    public String getStatusLocale() {
      ...
      @RUntainted String key = STATUS_MESSAGES.get(state);
      if (key == null) {
  ...
  class EntryStop
    ...
    public void executeAction(Set<@RUntainted String> data) {
      ...
      @RUntainted String jobid = data.iterator().next();
      CmsPublishJobBase job =
          OpenCms.getPublishManager().getJobByPublishHistoryId(new CmsUUID(jobid));
      if (job instanceof CmsPublishJobEnqueued) {
        try {
          OpenCms.getPublishManager()
              .abortPublishJob(A_CmsUI.getCmsObject(), (CmsPublishJobEnqueued) job, true);
        ...
        new CellStyleGenerator() {
          ...
          public String getStyle(Table source, Object itemId, Object propertyId) {
            ...
            if (PROP_STATUS_LOCALE.equals(propertyId)) {
              ...
              if (STATE_WARNING.equals(
                  source.getItem(itemId).getItemProperty(PROP_STATUS).getValue())) {
                return OpenCmsTheme.TABLE_COLUMN_BOX_ORANGE;
              }
              if (STATE_ERROR.equals(
                  source.getItem(itemId).getItemProperty(PROP_STATUS).getValue())) {
                return OpenCmsTheme.TABLE_COLUMN_BOX_RED;
              }
              if (STATE_RUNNING.equals(
                  source.getItem(itemId).getItemProperty(PROP_STATUS).getValue())) {
                return OpenCmsTheme.TABLE_COLUMN_BOX_DARKGRAY;
              }
              if (STATE_ENQUEUE.equals(
                  source.getItem(itemId).getItemProperty(PROP_STATUS).getValue())) {
  ...
  /**
   * Returns the status message map.
   *
   * <p>
   *
    ...
    if (event.getButton().equals(MouseButton.RIGHT) || (propertyId == null)) {
      m_menu.setEntries(
          ...
          Collections.singleton(
              ((((Row) getValue()).getJob()).getPublishHistoryId()).getStringValue()));
  ...
  /**
   * Returns the state of the given publish job.
   *
   * <p>
   *
  ...
  private void loadJobs() {
    ...
    if (OpenCms.getRoleManager().hasRole(A_CmsUI.getCmsObject(), CmsRole.ROOT_ADMIN)) {
      ...
      publishJobs =
          OpenCms.getPublishManager()
              .getPublishHistory(A_CmsUI.getCmsObject().getRequestContext().getCurrentUser());
    }
