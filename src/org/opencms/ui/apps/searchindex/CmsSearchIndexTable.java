...
public class CmsSearchIndexTable extends Table {
  ...
  public void loadTable() {
    ...
    for (I_CmsSearchIndex index : indexes) {
      if (index.isEnabled()) {
        ...
        item.getItemProperty(TableProperty.FieldConfig)
            .setValue(index.getFieldConfiguration().getName());
  ...
  void showSourcesWindow(String resource) {
    ...
    window.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_SEARCHINDEX_INDEXSOURCE_SHOW_1, resource));
  ...
  /**
   * Returns a list with the names of all searchindexes.
   *
   * <p>
   *
