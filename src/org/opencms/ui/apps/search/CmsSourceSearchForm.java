...
public class CmsSourceSearchForm extends VerticalLayout {
  ...
  public CmsSourceSearchForm(CmsSourceSearchApp app) {
    ...
    initFields();
    m_replace.addValueChangeListener(
    ...
    m_searchType.addValueChangeListener(
        new ValueChangeListener() {
  ...
  public void initFormValues(CmsSearchReplaceSettings settings) {
    ...
    if (settings.getType().isContentValuesOnly()) {
      if (settings.getLocale() != null) {
        ...
        if (!l.equals(CmsLocaleManager.getDefaultLocale())
            || l.toString().equals(settings.getLocale())) {
          m_locale.setValue(l);
        }
      }
                    ...
                    new CmsUUID(
                        settings
                            ...
                            .substring(
                                ...
                                settings.getSearchpattern().indexOf("</uuid>"))))
                .getRootPath());
  ...
  /**
   * Sets the download provider (which may be null), and shows or hides the download button based on
   * whether it is null.
   *
  ...
  void changedSearchType() {
    ...
    types.addContainerFilter(
        type.isContentValuesOnly()
            ? CmsVaadinUtils.FILTER_XML_CONTENTS
    ...
    if (m_replace.getValue().booleanValue()) {
      try {
        CmsProject workProject =
            A_CmsUI.getCmsObject().readProject((CmsUUID) m_workProject.getValue());
      ...
      if (SearchType.resourcetype.equals(m_searchType.getValue())) {
        try {
          ...
          settings.setReplacepattern(
              CmsSearchReplaceSettings.replaceElementInPagePattern(resource));
        } catch (CmsException e) {
          LOG.error("Unable to read resource.", e);
        }
  ...
  void updateReplace() {
    ...
    m_replaceResource.setVisible(
        replace ? SearchType.resourcetype.equals(m_searchType.getValue()) : replace);
    ...
    m_newName.setVisible(
        replace ? SearchType.renameContainer.equals(m_searchType.getValue()) : replace);
    m_oldName.setVisible(
        replace ? SearchType.renameContainer.equals(m_searchType.getValue()) : replace);
  ...
  private void initFields() {
    ...
    m_siteSelect.addValueChangeListener(
        new ValueChangeListener() {
    ...
    if (OpenCms.getSearchManager().getSolrServerConfiguration().isEnabled()) {
      ...
      for (CmsSearchIndex index : OpenCms.getSearchManager().getAllSolrIndexes()) {
        ...
        // in case the current project is offline, show offline indexes, otherwise show online
        // indexes
      ...
      if (selectIndex != null) {
        ...
        m_searchType.setItemCaption(
            ...
            CmsVaadinUtils.getMessageText(
                Messages.GUI_SOURCESEARCH_SERACH_TYPE_SOLR_CONTENT_VALUES_0));
      }
    }
