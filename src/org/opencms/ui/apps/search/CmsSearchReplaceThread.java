...
public class CmsSearchReplaceThread extends A_CmsReportThread {
  ...
  public CmsSearchReplaceThread(
      HttpSession session, CmsObject cms, CmsSearchReplaceSettings settings) {
   ...
   */
  public CmsSearchReplaceThread(
      HttpSession session, CmsObject cms, CmsSearchReplaceSettings settings, I_CmsReport report) {
  ...
  /**
   * @see
   *     org.opencms.report.A_CmsReportThread#getReportUpdate(org.opencms.report.I_CmsReportUpdateFormatter)
   */
  @Override
  ...
  /**
   * Returns true if the last report update is too far back in time, so the user has probably closed
   * the window/tab.
   *
  ...
  public void run() {
    ...
    report.println(
        Messages.get().container(Messages.RPT_SOURCESEARCH_PARAMETERS_0),
        I_CmsReport.FORMAT_HEADLINE);
    // the paths
    ...
    report.println(
        Messages.get()
            .container(Messages.RPT_SOURCESEARCH_PARAMETERS_PROJECT_1, m_settings.getProject()),
        I_CmsReport.FORMAT_NOTE);
    ...
    if (m_replace
        && !m_settings
            .getProject()
            .equals(cmsObject.getRequestContext().getCurrentProject().getName())) {
      try {
    ...
    for (CmsResource resource : resources) {
      ...
      try {
        ...
        if ((CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_settings.getXpath())
                || m_settings.isOnlyContentValues())
        ...
        report.print(
            org.opencms.report.Messages.get()
                .container(Messages.RPT_SOURCESEARCH_COULD_NOT_READ_FILE_0),
            I_CmsReport.FORMAT_ERROR);
        report.addError(e);
        report.println(
   ...
   */
  private boolean lockResource(CmsObject cms, CmsResource cmsResource, I_CmsReport report)
      throws CmsException {
  ...
  /**
   * Renames a nested container within a container page XML.
   *
   * <p>
   *
  ...
  private byte[] renameNestedContainers(
    ...
    try {
      CmsXmlContainerPage page =
          CmsXmlContainerPageFactory.unmarshal(getCms(), targetContainerPage);
      for (CmsContainerElementBean element : page.getContainerPage(getCms()).getElements()) {
        if (element.getId().equals(layoutResource.getStructureId())
            && (element.getInstanceId() != null)) {
  ...
  /**
   * Performs the replacement in content.
   *
   * <p>
   *
  ...
  private byte[] replaceInContent(CmsFile file, byte[] contents) throws Exception {
    ...
    if (CmsSourceSearchForm.REGEX_ALL.equals(m_settings.getSearchpattern()) & !m_replace) {
      ...
      getReport()
          .print(
              Messages.get().container(Messages.RPT_SOURCESEARCH_MATCHED_0), I_CmsReport.FORMAT_OK);
      return null;
    }
  ...
  /**
   * Performs a replacement for XML contents.
   *
   * <p>
   *
  ...
  /**
   * Replace properties of given resources.
   *
   * <p>
   *
  ...
  private void replaceProperties(Set<CmsResource> matchedResources) {
    ...
    for (CmsResource resource : matchedResources) {
      try {
        CmsProperty prop =
            getCms().readPropertyObject(resource, m_settings.getProperty().getName(), false);
        Matcher matcher = Pattern.compile(m_settings.getSearchpattern()).matcher(prop.getValue());
        if (m_settings.getReplacepattern().isEmpty()) {
          prop.setValue("", "");
        } else {
          prop.setValue(
              matcher.replaceAll(m_settings.getReplacepattern()), CmsProperty.TYPE_INDIVIDUAL);
        }
  ...
  /**
   * Reads the content as byte array of the given resource and prints a message to the report.
   *
   * <p>
   *
   * @param report the report
  ...
  /**
   * Prints the result messages into the report.
   *
   * <p>
   *
  ...
  private void reportResults(@RUntainted int nrOfFiles) {
    ...
    report.println(
        Messages.get().container(Messages.RPT_SOURCESEARCH_RESULT_0), I_CmsReport.FORMAT_HEADLINE);
    report.println(
        Messages.get()
            .container(
                Messages.RPT_SOURCESEARCH_NR_OF_FILES_TO_SEARCH_IN_1,
    ...
    if (m_replace) {
      ...
      report.println(
          Messages.get()
              .container(
                  Messages.RPT_SOURCESEARCH_LOCKED_FILES_1, new Integer(m_lockedFiles).toString()),
          I_CmsReport.FORMAT_NOTE);
  ...
  /**
   * Search and replace function for properties.
   *
   * <p>
   *
  ...
  /**
   * Searches/reads all resources that are relevant.
   *
   * <p>
   *
  ...
  private List<CmsResource> searchResources() {
    ...
    getReport()
        .println(
            Messages.get()
                .container(Messages.RPT_SOURCESEARCH_START_COLLECTING_FILES_TO_SEARCH_IN_0),
            I_CmsReport.FORMAT_HEADLINE);
    ...
    if (m_settings.isSolrSearch()) {
      ...
      CmsResourceFilter filter =
          CmsResourceFilter.ALL.addExcludeState(CmsResource.STATE_DELETED).addRequireVisible();
      ...
      while (iterPaths.hasNext()) {
        ...
        try {
            ...
            resources.addAll(
                getCms()
                    .readResourcesWithProperty(
                        path, m_settings.getProperty().getName(), null, filter));
          } else {
            // only read resources which are files and not deleted, which are in the current time
            // range window and where the current
            ...
            if (m_settings.ignoreSubSites()) {
              ...
              subsites.remove(
                  OpenCms.getADEManager()
                      .getSubSiteRoot(getCms(), getCms().readResource(path).getRootPath()));
            }
          ...
          getReport()
              .println(
                  Messages.get()
                      .container(Messages.RPT_SOURCESEARCH_ERROR_READING_RESOURCES_1, path),
                  I_CmsReport.FORMAT_ERROR);
        }
      }
    }
  ...
  private boolean writeContent(CmsFile file, byte[] content) {
    ...
    try {
      ...
      if (!lockResource(cmsObject, file, report)) {
        report.println(
            Messages.get()
                .container(Messages.RPT_SOURCESEARCH_LOCKED_FILE_0, cmsObject.getSitePath(file)),
            I_CmsReport.FORMAT_ERROR);
        success = false;
      }
    } catch (CmsException e) {
