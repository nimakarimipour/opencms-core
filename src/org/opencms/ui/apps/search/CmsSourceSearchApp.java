...
public class CmsSourceSearchApp extends A_CmsWorkplaceApp implements I_CmsCachableApp {
  ...
  public static String generateState(CmsSearchReplaceSettings settings) {
    ...
    state =
        A_CmsWorkplaceApp.addParamToState(
            state, IGNORE_SUBSITES, String.valueOf(settings.ignoreSubSites()));
  ...
  static CmsSearchReplaceSettings getSettingsFromState(String state) {
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(typeString)) {
      ...
      settings.setSiteRoot(
          A_CmsWorkplaceApp.getParamFromState(state, SITE_ROOT).replace("%2F", "/"));
      settings.setPaths(
          Collections.singletonList(
              A_CmsWorkplaceApp.getParamFromState(state, FOLDER).replace("%2F", "/")));
      ...
      settings.setSearchpattern(
          A_CmsWorkplaceApp.getParamFromState(state, SEARCH_PATTERN).replace("%2F", "/"));
        ...
        try {
          settings.setProperty(
              A_CmsUI.getCmsObject()
                  .readPropertyDefinition(A_CmsWorkplaceApp.getParamFromState(state, PROPERTY)));
  ...
  protected void displayResult() {
    ...
    if (m_thread.getMatchedResources().isEmpty()) {
      ...
      StreamResource downloadResource =
          new StreamResource(() -> new ByteArrayInputStream(m_resultTable.generateCsv()), filename);
  ...
  protected Component getComponentForState(String state) {
    ...
    m_resultTable.setContextProvider(
    ...
    if (m_resultTableFilter == null) {
      ...
      m_resultTableFilter.addTextChangeListener(
  ...
  protected void search(CmsSearchReplaceSettings settings, boolean updateState) {
    ...
    try {
      ...
      m_report.addReportFinishedHandler(
          new Runnable() {
