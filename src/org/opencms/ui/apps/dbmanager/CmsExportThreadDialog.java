...
public class CmsExportThreadDialog extends CmsBasicDialog {
  ...
  public CmsExportThreadDialog(
      CmsVfsImportExportHandler exportHandler, A_CmsReportThread thread, final Window window) {

    m_exportHandler = exportHandler;
    window.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_DATABASEAPP_EXPORT_THREAD_CAPTION_0));
    ...
    if (!exportHandler.getExportParams().isExportAsFiles()) {
      Button download =
          new Button(CmsVaadinUtils.getMessageText(org.opencms.ui.Messages.GUI_BUTTON_DOWNLOAD_0));
