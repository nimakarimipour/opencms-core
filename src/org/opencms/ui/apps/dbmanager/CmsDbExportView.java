...
public class CmsDbExportView extends VerticalLayout {
  ...
  /**
   * Validator for entered resources.
   *
   * <p>
   */
  class ResourceValidator implements Validator {
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      if ((value == null)) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_DATABASEAPP_EXPORT_INVALID_RESOURCE_EMPTY_0));
      }
      ...
      if (!m_cms.existsResource(resourcePath)) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_DATABASEAPP_EXPORT_INVALID_RESOURCE_NOTFOUND_0));
      }
    }
  }

  /**
   * Validator for the target field.
   *
   * <p>
   */
  ...
  protected Component getResourceRow(String path) {
    ...
    field.setDescription(
        CmsVaadinUtils.getMessageText(Messages.GUI_DATABASEAPP_EXPORT_RESOURCES_HELP_0));
  ...
  /**
   * Checks if resources exist in site. if not the row gets removed.
   *
   * <p>
   */
  ...
  /**
   * Starts the export thread and displays it's report.
   *
   * <p>
   */
  protected void startThread() {
    ...
    try {
      m_cms
          .getRequestContext()
          .setCurrentProject(m_cms.readProject((CmsUUID) m_project.getValue()));
    } catch (CmsException e) {
  ...
  /**
   * Checks if all resources are valid.
   *
   * <p>
   *
  ...
  private void openAddResourcesDialog() {
    ...
    window.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_DATABASEAPP_OPEN_ADD_RESOURCES_DIALOG_0));
    window.setContent(new CmsAddExportResourcesDialog(result -> updateExportResources(result)));
    A_CmsUI.get().addWindow(window);
  }

  /**
   * Sets the init values for check boxes.
   *
   * <p>
   */
  ...
  /**
   * Sets up the combo box for the target file.
   *
   * <p>
   */
  private void setupComboBoxFile() {

    m_target.setInputPrompt(
        CmsVaadinUtils.getMessageText(Messages.GUI_DATABASEAPP_EXPORT_FILE_NAME_EMPTY_0));
  ...
  /**
   * Sets up the combo box for the site choice.
   *
   * <p>
   */
  private void setupComboBoxSite() {
    ...
    IndexedContainer container =
        CmsVaadinUtils.getAvailableSitesContainer(A_CmsUI.getCmsObject(), "title");
  ...
  /**
   * Updates the Export parameter based on user input.
   *
   * <p>
   */
  private void updateExportParams() {
    ...
    ExportMode exportMode =
        m_reducedMetadata.getValue().booleanValue() ? ExportMode.REDUCED : ExportMode.DEFAULT;
  ...
  private void updateExportResources(String resourceListing) {
    ...
    List<String> lines =
        Arrays.stream(resourceListing.trim().split("\n"))
            .filter(r -> !CmsStringUtil.isEmptyOrWhitespaceOnly(r))
            .map(r -> r.trim())
