...
public final class CmsSqlConsoleExecutor {
   ...
   *
   * <p>This method also checks the permissions for executing sql sentences, according to the <code>
   * vfs-doctor.xml</code> configuration file.
   *
   * <p>
   *
   ...
   */
  public CmsSqlConsoleResults execute(
      String sql, String pool, I_CmsReport report, List<Throwable> errors) {
          ...
          if (!checkPermissions(sentence)) {
            writeError(
                ...
                new CmsSecurityException(
                    Messages.get()
                        .container(
                            Messages.ERR_SQLCONSOLE_NO_PERMISSIONS_EXEC_SENTENCE_1, sentence)));
          } else {
            if (!sentence.toUpperCase().startsWith("SELECT")
              ...
              message =
                  Messages.get()
                      .container(Messages.RPT_SQLCONSOLE_ROWS_AFFECTED_1, new Integer(res));
  ...
  /**
   * Checks the permission to be executed for a single sql sentence.
   *
   * <p>
   *
  ...
  private boolean checkPermissions(String sentence) {
    ...
    // protected user from the case where they accidentally left off the condition in DELETE
    // statements
  ...
  /**
   * Executes a single <code>SELECT</code> sql sentence.
   *
   * <p>
   *
  ...
  /**
   * Executes a single sql sentence.
   *
   * <p>For <code>SELECT</code> sentences use <code>{@link #executeQuery(String, String)}</code>.
   *
   * <p>
   *
   * @param sentence the sentence to execute
  ...
  /**
   * Returns a list of single sql sentences, removing all special chars like tabs or eol's.
   *
   * <p>
   *
  ...
  /**
   * Writes the given string to the report, and, if enabled, to the opencms log file.
   *
   * <p>
   *
  ...
  private void write(I_CmsReport report, @RUntainted String string) {
    ...
    report.println(
        org.opencms.report.Messages.get()
            .container(
                org.opencms.report.Messages.RPT_ARGUMENT_1, string.replaceAll("'", "\\\\'")));
    if (LOG.isInfoEnabled()) {
  ...
  /**
   * Writes the given error to the report, and, if enabled, to the opencms log file.
   *
   * <p>
   *
