  ...
  public void run() {
    ...
    try {
      ...
      getReport()
          .println(
              Messages.get()
                  .container(Messages.RPT_DATABASEAPP_DEL_PROPERTY_REMOVE_FROM_RESOURCE_START_0),
              I_CmsReport.FORMAT_DEFAULT);
      for (CmsResource res : getCms().readResourcesWithProperty(m_propName)) {
        ...
        try {
          ...
          getReport()
              .println(
                  Messages.get().container(Messages.RPT_DATABASEAPP_OK_0), I_CmsReport.FORMAT_OK);
