...
public class CmsLogFileView extends VerticalLayout {
  ...
  protected CmsLogFileView(final CmsLogFileApp app) {
    ...
    if (CmsVaadinUtils.getRequest().getSession().getAttribute(ATTR_FILE_VIEW_SIZE) == null) {
      CmsVaadinUtils.getRequest()
          .getSession()
          .setAttribute(ATTR_FILE_VIEW_SIZE, String.valueOf(m_logView.getWindowSize()));
    }
    initLogFileCombo();
    ...
    m_logfile.addValueChangeListener(
        new ValueChangeListener() {
          ...
          public void valueChange(ValueChangeEvent event) {
            ...
            CmsVaadinUtils.getRequest()
                .getSession()
                .setAttribute(ATTR_FILE_VIEW_PATH, getCurrentFile());
            updateView();
          }
        });
  ...
  private String getChar() {
    ...
    return ((Charset) CmsVaadinUtils.getRequest().getSession().getAttribute(ATTR_FILE_VIEW_CHARSET))
        .name();
  }

  /**
   * Gets the size to be displayed.
   *
   * <p>
   *
   * @return line number
  ...
  /**
   * Selects the currently set log file.
   *
   * <p>
   */
