...
public class CmsProjectHistoryTable extends Table {
  ...
  public CmsProjectHistoryTable() {
    ...
    m_container.addContainerProperty(
        CmsProjectsTable.PROP_RESOURCES, CmsProjectsTable.ProjectResources.class, "");

    setContainerDataSource(m_container);
    setColumnHeader(
        CmsProjectsTable.PROP_NAME, CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_NAME_0));
    setColumnHeader(
        CmsProjectsTable.PROP_DESCRIPTION,
        CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_DESCRIPTION_0));
    setColumnHeader(
        PROP_PUBLISH_DATE, CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_PUBLISH_DATE_0));
    setColumnHeader(
        PROP_PUBLISH_USER, CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_PUBLISHED_BY_0));
    setColumnHeader(
        CmsProjectsTable.PROP_ORG_UNIT,
        CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_ORG_UNIT_0));
    setColumnHeader(
        CmsProjectsTable.PROP_OWNER, CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_OWNER_0));
    setColumnHeader(
    ...
    try {
      ...
      for (CmsHistoryProject project : projects) {
        ...
        if (item != null) {
          ...
          item.getItemProperty(CmsProjectsTable.PROP_DESCRIPTION)
              .setValue(project.getDescription());
          ...
          try {
            item.getItemProperty(CmsProjectsTable.PROP_ORG_UNIT)
                .setValue(
