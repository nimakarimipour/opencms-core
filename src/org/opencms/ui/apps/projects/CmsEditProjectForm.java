...
public class CmsEditProjectForm extends CmsBasicDialog {
      ...
      if (m_fieldOU.isEnabled() && CmsStringUtil.isNotEmptyOrWhitespaceOnly((String) value)) {
        try {
          OpenCms.getOrgUnitManager()
              .readOrganizationalUnit(A_CmsUI.getCmsObject(), (String) value);
  ...
  /**
   * The resource field validator. Checks whether the resource is part of the project OU.
   *
   * <p>
   */
  protected class ResourceValidator implements Validator {
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly((String) value)
        ...
        try {
          List<CmsResource> ouRes =
              OpenCms.getOrgUnitManager()
                  .getResourcesForOrganizationalUnit(A_CmsUI.getCmsObject(), m_fieldOU.getValue());
   ...
   * Constructor.
   *
   * <p>Used to edit existing projects.
   *
   * <p>
   *
   ...
   * Constructor.
   *
   * <p>Use this to create a new project.
   *
   * <p>
   *
   * @param table the projects table
  ...
  public CmsEditProjectForm(CmsProjectsTable table, Window window) {
    ...
    try {
      CmsOrganizationalUnit ou =
          OpenCms.getRoleManager()
              .getOrgUnitsForRole(A_CmsUI.getCmsObject(), CmsRole.PROJECT_MANAGER, true)
              .get(0);
    ...
    m_fieldOU.addValueChangeListener(
    ...
    m_addResource.addClickListener(
        new ClickListener() {
    ...
    m_fields =
        new Field<?>[] {m_fieldName, m_fieldDescription, m_fieldManager, m_fieldUser, m_fieldOU};
  }
  ...
  void addResourceField(String value) {
    ...
    CmsRemovableFormRow<CmsPathSelectField> row =
        new CmsRemovableFormRow<CmsPathSelectField>(
            field, CmsVaadinUtils.getMessageText(Messages.GUI_PROJECTS_REMOVE_RESOURCE_0));
