...
public class CmsDefaultAppButtonProvider implements I_CmsAppButtonProvider {
  ...
  public static Component createAppButton(
      CmsObject cms, final I_CmsWorkplaceAppConfiguration appConfig, Locale locale) {
   ...
   * @param locale the locale
   * @return the button component
   *     <p>(I_CmsFolderAppCategory)childNode.getCategory(), childNode.getAppConfigurations())
   */
  public static Component createAppFolderButton(
      CmsObject cms, final CmsAppCategoryNode node, final Locale locale) {
        ...
        new ClickListener() {
          ...
          public void buttonClick(ClickEvent event) {
            ...
            if (node.getAppConfigurations().size() <= DEFAULT_MAX_APP_PER_ROW) {
              panel.setComponentAlignment(
                  panel.getComponent(0), com.vaadin.ui.Alignment.MIDDLE_CENTER);
            }
  ...
  /**
   * Creates a properly styled button for the given app, without adding a click handler or checking
   * visibility settings.
   *
   * <p>
   *
   ...
   * @return the button component
   */
  public static Button createAppIconButton(
      I_CmsWorkplaceAppConfiguration appConfig, Locale locale) {
  ...
  public static Button createIconButton(String name, String description, Resource icon) {
    ...
    return createIconButton(
        name, description, icon, I_CmsAppButtonProvider.BUTTON_STYLE_TRANSPARENT);
  }
  ...
  public static Button createIconButton(
      String name, String description, Resource icon, String buttonStyle) {
  ...
  /**
   * @see
   *     org.opencms.ui.apps.I_CmsAppButtonProvider#createAppButton(org.opencms.ui.apps.I_CmsWorkplaceAppConfiguration)
   */
  ...
  public Component createAppFolderButton(CmsAppCategoryNode node) {
    ...
    if (node.getAppConfigurations().size() == 1) {
      return createAppButton(
          A_CmsUI.getCmsObject(), node.getAppConfigurations().get(0), UI.getCurrent().getLocale());
    }
