...
public class CmsGitToolOptionsPanel extends VerticalLayout {
  ...
  /**
   * Enum describing the type of action to perform.
   *
   * <p>
   */
  enum ActionType {
  ...
  public CmsGitToolOptionsPanel(CmsGitCheckin checkinBean) {
    ...
    if (!checkinBean.hasValidConfiguration()) {
      setMargin(true);
      Label errorLabel =
          new Label(CmsVaadinUtils.getMessageText(Messages.GUI_GIT_APP_UNCONFIGURED_0));
    ...
    m_toggleOptions.addClickListener(
        new ClickListener() {
    ...
    m_deselectAll.addClickListener(
        new ClickListener() {
    ...
    m_tabs.addSelectedTabChangeListener(
  ...
  public void addSelectableModule(final String moduleName) {
    ...
    m_moduleSelectionContainer.addComponent(
        line, m_moduleSelectionContainer.getComponentCount() - 1);
    setTab(m_dialogTab);
  }
   ...
   * <p>
   *
   * @param enable true if the checkboxes for modules which are not installed should be enabled,
  ...
  public void runAction(ActionType action) {
    ...
    switch (action) {
      case checkIn:
        ...
        Button resetHead =
            new Button(CmsVaadinUtils.getMessageText(Messages.GUI_GIT_BUTTON_RESET_HEAD_0));
        resetHead.setDescription(
            CmsVaadinUtils.getMessageText(Messages.GUI_GIT_BUTTON_RESET_HEAD_DESC_0));
        resetHead.addClickListener(
            new ClickListener() {
        ...
        Button resetRemoteHead =
            new Button(CmsVaadinUtils.getMessageText(Messages.GUI_GIT_BUTTON_RESET_REMOTE_HEAD_0));
        resetRemoteHead.setDescription(
            CmsVaadinUtils.getMessageText(Messages.GUI_GIT_BUTTON_RESET_REMOTE_HEAD_DESC_0));
        resetRemoteHead.addClickListener(
            new ClickListener() {
        ...
        CmsGitActionResultPanel panel =
            new CmsGitActionResultPanel(message, log, error, resetButtons);
        addAsWindow(panel);
        break;
      case checkOut:
        if (result == 0) {
        ...
        CmsGitActionResultPanel checkoutResult =
            new CmsGitActionResultPanel(message, log, error, new ArrayList<Button>());
      ...
      case resetHead:
        ...
        CmsGitActionResultPanel resetResult =
            new CmsGitActionResultPanel(message, log, error, new ArrayList<Button>());
      ...
      case resetRemoteHead:
        ...
        CmsGitActionResultPanel resetRemoteResult =
            new CmsGitActionResultPanel(message, log, error, new ArrayList<Button>());
  ...
  public void updateNewModuleSelector() {
    ...
    ((AbstractLayout) (m_moduleSelector.getParent()))
        .replaceComponent(m_moduleSelector, newModuleSelector);
    m_moduleSelector = newModuleSelector;
    m_moduleSelector.addValueChangeListener(
  ...
  /**
   * Creates a new module selector, containing only the modules for which no check box is already
   * displayed.
   *
   * <p>
   *
  ...
  /**
   * Restores user/email/message input fields from saved values.
   *
   * <p>
   */
  ...
  /**
   * Transfers the parameters from the form to the check-in bean.
   *
   * <p>
   */
  ...
  /**
   * Sets an additional info value if it's not empty.
   *
   * <p>
   *
