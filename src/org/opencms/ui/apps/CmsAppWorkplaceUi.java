...
public class CmsAppWorkplaceUi extends A_CmsUI
  ...
  /**
   * View which directly changes the state to the launchpad.
   *
   * <p>
   */
    ...
    if (!queue.isEmpty()) {
      ...
      while (!queue.isEmpty()) {
        ...
        if ((broadcastMessage.getLastDisplay() + CmsBroadcast.DISPLAY_AGAIN_TIME)
          ...
          String date =
              CmsVaadinUtils.getWpMessagesForCurrentLocale()
                  .getDateTime(broadcastMessage.getSendTime());
          String content = broadcastMessage.getMessage();
          broadcasts
              .append("<p>" + getImgHTML(picPath) + "<em>")
              .append(date)
              .append("</em><br />");
          broadcasts
              .append(
                  CmsVaadinUtils.getMessageText(
                      org.opencms.workplace.Messages.GUI_LABEL_BROADCASTMESSAGEFROM_0))
              .append(" <b>")
              .append(from)
              .append("</b>:</p><div class='o-broadcast-message'>");
  ...
  public View getView(String viewName) {
    ...
    I_CmsWorkplaceAppConfiguration appConfig =
        OpenCms.getWorkplaceAppManager().getAppConfiguration(viewName);
    if (appConfig != null) {
  ...
  protected void init(VaadinRequest req) {
    ...
    getReconnectDialogConfiguration()
        .setDialogText(
            CmsVaadinUtils.getMessageText(
                org.opencms.ui.Messages.GUI_SYSTEM_CONNECTION_LOST_TRYING_RECONNECT_0));
    getReconnectDialogConfiguration()
        .setDialogTextGaveUp(
            CmsVaadinUtils.getMessageText(
                org.opencms.ui.Messages.GUI_SYSTEM_CONNECTION_LOST_GIVING_UP_0));
  }

  /**
   * Caches the given view in case it implements the I_CmsAppView interface and is cachable.
   *
   * <p>
   *
  ...
  /**
   * Generates html for image from given path.
   *
   * <p>
   *
  ...
  /**
   * Initializes client polling to avoid session expiration.
   *
   * <p>
   *
  ...
  /**
   * Navigates to the current URI fragment.
   *
   * <p>
   */
