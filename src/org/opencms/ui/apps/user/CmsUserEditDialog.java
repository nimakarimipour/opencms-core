...
public class CmsUserEditDialog extends CmsBasicDialog implements I_CmsPasswordFetcher {
      ...
      if (value == null) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_EMAIL_EMPTY_0));
      }
      try {
        ...
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_EMAIL_INVALID_0));
      }
    }
  }

  /**
   * Validator for the login name field.
   *
   * <p>
   */
  class LoginNameValidator implements Validator {
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      if (value == null) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_LOGINNAME_EMPTY_0));
      }
      ...
      try {
        ...
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_LOGINNAME_INVALID_0));
      }

      if (userAlreadyExists((String) value)) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_LOGINNAME_DOUBLE_0));
      }
    }
  }
  ...
  class PasswordValidator implements Validator {
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      if (!isPasswordValid()) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_PASSWORD_INVALID_0));
      }
    }
  }

  /**
   * Validator for start view and start site field.
   *
   * <p>
   */
  ...
  class StartSiteValidator implements Validator {
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      if (isSiteNull()) {
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_VALIDATION_STARTSITE_EMPTY_0));
      }
    }
  }
  ...
  /**
   * Label containing invisible dummy password fields to dissuade Firefox from saving the password
   * *after* the user edit dialog.
   */
   ...
   */
  public CmsUserEditDialog(
      CmsObject cms, CmsUUID userId, final Window window, final CmsAccountsApp app) {
    ...
    try {
      ...
      displayResourceInfoDirectly(
          Collections.singletonList(CmsAccountsApp.getPrincipalInfo(m_user)));
   ...
   */
  public CmsUserEditDialog(
      CmsObject cms, final Window window, String ou, final CmsAccountsApp app) {
  ...
  protected static void iniRole(
      CmsObject cms, String ou, com.vaadin.ui.ComboBox<CmsRole> roleComboBox, Log log) {
    ...
    try {
      ...
      roleComboBox.setItemCaptionGenerator(
          role -> {
            try {
   ...
   */
  protected static void iniRole(
      CmsObject cms, String ou, ComboBox roleComboBox, Log log, boolean includeNoRoleEntry) {
    ...
    try {
      ...
      for (CmsRole role : roles) {
        ...
        item.getItemProperty("caption")
            .setValue(role.getDisplayName(cms, A_CmsUI.get().getLocale()));
      }
      if (includeNoRoleEntry) {
        Item item = container.addItem("NoRole");
        item.getItemProperty("caption")
            .setValue(CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_USER_NO_ROLE_0));
      }
  ...
  protected static void sendMail(
      CmsObject cms, String password, CmsUser user, boolean newUser, boolean changePassword) {
  ...
  protected static void sendMail(
    ...
    CmsSendPasswordNotification notification =
        new CmsSendPasswordNotification(
            cms,
            password,
            user,
            ou,
   ...
   * <p>
   *
   * @return true if password is valid (and confirm field matches password field).
   *     <p>
   */
  ...
  protected boolean isSitePathValid() {
    ...
    try {
      ...
      return cmsLocal.existsResource(
          m_startfolder.getValue().substring(((String) m_site.getValue()).length()));
    } catch (CmsException e) {
    ...
    try {
      ...
      if (m_sendEmail.getValue().booleanValue() & m_sendEmail.isEnabled()) {
        sendMail(
            ...
            m_user,
            newUser,
            m_forceResetPassword.getValue().booleanValue());
      }
    } catch (CmsException e) {
  ...
  protected void setButtonVisibility() {
    ...
    int maxPos =
        m_isWebOU ? 3 : 3; // has to be changed if number of tabs is changed for web OU user
  ...
  protected void setupStartFolder(String startFolder) {
    ...
    try {
      ...
      String defaultFolder =
          OpenCms.getWorkplaceManager().getDefaultUserSettings().getStartFolder();
      if ((startFolder == null)
          && (CmsStringUtil.isEmptyOrWhitespaceOnly(defaultFolder)
              || !cmsCopy.existsResource(defaultFolder))) {
        defaultFolder = "/";
      }
      m_startfolder.setValue(
          cmsCopy
              .getRequestContext()
              .addSiteRoot(startFolder == null ? defaultFolder : startFolder));
      m_startfolder.setCmsObject(cmsCopy);
      m_startfolder.setUseRootPaths(true);
      if (!m_visSites) {
        try {
          List<CmsResource> ouResources =
              OpenCms.getOrgUnitManager()
                  .getResourcesForOrganizationalUnit(cmsCopy, m_ou.getValue());
  ...
  /**
   * Checks the security level of the given password.
   *
   * <p>
   *
  ...
  void checkSecurity(String password) {
    ...
    try {
      ...
      if (handler instanceof I_CmsPasswordSecurityEvaluator) {
        SecurityLevel level =
            ((I_CmsPasswordSecurityEvaluator) handler).evaluatePasswordSecurity(password);
  ...
  /**
   * Returns a CmsObject with request set to given site.
   *
   * <p>
   *
  ...
  /**
   * Initializes the site combo box.
   *
   * <p>
   *
  ...
  void iniSite(CmsUserSettings settings) {
    ...
    List<CmsSite> sitesList =
        OpenCms.getSiteManager().getAvailableSites(m_cms, true, false, m_ou.getValue());
        ...
        if (hasRole(CmsRole.VFS_MANAGER)
            | ((m_user == null)
                && Arrays.asList(
                        ...
                        CmsRole.ROOT_ADMIN.forOrgUnit(m_ou.getValue()))
                    .contains(
    ...
    if (container.size() == 0) {
      if (!container.containsId(A_CmsUI.getCmsObject().getRequestContext().getSiteRoot())) {
        Item defaultItem =
            container.addItem(A_CmsUI.getCmsObject().getRequestContext().getSiteRoot());
        defaultItem
            .getItemProperty("caption")
            .setValue(A_CmsUI.getCmsObject().getRequestContext().getSiteRoot());
      }
      m_visSites = false;
    }
    ...
    if (container.size() > 0) {
      ...
      if (container.getItemIds().size() > 0) {
        ...
        if ((settings != null) && container.containsId(settings.getStartView())) {
          ...
          String defaultView =
              OpenCms.getWorkplaceManager().getDefaultUserSettings().getStartView();
  ...
  /**
   * Shows or hides the not matching passwords error.
   *
   * <p>
   *
  ...
  void showPasswordMatchError(boolean show) {
    ...
    if (show) {
      ...
      m_pw.setErrorPassword2(
          new UserError(
              CmsVaadinUtils.getMessageText(
                  org.opencms.ui.Messages.GUI_PWCHANGE_PASSWORD_MISMATCH_0)),
          OpenCmsTheme.SECURITY_INVALID);
    } else {
  ...
  /**
   * Returns the start view container.
   *
   * <p>
   *
  ...
  private IndexedContainer getStartViewContainer(String caption) {
    ...
    List<I_CmsWorkplaceAppConfiguration> apps =
        OpenCms.getWorkplaceAppManager().getDefaultQuickLaunchConfigurations();
  ...
  /**
   * Checks if user, which gets edited, has given role.
   *
   * <p>
   *
  ...
  /**
   * Checks if user, which gets edited, has role for given app.
   *
   * <p>
   *
  ...
  /**
   * Initializes the language combo box.
   *
   * <p>
   *
   * @param settings user settings
  ...
  private void iniLanguage(CmsUserSettings settings) {
    ...
    if (settings != null) {
      ...
      if (container.containsId(
          OpenCms.getWorkplaceManager().getDefaultUserSettings().getLocale())) {
  ...
  private void iniProject(CmsUserSettings settings) {
    ...
    try {
      List<CmsProject> projects =
          OpenCms.getOrgUnitManager().getAllAccessibleProjects(m_cms, m_ou.getValue(), false);
      ...
      if (settings != null) {
        // Project names may start with "/" when stored via the old workplace, this slash has to be
        // removed, to match the name.
  ...
  private void init(
      final Window window,
      final CmsAccountsApp app,
      final CmsUserSettings settings,
      boolean enabled) {
    ...
    if (m_user == null) {
      ...
      m_group.addValueChangeListener(
          new ValueChangeListener() {
    ...
    m_generateButton.addClickListener(
        new Button.ClickListener() {
          ...
          public void buttonClick(ClickEvent event) {
            ...
            final Window windowDialog =
                CmsBasicDialog.prepareWindow(CmsBasicDialog.DialogWidth.content);
            windowDialog.setCaption(
                CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_GEN_PASSWORD_CAPTION_0));
            CmsGeneratePasswordDialog dialog =
                new CmsGeneratePasswordDialog(
  ...
  private void setPasswordFields() {
    ...
    // ugly hack to prevent Firefox from asking user to save password on every click which causes
    // the history token to change after editing a user
    ...
    if (OpenCms.getPasswordHandler() instanceof I_CmsPasswordSecurityEvaluator) {
      m_pw.setSecurityHint(
          ((I_CmsPasswordSecurityEvaluator) OpenCms.getPasswordHandler())
              .getPasswordSecurityHint(A_CmsUI.get().getLocale()));
    }
  ...
  /**
   * Sets the password status for the user.
   *
   * <p>
   *
   * @param user CmsUser
  ...
  /**
   * Read form and updates a given user according to form.
   *
   * <p>
   *
