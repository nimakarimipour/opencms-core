...
public class CmsDeleteOUDialog extends CmsBasicDialog {
  ...
  /**
   * public constructor used for OUs.
   *
   * <p>
   *
   * @param cms CmsObject
  ...
  public CmsDeleteOUDialog(CmsObject cms, String ouName, Window window, CmsAccountsApp app) {
    ...
    try {
      ...
      List<CmsOrganizationalUnit> oUs =
          OpenCms.getOrgUnitManager().getOrganizationalUnits(m_cms, ouName, true);
      ...
      if (userList.isEmpty() && oUs.isEmpty() && hasNoGroups()) {
        ...
        m_label.setValue(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_DELETE_OU_NOT_POSSIBLE_1, ouName));
          ...
          Collections.singletonList(
              CmsAccountsApp.getOUInfo(
                  OpenCms.getOrgUnitManager()
                      .readOrganizationalUnit(A_CmsUI.getCmsObject(), m_ouName))));
    } catch (CmsException e) {
      LOG.error("Unable to read OU", e);
    }
