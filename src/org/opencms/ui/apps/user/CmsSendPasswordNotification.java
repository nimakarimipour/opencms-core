  ...
  protected void appendXMLContent(StringBuffer msg) {
    ...
    msg.append(
        CmsMacroResolver.resolveMacros(
            m_mailContent.getStringValue(m_cms, xmlName, m_locale), m_macroResolver));
  ...
  protected String getNotificationContent() {
    ...
    if (m_new) {
      return OpenCms.getSystemInfo()
          .getConfigFilePath(m_cms, "notification/password-new-user-notification");
    }
    return OpenCms.getSystemInfo()
        .getConfigFilePath(m_cms, "notification/password-new-password-from-admin-notification");
  }
}
