...
public class CmsOuTree extends Tree {
    ...
    try {
      ...
      for (String subP : pathP) {
        ...
        CmsOrganizationalUnit ou =
            OpenCms.getOrgUnitManager().readOrganizationalUnit(m_cms, complPath);
  ...
  /**
   * Updates items of current ou for item.
   *
   * <p>
   *
  ...
  private void addChildrenForGroupsNode(I_CmsOuTreeType type, String ouItem) {
    ...
    try {
      ...
      for (CmsGroup group : groups) {
        ...
        groupItem
            .getItemProperty(PROP_NAME)
            .setValue(getIconCaptionHTML(group, CmsOuTreeType.GROUP));
  ...
  private void addChildrenForOUNode(CmsOrganizationalUnit item) {
    ...
    try {
      ...
      List<CmsOrganizationalUnit> ous =
          OpenCms.getOrgUnitManager().getOrganizationalUnits(m_cms, item.getName(), false);
  ...
  /**
   * Get HTML for icon and item caption.
   *
   * <p>
   *
