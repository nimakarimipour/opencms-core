      ...
      try {
        ...
        throw new InvalidValueException(
            CmsVaadinUtils.getMessageText(
                Messages.GUI_USERMANAGEMENT_USER_MOVE_OU_USERNAME_COLLISION_0));
  ...
  public CmsMoveUserToOU(
    ...
    try {
      ...
      if (!groups.isEmpty()) {
        ...
        m_removeAll.addValueChangeListener(
      ...
      m_ou.setContainerDataSource(
          CmsVaadinUtils.getOUComboBox(m_cms, "", LOG, false).getContainerDataSource());
      ...
      try {
        ...
        for (CmsRole role :
            OpenCms.getRoleManager()
                .getRolesOfUser(m_cms, m_user.getName(), m_user.getOuFqn(), true, true, false)) {
        ...
        OpenCms.getOrgUnitManager()
            .setUsersOrganizationalUnit(m_cms, (String) m_ou.getValue(), m_user.getName());
        // Reload moved user
        m_user = m_cms.readUser(m_user.getId());
        for (CmsRole role : directOURoles) {
          OpenCms.getRoleManager()
              .addUserToRole(m_cms, role.forOrgUnit((String) m_ou.getValue()), m_user.getName());
        }
