...
public class CmsOUEditDialog extends CmsBasicDialog {
  ...
  public CmsOUEditDialog(CmsObject cms, String ou, final Window window, final CmsAccountsApp app) {
            ...
            Collections.singletonList(
                CmsAccountsApp.getOUInfo(
                    OpenCms.getOrgUnitManager()
                        .readOrganizationalUnit(A_CmsUI.getCmsObject(), ou))));
    ...
    m_cms = cms;
    Supplier<Component> fieldFactory =
        new Supplier<Component>() {
          ...
          public Component get() {
            ...
            try {
              field.setValue(
                  OpenCms.getOrgUnitManager()
                      .getResourcesForOrganizationalUnit(m_cms, m_parentOu.getValue())
                      .get(0)
                      .getRootPath());
    ...
    try {
      if (ou != null) {
        ...
        for (CmsResource resource :
            OpenCms.getOrgUnitManager().getResourcesForOrganizationalUnit(m_cms, m_ou.getName())) {
          CmsPathSelectField field = new CmsPathSelectField();
  ...
  public CmsOUEditDialog(CmsObject cms, Window window, String ou, CmsAccountsApp app) {
    ...
    try {
      displayResourceInfoDirectly(
          Collections.singletonList(
              CmsAccountsApp.getOUInfo(
                  OpenCms.getOrgUnitManager()
                      .readOrganizationalUnit(A_CmsUI.getCmsObject(), m_parentOu.getValue()))));
    } catch (CmsException e1) {
      LOG.error("Unable to read OU", e1);
    }
    ...
    try {
      field.setValue(
          OpenCms.getOrgUnitManager()
              .getResourcesForOrganizationalUnit(cms, ou)
              .get(0)
              .getRootPath());
    } catch (Throwable e) {
      ...
      if (m_ou == null) {
        ...
        if (!resourceNames.isEmpty()) {
          ...
          while (itResourceNames.hasNext()) {
            OpenCms.getOrgUnitManager()
                .addResourceToOrgUnit(m_cms, m_ou.getName(), itResourceNames.next());
          }
        }
      } else {
        ...
        while (itResourceNamesNew.hasNext()) {
          ...
          if (!resourceNamesOld.contains(resourceNameNew)) {
            OpenCms.getOrgUnitManager()
                .addResourceToOrgUnit(m_cms, m_ou.getName(), resourceNameNew);
          }
        }
        ...
        while (itResourceNamesOld.hasNext()) {
          ...
          if (!resourceNamesNew.contains(resourceNameOld)) {
            OpenCms.getOrgUnitManager()
                .removeResourceFromOrgUnit(m_cms, m_ou.getName(), resourceNameOld);
          }
        }
