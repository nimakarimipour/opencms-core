...
public abstract class A_CmsEditUserGroupRoleDialog extends CmsBasicDialog {
   ...
   */
  public A_CmsEditUserGroupRoleDialog(
    ...
    try {
      ...
      window.setCaption(
          CmsVaadinUtils.getMessageText(getWindowCaptionMessageKey(), m_principal.getSimpleName()));
   ...
   */
  public abstract IndexedContainer getAvailableItemsIndexedContainer(
      String caption, String propIcon);
  ...
  protected void init() {
    ...
    if (getItemsOfUserIndexedContainer("prop1", "prop2", "prop3").size() > 0) {
      getLeftTableLayout()
          .addComponent(
              new FixedHeightPanel(
                  new CmsCurrentRoleOrPrincipalTable(this, m_cms, m_principal), ITEM_HEIGHT),
              0);
    } else {
      getLeftTableLayout()
          .addComponent(
  ...
  /**
   * Check if table exists in given layout. Removes all tables.
   *
   * <p>
   *
  ...
  private void removeExistingTable(VerticalLayout layout) {
    ...
    while (it.hasNext()) {
      ...
      if ((comp instanceof FixedHeightPanel)
          | (comp instanceof TextField)
          | (comp instanceof VerticalLayout)) {
