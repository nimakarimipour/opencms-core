...
public class CmsShowResourceTable extends Table {
  ...
  public CmsShowResourceTable(CmsObject cms, CmsUUID principalID, DialogType type) {
    ...
    try {
      ...
      setColumnHeader(
          PROP_NAME, CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_GROUP_NAME_0));
      setColumnHeader(
          PROP_PERMISSION,
        ...
        new ItemClickListener() {
          ...
          public void itemClick(ItemClickEvent event) {
            ...
            if (event.getButton().equals(MouseButton.RIGHT) || (event.getPropertyId() == null)) {
              m_menu.setEntries(
                  getMenuEntries(),
                  Collections.singleton(((CmsResource) getValue()).getRootPath()));
              m_menu.openForTable(
                  event, event.getItemId(), event.getPropertyId(), CmsShowResourceTable.this);
            }
          }
        });
  }
  ...
  /**
   * Opens the explorer for given path and selected resource.
   *
   * <p>
   *
   ...
   */
  private String getPermissionString(CmsObject cms, CmsResource res, DialogType type)
      throws CmsException {
    ...
    if (type.equals(DialogType.User)) {
      ...
      Iterator<CmsAccessControlEntry> itAces =
          cms.getAccessControlEntries(res.getRootPath(), false).iterator();
   ...
   */
  private CmsPrincipal getPrincipal(CmsObject cms, DialogType type, CmsUUID id)
      throws CmsException {
  ...
  /**
   * Get resources set for the given principal.
   *
   * <p>
   *
   * @param cms CmsObject
   ...
   */
  private Set<CmsResource> getResourcesFromPrincipal(CmsObject cms, CmsUUID id)
      throws CmsException {
