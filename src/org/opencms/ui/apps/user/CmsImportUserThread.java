...
public class CmsImportUserThread extends A_CmsReportThread {
  ...
  public void run() {
    ...
    getReport()
        .println(
            Messages.get().container(Messages.RPT_USERIMPORT_BEGIN_0), I_CmsReport.FORMAT_HEADLINE);
    getReport().println();
    getReport()
        .println(
            Messages.get()
                .container(
                    Messages.RPT_USERIMPORT_FILE_CONTAINS_1, String.valueOf(m_userList.size())),
    ...
    while (itUsers.hasNext()) {
      ...
      if (!isAlreadyAvailable(user.getName())) {
          ...
          if (!m_roleList.isEmpty()) {
            ...
            while (itRoles.hasNext()) {
              ...
              OpenCms.getRoleManager()
                  .addUserToRole(getCms(), itRoles.next().forOrgUnit(m_ou), createdUser.getName());
            }
          }
          String startAppId = CmsPageEditorConfiguration.APP_ID;
          if (OpenCms.getRoleManager()
              .hasRole(getCms(), createdUser.getName(), CmsRole.WORKPLACE_USER)) {
        ...
        if (createdUser != null) {
          getReport()
              .println(
                  Messages.get()
                      .container(
                          Messages.RPT_USERIMPORT_IMPORT_SUCCESFULL_1, createdUser.getName()),
        ...
        getReport()
            .println(
                Messages.get()
                    .container(Messages.RPT_USERIMPORT_IMPORT_ALREADY_IN_OU_1, user.getName()),
                I_CmsReport.FORMAT_ERROR);
      }
    }
    getReport()
        .println(
            Messages.get().container(Messages.RPT_USERIMPORT_END_0), I_CmsReport.FORMAT_DEFAULT);
  }
