...
public final class CmsImportExportUserDialog extends A_CmsImportExportUserDialog
   ...
   * @param window window
   * @param allowTechnicalFieldsExport flag indicates if technical field export option should be
   *     available
   */
  private CmsImportExportUserDialog(
      final String ou, CmsUUID groupID, Window window, boolean allowTechnicalFieldsExport) {
    ...
    m_includeTechnicalFields.addValueChangeListener(
            ...
            try {
              ...
              CmsConfirmationDialog.show(
                  CmsVaadinUtils.getMessageText(
                      Messages.GUI_USERMANAGEMENT_USER_IMEXPORT_INVALID_FILE_0),
                  CmsVaadinUtils.getMessageText(
                      Messages.GUI_USERMANAGEMENT_USER_IMEXPORT_INVALID_CSV_0),
                  new Runnable() {
   ...
   */
  public static Map<CmsUUID, CmsUser> addExportAllUsers(
      CmsObject cms, String ou, Map<CmsUUID, CmsUser> exportUsers) throws CmsException {
  ...
  public static Map<CmsUUID, CmsUser> addExportUsersFromGroups(
      CmsObject cms, List<String> groups, Map<CmsUUID, CmsUser> exportUsers) throws CmsException {
  ...
  public static Map<CmsUUID, CmsUser> addExportUsersFromRoles(
      CmsObject cms, String ou, List<String> roles, Map<CmsUUID, CmsUser> exportUsers)
      throws CmsException {

    if ((roles != null) && (roles.size() > 0)) {
      Iterator<String> itRoles = roles.iterator();
      while (itRoles.hasNext()) {
        List<CmsUser> roleUsers =
            OpenCms.getRoleManager()
                .getUsersOfRole(
                    cms, CmsRole.valueOfGroupName(itRoles.next()).forOrgUnit(ou), true, false);
  ...
  public static CmsImportExportUserDialog getExportUserDialogForGroup(
      CmsUUID groupID, String ou, Window window, boolean allowTechnicalFieldsExport) {

    CmsImportExportUserDialog res =
        new CmsImportExportUserDialog(ou, groupID, window, allowTechnicalFieldsExport);
    return res;
  }
  ...
  public static CmsImportExportUserDialog getExportUserDialogForOU(
      String ou, Window window, boolean allowTechnicalFieldsExport) {

    CmsImportExportUserDialog res =
        new CmsImportExportUserDialog(ou, null, window, allowTechnicalFieldsExport);
    return res;
  }
  ...
  protected void importUserFromFile() {
    ...
    CmsImportUserThread thread =
        new CmsImportUserThread(
            m_cms,
            m_ou,
            m_userImportList,
    ...
    thread.start();
    CmsShowReportDialog dialog =
        new CmsShowReportDialog(
            thread,
            new Runnable() {
  ...
  Map<CmsUUID, CmsUser> getUserToExport() {
    ...
    try {
      if (((groups.size() < 1)) && ((roles.size() < 1))) {
        ...
        exportUsers =
            CmsImportExportUserDialog.addExportUsersFromGroups(m_cms, groups, exportUsers);
        exportUsers =
            CmsImportExportUserDialog.addExportUsersFromRoles(m_cms, m_ou, roles, exportUsers);
      }
    } catch (CmsException e) {
