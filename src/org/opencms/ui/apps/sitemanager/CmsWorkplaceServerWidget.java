...
public class CmsWorkplaceServerWidget extends FormLayout {
  ...
  public CmsWorkplaceServerWidget(List<CmsSite> sites, String server) {
    ...
    m_encryption.setContainerDataSource(
        CmsEditSiteForm.getSSLModeContainer("caption", false, sslMode));
    ...
    List<CmsSite> sitesWithUrl =
        sites.stream().filter(site -> site.getSiteMatcher() != null).collect(Collectors.toList());
    m_serverContainer = setUpWorkplaceComboBox(sitesWithUrl, m_server, false, server, sslMode);

    m_encryption.select(sslMode);
    m_encryption.addValueChangeListener(
    ...
    m_server.addValueChangeListener(
        new ValueChangeListener() {
  ...
  private static BeanItemContainer<CmsSite> setUpWorkplaceComboBox(
    ...
    final BeanItemContainer<CmsSite> objects =
        new BeanItemContainer<CmsSite>(CmsSite.class, modSites);
    ...
    combo.setNewItemHandler(
        new NewItemHandler() {
          ...
          public void addNewItem(String newItemCaption) {
            ...
            newItem.setSSLMode(
                newItemCaption.contains("https:") ? CmsSSLMode.MANUAL : CmsSSLMode.NO);
  ...
  protected void adjustSSL() {
    ...
    m_encryption.setValue(
        siteMode.equals(CmsSSLMode.SECURE_SERVER)
            ? CmsSSLMode.NO
            : siteMode); // Set mode according to site, don't allow Secure Server
  }

  /**
   * Checks if adjust methods can early return.
   *
   * <p>
   *
