...
public class CmsSitesWebserverThread extends A_CmsReportThread {
  ...
  /**
   * Creates the new web server configuration files from the given template file.
   *
   * <p>
   *
    ...
    for (@RUntainted CmsSite site : sites) {
      if ((site.getSiteMatcher() != null) && site.isWebserver()) {
        ...
        if (site.hasSecureServer()) {
          content +=
              createConfigForSite(site, FileUtils.readFileToString(new File(m_secureTemplate)));
        }
  ...
  /**
   * Performs the template handling and returns the content.
   *
   * <p>
   *
   * @param site the site
  ...
  private String createConfigForSite(CmsSite site, String templateContent) {
    ...
    config.setAttribute(
        "SERVER_NAME_WITH_PORT", generateWebserverConfigName(site.getSiteMatcher(), ":"));
    ...
    if (site.hasSecureServer()) {
      ...
      if (site.getSecureServer() != null) {
        config.setAttribute(
            "SECURE_SRV_WITH_PORT", generateWebserverConfigName(site.getSecureServer(), ":"));
  ...
  /**
   * Deletes all web server's configuration files with the given prefix.
   *
   * <p>
   *
  ...
  /**
   * Generates the web server configuration filename for the given site.
   *
   * <p>
   *
      ...
      getReport()
          .println(
              org.opencms.ui.apps.Messages.get()
                  .container(org.opencms.ui.apps.Messages.RPT_LETSENCRYPT_FINISHED_0),
              I_CmsReport.FORMAT_OK);
    }
  }
}
