...
public class CmsCreateSiteThread extends A_CmsReportThread {
  ...
  public void run() {
    ...
    try {
      ...
      if (m_source.isEmpty()) {
        ...
        String sitemapConfig =
            CmsStringUtil.joinPaths(contentFolder, CmsADEManager.CONFIG_FILE_NAME);
      ...
      getReport()
          .println(Messages.get().container(Messages.RPT_SITE_ERROR_0), I_CmsReport.FORMAT_ERROR);
      getReport().println(e);
      getReport().println();
      getReport()
          .println(
    ...
    getReport()
        .println(Messages.get().container(Messages.RPT_SITE_FINISH_0), I_CmsReport.FORMAT_OK);
    m_finished.run();
  ...
  /**
   * Changes the folder type if necessary:
   *
   * <p>Type Folder -> Type SubsitemapFolder.
   *
   * <p>others -> no change.
   *
   * <p>
  ...
  private void adjustFolderType(CmsResource siteRootResource)
      throws CmsLoaderException, CmsException {

    if (OpenCms.getResourceManager().getResourceType(CmsResourceTypeFolder.RESOURCE_TYPE_NAME)
        == OpenCms.getResourceManager().getResourceType(siteRootResource)) {
    ...
    if (configFile != null) {
      try {
        ...
        CmsResource modelPage =
            m_cms.createResource(newFolder.getRootPath() + BLANK_HTML, containerType);
        String defTitle =
            Messages.get()
                .getBundle(m_cms.getRequestContext().getLocale())
  ...
  /**
   * Creates root-folder for the new site.
   *
   * <p>If folder exist, the existing one will be returned.
   *
   * <p>
   *
  ...
  private CmsResource createSiteRootIfNeeded(String siteRoot) throws CmsException {
    ...
    try {
      ...
      CmsProperty folderTitle =
          new CmsProperty(
              CmsPropertyDefinition.PROPERTY_TITLE, m_site.getTitle(), m_site.getTitle());
  ...
  /**
   * Creates online version of given CmsObject.
   *
   * <p>
   *
  ...
  /**
   * Handles OU related operations.
   *
   * <p>Creates OU, adds site root to existing OU or does nothing.
   *
   * <p>
   *
  ...
  private void handleOU(CmsResource siteRootResource) {
    ...
    if (m_createOU) {
      getReport()
          .println(
              Messages.get()
                  .container(
                      Messages.RPT_SITE_CREATE_OU_1, m_parentOU + siteRootResource.getName()),
              I_CmsReport.FORMAT_DEFAULT);
      try {
        OpenCms.getOrgUnitManager()
            .createOrganizationalUnit(
                ...
                ouDescription.replace(
                    "%(site)", m_site.getTitle() + " [" + siteRootResource.getRootPath() + "]"),
                0,
                siteRootResource.getRootPath());
        ouName = m_parentOU + siteRootResource.getName();
      } catch (CmsDataAccessException e) {
        LOG.info(
            "Can't create OU, an OU with same name exists. The existing OU is chosen for the new site");
        try {
          OpenCms.getOrgUnitManager()
              .addResourceToOrgUnit(
                  m_cms, m_parentOU + siteRootResource.getName(), siteRootResource.getRootPath());
          ouName = m_parentOU + siteRootResource.getName();
    ...
    if ((m_oldSite == null) & (m_selectedOU != null) & !m_selectedOU.equals("/")) {
      ...
      try {
        OpenCms.getOrgUnitManager()
            .addResourceToOrgUnit(m_cms, m_selectedOU, siteRootResource.getRootPath());
  ...
  private void saveFavIcon(String siteRoot) {
    ...
    getReport()
        .println(
            Messages.get().container(Messages.RPT_SITE_SET_FAVICON_0), I_CmsReport.FORMAT_DEFAULT);
  ...
  /**
   * Updates title property of site root resource in case of copy from template.
   *
   * <p>
   *
  ...
  private void setFolderTitle(CmsResource res) {
    ...
    try {
      CmsProperty titleProperty =
          m_cms.readPropertyObject(res, CmsPropertyDefinition.PROPERTY_TITLE, false);
      ...
      getReport()
          .println(
              Messages.get().container(Messages.RPT_SITE_ERROR_TITLE_0), I_CmsReport.FORMAT_ERROR);
      getReport().println(e);
    }
  }

  /**
   * Sets the selected template as property to site root folder.
   *
   * <p>
   *
   * @param siteRootResource Resource representing root folder
  ...
  private void setTemplate(CmsResource siteRootResource) {
    ...
    try {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_template)) {
        CmsProperty prop =
            new CmsProperty(CmsPropertyDefinition.PROPERTY_TEMPLATE, m_template, null);
