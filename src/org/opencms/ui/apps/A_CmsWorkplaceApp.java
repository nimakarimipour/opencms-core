...
public abstract class A_CmsWorkplaceApp implements I_CmsWorkplaceApp {
  ...
  public static String addParamToState(String state, String paramName, String value) {
    ...
    return state
        ...
        + paramName
        + PARAM_ASSIGN
        + CmsEncoder.encode(value, CmsEncoder.ENCODING_UTF_8);
  }
  ...
  public CmsObject getOfflineCmsObject(CmsObject cms) {
    ...
    try {
      ...
      List<CmsProject> projects =
          OpenCms.getOrgUnitManager().getAllAccessibleProjects(res, "/", true);
  ...
  protected void updateBreadCrumb(Map<String, String> breadCrumbEntries) {
    ...
    I_CmsWorkplaceAppConfiguration launchpadConfig =
        OpenCms.getWorkplaceAppManager().getAppConfiguration(CmsAppHierarchyConfiguration.APP_ID);
    if (launchpadConfig.getVisibility(A_CmsUI.getCmsObject()).isActive()) {
      entries.put(
          CmsAppHierarchyConfiguration.APP_ID,
          launchpadConfig.getName(UI.getCurrent().getLocale()));
    }
    if ((breadCrumbEntries != null) && !breadCrumbEntries.isEmpty()) {
      ...
      entries.put(
          ...
          OpenCms.getWorkplaceAppManager()
              .getAppConfiguration(m_uiContext.getAppId())
              .getName(UI.getCurrent().getLocale()));
    }
    m_breadCrumb.setEntries(entries);
  }
