...
public class CmsSendBroadcastDialog extends CmsBasicDialog {
  ...
  private static final Map<CmsUser, CmsBroadcast> USER_BROADCAST =
      new ConcurrentHashMap<CmsUser, CmsBroadcast>();
  ...
  public CmsSendBroadcastDialog(final Set<String> sessionIds, final Runnable closeRunnable) {
    ...
    if (sessionIds != null) {
      ...
      if (USER_BROADCAST.containsKey(A_CmsUI.getCmsObject().getRequestContext().getCurrentUser())) {
        m_message.setValue(
            USER_BROADCAST
                .get(A_CmsUI.getCmsObject().getRequestContext().getCurrentUser())
                .getMessage());
      }
    }
  ...
  /**
   * Validates the broadcast message, sets the error status of the field appropriately, and returns
   * the result.
   *
