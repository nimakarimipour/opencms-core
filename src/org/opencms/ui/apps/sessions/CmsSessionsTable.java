...
public class CmsSessionsTable extends Table {
  ...
  /**
   * The menu entry to switch to the explorer of concerning site.
   *
   * <p>
   */
    ...
    public void executeAction(Set<@RUntainted String> data) {
      ...
      try {
        ...
        A_CmsUI.getCmsObject()
            .getRequestContext()
            .setCurrentProject(A_CmsUI.getCmsObject().readProject(session.getProject()));
      ...
      showKillDialog(
          ...
          CmsVaadinUtils.getMessageText(
              ...
              CmsSessionsApp.getUserNames(
                  data, CmsVaadinUtils.getMessageText(Messages.GUI_MESSAGES_AND_0))),
  ...
  /**
   * The menu entry to switch to the page editor of concerning site.
   *
   * <p>
   */
  ...
  enum TableProperty {
    ...
    /**
     * Returns list of all properties with non-empty header.
     *
     * <p>
     *
    ...
    /**
     * Returns tye of value for given property.
     *
     * <p>
     *
     * @return type
    ...
    try {
      ...
      m_mySessionId =
          OpenCms.getSessionManager()
              .getSessionInfo(CmsVaadinUtils.getRequest())
              .getSessionId()
              .getStringValue();
      ...
      addGeneratedColumn(
          ...
          new ColumnGenerator() {
            ...
            public Object generateCell(Table source, Object itemId, Object columnId) {
              ...
              if (((Boolean)
                      source.getItem(itemId).getItemProperty(TableProperty.IS_WAITING).getValue())
                  .booleanValue()) {
                icon.setOverlay(
                    OpenCmsTheme.STATE_CHANGED + " " + CmsResourceIcon.ICON_CLASS_CHANGED);
              }
          ...
          new CellStyleGenerator() {
            ...
            public String getStyle(Table source, Object itemId, Object propertyId) {
              ...
              if (((Boolean)
                          source
                              .getItem(itemId)
                              .getItemProperty(TableProperty.IS_WAITING)
                              .getValue())
                      .booleanValue()
                  & (propertyId == null)) {
              ...
              if (TableProperty.IS_ACTIVE.equals(propertyId)) {
                return CmsUserInfoDialog.getStatusStyleForItem(
                    (Long)
                        source.getItem(itemId).getItemProperty(TableProperty.IS_ACTIVE).getValue());
              }
              ...
              if (TableProperty.IS_ACTIVE.equals(propertyId)) {
                ...
                if (Integer.parseInt(ret[1]) == 1) {
                  return CmsVaadinUtils.getMessageText(
                      Messages.GUI_MESSAGES_LAST_ACTIVITY_ONE_MINUTE_0);
                }
                if (Integer.parseInt(ret[1]) == 0) {
                  return CmsVaadinUtils.getMessageText(
                      Messages.GUI_MESSAGES_LAST_ACTIVITY_LESS_ONE_MINUTE_0);
                }
                return CmsVaadinUtils.getMessageText(
                    Messages.GUI_MESSAGES_LAST_ACTIVITY_MINUTES_1, new Integer(ret[1]));
              }
              return null;
            }
          });
  ...
  /**
   * Runnable called when a window should be closed.
   *
   * <p>Reinitializes the table.
   *
   * <p>
   *
  ...
  /**
   * Shows the dialog to destroy given sessions.
   *
   * <p>
   *
   ...
   * @param table to be updated
   */
  protected static void showKillDialog(
      Set<String> ids, String caption, final CmsSessionsTable table) {
  ...
  protected void ini() throws CmsException {
    ...
    List<CmsOrganizationalUnit> manageableOus =
        OpenCms.getRoleManager().getManageableOrgUnits(A_CmsUI.getCmsObject(), "", true, false);
    for (CmsSessionInfo session : sessionInfos) {
      ...
      CmsOrganizationalUnit userOu =
          OpenCms.getOrgUnitManager()
              .readOrganizationalUnit(A_CmsUI.getCmsObject(), user.getOuFqn());
      ...
      item.getItemProperty(TableProperty.DateCreated)
          .setValue(
              session.getAgeOfSession()
                  + " "
                  + CmsVaadinUtils.getMessageText(Messages.GUI_MESSAGES_HOUR_0));
      item.getItemProperty(TableProperty.IS_ACTIVE)
          .setValue(new Long(System.currentTimeMillis() - session.getTimeLastAction()));
      item.getItemProperty(TableProperty.OrgUnit).setValue(userOu.getName());
      item.getItemProperty(TableProperty.Project)
          .setValue(A_CmsUI.getCmsObject().readProject(session.getProject()).getName());
      ...
      item.getItemProperty(TableProperty.IS_WAITING)
          .setValue(new Boolean(!session.getBroadcastQueue().isEmpty()));
    }
  ...
  /**
   * Shows window with user information.
   *
   * <p>
   *
  ...
  /**
   * Handles the table item clicks, including clicks on images inside of a table item.
   *
   * <p>
   *
    ...
    if (!event.isCtrlKey() && !event.isShiftKey()) {
      ...
      if (event.getButton().equals(MouseButton.RIGHT) || (TableProperty.Icon.equals(propertyId))) {
        ...
        m_menu.openForTable(event, itemId, propertyId, this);
          ...
          && TableProperty.UserName.equals(propertyId)) {
