...
public class CmsUserInfoDialog extends CmsBasicDialog {
  ...
  public static void showUserInfo(CmsSessionInfo session) {
    ...
    CmsUserInfoDialog dialog =
        new CmsUserInfoDialog(
            session,
            new Runnable() {
  ...
  /**
   * Adds information lines to the user dialog.
   *
   * <p>
   *
  ...
  private List<String> getFurtherInfoLines(CmsSessionInfo currentSession) {
    ...
    if (!neverActive) {
      if (currentSession != null) {
        res.add("");
        res.add(
            "<p>" + CmsVaadinUtils.getMessageText(Messages.GUI_SESSIONS_SESSION_INFO_0) + "</p>");
        res.add(
    ...
    res.add(
        CmsVaadinUtils.getMessageText(Messages.GUI_USERMANAGEMENT_USER_DATE_CREATED_0)
            ...
            + CmsDateUtil.getDateTime(
                new Date(m_user.getDateCreated()), DateFormat.SHORT, A_CmsUI.get().getLocale()));
    if (currentSession != null) {
  ...
  /**
   * Get Message for show last login information.
   *
   * <p>
   *
  ...
  private String getLastLoginMessage(Long inacTime) {
    ...
    return CmsVaadinUtils.getMessageText(
        Messages.GUI_USER_INFO_LAST_LOGIN_DAYS_AGO_1, new Integer(days));
  }

  /**
   * Gets the most current session for given user.
   *
   * <p>
   *
  ...
  /**
   * Gets the title for the site of the session.
   *
   * <p>
   *
  ...
  private String getStatusHTML(Long inacTime) {
    ...
    return "<span class=\""
        + getStatusStyleForItem(inacTime)
        + "\">"
        + getStatusForItem(inacTime)
        + "</span> ";
  }
            ...
            public void buttonClick(ClickEvent event) {
              ...
              try {
                ...
                if (path == null) {
                  ...
                  parameters.put(
                      "_lrid", new String[] {String.valueOf(System.currentTimeMillis())});
                  path =
