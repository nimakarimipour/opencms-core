 ...
 *
 * <p>Generates a user data report either for a selected OpenCms user, or for an email address. The
 * report is constructed by the I_CmsUserDataDomain plugins configured in opencms-system.xml.
 */
public class CmsUserDataAppPanel extends VerticalLayout {
  ...
  public CmsUserDataAppPanel() {
    ...
    m_filterGroup =
        new CmsEditableGroup(
            ...
            CmsVaadinUtils.getMessageText(
                org.opencms.ui.apps.Messages.GUI_USERDATA_TEXT_FILTER_ADD_0));
    ...
    m_pickUserButton.addClickListener(
        evt -> {
          ...
          window.setCaption(
              CmsVaadinUtils.getMessageText(
                  org.opencms.ui.apps.Messages.GUI_USERDATA_SELECT_USER_0));
          window.setContent(dialog);
          A_CmsUI.get().addWindow(window);
        });

    m_searchByEmail.addClickListener(
        event -> {
          hideResult();
          List<String> filterStrings = new ArrayList<>();
          m_filterGroup.getRows().stream()
              .forEach(
                  row -> {
          ...
          try {
            ...
            try (InputStream stream =
                getClass()
                    .getClassLoader()
                    .getResourceAsStream("VAADIN/themes/opencms/userdata.css")) {
              String style =
                  new String(CmsFileUtil.readFully(stream, false), StandardCharsets.UTF_8);
            ...
            String headerSearchTermsString =
                headerSearchTerms.stream()
                    .map(s -> "\"" + s + "\"")
                    .collect(Collectors.joining(", "));
            doc.body()
                .appendElement("h1")
                .attr("class", "udr-header")
                .text(
                    Messages.get()
                        .getBundle(cms.getRequestContext().getLocale())
                        .key(Messages.GUI_USER_INFORMATION_FOR_1, headerSearchTermsString));

            CmsUserDataReportThread thread =
                new CmsUserDataReportThread(
                    cms,
                    report -> {
                      try {
                        boolean changed =
                            OpenCms.getUserDataRequestManager()
                                .getInfoForEmail(
                                    cms,
    ...
    m_searchByUser.addClickListener(
        evt -> {
          hideResult();
          ...
          try {
            ...
            m_user.setComponentError(
                new UserError(
                    CmsVaadinUtils.getMessageText(
                        org.opencms.ui.apps.Messages.GUI_USERDATA_USER_NOT_FOUND_0)));
            return;
          }
          ...
          try {
            ...
            doc.body()
                ...
                .text(
                    Messages.get()
                        .getBundle(cms.getRequestContext().getLocale())
                        .key(Messages.GUI_USER_INFORMATION_FOR_1, user));
   ...
   *
   * <p>The CmsObject's locale needs to be set to the current locale, because the plugins used to
   * generate the report do not use a separate locale parameter.
   *
  ...
  private void showResult(Document doc) {
    ...
    if (doc != null) {
      ...
      hideResult();
      String notfound =
          CmsVaadinUtils.getMessageText(org.opencms.ui.apps.Messages.GUI_USERDATA_NOT_FOUND_0);
