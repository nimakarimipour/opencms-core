...
public class CmsEditor
  ...
  public static String getEditState(CmsUUID structureId, boolean plainText, String backLink) {
    ...
    state =
        A_CmsWorkplaceApp.addParamToState(
            state, CmsEditor.RESOURCE_ID_PREFIX, structureId.toString());
    state =
        A_CmsWorkplaceApp.addParamToState(
            state, CmsEditor.PLAIN_TEXT_PREFIX, String.valueOf(plainText));
  ...
  public static String getEditStateForNew(
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(modelFilePath)) {
      ...
      state =
          A_CmsWorkplaceApp.addParamToState(
              state, CmsEditorConstants.PARAM_MODE, CmsEditorConstants.MODE_COPY);
    }
  ...
  public static void openBackLink(String backlink) {
    ...
    if (!CmsRequestUtil.checkBacklink(
        backlink, ((VaadinServletRequest) VaadinRequest.getCurrent()))) {
    ...
    try {
      ...
      if (CmsStringUtil.isEmptyOrWhitespaceOnly(newLink)) {
        ...
        OpenCms.getRoleManager()
            .checkRoleForResource(cms, CmsRole.ELEMENT_AUTHOR, cms.getSitePath(resource));
        editor =
            OpenCms.getWorkplaceAppManager()
                .getEditorForResource(cms, resource, isPlainText(state));
      } else {
  ...
  /**
   * Returns the back link info from the given state.
   *
   * <p>
   *
  ...
  /**
   * Returns the resource id form the given state.
   *
   * <p>
   *
   * @param state the state
  ...
  /**
   * Returns the resource path from the given state.
   *
   * <p>
   *
   * @param state the state
