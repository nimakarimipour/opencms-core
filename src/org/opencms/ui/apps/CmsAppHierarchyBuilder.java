...
/**
 * Helper class for building a tree of categories/apps which should be displayed from the list of
...
public class CmsAppHierarchyBuilder {
   ...
   *
   * <p>This tree will only include those categories which are reachable by following the parent
   * chain of an available app configuration up to the root category (null).
   *
  ...
  public CmsAppCategoryNode buildHierarchy() {
    ...
    Collections.sort(
        m_appCategoryList,
        new Comparator<I_CmsAppCategory>() {
  ...
  private Set<String> findReachableNodes(
      CmsAppCategoryNode rootNode, HashSet<String> reachableNodes) {
