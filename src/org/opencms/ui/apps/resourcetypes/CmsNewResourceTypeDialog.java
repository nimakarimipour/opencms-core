...
public class CmsNewResourceTypeDialog extends CmsBasicDialog {
  ...
  /**
   * Validator for the bundle resource field.
   *
   * <p>
   */
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      try {
        ...
        if (!OpenCms.getResourceManager()
            .getResourceType(res)
            .equals(OpenCms.getResourceManager().getResourceType("propertyvfsbundle"))) {
          throw new InvalidValueException(
              CmsVaadinUtils.getMessageText(
                  Messages.GUI_RESOURCETYPE_EDIT_INVALID_RESORUCE_NO_VFSBUNDLE_0));
        }
  ...
  class ResourceValidator implements Validator {
    ...
    public void validate(Object value) throws InvalidValueException {
      ...
      if (CmsResource.getName(resource).equals(SCHEMA)
          || CmsResource.getName(resource).equals(FORMATTER)) {
  ...
  /**
   * Tries to find the module folder under /system/modules for a given module.
   *
   * <p>
   *
  ...
  /**
   * Opens the module select dialog.
   *
   * <p>
   *
   * @param window window
  ...
  protected void setModule(String moduleName, CmsBasicDialog dialog) {
    ...
    CmsResourceInfo resInfo =
        new CmsResourceInfo(
            m_module.getName(), m_module.getNiceName(), CmsModuleApp.Icons.RESINFO_ICON);
  ...
  /**
   * Adds the given resource to the module if necessary.
   *
   * <p>
   *
  ...
  private void addTypeMessages(CmsExplorerTypeSettings setting, String moduleFolder)
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_typeName.getValue())) {
      String key =
          String.format(
              MESSAGE_KEY_FORMATTER, m_typeShortName.getValue(), MESSAGE_KEY_FORMATTER_NAME);
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_typeDescription.getValue())) {
      String key =
          String.format(
              MESSAGE_KEY_FORMATTER, m_typeShortName.getValue(), MESSAGE_KEY_FORMATTER_DESCRIPTION);
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_typeName.getValue())) {
      ...
      String key2 =
          String.format(
              MESSAGE_KEY_FORMATTER, m_typeShortName.getValue(), MESSAGE_KEY_FORMATTER_FORMATTER);
  ...
  private void adjustModuleConfig() {
    ...
    try {
      ...
      createParentXmlElements(
          ...
          l);
      I_CmsXmlContentValue v =
          xmlContent.getValue(
              XMLPath.CONFIG_RESOURCETYPE
                  + XMLPath.num(number)
                  + XMLPath.CONFIG_RESOURCETYPE_TYPENAME,
              l);
      v.setStringValue(m_cms, m_typeShortName.getValue());
      v =
          xmlContent.addValue(
              ...
              XMLPath.CONFIG_RESOURCETYPE
                  + XMLPath.num(number)
                  + XMLPath.CONFIG_RESOURCETYPE_NAMEPATTERN,
              l,
  ...
  private void adjustSchema(String schemaPath, String newElementString) {
    ...
    newElementString =
        newElementString.substring(0, 1).toUpperCase() + newElementString.substring(1);
    try {
      ...
      String encoding =
          m_cms
              .readPropertyObject(file, CmsPropertyDefinition.PROPERTY_CONTENT_ENCODING, true)
  ...
  private void createResourceType() {
    ...
    try {
      ...
      List<CmsExplorerTypeSettings> settings =
          new ArrayList<CmsExplorerTypeSettings>(m_module.getExplorerTypes());
      ...
      OpenCms.getPublishManager()
          .publishProject(
              m_cms, new CmsLogReport(m_cms.getRequestContext().getLocale(), getClass()));
  ...
  /**
   * Returns the property string for all available locales.
   *
   * <p>
   *
  ...
  /**
   * Returns a random valid resource type id.
   *
   * <p>
   *
   * @return valid id
  ...
  private CmsResource getMessageBundle() {
    ...
    String bundlePath =
        CmsStringUtil.joinPaths(
            moduleFolder, PATH_I18N, m_module.getName() + SUFFIX_BUNDLE_FILE + "_" + localString);
  ...
  /**
   * Creates a name pattern for the resource type.
   *
   * <p>
   *
  ...
  private void init(Window window, CmsResourceTypeApp app) {
    ...
    m_parentSchema.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
    m_parentFormatter.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
    m_typeID.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
    m_typeDescription.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
    m_typeShortName.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
    m_typeXPathName.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
    m_bundle.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));

    m_typeName.setRequiredError(
        CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_NOT_EMPTY_0));
