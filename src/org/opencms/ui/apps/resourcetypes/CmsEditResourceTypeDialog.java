...
public class CmsEditResourceTypeDialog extends CmsBasicDialog {
   ...
   */
  public CmsEditResourceTypeDialog(
      Window window, CmsResourceTypeApp app, I_CmsResourceType resourceType) {
    ...
    if (!CmsStringUtil.isEmptyOrWhitespaceOnly(m_type.getModuleName())) {
      ...
      CmsExplorerTypeSettings dummy =
          OpenCms.getWorkplaceManager().getExplorerTypeSetting(m_type.getTypeName());
    ...
    m_iconMode.addValueChangeListener(
  ...
  /**
   * Change Id, name or schema (all require to delete old type and create new one).
   *
   * <p>
   *
   * @param module Module
  ...
  private void changeIdNameOrSchema(CmsModule module) throws CmsConfigurationException {
    ...
    type.addConfigurationParameter(
        CmsResourceTypeXmlContent.CONFIGURATION_SCHEMA, m_schema.getValue());
    type.setAdditionalModuleResourceType(true);
    type.setModuleName(module.getName());
    type.initConfiguration(
        m_typeShortName.getValue(), m_typeID.getValue(), CmsResourceTypeXmlContent.class.getName());
    types.add(type);
    ...
    setting.setBigIcon(
        ICON_MODE_CSS.equals(m_iconMode.getValue())
            ? null
            : CmsStringUtil.isEmptyOrWhitespaceOnly(m_bigIconFile.getValue())
                ? null
                : m_bigIconFile.getValue());
    setting.setIcon(
        ICON_MODE_CSS.equals(m_iconMode.getValue())
            ? null
    ...
    setting.setBigIconStyle(
        ICON_MODE_CSS.equals(m_iconMode.getValue())
            ? CmsStringUtil.isEmptyOrWhitespaceOnly(m_bigIconCSS.getValue())
                ? null
                : m_bigIconCSS.getValue()
            : null);
    setting.setSmallIconStyle(
        ICON_MODE_CSS.equals(m_iconMode.getValue())
            ? CmsStringUtil.isEmptyOrWhitespaceOnly(m_smallIconCSS.getValue())
                ? null
                : m_smallIconCSS.getValue()
            : null);
  ...
  private boolean isKeepTypeCase() {
    ...
    if (m_type instanceof CmsResourceTypeXmlContent) {
      ...
      res =
          res
              && ((CmsResourceTypeXmlContent) m_type)
                  .getSchema()
                  .equals(m_schema.getValue()); // same schema
    }
