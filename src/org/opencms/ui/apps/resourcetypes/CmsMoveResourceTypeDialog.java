...
public class CmsMoveResourceTypeDialog extends CmsBasicDialog {
  ...
  public CmsMoveResourceTypeDialog(CmsNewResourceTypeDialog dialog) {
    ...
    m_cancel.addClickListener(
        e -> CmsVaadinUtils.getWindow(CmsMoveResourceTypeDialog.this).close());
  }
  ...
  public CmsMoveResourceTypeDialog(final Window window, I_CmsResourceType type) {
    ...
    m_table.setCurrentPageFirstItemId(
        new CmsModuleRow(OpenCms.getModuleManager().getModule(type.getModuleName())));

    if (m_type instanceof CmsResourceTypeXmlContent) {
      m_typeXML = (CmsResourceTypeXmlContent) m_type;
      if (!OpenCms.getModuleManager()
          .getModule(m_type.getModuleName())
          .getResources()
          .contains(m_typeXML.getSchema())) {
        m_schemaOK = false;
        m_ok.setEnabled(false);
        m_moveAnyway.addValueChangeListener(
  ...
  protected void updateResourceType(Window window) {
    ...
    if (!((CmsModuleRow) m_table.getValue())
        .equals(new CmsModuleRow(OpenCms.getModuleManager().getModule(m_type.getModuleName())))) {
      ...
      List<CmsExplorerTypeSettings> oldSettings =
          new ArrayList<CmsExplorerTypeSettings>(oldModule.getExplorerTypes());
      ...
      List<CmsExplorerTypeSettings> newSettings =
          new ArrayList<CmsExplorerTypeSettings>(newModule.getExplorerTypes());
  ...
  private void init(final Window window) {
    ...
    m_filter.addTextChangeListener(
