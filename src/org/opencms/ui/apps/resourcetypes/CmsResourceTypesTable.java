...
public class CmsResourceTypesTable extends Table {
  ...
  /**
   * The delete project context menu entry.
   *
   * <p>
   */
                  ...
                  try {
                    ...
                    while (it.hasNext()) {
                      I_CmsResourceType type =
                          OpenCms.getResourceManager().getResourceType(it.next());
                      CmsModule module;
                      ...
                      List<CmsExplorerTypeSettings> typeSettings =
                          Lists.newArrayList(module.getExplorerTypes());
                      List<CmsExplorerTypeSettings> newTypeSettings =
                          new ArrayList<CmsExplorerTypeSettings>();
                      ...
                      List<I_CmsResourceType> types =
                          new ArrayList<I_CmsResourceType>(module.getResourceTypes());
                    ...
                    for (String moduleName : modulesToBeUpdated.keySet()) {
                      OpenCms.getModuleManager()
                          .updateModule(m_cms, modulesToBeUpdated.get(moduleName));
                      OpenCms.getResourceManager().initialize(m_cms);
                      OpenCms.getWorkplaceManager()
                          .addExplorerTypeSettings(modulesToBeUpdated.get(moduleName));
                    }
          ...
          CmsAppWorkplaceUi.get()
              .showApp(
                  OpenCms.getWorkplaceAppManager()
                      .getAppConfiguration(CmsEditorConfiguration.APP_ID),
                  editState);
        }
  ...
  /**
   * Filters the table according to given string.
   *
   * <p>
   *
  ...
  void init() {
    ...
    for (I_CmsResourceType type : CmsVaadinUtils.getResourceTypes()) {
      ...
      item.getItemProperty(TableProperty.Icon)
          .setValue(CmsResourceUtil.getBigIconResource(typeSetting, null));
      item.getItemProperty(TableProperty.Name)
    ...
    setVisibleColumns(
        TableProperty.Name, TableProperty.ShortName, TableProperty.ID, TableProperty.Module);
  }
    ...
    if (!event.isCtrlKey() && !event.isShiftKey()) {
      ...
      if (event.getButton().equals(MouseButton.RIGHT) || (propertyId == null)) {
        ...
        m_menu.openForTable(event, itemId, propertyId, this);
          ...
          && TableProperty.Name.equals(propertyId)) {
    ...
    try {
      ...
      window.setContent(
          new CmsEditResourceTypeDialog(
              window, m_app, OpenCms.getResourceManager().getResourceType(typeName)));
      window.setCaption(
          CmsVaadinUtils.getMessageText(Messages.GUI_RESOURCETYPE_EDIT_WINDOW_CAPTION_0));
      A_CmsUI.get().addWindow(window);
    } catch (CmsLoaderException e) {
      LOG.error("Unable to read resource type by name", e);
    }
