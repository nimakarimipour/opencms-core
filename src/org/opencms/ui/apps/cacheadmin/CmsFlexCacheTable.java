...
public class CmsFlexCacheTable extends Table {
  ...
  /**
   * Menu entry for show variations option.
   *
   * <p>
   */
  ...
  public CmsFlexCacheTable() {
    ...
    m_container.addContainerProperty(
        PROP_ICON, Resource.class, new CmsCssIcon(OpenCmsTheme.ICON_CACHE));
    ...
    setColumnHeader(
        PROP_PROJECT,
        CmsVaadinUtils.getMessageText(Messages.GUI_CACHE_FLEXCACHE_LIST_COLS_PROJECT_0));
    setColumnHeader(
        PROP_KEY, CmsVaadinUtils.getMessageText(Messages.GUI_CACHE_FLEXCACHE_LIST_COLS_KEY_0));
    setColumnHeader(
        PROP_VARIATIONS,
        ...
        new ItemClickListener() {
          ...
          public void itemClick(ItemClickEvent event) {
            ...
            if (event.getButton().equals(MouseButton.RIGHT) || (event.getPropertyId() == null)) {
              ...
              m_menu.openForTable(
                  event, event.getItemId(), event.getPropertyId(), CmsFlexCacheTable.this);
            }

            if (event.getButton().equals(MouseButton.LEFT)
                & PROP_RESOURCENAME.equals(event.getPropertyId())) {
  ...
  /**
   * Returns a cms object at root-site.
   *
   * <p>
   *
   * @return cmsobject
  ...
  /**
   * Shows dialog for variations of given resource.
   *
   * <p>
   *
  ...
  void showVariationsWindow(@RUntainted String resource) {
    ...
    CmsVariationsDialog variationsDialog =
        new CmsVariationsDialog(
            ...
            Mode.FlexCache);
    try {
      CmsResource resourceObject =
          getRootCms().readResource(CmsFlexCacheKey.getResourceName(resource));
      variationsDialog.displayResourceInfo(Collections.singletonList(resourceObject));
    } catch (CmsException e) {
      //
    }
    window.setCaption(
        CmsVaadinUtils.getMessageText(Messages.GUI_CACHE_VIEW_FLEX_VARIATIONS_1, resource));
    window.setContent(variationsDialog);
    UI.getCurrent().addWindow(window);
  }

  /**
   * Reads flex cache entries and puts them to table.
   *
   * <p>
   */
  private void loadTableEntries() {
    ...
    Iterator<String> itResources =
        new ArrayList<String>(m_cache.getCachedResources(A_CmsUI.getCmsObject())).iterator();
    while (itResources.hasNext()) {
      ...
      if (resource.endsWith(CmsFlexCache.CACHE_OFFLINESUFFIX)) {
        resName =
            resource.substring(0, resource.length() - CmsFlexCache.CACHE_OFFLINESUFFIX.length());
        project = "Offline";
      }
      if (resource.endsWith(CmsFlexCache.CACHE_ONLINESUFFIX)) {
        resName =
            resource.substring(0, resource.length() - CmsFlexCache.CACHE_ONLINESUFFIX.length());
        project = "Online";
      }
      ...
      item.getItemProperty(PROP_VARIATIONS)
          .setValue(
