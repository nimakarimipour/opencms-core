...
public class CmsEditModuleForm extends CmsBasicDialog {
  ...
  public CmsEditModuleForm(CmsModule module, boolean newModule, Runnable updateCallback) {
    ...
    IndexedContainer importSitesModel =
        getModuleSiteContainer(A_CmsUI.getCmsObject(), PROPERTY_SITE_NAME, m_module.getSite());
    ...
    for (AbstractField<String> field :
        new AbstractField[] {m_name, m_niceName, m_group, m_importScript, m_actionClass}) {
    ...
    Supplier<Component> moduleResourceFieldFactory =
    ...
    String addResourceButtonText =
        CmsVaadinUtils.getMessageText(Messages.GUI_MODULES_ADD_RESOURCE_0);
    m_moduleResourcesGroup =
        new CmsEditableGroup(m_moduleResources, moduleResourceFieldFactory, addResourceButtonText);
    m_excludedResourcesGroup =
        new CmsEditableGroup(
            m_excludedResources, moduleResourceFieldFactory, addResourceButtonText);
    m_parameterGroup =
        new CmsEditableGroup(
            m_parameters,
            new Supplier<Component>() {
    ...
    String resourceListError =
        CmsVaadinUtils.getMessageText(Messages.GUI_MODULES_RESOURCE_LIST_ERROR_0);
    ...
    m_importSite.addValueChangeListener(
    ...
    m_info
        .get()
        .getResourceIcon()
        .initContent(null, CmsModuleApp.Icons.RESINFO_ICON, null, false, false);
   ...
   */
  public static IndexedContainer getModuleSiteContainer(
      CmsObject cms, String captionPropertyName, String prevValue) {
  ...
  public void updateModule() {
    ...
    try {
      ...
      for (CmsModuleDependencyWidget widget :
          getFormRowChildren(m_dependencies, CmsModuleDependencyWidget.class)) {
        String moduleName = widget.getModuleName();
        String moduleVersion = widget.getModuleVersion();
        try {
          CmsModuleDependency dep =
              new CmsModuleDependency(moduleName, new CmsModuleVersion(moduleVersion));
  ...
  /**
   * Adds a new module dependency widget.
   *
   * <p>
   *
  ...
  /**
   * Adds a new resource selection widget to the list of module resources.
   *
   * <p>
   *
  ...
  /**
   * Adds a new module resource row.
   *
   * <p>
   *
  ...
  /**
   * Creates a module resource selection field.
   *
   * <p>
   *
  ...
  /**
   * Helper method to get the descendants of a container with a specific widget type.
   *
   * <p>
   *
   ...
   */
  <T extends Component> List<T> getFormRowChildren(
      AbstractComponentContainer container, final Class<T> cls) {
  ...
  /**
   * Gets the site root currently selected in the module site combo box.
   *
   * <p>
   *
  ...
  /**
   * Gets the tab index for the given component.
   *
   * <p>
   *
  ...
  /**
   * Creates all module folders that are selected in the input form.
   *
   * <p>
   *
  ...
  /**
   * Updates the module site info display after the module site is changed.
   *
   * <p>
   *
      ...
      if (site != null) {
        bottom =
            CmsVaadinUtils.getMessageText(
                Messages.GUI_MODULES_MODULE_SITE_1, site.getTitle() + " (" + siteRoot + ")");
