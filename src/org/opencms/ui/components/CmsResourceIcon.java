 ...
 *
 * <p>Important: To avoid issues with click event propagation within tables, we are required to
 * extent the Label component.
 */
public class CmsResourceIcon extends Label {
   ...
   * Constuctor.
   *
   * <p>To be used in declarative layouts. Make sure to call initContent later on.
   *
   * <p>
   */
  ...
  public static String getDefaultFileOrDetailType(CmsObject cms, CmsResource resource) {
    ...
    if (resource.isFolder()
        && !(OpenCms.getResourceManager().getResourceType(resource)
            instanceof CmsResourceTypeFolderExtended)
  ...
  public static String getDetailType(
      CmsObject cms, CmsResource detailPage, CmsResource parentFolder) {
  ...
  public static String getIconHTML(
      CmsResourceUtil resUtil, CmsResourceState state, boolean showLocks) {
  ...
  /**
   * Gets the resource icon for a resource for use in a CmsResourceInfo widget when used in a
   * sitemap context.
   *
   * <p>
   *
   ...
   */
  public static Resource getSitemapResourceIcon(
      CmsObject cms, CmsResource resource, IconMode iconMode) {
    ...
    boolean skipDefaultFile =
        (iconMode == IconMode.sitemapSelect)
            && OpenCms.getResourceManager()
                .matchResourceType(
                    CmsResourceTypeFolderSubSitemap.TYPE_SUBSITEMAP, resource.getTypeId());
    if (resource.isFolder() && !skipDefaultFile) {
      ...
      try {
        ...
        // Shouldn't normally happen - readDefaultFile returns null instead of throwing an exception
        // when it doesn't find a default file
      }
    }
    ...
    if (CmsResourceTypeXmlContainerPage.isContainerPage(maybePage)) {
      CmsADEConfigData config =
          OpenCms.getADEManager().lookupConfiguration(cms, maybePage.getRootPath());
      for (CmsDetailPageInfo realInfo : config.getAllDetailPages(true)) {
        if (realInfo.getUri().equals(maybePage.getRootPath())
          ...
          CmsExplorerTypeSettings settings =
              OpenCms.getWorkplaceManager().getExplorerTypeSetting(realInfo.getIconType());
          if (settings != null) {
  ...
  public static String getTreeCaptionHTML(
      String resourceName, CmsResourceUtil resUtil, CmsResourceState state, boolean showLocks) {
  ...
  private static String getIconInnerHTML(
      CmsResourceUtil resUtil, CmsResourceState state, boolean showLocks, boolean showDetailIcon) {
  ...
  /**
   * Generates an overlay icon span.
   *
   * <p>
   *
  ...
  /**
   * Returns the URI of the small resource type icon for the given type.
   *
   * <p>
   *
        ...
        result =
            "<img src=\""
                + CmsWorkplace.getResourceUri(
                    CmsWorkplace.RES_PATH_FILETYPES + typeSettings.getIcon())
                + "\" class=\""
                + overlayClass
                + "\" />";
      } else {
