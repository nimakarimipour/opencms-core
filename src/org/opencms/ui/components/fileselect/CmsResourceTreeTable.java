...
public class CmsResourceTreeTable extends TreeTable {
      ...
      if (CAPTION_FOLDERS.equals(propertyId)) {
        ...
        return (sortDirection ? 1 : -1)
            * ComparisonChain.start()
                .compareTrueFirst(isFolder1.booleanValue(), isFolder2.booleanValue())
                .compare(name1, name2)
                .result();
  ...
  public class StyleGenerator implements CellStyleGenerator {
    ...
    /**
     * @see com.vaadin.ui.Table.CellStyleGenerator#getStyle(com.vaadin.ui.Table, java.lang.Object,
     *     java.lang.Object)
     */
  ...
  /**
   * Predicate which can be used to prevent selection of a node (this means the select handler will
   * not be called if this returns false, but the Vaadin selection mechanism will still mark it as
   * selected.
   */
  ...
  public CmsResourceTreeTable(CmsObject cms, CmsResource root, CmsResourceTreeContainer container) {
    ...
    ColumnGenerator captionGenerator =
        new ColumnGenerator() {
    ...
    setColumnHeader(
        ...
        CmsVaadinUtils.getMessageText(
            CmsResourceTableProperty.PROPERTY_NAVIGATION_TEXT.getHeaderKey()));
    setColumnHeader(
        CAPTION_FOLDERS,
        CmsVaadinUtils.getMessageText(
            CmsResourceTableProperty.PROPERTY_RESOURCE_NAME.getHeaderKey()));
    ...
    addExpandListener(
        new ExpandListener() {
          ...
          public void nodeExpand(@RUntainted ExpandEvent event) {
            ...
            markAsDirtyRecursive(); // required so open / close arrows on folders without contents
            ...
            if (isSelectable(event.getItem())) {
              CmsResource resource =
                  (CmsResource)
                      (event
                          .getItem()
                          .getItemProperty(CmsResourceTreeContainer.PROPERTY_RESOURCE)
                          .getValue());
  ...
  /**
   * Sets the selection filter, a predicate that, by returning false for an item, can veto a tree
   * entry selection.
   *
   * <p>
   *
