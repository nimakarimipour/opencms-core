...
public class CmsMacroResolverDialog extends CmsBasicDialog {
  ...
  public CmsMacroResolverDialog(Runnable okAction, Runnable cancelAction, CmsResource resource) {
    ...
    try {
      ...
      Map<String, String[]> bundleKeyDescriptorMap =
          CmsMacroResolver.getBundleMapFromResources(resourceBundle, descriptor, cms);
  ...
  /**
   * Reads the entered values for the macro and returns them together with their keys.
   *
   * <p>
   *
   * @return Maps
  ...
  /**
   * Returns the correct variant of a resource name according to locale.
   *
   * <p>
   *
   * @param cms CmsObject
   ...
   */
  private Properties getLocalizedBundle(CmsObject cms, String path)
        ...
        cms.readResource(
            path
                ...
                + getAvailableLocalVariant(
                    cms, path + CmsSiteManager.MACRO_FOLDER + "/", CmsSiteManager.BUNDLE_NAME));
