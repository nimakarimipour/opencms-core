...
public class CmsToolBar extends CssLayout implements BrowserWindowResizeListener {
  ...
  public CmsToolBar() {
    ...
    m_foldedButtonsMenu =
        new PopupView(getDropDownButtonHtml(FontOpenCms.CONTEXT_MENU_DOTS), layout);
    ...
    m_favButton.addClickListener(
        evt -> {
          CmsFileExplorer explorer =
              (CmsFileExplorer) m_appContext.getAttribute(CmsFileExplorer.ATTR_KEY);
   ...
   * @param icon the button icon
   * @param title the button title, will be used for the tooltip
   * @param alwaysShow <code>true</code> to prevent the button to be folded into a sub menu for
   *     small screens
   * @return the button
   */
  ...
  public static void openFavoriteDialog(CmsFileExplorer explorer) {
    ...
    try {
      CmsExplorerFavoriteContext context =
          new CmsExplorerFavoriteContext(A_CmsUI.getCmsObject(), explorer);
      CmsFavoriteDialog dialog =
          new CmsFavoriteDialog(context, new CmsFavoriteDAO(A_CmsUI.getCmsObject()));
      Window window = CmsBasicDialog.prepareWindow(DialogWidth.max);
      window.setContent(dialog);
      window.setCaption(
          CmsVaadinUtils.getMessageText(org.opencms.ui.Messages.GUI_FAVORITES_DIALOG_TITLE_0));
  ...
  private void createMenuEntry(
    ...
    MenuItem entry =
        parent.addItem((node.getData().getTitle(A_CmsUI.get().getLocale())), entryCommand);
        ...
        new PopupView(
            new PopupView.Content() {
              ...
              public Component getPopupComponent() {
                ...
                for (I_CmsWorkplaceAppConfiguration config :
                    OpenCms.getWorkplaceAppManager().getQuickLaunchConfigurations(cms)) {
                  layout.addComponent(
                      CmsDefaultAppButtonProvider.createAppButton(cms, config, locale));
                }
                return layout;
              }
            });
      ...
      for (Component comp : items) {
        if (comp == m_foldedButtonsMenu) {
          continue;
            ...
            || (comp instanceof PopupView)
            || (comp instanceof MenuBar)) {
  ...
  private void initContextMenu() {
    ...
    CmsTreeNode<I_CmsContextMenuItem> tree =
        treeBuilder.buildAll(OpenCms.getWorkplaceAppManager().getMenuItemProvider().getMenuItems());
