 ...
 *
 * <p>This class is not itself a widget, it just coordinates the other widgets actually used to
 * display the multivalue widget group.
 */
public class CmsEditableGroup {

  /**
   * Empty handler which shows or hides an 'Add' button to add new rows, depending on whether the
   * group is empty.
   */
  ...
  public static class DefaultRowBuilder implements CmsEditableGroup.I_RowBuilder {
    ...
    public I_CmsEditableGroupRow buildRow(CmsEditableGroup group, Component component) {
      ...
      if (component instanceof Layout.MarginHandler) {
        ...
        // widget, a margin on the widget causes it to be vertically offset from the buttons too
        // much
   ...
   * @param container the container in which to render the individual rows
   * @param componentFactory the factory used to create new input fields
   * @param addButtonCaption the caption for the button which is used to add a new row to an empty
   *     list
   */
  public CmsEditableGroup(
      AbstractOrderedLayout container,
  ...
  /**
   * Shows or hides the error label.
   *
   * <p>
   *
  ...
  /**
   * Updates the visibility of the error label based on errors in the group components.
   *
   * <p>
   */
