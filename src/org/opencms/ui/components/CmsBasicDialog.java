...
public class CmsBasicDialog extends VerticalLayout {
  ...
  public static Window prepareWindow(DialogWidth width) {
    ...
    if (pageWidth
        != 0) { // page width 0 can happen for first dialog opened in embedded mode in page editor
      ...
      if (width == DialogWidth.max) {
        // in case max width would result in a width very close to wide or narrow, use their static
        // width instead of relative width
  ...
  /**
   * For a given resource, display the resource info panel.
   *
   * <p>
   *
  ...
  /**
   * For a given list of resources, displays the resource info panels.
   *
   * <p>
   *
   * @param resources the resources
  ...
  /**
   * For a given list of resources, displays the resource info panels with panel messages.
   *
   * <p>
   *
      ...
      if (resources.size() == 1) {
        ...
        m_infoComponent =
            createResourceListPanel(
                messageKey == null
                    ? null
                    : Messages.get().getBundle(A_CmsUI.get().getLocale()).key(messageKey),
                resources);
        m_mainPanel.addComponent(m_infoComponent, 0);
  ...
  /**
   * @see com.vaadin.ui.AbstractOrderedLayout#readDesign(org.jsoup.nodes.Element,
   ...
   *
   * <p>Set this before opening the window, so it will be initialized properly.
   *
   * <p>
   *
  ...
  protected void enableMaxHeight() {
    ...
    // only center window for height changes that exceed the maximum height since the last window
    // resize
    // (window resize handler resets this)
    m_maxHeightExtension.addHeightChangeHandler(
        new CmsMaxHeightExtension.I_HeightChangeHandler() {
