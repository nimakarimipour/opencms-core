...
public class CmsPagingControls extends HorizontalLayout {
  ...
  /** The result count. */
  private @RUntainted int m_resultCount;
  ...
  public CmsPagingControls() {
    ...
    m_fastForward.addClickListener(
        new ClickListener() {
  ...
  /**
   * Notifies the registered listeners of a page change.
   *
   * <p>
   *
  ...
  /**
   * Resets the paging controls (used when the size of the underlying list changes).
   *
   * <p>
   *
  ...
  public void setPage(@RUntainted int page, boolean fireChanged) {
    ...
    @RUntainted int start = (m_page * m_pageSize) + 1;
    @RUntainted int end = Math.min((start + m_pageSize) - 1, m_resultCount);
    String resultsMsg =
        CmsVaadinUtils.getMessageText(
            Messages.GUI_DATAVIEW_RESULTS_3, "" + start, "" + end, "" + m_resultCount);
  ...
  /**
   * Updates the button state, i.e. enables/disables the buttons depending on whether we are on the
   * first or last page or not.
   *
   * <p>
   */
