 ...
 *
 * <p>The columns of the table correspond to getters of the given bean class with the Column
 * annotation.
 *
...
public class CmsBeanTableBuilder<T> {
  ...
  /**
   * Contains information about a single column.
   *
   * <p>
   */
   ...
   *
   * <p>Depending on the view configuration of the columns, columns may be hidden depending on the
   * view.
   *
  ...
  public CmsBeanTableBuilder(Class<T> cls, String view) {
    ...
    try {
      ...
      Collections.sort(
          m_columns,
          new Comparator<ColumnBean>() {

            public int compare(
                CmsBeanTableBuilder<T>.ColumnBean col1, CmsBeanTableBuilder<T>.ColumnBean col2) {

              return ComparisonChain.start()
                  .compare(col1.getInfo().order(), col2.getInfo().order())
                  .result();
            }
          });
  ...
  /**
   * Sets up a table and uses the given beans to fill its rows, but does not actually create the
   * table instance; it uses the passed in table instance instead.
   *
   * <p>
   *
  ...
  /**
   * Creates a default cell style generator which just returns the value of the styleName attribute
   * in a Column annotation for cells in that column.
   *
   * <p>
   *
  ...
  /**
   * Creates a default filter which just searches the lower case version of the result of the
   * toString() method applied to all columns with the annotation attribute filterable = true.
   *
   * <p>
   *
      ...
      public boolean passesFilter(Object itemId, Item item) throws UnsupportedOperationException {
        ...
        for (ColumnBean col : m_columns) {
          if (col.getInfo().filterable()) {
            if (("" + item.getItemProperty(col.getProperty().getName()).getValue())
                .toLowerCase()
  ...
  /**
   * Checks if the actual view matches a view declaration.
   *
   * <p>
   *
  ...
  private boolean matchView(String actualView, String declaredView) {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(declaredView)
        || CmsStringUtil.isEmptyOrWhitespaceOnly(actualView)) {
      return true;
    }
