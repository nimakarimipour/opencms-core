 ...
 *
 * <p>This thread executes all 60 seconds and checks if report threads are still active. A report
 * thread usually waits for a user to get the contents written to the report. However, if the user
 * does not request the reports content (e.g. because the browser was closed), then the report
 * thread becomes abandoned. This Grim Reaper will collect all such abandoned report threads and
 * remove them after further 60 seconds.
 *
 * <p>Moreover, the Grim Reaper checks for all invalid user sessions that have times out for 5 or
 * more minutes, and removes them as well.
 *
 * <p>
 *
 * @since 6.0.0
...
public final class CmsThreadStore extends Thread {
   ...
   * <p>
   *
   * @param securityManager needed for scheduling "undercover-jobs" that increase stability and
   *     fault tolerance
   */
  ...
  protected void checkPublishQueue() {
    ...
    try {
      ...
      if ((System.currentTimeMillis() - thread.getLastEntryTime())
          > (UPDATE_MINUTES_INTERVAL * ONE_MINUTE_IN_MILLIS)) {
        // remove it
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_THREADSTORE_CHECK_PUBLISH_THREAD_ERROR_0), t);
    }
  }
        ...
        if (thread.isDoomed()) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_THREADSTORE_DOOMED_2, thread.getName(), thread.getUUID()));
          }
        }
      }
      i = doomed.iterator();
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            org.opencms.lock.Messages.get()
                .getBundle()
                .key(org.opencms.lock.Messages.ERR_WRITE_LOCKS_0),
            t);
      }
    }
  }
  ...
  /**
   * Method to dump all currently known Threads.
   *
   * <p>
   */
