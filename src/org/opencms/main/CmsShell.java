 ...
 * can be used for scripting access to the OpenCms repository without the Workplace.
 *
 * <p>The CmsShell has direct access to all methods in the "command objects". Currently the
 * following classes are used as command objects: <code>{@link org.opencms.main.CmsShellCommands}
 * </code>, <code>{@link org.opencms.file.CmsRequestContext}</code> and <code>
 * {@link org.opencms.file.CmsObject}</code>.
 *
 * <p>It is also possible to add a custom command object when calling the script API, like in {@link
 * CmsShell#CmsShell(String, String, String, String, I_CmsShellCommands, PrintStream, PrintStream,
 * boolean)}.
 *
 * <p>Only public methods in the command objects that use supported data types as parameters can be
 * called from the shell. Supported data types are: <code>
 * String, {@link org.opencms.util.CmsUUID}, boolean, int, long, double, float</code>.
 *
 * <p>If a method name is ambiguous, i.e. the method name with the same number of parameter exist in
 * more then one of the command objects, the method is only executed on the first matching method
 * object.
 *
 * <p>
 *
...
public class CmsShell {
  ...
  private class CmsCommandObject {
    ...
    /**
     * Tries to execute a method for the provided parameters on this command object.
     *
     * <p>If methods with the same name and number of parameters exist in this command object, the
     * given parameters are tried to be converted from String to matching types.
     *
     * <p>
     *
    ...
    protected boolean executeMethod(String command, List<String> parameters) {
      ...
      try {
        ...
        m_out.println(
            Messages.get()
                .getBundle(getLocale())
                .key(Messages.GUI_SHELL_EXEC_METHOD_1, new Object[] {foundMethod.getName()}));
    ...
    /**
     * Returns a signature overview of all methods containing the given search String.
     *
     * <p>If no method name matches the given search String, the empty String is returned.
     *
     * <p>
     *
    ...
    /**
     * Returns the object to execute the methods on.
     *
     * <p>
     *
    ...
    /**
     * Initializes the map of accessible methods.
     *
     * <p>
     */
      ...
      for (int i = 0; i < methods.length; i++) {
        ...
        if ((methods[i].getDeclaringClass() == m_object.getClass())
          ...
          if (onlyPrimitive) {
            ...
            String lookup =
                buildMethodLookup(methods[i].getName(), methods[i].getParameterTypes().length);
            List<Method> l;
   ...
   *
   * <p>We need multiple ones because shell commands may cause another nested shell to be launched
   * (e.g. for module import scripts).
   */
  public static final ThreadLocal<ArrayList<CmsShell>> SHELL_STACK =
  ...
  /**
   * Flag to indicate whether an error was added to a shell report during the last command
   * execution.
   */
   ...
   * @param servletMapping the mapping of the servlet (or <code>null</code> to use the default
   *     <code>"/opencms/*"</code>)
   * @param defaultWebAppName the name of the default web application (or <code>null</code> to use
   *     the default <code>"ROOT"</code>)
   * @param prompt the prompt format to set
   * @param additionalShellCommands optional object for additional shell commands, or null
   */
  public CmsShell(
   ...
   * @param out stream to write the regular output messages to
   * @param err stream to write the error messages output to
   * @param interactive if <code>true</code> this is an interactive session with a user sitting on a
   *     console
   */
  public CmsShell(
      @RUntainted String webInfPath,
    ...
    try {
      ...
      out.println(
          Messages.get().getBundle(locale).key(Messages.GUI_SHELL_WEB_INF_PATH_1, webInfPath));
      // set the path to the WEB-INF folder (the 2nd and 3rd parameters are just reasonable dummies)
      CmsServletContainerSettings settings =
  ...
  public static void main(@RUntainted String[] args) {
    ...
    if (wrongUsage) {
      System.out.println(
          Messages.get().getBundle().key(Messages.GUI_SHELL_USAGE_1, CmsShell.class.getName()));
    } else {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(additional)) {
        try {
          ...
          System.out.println(
              Messages.get()
                  .getBundle()
                  .key(Messages.GUI_SHELL_ERR_ADDITIONAL_COMMANDS_1, additional));
          e.printStackTrace();
          return;
        }
      }
      ...
      if (script != null) {
        try {
          ...
          System.out.println(
              Messages.get().getBundle().key(Messages.GUI_SHELL_ERR_SCRIPTFILE_1, script));
        }
      }
      if (stream == null) {
  ...
  /**
   * If running in the context of a CmsShell, this method notifies the running shell instance that
   * an error has occured in a report.
   *
   * <p>
   */
  ...
  public void execute(Reader reader) {
    ...
    try {
      ...
      while (!m_exitCalled) {
        ...
        // In linux, the up and down arrows generate escape sequences that cannot be properly
        // handled.
    ...
    try {
      prompt =
          CmsStringUtil.substitute(
              prompt, "${user}", m_cms.getRequestContext().getCurrentUser().getName());
      prompt =
          CmsStringUtil.substitute(prompt, "${siteroot}", m_cms.getRequestContext().getSiteRoot());
      prompt =
          CmsStringUtil.substitute(
              prompt, "${project}", m_cms.getRequestContext().getCurrentProject().getName());
  ...
  /**
   * Set <code>true</code> if this is an interactive session with a user sitting on a console.
   *
   * <p>This controls the output of the prompt and some other info that is valuable on the console,
   * but not required in an automatic session.
   *
   * <p>
   *
   * @param interactive if <code>true</code> this is an interactive session with a user sitting on a
  ...
  protected void help(@RUntainted String searchString) {
    ...
    while (i.hasNext()) {
      ...
      if (!CmsStringUtil.isEmpty(commandList)) {
        ...
        m_out.println(
            m_messages.key(
                Messages.GUI_SHELL_AVAILABLE_METHODS_1, cmdObj.getObject().getClass().getName()));
   ...
   * information) important information additional information about the current user (an instance
   * of {@link CmsUserSettings}).
   *
   * <p>This step is performed within the <code>CmsShell</code> constructor directly after switching
   * to run-level 2 and obtaining the <code>CmsObject</code> for the guest user as well as when
   * invoking the CmsShell command <code>login</code>.
   *
   * <p>
   *
  ...
  /**
   * Executes all commands read from the given reader.
   *
   * <p>
   *
   ...
   *
   * <p>To set the prompt, the following variables are available:
   *
   * <p><code>$u</code> the current user name<br>
   * <code>$s</code> the current site root<br>
   * <code>$p</code> the current project name
   *
   * <p>
   *
