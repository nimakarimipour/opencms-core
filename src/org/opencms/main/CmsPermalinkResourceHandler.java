 ...
 *
 * <p>The permalink must have following format:<br>
 * <code>/${CONTEXT}/${SERVLET}/permalink/${UUID}.${EXT}</code>
 *
 * <p>for example:<br>
 * <code>/opencms/opencms/permalink/a7b5d298-b3ab-11d8-b3e3-514d35713fed.html</code>
 *
 * <p>
 *
 * @since 6.3
      ...
      if (matcher.find()) {
        ...
        try {
          ...
          CmsResourceFilter filter =
              online ? CmsResourceFilter.DEFAULT : CmsResourceFilter.IGNORE_EXPIRATION;
        ...
        // In the 'getI18NInfo' case, we return the container page so the locale in the
        // CmsRequestContext is set correctly for the 'showResource' case
        if (matcher.find()) {
          try {
            ...
            if (res != null) {
              ...
              // we're being called from getI18NInfo; assume that the locale for the container page
              // is the locale we want
              return pageResource;
            }
