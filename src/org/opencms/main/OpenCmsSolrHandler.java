 ...
 *
 * <p>Reachable under: "/opencms/opencms/handleSolrSelect".
 *
 * <p>Usage example:
 *
 * <p><code>
...
public class OpenCmsSolrHandler extends HttpServlet implements I_CmsRequestHandler {
  ...
  /**
   * An enum storing the handler names implemented by this class.
   *
   * <p>
   */
  private static enum HANDLER_NAMES {
    ...
    /**
     * A constant for the '/select' request handler of the embedded Solr server. This handler is
    ...
    /**
     * A constant for the '/spell' request handler of the embedded Solr server. This handler is
     * reachable under "/opencms/opencms/handleSolrSpell".
     *
     * <p>
     */
    SolrSpell
  ...
  public static final Collection<String> DEFAULT_ALLOWED_WRITE_TO_VALUES =
      Arrays.asList(new String[] {"json", "xml"});
  ...
  /**
   * @see
   *     org.opencms.main.I_CmsRequestHandler#initParameters(org.opencms.configuration.CmsParameterConfiguration)
   */
  @Override
    ...
    if ((null != wtValues) && (wtValues.length > 0)) {
      ...
      if (wtValues.length > 1) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "Called Solr handler with multiple 'wt' params. Keeping only the value '"
                  + origWtValue
                  + "'.");
        }
      }
        ...
        String indexName =
            context.m_params.get(PARAM_CORE) != null
                ...
                : (context.m_params.get(PARAM_INDEX) != null
                    ? context.m_params.get(PARAM_INDEX)[0]
                    : null);
