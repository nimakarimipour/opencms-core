 ...
 *
 * <p>Such additional commands can access OpenCms functions not available on "regular" OpenCms
 * classes. Also, wrapping methods to access some important functions in the CmsObject that require
 * complex data type parameters are provided.
 *
 * <p>
 *
 * @since 6.0.0
...
class CmsShellCommands implements I_CmsShellCommands {
   ...
   *
   * <p>To initialize the command processor, you must call {@link #initShellCmsObject(CmsObject,
   * CmsShell)}.
   *
  ...
  public void cd(String target) throws Exception {
    ...
    m_shell
        ...
        .println(
            '\n'
                + getMessages()
                    .key(Messages.GUI_SHELL_CURRENT_FOLDER_1, new Object[] {resolvedTarget}));
   ...
   * @param startView the start view - Direct edit (/system/workplace/views/explorer/directEdit.jsp)
   *     - Explorer (/system/workplace/views/explorer/explorer_fs.jsp) - Administration
   *     (/system/workplace/views/admin/admin-fs.jsp)
  ...
  public void clearCaches() throws Exception {
    ...
    OpenCms.fireCmsEvent(
        new CmsEvent(I_CmsEventListener.EVENT_CLEAR_CACHES, new HashMap<String, Object>()));
  }

  /**
   * Clears the login message.
   *
   * <p>
   *
   ...
   *
   * <p>This created project has the following properties:
   *
   * <ul>
   ...
   *   <li>The project will remain after publishing
   * </ul>
   *
  ...
  public CmsResource createFolder(String targetFolder, String folderName) throws Exception {
    ...
    if (m_cms.existsResource(targetFolder + folderName)) {
      m_shell
          ...
          .println(
              getMessages()
                  .key(Messages.GUI_SHELL_FOLDER_ALREADY_EXISTS_1, targetFolder + folderName));
      return null;
    }
   ...
   * @throws Exception if something goes wrong
   */
  public CmsOrganizationalUnit createOU(
      String ouFqn, String description, boolean hideLogin, String resource) throws Exception {
   ...
   */
  public void deleteHistoricalVersions(int versionsToKeep, int versionsDeleted, long timeDeleted)
      throws Exception {
  ...
  public void deleteModule(String moduleName) throws Exception {
    ...
    OpenCms.getModuleManager()
        .deleteModule(
            m_cms, moduleName, false, new CmsShellReport(m_cms.getRequestContext().getLocale()));
  }
  ...
  public void exportModule(String moduleName) throws Exception {
    ...
    if (module == null) {
      throw new CmsDbEntryNotFoundException(
          Messages.get().container(Messages.ERR_UNKNOWN_MODULE_1, moduleName));
    }
    ...
    OpenCms.getImportExportManager()
        .exportData(
            m_cms, moduleExportHandler, new CmsShellReport(m_cms.getRequestContext().getLocale()));
  }

  /**
   * Exports a list of resources from the current site root to a ZIP file.
   *
   * <p>The resource names in the list must be separated with a ";".
   *
   * <p>
   *
   * @param exportFile the name (absolute path) of the ZIP file to export to
   ...
   * @throws Exception if something goes wrong
   */
  public void exportResources(String exportFile, String pathList, boolean isReducedExportMode)
      throws Exception {
  ...
  public void exportResources(
      String exportFile, String pathList, boolean isReducedExportMode, boolean skipParentFolders)
      throws Exception {
  ...
  public void exportResourcesAndUserdata(
      String exportFile, String pathList, boolean isReducedExportMode) throws Exception {
  ...
  /**
   * Deletes a module. Will ignore any modules depending on the module to delete.
   *
   * <p>
   *
   * @param moduleName the name of the module
  ...
  public void forceDeleteModule(String moduleName) throws Exception {
    ...
    OpenCms.getModuleManager()
        .deleteModule(
            m_cms, moduleName, true, new CmsShellReport(m_cms.getRequestContext().getLocale()));
  }
  ...
  public void getAcl(String resourceName) throws Exception {
    ...
    while (principals.hasNext()) {
      ...
      m_shell
          .getOut()
          .println(p.getName() + ": " + acList.getPermissions(p.getId()).getPermissionString());
    }
  }

  /**
   * Returns the Locales available on the system ready to use on Method {@link #setLocale(String)}
   * from the <code>{@link CmsShell}</code>.
   *
   * <p>Note that the full name containing language, country and optional variant seperated by
   * underscores is returned always but the latter two parts may be left out.
   *
   * <p>
   */
  ...
  public void importModule(String importFile) throws Exception {
    ...
    OpenCms.getImportExportManager()
        .importData(m_cms, new CmsShellReport(m_cms.getRequestContext().getLocale()), params);
  }
  ...
  /**
   * Imports a folder or a ZIP file to the root folder of the current site, creating a temporary
   * project for this.
   *
   * <p>
   *
  ...
  public void listModules() throws Exception {
    ...
    m_shell
        ...
        .println(
            "\n"
                + getMessages()
                    .key(Messages.GUI_SHELL_LIST_MODULES_1, new Integer(modules.size())));
  ...
  public void ls() throws Exception {
    ...
    m_shell
        ...
        .println(
            "\n"
                + getMessages()
                    .key(Messages.GUI_SHELL_LS_2, folder, new Integer(resources.size())));
  ...
  public void lsacc(String resourceName) throws Exception {
    ...
    for (int i = 0; i < acList.size(); i++) {
      ...
      String pName =
          (acePrincipal != null) ? acePrincipal.getName() : ace.getPrincipal().toString();
      m_shell
          .getOut()
          .println(pName + ": " + ace.getPermissions().getPermissionString() + " " + ace);
    }
  }
  ...
  /**
   * Sets the error code which should be returned as exit code in case an exception is thrown by a
   * shell command.
   *
   * <p>A value of -1 means the shell should not exit on exceptions thrown by shell commands
   *
  ...
  public void purgeJspRepository() throws Exception {
    ...
    OpenCms.fireCmsEvent(
        new CmsEvent(
            I_CmsEventListener.EVENT_FLEX_PURGE_JSP_REPOSITORY, new HashMap<String, Object>()));
  }
  ...
  public String readFileContent(String filename) throws CmsException {
    ...
    filename =
        CmsLinkManager.getAbsoluteUri(
            filename, CmsResource.getFolderPath(m_cms.getRequestContext().getUri()));
  ...
  public void rebuildIndex(String index) throws Exception {
    ...
    OpenCms.getSearchManager()
        .rebuildIndex(index, new CmsShellReport(m_cms.getRequestContext().getLocale()));
  }

  /**
   * Reindexes either a single file or all files under a folder in the given project.
   *
   * @param path the path of either a file to reindex, or a folder under which resources should be
   *     indexed
  ...
  /**
   * Replaces elements on container pages.
   *
   * <p>
   *
  ...
  public void renameNestedContainers(
    ...
    try {
      ...
      CmsSearchReplaceThread thread =
          new CmsSearchReplaceThread(
              null, cms, settings, new CmsShellLogReport(m_cms.getRequestContext().getLocale()));
      thread.run();
    } catch (CmsException e) {
      //
    }
  ...
  public void replaceElementsInPages(
    ...
    try {
      ...
      settings.setReplacepattern(
          CmsSearchReplaceSettings.replaceElementInPagePattern(targetResource));
      String siteRoot = OpenCms.getSiteManager().getSiteRoot(basePath);
      if (siteRoot != null) {
        settings.setSiteRoot(siteRoot);
        settings.setPaths(Collections.singletonList(basePath.substring(siteRoot.length())));
  ...
  public void replaceModule(String importFile) throws Exception {
    ...
    OpenCms.getModuleManager()
        .replaceModule(
            m_cms, importFile, new CmsShellReport(m_cms.getRequestContext().getLocale()));
  }
   ...
   *
   * <p>This is only useful when the CmsShell is used via RMI.
   *
   * <p>
   *
  ...
  /**
   * Sets the rebuild mode for the requested index. Allowing to disable indexing during module
   * import.
   *
   * <p>This setting will not be written to the XML configuration file and will only take effect
   * within the current shell instance.
   *
   * <p>
   *
   ...
   *
   * <p>This method will always set a valid Locale for the current user! If the provided locale name
   * is not valid (i.e. leads to an Exception when trying to create the Locale, then the configured
  ...
  /**
   * Sets a site parameter and writes back the updated system configuration.
   *
   * <p>
   *
  ...
  /**
   * Sleeps for a duration given in milliseconds.
   *
   * <p>
   *
  ...
  /**
   * Sets the checkpoint time of all installed modules to the current time.
   *
   * <p>
   */
  ...
  /**
   * Sets the checkpoint time of a given module to the current time.
   *
   * <p>If the module does not exist, this just prints an error message but does not throw an
   * exception.
   *
  ...
  /**
   * Touches a resource and all its children.
   *
   * <p>This method also rewrites the content for all files in the subtree.
   *
  ...
  public void touchResource(String resourcePath) throws Exception {
    ...
    try {
      ...
      // One important reason for touching resources via the shell is to write mapped values
      // containing
      // localization macros to properties, so we flush the workplace messages immediately before
      // the touch operation
      // in case an older version of the workplace messages (not containing the keys we need) has
      // already been cached
  ...
  public void transformXmlContents(String path, String typeName, String transform)
    ...
    String additionalLogPath =
        OpenCms.getSystemInfo().getAbsoluteRfsPathRelativeToWebInf("logs/" + additionalLogName);
    ...
    try (FileOutputStream additionalLog = new FileOutputStream(additionalLogPath)) {
      ...
      report.addReport(
          new CmsPrintStreamReport(new PrintStream(additionalLog), Locale.ENGLISH, false));
      try (FileInputStream stream = new FileInputStream(transform)) {
        CmsXmlFileTransformer fileTransformer =
            new CmsXmlFileTransformer(m_cms, path, typeName, transform, stream, report);
        fileTransformer.run();
      }
    }
  }
  ...
  /**
   * Updates a module with another revision in case their module versions differ.
   *
   * <p>
   *
   * @param importFile the name of the import file
  ...
  public CmsResource uploadFile(String localfile, String folder, String filename, String type)
    ...
    return m_cms.createResource(
        folder + filename, t, CmsFileUtil.readFile(new File(localfile)), null);
  }
