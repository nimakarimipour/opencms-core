 ...
 * binary files, using the API authorization mechanism instead of the normal authorization handler.
 *
 * <p>Resources are accessed by appending their VFS root path to the /staticresource handler path.
 * When resources are requested this way, they are still loaded with the normal OpenCms loader
 * mechanism. This works for the intended use case (binary files, images) but may not work for other
 * types.
 *
 * <p>The resources accessible through this handler can be restricted by setting regex configuration
 * parameters for path and type which the requested resources have to match.
 *
 * <p>This can be used in combination with the CmsJsonResourceHandler class. When configured
 * correctly (using the linkrewrite.id parameter on this handler, and a matching linkrewrite.refid
 * on the CmsJsonResourceHandler), links to resources this handler is responsible for will be
 * rewritten to point to the URL for the resource using this handler.
 */
public class CmsProtectedStaticFileHandler
  ...
  public static String mergeLinkPrefix(String prefix, String path, String query) {
    ...
    try {
      ...
      // we can't give an URIBuilder an already escaped query string, so we parse a dummy URL with
      // the query string
      ...
      String result =
          new URIBuilder(baseUri)
              .setPath(CmsStringUtil.joinPaths(baseUri.getPath(), PREFIX, path))
  ...
  /**
   * Helper method for authorizing requests based on a comma-separated list of API authorization
   * handler names.
   *
   * <p>This will evaluate each authorization handler from authChain and return the first non-null
   * CmsObject returned. A special case is authChain contains the word 'default', this is not u
   *
  ...
  private static CmsObject authorize(
      CmsObject adminCms, CmsObject defaultCms, HttpServletRequest request, String authChain) {
  ...
  /**
   * @see org.opencms.relations.I_CmsCustomLinkRenderer#getLink(org.opencms.file.CmsObject,
  ...
  public String getLink(CmsObject cms, CmsLink link) {
    ...
    try {
      ...
      if (checkResourceAccessible(link.getResource())) {
        return mergeLinkPrefix(
            m_linkRewritePrefix, link.getResource().getRootPath(), link.getQuery());
      }
      return null;
    } catch (CmsException e) {
  ...
  public CmsResource initResource(
      CmsResource origRes, CmsObject cms, HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      if (OpenCms.getDefaultUsers()
          .isUserGuest(cms.getRequestContext().getCurrentUser().getName())) {
  ...
  /**
   * Checks if the resource is not hidden according to the filters configured in the resource
   * handler parameters.
   *
  ...
  private boolean checkResourceAccessible(CmsResource res) {
    ...
    return (res != null)
        && m_pathFilter.matcher(res.getRootPath()).matches()
        && checkType(res.getTypeId());
  }
