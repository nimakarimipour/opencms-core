...
/**
 * Implements a servlet filter for URL rewriting. It adds the servlet name (typically "/opencms") if
 * not already present, but necessary.
 */
public class OpenCmsUrlServletFilter implements Filter {
  ...
  /**
   * Name of the init-param of the filter configuration that is used to provide a pipe separated
   * list of additional prefixes for which the URIs should not be adjusted.
   */
  private static final String INIT_PARAM_ADDITIONAL_EXCLUDEPREFIXES = "additionalExcludePrefixes";

  /**
   * A pipe separated list of URI prefixes, for which URIs should not be adjusted, additionally to
  ...
  /**
   * A regex that matches if the requested URI starts with one of the default exclude prefixes or a
   * prefix listed in the value of the {@link #INIT_PARAM_ADDITIONAL_EXCLUDEPREFIXES} init-param.
   */
  ...
  /**
   * Creates a regex that matches all URIs starting with one of the <code>defaultExcludePrefixes
   * </code> or one of the <code>additionalExcludePrefixes</code>.
   *
   ...
   * @return a regex that matches all URIs starting with one of the <code>defaultExcludePrefixes
   *     </code> or one of the <code>additionalExcludePrefixes</code>.
   */
  static String createRegex(
      String contextPath, String[] defaultExcludePrefixes, String additionalExcludePrefixes) {
  ...
  /**
   * Adjusts the requested URIs by prepending the name of the {@link
   * org.opencms.main.OpenCmsServlet}, if the request should be handled by that servlet.
   *
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    try (CloseableThreadContext.Instance threadContext =
        CloseableThreadContext.putAll(loggingInfo)) {
  ...
  public void init(FilterConfig filterConfig) {
    ...
    m_additionalExcludePrefixes =
        filterConfig.getInitParameter(INIT_PARAM_ADDITIONAL_EXCLUDEPREFIXES);
  }

  /**
   * Checks if OpenCms has reached run level 4 and if so, it initializes the member variables. In
   * particular {@link #m_regex}, {@link #m_contextPath} and {@link
   * OpenCmsUrlServletFilter#m_servletPath} are initialized and sets {@link #m_isInitialized} to
   * true.
   *
  ...
  boolean tryToInitialize() {
    ...
    if (OpenCms.getRunLevel() == 4) {
      ...
      /**
       * The URI prefixes, for which the requested URI should not be rewritten, i.e., the ones that
       * should not be handled by the {@link org.opencms.main.OpenCmsServlet}.
       */
      String[] defaultExcludePrefixes =
          new String[] {
