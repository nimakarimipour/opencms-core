 ...
 *
 * <p>From here, all operations that are results of HTTP requests are invoked. Any incoming request
 * is handled in multiple steps:
 *
 * <ol>
 *   <li>The requesting <code>{@link org.opencms.file.CmsUser}</code> is authenticated and a <code>
 *       {@link org.opencms.file.CmsObject}</code> with this users context information is created.
 *       This <code>{@link org.opencms.file.CmsObject}</code> is used to access all functions of
 *       OpenCms, limited by the authenticated users permissions. If the user is not identified, it
 *       is set to the default user, usually named "Guest".
 *   <li>The requested <code>{@link org.opencms.file.CmsResource}</code> is loaded into OpenCms and
 *       depending on its type (and the users persmissions to display or modify it), it is send to
 *       one of the OpenCms <code>{@link org.opencms.loader.I_CmsResourceLoader}</code>
 *       implementations do be processed.
 *   <li>The <code>{@link org.opencms.loader.I_CmsResourceLoader}</code> will then decide what to do
 *       with the contents of the requested <code>{@link org.opencms.file.CmsResource}</code>. In
 *       case of a JSP resource the JSP handling mechanism is invoked with the <code>
 *       {@link org.opencms.loader.CmsJspLoader}</code>, in case of an image (or another static
 *       resource) this will be returned by the <code>{@link org.opencms.loader.CmsDumpLoader}
 *       </code> etc.
 * </ol>
 *
 * @since 6.0.0
...
public class OpenCmsServlet extends HttpServlet implements I_CmsRequestHandler {
  ...
  public static class RequestCache {
    ...
    /**
     * Gets the stored attribute for the given key, or lazily initializes it with the given provider
  ...
  public static final CmsThreadLocalStack<HttpServletRequest> currentRequestStack =
      new CmsThreadLocalStack<HttpServletRequest>();

  /** Map containing beans with information about currently running requests. */
  public static final ConcurrentHashMap<CmsUUID, RequestInfo> activeRequests =
      new ConcurrentHashMap<>();

  /** The current thread context for the request. */
  private static final CmsThreadLocalStack<RequestCache> requestCacheStack =
      new CmsThreadLocalStack<RequestCache>();
  ...
  /**
   * Name of the <code>DefaultWebApplication</code> parameter in the <code>web.xml</code> OpenCms
   * servlet configuration.
   */
  public static final String SERVLET_PARAM_DEFAULT_WEB_APPLICATION = "DefaultWebApplication";

  /**
   * Name of the <code>OpenCmsHome</code> parameter in the <code>web.xml</code> OpenCms servlet
   * configuration.
   */
  public static final String SERVLET_PARAM_OPEN_CMS_HOME = "OpenCmsHome";

  /**
   * Name of the <code>OpenCmsServlet</code> parameter in the <code>web.xml</code> OpenCms servlet
   * configuration.
   */
  public static final String SERVLET_PARAM_OPEN_CMS_SERVLET = "OpenCmsServlet";

  /**
   * Name of the <code>WebApplicationContext</code> parameter in the <code>web.xml</code> OpenCms
   * servlet configuration.
   */
  ...
  /**
   * Helper method used to conveniently access the request cache, does nothing if the request cache
   * isn't set.
   *
  ...
  public void doGet(@RUntainted HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      res.setHeader(
          CmsRequestUtil.HEADER_SERVER, OpenCmsCore.getInstance().getSystemInfo().getVersion());
      ...
      if (runlevel != OpenCms.RUNLEVEL_4_SERVLET_ACCESS) {
        ...
        if (runlevel == OpenCms.RUNLEVEL_3_SHELL_ACCESS) {
          ...
          // sending status code 403, indicating the server understood the request but refused to
          // fulfill it
      ...
      if (path.startsWith(HANDLE_BUILTIN_SERVICE)) {
        // built-in services are for small AJAX-related functionality in the core that doesn't need
        // to be configurable
        String remainder =
            path.substring(
                HANDLE_BUILTIN_SERVICE.length() - 1); // we want a leading slash in remainder
  ...
  /**
   * OpenCms servlet POST request handling method, will just call {@link #doGet(HttpServletRequest,
   * HttpServletResponse)}.
   *
   * <p>
   *
    ...
    try {
      ...
      if (Messages.ERR_CRITICAL_INIT_WIZARD_0.equals(e.getMessageContainer().getKey())) {
        ...
        if (CmsServletContainerSettings.isServletThrowsException()) {
          ...
          // this is needed since some servlet containers does not like the servlet to throw
          // exceptions,
  ...
  /**
   * Displays an error code handler loaded from the OpenCms VFS, or if such a page does not exist,
  ...
  protected void openErrorHandler(HttpServletRequest req, HttpServletResponse res, int errorCode)
    ...
    try {
      ...
      CmsMessageContainer container =
          Messages.get()
              .container(
                  Messages.LOG_INIT_CMSOBJECT_IN_HANDLER_2, new Integer(errorCode), handlerUri);
      if (LOG.isWarnEnabled()) {
    ...
    try {
      ...
      CmsMessageContainer container =
          Messages.get()
              .container(
                  Messages.ERR_SHOW_ERR_HANDLER_RESOURCE_2, new Integer(errorCode), handlerUri);
