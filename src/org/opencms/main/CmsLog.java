 ...
 *
 * <p>The OpenCms logging mechanism is based on Apache Commons Logging. However, log4j is shipped
 * with OpenCms and assumed to be used as default logging mechanism. Since apparently Commons
 * Logging may cause issues in more complex classloader scenarios, we may switch the logging
 * interface to log4j <code>UGLI</code> once the final release is available.
 *
 * <p>The log4j configuration file shipped with OpenCms is located in <code>
 * WEB-INF/classes/log4j2.xml</code>. By default, OpenCms will configure log4j using the first
 * {@code log4j2.xml} available found by {@link Loader#getResource(String, ClassLoader)}.
 *
 * <p>The following system properties are available for fine-tunning the logging configuration:
 *
 * <dl>
 *   <dt>{@code opencms.set.logfile} (default: {@code true})
 *   <dd>Set to {@code false} to bypass the OpenCms standard logging configuration mechanism,
 *       delegating this to the logging framework of chose. Note that in this case,
 *       <ul>
 *         <li>the Workplace logging visualization tools will not be available, and
 *         <li>you may need to enable automatic configuration of log4j2 (see <a
 *             href="http://logging.apache.org/log4j/2.x/manual/webapp.html">Using Log4j 2 in Web
 *             Applications</a>. This configuration takes place before OpenCms is correctly
 *             initialized, and has been disabled in the standard OpenCms distribution because at
 *             that point, it has not yet set default values for the relevant system properties
 *       </ul>
 *   <dt>{@code opencms.logfolder} (default: "{@code WEB-INF/logs/}")
 *   <dd>Path to the folder where OpenCms will look for logging files to show in the Workplace GUI.
 *   <dt>{@code opencms.logfile} (default: "{@code WEB-INF/logs/opencms.log}")
 *   <dd>Absolute path to the OpenCms log file.
 * </dl>
 *
 * If the logging configuration is not bypassed by setting {@code opencms.set.logfile=false},
 * OpenCms will first assing them default values if needed and then will make these system
 * properties available for the configuration of log4j. In the {@code log4j2.xml} configuration
 * file, system properties can be referenced using the prefix "{@code sys:}" as in this example:
 *
 * <pre>
...
public final class CmsLog {
  ...
  /**
   * Name of the system property that configures the path where OpenCms looks for logs to show in
   * the Workplace App.
   */
  public static final String PROPERTY_LOGFOLDER = "opencms.logfolder";

  /**
   * Name of the system property that configures the absolute path to the file where OpenCms will
   * write its log.
   */
  ...
  static {
    ...
    try {
      ...
      if (log4j2Url != null) {
        ...
        File log4jProps =
            new File(URLDecoder.decode(log4j2Url.getPath(), Charset.defaultCharset().name()));
        String path = log4jProps.getAbsolutePath();

        // check if OpenCms should set the log file environment variable
        boolean setLogFile =
            Boolean.parseBoolean(System.getProperty("opencms.set.logfile", "true"));
        if (setLogFile) {
          ...
          // in a default OpenCms configuration, the following path would point to the OpenCms
          // "WEB-INF" folder
          ...
          // Set "opencms.log*" variables if not set. These should be used in the log42.xml config
          // file
        ...
        System.err.println(
            "'log4j2.xml' not found. (Default location: WEB-INF/classes/log4j2.xml)");
      }
  ...
  /**
   * Helper for safely evaluating lambda functions to produce log output and catch exceptions they
   * might throw.
   *
   ...
   *
   * <p>This code copy from "org.apache.log4j.DefaultThrowableRenderer.render(Throwable throwable)"
   *
