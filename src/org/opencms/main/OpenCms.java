 ...
 * classes to access basic system features of OpenCms like logging etc.
 *
 * <p>This Object provides singleton access to the initialized OpenCms runtime system. Some methods
 * are for internal or advanced use only, but others are of also of interest for general OpenCms
 * development.
 *
 * <p>For example, to generate a new instance of <code>{@link org.opencms.file.CmsObject}</code>
 * class in your application, use <code>{@link org.opencms.main.OpenCms#initCmsObject(String)}
 * </code>. The argument String should be the name of the guest user, usually "Guest" and more
 * formally obtained by <code>{@link org.opencms.db.CmsDefaultUsers#getUserGuest()}</code>. This
 * will give you an initialized context with guest user permissions. Then use <code>
 * {@link CmsObject#loginUser(String, String)}</code> to log in the user you want. Obviously you
 * need the password for the new user.
 *
 * <p>Using <code>{@link #getSiteManager()}</code> you can obtain the initialized <code>
 * {@link org.opencms.site.CmsSiteManagerImpl}</code> which provides information about the sites
 * configured in the running OpenCms instance.
 *
 * <p>The <code>{@link org.opencms.db.CmsDefaultUsers}</code> instance returned by <code>
 * {@link #getDefaultUsers()}</code> provides information about the names of the OpenCms default
 * users.
 *
 * <p>Other objects of note that can be obtained by this class include the <code>
 * {@link org.opencms.module.CmsModuleManager}</code> or the <code>
 * {@link org.opencms.scheduler.CmsScheduleManager}</code>.
 *
 * <p>When using the instances returned by this object, keep in mind that applying changes to these
 * may alter the basic OpenCms system configuration, which in turn may affect the systems
...
public final class OpenCms {
  ...
  /**
   * Runlevel 2: Initializing the system, required since this may take some seconds because of
   ...
   * Notify all event listeners that a particular event has occurred.
   *
   * <p>The event will be given to all registered <code>{@link I_CmsEventListener}</code> objects.
   *
   * <p>
   *
  ...
  /**
   * Returns the configured list of default directory file names (instances of <code>{@link String}
   * </code>).
   *
   * <p>Caution: This list can not be modified.
   *
   * <p>
   *
  ...
  /**
   * Returns the initialized import/export manager, which contains information about how to handle
   * imported resources.
   *
   * <p>
   *
   ...
   *
   * <p>Returns null if LetsEncrypt integration is not configured at all.
   *
   ...
   *
   * <p>If the provided object is a String, this String will be used as channel name. Otherwise the
   * objects class name will be used as channel name.
   *
   * <p>
   *
   ...
   *
   * <p>The following runlevels are defined:
   *
   * <dl>
   *   <dt>Runlevel {@link OpenCms#RUNLEVEL_0_OFFLINE}:
   *   <dd>OpenCms is in the process of being shut down, the system is offline.
   *   <dt>Runlevel {@link OpenCms#RUNLEVEL_1_CORE_OBJECT}:
   *   <dd>OpenCms instance available, but configuration has not been processed. No database or VFS
   *       available.
   *   <dt>Runlevel {@link OpenCms#RUNLEVEL_2_INITIALIZING}:
   *   <dd>OpenCms is initializing, but the process is not finished. The database with the VFS is
   *       currently being connected but can't be accessed.
   *   <dt>Runlevel {@link OpenCms#RUNLEVEL_3_SHELL_ACCESS}:
   *   <dd>OpenCms database and VFS available, but http processing (i.e. servlet) not initialized.
   *       This is the runlevel the OpenCms shell operates in.
   *   <dt>Runlevel {@link OpenCms#RUNLEVEL_4_SERVLET_ACCESS}:
   *   <dd>OpenCms fully initialized, servlet and database available. This is the "default" when
   *       OpenCms is in normal operation.
   * </dl>
   *
  ...
  /**
   * Returns the list of system defined roles (instances of <code>{@link CmsRole}</code>).
   *
   * <p>Caution: This list can not be modified.
   *
   * <p>
   ...
   * @see OpenCms#initCmsObject(String)
   */
  public static CmsObject initCmsObject(CmsObject adminCms, CmsContextInfo contextInfo)
      throws CmsException {
   ...
   *
   * <p>In order to initialize another user (for example, the {@link
   * CmsDefaultUsers#getUserAdmin()}), you need to get the 'Guest' user context first, then login
   * the target user with his user name and password, using {@link CmsObject#loginUser(String,
   * String)}. There is no way to obtain a user context other then the 'Guest' or 'Export' user
  ...
  /**
   * Reads the requested resource from the OpenCms VFS, and in case a directory name is requested,
   * the default files of the directory will be looked up and the first match is returned.
   *
