 ...
 *
 * <p>There is only one instance of this event manager class used by the OpenCms runtime. This
 * instance can be obtained by calling {@link OpenCms#getEventManager()}.
 *
 * <p>Events can be used in OpenCms to notify custom event listeners that certain system events have
 * happened. Event listeners have to implement the interface {@link
...
public class CmsEventManager {
  ...
  /**
   * Notify all event listeners that a particular event has occurred without any additional event
   * data.
   *
   * <p>
   *
    ...
    if (!LOG.isDebugEnabled()) {
      ...
      if ((listeners != null) && (listeners.size() > 0)) {
        ...
        if (event.getData() != null) {
          ...
          while (i.hasNext()) {
            ...
            LOG.debug(
                Messages.get()
                    .getBundle()
                    .key(Messages.LOG_DEBUG_EVENT_VALUE_3, key, value, event.toString()));
          }
        } else {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_DEBUG_NO_EVENT_VALUE_1, event.toString()));
        }
        ...
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_DEBUG_EVENT_NO_LISTENER_1, event.toString()));
      }
      LOG.debug(
          Messages.get().getBundle().key(Messages.LOG_DEBUG_EVENT_COMPLETE_1, event.toString()));
    }
  }
