...
public class CmsStaticResourceHandler implements I_CmsRequestHandler {
  ...
  /**
   * Returns the regular expression to remove the static resource path prefix.
   *
   * <p>
   *
  ...
  /**
   * Returns the regular expression to identify static resource paths.
   *
   * <p>
   *
   ...
   */
  public void handle(@RUntainted HttpServletRequest request, HttpServletResponse response, String name)
      throws IOException {
  ...
  /**
   * Returns whether this servlet should attempt to serve a precompressed version of the given
   * static resource. If this method returns true, the suffix {@code .gz} is appended to the URL and
   * the corresponding resource is served if it exists. It is assumed that the compression method
   * used is gzip. If this method returns false or a compressed version is not found, the original
   * URL is used.
   *
   * <p>The base implementation of this method returns true if and only if the request indicates
   * that the client accepts gzip compressed responses and the filename extension of the requested
   * resource is .js, .css, or .html.
   *
   * <p>
   *
   * @param request the request for the resource
   * @param url the URL of the requested resource
   * @return true if the servlet should attempt to serve a precompressed version of the resource,
   *     false otherwise
   */
  ...
  /**
   * Calculates the cache lifetime for the given filename in seconds. By default filenames
   * containing ".nocache." return 0, filenames containing ".cache." return one year, all other
   * return the value defined in the web.xml using resourceCacheTime (defaults to 1 hour).
   *
   * <p>
   *
  ...
  protected void setResponseHeaders(
      HttpServletRequest request, HttpServletResponse response, String filename, URL resourceURL) {
  ...
  /**
   * Writes the contents of the given resourceUrl in the response. Can be overridden to add/modify
   * response headers and similar.
   *
   * <p>
   *
   * @param request the request for the resource
  ...
  protected void writeStaticResourceResponse(
      HttpServletRequest request, HttpServletResponse response, URL resourceUrl)
      throws IOException {
  ...
  /**
   * Checks if the browser has an up to date cached version of requested resource. Currently the
   * check is performed using the "If-Modified-Since" header. Could be expanded if needed.
   *
   * <p>
   *
   * @param request the HttpServletRequest from the browser
   * @param resourceLastModifiedTimestamp the timestamp when the resource was last modified. 0 if
   *     the last modification time is unknown
   * @return true if the If-Modified-Since header tells the cached version in the browser is up to
   *     date, false otherwise
   */
  private boolean browserHasNewestVersion(
      HttpServletRequest request, long resourceLastModifiedTimestamp) {
  ...
  /**
   * Streams the input stream to the response.
   *
   * <p>
   *
   * @param response the response
