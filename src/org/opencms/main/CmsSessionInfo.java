 ...
 *
 * <p>This object is used to provide information about all authenticated users in the system with
 * the {@link org.opencms.main.CmsSessionManager}.
 *
 * <p>This object is available for all authenticated users after login. If a user has not logged in,
 * he may have a session on the servlet engine, but he will have no session info object attached.
 * For example the "Guest" user may have multiple sessions, but no session info is created for him.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsSessionInfo implements Comparable<CmsSessionInfo>, Serializable {
   ...
   * @param maxInactiveInterval the maximum time, in seconds, this session info is allowed to be
   *     inactive
   */
  ...
  /**
   * Formats an integer to a two chars string.
   *
   * <p>
   *
   ...
   *
   * <p>The inactive time is the time since the last call to the {@link #update(CmsRequestContext)}
   * method. If the inactive time is greater then the maximum allowed time, this session info will
   * be removed from the session manager.
   *
   * <p>
   *
  ...
  /**
   * Returns the time, in milliseconds, this session has been active, that is the time of the last
   * update minus the creation time.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if this session info has expired, that is it has not been updated in
   * the time set by the maximum inactivity interval.
   *
   * <p>
   *
