 ...
 *
 * <ul>
 *   <li>version and build number
 *   <li>server name
 *   <li>mail settings
 *   <li>configuration paths
 *   <li>default character encoding
 *   <li>...and more.
 * </ul>
 *
 * @since 6.0.0
...
public class CmsSystemInfo {
  ...
  public static final @RUntainted String FILE_PERSISTENCE =
      "classes" + File.separatorChar + "META-INF" + File.separatorChar + "persistence.xml";
  ...
  public static final @RUntainted String FOLDER_CONFIG_DEFAULT = "config" + File.separatorChar;

  /** The name of the module folder in the package path. */
  public static final @RUntainted String FOLDER_MODULES = "modules" + File.separatorChar;
   ...
   * (in days).
   */
  private @RUntainted int m_notificationTime;
  ...
  /**
   * The version identifier of this OpenCms installation, contains "OpenCms/" and the version
   * number.
   */
  private String m_version;

  /** The version ID of this OpenCms installation, usually set by the build system. */
  private @RUntainted String m_versionId;
  ...
  public CmsSystemInfo() {
    ...
    // this may look odd, but initMembers in OpenCms core has to initialize this (e.g. for setup to
    // avoid NPE)
    m_servletContainerSettings = new CmsServletContainerSettings(null);
  }

  /**
   * Returns an absolute path (to a directory or a file in the "real" file system) from a path
   * relative to the web application folder of OpenCms.
   *
   * <p>If the provided path is already absolute, then it is returned unchanged. If the provided
   * path is a folder, the result will always end with a folder separator.
   *
   * <p>
   *
   * @param path the path (relative) to generate an absolute path from
   * @return an absolute path (to a directory or a file) from a path relative to the web application
   *     folder of OpenCms
   */
  ...
  /**
   * Returns an absolute path (to a directory or a file in the "real" file system) from a path
   * relative to the "WEB-INF" folder of the OpenCms web application.
   *
   * <p>If the provided path is already absolute, then it is returned unchanged.
   *
   * <p>
   *
   * @param path the path (relative) to generate an absolute path from
   * @return an absolute path (to a directory or a file) from a path relative to the "WEB-INF"
   *     folder
   */
  ...
  /**
   * Returns the map of additional build information that was contained in the version.properties
   * file.
   *
   * <p>The values are String arrays of length 2. First in this array is the actual value, and
   * second the "nice name" for the value that can be used to display the value somewhere. In case
   * no nice name was provided, the second value will repeat the key name.
   *
   * <p>
   *
   * @return the map of additional build information that was contained in the version.properties
   *     file
   * @since 9.5.0
   */
   ...
   *
   * <p>This will either be a file below /system/config/ or in case an override file exists below
   ...
   *
   * <p>Per default this is the "/WEB-INF/config/ folder. If configured with the
   * "-Dopencms.config=..." java startup parameter, OpenCms can access an external config folder
   * outside its webapplication.
   *
  ...
  /**
   * Returns the absolute path to the "opencms.properties" configuration file (in the "real" file
   * system).
   *
   * <p>
   *
  ...
  /**
   * Returns the web application context path, e.g. "" (empty String) if the web application is the
   * default web application (usually "ROOT"), or "/opencms" if the web application is called
   * "opencms".
   *
   * <p><i>From the Java Servlet Specification v2.4:</i><br>
   * <b>Context Path:</b> The path prefix associated with the ServletContext that this servlet is a
   * part of. If this context is the "default" context rooted at the base of the web server's URL
   * name space, this path will be an empty string. Otherwise, if the context is not rooted at the
   * root of the server's name space, the path starts with a "/" character but does not end with a
   * "/" character.
   *
   * <p>
   *
   ...
   *
   * <p>The default is set in the opencms-system.xml file. If this is not set in opencms-system.xml
  ...
  /**
   * Returns the number of versions in the VFS version history that should be kept after a resource
   * is deleted.
   *
   * <p>For resources in folders where the history.removedeleted property is set to "true", this
   * setting is ignored.
   *
   ...
   *
   * <p>If the method returns <code>null</code>, this means that the log file is not managed by
   * OpenCms.
   *
   * <p>
   *
  ...
  /**
   * Returns the absolute path to the folder of the main OpenCms log file (in the "real" file
   * system).
   *
   * <p>If the method returns <code>null</code>, this means that the log file is not managed by
   * OpenCms.
   ...
   * <p>
   *
   * @return the absolute path to the folder of the main OpenCms log file (in the "real" file
   *     system)
   */
  ...
  /**
   * Returns the duration after which responsible resource owners will be notified about out-dated
   * content (in days).
   *
   * <p>
   *
   * @return the duration after which responsible resource owners will be notified about out-dated
   *     content
   */
   ...
   *
   * <p>The OpenCms context will always start with a "/" and never have a trailing "/". The OpenCms
   * context is identical to <code>getContexPath() + getServletPath()</code>.
   *
   * <p>
   *
   ...
   *
   * <p>The server name is set in <code>opencms.properties</code>. It is not related to any DNS name
   * the server might also have. The server name is useful e.g. in a cluster to distinguish
   * different servers, or if you compare log files from multiple servers.
   *
   * <p>
   *
   ...
   *
   * <p><i>From the Java Servlet Specification v2.4:</i><br>
   * <b>Servlet Path:</b> The path section that directly corresponds to the mapping which activated
   * this request. This path starts with a?/? character except in the case where the request is
   * matched with the ?/*? pattern, in which case it is the empty string.
   *
   * <p>
   *
  ...
  public String getStaticResourceContext() {
    ...
    if (m_staticResourcePathFragment == null) {
      m_staticResourcePathFragment =
          CmsStaticResourceHandler.getStaticResourceContext(
              OpenCms.getStaticExportManager().getVfsPrefix(), getVersionNumber());
    }
   ...
   *
   * <p>This information is used for example to identify OpenCms in HTTP response headers.
   *
   * <p>
   *
   ...
   *
   * <p>The version ID is usually set dynamically by the build system. It can be used to identify
   * intermediate builds when the main version number has not changed.
   *
   * <p>
   *
  ...
  /**
   * Returns the version number of this OpenCms system, for example <code>9.5.0</code>.
   *
   * <p>
   *
   ...
   * "/").
   *
   * <p>The web application name is stored for informational purposes only. If you want to construct
   * an URI, use either {@link #getContextPath()} and {@link #getServletPath()}, or for links to the
   * OpenCms VFS use {@link #getOpenCmsContext()}.
   *
   * <p>
   *
   ...
   *
   * <p>The OpenCms workplace context will always start with a "/" and never have a trailing "/".
   * The OpenCms context is identical to <code>getContexPath() + WORKPLACE_PATH</code>.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param notificationProject the project in which time stamps for the content notification are
   *     read
   */
  ...
  /**
   * Sets the duration after which responsible resource owners will be notified about out-dated
   * content (in days).
   *
   * <p>
   *
   * @param notificationTime the duration after which responsible resource owners will be notified
   *     about out-dated content
   */
  public void setNotificationTime(@RUntainted int notificationTime) {
   ...
   * @param historyVersionsAfterDeletion the maximum number of versions that are kept for deleted
   *     resources
   */
  public void setVersionHistorySettings(
      @RUntainted boolean historyEnabled, @RUntainted int historyVersions, @RUntainted int historyVersionsAfterDeletion) {
  ...
  protected void setDefaultEncoding(@RUntainted String encoding) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.LOG_SET_DEFAULT_ENCODING_1, m_defaultEncoding));
    }
  }
   ...
   */
  protected void setHttpAuthenticationSettings(
  ...
  /**
   * Initializes the version for this OpenCms, will be called by {@link OpenCmsServlet} or {@link
   * CmsShell} upon system startup.
   *
   * <p>
   */
  private void initVersion() {
    ...
    try {
      props.load(
          this.getClass()
              .getClassLoader()
              .getResourceAsStream("org/opencms/main/version.properties"));
    } catch (Throwable t) {
    ...
    for (String key : props.stringPropertyNames()) {
      if (!"version.number".equals(key)
          && !"version.id".equals(key)
          && !key.startsWith("nicename")) {
