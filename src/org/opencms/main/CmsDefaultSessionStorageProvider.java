 ...
 *
 * <p>Implementation based on a {@link FastHashMap}.
 *
 * <p>
 *
 * @since 6.5.5
...
public class CmsDefaultSessionStorageProvider implements I_CmsSessionStorageProvider {
  ...
  public List<CmsSessionInfo> getAllOfUser(CmsUUID userId) {
    ...
    try {
      return getAllSelected(
          CmsCollectionsGenericWrapper.<CmsUUID, CmsSessionInfo>map(m_sessions), userId);
    } catch (ConcurrentModificationException e) {
      // try again with a clone this time
      return getAllSelected(
          CmsCollectionsGenericWrapper.<CmsUUID, CmsSessionInfo>map(m_sessions.clone()), userId);
    }
  }
  ...
  public void validate() {
    ...
    try {
      ...
      Iterator<Map.Entry<CmsUUID, CmsSessionInfo>> sessions =
          CmsCollectionsGenericWrapper.<CmsUUID, CmsSessionInfo>map(m_sessions)
              .entrySet()
              .iterator();
  ...
  /**
   * Returns all sessions or all sessions matching the user id from the provided Map.
   *
   * <p>
   *
   ...
   */
  private List<CmsSessionInfo> getAllSelected(
      Map<CmsUUID, CmsSessionInfo> allSessions, CmsUUID userId) {
