...
public class CmsUIServlet extends VaadinServlet
    implements SystemMessagesProvider, SessionInitListener {
      ...
      new BootstrapListener() {
        ...
        public void modifyBootstrapPage(BootstrapPageResponse response) {
          ...
          try {
            ...
            doc.head()
                .append(org.opencms.ade.publish.ClientMessages.get().export(currentLocale, true));
            doc.head()
                .append(org.opencms.ade.upload.ClientMessages.get().export(currentLocale, true));
            doc.head()
                .append(org.opencms.ade.galleries.ClientMessages.get().export(currentLocale, true));
            for (String cssURI : OpenCms.getWorkplaceAppManager().getWorkplaceCssUris()) {
              doc.head()
                  .append(
                      "<link rel=\"stylesheet\" href=\""
                          + CmsWorkplace.getResourceUri(cssURI)
                          + "\">");
            }
          } catch (Exception e) {
            LOG.error(e.getLocalizedMessage(), e);
  ...
  static final UIProvider LOGIN_UI_PROVIDER =
      new UIProvider() {
  ...
  static final RequestHandler REQUEST_AUTHORIZATION_HANDLER =
      new RequestHandler() {
          ...
          if (shouldShowLogin() && !isLoginUIRequest(request)) {
            ...
            OpenCms.getAuthorizationHandler()
                .requestAuthorization(
                    (HttpServletRequest) request, (HttpServletResponse) response, link);
            return true;
          }
          return false;
        }
  ...
  private Map<Locale, SystemMessages> m_systemMessages =
      new ConcurrentHashMap<Locale, SystemMessages>();
  ...
  /**
   * Checks whether the given request was referred from the login page.
   *
   * <p>
   *
  ...
  static boolean shouldShowLogin() {
    ...
    return ((CmsUIServlet) getCurrent())
        .getCmsObject()
        .getRequestContext()
        .getCurrentUser()
        .isGuestUser();
  ...
  /**
   * @see
   *     com.vaadin.server.VaadinServlet#createServletService(com.vaadin.server.DeploymentConfiguration)
   */
  @Override
  ...
  /**
   * @see com.vaadin.server.VaadinServlet#service(javax.servlet.http.HttpServletRequest,
  ...
  protected void service(@RUntainted HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      OpenCms.getSessionManager()
          .updateSessionInfo(getCmsObject(), request, isHeartbeatRequest(request));
  ...
  private SystemMessages createSystemMessages(@RUntainted Locale locale) {
    ...
    systemMessages.setCommunicationErrorCaption(
        messages.key(Messages.GUI_SYSTEM_COMMUNICATION_ERROR_CAPTION_0));
    systemMessages.setCommunicationErrorMessage(
        messages.key(Messages.GUI_SYSTEM_COMMUNICATION_ERROR_MESSAGE_0));
    systemMessages.setCommunicationErrorNotificationEnabled(true);
    systemMessages.setAuthenticationErrorCaption(
        messages.key(Messages.GUI_SYSTEM_AUTHENTICATION_ERROR_CAPTION_0));
    systemMessages.setAuthenticationErrorMessage(
        messages.key(Messages.GUI_SYSTEM_AUTHENTICATION_ERROR_MESSAGE_0));
    systemMessages.setAuthenticationErrorNotificationEnabled(true);
    systemMessages.setSessionExpiredCaption(
        messages.key(Messages.GUI_SYSTEM_SESSION_EXPIRED_ERROR_CAPTION_0));
    systemMessages.setSessionExpiredMessage(
        messages.key(Messages.GUI_SYSTEM_SESSION_EXPIRED_ERROR_MESSAGE_0));
    systemMessages.setSessionExpiredNotificationEnabled(true);
    systemMessages.setInternalErrorCaption(
        messages.key(Messages.GUI_SYSTEM_INTERNAL_ERROR_CAPTION_0));
    systemMessages.setInternalErrorMessage(
        messages.key(Messages.GUI_SYSTEM_INTERNAL_ERROR_MESSAGE_0));
    systemMessages.setInternalErrorNotificationEnabled(true);
    return systemMessages;
  }

  /**
   * Checks whether the given request is a heartbeat request.
   *
   * <p>
   *
   * @param request the request
