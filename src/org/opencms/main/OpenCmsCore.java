 ...
 *
 * <p>All access to this class must be done through the public static methods of the <code>
 * {@link org.opencms.main.OpenCms}</code> object. Under no circumstances should you ever try to
 * access this class directly.
 *
 * <p>This class is so OpenCms internal you should not even be reading this documentation ;-)
 *
 * <p>Any request to the <code>{@link org.opencms.main.OpenCmsServlet}</code> will be forwarded to
 * this core class. The core will then try to map the request to a VFS (Virtual File System) URI,
 * that is a <code>{@link org.opencms.file.CmsResource}</code> in the OpenCms database. If a
 * resource is found, it will be read and forwarded to to the corresponding <code>
 * {@link org.opencms.loader.I_CmsResourceLoader}</code>, which will then generate the output for
 * the requested resource and return it to the requesting client.
 *
 * <p>There will be only one singleton instance of this object created for this core class. This
 * means that in the default configuration, where OpenCms is accessed through a servlet context,
 * there will be only one instance of the core in that servlet context.
 *
 * <p>
 *
 * @since 6.0.0
...
public final class OpenCmsCore {
  ...
  /**
   * The configured request handlers that handle "special" requests, for example in the static
   * export on demand.
   */
  ...
  /**
   * Protected constructor that will initialize the singleton OpenCms instance with runlevel {@link
   ...
   *
   * <p>First checks the {@link HttpServletRequest#getPathInfo()}, then the configured request error
   * page attribute (if set), and then if still undefined the <code>/</code> is returned as path
   * info.
   *
   * <p>This is only needed when the {@link HttpServletRequest#getPathInfo()} is not really working
   * as expected like in BEA WLS 9.x, where we have to use the 'weblogic.servlet.errorPage' request
   * attribute.
   *
   * <p>
   *
    ...
    if ((m_instance != null) && (m_instance.getRunLevel() < OpenCms.RUNLEVEL_3_SHELL_ACCESS)) {
      if (!Messages.ERR_CRITICAL_INIT_WIZARD_0.equals(errorCondition.getKey())) {
        ...
        System.err.println(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_INIT_FAILURE_MESSAGE_1, errorCondition.key()));
      }
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_ADDED_REQUEST_HANDLER_2, name, handler.getClass().getName()));
      }
    }
  }
  ...
  protected I_CmsCredentialsResolver getCredentialsResolver() {
    ...
    if (m_credentialsResolver == null) {
      CmsSystemConfiguration systemConfig =
          (CmsSystemConfiguration)
              m_configurationManager.getConfiguration(CmsSystemConfiguration.class);
  ...
  /**
   * Gets a string containing all keys and variations currently in the flex cache, for debug
   * purposes.
   *
   * <p>
   *
  ...
  /**
   * Returns the initialized import/export manager, which contains information about the Cms
   * import/export.
   *
   * <p>
   *
  ...
  /**
   * Returns the handler instance for the specified name, or null if the name does not match any
   * handler name.
   *
   * <p>
   *
  ...
  /**
   * Gets the template context manager instance.
   *
   * <p>
   *
  ...
  /**
   * Returns the initialized workplace manager, which contains information about the global
   * workplace settings.
   *
   * <p>
   *
  ...
  protected void initCmsContextForUI(
    ...
    boolean allowPrivilegedLogin =
        (referrer == null) || !referrer.contains(CmsWorkplaceLoginHandler.LOGIN_HANDLER);
   ...
   *
   * <p>This can be useful in case a permanent reference to a CmsObject is stored. Changing the
   * request context values (for example project, siteroot) in the new CmsObject will have no side
   * effects to the CmsObject it was copied form.
   *
   * <p>The request time (<code>{@link CmsRequestContext#getRequestTime()}</code>) is set to the
   * current time.
   *
   * <p>
   *
   ...
   *
   * <p>Note: Only if the provided <code>adminCms</code> CmsObject has admin permissions, this
   * method allows the creation a CmsObject for any existing user. Otherwise only the default users
   * 'Guest' and 'Export' can initialized with this method, all other user names will throw an
   * Exception.
   *
   * <p>
   *
   ...
   * @throws CmsRoleViolationException if the current user does not have the role permissions to
   *     create a context for the requested user
        ...
        CmsMessageContainer message =
            Messages.get()
                .container(
                    ...
                    userName,
                    ((adminCms != null)
                        ? (adminCms.getRequestContext().getCurrentUser().getName())
                        : ""));
   ...
   *
   * <p>User authentification is done in three steps:
   *
   * <ol>
   *   <li>Session authentification: OpenCms stores information of all authentificated users in an
   *       internal storage based on the users session.
   *   <li>Authorization handler authentification: If the session authentification fails, the
   *       current configured authorization handler is called.
   *   <li>Default user: When both authentification methods fail, the user is set to the default
   *       (Guest) user.
   * </ol>
   *
  ...
  protected CmsObject initCmsObject(
    ...
    if (allowPrivilegedLogin) {
      ...
      I_CmsAuthorizationHandler.I_PrivilegedLoginAction loginAction =
          new I_CmsAuthorizationHandler.I_PrivilegedLoginAction() {
              ...
              try {
                ...
                if (!user.isEnabled()) {
                  throw new CmsException(
                      Messages.get()
                          .container(Messages.ERR_INVALID_INIT_USER_2, user.getName(), "-"));
                }
            ...
            /**
             * @see
             *     org.opencms.security.I_CmsAuthorizationHandler.I_PrivilegedLoginAction#getCmsObject()
             */
            ...
            /**
             * @see
             *     org.opencms.security.I_CmsAuthorizationHandler.I_PrivilegedLoginAction#setCmsObject(org.opencms.file.CmsObject)
             */
      ...
      loginAction.setCmsObject(
          initCmsObject(req, res, OpenCms.getDefaultUsers().getUserAdmin(), null, null));
  ...
  /**
   * Returns an initialized CmsObject with the user initialized as provided, with the "Online"
   * project selected and "/" set as the current site root.
   *
   * <p>Note: Only the default users 'Guest' and 'Export' can initialized with this method, all
   * other user names will throw an Exception.
   *
   * <p>
   *
   * @param user the user name to initialize, can only be {@link
   *     org.opencms.db.CmsDefaultUsers#getUserGuest()} or {@link
   *     org.opencms.db.CmsDefaultUsers#getUserExport()}
   * @return an initialized CmsObject with the given users permissions
  ...
  protected CmsObject initCmsObjectFromSession(@RUntainted HttpServletRequest req) throws CmsException {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          p + "Trying to init cms object from session for request \"" + req.toString() + "\".");
    }
    ...
    if (siteroot == null) {
      ...
      LOG.debug(
          p
              + "site root from session info was null, determining site root from current request's host");
   ...
   *
   * <p>It reads the configurations from the <code>opencms.properties</code> file in the <code>
   * config/</code> subdirectory. With the information from this file is inits a ResourceBroker
   * (Database access module), various caching systems and other options.
   *
   * <p>This will only be done once per accessing class.
   *
  ...
  protected synchronized void initConfiguration(CmsParameterConfiguration configuration)
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      ...
      CmsLog.INIT.info(
          ". "
              + Messages.get()
                  .getBundle()
                  .key(Messages.GUI_SHELL_VERSION_1, OpenCms.getSystemInfo().getVersionNumber()));
      ...
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_STARTUP_TIME_1, new Date(System.currentTimeMillis())));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(
      ...
      if (serverInfo != null) {
        CmsLog.INIT.info(
            Messages.get().getBundle().key(Messages.INIT_SERVLET_CONTAINER_1, serverInfo));
      }
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_WEBAPP_NAME_1, getSystemInfo().getWebApplicationName()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_SERVLET_PATH_1, getSystemInfo().getServletPath()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_OPENCMS_CONTEXT_1, getSystemInfo().getOpenCmsContext()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_WEBINF_PATH_1, getSystemInfo().getWebInfRfsPath()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_FILE_ENCODING_1, systemEncoding));
    }
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_ETHERNET_ADDRESS_1, ethernetAddress));
    }
    ...
    CmsMemoryMonitorConfiguration memoryMonitorConfiguration =
    ...
    try {
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(memoryMonitorConfiguration.getClassName())) {
        m_memoryMonitor =
            (CmsMemoryMonitor)
                Class.forName(memoryMonitorConfiguration.getClassName()).newInstance();
      } else {
    ...
    // get the event manager from the configuration and initialize it with the events already
    // registered
    ...
    if (defaultEncoding == null) {
      ...
      throw new CmsInitException(
          Messages.get().container(Messages.ERR_CRITICAL_INIT_ENCODING_1, setEncoding));
    }
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_OPENCMS_ENCODING_1, defaultEncoding));
    }
    ...
    getSystemInfo()
        .setHttpAuthenticationSettings(systemConfiguration.getHttpAuthenticationSettings());
    ...
    m_executor =
        new ScheduledThreadPoolExecutor(
            3, new ThreadFactoryBuilder().setNameFormat("OpenCmsCore-exec-%d").build());
    ...
    while (it.hasNext()) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_REQUEST_HANDLER_CLASS_1, handler.getClass().getName()));
      }
    }
    ...
    CmsSitesConfiguration sitesConfiguration =
        (CmsSitesConfiguration)
            m_configurationManager.getConfiguration(CmsSitesConfiguration.class);
    m_siteManager = sitesConfiguration.getSiteManager();

    CmsSchedulerConfiguration schedulerConfiguration =
        (CmsSchedulerConfiguration)
            m_configurationManager.getConfiguration(CmsSchedulerConfiguration.class);
    ...
    CmsVariablesConfiguration variablesConfiguration =
        (CmsVariablesConfiguration)
            m_configurationManager.getConfiguration(CmsVariablesConfiguration.class);

    // get the VFS / resource configuration
    CmsVfsConfiguration vfsConfiguation =
        (CmsVfsConfiguration) m_configurationManager.getConfiguration(CmsVfsConfiguration.class);
    ...
    try {
      ...
      CmsFlexCacheConfiguration flexCacheConfiguration =
          systemConfiguration.getCmsFlexCacheConfiguration();
      ...
      if (CmsLog.INIT.isWarnEnabled()) {
        CmsLog.INIT.warn(
            Messages.get().getBundle().key(Messages.INIT_FLEX_CACHE_ERROR_1, e.getMessage()));
      }
    }
    ...
    CmsImportExportConfiguration importExportConfiguration =
        (CmsImportExportConfiguration)
            m_configurationManager.getConfiguration(CmsImportExportConfiguration.class);
    ...
    CmsSearchConfiguration searchConfiguration =
        (CmsSearchConfiguration)
            m_configurationManager.getConfiguration(CmsSearchConfiguration.class);
    ...
    CmsWorkplaceConfiguration workplaceConfiguration =
        (CmsWorkplaceConfiguration)
            m_configurationManager.getConfiguration(CmsWorkplaceConfiguration.class);
    ...
    CmsModuleConfiguration moduleConfiguration =
        (CmsModuleConfiguration)
            m_configurationManager.getConfiguration(CmsModuleConfiguration.class);
    ...
    m_securityManager =
        CmsSecurityManager.newInstance(
            m_configurationManager, systemConfiguration.getRuntimeInfoFactory(), m_publishEngine);
    ...
    I_CmsSessionStorageProvider sessionStorageProvider =
        systemConfiguration.getSessionStorageProvider();

    // get an Admin cms context object with site root set to "/"
    CmsObject adminCms;
    try {
      adminCms =
          initCmsObject(null, null, getDefaultUsers().getUserAdmin(), (String) null, (String) null);
    } catch (CmsException e) {
      throw new CmsInitException(
          Messages.get().container(Messages.ERR_CRITICAL_INIT_ADMINCMS_0), e);
    }
    ...
    try {
      ...
      m_adeManager =
          new CmsADEManager(initCmsObject(adminCms), m_memoryMonitor, systemConfiguration);
      ...
      m_twoFactorAuthenticationHandler =
      ...
      throw new CmsInitException(
          Messages.get().container(Messages.ERR_CRITICAL_INIT_MANAGERS_0), e);
    }

    try {
      // mitigate potential stringtemplate 3 class loading deadlock by making sure the class is
      // loaded on startup
      @SuppressWarnings("unused")
      StringTemplate stringTemplate = new org.antlr.stringtemplate.StringTemplate();
    } catch (Exception e) {
      CmsLog.INIT.error(
          "Problem with initializing stringtemplate class: " + e.getLocalizedMessage(), e);
    }
    ...
    try {
      ...
      CmsLog.INIT.error(
          "Problem with clearing caches after initialization: " + e.getLocalizedMessage(), e);
    }
   ...
   *
   * <p>The connection information for the database is read from the <code>opencms.properties</code>
   * configuration file and all driver manager are initialized via the initializer, which usually
   * will be an instance of a <code>OpenCms</code> class.
   *
  ...
  protected void initMembers() {
    ...
    synchronized (LOCK) {
      ...
      // the default event manager must be available because the configuration already registers
      // events
  ...
  /**
   * Reads the requested resource from the OpenCms VFS, in case a directory name is requested, the
   * default files of the directory will be looked up and the first match is returned.
   *
   * <p>The resource that is returned is always a <code>{@link org.opencms.file.CmsFile}</code>,
   * even though the content will usually not be loaded in the result. Folders are never returned
   * since the point of this method is really to load the default file if just a folder name is
   * requested. If there is no default file in a folder, then the return value is null and no
   * CmsException is thrown.
   *
   * <p>The URI stored in the given OpenCms user context will be changed to the URI of the resource
   * that was found and returned.
   *
   * <p>Implementing and configuring an <code>{@link I_CmsResourceInit}</code> handler allows to
   * customize the process of default resource selection.
   *
   * <p>
   *
   ...
   * @throws CmsException in case the requested file does not exist or the user has insufficient
   *     access permissions
    ...
    if (resource != null) {
      ...
      if (resource.isInternalOrInInternalFolder()) {
        throw new CmsException(
            Messages.get()
                .container(
                    Messages.ERR_READ_INTERNAL_RESOURCE_1, cms.getRequestContext().getUri()));
      }
      ...
      if (tmpException != null) {
        ...
        throw new CmsVfsResourceNotFoundException(
            org.opencms.main.Messages.get()
                .container(org.opencms.main.Messages.ERR_PATH_NOT_FOUND_1, resourceName));
      }
    } else {
      if (!handledSecure) {
        if (cms.getRequestContext().getDetailContentId() != null) {
          // in theory we should do this for all kinds of resource init handlers,
          // but I'm not clear on how to handle this in general, so only do this for detail pages
          // for now
          resource = handleSecureResource(cms, req, res, resource, resourceName);
          handledSecure = true;
        }
   ...
   *
   * <p>This is the final step that is called on the servlets "init()" method. It registers the
   * servlets request handler and also outputs the final startup message. The servlet should
   * auto-load since the &ltload-on-startup&gt; parameter is set in the 'web.xml' by default.
   *
   * <p>
   *
  ...
  /**
   * Handler for built-in AJAX services that don't belong anywhere else and don't deserve their own
   * request handler.
   *
    ...
    try {
      ...
      if (page != null) {
        ...
        cms.getRequestContext()
            .setAttribute(CmsRequestContext.ATTRIBUTE_ADE_CONTEXT_PATH, page.getRootPath());
      }
  ...
  /**
   * This method adds an Object to the OpenCms runtime properties. The runtime properties can be
   * used to store Objects that are shared in the whole system.
   *
   * <p>
   *
  ...
  /**
   * Displays a resource from the OpenCms by writing the result to the provided Servlet response
   * output stream.
   *
   * <p>
   *
      ...
      try (CloseableThreadContext.Instance threadContext = CloseableThreadContext.putAll(logInfo)) {
        ...
        if (cms.getRequestContext().getCurrentProject().isOnlineProject()) {
          ...
          if (uri.startsWith(CmsWorkplace.VFS_PATH_SITES)) {
            ...
            throw new CmsVfsResourceNotFoundException(
                org.opencms.main.Messages.get()
                    .container(org.opencms.main.Messages.ERR_PATH_NOT_FOUND_1, uri));
          }
        ...
        if (currentSiteAliases.contains(currentSiteMatcher.forDifferentScheme("http"))
          ...
          switch (currentSiteAliases.get(pos).getRedirectMode()) {
            ...
            case temporary:
              res.sendRedirect(
                  m_siteManager.getCurrentSite(cms).getUrl()
                      + req.getContextPath()
                      + req.getPathInfo());
              return;
            case permanent:
        ...
        // a resource init handler may use its own authentication, but return a resource to be
        // loaded instead of handling the complete request processing by itself.
        // For this case, a request context attribute is used to pass  the CmsObject that should be
        // used for loading the resource.

        Object alternativeCmsObject =
            cms.getRequestContext().removeAttribute(I_CmsResourceInit.ATTR_ALTERNATIVE_CMS_OBJECT);
        ...
        if (resource != null) {
          ...
          if (cmsForLoad == cms) {
            ...
            // CmsObject - it's not necessary to do it for the original CmsObject, and using the
            // alternative CmsObject
            // if there is already a session would switch the session user to the one of that
            // CmsObject. We don't
            // want that, since the primary use case for the alternative CmsObject mechanism is
  ...
  protected void shutDown() {
    ...
    synchronized (LOCK) {
      if (getRunLevel() > OpenCms.RUNLEVEL_0_OFFLINE) {
          ...
          CmsLog.INIT.info(
              Messages.get()
                  ...
                  .key(
                      ...
                      getSystemInfo().getVersionNumber()
                          + " ["
                          + getSystemInfo().getVersionId()
                          + "]"));
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_CURRENT_RUNLEVEL_1, new Integer(getRunLevel())));
          CmsLog.INIT.info(
              Messages.get()
                  .getBundle()
                  .key(Messages.INIT_SHUTDOWN_TIME_1, new Date(System.currentTimeMillis())));
        }
        ...
        try {
          // search manager must be shut down early since there may be background indexing still
          // ongoing
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_SEARCH_MANAGER_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
          // remote shell server must be shut down early since there is a background thread ongoing
          // that reloads from the VFS
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_REMOTESHELL_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
          // VFS bundle manager must be shut down early since there is a background thread ongoing
          // that reloads from the VFS
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_VFSBUNDLE_MANAGER_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
        ...
        try {
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_SCHEDULE_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_RESOURCE_SHUTDOWN_1, e.getMessage()),
              e);
        }
        ...
        try {
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_SECURITY_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_SESSION_MANAGER_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_MEMORY_MONITOR_SHUTDOWN_1, e.getMessage()),
              e);
        }
        try {
          ...
          CmsLog.INIT.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_ERROR_ADE_MANAGER_SHUTDOWN_1, e.getMessage()),
              e);
        }
        ...
        System.err.println(
            Messages.get().getBundle().key(Messages.LOG_CONSOLE_TOTAL_RUNTIME_1, runtime));
      }
      m_instance = null;
    }
  }
   ...
   *
   * <p>The update information is:<br>
   *
   * <ul>
   *   <li>Requested Url
   *   <li>Locale
  ...
  protected CmsObject updateContext(@RUntainted HttpServletRequest request, CmsObject cms) throws CmsException {
    ...
    boolean isWorkplace =
        cms.getRequestContext().getUri().startsWith("/system/workplace/")
  ...
  /**
   * Upgrades to runlevel {@link OpenCms#RUNLEVEL_3_SHELL_ACCESS}, this is shell access to the
   * database but no Servlet context.
   *
   * <p>To upgrade the runlevel, the system must be in runlevel {@link
   * OpenCms#RUNLEVEL_1_CORE_OBJECT}, otherwise an exception is thrown.
   *
   * <p>
   *
   ...
   */
  protected OpenCmsCore upgradeRunlevel(CmsParameterConfiguration configuration)
      throws CmsInitException {
  ...
  /**
   * Upgrades to runlevel {@link OpenCms#RUNLEVEL_4_SERVLET_ACCESS}, this is the final runlevel with
   * an initialized database and Servlet context.
   *
   * <p>To upgrade the runlevel, the system must be in runlevel {@link
   * OpenCms#RUNLEVEL_1_CORE_OBJECT}, otherwise an exception is thrown.
   *
  ...
  protected void writeConfiguration(@RUntainted Class<?> clazz) {
    ...
    try {
      ...
      CmsLog.getLog(CmsConfigurationManager.class)
          .error(
  ...
  private void addExportPoints(Set<CmsExportPoint> exportPoints) {
    ...
    HashSet<CmsExportPoint> newSet =
        new HashSet<CmsExportPoint>(m_exportPoints.size() + exportPoints.size());
  ...
  private void afterUpgradeRunlevel() {
      ...
      if (LOG.isErrorEnabled()) {
        LOG.error(
            org.opencms.lock.Messages.get()
                .getBundle()
                .key(org.opencms.lock.Messages.ERR_READ_LOCKS_0),
            e);
      }
    }

    if (OpenCms.getRunLevel() == OpenCms.RUNLEVEL_4_SERVLET_ACCESS) {
      // only init ADE manager in case of servlet initialization, it won't be needed in case of
      // shell access
      ...
      try {
        ...
        CmsObject adminCms =
            initCmsObject(
                null, null, getDefaultUsers().getUserAdmin(), (String) null, (String) null);
    ...
    try {
      // Workaround: The GWT serializer for LinkedHashMaps uses reflection on
      // java.util.LinkedHashMap that is disallowed in newer Java versions that use modules.
      // This can be bypassed by setting a private 'reflectionHasFailed' field in the serializer
      // class. *This* reflective access is OK, because it does not access a different module.
      // GWT should really handle that problem, but as of version 2.9.0 it does not. This workaround
      // can be removed if a newer GWT version handles the problem correctly.
   ...
   */
  private boolean checkForceAbsoluteLinks(
      HttpServletRequest req, CmsObject cms, CmsResource resource) {

    try {
      boolean forceAbsoluteLinks =
          Boolean.parseBoolean(req.getParameter(PARAM_FORCE_ABSOLUTE_LINKS));
      if (forceAbsoluteLinks) {
        // only need to read the property if the request parameter is actually set
        CmsProperty enableForceAbsoluteProp =
            cms.readPropertyObject(
                resource, CmsPropertyDefinition.PROPERTY_LINKS_FORCEABSOLUTE_ENABLED, true);
   ...
   */
  private synchronized CmsGwtService getGwtService(String serviceName, ServletConfig servletConfig)
      throws Throwable {
  ...
  private @RUntainted String getLoginFormURL(@RUntainted HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      throw new IOException(
          Messages.get()
              .getBundle()
              .key(
                  Messages.ERR_INVALID_INIT_USER_2, OpenCms.getDefaultUsers().getUserAdmin(), null),
          e);
    }
    ...
    try {
      propertyLoginForm =
          adminCms.readPropertyObject(path, CmsPropertyDefinition.PROPERTY_LOGIN_FORM, true);
    } catch (Throwable t) {
      if (t instanceof CmsVfsResourceNotFoundException) {
        // if we can't read the property from the path, try to use the resource init handlers to
        // find the
    ...
    if ((propertyLoginForm != null)
      ...
      params = "__loginform=true";
        ...
        && CmsStringUtil.isNotEmpty(
            httpAuthenticationSettings.getFormBasedHttpAuthenticationUri())) {
  ...
  /**
   * If we are in the Online project, check if the given resource is marked as secure, and handle it
   * according to the secure server configuration.
   *
   * <p>
   *
    ...
    if (cms.getRequestContext().getCurrentProject().isOnlineProject() && (res != null)) {
      ...
      try {
        ...
        secure =
            Boolean.valueOf(
                    cms.readPropertyObject(
                            cms.getSitePath(resource), CmsPropertyDefinition.PROPERTY_SECURE, true)
                        .getValue())
                .booleanValue();
      ...
      if (secure) {
        ...
        try {
          ...
          LOG.error(
              Messages.get()
                  .getBundle()
                  .key(Messages.ERR_SECURE_SITE_NOT_CONFIGURED_1, resourceName),
              e);
          throw new CmsException(
              Messages.get().container(Messages.ERR_SECURE_SITE_NOT_CONFIGURED_1, resourceName), e);
        }
        boolean usingSec = true;
        if (req != null) {
          usingSec =
              req.getRequestURL().toString().toUpperCase().startsWith(secureUrl.toUpperCase());
        }
  ...
  private CmsObject initCmsObject(
    ...
    if (m_localeManager.isInitialized()) {
      ...
      if ((request != null)
          && (requestedResource.endsWith(OpenCmsServlet.HANDLE_GWT)
              || isWorkplaceServletRequest(request))) {
        // GWT RPC or workplace servlet call, always keep the request encoding and use the default
        // locale
        i18nInfo =
    ...
    // in case the current site could be configured for single tree localization, if so, remove the
   ...
   *
   * <p>In case the password is <code>null</code>, or the user is the <code>Guest</code> user, no
   * password check is done. Therefore you can initialize all users without knowing their passwords
   * by just supplying <code>null</code> as password. This is intended only for internal operation
   * in the core.
   *
   * <p>
   *
  ...
  /**
   * Checks whether the given request targets the workplace UI servlet.
   *
   * <p>
   *
   * @param req the request
