...
public class CmsServletContainerSettings {
  ...
  public static final class CmsServletContainerCfgMode extends A_CmsModeStringEnumeration {
    ...
    protected static final CmsServletContainerCfgMode MODE_AUTO =
        new CmsServletContainerCfgMode("auto");

    /** Manual configuration mode. */
    protected static final CmsServletContainerCfgMode MODE_MANUAL =
        new CmsServletContainerCfgMode("manual");

    /** No set configuration mode. */
    protected static final CmsServletContainerCfgMode MODE_NONE =
        new CmsServletContainerCfgMode("none");
  ...
  public static final CmsServletContainerCfgMode CFG_MODE_AUTO =
      CmsServletContainerCfgMode.MODE_AUTO;

  /** Map remote command execution return type. */
  public static final CmsServletContainerCfgMode CFG_MODE_MANUAL =
      CmsServletContainerCfgMode.MODE_MANUAL;

  /** List remote command execution return type. */
  public static final CmsServletContainerCfgMode CFG_MODE_NONE =
      CmsServletContainerCfgMode.MODE_NONE;

  /** The static log object for this class. */
  private static final Log LOG = CmsLog.getLog(CmsServletContainerSettings.class);

  /**
   * If the servlet can throw an exception if initialization fails, for instance, Weblogic and Resin
   * have problems with the exception.
   */
  private static boolean m_servletThrowsException = true;

  /**
   * The replacement request attribute for the {@link
   * javax.servlet.http.HttpServletRequest#getPathInfo()} method, which is needed because this
   * method is not properly implemented in BEA WLS 9.x.
   *
   * <p>
   */
   ...
   * set headers afterwards, so we have to prevent it.
   */
  private boolean m_preventResponseFlush;

  /**
   * If the tags need to be released after ending, this has to be prevented when running with Resin,
   * for example.
   */
  private boolean m_releaseTagsAfterEnd = true;

  /**
   * The request error page attribute to use if {@link
  ...
  public CmsServletContainerSettings(@RUntainted ServletContext context) {
    ...
    if (context != null) {
      ...
      //      this is needed in case an application server specific deployment descriptor is used to
      // changed the webapp context
   ...
   *
   * <p>This is intended for <code>{@link CmsShell}</code> access.
   *
   * <p>
   *
   * @param webInfRfsPath the OpenCms web application "WEB-INF" path in the "real" file system) to
   *     set
   * @param servletMapping the OpenCms servlet mapping (e.g. "/opencms/*")
   * @param webApplicationContext the name/path of the OpenCms web application context (optional,
   *     will be calculated form the path if null)
   ...
   *
   * <p>While the "webapplication - mode" constructor obtains all values from the <code>
   * {@link ServletContext}</code> the <code>{@link CmsShell}</code> constructor accepts them as
   * arguments and passes them here.
   *
   * <p>
   *
   * @param webInfRfsPath the OpenCms web application "WEB-INF" path in the "real" file system) to
  ...
  private void init(
    ...
    if (defaultWebApplication.endsWith("/")) {
      defaultWebApplication =
          defaultWebApplication.substring(0, defaultWebApplication.length() - 1);
    }
   ...
   * <p>
   *
   * @param context the servlet context to find out information about the servlet container
   */
