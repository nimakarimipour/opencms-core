 ...
 * which is used to get an overview about currently authenticated OpenCms users, as well as sending
 * broadcasts between users.
 *
 * <p>For each authenticated OpenCms user, a {@link org.opencms.main.CmsSessionInfo} object holds
 * the information about the users status.
 *
 * <p>When a user session is invalidated, the user info will be removed. This happens when a user
 * log out, or when his session times out.
 *
 * <p><b>Please Note:</b> The current implementation does not provide any permission checking, so
 * all users can access the methods of this manager. Permission checking based on the current users
 * OpenCms context may be added in a future OpenCms release.
 *
 * <p>
 *
 * @since 6.0.0
...
public class CmsSessionManager {
      ...
      if (!infos.isEmpty()) {
        throw new CmsCustomLoginException(
            org.opencms.security.Messages.get()
                .container(org.opencms.security.Messages.ERR_ALREADY_LOGGED_IN_0));
      }
    }
  }
  ...
  /**
   * Returns the number of current sessions, including the sessions of not authenticated guest
   * users.
   *
   * <p>
   *
  ...
  /**
   * Returns the complete user session info of a user from the session storage, or <code>null</code>
  ...
  /**
   * Returns the OpenCms user session info for the given request, or <code>null</code> if no user
   * session is available.
   *
   * <p>
   *
   * @param req the current request
   * @return the OpenCms user session info for the given request, or <code>null</code> if no user
   *     session is available
   */
  ...
  /**
   * Returns the OpenCms user session info for the given http session, or <code>null</code> if no
   * user session is available.
   *
   * <p>
   *
   * @param session the current http session
   * @return the OpenCms user session info for the given http session, or <code>null</code> if no
   *     user session is available
   */
   ...
   * <p>
   *
   * @param sessionId the OpenCms session id to return the session info for, this must be a String
   *     representation of a {@link CmsUUID}
   * @return the complete user session info of a user from the session storage
   ...
   *
   * <p>An OpenCms user can have many active sessions. This is e.g. possible when two people have
   * logged in to the system using the same username. Even one person can have multiple sessions if
   * he is logged in to OpenCms with several browser windows at the same time.
   *
   * <p>
   *
   ...
   *
   * <p>Used to prevent session hijacking.
   *
   * <p>
   *
   * @param req the current request
  ...
  /**
   * Destroys a session given the session id. Only allowed for users which have the "account
   * manager" role.
   *
   * <p>
   *
  ...
  public void sendBroadcast(
    ...
    if (sessionInfo != null) {
      ...
      sessionInfo
          .getBroadcastQueue()
          .add(new CmsBroadcast(cms.getRequestContext().getCurrentUser(), message, repeat, mode));
    }
  }
   ...
   *
   * <p>The user sending the message may be a real user like <code>
   * cms.getRequestContext().currentUser()</code> or <code>null</code> for a system message.
   *
   * <p>
   *
   ...
   */
  public void sendBroadcast(
      CmsUser fromUser, String message, CmsUser toUser, CmsBroadcast.ContentMode mode) {
   ...
   * @throws CmsException if something goes wrong
   */
  public String switchUser(CmsObject cms, HttpServletRequest req, CmsUser user)
      throws CmsException {
   ...
   * @throws CmsException if something goes wrong
   */
  public String switchUserFromSession(
      CmsObject cms, HttpServletRequest req, CmsUser user, CmsSessionInfo sessionInfo)
      throws CmsException {
    ...
    if (!OpenCms.getRoleManager().hasRole(cms, user.getName(), CmsRole.ELEMENT_AUTHOR)) {
      throw new CmsSecurityException(
          Messages.get().container(Messages.ERR_NO_WORKPLACE_PERMISSIONS_0));
    }
    ...
    CmsSessionInfo newInfo =
        new CmsSessionInfo(context, info.getSessionId(), info.getMaxInactiveInterval());
    ...
    String directEditTarget =
        CmsLoginHelper.getDirectEditPath(cms, new CmsUserSettings(user), false);
   ...
   *
   * <p>This is required if the user data (current group or project) was changed in the requested
   * document.
   *
   * <p>The user data is only updated if the user was authenticated to the system.
   *
    ...
    if (!cms.getRequestContext().getCurrentUser().isGuestUser()) {
      ...
      if (sessionInfo != null) {
        ...
        if (session != null) {
          ...
          sessionInfo =
              new CmsSessionInfo(
                  cms.getRequestContext(), new CmsUUID(), session.getMaxInactiveInterval());
          // append the session info to the http session
          session.setAttribute(
              CmsSessionInfo.ATTRIBUTE_SESSION_ID, sessionInfo.getSessionId().clone());
  ...
  protected void addSessionInfo(CmsSessionInfo sessionInfo) {
    ...
    if (getUserSessionMode() == UserSessionMode.standard) {
      ...
      if (infos.isEmpty()
          || ((infos.size() == 1)
              && infos.get(0).getSessionId().equals(sessionInfo.getSessionId()))) {
        m_sessionStorageProvider.put(sessionInfo);
      } else {
  ...
  protected void initialize(
      I_CmsSessionStorageProvider sessionStorageProvider, CmsObject adminCms) {
  ...
  /**
   * Called by the {@link OpenCmsListener} when a http session is created.
   *
   * <p>
   *
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          tid
              + Messages.get()
                  .getBundle()
                  .key(Messages.LOG_SESSION_CREATED_1, event.getSession().getId()));
      if (request != null) {
  ...
  /**
   * Called by the {@link OpenCmsListener} when a http session is destroyed.
   *
   * <p>
   *
   * @param event the http session event
  ...
  protected void sessionDestroyed(@RUntainted HttpSessionEvent event) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_SESSION_DESTROYED_1, event.getSession().getId()));
    }
  }
  ...
  /**
   * Validates the sessions stored in this manager and removes any sessions that have become
   * invalidated.
   *
   * <p>
   */
  ...
  /**
   * Generates a token based on hashed client ip and user agent.
   *
   * <p>Used to prevent session hijacking.
   *
   * <p>
