...
public class CmsSearchConfiguration extends A_CmsXmlConfiguration {
  ...
  public void addXmlDigesterRules(Digester digester) {
    ...
    digester.addCallMethod(
        XPATH_SEARCH + "/" + N_OFFLINE_UPDATE_FREQUENCY, "setOfflineUpdateFrequency", 0);
    ...
    digester.addCallMethod(
        XPATH_SEARCH + "/" + N_EXTRACTION_CACHE_MAX_AGE, "setExtractionCacheMaxAge", 0);
    ...
    digester.addCallMethod(
        xPath + "/" + N_RESOURCETYPES + "/" + N_RESOURCETYPE, "addResourceType", 0);
    ...
    digester.addCallMethod(
        xPath + "/" + N_DOCUMENTTYPES_INDEXED + "/" + N_NAME, "addDocumentType", 0);
  ...
  public Element generateXml(Element parent) {
    ...
    searchElement
        .addElement(N_OFFLINE_UPDATE_FREQUENCY)
        .addText(String.valueOf(m_searchManager.getOfflineUpdateFrequency()));
    // add <maxIndexWaitTime> element
    searchElement
        .addElement(N_MAX_INDEX_WAIT_TIME)
        .addText(String.valueOf(m_searchManager.getMaxIndexWaitTime()));
    ...
    // add <exerpt> element
    searchElement
        .addElement(N_EXCERPT)
        .addText(String.valueOf(m_searchManager.getMaxExcerptLength()));
    // add <extractionCacheMaxAge> element
    searchElement
        .addElement(N_EXTRACTION_CACHE_MAX_AGE)
        .addText(String.valueOf(m_searchManager.getExtractionCacheMaxAge()));
    // add <maxModificationsBeforeCommit> element
    searchElement
        .addElement(N_MAX_MODIFICATIONS_BEFORE_COMMIT)
        .addText(String.valueOf(m_searchManager.getMaxModificationsBeforeCommit()));
    // add <highlighter> element
    searchElement
        .addElement(N_HIGHLIGHTER)
        .addText(m_searchManager.getHighlighter().getClass().getName());
    ...
    ArrayList<Locale> analyzerLocaleList =
        new ArrayList<Locale>(m_searchManager.getAnalyzers().keySet());
    ...
    while (analyzersLocaleInterator.hasNext()) {
      CmsSearchAnalyzer searchAnalyzer =
          m_searchManager.getCmsSearchAnalyzer(analyzersLocaleInterator.next());
    ...
    for (CmsSearchIndexSource searchIndexSource :
        m_searchManager.getSearchIndexSources().values()) {
      ...
      Element indexerElement =
          indexsourceElement
              .addElement(N_INDEXER)
              .addAttribute(N_CLASS, searchIndexSource.getIndexerClassName());
      for (Entry<String, String> entry : searchIndexSource.getParams().entrySet()) {
        // add <param name=""> element(s)
        indexerElement
            .addElement(I_CmsXmlConfiguration.N_PARAM)
            .addAttribute(I_CmsXmlConfiguration.A_NAME, entry.getKey())
    ...
    for (I_CmsSearchFieldConfiguration fieldConfiguration :
        m_searchManager.getFieldConfigurations()) {
      Element fieldConfigurationElement =
          fieldConfigurationsElement.addElement(N_FIELDCONFIGURATION);
      // add class attribute (if required)
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(fieldConfiguration.getDescription())) {
        fieldConfigurationElement
            .addElement(N_DESCRIPTION)
            .setText(fieldConfiguration.getDescription());
      }
      // search fields
