 ...
 *
 * <p>This is required because the default digester implementation will cause an exception in case a
 * resource type class is missing. However, a missing class is common if a module with a new
 * resource type class is imported. In this case, the resource type class is changes to <code>
 * {@link org.opencms.file.types.CmsResourceTypeUnknown}</code>, so that the import of the resources
 * can proceed.
 *
 * <p>
 *
...
public class CmsDigesterResourceTypeCreationFactory
    extends AbstractObjectCreationFactory<I_CmsResourceType> {
  ...
  public I_CmsResourceType createObject(@RUntainted Attributes attributes) throws Exception {
    ...
    try {
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.ERR_UNKNOWN_RESTYPE_CLASS_2, className, type.getClass().getName()),
          e);
    }
    return type;
  }
