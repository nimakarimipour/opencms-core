...
public class CmsSystemConfiguration extends A_CmsXmlConfiguration {
  ...
  private List<@RUntainted I_CmsRequestHandler> m_requestHandlers;
    ...
    try {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_EVENTMANAGER_CLASS_SUCCESS_1, m_eventManager));
      }
    } catch (Throwable t) {
      LOG.error(
          Messages.get().getBundle().key(Messages.INIT_EVENTMANAGER_CLASS_INVALID_1, clazz), t);
      return;
    }
  }
  ...
  public void addRequestHandler(@RUntainted String clazz, CmsParameterConfiguration params) {
    ...
    @RUntainted Object handler;
    try {
      handler = Class.forName(clazz).newInstance();
    } catch (Throwable t) {
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_INIT_REQUEST_HANDLER_FAILURE_1, clazz), t);
      return;
    }
    if (handler instanceof I_CmsRequestHandler) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get().getBundle().key(Messages.INIT_REQUEST_HANDLER_SUCCESS_1, clazz));
      }
    } else {
      if (CmsLog.INIT.isErrorEnabled()) {
        CmsLog.INIT.error(
            Messages.get().getBundle().key(Messages.INIT_REQUEST_HANDLER_INVALID_1, clazz));
      }
    }
  }
   ...
   * @param params the parameters set for the resource init handler (parameters need to be copied
   *     out, the object will be modified after use)
   */
  public void addResourceInitHandler(@RUntainted String clazz, CmsParameterConfiguration params)
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_RESOURCE_INIT_CLASS_INVALID_1, clazz), t);
      return;
    }
    if (initClass instanceof I_CmsResourceInit) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get().getBundle().key(Messages.INIT_RESOURCE_INIT_SUCCESS_1, clazz));
      }
    } else {
      if (CmsLog.INIT.isErrorEnabled()) {
        CmsLog.INIT.error(
            Messages.get().getBundle().key(Messages.INIT_RESOURCE_INIT_INVALID_CLASS_1, clazz));
      }
    }
  }
  ...
  public void addXmlDigesterRules(Digester digester) {
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_I18N + "/" + N_REUSE_ELEMENTS, "setReuseElements", 0);
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_MAIL + "/" + N_MAILFROM, "setMailFromDefault", 0);
    ...
    // add event classes
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_EVENTS + "/" + N_EVENTMANAGER, "addEventManager", 1);
    digester.addCallParam("*/" + N_SYSTEM + "/" + N_EVENTS + "/" + N_EVENTMANAGER, 0, A_CLASS);

    // use array so we can modify it in the inner class and give each resource init handler a fresh
    // CmsParameterConfiguration instance
    ...
    digester.addBeanPropertySetter(
        "*/" + N_SYSTEM + "/" + N_PASSWORDHANDLER + "/" + N_PASSWORDENCODING, "inputEncoding");
    digester.addBeanPropertySetter(
        "*/" + N_SYSTEM + "/" + N_PASSWORDHANDLER + "/" + N_DIGESTTYPE, "digestType");
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_SAX_IMPL_SYSTEM_PROPERTIES, "setUseSaxImplSystemProperties", 1);
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RUNTIMECLASSES + "/" + N_RUNTIMEINFO, "setRuntimeInfoFactory", 1);
    digester.addCallParam(
        "*/" + N_SYSTEM + "/" + N_RUNTIMECLASSES + "/" + N_RUNTIMEINFO, 0, A_CLASS);
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_DEFAULT_CONTENT_ENCODING, "setDefaultContentEncoding", 1);
    digester.addCallParam("*/" + N_SYSTEM + "/" + N_DEFAULT_CONTENT_ENCODING, 0);

    // add memorymonitor configuration rule
    digester.addObjectCreate(
        "*/" + N_SYSTEM + "/" + N_MEMORYMONITOR, CmsMemoryMonitorConfiguration.class);
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_MEMORYMONITOR + "/" + N_EMAIL_SENDER, "setEmailSender", 0);
    digester.addCallMethod(
    ...
    digester.addSetNext(
        "*/" + N_SYSTEM + "/" + N_MEMORYMONITOR, "setCmsMemoryMonitorConfiguration");
    ...
    digester.addObjectCreate(
        "*/" + N_SYSTEM + "/" + N_HTTP_AUTHENTICATION, CmsHttpAuthenticationSettings.class);
    digester.addCallMethod(
    ...
    digester.addCallMethod(
        ...
        0);
    digester.addSetNext(
        "*/" + N_SYSTEM + "/" + N_HTTP_AUTHENTICATION, "setHttpAuthenticationSettings");

    // cache rules
    digester.addObjectCreate("*/" + N_SYSTEM + "/" + N_RESULTCACHE, CmsCacheSettings.class);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_KEYGENERATOR, "setCacheKeyGenerator", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_USERS, "setUserCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_GROUPS, "setGroupCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_ORGUNITS, "setOrgUnitCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_USERGROUPS,
        "setUserGroupsCacheSize",
        0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_PROJECTS, "setProjectCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_PROJECTRESOURCES,
        "setProjectResourcesCacheSize",
        0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_RESOURCES, "setResourceCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_ROLES, "setRolesCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_RESOURCELISTS,
        "setResourcelistCacheSize",
        0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_PROPERTIES, "setPropertyCacheSize", 0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_PROPERTYLISTS,
        "setPropertyListsCacheSize",
        0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESULTCACHE + "/" + N_SIZE_ACLS, "setAclCacheSize", 0);
    digester.addCallMethod(
    ...
    digester.addCallMethod(
        ...
        1);
    digester.addCallParam(
        "*/" + N_SYSTEM + "/" + N_CONTENT_NOTIFICATION + "/" + N_NOTIFICATION_TIME, 0);
    ...
    digester.addCallMethod(
        ...
        1);
    digester.addCallParam(
        "*/" + N_SYSTEM + "/" + N_CONTENT_NOTIFICATION + "/" + N_NOTIFICATION_PROJECT, 0);

    // add authorization handler creation rules
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_AUTHORIZATIONHANDLER, "setAuthorizationHandler", 1);
    ...
    digester.addCallMethod(
        ...
        2);
    digester.addCallParam(
        "*/" + N_SYSTEM + "/" + N_AUTHORIZATIONHANDLER + "/parameters/param", 0, "name");
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_PUBLISHMANAGER + "/" + N_HISTORYSIZE, "setPublishHistorySize", 0);
    digester.addCallMethod(
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_SESSION_STORAGEPROVIDER, "setSessionStorageProvider", 1);
    ...
    digester.addCallMethod(
        ...
        0);
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_SERVLETCONTAINERSETTINGS, "setServletContainerSettingsMode", 1);
    ...
    digester.addCallMethod(
        adeCachePath + "/" + N_GROUPCONTAINERS, "setGroupContainerOfflineSize", 1);
    digester.addCallParam(adeCachePath + "/" + N_GROUPCONTAINERS, 0, A_OFFLINE);
    digester.addCallMethod(
        adeCachePath + "/" + N_GROUPCONTAINERS, "setGroupContainerOnlineSize", 1);
    ...
    digester.addObjectCreate(
        "*/" + N_SYSTEM + "/" + N_SUBSCRIPTIONMANAGER, CmsSubscriptionManager.class);
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_PUBLISH_LIST_REMOVE_MODE, "setPublishListRemoveMode", 1);
    ...
    digester.addSetNext(
        CmsLetsEncryptConfiguration.CONFIG_HELPER.getBasePath(), "setLetsEncryptConfig");

    digester.addRule(
        "*/" + N_SYSTEM + "/" + N_ENCRYPTION + "/" + N_TEXT_ENCRYPTION,
        new Rule() {
          ...
          public void begin(String namespace, String name, Attributes attributes) throws Exception {
            ...
            I_CmsTextEncryption encrypter =
                (I_CmsTextEncryption) Class.forName(className).newInstance();
          ...
          public void end(String namespace, String name) throws Exception {
            ...
            if (m_textEncryptions.get("default") == null) {
              ...
              defaultEncryption.addConfigurationParameter(
                  CmsAESTextEncryption.PARAM_SECRET, RandomStringUtils.randomAlphanumeric(24));
    ...
    digester.addCallMethod(
        "*/" + N_SYSTEM + "/" + N_RESTRICT_DETAIL_CONTENTS, "setRestrictDetailContents", 1);
    ...
    digester.addObjectCreate(
        detailPageHandlerPath, CmsDefaultDetailPageHandler.class.getName(), A_CLASS);
  ...
  public Element generateXml(Element parent) {
    ...
    i18nElement
        .addElement(N_LOCALEHANDLER)
        .addAttribute(A_CLASS, m_localeManager.getLocaleHandler().getClass().getName());
    Iterator<Locale> loc;
    ...
    // password handler
    Element passwordhandlerElement =
        systemElement
            .addElement(N_PASSWORDHANDLER)
            .addAttribute(A_CLASS, m_passwordHandler.getClass().getName());
    passwordhandlerElement
        .addElement(N_PASSWORDENCODING)
        .addText(m_passwordHandler.getInputEncoding());
    ...
    if (m_loginManager != null) {
      ...
      managerElement
          ...
          .addText(String.valueOf(m_loginManager.getDisableMinutes()));
      managerElement
          .addElement(N_MAXBADATTEMPTS)
          .addText(String.valueOf(m_loginManager.getMaxBadAttempts()));
      managerElement
          .addElement(N_ENABLESCURITY)
          .addText(String.valueOf(m_loginManager.isEnableSecurity()));
      ...
      if (m_loginManager.getPasswordChangeIntervalStr() != null) {
        managerElement
            .addElement(N_PASSWORD_CHANGE_INTERVAL)
            .addText(m_loginManager.getPasswordChangeIntervalStr());
      }

      if (m_loginManager.getUserDataCheckIntervalStr() != null) {
        managerElement
            .addElement(N_USER_DATA_CHECK_INTERVAL)
            .addText(m_loginManager.getUserDataCheckIntervalStr());
      }
    ...
    if (m_runtimeProperties != null) {
      ...
      while (it.hasNext()) {
        ...
        runtimepropertiesElement
            .addElement(N_PARAM)
            .addAttribute(A_NAME, key)
            .addText(m_runtimeProperties.get(key));
      }
    }
    ...
    if (!m_cmsDefaultUsers.getUserDeletedResource().equals(m_cmsDefaultUsers.getUserAdmin())) {
      ...
      defaultusersElement
          ...
          .addText(m_cmsDefaultUsers.getUserDeletedResource());
    }
    // create <group-administrators> subnode
    defaultusersElement
        .addElement(N_GROUP_ADMINISTRATORS)
        .addText(m_cmsDefaultUsers.getGroupAdministrators());
    ...
    if (m_cmsMemoryMonitorConfiguration != null) {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(
          m_cmsMemoryMonitorConfiguration.getClassName())) {
        memorymonitorElement.addAttribute(A_CLASS, m_cmsMemoryMonitorConfiguration.getClassName());
      }

      memorymonitorElement
          .addElement(N_MAXUSAGE_PERCENT)
          .addText(String.valueOf(m_cmsMemoryMonitorConfiguration.getMaxUsagePercent()));

      memorymonitorElement
          .addElement(N_LOG_INTERVAL)
          .addText(String.valueOf(m_cmsMemoryMonitorConfiguration.getLogInterval()));

      if (m_cmsMemoryMonitorConfiguration.getEmailInterval() >= 0) {
        memorymonitorElement
            .addElement(N_EMAIL_INTERVAL)
            .addText(String.valueOf(m_cmsMemoryMonitorConfiguration.getEmailInterval()));
      }

      memorymonitorElement
          .addElement(N_WARNING_INTERVAL)
          .addText(String.valueOf(m_cmsMemoryMonitorConfiguration.getWarningInterval()));

      if (m_cmsMemoryMonitorConfiguration.getEmailSender() != null) {
        memorymonitorElement
            .addElement(N_EMAIL_SENDER)
            .addText(m_cmsMemoryMonitorConfiguration.getEmailSender());
      }
    ...
    flexcacheElement
        .addElement(N_CACHE_ENABLED)
        .addText(String.valueOf(m_cmsFlexCacheConfiguration.isCacheEnabled()));
    flexcacheElement
        .addElement(N_CACHE_OFFLINE)
        .addText(String.valueOf(m_cmsFlexCacheConfiguration.isCacheOffline()));
    flexcacheElement
        .addElement(N_MAXCACHEBYTES)
        .addText(String.valueOf(m_cmsFlexCacheConfiguration.getMaxCacheBytes()));
    flexcacheElement
        .addElement(N_AVGCACHEBYTES)
        .addText(String.valueOf(m_cmsFlexCacheConfiguration.getAvgCacheBytes()));
    flexcacheElement
        .addElement(N_MAXENTRYBYTES)
        .addText(String.valueOf(m_cmsFlexCacheConfiguration.getMaxEntryBytes()));
    flexcacheElement
        .addElement(N_MAXKEYS)
        .addText(String.valueOf(m_cmsFlexCacheConfiguration.getMaxKeys()));
    if (m_cmsFlexCacheConfiguration.getDeviceSelectorConfiguration() != null) {
      ...
      flexcacheDeviceSelectorElement.addAttribute(
          A_CLASS, m_cmsFlexCacheConfiguration.getDeviceSelectorConfiguration());
    }
    ...
    httpAuthenticationElement
        .addElement(N_BROWSER_BASED)
        .setText(m_httpAuthenticationSettings.getConfigBrowserBasedAuthentication());
    if (m_httpAuthenticationSettings.getFormBasedHttpAuthenticationUri() != null) {
      httpAuthenticationElement
          .addElement(N_FORM_BASED)
          .setText(m_httpAuthenticationSettings.getFormBasedHttpAuthenticationUri());
    }
    ...
    cacheElement
        .addElement(N_SIZE_USERS)
        .setText(Integer.toString(m_cacheSettings.getUserCacheSize()));
    cacheElement
        .addElement(N_SIZE_GROUPS)
        .setText(Integer.toString(m_cacheSettings.getGroupCacheSize()));
    if (m_cacheSettings.getConfiguredOrgUnitCacheSize() > -1) {
      cacheElement
          .addElement(N_SIZE_ORGUNITS)
          .setText(Integer.toString(m_cacheSettings.getConfiguredOrgUnitCacheSize()));
    }
    cacheElement
        .addElement(N_SIZE_USERGROUPS)
        .setText(Integer.toString(m_cacheSettings.getUserGroupsCacheSize()));
    cacheElement
        .addElement(N_SIZE_PROJECTS)
        .setText(Integer.toString(m_cacheSettings.getProjectCacheSize()));
    if (m_cacheSettings.getConfiguredProjectResourcesCacheSize() > -1) {
      cacheElement
          .addElement(N_SIZE_PROJECTRESOURCES)
          .setText(Integer.toString(m_cacheSettings.getConfiguredProjectResourcesCacheSize()));
    }
    cacheElement
        .addElement(N_SIZE_RESOURCES)
        .setText(Integer.toString(m_cacheSettings.getResourceCacheSize()));
    if (m_cacheSettings.getConfiguredRolesCacheSize() > -1) {
      cacheElement
          .addElement(N_SIZE_ROLES)
          .setText(Integer.toString(m_cacheSettings.getConfiguredRolesCacheSize()));
    }
    cacheElement
        .addElement(N_SIZE_RESOURCELISTS)
        .setText(Integer.toString(m_cacheSettings.getResourcelistCacheSize()));
    cacheElement
        .addElement(N_SIZE_PROPERTIES)
        .setText(Integer.toString(m_cacheSettings.getPropertyCacheSize()));
    if (m_cacheSettings.getConfiguredPropertyListsCacheSize() > -1) {
      cacheElement
          .addElement(N_SIZE_PROPERTYLISTS)
          .setText(Integer.toString(m_cacheSettings.getConfiguredPropertyListsCacheSize()));
    }
    cacheElement
        .addElement(N_SIZE_ACLS)
        .setText(Integer.toString(m_cacheSettings.getAclCacheSize()));
    cacheElement
        .addElement(N_SIZE_PERMISSIONS)
        .setText(Integer.toString(m_cacheSettings.getPermissionCacheSize()));
    ...
    for (I_CmsTextEncryption encrypter : m_textEncryptions.values()) {
      ...
      for (Map.Entry<String, String> entry : config.entrySet()) {
        textEncryption
            .addElement(N_PARAM)
            .addAttribute(A_NAME, entry.getKey())
    ...
    if (m_publishManager != null) {
      ...
      pubHistElement
          .addElement(N_HISTORYSIZE)
          .setText(String.valueOf(m_publishManager.getPublishHistorySize()));
      // optional nodes for publish queue
      pubHistElement
          .addElement(N_QUEUEPERSISTANCE)
          .setText(String.valueOf(m_publishManager.isPublishQueuePersistanceEnabled()));
      pubHistElement
          .addElement(N_QUEUESHUTDOWNTIME)
          .setText(String.valueOf(m_publishManager.getPublishQueueShutdowntime()));
      pubHistElement
          .addElement(N_AUTO_CLEANUP_HISTORY_ENTRIES)
          .setText(String.valueOf(m_publishManager.isAutoCleanupHistoryEntries()));
    }
    ...
    CmsServletContainerSettings servletContainerSettings =
        OpenCms.getSystemInfo().getServletContainerSettings();
    if (!servletContainerSettings.getMode().isNone()) {
      ...
      servletContainerSettingsElem.addAttribute(
          A_MODE, servletContainerSettings.getMode().getMode());
      if (!servletContainerSettings.getMode().isAuto()) {
        servletContainerSettingsElem
            .addElement(N_PREVENTRESPONSEFLUSH)
            .addText("" + servletContainerSettings.isPreventResponseFlush());
        servletContainerSettingsElem
            .addElement(N_RELEASETAGSAFTEREND)
            .addText("" + servletContainerSettings.isReleaseTagsAfterEnd());
      }
      // always write back the error page attribute
      if (servletContainerSettings.getRequestErrorPageAttribute() != null) {
        servletContainerSettingsElem
            .addElement(N_REQUESTERRORPAGEATTRIBUTE)
            .addText(servletContainerSettings.getRequestErrorPageAttribute());
      }
    }

    // ADE settings
    if ((getAdeConfiguration() != null)
        || (getAdeCacheSettings() != null)
        || !m_adeParameters.isEmpty()) {
      ...
      if (getAdeCacheSettings() != null) {
        ...
        cntPageCacheElem.addAttribute(
            A_OFFLINE, "" + getAdeCacheSettings().getContainerPageOfflineSize());
        cntPageCacheElem.addAttribute(
            A_ONLINE, "" + getAdeCacheSettings().getContainerPageOnlineSize());
        ...
        groupContainerCacheElem.addAttribute(
            A_OFFLINE, "" + getAdeCacheSettings().getGroupContainerOfflineSize());
        groupContainerCacheElem.addAttribute(
            A_ONLINE, "" + getAdeCacheSettings().getGroupContainerOnlineSize());
      }
    }
    ...
    if (getSubscriptionManager() != null) {
      ...
      subscrManElem.addAttribute(
          A_MAXVISITED, String.valueOf(getSubscriptionManager().getMaxVisitedCount()));
    }
    ...
    if (m_publishListRemoveMode != null) {
      systemElement
          .addElement(N_PUBLISH_LIST_REMOVE_MODE)
          .addAttribute(A_MODE, m_publishListRemoveMode);
    }

    if (m_detailPageHandler != null) {
      Element handlerElement =
          systemElement
              .addElement(N_DETAIL_PAGE_HANDLER)
              .addAttribute(A_CLASS, m_detailPageHandler.getClass().getName());
    ...
    if (m_shellServerOptions != null) {
      systemElement
          .addElement(N_SHELL_SERVER)
          .addAttribute(A_ENABLED, "" + m_shellServerOptions.isEnabled())
          .addAttribute(A_PORT, "" + m_shellServerOptions.getPort());
    }
  ...
  /**
   * Gets the ADE configuration parameters.
   *
   * <p>
   *
  ...
  public I_CmsAuthorizationHandler getAuthorizationHandler() {
    ...
    try {
      I_CmsAuthorizationHandler authorizationHandler =
          (I_CmsAuthorizationHandler) Class.forName(m_authorizationHandler).newInstance();
      if (LOG.isInfoEnabled()) {
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_AUTHORIZATION_HANDLER_CLASS_SUCCESS_1, m_authorizationHandler));
      }
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_AUTHORIZATION_HANDLER_CLASS_INVALID_1, m_authorizationHandler),
          t);
      return new CmsDefaultAuthorizationHandler();
    }
  }
  ...
  /**
   * Gets the configured credentials resolver class name (null if no class is explicity configured).
   *
   * <p>
   *
   ...
   *
   * <p>If the version history is disabled, this setting has no effect.
   *
   * <p>
   *
  ...
  /**
   * Returns the maximum number of versions that are kept in the VFS version history for deleted
   * resources.
   *
   * <p>If the version history is disabled, this setting has no effect.
   *
  ...
  /**
   * Returns the duration after which responsibles will be notified about out-dated content (in
   * days).
   *
   * <p>
   *
  ...
  /**
   * Returns the configured publish list remove mode, or a default value if there is no configured
   * value or an erroneous configured value.
   *
   * <p>
   *
  ...
  public CmsPublishManager.PublishListRemoveMode getPublishListRemoveMode() {
    ...
    try {
      // trim preserves null
      return CmsPublishManager.PublishListRemoveMode.valueOf(
          StringUtils.trim(m_publishListRemoveMode));
    } catch (Exception e) {
  ...
  /**
   * Returns the configured publish list remove mode as a string, or null if no publish list remove
   ...
   */
  public List<@RUntainted I_CmsRequestHandler> getRequestHandlers() {
  ...
  public I_CmsSessionStorageProvider getSessionStorageProvider() {
    ...
    try {
      I_CmsSessionStorageProvider sessionCacheProvider =
          (I_CmsSessionStorageProvider) Class.forName(m_sessionStorageProvider).newInstance();
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_SESSION_STORAGEPROVIDER_SUCCESS_1, m_sessionStorageProvider));
      }
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_INIT_SESSION_STORAGEPROVIDER_FAILURE_1, m_sessionStorageProvider),
          t);
      return new CmsDefaultSessionStorageProvider();
    }
  }
   ...
   * <p>
   *
   * @param useDefault if true, and no user session mode was configured, this will return the
   *     default value
  ...
  public I_CmsValidationHandler getValidationHandler() {
    ...
    try {
      I_CmsValidationHandler validationHandler =
          (I_CmsValidationHandler) Class.forName(m_validationHandler).newInstance();
      if (LOG.isInfoEnabled()) {
        LOG.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_VALIDATION_HANDLER_CLASS_SUCCESS_1, m_validationHandler));
      }
      ...
      LOG.error(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_VALIDATION_HANDLER_CLASS_INVALID_1, m_validationHandler),
          t);
      return new CmsDefaultValidationHandler();
    }
  }
  ...
  /**
   * Returns true if detail contents are restricted to detail pages from the same site.
   *
   * <p>
   *
  ...
  public boolean isRestrictDetailContents() {
    ...
    return (m_restrictDetailContents == null)
        || Boolean.parseBoolean(m_restrictDetailContents.trim());
  }
  ...
  public void setCmsDefaultUsers(
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_ADMIN_USER_1, m_cmsDefaultUsers.getUserAdmin()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_GUEST_USER_1, m_cmsDefaultUsers.getUserGuest()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_EXPORT_USER_1, m_cmsDefaultUsers.getUserExport()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(
      ...
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_ADMIN_GROUP_1, m_cmsDefaultUsers.getGroupAdministrators()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_USERS_GROUP_1, m_cmsDefaultUsers.getGroupUsers()));
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_GUESTS_GROUP_1, m_cmsDefaultUsers.getGroupGuests()));
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_DEFAULT_USER_NAMES_INITIALIZED_0));
    }
  }
   ...
   */
  public void setCmsMemoryMonitorConfiguration(
      CmsMemoryMonitorConfiguration cmsMemoryMonitorConfiguration) {

    m_cmsMemoryMonitorConfiguration = cmsMemoryMonitorConfiguration;
  }

  /**
   * Sets the credentials resolver class.
   *
   * <p>
   *
   ...
   */
  public void setHistorySettings(
      @RUntainted String historyEnabled, @RUntainted String historyVersions, @RUntainted String historyVersionsAfterDeletion) {
   ...
   */
  public void setHttpAuthenticationSettings(
   ...
   * @param enableSecurityStr flag to determine if the security option should be enabled on the
   *     login dialog
   * @param tokenLifetime the token lifetime
   * @param maxInactive maximum time since last login before CmsLockInactiveAccountsJob locks an
   *     account
   ...
   * <p>
   *
   * @param notificationProject the project in which timestamps for the content notification are
   *     read
   */
  public void setNotificationProject(@RUntainted String notificationProject) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_NOTIFICATION_PROJECT_1, m_notificationProject));
    }
  }
   ...
   * <p>
   *
   * @param notificationTime the duration after which responsibles will be notified about out-dated
   *     content
   */
  public void setNotificationTime(@RUntainted String notificationTime) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_NOTIFICATION_TIME_1, m_notificationTime));
    }
  }
   ...
   */
  public void setPasswordHandler(@RUntainted I_CmsPasswordHandler passwordHandler) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_PWD_HANDLER_SUCCESS_1, passwordHandler.getClass().getName()));
    }
  }
  ...
  public void setPreventResponseFlush(String configValue) {
    ...
    OpenCms.getSystemInfo()
        ...
        .setPreventResponseFlush(Boolean.valueOf(configValue).booleanValue());
  }
  ...
  public void setReleaseTagsAfterEnd(String configValue) {
    ...
    OpenCms.getSystemInfo()
        .getServletContainerSettings()
        .setReleaseTagsAfterEnd(Boolean.valueOf(configValue).booleanValue());
  }
  ...
  /**
   * Sets the 'restrict detail contents' option.
   *
   * <p>
   *
    ...
    if (objectInstance instanceof I_CmsDbContextFactory) {
      ...
      if (CmsLog.INIT.isInfoEnabled()) {
        CmsLog.INIT.info(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_RUNTIME_INFO_FACTORY_SUCCESS_1, className));
      }
    } else {
      if (CmsLog.INIT.isFatalEnabled()) {
        CmsLog.INIT.fatal(
            Messages.get()
                .getBundle()
                .key(Messages.INIT_RUNTIME_INFO_FACTORY_FAILURE_1, className));
      }
    }
  }
  ...
  /**
   * Sets the shell server options from the confriguration.
   *
   * <p>
   *
  ...
  public void setTempFileProjectId(@RUntainted String tempFileProjectId) {
    ...
    if (CmsLog.INIT.isInfoEnabled()) {
      CmsLog.INIT.info(
          Messages.get()
              .getBundle()
              .key(Messages.INIT_TEMPFILE_PROJECT_ID_1, new Integer(m_tempFileProjectId)));
    }
  }
  ...
  /**
   * Sets if the SAX parser implementation classes should be stored in system properties to improve
   * the unmarshalling performance.
   *
   * <p>
   *
   * @param enabled <code>true</code> to store SAX parser implementation classes in system
   *     properties
   */
  ...
  /**
   * Returns whether the SAX parser implementation classes should be stored in system properties to
   * improve the unmarshalling performance.
   *
   * <p>
   *
   * @return <code>true</code> if the SAX parser implementation classes should be stored in system
   *     properties
   */
  ...
  protected void addApiAuthorization(ApiAuthorizationConfig config) {
    ...
    try {
      ...
      I_CmsApiAuthorizationHandler handler =
          (I_CmsApiAuthorizationHandler) (cls.getDeclaredConstructor().newInstance());
