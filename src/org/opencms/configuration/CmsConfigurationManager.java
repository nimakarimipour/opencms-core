 ...
 *
 * <p>Reads the individual configuration class nodes first and creaes new instances of the "base"
...
public class CmsConfigurationManager implements I_CmsXmlConfiguration {
  ...
  public CmsConfigurationManager(String baseFolder) {
    ...
    if (!m_baseFolder.exists()) {
      if (LOG.isErrorEnabled()) {
        LOG.error(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_INVALID_CONFIG_BASE_FOLDER_1, m_baseFolder.getAbsolutePath()));
      }
    }
    ...
    if (!m_backupFolder.exists()) {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            Messages.get()
                .getBundle()
                .key(Messages.LOG_CREATE_CONFIG_BKP_FOLDER_1, m_backupFolder.getAbsolutePath()));
      }
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_CONFIG_BASE_FOLDER_1, m_baseFolder.getAbsolutePath()));
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_CONFIG_BKP_FOLDER_1, m_backupFolder.getAbsolutePath()));
    }
  ...
  /**
   * Returns the properties read from <code>opencms.properties</code>.
   *
   * <p>
   *
  ...
  /**
   * Sets the configuration read from the <code>opencms.properties</code>.
   *
   * <p>
   *
  ...
  public void writeConfiguration(Class<?> clazz) throws IOException, CmsConfigurationException {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          Messages.get()
              .getBundle()
              .key(Messages.LOG_WRITE_CONFIG_XMLFILE_1, file.getAbsolutePath()));
    }
  ...
  /**
   * Gets the path to the XSLT transformation file that should be used for the configuration.
   *
   * <p>
   *
  ...
  /**
   * Checks if an XSLT transformation file is available.
   *
   * <p>
   *
  ...
  boolean hasTransformation() {
    ...
    if (result) {
      CmsLog.INIT.info(
          Messages.get().getBundle().key(Messages.INIT_XSLT_CONFIG_ENABLED_1, transformationPath));
    } else {
  ...
  /**
   * Transforms the given configuration using an XSLT transformation.
   *
   * <p>
   *
  ...
  InputSource transformConfiguration(URL url, I_CmsXmlConfiguration config)
    ...
    try {
      ...
      // We can't set the doctype dynamically from inside the XSLT transform using XSLT 1.0, and
      // XSLT 2.0
      // isn't supported by the standard implementation in the JDK. So we do some macro replacement
      // after the
      // transformation.
      String transformedConfigStr =
          new String(transformedConfig, "UTF-8")
              .replaceFirst("@dtd@", config.getDtdUrlPrefix() + config.getDtdFilename());
      if (LOG.isDebugEnabled()) {
        LOG.debug("");
        LOG.debug(
            "=================== Transformation result for config file '"
                + config.getXmlFileName()
                + "':");
  ...
  /**
   * Creates a backup of the given XML configurations input file.
   *
   * <p>
   *
  ...
  private void backupXmlConfiguration(I_CmsXmlConfiguration configuration) {
    ...
    try {
      ...
      LOG.error(
          Messages.get().getBundle().key(Messages.LOG_CREATE_CONFIG_BKP_FAILURE_1, toName), e);
    }
  }
  ...
  /**
   * Loads the OpenCms configuration from the given XML URL.
   *
   * <p>
   *
  ...
  private void loadXmlConfiguration(URL url, I_CmsXmlConfiguration configuration)
    ...
    // TODO: For this to work with transformed configurations, we need to add the correct DOCTYPE
    // declarations to the transformed files
    ...
    if (hasTransformation) {
      try {
        ...
        LOG.error(
            "Error transforming " + configuration.getXmlFileName() + ": " + e.getLocalizedMessage(),
            e);
      }
    }
    if (inputSource == null) {
  ...
  /**
   * Removes all backups that are older then the given number of days.
   *
   * <p>
   *
      ...
      if ((lastMod < maxAge)
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              Messages.get()
                  .getBundle()
                  .key(Messages.LOG_REMOVE_CONFIG_FILE_1, file.getAbsolutePath()));
        }
      }
    }
  }
