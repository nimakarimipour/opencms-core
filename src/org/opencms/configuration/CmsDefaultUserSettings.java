...
public class CmsDefaultUserSettings extends CmsUserSettings {
  ...
  public static final class CmsPublishRelatedResourcesMode extends A_CmsModeStringEnumeration {
    ...
    protected static final CmsPublishRelatedResourcesMode MODE_FALSE =
        new CmsPublishRelatedResourcesMode(CmsStringUtil.FALSE);

    /**
     * Constant for the publish related resources mode, only {@link
     * org.opencms.security.CmsRole#VFS_MANAGER}s may publish resources without publishing the
     * related resources.
     */
    protected static final CmsPublishRelatedResourcesMode MODE_FORCE =
        new CmsPublishRelatedResourcesMode("FORCE");

    /** Constant for the publish related resources mode, checkbox enabled by default. */
    protected static final CmsPublishRelatedResourcesMode MODE_TRUE =
        new CmsPublishRelatedResourcesMode(CmsStringUtil.TRUE);
    ...
    /**
     * Returns the parsed mode object if the string representation matches, or <code>null</code> if
     * not.
     *
     * <p>
     *
  ...
  public enum SubsitemapCreationMode {
    ...
    /**
     * In this mode, new subsitemap folders are created, giving the user a choice of which folder
  ...
  /** Constant for the publish related resources mode, checkbox disabled by default. */
  public static final CmsPublishRelatedResourcesMode PUBLISH_RELATED_RESOURCES_MODE_FALSE =
      CmsPublishRelatedResourcesMode.MODE_FALSE;

  /**
   * Constant for the publish related resources mode, only {@link
   * org.opencms.security.CmsRole#VFS_MANAGER}s may publish resources without publishing the related
   * resources.
   */
  public static final CmsPublishRelatedResourcesMode PUBLISH_RELATED_RESOURCES_MODE_FORCE =
      CmsPublishRelatedResourcesMode.MODE_FORCE;

  /** Constant for the publish related resources mode, checkbox enabled by default. */
  public static final CmsPublishRelatedResourcesMode PUBLISH_RELATED_RESOURCES_MODE_TRUE =
      CmsPublishRelatedResourcesMode.MODE_TRUE;
  ...
  public static final List<String> BUTTON_STYLES_LIST =
      Collections.unmodifiableList(Arrays.asList(new String[] {"image", "textimage", "text"}));
  ...
  private LinkedHashMap<String, I_CmsPreference> m_preferences =
      new LinkedHashMap<String, I_CmsPreference>();
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file creation date should be shown, otherwise <code>"false"
   *     </code>
   */
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file date expired by should be shown, otherwise <code>
   *     "false"</code>
   */
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file last modified date should be shown, otherwise <code>
   *     "false"</code>
   */
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file date released by should be shown, otherwise <code>
   *     "false"</code>
   */
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file locked by should be shown, otherwise <code>"false"
   *     </code>
   */
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file permissions should be shown, otherwise <code>"false"
   *     </code>
   */
   ...
   * <p>
   *
   * @return <code>"true"</code> if the file last modified by should be shown, otherwise <code>
   *     "false"</code>
   */
  ...
  /**
   * Gets the subsitemap creation mode, or returns a default value given as a parameter if the mode
   * is not set.
   *
   * <p>
   *
   ...
   *
   * <p>Note that this method should only be called once the resource types have been initialized,
   * but after addPreference has been called for all configured preferences.
   *
  ...
  public void initPreferences(CmsWorkplaceManager wpManager) {
    ...
    if (wpManager.getWorkplaceEditorManager() != null) {
      for (String resType :
          wpManager.getWorkplaceEditorManager().getConfigurableEditors().keySet()) {
    ...
    for (Map.Entry<String, String> editorSettingEntry : editorValues.entrySet()) {
      CmsEditorPreference pref =
          new CmsEditorPreference(editorSettingEntry.getKey(), editorSettingEntry.getValue());
      m_preferences.put(pref.getName(), pref);
    }
    ...
    for (Map.Entry<String, String> galleryEntry : galleryValues.entrySet()) {
      CmsStartGallleryPreference pref =
          new CmsStartGallleryPreference(galleryEntry.getKey(), galleryEntry.getValue());
      m_preferences.put(pref.getName(), pref);
    }
    ...
    for (CmsPreferenceData prefData : m_preferenceData) {
      ...
      if (m_preferences.containsKey(name)) {
        // we first remove the existing preference, because in a LinkedHashMap, put(key, value) will
        // not
   ...
   * <p>
   *
   * @return <code>true</code> if the deletion of relation targets is enabled, otherwise <code>false
   *     </code>
   */
   ...
   * <p>
   *
   * @param allowBrokenRelations <code>true</code> if relation deletion should be enabled, otherwise
   *     <code>false</code>
   */
   ...
   * <p>
   *
   * @param dialogExpandInheritedPermissions the default setting for expanding inherited permissions
   *     in the dialog
   */
  public void setDialogExpandInheritedPermissions(String dialogExpandInheritedPermissions) {
    ...
    setDialogExpandInheritedPermissions(
        Boolean.valueOf(dialogExpandInheritedPermissions).booleanValue());
  }
   ...
   * <p>
   *
   * @param dialogExpandUserPermissions the default setting for expanding the users permissions in
   *     the dialog
   */
  ...
  public void setDialogPermissionsInheritOnFolder(String dialogPermissionsInheritOnFolder) {
    ...
    setDialogPermissionsInheritOnFolder(
        Boolean.valueOf(dialogPermissionsInheritOnFolder).booleanValue());
  }
   ...
   *
   * <p>The given <code>String</code> value is interpreted as a {@link Boolean} by the means of
   * <code>{@link Boolean#valueOf(String)}</code>.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @param booleanValue a <code>String</code> that is interpreted as a <code> {@link Boolean}
   *     </code> by the means of <code>{@link Boolean#valueOf(String)}</code>
   */
  ...
  public void setPublishRelatedResourcesMode(String publishRelatedResourcesMode) {
    ...
    m_publishRelatedResourcesMode =
        CmsPublishRelatedResourcesMode.valueOf(publishRelatedResourcesMode);
   ...
   * <p>
   *
   * @param flag <code>"true"</code> or <code>"false"</code> to flag the use of the file upload
   *     button
   */
  ...
  /**
   * Digester support method for configuration if the resource type selection checkbox should show
   * up when uploading a new file in non-applet mode.
   *
   * <p>The given <code>String</code> value is interpreted as a {@link Boolean} by the means of
   * <code>{@link Boolean#valueOf(String)}</code>.
   *
