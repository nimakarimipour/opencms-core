 ...
 *
 * <p>This class handles warnings differently from the default implementation, since we don't need
 * publish permissions for the release workflow, but need to check whether resources are already in
 * a workflow.
 *
 * <p>
 */
public class CmsExtendedPublishResourceFormatter extends CmsDefaultPublishResourceFormatter {
  ...
  /**
   * Rule which checks whether a resource is already in a workflow.
   *
   * <p>
   */
  class ExcludeAlreadyInWorkflow implements I_PublishResourceValidator {
    ...
    public Set<CmsResource> findInvalidResources(Set<CmsResource> input) {
      ...
      for (CmsResource resource : input) {
        ...
        if (isWorkflowProject(projectId)
            && ((optionsProject == null) || !projectId.equals(optionsProject))) {
          result.add(resource);
        }
      }
      return result;
    ...
    public CmsPublishResourceInfo getInfoForResource(CmsResource resource) {
      ...
      return new CmsPublishResourceInfo(
          getMessage(Messages.GUI_ALREADY_IN_WORKFLOW_0), CmsPublishResourceInfo.Type.WORKFLOW);
    }
  }
