...
public class CmsExtendedWorkflowManager extends CmsDefaultWorkflowManager {
  ...
  /**
   * @see org.opencms.workflow.CmsDefaultWorkflowManager#createFormatter(org.opencms.file.CmsObject,
  ...
  /**
   * @see org.opencms.workflow.CmsDefaultWorkflowManager#executeAction(org.opencms.file.CmsObject,
   *     org.opencms.ade.publish.shared.CmsWorkflowAction,
   *     org.opencms.ade.publish.shared.CmsPublishOptions, java.util.List)
   */
  @Override
  ...
  /**
   * @see
   *     org.opencms.workflow.CmsDefaultWorkflowManager#getRealOrVirtualProject(org.opencms.util.CmsUUID)
   */
  @Override
  ...
  /**
   * Gets the name of the group which should be used as the 'manager' group for newly created
   * workflow projects.
   *
   * <p>
   *
  ...
  public String getWorkflowProjectManagerGroup() {
    ...
    return getParameter(
        PARAM_WORKFLOW_PROJECT_MANAGER_GROUP, OpenCms.getDefaultUsers().getGroupAdministrators());
  }

  /**
   * Gets the name of the group which should be used as the 'user' group for newly created workflow
   * projects.
   *
   * <p>
   *
  ...
  public String getWorkflowProjectUserGroup() {
    ...
    return getParameter(
        PARAM_WORKFLOW_PROJECT_USER_GROUP, OpenCms.getDefaultUsers().getGroupAdministrators());
  }

  /**
   * @see
   *     org.opencms.workflow.CmsDefaultWorkflowManager#getWorkflowResources(org.opencms.file.CmsObject,
   *     org.opencms.ade.publish.shared.CmsWorkflow,
   *     org.opencms.ade.publish.shared.CmsPublishOptions, boolean, boolean)
   */
  @Override
    ...
    if (WORKFLOW_RELEASE.equals(workflowKey)) {
      ...
      if (canOverrideWorkflow && !workflowResourcesBean.isTooMany()) {
        ...
        for (CmsResource permCheckResource : result) {
          try {
            boolean canPublish =
                cms.hasPermissions(permCheckResource, CmsPermissionSet.ACCESS_DIRECT_PUBLISH);
            if (canPublish) {
              override = true;
            }
          } catch (Exception e) {
          ...
          if (override) {
            CmsWorkflowResources overrideWorkflowResources =
      ...
      CmsWorkflowResources realResult =
          new CmsWorkflowResources(result, getWorkflows(cms).get(overrideId), tooManyCount);
      return realResult;
    } else {
      CmsWorkflowResources realResult =
          super.getWorkflowResources(cms, workflow, options, canOverrideWorkflow, ignoreLimit);
    ...
    OpenCms.getPublishManager()
        .addPublishListener(
            new CmsPublishEventAdapter() {
              ...
              /**
               * @see
               *     org.opencms.publish.CmsPublishEventAdapter#onStart(org.opencms.publish.CmsPublishJobEnqueued)
               */
              @Override
  ...
  /**
   * Checks that the parent folders of new resources which are released are either not new or are
   * also released.
   *
   * <p>
   *
   ...
   */
  protected void checkNewParentsInList(CmsObject userCms, List<CmsResource> resources)
      throws CmsException {
  ...
  /**
   * Removes a project if there are no longer any resources which have been last modified in that
   * project.
   *
   * <p>
   *
  ...
  protected void cleanupProjectIfEmpty(CmsProject project) throws CmsException {
    ...
    if ((project.getType().getMode() == CmsProject.PROJECT_TYPE_WORKFLOW.getMode())
      ...
      LOG.info(
          "Removing project " + project.getName() + " because it is an empty workflow project.");
      m_adminCms.deleteProject(project.getUuid());
    }
  }
  ...
  protected String generateProjectDescription(CmsObject userCms) {
    ...
    String result =
        Messages.get()
            .getBundle(locale)
            .key(Messages.GUI_WORKFLOW_PROJECT_DESCRIPTION_2, user.getName(), dateString);
    return result;
  }
  ...
  protected String generateProjectName(CmsObject userCms, boolean shortTime) {
    ...
    String result =
        Messages.get()
            .getBundle(locale)
            .key(Messages.GUI_WORKFLOW_PROJECT_NAME_2, username, dateStr);
  ...
  /**
   * Helper method for generating the workflow response which should be sent when publishing the
   * resources would break relations.
   *
   * <p>
   *
  ...
  protected CmsWorkflowResponse getPublishBrokenRelationsResponse(
      CmsObject userCms, List<CmsPublishResource> publishResources) {

    List<CmsWorkflowAction> actions = new ArrayList<CmsWorkflowAction>();
    String forcePublishLabel =
        Messages.get()
            .getBundle(getLocale(userCms))
            .key(Messages.GUI_WORKFLOW_ACTION_FORCE_PUBLISH_0);

    CmsWorkflowAction forcePublish =
        new CmsWorkflowAction(ACTION_FORCE_PUBLISH, forcePublishLabel, true, true);
  ...
  /**
   * Gets the workflow response which should be sent when the resources have successfully been
   * published.
   *
   * <p>
   *
  ...
  protected CmsWorkflowResponse getSuccessResponse() {
    ...
    return new CmsWorkflowResponse(
        true, "", new ArrayList<CmsPublishResource>(), new ArrayList<CmsWorkflowAction>(), null);
  }
  ...
  protected boolean isProjectEmpty(CmsProject project) throws CmsException {
    ...
    List<CmsResource> resources =
        m_adminCms.readProjectView(project.getUuid(), CmsResourceState.STATE_KEEP);
  ...
  protected boolean isProjectManager(CmsObject userCms) throws CmsException {
    ...
    List<CmsGroup> groups =
        m_adminCms.getGroupsOfUser(userCms.getRequestContext().getCurrentUser().getName(), false);
    return groups.contains(managerGroup);
  }

  /**
   * Handles finished publish jobs by removing projects of resources in the publish job if they are
          ...
          OpenCms.getLinkManager()
              .getServerLink(
                  ...
                  "/system/workplace/commons/publish.jsp?"
                      ...
                      + "=true");
      CmsWorkflowNotification notification =
          new CmsWorkflowNotification(
              m_adminCms,
              userCms,
              recipient,
