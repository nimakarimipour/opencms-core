...
public class CmsDefaultPublishResourceFormatter implements I_CmsPublishResourceFormatter {
  ...
  public class AlreadyPublishedValidator implements I_PublishResourceValidator {
    ...
    /**
     * @see
     *     org.opencms.workflow.CmsDefaultPublishResourceFormatter.I_PublishResourceValidator#findInvalidResources(java.util.Set)
     */
    ...
    /**
     * @see
     *     org.opencms.workflow.CmsDefaultPublishResourceFormatter.I_PublishResourceValidator#getInfoForResource(org.opencms.file.CmsResource)
     */
    public CmsPublishResourceInfo getInfoForResource(CmsResource resource) throws CmsException {
      ...
      info =
          Messages.get()
              .getBundle(getLocale())
              .key(Messages.GUI_RESOURCE_PUBLISHED_BY_2, publishUser, publishDate);
  ...
  public class BlockingLockedValidator implements I_PublishResourceValidator {
    ...
    public Set<CmsResource> findInvalidResources(Set<CmsResource> resources) {
      ...
      for (CmsResource resource : resources) {
        try {
          List<CmsResource> blockingLocked =
              m_cms.getLockedResourcesWithCache(resource, blockingFilter, m_lockedResourceCache);
  ...
  /**
   * Validator which can exclude some resources from publishing and supplies a status object for the
   * excluded resources.
   *
   * <p>
   */
  ...
  public class NoPermissionsValidator implements I_PublishResourceValidator {
    ...
    public Set<CmsResource> findInvalidResources(Set<CmsResource> resources) {
      ...
      try {
        for (CmsProject project :
            OpenCms.getOrgUnitManager().getAllManageableProjects(m_cms, "", true)) {
    ...
    public CmsPublishResourceInfo getInfoForResource(CmsResource resource) {
      ...
      CmsPublishResourceInfo infoObj;
      info =
          Messages.get().getBundle(getLocale()).key(Messages.GUI_RESOURCE_NOT_ENOUGH_PERMISSIONS_0);
    ...
    public boolean apply(CmsResource input) {
      ...
      try {
        return m_cms.hasPermissions(
            input, CmsPermissionSet.ACCESS_DIRECT_PUBLISH, false, CmsResourceFilter.ALL);
      } catch (CmsException e) {
        LOG.error(e.getLocalizedMessage(), e);
        return true;
      }
  ...
  /**
   * @see
   *     org.opencms.workflow.I_CmsPublishResourceFormatter#initialize(org.opencms.ade.publish.shared.CmsPublishOptions,
  ...
  protected CmsPublishResource createPublishResource(CmsResource resource) throws CmsException {
    ...
    CmsPermissionInfo permissionInfo =
        OpenCms.getADEManager().getPermissionInfo(m_cms, resource, null);
    ...
    CmsPublishResource pubResource =
        new CmsPublishResource(
   ...
   *
   * <p>This can be used to remove resources which shouldn't be displayed.
   *
   * <p>
   *
