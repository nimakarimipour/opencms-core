 ...
 *
 * <p>For more info about xpath see: <br>
 *
 * <ul>
 *   <li>http://www.w3.org/TR/xpath.html
 *   <li>http://www.zvon.org/xxl/XPathTutorial/General/examples.html
 * </ul>
 *
 * <p>
 *
...
public class CmsSetupXmlHelper {
  ...
  private static final EntityResolver NO_ENTITY_RESOLVER =
      new EntityResolver() {
  ...
  public static String format(String xml) throws CmsXmlException {
    ...
    return CmsXmlUtils.marshal(
        (Node) CmsXmlUtils.unmarshalHelper(xml, null), CmsEncoder.ENCODING_UTF_8);
  }
   ...
   * @return the value in the given xpath of the given xml file, or <code>null</code> if no matching
   *     node
   */
  ...
  public static boolean setAttribute(
      Document document, String xPath, String attribute, String value) {
   ...
   *
   * <p>If value is <code>null</code>, all nodes identified by the given xpath will be deleted.
   *
   * <p>If the node identified by the given xpath does not exists, the missing nodes will be created
  ...
  public static int setValue(Document document, String xPath, String value, String nodeToInsert) {
    ...
    Iterator<Node> itNodes =
        CmsCollectionsGenericWrapper.<Node>list(document.selectNodes(xPath)).iterator();
   ...
   *
   * <p>It can be new read or come from the document cache.
   *
   * <p>
   *
      ...
      try {
        document =
            CmsXmlUtils.unmarshalHelper(
                new InputSource(new FileReader(getFile(xmlFilename))), NO_ENTITY_RESOLVER);
        ...
        throw new CmsXmlException(
            Messages.get().container(Messages.ERR_XML_COULD_NOT_PARSE_FILE_1, xmlFilename), e);
      }
      // cache the doc
   ...
   * @throws CmsXmlException if something goes wrong
   */
  public int setValue(String xmlFilename, String xPath, String value, String nodeToInsert)
      throws CmsXmlException {
    ...
    if (document != null) {
      try {
        CmsXmlUtils.validateXmlStructure(
            document, CmsEncoder.ENCODING_UTF_8, new CmsXmlEntityResolver(null));
        ...
        // write invalid config files to the file system with a prefix of "invalid-" so they can be
