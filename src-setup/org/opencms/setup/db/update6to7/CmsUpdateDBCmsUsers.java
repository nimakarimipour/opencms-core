...
public class CmsUpdateDBCmsUsers extends A_CmsUpdateDBPart {
  ...
  /**
   * Removes the columns USER_INFO, USER_ADDRESS, USER_DESCRIPTION and USER_TYPE from the CMS_USERS
   * table.
   *
   * <p>
   *
  ...
  protected void removeUnnecessaryColumns(CmsSetupDb dbCon) throws SQLException {
    ...
    if (dbCon.hasTableOrColumn(CMS_USERS_TABLE, USER_DESCRIPTION)) {
      ...
      System.out.println(
          "no column " + USER_DESCRIPTION + " in table " + CMS_USERS_TABLE + " found");
    }
    if (dbCon.hasTableOrColumn(CMS_USERS_TABLE, USER_TYPE)) {
   ...
   */
  protected void writeAdditionalUserInfo(
      CmsSetupDb dbCon, String id, Map<String, Object> additionalInfo) {
