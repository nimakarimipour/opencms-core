...
public class CmsSetupTests {
  ...
  /**
   * Returns true if one of the tests found a violated condition. It is assumed that it will be
   * impossible to run OpenCms.
   *
   * <p>
   *
  ...
  /**
   * Returns true if one of the tests found a questionable condition. It is possible that OpenCms
   * will not run.
   *
   * <p>
   *
   ...
   *
   * @param setupBean the CmsSetup bean of the setup wizard
   * @param serverInfo optional server info, if not present the server info is retrieved from the
   *     bean
   */
  public void runTests(CmsSetupBean setupBean, String serverInfo) {
    ...
    if (serverInfo == null) {
      ...
      writeVersionInfo(
          serverInfo, System.getProperty("java.version"), setupBean.getWebAppRfsPath());
    }
  }
