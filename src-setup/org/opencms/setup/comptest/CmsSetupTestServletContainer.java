...
public class CmsSetupTestServletContainer implements I_CmsSetupTest {
  ...
  public CmsSetupTestResult execute(CmsSetupBean setupBean) {
    ...
    String[][] unsupportedContainers = {
      {
        "Tomcat Web Server/3",
        "Tomcat 3.x is no longer supported. Please use at least Tomcat 4.1 instead."
      },
      {
        "Apache Tomcat/4.0",
        "Tomcat 4.0.x is no longer supported. Please use at least Tomcat 4.1 instead."
      },
      {
        "Resin/2",
        "The OpenCms JSP integration does not work with Resin 2.x. Please use Resin 3 instead."
      },
      {
    ...
    int supportedServletContainer =
        hasSupportedServletContainer(servletContainer, supportedContainers);
    int unsupportedServletContainer =
        unsupportedServletContainer(servletContainer, unsupportedContainers);
  ...
  /**
   * Checks if the used servlet container is part of the servlet containers OpenCms does NOT
   * support.
   *
   * <p>
   *
