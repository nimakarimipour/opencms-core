...
public class CmsAutoSetup {
  ...
  public CmsAutoSetup(CmsAutoSetupProperties props) {
    ...
    m_bean.init(
        props.getSetupWebappPath(), props.getServeltMapping(), props.getSetupDefaultWebappName());
  }
  ...
  public void run() throws Exception {
    ...
    if (m_bean.getWizardEnabled()) {
      ...
      setupTests.runTests(
          m_bean, "No server info present, because this test is running in auto mode.");
      if (setupTests.isRed()) {
      ...
      if (m_props.isShowProgress()) {
        ...
        System.out.println("\nImporting modules:");
        // System.out will be redirected by the setup bean, so keep a reference for the progress
        // indicator
  ...
  public void setupDB() throws Exception {
    ...
    if (m_bean.isInitialized()) {
      if (m_props.isCreateDb() || m_props.isCreateTables()) {
        ...
        if (!dbExists || m_props.isDropDb()) {
          db.closeConnection();
          if (!m_bean.getDatabase().startsWith("db2")
              && !m_bean.getDatabase().startsWith("as400")) {
            db.setConnection(
                m_bean.getDbDriver(),
                m_bean.getDbCreateConStr(),
    ...
    if (dbExists && m_props.isCreateTables() && !m_props.isDropDb() && (db != null)) {
      throw new Exception(
          "You have selected to not drop existing DBs, but a DB with the given name exists.");
    }
