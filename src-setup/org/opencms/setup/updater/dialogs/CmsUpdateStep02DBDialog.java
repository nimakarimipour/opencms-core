...
public class CmsUpdateStep02DBDialog extends A_CmsUpdateDialog {
  ...
  public boolean init(CmsUpdateUI ui) {
    ...
    try {
      ...
      CmsSetupErrorDialog.showErrorDialog(
          "Database error", "Your database version is not compatible with OpenCms 11.");
      return false;
    }
  }
  ...
  private Panel getDBPoolPanel(CmsUpdateDBManager dbBean, String pool) {
    ...
    layout.addComponent(
        getTableLikeLabel("JDBC Connection Url", dbBean.getDbUrl(pool), widthString));
    layout.addComponent(
        getTableLikeLabel("JDBC Connection Url Params", dbBean.getDbParams(pool), widthString));
  ...
  private Label getTableLikeLabel(String key, String value, String keyWidth) {
    ...
    String html =
        "<div style='display:flex'><div style='width:"
            + keyWidth
            + "'>"
            + key
            + "</div><div>"
