...
public class CmsUploadPropertyDialog {
  ...
  public CmsUploadPropertyDialog() {
    ...
    m_dialogContent.addStyleName(
        org.opencms.ade.postupload.client.ui.css.I_CmsLayoutBundle.INSTANCE
            .dialogCss()
            .propertyDialog());
    try {
      m_dialogData =
          (CmsPostUploadDialogBean)
              CmsRpcPrefetcher.getSerializedObjectFromDictionary(
                  getDialogService(), CmsPostUploadDialogBean.DICT_NAME);
  ...
  /**
   * Returns <code>true</code> if we are currently in the explorer mode.
   *
   * <p>
   *
  ...
  protected void actionAdvanced() {
    ...
    String url =
        CmsCoreProvider.get()
            .link("/system/workplace/commons/property_advanced.jsp?resource=" + resPath);
  ...
  protected I_CmsPostUploadDialogServiceAsync getDialogService() {
    ...
    if (m_dialogService == null) {
      ...
      String serviceUrl =
          CmsCoreProvider.get().link("org.opencms.ade.postupload.CmsPostUploadDialogService.gwt");
  ...
  /**
   * Retrieves the resource information from the server, creates a the dialogs content and puts the
   * content into the mainpanel.
   *
   * <p>
   *
  ...
  protected void loadDialogBean(final CmsUUID uuid) {
    ...
    CmsRpcAction<CmsPostUploadDialogPanelBean> callback =
        new CmsRpcAction<CmsPostUploadDialogPanelBean>() {
    ...
    if (m_dialogData.getResources().size() > 1) {
      if (m_dialogIndex == 0) {
        m_buttonBack.disable(
            org.opencms.ade.postupload.client.Messages.get()
                .key(org.opencms.ade.postupload.client.Messages.GUI_DIALOG_INFO_FIRST_RESOURCE_0));
      } else {
        m_buttonBack.enable();
      }
      if (m_dialogIndex == (m_dialogData.getResources().size() - 1)) {
        m_buttonNext.disable(
            org.opencms.ade.postupload.client.Messages.get()
                .key(org.opencms.ade.postupload.client.Messages.GUI_DIALOG_INFO_LAST_RESOURCE_0));
      } else {
        m_buttonNext.enable();
      }
    }
  ...
  private void createButtons() {
    ...
    if (isExplorerMode() && (m_resources.size() == 1)) {
      ...
      m_buttonAdvanced.addClickHandler(
          new ClickHandler() {
