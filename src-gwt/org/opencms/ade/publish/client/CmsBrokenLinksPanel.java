...
public class CmsBrokenLinksPanel extends Composite {
  ...
  private static final I_CmsBrokenLinksPanelUiBinder UI_BINDER =
      GWT.create(I_CmsBrokenLinksPanelUiBinder.class);
  ...
  public CmsListItemWidget addEntry(CmsPublishResource res) {
    ...
    final CmsListItemWidget itemWidget =
        CmsPublishGroupPanel.createListItemWidget(res, SLOT_MAPPING);
    CmsTreeItem item = new CmsTreeItem(false, itemWidget);
    addContextMenu(item, res);

    item.setOpen(true);
    for (CmsPublishResource subRes : res.getRelated()) {
      final CmsListItemWidget subWidget =
          CmsPublishGroupPanel.createListItemWidget(subRes, SLOT_MAPPING);
  ...
  public void setEntries(
      Collection<CmsPublishResource> resourceBeans, List<CmsWorkflowAction> actions) {
  ...
  /**
   * Adds a context menu button to the resource box, unless the structure id is null (this can
   * happen with already broken relations).
   *
    ...
    if (!res.getId().isNullUUID()) {
      CmsContextMenuButton button =
          new CmsContextMenuButton(
              res.getId(), m_publishDialog.getContextMenuHandler(), AdeContext.resourceinfo);
      CmsPublishGroupPanel.fillButtonSlot(
          item.getListItemWidget(), CmsPublishGroupPanel.SLOT_MENU, button, SLOT_MAPPING);
    } else if (CmsCoreProvider.get().getUserInfo().isWorkplaceUser()) {
      // Null UUID, so resource probably doesn't exist.
      // We can't use the normal context menu handler, since it uses the structure id for loading
          ...
          new CmsContextMenuButton(
              ...
              new CmsContextMenuHandler() {
                ...
                public void loadContextMenu(
                    CmsUUID structureId, AdeContext context, CmsContextMenuButton menuButton) {

                  List<I_CmsContextMenuEntry> menuEntries = new ArrayList<>();
                  I_CmsContextMenuEntry entry =
                      new I_CmsContextMenuEntry() {

                        public void execute() {

                          CmsRpcAction<String> rpcAction =
                              new CmsRpcAction<String>() {
                                ...
                                public void execute() {
                                  ...
                                  CmsCoreProvider.getService()
                                      .getWorkplaceLinkForPath(res.getSubTitle(), this);
                                }
                                ...
                                protected void onResponse(String result) {
                                  ...
                                  if (result != null) {
                                    ...
                                    // In case we already are in the workplace, setting the URL does
                                    // not necessarily update the state.
                                    // Schedule a timer to trigger a reload, which only fires in
                                    // that scenario.
                                    Timer timer =
                                        new Timer() {
