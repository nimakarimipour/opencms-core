 ...
 *
 * <p>This class is mostly responsible for the control flow and RPC calls of the publish dialog. It
 * delegates most of the actual GUI work to the {@link CmsPublishSelectPanel} and {@link
 * CmsBrokenLinksPanel} classes.
 *
 * @since 8.0.0
 */
public class CmsPublishDialog extends CmsPopup {
  ...
  /**
   * The action for publishing and/or removing resources from the publish list.
   *
   * <p>
   */
  ...
  /**
   * The action for loading the publish list.
   *
   * <p>
   */
  ...
  /**
   * The root panel of this dialog which contains both the selection panel and the panel for
  ...
  public static void showPublishDialog(
    ...
    List<Element> collectorInfos =
        CmsDomUtil.getElementsByClass(CmsGwtConstants.CLASS_COLLECTOR_INFO);
    int j = 1;
  ...
  protected static I_CmsPublishServiceAsync getService() {
    ...
    if (SERVICE == null) {
      ...
      String serviceUrl =
          CmsCoreProvider.get().link("org.opencms.ade.publish.CmsPublishService.gwt");
  ...
  public void onCancel() {
    ...
    if (m_publishSelectPanel.m_model != null) {
      final List<CmsUUID> toRemove =
          m_publishSelectPanel.m_model.getIdsOfAlreadyPublishedResources();
      if (toRemove.isEmpty()) {
        hide();
      } else {
        CmsRpcAction<CmsWorkflowResponse> action =
            new CmsRpcAction<CmsWorkflowResponse>() {
      ...
      CmsNotification.get()
          .send(
              ...
              org.opencms.gwt.client.Messages.get()
                  .key(org.opencms.gwt.client.Messages.GUI_DONE_0));

    } else {
      m_failureMessage = brokenResources.getMessage();
      m_state = State.failure;
      m_brokenLinksPanel.setEntries(
          brokenResources.getResources(), brokenResources.getAvailableActions());
  ...
  protected CmsWorkflowActionParams getWorkflowActionParams() {
    ...
    if (m_publishSelectPanel.isShowResources()) {
      List<CmsUUID> resourcesToPublish =
          new ArrayList<CmsUUID>(m_publishSelectPanel.getResourcesToPublish());
      List<CmsUUID> resourcesToRemove =
          new ArrayList<CmsUUID>(m_publishSelectPanel.getResourcesToRemove());
      CmsWorkflowActionParams actionParams =
          new CmsWorkflowActionParams(resourcesToPublish, resourcesToRemove);
      return actionParams;
    } else {
      CmsPublishOptions options = getPublishOptions();
  ...
  /**
   * Ensures all style sheets are loaded.
   *
   * <p>
   */
