...
public class CmsPublishGroupPanel extends Composite {
  ...
  /**
   * The global map of selection controllers of *ALL* groups (to which this group's selection
  ...
  /**
   * The number of loaded publish item widgets for this group (used for scrolling).
   *
   * <p>
   */
  ...
  /**
   * The publish resources of the current group.
   *
   * <p>
   */
   ...
   * @param controllersById the map of selection controllers to which this panel's selection
   ...
   */
  public static CmsListItemWidget createListItemWidget(
      CmsPublishResource resourceBean, int[] slotMapping) {

    CmsListItemWidget itemWidget = new CmsListItemWidget(resourceBean);
    if (resourceBean.getUserLastModified() != null) {
      String userLabel =
          org.opencms.ade.publish.client.Messages.get()
              .key(org.opencms.ade.publish.client.Messages.GUI_LABEL_USER_LAST_MODIFIED_0);
    ...
    if (resourceBean.getDateLastModifiedString() != null) {
      String dateLabel =
          org.opencms.ade.publish.client.Messages.get()
              .key(org.opencms.ade.publish.client.Messages.GUI_LABEL_DATE_LAST_MODIFIED_0);
      itemWidget.addAdditionalInfo(
   ...
   */
  public static void fillButtonSlot(
      CmsListItemWidget listItemWidget, int index, Widget widget, int[] slotMapping) {
  ...
  /**
   * Adds the list item for the next publish resource and returns true on success, while also
   * incrementing the internal item index.
   *
   * <p>
   *
  ...
  protected boolean hasOnlyProblemResources() {
    ...
    return m_model.getGroups().get(m_groupIndex).getResources().size()
        == m_model.countResourcesInGroup(
            new CmsPublishDataModel.HasProblems(),
            m_model.getGroups().get(m_groupIndex).getResources());
  }
  ...
  /**
   * Adds a resource bean to this group.
   *
   * <p>
   *
  ...
  /**
   * Creates a widget from resource bean data.
   *
   * <p>
   *
  ...
  private CmsTreeItem buildItem(
      final CmsPublishResource resourceBean, CmsPublishItemStatus status, boolean isSubItem) {
          ...
          new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
              ...
              CmsContentEditorDialog.get()
                  .openEditDialog(editableData, false, null, new DialogOptions(), m_editorHandler);
            }
          });
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(
          resourceBean.getPermissionInfo().getNoEditReason())) {
    ...
    if (resourceBean.getPermissionInfo().hasViewPermission()) {
      CmsContextMenuButton button =
          new CmsContextMenuButton(resourceBean.getId(), m_contextMenuHandler, AdeContext.publish);
    ...
    if (!isSubItem) {
      ...
      final CmsPublishItemSelectionController controller =
          new CmsPublishItemSelectionController(
              resourceBean.getId(), checkbox, remover, styleVar, hasProblem);
  ...
  /**
   * Initializes the "select all/none" buttons, adds them to the group header and attaches event
   * handlers to them.
   *
   * <p>
   */
  private void initSelectButtons() {
    ...
    m_selectGroup.addValueChangeHandler(
