 ...
 *
 * <p>This class handles the communication between gallery dialog and the server. It contains the
 * gallery data, but no references to the gallery dialog widget.
 *
 * @since 8.0.0
 */
public class CmsGalleryController implements HasValueChangeHandlers<CmsGallerySearchBean> {
  ...
  private static Map<String, I_CmsPreviewFactory> m_previewFactoryRegistration =
      new HashMap<String, I_CmsPreviewFactory>();
  ...
  /**
   * Flag which controls whether the results should be selectable. If this is false, the results
   * will not be selectable, but if it is true, the results may still be unselectable for a
   * different reason.
   */
  ...
  public CmsGalleryController(
    ...
    JavaScriptObject embeddedConfig =
        CmsJsUtil.getAttribute(CmsJsUtil.getWindow(), "embeddedConfiguration");
   ...
   */
  public CmsGalleryController(
      CmsGalleryControllerHandler handler, final I_CmsGalleryConfiguration conf) {
    ...
    CmsRpcAction<CmsGalleryDataBean> initAction =
        new CmsRpcAction<CmsGalleryDataBean>() {
          ...
          protected void onResponse(CmsGalleryDataBean result) {
            ...
            if (m_dialogBean.getStartTab() != GalleryTabId.cms_tab_results) {
              ...
              if (!tabs.contains(m_dialogBean.getStartTab())) {
                if ((m_dialogMode == GalleryMode.widget)
                    && tabs.contains(GalleryTabId.cms_tab_vfstree)) {
  ...
  public static I_CmsGalleryServiceAsync createGalleryService() {
    ...
    String serviceUrl =
        CmsCoreProvider.get().link("org.opencms.ade.galleries.CmsGalleryService.gwt");
   ...
   */
  public static void registerPreviewFactory(
      String previewProviderName, I_CmsPreviewFactory factory) {
  ...
  public HandlerRegistration addValueChangeHandler(
      ValueChangeHandler<CmsGallerySearchBean> handler) {
  ...
  public String getPreselectOption(String siteRoot, List<CmsSiteSelectorOption> options) {
    ...
    for (CmsSiteSelectorOption option : options) {
      if (CmsStringUtil.joinPaths(siteRoot, "/")
          .equals(CmsStringUtil.joinPaths(option.getSiteRoot(), "/"))) {
  ...
  public void getSubEntries(
    ...
    CmsRpcAction<List<CmsSitemapEntryBean>> action =
        new CmsRpcAction<List<CmsSitemapEntryBean>>() {
  ...
  public void getSubFolders(
      final String folder, final AsyncCallback<List<CmsVfsEntryBean>> callback) {

    CmsRpcAction<List<CmsVfsEntryBean>> action =
        new CmsRpcAction<List<CmsVfsEntryBean>>() {
  ...
  /**
   * Gets the tree token, which is used to determine which tree state is loaded/saved for the VFS
   * and sitemap tabs.
   *
   * <p>
   *
  ...
  public boolean isOpenInResults() {
    ...
    if (I_CmsGalleryProviderConstants.GalleryTabId.cms_tab_results
        .name()
        .equals(m_searchObject.getTabId())) {
      return true;
    }
    return false;
  }
  ...
  /**
   * Returns <code>true</code>, if the search object was manipulated by the controller <code>false
   * </code> otherwise.
   *
   * <p>
   *
   ...
   * <p>
   *
   * @return <code>false</code> if any search parameter is selected, <code>true</code> if there are
  ...
  public void loadVfsEntryBean(
    ...
    CmsRpcAction<CmsVfsEntryBean> action =
        new CmsRpcAction<CmsVfsEntryBean>() {
  ...
  public void saveTreeState(
      final String treeName, final String siteRoot, final Set<CmsUUID> openItemIds) {

    CmsRpcAction<Void> treeStateAction =
        new CmsRpcAction<Void>() {
          ...
          public void execute() {
            ...
            start(600, false);
            getGalleryService()
                .saveTreeOpenState(treeName, getTreeToken(), siteRoot, openItemIds, this);
          }
   ...
   * @param nextAction the next action to execute after the search data for the element has been
   *     loaded into the gallery dialog
   */
  public void searchElement(final String path, final Runnable nextAction) {
    ...
    CmsRpcAction<CmsGallerySearchBean> searchAction =
        new CmsRpcAction<CmsGallerySearchBean>() {
  ...
  public void selectResource(
      String resourcePath, CmsUUID structureId, String title, String resourceType) {

    String provider = getProviderName(resourceType);
    if (provider == null) {
      // use {@link org.opencms.ade.galleries.client.preview.CmsBinaryPreviewProvider} as default to
      // select a resource
      provider = I_CmsBinaryPreviewProvider.PREVIEW_NAME;
    }
    if (m_previewFactoryRegistration.containsKey(provider)) {
      m_previewFactoryRegistration
          .get(provider)
          .getPreview(m_handler.m_galleryDialog)
          .selectResource(resourcePath, structureId, title);
  ...
  /**
   * Sets the search object changed flag to <code>true</code>.
   *
   * <p>
   */
  ...
  public void sortCategories(String sortParams, String filter) {
    ...
    switch (sort) {
      case tree:
        m_handler.onUpdateCategoriesTree(
            m_dialogBean.getCategories(), m_searchObject.getCategories());
        break;
      case title_asc:
   ...
   *
   * <p>This will only take effect when executed before tab initialization.
   *
   * <p>
   */
  ...
  /**
   * Does the initial search if not already pre-fetched.
   *
   * <p>
   */
  void initialSearch() {
    ...
    CmsRpcAction<CmsGallerySearchBean> searchAction =
        new CmsRpcAction<CmsGallerySearchBean>() {
          ...
          protected void onResponse(CmsGallerySearchBean result) {
            ...
            stop(false);
            m_searchObject = result;
            m_handler.onInitialSearch(
                m_searchObject, m_dialogBean, CmsGalleryController.this, true);
          }
        };
    searchAction.execute();
  }
  ...
  /**
   * Returns a consistent search object to be used for the search.
   *
   * <p>For the search at least one resource type should be provided. The corresponding resource
   * types will be added to the search object, if no or only gallery folder are selected.
   *
  ...
  CmsGallerySearchBean prepareSearchObject() {
    ...
    if (CmsClientCollectionUtil.isEmptyOrNull(m_searchObject.getTypes())) {
      ...
      if (CmsClientCollectionUtil.isEmptyOrNull(m_searchObject.getGalleries())) {
        // additionally provide all available gallery folders if in 'widget' and 'editor'
        // dialog-mode and no folder has been selected
        ...
        preparedSearchObj.setServerSearchTypes(
            new ArrayList<String>(
                CmsClientCollectionUtil.intersection(availableTypes, galleryTypes)));
      }
    } else {
  ...
  /**
   * Converts categories tree to a list of info beans.
   *
   * <p>
   *
   ...
   */
  private void categoryTreeToList(
      List<CmsCategoryBean> categoryList, List<CmsCategoryTreeEntry> entries) {
  ...
  /**
   * Creates a tree structure from the given gallery folder list.
   *
   * <p>The tree may have several entries at root level.
   *
   * <p>
   *
  ...
  /**
   * Helper method for getting the default (sub)site root for a list of site selector options.
   *
   * <p>
   *
   * @param options the list of options
  ...
  /**
   * Returns the preview provider name for the given resource type, or <code>null</code> if none
   * available.
   *
   * <p>
   *
  ...
  private void loadCategories() {
    ...
    CmsRpcAction<List<CmsCategoryTreeEntry>> action =
        new CmsRpcAction<List<CmsCategoryTreeEntry>>() {
          ...
          public void execute() {
            ...
            CmsCoreProvider.getService()
                .getCategoriesForSitePath(m_dialogBean.getReferenceSitePath(), this);
          }
  ...
  /**
   * Loading all available galleries.
   *
   * <p>
   */
  private void loadGalleries() {

    CmsRpcAction<List<CmsGalleryFolderBean>> action =
        new CmsRpcAction<List<CmsGalleryFolderBean>>() {
  ...
  /**
   * Looks for an ancestor tree entry for the given path.
   *
   * <p>
   *
