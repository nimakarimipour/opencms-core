 ...
 * @since 8.0.
 */
public abstract class A_CmsListTab extends A_CmsTab
    implements ValueChangeHandler<String>, I_CmsTruncable {
  ...
  protected abstract class A_SelectionHandler implements ClickHandler {
    ...
    /**
     * Executed on selection change. Either when the check box was clicked or on double click on a
     * list item.
     *
     * <p>
     */
    ...
    /**
     * This method is called if a list item is selected in a way such that the result tab should be
  ...
  protected static final I_CmsGalleryDialogCss DIALOG_CSS =
      I_CmsLayoutBundle.INSTANCE.galleryDialogCss();
  ...
  /** The option panel. */
  @UiField protected FlowPanel m_options;
  ...
  /** The option panel. */
  @UiField protected FlowPanel m_tab;
  ...
  @UiField FlowPanel m_additionalWidgets;
  ...
  public void onValueChange(ValueChangeEvent<String> event) {
    ...
    if (event.getSource() == m_sortSelectBox) {
      ...
      getTabHandler()
          .onSort(event.getValue(), hasQuickFilter() ? m_quickSearch.getFormValueAsString() : null);
    }
    if ((event.getSource() == m_quickSearch)) {
      if (hasQuickFilter()) {

        if ((CmsStringUtil.isEmptyOrWhitespaceOnly(event.getValue())
            || (event.getValue().length() >= 3))) {
          // only act if filter length is at least 3 characters or empty
          if ((m_lastQuickSearchValue == null)
              || !m_lastQuickSearchValue.equals(event.getValue())) {
   ...
   *
   * <p>The option panel should contain drop down boxes or other list options.
   *
    ...
    uploadButton.addClickHandler(
        new ClickHandler() {
          ...
          public void onClick(ClickEvent event) {
            ...
            customUploadAction(nativeMethod, target);
            /** TODO: Callback */
          }
        });
    return uploadButton;
  ...
  protected CmsPushButton createNewExternalLinkButton(final String parentPath) {
    ...
    final CmsResourceTypeBean typeInfo =
        getTabHandler().getTypeInfo(CmsEditExternalLinkDialog.POINTER_RESOURCE_TYPE_NAME);
    ...
    if (typeInfo != null) {
      ...
      createNewButton.setTitle(
          org.opencms.gwt.client.Messages.get()
              .key(org.opencms.gwt.client.Messages.GUI_CREATE_NEW_LINK_DIALOG_TITLE_0));
      createNewButton.setButtonStyle(ButtonStyle.FONT_ICON, null);
      createNewButton.addClickHandler(
          new ClickHandler() {

            public void onClick(ClickEvent event) {

              CmsEditExternalLinkDialog dialog =
                  CmsEditExternalLinkDialog.showNewLinkDialog(typeInfo, parentPath);
  ...
  protected void quickSearch() {
    ...
    if ((m_quickSearch != null)
        && CmsStringUtil.isNotEmptyOrWhitespaceOnly(m_quickSearch.getFormValueAsString())) {
  ...
  private void createQuickBox() {
    ...
    if (hasQuickSearch() || hasQuickFilter()) {
      ...
      m_searchButton.setImageClass(
          hasQuickFilter() ? I_CmsButton.FILTER : I_CmsButton.SEARCH_SMALL);
      ...
      if (hasQuickFilter()) {
        ...
        m_quickSearch.addKeyPressHandler(
            new KeyPressHandler() {
        ...
        m_searchButton.addClickHandler(
            new ClickHandler() {
