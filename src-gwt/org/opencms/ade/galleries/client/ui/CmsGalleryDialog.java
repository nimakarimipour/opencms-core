...
public class CmsGalleryDialog extends Composite
  ...
  public void fillResultTab(CmsGallerySearchBean searchObj) {
    ...
    if (!searchObj.isEmpty()) {
      ...
      while (it.hasNext()) {
        ...
        if (tab
            != m_resultsTab) { // parameters from the results tab are handled in the fillContent
                               // method
    ...
    for (i = 0; i < tabIds.length; i++) {
      switch (tabIds[i]) {
        ...
        case cms_tab_categories:
          ...
          m_tabbedPanel.add(
              m_categoriesTab, Messages.get().key(Messages.GUI_TAB_TITLE_CATEGORIES_0));
          break;
        case cms_tab_search:
          m_searchTab =
              new CmsSearchTab(
        ...
        case cms_tab_results:
          ...
          m_tabbedPanel.addWithLeftMargin(
              m_resultsTab, Messages.get().key(Messages.GUI_TAB_TITLE_RESULTS_0));
          disableSearchTab();
          break;
        default:
          break;
  ...
  /**
   * Returns true if the formats from this dialog object should be prioritized by the format
   * handler.
   *
   * <p>
   *
  ...
  /**
   * @see
   *     com.google.gwt.event.logical.shared.BeforeSelectionHandler#onBeforeSelection(com.google.gwt.event.logical.shared.BeforeSelectionEvent)
   */
  ...
  /**
   * Sets the 'override formats' flag, which tells the format handler to prioritize the formats from
   * the gallery dialog object.
   *
   * <p>
   *
  ...
  /**
   * Updates variable ui-element dimensions, execute after dialog has been attached and it's content
   * is displayed.
   *
   * <p>
   */
  ...
  /**
   * Truncates the gallery tabs to the available width.
   *
   * <p>
   */
