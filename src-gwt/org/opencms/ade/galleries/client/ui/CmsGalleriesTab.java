 ...
 *
 * <p>It displays the available gallery folders in the given order.
 *
 * @since 8.0.
 */
public class CmsGalleriesTab extends A_CmsListTab {
  ...
  /**
   * Handles the change of the item selection.
   *
   * <p>
   */
  private class SelectionHandler extends A_SelectionHandler {
    ...
    /**
     * @see
     *     org.opencms.ade.galleries.client.ui.A_CmsListTab.A_SelectionHandler#selectBeforeGoingToResultTab()
     */
    @Override
  ...
  /**
   * Flag which indicates whether new elements are currently being inserted into the galleries tab.
   *
   * <p>
   */
    ...
    for (String galleryPath : searchObj.getGalleries()) {
      ...
      if (galleryBean != null) {
        ...
        CmsSearchParamPanel panel =
            new CmsSearchParamPanel(
                Messages.get().key(Messages.GUI_PARAMS_LABEL_GALLERIES_0), this);
  ...
  /**
   * Returns the value of the "loading" flag, which indicates whether new elements are currently
   * being added into the galleries tab.
   *
   * <p>
   *
  ...
  public void updateListContent(
      List<CmsGalleryFolderBean> galleries, List<String> selectedGalleries) {
  ...
  public void updateTreeContent(
      List<CmsGalleryTreeEntry> galleryTreeEntries, List<String> selectedGalleries) {
  ...
  protected void addChildren(
      CmsTreeItem parent, List<CmsGalleryTreeEntry> children, List<String> selectedGalleries) {
  ...
  protected CmsTreeItem createTreeItem(
      CmsGalleryFolderBean galleryInfo, List<String> selectedGalleries, boolean forTree) {
    ...
    if (galleryInfo.isEditable()) {
      ...
      if (null != uploadAction) {
        ...
        uploadButton.setTitle(
            Messages.get().key(Messages.GUI_GALLERY_UPLOAD_TITLE_1, galleryInfo.getPath()));
                    ...
                    new CmsRpcAction<CmsUUID>() {
                      ...
                      protected void onResponse(CmsUUID result) {
                        ...
                        CmsEmbeddedDialogHandler.openDialog(
                            uploadAction, resultIds, id -> getTabHandler().updateIndex());
                      }
                    };
                action.execute();
              }
        ...
        if (CmsEditExternalLinkDialog.LINK_GALLERY_RESOURCE_TYPE_NAME.equals(
            galleryInfo.getType())) {
          ...
          if (!CmsCoreProvider.get().isUploadDisabled()) {
            String uploadPath =
                CmsStringUtil.joinPaths(CmsCoreProvider.get().getSiteRoot(), galleryInfo.getPath());
    ...
    if (m_tabHandler.hasGalleriesSelectable()) {
      CmsPushButton selectButton =
          createSelectResourceButton(galleryInfo.getPath(), CmsUUID.getNullUUID(), "", "");
