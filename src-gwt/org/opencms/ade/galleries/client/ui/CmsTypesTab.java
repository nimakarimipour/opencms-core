 ...
 *
 * <p>It displays the available types in the given sort order.
 *
 * @since 8.0.
 */
public class CmsTypesTab extends A_CmsListTab {
   ...
   */
  public CmsTypesTab(
      CmsTypesTabHandler tabHandler, CmsDNDHandler dndHandler, Widget additionalControl) {
  ...
  /**
   * Fill the content of the types tab panel.
   *
   * <p>
   *
    ...
    for (CmsResourceTypeBean typeBean : typeInfos) {
      ...
      if (typeBean.isDeactivated()) {
        ...
        if (typeBean.isCreatableType() && (m_dndHandler != null)) {
          ...
          listItem
              .getMoveHandle()
              .setTitle(Messages.get().key(Messages.GUI_TAB_TYPES_CREATE_NEW_0));
        }
      }
      ...
      if (typeBean.getVisibility() == TypeVisibility.showOptional) {
        listItem.addStyleName(
            I_CmsLayoutBundle.INSTANCE.galleryDialogCss().shouldOnlyShowInFullTypeList());
      }
  ...
  public List<CmsSearchParamPanel> getParamPanels(CmsGallerySearchBean searchObj) {
    ...
    for (String type : searchObj.getTypes()) {
      ...
      CmsSearchParamPanel panel =
          new CmsSearchParamPanel(Messages.get().key(Messages.GUI_PARAMS_LABEL_TYPES_0), this);
  ...
  protected void init() {
    ...
    final CmsCheckBox typeToggle =
        new CmsCheckBox(Messages.get().key(Messages.GUI_SHOW_SYSTEM_TYPES_0));
    ...
    typeToggle.addValueChangeHandler(
  ...
  /**
   * Enables/disables full type list mode.
   *
   * <p>
   *
  ...
  protected void setShowAllTypes(boolean showAllTypes) {
    ...
    m_typeListMode.setValue(
        showAllTypes ? null : I_CmsLayoutBundle.INSTANCE.galleryDialogCss().typesImportant());
