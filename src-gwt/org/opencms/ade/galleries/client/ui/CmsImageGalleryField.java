...
public class CmsImageGalleryField extends CmsGalleryField {
  ...
  public CmsImageGalleryField(I_CmsGalleryConfiguration configuration, boolean allowUploads) {
    ...
    m_descriptionArea.addStyleName(
        org.opencms.ade.galleries.client.ui.css.I_CmsLayoutBundle.INSTANCE
            .galleryFieldCss()
            .descriptionField());
    m_descriptionArea
        .getTextArea()
        .setStyleName(I_CmsLayoutBundle.INSTANCE.globalWidgetCss().textAreaBox());
    m_descriptionArea
        .getTextAreaContainer()
        .addStyleName(I_CmsLayoutBundle.INSTANCE.globalWidgetCss().textAreaBoxPanel());
    ...
    m_formatSelection
        .getOpener()
        .addStyleName(I_CmsLayoutBundle.INSTANCE.globalWidgetCss().selectBoxSelected());
    m_formatSelection
        .getSelectorPopup()
  ...
  /**
   * @see org.opencms.ade.galleries.client.ui.CmsGalleryField#setValueFromGallery(java.lang.String,
   *     org.opencms.util.CmsUUID, org.opencms.ade.galleries.client.preview.CmsCroppingParamBean)
   */
  @Override
  ...
  /**
   * Generates the format select box.
   *
   * <p>
   */
  ...
  /**
   * Parses the value and all its informations.
   *
   * <p>First part is the URL of the image. The second one describes the scale of this image.
   *
   * <p>The last one sets the selected format.
   *
   * <p>
   *
  ...
  private String parseValue(String value) {
    ...
    if (indexofformat > -1) {
      ...
      if (hasmoreValues > indexofformat) {
        m_selectedFormat =
            params.substring(indexofformat, hasmoreValues).replace(PARAMETER_FORMAT, "");
      } else {
    ...
    if (indexofdescritption > -1) {
      ...
      if (hasmoreValues > indexofdescritption) {
        m_description =
            params.substring(indexofdescritption, hasmoreValues).replace(PARAMETER_DESC, "");
      } else {
