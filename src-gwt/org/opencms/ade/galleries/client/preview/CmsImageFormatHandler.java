...
public class CmsImageFormatHandler implements HasValueChangeHandlers<CmsCroppingParamBean> {
  ...
  private static final String[] DEFAULT_FORMAT_NAMES = {
    ...
    DefaultRestriction.user.name()
        + ":"
        + Messages.get().key(Messages.GUI_IMAGE_USER_FORMAT_LABEL_0),
    DefaultRestriction.free.name()
        + ":"
        + Messages.get().key(Messages.GUI_IMAGE_FREE_FORMAT_LABEL_0),
    DefaultRestriction.small.name()
        + ":"
        + Messages.get().key(Messages.GUI_IMAGE_SMALL_FORMAT_LABEL_0),
  ...
  public HandlerRegistration addValueChangeHandler(
      ValueChangeHandler<CmsCroppingParamBean> handler) {
  ...
  public void init(CmsImageFormatsForm formatForm, CmsCroppingDialog croppingDialog) {
    ...
    m_croppingDialog.addValueChangeHandler(
        new ValueChangeHandler<CmsCroppingParamBean>() {

          /**
           * Executed on value change. Sets the returned cropping parameters.
           *
           * <p>
           *
  ...
  public void onHeightChange(String height) {
    ...
    // in case the width and height parameter don't match the current format any longer, switch to
    // user defined format
    if ((!m_currentFormat.isHeightEditable()
            || (m_ratioLocked && !m_currentFormat.isWidthEditable()))
  ...
  public void onWidthChange(String width) {
    ...
    if ((!m_currentFormat.isWidthEditable()
            || (m_ratioLocked && !m_currentFormat.isHeightEditable()))
        && hasUserFormatRestriction()) {
      m_formatForm.setFormatSelectValue(m_userFormatKey);
    } else {
      fireValueChangedEvent();
  ...
  public void setCropping(CmsCroppingParamBean croppingParam) {
    ...
    // only in case of the original-format-restriction, the cropping dialog may be opened to
    // override the selected format
  ...
  /**
   * Adjusts the current format to the cropping parameter.
   *
   * <p>
   */
  private void adjustToCurrentFormat() {
    ...
    if (m_currentFormat.isFixedRatio()) {
      m_formatForm.setRatioButton(
          false, false, Messages.get().key(Messages.GUI_PRIVIEW_BUTTON_RATIO_FIXED_0));
      m_ratioLocked = true;
    } else {
      if (!m_currentFormat.isHeightEditable() && !m_currentFormat.isWidthEditable()) {
        ...
        m_formatForm.setRatioButton(
            false, false, Messages.get().key(Messages.GUI_PRIVIEW_BUTTON_NOT_EDITABLE_0));
      } else {
    ...
    m_formatForm.setHeightInputEnabled(
        m_currentFormat.isHeightEditable() || hasUserFormatRestriction());
    m_formatForm.setWidthInputEnabled(
        m_currentFormat.isWidthEditable() || hasUserFormatRestriction());
  }
  ...
  /**
   * Generates the format restriction objects.
   *
   * <p>
   */
  ...
  /**
   * Checks the format restrictions if the match the giving cropping parameter.
   *
   * <p>
   *
   * @param croppingParam the cropping parameter
   ...
   */
  private I_CmsFormatRestriction getMatchingFormat(
      CmsCroppingParamBean croppingParam, boolean forceByName) {
  ...
  /**
   * Returns if the user defined format restriction is available.
   *
   * <p>
   *
  ...
  /**
   * Reads the format configuration for the given gallery mode.
   *
   * <p>
   *
   * @param mode the gallery mode
   * @param isNativeWidget if the dialog is used as a native widget
   * @param overrideFormats true if the formats from the gallery dialog should take priority in
   *     'editor' mode
   */
  private void readFormatsConfig(
      GalleryMode mode, boolean isNativeWidget, boolean overrideFormats) {
