...
public class CmsCroppingParamBean {
  ...
  public static CmsCroppingParamBean parseImagePath(String selectedPath) {
    ...
    if (pos > -1) {
      ...
      String param =
          selectedPath.substring(pos + SCALE_PARAM_NAME.length() + SCALE_PARAM_EQ.length());
    ...
    if (pos > -1) {
      String param =
          selectedPath.substring(pos + SCALE_FORMAT_NAME_PARAM.length() + SCALE_PARAM_EQ.length());
  ...
  /**
   * Parses a single scale value. Returning <code>-1</code> --> {@link
   * I_CmsFormatRestriction#DIMENSION_NOT_SET} invalid parameters.
   *
   * <p>
   *
  ...
  public CmsCroppingParamBean getRestrictedSizeParam(int maxHeight, int maxWidth) {
    ...
    if ((getTargetHeight() <= maxHeight) && (getTargetWidth() <= maxWidth)) {
      if ((getTargetHeight() == I_CmsFormatRestriction.DIMENSION_NOT_SET)
          && (getOrgHeight() > maxHeight)) {
        result.setTargetHeight(maxHeight);
      }
      if ((getTargetWidth() == I_CmsFormatRestriction.DIMENSION_NOT_SET)
          && (getOrgWidth() > maxWidth)) {
   ...
   *
   * <p>TODO: This does not work correctly if there isn't any cropping/scaling defined.
   *
   * @param maxHeight the max height
   * @param maxWidth the max width
  ...
  public String getScaleParam() {
    ...
    if ((m_targetHeight > -1) || (m_targetWidth > -1)) {
      result
          .append(SCALE_PARAM_TARGETHEIGHT)
          .append(SCALE_PARAM_COLON)
          .append(getResultingTargetHeight())
          .append(SCALE_PARAM_DELIMITER);
      result
          .append(SCALE_PARAM_TARGETWIDTH)
          .append(SCALE_PARAM_COLON)
          .append(getResultingTargetWidth())
          .append(SCALE_PARAM_DELIMITER);
    }
    if (m_cropX > -1) {
      result
          .append(SCALE_PARAM_CROP_X)
          .append(SCALE_PARAM_COLON)
          .append(m_cropX)
          .append(SCALE_PARAM_DELIMITER);
    }
    ...
    if (m_cropHeight > -1) {
      result
          .append(SCALE_PARAM_CROP_HEIGHT)
          .append(SCALE_PARAM_COLON)
          .append(m_cropHeight)
          .append(SCALE_PARAM_DELIMITER);
    }
  ...
  /**
   * Returns the resulting target height if set, otherwise '-1'.
   *
   * <p>
   *
   * @return the height
  ...
  /**
   * Returns the resulting target width if set, otherwise '-1'.
   *
   * <p>
   *
   * @return the width
