...
public final class CmsImageResourcePreview extends A_CmsResourcePreview<CmsImageInfoBean> {
  ...
  /**
   * @see
   *     org.opencms.ade.galleries.client.preview.I_CmsResourcePreview#loadResourceInfo(java.lang.String)
   */
  ...
  public void selectResource(
      final String resourcePath, final CmsUUID structureId, final String title) {

    switch (getGalleryMode()) {
      case widget:
        getInitialCroppingParameter(
            resourcePath,
            new I_CmsSimpleCallback<CmsCroppingParamBean>() {
              ...
              public void execute(CmsCroppingParamBean param) {
                ...
                if (getGalleryDialog().getWidgetHandler() != null) {
                  getGalleryDialog()
                      .getWidgetHandler()
                      .setWidgetValue(resourcePath, structureId, param);
                } else {
                  if (CmsPreviewUtil.isAdvancedWidget()) {
                    ...
                    CmsPreviewUtil.setResourcePath(
                        resourcePath
                            + ((param.isCropped() || param.isScaled())
                                ? "?" + param.toString()
                                : ""));
                  }
                }
    ...
    switch (getGalleryMode()) {
      case widget:
        if (getGalleryDialog().getWidgetHandler() != null) {
          getGalleryDialog()
              .getWidgetHandler()
              .setWidgetValue(
                  m_infoBean.getResourcePath(), m_infoBean.getStructureId(), croppingParam);
        } else {
          if (CmsPreviewUtil.isAdvancedWidget()) {
            CmsPreviewUtil.setVfsImage(
      ...
      case editor:
        m_handler.getImageAttributes(
            new I_CmsSimpleCallback<Map<String, String>>() {
  ...
  private void getImageInfo(
      final String resourcePath, final I_CmsSimpleCallback<CmsImageInfoBean> callback) {
  ...
  /**
   * Returns the initial cropping parameter bean for a given resource.
   *
   * <p>
   *
   * @param resourcePath the resource path
  ...
  private void getInitialCroppingParameter(
      final String resourcePath, final I_CmsSimpleCallback<CmsCroppingParamBean> callback) {

    getImageInfo(
        resourcePath,
        new I_CmsSimpleCallback<CmsImageInfoBean>() {
