...
public final class CmsPreviewUtil {
   ...
   * to the window object for use via JSNI.
   *
   * <p>
   *
  ...
  public static native void exportFunctions(
      String previewName, I_CmsResourcePreview<?> preview) /*-{
   ...
   *     <p>alt, class, height, hspace, linkPath, linkTarget, longDesc, style, title, vspace, width
   *     <p>all keys represent a tag attribute by the same name, only linkPath and linkTarget
   *     contain information on an surrounding link tag
   */
   ...
   */
  public static void setImageLink(
      String path, Map<String, String> attributes, String linkPath, String target) {
   ...
   *
   * <p>Widget mode: Use this function inside the xmlcontent.
   *
   ...
   */
  public static native void setVfsImage(
      String path, String scale, String formatName, String ratio) /*-{
        //the id of the input field in the xml content
        var fieldId = $wnd[@org.opencms.ade.galleries.shared.I_CmsGalleryProviderConstants::KEY_FIELD_ID];
  ...
  /**
   * Calls the integrator's set image function to set the image tag within the rich text editor
   * (FCKEditor, CKEditor, ...).
   *
   * <p>
   *
  ...
  private static native void nativeSetImageLink(
      String path, CmsJSONMap attributes, String linkPath, String target) /*-{
        $wnd[@org.opencms.ade.galleries.client.preview.CmsPreviewUtil::KEY_SET_IMAGE_LINK_FUNCTION]
                (path, attributes, linkPath, target);
    }-*/;

  /**
   * Calls the integrator's set link function to set the resource link within the rich text editor
   * (FCKEditor, CKEditor, ...).
   *
   * <p>
   *
