...
public class CmsFocalPointController {
  ...
  /**
   * The transformation for transforming from the image parent's coordinate system to the true
   * underlying image's native coordinate system.
   */
  ...
  /**
   * Clears the static event handler for the drag and drop.
   *
   * <p>
   */
  ...
  /**
   * Gets the pageX offset for a mouse event.
   *
   * <p>
   *
  ...
  /**
   * Gets the pageY offset for a mouse event.
   *
   * <p>
   *
   * @param event the event
  ...
  /**
   * Transforms a rectangle with the inverse of a coordinate transform.
   *
   * <p>
   *
  ...
  private static CmsRectangle transformRegionBack(I_CmsTransform transform, CmsRectangle region) {
    ...
    return CmsRectangle.fromPoints(
        transform.transformBack(topLeft), transform.transformBack(bottomRight));
  }

  /**
   * Called when the user clicks on the focal point widget.
   *
   * <p>This starts drag and drop.
   */
  ...
  public void reset() {
    ...
    if (isEditable()) {
      ...
      propChanges.add(
          new CmsPropertyModification(sid, CmsGwtConstants.PROPERTY_IMAGE_FOCALPOINT, val, false));
      propChanges.add(
          new CmsPropertyModification(sid, CmsGwtConstants.PROPERTY_IMAGE_FOCALPOINT, val, true));
  ...
  /**
   * Removes the focal point widget.
   *
   * <p>
   */
  ...
  /**
   * Gets the point which is the center of the crop region, or the center of the original image if
   * it isn't cropped, in the image's coordinate system.
   *
   * <p>
   *
  ...
  /**
   * Gets the rectangle in the image's coordinate system which corresponds to the crop region (or
   * the whole image, in case cropping is not used).
   *
   * <p>
   *
  ...
  private CmsRectangle getNativeCropRegion() {
    ...
    if ((crop == null) || !crop.isCropped()) {
      ...
      return CmsRectangle.fromLeftTopWidthHeight(
          crop.getCropX(), crop.getCropY(), crop.getCropWidth(), crop.getCropHeight());
    }
  }
  ...
  private void handleMove(NativeEvent nativeEvent) {
    ...
    if (m_coordinateTransform != null) {
      ...
      screenPoint =
          m_region.constrain(
              screenPoint); // make sure we remain in the screen region corresponding to original
                            // image (or crop).
      m_pointWidget.setCenterCoordsRelativeToParent(
  ...
  /**
   * Registers the preview event handler used for drag and drop.
   *
   * <p>
   */
  ...
  /**
   * Updates the focal point widget.
   *
   * <p>
   */
  ...
  /**
   * Sets up the coordinate transformations between the coordinate system of the parent element of
   * the image element and the native coordinate system of the original image.
   */
  private void updateScaling() {
    ...
    if (crop == null) {
      transforms.add(
          new CmsBoxFit(
              CmsBoxFit.Mode.scaleOnlyIfNecessary, wv, hv, info.getWidth(), info.getHeight()));
    } else {
      int wt, ht;
      ...
      if (crop.isCropped()) {
        transforms.add(
            new CmsBoxFit(
                CmsBoxFit.Mode.scaleAlways, wt, ht, crop.getCropWidth(), crop.getCropHeight()));
        transforms.add(new CmsTranslate(crop.getCropX(), crop.getCropY()));
      } else {
        transforms.add(
            new CmsBoxFit(
                CmsBoxFit.Mode.scaleAlways, wt, ht, crop.getOrgWidth(), crop.getOrgHeight()));
      }
    }
