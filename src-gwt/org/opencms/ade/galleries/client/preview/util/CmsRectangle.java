...
public class CmsRectangle {
   ...
   */
  public static CmsRectangle fromLeftTopWidthHeight(
  ...
  /**
   * Helper method to constrain a number inside a given interval.
   *
   * <p>If the number is inside the given range, it will be returned unchanged, otherwise either the
   * maximum or minimum of the range is returned depending on on which side the value lies.
   *
   ...
   *
   * <p>If any of the coordinates of the point lie in the projection of this rectangle on the
   * corresponding axis, that coordinate in the result will be unchanged, otherwise it will be
   * either the maximum or the minimum depending on on which side the original coordinate is
   * located.
   *
  ...
  public CmsPoint constrain(CmsPoint point) {
    ...
    return new CmsPoint(
        constrainNum(m_left, m_width, point.getX()), constrainNum(m_top, m_height, point.getY()));
  }
