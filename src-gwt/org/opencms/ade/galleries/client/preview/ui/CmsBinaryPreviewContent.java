...
public class CmsBinaryPreviewContent extends Composite {
   ...
   * <p>
   */
  public interface I_CmsPreviewContentUiBinder
      extends UiBinder<HTMLPanel, CmsBinaryPreviewContent> {
    // GWT interface, nothing to do
  }

  /** The ui binder instance for this widget class. */
  private static I_CmsPreviewContentUiBinder uiBinder =
      GWT.create(I_CmsPreviewContentUiBinder.class);

  /** The preview handler. */
  protected CmsBinaryPreviewHandler m_binaryPreviewHandler;

  /**
   * The list which only contains the list item for the previewed resource (required for drag and
   * drop).
   */
  @UiField protected CmsList<CmsListItem> m_list;

  /** The preview content HTML. */
  @UiField protected HTML m_previewContent;
  ...
  public CmsBinaryPreviewContent(CmsResourceInfoBean info, CmsBinaryPreviewHandler previewHandler) {
    ...
    CmsListItem listItem =
        createListItem(info, m_binaryPreviewHandler.getGalleryDialog().getDndHandler());
  ...
  /**
   * Creates the list item for the resource information bean.
   *
   * <p>
   *
  ...
  private CmsListItem createListItem(CmsResourceInfoBean resourceInfo, CmsDNDHandler dndHandler) {
    ...
    infoBean.addAdditionalInfo(
        Messages.get().key(Messages.GUI_PREVIEW_LABEL_SIZE_0), resourceInfo.getSize());
    ...
    final DeleteHandler deleteHandler =
        resultsTab.makeDeleteHandler(resourceInfo.getResourcePath());
