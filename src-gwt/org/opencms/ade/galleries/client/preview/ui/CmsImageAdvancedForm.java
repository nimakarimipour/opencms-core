...
public class CmsImageAdvancedForm extends Composite {
  ...
  /** Ui binder instance. */
  private static I_CmsImageAdvancedFormUiBinder m_uiBinder =
      GWT.create(I_CmsImageAdvancedFormUiBinder.class);
  ...
  @UiField protected Label m_labelDescription;

  /** Id field label. */
  @UiField protected Label m_labelId;

  /** Language code field label. */
  @UiField protected Label m_labelLanguageCode;

  /** Set link checkbox label. */
  @UiField protected Label m_labelSetLink;

  /** Style field label. */
  @UiField protected Label m_labelStyle;

  /** Style classes field label. */
  @UiField protected Label m_labelStyleClasses;

  /** Additional attributes label. */
  @UiField protected Label m_labelTagAttributes;

  /** Target field label. */
  @UiField protected Label m_labelTarget;

  /** Language direction field label. */
  @UiField protected Label m_labelTextDirection;

  /** URL field label. */
  @UiField protected Label m_labelUrl;
  ...
  public CmsImageAdvancedForm() {
    ...
    m_labelStyleClasses.setText(
        Messages.get().key(Messages.GUI_ADVANCED_TAB_LABEL_STYLE_CLASSES_0));
    ...
    m_labelLanguageCode.setText(
        Messages.get().key(Messages.GUI_ADVANCED_TAB_LABEL_LANGUAGE_CODE_0));
    m_labelTextDirection.setText(
        Messages.get().key(Messages.GUI_ADVANCED_TAB_LABEL_TEXT_DIRECTION_0));

    m_selectTarget.addOption("", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_NOT_SET_0));
    m_selectTarget.addOption(
        "_blank", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_NEW_WINDOW_0));
    m_selectTarget.addOption(
        "_top", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_TOP_WINDOW_0));
    m_selectTarget.addOption(
        "_self", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_SELF_WINDOW_0));
    m_selectTarget.addOption(
        "_parent", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_PARENT_WINDOW_0));

    m_selectTextDirection.addOption(
        "ltr", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_LEFT_TO_RIGHT_0));
    m_selectTextDirection.addOption(
        "rtl", Messages.get().key(Messages.GUI_ADVANCED_TAB_VALUE_RIGHT_TO_LEFT_0));
  ...
  public void fillContent(
      CmsImageInfoBean imageInfo, CmsJSONMap imageAttributes, boolean initialFill) {
    ...
    for (Entry<Attribute, I_CmsFormWidget> entry : m_fields.entrySet()) {
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(val)) {
        if ((entry.getKey() == Attribute.linkPath)
            && val.startsWith(CmsCoreProvider.get().getVfsPrefix())) {
          entry
              .getValue()
              .setFormValueAsString(val.substring(CmsCoreProvider.get().getVfsPrefix().length()));
        } else {
