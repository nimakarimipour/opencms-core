...
public class CmsCroppingDialog extends Composite
  ...
  private static I_CmsCroppingDialogUiBinder m_uiBinder =
      GWT.create(I_CmsCroppingDialogUiBinder.class);

  /** The cancel button. */
  @UiField protected CmsPushButton m_cancelButton;

  /** The cropping panel. */
  @UiField protected CmsAreaSelectPanel m_croppingPanel;

  /** The height label. */
  @UiField protected Label m_heightDisplay;

  /** The height label. */
  @UiField protected Label m_heightLabel;
  ...
  /** The height label. */
  @UiField protected Label m_scaleDisplay;

  /** The height label. */
  @UiField protected Label m_scaleLabel;

  /** The top panel holding the cropping area. */
  @UiField protected Element m_topPanel;
  /** The height label. */
  @UiField protected Label m_widthDisplay;

  /** The height label. */
  @UiField protected Label m_widthLabel;
      ...
      if (m_croppingParam.getTargetHeight() > 0) {
        scale =
            String.valueOf(
                    (int)
                        Math.floor(
                            (100.00 * m_croppingParam.getCropHeight())
        ...
        scale =
            String.valueOf(
                    (int)
                        Math.floor(
                            (100.00 * m_croppingParam.getCropWidth())
                                / m_croppingParam.getTargetWidth()))
                + "%";
      }
  ...
  public void show(CmsCroppingParamBean targetParam) {
    ...
    m_displayCropping =
        m_displayCropping.getRestrictedSizeParam(
            getElement().getOffsetHeight() - 35, getElement().getOffsetWidth() - 4);
    ...
    if ((targetParam.getTargetHeight() > 0) && (targetParam.getTargetWidth() > 0)) {
      m_croppingPanel.setRatio(
          (1.00 * targetParam.getTargetHeight()) / targetParam.getTargetWidth());
    } else {
  ...
  /**
   * Calculates the resulting cropping parameter from the supplied selection position.
   *
   * <p>
   *
  ...
  /**
   * Calculates the select area position for the current cropping parameter.
   *
   * <p>
   *
