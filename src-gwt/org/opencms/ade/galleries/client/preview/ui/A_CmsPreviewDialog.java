 ...
 *
 * <p>This widget contains a panel with the resource preview and a set of tabs with resource
 * information under the preview panel.
 *
 * <p>
 *
...
public abstract class A_CmsPreviewDialog<T extends CmsResourceInfoBean> extends Composite {
  ...
  /**
   * Displays a confirm save changes dialog with the given message. May insert individual message
   * before the given one for further information.
   *
   * <p>Will call the appropriate command after saving/cancel.
   *
   * <p>
   *
  ...
  /**
   * Will be triggered, when the select button is clicked.
   *
   * <p>
   *
  ...
  public void onSelectClick(ClickEvent event) {
    ...
    if (m_galleryMode == GalleryMode.editor) {
      // note: the select button isn't necessarily visible in editor mode (depending on the WYSIWYG
      // editor), but
