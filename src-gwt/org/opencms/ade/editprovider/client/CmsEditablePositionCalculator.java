...
/**
 * This class is used to calculate positions for a set of direct edit buttons so that they don't
 * overlap.
 *
 * <p>
 *
...
public class CmsEditablePositionCalculator {
  ...
  /**
   * Calculates non-overlapping positions for the button bars and returns them in a map with the
   * element ids as keys.
   *
   * <p>
   *
  ...
  protected boolean checkCollision() {
    ...
    for (i = 0; i < m_positions.size(); i++) {
      for (int j = i + 1;
          (j < m_positions.size())
              && intersectsHorizontally(m_positions.get(i), m_positions.get(j));
          j++) {
  ...
  protected boolean intersectsHorizontally(CmsPositionBean p1, CmsPositionBean p2) {
    ...
    return intersectIntervals(
        p1.getLeft(), p1.getLeft() + WIDTH, p2.getLeft(), p2.getLeft() + WIDTH);
  }
