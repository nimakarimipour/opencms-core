 ...
 * @since 8.0.0
 */
public class CmsDirectEditButtons extends A_CmsDirectEditButtons
   ...
   *     org.opencms.util.CmsUUID, boolean, boolean)
   */
  public void onClose(
      String sitePath,
      CmsUUID structureId,
  ...
  public void setPosition(
      CmsPositionBean position, CmsPositionBean buttonsPosition, Element containerElement) {
  ...
  protected void deleteElement() {
    ...
    // CmsContainerpageController.get().deleteElement(m_editableData.getStructureId(),
    // m_parentResourceId);
  }
  ...
  protected void onClickDelete() {
    ...
    CmsDeleteWarningDialog deleteDialog =
        new CmsDeleteWarningDialog(m_editableData.getSitePath()) {
  ...
  protected void openEditDialog(boolean isNew, String mode) {
    ...
    if (Window.Location.getPath().endsWith(backlink)) {
      // CmsCoreProvider.get().getUri() is the request context uri from the time the direct edit
      // provider
      // includes are generated. In case the template has changed the request context uri before
      // that point,
