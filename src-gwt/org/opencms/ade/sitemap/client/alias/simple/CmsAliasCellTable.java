...
public class CmsAliasCellTable extends CellTable<CmsAliasTableRow> {
  ...
  public CmsAliasCellTable(CmsAliasTableController controller) {
    ...
    A_CmsAliasTableColumn<CmsAliasTableRow, ?, CmsAliasCellTable>[] columns =
        new A_CmsAliasTableColumn[] {
    ...
    ColumnSortEvent.ListHandler<CmsAliasTableRow> sortHandler =
        new ColumnSortEvent.ListHandler<CmsAliasTableRow>(m_dataProvider.getList());
    for (A_CmsAliasTableColumn<CmsAliasTableRow, ?, CmsAliasCellTable> column : columns) {
      column.initSortHandler(sortHandler);
    }
    addColumnSortHandler(sortHandler);

    final MultiSelectionModel<CmsAliasTableRow> selectionModel =
        new MultiSelectionModel<CmsAliasTableRow>(getKeyProvider());
    ...
    setSelectionModel(
        selectionModel, DefaultSelectionEventManager.<CmsAliasTableRow>createCheckboxManager());
  ...
  /**
   * Gets the alias editor controller used by this table.
   *
   * <p>
   *
   ...
   *     <p>(Overridden to use a less general return type)
   */
  @SuppressWarnings("unchecked")
  @Override
