...
public class CmsSitemapTreeItem extends CmsLazyTreeItem {
  ...
  private static Map<CmsUUID, CmsSitemapTreeItem> m_itemsById =
      new HashMap<CmsUUID, CmsSitemapTreeItem>();
  ...
  public CmsSitemapTreeItem(CmsClientSitemapEntry entry) {
    ...
    getListItemWidget()
        .setTitleEditHandler(
            new I_CmsTitleEditHandler() {

              /**
               * @see
               *     org.opencms.gwt.client.ui.CmsListItemWidget.I_CmsTitleEditHandler#handleEdit(org.opencms.gwt.client.ui.input.CmsLabel,
              ...
              public void handleEdit(CmsLabel titleLabel, TextBox box) {
                ...
                if (CmsStringUtil.isEmpty(newTitle)) {
                  ...
                  String dialogTitle =
                      Messages.get().key(Messages.GUI_EDIT_TITLE_ERROR_DIALOG_TITLE_0);
                ...
                if (!oldTitle.equals(newTitle)) {
                  CmsPropertyModification propMod =
                      new CmsPropertyModification(
                          editEntry.getId(), CmsClientProperty.PROPERTY_NAVTEXT, newTitle, true);
                  final List<CmsPropertyModification> propChanges =
                      new ArrayList<CmsPropertyModification>();
                  propChanges.add(propMod);
                  final CmsSitemapController controller =
                      CmsSitemapView.getInstance().getController();
                  if (editEntry.isNew()
                      && !editEntry.isRoot()
                      && !oldTitle.equalsIgnoreCase(newTitle)) {
                    if (oldTitle.equals(
                        editEntry.getPropertyValue(CmsClientProperty.PROPERTY_TITLE))) {
                      CmsPropertyModification titleMod =
                          new CmsPropertyModification(
                              editEntry.getId(), CmsClientProperty.PROPERTY_TITLE, newTitle, true);
                        ...
                        new I_CmsSimpleCallback<String>() {
                          ...
                          public void execute(String urlName) {
                            ...
                            controller.editAndChangeName(
                                editEntry, urlName, propChanges, true, CmsReloadMode.reloadEntry);
                          }
                        });
                  } else {
  ...
  /**
   * Returns the list info bean for the given entry.
   *
   * <p>
   *
   * @param entry the sitemap entry
  ...
  static CmsListInfoBean getInfoBean(CmsClientSitemapEntry entry, boolean navMode) {
    ...
    if (navMode) {
      ...
      CmsClientProperty titleProperty =
          defaultFileProps == null ? null : defaultFileProps.get(CmsClientProperty.PROPERTY_TITLE);
    ...
    if (!isTitleSet) {
      CmsClientProperty titleProperty =
          entry.getOwnProperties().get(CmsClientProperty.PROPERTY_TITLE);
      if ((titleProperty != null) && !titleProperty.isEmpty()) {
        addInfo(infos, msg.key(Messages.GUI_TITLE_PROPERTY_0), titleProperty.getEffectiveValue());
        isTitleSet = true;
      }
  ...
  /**
   * Generates the list item widget for the tree item.
   *
   * <p>
   *
   * @param entry the sitemap entry
  ...
  /**
   * Sets the additional style to mark expired entries or those that have the hide in navigation
   * property set.
   *
   * <p>
   *
   ...
   */
  private static void setAdditionalStyles(
      CmsClientSitemapEntry entry, CmsListItemWidget itemWidget) {
    ...
    if (!entry.isResleasedAndNotExpired()
      ...
      itemWidget
          .getContentPanel()
          .addStyleName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().expiredOrNotReleased());
    } else {
      itemWidget
          .getContentPanel()
          .removeStyleName(
    ...
    if (entry.isHiddenNavigationEntry()) {
      itemWidget
          .getContentPanel()
          .addStyleName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().hiddenNavEntry());
    } else {
      itemWidget
          .getContentPanel()
          .removeStyleName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().hiddenNavEntry());
    }
  }

  /**
   * Given the path of a sitemap entry, this method returns the URL which should be displayed to the
   * user.
   *
   * <p>
   *
  ...
  public String getDisplayedUrl(String sitePath) {
    ...
    if ("true".equals(exportProp)) {
      ...
      String rfsPrefix =
          CmsSitemapView.getInstance().getController().getData().getExportRfsPrefix();
      if (rfsPrefix != null) {
  ...
  public boolean isDropEnabled() {
    ...
    return !entry.hasForeignFolderLock()
  ...
  /**
   * @see
   *     org.opencms.gwt.client.ui.CmsListItem#onStartDrag(org.opencms.gwt.client.dnd.I_CmsDropTarget)
   */
  @Override
   ...
   *
   * <p>If the background is <code>null</code>, the widget will be shown with its default style.
   *
   * <p>
   *
  ...
  public void updateDetailPageStatus() {
    ...
    CmsDetailPageTable detailPageTable =
        CmsSitemapView.getInstance().getController().getDetailPageTable();
    String type;
    String text = null;
  ...
  public void updateEntry(CmsClientSitemapEntry entry) {
    ...
    getListItemWidget()
        .reInitAdditionalInfo(getInfoBean(entry, CmsSitemapView.getInstance().isNavigationMode()));
    updateSitePath(entry.getSitePath());
    updateDetailPageStatus();
    updateLock(entry);
    updateInNavigation(entry);
  ...
  public void updateSitePath(String sitePath) {
    ...
    if (getLoadState() == LoadState.LOADED) {
      for (int i = 0; i < getChildCount(); i++) {
        ...
        if ((item != null)
            && (CmsSitemapView.getInstance().getController().getEntryById(item.getEntryId())
                != null)) {
  ...
  protected void removeInvalidChildren() {
    ...
    if (getLoadState() == LoadState.LOADED) {
      ...
      for (int i = 0; i < getChildCount(); i++) {
        ...
        if ((id != null)
            && (CmsSitemapView.getInstance().getController().getEntryById(id) == null)) {
          toDelete.add(item);
        }
      }
  ...
  /**
   * Retrieves the hoverbar, can be <code>null</code> if not attached.
   *
   * <p>
   *
  ...
  /**
   * Updates the lock icon according to the entry information.
   *
   * <p>
   *
