...
public class CmsToolbarChooseEditorModeButton extends CmsMenuButton {
  ...
  /**
   * Context menu entry used to select a sitemap editor mode.
   *
   * <p>
   */
    ...
    addClickHandler(
        new ClickHandler() {
          ...
          public void onClick(ClickEvent event) {
            ...
            if (!isOpen()) {
              m_menuPanel.setWidget(
                  0,
                  0,
                  createContextMenu()); // we have to create the menu every time because the active
                                        // mode may change
              openMenu();
            } else {
              closeMenu();
  ...
  /**
   * Creates the menu widget for this button.
   *
   * <p>
   *
  ...
  public CmsContextMenu createContextMenu() {
    ...
    m_entries.add(
        new EditorModeEntry(
            Messages.get().key(Messages.GUI_NON_NAVIGATION_BUTTON_TITLE_0), EditorMode.vfs));
    m_entries.add(
        new EditorModeEntry(
            Messages.get().key(Messages.GUI_ONLY_GALLERIES_BUTTON_TITLE_0), EditorMode.galleries));
    if (CmsCoreProvider.get().getUserInfo().isCategoryManager()) {
      m_entries.add(
          new EditorModeEntry(
              Messages.get().key(Messages.GUI_CONTEXTMENU_CATEGORY_MODE_0), EditorMode.categories));
    }
    if (m_canEditModelPages) {
      m_entries.add(
          new EditorModeEntry(
              Messages.get().key(Messages.GUI_MODEL_PAGES_0), EditorMode.modelpages));
    }
    if (CmsSitemapView.getInstance().getController().isLocaleComparisonEnabled()) {
      m_entries.add(
          new EditorModeEntry(
              Messages.get().key(Messages.GUI_LOCALECOMPARE_MODE_0), EditorMode.compareLocales));
    }
