...
public class CmsToolbarNewButton extends A_CmsToolbarListMenuButton {
  ...
  protected boolean initContent() {
    ...
    if (!hasTabs) {
      ...
      messageLabel.addStyleName(
          I_CmsSitemapLayoutBundle.INSTANCE.clipboardCss().menuTabContainer());
  ...
  private CmsCreatableListItem makeDetailPageItem(CmsNewResourceInfo typeInfo) {
    ...
    String title =
        Messages.get()
            .key(
                typeInfo.isFunction()
                    ? Messages.GUI_FUNCTION_PAGE_TITLE_1
                    : Messages.GUI_DETAIL_PAGE_TITLE_1,
                typeInfo.getTitle());
    ...
    CmsCreatableListItem listItem =
        new CmsCreatableListItem(widget, typeInfo, NewEntryType.detailpage);
  ...
  private CmsCreatableListItem makeNavigationLevelItem() {
    ...
    CmsCreatableListItem listItem =
        new CmsCreatableListItem(widget, typeInfo, NewEntryType.regular);
    listItem.initMoveHandle(CmsSitemapView.getInstance().getTree().getDnDHandler());
    return listItem;
  }
  ...
  private CmsCreatableListItem makeNewElementItem(final CmsNewResourceInfo typeInfo) {
    ...
    if (typeInfo.getVfsPath() != null) {
      widget.addAdditionalInfo(
          new CmsAdditionalInfoBean(
              Messages.get().key(Messages.GUI_LABEL_VFSPATH_0), typeInfo.getVfsPath(), null));
    }
    if (typeInfo.getDate() != null) {
      widget.addAdditionalInfo(
          new CmsAdditionalInfoBean(
              Messages.get().key(Messages.GUI_LABEL_DATE_0), typeInfo.getDate(), null));
    }
    CmsCreatableListItem listItem =
        new CmsCreatableListItem(widget, typeInfo, NewEntryType.regular);
  ...
  private CmsCreatableListItem makeRedirectItem() {
    ...
    widget.setIcon(typeInfo.getBigIconClasses());
    CmsCreatableListItem listItem =
        new CmsCreatableListItem(widget, typeInfo, NewEntryType.redirect);
    listItem.initMoveHandle(CmsSitemapView.getInstance().getTree().getDnDHandler());
    return listItem;
  }
}
