...
public class CmsCreateCategoryMenuEntry extends A_CmsSitemapMenuEntry {
   ...
   */
  public static void askForNewCategoryInfo(
      CmsUUID parentId, final AsyncCallback<CmsCategoryTitleAndName> callback) {

    CmsSitemapCategoryData categoryData =
        CmsSitemapView.getInstance().getController().getCategoryData();
    ...
    if (entry == null) {
      ...
      caption =
          Messages.get().key(Messages.GUI_SITEMAP_CREATE_SUBCATEGORY_TITLE_1, entry.getPath());
    }
    ...
    dlg.getForm()
        .addField(
            CmsBasicFormField.createField(
                createBasicStringProperty("title", titleLabel, ".+", notEmptyMessage)),
            "");
    dlg.getForm()
        .addField(
  ...
  /**
   * Creates a property configuration for a simple named string field.
   *
   * <p>
   *
  ...
  public static CmsXmlContentProperty createBasicStringProperty(
      String name, String niceName, String validationRegex, String error) {
