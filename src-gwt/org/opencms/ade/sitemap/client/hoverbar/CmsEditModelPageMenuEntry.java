...
public class CmsEditModelPageMenuEntry extends A_CmsSitemapMenuEntry {
  ...
  public static boolean checkVisible(CmsUUID id) {
    ...
    if (CmsSitemapView.getInstance().getController().isEditable()) {
      CmsNewResourceInfo info =
          CmsSitemapView.getInstance().getController().getData().getNewResourceInfoById(id);
      show =
          CmsSitemapView.getInstance().isModelPageMode()
              && (((info != null) && info.isEditable())
                  || CmsSitemapView.getInstance().isModelGroupEntry(id));
    }
    return show;
  }
  ...
  /**
   * Opens the container page editor for the given model resource.
   *
   * <p>
   *
