...
public class CmsSitemapController implements I_CmsSitemapController {
  ...
  protected Map<CmsUUID, CmsDetailPageInfo> m_allDetailPageInfos =
      new HashMap<CmsUUID, CmsDetailPageInfo>();
  ...
  private List<I_CmsPropertyUpdateHandler> m_propertyUpdateHandlers =
      new ArrayList<I_CmsPropertyUpdateHandler>();
      ...
      m_data =
          (CmsSitemapData)
              CmsRpcPrefetcher.getSerializedObjectFromDictionary(
                  getService(), CmsSitemapData.DICT_NAME);
    ...
    CmsRpcAction<CmsListInfoBean> action =
        new CmsRpcAction<CmsListInfoBean>() {
          ...
          protected void onResponse(CmsListInfoBean infoResult) {
            ...
            stop(false);
            final CmsLocaleComparePropertyHandler handler =
                new CmsLocaleComparePropertyHandler(data);
            handler.setPageInfo(infoResult);

            CmsRpcAction<Map<CmsUUID, Map<String, CmsXmlContentProperty>>> propertyAction =
                new CmsRpcAction<Map<CmsUUID, Map<String, CmsXmlContentProperty>>>() {
                  ...
                  protected void onResponse(
                      Map<CmsUUID, Map<String, CmsXmlContentProperty>> propertyResult) {

                    stop(false);
                    Map<String, CmsXmlContentProperty> propConfig =
                        new LinkedHashMap<String, CmsXmlContentProperty>();
                    for (Map<String, CmsXmlContentProperty> defaultProps :
                    ...
                    editor.setPropertyNames(
                        CmsSitemapView.getInstance()
                            ...
                            .getData()
                            .getAllPropertyNames());
                    final CmsFormDialog dialog =
                        new CmsFormDialog(handler.getDialogTitle(), editor.getForm());
                    CmsPropertyDefinitionButton defButton =
  ...
  public void addToNavigation(CmsClientSitemapEntry entry) {
    ...
    CmsPropertyModification mod =
        new CmsPropertyModification(
            entry.getId(), CmsClientProperty.PROPERTY_NAVTEXT, entry.getTitle(), true);
  ...
  public void bump(CmsClientSitemapEntry entry) {
    ...
    CmsSitemapChange change =
        new CmsSitemapChange(entry.getId(), entry.getSitePath(), ChangeType.bumpDetailPage);
   ...
   * @param parameter an additional parameter which may contain more information needed to create
   *     the new resource
  ...
  public void createNewGallery(
      final CmsUUID parentId, final int galleryTypeId, final String title) {

    final String parentFolder =
        parentId != null
            ? getEntryById(parentId).getSitePath()
            : CmsStringUtil.joinPaths(
                m_data.getRoot().getSitePath(), m_data.getDefaultGalleryFolder());

    CmsRpcAction<CmsGalleryFolderEntry> action =
        new CmsRpcAction<CmsGalleryFolderEntry>() {
   ...
   * @param copyId the structure id of the resource which should be used as a copy model for the new
   *     page
  ...
  public void createNewModelPage(
    ...
    CmsRpcAction<CmsModelPageEntry> action =
        new CmsRpcAction<CmsModelPageEntry>() {
          ...
          public void execute() {
            ...
            getService()
                .createNewModelPage(
                    getEntryPoint(), title, description, copyId, isModelGroup, this);
          }
   ...
   */
  public void createSitemapSubEntry(
      final CmsClientSitemapEntry newEntry, CmsUUID parentId, String sitemapType) {
   ...
   */
  public void createSitemapSubEntry(
      final CmsClientSitemapEntry parent, final String sitemapFolderType) {

    CmsSitemapTreeItem item = CmsSitemapTreeItem.getItemById(parent.getId());
    AsyncCallback<CmsClientSitemapEntry> callback =
        new AsyncCallback<CmsClientSitemapEntry>() {
          ...
          public void onSuccess(CmsClientSitemapEntry result) {
            ...
            makeNewEntry(
                parent,
                new I_CmsSimpleCallback<CmsClientSitemapEntry>() {
  ...
  public void createSubEntry(
      final CmsClientSitemapEntry newEntry, CmsUUID parentId, CmsUUID structureId) {
  ...
  public void createSubSitemap(final CmsUUID entryId) {
    ...
    CmsRpcAction<CmsSitemapChange> subSitemapAction =
        new CmsRpcAction<CmsSitemapChange>() {
  ...
  public void delete(String sitePath) {
    ...
    CmsSitemapChange change =
        new CmsSitemapChange(entry.getId(), entry.getSitePath(), ChangeType.delete);
    change.setParentId(parent.getId());
    change.setDefaultFileId(entry.getDefaultFileId());
    CmsSitemapClipboardData data =
        CmsSitemapView.getInstance().getController().getData().getClipboardData().copy();
    if (!entry.isNew()) {
    ...
    CmsDetailPageTable detailPageTable =
        CmsSitemapView.getInstance().getController().getData().getDetailPageTable();
  ...
  public void deleteCategory(final CmsUUID id) {
    ...
    CmsDeleteWarningDialog deleteWarningDialog =
        new CmsDeleteWarningDialog(id) {
  ...
  public void edit(
    ...
    final String updateTarget =
        ((reloadStatus == CmsReloadMode.reloadParent))
    ...
    Command callback =
        new Command() {
          ...
          public void execute() {
            ...
            if ((reloadStatus == CmsReloadMode.reloadParent)
                || (reloadStatus == CmsReloadMode.reloadEntry)) {
  ...
  public void editAndChangeName(
    ...
    final CmsUUID updateTarget =
        ((reloadStatus == CmsReloadMode.reloadParent)) ? getParentEntry(entry).getId() : entryId;
    Command callback =
        new Command() {
  ...
  /**
   * Ensure the uniqueness of a given URL-name within the children of the given parent site-map
   * entry.
   *
   * <p>
   *
  ...
  public void ensureUniqueName(
      CmsClientSitemapEntry parent, String newName, I_CmsSimpleCallback<String> callback) {
   ...
   */
  public void ensureUniqueName(
      final String parentFolder, String newName, final I_CmsSimpleCallback<String> callback) {
   ...
   * @param entryId the entry id
   * @param setOpen if the entry should be opened
   * @param continueIfParentNotLoaded if false, and the entry identified by the id has not been
   *     loaded, stop; else store the entry in memory
   * @param callback the callback to execute after the children have been loaded
   */
  public void getChildren(
      final CmsUUID entryId,
    ...
    CmsRpcAction<CmsClientSitemapEntry> getChildrenAction =
        new CmsRpcAction<CmsClientSitemapEntry>() {
          ...
          public void onResponse(CmsClientSitemapEntry result) {
            ...
            m_eventBus.fireEventFromSource(
                new CmsSitemapLoadEvent(target, setOpen), CmsSitemapController.this);
            stop(false);
            if (callback != null) {
  ...
  /**
   * Gets the value for a property which a sitemap entry would inherit if it didn't have its own
   * properties.
   *
   * <p>
   *
  ...
  /**
   * Returns a list of all descendant sitemap entries of a given path which have already been loaded
   * on the client.
   *
   * <p>
   *
  ...
  /**
   * Returns the no edit reason or <code>null</code> if editing is allowed.
   *
   * <p>
   *
  ...
  public String getNoEditReason(CmsClientSitemapEntry entry, boolean checkChildLocks) {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(reason)) {
      ...
      if ((entry.getLock() != null)
        ...
        reason =
            Messages.get().key(Messages.GUI_DISABLED_LOCKED_BY_1, entry.getLock().getLockOwner());
      }
  ...
  public I_CmsSitemapServiceAsync getService() {
    ...
    if (m_service == null) {
      ...
      String serviceUrl =
          CmsCoreProvider.get().link("org.opencms.ade.sitemap.CmsVfsSitemapService.gwt");
   ...
   *
   * <p>Hidden entries will still be visible in the navigation mode of the sitemap editor. They will
   * also have a NavText and a NavPos. Only when using the NavBuilder get navigation for folder
   * method, they will not be included.
   *
   * <p>
   *
   * @param entryId the entry id
   ...
   *
   * <p>A hidden property is a property which should not appear in the property editor because it
  ...
  public void loadCategories(final boolean openLocalCategories, final CmsUUID openItemId) {
    ...
    CmsRpcAction<CmsSitemapCategoryData> action =
        new CmsRpcAction<CmsSitemapCategoryData>() {
          ...
          protected void onResponse(CmsSitemapCategoryData result) {
            ...
            CmsSitemapView.getInstance()
                .displayCategoryData(result, openLocalCategories, openItemId);
          }
        };
    action.execute();
  }
  ...
  public void loadGalleries() {
    ...
    CmsRpcAction<Map<CmsGalleryType, List<CmsGalleryFolderEntry>>> action =
        new CmsRpcAction<Map<CmsGalleryType, List<CmsGalleryFolderEntry>>>() {
  ...
  public void loadNewElementInfo(final AsyncCallback<Void> callback) {
    ...
    CmsRpcAction<List<CmsNewResourceInfo>> newResourceInfoAction =
        new CmsRpcAction<List<CmsNewResourceInfo>>() {
  ...
  public void mergeSubSitemap(final CmsUUID entryId) {
    ...
    CmsRpcAction<CmsSitemapChange> mergeAction =
        new CmsRpcAction<CmsSitemapChange>() {
   ...
   * @param rootId the structure id of the resource comparing to the tree root in sitemap compare
   *     mode
   */
  public void openPropertyDialogForVaadin(final CmsUUID structureId, final CmsUUID rootId) {

    CmsRpcAction<CmsLocaleComparePropertyData> action =
        new CmsRpcAction<CmsLocaleComparePropertyData>() {
          ...
          protected void onResponse(CmsLocaleComparePropertyData result) {
            ...
            CmsSitemapController.editPropertiesForLocaleCompareMode(
                result, structureId, result.getDefaultFileId(), null);
          }
        };
    action.execute();
  }
  ...
  /**
   * @see
   *     org.opencms.ade.sitemap.shared.I_CmsSitemapController#registerEntry(org.opencms.ade.sitemap.shared.CmsClientSitemapEntry)
   */
  ...
  /**
   * @see
   *     org.opencms.ade.sitemap.shared.I_CmsSitemapController#registerPathChange(org.opencms.ade.sitemap.shared.CmsClientSitemapEntry,
   *     java.lang.String)
   */
  ...
  public void removeFromNavigation(CmsUUID entryId) {
    ...
    CmsClientSitemapEntry parent = getEntry(CmsResource.getParentFolder(entry.getSitePath()));
    CmsSitemapChange change =
        new CmsSitemapChange(entry.getId(), entry.getSitePath(), ChangeType.remove);
    change.setParentId(parent.getId());
    change.setDefaultFileId(entry.getDefaultFileId());
    CmsSitemapClipboardData data =
        CmsSitemapView.getInstance().getController().getData().getClipboardData().copy();
  ...
  /**
   * @see
   *     org.opencms.ade.sitemap.shared.I_CmsSitemapController#replaceProperties(org.opencms.util.CmsUUID,
   *     java.util.Map)
   */
  public Map<String, CmsClientProperty> replaceProperties(
      CmsUUID id, Map<String, CmsClientProperty> properties) {
  ...
  protected void applyChange(CmsSitemapChange change) {
    ...
    switch (change.getChangeType()) {
      ...
      case remove:
        ...
        CmsPropertyModification propMod =
            new CmsPropertyModification(
                entry.getId(), CmsClientProperty.PROPERTY_NAVTEXT, null, true);
      ...
      case modify:
        if (change.hasNewParent() || change.hasChangedPosition()) {
          ...
          CmsClientSitemapEntry destParent =
              change.hasNewParent() ? getEntryById(change.getParentId()) : sourceParent;
        ...
        if (change.getUpdatedEntry() != null) {
          ...
          CmsClientSitemapEntry parent =
              getEntry(CmsResource.getParentFolder(oldEntry.getSitePath()));
  ...
  protected void commitChange(final CmsSitemapChange change, final Command callback) {
    ...
    if (change != null) {
      // save the sitemap
      CmsRpcAction<CmsSitemapChange> saveAction =
          new CmsRpcAction<CmsSitemapChange>() {
  ...
  protected CmsSitemapChange getChangeForEdit(
      CmsClientSitemapEntry entry, List<CmsPropertyModification> propertyChanges) {
  ...
  /**
   * Creates a new client sitemap entry bean to use for the RPC call which actually creates the
            ...
            newEntry
                ...
                .put(
                    ...
                    new CmsClientProperty(
                        CmsClientProperty.PROPERTY_TITLE, NEW_ENTRY_NAME, NEW_ENTRY_NAME));
  ...
  /**
   * Store the gallery type information.
   *
   * <p>
   *
  ...
  private void cleanupOldPaths(String oldSitepath, String newSitepath) {
    ...
    List<CmsClientSitemapEntry> entries =
        new ArrayList<CmsClientSitemapEntry>(m_entriesById.values());
    for (CmsClientSitemapEntry entry : entries) {
      if (entry.getSitePath().startsWith(oldSitepath)) {
        String currentPath = entry.getSitePath();
        String updatedSitePath =
            CmsStringUtil.joinPaths(newSitepath, currentPath.substring(oldSitepath.length()));
   ...
   */
  private void removeDeletedFromModified(
      CmsClientSitemapEntry entry, CmsSitemapClipboardData data) {
