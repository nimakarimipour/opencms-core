...
public class CmsSitemapDNDController implements I_CmsDNDController {
  ...
  public boolean onDragStart(
    ...
    CmsDebugLog.getInstance()
        .printLine("Starting path: " + m_originalPath + ", Index: " + m_originalIndex);
    return true;
  }
  ...
  public void onDrop(I_CmsDraggable draggable, I_CmsDropTarget target, CmsDNDHandler handler) {
    ...
    CmsClientSitemapEntry parent =
        CmsSitemapView.getInstance().getController().getEntry(m_insertPath);
    if (draggable instanceof CmsSitemapTreeItem) {
   ...
   * @param strict if <code>false</code> only the parent path is considered, the index position will
   *     be ignored
  ...
  /**
   * Adjust the original position indicator by styling the draggable element for this purpose.
   *
   * <p>
   *
   * @param draggable the draggable
  ...
  private void adjustOriginalPositionIndicator(
      CmsSitemapTreeItem draggable, I_CmsDropTarget target, CmsDNDHandler handler) {

    if (!isChangedPosition(draggable, target, true)) {
      draggable
          .getElement()
          .addClassName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().markUnchanged());
      List<Element> itemWidget =
          CmsDomUtil.getElementsByClass(
              org.opencms.gwt.client.ui.css.I_CmsLayoutBundle.INSTANCE
                  .listItemWidgetCss()
                  .itemContainer(),
      ...
      draggable
          .getElement()
          .removeClassName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().markUnchanged());
      CmsDomUtil.removeDisablingOverlay(handler.getPlaceholder());
    }
  }

  /**
   * Handles a dropped detail page.
   *
   * <p>
   *
   ...
   */
  private void handleDropNewEntry(
      CmsCreatableListItem createItem, final CmsClientSitemapEntry parent) {
      ...
      case detailpage:
        ...
        if (typeInfo.isFunction()) {
          ...
          CmsClientProperty titleProp =
              new CmsClientProperty(CmsClientProperty.PROPERTY_TITLE, typeInfo.getTitle(), null);
          CmsClientProperty navtextProp =
              new CmsClientProperty(CmsClientProperty.PROPERTY_NAVTEXT, typeInfo.getTitle(), null);
  ...
  /**
   * Handles the drop for a sitemap item which was dragged to a different position.
   *
   * <p>
   *
          ...
          new I_CmsSimpleCallback<String>() {
            ...
            public void execute(String uniqueName) {
              ...
              if (!uniqueName.equals(entry.getName())
                  && isChangedPosition(sitemapEntry, target, false)) {
  ...
  /**
   * Hides the content of list items by setting a specific css class.
   *
   * <p>
   *
  ...
  private void hideItemContent(Element element) {
    ...
    if ((itemWidget != null) && (itemWidget.size() > 0)) {
      itemWidget
          .get(0)
          .addClassName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().contentHide());
    }
  }
}
