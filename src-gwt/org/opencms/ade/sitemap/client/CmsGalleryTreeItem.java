...
public class CmsGalleryTreeItem extends CmsTreeItem {
    ...
    public void setAdditionalInfoVisible(boolean visible) {
      ...
      if (visible && !m_openClose.isDown()) {
        CmsClientSitemapEntry entry =
            CmsSitemapView.getInstance().getController().getEntryById(getEntryId());
        if (entry != null) {
  ...
  protected void showGallery(CmsClientSitemapEntry entry) {
    ...
    List<String> typeNames =
        CmsSitemapView.getInstance()
            .getController()
            .getGalleryType(new Integer(entry.getResourceTypeId()))
            .getContentTypeNames();
  ...
  /**
   * Handles direct editing of the gallery title.
   *
   * <p>
   *
    ...
    if (!oldTitle.equals(newTitle)) {
      CmsPropertyModification propMod =
          new CmsPropertyModification(
              getEntryId(), CmsClientProperty.PROPERTY_TITLE, newTitle, true);
  ...
  /**
   * Creates the list item widget for the given folder.
   *
   * <p>
   *
   * @param galleryFolder the gallery folder
  ...
  private CmsListItemWidget createListWidget(CmsGalleryFolderEntry galleryFolder) {
    ...
    if (galleryFolder.getOwnProperties().containsKey(CmsClientProperty.PROPERTY_TITLE)) {
      title =
          galleryFolder
              .getOwnProperties()
              .get(CmsClientProperty.PROPERTY_TITLE)
              .getStructureValue();
    } else {
    ...
    if ((CmsSitemapView.getInstance().getController().getEntryById(galleryFolder.getStructureId())
            == null)
        || CmsSitemapView.getInstance()
            .getController()
            .getEntryById(galleryFolder.getStructureId())
            .isEditable()) {
      result.addTitleStyleName(I_CmsSitemapLayoutBundle.INSTANCE.sitemapItemCss().itemTitle());
      result.setTitleEditable(true);
      result.setTitleEditHandler(
