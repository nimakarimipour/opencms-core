...
public class CmsEntityObserver implements ValueChangeHandler<CmsEntity> {
   ...
   */
  public void addEntityChangeListener(
      I_CmsEntityChangeListener changeListener, String changeScope) {

    if (m_observerdEntity == null) {
      throw new RuntimeException(
          "The Observer has been cleared, no listener registration possible.");
    }
    if (!m_changeListeners.containsKey(changeScope)) {
      ...
      if (changeScope != null) {
        ...
        m_scopeValues.put(
            changeScope, CmsContentDefinition.getValueForPath(m_observerdEntity, changeScope));
      }
    }
