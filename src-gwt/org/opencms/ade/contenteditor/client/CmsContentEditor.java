...
public final class CmsContentEditor extends CmsEditorBase {
  ...
  /**
   * CmsEntity change handler to watch for changes within given scopes and call the editor change
  ...
  class EditorChangeHandler implements ValueChangeHandler<CmsEntity> {
    ...
    /**
     * Removes this observer from the entities change handler registration and clears registered
     * listeners.
     *
     * <p>
     */
  ...
  private static final String ENTITY_ID_SELECTOR_PREFIX =
  ...
  private static final String FIELD_SELECTOR =
      "[" + CmsGwtConstants.ATTR_DATA_FIELD + "*=\"opencms://\"]";
  ...
  /**
   * Flag indicating the editor was opened as the stand alone version, not from within any other
   * module.
   */
  ...
  private CmsContentEditor() {
    ...
    super(
        (I_CmsContentServiceAsync) GWT.create(I_CmsContentService.class),
        new CmsDefaultWidgetService());
    m_service = (I_CmsContentServiceAsync) super.getService();
    String serviceUrl =
        CmsCoreProvider.get().link("org.opencms.ade.contenteditor.CmsContentService.gwt");
    ...
    addValidationChangeHandler(
        new ValueChangeHandler<CmsValidationContext>() {
   ...
   */
  public static void addEntityChangeListener(
      I_CmsEntityChangeListener changeListener, String changeScope) {
      ...
      if (isObserverExported()) {
        ...
        try {
          ...
          CmsDebugLog.getInstance()
              .printLine("Exception occured during listener registration" + e.getMessage());
        }
      } else {
   ...
   * @param changeScope the change scope
   */
  static native void addNativeListener(
      I_CmsEntityChangeListener changeListener, String changeScope) /*-{
  ...
  /**
   * Checks whether the add entity change listener method has been exported.
   *
   * <p>
   *
   ...
   * Closes the editor.
   *
   * <p>May be used from outside the editor module.
   *
   * <p>
   */
        ...
        CmsConfirmDialog dialog =
            new CmsConfirmDialog(
                org.opencms.gwt.client.Messages.get()
                    .key(org.opencms.gwt.client.Messages.GUI_DIALOG_RESET_TITLE_0),
  ...
  /**
   * Bypasses a focus bug in IE which can happen if the user opens the HTML code editor from the
   * WYSIWYG editor.
   *
   * <p>The next time they open the editor form from the same container page, the user may be unable
   * to focus on any input fields. To prevent this, we create a dummy input field outside the
   * visible screen region and focus it when opening the editor.
   */
  ...
  public void loadDefinition(
    ...
    CmsRpcAction<CmsContentDefinition> action =
        new CmsRpcAction<CmsContentDefinition>() {
   ...
   *     yet
   * @param editHandlerData the data for the edit handler, if one is used to create a new content;
   *     null otherwise
    ...
    CmsRpcAction<CmsContentDefinition> action =
        new CmsRpcAction<CmsContentDefinition>() {
          ...
          public void execute() {
            ...
            getService()
                .loadNewDefinition(
                    entityId, m_clientId, editedEntity, getSkipPaths(), presets, this);
          }
   ...
   * @param mainLocale the main language to copy in case the element language node does not exist
   *     yet
   * @param editHandlerData the edit handler data, if we are using an edit handler to create a new
      ...
      definition =
          (CmsContentDefinition)
              CmsRpcPrefetcher.getSerializedObjectFromDictionary(
                  getService(), I_CmsContentService.DICT_CONTENT_DEFINITION);
  ...
  public void registerContentDefinition(CmsContentDefinition definition) {
    ...
    for (Map.Entry<String, CmsComplexWidgetData> entry :
        definition.getComplexWidgetData().entrySet()) {
      String attrName = entry.getKey();
      CmsComplexWidgetData widgetData = entry.getValue();
      getWidgetService()
          .registerComplexWidgetAttribute(
              attrName, widgetData.getRendererName(), widgetData.getConfiguration());
    }
  }
  ...
  public void saveAndDeleteEntities(
      final boolean clearOnSuccess, final I_CmsSimpleCallback<Boolean> callback) {

    final CmsEntity entity = m_entityBackend.getEntity(m_entityId);
    saveAndDeleteEntities(
        entity, new ArrayList<String>(m_deletedEntities), clearOnSuccess, callback);
  }
  ...
  public void saveAndDeleteEntities(
    ...
    CmsRpcAction<CmsSaveResult> asyncCallback =
        new CmsRpcAction<CmsSaveResult>() {
  ...
  /**
   * Adds a content definition to the internal store.
   *
   * <p>
   *
            ...
            public void execute() {
              ...
              getService()
                  .callEditorChangeHandlers(
                      getEntityId(), currentState, getSkipPaths(), scopesToSend, this);
            }
  ...
  /**
   * Checks if the content is valid and sends a notification if not.
   *
   * <p>This will not trigger a validation run, but will use the latest state.
   *
   * <p>
   *
  ...
  /**
   * Asks the user to confirm resetting all changes.
   *
   * <p>
   */
  void confirmCancel() {
      ...
      CmsConfirmDialog dialog =
          new CmsConfirmDialog(
              org.opencms.gwt.client.Messages.get()
                  .key(org.opencms.gwt.client.Messages.GUI_DIALOG_RESET_TITLE_0),
              org.opencms.gwt.client.Messages.get()
                  .key(org.opencms.gwt.client.Messages.GUI_DIALOG_RESET_TEXT_0));
      dialog.setHandler(
          new I_CmsConfirmDialogHandler() {
  ...
  /**
   * Opens the confirm delete locale dialog.
   *
   * <p>
   */
  ...
  /**
   * Copies the current entity values to the given locales.
   *
   * <p>
   *
  ...
  /**
   * Deferrers the save action to the end of the browser event queue.
   *
   * <p>
   */
  void deferSave() {
  ...
  /**
   * Deferrers the save and exit action to the end of the browser event queue.
   *
   * <p>
   */
  ...
  /**
   * Disables the save buttons with the given message.
   *
   * <p>
   *
  ...
  /**
   * Leaves the editor saving the content if necessary.
   *
   * <p>
   */
  void exitWithSaving() {
  ...
  void hideHelpBubbles(boolean hide) {
    ...
    if (!hide) {
      m_hideHelpBubblesButton.setTitle(
          Messages.get().key(Messages.GUI_TOOLBAR_HELP_BUBBLES_SHOWN_0));
    } else {
      m_hideHelpBubblesButton.setTitle(
          Messages.get().key(Messages.GUI_TOOLBAR_HELP_BUBBLES_HIDDEN_0));
    }
  }
    ...
    if (inline && (formParent != null)) {
      if ((mainLocale != null)
          && (CmsDomUtil.querySelector(
              ...
              == null)) {
        // in case a main locale is given and there are not any HTML elements attributed to the
        // current entity id,
        ...
        if (elements.getLength() > 0) {
          for (int i = 0; i < elements.getLength(); i++) {
            ...
            element.setAttribute(
                CmsGwtConstants.ATTR_DATA_ID,
                element
                    .getAttribute(CmsGwtConstants.ATTR_DATA_ID)
                    .replace(mainLocaleEntityId, m_entityId));
          }
        }
      }
  ...
  /**
   * Initializes the editor change handler.
   *
   * <p>
   *
  ...
  /**
   * Initializes the entity observer.
   *
   * <p>
   */
  ...
  void initFormPanel() {
    ...
    if (m_isStandAlone) {
      RootPanel.getBodyElement()
          .addClassName(I_CmsLayoutBundle.INSTANCE.editorCss().standAloneEditor());
    } else {
  ...
  /**
   * Opens the copy locale dialog.
   *
   * <p>
   */
  void openCopyLocaleDialog() {

    CmsCopyLocaleDialog dialog =
        new CmsCopyLocaleDialog(
            m_availableLocales,
            m_contentLocales,
            m_locale,
  ...
  /**
   * Opens the model file select dialog.
   *
   * <p>
   *
   * @param context the editor context
   * @param definition the content definition
   */
  void openModelSelectDialog(
      final CmsEditorContext context, final CmsContentDefinition definition) {
    ...
    String title =
        org.opencms.gwt.client.Messages.get()
            .key(org.opencms.gwt.client.Messages.GUI_MODEL_SELECT_TITLE_0);
    String message =
        org.opencms.gwt.client.Messages.get()
            .key(org.opencms.gwt.client.Messages.GUI_MODEL_SELECT_MESSAGE_0);
    CmsModelSelectDialog dialog =
        new CmsModelSelectDialog(handler, definition.getModelInfos(), title, message);
    dialog.center();
  }

  /**
   * Previews the native event to enable keyboard short cuts.
   *
   * <p>
   *
  ...
  void renderFormContent() {
    ...
    content.setStyleName(
        org.opencms.acacia.client.css.I_CmsLayoutBundle.INSTANCE.form().formParent());
          ...
          .addSelectionHandler(
              new SelectionHandler<Integer>() {
                ...
                public void onSelection(SelectionEvent<Integer> event) {
                  ...
                  updateInfoHeader(
                      CmsContentDefinition.SETTINGS_TAB_ID.equals(getFormTabs().getSelectedId()));
                }
              });
    }
  }
  ...
  /**
   * Saves the content and closes the editor.
   *
   * <p>
   */
  void save() {
  ...
  /**
   * Sets the has changed flag and enables the save button.
   *
   * <p>
   */
  void setChanged() {
  ...
  void setContentDefinition(CmsContentDefinition definition) {
    ...
    addEntityChangeHandler(
        ...
        new ValueChangeHandler<CmsEntity>() {
  ...
  /**
   * Removes the delete on cancel flag for new resources.
   *
   * <p>
   */
  void setSaved() {
  ...
  /**
   * Enables and disabler the undo redo buttons according to the state.
   *
   * <p>
   *
  ...
  /**
   * Returns true if the edited resource should be unlocked automatically after pressing Save/Exit.
   *
   * <p>
   *
  ...
  /**
   * Shows the locked resource error message.
   *
   * <p>
   */
  void showLockedResourceMessage() {
    ...
    CmsErrorDialog dialog =
        new CmsErrorDialog(
            Messages.get().key(Messages.ERR_RESOURCE_ALREADY_LOCKED_BY_OTHER_USER_0), null);
  ...
  void showValidationErrorDialog(CmsValidationResult validationResult) {
    ...
    CmsErrorDialog dialog =
        new CmsErrorDialog(Messages.get().key(Messages.GUI_VALIDATION_ERROR_1, errorLocales), null);
    dialog.center();
  }

  /**
   * Switches to the selected locale. Will save changes first.
   *
   * <p>
   *
  ...
  /**
   * Synchronizes the locale independent fields to the other locales.
   *
   * <p>
   */
  ...
  /**
   * Adds the change listener to the observer.
   *
   * <p>
   *
   * @param changeListener the change listener
  ...
  private void addChangeListener(JavaScriptObject changeListener, String changeScope) {
    ...
    try {
      ...
      m_entityObserver.addEntityChangeListener(
          new CmsEntityChangeListenerWrapper(changeListener), changeScope);
    } catch (Exception e) {
  ...
  /**
   * Changes a simple type entity value.
   *
   * <p>
   *
  ...
  private void changeSimpleValue(
      String attributeName, int index, String value, List<String> parentPathElements) {
  ...
  /**
   * Creates a push button for the edit tool-bar.
   *
   * <p>
   *
  ...
  /**
   * Exports the add entity change listener method.
   *
   * <p>
   */
  ...
  /**
   * Returns the attribute handler for the given attribute.
   *
   * <p>
   *
   * @param attributeName the attribute name
   ...
   */
  private CmsAttributeHandler getAttributeHandler(
      String attributeName, List<String> parentPathElements) {
    ...
    CmsAttributeHandler handler =
        getRootAttributeHandler()
            .getHandlerByPath(childPathElements.toArray(new String[childPathElements.size()]));
    return handler;
  }

  /**
   * Returns the entity id for the given locale.
   *
   * <p>
   *
  ...
  private String getIdForLocale(String locale) {
    ...
    return CmsContentDefinition.uuidToEntityId(
        CmsContentDefinition.entityIdToUuid(m_entityId), locale);
  }
  ...
  /**
   * Initializes the window closing handler to ensure the resource will be unlocked when leaving the
   * editor.
   *
   * <p>
   */
    ...
    m_closingHandlerRegistration =
        Window.addWindowClosingHandler(
            new ClosingHandler() {
              ...
              /**
               * @see
               *     com.google.gwt.user.client.Window.ClosingHandler#onWindowClosing(com.google.gwt.user.client.Window.ClosingEvent)
               */
  ...
  /**
   * Initializes the event preview handler.
   *
   * <p>
   */
  ...
  /**
   * Initializes the locale selector.
   *
   * <p>
   */
  private void initLocaleSelect() {
    ...
    if (m_localeSelect == null) {
      ...
      m_localeSelect.addValueChangeHandler(
    ...
    if (m_contentLocales.size() > 1) {
      ...
      m_deleteLocaleButton.disable(
          Messages.get().key(Messages.GUI_TOOLBAR_CANT_DELETE_LAST_LOCALE_0));
    }
    if (m_copyLocaleButton == null) {
      ...
      m_copyLocaleButton.addClickHandler(
          new ClickHandler() {
  ...
  /**
   * Generates the button bar displayed beneath the editable fields.
   *
   * <p>
   */
  private void initToolbar() {
    ...
    m_publishButton.addClickHandler(
        new ClickHandler() {
                ...
                new I_CmsSimpleCallback<Boolean>() {
                  ...
                  public void execute(final Boolean hasChangedSeetings) {
                    ...
                    HashMap<String, String> params =
                        new HashMap<String, String>(getContext().getPublishParameters());
    ...
    m_saveExitButton =
        createButton(
            Messages.get().key(Messages.GUI_TOOLBAR_SAVE_AND_EXIT_0), "opencms-icon-save-exit");
    m_saveExitButton.addClickHandler(
        new ClickHandler() {
    ...
    disableSave(Messages.get().key(Messages.GUI_TOOLBAR_NOTHING_CHANGED_0));
    m_undoButton =
        createButton(Messages.get().key(Messages.GUI_TOOLBAR_UNDO_0), "opencms-icon-undo");
    m_undoButton.addClickHandler(
        new ClickHandler() {
    ...
    m_redoButton =
        createButton(Messages.get().key(Messages.GUI_TOOLBAR_REDO_0), "opencms-icon-redo");
    m_redoButton.addClickHandler(
        new ClickHandler() {
    ...
    m_undoRedoHandlerRegistration =
        CmsUndoRedoHandler.getInstance()
    ...
    m_openFormButton.addClickHandler(
        new ClickHandler() {
    ...
    m_hideHelpBubblesButton.addClickHandler(
        new ClickHandler() {
    ...
    CmsValueFocusHandler.getInstance()
        .hideHelpBubbles(RootPanel.get(), !CmsCoreProvider.get().isShowEditorHelp());
  ...
  /**
   * Sets the resource info to native window context variables.
   *
   * <p>
   *
  ...
  /**
   * Updates the editor values according to the given entity.
   *
   * <p>
   *
   * @param previous the previous entity state
  ...
  private void updateEditorValues(
      CmsEntity previous, CmsEntity updated, CmsEntity target, List<String> parentPathElements) {
          ...
          if ((updatedAttribute.getValueCount() == previousAttribute.getValueCount())
            ...
            for (int i = 0; i < updatedAttribute.getValueCount(); i++) {
              if (!updatedAttribute
                      .getSimpleValues()
                      .get(i)
                      .equals(previousAttribute.getSimpleValues().get(i))
                  && previousAttribute
                      .getSimpleValues()
                      .get(i)
            ...
            if (targetAttribute.getValueCount() == previousAttribute.getValueCount()) {
              // only act, if the value count has not been altered while executing the server
              // request
              if (updatedAttribute.getValueCount() > previousAttribute.getValueCount()) {
                ...
                if (i >= previousAttribute.getSimpleValues().size()) {
                  handler.addNewAttributeValue(
                      m_entityBackend.registerEntity(
                          updatedAttribute.getComplexValues().get(i), true));
                } else {
                  List<String> childPathElements = new ArrayList<String>(parentPathElements);
