...
public class CmsContainerpageDNDController implements I_CmsDNDController {
  ...
  /**
   * DND controller for images. We set this in onDragStart if we are dragging an image, and then
   * delegate most of the other method calls to it if it is set.
   */
  ...
  public boolean onDragStart(
      final I_CmsDraggable draggable, I_CmsDropTarget target, final CmsDNDHandler handler) {
    ...
    if (isImage(draggable)) {
      ...
      // We are going to delegate further method calls to this, and set it to null again if the
      // image is dragged or the
    ...
    if (!m_controller.isGroupcontainerEditing()) {
      m_controller.lockContainerpage(
    ...
    if (isNewId(clientId)) {
      // for new content elements dragged from the gallery menu, the given id contains the resource
      // type name
      m_isNew = true;
      m_controller.getNewElement(clientId, callback);
    } else {
      m_controller.getElementForDragAndDropFromContainer(
          clientId, m_originalContainerId, false, callback);
    }
    if (!(target instanceof CmsContainerPageContainer)) {
   ...
   */
  public void onDrop(
      final I_CmsDraggable draggable, final I_CmsDropTarget target, CmsDNDHandler handler) {
    ...
    if (target != m_initialDropTarget) {
      if (target instanceof I_CmsDropContainer) {
        ...
        if (!m_isNew && (draggable instanceof CmsListItem)) {
          ...
          AsyncCallback<String> modeCallback =
          ...
          if (reuseMode != ElementReuseMode.reuse) {
            ...
            if ((cachedElementData != null)
              ...
              // User is not allowed to create this element in current view, so reuse the element
              // instead
          ...
          switch (reuseMode) {
            case ask:
              ...
              // element should
              // be used, or a copy of it. If the user wants a copy, we copy the corresponding
              // resource and replace the element
              // in the page
  ...
  protected void prepareHelperElements(
      CmsContainerElementData elementData, CmsDNDHandler handler, I_CmsDraggable draggable) {
    ...
    if ((elementData.getDndId() != null)
        && (m_controller.getCachedElement(elementData.getDndId()) != null)) {
    ...
    if (m_controller.isGroupcontainerEditing()) {
      ...
      if ((groupContainer != m_initialDropTarget)
        ...
        try {
          ...
          // ensure any embedded flash players are set opaque so UI elements may be placed above
          // them
      ...
      for (CmsContainerPageContainer container : m_controller.getContainerTargets().values()) {
        ...
        if ((container != m_initialDropTarget)
          ...
          if (elementData.isGroupContainer() || elementData.isInheritContainer()) {
            ...
            if ((cssResources != null) && !cssResources.isEmpty()) {
              // the element requires certain CSS resources, check if present and include if
              // necessary
              for (String cssResourceLink : cssResources) {
                CmsDomUtil.ensureStyleSheetIncluded(cssResourceLink);
              }
  ...
  /**
   * Inserts e new container element into the container once the drag and drop is finished.
   *
   * <p>
   *
  ...
  void insertDropElement(
    ...
    try {
      CmsContainerPageElementPanel containerElement =
          m_controller.getContainerpageUtil().createElement(elementData, container, m_isNew);
      if (m_isNew) {
      ...
      if (m_controller.isGroupcontainerEditing()) {
        container
            .getElement()
            .removeClassName(I_CmsLayoutBundle.INSTANCE.containerpageCss().emptyGroupContainer());
      }
  ...
  /**
   * Checks and sets if the place holder should be visible.
   *
   * <p>
   *
  ...
  /**
   * Checks if the placeholder position has changed.
   *
   * <p>
   *
  ...
  /**
   * Initializes the nested container infos.
   *
   * <p>
   */
  private void initNestedContainers() {
    ...
    for (CmsContainer container : m_controller.getContainers().values()) {
      if (container.isSubContainer()) {
        CmsContainerPageContainer containerWidget =
            m_controller.m_targetContainers.get(container.getName());
        // check if the sub container is a valid drop targets
        if (m_dragInfos.keySet().contains(containerWidget)) {
          CmsContainer parentContainer =
              m_controller.getContainers().get(container.getParentContainerName());
          // add the container to all it's ancestors as a dnd child
          while (parentContainer != null) {
            if (m_dragInfos
                .keySet()
                .contains(m_controller.m_targetContainers.get(parentContainer.getName()))) {
              m_controller
                  .m_targetContainers
                  .get(parentContainer.getName())
                  .addDndChild(containerWidget);
            }
            if (parentContainer.isSubContainer()) {
              parentContainer =
                  m_controller.getContainers().get(parentContainer.getParentContainerName());
            } else {
  ...
  /**
   * Checks whether the current placeholder position represents a change to the original draggable
  ...
  /**
   * Checks if the given draggable item is an image.
   *
   * <p>
   *
  ...
  /**
   * Sets styles of helper elements, appends the to the drop target and puts them into a drag info
   * bean.
   *
   * <p>
   *
  ...
  private void prepareDragInfo(
      Element placeholder, I_CmsDropContainer target, CmsDNDHandler handler) {

    target.getElement().addClassName(I_CmsLayoutBundle.INSTANCE.dragdropCss().dragging());
    String positioning =
        CmsDomUtil.getCurrentStyle(
            target.getElement(), org.opencms.gwt.client.util.CmsDomUtil.Style.position);
    ...
    if (!Position.ABSOLUTE.getCssName().equals(positioning)
      ...
      // check for empty containers that don't have a minimum top and bottom margin to avoid
      // containers overlapping
      if (target.getElement().getFirstChildElement() == null) {
        if (CmsDomUtil.getCurrentStyleInt(target.getElement(), CmsDomUtil.Style.marginTop)
            < MINIMUM_CONTAINER_MARGIN) {
          target.getElement().getStyle().setMarginTop(MINIMUM_CONTAINER_MARGIN, Unit.PX);
        }
        if (CmsDomUtil.getCurrentStyleInt(target.getElement(), CmsDomUtil.Style.marginBottom)
            < MINIMUM_CONTAINER_MARGIN) {
  ...
  private void prepareTargetContainer(
      I_CmsDropContainer targetContainer, I_CmsDraggable draggable, Element placeholder) {

    String positioning =
        CmsDomUtil.getCurrentStyle(
            targetContainer.getElement(), org.opencms.gwt.client.util.CmsDomUtil.Style.position);
    // set target relative, if not absolute or fixed
    if (!Position.ABSOLUTE.getCssName().equals(positioning)
        && !Position.FIXED.getCssName().equals(positioning)) {
  ...
  /**
   * Updates the drag target highlighting.
   *
   * <p>
   *
  ...
  private void updateHighlighting(boolean initial) {
    ...
    Map<I_CmsDropContainer, CmsPositionBean> containers =
        new HashMap<I_CmsDropContainer, CmsPositionBean>();
    for (I_CmsDropTarget target : m_dragInfos.keySet()) {
      if ((target instanceof I_CmsDropContainer)
          && !Display.NONE
              .getCssName()
        ...
        containers.put(
            (I_CmsDropContainer) target, ((I_CmsDropContainer) target).getPositionInfo());
      }
    }
    List<I_CmsDropContainer> containersToMatch =
        new ArrayList<I_CmsDropContainer>(containers.keySet());
