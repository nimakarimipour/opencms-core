...
public class CmsContainerpageHandler extends A_CmsToolbarHandler {
  ...
  public CmsContainerpageHandler(
      CmsContainerpageController controller, CmsContainerpageEditor editor) {
  ...
  public CmsContextMenuEntry createSimpleContextMenuEntry(
      CmsUUID structureId, String name, boolean checked, final Runnable action) {
    ...
    m_controller.getElementSettingsConfig(
        ...
        new I_CmsSimpleCallback<CmsElementSettingsConfig>() {
          ...
          public void execute(final CmsElementSettingsConfig settingsConfig) {
            ...
            try {
              CmsElementSettingsDialog dialog =
                  new CmsElementSettingsDialog(
                      m_controller, elementWidget, settingsConfig, settingPresets);
              dialog.show();
              ...
              CmsAlertDialog alert =
                  new CmsAlertDialog(
                      org.opencms.gwt.client.Messages.get()
                          .key(org.opencms.gwt.client.Messages.GUI_ERROR_0),
          ...
          /**
           * Gets the map of setting presets.
           *
           * <p>
           *
           ...
           */
          private Map<String, String> getSettingPresets(
              CmsContainerPageElementPanel elementWidget2) {
  ...
  public void enableToolbarButtons() {
    ...
    m_editor.enableToolbarButtons(
        m_controller.hasPageChanged(), m_controller.getData().getNoEditReason());
  }
  ...
  public void gotoSitemap() {
    ...
    if (sitemapUri.equals("")) {
      return; // normally, we shouldn't even get to this point because the sitemap button should be
              // disabled
    }
   ...
   * @param structureId the structure id of the resource for which the context menu entries should
   *     be generated
   */
  ...
  public void leavePage(final Command leavingCommand) {
    ...
    leavingDialog.setHandler(
        new I_CmsAcceptDeclineCancelHandler() {
  ...
  public void loadFavorites() {
    ...
    m_controller.loadFavorites(
        new I_CmsSimpleCallback<List<CmsContainerElementData>>() {
  ...
  public void onLockFail(String errorMessage) {
    ...
    CmsAlertDialog alert =
        new CmsAlertDialog(
            Messages.get().key(Messages.ERR_LOCK_TITLE_RESOURCE_LOCKED_0), errorMessage);
   ...
   */
  public void openEditorForElement(
      final CmsContainerPageElementPanel element, boolean inline, boolean wasNew) {

    if (element.isNew()) {
      // openEditorForElement will be called again asynchronously when the RPC for creating the
    ...
    if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(element.getNoEditReason())) {
      CmsNotification.get()
          .send(
              CmsNotification.Type.WARNING, "should be deactivated: " + element.getNoEditReason());
      return;
    }

    if (CmsDomUtil.hasClass(
        CmsContainerElement.CLASS_GROUP_CONTAINER_ELEMENT_MARKER, element.getElement())) {
  ...
  public void openEditorForNewListContent(
      CmsListElementCreationOption option, String reloadId, String postCreateHandler) {

    I_CmsEditableData editableData =
  ...
  public void openListAddDialog(CmsUUID structureId, String listAddMetadata) {
    ...
    CmsRpcAction<CmsListElementCreationDialogData> action =
        new CmsRpcAction<CmsListElementCreationDialogData>() {

          @Override
          public void execute() {

            start(0, false);
            CmsContainerpageHandler.this
                .m_controller
                .getContainerpageService()
                .getListElementCreationOptions(structureId, listAddMetadata, this);
          }
          ...
          protected void onResponse(CmsListElementCreationDialogData result) {
            ...
            if (result.getOptions().size() == 1) {
              ...
              openEditorForNewListContent(
                  result.getOptions().get(0), "" + structureId, result.getPostCreateHandler());
            } else {
              CmsListAddDialog dialog =
                  new CmsListAddDialog(
                      structureId,
                      result,
                      option ->
                          openEditorForNewListContent(
                              option, "" + structureId, result.getPostCreateHandler()));
              dialog.center();
            }
          }
        };
  ...
  public void openModelResourceSelect(
      final CmsContainerPageElementPanel element, List<CmsModelResourceInfo> modelResources) {
  ...
  public void refreshResource(CmsUUID structureId) {
    ...
    CmsConfirmDialog dialog =
        new CmsConfirmDialog(
            Messages.get().key(Messages.GUI_DIALOG_RELOAD_TITLE_0), message.toString());
  ...
  public void replaceElement(
      CmsContainerPageElementPanel element, String elementId, Runnable callback) {
   ...
   * @see
   *     org.opencms.ade.containerpage.client.CmsContainerpageController#saveFavoriteList(java.util.List)
   */
    ...
    if (dropContainer instanceof CmsContainerPageContainer) {
      ...
      String pageRootPath =
          CmsStringUtil.joinPaths(
              CmsCoreProvider.get().getSiteRoot(), CmsCoreProvider.get().getUri());
   ...
   * <p>
   *
   * @param buttonLeft the selection button absolute left position, needed to show the leave preview
  ...
  public void showPreview(int buttonLeft) {
    ...
    if (detailId != null) {
      // current URL may no longer be valid because the field mapped to URL name may have been
      // edited.
          ...
          new CmsRpcAction<String>() {
            ...
            public void execute() {
              ...
              CmsCoreProvider.getVfsService()
                  .getDetailName(detailId, CmsCoreProvider.get().getLocale(), this);
            }
  ...
  public void showPublishDialog() {
    ...
    if (m_controller.hasPageChanged()) {
      ...
      warningMessage.append(
          "<p>" + Messages.get().key(Messages.GUI_DIALOG_PUBLISH_NOT_SAVED_0) + "</p>");
      warningMessage.append(
          "<p>" + Messages.get().key(Messages.GUI_DIALOG_SAVE_QUESTION_0) + "</p>");

      CmsAcceptDeclineCancelDialog leavingDialog =
          new CmsAcceptDeclineCancelDialog(
              Messages.get().key(Messages.GUI_DIALOG_NOT_SAVED_TITLE_0), warningMessage.toString());
  ...
  /**
   * @see
   *     org.opencms.gwt.client.ui.A_CmsToolbarHandler#transformSingleEntry(org.opencms.util.CmsUUID,
   *     org.opencms.gwt.shared.CmsContextMenuEntryBean)
   */
  @Override
  public I_CmsContextMenuEntry transformSingleEntry(
  ...
  public void updateClipboard(Map<String, CmsContainerElementData> elements) {
    ...
    if (m_editor.getClipboard().isOpen()) {
      for (CmsContainerElementData elementData : elements.values()) {
        m_editor
            .getClipboard()
            .replaceFavoriteItem(m_controller.getContainerpageUtil().createListItem(elementData));
        m_editor
            .getClipboard()
            .replaceRecentItem(m_controller.getContainerpageUtil().createListItem(elementData));
      }
    }
  }

  /**
   * Creates the element view selection menu entry, returns <code>null</code> in case no other views
   * available.
   *
   * <p>
   *
  ...
  protected I_CmsContextMenuEntry createElementViewSelectionMenuEntry() {
    ...
    List<CmsElementViewInfo> elementViews = m_controller.getData().getElementViews();
    if (elementViews.size() > 1) {
      CmsContextMenuEntry parentEntry =
          new CmsContextMenuEntry(
              this,
              null,
        ...
        if (viewInfo.isRoot()) {
          viewEntries.add(
              createMenuEntryForElementView(
                  viewInfo, m_controller.matchRootView(viewInfo.getElementViewId()), this));
        }
      }
  ...
  protected I_CmsContextMenuEntry createTemplateContextSelectionMenuEntry(CmsUUID structureId) {
    ...
    if ((info.getCookieName() != null) && info.shouldShowTemplateContextContextMenuEntry()) {
      ...
      parentBean.setLabel(
          org.opencms.gwt.client.Messages.get()
              .key(org.opencms.gwt.client.Messages.GUI_TEMPLATE_CONTEXT_PARENT_0));
      parentBean.setActive(true);
      parentBean.setVisible(true);
      parentEntry.setBean(parentBean);
      ...
      for (Map.Entry<String, String> entry : contextNames.entrySet()) {
        ...
        if (info.hasClientVariants(key)) {
          ...
          CmsContextMenuEntry editVariantEntry =
              createMenuEntryForTemplateContext(
                  info.getCookieName(),
                  key,
                  org.opencms.ade.containerpage.client.Messages.get()
                      .key(
                          org.opencms.ade.containerpage.client.Messages
                              .GUI_TEMPLATE_CONTEXT_NO_VARIANT_0),
                  false,
                  this,
                  structureId);
          ...
          for (CmsClientVariantInfo variant : variants.values()) {
            CmsContextMenuEntry currentVariantEntry =
                createRawMenuEntry(structureId, new ClientVariantSelectAction(key, variant));
      ...
      templateContextEntries.add(
          createMenuEntryForTemplateContext(
              ...
              null,
              org.opencms.gwt.client.Messages.get()
                  .key(org.opencms.gwt.client.Messages.GUI_TEMPLATE_CONTEXT_NONE_0),
  ...
  protected I_CmsContextMenuEntry createToggleEditSmallElementsMenuEntry() {
    ...
    entryBean.setIconClass(
        isActive ? inputCss.checkBoxImageChecked() : inputCss.checkBoxImageUnchecked());
    I_CmsContextMenuCommand command =
        new I_CmsContextMenuCommand() {
  ...
  /**
   * Creates the menu entry for a single element view.
   *
   * <p>
   *
   * @param elementView the element view
  ...
  private CmsContextMenuEntry createMenuEntryForElementView(
      final CmsElementViewInfo elementView, boolean isActive, I_CmsContextMenuHandler handler) {
  ...
  /**
   * Creates the real entries for the "Show locale" option.
   *
   * <p>
   *
  ...
  private I_CmsContextMenuEntry createShowLocaleMenuEntry() {
    ...
    Map<String, CmsLocaleLinkBean> localeLinkBeans =
        CmsContainerpageController.get().getData().getLocaleLinkBeans();
    ...
    parentBean.setLabel(
        org.opencms.ade.containerpage.client.Messages.get()
            .key(org.opencms.ade.containerpage.client.Messages.GUI_SHOW_LOCALE_0));
    parentBean.setActive(true);
    parentBean.setVisible(true);
    parentEntry.setBean(parentBean);
    ...
    for (Map.Entry<String, CmsLocaleLinkBean> entry : localeLinkBeans.entrySet()) {
      ...
      CmsContextMenuEntry childEntry =
          new CmsContextMenuEntry(
              this,
              null,
              new I_CmsContextMenuCommand() {
  ...
  private CmsAcceptDeclineCancelDialog getLeaveDialog() {
    ...
    CmsAcceptDeclineCancelDialog leavingDialog =
        new CmsAcceptDeclineCancelDialog(
            Messages.get().key(Messages.GUI_DIALOG_NOT_SAVED_TITLE_0), message.toString());
    leavingDialog.setAcceptText(Messages.get().key(Messages.GUI_BUTTON_SAVE_TEXT_0));
    leavingDialog.setDeclineText(Messages.get().key(Messages.GUI_BUTTON_DISCARD_TEXT_0));
    leavingDialog.setCloseText(Messages.get().key(Messages.GUI_BUTTON_RETURN_TEXT_0));
