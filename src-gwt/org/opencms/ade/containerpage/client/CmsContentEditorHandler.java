...
public class CmsContentEditorHandler implements I_CmsContentEditorHandler {
  ...
  public void onClose(
    ...
    if (m_replaceElement != null) {
      if ((m_handler.m_controller.getData().getDetailId() != null)
          && m_replaceElement
              .getId()
              .startsWith(m_handler.m_controller.getData().getDetailId().toString())) {
      ...
      m_handler.reloadElements(
          new String[] {m_currentElementId, m_dependingElementId}, checkPublishLocks);
      m_dependingElementId = null;
    } else {
  ...
  public void openDialog(
      final CmsContainerPageElementPanel element, final boolean inline, boolean wasNew) {
      ...
      m_handler.m_controller.getEditOptions(
          ...
          new I_CmsSimpleCallback<CmsDialogOptionsAndInfo>() {
            ...
            public void execute(CmsDialogOptionsAndInfo editOptions) {
              ...
              final I_CmsSimpleCallback<CmsUUID> editCallBack =
                  new I_CmsSimpleCallback<CmsUUID>() {
                    ...
                    public void execute(CmsUUID arg) {
                      ...
                      if (!element.getId().startsWith(arg.toString())) {
                        // the content structure ID has changed, the current element needs to be
                    ...
                    new CmsOptionDialog(
                        ...
                        new I_CmsSimpleCallback<String>() {
                          ...
                          public void execute(String arg) {
                            ...
                            m_handler.m_controller.prepareForEdit(
                                element.getId(), arg, editCallBack);
                          }
                        });
                dialog.addDialogClose(
                    new Command() {
   ...
   * @param handlerDataForNew the edit handler data, if we are using an edit handler to create a new
  ...
  /**
   * Adds a history item for the closed editor.
   *
   * <p>
   */
  ...
  /**
   * Gets the editor context to use for the Acacia editor.
   *
   * <p>
   *
  ...
  CmsEditorContext getEditorContext() {
    ...
    result
        ...
        .put(CmsPublishOptions.PARAM_CONTAINERPAGE, "" + CmsCoreProvider.get().getStructureId());
    result
        .getPublishParameters()
        .put(
    ...
    elemental2.dom.HTMLMetaElement meta =
        Js.cast(
    ...
    if (!m_editorOpened) {
          ...
          new Runnable() {
            ...
            public void run() {
              ...
              CmsContentEditorDialog.get()
                  .openEditDialog(
                      editableData, false, null, new DialogOptions(), CmsContentEditorHandler.this);
            }
          };
      ...
      if (m_handler.m_controller.getData().isUseClassicEditor() || element.isNewEditorDisabled()) {
        ...
        if (inline && CmsContentEditor.hasEditable(element.getElement())) {
          ...
          boolean allowSettings =
              m_handler.m_controller.getData().allowSettingsInEditor()
                  && !m_handler.m_controller.isEditingDisabled()
                  && !serverId.equals(
                      String.valueOf(m_handler.m_controller.getData().getDetailId()));
          I_CmsSimpleCallback<Boolean> openEditor =
  ...
  /**
   * Opens the XML content editor internally.
   *
   * <p>
   *
   * @param editableData the data of the element to edit
  ...
  void internalOpenDialog(
    ...
    if (m_handler.m_controller.getData().isUseClassicEditor()) {
      CmsContentEditorDialog.get()
          .openEditDialog(editableData, isNew, mode, new DialogOptions(), this);
    } else {
      String newLink = null;
      if (isNew) {
  ...
  /**
   * Adds a history item for the opened editor.
   *
   * <p>Use the prohibitReturn flag to deny a return to the opened editor through the browser
   * history. Use this feature for inline editing or when opening the editor for new resources.
   *
   * <p>
   *
   * @param prohibitReturn if <code>true</code> returning to the opened editor through the browser
   *     history is denied
   */
  ...
  /**
   * Returns the HTML context info for the given element.
   *
   * <p>
   *
  ...
  private String getContextInfo(CmsContainerPageElementPanel element) {
    ...
    if (m_handler.m_controller.isGroupcontainerEditing()) {
      container =
          (CmsContainerPageContainer)
              ((CmsContainerPageElementPanel) element.getParentTarget()).getParentTarget();
    } else {
    ...
    result.put(
        CmsCntPageData.JSONKEY_DETAILVIEW, JSONBoolean.getInstance(container.isDetailView()));
    result.put(
        CmsCntPageData.JSONKEY_ISDETAILVIEWCONTAINER,
        JSONBoolean.getInstance(container.isDetailViewContainer()));
    result.put(
        CmsCntPageData.JSONKEY_DETAILONLY, JSONBoolean.getInstance(container.isDetailOnly()));
