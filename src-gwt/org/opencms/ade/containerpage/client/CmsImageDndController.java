 ...
 *
 * <p>Since the image drag and drop logic is mostly separate from the container drag and drop logic,
 * the container page DND controller delegates most of the drag and drop logic to this class if it
 * detects that an image is being dragged.
 *
 * <p>
 */
public class CmsImageDndController implements I_CmsDNDController {
  ...
  class ImageDropTarget implements I_CmsDropTarget {
    ...
    public ImageDropTarget(
        Element element, Optional<CmsContainerPageElementPanel> containerElement) {

      m_element = element;
      m_border =
          new CmsHighlightingBorder(
              CmsPositionBean.getBoundingClientRect(m_element), BorderColor.red);
     ...
     *     org.opencms.gwt.client.dnd.CmsDNDHandler.Orientation)
     */
    ...
    /**
     * Cleans up widgets which are part of this drop target and are no longer needed.
     *
     * <p>
     */
    public void cleanup() {
     ...
     *     int, int, org.opencms.gwt.client.dnd.CmsDNDHandler.Orientation)
     */
    ...
    /**
     * Saves the image with the given path to the content to which this drop zone belongs.
     *
     * <p>
     *
              ...
              m_pageController.createNewElement(
                  ...
                  new AsyncCallback<CmsContainerElement>() {
                    ...
                    public void onSuccess(final CmsContainerElement result) {
                      ...
                      m_pageController.setPageChanged(
                          new Runnable() {

                            public void run() {

                              String serverId =
                                  CmsContainerpageController.getServerId(result.getClientId());
    ...
    /**
     * Saves the value to the content and reloads the corresponding container element.
     *
     * <p>
     *
     ...
     */
    protected void saveAndReloadElement(
        String contentId, String contentPath, String locale, String value) {
              ...
              new AsyncCallback<String>() {
                ...
                public void onSuccess(String result) {
                  ...
                  if (m_containerElement.isPresent()) {
                    ...
                    m_pageController.reloadElements(
                        new String[] {clientId},
                        () -> {
                          /*do nothing*/
                        });
  ...
  /**
   * Collects the valid drop targets for images from the page and initializes them.
   *
   * <p>
   *
  ...
  protected List<ImageDropTarget> findImageTargets() {
    ...
    List<CmsContainerPageElementPanel> modelGroups =
        CmsContainerpageController.get().getModelGroups();
    elementLoop:
    for (Element element :
        CmsDomUtil.nodeListToList(
            CmsDomUtil.querySelectorAll(
                "*[" + ATTR_DATA_IMAGEDND + "]", RootPanel.getBodyElement()))) {
      Optional<CmsContainerPageElementPanel> optElemWidget =
          CmsContainerpageController.get().getContainerElementWidgetForElement(element);
      ...
      if (!CmsContainerpageController.get().getData().isModelGroup()) {
        // Don't make images in model groups into drop targets, except when we are in model group
        // editing mode
  ...
  /**
   * Cleans up all the drop targets.
   *
   * <p>
   */
