...
public class CmsSmallElementsHandler {
  ...
  public static boolean isSmall(Widget widget) {
    ...
    return (CmsPositionBean.generatePositionInfo(widget.getElement()).getHeight()
            < NECESSARY_HEIGHT)
        && (CmsPositionBean.getBoundingClientRect(widget.getElement()).getHeight()
            < NECESSARY_HEIGHT);
  }
   ...
   * <p>
   *
   * @param editable if true, small elements will be enlarged and their edit buttons shown; if
   *     false, the edit buttons will be hidden
   */
