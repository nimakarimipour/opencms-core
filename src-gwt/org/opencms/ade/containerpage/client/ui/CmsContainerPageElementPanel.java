...
public class CmsContainerPageElementPanel extends AbsolutePanel
  ...
  /**
   * Parses CSS classess of the form 'oc-point-TY_LX', where X and Y are strings of decimal digits
   * possibly preceded by a minus sign.
   *
   * <p>The numeric values of Y and X will be available after a successful parse using the methods
   * getOffsetTop() and getOffsetLeft().
   *
   * <p>This is used to offer the formatter developer some control over the edit point positioning.
   */
  public static class PointPositioningParser {
    ...
    /**
     * Tries to parse a point positioning instruction from an element's class attribute and returns
  ...
  /**
   * Flag which indicates whether the new editor is disabled for this element.
   *
   * <p>
   */
  ...
  /**
   * Indicates if the current user has view permissions on the element resource. Without view
   * permissions, the element can neither be edited, nor moved.
   */
  ...
  /**
   * Indicates if the current user has write permissions on the element resource. Without write
   * permissions, the element can not be edited.
   */
   ...
   * @param hasViewPermission indicates if the current user has view permissions on the element
   *     resource
   * @param hasWritePermission indicates if the current user has write permissions on the element
   *     resource
   * @param releasedAndNotExpired <code>true</code> if the element resource is currently released
   *     and not expired
  ...
  /**
   * @see
   *     org.opencms.gwt.client.dnd.I_CmsDraggable#getDragHelper(org.opencms.gwt.client.dnd.I_CmsDropTarget)
   */
  public Element getDragHelper(I_CmsDropTarget target) {
    ...
    helper.addClassName(
        org.opencms.gwt.client.ui.css.I_CmsLayoutBundle.INSTANCE.generalCss().shadow());
    ...
    helper.addClassName(
        org.opencms.gwt.client.ui.css.I_CmsLayoutBundle.INSTANCE.listItemWidgetCss().dragging());
    ...
    m_provisionalParent
        .getStyle()
        .setZIndex(I_CmsLayoutBundle.INSTANCE.constants().css().zIndexDND());
  ...
  /**
   * @see
   *     org.opencms.gwt.client.dnd.I_CmsDraggable#getPlaceholder(org.opencms.gwt.client.dnd.I_CmsDropTarget)
   */
  ...
  public void initInlineEditor(final CmsContainerpageController controller) {
    ...
    if (CmsStringUtil.isEmptyOrWhitespaceOnly(m_noEditReason)
        && !m_disableNewEditor
        && CmsContentEditor.setEditable(
            getElement(), CmsContainerpageController.getServerId(m_clientId), true)) {
      if (m_editorClickHandlerRegistration != null) {
        m_editorClickHandlerRegistration.removeHandler();
      }
      m_editorClickHandlerRegistration =
                ...
                public void onPreviewNativeEvent(NativePreviewEvent event) {
                  ...
                  if (event.getTypeInt() == Event.ONCLICK) {
                    ...
                    if (getElement().isOrHasChild(eventTarget)) {
                      ...
                      Element parentContainerElement =
                          CmsDomUtil.getAncestor(
                              target, I_CmsLayoutBundle.INSTANCE.dragdropCss().dragElement());
                          ...
                          if (CmsContentEditor.isEditable(target)) {
                            ...
                            controller
                                ...
                                .openEditorForElement(
                                    CmsContainerPageElementPanel.this, true, isNew());
                            removeEditorHandler();
                            event.cancel();
                            break;
                          } else {
  ...
  /**
   * Checks if this element has 'createNew' status, i.e. will be copied when using this page as a
   * model for a new container page.
   *
   * <p>
   *
  ...
  public void removeInlineEditor() {
    ...
    CmsContentEditor.setEditable(
        getElement(), CmsContainerpageController.getServerId(m_clientId), false);
    removeEditorHandler();
  }
      ...
      while (it.hasNext()) {
        ...
        if (!editor.isValid()) {
          ...
          if (CmsDomUtil.hasDimension(editor.getElement().getParentElement())) {
            ...
            editor.setPosition(
                CmsDomUtil.getEditablePosition(entry.getValue().getMarkerTag()), getElement());
          } else {
  ...
  protected List<Element> getEditableElements() {
    ...
    List<Element> elems =
        CmsDomUtil.getElementsByClass(CmsGwtConstants.CLASS_EDITABLE, Tag.div, getElement());
                ...
                public void execute() {
                  ...
                  if (!hasCheckedHeight() && CmsSmallElementsHandler.isSmall(thisElement)) {
                    CmsContainerpageController.get()
                        ...
                        .prepareSmallElement(thisElement);
                  }
  ...
  /**
   * Adds the collector edit buttons.
   *
   * <p>
   *
  ...
  /**
   * Gets the container id of the most deeply nested container containing the given element, or null
   * if no such container can be found.
   *
   * <p>
   *
   ...
   * @return <code>true</code> if there are iframe child elements located no less than 25px below
   *     the upper edge of the element
   */
  ...
  /**
   * This method removes the option-bar widget from DOM and re-attaches it at it's original
   * position.
   *
   * <p>Use to avoid mouse-over and mouse-down malfunction.
   *
   * <p>
   */
