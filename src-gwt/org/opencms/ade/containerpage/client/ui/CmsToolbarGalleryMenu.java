...
public class CmsToolbarGalleryMenu extends A_CmsToolbarMenu<CmsContainerpageHandler> {
  ...
  /**
   * The gallery handler for the container page editor.
   *
   * <p>
   */
  class GalleryHandler implements I_CmsGalleryHandler {
    ...
    public void processResultItem(CmsResultListItem item) {
      ...
      if (item.getResult().isCopyModel()) {
        ...
        String newText =
            Messages.get().key(Messages.GUI_COPY_MODEL_TITLE_WRAPPER_1, titleWidget.getText());
  ...
  public void onToolbarActivate() {
    ...
    if (m_dialog == null) {
      ...
      if (CmsContainerpageController.get().getData().getTemplateContextInfo().getCurrentContext()
          != null) {
      ...
      CmsGalleryController galleryController =
          new CmsGalleryController(
              new CmsGalleryControllerHandler(galleryDialog), m_galleryData, m_search);
