...
public class CmsInheritanceContainerEditor extends A_CmsGroupEditor {
  ...
  private static final String HIDDEN_ELEMENT_CLASS =
      I_CmsLayoutBundle.INSTANCE.containerpageCss().hiddenElement();

  /** Css class to hide elements. */
  private static final String HIDDEN_ELEMENT_OVERLAY_CLASS =
      I_CmsLayoutBundle.INSTANCE.containerpageCss().hiddenElementOverlay();

  /** Css class to hide elements. */
  private static final String HIDE_ELEMENTS_CLASS =
      I_CmsLayoutBundle.INSTANCE.containerpageCss().hideElements();
    ...
    m_optionClickHandler =
        new ClickHandler() {
          ...
          public void onClick(ClickEvent event) {
            ...
            CmsDomUtil.ensureMouseOut(
                ((CmsPushButton) optionButton).getElement().getParentElement());
  ...
  public static CmsInheritanceContainerEditor openInheritanceContainerEditor(
    ...
    if (INSTANCE != null) {
      ...
      CmsInheritanceContainerEditor editor =
          new CmsInheritanceContainerEditor(groupContainer, controller, handler);
  ...
  /**
   * Method which should be called after the settings of an element in the inheritance containerhave
   * been edited.
   *
   * <p>
   */
  ...
  public void showElement(CmsContainerPageElementPanel elementWidget) {
    ...
    for (index = 0; index < getGroupContainerWidget().getWidgetCount(); index++) {
      if (CmsDomUtil.hasClass(
          HIDDEN_ELEMENT_CLASS, getGroupContainerWidget().getWidget(index).getElement())) {
        break;
      }
    }
    ...
    List<Element> elements =
        CmsDomUtil.getElementsByClass(
            HIDDEN_ELEMENT_OVERLAY_CLASS, Tag.div, elementWidget.getElement());
    for (Element element : elements) {
      element.removeFromParent();
    }
    if (CmsDomUtil.getElementsByClass(
            HIDDEN_ELEMENT_OVERLAY_CLASS, Tag.div, getGroupContainerWidget().getElement())
        .isEmpty()) {
      // if no other hidden elements present disable toggle button
      m_showElementsButton.disable(
          Messages.get().key(Messages.GUI_INHERITANCECONTAINER_NO_HIDDEN_ELEMENTS_0));
    }
    m_changedInheritanceInfo = true;
  }
  ...
  protected void addButtons() {
    ...
    m_showElementsButton.setText(
        Messages.get().key(Messages.GUI_INHERITANCECONTAINER_SHOW_HIDDEN_0));
    m_showElementsButton.setDownFace(
        Messages.get().key(Messages.GUI_INHERITANCECONTAINER_HIDE_ELEMENTS_0), null);
    m_showElementsButton.setUseMinWidth(true);
    m_showElementsButton.setButtonStyle(ButtonStyle.TEXT, ButtonColor.RED);
    m_showElementsButton.addClickHandler(
        new ClickHandler() {
  ...
  protected void breakUpContainer() {
    ...
    for (Widget w : getGroupContainerWidget()) {
      if (w instanceof CmsContainerPageElementPanel) {
        ...
        if ((elementWidget.getInheritanceInfo() == null)
            || elementWidget.getInheritanceInfo().isVisible()) {
      ...
      try {
        ...
        CmsContainerPageElementPanel containerElement =
            getController()
                ...
                .createElement(elementData, getParentContainer(), false);
  ...
  protected void closeDialog(boolean breakingUp) {
    ...
    getGroupContainerWidget()
        .removeStyleName(I_CmsLayoutBundle.INSTANCE.containerpageCss().hideElements());
  ...
  protected void saveEdit() {
    ...
    for (Widget widget : getGroupContainerWidget()) {
      if (widget instanceof CmsContainerPageElementPanel) {
        ...
        CmsContainerElement element =
            getController().getCachedElement(elementWidget.getId()).copy();
        ...
        if (info.isVisible()) {
          ...
          try {
            CmsContainerPageElementPanel elementWidget =
                util.createElement(element, getGroupContainerWidget(), false);
  ...
  /**
   * Creates an option bar for the given element.
   *
   * <p>
   *
   * @param elementWidget the element widget
  ...
  /**
   * Updates the visibility of the option bar buttons of the given element.
   *
   * <p>
   *
   * @param elementWidget the element widget
