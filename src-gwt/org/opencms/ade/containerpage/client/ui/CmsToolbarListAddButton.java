...
public class CmsToolbarListAddButton extends A_CmsToolbarOptionButton {
  ...
  public boolean isOptionAvailable(CmsContainerPageElementPanel element) {
    ...
    return !CmsContainerpageController.get().isEditingDisabled()
        && (getListAddInfo(element) != null);
  }
  ...
  /**
   * Validates that a list creation metadata element directly belongs to a given container element
   * and not to some other element in a nested container.
   *
  ...
  private boolean checkIfListDataBelongsToContainerElement(
      CmsContainerPageElementPanel containerElement, Element dataElement) {
  ...
  /**
   * Finds an element with the data-oc-listadd attribute in the container element and return the
   * attribute's content, or null if no element with that attribute exists in the content element.
   *
  ...
  private String getListAddInfo(CmsContainerPageElementPanel containerElement) {
    ...
    if (containerElement.isModelGroup()) {
      // in principle, the list element adding mechanism should also work when used inside a model
      // group,
      // but this could cause ambiguities/conflicts (multiple lists in a model group or similar
      // cases).
      return null;
    }
    // first try to find a sub-element with the necessary metadata in the DOM
    NodeList<Element> dataElements =
        element.querySelectorAll("[" + CmsGwtConstants.ATTR_DATA_LISTADD + "]");
    for (int i = 0; i < dataElements.getLength(); i++) {
      Element dataElement = dataElements.item(i);
      // just having an element with the data is not enough, we have to check if it's really part of
      // *this*
      ...
      if (checkIfListDataBelongsToContainerElement(containerElement, dataElement)) {
        // we do not currently support more than one list-add configuration - just return the first
        // one
