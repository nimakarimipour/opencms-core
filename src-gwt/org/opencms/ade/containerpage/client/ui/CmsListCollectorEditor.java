...
public class CmsListCollectorEditor extends A_CmsDirectEditButtons {
  ...
  /**
   * Returns true if the element view of the element is compatible with the currently set element
   * view in the container page editor.
   *
   * <p>
   *
  ...
  /**
   * @see
   *     org.opencms.gwt.client.ui.A_CmsDirectEditButtons#setPosition(org.opencms.gwt.client.util.CmsPositionBean,
  ...
  public void setPosition(CmsPositionBean position, Element containerElement) {
    ...
    if (top < 25) {
      ...
      Element parentOptionBar =
          CmsDomUtil.getFirstChildWithClass(
              containerElement, I_CmsLayoutBundle.INSTANCE.containerpageCss().optionBar());
  ...
  /**
   * Shows or hides the widget depending on the current view and whether the parent element has
   * width or height.
   *
   * <p>
   *
   * @param editableContainer true if this list element is part of an element in an editable
   *     container
   */
  ...
  protected void deleteElement() {
    ...
    CmsContainerpageController.get()
        .deleteElement(m_editableData.getStructureId().toString(), m_parentResourceId);
  }
  ...
  protected Map<Integer, CmsPushButton> getAdditionalButtons() {
    ...
    // only show add to favorites and info button, in case there actually is a resource and not in
    // case of create new only
  ...
  protected String getUploadButtonTitle(String uploadFolder) {
    ...
    return org.opencms.ade.galleries.client.Messages.get()
        .key(org.opencms.ade.galleries.client.Messages.GUI_GALLERY_UPLOAD_TITLE_1, uploadFolder);
  }
  ...
  protected void onClickDelete() {
    ...
    if (m_editableData.hasEditHandler()) {
      ...
      final I_CmsSimpleCallback<String> deleteCallback =
  ...
  protected void onClickNew(boolean askCreateMode) {
    ...
    if (!askCreateMode) {
      ...
      if (m_editableData.hasEditHandler()) {
        ...
        final I_CmsSimpleCallback<String> newCallback =
            new I_CmsSimpleCallback<String>() {

              public void execute(String choice) {

                CmsEditHandlerData data =
                    new CmsEditHandlerData(elementId, choice, pageId, requestParamStr);
                ...
                new I_CmsSimpleCallback<CmsDialogOptionsAndInfo>() {
                  ...
                  public void execute(CmsDialogOptionsAndInfo arg) {
                    ...
                    if (arg == null) {
                      ...
                      CmsOptionDialog dialog =
                          new CmsOptionDialog(null, arg.getOptions(), arg.getInfo(), newCallback);
                      dialog.center();
                    }
                  }
                });
          ...
          new CmsRpcAction<CmsListInfoBean>() {
            ...
            public void execute() {
              ...
              CmsCoreProvider.getVfsService()
                  .getUploadFolderInfo(m_editableData.getExtensions().getUploadFolder(), this);
            }
  ...
  protected void openEditDialog(boolean isNew, String mode, CmsEditHandlerData handlerDataForNew) {
    ...
    CmsContainerpageController.get()
        ...
        .openDialog(m_editableData, isNew, m_parentResourceId, mode, handlerDataForNew);
  }
  ...
  /**
   * Sets the display CSS property to none, or clears it, depending on the given parameter.
   *
   * <p>
   *
