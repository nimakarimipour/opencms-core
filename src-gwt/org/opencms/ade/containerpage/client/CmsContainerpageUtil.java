...
public class CmsContainerpageUtil {
  ...
  public CmsContainerpageUtil(
      CmsContainerpageController controller, A_CmsToolbarOptionButton... optionButtons) {
  ...
  public void addOptionBar(CmsContainerPageElementPanel element) {
    ...
    if (element.hasViewPermission()) {
      CmsElementOptionBar optionBar =
          CmsElementOptionBar.createOptionBarForElement(
              element, m_controller.getDndHandler(), m_optionButtons);
      element.setElementOptionBar(optionBar);
    }
  }

  /**
   * Transforms all contained elements into {@link CmsContainerPageElementPanel}.
   *
   * <p>
   *
  ...
  public void consumeContainerElements(I_CmsDropContainer container) {
    ...
    while (child != null) {
      boolean isContainerElement =
          CmsDomUtil.hasClass(CmsContainerElement.CLASS_CONTAINER_ELEMENT_START_MARKER, child);
      boolean isGroupcontainerElement =
          CmsDomUtil.hasClass(CmsContainerElement.CLASS_GROUP_CONTAINER_ELEMENT_MARKER, child);
      if (isContainerElement || isGroupcontainerElement) {
        ...
        if (isContainerElement) {
          ...
          if (CmsDomUtil.hasClass(
              CmsContainerElement.CLASS_CONTAINER_ELEMENT_START_MARKER, elementRoot)) {
          ...
          if (CmsDomUtil.hasClass(
              CmsContainerElement.CLASS_CONTAINER_ELEMENT_END_MARKER, elementRoot)) {
            ...
            // only if the end marker node is not null and has neither the end-marker class or
            // start-marker class
            // remove the current node and check the next sibling
            while (!((endMarker == null)
                || ((endMarker.getNodeType() == Node.ELEMENT_NODE)
                    && (CmsDomUtil.hasClass(
                            CmsContainerElement.CLASS_CONTAINER_ELEMENT_END_MARKER, endMarker)
                        || CmsDomUtil.hasClass(
            ...
            if (CmsDomUtil.hasClass(
                CmsContainerElement.CLASS_CONTAINER_ELEMENT_START_MARKER, endMarker)) {
              if (elementData != null) {
                alertParsingError(elementData.getSitePath());
              }
              // broken element, end marker missing
            ...
            CmsContainerPageElementPanel containerElement =
                createElement(elementRoot, container, elementData);
          ...
          CmsGroupContainerElementPanel groupContainer =
              createGroupcontainer(child, container, elementData);
        ...
        if (!containsElements
          ...
          // this element is no container element and is the container only child, assume it is an
          // empty container marker
  ...
  /**
   * The method will create {@link CmsContainerPageContainer} object for all given containers by
   * converting the associated DOM elements. The contained elements will be transformed into {@link
  ...
  public Map<String, CmsContainerPageContainer> consumeContainers(
      Map<String, CmsContainer> containers, Element context) {

    Map<String, CmsContainerPageContainer> result =
        new HashMap<String, CmsContainerPageContainer>();
    List<Element> containerElements =
        CmsDomUtil.getElementsByClass(CmsContainerElement.CLASS_CONTAINER, context);
    for (Element containerElement : containerElements) {
      ...
      try {
        ...
        try {
          CmsContainerPageContainer dragContainer =
              new CmsContainerPageContainer(container, containerElement);
  ...
  public CmsContainerPageElementPanel createElement(
      CmsContainerElementData containerElement, I_CmsDropContainer container, boolean isNew)
      throws Exception {
    ...
    Element element =
        CmsDomUtil.createElement(containerElement.getContents().get(container.getContainerId()));
  ...
  public CmsContainerPageElementPanel createGroupcontainerElement(
    ...
    CmsGroupContainerElementPanel groupContainer =
        createGroupcontainer(element, container, containerElement);
    groupContainer.setContainerId(container.getContainerId());
    // adding sub-elements
    ...
    while (it.hasNext()) {
      ...
      if (subElement.getContents().containsKey(container.getContainerId())) {
        CmsContainerPageElementPanel subDragElement =
            createElement(subElement, groupContainer, false);
  ...
  public CmsMenuListItem createListItem(final CmsContainerElementData containerElement) {
    ...
    if (!containerElement.isGroupContainer()
          ...
          new ClickHandler() {
            ...
            public void onClick(ClickEvent event) {
              ...
              getController()
                  ...
                  .openDialog(editableData, false, null, null, null);
      ...
      if (CmsStringUtil.isNotEmptyOrWhitespaceOnly(containerElement.getNoEditReason())) {
        ...
        listItem.disableEdit(
            Messages.get().key(Messages.GUI_CLIPBOARD_ITEM_CAN_NOT_BE_EDITED_0), false);
      }
    }
  ...
  /**
   * Displays the element parsing error dialog.
   *
   * <p>
   *
  ...
  private CmsContainerPageElementPanel createElement(
      Element element, I_CmsDropContainer dragParent, CmsContainerElement elementData) {

    CmsContainerPageElementPanel dragElement =
        new CmsContainerPageElementPanel(
            element,
            dragParent,
  ...
  /**
   * Creates a drag container element. This will not add an option-bar!
   *
   * <p>
   *
   * @param element the DOM element
  ...
  private CmsGroupContainerElementPanel createGroupcontainer(
    ...
    CmsGroupContainerElementPanel groupContainer =
        new CmsGroupContainerElementPanel(
            element,
            dragParent,
            elementData.getClientId(),
            elementData.getSitePath(),
