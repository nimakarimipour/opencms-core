...
public class CmsUploadButtonConnector extends ButtonConnector {
  ...
  /**
   * Creates a button handler according to the component state.
   *
   * <p>
   *
  ...
  private I_CmsUploadButtonHandler createButtonHandler() {
    ...
    switch (getState().getUploadType()) {
      case singlefile:
        buttonHandler =
            new CmsSingleFileUploadHandler(
                new ButtonContextSupplier(), getState().getDialogTitle());
        ((CmsSingleFileUploadHandler) buttonHandler)
            .setTargetFolderPath(getState().getTargetFolderRootPath());
        ((CmsSingleFileUploadHandler) buttonHandler)
            .setTargetFileName(getState().getTargetFileName());
        ((CmsSingleFileUploadHandler) buttonHandler)
            .setTargetFileNamePrefix(getState().getTargetFileNamePrefix());
        break;
      case multifile:
      default:
        ...
        ((CmsDialogUploadButtonHandler) buttonHandler)
            .setTargetFolder(getState().getTargetFolderRootPath());
        break;
    }
