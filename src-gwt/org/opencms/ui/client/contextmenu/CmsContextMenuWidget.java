...
public class CmsContextMenuWidget extends Widget {
  ...
  private final NativePreviewHandler m_nativeEventHandler =
      new NativePreviewHandler() {
        ...
        public void onPreviewNativeEvent(NativePreviewEvent event) {
          ...
          if (!targetsContextMenu
              && (nativeEvent.getTypeInt() == Event.ONMOUSEDOWN)
              && isHideAutomatically()) {
            hide();
          }
        }
  ...
  public void addRootMenuItem(ContextMenuItemState rootItem, CmsContextMenuConnector connector) {
    ...
    CmsContextMenuItemWidget itemWidget =
        createEmptyItemWidget(
            rootItem.getId(), rootItem.getCaption(), rootItem.getDescription(), connector);
  ...
  private CmsContextMenuItemWidget createEmptyItemWidget(
      String id, String caption, String description, CmsContextMenuConnector contextMenuConnector) {
    ...
    widget.setIcon(
        contextMenuConnector.getConnection().getIcon(contextMenuConnector.getResourceUrl(id)));

    CmsContextMenuItemWidgetHandler handler =
        new CmsContextMenuItemWidgetHandler(widget, contextMenuConnector);
  ...
  private void createSubMenu(
    ...
    CmsContextMenuItemWidget childWidget =
        createEmptyItemWidget(
            childState.getId(), childState.getCaption(), childState.getDescription(), connector);
  ...
  /**
   * Adds the given style names to the item widget.
   *
   * <p>
   *
   * @param item the item
