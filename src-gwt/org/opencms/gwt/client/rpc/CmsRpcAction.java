...
public abstract class CmsRpcAction<T> implements AsyncCallback<T> {
  ...
  /**
   * The sync token value, used to allow synchronous RPC calls within vaadin, see also
   * com.google.gwt.http.client.RequestBuilder within the super source
   */
   ...
   *
   * <p>Initializes client-server communication and will
   */
  ...
  public void onFailure(Throwable t) {
    ...
    if ((t instanceof StatusCodeException) && (((StatusCodeException) t).getStatusCode() == 0)) {
      // a status code 0 indicates the client aborted the request, most likely when leaving the
      // page, this should be ignored
      return;
    } else if ((t instanceof StatusCodeException)
        && (((StatusCodeException) t).getStatusCode() == 401)) {
      // a server error 500 most likely indicates an expired session and there for insufficient user
      // rights to access any GWT service
      CmsErrorDialog dialog =
          new CmsErrorDialog(Messages.get().key(Messages.GUI_SESSION_EXPIRED_0), null);
      dialog.center();
    } else {
   ...
   *
   * <p>Note: Has to be called manually before calling the RPC service.
   *
   * <p>
   *
   ...
   * Stops the timer.
   *
   * <p>Note: Has to be called manually on success.
   *
   * <p>
   *
   * @param displayDone <code>true</code> if you want to tell the user that the operation was
   *     successful
   */
  public void stop(boolean displayDone) {
    ...
    if (displayDone) {
      CmsNotification.get()
          .send(CmsNotification.Type.NORMAL, Messages.get().key(Messages.GUI_DONE_0));
    }
  }
   ...
   *
   * <p>Overwrite to customize the message.
   *
   * <p>
   *
   * @param blocking shows an blocking overlay if <code>true</code>
  ...
  protected void show(boolean blocking) {
    ...
    if (blocking) {
      m_notification =
          CmsNotification.get().sendBusy(CmsNotification.Type.NORMAL, m_loadingMessage);
    } else {
      m_notification =
