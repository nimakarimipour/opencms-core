...
public class CmsDNDHandler implements MouseDownHandler {
    ...
    public void run() {
      ...
      switch (m_direction) {
        case down:
          ...
          element
              .getStyle()
              .setTop(CmsDomUtil.getCurrentStyleInt(element, Style.top) + m_scrollSpeed, Unit.PX);
          break;
        case up:
          if (top <= m_scrollSpeed) {
            ...
            element
                .getStyle()
                .setTop(CmsDomUtil.getCurrentStyleInt(element, Style.top) - top, Unit.PX);
            break;
          }
          top -= m_scrollSpeed;
          element
        ...
        case left:
          if (left <= m_scrollSpeed) {
            ...
            element
                .getStyle()
                .setLeft(CmsDomUtil.getCurrentStyleInt(element, Style.left) - left, Unit.PX);
            left = 0;
            break;
          }
          left -= m_scrollSpeed;
          element
              .getStyle()
              .setLeft(CmsDomUtil.getCurrentStyleInt(element, Style.left) - m_scrollSpeed, Unit.PX);
          break;
        case right:
          left += m_scrollSpeed;
          element
  ...
  protected class DNDEventPreviewHandler implements NativePreviewHandler {
    ...
    public void onPreviewNativeEvent(NativePreviewEvent event) {
      ...
      if (!isDragging()) {
        // this should never happen, as the preview handler should be removed after the dragging
        // stopped
        CmsDebugLog.getInstance()
            .printLine("Preview handler still registered, even though dragging stopped.");
        stopDragging();
        return;
      }
  ...
  /**
   * Fancy animation fading out the helper and shrinking away the original element.
   *
   * <p>
   */
  class SpecialAnimation extends A_CmsAnimation {
    ...
    public SpecialAnimation(
        Element helper, Element original, Element overlay, Command callback, boolean hideOriginal) {
   ...
   *
   * <p>In case of a canceled drag and drop and enabled animation, the draggable helper element will
   * be reverted to the start position.
   *
   * <p>Values <code>&lt;0</code> will be ignored.
   *
   * <p>
   *
   ...
   *
   * <p>Needs to be executed on mouse move or when the list of allowed targets changes.
   *
   * <p>Uses the currently stored cursor position.
   *
   * <p>
   */
  ...
  protected void animateCancel(
      final I_CmsDraggable draggable, final I_CmsDNDController controller) {
   ...
   * This method assumes that the element parent is positioned relative.
   *
   * <p>
   */
  ...
  /**
   * Method will check all registered drop targets if the element is positioned over one of them.
   *
   * <p>
   */
  private void checkTargets() {
    ...
    if ((m_currentTarget != null)
        && m_currentTarget.checkPosition(m_clientX, m_clientY, m_orientation)) {
      ...
      if ((m_currentTarget instanceof I_CmsNestedDropTarget)
        ...
        for (I_CmsDropTarget target : ((I_CmsNestedDropTarget) m_currentTarget).getDnDChildren()) {
          if ((target != m_currentTarget)
              && target.checkPosition(m_clientX, m_clientY, m_orientation)) {
            // notifying controller, if false is returned, placeholder will not be positioned inside
            // target
  ...
  /**
   * Convenience method to get the appropriate scroll direction.
   *
   * <p>
   *
   * @param offset the scroll parent border offset, if the cursor is within the border offset,
  ...
  private Direction getScrollDirection(int offset) {
    ...
    if (m_scrollElement == null) {
      ...
      if (windowWidth < bodyWidth) {
        if (((windowWidth - m_clientX) < offset)
            && (Window.getScrollLeft() < (bodyWidth - windowWidth))) {
  ...
  /**
   * Shows the end animation on drop or cancel. Executes the given callback afterwards.
   *
   * <p>
   *
  ...
  private void showEndAnimation(Command callback, int top, int left, boolean isDrop) {
    ...
    switch (m_animationType) {
      case SPECIAL:
        ...
        m_currentAnimation =
            new SpecialAnimation(m_dragHelper, m_draggable.getElement(), overlay, callback, isDrop);
      ...
      case MOVE:
        ...
        m_currentAnimation =
            new CmsMoveAnimation(m_dragHelper, startTop, startLeft, endTop, endLeft, callback);
        break;
