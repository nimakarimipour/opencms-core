...
/**
 * The SEO options dialog, which makes it possible to both edit the SEO relevant properties of a
 * resource as well as alias paths for the resource.
 *
 * <p>
 */
public class CmsSeoOptionsDialog extends CmsPopup implements I_CmsFormHandler {
  ...
  public CmsSeoOptionsDialog(
    ...
    // ------------------------ PROPERTIES ------------------------------------------
    LinkedHashMap<String, CmsXmlContentProperty> props =
    ...
    m_propertyFieldset.setLegend(
        org.opencms.gwt.client.Messages.get()
            .key(org.opencms.gwt.client.Messages.GUI_PROPERTIES_0));
  ...
  public static void loadAliases(
      final CmsUUID structureId, final AsyncCallback<List<CmsAliasBean>> callback) {

    final CmsRpcAction<List<CmsAliasBean>> action =
        new CmsRpcAction<List<CmsAliasBean>>() {
  ...
  public void show() {
    ...
    m_propertyFieldset.addStyleName(
  ...
  public void update(boolean submit) {
    ...
    boolean ok =
        (m_propertyValidationStatus == VALIDATION_OK) && (m_aliasValidationStatus == VALIDATION_OK);
    if (submit && ok) {
      saveProperties();
      saveAliases();
      hide();
  ...
  protected void onClickSave() {
    ...
    Timer timer =
        new Timer() {
          ...
          public void run() {
            ...
            m_aliasList.validate(
                new Runnable() {

                  public void run() {

                    m_aliasValidationStatus =
                        !m_aliasList.hasValidationErrors() ? VALIDATION_OK : VALIDATION_FAILED;
                    update(true);
                  }
                });
    ...
    // slight delay so that the validation doesn't interfere with validations triggered by the
    // change event
    timer.schedule(20);
  }
  ...
  /**
   * Tells all widgets that the dialog has been opened.
   *
   * <p>
   */
  private void notifyWidgetsOfOpen() {

    for (Map.Entry<String, I_CmsFormField> fieldEntry :
        m_propertyEditor.getForm().getFields().entrySet()) {
