...
public final class CmsCoreProvider extends CmsCoreData {
  ...
  public static final I_CmsAutoBeanFactory AUTO_BEAN_FACTORY =
      GWT.create(I_CmsAutoBeanFactory.class);
  ...
  protected CmsCoreProvider() throws SerializationException {
    ...
    super(
        (CmsCoreData) CmsRpcPrefetcher.getSerializedObjectFromDictionary(getService(), DICT_NAME));
   ...
   * @return the content attribute value of the found meta tag, or null if no meta tag with the
   ...
   *
   * <p>This uses media queries, but the touch-only status can also be forcibly turned on with the
   * request parameter __touchOnly=1.
   *
  ...
  /**
   * Gets the unique active item container which holds a reference to the currently active content
   * element flyout menu.
   *
   * <p>
   *
  ...
  public void getResourceState(
      final CmsUUID structureId, final AsyncCallback<CmsResourceState> callback) {

    CmsRpcAction<CmsResourceState> action =
        new CmsRpcAction<CmsResourceState>() {
  ...
  public void lock(final CmsUUID structureId, final I_CmsSimpleCallback<Boolean> callback) {
    ...
    CmsRpcAction<String> lockAction =
        new CmsRpcAction<String>() {
          ...
          public void onResponse(String result) {
            ...
            if (result != null) {
              // unable to lock
              String text =
                  Messages.get().key(Messages.GUI_LOCK_NOTIFICATION_2, structureId, result);
   ...
   */
  public void lock(
      final CmsUUID structureId, long loadTime, final I_CmsSimpleCallback<Boolean> callback) {
   ...
   */
  public void lock(
      final String sitePath, long loadTime, final I_CmsSimpleCallback<Boolean> callback) {
  ...
  public void lockOrReturnError(
      final CmsUUID structureId, final I_CmsSimpleCallback<String> callback) {
   ...
   */
  public void lockOrReturnError(
      final CmsUUID structureId, final long loadTime, final I_CmsSimpleCallback<String> callback) {
   ...
   *
   * <p>If the resource does not exist yet, the next existing ancestor folder will be checked if it
   * is lockable.
   *
   * <p>
   *
        ...
        new CmsRpcAction<String>() {
          ...
          public void onResponse(String result) {
            ...
            if (result != null) {
              // unable to lock
              final String text =
                  Messages.get().key(Messages.GUI_LOCK_NOTIFICATION_2, sitePath, result);
              CmsNotification.get().sendDeferred(CmsNotification.Type.WARNING, text);
            }
            callback.execute(result);
          }
   ...
   */
  public void lockOrReturnError(
      final String sitePath, final long loadTime, final I_CmsSimpleCallback<String> callback) {
  ...
  /**
   * Removes the current site root prefix from the given root path, that is adjusts the resource
   * name for the current site root.
   *
  ...
  public String removeSiteRoot(String rootPath) {
    ...
    if ((siteRoot != null)
        ...
        && ((rootPath.length() == siteRoot.length())
            || (rootPath.charAt(siteRoot.length()) == '/'))) {
   ...
   * @param callback the callback to execute
   */
  public void substituteLinkForRootPath(
      final String rootPath, final I_CmsSimpleCallback<String> callback) {
  ...
  public boolean unlock(final CmsUUID structureId) {
    ...
    // lock the sitemap
    CmsRpcAction<String> unlockAction =
        new CmsRpcAction<String>() {
          ...
          public void onResponse(String result) {
            ...
            String text =
                Messages.get()
                    .key(Messages.GUI_UNLOCK_NOTIFICATION_2, structureId.toString(), result);
