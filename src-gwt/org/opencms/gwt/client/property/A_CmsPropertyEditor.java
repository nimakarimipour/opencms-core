...
public abstract class A_CmsPropertyEditor implements I_CmsFormWidgetMultiFactory {
  ...
  public A_CmsPropertyEditor(
      Map<String, CmsXmlContentProperty> propertyConfig, final I_CmsPropertyEditorHandler handler) {
  ...
  /**
   * Checks whether a widget can be used in the sitemap entry editor, and throws an exception
   * otherwise.
   *
   * <p>
   *
  ...
  public I_CmsFormWidget createFormWidget(
    ...
    if ("template".equals(key)) {
      ...
      result =
          CmsWidgetFactoryRegistry.instance().createFormWidget(key, widgetParams, defaultValue);
  ...
  protected CmsBasicFormField createUrlNameField() {
    ...
    CmsBasicFormField result =
        new CmsBasicFormField(FIELD_URLNAME, description, label, null, textbox);
  ...
  /**
   * Sets the ghost value for a form field if its normal value is empty and the field's widget
   * supports ghost values.
   *
   * <p>
   *
  ...
  /**
   * Helper method for creating the template selection widget.
   *
   * <p>
   *
  ...
  private I_CmsFormWidget createTemplateSelector() {
    ...
    if (m_handler.useAdeTemplates()) {
      ...
      for (Map.Entry<String, CmsClientTemplateBean> templateEntry :
          m_handler.getPossibleTemplates().entrySet()) {
  ...
  /**
   * Helper method for removing hidden properties from a map of property configurations.
   *
   * <p>The map passed into the method is not changed; a map which only contains the non-hidden
   ...
   */
  private Map<String, CmsXmlContentProperty> removeHiddenProperties(
