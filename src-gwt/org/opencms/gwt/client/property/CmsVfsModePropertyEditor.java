...
public class CmsVfsModePropertyEditor extends A_CmsPropertyEditor {
  ...
  public CmsVfsModePropertyEditor(
      Map<String, CmsXmlContentProperty> propConfig, I_CmsPropertyEditorHandler handler) {
  ...
  /**
   * @see org.opencms.gwt.client.property.A_CmsPropertyEditor#createFormWidget(java.lang.String,
  ...
  /**
   * @see
   *     org.opencms.gwt.client.property.A_CmsPropertyEditor#initializeWidgets(org.opencms.gwt.client.ui.CmsPopup)
   */
  @Override
        ...
        .scheduleFixedDelay(
            new RepeatingCommand() {
              ...
              public boolean execute() {
                ...
                if (!getPropertyPanel().getTabPanel().isAttached()
                    || !getPropertyPanel().getTabPanel().isVisible()) {
                  return false;
                }
                if (!m_resizeDisabled) {
                  updateHeight(dialog);
  ...
  /**
   * Sets the "show resource properties" flag which controls whether the resource value fields
   * should be built.
   *
   * <p>
   *
  ...
  protected void setupFieldContainer() {
    ...
    m_panel.addBeforeSelectionHandler(
  ...
  private void buildField(
      Map<String, CmsClientProperty> ownProps, final String propName, CmsClientProperty.Mode mode) {
    ...
    if (propDef == null) {
      ...
      propDef =
          new CmsXmlContentProperty(
              propName, "string", widget, "", null, null, null, null, null, null, null);
    }
    ...
    CmsPathValue pathValue =
        CmsClientProperty.getPathValue(ownProp, mode).prepend(entryId + "/" + propName + "/");

    // CHECK: should fields other than NavText be really automatically allowed to be empty in
    // navigation mode?
    final String tab = tabs.get(mode);
    final CmsBasicFormField field =
   ...
   * @param isStructure if true, the structure value field should be used, else the resource value
   *     field
   * @return the new model object
   */
  private I_CmsStringModel createStringModel(
      final CmsUUID id, final String propName, final boolean isStructure) {
    ...
    return new I_CmsStringModel() {
      ...
      /**
       * @see
       *     com.google.gwt.event.shared.HasHandlers#fireEvent(com.google.gwt.event.shared.GwtEvent)
       */
  ...
  /**
   * Gets a pair of strings containing the default value to display for a given property and its
   * source.
   *
   * <p>
   *
  ...
  private CmsPair<String, String> getDefaultValueToDisplay(CmsClientProperty prop, Mode mode) {
    ...
    String message =
        Messages.get().key(Messages.GUI_ORIGIN_INHERITED_1, inheritedProperty.getOrigin());
    return CmsPair.create(pathValue.getValue(), message);
  }

  /**
   * Creates a string model for a given property path value, and returns the same model if the same
   * path value is passed in.
   *
   * <p>
   *
