...
/**
 * A simpler implementation of the property editor handler interface which only provides the data to
 * edit a single file's properties (i.e. does not support combined folder/default file property
 * editing.
 *
 * <p>
 *
...
public class CmsSimplePropertyEditorHandler implements I_CmsPropertyEditorHandler {
            ...
            if (!CmsStringUtil.isEmptyOrWhitespaceOnly(newUrlName) && editedName) {
              propertyChanges.add(
                  new CmsPropertyModification(
                      null, CmsPropertyModification.FILE_NAME_PROPERTY, newUrlName, true));
            }
            CmsPropertyChangeSet changeBean =
                new CmsPropertyChangeSet(m_propertiesBean.getStructureId(), propertyChanges);
  ...
  public boolean useAdeTemplates() {
    ...
    return (m_propertiesBean != null)
        && (m_propertiesBean.isContainerPage() || m_propertiesBean.isFolder());
  }
