...
public class CmsMessages {
  ...
  public static String formatMessage(String result, Object... args) {
    ...
    for (int i = 0; i < args.length; i++) {
      if (args[i] instanceof Date) {
        ...
        result =
            result.replace(
                getRegEx(i), CmsDateTimeUtil.getDateTime(date, CmsDateTimeUtil.Format.MEDIUM));
        result =
            result.replace(
                getRegEx(i, "time"), CmsDateTimeUtil.getTime(date, CmsDateTimeUtil.Format.MEDIUM));
        result =
            result.replace(
        ...
        result =
            result.replace(
                getRegEx(i, "date"), CmsDateTimeUtil.getDate(date, CmsDateTimeUtil.Format.MEDIUM));
        result =
            result.replace(
  ...
  /**
   * Returns a regular expression for replacement.
   *
   * <p>
   *
   ...
   *
   * <p>Mainly for API compatibility with the core localization methods.
   *
   * <p>
   *
  ...
  /**
   * Returns a formated date String from a Date value, the format being {@link
   * CmsDateTimeUtil.Format#SHORT} and the locale based on this instance.
   *
   * <p>
   *
   ...
   *
   * <p>If the key was found in the bundle, it will be formatted using a <code>
   * {@link java.text.MessageFormat}</code> using the provided parameters. The parameters have to be
   * appended to the key separated by a "|". For example, the keyName <code>
   * error.message|First|Second</code> would use the key <code>error.message</code> with the
   * parameters <code>First</code> and <code>Second</code>. This would be the same as calling <code>
   * {@link CmsMessages#key(String, Object[])}</code>.
   *
