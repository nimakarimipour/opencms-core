...
public class CmsPositionBean {
  ...
  /**
   * Copy constructor. Generating a copy of the given model.
   *
   * <p>
   *
   ...
   *
   * @param panel the panel
   * @param addScroll if true, the result will contain the coordinates in the document's coordinate
   *     system, not the viewport coordinate system
   * @return the position info
   */
  ...
  private static native void getBoundingClientRect(
      Element element, CmsPositionBean pos, int scrollLeft, int scrollTop) /*-{
  ...
  /**
   * Returns over which area of this the given position is. Will return <code>null</code> if the
   * provided position is not within this position.
   *
   * <p>
   *
  ...
  public boolean isInside(CmsPositionBean child, int padding) {
    ...
    return ((getLeft() + padding) < child.getLeft()) // checking left border
        ...
        && (((getLeft() + getWidth()) - padding)
            > (child.getLeft() + child.getWidth())) // checking right border
        && (((getTop() + getHeight()) - padding)
            > (child.getTop() + child.getHeight())); // checking bottom border
  }
