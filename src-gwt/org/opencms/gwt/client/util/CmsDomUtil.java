...
public final class CmsDomUtil {
   ...
   *
   * <p>Use <code>-1</code> for x or y to ignore one ordering orientation.
   *
   * <p>
   *
   * @param element the element
   ...
   *
   * <p>It creates a new element with the same tag, and sets the class attribute, and sets the
   * innerHTML.
   *
   * <p>
   *
  ...
  /**
   * This method will create an {@link com.google.gwt.dom.client.Element} for the given HTML. The
   * HTML should have a single root tag, if not, the first tag will be used and all others
   * discarded.
   *
   * <p>Script-tags will be removed.
   *
   * <p>
   *
  ...
  public static Element createElement(String html) throws Exception {
    ...
    while ((elementRoot != null)
        && (elementRoot.getTagName().toLowerCase().equals(Tag.script.name()))) {
   ...
   * @return the HTML
   */
  public static String createFaceHtml(
      String text, String imageClass, HorizontalAlignmentConstant align) {
   ...
   * @param javascriptLink the link to the java script resource
   */
  public static void ensureJavaScriptIncluded(
      String javascriptLink, boolean async, JavaScriptObject callback) {
   ...
   *
   * <p>Useful in case something is capturing all events.
   *
   * <p>
   *
  ...
  public static void ensureMouseOut(Element element) {
    ...
    NativeEvent nativeEvent =
        Document.get().createMouseOutEvent(0, 0, 0, 0, 0, false, false, false, false, 0, null);
  ...
  public static void ensureMouseOver(Element element) {
    ...
    NativeEvent nativeEvent =
        Document.get().createMouseOverEvent(0, 0, 0, 0, 0, false, false, false, false, 0, null);
    element.dispatchEvent(nativeEvent);
  }
   ...
   *
   * <p>Assuming the scrollbars are on the container element, and that the element is a child of the
   * container element.
   *
   * <p>
   *
  ...
  public static void ensureVisible(
      final Element containerElement, Element element, int animationTime) {
    ...
    (new Animation() {
          ...
          protected void onUpdate(double progress) {
            ...
            containerElement.setScrollTop(
                startScrollTop + (int) ((endScrollTop - startScrollTop) * progress));
          }
        })
        .run(animationTime);
  }
  ...
  public static FormElement generateHiddenForm(
      String action, Method method, String target, Map<String, String> values) {
   ...
   */
  public static FormElement generateHiddenForm(
      String action, Method method, Target target, Map<String, String> values) {
   ...
   *
   * <p>Returns <code>null</code> if no appropriate element was found.
   *
   * <p>
   *
   * @param element the element
        ...
        if (!CmsDomUtil.hasClass(CmsGwtConstants.CLASS_EDITABLE_SKIP, sibling)) {
          ...
          result.setTop(
              ((result.getTop() == dummy) || (siblingPos.getTop() < result.getTop()))
                  ...
                  : result.getTop());
          result.setHeight(
              ((result.getTop() + result.getHeight())
                      > (siblingPos.getTop() + siblingPos.getHeight()))
                  ? result.getHeight()
                  : (siblingPos.getTop() + siblingPos.getHeight()) - result.getTop());
          result.setWidth(
              ((result.getLeft() + result.getWidth())
                      > (siblingPos.getLeft() + siblingPos.getWidth()))
                  ? result.getWidth()
  ...
  public static int getRelativeY(int y, Element target) {
    ...
    return (y - target.getAbsoluteTop())
        + /* target.getScrollTop() +*/ target.getOwnerDocument().getScrollTop();
  }
   ...
   *
   * <p>This is a workaround for a bug with {@link com.google.gwt.dom.client.Style#getZIndex()}
   * which occurs with IE in hosted mode.
   *
   * <p>
   *
  ...
  public static boolean hasBorder(Element element) {
    ...
    if ((borderStyle == null)
        || borderStyle.equals(StyleValue.none.toString())
   ...
   *
   * <p>All visible elements should have a dimension.
   *
   * <p>
   *
  ...
  /**
   * Positions an element inside the given parent, reordering the content of the parent and returns
   * the new position index.
   *
   * <p>This is none absolute positioning. Use for drag and drop reordering of drop targets.
   *
   * <p>Use <code>-1</code> for x or y to ignore one ordering orientation.
   *
   * <p>
   ...
   * @param currentIndex the current index position of the element, use -1 if element is not
   *     attached to the parent yet
   * @param x the client x position, use <code>-1</code> to ignore x position
   * @param y the client y position, use <code>-1</code> to ignore y position
   * @return the new index position
   */
  public static int positionElementInside(
      Element element, Element parent, int currentIndex, int x, int y) {
  ...
  public static void removeDisablingOverlay(Element element) {
    ...
    List<Element> overlays =
        CmsDomUtil.getElementsByClass(
            I_CmsLayoutBundle.INSTANCE.generalCss().disablingOverlay(), Tag.div, element);
    if (overlays == null) {
      return;
    }
  ...
  /**
   * Calls {@link org.opencms.gwt.client.I_CmsDescendantResizeHandler#onResizeDescendant()} on the
   * closest resizable ancestor.
   *
   * <p>
   *
  ...
  public static void safeLoadStylesheets(String[] stylesheets, JavaScriptObject callback) {
    ...
    CmsStylesheetLoader loader =
        new CmsStylesheetLoader(
  ...
  public static native void setAttribute(
      JavaScriptObject jso, String key, JavaScriptObject value) /*-{
        jso[key] = value;
    }-*/;
   ...
   *
   * <p>If the stylesheet with the id does not already exist, it is created.
   *
  ...
  public static void showPopupBlockerMessage() {
    ...
    CmsAlertDialog alertDialog =
        new CmsAlertDialog(messagePopupBlockedTitle(), messagePopupBlocked());
    alertDialog.center();
  }
  ...
  /**
   * Updates a set of style properties on the given style object, and returns a map with the
   * previous values.
   *
   * <p>If a value in the map is null, it is interpreted as clearing the style property with that
   * name.
   *
  ...
  public static Map<String, String> updateStyle(
      com.google.gwt.dom.client.Style style, Map<String, String> properties) {
  ...
  /**
   * Injects a script tag into the page head.
   *
   * <p>
   *
   ...
   */
  private static native void injectScript(
      String scriptLink, boolean async, JavaScriptObject onload) /*-{
        var headID = $wnd.document.getElementsByTagName("head")[0];
  ...
  /**
   * Internal method to indicate if the given element has a CSS class.
   *
   * <p>
   *
   * @param className the class name to look for
  ...
  /**
   * Checks if the given color value is transparent.
   *
   * <p>
   *
