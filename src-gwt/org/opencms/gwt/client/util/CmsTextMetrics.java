...
/**
 * Provides precise pixel measurements for blocks of text so that you can determine exactly how high
 * and wide, in pixels, a given block of text will be.
 *
 * <p>Normal usage would be:
 *
 * <pre>
 ...
 *
 * Based on <a
 * href="http://code.google.com/p/my-gwt/source/browse/trunk/user/src/net/mygwt/ui/client/util/TextMetrics.java">my-gwt
 * TextMetrics</a>.
 *
 * <p>
 *
...
public final class CmsTextMetrics {
   ...
   *
   * <p>If the key is null, or the method has been never called with the same key before, a new text
   * metrics object will be created, with its style taken from the element parameter. Otherwise, the
   * text metrics object for the given key will be returned, and the element parameter will be
   * ignored.
   *
  ...
  /**
   * Returns the measured height of the specified text. For multiline text, be sure to call {@link
  ...
  /**
   * Binds this text metrics instance to an element from which to copy existing CSS styles that can
   * affect the size of the rendered text.
   *
   * <p>
   *
