...
public final class CmsPreviewDialog extends CmsPopup {
  ...
  public static void showPreviewForResource(CmsPreviewInfo previewInfo) {
    ...
    if (previewInfo.isNewWindowRequired()) {
      CmsDomUtil.openWindow(
          previewInfo.getPreviewUrl(), CmsDomUtil.Target.BLANK.getRepresentation(), "");
    } else {
    ...
    CmsRpcAction<CmsPreviewInfo> previewAction =
        new CmsRpcAction<CmsPreviewInfo>() {
          ...
          public void execute() {
            ...
            CmsCoreProvider.getVfsService()
                .getPreviewInfo(sitePath, CmsCoreProvider.get().getLocale(), this);
            start(0, true);
          }
  ...
  private static String generateCaption(CmsPreviewInfo previewInfo) {
    ...
    return (CmsStringUtil.isNotEmptyOrWhitespaceOnly(previewInfo.getTitle())
            ? previewInfo.getTitle() + " / "
            : "")
  ...
  /**
   * Initializes the locale selector if needed.
   *
   * <p>
   *
   * @param previewInfo the preview info
