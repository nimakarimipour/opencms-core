...
public class CmsResourceRelationView extends Composite implements I_CmsDescendantResizeHandler {
  ...
  public CmsResourceRelationView(
      CmsResourceStatusBean status, Mode mode, CmsContextMenuHandler menuHandler) {
  ...
  protected void fill() {
      ...
      for (CmsResourceStatusRelationBean relationBean : relationBeans) {
        ...
        CmsContextMenuButton button =
            new CmsContextMenuButton(
                relationBean.getStructureId(), m_menuHandler, AdeContext.resourceinfo);
        item.getListItemWidget().addButton(button);
        final CmsResourceStatusRelationBean currentRelationBean = relationBean;
        final boolean isContainerpage =
            CmsGwtConstants.TYPE_CONTAINERPAGE.equals(relationBean.getInfoBean().getResourceType());
        final boolean isXmlContent = relationBean.isXmlContent();
        final boolean isEditable =
            (isXmlContent || isContainerpage)
        ...
        if (isContainerpage) {
          if (relationBean.getLink() != null) {
            ...
            itemWidget.addIconClickHandler(
                new ClickHandler() {
    ...
    if ((m_mode == Mode.sources) && !m_statusBean.getOtherSiteRelationSources().isEmpty()) {
      m_otherSitesList =
          createList(Messages.get().key(Messages.GUI_RESOURCEINFO_OTHERSITES_LEGEND_0));
      for (CmsResourceStatusRelationBean relationBean :
  ...
  /**
   * Creates a relation item list wrapped in a field set and appends it to the dialog panel.
   *
   * <p>
   *
  ...
  private CmsList<CmsListItem> createList(String label) {
    ...
    scrollPanel
        .getElement()
        .getStyle()
        .setHeight(CmsResourceInfoDialog.SCROLLPANEL_HEIGHT, Style.Unit.PX);
    ...
    switch (m_mode) {
      case sources:
        ...
        return org.opencms.gwt.client.Messages.get()
            .key(org.opencms.gwt.client.Messages.GUI_USAGE_EMPTY_0);
      case targets:
      ...
      default:
        return org.opencms.gwt.client.Messages.get()
            .key(org.opencms.gwt.client.Messages.GUI_TARGETS_EMPTY_0);
    }
  }

  /**
   * Gets the label to use for the fieldset.
   *
   * <p>
   *
  ...
  /**
   * Gets the relation beans to display.
   *
   * <p>
   *
