...
public class CmsResourceInfoDialog extends CmsPopup {
   ...
   */
  public CmsResourceInfoDialog(
      CmsResourceStatusBean statusBean, boolean includeTargets, CmsUUID detailContentId) {
    ...
    for (Map.Entry<CmsResourceStatusTabId, String> tabEntry : statusBean.getTabs().entrySet()) {
      switch (tabEntry.getKey()) {
        case tabRelationsFrom:
          CmsResourceRelationView targets =
              new CmsResourceRelationView(statusBean, Mode.targets, menuHandler);
          ...
          break;
        case tabRelationsTo:
          CmsResourceRelationView usage =
              new CmsResourceRelationView(statusBean, Mode.sources, menuHandler);
        ...
        case tabSiblings:
          if (statusBean.getSiblings().size() > 0) {
            CmsResourceRelationView siblings =
                new CmsResourceRelationView(statusBean, Mode.siblings, menuHandler);
    ...
    m_tabPanel.addSelectionHandler(
        new SelectionHandler<Integer>() {

          public void onSelection(SelectionEvent<Integer> event) {

            Widget tabContent =
                m_tabPanel.getWidget(event.getSelectedItem().intValue()).getWidget();
   ...
   * <p>
   *
   * @param structureId the structure id of the resource for which the resource info should be
   *     loaded
   ...
   * @param closeHandler the close handler for the dialog (may be null if no close handler is
   *     needed)
   */
  ...
  public static void load(
    ...
    CmsRpcAction<CmsResourceStatusBean> action =
        new CmsRpcAction<CmsResourceStatusBean>() {
          ...
          protected void onResponse(CmsResourceStatusBean result) {
            ...
            CmsResourceInfoDialog dialog =
                new CmsResourceInfoDialog(result, includeTargets, detailContentId);
