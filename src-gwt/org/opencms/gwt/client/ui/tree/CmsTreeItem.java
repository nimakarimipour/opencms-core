 ...
 * </pre>
 *
 * Where state can be <code>opened</code>, <code>closed</code> or <code>leaf</code>.
 *
 * <p>
 *
 * @since 8.0.0
...
public class CmsTreeItem extends CmsListItem {
   ...
   * @param requiresDropEnabled <code>true</code> if it is required the returned element to be drop
   *     enabled
   * @return the last visible item of a tree fragment
   */
  protected static CmsTreeItem getLastOpenedItem(
      CmsTreeItem item, int stopLevel, boolean requiresDropEnabled) {
   ...
   *
   * <p>Example: '/xxx/xxx/' has a path-level of 2.
   *
   * <p>
   *
    ...
    if (top < height) {
      ...
      if (shouldInsertIntoSiblingList(originalPathLevel, parentItem, index)) {
        ...
        if (previousSibling.isOpen()) {
          ...
          return CmsTreeItem.getLastOpenedItem(previousSibling, originalPathLevel, true)
              .insertPlaceholderAsLastChild(placeholder);
        }
      }
   ...
   *
   * <p>This is automatically called when this tree item or one of its ancestors is inserted into a
   * tree.
   *
   * <p>
   *
  ...
  /**
   * Inserts the placeholder element as last child of the children list. Setting it's path as the
   * current placeholder path and returning the new index.
   *
   * <p>
   *
   ...
   * @return <code>true</code> if the item should be inserted into the previous siblings children
   *     list
   */
  private boolean shouldInsertIntoSiblingList(
      int originalPathLevel, CmsTreeItem parent, int index) {
