...
public class CmsTree<I extends CmsTreeItem> extends CmsList<I>
  ...
  /**
   * Timer to set sub item list visible.
   *
   * <p>
   */
  private class OpenTimer extends Timer {
    ...
    /**
     * Checks if the timer is running for the given tree item.
     *
     * <p>
     *
          ...
          if (relativeTop > elementHeight) {
            ...
            if (item.isOpen() && (item.getChildCount() > 0)) {
              ...
              if ((getDnDHandler() != null)
                  && (getDnDHandler().getDraggable() instanceof CmsTreeItem)) {
                originalPathLevel =
                    CmsTreeItem.getPathLevel(
                            ((CmsTreeItem) getDnDHandler().getDraggable()).getPath())
                        - 1;
              }
   ...
   *
   * <p>Use {@link CmsTreeItem#setDropEnabled(boolean)} for dropping on tree items.
   *
   * <p>
   *
  ...
  /**
   * Sets the timer reference to <code>null</code>.
   *
   * <p>
   */
