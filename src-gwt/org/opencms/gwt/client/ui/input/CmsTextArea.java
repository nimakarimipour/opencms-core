...
public class CmsTextArea extends Composite
  ...
  /**
   * The real, logical value which might be different from the value displayed in the internal
   * textarea.
   */
  ...
  public CmsTextArea() {
    ...
    m_textArea.addValueChangeHandler(
  ...
  public void setEnabled(boolean enabled) {
    ...
    if (enabled) {
      ...
      if (m_disabledOverlay == null) {
        ...
        m_disabledOverlay.setClassName(
            I_CmsInputLayoutBundle.INSTANCE.inputCss().disableTextArea());
   ...
   * Not used.
   *
   * <p>This widget automatically decides whether it's in ghost mode depending on the real value and
   * ghost value set.
   *
   * <p>
   *
  ...
  protected void updateContentSize() {
    ...
    // sanity check: don't do anything, if the measured height doesn't make any sense, e.g. if
    ...
    if (offsetHeight > 5) {
      ...
      int rows =
          Math.max(
              0,
              m_defaultRows
                  - 1); // we add 1 to it later, and the sum should be at least 1 and at least the
                        // default row number
      int scrollHeight;
      ...
      do {
        ...
        if (offsetHeight <= prevOffsetHeight) {
          ...
          // If it doesn't, e.g. because of CSS rules limiting the text area height, there is no
          // point in continuing with the loop
          break;
        }
  ...
  /**
   * This method is called when the value of the internal textarea changes.
   *
   * <p>
   *
  ...
  /**
   * Converts null to an empty string, leaves other strings unchanged.
   *
   * <p>
   *
  ...
  /**
   * Updates the styling and content of the internal text area based on the real value, the ghost
   * value, and whether it has focus.
   */
  ...
  /**
   * Updates the content of the internal textarea.
   *
   * <p>
   *
