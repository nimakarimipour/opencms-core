 ...
 * @since 8.0.0
 */
public class CmsCategoryField extends Composite
    implements I_CmsFormWidget, I_CmsHasInit, I_CmsHasResizeOnShow {
   ...
   *
   * <p>The selection might contain either category paths or site paths of categories.
   *
  ...
  public void buildCategoryTree(
      List<CmsCategoryTreeEntry> treeEntries, Collection<String> selectedCategories) {
    ...
    if ((treeEntries != null) && !treeEntries.isEmpty()) {
      ...
      for (CmsCategoryTreeEntry category : treeEntries) {
        ...
        boolean hasSelectedChildren =
            hasSelectedChildren(category.getChildren(), selectedCategories);
  ...
  private CmsTreeItem buildTreeItem(
      CmsCategoryTreeEntry category, Collection<String> selectedCategories, boolean inactive) {

    CmsDataValue categoryTreeItem =
        new CmsDataValue(
            500, 4, CmsCategoryBean.SMALL_ICON_CLASSES, category.getTitle(), category.getPath());
    if (inactive) {
  ...
  /**
   * Checks if it has selected children.
   *
   * <p>
   *
  ...
  private boolean hasSelectedChildren(
      List<CmsCategoryTreeEntry> children, Collection<String> selectedCategories) {
