...
public class CmsDataValue extends Composite implements I_CmsTruncable, HasClickHandlers {
  ...
  /** The image panel. */
  @UiField SimplePanel m_imagePanel;

  /** The label field. */
  @UiField Label m_label;

  /** The CSS bundle instance. */
  @UiField I_Style m_style;

  /** The table. */
  @UiField FlexTable m_table;
   ...
   * @param parameters the values that should be shown in this widget. The first value is used for
   *     the label
   */
  public CmsDataValue(
      int width, int part, String image, boolean complementPartWidth, String... parameters) {
   ...
   * @return <code>true</code> if the gallery matches the given filter.
   *     <p>
   */
  ...
  public void setInactive() {
    ...
    getElement()
        .getStyle()
        .setBorderColor(I_CmsLayoutBundle.INSTANCE.constants().css().borderColor());
    getElement()
        .getStyle()
        .setColor(I_CmsLayoutBundle.INSTANCE.constants().css().textColorDisabled());
  }
  ...
  /**
   * Generates the widget an adds all parameter to the right place.
   *
   * <p>
   */
