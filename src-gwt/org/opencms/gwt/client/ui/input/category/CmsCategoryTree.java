...
public class CmsCategoryTree extends Composite
    implements I_CmsTruncable, HasValueChangeHandlers<List<String>> {
  ...
  /**
   * Inner class for select box handler.
   *
   * <p>
   */
  ...
  /**
   * Inner class for check box handler.
   *
   * <p>
   */
  private class CheckBoxValueChangeHandler implements ValueChangeHandler<Boolean> {
    ...
    /**
     * Is triggered if an check box is selected or deselected.
     *
     * <p>
     *
  ...
  private class DataValueClickHander implements ClickHandler {
    ...
    /**
     * Constructor to set the right CmsTreeItem for this handler.
     *
     * <p>
     *
    ...
    /**
     * Is triggered if the DataValue widget is clicked.
     *
     * <p>If its check box was selected the click will deselect this box otherwise it will select
     * it.
     *
     * @param event The event that is triggered
     */
  ...
  /** The scroll panel. */
  @UiField CmsScrollPanel m_list;

  /** The main panel. */
  @UiField FlowPanel m_tab;
  ...
  public void disable(String disabledReason) {
    ...
    if (m_isEnabled
        || (CmsStringUtil.isNotEmptyOrWhitespaceOnly(disabledReason)
            && !disabledReason.equals(m_disabledReason))) {
      m_isEnabled = false;
  ...
  protected void quickSearch() {
    ...
    if ((m_quickSearch != null)) {
      categories =
          getFilteredCategories(hasQuickFilter() ? m_quickSearch.getFormValueAsString() : null);
   ...
   *
   * <p>The checkbox state of the selected/deselected item has to be the state BEFORE toggling.
   *
  ...
  /**
   * Builds a tree item for the given category.
   *
   * <p>
   *
  ...
  private CmsTreeItem buildTreeItem(
      CmsCategoryTreeEntry category, Collection<String> selectedCategories) {

    // generate the widget that should be shown in the list
    CmsDataValue dataValue =
        new CmsDataValue(
            600,
            3,
    ...
    isPartofPath =
        CmsCategoryField.isParentCategoryOfSelected(category.getPath(), selectedCategories);
    if (isPartofPath) {
  ...
  /**
   * Schedules the execution of onResize deferred.
   *
   * <p>
   */
  ...
  /**
   * Sets the given tree list enabled/disabled.
   *
   * <p>
   *
  ...
  private void setListEnabled(
      CmsList<? extends I_CmsListItem> list, boolean enabled, String disabledReason) {
