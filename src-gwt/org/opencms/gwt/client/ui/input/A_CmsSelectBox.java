...
public abstract class A_CmsSelectBox<OPTION extends A_CmsSelectCell> extends Composite
  ...
  /** The opener widget. */
  @UiField protected FocusPanel m_opener;

  /** Container for the opener and error widget. */
  @UiField protected Panel m_panel;
   ...
   *
   * <p>This method does not trigger the "value changed" event.
   *
   * <p>
   *
  ...
  /**
   * Returns the offset height of the popup panel, should also work when the popup is currently not
   * showing.
   *
   * <p>
   *
  ...
  /**
   * The implementation of this method should update the opener when a new value is selected by the
   * user.
   *
   * <p>
   *
  ...
  void positionPopup() {
    ...
    if (m_popup.isShowing()) {
      ...
      int openerHeight =
          CmsDomUtil.getCurrentStyleInt(m_opener.getElement(), CmsDomUtil.Style.height);
      int popupHeight = getPopupHeight();
      int dx = 0;
      if (width > (m_opener.getOffsetWidth())) {
        int spaceOnTheRight =
            (Window.getClientWidth() + Window.getScrollLeft()) - m_opener.getAbsoluteLeft() - width;
      ...
      if (displayAbove) {
        ...
        CmsDomUtil.positionElement(
            m_popup.getElement(), m_panel.getElement(), dx, -(popupHeight - 2));
        ...
        CmsDomUtil.positionElement(
            m_popup.getElement(), m_panel.getElement(), dx, openerHeight - 1);
  ...
  /**
   * Helper method for adding event handlers for a 'hover' effect to the main panel.
   *
   * <p>
   *
  ...
  /**
   * Adds a scroll panel to the selector popup.
   *
   * <p>
   *
