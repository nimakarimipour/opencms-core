 ...
 * @since 8.0.0
 */
public class CmsVfsSelection extends Composite
    implements I_CmsFormWidget, HasValueChangeHandlers<String> {

  /**
   * Event preview handler.
   *
   * <p>To be used while popup open.
   *
   * <p>
   */
  ...
  public CmsVfsSelection(String iconImage, String type, String config) {
    ...
    m_selectionInput.m_textbox.addMouseUpHandler(
        new MouseUpHandler() {
    ...
    m_selectionInput.m_textbox.addBlurHandler(
        new BlurHandler() {

          public void onBlur(BlurEvent event) {

            if ((m_selectionInput.m_textbox.getValue().length() * 6.88)
                > m_selectionInput.m_textbox.getOffsetWidth()) {
              setTitle(m_selectionInput.m_textbox.getValue());
            }
            m_selectionInput.showFader();
          }
        });
    m_selectionInput.setOpenCommand(
        new Command() {
      ...
      m_popup.setCloseHandler(
          new Runnable() {
            ...
            public void run() {
              ...
              m_selectionInput.m_textbox.setFocus(true);
              m_selectionInput.m_textbox.setCursorPos(
                  m_selectionInput.m_textbox.getText().length());
            }
          });
  ...
  /**
   * Exporting the set principal function to the window scope.
   *
   * <p>
   */
