...
/**
 * This class is an Helper with mostly static methods that convert a given date object or that
...
public final class CmsDateConverter {
  ...
  private static final String DATE_PATTERN =
      Messages.get().key(Messages.GUI_DATEBOX_DATE_PATTERN_0);

  /** A pattern for date time representation. */
  private static final String DATETIME_PATTERN =
      Messages.get().key(Messages.GUI_DATEBOX_DATETIME_PATTERN_0);

  /** A pattern for time representation. */
  private static final String TIME_PATTERN =
      Messages.get().key(Messages.GUI_DATEBOX_TIME_PATTERN_0);

  /** The formatter for the date format. */
  private static final DateTimeFormat Z_DATE_FORMAT = DateTimeFormat.getFormat(DATE_PATTERN);

  /**
   * The format used to parse dates (this needs to be different from Z_DATE_FORMAT because we want
   * to interpret the year 22 as 2022, but don't want to format 1922 as 22.
   */
  private static final DateTimeFormat Z_DATE_FORMAT_SHORTYEAR =
      DateTimeFormat.getFormat(DATE_PATTERN.replace("yyyy", "yy"));

  /** The formatter for the date time format. */
  private static final DateTimeFormat Z_DATETIME_FORMAT =
      DateTimeFormat.getFormat(DATETIME_PATTERN);

  /**
   * The format used to parse date/time combinations (this needs to be different from
   * Z_DATETIME_FORMAT because we want to interpret the year 22 as 2022, but don't want to format
   * 1922 as 22.
   */
  private static final DateTimeFormat Z_DATETIME_FORMAT_SHORTYEAR =
      DateTimeFormat.getFormat(DATETIME_PATTERN.replace("yyyy", "yy"));

  /** The formatter for the time format. */
  private static final DateTimeFormat Z_TIME_FORMAT = DateTimeFormat.getFormat(TIME_PATTERN);

  /**
   * Hiding constructor for final class.
   *
   * <p>
   */
   ...
   *
   * <p>If the given String has less than 5 characters an dosen't contains an am or pm in it the
   * original String is returned.
   *
   * <p>
   *
   * @param time the time String to cut the suffix from
   * @return the time String without the suffix or the original String if the format of the String
   *     is incorrect
   */
  public static String cutSuffix(String time) {

    String ret = time.toLowerCase();
    if (ret.toLowerCase().contains(CmsDateConverter.AM)
        || ret.toLowerCase().contains(CmsDateConverter.PM)) {
      if (ret.length() > 5) {
   ...
   *
   * <p>Returns a <code>null</code> if the given time format coudn't be parsed.
   *
   * <p>The expected time String should include the am pm information if the time format is in 12
   * hour presentation.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the current date format is in the 12 hour representation mode
  ...
  /**
   * Returns <code>true</code> if an am is in the given date object <code>false</code> otherwise.
   *
   * <p>
   *
   ...
   *
   * <p>First try to parse the String with the given time format.
   *
   * <p>If that fails try to parse the date with the browser settings.
   *
   * <p>
   *
  ...
  /**
   * Formats the provided date as only a date format (dd/mm/yyyy). Note, a null date is a possible
   * input.
   *
   * @param date the date to format
   * @return the formatted date as a string
   ...
   *
   * <p>Returns <code>true</code> if the given date matches to one of the regular expressions,
   * <code>false</code> otherwise.
   *
   * <p>
   *
   * @param date the date String to check
   * @return <code>true</code> if the given time matches to one of the regular expressions, <code>
   ...
   *
   * <p>Returns <code>true</code> if the given time matches the regular expressions, <code>false
   * </code> otherwise.
   *
   * <p>
   *
   * @param time the time String to check
   * @return <code>true</code> if the given time matches the regular expressions, <code>false</code>
   *     otherwise
   */
