 ...
 *
 * <p>This widget behaves like a select box, until a button on it is pressed, after which the select
 * box is transformed into a combo box. The reason for this is that the combo box always displays
 * the currently selected value itself, rather than the label from the widget configuration, which
 * may be confusing to nontechnical users in some cases.
 */
public class CmsSelectComboBox extends Composite
  ...
  /**
   * Gets the active widget (select or combo box).
   *
   * <p>
   *
