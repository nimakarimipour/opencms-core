 ...
 *
 * <p>Has a progressbar and a table for showing details.
 *
 * <p>
 */
public class CmsUploadProgressInfo extends FlowPanel {
  ...
  public void finish() {
    ...
    m_fileinfo.setHTML(
        0, 0, "<b>" + Messages.get().key(Messages.GUI_UPLOAD_FINISH_UPLOADED_0) + "</b>");
    m_fileinfo.setText(
        0,
        1,
        Messages.get()
  ...
  public void setProgress(CmsUploadProgessInfo info) {
    ...
    if (!m_started) {
      m_started = true;
      m_fileinfo.setHTML(
          0, 0, "<b>" + Messages.get().key(Messages.GUI_UPLOAD_PROGRESS_CURRENT_FILE_0) + "</b>");
      m_fileinfo.setHTML(
          1, 0, "<b>" + Messages.get().key(Messages.GUI_UPLOAD_PROGRESS_UPLOADING_0) + "</b>");
    ...
    m_fileinfo.setText(
        ...
        1,
        Messages.get()
            .key(Messages.GUI_UPLOAD_PROGRESS_UPLOADING_VALUE_2, readBytes, contentLength));
  }

  /**
   * Returns the bytes that are read so far.
   *
   * <p>The total request size is larger than the sum of all file sizes that are uploaded. Because
   * boundaries and the target folder or even some other information than only the plain file
   * contents are submited to the server.
   *
   * <p>This method calculates the bytes that are read with the help of the file sizes.
   *
   * <p>
   *
