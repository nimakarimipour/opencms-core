...
public class CmsLinkWarningPanel extends Composite {
  ...
  private static I_CmsLinkWarningPanelUiBinder uiBinder =
      GWT.create(I_CmsLinkWarningPanelUiBinder.class);
  ...
  private final CmsContextMenuHandler m_menuHandler =
   ...
   */
  protected CmsListItemWidget createListItemWidget(
      CmsBrokenLinkBean brokenLinkBean, boolean contextMenu) {
    ...
    if (contextMenu && (structureId != null) && !structureId.isNullUUID()) {
      ...
      CmsContextMenuButton button =
          new CmsContextMenuButton(structureId, m_menuHandler, AdeContext.resourceinfo);
  ...
  protected CmsTreeItem createTreeItem(CmsBrokenLinkBean brokenLinkBean) {
    ...
    for (CmsBrokenLinkBean child : brokenLinkBean.getChildren()) {
      ...
      childItemWidget.addTitleStyleName(
          I_CmsLayoutBundle.INSTANCE.linkWarningCss().deletedEntryLabel());
