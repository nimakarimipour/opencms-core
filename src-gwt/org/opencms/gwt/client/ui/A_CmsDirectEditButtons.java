...
public abstract class A_CmsDirectEditButtons extends FlowPanel
  ...
  private class MouseHandler extends A_CmsHoverHandler implements ClickHandler {
    ...
    /**
     * @see
     *     org.opencms.gwt.client.ui.A_CmsHoverHandler#onHoverIn(com.google.gwt.event.dom.client.MouseOverEvent)
     */
    @Override
    ...
    /**
     * @see
     *     org.opencms.gwt.client.ui.A_CmsHoverHandler#onHoverOut(com.google.gwt.event.dom.client.MouseOutEvent)
     */
    @Override
  ...
  public A_CmsDirectEditButtons(Element editable, String parentId) {
    ...
    try {
      ...
      CmsScriptCallbackHelper callbackForElement =
      ...
      CmsNewLinkFunctionTable.INSTANCE.setHandler(
          m_editableData.getContextId(),
          new Runnable() {
      ...
      if (m_editableData.hasDelete()
          && CmsStringUtil.isEmptyOrWhitespaceOnly(m_editableData.getNoEditReason())) {
      ...
      if ((buttonMap.size() > 0) || m_editableData.hasEdit()) {
        if (!m_editableData
            .getExtensions()
            .isUploadEnabled()) { // for the upload case, the edit button is not needed, the
                                  // bullseye edit point is displayed on the upload button instead
            ...
            new ClickHandler() {
              ...
              public void onClick(ClickEvent e) {
                ...
                CmsCoreProvider.get()
                    ...
                    .clearIfMatches(A_CmsDirectEditButtons.this);
              }
            });
      }
      ...
      if (m_editableData.isUnreleasedOrExpired()) {
        ...
        m_expiredOverlay.setClassName(
            org.opencms.gwt.client.ui.css.I_CmsLayoutBundle.INSTANCE
                .directEditCss()
                .expiredListElementOverlay());
        m_markerTag.getParentElement().insertBefore(m_expiredOverlay, m_markerTag);
      }

    } catch (Exception e) {
      throw new UnsupportedOperationException(
          "Error while parsing editable tag information: " + e.getMessage());
    }
  }
  ...
  public CmsPushButton createInfoButton() {
    ...
    add(infoButton);
    infoButton.addClickHandler(
        new ClickHandler() {

          public void onClick(ClickEvent event) {

            CmsResourceInfoDialog.load(
                m_editableData.getStructureId(), true, null, getInfoContext(), null);
          }
        });
    return infoButton;
  }
  ...
  /**
   * Returns a map of additional buttons in a map, with the button position as key (buttons will be
  ...
  /**
   * Method to be executed when the "new" direct edit button is clicked, and the corresponding file
   * has a type for which the upload dialog should be triggered.
   */
