...
public class CmsScrollBar extends FocusPanel
  ...
  /**
   * The timer used to continue to shift the knob as the user holds down one of the left/right arrow
   * keys. Only IE auto-repeats, so we just keep catching the events.
   */
  ...
  /**
   * @see
   *     com.google.gwt.event.dom.client.HasScrollHandlers#addScrollHandler(com.google.gwt.event.dom.client.ScrollHandler)
   */
  ...
  /**
   * Calculates the scroll knob height.
   *
   * <p>
   *
  ...
  /**
   * Sets the scroll knob position according to the given value.
   *
   * <p>
   *
  ...
  /**
   * Sides the scroll knob according to the mouse event.
   *
   * <p>
   *
  ...
  /**
   * Returns <code>true</code> if the events mouse position is above the scroll bar knob.
   *
   * <p>
   *
   * @param event the mouse event
  ...
  private boolean sliderClicked(Event event) {
    ...
    boolean result =
        CmsPositionBean.generatePositionInfo(m_knob)
            .isOverElement(
