...
public class CmsErrorDialog extends CmsPopup {
  ...
  public CmsErrorDialog(String message, String details) {
    ...
    if (details != null) {
      ...
      m_detailsFieldset.addOpenHandler(
          new OpenHandler<CmsFieldSet>() {
  ...
  /**
   * Handles the exception by logging the exception to the server log and displaying an error dialog
   * on the client.
   *
   * <p>
   *
  ...
  public static void handleException(String message, Throwable t) {
    ...
    String ticket =
        CmsLog.log(
            message + LINE_BREAK + CmsClientStringUtil.getStackTraceAsString(trace, LINE_BREAK));
  ...
  /**
   * Creates a field-set containing the error details.
   *
   * <p>
   *
  ...
  /**
   * Creates the message HTML widget containing error icon and message.
   *
   * <p>
   *
  ...
  private void onShow() {
    ...
    if (m_detailsFieldset != null) {
      m_detailsFieldset
          ...
          .getElement()
          .getStyle()
          .setPropertyPx("maxHeight", getAvailableHeight(m_messageWidget.getOffsetHeight()));
    }
  }
}
