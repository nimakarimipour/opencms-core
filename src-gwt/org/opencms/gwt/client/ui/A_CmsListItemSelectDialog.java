...
public abstract class A_CmsListItemSelectDialog<INFO extends CmsListInfoBean> extends CmsPopup {
  ...
  protected void ensureChecked(CmsUUID structureId) {
    ...
    CmsRadioButton button =
        (CmsRadioButton)
            m_listPanel.getItem(String.valueOf(structureId)).getDecorationWidgets().get(0);
    if (!button.isChecked()) {
  ...
  /**
   * Adjusts the max height setting of the model list.
   *
   * <p>
   */
  ...
  /**
   * Generates internal names for the radio button widgets.
   *
   * <p>
   *
