...
public final class CmsLockReportDialog extends CmsPopup {
  ...
  /**
   * Handles the scroll panel height.
   *
   * <p>
   */
  ...
  private CmsLockReportDialog(
      String title, CmsUUID structureId, Command onUnlock, final Command optionalOnCloseCommand) {
    ...
    m_unlockButton.addClickHandler(
        new ClickHandler() {
   ...
   * @param optionalOnCloseCommand the optional command to execute when the lock report dialog is
   *     closed
   */
  public static void openDialogForResource(
      String title, final CmsUUID structureId, Command onUnlock, Command optionalOnCloseCommand) {

    final CmsLockReportDialog dialog =
        new CmsLockReportDialog(title, structureId, onUnlock, optionalOnCloseCommand);
    CmsRpcAction<CmsLockReportInfo> action =
        new CmsRpcAction<CmsLockReportInfo>() {
  ...
  protected void adjustHeight() {
    ...
    if ((m_scrollPanel != null) && (m_resourceItem != null)) {
      m_scrollPanel
          .getElement()
          .getStyle()
          .setPropertyPx("maxHeight", getAvailableHeight(m_resourceItem.getOffsetHeight()));
    }
    center();
  }
  ...
  /**
   * Returns the dialog message for the given lock.
   *
   * <p>
   *
