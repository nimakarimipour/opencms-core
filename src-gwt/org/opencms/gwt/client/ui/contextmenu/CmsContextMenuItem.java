...
/**
 * An entry in a {@link org.opencms.gwt.client.ui.contextmenu.CmsContextMenu}. Menu items can either
 * fire a {@link com.google.gwt.user.client.Command} when they are clicked, or open a cascading
 * sub-menu.
 *
 * <p>This implementation of the abstract context menu item provides a possible image in front of
 * the text and a arrow for a sub menu entry.
 *
 * <p>Furthermore constructs the HTML for such a menu entry.
 *
 * <p>
 *
 * @since version 8.0.0
...
public final class CmsContextMenuItem extends A_CmsContextMenuItem {
  ...
  /**
   * The panel containing the menu item text and optional the arrow and or a image in front of the
   * text.
   */
  ...
  public void onClick(ClickEvent event) {
    ...
    // hide menu *after* executing the action, because hiding the menu may trigger mouseover events
    // of the elements lying under it,
    // and executing the action first gives it the opportunity to add a 'blocking notification' to
    // prevent this
  ...
  protected String getMenuItemHtml(boolean hasSubMenu) {
    ...
    if (hasSubMenu) {
      ...
      html.append(I_CmsLayoutBundle.INSTANCE.contextmenuCss().arrow())
          .append(" ")
          .append(I_CmsButton.ICON_FONT)
          .append(" ")
          .append(I_CmsButton.TRIANGLE_RIGHT);
      html.append("\"></div>");
    }
    ...
    html.append(
        "<div class='"
            + I_CmsLayoutBundle.INSTANCE.contextmenuCss().iconBox()
            + "'>"
            + iconHtml
            + "</div>");
