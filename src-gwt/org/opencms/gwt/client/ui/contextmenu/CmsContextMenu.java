...
public class CmsContextMenu extends Composite implements ResizeHandler, I_CmsAutoHider {
  ...
  public CmsContextMenu(
      List<I_CmsContextMenuEntry> menuData, boolean isFixed, I_CmsAutoHider autoHideParent) {
  ...
  /**
   * @see
   *     org.opencms.gwt.client.ui.I_CmsAutoHider#addAutoHidePartner(com.google.gwt.dom.client.Element)
   */
  ...
  /**
   * @see
   *     org.opencms.gwt.client.ui.I_CmsAutoHider#removeAutoHidePartner(com.google.gwt.dom.client.Element)
   */
   ...
   * Action on close.
   *
   * <p>On close all sub menus should be hidden, the currently selected item should be deselected
   * and the popup will be closed.
   *
   * <p>
   */
   ...
   *
   * <p>First calculates the best space where to show the popup.
   *
   * <p>The following list shows the possibilities, beginning with the best and ending with the
   * worst.
   *
   * <p>
   *
   ...
   * </ul>
   *
   * Then the position (top and left coordinate) are calculated.
   *
   * <p>Finally the position of the sub menu popup is set to the calculated values.
   *
   * <p>
   *
   * @param item the item to show the sub menu of
  ...
  protected void setSubMenuPosition(final A_CmsContextMenuItem item) {
    ...
    // add 10 because of the shadow and for avoiding that the browser's right window border touches
    // the sub menu
    int leftSpace = item.getAbsoluteLeft() - (scrollLeft + 10);
    // calculate the right space
    // add 10 because of the shadow and for avoiding that the browser's left window border touches
    // the sub menu
    int rightSpace =
        Window.getClientWidth() - (item.getAbsoluteLeft() + item.getOffsetWidth() + 10);
    ...
    if (!showRight) {
      // if the width of the sub menu is larger than the right space, compare the left space with
      // the right space
    ...
    // add 10 because of the shadow and for avoiding that the browser's top window border touches
    // the sub menu
    int topSpace = (item.getAbsoluteTop() - scrollTop) + 10;
    // calculate the bottom space
    // add 10 because of the shadow and for avoiding that the browser's bottom window border touches
    // the sub menu
    int bottomSpace = (Window.getClientHeight() + scrollTop) - (item.getAbsoluteTop() + 10);
    // if the height of the sub menu is smaller than the bottom space, show the sub menu on the
    // bottom
    ...
    if (!showBottom) {
      ...
      // the bottom space and show the sub menu on the bottom if on the bottom is more space than on
      // the top
    ...
    if (showBottom) {
      ...
      top =
          ((item.getAbsoluteTop() - item.getSubMenu().getOffsetHeight()) + item.getOffsetHeight())
              - 4;
    }
