 ...
 *
 * <p>Use to render the dialog content within an iFrame on top of a regular {@link
 * org.opencms.gwt.client.ui.CmsPopup}. May also be used to wrap the popup if no iFrame is needed.
 *
 * <p>Provides function to show an iFrame dialog.
 *
 * <p>
 *
 * @since 8.5
...
public class CmsFrameDialog {
  ...
  public static CmsPopup showFrameDialog(
    ...
    final FormElement formElement =
        CmsDomUtil.generateHiddenForm(dialogUri, Method.post, IFRAME_NAME, parameters);
  ...
  /**
   * Installs the Javascript function which should be called by the child iframe when the dialog
   * should be closed.
   *
   * <p>
   *
    ...
    if (m_isFrame && m_isShowing) {
      if (m_buttonPanel != null) {
        m_contentPanel
            .getElement()
            .getStyle()
            .setBottom(m_buttonPanel.getOffsetHeight() + 6, Unit.PX);
      } else {
  ...
  /**
   * Enables the dialog close button on the parent frame popup.
   *
   * <p>
   */
