...
public class CmsEditorBase implements I_CmsInlineHtmlUpdateHandler {
  ...
  public CmsEditorBase(I_CmsContentServiceAsync service, I_CmsWidgetService widgetService) {
    ...
    m_widgetService.addWidgetFactory(
        "string",
        new I_CmsWidgetFactory() {
    ...
    m_widgetService.addRenderer(
        new CmsRenderer(CmsEntityBackend.getInstance(), getWidgetService()));
  ...
  /**
   * Checks whether the given element is marked to receive focus once the inline editing is
   * initialized.
   *
   * <p>
   *
   * @param element the element to check
   * @return <code>true</code> if the given element is marked to receive focus once the inline
   *     editing is initialized
   */
   ...
   */
  public HandlerRegistration addValidationChangeHandler(
      ValueChangeHandler<CmsValidationContext> handler) {
  ...
  public void loadContentDefinition(final String entityId, final Command callback) {
    ...
    AsyncCallback<CmsContentDefinition> asyncCallback =
        new AsyncCallback<CmsContentDefinition>() {
  ...
  /**
   * @see
   *     org.opencms.acacia.client.I_CmsInlineHtmlUpdateHandler#reinitWidgets(org.opencms.acacia.client.I_CmsInlineFormParent)
   */
   ...
   */
  public void renderEntityForm(
      String entityId, List<CmsTabInfo> tabInfos, Panel context, Element scrollParent) {
    ...
    if (entity != null) {
      ...
      m_formTabs =
          m_widgetService
              ...
              .renderForm(m_entity, m_tabInfos, m_formPanel, m_rootHandler, 0);
  ...
  public void renderInlineEntity(String entityId, I_CmsInlineFormParent formParent) {
    ...
    if (m_entity != null) {
      ...
      m_widgetService
          .getRendererForType(type)
          .renderInline(m_entity, formParent, this, m_rootHandler, 0);
      CmsUndoRedoHandler.getInstance().initialize(m_entity, this, m_rootHandler);
    }
  }
  ...
  public void saveEntities(
      List<CmsEntity> entities, final boolean clearOnSuccess, final Command callback) {

    AsyncCallback<CmsValidationResult> asyncCallback =
  ...
  /**
   * @see
   *     org.opencms.acacia.client.I_CmsInlineHtmlUpdateHandler#updateHtml(org.opencms.acacia.client.I_CmsInlineFormParent,
  ...
  public void updateHtml(final I_CmsInlineFormParent formParent, final Command onSuccess) {
    ...
    AsyncCallback<CmsEntityHtml> callback =
        new AsyncCallback<CmsEntityHtml>() {
    ...
    getService()
        .updateEntityHtml(getCurrentEntity(), getContextUri(), getHtmlContextInfo(), callback);
  }
   ...
   *
   * <p>Needed when updating the HTML due to content data changes.
   *
   * <p>Override to supply the required info.
   *
   * <p>
   *
   ...
   * Handles RPC errors.
   *
   * <p>Override this for better error handling
   *
