...
public class CmsRenderer implements I_CmsEntityRenderer {
  ...
  protected class TabSelectionHandler implements SelectionHandler<Integer> {
    ...
    /**
     * @see
     *     com.google.gwt.event.logical.shared.SelectionHandler#onSelection(com.google.gwt.event.logical.shared.SelectionEvent)
     */
   ...
   * <p>
   */
  protected class TabSizeHandler
      implements SelectionHandler<Integer>, ValueChangeHandler<CmsEntity>, ResizeHandler {
    ...
    protected void adjustContextHeight() {
      ...
      int height =
          CmsPositionBean.getInnerDimensions(tab.getElement()).getHeight()
  ...
  /**
   * Gets the paths of nested choice attributes starting from a given type.
   *
   * <p>
   *
  ...
  public static List<CmsChoiceMenuEntryBean> getChoiceEntries(
      CmsType attributeType, boolean startingAtChoiceAttribute) {
  ...
  public static void setAttributeChoice(
      I_CmsWidgetService widgetService, CmsAttributeValueView valueWidget, CmsType attributeType) {
  ...
  /**
   * Recursive helper method to create a tree structure of choice menu entries for a choice type.
   *
   * <p>
   *
  ...
  private static void collectChoiceEntries(
      CmsType startType, boolean startingAtChoiceAttribute, CmsChoiceMenuEntryBean currentEntry) {
  ...
  public void renderAttributeValue(
    ...
    if ((attribute == null) && (minOccurrence > 0)) {
      attribute =
          createEmptyAttribute(parentEntity, attributeName, attributeHandler, minOccurrence);
    }
      ...
      if (attribute.isSimpleValue()) {
        valueWidget.setValueWidget(
            ...
            m_widgetService.getDefaultAttributeValue(
                attributeName, attributeHandler.getSimplePath(attributeIndex)),
            true);
  ...
  public CmsTabbedPanel<FlowPanel> renderForm(
    ...
    if ((tabInfos == null) || (tabInfos.size() < 2)) {
      ...
      CmsTabbedPanel<FlowPanel> tabbedPanel =
          new CmsTabbedPanel<FlowPanel>(CmsTabbedPanelStyle.classicTabs);
      ...
      boolean collapsed =
          currentTab.isCollapsed()
              && ((nextTab != null)
                  && attributeNames.get(1).endsWith("/" + nextTab.getStartName()));
      for (final String attributeName : attributeNames) {
        ...
        if ((nextTab != null) && attributeName.endsWith("/" + nextTab.getStartName())) {
          ...
          if (currentTab.isCollapsed()) {
            ...
            collapsed =
                ((currentIndex + 1) == attributeNames.size())
                    || ((nextTab != null)
                        && attributeNames
                            .get(currentIndex + 1)
                            .endsWith("/" + nextTab.getStartName()));
          }
        ...
        CmsAttributeHandler handler =
            new CmsAttributeHandler(m_entityBackEnd, entity, attributeName, m_widgetService);
  ...
  public void renderForm(
      CmsEntity entity, Panel context, I_CmsAttributeHandler parentHandler, int attributeIndex) {
    ...
    if (entityType.isChoice()) {
      ...
      CmsAttributeHandler handler =
          new CmsAttributeHandler(
              m_entityBackEnd, entity, CmsType.CHOICE_ATTRIBUTE_NAME, m_widgetService);
      ...
      if ((attribute != null) && attribute.isComplexValue()) {
        for (CmsEntity choiceEntity : attribute.getComplexValues()) {
          ...
          I_CmsEntityRenderer renderer =
              m_widgetService.getRendererForAttribute(
                  choiceAttribute.getAttributeName(), attributeType);
          ...
          if (choiceAttribute.isSimpleValue()) {
            valueWidget.setValueWidget(
                ...
                m_widgetService.getDefaultAttributeValue(
                    choiceAttribute.getAttributeName(), handler.getSimplePath(attributeIndex)),
                true);
  ...
  public void renderInline(
    ...
    CmsEntityAttribute attribute = parentEntity.getAttribute(attributeName);
    CmsAttributeHandler handler =
        new CmsAttributeHandler(m_entityBackEnd, parentEntity, attributeName, m_widgetService);
    parentHandler.setHandler(attributeIndex, attributeName, handler);
    if (attribute != null) {
      List<Element> elements =
          m_entityBackEnd.getAttributeElements(
              parentEntity, attributeName, formParent.getElement());
      if (!elements.isEmpty()) {
          ...
          if (attribute.isSimpleValue() && (widget != null)) {
            ...
            if (innerHtml.equals(value)) {
              ...
              CmsInlineEntityWidget.createWidgetForEntity(
                  element, formParent, parentEntity, handler, i, updateHandler, m_widgetService);
            }
          } else {
            CmsInlineEntityWidget.createWidgetForEntity(
                element, formParent, parentEntity, handler, i, updateHandler, m_widgetService);
      ...
      if (!elements.isEmpty() && (elements.size() == 1)) {
        CmsInlineEntityWidget.createWidgetForEntity(
            elements.get(0), formParent, parentEntity, handler, -1, updateHandler, m_widgetService);
      }
    }
  }
  ...
  protected CmsEntityAttribute createEmptyAttribute(
    ...
    CmsType attributeType =
        m_entityBackEnd.getType(parentEntity.getTypeName()).getAttributeType(attributeName);
    if (attributeType.isSimpleType()) {
      ...
      for (int i = 0; i < minOccurrence; i++) {
        parentEntity.addAttributeValue(
            attributeName, m_entityBackEnd.createEntity(null, attributeType.getId()));
      }
      result = parentEntity.getAttribute(attributeName);
    }
    return result;
  ...
  /**
   * This is called after the last attribute for a tab (or for the whole content, if no tabs are
   * used) is rendered.
   *
  ...
  /**
   * Renders the tab description in a given panel.
   *
   * <p>
   *
