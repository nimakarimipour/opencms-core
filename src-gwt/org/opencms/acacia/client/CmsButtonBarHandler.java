...
public final class CmsButtonBarHandler implements MouseOverHandler, MouseOutHandler {
            ...
            if (Element.is(target)) {
              ...
              if (widgetElement != null) {
                EventListener widget =
                    com.google.gwt.user.client.DOM.getEventListener(widgetElement);
  ...
  public void onMouseOut(MouseOutEvent event) {
    ...
    if ((source instanceof CmsAttributeChoiceWidget)
        || (source instanceof CmsChoiceMenuEntryWidget)) {
  ...
  protected void cleanUpSubmenus(CmsChoiceMenuEntryWidget entryWidget) {
    ...
    // First remove all submenus which are deeper than the submenu in which the current entry is
    // located
  ...
  /**
   * Gets the last entry in the current list of active submenus.
   *
   * <p>
   *
  ...
  /**
   * In touch-only mode, this is used to process some clicks to trigger actions that would be
   * normally triggered by hovering over the same GUI element.
   *
  ...
  protected boolean useClickAsFakeHover(EventListener source, boolean isMouseDown) {
    ...
    if (source instanceof CmsAttributeChoiceWidget) {
      ...
      if (m_buttonBar != source) {
        ...
        // In the first case we have to show the menu bar and cancel the click event, in the second
        // case we don't.
        // We distinguish between the two cases with a special attribute.
        String hasHoverStr =
            ((Widget) source).getElement().getAttribute(CmsAttributeValueView.ATTR_HAS_HOVER);
  ...
  /**
   * Closes all currently active submenus and the root menu.
   *
   * <p>
   */
  ...
  /**
   * Handles the mouse over event for a choice widget.
   *
   * <p>
   *
  ...
  /**
   * Handles the mouse over event for a button bar.
   *
   * <p>
   *
  ...
  /**
   * Handles the mouse over event for a choice menu entry.
   *
   * <p>
   *
  ...
  /**
   * Reschedules the timer that hides the currently visible button bar.
   *
   * <p>
   */
  ...
  /**
   * Reschedules the timer that hides the currently visible choice menu.
   *
   * <p>
   */
  ...
  /**
   * Sets the button bar visibility.
   *
   * <p>
   *
