...
/**
 * The attribute handler. Handles value changes, addition of new values, remove and move operations
 * on values.
 *
 * <p>
 */
public class CmsAttributeHandler extends CmsRootHandler {
  ...
  /**
   * Returns <code>true</code> if a global widget resize handler is present.
   *
   * <p>
   *
  ...
  public void addNewAttributeValue(CmsAttributeValueView reference) {
    ...
    if (mayHaveMore) {
      if (getAttributeType().isSimpleType()) {
        ...
        String defaultValue =
            m_widgetService.getDefaultAttributeValue(m_attributeName, getSimplePath(valueIndex));
      ...
      if (handler.isIntitalized()) {
        handler.addChange(
            m_entity.getId(), m_attributeName, reference.getValueIndex() + 1, ChangeType.add);
      }
    }
  ...
  public void addNewAttributeValue(String value) {
    ...
    if (mayHaveMore && getAttributeType().isSimpleType()) {
      ...
      String defaultValue =
          m_widgetService.getDefaultAttributeValue(m_attributeName, getSimplePath(valueCount - 1));
      CmsAttributeValueView valueView = null;
      if ((m_attributeValueViews.size() == 1) && !m_attributeValueViews.get(0).hasValue()) {
        valueView = m_attributeValueViews.get(0);
        valueView.setActive();
   ...
   *
   * <p>This will not execute any DOM manipulations.
   *
   * <p>
   *
      ...
      if (getAttributeType().isSimpleType()) {
        String defaultValue =
            m_widgetService.getDefaultAttributeValue(
                m_attributeName, getSimplePath(referenceIndex + 1));
   ...
   *
   * @param reference the reference value view
   * @param choicePath the path of the selected (possibly nested) choice attribute, consisting of
   *     attribute names
   */
  public void addNewChoiceAttributeValue(CmsAttributeValueView reference, List<String> choicePath) {
    ...
    if (handler.isIntitalized()) {
      handler.addChange(
          m_entity.getId(), m_attributeName, reference.getValueIndex() + 1, ChangeType.choice);
    }
  }
  ...
  public void createNestedEntitiesForChoicePath(CmsEntity value, List<String> choicePath) {
    ...
    for (String attributeChoice : choicePath) {
      CmsType choiceType =
          m_entityBackEnd
              .getType(parentValue.getTypeName())
              .getAttributeType(CmsType.CHOICE_ATTRIBUTE_NAME);
      ...
      if (choiceOptionType.isSimpleType()) {
        String choiceValue =
            m_widgetService.getDefaultAttributeValue(attributeChoice, getSimplePath(0));
   ...
   *
   * <p>This will use the last fragment of the attribute name and concatenate it with the parent
   * path.
   *
   * <p>If the given index equals -1 no value index will be appended
   *
   ...
   */
  public void moveAttributeValue(
      CmsAttributeValueView valueView, int currentPosition, int targetPosition) {
    ...
    if (isChoiceHandler()) {
      ...
      CmsEntity value =
          m_entity.getAttribute(m_attributeName).getComplexValues().get(currentPosition);
      ...
      if (optionType.isSimpleType()) {
        valueWidget.setValueWidget(
            ...
            m_widgetService.getDefaultAttributeValue(
                attributeChoice, getSimplePath(targetPosition)),
            true);
      } else {
      ...
      List<CmsChoiceMenuEntryBean> menuEntries =
          CmsRenderer.getChoiceEntries(getAttributeType(), true);
      ...
      insertHandlers(targetPosition);
      valueWidget.setValueEntity(
          m_widgetService.getRendererForAttribute(m_attributeName, getAttributeType()), value);
      if (getAttributeType().getAttributeType(CmsType.CHOICE_ATTRIBUTE_NAME) != null) {
        List<CmsChoiceMenuEntryBean> menuEntries =
            CmsRenderer.getChoiceEntries(getAttributeType(), false);
  ...
  public void moveAttributeValueDown(final CmsAttributeValueView reference) {
    ...
    new CmsMoveAnimation(
            reference.getElement(),
            top,
            left,
            placeHolder.getOffsetTop(),
            left,
            new Command() {
  ...
  /**
   * Removes the attribute value from the given index, also manipulating the editor DOM to display
   * the change.
   *
   * <p>
   *
  ...
  /**
   * Removes the attribute value (and corresponding widget) with the given index, and returns the
   * parent widget.
   *
   * <p>
   *
  ...
  /**
   * @see org.opencms.acacia.client.CmsRootHandler#setHandlerById(java.lang.String,
  ...
  /**
   * Updates the add, remove and sort button visibility on the given inline widget or all registered
  ...
  public void updateButtonVisibilty(CmsInlineEntityWidget inlineWidget) {
    ...
    boolean mayHaveMore =
        (maxOccurrence > minOccurrence)
            && ((((attribute == null)
                    && (!getAttributeType().isSimpleType() || (inlineWidget != null)))
                || ((attribute != null) && (attribute.getValueCount() < maxOccurrence))));
    boolean needsRemove = false;
    boolean needsSort = false;
    if ((isChoiceHandler() || !getEntityType().isChoice())
        && m_entity.hasAttribute(m_attributeName)) {
      int valueCount = m_entity.getAttribute(m_attributeName).getValueCount();
    ...
    if (inlineWidget != null) {
      boolean mayEdit =
          (attribute != null) && (attribute.getValueCount() > inlineWidget.getAttributeIndex());
  ...
  private void addChoiceOption(CmsAttributeValueView reference, List<String> choicePath) {
    ...
    if (optionType.isSimpleType()) {
      String defaultValue =
          m_widgetService.getDefaultAttributeValue(attributeChoice, getSimplePath(valueIndex));
      ...
      I_CmsEntityRenderer renderer =
          m_widgetService.getRendererForAttribute(attributeChoice, optionType);
  ...
  private void changeEntityValue(String value, int valueIndex) {
    ...
    if (getEntityType().isChoice()) {
      CmsEntity choice =
          m_entity.getAttribute(CmsType.CHOICE_ATTRIBUTE_NAME).getComplexValues().get(valueIndex);
