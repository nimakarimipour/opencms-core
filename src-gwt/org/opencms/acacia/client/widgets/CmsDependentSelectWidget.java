 ...
 * omit indexes on a component of the first path, all indexes will be matched. You can also use
 * relative paths for the first path. Like '../NestedNode' or '../../OtherNode'.
 *
 * <p>The widget attaches event listeners to the editor so it can dynamically update the list of
 * select options when the content changes.
 */
public class CmsDependentSelectWidget extends Composite
  ...
  /**
   * The last value set through the setValue method. This is not necessarily the current widget
   * value.
   */
  ...
  /**
   * Returns the absolute path elements. In case off a relative path indicated by leading '..'
   * elements, the local value path will be used as a starting point. Otherwise the given path
  ...
  private String[] getAbsolutePath(String[] pathElements) {
    ...
    if (pathElements[0].equals("..")) {
      ...
      if (valueView != null) {
        ...
        result =
            Arrays.copyOf(
                localPathElements, (localPathElements.length + pathElements.length) - pathIndex);
        System.arraycopy(
            pathElements,
            pathIndex,
            result,
  ...
  /**
   * Replaces the select options with the given options.
   *
   * <p>
   *
   * @param options the map of select options (keys are option values, values are option
   *     descriptions)
   */
    ...
    if (options.containsKey("")) {
      options.put(
          ...
          org.opencms.gwt.client.Messages.get()
              .key(org.opencms.gwt.client.Messages.GUI_SELECTBOX_EMPTY_SELECTION_0));
    }
