...
/**
 * An attribute select widget acts as a select widget and consists of several attribute filter
 * select boxes and one main select box, such that choosing values from the attribute filters
 * restricts the available options in the main select box to those which have a matching value for
 * every filter attribute.
 *
 * <p>All data related to the options and filter attributes must be passed into the constructor,
 * this widget does not use any RPC calls.
 */
public class CmsAttributeSelectWidget extends Composite implements I_CmsEditWidget {
  ...
  public CmsAttributeSelectWidget(I_CmsAttributeSelectData data) {
    ...
    for (AttributeDefinition attrDef : data.getAttributeDefinitions()) {
      ...
      // set the value before adding the change handler, since we already call handleFilterChange()
      // below to initialize the main select box
  ...
  public void setValue(String value, boolean fireEvent) {
    ...
    if (option != null) {
      for (String optionAttribute : option.getAttributes().keySet()) {
        m_attributeSelects
            .get(optionAttribute)
            .setFormValue(option.getAttributes().get(optionAttribute).get(0), false);
      }
      handleFilterChange();
      ...
      for (AttributeDefinition attrDef : m_attributeDefinitions.values()) {
        ...
        // the editor initializes new widgets with the empty value, so we want to use the default
        // option instead
        // the neutral option for the attribute in that case.
        String attrSelectValue =
            CmsStringUtil.isEmpty(value) ? getDefaultOption(attrDef) : getNeutralOption(attrDef);
  ...
  protected void handleFilterChange() {
    ...
    // first generate a map of all options, then reduce the keys for each chosen filter using the
    // option index
    ...
    for (Map.Entry<String, String> filterEntry : attributes.entrySet()) {
      newMainOptions
          .keySet()
          .retainAll(m_optionIndex.get(new IndexKey(filterEntry.getKey(), filterEntry.getValue())));
    }
