...
public class CmsCodeMirrorWidget extends ComplexPanel
    implements I_CmsEditWidget, HasResizeHandlers {

  /** Map from modes to labels. */
  private static LinkedHashMap<String, String> m_modes;

  /**
   * Map from simplified mode names to actual mode names (e.g. from 'javascript' to
   * 'text/javascript').
   */
  ...
  public CmsCodeMirrorWidget(String config) {
    ...
    m_config =
        AutoBeanCodex.decode(
                CmsCoreProvider.AUTO_BEAN_FACTORY, I_CmsCodeMirrorClientConfiguration.class, config)
            .as();
  ...
  /**
   * Gets the effective mode to use, which depends both on the selected mode and on whether the user
   * has enabled syntax highlighting.
   *
   ...
   */
  private CmsCodeMirrorToggleButton createOptionButton(
      FontOpenCms icon, String optionName, boolean initialValue) {
    ...
    result.addValueChangeHandler(
        event -> nativeSetOption(optionName, event.getValue().booleanValue()));
    return result;
  }
  ...
  private FlowPanel createToolbar() {
    ...
    {
      CmsCodeMirrorToolbarButton searchReplace =
          new CmsCodeMirrorToolbarButton(FontOpenCms.SEARCH_REPLACE);
    ...
    }
    {
      ...
      highlight.addValueChangeHandler(
          event -> setSyntaxHighlightingEnabled(event.getValue().booleanValue()));
      result.add(highlight);
    }

    {
      CmsCodeMirrorToggleButton wrap =
          createOptionButton(FontOpenCms.WRAP_LINES, "lineWrapping", false);
      wrap.setTitle(Messages.get().key(Messages.GUI_CODEMIRROR_TOGGLE_LINE_WRAP_0));
      result.add(wrap);
    }

    {
      CmsCodeMirrorToggleButton showTabs =
          new CmsCodeMirrorToggleButton(FontOpenCms.INVISIBLE_CHARS);
...

    {
      ...
      brackets.addValueChangeHandler(
          event -> setAutoCloseBrackets(event.getValue().booleanValue()));
    ...
    m_modeSelect.addValueChangeHandler(
        event -> {
  ...
  private native void initCodeMirror(
      CmsCodeMirrorWidget instance, Element elem, JavaScriptObject phrases, int height) /*-{
        var config = {
            theme: "eclipse",
            mode: "text/plain",
            lineNumbers: true,
   ...
   */
  private native void nativeSetProperty(
      com.google.gwt.dom.client.Element element, String name, String value) /*-{
  ...
  /**
   * Sets the mode selected by the user (this may not result in the mode actually being changed in
   * the editor if syntax highlighting is turned off).
   *
