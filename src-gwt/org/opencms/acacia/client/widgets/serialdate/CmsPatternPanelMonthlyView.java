...
public class CmsPatternPanelMonthlyView extends Composite implements I_CmsSerialDatePatternView {
  ...
  interface I_CmsPatternPanelMonthlyUiBinder
      extends UiBinder<HTMLPanel, CmsPatternPanelMonthlyView> {
    // nothing to do
  }
  ...
  private static I_CmsPatternPanelMonthlyUiBinder uiBinder =
      GWT.create(I_CmsPatternPanelMonthlyUiBinder.class);
  ...
  @UiField CmsFocusAwareTextBox m_everyMonth;

  /** The days label. */
  @UiField Element m_labelDays;

  /** The every label. */
  @UiField Element m_labelEvery;

  /** The months label. */
  @UiField Element m_everyLabelMonth;
  ...
  @UiField FlowPanel m_weekPanel;

  /** The select box for the day selection. */
  @UiField CmsSelectBox m_atDay;

  /** The select box for the month selection. */
  @UiField CmsFocusAwareTextBox m_atMonth;

  /** The month label. */
  @UiField Element m_atLabelMonth;
  ...
  public CmsPatternPanelMonthlyView(
      CmsPatternPanelMonthlyController controller, I_CmsObservableSerialDateValue model) {
    m_controller = controller;
    m_model = model;
    m_model.registerValueChangeObserver(this);

    m_dayMonthRadioButton =
        new CmsRadioButton(
            DAYS_RADIOBUTTON, Messages.get().key(Messages.GUI_SERIALDATE_MONTHLY_MONTHDAY_AT_0));
    m_dayMonthRadioButton.setGroup(m_group);
    m_dayMonthRadioButton.setChecked(true);
    m_weekDayMonthRadioButton =
        new CmsRadioButton(
            WEEKS_RADIOBUTTON, Messages.get().key(Messages.GUI_SERIALDATE_MONTHLY_WEEKDAY_AT_0));
    ...
    m_group.addValueChangeHandler(
        new ValueChangeHandler<String>() {
          ...
          public void onValueChange(ValueChangeEvent<String> event) {
            ...
            if (handleChange()) {
              m_controller.setPatternScheme(
                  event.getValue().equals(m_weekDayMonthRadioButton.getName()), true);
            }
          }
        });
    ...
    m_labelDays.setInnerText(
        Messages.get().key(Messages.GUI_SERIALDATE_MONTHLY_MONTHDAY_DAY_EVERY_0));
