...
public class CmsSerialDateView extends Composite
  ...
  @UiField protected FlowPanel m_exceptionsPanelContainer;
  ...
  /**
   * The check box, indicating if the event should be displayed as "current" till it ends (checked
   * state), or only till it starts (unchecked).
   */
  @UiField CmsCheckBox m_currentTillEndCheckBox;

  /** The daily pattern. */
  CmsPatternPanelDailyView m_dailyPattern;

  /** The panel with the basic dates. */
  @UiField FlowPanel m_datesPanel;

  /** Panel only shown when the widget is deactivated. */
  @UiField FlowPanel m_deactivationPanel;

  /** Information on the widget in deactivated state. */
  @UiField Label m_deactivationText;

  /** The panel with the serial date duration options. */
  @UiField FlowPanel m_durationPanel;

  /** The end time label. */
  @UiField Label m_endLabel;
  ...
  @UiField CmsFieldSet m_exceptionsPanel;
  ...
  @UiField CmsPushButton m_manageExceptionsButton;
  ...
  /** The times text box. */
  @UiField CmsFocusAwareTextBox m_occurrences;

  /** The label that displays the dependency to another series. */
  @UiField CmsLabel m_origSeriesLabel;
  ...
  private String m_dateFormat =
      Messages.get().keyDefault(Messages.GUI_SERIALDATE_DATE_FORMAT_0, null);
   ...
   */
  public CmsSerialDateView(
      CmsSerialDateController controller, I_CmsObservableSerialDateValue model) {
    ...
    m_exceptionsList =
        new CmsCheckableDatePanel(m_dateFormat, CmsCheckableDatePanel.Style.THREE_COLUMNS, true);
    ...
    m_origSeriesLabel.setText(
        Messages.get().key(Messages.GUI_SERIALDATE_FROM_SERIES_INFORMATION_0));
   ...
   *
   * @param dates the current dates to show, accompanied with the information if they are exceptions
   *     or not.
   */
  ...
  /**
   * Handles the ends after focus event.
   *
   * <p>
   *
  ...
  /**
   * Handles the end date change event.
   *
   * <p>
   *
   * @param event the change event
  ...
  /**
   * Sets the radio buttons active or inactive.
   *
   * <p>
   *
  ...
  /**
   * Returns the duration button for type.
   *
   * <p>
   *
   * @param type the type
  ...
  private void initDatesPanel() {
    ...
    m_currentTillEndCheckBox.setText(
        Messages.get().key(Messages.GUI_SERIALDATE_CURRENT_TILL_END_CHECKBOX_0));
    m_currentTillEndCheckBox
        .getButton()
        .setTitle(Messages.get().key(Messages.GUI_SERIALDATE_CURRENT_TILL_END_CHECKBOX_HELP_0));
  }
  ...
  private void initDurationButtonGroup() {
    ...
    m_groupDuration.addValueChangeHandler(
  ...
  private void initDurationPanel() {
    ...
    m_durationAfterPostfixLabel.setText(
        Messages.get().key(Messages.GUI_SERIALDATE_DURATION_ENDTYPE_OCC_POSTFIX_0));
  ...
  private void initManagementPart() {
    ...
    m_manageExceptionsButton.setText(
        Messages.get().key(Messages.GUI_SERIALDATE_BUTTON_MANAGE_EXCEPTIONS_0));
  ...
  private void initOverviewPopup() {
    ...
    m_overviewPopup =
        new CmsPopup(Messages.get().key(Messages.GUI_SERIALDATE_OVERVIEW_POPUP_TITLE_0), 800);
    m_overviewPopup.center();
    ...
    m_overviewList =
        new CmsCheckableDatePanel(m_dateFormat, CmsCheckableDatePanel.Style.TWO_COLUMNS);
    m_overviewList.addDate(new Date());
    panel.getElement().getStyle().setPaddingLeft(10, Unit.PX);
    panel
        .getElement()
        .getStyle()
        .setProperty("maxHeight", m_overviewPopup.getAvailableHeight(0), Unit.PX);
  ...
  private void initPatternButtonGroup() {
    ...
    m_groupPattern.addValueChangeHandler(
        new ValueChangeHandler<String>() {
  ...
  private void initSerialOptionsPanel() {
    ...
    m_serialOptionsPanel.setLegend(
        Messages.get().key(Messages.GUI_SERIALDATE_PANEL_SERIAL_OPTIONS_0));
  ...
  /**
   * Shows / hides serial options and exceptions.
   *
   * <p>
   *
