...
public class CmsSerialDateController extends Composite
  ...
  /**
   * Data wrapper for the default values to set when changing the pattern, dependent on the event's
   * start date.
   */
  public static class PatternDefaultValues {
    ...
    protected PatternDefaultValues(Date startDate) {
      ...
      m_weekOfMonth =
          null == startDate ? WeekOfMonth.FIRST : WeekOfMonth.values()[(startDate.getDate() / 7)];
   ...
   *
   * <p>Status update takes place at most every 500 milliseconds. Whenever re-update is triggered
   * during the time waiting, the timer is reset, i.e., it is waited for 500 milliseconds again.
   */
  ...
  private final Map<PatternType, I_CmsSerialDatePatternController> m_patternControllers =
      new HashMap<>();
  ...
  /**
   * The default values to use when switching the pattern. They depend on the start date of the
   * events.
   */
  ...
  public CmsSerialDateController() {
    ...
    m_getDatesAction =
        new CmsRpcAction<Collection<CmsPair<Date, Boolean>>>() {
    ...
    m_statusUpdateAction =
        new CmsRpcAction<CmsPair<Boolean, String>>() {
  ...
  /**
   * @see
   *     org.opencms.acacia.client.widgets.serialdate.I_ChangeHandler#conditionallyRemoveExceptionsOnChange(com.google.gwt.user.client.Command,
   *     boolean)
   */
  @Override
  ...
  /**
   * @see
   *     org.opencms.acacia.client.widgets.serialdate.I_ChangeHandler#removeExceptionsOnChange(com.google.gwt.user.client.Command)
   */
  @Override
  ...
  /**
   * Set the flag, indicating if the events are displayed as "current" till they end (true) or only
   * till they start (false).
   *
   * @param isCurrentTillEnd the flag, indicating if the events are displayed as "current" till they
   *     end (true) or only till they start (false).
   */
  public void setCurrentTillEnd(Boolean isCurrentTillEnd) {

    if (m_model.isCurrentTillEnd()
        ^ ((null != isCurrentTillEnd) && isCurrentTillEnd.booleanValue())) {
          ...
          new Command() {
            ...
            public void execute() {
              ...
              switch (endType) {
                ...
                case DATE:
                  m_model.setOccurrences(0);
                  m_model.setSeriesEndDate(
                      m_model.getStart() == null ? new Date() : m_model.getStart());
                  break;
                default:
                  break;
              }
  ...
  public void setIsSeries(Boolean isSeries) {
    ...
    if (null != isSeries) {
      ...
      if ((null != m_model.getParentSeriesId()) && series) {
        m_removeSeriesBindingConfirmDialog.show(
            new Command() {
  ...
  /**
   * Set the occurrences. If the String is invalid, the occurrences will be set to "-1" to cause
  ...
  I_CmsSerialDateServiceAsync getService() {
    ...
    if (SERVICE == null) {
      ...
      String serviceUrl =
          CmsCoreProvider.get().link("org.opencms.ade.contenteditor.CmsSerialDateService.gwt");
  ...
  void setPatternDefaultValues(Date startDate) {
    ...
    if ((m_patternDefaultValues == null)
        || !Objects.equals(m_patternDefaultValues.getDate(), startDate)) {
  ...
  private void initPatternControllers() {
    ...
    m_patternControllers.put(
        PatternType.WEEKLY, new CmsPatternPanelWeeklyController(m_model, this));
    m_patternControllers.put(
        PatternType.MONTHLY, new CmsPatternPanelMonthlyController(m_model, this));
    m_patternControllers.put(
        PatternType.YEARLY, new CmsPatternPanelYearlyController(m_model, this));
    // m_patternControllers.put(PatternType.INDIVIDUAL, new
    // CmsPatternPanelIndividualController(m_model, this));
  }
}
