...
public class CmsCheckableDatePanel extends Composite
    implements HasValueChangeHandlers<SortedSet<Date>> {
  ...
  public CmsCheckableDatePanel(String dateFormat, Style style, boolean onlyLabels) {
    ...
    initWidget(m_panel);
    m_checkBoxes =
        new TreeSet<CmsCheckBox>(
            new Comparator<CmsCheckBox>() {
  ...
  /**
   * Returns all dates with the specified check state, if the check state is <code>null</code>, all
   * dates are returned.
   *
   * @param checkState the check state, the returned dates should have.
   * @return all dates with the specified check state, if the check state is <code>null</code>, all
   *     dates are returned.
   */
