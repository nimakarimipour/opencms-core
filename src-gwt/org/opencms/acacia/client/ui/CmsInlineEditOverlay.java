...
public class CmsInlineEditOverlay extends Composite implements HasClickHandlers {
  ...
  private static I_CmsInlineEditOverlayUiBinder uiBinder =
      GWT.create(I_CmsInlineEditOverlayUiBinder.class);

  /** Bottom border. */
  @UiField protected Element m_borderBottom;

  /** Left border. */
  @UiField protected Element m_borderLeft;

  /** Right border. */
  @UiField protected Element m_borderRight;

  /** Top border. */
  @UiField protected Element m_borderTop;
  ...
  @UiField FlowPanel m_buttonPanel;
  ...
  /**
   * Returns the available absolute top position for the given button.
   *
   * <p>
   *
   * @param widget the button widget
  ...
  /**
   * Increases the overlay height to make space for edit buttons.
   *
   * <p>
   *
    ...
    if (m_hasButtonBar != visible) {
      ...
      if (m_hasButtonBar) {
        ...
        m_borderRightStyle.setLeft(
            CmsClientStringUtil.parseInt(m_borderRightStyle.getLeft()) + BUTTON_BAR_WIDTH, Unit.PX);
      } else {
        m_buttonBar.getStyle().setDisplay(Display.NONE);
        int width = CmsClientStringUtil.parseInt(m_borderTopStyle.getWidth()) - BUTTON_BAR_WIDTH;
        m_borderTopStyle.setWidth(width, Unit.PX);
  ...
  /**
   * Sets position and size of the overlay area.
   *
   * <p>
   *
  ...
  private void setPosition(CmsPositionBean position) {
    ...
    setSelectPosition(
        position.getLeft(), position.getTop(), position.getHeight(), position.getWidth());
  }
  ...
  private void setSelectPosition(int posX, int posY, int height, int width) {
    ...
    int bodyWidth =
        RootPanel.getBodyElement().getClientWidth() + RootPanel.getBodyElement().getOffsetLeft();
    if (bodyWidth > useWidth) {
      useWidth = bodyWidth;
    }
    int useHeight = Window.getClientHeight();
    int bodyHeight =
        RootPanel.getBodyElement().getClientHeight() + RootPanel.getBodyElement().getOffsetTop();
