...
public class CmsAttributeValueView extends Composite
  ...
  /**
   * Attribute for marking menu bars that are actually menu bars which should open on hovering
  ...
  /** The button bar. */
  @UiField protected FlowPanel m_buttonBar;

  /** The down button. */
  @UiField protected CmsPushButton m_downButton;

  /** The help bubble element. */
  @UiField protected DivElement m_helpBubble;

  /** The help bubble close button. */
  @UiField protected CmsPushButton m_helpBubbleClose;

  /** The help bubble text element. */
  @UiField protected DivElement m_helpBubbleText;

  /** The message text element. */
  @UiField protected SpanElement m_messageText;
  ...
  @UiField protected FlowPanel m_widgetHolder;
  ...
  /**
   * Enables or disables the "collapsed" style, which is used for choice elements to reduce the
   * nesting level visually.
   *
   * <p>
   *
   ...
   */
  public void setValueWidget(
      I_CmsFormEditWidget widget, String value, String defaultValue, boolean active) {
  ...
  /**
   * Tells the attribute value view to change its display state between focused/unfocused (this
   * doesn't actually change the focus).
   *
   * <p>
   *
  ...
  public void updateButtonVisibility(
      boolean hasAddButton, boolean hasRemoveButton, boolean hasSortButtons) {
    ...
    if (hasSortButtons) {
      ...
      if (CmsEditorBase.hasDictionary()) {
        m_moveButton.setTitle(
            CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_VIEW_MOVE_1, m_label));
      }
    } else {
      m_moveButton.setTitle("");
  ...
  /**
   * Activates the value widget if prNamet.
   *
   * <p>
   */
  void activateWidget() {
  ...
  /**
   * Toggles the label hover CSS class.
   *
   * <p>
   *
  ...
  void toggleLabelHover(boolean hovered) {
    ...
    CmsValueFocusHandler.getInstance()
        .hideHelpBubbles(RootPanel.get(), hover || !CmsCoreProvider.get().isShowEditorHelp());
  }

  /**
   * Updates the widget width according to the compact mode setting.
   *
   * <p>
   */
  void updateWidth() {
    ...
    if (formCss().firstColumn().equals(m_compacteModeStyle.getValue())) {
      int width =
          getElement().getParentElement().getOffsetWidth() - formCss().SECOND_COLUMN_WIDTH();
  ...
  /**
   * Adds a mouse down handler to activate the editing widget.
   *
   * <p>
   */
  private void addActivationHandler() {
    ...
    if (m_activationHandlerRegistration == null) {
      m_activationHandlerRegistration =
          addMouseDownHandler(
              new MouseDownHandler() {
                ...
                public void onMouseDown(MouseDownEvent event) {
                  ...
                  if (!m_buttonBar
                      .getElement()
                      .isOrHasChild((Node) event.getNativeEvent().getEventTarget().cast())) {
                    activateWidget();
                  }
                }
              });
  ...
  /**
   * Adds the label hover mouse handler.
   *
   * <p>
   *
  ...
  /**
   * Called when a drag operation for this widget is stopped.
   *
   * <p>
   */
  ...
  private void initButtons() {
    ...
    if (CmsEditorBase.hasDictionary()) {
      ...
      m_attributeChoice.setTitle(
          CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_CHOICE_ADD_CHOICE_1, m_label));
      m_removeButton.setTitle(
          CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_VIEW_DELETE_1, m_label));
      m_helpBubbleClose.setTitle(CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_VIEW_CLOSE_0));
      m_upButton.setTitle(
          CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_VIEW_MOVE_UP_0, m_label));
      m_downButton.setTitle(
          CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_VIEW_MOVE_DOWN_0, m_label));
    }
  }
  ...
  /**
   * Removes the label hover mouse handler.
   *
   * <p>
   */
  ...
  private boolean shouldDisplayTooltipAbove() {
    ...
    if (displayAbove
        && ((distanceFromWindowTop < bubbleHeight)
            && (distanceToWindowBottom > distanceFromWindowTop))) {
      ...
      displayAbove = false;
        ...
        && ((distanceToWindowBottom < bubbleHeight)
            && (distanceFromWindowTop > distanceToWindowBottom))) {
