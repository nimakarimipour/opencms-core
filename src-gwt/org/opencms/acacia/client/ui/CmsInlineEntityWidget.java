...
public final class CmsInlineEntityWidget extends Composite {
   ...
   * <p>
   */
  protected class FormPanel extends FlowPanel
      implements org.opencms.gwt.client.I_CmsDescendantResizeHandler {
  ...
  private static I_InlineEntityWidgetUiBinder uiBinder =
      GWT.create(I_InlineEntityWidgetUiBinder.class);
  ...
  public static CmsInlineEntityWidget createWidgetForEntity(
    ...
    CmsInlineEntityWidget widget =
        new CmsInlineEntityWidget(
            element,
            formParent,
            parentEntity,
            attributeHandler,
  ...
  /**
   * Returns the button top position for the given reference element.
   *
   * <p>
   *
  ...
  /**
   * Repositions the edit overlay after the HTML has been updated.
   *
   * <p>
   */
    ...
    List<Element> elements =
        CmsEntityBackend.getInstance()
            .getAttributeElements(
                m_parentEntity, m_attributeHandler.getAttributeName(), m_formParent.getElement());
    if (m_popupClosed) {
  ...
  /**
   * Cleanup after the edit pop-up was opened.
   *
   * <p>
   */
  void onPopupClose() {
  ...
  /**
   * Handles the remove attribute click.
   *
   * <p>
   *
  ...
  /**
   * Positions the given pop-up relative to the reference element.
   *
   * <p>
   */
  void positionPopup() {
    ...
    if (m_popup != null) {
      if (m_referenceElement != null) {
        CmsPositionBean referencePosition =
            CmsPositionBean.getBoundingClientRect(m_referenceElement);
        ...
        if (((windowHeight + scrollTop)
                < (top + referencePosition.getHeight() + contentHeight + 20))
  ...
  /**
   * Updates the HTML according to the entity data.
   *
   * <p>
   */
  void runHtmlUpdate() {
    ...
    if (m_runningUpdate) {
      ...
      m_htmlUpdateHandler.updateHtml(
          m_formParent,
          new Command() {
  ...
  void showEditPopup(ClickEvent clickEvent) {
    ...
    m_entityChangeHandlerRegistration =
        m_parentEntity.addValueChangeHandler(
    ...
    CmsAttributeHandler.setResizeHandler(
        new ResizeHandler() {
    ...
    m_widgetService
        ...
        .renderAttributeValue(m_parentEntity, m_attributeHandler, m_attributeIndex, formPanel);
    CmsInlineEditOverlay.addOverlayForElement(m_referenceElement);
    positionPopup();
    m_popup
        .getElement()
        .getStyle()
        .setZIndex(I_CmsLayoutBundle.INSTANCE.constants().css().zIndexPopup());
    m_popupClosed = false;
  }
  ...
  private void initButtons() {
    ...
    if (CmsEditorBase.hasDictionary()) {
      ...
      m_removeButton.setTitle(
          CmsEditorBase.getMessageForKey(CmsEditorBase.GUI_VIEW_DELETE_1, label));
